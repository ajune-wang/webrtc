...
int VP9EncoderImpl::Encode(const VideoFrame& input_image,
  ...
  if (is_flexible_mode_) {
  ...
  } else if (codec_.mode == kRealtimeVideo && num_spatial_layers_ > 0) {
    ...
    svc_drop_frame.framedrop_thresh[0] = config_->rc_dropframe_thresh;
    svc_drop_frame.framedrop_thresh[1] = 0;
    svc_drop_frame.framedrop_thresh[2] = 0;
