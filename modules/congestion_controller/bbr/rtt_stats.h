...
class RttStats {
 public:
  ...
  void UpdateRtt(units::TimeDelta send_delta,
                 units::TimeDelta ack_delay,
                 units::Timestamp now);
  ...
  units::TimeDelta smoothed_rtt() const { return smoothed_rtt_; }

  // Returns the EWMA smoothed RTT prior to the most recent RTT sample.
  units::TimeDelta previous_srtt() const { return previous_srtt_; }
  ...
  // May return Zero if no valid updates have occurred.
  units::TimeDelta latest_rtt() const { return latest_rtt_; }

  // Returns the min_rtt for the entire connection.
  // May return Zero if no valid updates have occurred.
  units::TimeDelta min_rtt() const { return min_rtt_; }

  units::TimeDelta mean_deviation() const { return mean_deviation_; }

 private:
  units::TimeDelta latest_rtt_;
  units::TimeDelta min_rtt_;
  units::TimeDelta smoothed_rtt_;
  units::TimeDelta previous_srtt_;
  // Mean RTT deviation during this session.
  // Approximation of standard deviation, the error is roughly 1.25 times
  // larger than the standard deviation, for a normally distributed signal.
  units::TimeDelta mean_deviation_;
