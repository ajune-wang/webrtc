...
VideoCodecAnalyser::VideoCodecAnalyser(VideoFrameReader* frame_reader)
    : frame_reader_(frame_reader), task_queue_("VideoCodecAnalyser") {}
...
void VideoCodecAnalyser::FinishDecode(const VideoFrame& frame,
  ...
  if (frame_reader_ != nullptr) {
    ...
    task_queue_.PostTask([this, decoded_buffer, frame_num, spatial_idx]() {
      auto ref_frame = frame_reader_->ReadFrame(frame_num);
...

      {
        MutexLock lock(&mutex_);
        VideoCodecTestStats::FrameStatistics* fs =
            this->stats_.GetFrame(frame_num, spatial_idx);
