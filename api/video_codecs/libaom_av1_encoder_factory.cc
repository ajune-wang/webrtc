...
bool ValidateEncodeParams(
  ...
  for (size_t i = 0; i < frame_settings.size(); ++i) {
    ...
    if (i == 0) {
      // TODO: This is not correct, libaom can do "pre-scaling" of an input
      //       image, only when reference scaling is used do the AV1 specific
      //       rules apply.
      if (!GetScalingFactor({frame_buffer.width(), frame_buffer.height()},
                            settings.resolution)) {
        RTC_LOG(LS_ERROR)
...
void LibaomAv1Encoder::Encode(
  ...
  for (int sid = frame_settings[0].spatial_id;
    ...
    RTC_LOG(LS_WARNING)
        << __FUNCTION__ << " timestamp=" << encode_timestamp_
