  ...
  task_queue_->PostTask([this, timestamp_us, utc_time_us,
    ...
    WriteToOutput(event_encoder_->EncodeLogStart(timestamp_us, utc_time_us));
    {
      ...
      if (!all_config_history_.empty()) {
        recent_.config_history.insert(
            recent_.config_history.begin(),
            std::make_move_iterator(all_config_history_.begin()),
            std::make_move_iterator(all_config_history_.end()));
        RTC_DCHECK_LE(recent_.config_history.size(),
  ...
  if (logging_state_started_) {
    if (ShouldOutputImmediately()) {
      ...
      task_queue_->PostTask(
          [this, events = ExtractRecentHistories()]() mutable {
            RTC_DCHECK_RUN_ON(task_queue_.get());
            if (event_output_) {
              RTC_DCHECK(event_output_->IsActive());
              LogEventsToOutput(std::move(events));
            }
          });
    } else {
      ScheduleOutput();
...
void RtcEventLogImpl::LogToMemory(std::unique_ptr<RtcEvent> event) {
  ...
  if (container.size() >= container_max_size) {
    ...
    RTC_DCHECK(!logging_state_started_);
    container.pop_front();
  }
...
void RtcEventLogImpl::LogEventsFromMemoryToOutput() {
  ...
  mutex_.Lock();
  EventHistories events = ExtractRecentHistories();
  mutex_.Unlock();

  LogEventsToOutput(std::move(events));
}

void RtcEventLogImpl::LogEventsToOutput(EventHistories events) {
  RTC_DCHECK_RUN_ON(task_queue_.get());
  ...
  std::string encoded_configs = event_encoder_->EncodeBatch(
      events.config_history.begin(), events.config_history.end());
  ...
  std::string encoded_history =
      event_encoder_->EncodeBatch(events.history.begin(), events.history.end());
  ...
  all_config_history_.insert(
      ...
      std::make_move_iterator(events.config_history.begin()),
      std::make_move_iterator(events.config_history.end()));
  RTC_DCHECK_LE(all_config_history_.size(), max_config_events_in_history_);
}
