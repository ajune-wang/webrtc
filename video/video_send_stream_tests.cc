...
TEST_F(VideoSendStreamTest,
  ...
  class EncoderObserver : public test::FakeEncoder {
   public:
    EncoderObserver() : FakeEncoder(Clock::GetRealTimeClock()) {}

    void WaitForResolution(int width, int height) {
      if (FakeEncoder::config().width == width &&
          FakeEncoder::config().height == height) {
        return;
      }
      EXPECT_TRUE(
          init_encode_called_.Wait(VideoSendStreamTest::kDefaultTimeoutMs));

      EXPECT_EQ(width, FakeEncoder::config().width);
      EXPECT_EQ(height, FakeEncoder::config().height);
    }
