...
bool FeedbackParam::operator==(const FeedbackParam& other) const {
  return absl::EqualsIgnoreCase(other.id().c_str(), id().c_str()) == 0 &&
         absl::EqualsIgnoreCase(other.param().c_str(), param().c_str()) == 0;
}
...
bool Codec::Matches(const Codec& codec) const {
  ...
  return (id <= kMaxStaticPayloadId || codec.id <= kMaxStaticPayloadId)
             ? (id == codec.id)
             : (absl::EqualsIgnoreCase(name.c_str(), codec.name.c_str()) == 0);
}
...
void VideoCodec::SetDefaultParameters() {
  if (absl::EqualsIgnoreCase(kH264CodecName, name.c_str()) == 0) {
...
VideoCodec::CodecType VideoCodec::GetCodecType() const {
  ...
  if (absl::EqualsIgnoreCase(payload_name, kRedCodecName) == 0) {
    return CODEC_RED;
  }
  if (absl::EqualsIgnoreCase(payload_name, kUlpfecCodecName) == 0) {
    return CODEC_ULPFEC;
  }
  if (absl::EqualsIgnoreCase(payload_name, kFlexfecCodecName) == 0) {
    return CODEC_FLEXFEC;
  }
  if (absl::EqualsIgnoreCase(payload_name, kRtxCodecName) == 0) {
    return CODEC_RTX;
  }
...
bool CodecNamesEq(const char* name1, const char* name2) {
  return absl::EqualsIgnoreCase(name1, name2) == 0;
}
