...
bool DcSctpSocket::IsConsistent() const {
  ...
  switch (state_) {
    case State::kClosed:
      consistent &= (tcb_ == nullptr);
      consistent &= (!t1_init_->is_running());
      consistent &= (!t1_cookie_->is_running());
      consistent &= (!t2_shutdown_->is_running());
      break;
    ...
    case State::kCookieEchoed:
      ...
      consistent &= (t1_cookie_->is_running());
      consistent &= (!t2_shutdown_->is_running());
      consistent &= (cookie_echo_chunk_.has_value());
      break;
