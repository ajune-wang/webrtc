...
class PacingControllerTest : public ::testing::TestWithParam<bool> {
 protected:
  ...
  TimeDelta TimeUntilNextProcess() {
    ...
    auto next_probe = pacer_->NextProbeTime();
    if (next_probe) {
  ...
  void AdvanceTimeAndProcess() {
    ...
    pacer_->ProcessPacketsAt(next_send_time);
  }
...
class PacingControllerFieldTrialTest : public ::testing::TestWithParam<bool> {
 protected:
  struct MediaStream {
  ...
  void ProcessNext(PacingController* pacer) {
    ...
    pacer->ProcessPacketsAt(next_send_time);
  }
...
TEST_P(PacingControllerFieldTrialTest, DefaultBudgetAffectsAudio) {
  ...
  EXPECT_LT(((wait_end_time - wait_start_time) - expected_wait_time).Abs(),
            GetParam() ? TimeDelta::ms(5) : PacingController::kMinSleepTime);
}
...
TEST_P(PacingControllerTest, HighPrioDoesntAffectBudget) {
  ...
  EXPECT_NEAR(
      pacing_time.us<double>(), expected_pacing_time.us<double>(),
      GetParam() ? 5000.0 : PacingController::kMinSleepTime.us<double>());
}
...
TEST_P(PacingControllerTest, TaskEarly) {
  ...
  // Check that next scheduled send time is still the same (within 1ms).
  EXPECT_EQ(pacer_->NextSendTime().ms(), next_send_time.ms());
...
TEST_P(PacingControllerTest, TaskLate) {
  ...
  // Check that next scheduled send time is witin sleep-time + 1ms.
