...
class LossBasedBweV2Test : public ::testing::TestWithParam<bool> {
 protected:
  ...
  std::vector<PacketResult> CreatePacketResultWithReceivedPackets(
  ...
  std::vector<PacketResult> CreatePacketResultWithLoss(
      Timestamp first_packet_timestamp) {
    std::vector<PacketResult> enough_feedback(2);
    enough_feedback[0].sent_packet.size = DataSize::Bytes(15'000);
    enough_feedback[1].sent_packet.size = DataSize::Bytes(15'000);
  ...
  void SetUpLossBasedBweWithPacketResults(
      LossBasedBweV2& loss_based_bwe,
      const std::vector<PacketResult>& packet_results) {
    loss_based_bwe.SetBandwidthEstimate(DataRate::KilobitsPerSec(600));
    loss_based_bwe.SetAcknowledgedBitrate(DataRate::KilobitsPerSec(300));
    loss_based_bwe.UpdateBandwidthEstimate(
        packet_results, DataRate::PlusInfinity(), BandwidthUsage::kBwNormal);
  }

  void SetUpLossBasedBweWithPacketResultsAndDelayBasedEstimate(
      LossBasedBweV2& loss_based_bwe,
      const std::vector<PacketResult>& packet_results,
      const DataRate& delay_based_estimate) {
    loss_based_bwe.SetBandwidthEstimate(DataRate::KilobitsPerSec(600));
    loss_based_bwe.SetAcknowledgedBitrate(DataRate::KilobitsPerSec(300));
    loss_based_bwe.UpdateBandwidthEstimate(packet_results, delay_based_estimate,
                                           BandwidthUsage::kBwNormal);
  }

  void LossBasedBweIncreasesAfterBackingOff(
      LossBasedBweV2& loss_based_bwe,
      const std::vector<PacketResult>& packet_results,
      const DataRate& delay_based_estimate) {
    // Increase the acknowledged bitrate to make sure that the estimate is not
    // capped too low.
    loss_based_bwe.SetAcknowledgedBitrate(DataRate::KilobitsPerSec(5000));
    loss_based_bwe.UpdateBandwidthEstimate(packet_results, delay_based_estimate,
                                           BandwidthUsage::kBwNormal);
  }
};
...
TEST_P(LossBasedBweV2Test,
  ...
  std::vector<PacketResult> enough_feedback =
      CreatePacketResultWithReceivedPackets(
...
TEST_P(LossBasedBweV2Test, BandwidthEstimateDecreasesWhenOverusing) {
  std::vector<PacketResult> enough_feedback_1 =
      CreatePacketResultWithLoss(/*first_packet_timestamp=*/Timestamp::Zero());
  std::vector<PacketResult> enough_feedback_2 =
      CreatePacketResultWithLoss(/*first_packet_timestamp=*/Timestamp::Zero() +
                                 2 * kObservationDurationLowerBound);
...
TEST_P(LossBasedBweV2Test, UseAckedBitrateForEmegencyBackOff) {
  ...
  // has 100% loss rate.
  std::vector<PacketResult> enough_feedback_1 = CreatePacketResultWithLoss(
      /*first_packet_timestamp=*/Timestamp::Zero());
...
TEST_P(LossBasedBweV2Test,
  ...
  ExplicitKeyValueConfig key_value_config(
      Config(/*enabled=*/true, /*valid=*/true,
             /*trendline_integration_enabled=*/GetParam()));
  LossBasedBweV2 loss_based_bandwidth_estimator(&key_value_config);
  SetUpLossBasedBweWithPacketResults(loss_based_bandwidth_estimator,
                                     enough_feedback_1);
  DataRate estimate_1 = loss_based_bandwidth_estimator.GetBandwidthEstimate(
      /*delay_based_limit=*/DataRate::PlusInfinity());
...
TEST_P(LossBasedBweV2Test,
  ...
  std::vector<PacketResult> enough_feedback_1 =
      CreatePacketResultWithReceivedPackets(
          /*first_packet_timestamp=*/Timestamp::Zero());
  std::vector<PacketResult> enough_feedback_2 = CreatePacketResultWithLoss(
      /*first_packet_timestamp=*/Timestamp::Zero() +
      kObservationDurationLowerBound - TimeDelta::Millis(1));
  ExplicitKeyValueConfig key_value_config(
      Config(/*enabled=*/true, /*valid=*/true,
...
TEST_P(LossBasedBweV2Test,
  ...
  std::vector<PacketResult> enough_feedback_1 =
      CreatePacketResultWithReceivedPackets(
          /*first_packet_timestamp=*/Timestamp::Zero());
  std::vector<PacketResult> enough_feedback_2 = CreatePacketResultWithLoss(
      /*first_packet_timestamp=*/Timestamp::Zero() + TimeDelta::Millis(1));
  ExplicitKeyValueConfig key_value_config(
      Config(/*enabled=*/true, /*valid=*/true,
             /*trendline_integration_enabled=*/GetParam()));
  LossBasedBweV2 loss_based_bandwidth_estimator(&key_value_config);
  ...
  SetUpLossBasedBweWithPacketResultsAndDelayBasedEstimate(
      loss_based_bandwidth_estimator, enough_feedback_1, delay_based_estimate);
  DataRate estimate_1 =
      loss_based_bandwidth_estimator.GetBandwidthEstimate(delay_based_estimate);
  LossBasedBweIncreasesAfterBackingOff(loss_based_bandwidth_estimator,
                                       enough_feedback_2, delay_based_estimate);
