    ...
    if (layer_id.has_value()) {
      ...
      bool is_simulcast =
          (num_spatial_layers == 1 &&
           static_cast<int>(encoding_settings.layers_settings.size()) > num_temporal_layers);
      if (is_simulcast) {
        num_spatial_layers =
...
EncodingSettings VideoCodecTester::CreateEncodingSettings(
  ...
  if (scalability_names.size() > 1) {
    ...
    RTC_CHECK_EQ(num_spatial_layers,
                 1);  // Only L1Tx is supported in simulcast for now.
  }
  int num_temporal_layers =
      ScalabilityModeToNumTemporalLayers(scalability_mode);
  ...
  if (num_bitrates != num_spatial_layers * num_temporal_layers) {
    ...
    if (bitrates.size() == 1) {
      ...
      for (size_t sidx = 0; sidx < encoder_config.number_of_streams; ++sidx) {
        VideoStream& ss = encoder_config.simulcast_layers[sidx];
        ss.scale_resolution_down_by = screencast ? 1.0 : (1 << sidx);
        ss.scalability_mode =
            ScalabilityModeFromString(scalability_names[sidx]);
        ss.num_temporal_layers =
            ScalabilityModeToNumTemporalLayers(*ss.scalability_mode);
        ss.active = true;
      }
      ...
      video_codec = VideoCodecInitializer::SetupCodec(field_trials,
    ...
    std::unique_ptr<VideoBitrateAllocator> bitrate_allocator =
        CreateBuiltinVideoBitrateAllocatorFactory()->Create(env, video_codec);
    VideoBitrateAllocation bitrate_allocation =
        bitrate_allocator->Allocate(VideoBitrateAllocationParameters(
