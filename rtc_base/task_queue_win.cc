...
TaskQueueWin::~TaskQueueWin() = default;
...
bool TaskQueue::Impl::ProcessQueuedMessages() {
  MSG msg = {};
...
class WinTaskQueueFactory : public TaskQueueFactory {
 public:
...
std::unique_ptr<TaskQueueFactory> CreateWinTaskQueueFactory() {
  return absl::make_unique<WinTaskQueueFactory>();
}
