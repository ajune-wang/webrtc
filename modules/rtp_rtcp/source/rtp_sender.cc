...
// Called from pacer when we can send the packet.
bool RTPSender::TrySendPacket(std::unique_ptr<RtpPacketToSend>* packet,
                              const PacedPacketInfo& pacing_info) {
  RTC_DCHECK(packet && packet->get());
  RtpPacketToSend& packet_to_send = **packet;

  const uint32_t packet_ssrc = packet_to_send.Ssrc();
  const auto packet_type = packet_to_send.packet_type();
  ...
  // the FEC.
  int64_t now_ms = clock_->TimeInMilliseconds();
  int64_t diff_ms = now_ms - packet_to_send.capture_time_ms();
  ...
  options.is_retransmit = packet_type == RtpPacketToSend::Type::kRetransmission;
  if (auto packet_id = packet_to_send.GetExtension<TransportSequenceNumber>()) {
    options.packet_id = *packet_id;
    options.included_in_feedback = true;
    options.included_in_allocation = true;
    AddPacketToTransportFeedback(*packet_id, packet_to_send, pacing_info);
  }

  options.application_data.assign(packet_to_send.application_data().begin(),
                                  packet_to_send.application_data().end());

  if (packet_to_send.packet_type() != RtpPacketToSend::Type::kPadding &&
      packet_to_send.packet_type() != RtpPacketToSend::Type::kRetransmission) {
    UpdateDelayStatistics(packet_to_send.capture_time_ms(), now_ms,
                          packet_ssrc);
    UpdateOnSendPacket(options.packet_id, packet_to_send.capture_time_ms(),
                       packet_ssrc);
  }
  ...
  if (is_media && packet_to_send.allow_retransmission()) {
    packet_history_.PutRtpPacket(
        absl::make_unique<RtpPacketToSend>(packet_to_send),
        StorageType::kAllowRetransmission, now_ms);
  } else if (packet_to_send.retransmitted_sequence_number()) {
    packet_history_.MarkPacketAsSent(
        *packet_to_send.retransmitted_sequence_number());
  }

  if (send_success) {
    UpdateRtpStats(packet_to_send, is_rtx,
