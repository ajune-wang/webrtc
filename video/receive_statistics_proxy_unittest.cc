...
TEST_F(ReceiveStatisticsProxyTest, DecodedFpsIsReportedWithQpReset) {
  ...
  for (int i = 0; i < metrics::kMinRunTimeInSeconds * kFps2; ++i) {
    statistics_proxy_->OnDecodedFrame(frame, 1u, 0,
                                      VideoContentType::UNSPECIFIED);
...
TEST_F(ReceiveStatisticsProxyTest,
       OnDecodedFrameWithQpResetsFramesDecodedAndTotalDecodeTime) {
  EXPECT_EQ(0u, statistics_proxy_->GetStats().frames_decoded);
  webrtc::VideoFrame frame = CreateFrame(kWidth, kHeight);
  for (uint32_t i = 1; i <= 3; ++i) {
  ...
  EXPECT_EQ(3u, statistics_proxy_->GetStats().total_decode_time_ms);
}
...
TEST_F(ReceiveStatisticsProxyTest, OnDecodedFrameIncreasesTotalEncodeTime) {
  EXPECT_EQ(absl::nullopt, statistics_proxy_->GetStats().qp_sum);
  webrtc::VideoFrame frame = CreateFrame(kWidth, kHeight);
