...
class SdpSerializerTest : public ::testing::TestWithParam<const char*> {
 public:
  ...
  void ExpectEqual(const SimulcastStreamAlternativeList& expected,
                   const SimulcastStreamAlternativeList& actual) const {
    EXPECT_EQ(expected.size(), actual.size());
    for (size_t i = 0; i < expected.size(); i++) {
  ...
  void ExpectEqual(const SimulcastDescription& expected,
    ...
    ExpectEqual(expected.send_streams(), actual.send_streams());
    ExpectEqual(expected.receive_streams(), actual.receive_streams());
  }
};
...
TEST_F(SdpSerializerTest, DeserializeSimulcast_SimpleCaseNoAlternatives) {
  ...
  expected.send_streams().AddStream("1");
  expected.send_streams().AddStream("2");
  expected.receive_streams().AddStream("3");
  expected.receive_streams().AddStream("4");
...
TEST_F(SdpSerializerTest, DeserializeSimulcast_SimpleCaseWithAlternatives) {
  ...
  SimulcastDescription expected;
  expected.send_streams().AddStreams({"1", "5"});
  expected.send_streams().AddStreams({"2", "6"});
  expected.receive_streams().AddStreams({"3", "7"});
  expected.receive_streams().AddStreams({"4", "8"});
  TestSimulcastDeserialization(simulcast_str, expected);
}
...
TEST_F(SdpSerializerTest, DeserializeSimulcast_OnlySendStreams) {
  ...
  expected.send_streams().AddStream("1");
  expected.send_streams().AddStreams({"2", "6"});
  expected.send_streams().AddStreams({"3", "7"});
  expected.send_streams().AddStream("4");
  TestSimulcastDeserialization(simulcast_str, expected);
}
...
TEST_F(SdpSerializerTest, DeserializeSimulcast_OnlyReceiveStreams) {
  ...
  SimulcastDescription expected;
  expected.receive_streams().AddStream("1");
  expected.receive_streams().AddStreams({"2", "6"});
  expected.receive_streams().AddStreams({"3", "7"});
  expected.receive_streams().AddStream("4");
  TestSimulcastDeserialization(simulcast_str, expected);
}
...
TEST_F(SdpSerializerTest, DeserializeSimulcast_PausedStreams) {
  ...
  SimulcastDescription expected;
  expected.receive_streams().AddStream("1");
  expected.receive_streams().AddStreams(
      {RidDescription("2", true), RidDescription("6")});
  expected.send_streams().AddStreams({"3", "7"});
  expected.send_streams().AddStream("4", true);
  TestSimulcastDeserialization(simulcast_str, expected);
}
...
TEST_F(SdpSerializerTest, SerializeSimulcast_SimpleCase) {
  ...
  simulcast.send_streams().AddStream("1");
  simulcast.receive_streams().AddStream("2");
...
TEST_F(SdpSerializerTest, SerializeSimulcast_OnlySend) {
  SimulcastDescription simulcast;
  simulcast.send_streams().AddStream("1");
  simulcast.send_streams().AddStream("2");
...
TEST_F(SdpSerializerTest, SerializeSimulcast_OnlyReceive) {
  SimulcastDescription simulcast;
  simulcast.receive_streams().AddStream("1");
  simulcast.receive_streams().AddStream("2");
...
TEST_F(SdpSerializerTest, SerializeSimulcast_ComplexSerialization) {
  SimulcastDescription simulcast;
  simulcast.send_streams().AddStreams(
      {RidDescription("2"), RidDescription("1", true)});
  simulcast.send_streams().AddStreams({"4", "3"});

  simulcast.receive_streams().AddStreams({"6", "7"});
  simulcast.receive_streams().AddStream("8", true);
  simulcast.receive_streams().AddStreams(
