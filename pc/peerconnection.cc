...
bool PeerConnection::GetSctpSslRole(rtc::SSLRole* role) {
  ...
  return transport_controller_->GetSslRole(*sctp_content_mid_, role);
}
...
std::unique_ptr<SessionStats> PeerConnection::GetSessionStats_s() {
  ...
  if (sctp_transport_) {
    RTC_DCHECK(sctp_content_mid_);
    channel_name_pairs.data =
        ChannelNamePair(*sctp_content_mid_, *sctp_transport_name());
  }
...
cricket::VoiceChannel* PeerConnection::CreateVoiceChannel(
  ...
  voice_channel->SetRtpTransport(transport_controller_->GetRtpTransport(mid));
...
cricket::VideoChannel* PeerConnection::CreateVideoChannel(
  ...
  video_channel->SignalSentPacket.connect(this,
                                          &PeerConnection::OnSentPacket_w);
  video_channel->SetRtpTransport(transport_controller_->GetRtpTransport(mid));
...
bool PeerConnection::CreateDataChannel(const std::string& mid) {
  ...
  if (sctp) {
    ...
    rtp_data_channel_->SetRtpTransport(
        transport_controller_->GetRtpTransport(mid));
  }
...
bool PeerConnection::CreateSctpTransport_n(const std::string& mid) {
  ...
  cricket::DtlsTransportInternal* tc = transport_controller_->GetDtlsTransport(
      mid, cricket::ICE_CANDIDATE_COMPONENT_RTP);
  RTC_DCHECK(tc);
  ...
  sctp_content_mid_ = mid;
  sctp_transport_name_ = mid;
  sctp_transport_->SetTransportChannel(transport_controller_->GetDtlsTransport(
      mid, cricket::ICE_CANDIDATE_COMPONENT_RTP));
  return true;
}
...
const std::string PeerConnection::GetTransportName(
  ...
  if (sctp_transport_) {
    RTC_DCHECK(sctp_content_mid_);
    RTC_DCHECK(sctp_transport_name_);
    if (content_name == *sctp_content_mid_) {
...
void PeerConnection::OnDtlsTransportChanged(
  ...
  if (sctp_transport_) {
    RTC_DCHECK(mid == sctp_content_mid_);
