...
H264SpsPpsTracker::PacketAction H264SpsPpsTracker::CopyAndFixBitstream(
  ...
  // flag to prevent it being interpreted as a frame by the packet buffer.
  if (reset_end_of_frame_flag_on_nonvlc_packet_ &&
      packet->is_last_packet_in_frame() && !HasVclData(*packet)) {
