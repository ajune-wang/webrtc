...
void GoogCcStatePrinter::PrintHeaders() {
  int ix = 0;
  for (const auto& logger : loggers_) {
    if (ix++)
      output_->Write(" ");
    output_->Write(logger->name());
  }
}

void GoogCcStatePrinter::PrintState(Timestamp at_time) {
  if (!controller_)
    return;
...
GoogCcDebugFactory::GoogCcDebugFactory(
    ...
    : GoogCcDebugFactory(std::move(log_writer), GoogCcFactoryConfig()) {}

GoogCcDebugFactory::GoogCcDebugFactory(
    std::unique_ptr<RtcEventLogOutput> log_writer,
    GoogCcFactoryConfig config)
    : GoogCcNetworkControllerFactory(std::move(config)),
      printer_(log_writer ? new GoogCcStatePrinter(std::move(log_writer))
                          : nullptr) {
  if (printer_)
    printer_->PrintHeaders();
}
...
ControlStatePrinter* GoogCcDebugFactory::printer() {
  return printer_.get();
}
