...
class PeerConnection : public PeerConnectionInternal,
 ...
 public:
  ...
  enum class UsageEvent : int {
    ...
    CLOSE_CALLED = 0x400,
    // A local host candidate with private IP is collected.
    PRIVATE_CANDIDATE_COLLECTED = 0x800,
    // A remote host candidate with private IP is added, either via
    ...
    // A local host candidate with IPv6 address is collected.
    IPV6_HOST_CANDIDATE_COLLECTED = 0x8000,
    // A remote host candidate with IPv6 address is added, either via
    // addIceCandidate or from the remote description.
