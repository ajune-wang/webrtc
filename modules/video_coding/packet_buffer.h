...
class PacketBuffer {
 public:
  ...
  InsertResult InsertPacket(std::unique_ptr<Packet> packet) ABSL_MUST_USE_RESULT
      RTC_LOCKS_EXCLUDED(mu_);
  InsertResult InsertPadding(uint16_t seq_num) ABSL_MUST_USE_RESULT
      RTC_LOCKS_EXCLUDED(mu_);
  void ClearTo(uint16_t seq_num) RTC_LOCKS_EXCLUDED(mu_);
  void Clear() RTC_LOCKS_EXCLUDED(mu_);

  // Timestamp (not RTP timestamp) of the last received packet/keyframe packet.
  absl::optional<int64_t> LastReceivedPacketMs() const RTC_LOCKS_EXCLUDED(mu_);
 ...
 private:
  ...
  // Clears with |mu_| taken.
  void ClearInternal() RTC_EXCLUSIVE_LOCKS_REQUIRED(mu_);

  // Tries to expand the buffer.
  bool ExpandBufferSize() RTC_EXCLUSIVE_LOCKS_REQUIRED(mu_);
  ...
  void UpdateMissingPackets(uint16_t seq_num) RTC_EXCLUSIVE_LOCKS_REQUIRED(mu_);
  ...
  uint16_t first_seq_num_ RTC_GUARDED_BY(mu_);

  // If the packet buffer has received its first packet.
  bool first_packet_received_ RTC_GUARDED_BY(mu_);

  // If the buffer is cleared to |first_seq_num_|.
  bool is_cleared_to_first_seq_num_ RTC_GUARDED_BY(mu_);

  // Buffer that holds the the inserted packets and information needed to
  // determine continuity between them.
  std::vector<std::unique_ptr<Packet>> buffer_ RTC_GUARDED_BY(mu_);

  // Timestamp of the last received packet/keyframe packet.
  absl::optional<int64_t> last_received_packet_ms_ RTC_GUARDED_BY(mu_);
  absl::optional<int64_t> last_received_keyframe_packet_ms_ RTC_GUARDED_BY(mu_);
  absl::optional<uint32_t> last_received_keyframe_rtp_timestamp_
      RTC_GUARDED_BY(mu_);

  absl::optional<uint16_t> newest_inserted_seq_num_ RTC_GUARDED_BY(mu_);
