...
TEST_P(RtpSenderTest, SendsPacketsWithTransportSequenceNumber) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    rtp_sender_->TimeToSendPacket(std::move(packet), false, PacedPacketInfo());
  }
...
TEST_P(RtpSenderTest, WritesPacerExitToTimingExtension) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_CALL(mock_paced_sender_,
                InsertPacket(Pointee(Property(&RtpPacketToSend::Ssrc, kSsrc)),
                             RtpPacketSender::kNormalPriority, _));
    EXPECT_TRUE(rtp_sender_->SendToNetwork(
        absl::make_unique<RtpPacketToSend>(*packet), kAllowRetransmission,
        RtpPacketSender::kNormalPriority));
    fake_clock_.AdvanceTimeMilliseconds(kStoredTimeInMs);
...
TEST_P(RtpSenderTest, WritesNetwork2ToTimingExtensionWithPacer) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_CALL(mock_paced_sender_,
                InsertPacket(Pointee(Property(&RtpPacketToSend::Ssrc, kSsrc)),
                             RtpPacketSender::kNormalPriority, false));
    EXPECT_TRUE(rtp_sender_->SendToNetwork(
        absl::make_unique<RtpPacketToSend>(*packet), kAllowRetransmission,
        RtpPacketSender::kNormalPriority));
    fake_clock_.AdvanceTimeMilliseconds(kStoredTimeInMs);
...
TEST_P(RtpSenderTest, TrafficSmoothingRetransmits) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_TRUE(rtp_sender_->SendToNetwork(
        ...
        RtpPacketSender::kNormalPriority));
    // Immediately process send bucket and send packet.
    rtp_sender_->TimeToSendPacket(absl::make_unique<RtpPacketToSend>(*packet),
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_CALL(
        ...
        InsertPacket(
            ...
            _, _));
    auto packet = SendGenericPacket();
    rtp_sender_->TimeToSendPacket(std::move(packet), kIsRetransmit,
                                  PacedPacketInfo());
  }
...
TEST_P(RtpSenderTest, SendRedundantPayloads) {
  ...
  for (size_t i = 0; i < kNumPayloadSizes; ++i) {
    ...
    if (GetParam().pacer_references_packets) {
      ...
      EXPECT_CALL(
          mock_paced_sender_,
          InsertPacket(AllOf(Pointee(Property(&RtpPacketToSend::Ssrc, kSsrc)),
                             Pointee(Property(&RtpPacketToSend::SequenceNumber,
                                              seq_num))),
                       _, _));
      auto packet = SendPacket(capture_time_ms, kPayloadSizes[i]);
      rtp_sender_->TimeToSendPacket(std::move(packet), false,
                                    PacedPacketInfo());
    }
...
TEST_P(RtpSenderTest, SendFlexfecPackets) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_CALL(mock_paced_sender_,
                InsertPacket(_, RtpPacketSender::kLowPriority, false))
        .Times(2)
        .WillRepeatedly([&](std::unique_ptr<RtpPacketToSend> packet,
                            RtpPacketSender::Priority priority,
                            bool retransmission) {
          if (packet->Ssrc() == kMediaSsrc) {
    ...
    rtp_sender_->TimeToSendPacket(std::move(media_packet), false,
                                  PacedPacketInfo());
    rtp_sender_->TimeToSendPacket(std::move(fec_packet), false,
                                  PacedPacketInfo());
  }
...
TEST_P(RtpSenderTest, NoFlexfecForTimingFrames) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_CALL(
        mock_paced_sender_,
        InsertPacket(
            AllOf(Pointee(Property(&RtpPacketToSend::Ssrc, kMediaSsrc)),
                  Pointee(Property(&RtpPacketToSend::SequenceNumber, kSeqNum))),
            RtpPacketSender::kLowPriority, false))
    ...
    EXPECT_CALL(
        mock_paced_sender_,
        InsertPacket(Pointee(Property(&RtpPacketToSend::Ssrc, kFlexfecSsrc)),
                     RtpPacketSender::kLowPriority, false))
        .Times(0);  // Not called because packet should not be protected.
    ...
    EXPECT_EQ(RtpPacketSendResult::kSuccess,
              rtp_sender_->TimeToSendPacket(std::move(rtp_packet), false,
                                            PacedPacketInfo()));
  }
...
TEST_P(RtpSenderTest, FecOverheadRate) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_CALL(mock_paced_sender_, InsertPacket(_, _, false))
        .Times(kNumMediaPackets + kNumFecPackets);
  }
