...
//
// This class owns the RtpSenders and RtpReceivers associated with this m=
// section. Additionally, it effectively owns the BaseChannel used for
// processing SDP and interfacing with the underlying media engine. Note that
// the BaseChannel memory is actually owned by ChannelManager, so an appropriate
// call to Destroy{Voice|Video}Channel must be done (currently this is in
// PeerConnection).
//
