...
// one of these a task queues depending on the field trial
...
class MaybeWorkerThread {
 public:
  ...
  // Hack to use a SafeTask only if the target sequence is the worker thread.
  // This is used when a SafeTask can not be used because the object that post
  // the task is not destroyed on the target sequence. Instead, caller has to
  // guarantee that this MaybeWorkerThread is destroyed first
