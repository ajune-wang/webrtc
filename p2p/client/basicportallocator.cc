...
PortAllocationResult AllocationSequence::CreateRelayPorts() {
  ...
  AllocationProgress relay_ports_progress;
  for (RelayServerConfig& relay : config_->relays) {
    if (relay.type == RELAY_GTURN) {
      relay_ports_progress.Update(CreateGturnPort(relay));
    } else if (relay.type == RELAY_TURN) {
      relay_ports_progress.Update(CreateTurnPort(relay));
    } else {
      RTC_NOTREACHED();
    }
  }
  if (!relay_ports_progress.tried_creation_with_feasible_config()) {
    return PortAllocationResult::FAILURE_INFEASIBLE_CONFIG;
  }
  if (!relay_ports_progress.created_valid_port()) {
    return PortAllocationResult::FAILURE_CREATION_ERROR;
  }
  return PortAllocationResult::SUCCESS;
}
...
void AllocationSequence::MaybeRetryPortAllocation() {
  RTC_DCHECK(rtc::Thread::Current() == session_->network_thread());
  // We may retry the port allocation if there is no valid port allocated for a
  // phase and not all allocation failures are due to configuration.
