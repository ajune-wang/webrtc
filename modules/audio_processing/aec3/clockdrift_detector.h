...
class ClockdriftDetector {
 public:
  ...
  // Returns true if clockdrift is detected.
  bool VerifiedClockdrift() { return level_ == ClockdriftLevel::kVerified; }
  // Like VerifiedClockdrift() but the detection is based on less data. This
  // means that clockdrift is found faster but with slightly increased risk of
  // false positives.
