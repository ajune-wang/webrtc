...
class RtcEventLogSession
 ...
 public:
  RtcEventLogSession()
    ...
    temp_filename = test::OutputPath() + test_name;
  }
  void WriteLog(EventCounts count, size_t num_events_before_log_start);
  void ReadAndVerifyLog();

 private:
  void WriteConfigs(EventCounts count, RtcEventLog* event_log);
...
void RtcEventLogSession::WriteConfigs(EventCounts count,
...
void RtcEventLogSession::WriteLog(EventCounts count,
  ...
  // TODO(terelius): Allow test to run wth either a real or a fake clock_.
  ...
  WriteConfigs(count, event_log.get());
