...
class StructuredForm : public Describable {
 public:
  ...
  explicit StructuredForm(const StructuredForm& other) { Init(other); }
  ...
  // Check using IsStump() if the string value should not be cleared in case the
  // parent is a stump.
   ...
   public:                                                            \
    ...
    explicit enum_name(Value enum_val)                                \
      ...
      if (enum_val == enum_name::kUndefined) {                        \
        value_sf_["value"].reset(new StructuredForm("undefined"));    \
        value_sf_["comment"].reset(new StructuredForm(enum_val_str)); \
        return;                                                       \
      }                                                               \
