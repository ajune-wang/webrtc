...
void AppendFrames(std::vector<std::unique_ptr<RtpFrameObject>> from,
                  std::vector<std::unique_ptr<RtpFrameObject>>& to) {
...
TEST(RtpVideoFrameAssembler, RawPacketizationGenericPayloadDescriptor) {
  RtpVideoFrameAssembler assembler(RtpVideoFrameAssembler::kGenericPayload);
  std::vector<std::unique_ptr<RtpFrameObject>> frames;
  uint8_t kPayload[] = "SomePayload";
