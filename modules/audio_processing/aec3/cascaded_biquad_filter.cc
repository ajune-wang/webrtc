...
CascadedBiQuadFilter::BiQuadParam::BiQuadParam(const BiQuadParam& parm) =
    default;
...
CascadedBiQuadFilter::BiQuad::BiQuad(
  ...
  coefficients.b[0] = gain * 1.0f;
  coefficients.b[1] = gain * -2.0f * z_r;
  coefficients.b[2] = gain * (z_r * z_r + z_i * z_i);

  coefficients.a[0] = -2.0f * p_r;
...
CascadedBiQuadFilter::CascadedBiQuadFilter(
    ...
    size_t num_biquads) {
  for (size_t i = 0; i < num_biquads; i++) {
    biquads_.push_back(BiQuad(coefficients));
  }
}
