...
PeerConnection::PeerConnection(PeerConnectionFactory* factory,
    ...
    : factory_(factory),
      ...
      datagram_transport_config_(
          factory_->trials().Lookup(kDatagramTransportFieldTrial)),
      datagram_transport_data_channel_config_(
          factory_->trials().Lookup(kDatagramTransportDataChannelFieldTrial)),
...
bool PeerConnection::IsFieldTrialEnabled(absl::string_view name) const {
  return absl::StartsWith(factory_->trials().Lookup(name), "Enabled");
}

bool PeerConnection::IsFieldTrialDisabled(absl::string_view name) const {
  return absl::StartsWith(factory_->trials().Lookup(name), "Disabled");
}
