...
// Responsible for adjusting the inputs to VideoStreamAdapter
// (SetInputForTesting), such as pixels and frame rate, according to the most
// recent source restrictions. This helps tests that apply adaptations multiple
// times: if the input is not adjusted between adaptations, the subsequent
// adaptations fail with kAwaitingPreviousAdaptation.
class FakeVideoStream {
 public:
  FakeVideoStream(VideoStreamAdapter* adapter,
    ...
    adapter_->SetInputForTesting(input_pixels_, input_fps_, encoder_settings_);
  }

  int input_pixels() const { return input_pixels_; }
  int input_fps() const { return input_fps_; }

  // Performs ApplyAdaptation() followed by SetInputForTesting() with input
  // pixels and frame rate adjusted according to the resulting restrictions.
...
TEST(VideoStreamAdapterTest, MaintainFramerate_DecreasesPixelsToThreeFifths) {
  ...
  adapter.SetInputForTesting(kInputPixels, 30, absl::nullopt);
...
TEST(VideoStreamAdapterTest, MaintainResolution_DecreasesFpsToTwoThirds) {
  ...
  adapter.SetInputForTesting(1280 * 720, kInputFps, absl::nullopt);
  Adaptation adaptation = adapter.GetAdaptationDown();
  EXPECT_EQ(Adaptation::Status::kValid, adaptation.status());
  adapter.ApplyAdaptation(adaptation);
  EXPECT_EQ(absl::nullopt,
...
TEST(VideoStreamAdapterTest, Balanced_DecreaseFrameRate) {
  ...
  adapter.SetInputForTesting(kBalancedMediumResolutionPixels,
...
//   adapter.SetInputForTesting(1280 * 720, 30, absl::nullopt);
...
//   adapter.SetInputForTesting(VideoStreamAdapter::VideoInputMode::kNoVideo,
//   1280 * 720, 30,
//                    absl::nullopt);
...
TEST(VideoStreamAdapterDeathTest, SetInputForTestingInvalidatesAdaptations) {
  VideoStreamAdapter adapter;
  adapter.SetDegradationPreference(DegradationPreference::MAINTAIN_RESOLUTION);
  adapter.SetInputForTesting(1280 * 720, 30, absl::nullopt);
  Adaptation adaptation = adapter.GetAdaptationDown();
  adapter.SetInputForTesting(1280 * 720, 31, absl::nullopt);
