...
void VideoProcessorIntegrationTest::CreateEncoderAndDecoder() {
  ...
  if (config_.hw_encoder) {
    ...
    encoder_factory.reset(
        new webrtc::jni::VideoEncoderFactoryWrapper(env, factory_object));
#elif defined(WEBRTC_IOS)
    ...
    encoder_factory.reset(new InternalEncoderFactory());
  }
  ...
  if (config_.hw_decoder) {
    ...
    decoder_factory.reset(
        new webrtc::jni::VideoDecoderFactoryWrapper(env, factory_object));
#elif defined(WEBRTC_IOS)
    EXPECT_EQ(kVideoCodecH264, config_.codec_settings.codecType)
        << "iOS HW codecs only support H264.";
    ...
    decoder_factory.reset(new InternalDecoderFactory());
  }
