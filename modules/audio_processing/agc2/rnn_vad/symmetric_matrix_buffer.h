...
class SymmetricMatrixBuffer {
 ...
 public:
  ...
  ~SymmetricMatrixBuffer() = default;
  // Set the buffer values to zero.
  void Reset() {
    static_assert(std::is_arithmetic<T>::value,
                  "Integral or floating point required.");
    buf_.fill(0);
  }
  // Push the results from the comparison between the most recent item and those
  // that are still in the ring buffer. The first element in |values| must
  ...
  void Push(rtc::ArrayView<T, S - 1> values) {
    // Move the lower-right sub-matrix of size (S-1) x (S-1) one row up and one
    // column left.
    std::memcpy(buf_.data(), buf_.data() + S, (buf_.size() - S) * sizeof(T));
  ...
  // Read the value that corresponds to comparison of two items in the ring
  // buffer having delay |delay1| and |delay2|.
