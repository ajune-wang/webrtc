...
WebRtcRTPHeader CreateFakeWebrtcRTPHeader(
  ...
  // Convert MediaTransportEncodedAudioFrame::FrameType to webrtc::FrameType.
  switch (frame.frame_type()) {
    case MediaTransportEncodedAudioFrame::FrameType::kSpeech:
      webrtc_header.frameType = kAudioFrameSpeech;
      break;

    case MediaTransportEncodedAudioFrame::FrameType::
        kDiscountinuousTransmission:
      webrtc_header.frameType = kAudioFrameCN;
      break;
  }
...
void ChannelReceive::OnData(uint64_t channel_id,
  ...
  if (audio_coding_->IncomingPacket(
          ...
          // Fill out rtp header.
          CreateFakeWebrtcRTPHeader(frame)) != 0) {
