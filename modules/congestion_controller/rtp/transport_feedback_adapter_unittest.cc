...
TEST_F(TransportFeedbackAdapterTest, ObserverSanity) {
  ...
  EXPECT_TRUE(
      second_feedback.AddReceivedPacket(new_packet.sent_packet.sequence_number,
                                        new_packet.receive_time.ms() * 1000));
...
TEST_F(TransportFeedbackAdapterTest, AdaptsFeedbackAndPopulatesSendTimes) {
  ...
  auto result = adapter_->ProcessTransportFeedback(
      feedback, Timestamp::ms(clock_.TimeInMilliseconds()));
...
TEST_F(TransportFeedbackAdapterTest, FeedbackVectorReportsUnreceived) {
  ...
  auto res = adapter_->ProcessTransportFeedback(
      feedback, Timestamp::ms(clock_.TimeInMilliseconds()));
