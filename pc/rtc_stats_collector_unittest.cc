...
rtc::scoped_refptr<MediaStreamTrackInterface> CreateFakeTrack(
  ...
  if (media_type == cricket::MEDIA_TYPE_AUDIO) {
    return FakeAudioTrackForStats::Create(track_identifier, track_state,
                                          create_fake_audio_processor);
  } else {
    RTC_DCHECK_EQ(media_type, cricket::MEDIA_TYPE_VIDEO);
    return FakeVideoTrackForStats::Create(track_identifier, track_state,
                                          nullptr);
  }
}
...
class RTCStatsCollectorWrapper {
 public:
  ...
  rtc::scoped_refptr<MockRtpSenderInternal> SetupLocalTrackAndSender(
    ...
    rtc::scoped_refptr<MediaStreamTrackInterface> track;
    if (media_type == cricket::MEDIA_TYPE_AUDIO) {
      track = CreateFakeTrack(media_type, track_identifier,
...
// TODO(hbos): FIX AND RE-ENABLE
TEST_F(RTCStatsCollectorTest, DISABLED_GetStatsWithSenderSelector) {
  ExampleStatsGraph graph = SetupExampleStatsGraphForSelectorTests();
...
// TODO(hbos): FIX AND RE-ENABLE
TEST_F(RTCStatsCollectorTest, DISABLED_GetStatsWithReceiverSelector) {
  ExampleStatsGraph graph = SetupExampleStatsGraphForSelectorTests();
...
// SSRC 0, meaning "unconnected". In this state, we don't report RTP stats yet.
