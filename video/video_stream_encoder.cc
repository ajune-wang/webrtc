...
EncodedImageCallback::Result VideoStreamEncoder::OnEncodedImage(
  ...
  std::unique_ptr<CodecSpecificInfo> codec_info_copy;
  {
    ...
    if (codec_specific_info && codec_specific_info->generic_frame_info) {
      ...
      for (const CodecBuffer& buffer : generic_info.encoder_buffers) {
        ...
        int64_t diff = frame_id - state[buffer.id];
        if (buffer.referenced &&
            !std::any_of(generic_info.frame_diffs.begin(),
                         generic_info.frame_diffs.end(),
                         [diff](int64_t d) { return d == diff; })) {
