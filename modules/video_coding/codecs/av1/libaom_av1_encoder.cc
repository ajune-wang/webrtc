...
int LibaomAv1Encoder::InitEncode(const VideoCodec* codec_settings,
  ...
  webrtc::FieldTrialOptional<int> max_consec_drop_ms("max_consec_drop_ms");
  webrtc::FieldTrialOptional<int> aq_mode("aq_mode");
  webrtc::FieldTrialOptional<int> lossless("lossless");
  webrtc::ParseFieldTrial(
      {&num_threads, &rc_end_usage, &rc_min_quantizer, &rc_max_quantizer,
       ...
       &palette, &max_consec_drop, &max_consec_drop_ms, &aq_mode, &lossless},
  ...
  if (max_consec_drop_ms.GetOptional().has_value()) {
    SET_ENCODER_PARAM_OR_RETURN_ERROR(AV1E_SET_MAX_CONSEC_FRAME_DROP_MS_CBR,
                                      max_consec_drop_ms.Value());
  }
  if (aq_mode.GetOptional().has_value()) {
    SET_ENCODER_PARAM_OR_RETURN_ERROR(AV1E_SET_AQ_MODE, aq_mode.Value());
  }
  if (lossless.GetOptional().has_value()) {
    SET_ENCODER_PARAM_OR_RETURN_ERROR(AV1E_SET_LOSSLESS, lossless.Value());
  }
