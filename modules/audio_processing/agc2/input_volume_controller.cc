...
InputVolumeController::InputVolumeController(int num_capture_channels,
  ...
  RTC_LOG(LS_INFO)
      ...
      << min_input_volume_;
  RTC_DCHECK_EQ(channel_controllers_.size(), num_capture_channels_);
...
void InputVolumeController::AnalyzeInputAudio(int applied_input_volume,
  ...
  // `channel_controllers_.size()`, `num_capture_channels_` and `audio_buffer`
  // all represent the same number of channels.
