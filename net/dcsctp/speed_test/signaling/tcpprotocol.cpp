...
bool TCPProtocol::SignalInputData(int32_t recvAmount) {
  std::cout << "#-> TCPProtocol::SignalInputData id=" << GetId()
            << " recv=" << recvAmount
            << " avail=" << GETAVAILABLEBYTESCOUNT(_inputBuffer) << std::endl;

  int i = 0;
  while (true) {
    i++;

    std::cout << "    TCPProtocol::SignalInputData id=" << GetId() << " " << i
              << ") " << std::endl;
    ...
    if (_observer) {
      _observer->OnMessage(this, GETIBPOINTER(_inputBuffer) + HEADER_SIZE,
                           length);
    } else {
      std::cout << "    TCPProtocol::SignalInputData id=" << GetId() << " " << i
                << ") NO OBSERVER" << std::endl;
    }
...
bool TCPProtocol::SendOutOfBandData(const IOBuffer& buffer) {
  if (!_outputBuffer.ReadFromBuffer(GETIBPOINTER(buffer),
...
bool TCPProtocol::SendMessage(std::string msg) {
  ...
  std::cout << "TCPProtocol::SendMessage() len=" << length << " | " << msg
            << std::endl;
