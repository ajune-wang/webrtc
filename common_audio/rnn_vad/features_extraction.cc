...
static_assert(
    kHalfFftSize / 2 < kHalfFrameSize && kHalfFrameSize <= kHalfFftSize,
    "Invalid FFT size.");
...
void ComputeBandEnergies(
  ...
  RTC_DCHECK(fft_buf);
  RTC_DCHECK_LT(0.f, fft_scaling);
  std::memset(band_energies.data(), 0, sizeof(float) * band_energies.size());
  size_t i;
  ...
  for (i = 0; i < band_energies.size() - 1; ++i) {
    ...
    const size_t last_freq_bin =
        std::min(max_freq_bin_index,
                 first_freq_bin + bands_indexes[i + 1] - bands_indexes[i]);
    const size_t band_size = last_freq_bin - first_freq_bin;
    ...
    for (size_t j = first_freq_bin; j < last_freq_bin; ++j) {
      ...
      const float freq_bin_energy = std::norm(fft_buf->Row(0)[j] / fft_scaling);
...
void RnnVadFeaturesExtractor::ComputeFeatures(
  ...
  ComputeForwardFft(half_band_buf_.GetBufferView(kHalfPitchMaxPeriod),
                    {half_analysis_win_}, fft_.get(),
                    &fft_input_buf_, &fft_output_buf_);
  ComputeBandEnergies(&fft_output_buf_, kHalfFftSize,
