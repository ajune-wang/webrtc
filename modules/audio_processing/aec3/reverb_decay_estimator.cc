...
float BlockEnergyAverage(rtc::ArrayView<const float> h, int block_index) {
  ...
  std::transform(h.begin() + block_index * kFftLengthBy2,
                 h.begin() + (block_index + 1) * kFftLengthBy2, h2.begin(),
                 [](float a) { return a * a; });
...
void ReverbDecayEstimator::EarlyReverbLengthEstimator::Accumulate(
  ...
  // needs to be updated.
  int first_section = std::max(block_counter_ - kBlocksPerSection + 1, 0);
  int last_section = std::min(block_counter_, static_cast<int>(nz_.size() - 1));
  ...
  float value_to_inc = kFftLengthBy2 * value;
  value_to_add += (block_counter_ - last_section) * value_to_inc;
  for (int section = last_section; section >= first_section;
...
int ReverbDecayEstimator::EarlyReverbLengthEstimator::Estimate() {
  ...
  // Estimation of the blocks that corresponds to early reverberations. The
  ...
  // considered to be part of the early reverberation. Furthermore, the blocks
  // where the energy is decreasing faster than what it does at the end of the
  // impulse response are also considered to be part of the early reverberation
  // regions. The estimation is limited to the first kNumSectionsToAnalyze
  // blocks.
