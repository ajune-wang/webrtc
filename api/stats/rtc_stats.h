...
class RTC_EXPORT RTCStats {
 ...
 protected:
  ...
  virtual void PopulateAttributes() const;

  std::string const id_;
  Timestamp timestamp_;

  // Because the const method Members() return raw pointers, we need to cache
  // attribues to avoid UAF. Mutable to allow lazily instantiation.
  // TODO(https://crbug.com/webrtc/15164): Migrate callers to Attributes() and
  // delete Members() and `attributes_`.
  mutable std::vector<std::unique_ptr<RTCStatsMemberInterface>> attributes_;
};
...
#define WEBRTC_RTCSTATS_DECL()                             \
 protected:                                                \
  void PopulateAttributes() const override;                \
                                                           \
 public:                                                   \
  static const char kType[];                               \
                                                           \
...
#define WEBRTC_RTCSTATS_IMPL(this_class, parent_class, type_str, ...) \
  ...
  const char* this_class::type() const {                              \
    return this_class::kType;                                         \
                                                                      ...
                                                                      \
  void this_class::PopulateAttributes() const {                       \
