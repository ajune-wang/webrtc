...
size_t GetFirstActiveLayer(const VideoCodec& codec) {
  RTC_DCHECK_EQ(codec.codecType, kVideoCodecAV1);
  RTC_DCHECK_GT(codec.AV1().numberOfSpatialLayers, 0u);
  size_t layer = 0;
  for (; layer < codec.AV1().numberOfSpatialLayers; ++layer) {
...
static size_t GetNumActiveSpatialLayers(const VideoCodec& codec) {
  ...
  for (; last_active_layer < codec.AV1().numberOfSpatialLayers;
       ++last_active_layer) {
...
LibaomSvcRateAllocator::LibaomSvcRateAllocator(const VideoCodec& codec)
    : codec_(codec),
  ...
  RTC_DCHECK_EQ(codec.codecType, kVideoCodecAV1);
  RTC_DCHECK_GT(codec.AV1().numberOfSpatialLayers, 0u);
  RTC_DCHECK_GT(codec.AV1().numberOfTemporalLayers, 0u);
  for (size_t layer_idx = 0; layer_idx < codec.AV1().numberOfSpatialLayers;
       ++layer_idx) {
...
VideoBitrateAllocation LibaomSvcRateAllocator::Allocate(
  ...
  if (codec_.spatialLayers[0].targetBitrate == 0) {
    // Delegate rate distribution to AV1 encoder wrapper if bitrate thresholds
    // are not set.
...
VideoBitrateAllocation LibaomSvcRateAllocator::GetAllocationNormalVideo(
    DataRate total_bitrate,
    size_t first_active_layer,
  ...
  const size_t num_temporal_layers = codec_.AV1().numberOfTemporalLayers;
...
// bit-rate allocated.
VideoBitrateAllocation LibaomSvcRateAllocator::GetAllocationScreenSharing(
    DataRate total_bitrate,
    size_t first_active_layer,
    size_t num_spatial_layers) const {
  VideoBitrateAllocation bitrate_allocation;
...
size_t LibaomSvcRateAllocator::FindNumEnabledLayers(
...
DataRate LibaomSvcRateAllocator::GetMaxBitrate(const VideoCodec& codec) {
  const size_t first_active_layer = GetFirstActiveLayer(codec);
...
DataRate LibaomSvcRateAllocator::GetPaddingBitrate(const VideoCodec& codec) {
...
LibaomSvcRateAllocator::GetLayerStartBitrates(const VideoCodec& codec) {
