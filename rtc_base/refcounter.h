...
class RefCounter {
 public:
  ...
  void IncRef() {
    ...
    // Read and writes to the resource can be safely reordered past this
  ...
  rtc::RefCountReleaseStatus DecRef() {
    ...
    // The acquire-release memory order prevents normal accesses to the
    ...
    return references == 1 ? rtc::RefCountReleaseStatus::kDroppedLastRef
