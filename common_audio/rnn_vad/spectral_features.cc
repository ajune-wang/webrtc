...
float ComputeDctScalingFactor(const size_t input_size) {
...
void SpectralFeaturesExtractor::CopySpectralCoefficients(
  ...
  auto band_energy_coeffs = spectral_coeffs_ring_buf_.GetArrayView(0);
  RTC_DCHECK_LE(dst.size(), band_energy_coeffs.size() - offset);
  std::memcpy(dst.data(), band_energy_coeffs.data() + offset,
