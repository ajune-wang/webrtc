...
template <>
absl::optional<float> ParseFloatingPoint(const char* str) {
  RTC_DCHECK(str);
  if (*str == '\0')
    return absl::nullopt;
  char* end = nullptr;
...
template <>
absl::optional<double> ParseFloatingPoint(const char* str) {
  RTC_DCHECK(str);
  if (*str == '\0')
    return absl::nullopt;
  char* end = nullptr;
...
template <>
absl::optional<long double> ParseFloatingPoint(const char* str) {
  RTC_DCHECK(str);
  if (*str == '\0')
    return absl::nullopt;
  char* end = nullptr;
