...
void PeerConnection::GetOptionsForUnifiedPlanOffer(
  ...
  for (size_t i = 0;
    ...
    if (had_been_rejected) {
      ...
      // CreateOffer shouldn't really cause any state changes in PeerConnection,
      // but we need a way to match new transceivers to new media sections in
      // SetLocalDescription and JSEP specifies this is done by recording the
      // index of the media section generated for the transceiver in the offer.
