...
class RtcpTransceiverImpl {
 ...
 private:
  void SchedulePeriodicCompoundPackets(int64_t start_delay_ms);
  // Sends RTCP packets, returns desired time until next report in milliseconds.
  int64_t SendPacket();

  const RtcpTransceiverConfig config_;

  rtc::Optional<rtc::WeakPtrFactory<RtcpTransceiverImpl>> ptr_factory_;
