...
H264BitstreamParser::Result H264BitstreamParser::ParseNonParameterSetNalu(
  ...
  BitReader slice_reader(
      rtc::MakeArrayView(slice_rbsp.data() + H264::kNaluTypeSize,
  ...
  if (sps_->separate_colour_plane_flag == 1) {
    // colour_plane_id
    slice_reader.Consume(2);
  }
  // frame_num: u(v)
  // Represented by log2_max_frame_num bits.
  slice_reader.Consume(sps_->log2_max_frame_num);
  uint32_t field_pic_flag = 0;
  if (sps_->frame_mbs_only_flag == 0) {
    // field_pic_flag: u(1)
    field_pic_flag = slice_reader.Read(1);
    if (field_pic_flag != 0) {
      // bottom_field_flag: u(1)
      slice_reader.Consume(1);
    }
  }
  if (is_idr) {
    // idr_pic_id: ue(v)
  ...
  if (sps_->pic_order_cnt_type == 0) {
    slice_reader.Consume(sps_->log2_max_pic_order_cnt_lsb);
  ...
  switch (slice_type) {
    ...
    case H264::SliceType::kSp: {
      ...
      uint32_t num_ref_idx_active_override_flag = slice_reader.Read(1);
  ...
  // else
  {
    ...
    if (slice_type % 5 != 2 && slice_type % 5 != 4) {
      ...
      uint32_t ref_pic_list_modification_flag_l0 = slice_reader.Read(1);
    ...
    if (slice_type % 5 == 1) {
      ...
      uint32_t ref_pic_list_modification_flag_l1 = slice_reader.Read(1);
  ...
  if (nal_ref_idc != 0) {
    ...
    if (is_idr) {
      ...
      uint32_t adaptive_ref_pic_marking_mode_flag = slice_reader.Read(1);
  ...
  return slice_reader.Ok() ? kOk : kInvalidStream;
}
