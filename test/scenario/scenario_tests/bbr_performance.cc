...
struct CallTestConfig {
  struct Scenario {
    ...
    Scenario()
        : random_seed("rs", 1),
          return_traffic("ret"),
          capacity("bw", DataRate::KilobitsPerSecond(300)),
          propagation_delay("dl", TimeDelta::Milliseconds(100)),
...
TEST_P(BbrScenarioTest, ReceivesVideo) {
  ...
  call_config.transport.rates.min_rate = DataRate::KilobitsPerSecond(30);
  call_config.transport.rates.max_rate = DataRate::KilobitsPerSecond(1800);
  ...
  VideoStreamPair* alice_video =
      s.CreateVideoStream(route->forward(), [&](VideoStreamConfig* c) {
        c->encoder.fake.max_rate = DataRate::KilobitsPerSecond(1800);
      });
  s.CreateAudioStream(route->forward(), [&](AudioStreamConfig* c) {
    if (conf_.tuning.use_bbr) {
      c->stream.in_bandwidth_estimation = true;
      c->encoder.fixed_rate = DataRate::KilobitsPerSecond(31);
    }
  });
  ...
  s.CreatePrinter("send.stats.txt", TimeDelta::Milliseconds(100),
  ...
  s.CreatePrinter("return.stats.txt", TimeDelta::Milliseconds(100),
                  return_printers);

  s.RunFor(TimeDelta::Milliseconds(kRunTimeMs));
}
