...
// Fake / mock object written specially for this test.
class TestCricketSctpTransport : public cricket::SctpTransportInternal {
 public:
 ...
 public:
  ...
  void CreateTransport() {
    auto cricket_sctp_transport =
        absl::WrapUnique(new TestCricketSctpTransport());
  ...
  TestCricketSctpTransport* CricketSctpTransport() {
    return static_cast<TestCricketSctpTransport*>(transport_->internal());
  }
...
TEST_F(SctpTransportTest, CreateClearDelete) {
  std::unique_ptr<cricket::SctpTransportInternal> fake_cricket_sctp_transport =
      absl::WrapUnique(new FakeSctpTransport());
...
TEST_F(SctpTransportTest, EventsObservedWhenConnecting) {
  ...
  EXPECT_THAT(observer_.states_, ElementsAre(SctpTransportState::kConnecting,
...
TEST_F(SctpTransportTest, CloseWhenClearing) {
  ...
  AddDtlsTransport();
  CompleteSctpHandshake();
  ASSERT_TRUE_WAIT(observer_.state() == SctpTransportState::kConnected,
                   kDefaultTimeout);
  transport()->Clear();
  ASSERT_TRUE_WAIT(observer_.state() == SctpTransportState::kClosed,
                   kDefaultTimeout);
}

TEST_F(SctpTransportTest, RemoteDisconnectCloses) {
  CreateTransport();
  transport()->RegisterObserver(observer());
  AddDtlsTransport();
  CompleteSctpHandshake();
  CricketSctpTransport()->SendSignalClosingProcedureStartedRemotely();
  ASSERT_TRUE_WAIT(observer_.state() == SctpTransportState::kClosed,
                   kDefaultTimeout);
  EXPECT_THAT(observer_.states_, ElementsAre(SctpTransportState::kConnecting,
                                             SctpTransportState::kConnected,
                                             SctpTransportState::kClosed));
}

TEST_F(SctpTransportTest, ClosignProcedureCloses) {
  CreateTransport();
  transport()->RegisterObserver(observer());
  AddDtlsTransport();
  CompleteSctpHandshake();
  CricketSctpTransport()->SendSignalClosingProcedureComplete();
  ASSERT_TRUE_WAIT(observer_.state() == SctpTransportState::kClosed,
                   kDefaultTimeout);
  EXPECT_THAT(observer_.states_, ElementsAre(SctpTransportState::kConnecting,
                                             SctpTransportState::kConnected,
