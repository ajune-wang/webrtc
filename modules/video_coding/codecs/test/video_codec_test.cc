...
ABSL_FLAG(std::string, source_video, "FourPeople_1280x720_30", "Source video.");
...
ABSL_FLAG(std::string, csv_file_name, "", "CSV file name.");
...
std::string TestParamsToString(std::string video_name,
  ...
  name += sep + codec_type;
  name += sep + codec_impl;
  name += sep + scalability_mode;
  name += sep + std::to_string(width) + "x" + std::to_string(height);
  name += sep + StrJoin(rounded_framerate_fps, "_") + "fps";
  name += sep + StrJoin(bitrate_kbps, "_") + "kbps";
  return name;
}
...
std::unique_ptr<VideoCodecStats> RunEncodeDecodeTest(
    ...
    absl::optional<std::string> output_path = absl::nullopt) {
  ...
  std::string output_path_str = output_path.value_or(TestOutputPath());

  VideoCodecTester::EncoderSettings encoder_settings;
  if (absl::GetFlag(FLAGS_dump_encoder_input)) {
    encoder_settings.encoder_input_base_path = output_path_str + "_enc_input";
  }
  if (absl::GetFlag(FLAGS_dump_encoder_output)) {
    encoder_settings.encoder_output_base_path = output_path_str + "_enc_output";
  }

  VideoCodecTester::DecoderSettings decoder_settings;
  if (absl::GetFlag(FLAGS_dump_decoder_input)) {
    decoder_settings.decoder_input_base_path = output_path_str + "_dec_input";
  }
  if (absl::GetFlag(FLAGS_dump_decoder_output)) {
    decoder_settings.decoder_output_base_path = output_path_str + "_dec_output";
  }
...
std::tuple<std::vector<int>, ScalabilityMode>
  ...
  if (bitrate_kbps.size() == 1 &&
    ...
    VideoBitrateAllocation bitrate_allocation =
        bitrate_allocator->Allocate(VideoBitrateAllocationParameters(
            1000 * bitrate_kbps[0], framerate_fps));

    std::vector<int> layer_bitrate_kbps;
    for (int sidx = 0; sidx < num_spatial_layers; ++sidx) {
      for (int tidx = 0; tidx < num_temporal_layers; ++tidx) {
        layer_bitrate_kbps.push_back(bitrate_allocation.GetBitrate(sidx, tidx) /
                                     1000);
      }
    }

    return std::make_tuple(layer_bitrate_kbps, *vc.GetScalabilityMode());
  }

  RTC_CHECK(static_cast<int>(bitrate_kbps.size()) ==
            num_spatial_layers * num_temporal_layers)
      << "When configured explicitly, the bitrates must be provided for all "
         "spatial and temporal layers.";

  return std::make_tuple(bitrate_kbps, scalability_mode);
}
      ...
      layer_settings.emplace(
          ...
          LayerSettings{
              ...
              .bitrate = DataRate::KilobitsPerSec(
                  layer_bitrate_kbps[sidx * num_temporal_layers + tidx])});
    }
  }
...
std::unique_ptr<VideoCodecStats> TestEncodeDecode(
    RawVideoInfo video_info,
    std::string codec_type,
    std::string codec_impl,
    std::string scalability_mode,
    int width,
  ...
  std::unique_ptr<VideoCodecStats> stats = RunEncodeDecodeTest(
      codec_type, codec_impl, video_info, frame_settings, output_path);
  ...
  for (int sidx = 0; sidx < num_spatial_layers; ++sidx) {
    for (int tidx = 0; tidx < num_temporal_layers; ++tidx) {
      VideoCodecStats::Stream stream;
      if (stats != nullptr) {
        stream = stats->Aggregate({.spatial_idx = sidx, .temporal_idx = tidx});
      }

      // TODO: All metrics will be prefixed. This is Ok for reporting and dash
      // boards. But this is not convenient for analysis.
      std::string prefix =
...
TEST(VideoCodecTest, EncodeDecode) {
  ...
  output_path += TestParamsToString(
      absl::GetFlag(FLAGS_source_video), absl::GetFlag(FLAGS_codec_type),
      absl::GetFlag(FLAGS_codec_impl), absl::GetFlag(FLAGS_scalability_mode),
      absl::GetFlag(FLAGS_width), absl::GetFlag(FLAGS_height),
      {absl::GetFlag(FLAGS_framerate_fps)}, bitrate_kbps, /*sep=*/"-");

  std::unique_ptr<VideoCodecStats> stats = TestEncodeDecode(
      kSourceVideos.at(absl::GetFlag(FLAGS_source_video)),
      ...
      bitrate_kbps, absl::GetFlag(FLAGS_num_frames), output_path);

  std::string csv_file_name = absl::GetFlag(FLAGS_csv_file_name);
  if (!csv_file_name.empty()) {
    std::string csv_file_path = TestOutputPath() + csv_file_name;
    stats->LogMetrics(
        csv_file_path, stats->Slice(),
        /*metadata=*/
        {{"codec_type", absl::GetFlag(FLAGS_codec_type)},
         {"codec_impl", absl::GetFlag(FLAGS_codec_impl)},
         {"video_name", kFourPeople_1280x720_30.name},
 ...
 public:
  static std::string ToString(
    ...
    auto [width, height, framerate_fps, scalability_mode, bitrate_kbps] =
        coding_settings;
    return TestParamsToString(video_info.name, codec_type, codec_impl,
...
TEST_P(SpatialQualityTest, SpatialQuality) {
  ...
  TestEncodeDecode(video_info, codec_type, codec_impl, scalability_mode, width,
                   height, framerate_fps, {bitrate_kbps}, num_frames);
}
