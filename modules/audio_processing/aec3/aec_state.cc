...
void AecState::Update(
  ...
  bool any_filter_consistent = filter_analyzer_.AnyFilterConsistent();
  float max_echo_path_gain = filter_analyzer_.MaxFilterGain();
...
void AecState::FilterDelay::Update(
  ...
  if (delay_estimator_may_not_have_converged && external_delay_) {
    ...
    const auto& delays = filter_analyzer.FilterDelaysBlocks();
    RTC_DCHECK_EQ(delays.size(), delays.size());
    std::copy(delays.begin(), delays.end(), filter_delays_blocks_.begin());
  }
}
