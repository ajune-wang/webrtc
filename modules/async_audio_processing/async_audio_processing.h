...
class AsyncAudioProcessing final {
 ...
 private:
  ...
  // TODO(bugs.webrtc.org/15111):
  //   Remove `frame_processor_` in favour of `owned_frame_processor_` in the
  //   follow-up.
  //   While transitioning this API from AudioFrameProcessor& to
  //   std::unique_ptr<AudioFrameProcessor>, we have both `frame_processor_` and
  //   `owned_frame_processor_`. Only one of them is used, depending on which
