...
#define RUN_ON_VOIP_THREAD_RETURN_TRUE(method, ...)                  \
  if (!voip_thread_->IsCurrent()) {                                  \
    voip_thread_->PostTask(                                          \
        RTC_FROM_HERE,                                               \
        std::bind(&AndroidVoipClient::method, this, ##__VA_ARGS__)); \
...
void AndroidVoipClient::SetEncoder(
  ...
  voip_thread_->PostTask(RTC_FROM_HERE, [=] { SetEncoder(chosen_encoder); });
}
...
void AndroidVoipClient::SetDecoders(
  ...
  voip_thread_->PostTask(RTC_FROM_HERE, [=] { SetDecoders(chosen_decoders); });
}
...
void AndroidVoipClient::SetLocalAddress(
  ...
  voip_thread_->PostTask(
      RTC_FROM_HERE, [=] { SetLocalAddress(ip_address, j_port_number_int); });
}
...
void AndroidVoipClient::SetRemoteAddress(
  ...
  const std::string& ip_address =
      webrtc::JavaToNativeString(env, j_ip_address_string);
  voip_thread_->PostTask(
      RTC_FROM_HERE, [=] { SetRemoteAddress(ip_address, j_port_number_int); });
}
