    ...
    if (encoder_bitrate_limits) {
      if (streams.size() == 1 && encoder_config_.simulcast_layers.size() == 1) {
        ...
        if (encoder_config_.simulcast_layers[0].max_bitrate_bps > 0) {
          api_max_bitrate = encoder_config_.simulcast_layers[0].max_bitrate_bps;
        }
        if (encoder_config_.max_bitrate_bps > 0) {
          api_max_bitrate =
              api_max_bitrate.has_value()
                  ? std::min(encoder_config_.max_bitrate_bps, *api_max_bitrate)
        ...
        if (min_bitrate_bps < max_bitrate_bps) {
          ...
          RTC_LOG(LS_WARNING)
              ...
              << ", max=" << api_max_bitrate.value_or(-1)
