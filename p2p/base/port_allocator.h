...
class RTC_EXPORT PortAllocatorSession : public sigslot::has_slots<> {
 public:
  ...
  virtual void GetCandidateStatsFromReadyPorts(
      CandidateStatsList* candidate_stats_list) const = 0;
...
class RTC_EXPORT PortAllocator : public sigslot::has_slots<> {
 public:
  ...
  // Clears the address and the related address fields to avoid IP leakage. This
  // is applicable in several scenarios:
  ...
  // 4. mDNS candidates are received.
  // 5. Peer-reflexive candidates.
  Candidate SanitizeCandidate(const Candidate& c, bool is_local) const;
