...
ABSL_FLAG(std::string,
          yuv_input_file,
          "",
          "Specify yuv input file of YuvFileGenerator");
...
class BitstreamProcessor final : public EncodedImageCallback,
 ...
 public:
  ...
  double ValidatePNSR(uint32_t frame_index, webrtc::VideoFrame& frame) {
    ...
    RTC_LOG(LS_VERBOSE) << __func__ << " frame_index: " << frame_index
                        << ", psnr: " << psnr;
    if (psnr_.size() == maximum_number_of_frames_) {
      double average_psnr = 0;
      for (const auto& psnr : psnr_) {
        average_psnr += psnr.second;
      }
      average_psnr /= psnr_.size();

      const size_t average_frame_size_in_bits =
          total_encoded_frames_size_ * 8 / maximum_number_of_frames_;
 ...
 private:
  ...
  bool validate_psnr_ = false;
  uint32_t maximum_number_of_frames_ = 0;
  size_t total_encoded_frames_size_ = 0;
  uint32_t frame_rate_fps_ = 0;
...
int main(int argc, char* argv[]) {
  absl::SetProgramUsageMessage(
      ...
      "--bitrate_kbps=500\n"
      "\n"
      "./video_encoder --yuv_input_file=input.yuv --video_codec=av1 "
  ...
  const std::string yuv_input_file = absl::GetFlag(FLAGS_yuv_input_file);
  ...
  if (!yuv_input_file.empty()) {
    // Use `YuvFileGenerator` if specify `--yuv_input_file`.
    frame_buffer_generator = webrtc::test::CreateFromYuvFileFrameGenerator(
        {yuv_input_file}, width, height, maximum_number_of_frames);

    RTC_LOG(LS_INFO) << "Create YuvFileGenerator: " << width << "x" << height;
  } else if (!y4m_input_file.empty()) {
  ...
  std::unique_ptr<webrtc::BitstreamProcessor> bitstream_processor =
      std::make_unique<webrtc::BitstreamProcessor>(
          env, video_codec_setting, validate_psnr, maximum_number_of_frames,
          frame_rate_fps);
  ...
  for (uint32_t i = 0; i < maximum_number_of_frames; ++i) {
    ...
    if (validate_psnr) {
      bitstream_processor->ValidatePNSR(i, frame);
    }
    RTC_CHECK_EQ(ret, WEBRTC_VIDEO_CODEC_OK);
