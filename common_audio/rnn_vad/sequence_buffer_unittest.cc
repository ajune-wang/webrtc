...
using rnn_vad::kFrameSize20ms48kHz;
...
// TODO(alessiob): No need to include this test.
TEST(RnnVadTest, SequenceBufferBitExactness) {
  ...
  rtc::ArrayView<float, kFrameSize10ms48kHz> samples_view(samples.data(),
                                                          kFrameSize10ms48kHz);
  // Expected first and last values for the sequence buffer at each step.
  auto expected_result_reader = CreateAnalysisBufFirstLastReader();
  ASSERT_EQ(num_frames, expected_result_reader.second);
  float expected_first_last[2];
  // Init sequence buffer.
  SequenceBuffer<float, kFrameSize20ms48kHz, kFrameSize10ms48kHz> seq_buf;
  auto seq_buf_view = seq_buf.GetBufferView();
  // Test sequence buffer.
  for (size_t i = 0; i < num_frames; ++i) {
    SCOPED_TRACE(i);
    expected_result_reader.first->ReadChunk({expected_first_last, 2});
    samples_reader.first->ReadChunk(samples_view);
    seq_buf.Push(samples_view);
    EXPECT_EQ(expected_first_last[0], seq_buf_view[0]);
    EXPECT_EQ(expected_first_last[1], seq_buf_view[seq_buf_view.size() - 1]);
  }
}
