...
class VCMDecoderDataBase {
 ...
 private:
  ...
  // Initialization paramaters for decoder keyed by payload type.
  std::map<uint8_t, DecoderSettings> decoder_configuration_;
  // Excternal decoders keyed by payload type.
  std::map<uint8_t, VideoDecoder*> external_decoder_;
};
