...
void DataChannelController::OnReadyToSend() {
  ...
  // TODO(tommi): When running
  // ./peerconnection_unittests --gtest_filter=*CalleeClosesSctpDataChannel*
  // --gtest_repeat=100 --gtest_break_on_failure
  // We hit a problem whereby OnReadyToSend() is called before AllocateSctpSids
  // is called.
  // This loop is a workaround to see if things pass if this is done.
  // We need to figure out where the order of operations is incorrectly queued.

  for (const auto& c : sctp_data_channels_n_) {
    if (!c->sid_n().HasValue()) {
      auto role = pc_->GetSctpSslRole_n();
      RTC_DCHECK(role);
      absl::string_view name = (*role) == rtc::SSL_CLIENT ? "client" : "server";
      RTC_LOG(LS_ERROR) << "***** Postponing OnReadyToSend. Count="
                        << sctp_data_channels_n_.size() << " role=" << name;
      network_thread()->PostTask([this]() { OnReadyToSend(); });
      return;
    }
  }

  RTC_LOG(LS_ERROR) << "***** OnReadyToSend good to go Count="
                    << sctp_data_channels_n_.size();
...
void DataChannelController::OnDataChannelOpenMessage(
    ...
    bool ready_to_send) {
  RTC_LOG(LS_ERROR) << "******* OnDataChannelOpenMessage";
...
DataChannelController::InternalCreateDataChannelWithProxy(
  ...
  RTC_LOG(LS_ERROR) << "******* InternalCreateDataChannelWithProxy";
  ...
  auto ret = network_thread()->BlockingCall(
      [&]() -> RTCErrorOr<rtc::scoped_refptr<SctpDataChannel>> {
        ...
        RTC_LOG(LS_ERROR) << "******* InternalCreateDataChannelWithProxy state="
                          << channel.value()->state();
...
void DataChannelController::AllocateSctpSids(rtc::SSLRole role) {
  ...
  signaling_thread()->PostTask(SafeTask(
      signaling_safety_.flag(),
      [this, channels_to_update = std::move(channels_to_update)] {
        RTC_DCHECK_RUN_ON(signaling_thread());
...
void DataChannelController::OnSctpDataChannelClosed(SctpDataChannel* channel) {
  ...
  signaling_thread()->PostTask(
      SafeTask(signaling_safety_.flag(), [this, channel] {
        RTC_DCHECK_RUN_ON(signaling_thread());
        auto it = absl::c_find_if(sctp_data_channels_, [&](const auto& c) {
