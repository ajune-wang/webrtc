...
void VideoQualityMetricsReporter::OnStatsReports(
  ...
  for (auto& s : outbound_rtp_stats) {
    ...
    if (s->timestamp_us() > sample.sample_time_us) {
      sample.sample_time_us = s->timestamp_us();
    }
  ...
  if (prev_sample.sample_time_us == 0) {
  ...
  if (time_between_samples_us == 0) {
    return;
  }
  double time_between_samples_sec =
      absl::ToDoubleSeconds(absl::Microseconds(time_between_samples_us));
