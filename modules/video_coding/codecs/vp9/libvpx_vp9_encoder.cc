...
void LibvpxVp9Encoder::SetActiveSpatialLayers() {
  ...
  for (const auto& layer : layer_frames_) {
    // This function assumes layer_frames_ are ordered by spatial id.
    RTC_DCHECK_LE(sid, layer.SpatialId());
    for (; sid < layer.SpatialId(); ++sid) {
      DisableSpatialLayer(sid);
    }
    EnableSpatialLayer(layer.SpatialId());
    ++sid;
  }
  for (; sid < num_spatial_layers_; ++sid) {
    DisableSpatialLayer(sid);
  }
}
...
int LibvpxVp9Encoder::InitEncode(const VideoCodec* inst,
  ...
  if (external_ref_control_ || svc_controller_) {
...
bool LibvpxVp9Encoder::PopulateCodecSpecific(CodecSpecificInfo* codec_specific,
  ...
  if (vp9_info->flexible_mode) {
    ...
    if (vp9_info->num_ref_pics > 0) {
      RTC_DCHECK_EQ(vp9_info->num_ref_pics,
                    gof_.num_ref_pics[vp9_info->gof_idx])
          << vp9_info->gof_idx;
    }
  }
