...
class ClippingEventPredictor : public ClippingPredictor {
 public:
  ...
  ClippingEventPredictor(int num_channels,
    ...
    for (auto& buffer : ch_buffers_) {
      buffer = std::make_unique<ClippingPredictorLevelBuffer>(buffer_length);
    }
    RTC_DCHECK(!ch_buffers_.empty());
  }
  ...
  // Estimates the analog gain adjustment using a sliding window over the
  // frame-wise metrics in `ch_buffers_`. Returns an estimate for the clipped
  // level step equal to `default_clipped_level_step_` if at least
  // `GetMinFramesProcessed()` frames have been processed since the last reset
  // and a clipping event is predicted. `level`, `min_mic_level`, and
  // `max_mic_level` are limited to [0, 255] and `default_step` to [1, 255].
  absl::optional<int> EstimateClippedLevelStep(int level,
                                               int default_step,
                                               int min_mic_level,
 ...
 private:
  ...
  bool PredictClippingEvent() const {
    for (const auto& buffer : ch_buffers_) {
      const auto metrics = buffer->ComputePartialMetrics(0, window_length_);
      if (!metrics.has_value() ||
          !(FloatS16ToDbfs(metrics.value().max) > clipping_threshold_)) {
        continue;
      }
      const auto reference_metrics = buffer->ComputePartialMetrics(
      ...
      if (reference_metrics.has_value()) {
        ...
        const bool crest_factor_drops =
            crest_factor < reference_crest_factor - crest_factor_margin_;
        if (crest_factor_drops) {
          return true;
        }
      }
    }
...
class ClippingPeakPredictor : public ClippingPredictor {
 public:
  ...
  // Estimates the analog gain adjustment using a sliding window over the
  // frame-wise metrics in `ch_buffers_`. Returns an estimate for the clipped
  // level step (equal to `default_clipped_level_step_` if
  // `adaptive_estimation_` is false) if at least `GetMinFramesProcessed()`
  // frames have been processed since the last reset and a clipping event is
  // predicted. `level`, `min_mic_level`, and `max_mic_level` are limited to [0,
  // 255] and `default_step` to [1, 255].
  absl::optional<int> EstimateClippedLevelStep(int level,
                                               int default_step,
                                               int min_mic_level,
                                               int max_mic_level) const {
 ...
 private:
  ...
  absl::optional<float> EstimatePeakValue() const {
    absl::optional<float> max_value = absl::nullopt;
    for (const auto& buffer : ch_buffers_) {
      const auto reference_metrics = buffer->ComputePartialMetrics(
          reference_window_delay_, reference_window_length_);
      if (!reference_metrics.has_value()) {
      ...
      max_value = (max_value.has_value())
                      ? std::fmax(max_value.value(), projected_peak)
                      : projected_peak;
    }
    return max_value;
  }
...
std::unique_ptr<ClippingPredictor> CreateClippingEventPredictor(
  ...
  return std::make_unique<ClippingEventPredictor>(
      num_channels, window_length, reference_window_length,
      reference_window_delay, clipping_threshold, crest_factor_margin);
}
...
std::unique_ptr<ClippingPredictor> CreateFixedStepClippingPeakPredictor(
  ...
  return std::make_unique<ClippingPeakPredictor>(
      num_channels, window_length, reference_window_length,
      reference_window_delay, clipping_threshold,
