...
void VideoProcessor::FrameEncoded(webrtc::VideoCodecType codec,
  ...
  if (frame_number > 0) {
    ...
    if (num_dropped_from_last_encode > 0) {
      // For dropped frames, we write out the last decoded frame to make
      // it look like a freeze at playback.
...
void VideoProcessor::FrameDecoded(const VideoFrame& decoded_frame) {
  ...
  if (frame_number > 0) {
    ...
    if (decoded_frame_writer_ && last_decoded_frame_num_ >= 0) {
      // For dropped frames, write out the last decoded frame to make it
