...
class RtpReceiverInternal : public RtpReceiverInterface {
 public:
  ...
  // Call this method on the worker thread to avoid mixing up state between
  // the signaling and worker threads.
  virtual void Stop_w() = 0;

  // Call on the signaling thread, to let the receiver know that the the
  // embedded source object should enter a stopped/ended state.
  // When `end_track` is set to true, the track state will be set to `kEnded`
  // which is a final state (cannot be reversed).
  // NOTE: Calling this function should be followed with a call to `Stop()`
  // on the worker thread, to complete the operation.
  virtual void StopSource(bool end_track) = 0;
