...
bool BasicPortAllocatorSession::CandidatePairable(const Candidate& c,
  ...
  bool candidate_from_any_address_network = c.address().IsAnyIP();
  ...
  return candidate_signalable ||
         ((network_enumeration_disabled ||
           candidate_from_any_address_network) &&
...
void BasicPortAllocatorSession::MaybeSignalCandidatesAllocationDone() {
  if (CandidatesAllocationDone()) {
    MaybeSignalCandidatesReadyFromAnyAddressNetworks();
    if (pooled()) {
...
void BasicPortAllocatorSession::MaybeSignalCandidateReady(const Candidate& c) {
  if (c.network_name() == "any" &&
      !(flags() & PORTALLOCATOR_DISABLE_ADAPTER_ENUMERATION)) {
    candidates_on_default_route_.push_back(SanitizeRelatedAddress(c));
...
void BasicPortAllocatorSession::
    MaybeSignalCandidatesReadyFromAnyAddressNetworks() {
  std::vector<Candidate> candidates_on_default_route;
  std::vector<PortData*> ports_on_default_route;
  bool has_ready_ports_on_non_default_route = false;
  for (PortData& port_data : ports_) {
    if (port_data.port()->Network()->name() == "any") {
      ports_on_default_route.push_back(&port_data);
    } else {
      has_ready_ports_on_non_default_route |= port_data.ready();
    }
  }
  if ((flags() & PORTALLOCATOR_DISABLE_ANY_ADDRESS_PORTS) ||
      (has_ready_ports_on_non_default_route &&
       !(flags() & PORTALLOCATOR_DISABLE_ADAPTER_ENUMERATION) &&
       !(flags() & PORTALLOCATOR_DISABLE_PRUNE_ANY_ADDRESS_PORTS))) {
    PrunePortsAndRemoveCandidates(ports_on_default_route);
    return;
  } else {
    RTC_LOG(INFO) << "Signal candidates from the default route.";
    SignalCandidatesReady(this, candidates_on_default_route_);
  }
}
