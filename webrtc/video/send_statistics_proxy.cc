...
SendStatisticsProxy::SendStatisticsProxy(
    ...
    : clock_(clock),
      ...
      cpu_downscales_(-1),
      media_byte_rate_tracker_(100, 10u),
      encoded_frame_rate_tracker_(100, 10u),
      uma_container_(
...
void SendStatisticsProxy::UmaSamplesContainer::RemoveOld(
    int64_t now_ms,
    EncodedFrameMap* encoded_frames) {
  while (!encoded_frames->empty()) {
    auto it = encoded_frames->begin();
...
bool SendStatisticsProxy::UmaSamplesContainer::InsertEncodedFrame(
  ...
  RemoveOld(now_ms, &encoded_frames_);
