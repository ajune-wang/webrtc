...
void FakeVideoSendStream::UpdateActiveSpatialLayers(
    const rtc::ArrayView<const webrtc::VideoStream> layers) {
  sending_ = false;
  for (const webrtc::VideoStream& simulcast_layer : layers) {
    if (simulcast_layer.active) {
      sending_ = true;
      break;
    }
  }
