...
VideoEncoderFactory::CodecSupport InternalEncoderFactory::QueryCodecSupport(
  ...
  if (scalability_mode) {
    ...
    if (codec != kVideoCodecVP8 && codec != kVideoCodecVP9 &&
        codec != kVideoCodecAV1) {
      // Scalability mode is unsupported for this codec.
      return kUnsupported;
    }
    if (codec == kVideoCodecVP8 && *spatial_layers > 1) {
      // VP8 doesn't support more than one spatial layers.
      return kUnsupported;
    }
  }
