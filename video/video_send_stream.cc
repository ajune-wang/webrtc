 ...
 private:
  bool Run() override {
    send_stream_->reset(new VideoSendStreamImpl(
        ...
        std::move(protection_bitrate_calculator_)));
    return true;
  }
...
VideoSendStreamImpl::VideoSendStreamImpl(
  ...
  protection_bitrate_calculator_->SetProtectionCallback(this);
...
uint32_t VideoSendStreamImpl::OnBitrateUpdated(uint32_t bitrate_bps,
  ...
  // protection overhead.
  if (!protection_bitrate_calculator_->UseLossMaskVector()) {
    ...
    encoder_target_rate_bps_ = protection_bitrate_calculator_->SetTargetRates(
        payload_bitrate_bps, stats_proxy_->GetSendFrameRate(),
...
void VideoSendStreamImpl::OnPacketAdded(uint32_t ssrc, uint16_t seq_num) {
  if (!worker_queue_->IsCurrent()) {
    worker_queue_->PostTask(
        [&, ssrc, seq_num] { weak_ptr_->OnPacketAdded(ssrc, seq_num); });
    return;
  }
...
void VideoSendStreamImpl::OnPacketFeedbackVector(
  ...
  if (!worker_queue_->IsCurrent()) {
    worker_queue_->PostTask([&, packet_feedback_vector] {
