...
TEST_F(RtpVideoStreamReceiverTest, TransformFrame) {
  ...
  EXPECT_CALL(*mock_frame_transformer, RegisterTransformedFrameCallback(_));
  std::unique_ptr<RtpVideoStreamReceiver> receiver =
      std::make_unique<RtpVideoStreamReceiver>(
          Clock::GetRealTimeClock(), &mock_transport_, nullptr, nullptr,
          &config_, rtp_receive_statistics_.get(), nullptr,
          process_thread_.get(), &mock_nack_sender_, nullptr,
