...
// Get the program counter of the call site.
uint32_t GetAddress() {
...
bool StackTraceContainsRange(const std::vector<StackTraceElement>& stack_trace,
  ...
  for (const StackTraceElement& stack_trace_element : stack_trace) {
    if (pc_low < stack_trace_element.program_counter &&
        stack_trace_element.program_counter < pc_high) {
      return true;
    }
  }
  return false;
...
class RtcCriticalSectionDeadlock : public DeadlockInterface {
 public:
  RtcCriticalSectionDeadlock() { critscope.reset(new rtc::CritScope(&crit)); }

 private:
  void Deadlock() override { rtc::CritScope lock(&crit); }

  void Release() override { critscope.reset(); }

  rtc::CriticalSection crit;
  std::unique_ptr<rtc::CritScope> critscope;
};
...
class SpinDeadlock : public DeadlockInterface {
 private:
  ...
  void Release() override { is_deadlocked = false; }

  volatile bool is_deadlocked = true;
};
...
void ThreadFunction(void* void_params) {
  ...
  params->deadlock_region_start_address = GetAddress();
  params->deadlock_start_event.Set();
  params->deadlock_impl->Deadlock();
  params->deadlock_region_end_address = GetAddress();
...
void TestStacktrace(std::unique_ptr<DeadlockInterface> deadlock_impl) {
  ...
  // Assert that the stack trace should contain the deadlock region.
