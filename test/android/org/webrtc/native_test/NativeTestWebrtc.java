...
// Based on
// third_party/angle/src/tests/test_utils/runner/android/java/
// (cont) src/com/android/angle/test/AngleNativeTest.java
// and testing/android/native_test/java/src/org/chromium/native_test/NativeTest.java
...
public class NativeTestWebrtc {
    ...
    private void runTests(Activity activity) {
        NativeTestWebrtcJni.get()
                .runTests(
                        mCommandLineFlags.toString(),
                        mCommandLineFilePath,
                        mStdoutFilePath,
                        activity.getApplicationContext(),
                        NativeTestWebrtc.getIsolatedTestRoot());
        activity.finish();
        mReporter.testRunFinished(Process.myPid());
    }


    // @CalledByNative TODO(DO_NOT_SUBMIT): Needed?
