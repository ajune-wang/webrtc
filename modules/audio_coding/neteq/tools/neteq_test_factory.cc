...
std::unique_ptr<NetEqTest> NetEqTestFactory::InitializeTest(
  ...
  if (config.matlabplot || config.pythonplot) {
    RTC_CHECK(config.plot_scripts_basename.has_value());
    ...
    // |stats_plotter_| must always be instanced.
    stats_plotter_ = absl::make_unique<NetEqStatsPlotter>(
        /*make_matlab_plot=*/false,
        /*make_python_plot=*/false,
        /*show_concealment_events=*/config.concealment_events,
        /*base_file_name=*/"");
  }
