...
void Scenario::ChangeRoute(std::pair<CallClient*, CallClient*> clients,
  ...
  NetworkNodeHelper::Route(route_id, over_nodes, clients.second);
...
BuiltInEmulatedNode* Scenario::CreateBuiltInEmulatedNode(
    std::function<void(NetworkNodeConfig*)> config_modifier) {
  NetworkNodeConfig config;
  config_modifier(&config);
  return CreateBuiltInEmulatedNode(config);
}

BuiltInEmulatedNode* Scenario::CreateBuiltInEmulatedNode(
    NetworkNodeConfig config) {
  RTC_DCHECK(config.mode == NetworkNodeConfig::TrafficMode::kSimulation);
  auto network_node = BuiltInEmulatedNode::Create(config);
  BuiltInEmulatedNode* sim_node = network_node.get();
...
NetworkNode* Scenario::CreateNetworkNode(
  ...
  // TODO(titovartem) propagate overhead from config to the node.
...
void Scenario::TriggerPacketBurst(std::vector<NetworkNode*> over_nodes,
                                  ...
                                  size_t packet_size) {
  uint64_t route_id = next_route_id_++;
  NetworkNodeHelper::Route(route_id, over_nodes, &null_receiver_);
...
void Scenario::NetworkDelayedAction(std::vector<NetworkNode*> over_nodes,
  ...
  NetworkNodeHelper::Route(route_id, over_nodes,
