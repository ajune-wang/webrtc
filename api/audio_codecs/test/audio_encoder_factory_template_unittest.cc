...
TEST(AudioEncoderFactoryTemplateTest, OneEncoderType) {
  ...
  EXPECT_THAT(factory->Create(env, {"bar", 16000, 1}, {.payload_type = 17}),
              IsNull());
  EXPECT_THAT(factory->Create(env, {"bogus", 8000, 1}, {.payload_type = 17}),
...
TEST(AudioEncoderFactoryTemplateTest, TwoEncoderTypes) {
  ...
  EXPECT_THAT(factory->Create(env, {"bogus", 8000, 1}, {.payload_type = 17}),
              Pointer(Property(&AudioEncoder::SampleRateHz, 8000)));
  EXPECT_THAT(factory->Create(env, {"sham", 16000, 2}, {.payload_type = 17}),
              IsNull());
  EXPECT_THAT(factory->Create(env, {"sham", 16000, 2, {{"param", "value"}}},
                              {.payload_type = 17}),
...
TEST(AudioEncoderFactoryTemplateTest, G711) {
  ...
  EXPECT_THAT(factory->Create(env, {"PCMU", 16000, 1}, {.payload_type = 17}),
              IsNull());
  EXPECT_THAT(factory->Create(env, {"PCMU", 8000, 1}, {.payload_type = 17}),
              Pointer(Property(&AudioEncoder::SampleRateHz, 8000)));
  EXPECT_THAT(factory->Create(env, {"PCMA", 8000, 1}, {.payload_type = 17}),
              Pointer(Property(&AudioEncoder::SampleRateHz, 8000)));
}
...
TEST(AudioEncoderFactoryTemplateTest, G722) {
  ...
  EXPECT_THAT(factory->Create(env, {"bar", 16000, 1}, {.payload_type = 17}),
              IsNull());
  EXPECT_THAT(factory->Create(env, {"G722", 8000, 1}, {.payload_type = 17}),
              Pointer(Property(&AudioEncoder::SampleRateHz, 16000)));
}
...
TEST(AudioEncoderFactoryTemplateTest, Ilbc) {
  ...
  EXPECT_THAT(factory->Create(env, {"bar", 8000, 1}, {.payload_type = 17}),
              IsNull());
  EXPECT_THAT(factory->Create(env, {"ilbc", 8000, 1}, {.payload_type = 17}),
              Pointer(Property(&AudioEncoder::SampleRateHz, 8000)));
}
...
TEST(AudioEncoderFactoryTemplateTest, L16) {
  ...
  EXPECT_THAT(factory->Create(env, {"L16", 8000, 0}, {.payload_type = 17}),
              IsNull());
  EXPECT_THAT(factory->Create(env, {"L16", 48000, 2}, {.payload_type = 17}),
