...
<<<<<<< PATCH SET (638de6 Change flexfec header reader to parse according to updated R)
/*
...
>>>>>>> BASE      (cc8590 Initial copy of flexfec_header_reader_writer.)
...
void VerifyReadHeaders(
  ...
  for (size_t i = 0; i < protected_streams_num; ++i) {
    auto protected_stream = read_packet.protected_streams[i];
    EXPECT_EQ(protected_stream.seq_num_base,
              ByteReader<uint16_t>::ReadBigEndian(kSnBases[i]));
    EXPECT_EQ(protected_stream.packet_mask_offset, expected_mask_offsets[i]);
    EXPECT_EQ(protected_stream.packet_mask_size, expected_packet_mask_sizes[i]);
    // Ensure that the K-bits are removed and the packet mask has been packed.
    EXPECT_THAT(
        ::testing::make_tuple(read_packet.pkt->data.cdata() +
                                  protected_stream.packet_mask_offset.value(),
                              protected_stream.packet_mask_size.value()),
...
void VerifyWrittenAndReadHeaders(size_t expected_fec_header_size,
  ...
  EXPECT_THAT(
      make_tuple(read_packet.pkt->data.cdata() + kFlexfecPacketMaskOffset,
                 read_packet.protected_streams[0].packet_mask_size.value()),
      ElementsAreArray(expected_packet_mask, expected_packet_mask_size));
