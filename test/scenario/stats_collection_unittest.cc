...
TEST(ScenarioAnalyzerTest, PsnrIsHighWhenNetworkIsGood) {
  ...
  EXPECT_NEAR(stats.call.stats().target_rate.Mean().kbps(), 730, 100);
  EXPECT_NEAR(stats.video_send.stats().media_bitrate.Mean().kbps(), 330, 50);
  EXPECT_NEAR(stats.video_receive.stats().resolution.Mean(), 180, 10);
  EXPECT_NEAR(stats.audio_receive.stats().jitter_buffer.Mean().ms(), 48, 20);
}
...
TEST(ScenarioAnalyzerTest, PsnrIsLowWhenNetworkIsBad) {
  ...
  EXPECT_NEAR(stats.video_receive.stats().resolution.Mean(), 180, 10);
  EXPECT_NEAR(stats.audio_receive.stats().jitter_buffer.Mean().ms(), 35, 10);
}
