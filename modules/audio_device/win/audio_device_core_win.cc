...
bool AudioDeviceWindowsCore::CoreAudioIsSupported() {
  ...
  if (FAILED(hr)) {
    ...
    RTC_DCHECK(messageLength <= MAXERRORLENGTH);
...
AudioDeviceWindowsCore::AudioDeviceWindowsCore()
  ...
  RTC_DCHECK(NULL != _ptrEnumerator);
...
int32_t AudioDeviceWindowsCore::SetPlayoutDevice(uint16_t index) {
  ...
  RTC_DCHECK(_ptrRenderCollection != NULL);
...
int32_t AudioDeviceWindowsCore::SetPlayoutDevice(
  ...
  RTC_DCHECK(_ptrEnumerator != NULL);
...
int32_t AudioDeviceWindowsCore::SetRecordingDevice(uint16_t index) {
  ...
  RTC_DCHECK(_ptrCaptureCollection != NULL);
  ...
  while (keepRecording) {
    ...
    while (keepRecording) {
      ...
      if (bytesProduced > 0) {
        ...
        RTC_DCHECK(kSamplesProduced == static_cast<int>(_recBlockSize));
        RTC_DCHECK(sizeof(BYTE) == sizeof(int8_t));
  ...
  while (keepRecording) {
    ...
    while (keepRecording) {
      ...
      if (SUCCEEDED(hr)) {
        ...
        RTC_DCHECK(syncBufferSize >= (syncBufIndex * _recAudioFrameSize) +
                                         framesAvailable * _recAudioFrameSize);
...
int32_t AudioDeviceWindowsCore::_GetDeviceName(IMMDevice* pDevice,
  ...
  RTC_DCHECK(pszBuffer != NULL);
  RTC_DCHECK(bufferLen > 0);
