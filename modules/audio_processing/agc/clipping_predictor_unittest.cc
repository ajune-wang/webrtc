...
TEST_P(ClippingPredictorParameterization,
  ...
  if (reference_window_length() + reference_window_delay() > window_length()) {
    auto predictor = CreateClippingEventPredictor(
        num_channels(), window_length(), reference_window_length(),
        reference_window_delay(), /*clipping_threshold=*/-1.0f,
    ...
    EXPECT_EQ(predictor->EstimateClippedLevelStep(
                  /*level=*/255, kDefaultClippedLevelStep, kMinMicLevel,
                  kMaxMicLevel),
              absl::nullopt);
    ProcessZeroCrestFactorAudio(window_length(), num_channels(),
                                /*peak_ratio=*/0.99f, *predictor);
    EXPECT_THAT(predictor->EstimateClippedLevelStep(/*level=*/255,
                                                    kDefaultClippedLevelStep,
                                                    kMinMicLevel, kMaxMicLevel),
                Optional(Eq(kDefaultClippedLevelStep)));
  }
}
...
TEST_P(ClippingPredictorParameterization,
  ...
  if (reference_window_length() + reference_window_delay() > window_length()) {
    ...
    ProcessNonZeroCrestFactorAudio(
        reference_window_length() + reference_window_delay() - window_length(),
        num_channels(), /*peak_ratio=*/0.99f, *predictor);
    EXPECT_EQ(predictor->EstimateClippedLevelStep(/*level=*/255,
                                                  kDefaultClippedLevelStep,
                                                  kMinMicLevel, kMaxMicLevel),
              absl::nullopt);
...
TEST_P(ClippingPredictorParameterization,
  ...
  if (reference_window_length() + reference_window_delay() > window_length()) {
    auto predictor = CreateAdaptiveStepClippingPeakPredictor(
        num_channels(), window_length(), reference_window_length(),
        reference_window_delay(), /*clipping_threshold=*/-1.0f);
    ProcessNonZeroCrestFactorAudio(
        reference_window_length() + reference_window_delay() - window_length(),
...
TEST_P(ClippingEventPredictorParameterization,
  ...
  auto predictor = CreateClippingEventPredictor(
      kNumChannels, kWindowLength, kReferenceWindowLength,
      kReferenceWindowDelay, clipping_threshold(), crest_factor_margin());
  ProcessNonZeroCrestFactorAudio(kReferenceWindowLength, kNumChannels,
                                 /*peak_ratio=*/0.99f, *predictor);
  EXPECT_EQ(
      predictor->EstimateClippedLevelStep(
          /*level=*/255, kDefaultClippedLevelStep, kMinMicLevel, kMaxMicLevel),
      absl::nullopt);
  ...
  auto predictor =
      adaptive_step_estimation()
          ? CreateAdaptiveStepClippingPeakPredictor(
                kNumChannels, kWindowLength, kReferenceWindowLength,
                kReferenceWindowDelay, clipping_threshold())
...
TEST(ClippingEventPredictorTest, CheckEstimateAfterReset) {
  auto predictor = CreateClippingEventPredictor(
      kNumChannels, kWindowLength, kReferenceWindowLength,
      kReferenceWindowDelay, /*clipping_threshold=*/-1.0f,
...
TEST(ClippingPeakPredictorTest, CheckFixedStepEstimate) {
  auto predictor = CreateFixedStepClippingPeakPredictor(
      kNumChannels, kWindowLength, kReferenceWindowLength,
      kReferenceWindowDelay, /*clipping_threshold=*/-1.0f);
  ProcessNonZeroCrestFactorAudio(kReferenceWindowLength, kNumChannels,
                                 /*peak_ratio=*/0.99f, *predictor);
