...
bool IsInitialFramedropEnabled() {
...
VideoStreamEncoder::VideoStreamEncoder(
    ...
    : shutdown_event_(true /* manual_reset */, false),
      ...
      initial_framedrop_(InitialFramedrop::Ready),
...
void VideoStreamEncoder::SetSource(
  ...
  encoder_queue_.PostTask([this, degradation_preference] {
    ...
    initial_framedrop_ =
        allow_scaling ? InitialFramedrop::Ready : InitialFramedrop::Finished;
...
void VideoStreamEncoder::ConfigureQualityScaler() {
  ...
  if (quality_scaling_allowed) {
    ...
    initial_framedrop_ = InitialFramedrop::Finished;
  }
...
void VideoStreamEncoder::MaybeEncodeVideoFrame(const VideoFrame& video_frame,
  ...
  if (initial_framedrop_ >= InitialFramedrop::On)
    initial_framedrop_ = InitialFramedrop::Finished;
...
void VideoStreamEncoder::OnBitrateUpdated(uint32_t bitrate_bps,
  ...
  if (bitrate_bps != encoder_start_bitrate_bps_ &&
      initial_framedrop_ == InitialFramedrop::Ready &&
      IsInitialFramedropEnabled()) {
    // Turn on initial framedrop feature when first real BW estimate arrives.
    // TODO(kthelgason): Update BitrateAllocator to not call OnBitrateUpdated
    // without an actual BW estimate.
    initial_framedrop_ = InitialFramedrop::On;
  }
...
bool VideoStreamEncoder::DropDueToSize(uint32_t pixel_count) const {
  if (initial_framedrop_ != InitialFramedrop::Finished &&
      encoder_start_bitrate_bps_ > 0) {
