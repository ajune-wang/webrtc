...
int32_t VideoCaptureDS::Init(const char* deviceUniqueIdUTF8) {
  ...
  if (!_captureFilter) {
    NLOG(LS_INFO, "Failed to create capture filter.");
    return -1;
  }
  ...
  if (FAILED(hr)) {
    NLOG(LS_INFO, "Failed to create graph builder.");
    return -1;
  }

  hr = _graphBuilder->QueryInterface(IID_IMediaControl, (void**)&_mediaControl);
  if (FAILED(hr)) {
    NLOG(LS_INFO, "Failed to create media control builder.");
    return -1;
  }
  hr = _graphBuilder->AddFilter(_captureFilter, CAPTURE_FILTER_NAME);
  if (FAILED(hr)) {
    NLOG(LS_INFO, "Failed to add the capture device to the graph.");
    return -1;
  }
  ...
  if (hr != S_OK) {
    NLOG(LS_INFO, "Failed to create send filter");
    return -1;
  }
  _sinkFilter->AddRef();
  ...
  if (FAILED(hr)) {
    NLOG(LS_INFO, "Failed to add the send filter to the graph.");
    return -1;
  }
  ...
  NLOG(LS_INFO, "Capture device '", deviceUniqueIdUTF8, "' initialized.");
  return 0;
}
...
int32_t VideoCaptureDS::StartCapture(const VideoCaptureCapability& capability) {
  ...
  if (FAILED(hr)) {
    NLOG(LS_INFO, "Failed to start the Capture device.");
    return -1;
  }
  return 0;
}
...
int32_t VideoCaptureDS::StopCapture() {
  ...
  if (FAILED(hr)) {
    NLOG(LS_INFO, "Failed to stop the capture graph. ", hr);
    return -1;
  }
  return 0;
}
bool VideoCaptureDS::CaptureStarted() {
  ...
  if (hr != S_OK && hr != VFW_S_CANT_CUE) {
    NLOG(LS_INFO, "Failed to get the CaptureStarted status");
  }
  NLOG(LS_INFO, "CaptureStarted ", state);
...
int32_t VideoCaptureDS::SetCameraOutput(
  ...
  HRESULT hr = _outputCapturePin->QueryInterface(IID_IAMStreamConfig,
                                                 (void**)&streamConfig);
  if (hr) {
    NLOG(LS_INFO, "Can't get the Capture format settings.");
    return -1;
  }
  ...
  if (FAILED(hr)) {
    NLOG(LS_INFO, "Failed to set capture device output format");
    return -1;
  }
  ...
  if (hr != S_OK) {
    NLOG(LS_INFO, "Failed to connect the Capture graph ", hr);
    return -1;
  }
  return 0;
}
...
int32_t VideoCaptureDS::DisconnectGraph() {
  ...
  if (hr != S_OK) {
    NLOG(LS_ERROR, "Failed to Stop the Capture device for reconfiguration ",
         hr);
    return -1;
  }
  return 0;
...
HRESULT VideoCaptureDS::ConnectDVCamera() {
  ...
  if (!_dvFilter) {
    ...
    if (hr != S_OK) {
      NLOG(LS_INFO, "Failed to create the dv decoder: ", hr);
      return hr;
    }
    hr = _graphBuilder->AddFilter(_dvFilter, L"VideoDecoderDV");
    if (hr != S_OK) {
      NLOG(LS_INFO, "Failed to add the dv decoder to the graph: ", hr);
      return hr;
    }
    _inputDvPin = GetInputPin(_dvFilter);
    if (_inputDvPin == NULL) {
      NLOG(LS_INFO, "Failed to get input pin from DV decoder");
      return -1;
    }
    _outputDvPin = GetOutputPin(_dvFilter, GUID_NULL);
    if (_outputDvPin == NULL) {
      NLOG(LS_INFO, "Failed to get output pin from DV decoder");
      return -1;
    }
  }
  hr = _graphBuilder->ConnectDirect(_outputCapturePin, _inputDvPin, NULL);
  if (hr != S_OK) {
    NLOG(LS_INFO, "Failed to connect capture device to the dv devoder: ", hr);
    return hr;
  }
  ...
  if (hr != S_OK) {
    if (hr == HRESULT_FROM_WIN32(ERROR_TOO_MANY_OPEN_FILES)) {
      NLOG(LS_INFO, "Failed to connect the capture device, busy");
    } else {
      NLOG(LS_INFO, "Failed to connect capture device to the send graph: ", hr);
    }
    return hr;
  }
  return hr;
