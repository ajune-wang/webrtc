...
void DefaultVideoQualityAnalyzer::OnFramePreDecode(
  ...
  if (frame_id == VideoFrame::kNotSetId) {
    ...
    unknown_sender_frame_counters_[peer_index].received++;
    return;
  }
...
void DefaultVideoQualityAnalyzer::OnFrameDecoded(
  ...
  if (frame.id() == VideoFrame::kNotSetId) {
    ...
    unknown_sender_frame_counters_[peer_index].decoded++;
    return;
  }
...
void DefaultVideoQualityAnalyzer::OnFrameRendered(
  ...
  if (frame.id() == VideoFrame::kNotSetId) {
    ...
    unknown_sender_frame_counters_[peer_index].rendered++;
    return;
  }
...
DefaultVideoQualityAnalyzer::GetUnknownSenderFrameCounters() const {
  MutexLock lock(&mutex_);
  std::map<std::string, FrameCounters> out;
  for (auto& item : unknown_sender_frame_counters_) {
    out.emplace(peers_->name(item.first), item.second);
  }
  return out;
}
...
void DefaultVideoQualityAnalyzer::ReportResults() {
  ...
  if (!unknown_sender_frame_counters_.empty()) {
    ...
    for (const auto& [peer_index, frame_counters] :
         unknown_sender_frame_counters_) {
      LogFrameCounters(peers_->name(peer_index), frame_counters);
    }
  }
