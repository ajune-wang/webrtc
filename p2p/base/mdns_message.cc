...
bool MDnsHeader::Read(MDnsMessageBufferReader* buf) {
  bool all_success = buf->ReadUInt16(&id) && buf->ReadUInt16(&flags) &&
...
bool MDnsQuestion::Read(MDnsMessageBufferReader* buf) {
  if (!buf->ReadDomainName(&name_)) {
...
bool MDnsResourceRecord::Read(MDnsMessageBufferReader* buf) {
  if (!buf->ReadDomainName(&name_)) {
    return false;
  }
  if (!buf->ReadUInt16(&type_) || !buf->ReadUInt16(&class_) ||
...
bool MDnsResourceRecord::ReadARData(MDnsMessageBufferReader* buf) {
  // A RDATA contains a 32-bit IPv4 address.
  return buf->ReadString(&rdata_, 4);
}

bool MDnsResourceRecord::ReadQuadARData(MDnsMessageBufferReader* buf) {
...
bool MDnsMessage::Read(MDnsMessageBufferReader* buf) {
  if (!header_.Read(buf)) {
    return false;
  }
...
MDnsMessage::MessageInfo::MessageInfo() = default;
MDnsMessage::MessageInfo::MessageInfo(const MessageInfo& other)
    : message_start(other.message_start),
      message_size(other.message_size),
      pos_of_pointers(other.pos_of_pointers) {}
MDnsMessage::MessageInfo::~MessageInfo() = default;

MDnsMessageBufferReader::MDnsMessageBufferReader(
    const MDnsMessage::MessageInfo& message_info)
    : ByteBufferReader(message_info.message_start, message_info.message_size),
      message_info_(message_info) {}

MDnsMessageBufferReader::MDnsMessageBufferReader(
    const char* read_start_from,
    size_t len_to_read,
    const MDnsMessage::MessageInfo& message_info)
    : ByteBufferReader(read_start_from, len_to_read),
      message_info_(message_info) {}

MDnsMessageBufferReader::~MDnsMessageBufferReader() = default;

MDnsHeader::MDnsHeader() = default;

bool MDnsMessageBufferReader::ReadDomainName(std::string* name) {
  uint8_t label_length;
  if (!ReadUInt8(&label_length)) {
    return false;
  }
  ...
  while (label_length && !is_pointer(label_length)) {
    ...
    std::string label;
    if (!ReadString(&label, label_length)) {
      return false;
    }
    (*name) += label + ".";
    if (!ReadUInt8(&label_length)) {
  ...
  if (is_pointer(label_length)) {
    uint8_t next_octet;
    if (!ReadUInt8(&next_octet)) {
      return false;
    }
    size_t pos_jump_to = ((label_length & 0x3f) << 8) + next_octet;
    RTC_DCHECK(message_size() >= Length());
    // We consumed two octets for reading the pointer itself.
    size_t pos_pointer = message_size() - Length() - 2;
    if (PointerIsVisited(pos_pointer)) {
      return false;
    }
    // A legitimate pointer only refers to a prior occurrence of the same name,
    // and we should only move strictly backward in offset without reaching the
    // header section.
    if (pos_jump_to >= pos_pointer || pos_jump_to < kMDnsHeaderSizeBytes) {
      return false;
    }
    AddPointerVisited(pos_pointer);
    webrtc::MDnsMessageBufferReader new_buf(message_start(), message_size(),
                                            message_info_);
    if (!new_buf.Consume(pos_jump_to)) {
      return false;
    }
    return new_buf.ReadDomainName(name);
  }
  return true;
}

void MDnsMessageBufferReader::AddPointerVisited(size_t pos) {
  message_info_.pos_of_pointers.insert(pos);
}

bool MDnsMessageBufferReader::PointerIsVisited(size_t pos) const {
  return message_info_.pos_of_pointers.find(pos) !=
         message_info_.pos_of_pointers.end();
}
