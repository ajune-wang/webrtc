...
bool Remb::Parse(const CommonHeader& packet) {
  ...
  if (kUniqueIdentifier != ByteReader<uint32_t>::ReadBigEndian(&payload[8])) {
    NLOG(LS_WARNING, "REMB identifier not found, not a REMB packet.");
    return false;
  }
  uint8_t number_of_ssrcs = payload[12];
  if (packet.payload_size_bytes() !=
      kCommonFeedbackLength + (2 + number_of_ssrcs) * 4) {
    NLOG(LS_WARNING, "Payload size ", packet.payload_size_bytes(),
  ...
  if (shift_overflow) {
    NLOG(LS_ERROR, "Invalid remb bitrate value : ", mantissa, "*2^",
...
bool Remb::SetSsrcs(std::vector<uint32_t> ssrcs) {
  if (ssrcs.size() > kMaxNumberOfSsrcs) {
    NLOG(LS_WARNING, "Not enough space for all given SSRCs.");
    return false;
  }
