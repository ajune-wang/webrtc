...
bool D3dDevice::Initialize(const ComPtr<IDXGIAdapter>& adapter) {
  ...
  if (!dxgi_adapter_) {
    RTC_DLOG(LS_WARNING) << "An empty IDXGIAdapter instance has been received.";
    return false;
  }
  ...
  if (error.Error() != S_OK || !d3d_device_ || !context_) {
    RTC_DLOG(LS_WARNING) << "D3D11CreateDeivce returns error "
...
std::vector<D3dDevice> D3dDevice::EnumDevices() {
  ...
  if (error.Error() != S_OK || !factory) {
    RTC_DLOG(LS_WARNING) << "Cannot create IDXGIFactory1.";
