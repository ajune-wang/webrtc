...
TEST_F(UsrSctpReliabilityTest,
  ...
  constexpr int32_t wait_timeout = std::min<int64_t>(
      std::numeric_limits<int32_t>::max(),
      std::max<int64_t>(messages_count * 100, kDefaultTimeout));
...
TEST_F(UsrSctpReliabilityTest,
  ...
  auto thread1 = rtc::Thread::Create();
  auto thread2 = rtc::Thread::Create();
  thread1->SetName("Sctp-A", nullptr);
  thread2->SetName("Sctp-B", nullptr);
  thread1->Start();
  thread2->Start();
...
TEST_F(UsrSctpReliabilityTest,
  ...
  constexpr int32_t wait_timeout = std::min<int64_t>(
      std::numeric_limits<int32_t>::max(),
      std::max<uint64_t>(
          1ULL * messages_count * total_ping_pong_tests * 100 *
