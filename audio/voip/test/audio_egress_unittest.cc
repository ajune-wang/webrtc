...
// configure rtp stack in audio egress.
class AudioEgressTest : public ::testing::Test {
 public:
  const int16_t kAudioLevel = 3004;  // used for sine wave level
  const uint16_t kSeqNum = 12345;
  const uint64_t kStartTime = 123456789;
  const uint32_t kRemoteSsrc = 0xDEADBEEF;
  ...
  // increased per rtp spec. which is the number of samples it contains.
