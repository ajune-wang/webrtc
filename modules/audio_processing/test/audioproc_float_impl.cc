...
void PerformBasicParameterSanityChecks(const SimulationSettings& settings,
  ...
  ReportConditionalErrorAndExit(
      settings.aec_settings_filename && !builtin_builder_provided,
      "Error: The aec_settings_filename cannot be specified when a "
      "pre-constructed audio processing object is provided.\n");

  ReportConditionalErrorAndExit(
      settings.print_aec_parameter_values && !builtin_builder_provided,
      "Error: The print_aec_parameter_values cannot be set when a "
      "pre-constructed audio processing object is provided.\n");

  if (settings.linear_aec_output_filename && !builtin_builder_provided) {
...
int RunSimulation(
  ...
  PerformBasicParameterSanityChecks(settings, builtin_builder_provided);
...
int AudioprocFloatImpl(
    ...
    int argc,
    char* argv[]) {
  return RunSimulation(std::move(ap_builder), /*builtin_builder_provider=*/true,
                       argc, argv);
}
...
int AudioprocFloatImpl(
  ...
  return RunSimulation(std::move(ap_builder),
                       /*builtin_builder_provider=*/false, argc, argv);
}
