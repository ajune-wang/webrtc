...
//     MonoToStereo(data_copy, frame->samples_per_channel_,
//     frame->mutable_data());
//   }
...
int AudioFrameOperations::UpmixChannels(size_t target_number_of_channels,
                                        AudioFrame* frame) {
  RTC_DCHECK_NE(frame->num_channels_, 1);
  if (frame->num_channels_ != 1) {
    return -1;
  }
