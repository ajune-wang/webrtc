...
void RtpVideoStreamReceiver2::SetLastCorruptionDetectionIndex(
  ...
  if (absl::holds_alternative<FrameInstrumentationSyncData>(
          frame_instrumentation_data)) {
    last_corruption_detection_index_ =
        absl::get<FrameInstrumentationSyncData>(frame_instrumentation_data)
            .sequence_index;
  } else if (absl::holds_alternative<FrameInstrumentationData>(
                 frame_instrumentation_data)) {
    auto data = absl::get<FrameInstrumentationData>(frame_instrumentation_data);
    last_corruption_detection_index_ =
