...
void PacedSender::SetOutstandingData(int64_t outstanding_bytes) {
  rtc::CritScope cs(&critsect_);
  outstanding_bytes_ = outstanding_bytes;
}

void PacedSender::OnBytesSent(size_t bytes_sent) {
  // The number of outstanding bytes is tracked both locally and in the
  // TransportFeedbackAdapter. The local estimate is updated when packets are
  // sent, but overwritten by the TransportFeedbackAdapter estimate when
  // feedback is received. The reason for updating a local estimate is that it
  // takes some time to get a new update from the TransportFeedbackAdapter, and
  // we don't want to send a huge burst of data before the update reaches the
  // PacedSender. The TransportFeedbackAdapter also keeps track of which network
  // connection is being used, and correctly calculate the value after the
  // network route has changed.

  // TODO(srte): See if the congestion window tracking can be fully moved to
  // this class to avoid tracking double state.
  rtc::CritScope cs(&critsect_);
...
void PacedSender::Process() {
  ...
  if (packets_->Empty() && !Congested()) {
    ...
    if (packet_counter_ > 0) {
      ...
      if (padding_needed > 0) {
        auto sent_bytes = SendPadding(padding_needed, pacing_info);
        bytes_sent += sent_bytes;
        OnBytesSent(sent_bytes);
      }
    }
  }
  if (is_probing) {
