...
void PhysicalSocketServer::Add(Dispatcher* pdispatcher) {
  {
    CritScope cs(&crit_);
    // Defer adding the dispatcher until needed by the wait loop.
...
void PhysicalSocketServer::Remove(Dispatcher* pdispatcher) {
  {
    CritScope cs(&crit_);
    // Defer removing the dispatcher until accessed by the wait loop.
...
void PhysicalSocketServer::AddRemovePendingDispatchers() {
  ...
  dispatchers_.insert(pending_add_dispatchers_.begin(),
                      pending_add_dispatchers_.end());
...
// http://crbug.com/344505
  ...
  // Assume calls to Wait are synchronized, and thus it is safe to access
  // variables protected by the sequence_checker_.
