...
BitrateAllocation SvcRateAllocator::GetAllocation(uint32_t total_bitrate_bps,
  ...
  for (size_t sl_idx = 0; sl_idx < num_spatial_layers; ++sl_idx) {
    ...
    if (num_temporal_layers == 1) {
      ...
      // The high layer has two frames and the middle layer has one frame
      // within GOP (in between two consecutive low layer frames). Thus
      // high layer requires more bits (comparing pure bitrate of layer,
      // excluding bitrate of base layers) to keep quality on pair with base
      // layers.
