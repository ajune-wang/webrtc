...
void RtcEventLogEncoderNewFormat::EncodeRemoteEstimate(
  ...
  if (base_event->link_capacity_lower_.IsFinite()) {
    base_link_capacity_lower = base_event->link_capacity_lower_.kbps();
  ...
  if (base_event->link_capacity_upper_.IsFinite()) {
    base_link_capacity_upper = base_event->link_capacity_upper_.kbps();
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    const auto* event = batch[i + 1];
    if (base_event->link_capacity_lower_.IsFinite()) {
      values[i] = event->link_capacity_lower_.kbps();
    } else {
      values[i].reset();
    }
  }
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    const auto* event = batch[i + 1];
    if (base_event->link_capacity_upper_.IsFinite()) {
      values[i] = event->link_capacity_upper_.kbps();
    } else {
      values[i].reset();
    }
  }
