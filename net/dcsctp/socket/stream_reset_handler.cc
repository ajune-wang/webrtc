...
bool StreamResetHandler::ValidateReqSeqNbr(
  ...
  if (req_seq_nbr == last_processed_req_seq_nbr_) {
    ...
    return false;
             ...
             ReconfigRequestSN(*last_processed_req_seq_nbr_ + 1)) {
...
void StreamResetHandler::HandleResponse(const ParameterDescriptor& descriptor) {
  ...
  if (current_request_.has_value() &&
      current_request_->req_seq_nbr.has_value() &&
      resp->response_sequence_number() ==
          current_request_->req_seq_nbr.value()) {
    ...
    switch (resp->result()) {
      ...
      case ResponseResult::kSuccessPerformed: {
        RTC_DLOG(LS_VERBOSE)
            ...
            << *current_request_->req_seq_nbr.value() << ", streams="
      ...
      case ResponseResult::kInProgress:
        RTC_DLOG(LS_VERBOSE)
            ...
            << *current_request_->req_seq_nbr.value_or(ReconfigRequestSN(0))
            << ", streams="
            << StrJoin(current_request_->streams, ",",
                       [](rtc::StringBuilder& sb, StreamID stream_id) {
                         sb << *stream_id;
        ...
        current_request_->req_seq_nbr = absl::nullopt;
...
ReConfigChunk StreamResetHandler::MakeReconfigChunk() {
  ...
  if (!current_request_->req_seq_nbr.has_value()) {
    current_request_->req_seq_nbr = next_outgoing_req_seq_nbr_;
...
absl::optional<DurationMs> StreamResetHandler::OnReconfigTimerExpiry() {
  if (current_request_->req_seq_nbr.has_value()) {
    ...
    // The receiver responded "in progress", which will result in retrying the
    // request (but with a new req_seq_nbr) after a while.
  }
