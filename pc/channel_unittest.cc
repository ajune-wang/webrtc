 ...
 public:
  ...
  bool IsSrtpActive(cricket::BaseChannel* channel) {
    ...
    return network_thread_->Invoke<bool>(
        RTC_FROM_HERE, [channel] { return channel->srtp_active(); });
  }

  // Returns true iff the transport is set for a channel and rtcp_mux_enabled()
  // returns true.
  bool IsRtcpMuxEnabled(cricket::BaseChannel* channel) {
    RTC_DCHECK(channel);
    return network_thread_->Invoke<bool>(RTC_FROM_HERE, [channel] {
  ...
  void TestInit() {
    CreateChannels(0, 0);
    EXPECT_FALSE(IsSrtpActive(channel1_.get()));
  ...
  void SendRtpToRtp() {
    ...
    EXPECT_TRUE(SendInitiate());
    EXPECT_TRUE(SendAccept());
    EXPECT_TRUE(IsRtcpMuxEnabled(channel1_.get()));
    EXPECT_TRUE(IsRtcpMuxEnabled(channel2_.get()));
    SendRtp1();
    SendRtp2();
    WaitForThreads();
  ...
  void SendDtlsSrtpToDtlsSrtp(int flags1, int flags2) {
    ...
    EXPECT_FALSE(IsSrtpActive(channel1_.get()));
    EXPECT_FALSE(IsSrtpActive(channel2_.get()));
    EXPECT_TRUE(SendInitiate());
    WaitForThreads();
    EXPECT_TRUE(SendAccept());
    EXPECT_TRUE(IsSrtpActive(channel1_.get()));
    EXPECT_TRUE(IsSrtpActive(channel2_.get()));
    SendRtp1();
    SendRtp2();
    WaitForThreads();
    EXPECT_TRUE(CheckRtp1());
