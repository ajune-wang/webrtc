...
SimulationNode* Scenario::CreateNetworkNode() {
  return CreateNetworkNode(SimulatedNetwork::Config());
}

SimulationNode* Scenario::CreateNetworkNode(SimulatedNetwork::Config config) {
  ...
  Every(TimeDelta::ms(1), [network_node_ptr] { network_node_ptr->Process(); });
...
NetworkNode* Scenario::CreateNetworkNode(
  ...
  network_nodes_.emplace_back(
      new NetworkNode(Clock::GetRealTimeClock(), std::move(simulation)));
