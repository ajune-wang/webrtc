...
using Decision = VideoEncoderFactory::EncoderSelectorInterface::Decision;
  ...
  if (encode_status < 0) {
    if (encode_status == WEBRTC_VIDEO_CODEC_ENCODER_FAILURE) {
      ...
      if (settings_.encoder_switch_request_callback) {
        if (encoder_selector_) {
          if (encoder_selector_->OnEncoderBroken() ==
              Decision::kSwitchEncoder) {
            SwitchToSuggestedEncoder();
          }
        } else {
...
void VideoStreamEncoder::OnBitrateUpdated(DataRate target_bitrate,
  ...
  if (settings_.encoder_switch_request_callback) {
    if (encoder_selector_) {
      if (encoder_selector_->OnEncodingBitrate(target_bitrate) ==
          Decision::kSwitchEncoder) {
        SwitchToSuggestedEncoder();
      }
...
void VideoStreamEncoder::SwitchToSuggestedEncoder() const {
  RTC_DCHECK(settings_.encoder_switch_request_callback);
  RTC_DCHECK(encoder_selector_);
  settings_.encoder_switch_request_callback->RequestEncoderSwitch(
      encoder_selector_->SuggestedEncoder());
}
