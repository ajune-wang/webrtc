 ...
 public:
  explicit EncodedImageFileWriter(const VideoCodec& video_codec_setting)
    ...
    if (spatial_layers_ == 1) {
      ...
      // Create writer for each spatial layer with "-Lx" postfix.
...
class TestVideoEncoderFactoryWrapper final {
 public:
  ...
  VideoCodec CreateVideoCodec(const std::string& video_codec_string,
    ...
    video_codec.startBitrate = bitrate;
    video_codec.maxBitrate = bitrate;
    video_codec.minBitrate = bitrate;
...
int main(int argc, char* argv[]) {
  ...
  if (video_codec_string.empty()) {
    RTC_LOG(LS_ERROR) << "video codec is not set";
    return EXIT_FAILURE;
  }
  ...
  if (scalability_mode_string[0] == 'S') {
    RTC_LOG(LS_ERROR) << "Not implemented format: " << scalability_mode_string;
    return EXIT_FAILURE;
  }
  ...
  std::unique_ptr<webrtc::VideoEncoder> video_encoder =
      test_video_encoder_factory_wrapper->CreateVideoEncoder(
          video_codec_string);
  RTC_CHECK(video_encoder);
