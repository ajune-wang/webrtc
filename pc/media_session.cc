  ...
  for (const C& ours : local_codecs) {
    ...
    if (FindMatchingCodec(local_codecs, offered_codecs, ours, &theirs)) {
      ...
      if (IsRtxCodec(negotiated)) {
        ...
        const auto red_it = theirs.params.find(kCodecParamEmpty);
        if (red_it != theirs.params.end()) {
          negotiated.SetParam(kCodecParamEmpty, red_it->second);
        }
      }
  ...
  for (const C& potential_match : codecs2) {
    if (potential_match.Matches(codec_to_match)) {
      if (IsRtxCodec(codec_to_match)) {
        ...
        auto red_parameters_1 = codec_to_match.params.find(kCodecParamEmpty);
        auto red_parameters_2 = potential_match.params.find(kCodecParamEmpty);
...
static const C* GetAssociatedCodecRed(const std::vector<C>& codec_list,
                                      const C& red_codec) {
  std::string fmtp;
  if (!red_codec.GetParam(kCodecParamEmpty, &fmtp)) {
  ...
  // Find the associated reference codec for the reference RED codec.
  const C* associated_codec = FindCodecById(codec_list, associated_pt);
  if (!associated_codec) {
    RTC_LOG(LS_WARNING) << "Couldn't find associated codec with payload type "
  ...
  for (const C& reference_codec : reference_codecs) {
    if (IsRtxCodec(reference_codec) &&
      ...
      if (associated_codec) {
        ...
        red_codec.params[kCodecParamEmpty] = rtc::ToString(matching_codec.id) +
                                             "/" +
                                             rtc::ToString(matching_codec.id);
      }
