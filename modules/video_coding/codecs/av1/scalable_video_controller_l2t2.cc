...
FrameDependencyStructure ScalableVideoControllerL2T2::DependencyStructure()
  ...
  structure.templates = {
      GenericFrameInfo::Builder().S(0).T(0).Dtis("SSSS").Build(),
      GenericFrameInfo::Builder().S(0).T(0).Dtis("SSSS").Fdiffs({4}).Build(),
      GenericFrameInfo::Builder().S(1).T(0).Dtis("--SS").Fdiffs({1}).Build(),
      GenericFrameInfo::Builder().S(1).T(0).Dtis("--SR").Fdiffs({1, 4}).Build(),
      GenericFrameInfo::Builder().S(0).T(1).Dtis("-D-R").Fdiffs({2}).Build(),
      GenericFrameInfo::Builder().S(1).T(1).Dtis("---D").Fdiffs({1, 2}).Build(),
  };
  return structure;
}
