...
#include "modules/video_coding/codecs/vp8/webm_vp8_decoder.h"
...
void GetPostProcParamsFromFieldTrialGroup(
    WebMVp8Decoder::DeblockParams* deblock_params) {
  std::string group =
      webrtc::field_trial::FindFullName(kVp8PostProcArmFieldTrial);
  if (group.empty())
    return;

  WebMVp8Decoder::DeblockParams params;
...
std::unique_ptr<VP8Decoder> VP8Decoder::Create() {
  return rtc::MakeUnique<WebMVp8Decoder>();
}

class WebMVp8Decoder::QpSmoother {
 public:
...
int WebMVp8Decoder::InitDecode(const VideoCodec* inst, int number_of_cores) {
  int ret_val = Release();
  if (ret_val < 0) {
    return ret_val;
  }
...
int WebMVp8Decoder::Decode(const EncodedImage& input_image,
                           bool missing_frames,
...
int WebMVp8Decoder::ReturnFrame(const vpx_image_t* img,
                                uint32_t timestamp,
                                int64_t ntp_time_ms,
                                int qp) {
  if (img == NULL) {
  ...
  if (!buffer.get()) {
    ...
    RTC_HISTOGRAM_BOOLEAN("WebRTC.Video.WebMVp8Decoder.TooManyPendingFrames",
                          1);
    return WEBRTC_VIDEO_CODEC_NO_OUTPUT;
  }
...
int WebMVp8Decoder::RegisterDecodeCompleteCallback(
    DecodedImageCallback* callback) {
  decode_complete_callback_ = callback;
  return WEBRTC_VIDEO_CODEC_OK;
}

int WebMVp8Decoder::Release() {
...
const char* WebMVp8Decoder::ImplementationName() const {
  return "libvpx";
}
}  // namespace webrtc
