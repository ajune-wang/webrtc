...
class VideoEncoderInterface {
 public:
  ...
  struct FrameEncodeSettings {
    ...
    // TODO: What about Keyframes, is update_buffer = 0 implied?
    absl::optional<int> update_buffer;
  };

  // Results from calling Encode. Called once for each configured frame.
  // TODO: What to do when encoder breaks? Promise one error callback
  // and then no more? One error callback per expected frame? Something else?
