...
std::array<float, 3> ComputeWeights(int delay_num_samples, int sample_rate_hz) {
  ...
  if (delay_num_samples <= frame_size) {
    weights[0] = 0.0f;
    weights[1] = static_cast<float>(delay_num_samples) / frame_size;
    weights[2] =
        static_cast<float>(frame_size - delay_num_samples) / frame_size;
  } else {
    delay_num_samples -= frame_size;
    weights[0] = static_cast<float>(delay_num_samples) / frame_size;
    weights[1] =
        static_cast<float>(frame_size - delay_num_samples) / frame_size;
    weights[2] = 0.0f;
  }
...
VoiceProbabilityDelayUnit::VoiceProbabilityDelayUnit(int delay_num_samples,
                                                     int sample_rate_hz)
    : weights_(ComputeWeights(delay_num_samples, sample_rate_hz)),
      last_probabilities_({}) {}

float VoiceProbabilityDelayUnit::Delay(float voice_probability) {
  float weighted_voice_probabilty = weights_[0] * last_probabilities_[0] +
  ...
  return weighted_voice_probabilty;
}
