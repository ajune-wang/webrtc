...
void DefaultVideoQualityAnalyzer::OnFrameEncoded(
  ...
  it->second.OnFrameEncoded(now, encoded_image._frameType, encoded_image.size(),
...
void DefaultVideoQualityAnalyzer::OnFramePreDecode(
  ...
  it->second.OnFramePreDecode(peer_index,
                              ...
                              input_image._frameType, input_image.size());
}
...
void DefaultVideoQualityAnalyzer::ReportResults(
  ...
  if (true /* report detailed frame counts */) {
    test::PrintResult("num_send_frames", "", test_case_name,
                      stats.encode_time_ms.NumSamples(), "count",
                      /*important=*/false, ImproveDirection::kBiggerIsBetter);
    test::PrintResult("num_recv_frames", "", test_case_name,
                      stats.decode_time_ms.NumSamples(), "count",
                      /*important=*/false, ImproveDirection::kBiggerIsBetter);
    ...
    ReportResult("send_key_frame_size_bytes", test_case_name,
                 stats.send_key_frame_size_bytes, "count",
                 ImproveDirection::kBiggerIsBetter);
    ReportResult("send_delta_frame_size_bytes", test_case_name,
                 stats.send_delta_frame_size_bytes, "count",
                 ImproveDirection::kBiggerIsBetter);
...
void DefaultVideoQualityAnalyzer::FrameInFlight::OnFrameEncoded(
    ...
    StreamCodecInfo used_encoder) {
  encoded_time_ = time;
  encoded_frame_type_ = encoded_frame_type;
...
void DefaultVideoQualityAnalyzer::FrameInFlight::OnFramePreDecode(
  ...
  receiver_stats_[peer].pre_decoded_frame_type = pre_decoded_frame_type;
  receiver_stats_[peer].pre_decoded_image_size = pre_decoded_image_size;
}
...
FrameStats DefaultVideoQualityAnalyzer::FrameInFlight::GetStatsForPeer(
  ...
  stats.encoded_frame_type = encoded_frame_type_;
  ...
  if (receiver_stats.has_value()) {
    ...
    stats.pre_decoded_frame_type = receiver_stats->pre_decoded_frame_type;
    stats.pre_decoded_image_size = receiver_stats->pre_decoded_image_size;
  }
  return stats;
}
