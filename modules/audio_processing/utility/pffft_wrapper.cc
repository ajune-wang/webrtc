...
Pffft::FloatBuffer::FloatBuffer(size_t fft_size, FftType fft_type)
    : size_(GetBufferSize(fft_size, fft_type)),
      data_(static_cast<float*>(pffft_aligned_malloc(size_ * sizeof(float)))) {}
...
void Pffft::ForwardTransform(const FloatBuffer& in, FloatBuffer* out) {
  ...
  float* scratch_buffer = scratch_buffer_->GetView().data();
  pffft_transform(pffft_status_, in.const_data(), out->data(), scratch_buffer,
                  PFFFT_FORWARD);
}
