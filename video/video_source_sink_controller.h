...
class VideoSourceSinkController {
 public:
  ...
  const std::vector<rtc::VideoSinkWants::FrameSize>& encoder_resolutions()
      const;
 ...
 private:
  ...
  std::vector<rtc::VideoSinkWants::FrameSize> encoder_resolutions_
      RTC_GUARDED_BY(&sequence_checker_);
};
