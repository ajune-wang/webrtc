...
bool ChannelMixingMatrix::CreateTransformationMatrix(
  ...
  // Default assumption is that |matrix_| is simply a remapping.
  bool transformation_is_a_simple_remapping = true;
  ...
  if (!unaccounted_inputs_.empty()) {
    ...
    if (IsUnaccounted(LEFT)) {
      // When down mixing to mono from stereo, we need to be careful of full
      // scale stereo mixes.  Scaling by 1 / sqrt(2) here will likely lead to
    ...
    if (IsUnaccounted(BACK_LEFT)) {
      if (HasOutputChannel(SIDE_LEFT)) {
        // If the input has side LR, mix back LR into side LR, but instead if
        // the input doesn't have side LR (but output does) copy back LR to side
        // LR.
    ...
    if (IsUnaccounted(SIDE_LEFT)) {
      if (HasOutputChannel(BACK_LEFT)) {
        // If the input has back LR, mix side LR into back LR, but instead if
        // the input doesn't have back LR (but output does) copy side LR to back
        // LR.
    ...
    for (int output_ch = 0; output_ch < output_channels_; ++output_ch) {
      ...
      for (int input_ch = 0; input_ch < input_channels_; ++input_ch) {
        // We can only remap if each row contains a single scale of 1.  I.e.,
        // each output channel is mapped from a single unscaled input channel.
        if ((*matrix_)[output_ch][input_ch] != 1 || ++input_mappings > 1) {
          transformation_is_a_simple_remapping = false;
          break;
        }
      }
      if (!transformation_is_a_simple_remapping) {
        break;
      }
    }
  }

  // If specified, adjust the channel mapping for the VoIP scenario.
  if (use_voip_channel_mapping_adjustments_) {
    if (input_layout_ == CHANNEL_LAYOUT_MONO &&
        output_layout_ != CHANNEL_LAYOUT_MONO &&
        output_layout_ != CHANNEL_LAYOUT_STEREO) {
  ...
  return transformation_is_a_simple_remapping;
}
