...
constexpr TimeDelta kMinDelta = TimeDelta::Millis(1);
...
void RemoteBitrateEstimatorAbsSendTime::MaybeAddCluster(
  ...
  if (cluster_aggregate.count < kMinClusterSize ||
      cluster_aggregate.send_mean < kMinDelta ||
      cluster_aggregate.recv_mean < kMinDelta) {
    return;
  }
...
RemoteBitrateEstimatorAbsSendTime::ComputeClusters() const {
  ...
  for (const Probe& probe : probes_) {
    if (prev_send_time.IsFinite()) {
      ...
      if (send_delta >= kMinDelta && recv_delta >= kMinDelta) {
...
RemoteBitrateEstimatorAbsSendTime::FindBestProbe(
  ...
  for (const auto& cluster : clusters) {
    if (cluster.send_mean < kMinDelta || cluster.recv_mean < kMinDelta) {
      continue;
    }
    if (cluster.num_above_min_delta > cluster.count / 2 &&
      ...
      DataRate probe_bitrate_bps =
          std::min(cluster.SendBitrate(), cluster.RecvBitrate());
      if (probe_bitrate_bps > highest_probe_bitrate) {
        highest_probe_bitrate = probe_bitrate_bps;
        best = &cluster;
      }
    } else {
  ...
  std::vector<uint32_t> ssrcs;
  {
    ...
    if (!update_estimate) {
      ...
      if (last_update_.IsInfinite() ||
          now - last_update_ > remote_rate_.GetFeedbackInterval() + kMinDelta) {
        update_estimate = true;
