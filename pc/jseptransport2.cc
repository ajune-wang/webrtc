...
bool JsepTransport2::GetLocalCertificate(
  ...
  *local_certificate = local_certificate_;
  return true;
}
...
webrtc::RTCError JsepTransport2::SetLocalTransportDescription(
  ...
  } else if (!VerifyCertificateFingerprint(local_certificate_.get(), local_fp,
                                           &error_desc)) {
    return webrtc::RTCError(webrtc::RTCErrorType::INVALID_PARAMETER,
                            std::move(error_desc));
  }
...
webrtc::RTCError JsepTransport2::SetRemoteTransportDescription(
  ...
  ret &=
      ApplyRemoteTransportDescription(rtp_dtls_transport_.get(), &error_desc);
  if (rtcp_dtls_transport_) {
    ret &= ApplyRemoteTransportDescription(rtcp_dtls_transport_.get(),
                                           &error_desc);
  }
...
void JsepTransport2::ActivateRtcpMux() {
  if (unencrypted_rtp_transport_) {
    ...
    RTC_DCHECK(!unencrypted_rtp_transport_);
    RTC_DCHECK(!sdes_transport_);
    dtls_srtp_transport_->SetDtlsTransports(GetDtlsTransport(),
