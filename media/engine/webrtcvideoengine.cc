...
class NullVideoDecoder : public webrtc::VideoDecoder {
 public:
  int32_t InitDecode(const webrtc::VideoCodec* codec_settings,
    ...
    NLOG(LS_ERROR, "Can't initialize NullVideoDecoder.");
  ...
  int32_t Decode(const webrtc::EncodedImage& input_image,
    ...
    NLOG(LS_ERROR, "The NullVideoDecoder doesn't support decoding.");
    return WEBRTC_VIDEO_CODEC_OK;
  }
...
static bool ValidateCodecFormats(const std::vector<VideoCodec>& codecs) {
  ...
  if (!has_video) {
    NLOG(LS_ERROR, "Setting codecs without a video codec is invalid: ",
...
static bool ValidateStreamParams(const StreamParams& sp) {
  if (sp.ssrcs.empty()) {
    NLOG(LS_ERROR, "No SSRCs in stream parameters: ", sp.ToString());
    return false;
  }
  ...
  for (uint32_t rtx_ssrc : rtx_ssrcs) {
    ...
    if (!rtx_ssrc_present) {
      NLOG(LS_ERROR, "RTX SSRC '", rtx_ssrc,
           "' missing from StreamParams ssrcs: ", sp.ToString());
      return false;
    }
  }
  if (!rtx_ssrcs.empty() && primary_ssrcs.size() != rtx_ssrcs.size()) {
    NLOG(LS_ERROR, "RTX SSRCs exist, but don't cover all SSRCs (unsupported): ",
         sp.ToString());
    return false;
  }
...
UnsignalledSsrcHandler::Action DefaultUnsignalledSsrcHandler::OnUnsignalledSsrc(
  ...
  if (default_recv_ssrc) {
    NLOG(LS_INFO, "Destroying old default receive stream for SSRC=", ssrc, ".");
    channel->RemoveRecvStream(*default_recv_ssrc);
  }

  StreamParams sp;
  sp.ssrcs.push_back(ssrc);
  NLOG(LS_INFO, "Creating default receive stream for SSRC=", ssrc, ".");
  if (!channel->AddRecvStream(sp, true)) {
    NLOG(LS_WARNING, "Could not create default receive stream.");
  }
...
WebRtcVideoEngine::WebRtcVideoEngine(
  ...
  NLOG(LS_INFO, "WebRtcVideoEngine::WebRtcVideoEngine()");
}
#endif
...
WebRtcVideoChannel* WebRtcVideoEngine::CreateChannel(
  ...
  NLOG(LS_INFO, "CreateChannel. Options: ", options.ToString());
...
bool WebRtcVideoChannel::GetChangedSendParameters(
  ...
  if (!selected_send_codec) {
    NLOG(LS_ERROR, "No video codecs supported.");
    return false;
  }
  ...
  if (!IsFlexfecFieldTrialEnabled()) {
    if (selected_send_codec->flexfec_payload_type != -1) {
      NLOG(LS_INFO, "Remote supports flexfec-03, but we will not send since ",
...
bool WebRtcVideoChannel::SetSendParameters(const VideoSendParameters& params) {
  ...
  NLOG(LS_INFO, "SetSendParameters: ", params.ToString());
  ...
  if (changed_params.codec) {
    ...
    NLOG(LS_INFO, "Using codec: ", codec_settings.codec.ToString());
  }
...
webrtc::RtpParameters WebRtcVideoChannel::GetRtpSendParameters(
  ...
  if (it == send_streams_.end()) {
    NLOG(LS_WARNING, "Attempting to get RTP send parameters for stream ",
...
webrtc::RTCError WebRtcVideoChannel::SetRtpSendParameters(
  ...
  auto it = send_streams_.find(ssrc);
  if (it == send_streams_.end()) {
    NLOG(LS_ERROR, "Attempting to set RTP send parameters for stream ",
         "with ssrc ", ssrc, " which doesn't exist.");
  ...
  if (current_parameters.codecs != parameters.codecs) {
    NLOG(LS_ERROR, "Using SetParameters to change the set of codecs ",
...
webrtc::RtpParameters WebRtcVideoChannel::GetRtpReceiveParameters(
  ...
  if (ssrc == 0) {
    ...
    if (it == receive_streams_.end()) {
      NLOG(LS_WARNING, "Attempting to get RTP receive parameters for stream ",
           "with SSRC ", ssrc, " which doesn't exist.");
      return webrtc::RtpParameters();
    }
...
bool WebRtcVideoChannel::SetRtpReceiveParameters(
  ...
  if (ssrc == 0) {
    ...
    auto it = receive_streams_.find(ssrc);
    if (it == receive_streams_.end()) {
      NLOG(LS_WARNING, "Attempting to set RTP receive parameters for stream ",
           "with SSRC ", ssrc, " which doesn't exist.");
      return false;
    }
  }
  ...
  if (current_parameters != parameters) {
    NLOG(LS_ERROR, "Changing the RTP receive parameters is currently ",
         "unsupported.");
    return false;
  }
  return true;
...
bool WebRtcVideoChannel::GetChangedRecvParameters(
  ...
  if (mapped_codecs.empty()) {
    NLOG(LS_ERROR, "SetRecvParameters called without any video codecs.");
    return false;
  }
  ...
  for (const VideoCodecSettings& mapped_codec : mapped_codecs) {
    if (!FindMatchingCodec(local_supported_codecs, mapped_codec.codec)) {
      NLOG(LS_ERROR, "SetRecvParameters called with unsupported video codec: ",
...
bool WebRtcVideoChannel::SetRecvParameters(const VideoRecvParameters& params) {
  ...
  NLOG(LS_INFO, "SetRecvParameters: ", params.ToString());
  ...
  if (changed_params.flexfec_payload_type) {
    NLOG(LS_INFO, "Changing FlexFEC payload type (recv) from ",
  ...
  if (changed_params.codec_settings) {
    NLOG(LS_INFO, "Changing recv codecs from ",
...
bool WebRtcVideoChannel::GetSendCodec(VideoCodec* codec) {
  if (!send_codec_) {
    NLOG(LS_VERBOSE, "GetSendCodec: No send codec set.");
    return false;
  }
...
bool WebRtcVideoChannel::SetSend(bool send) {
  ...
  NLOG(LS_VERBOSE, "SetSend: ", (send ? "true" : "false"));
  if (send && !send_codec_) {
    NLOG(LS_ERROR, "SetSend(true) called before setting codec.");
    return false;
  }
  {
    rtc::CritScope stream_lock(&stream_crit_);
...
bool WebRtcVideoChannel::SetVideoSend(
  ...
  RTC_DCHECK(ssrc != 0);
  NLOG(LS_INFO, "SetVideoSend (ssrc= ", ssrc, ", enable = ", enable,
  ...
  if (kv == send_streams_.end()) {
    ...
    NLOG(LS_ERROR, "No sending stream on ssrc ", ssrc);
    return false;
  }
...
bool WebRtcVideoChannel::ValidateSendSsrcAvailability(
  ...
  for (uint32_t ssrc : sp.ssrcs) {
    if (send_ssrcs_.find(ssrc) != send_ssrcs_.end()) {
      NLOG(LS_ERROR, "Send stream with SSRC '", ssrc, "' already exists.");
      return false;
    }
  }
  return true;
...
bool WebRtcVideoChannel::ValidateReceiveSsrcAvailability(
  ...
  for (uint32_t ssrc : sp.ssrcs) {
    if (receive_ssrcs_.find(ssrc) != receive_ssrcs_.end()) {
      NLOG(LS_ERROR, "Receive stream with SSRC '", ssrc, "' already exists.");
      return false;
    }
  }
  return true;
...
bool WebRtcVideoChannel::AddSendStream(const StreamParams& sp) {
  NLOG(LS_INFO, "AddSendStream: ", sp.ToString());
...
bool WebRtcVideoChannel::RemoveSendStream(uint32_t ssrc) {
  NLOG(LS_INFO, "RemoveSendStream: ", ssrc);
  WebRtcVideoSendStream* removed_stream;
  {
    ...
    if (rtcp_receiver_report_ssrc_ == ssrc) {
      ...
      NLOG(LS_INFO,
           "SetLocalSsrc on all the receive streams because the "
...
bool WebRtcVideoChannel::AddRecvStream(const StreamParams& sp,
  ...
  NLOG(LS_INFO, "AddRecvStream", (default_stream ? " (default stream)" : ""),
       ": ", sp.ToString());
  if (!ValidateStreamParams(sp))
    return false;
  ...
  if (prev_stream != receive_streams_.end()) {
    if (default_stream || !prev_stream->second->IsDefaultStream()) {
      NLOG(LS_ERROR, "Receive stream for SSRC '", ssrc, "' already exists.");
      return false;
    }
...
bool WebRtcVideoChannel::RemoveRecvStream(uint32_t ssrc) {
  NLOG(LS_INFO, "RemoveRecvStream: ", ssrc);
  if (ssrc == 0) {
    NLOG(LS_ERROR, "RemoveRecvStream with 0 ssrc is not supported.");
    return false;
  }
  ...
  if (stream == receive_streams_.end()) {
    NLOG(LS_ERROR, "Stream not found for ssrc: ", ssrc);
    return false;
  }
...
bool WebRtcVideoChannel::SetSink(
    uint32_t ssrc,
    rtc::VideoSinkInterface<webrtc::VideoFrame>* sink) {
  NLOG(LS_INFO, "SetSink: ssrc:", ssrc, " ", (sink ? "(ptr)" : "nullptr"));
  if (ssrc == 0) {
...
bool WebRtcVideoChannel::GetStats(VideoMediaInfo* info) {
  ...
  if (log_stats)
    NLOG(LS_INFO, stats.ToString(now_ms));
  return true;
}
...
void WebRtcVideoChannel::OnPacketReceived(
  ...
  if (call_->Receiver()->DeliverPacket(webrtc::MediaType::VIDEO, *packet,
    ...
    NLOG(LS_WARNING, "Failed to deliver RTP packet on re-delivery.");
    return;
  }
}
...
void WebRtcVideoChannel::OnReadyToSend(bool ready) {
  NLOG(LS_VERBOSE, "OnReadyToSend: ", (ready ? "Ready." : "Not ready."));
...
void WebRtcVideoChannel::WebRtcVideoSendStream::SetCodec(
  ...
  NLOG(LS_INFO, "RecreateWebRtcStream (send) because of SetCodec.");
...
void WebRtcVideoChannel::WebRtcVideoSendStream::SetSendParameters(
  ...
  if (recreate_stream) {
    NLOG(LS_INFO, "RecreateWebRtcStream (send) because of SetSendParameters");
    RecreateWebRtcStream();
  }
}
...
VideoSenderInfo WebRtcVideoChannel::WebRtcVideoSendStream::GetVideoSenderInfo(
  ...
  if (log_stats)
    NLOG(LS_INFO, stats.ToString(rtc::TimeMillis()));
...
WebRtcVideoChannel::WebRtcVideoReceiveStream::GetFirstPrimarySsrc() const {
  ...
  if (primary_ssrcs.empty()) {
    NLOG(LS_WARNING, "Empty primary ssrcs vector, returning empty optional");
    return rtc::nullopt;
  } else {
  ...
  *old_decoders = std::move(allocated_decoders_);
  ...
  for (const auto& recv_codec : recv_codecs) {
    ...
    if (allocated_decoders_.count(video_format) > 0) {
      NLOG(LS_WARNING, "VideoReceiveStream configured with duplicate codecs: ",
           video_format.name);
      continue;
    }
...
void WebRtcVideoChannel::WebRtcVideoReceiveStream::SetRecvParameters(
  ...
  if (flexfec_needs_recreation) {
    NLOG(LS_INFO,
         "MaybeRecreateWebRtcFlexfecStream (recv) because of "
         "SetRecvParameters");
    MaybeRecreateWebRtcFlexfecStream();
  }
...
void WebRtcVideoChannel::WebRtcVideoReceiveStream::OnFrame(
  ...
  if (sink_ == NULL) {
    NLOG(LS_WARNING, "VideoReceiveStream not connected to a VideoSink.");
    return;
  }
...
WebRtcVideoChannel::MapCodecs(const std::vector<VideoCodec>& codecs) {
  ...
  for (size_t i = 0; i < codecs.size(); ++i) {
    ...
    if (payload_used[payload_type]) {
      NLOG(LS_ERROR, "Payload type already registered: ", in_codec.ToString());
  ...
  for (std::map<int, int>::const_iterator it = rtx_mapping.begin();
    ...
    if (!payload_used[it->first]) {
      NLOG(LS_ERROR, "RTX mapped to payload not in codec list.");
      return std::vector<VideoCodecSettings>();
    }
    if (payload_codec_type[it->first] != VideoCodec::CODEC_VIDEO &&
        payload_codec_type[it->first] != VideoCodec::CODEC_RED) {
      NLOG(LS_ERROR, "RTX not mapped to regular video codec or RED codec.");
      return std::vector<VideoCodecSettings>();
    }
