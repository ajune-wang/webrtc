    ...
    [RTCDispatcher dispatchAsyncOnType:RTCDispatcherTypeCaptureSession
                                 block:^{
                                     ...
                                     for (int i = 0; i < kNumBuffer; i++) {
                                         ...
                                         CVPixelBufferCreate(nil, 1280, 720, kCVPixelFormatType_OneComponent8, nil, &_mask_buffer_queue[i]);
...
- (void)segmentAndStreamDownAtIndex: (int) index
    ...
    [_segmenter rgbdSegmentAlpha:_color_buffer_queue[index]
...
- (void)session:(ARSession *)sessionn didUpdateFrame:(ARFrame *)frame {
    ...
    if (_all_num % 100 == 0) {
        _all_num = 0;
        RTCLogError("AllFPS: %lf", -100/[_all_ref timeIntervalSinceNow]);
    ...
    if (depth_data){
        ...
        {
            _depth_num = 0;
            RTCLogError("DepthFPS: %lf", -100/[_depth_ref timeIntervalSinceNow]);
    ...
    if (buffer_index < 0) {
        if (buffer_index == -1)
            RTCLogError("Drop Frame Due To Buffer Unavailable!");
        if (buffer_index == -2)
            RTCLogError("Drop Frame: Severe Error Buffer Management Algorithm Is Problematic!");
        return;
    }
