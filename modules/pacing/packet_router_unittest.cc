...
TEST(PacketRouterTest, GeneratePadding) {
  PacketRouter packet_router;
  ...
  NiceMock<MockRtpRtcp> rtp_1;
  EXPECT_CALL(rtp_1, RtxSendStatus()).WillOnce(Return(kRtxRedundantPayloads));
  EXPECT_CALL(rtp_1, SSRC()).WillRepeatedly(Return(kSsrc1));
  EXPECT_CALL(rtp_1, SendingMedia()).WillRepeatedly(Return(false));
  EXPECT_CALL(rtp_1, HasBweExtensions()).WillRepeatedly(Return(false));

  NiceMock<MockRtpRtcp> rtp_2;
  EXPECT_CALL(rtp_2, RtxSendStatus()).WillOnce(Return(kRtxOff));
  EXPECT_CALL(rtp_2, SSRC()).WillRepeatedly(Return(kSsrc2));
  packet_router.AddSendRtpModule(&rtp_1, false);
  packet_router.AddSendRtpModule(&rtp_2, false);
