...
TEST(ThreadTest, ThreeThreadsInvoke) {
  ...
  struct LocalFuncs {
    ...
    static void AsyncInvokeSetAndWait(Thread* thread1,
      ...
      thread1->PostTask(ToQueuedTask([&async_invoked, thread2, out] {
  ...
  thread_b->Invoke<void>(
      RTC_FROM_HERE, [thread_c_ptr, thread_a, &thread_a_called] {
        LocalFuncs::AsyncInvokeSetAndWait(thread_c_ptr, thread_a,
                                          &thread_a_called);
      });
