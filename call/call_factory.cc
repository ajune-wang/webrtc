...
namespace webrtc {
namespace {
using ScopedNetworkConfig = DegradedCall::ScopedNetworkConfig;
...
absl::optional<ScopedNetworkConfig> ParseDegradationConfig(
  ...
  return configured ? absl::optional<ScopedNetworkConfig>(config)
                    : absl::nullopt;
}

std::vector<ScopedNetworkConfig> GetNetworkConfigs(
    const WebRtcKeyValueConfig& trials,
    bool send) {
  FieldTrialStructList<ScopedNetworkConfig> trials_list(
      {FieldTrialStructMember("queue_length_packets",
                              [](ScopedNetworkConfig* p) {
                                return reinterpret_cast<unsigned*>(
                                    &p->queue_length_packets);
                              }),
       FieldTrialStructMember(
           "queue_delay_ms",
           [](ScopedNetworkConfig* p) { return &p->queue_delay_ms; }),
       ...
       FieldTrialStructMember(
           "link_capacity_kbps",
           [](ScopedNetworkConfig* p) { return &p->link_capacity_kbps; }),
       FieldTrialStructMember(
           "loss_percent",
           [](ScopedNetworkConfig* p) { return &p->loss_percent; }),
       FieldTrialStructMember(
           "allow_reordering",
           [](ScopedNetworkConfig* p) { return &p->allow_reordering; }),
       FieldTrialStructMember(
           "avg_burst_loss_length",
           [](ScopedNetworkConfig* p) { return &p->avg_burst_loss_length; }),
       FieldTrialStructMember(
           "packet_overhead",
           [](ScopedNetworkConfig* p) { return &p->packet_overhead; }),
       FieldTrialStructMember("codel_active_queue_management",
                              [](ScopedNetworkConfig* p) {
                                return &p->codel_active_queue_management;
                              }),
       FieldTrialStructMember(
           "duration", [](ScopedNetworkConfig* p) { return &p->duration; })},
      {});
  ParseFieldTrial({&trials_list},
                  trials.Lookup(send ? "WebRTC-FakeNetworkSendConfig"
                                     : "WebRTC-FakeNetworkReceiveConfig"));
  std::vector<ScopedNetworkConfig> configs = trials_list.Get();
  if (configs.empty()) {
    // Try legacy fallback trials.
    absl::optional<DegradedCall::ScopedNetworkConfig> fallback_config =
...
Call* CallFactory::CreateCall(const Call::Config& config) {
  ...
  std::vector<DegradedCall::ScopedNetworkConfig> send_degradation_configs =
      GetNetworkConfigs(*config.trials, /*send=*/true);
  std::vector<DegradedCall::ScopedNetworkConfig> receive_degradation_configs =
      GetNetworkConfigs(*config.trials, /*send=*/true);
