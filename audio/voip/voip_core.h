...
class VoipCore : public VoipEngine,
 ...
 public:
  ...
  // would render further processing useless. Note that failures on initializing
 ...
 private:
  // Maximum value range limit on ChannelId. This is rather arbitrary and can be
  // increased for any need arises later.
  static constexpr int kMaxChannelId = 100000;

  // Fetch the corresponding AudioChannel assigned with given |channel|.
  // Returns nullptr if not found.
  rtc::scoped_refptr<AudioChannel> GetChannel(ChannelId channel);

  // Update AudioTransportImpl with a new set of actively sending AudioSender
