...
class EmulatedEndpoint : public EmulatedNetworkReceiverInterface {
 public:
  ...
  void SetSendNode(rtc::IPAddress dest_ip, EmulatedNetworkNode* send_node);
  void ClearSendNode(rtc::IPAddress dest_ip);
 ...
 private:
  ...
  // Stores which network node to use for each destination IP.
  std::map<rtc::IPAddress, EmulatedNetworkNode*> send_nodes_
