...
bool NetEqImpl::DoCodecPlc() {
  ...
  while (concealment_audio_.size() + sync_buffer_->FutureLength() -
             expand_->overlap_length() <
         output_size_samples_) {
    size_t size_before = concealment_audio_.size();
    decoder->GeneratePlc(&concealment_audio_);
    if (concealment_audio_.size() == size_before) {
      // Nothing produced. Resort to regular expand.
      return false;
    }
  }
  sync_buffer_->PushBack(concealment_audio_);
  RTC_DCHECK_NE(algorithm_buffer_->Channels(), 0);
  const size_t concealed_samples_per_channel =
      concealment_audio_.size() / algorithm_buffer_->Channels();
