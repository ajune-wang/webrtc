...
RTPSender::DirectPacketSender::DirectPacketSender(RTPSender* rtp_sender)
    : transport_sequence_number_(0), rtp_sender_(rtp_sender) {}
RTPSender::DirectPacketSender::~DirectPacketSender() = default;

void RTPSender::DirectPacketSender::EnqueuePacket(
    std::unique_ptr<RtpPacketToSend> packet) {
  if (packet->IsExtensionReserved<TransportSequenceNumber>()) {
    packet->SetExtension<TransportSequenceNumber>(++transport_sequence_number_);
  }
...
RTPSender::RTPSender(const RtpRtcp::Configuration& config)
    : clock_(config.clock),
      ...
      flexfec_ssrc_(config.flexfec_sender
                        ...
                        : absl::nullopt),
      direct_packet_sender_(config.paced_sender ? nullptr
                                                : new DirectPacketSender(this)),
      paced_sender_(config.paced_sender ? config.paced_sender
                                        : direct_packet_sender_.get()),
