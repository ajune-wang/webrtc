...
bool ParseCandidate(absl::string_view message,
  ...
  // TODO(bugs.webrtc.org:13579): StringToProto should accept string_view input.
...
bool ParseGroupAttribute(absl::string_view line,
  ...
  for (size_t i = 1; i < fields.size(); ++i) {
    // TODO(bugs.webrtc.org:13579): AddContentName should accept string_view
    // input.
...
static bool ParseDtlsSetup(absl::string_view line,
  ...
  absl::string_view role_str = fields[1];
  // TODO(bugs.webrtc.org:13579): StringToConnectionRole should accept
  // string_view input.
  if (!cricket::StringToConnectionRole(std::string(role_str), role)) {
...
bool ParseMediaDescription(
  ...
  while (absl::optional<absl::string_view> mline =
    ...
    // TODO(bugs.webrtc.org:13579): Metods set_protocol and similar below should
    // accept string_view input.
    ...
    if (media_type == kMediaTypeVideo) {
      ...
      // TODO(bugs.webrtc.org:13579): UnsupportedContentDescription should
      // accept string_view input.
      auto unsupported_desc = std::make_unique<UnsupportedContentDescription>(
...
bool ParseCryptoAttribute(absl::string_view line,
  ...
  // TODO(bugs.webrtc.org:13579): CryptoParams should accept string_view input.
  media_desc->AddCrypto(CryptoParams(tag, std::string(crypto_suite),
...
bool ParseRtcpFbAttribute(absl::string_view line,
  ...
  for (auto iter = rtcp_fb_fields.begin() + 2; iter != rtcp_fb_fields.end();
       ++iter) {
    param.append(std::string(*iter));
  }
  // TODO(bugs.webrtc.org:13579): FeedbackParam should accept string_view input.
  const cricket::FeedbackParam feedback_param(std::string(id), param);
