...
TEST_F(ReceiveStatisticsProxyTest, ReportsFreezeMetrics) {
  ...
  const int kCallDurationMs =
      kMinFrameSamplesToDetectFreeze * kFrameDurationMs + kFreezeDurationMs;

  VideoReceiveStream::Stats stats = statistics_proxy_->GetStats();
  EXPECT_EQ(0u, stats.num_freezes);
  EXPECT_FALSE(stats.mean_freeze_duration_ms);
  EXPECT_FALSE(stats.mean_time_between_freezes_ms);
  EXPECT_EQ(0u, stats.harmonic_framerate_fps);
  ...
  const uint32_t kExpectedHarmonicFrameRateFps =
      std::round(kCallDurationMs / (1000 * kSumSquaredInterframeDelaysSecs));
  EXPECT_EQ(kExpectedHarmonicFrameRateFps, stats.harmonic_framerate_fps);
}
