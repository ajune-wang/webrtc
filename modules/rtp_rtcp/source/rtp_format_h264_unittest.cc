...
rtc::Buffer<uint8_t> CreateFrame(size_t frame_size) {
  rtc::Buffer<uint8_t> frame(frame_size);
...
rtc::Buffer<uint8_t> CreateFrame(const RTPFragmentationHeader& fragmentation) {
  ...
  rtc::Buffer<uint8_t> frame = CreateFrame(frame_size);
  // Set some headers.
...
TEST_P(RtpPacketizerH264ModeTest, SingleNaluTwoPackets) {
  ...
  rtc::Buffer<uint8_t> frame = CreateFrame(fragmentation);
...
TEST_P(RtpPacketizerH264ModeTest,
  ...
  rtc::Buffer<uint8_t> frame = CreateFrame(150);
...
std::vector<int> TestFua(size_t frame_payload_size,
  ...
  rtc::Buffer<uint8_t> frame = CreateFrame(kNalHeaderSize + frame_payload_size);
...
TEST(RtpPacketizerH264Test, RejectsOverlongDataInPacketizationMode0) {
  RtpPacketizer::PayloadSizeLimits limits;
  rtc::Buffer<uint8_t> frame = CreateFrame(kMaxPayloadSize + 1);
...
TEST_F(RtpDepacketizerH264Test, DepacketizeWithRewriting) {
  rtc::Buffer<uint8_t> in_buffer;
  rtc::Buffer<uint8_t> out_buffer;
