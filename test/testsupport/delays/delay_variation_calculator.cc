...
#include "test/testsupport/delays/delay_variation_calculator.h"
...
void DelayVariationCalculator::InsertFirstFrame(const Frame& frame) {
  ...
  MetadataT ts_metadata = BuildMetadata(frame);
  const auto s = [=](double ts_value) {
    return SamplesStatsCounter::StatsSample{
        .value = ts_value, .time = ts_time, .metadata = ts_metadata};
  };
  ...
  time_series_.sizes_bytes.AddSample(frame.size.bytes<double>());
