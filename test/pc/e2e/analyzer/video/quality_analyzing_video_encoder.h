...
//
// Quality encoder registers its own callback in origin encoder in the same
...
class QualityAnalyzingVideoEncoder : public VideoEncoder {
 ...
 private:
  class EncoderCallback : public EncodedImageCallback {
   public:
    explicit EncoderCallback(QualityAnalyzingVideoEncoder* encoder);
    ~EncoderCallback() override;
   ...
   private:
    QualityAnalyzingVideoEncoder* const encoder_;
  };
  ...
  std::unique_ptr<EncoderCallback> analyzing_callback_;
