...
ScalabilityStructureFullSvc::ScalabilityStructureFullSvc(Settings settings)
    : settings_(settings),
      active_decode_targets_((uint32_t{1} << (settings_.num_spatial_layers *
                                              settings_.num_temporal_layers)) -
                             1) {
  RTC_DCHECK_LE(settings_.num_spatial_layers, kMaxNumSpatialLayers);
  RTC_DCHECK_LE(settings_.num_temporal_layers, kMaxNumTemporalLayers);
}
...
ScalabilityStructureFullSvc::StreamConfig() const {
  ...
  result.num_spatial_layers = settings_.num_spatial_layers;
  result.num_temporal_layers = settings_.num_temporal_layers;
  result.scaling_factor_num[settings_.num_spatial_layers - 1] = 1;
  result.scaling_factor_den[settings_.num_spatial_layers - 1] = 1;
  for (int sid = settings_.num_spatial_layers - 1; sid > 0; --sid) {
    result.scaling_factor_num[sid - 1] =
        settings_.scaling_factor_num * result.scaling_factor_num[sid];
    result.scaling_factor_den[sid - 1] =
        settings_.scaling_factor_den * result.scaling_factor_den[sid];
  }
  return result;
}

bool ScalabilityStructureFullSvc::TemporalLayerIsActive(int tid) const {
  if (tid >= settings_.num_temporal_layers) {
    return false;
  }
  for (int sid = 0; sid < settings_.num_spatial_layers; ++sid) {
...
ScalabilityStructureFullSvc::NextFrameConfig(bool restart) {
  ...
  configs.reserve(settings_.num_spatial_layers);
  ...
  switch (current_pattern) {
    ...
    case kDeltaT1:
      for (int sid = 0; sid < settings_.num_spatial_layers; ++sid) {
        ...
        if (settings_.num_temporal_layers > 2 ||
            sid < settings_.num_spatial_layers - 1) {
    ...
    case kDeltaT2B:
      for (int sid = 0; sid < settings_.num_spatial_layers; ++sid) {
        ...
        // No frame reference top layer frame, so no need save it into a buffer.
        if (sid < settings_.num_spatial_layers - 1) {
...
GenericFrameInfo ScalabilityStructureFullSvc::OnEncodeDone(
  ...
  frame_info.decode_target_indications.reserve(settings_.num_spatial_layers *
                                               settings_.num_temporal_layers);
  for (int sid = 0; sid < settings_.num_spatial_layers; ++sid) {
    for (int tid = 0; tid < settings_.num_temporal_layers; ++tid) {
      frame_info.decode_target_indications.push_back(Dti(sid, tid, config));
    }
  }
  if (config.TemporalId() == 0) {
    frame_info.part_of_chain.resize(settings_.num_spatial_layers);
    for (int sid = 0; sid < settings_.num_spatial_layers; ++sid) {
      frame_info.part_of_chain[sid] = config.SpatialId() <= sid;
    }
  } else {
    frame_info.part_of_chain.assign(settings_.num_spatial_layers, false);
  }
...
FrameDependencyStructure ScalabilityStructureFullSvc::L1T2() {
...
FrameDependencyStructure ScalabilityStructureFullSvc::L1T3() {
  FrameDependencyStructure structure;
...
FrameDependencyStructure ScalabilityStructureFullSvc::L2T1() {
  FrameDependencyStructure structure;
  structure.num_decode_targets = 2;
...
FrameDependencyStructure ScalabilityStructureFullSvc::L2T2() {
  FrameDependencyStructure structure;
...
FrameDependencyStructure ScalabilityStructureFullSvc::L3T1() {
  FrameDependencyStructure structure;
  structure.num_decode_targets = 3;
...
FrameDependencyStructure ScalabilityStructureFullSvc::L3T3() {
  FrameDependencyStructure structure;
