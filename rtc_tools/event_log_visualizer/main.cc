...
struct PlotWrapper {
  PlotWrapper(const std::string& label, std::function<void(Plot*)> f)
  ...
  bool EnablePlotsByFlags(
    ...
    for (const std::string& flag : flags) {
      ...
      if (alias_it != flag_aliases.end()) {
        std::vector<std::string> replacements = StrSplit(alias_it->second, ",");
...
int main(int argc, char* argv[]) {
  ...
  std::map<std::string, std::string> flag_aliases = {
      ...
      {"receiveside_bwe",
       "incoming_packet_sizes,incoming_delay,incoming_loss_rate,incoming_"
  ...
  for (const auto& plot_wrapper : plots) {
    if (plot_wrapper.enabled) {
      Plot* plot = collection->AppendNewPlot();
      plot_wrapper.f(plot);
  ...
  // calls.
  if (Contains(plot_flags, "simulated_neteq_jitter_buffer_delay")) {
    if (!neteq_stats) {
      neteq_stats = analyzer.SimulateNetEq(wav_path, 48000);
    }
