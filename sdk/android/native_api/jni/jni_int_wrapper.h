...
class JniIntWrapper {
 ...
 private:
  // If you get an "is private" error at the lines below it is because you used
  // an implicit conversion to convert e.g. a long to an int when calling Java.
  // We disallow this. If you want a lossy conversion, please use an explicit
  // conversion in your C++ code.
  JniIntWrapper(uint32_t);  // NOLINT(runtime/explicit)
  JniIntWrapper(uint64_t);  // NOLINT(runtime/explicit)
  JniIntWrapper(int64_t);   // NOLINT(runtime/explicit)
  jint i_;
};
