...
constexpr size_t kPayloadHeaderSizeInBytes = 2;
// Unlike H.264, H.265 NAL header is 2-bytes.
constexpr size_t kNalHeaderSizeInBytes = 2;
// H.265's FU is constructed of 2-byte payload header, 1-byte FU header and FU
// payload.
constexpr size_t kFuHeaderSizeInBytes = 1;
// The NALU size for H.265 RTP aggregated packet indicates the size of the NAL
// unit is 2-bytes.
constexpr size_t kLengthFieldSizeInBytes = 2;
constexpr size_t kApHeaderSizeInBytes =
    kNalHeaderSizeInBytes + kLengthFieldSizeInBytes;
...
enum NalHdrMasks {
  ...
  kTIDMask = 0x7,
  kTypeMaskN = 0x81,
  kTypeMaskInFuHeader = 0x3F
};

// Bit masks for FU headers.
enum FuBitmasks { kSBitMask = 0x80, kEBitMask = 0x40, kFuTypeBitMask = 0x3F };
...
bool ParseApStartOffsets(const uint8_t* nalu_ptr,
  ...
  while (length_remaining > 0) {
    ...
    if (length_remaining < kLengthFieldSizeInBytes)
      return false;
    // Read 16-bit NALU size defined in RFC7798 section 4.4.2.
    uint16_t nalu_size = ByteReader<uint16_t>::ReadBigEndian(nalu_ptr);
    nalu_ptr += kLengthFieldSizeInBytes;
    length_remaining -= kLengthFieldSizeInBytes;
    if (nalu_size > length_remaining)
      return false;
    nalu_ptr += nalu_size;
    length_remaining -= nalu_size;

    offsets->push_back(offset + kApHeaderSizeInBytes);
    offset += kLengthFieldSizeInBytes + nalu_size;
  }
  return true;
}
...
absl::optional<VideoRtpDepacketizer::ParsedRtpPayload> ProcessApOrSingleNalu(
  ...
  // be checked later
  if (rtp_payload.size() <= kPayloadHeaderSizeInBytes) {
  ...
  const uint8_t* nalu_start = payload_data + kPayloadHeaderSizeInBytes;
  const size_t nalu_length = rtp_payload.size() - kPayloadHeaderSizeInBytes;
  uint8_t nal_type = (payload_data[0] & kTypeMask) >> 1;
  std::vector<size_t> nalu_start_offsets;
  rtc::CopyOnWriteBuffer video_payload;
  if (nal_type == H265::NaluType::kAp) {
    // Skip the aggregated packet header (Aggregated packet NAL type + length).
    if (rtp_payload.size() <= kApHeaderSizeInBytes) {
    ...
    nal_type = (payload_data[kApHeaderSizeInBytes] & kTypeMask) >> 1;
  } else {
  ...
  nalu_start_offsets.push_back(rtp_payload.size() +
                               kLengthFieldSizeInBytes);  // End offset.
  for (size_t i = 0; i < nalu_start_offsets.size() - 1; ++i) {
    size_t start_offset = nalu_start_offsets[i];
    // End offset is actually start offset for next unit, excluding length field
    // so remove that from this units length.
    size_t end_offset = nalu_start_offsets[i + 1] - kLengthFieldSizeInBytes;
    if (end_offset - start_offset < kNalHeaderSizeInBytes) {
    ...
    uint8_t nalu_type = (payload_data[start_offset] & kTypeMask) >> 1;
    start_offset += kNalHeaderSizeInBytes;
    switch (nalu_type) {
...
absl::optional<VideoRtpDepacketizer::ParsedRtpPayload> ParseFuNalu(
    rtc::CopyOnWriteBuffer rtp_payload) {
  if (rtp_payload.size() < kFuHeaderSizeInBytes + kNalHeaderSizeInBytes) {
  ...
  uint8_t f = rtp_payload.cdata()[0] & kFBit;
  uint8_t layer_id_h = rtp_payload.cdata()[0] & kLayerIDHMask;
  uint8_t layer_id_l_unshifted = rtp_payload.cdata()[1] & kLayerIDLMask;
  uint8_t tid = rtp_payload.cdata()[1] & kTIDMask;

  uint8_t original_nal_type = rtp_payload.cdata()[2] & kTypeMaskInFuHeader;
  bool first_fragment = rtp_payload.cdata()[2] & kSBitMask;
  if (first_fragment) {
    rtp_payload = rtp_payload.Slice(kFuHeaderSizeInBytes,
                                    rtp_payload.size() - kFuHeaderSizeInBytes);
    ...
    parsed_payload->video_payload = rtp_payload.Slice(
        kNalHeaderSizeInBytes + kFuHeaderSizeInBytes,
        rtp_payload.size() - kNalHeaderSizeInBytes - kFuHeaderSizeInBytes);
  }
...
VideoRtpDepacketizerH265::Parse(rtc::CopyOnWriteBuffer rtp_payload) {
  ...
  uint8_t nal_type = (rtp_payload.cdata()[0] & kTypeMask) >> 1;
