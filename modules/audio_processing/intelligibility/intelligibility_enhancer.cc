...
IntelligibilityEnhancer::~IntelligibilityEnhancer() {
  ...
  // app/tab is killed.
  if (num_chunks_ > 0) {
    NLOG(LS_INFO, "Intelligibility Enhancer was active for ",
         100.f * static_cast<float>(num_active_chunks_) / num_chunks_,
         "% of the call.");
  } else {
    NLOG(LS_INFO, "Intelligibility Enhancer processed no chunk.");
  }
}
...
void IntelligibilityEnhancer::SnrBasedEffectActivation() {
  ...
  if (is_active_) {
    if (snr_ > kMaxActiveSNR) {
      NLOG(LS_INFO, "Intelligibility Enhancer was deactivated at chunk ",
           num_chunks_);
      is_active_ = false;
    ...
    if (snr_ < kMinInactiveSNR) {
      NLOG(LS_INFO, "Intelligibility Enhancer was activated at chunk ",
           num_chunks_);
      is_active_ = true;
    }
  }
