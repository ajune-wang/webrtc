...
void BufferStatistics::AddUnderrun(size_t timestamp) {
  ...
  next_underrun_index_ =
      next_underrun_index_ < static_cast<int>(render_underruns_.size()) - 1
          ? next_underrun_index_ + 1
          : 0;
}
...
void BufferStatistics::AddSurplusOverflow(size_t timestamp) {
  ...
  next_overflow_index_ =
      next_overflow_index_ <
              static_cast<int>(render_surplus_overflows_.size()) - 1
          ? next_overflow_index_ + 1
          : 0;
}
