...
void EventLogAnalyzer::CreateNetEqStatsGraph(
    const NetEqStatsGetterMap& neteq_stats,
    const std::function<size_t(const test::NetEqStatsGetter*)>& size_extractor,
    const std::function<uint64_t(const test::NetEqStatsGetter*, int i)>&
        time_extractor,
    const std::function<float(const test::NetEqStatsGetter*, int i)>&
        stats_extractor,
