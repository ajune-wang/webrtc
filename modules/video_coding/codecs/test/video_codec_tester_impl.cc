 ...
 public:
  ...
  void Decode(const EncodedImage& encoded_frame,
    ...
    if (auto gfi = codec_specific_info->generic_frame_info; gfi.has_value()) {
      RTC_CHECK(codec_specific_info->scalability_mode);
      int num_spatial_layers = ScalabilityModeToNumSpatialLayers(*codec_specific_info->scalability_mode);
      int num_temporal_layers = ScalabilityModeToNumTemporalLayers(*codec_specific_info->scalability_mode);
      for (int sidx = 0; sidx < num_spatial_layers; ++sidx) {
        if (gfi->decode_target_indications[sidx * num_temporal_layers] != DecodeTargetIndication::kNotPresent) {

        }
      }
      RTC_CHECK_GE(gfi->decode_target_indications.size());
      for (const auto& dti : generic_frame_info->decode_target_indications) {
      }  
    }
    const auto& generic_frame_info = codec_specific_info->generic_frame_info;
    for (const auto& dti : generic_frame_info->decode_target_indications) {
      RTC_LOG(LS_INFO) << "dti " << dti;
    }
