...
class BandwidthQualityScaler {
 public:
  ...
  // We will priority to using the |resolution_bitrate_limits| provided by
  // the current decoder. If not provided, we will use the default data by
 ...
 private:
  ...
  absl::optional<int> frame_size_pixels_ RTC_GUARDED_BY(&task_checker_);
