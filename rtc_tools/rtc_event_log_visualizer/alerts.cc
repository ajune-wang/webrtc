...
void TriageHelper::AnalyzeStreamGaps(const ParsedRtcEventLog& parsed_log,
  ...
  int64_t end_time_us = parsed_log.log_segments().empty()
                            ? std::numeric_limits<int64_t>::max()
                            : parsed_log.log_segments().front().second;
    ...
    for (const auto& packet : stream.packet_view) {
      ...
      if (last_capture_time.has_value() &&
          std::abs(capture_time - last_capture_time.value()) >
              kMaxCaptureTimeJump) {
        fprintf(stderr, "JUMP %lf %lld %lld %u\n",
                config_.GetCallTimeSec(packet.log_time_us()), capture_time,
                last_capture_time.value(), packet.header.ssrc);
