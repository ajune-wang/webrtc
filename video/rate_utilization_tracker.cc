...
void RateUtilizationTracker::OnDataProduced(DataSize size, Timestamp time) {
  RTC_CHECK(data_points_.empty() || time >= data_points_.back().time);
...
absl::optional<double> RateUtilizationTracker::GetRateUtilizationFactor(
  ...
  const Timestamp start_time = time - max_duration_;
  ...
  for (size_t i = 0; i < rate_updates_.size(); ++i) {
    ...
    if (is_last_update) {
      data_allocated_for_last_data_point += (time - update.time) * update.rate;
    } else {
