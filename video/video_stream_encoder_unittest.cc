...
TEST_P(VideoStreamEncoderWithRealEncoderTest, Foo) {
  if (codec_type_ == kVideoCodecMultiplex) {
    // Multiplex test uses wrapped mock codecs, ignore for this test.
    return;
  }
  ...
  // Bitrate target adjuster back up to enable HD layer...
  video_stream_encoder_->OnBitrateUpdatedAndWaitForManagedResources(
