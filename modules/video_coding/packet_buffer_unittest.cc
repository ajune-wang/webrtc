...
class PacketBufferTest : public ::testing::Test {
 protected:
  ...
  PacketBufferInsertResult Insert(uint16_t seq_num,  // packet sequence number
    ...
    packet.video_payload.SetData(data, data_size);
...
TEST_F(PacketBufferTest, GetBitstream) {
  ...
  Insert(seq_num, kKeyFrame, kFirst, kNotLast, sizeof(many), many);
  Insert(seq_num + 1, kDeltaFrame, kNotFirst, kNotLast, sizeof(bitstream),
         bitstream);
  Insert(seq_num + 2, kDeltaFrame, kNotFirst, kNotLast, sizeof(such), such);
  auto frames =
...
TEST_F(PacketBufferTest, GetBitstreamOneFrameOnePacket) {
  ...
  auto frames =
      Insert(0, kKeyFrame, kFirst, kLast, sizeof(bitstream), bitstream).frames;
