...
ScalabilityStructureL2T2KeyShift::NextFrameConfig(bool restart) {
  ...
  if (restart || next_pattern_ == kKey) {
    next_pattern_ = kKey;
  } else if (active_decode_targets_ == 0b1) {
    // The only active layer is S0T0.
    RTC_DCHECK(DecodeTargetIsActive(/*sid=*/0, /*tid=*/0));
    configs.emplace_back();
    configs.back().S(0).T(0).ReferenceAndUpdate(0);
    return configs;
  } else if (active_decode_targets_ == 0b100) {
    // The only active layer is S1T0.
    RTC_DCHECK(DecodeTargetIsActive(/*sid=*/1, /*tid=*/0));
    configs.emplace_back();
    configs.back().S(1).T(0).ReferenceAndUpdate(1);
    return configs;
  }
  // With other sets of active decode targets there is no need to skip patterns.
  configs.reserve(2);
  ...
  switch (next_pattern_) {
    ...
    case kDelta0:
      if (DecodeTargetIsActive(/*sid=*/0, /*tid=*/0)) {
        ...
        spatial_id_is_enabled_.reset(0);
      }
    ...
    case kDelta1:
      ...
      if (DecodeTargetIsActive(/*sid=*/1, /*tid=*/0)) {
        ...
        spatial_id_is_enabled_.reset(1);
      }
      next_pattern_ = kDelta0;
      break;
  }
