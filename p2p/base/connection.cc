...
void Connection::Destroy() {
  ...
  network_thread_->PostTask(webrtc::ToQueuedTask([signaled, me = this]() {
    if (!signaled) {
      me->SignalDestroyed(me);
    } else {
      // TODO(tommi): This is firing in
      // rtc_unittest P2PTransportChannelMultihomedTest.TestVpnOnlyVpn
      // RTC_DCHECK(me->SignalDestroyed.is_empty());
    }
    me->check_in_dtor_ = false;
