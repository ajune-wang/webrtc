...
bool Packet::ExtractRedHeaders(std::list<RTPHeader*>* headers) const {
  ...
  // Last header.
  RTC_DCHECK(payload_ptr < payload_end_ptr);
...
bool Packet::ParseHeader(const RtpHeaderParser& parser,
  ...
  RTC_DCHECK(header_.headerLength <= packet_length_bytes_);
  payload_ = &payload_memory_[header_.headerLength];
  RTC_DCHECK(packet_length_bytes_ >= header_.headerLength);
  ...
  RTC_DCHECK(virtual_packet_length_bytes_ >= header_.headerLength);
