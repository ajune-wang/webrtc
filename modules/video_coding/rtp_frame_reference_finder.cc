...
RtpFrameReferenceFinder::FrameDecision RtpFrameReferenceFinder::ManageFrameVp9(
  ...
  if (codec_header.ss_data_available) {
    ...
    GofInfo new_info(&scalability_structures_[current_ss_idx_],
                     frame->picture_id);
    auto gof_info_it =
        gof_info_.emplace(codec_header.tl0_pic_idx, new_info).first;
    ...
    if (codec_header.temporal_idx == 0) {
      GofInfo new_info(gof_info_it->second.gof, frame->picture_id);
      gof_info_it = gof_info_.emplace(codec_header.tl0_pic_idx, new_info).first;
    }
  ...
  if (codec_header.temporal_up_switch)
    up_switch_.emplace(frame->picture_id, codec_header.temporal_idx);
