...
bool ReadMDnsMessageTestCase(MDnsMessage* msg,
  ...
  info.message_start = reinterpret_cast<const char*>(testcase);
...
int GetNumQuestions(const MDnsMessage& msg) {
  return msg.question_section().size();
}

int GetNumAnswerRecords(const MDnsMessage& msg) {
  return msg.answer_section().size();
}

int GetNumNameServerRecords(const MDnsMessage& msg) {
  return msg.authority_section().size();
}

int GetNumAdditionalRecords(const MDnsMessage& msg) {
  return msg.additional_section().size();
}

bool GetQueriedNames(MDnsMessage* msg, std::set<std::string>* names) {
  if (!msg->IsQuery() || GetNumQuestions(*msg) == 0) {
    return false;
  }
...
bool GetResolution(MDnsMessage* msg,
  ...
  if (msg->IsQuery() || GetNumAnswerRecords(*msg) == 0) {
    return false;
  }
...
TEST(MDnsMessageTest, ReadSingleQuestionForIPv4Address) {
  ...
  EXPECT_EQ(1, GetNumQuestions(msg));
  EXPECT_EQ(0, GetNumAnswerRecords(msg));
  EXPECT_EQ(0, GetNumNameServerRecords(msg));
  EXPECT_EQ(0, GetNumAdditionalRecords(msg));
...
TEST(MDnsMessageTest, ReadTwoQuestionsForIPv4AndIPv6Addr) {
  ...
  EXPECT_TRUE(msg.IsQuery());
  EXPECT_EQ(0x1234, msg.GetId());
  EXPECT_EQ(2, GetNumQuestions(msg));
  EXPECT_EQ(0, GetNumAnswerRecords(msg));
  EXPECT_EQ(0, GetNumNameServerRecords(msg));
  EXPECT_EQ(0, GetNumAdditionalRecords(msg));
  ...
  std::set<std::string> queried_names;
  EXPECT_TRUE(GetQueriedNames(&msg, &queried_names));
  EXPECT_THAT(queried_names, SizeIs(2));
  EXPECT_THAT(queried_names, Contains("webrtc4.org."));
  EXPECT_THAT(queried_names, Contains("webrtc6.org."));
}
...
TEST(MDnsMessageTest, ReadTwoQuestionsForIPv4AndIPv6AddrWithNameCompression) {
  ...
  EXPECT_TRUE(GetQueriedNames(&msg, &queried_names));
  EXPECT_THAT(queried_names, SizeIs(2));
  EXPECT_THAT(queried_names, Contains("www.webrtc.org."));
  EXPECT_THAT(queried_names, Contains("mdns.webrtc.org."));
}
...
TEST(MDnsMessageTest, ReadSingleAnswerForIPv4Addr) {
  ...
  EXPECT_EQ(0x1234, msg.GetId());
  EXPECT_EQ(0, GetNumQuestions(msg));
  EXPECT_EQ(1, GetNumAnswerRecords(msg));
  EXPECT_EQ(0, GetNumNameServerRecords(msg));
  EXPECT_EQ(0, GetNumAdditionalRecords(msg));
...
TEST(MDnsMessageTest, ReadTwoAnswersForIPv4AndIPv6Addr) {
  ...
  EXPECT_EQ(0x1234, msg.GetId());
  EXPECT_EQ(0, GetNumQuestions(msg));
  EXPECT_EQ(2, GetNumAnswerRecords(msg));
  EXPECT_EQ(0, GetNumNameServerRecords(msg));
  EXPECT_EQ(0, GetNumAdditionalRecords(msg));
  ...
  EXPECT_THAT(resolution, SizeIs(2));
  EXPECT_THAT(resolution, Contains(Pair("webrtc4.org.", expected_addr_ipv4)));
  EXPECT_THAT(resolution, Contains(Pair("webrtc6.org.", expected_addr_ipv6)));
}
...
TEST(MDnsMessageTest, ReadTwoAnswersForIPv4AndIPv6AddrWithNameCompression) {
  ...
  EXPECT_THAT(resolution,
              Contains(Pair("www.webrtc.org.", expected_addr_ipv4)));
  EXPECT_THAT(resolution, Contains(Pair("webrtc.org.", expected_addr_ipv6)));
}
