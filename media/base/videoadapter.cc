...
bool VideoAdapter::AdaptFrameResolution(int in_width,
  ...
  if (max_pixel_count <= 0 || !KeepFrame(in_timestamp_ns)) {
    ...
    if ((frames_in_ - frames_out_) % 90 == 0) {
      ...
      // in default calls.
      NLOG(LS_INFO, "VAdapt Drop Frame: scaled ", frames_scaled_, " / out ",
           frames_out_, " / in ", frames_in_, " Changes: ", adaption_changes_,
  ...
  *out_height = *cropped_height / scale.denominator * scale.numerator;
  ...
  if (previous_width_ && (previous_width_ != *out_width ||
    ...
    ++adaption_changes_;
    NLOG(LS_INFO, "Frame size changed: scaled ", frames_scaled_, " / out ",
         frames_out_, " / in ", frames_in_, " Changes: ", adaption_changes_,
         " Input: ", in_width, "x", in_height, " Scale: ", scale.numerator, "/",
         scale.denominator, " Output: ", *out_width, "x", *out_height, " i",
         (requested_format_ ? requested_format_->interval : 0));
  }
