...
TEST(PeerConnectionE2EQualityTestSmokeTest, RunWithEmulatedNetwork) {
  ...
  // 150 = 30fps * 5s, but some frames can be dropped and can be in pipeline
  // when test will be stopped. so assume 10% can be lost.
  EXPECT_GE(video_analyzer->GetGlobalCounters().captured, 150lu);
  EXPECT_GE(video_analyzer->GetGlobalCounters().pre_encoded, 135lu);
  EXPECT_GE(video_analyzer->GetGlobalCounters().encoded, 135lu);
  EXPECT_GE(video_analyzer->GetGlobalCounters().received, 135lu);
  EXPECT_GE(video_analyzer->GetGlobalCounters().decoded, 135lu);
  EXPECT_GE(video_analyzer->GetGlobalCounters().rendered, 135lu);
  video_analyzer->ReportResults("alice-video");
}
