...
constexpr uint32_t kVerificationTag = 0x12345678;
...
TEST(SctpPacketTest, DeserializeSimplePacketFromCapture) {
  ...
  */
  ...
  EXPECT_EQ(packet.common_header().verification_tag, 0u);
  ...
  EXPECT_EQ(init.initial_tsn(), 1426601527u);
}
...
TEST(SctpPacketTest, DeserializePacketWithTwoChunks) {
  ...
  */
  ...
  EXPECT_EQ(packet.common_header().verification_tag, 0x697e3a4eu);
...
TEST(SctpPacketTest, DeserializePacketDontValidateChecksum) {
  ...
  */
  ...
  EXPECT_EQ(packet.common_header().source_port, 5000);
  EXPECT_EQ(packet.common_header().destination_port, 5000);
  EXPECT_EQ(packet.common_header().verification_tag, 0x0eddca08u);
...
TEST(SctpPacketTest, SerializeAndDeserializeSingleChunk) {
  ...
  InitChunk init(/*initiate_tag=*/123, /*a_rwnd=*/456,
...
TEST(SctpPacketTest, SerializeAndDeserializeThreeChunks) {
  SctpPacket::Builder b(kVerificationTag, {});
  b.Add(SackChunk(/*cumulative_tsn_ack=*/999, /*a_rwnd=*/456,
                  {SackChunk::GapAckBlock(2, 3)},
                  /*duplicate_tsns=*/{1, 2, 3}));
  b.Add(DataChunk(/*tsn=*/123, /*stream_id=*/456,
                  /*ssn=*/789, /*ppid=*/9090, /*payload=*/{1, 2, 3, 4, 5},
                  /*options=*/{}));
  b.Add(DataChunk(/*tsn=*/124, /*stream_id=*/654,
                  /*ssn=*/987, /*ppid=*/909, /*payload=*/{5, 4, 3, 3, 1},
                  /*options=*/{}));
