...
class RRSendQueue : public SendQueue {
 public:
  ...
  size_t total_buffered_amount() const override {
    return total_buffered_amount_.amount();
  }
  size_t buffered_amount_low_threshold(StreamID stream_id) const override;
  void SetBufferedAmountLowThreshold(StreamID stream_id, size_t bytes) override;

 private:
  // Represents an amount of buffered data and a "low threshold" that when the
  // `amount()` reaches or goes under `low_threshold()`, will trigger
  // `on_buffered_amount_low` callback.
  class BufferedAmount {
   public:
    explicit BufferedAmount(std::function<void()> on_buffered_amount_low)
        : on_buffered_amount_low_(std::move(on_buffered_amount_low)) {}
    // Increases the amount of buffered data.
    void Increase(size_t bytes) { amount_ += bytes; }
    // Decreases the amount of buffered data and triggers
    // `on_buffered_amount_low` if it's at or below `low_threshold()`.
  ...
  class OutgoingStream {
   public:
    explicit OutgoingStream(std::function<void()> on_buffered_amount_low,
                            BufferedAmount& total_buffered_amount)
    ...
    const BufferedAmount& buffered_amount() const { return buffered_amount_; }
    BufferedAmount& buffered_amount() { return buffered_amount_; }
