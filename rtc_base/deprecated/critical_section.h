...
#ifndef RTC_BASE_DEPRECATED_CRITICAL_SECTION_H_
#define RTC_BASE_DEPRECATED_CRITICAL_SECTION_H_
...
// DEPRECATED_CriticalSections everywhere. DEPRECATED_CriticalSection is
// reentrant lock.
class RTC_LOCKABLE RTC_EXPORT DEPRECATED_CriticalSection {
 public:
  DEPRECATED_CriticalSection();
  ~DEPRECATED_CriticalSection();
...
class RTC_SCOPED_LOCKABLE CritScope {
 public:
  explicit CritScope(const DEPRECATED_CriticalSection* cs)
      RTC_EXCLUSIVE_LOCK_FUNCTION(cs);
  ~CritScope() RTC_UNLOCK_FUNCTION();

 private:
  const DEPRECATED_CriticalSection* const cs_;
  RTC_DISALLOW_COPY_AND_ASSIGN(CritScope);
};

}  // namespace rtc

#endif  // RTC_BASE_DEPRECATED_CRITICAL_SECTION_H_
