...
bool IvfFileWriter::WriteHeader() {
  if (!file_.Seek(0)) {
    NLOG(LS_WARNING, "Unable to rewind ivf output file.");
    return false;
  }
  ...
  switch (codec_type_) {
    ...
    case kVideoCodecH264:
      ...
      break;
    default:
      NLOG(LS_ERROR, "Unknown CODEC type: ", codec_type_);
      return false;
  }
  ...
  if (file_.Write(ivf_header, kIvfHeaderSize) < kIvfHeaderSize) {
    NLOG(LS_ERROR, "Unable to write IVF header for ivf output file.");
    return false;
  }
...
bool IvfFileWriter::InitFromFirstFrame(const EncodedImage& encoded_image,
  ...
  NLOG(LS_WARNING, "Created IVF file for codec data of type ", codec_name,
       " at resolution ", width_, " x ", height_, ", using ",
       (using_capture_timestamps_ ? "1" : "90"), "kHz clock resolution.");
  return true;
}
...
bool IvfFileWriter::WriteFrame(const EncodedImage& encoded_image,
  ...
  if ((encoded_image._encodedWidth > 0 || encoded_image._encodedHeight > 0) &&
    ...
    NLOG(LS_WARNING, "Incomig frame has diffferent resolution then previous: (",
         width_, "x", height_, ") -> (", encoded_image._encodedWidth, "x",
  ...
  if (last_timestamp_ != -1 && timestamp <= last_timestamp_) {
    NLOG(LS_WARNING, "Timestamp no increasing: ", last_timestamp_, " -> ",
         timestamp);
  }
  ...
  if (byte_limit_ != 0 &&
    ...
    NLOG(LS_WARNING,
         "Closing IVF file due to reaching size limit: ", byte_limit_,
         " bytes.");
    Close();
    return false;
  }
  ...
  if (file_.Write(frame_header, kFrameHeaderSize) < kFrameHeaderSize ||
    ...
    NLOG(LS_ERROR, "Unable to write frame to file.");
    return false;
  }
