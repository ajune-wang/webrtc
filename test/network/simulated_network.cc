...
bool SimulatedNetwork::EnqueuePacket(PacketInFlightInfo packet) {
  ...
  // Only update `next_process_time_us_` if not already set (if set,
  // there is no way that a new packet will make the
  // `next_process_time_us_` change).
...
bool SimulatedNetwork::UpdateNextProcessTime() {
  ...
  if (!delay_link_.empty()) {
    next_process_time_ = delay_link_.front().arrival_time;
