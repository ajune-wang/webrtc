...
class FCFSSendQueue : public SendQueue {
 public:
  ...
  // Adds the message to be sent using the `send_options` provided. Note that
  // it's the responsibility of the caller to ensure that the buffer is not full
  // (by calling `IsFull`) before adding messages to it.
  void Add(int64_t now_ms,
           DcSctpMessage message,
           const SendOptions& send_options = {});

  // Methods inherited from SendQueue
  absl::optional<DataToSend> Produce(int64_t now_ms, size_t max_size) override;
 ...
 private:
  ...
  Item* GetFirstNonExpiredMessage(int64_t now_ms);
