    ...
    for (const auto& packet_info : packet_infos) {
      for (const auto& csrc : packet_info.csrcs()) {
        entries_.emplace_front(
            now_ms, csrc, RtpSourceType::CSRC, packet_info.rtp_timestamp(),
            RtpSource::Extensions{packet_info.audio_level(),
                                  packet_info.absolute_capture_time()});
      }

      entries_.emplace_front(
          now_ms, packet_info.ssrc(), RtpSourceType::SSRC,
...
TEST(SourceTrackerTest, OnFrameDeliveredRecordsSources) {
  ...
  EXPECT_THAT(
      tracker.GetSources(),
      ElementsAre(
          RtpSource(timestamp_ms, kSsrc, RtpSourceType::SSRC, kRtpTimestamp,
                    {kAudioLevel, kAbsoluteCaptureTime}),
          RtpSource(timestamp_ms, kCsrcs1, RtpSourceType::CSRC, kRtpTimestamp,
                    {kAudioLevel, kAbsoluteCaptureTime}),
          RtpSource(timestamp_ms, kCsrcs0, RtpSourceType::CSRC, kRtpTimestamp,
                    {kAudioLevel, kAbsoluteCaptureTime})));
}
...
TEST(SourceTrackerTest, OnFrameDeliveredUpdatesSources) {
  ...
  EXPECT_THAT(
      tracker.GetSources(),
      ElementsAre(
          RtpSource(timestamp_ms_1, kSsrc, RtpSourceType::SSRC, kRtpTimestamp1,
                    RtpSource::Extensions{kAudioLevel1, kAbsoluteCaptureTime1}),
          ...
          RtpSource(timestamp_ms_1, kCsrcs0, RtpSourceType::CSRC,
                    ...
                    RtpSource::Extensions{kAudioLevel1, kAbsoluteCaptureTime1}),
          RtpSource(
              timestamp_ms_0, kCsrcs1, RtpSourceType::CSRC, kRtpTimestamp0,
              RtpSource::Extensions{kAudioLevel0, kAbsoluteCaptureTime0})));
}
  ...
  EXPECT_THAT(
      ...
      ElementsAre(
          ...
          RtpSource(timestamp_ms_1, kCsrcs2, RtpSourceType::CSRC,
                    ...
                    RtpSource::Extensions{kAudioLevel1, kAbsoluteCaptureTime1}),
          RtpSource(
              timestamp_ms_1, kCsrcs0, RtpSourceType::CSRC, kRtpTimestamp1,
              RtpSource::Extensions{kAudioLevel1, kAbsoluteCaptureTime1})));
}
