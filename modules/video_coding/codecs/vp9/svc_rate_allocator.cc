...
uint32_t SvcRateAllocator::GetMinBitrateToEncodeActiveLayersBps(
  ...
  if (codec.mode == VideoCodecMode::kRealtimeVideo) {
    ...
    for (size_t sl_idx = 0; sl_idx < num_spatial_layers; ++sl_idx) {
      min_bitrate_bps += codec.spatialLayers[sl_idx].minBitrate * 1000;
      max_bitrate_bps += codec.spatialLayers[sl_idx].maxBitrate * 1000;
    }

    // Use binary search.
    SvcRateAllocator allocator(codec);
    while (max_bitrate_bps > min_bitrate_bps + 1) {
      uint32_t mid_bitrate_bps =
          min_bitrate_bps + (max_bitrate_bps - min_bitrate_bps) / 2;
      VideoBitrateAllocation allocation =
          allocator.GetAllocation(mid_bitrate_bps, 30);
      if (allocation.GetSpatialLayerSum(num_spatial_layers - 1) > 0) {
        max_bitrate_bps = mid_bitrate_bps;
      } else {
        min_bitrate_bps = mid_bitrate_bps;
      }
    }
