...
// TODO(ssilkin): Declare in VideoCodecTestStats?
struct YuvPsnr {
  double y;
  double u;
  double v;
...
YuvPsnr CalcPsnr(const I420BufferInterface& ref_buffer,
...
VideoCodecAnalyzer::VideoCodecAnalyzer(
    ...
    : reference_video_source_(reference_video_source),
      task_queue_("VideoCodecAnalyzer") {
...
void VideoCodecAnalyzer::FinishDecode(const VideoFrame& frame,
  ...
  if (reference_video_source_ != nullptr) {
    // Hardware decoders may have limited number of output buffers
    ...
    rtc::scoped_refptr<I420Buffer> decoded_buffer =
        I420Buffer::Copy(*frame.video_frame_buffer());

    task_queue_.PostTask([this, decoded_buffer,
                          timestamp_rtp = frame.timestamp(), spatial_idx]() {
      RTC_DCHECK_RUN_ON(&sequence_checker_);
      VideoFrame ref_frame = reference_video_source_->GetFrame(timestamp_rtp);
      rtc::scoped_refptr<I420BufferInterface> ref_buffer =
          ref_frame.video_frame_buffer()->ToI420();

      // TODO(ssilkin): Move scaling functionality to video source. We may want
      // to test different downsamplers. It would be convenient to have them all
      // in one place: test.
      ...
      YuvPsnr psnr = CalcPsnr(*decoded_buffer, *ref_buffer);
      VideoCodecTestStats::FrameStatistics* fs =
...
std::unique_ptr<VideoCodecTestStats> VideoCodecAnalyzer::GetStats() {
  ...
  task_queue_.SendTask([this, &stats]() mutable {
    RTC_DCHECK_RUN_ON(&sequence_checker_);
