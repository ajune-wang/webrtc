...
TransportDescription* TransportDescriptionFactory::CreateAnswer(
  ...
  if (!offer) {
    NLOG(LS_WARNING, "Failed to create TransportDescription answer ",
  ...
  if (offer && offer->identity_fingerprint.get()) {
    ...
    NLOG(LS_WARNING,
         "Failed to create TransportDescription answer "
...
bool TransportDescriptionFactory::SetSecurityInfo(
  ...
  if (!certificate_) {
    NLOG(LS_ERROR, "Cannot create identity digest with no certificate");
    return false;
  }
