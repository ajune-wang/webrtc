 ...
 protected:
  RtpRtcpImplTest()
      : clock_(133590000000000),
        sender_(&clock_),
        sender_video_(&clock_,
                      sender_.impl_->rtp_sender(),
                      nullptr,
                      nullptr,
                      false),
        receiver_(&clock_) {}
  ...
  void SendFrame(RTPSenderVideo* sender, uint8_t tid) {
    ...
    EXPECT_EQ(true, sender->SendVideo(kVideoFrameKey, codec_.plType, 0, 0,
...
TEST_F(RtpRtcpImplTest, RetransmitsAllLayers) {
  ...
  SendFrame(&sender_video_, kBaseLayerTid);    // kSequenceNumber
  SendFrame(&sender_video_, kHigherLayerTid);  // kSequenceNumber + 1
  SendFrame(&sender_video_, kNoTemporalIdx);   // kSequenceNumber + 2
...
TEST_F(RtpRtcpImplTest, Rtt) {
  ...
  SendFrame(&sender_video_, kBaseLayerTid);
