...
TEST_P(PeerConnectionIceTest,
       RestartIceWhileOfferIsPendingGeneratesNewCredentialsInNextOffer) {
  auto caller = CreatePeerConnectionWithAudioVideo();
  auto callee = CreatePeerConnectionWithAudioVideo();
...
// signaling state if negotiation is still needed. In Plan B,
// "onnegotiationneeded" fires more frequently, more akin to "something
// changed", which requires some of the tests to expect different things.
...
TEST_F(PeerConnectionIceTestUnifiedPlan,
       RestartIceWhileOfferIsPendingCausesNegotiationNeededWhenStable) {
  auto caller = CreatePeerConnectionWithAudioVideo();
  auto callee = CreatePeerConnectionWithAudioVideo();
...
// something changed rather than if negotiation is needed. As such, some test
// expectations are different in Plan B versus spec-compliant Unified Plan.
