...
class PeerConnectionInterface : public rtc::RefCountInterface {
 public:
  ...
  struct RTCConfiguration {
    ...
    // most one audio and one video m= section with multiple streams specified
    // as multiple a=ssrc lines within the section. This will also cause
    // PeerConnection to reject offers/answers with multiple m= sections of the
    // same media type.
    //
    // kUnifiedPlan will cause PeerConnection to create offers and answers with
    // multiple m= sections where each m= section maps to either an audio or
    // video stream. Plan B style offers or answers will be rejected in calls to
  ...
  // TODO(steveanton): Default implementations provided to avoid breaking
  // Chromium.
  ...
  // Adds a transceiver with the given kind. Can either be 'audio' or 'video'.
  // Errors:
  // - INVALID_PARAMETER: |kind| is not 'audio' or 'video'.
  virtual RTCErrorOr<rtc::scoped_refptr<RtpTransceiverInterface>>
  AddTransceiver(const std::string& kind) {
    LOG_AND_RETURN_ERROR(RTCErrorType::INTERNAL_ERROR, "not implemented");
  }
  virtual RTCErrorOr<rtc::scoped_refptr<RtpTransceiverInterface>>
  AddTransceiver(const std::string& kind, const RtpTransceiverInit& init) {
    LOG_AND_RETURN_ERROR(RTCErrorType::INTERNAL_ERROR, "not implemented");
  }
