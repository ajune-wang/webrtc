...
class VideoReceiveStream2
 ...
 private:
  void OnFrameWithRtpTimestampReady(uint32_t rtp_timestamp,
                                    Timestamp render_time);
  void OnEncodedFrameReadyForDecoding(
  ...
  // TODO(eshr): This should instead mark that there is an outstanding decode.
  ...
  rtc::scoped_refptr<PendingTaskSafetyFlag> decode_safety_
      RTC_GUARDED_BY(decode_queue_) = PendingTaskSafetyFlag::CreateDetached();
};
