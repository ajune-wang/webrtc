...
void SetupInternalTracer(bool enable_all_categories) {
  RTC_LOG(LS_WARNING)
      << "Internal tracer is not supported with Perfetto. Instead, use the "
         "official Perfetto tracing APIs and backend in your application.";
}
...
class EventLogger final {
 ...
 private:
  struct TraceArg {
    ...
    union TraceArgValue {
      bool as_bool;
      unsigned long long as_uint;  // NOLINT(runtime/int)
    ...
    static_assert(sizeof(TraceArgValue) ==
      ...
      switch (arg.type) {
        case TRACE_VALUE_TYPE_BOOL:
          if (arg.value.as_bool) {
            strcpy(&output[0], "true");  // NOLINT(runtime/printf)
            print_length = 4;
          } else {
            strcpy(&output[0], "false");  // NOLINT(runtime/printf)
            print_length = 5;
          }
          break;
