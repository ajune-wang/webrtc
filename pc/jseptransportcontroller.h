...
class JsepTransportController : public sigslot::has_slots<>,
 ...
 public:
  ...
  std::string bundled_mid() const {
    return bundle_group_ ? *(bundle_group_->FirstContentName()) : "";
  }
  ...
  // the signal and set the RtpTransport of the BaseChannel.
  sigslot::signal2<const std::string&, RtpTransportInternal*>
      SignalRtpTransportChanged;

  // This will be fired when BUNDLE is enabled, the PeerConnection will handle
  // the signal and set the DtlsTransport of the SctpTransport.
 ...
 private:
  ...
  // Called when the m= section is rejected or the BUNDLE is enabled. Notify the
  // peerconnection to update the RtpTransport/DtlsTransport of the
  // BaseChannel/SctpTransport on the signaling thread.
  void SignalRtpTransportOrDtlsTransportChanged(
      const cricket::ContentInfo& content_info);
