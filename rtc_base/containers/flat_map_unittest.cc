...
TEST(FlatMap, CopySwap) {
  webrtc::flat_map<int, int> original;
  original.insert({1, 1});
  original.insert({2, 2});
  EXPECT_THAT(original,
              ElementsAre(std::make_pair(1, 1), std::make_pair(2, 2)));

  webrtc::flat_map<int, int> copy(original);
...
TEST(FlatMap, SubscriptConstKey) {
  webrtc::flat_map<std::string, int> m;
...
TEST(FlatMap, SubscriptMoveOnlyKey) {
  webrtc::flat_map<MoveOnlyInt, int> m;
...
TEST(FlatMap, AtFunction) {
  webrtc::flat_map<int, std::string> m = {{1, "a"}, {2, "b"}};
  ...
  webrtc::flat_map<std::string, int> m2 = {{"a", 1}, {"b", 2}};
...
TEST(FlatMap, InsertOrAssignMoveOnlyKey) {
  webrtc::flat_map<MoveOnlyInt, MoveOnlyInt> m;
  ...
  webrtc::flat_map<MoveOnlyInt, int> map;
...
TEST(FlatMap, TryEmplaceMoveOnlyKey) {
  webrtc::flat_map<MoveOnlyInt, std::pair<MoveOnlyInt, MoveOnlyInt>> m;
...
TEST(FlatMap, UsingTransparentCompare) {
  ...
  webrtc::flat_map<ExplicitInt, int> m;
  const auto& m1 = m;
  int x = 0;
