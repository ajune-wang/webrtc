...
TEST_F(TestVp8Impl, MAYBE_AlignedStrideEncodeDecode) {
  SetupInputFrame();
  input_frame_->set_timestamp(kInitialTimestampRtp);
  EncodedImage encoded_frame;
  CodecSpecificInfo codec_specific_info;
  EncodeAndWaitForFrame(&encoded_frame, &codec_specific_info);
...
TEST_F(TestVp8Impl, EncoderWith2TemporalLayersRetainsRtpStateAfterRelease) {
  ...
  int16_t picture_id = codec_specific_info.codecSpecific.VP8.pictureId;
  int tl0_pic_idx = codec_specific_info.codecSpecific.VP8.tl0PicIdx;
  // Temporal layer 1.
