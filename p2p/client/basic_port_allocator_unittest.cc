...
TEST_F(BasicPortAllocatorTest, TestCreateIceGathererForForking) {
  ...
  EXPECT_EQ(allocator_->turn_port_prune_policy(),
            forked->turn_port_prune_policy());
  EXPECT_EQ(allocator_->stun_candidate_keepalive_interval(),
            forked->stun_candidate_keepalive_interval());
}
