...
void AudioSendStream::SendAudioData(std::unique_ptr<AudioFrame> audio_frame) {
                    ...
                    audio_frame->sample_rate_hz_;
  {
    ...
    // data such as if we are disconnected. On this stage in the pipeline we are
    // concerned with the input audio to the send stream, not necessarily with
    // the samples sent.
    // TODO(https://crbug.com/webrtc/10771): All "media-source" related stats
    // should move from senders and send-streams to the local audio sources or
    // tracks; a send-stream should not be required to read the microphone audio
    // levels.
