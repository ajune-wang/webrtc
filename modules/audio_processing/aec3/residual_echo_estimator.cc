...
void GetRenderIndexesToAnalyze(
  ...
  if (!gain_limiter_running) {
    ...
    *idx_stop =
    ...
    if ((size_t)headroom > echo_model.render_post_window_size_init) {
      *idx_start = spectrum_buffer.OffsetIndex(
          spectrum_buffer.read, -echo_model.render_post_window_size_init);
    } else {
      *idx_start = spectrum_buffer.write;
    }
...
void ResidualEchoEstimator::EchoGeneratingPower(
  ...
  X2->fill(0.f);
  for (size_t k = idx_start; k != idx_stop; k = spectrum_buffer.IncIndex(k)) {
