...
class TestVp9ImplRateControlDrops : public TestVp9Impl {
 protected:
  void SetUp() override {
    TestVp9Impl::SetUp();

    // Recreate frame generator to make difficult to encode frames.
    input_frame_generator_ = test::FrameGenerator::CreateFromYuvFile(
        {test::ResourcePath("difficult_photo_1850_1110", "yuv")},
        codec_settings_.width, codec_settings_.height, 1);
  }
};

TEST_F(TestVp9ImplRateControlDrops, ScreenshareFrameDropping) {
