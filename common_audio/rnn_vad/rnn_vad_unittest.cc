...
TEST(RnnVad, CheckValidVadProbabilities) {
  ...
  std::array<float, kFrameSize10ms24kHz> samples_decimated;
  ...
  for (size_t i = 0; i < num_frames; ++i) {
    ...
    std::memcpy(&prefetched_decimated_samples[i * kFrameSize10ms24kHz],
                samples_decimated.data(), kFrameSize10ms24kHz * sizeof(float));
  }
