...
void PeerConnectionE2EQualityTest::Run(RunParams run_params) {
  ...
  SetDefaultValuesForMissingParams(
      {ValidatableParams(alice_params.get(), &alice_video_sources),
       ValidatableParams(bob_params.get(), &bob_video_sources)});
  ValidateParams(run_params,
                 {ValidatableParams(alice_params.get(), &alice_video_sources),
                  ValidatableParams(bob_params.get(), &bob_video_sources)});
...
void PeerConnectionE2EQualityTest::SetDefaultValuesForMissingParams(
  ...
  for (auto& p : params) {
    for (size_t i = 0; i < p.params->video_configs.size(); ++i) {
      VideoConfig& video_config = p.params->video_configs[i];
    ...
    if (p.params->audio_config) {
      if (!p.params->audio_config->stream_label) {
        std::string label;
        do {
          label = "_auto_audio_stream_label_" + std::to_string(audio_counter);
          ++audio_counter;
        } while (!audio_labels.insert(label).second);
        p.params->audio_config->stream_label = label;
      }
    }
  }
}
