...
TEST(LibaomAv1Encoder, ResolutionSwitching) {
  ...
  EXPECT_TRUE(enc->Encode(
      in0, {}, {Fb().Rate(kCbr).Res(320, 180).Key().Build()}, res.CallBack()));
...
TEST(LibaomAv1Encoder, KeyframeAndStartrameAreApproximatelyEqual) {
  ...
  for (int sid = 0; sid < max_spatial_layers; ++sid) {
    ...
    ASSERT_TRUE(enc_key->Encode(
        frame_in, {}, {Fb().Rate(kRate).Res(640, 360).S(sid).Key().Build()},
        res_key.CallBack()));
    ASSERT_TRUE(enc_start->Encode(
        frame_in, {},
