...
static constexpr int kDefaultJitterEstimateUpperBoundMs = 250;
}  // namespace
...
VCMJitterEstimator::VCMJitterEstimator(const Clock* clock,
    ...
    : _vcmId(vcmId),
      ...
      jitter_upper_bound_ms_(JitterUpperBoundExperiment::GetUpperBoundMs()),
      clock_(clock) {
  Reset();
}
...
int VCMJitterEstimator::GetJitterEstimate(double rttMultiplier) {
  ...
  const int jitter_upper_bound =
      jitter_upper_bound_ms_.value_or(kDefaultJitterEstimateUpperBoundMs);
  ...
  if (fps < kJitterScaleLowThreshold) {
    if (fps == 0.0) {
      return std::min(jitter_upper_bound,
                      rtc::checked_cast<int>(std::max(0.0, jitterMS) + 0.5));
    }
    return 0;
  }
