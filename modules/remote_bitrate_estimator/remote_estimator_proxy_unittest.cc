...
TEST_F(RemoteEstimatorProxyTest, OutOfOrderPacketReportsWithSameSendTime) {
  // abs send time use 24bit precision.
  const uint32_t kFirstAbsSendTime =
      AbsoluteSendTime::To24Bits(Timestamp::Millis(40'000'15));
  // Second abs send time has wrapped.
  const uint32_t kSecondAbsSendTime =
      AbsoluteSendTime::To24Bits(Timestamp::Millis(40'000'30));
  const uint32_t kThirdAbsSendTime =
      AbsoluteSendTime::To24Bits(Timestamp::Millis(40'000'45));
...
TEST_F(RemoteEstimatorProxyTest, AdjustAbsSendTimeAfterUnapusing) {
  const uint32_t kAbsSendTimeBeforePause =
      AbsoluteSendTime::To24Bits(Timestamp::Millis(15));
  // abs send time use 24bit precision.
  const uint32_t kFirstAbsSendTime =
      AbsoluteSendTime::To24Bits(Timestamp::Millis(40'000'15));
  // Second abs send time has wrapped.
  ...
  Timestamp send_timestamp_after_unpause = Timestamp::Zero();
  EXPECT_CALL(network_state_estimator_, OnReceivedPacket)
      .WillOnce([&](const PacketResult& packet) {
        // Do nothing before pause.
      })
      .WillOnce([&](const PacketResult& packet) {
        send_timestamp_after_unpause = packet.sent_packet.send_time;
      })
      .WillOnce([&](const PacketResult& packet) {
        EXPECT_EQ(
            (packet.sent_packet.send_time - send_timestamp_after_unpause).ms(),
            15);
      });
  ...
  proxy_.IncomingPacket({.arrival_time = kBaseTime,
                         .size = kDefaultPacketSize,
                         .ssrc = kMediaSsrc,
                         .absolute_send_time_24bits = kAbsSendTimeBeforePause,
                         .transport_sequence_number = kBaseSeq + 1});
  proxy_.IncomingPacket(
      {.arrival_time = kBaseTime + TimeDelta::Millis(40'000'10),
       .size = kDefaultPacketSize,
       .ssrc = kMediaSsrc,
       .absolute_send_time_24bits = kFirstAbsSendTime,
       .transport_sequence_number = kBaseSeq + 2});
  proxy_.IncomingPacket(
      {.arrival_time = kBaseTime + TimeDelta::Millis(40'000'20),
       .size = kDefaultPacketSize,
       .ssrc = kMediaSsrc,
       .absolute_send_time_24bits = kSecondAbsSendTime,
       .transport_sequence_number = kBaseSeq + 3});
