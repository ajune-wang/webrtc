...
bool FeedbackParam::operator==(const FeedbackParam& other) const {
  return rtc::ascicmp(other.id().c_str(), id().c_str()) == 0 &&
         rtc::ascicmp(other.param().c_str(), param().c_str()) == 0;
}
...
bool Codec::Matches(const Codec& codec) const {
  ...
  return (id <= kMaxStaticPayloadId || codec.id <= kMaxStaticPayloadId)
             ? (id == codec.id)
             : (rtc::ascicmp(name.c_str(), codec.name.c_str()) == 0);
}
...
void VideoCodec::SetDefaultParameters() {
  if (rtc::ascicmp(kH264CodecName, name.c_str()) == 0) {
...
VideoCodec::CodecType VideoCodec::GetCodecType() const {
  ...
  if (rtc::ascicmp(payload_name, kRedCodecName) == 0) {
    return CODEC_RED;
  }
  if (rtc::ascicmp(payload_name, kUlpfecCodecName) == 0) {
    return CODEC_ULPFEC;
  }
  if (rtc::ascicmp(payload_name, kFlexfecCodecName) == 0) {
    return CODEC_FLEXFEC;
  }
  if (rtc::ascicmp(payload_name, kRtxCodecName) == 0) {
    return CODEC_RTX;
  }
