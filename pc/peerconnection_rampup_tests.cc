...
// The video's configured max bitrate in webrtcvideoengine.cc is 1.7 Mbps, while
// the the encoder can get a max throughput of around 1 Mbps using the
// FrameGeneratorCapturer (SquareGenerator) with the video configuration set
// for this test. Setting the network bandwidth at 4/3 of 1 Mbps allows the
// video's bitrate to push the network's limitations.
static const int kHighNetworkBandwidth = 1000000 * (4 / 3);
}  // namespace
...
class PeerConnectionRampUpTest : public ::testing::Test {
 public:
  PeerConnectionRampUpTest()
    ...
    virtual_socket_server_->set_bandwidth(kHighNetworkBandwidth / 8);
