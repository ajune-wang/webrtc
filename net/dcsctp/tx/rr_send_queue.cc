...
bool RRSendQueue::OutgoingStream::IsConsistent() const {
  ...
  return bytes == buffered_amount_.amount();
}

void RRSendQueue::BufferedAmount::Decrease(size_t bytes) {
  RTC_DCHECK(bytes <= amount_);
  size_t old_buffered_amount = amount_;
  amount_ -= bytes;

  // Maybe trigger `on_buffered_amount_low_`.
  if (old_buffered_amount > low_threshold_ && amount_ <= low_threshold_) {
...
size_t RRSendQueue::buffered_amount(StreamID stream_id) const {
  ...
  return it->second.buffered_amount().amount();
}
