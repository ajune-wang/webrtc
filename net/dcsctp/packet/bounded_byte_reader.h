...
inline uint8_t LoadBigEndian8(const uint8_t* data) { return data[0]; }
...
class BoundedByteReader {
 public:
  ...
  uint8_t Load8() const {
    ...
    return LoadBigEndian8(&data_[offset]);
  }
  ...
  uint16_t Load16() const {
    ...
    return LoadBigEndian16(&data_[offset]);
  }
  ...
  uint32_t Load32() const {
    ...
    return LoadBigEndian32(&data_[offset]);
  }
