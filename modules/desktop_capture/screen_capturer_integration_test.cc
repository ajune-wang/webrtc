 ...
 protected:
  void TestCaptureUpdatedRegion(
    ...
    if (!drawer || drawer->DrawableRegion().is_empty()) {
      RTC_LOG(LS_WARNING) << "No ScreenDrawer implementation for current "
                          << "platform.";
      return;
    }
    if (drawer->DrawableRegion().width() < kTestArea ||
        drawer->DrawableRegion().height() < kTestArea) {
      RTC_LOG(LS_WARNING) << "ScreenDrawer::DrawableRegion() is too small for "
                          << "the CaptureUpdatedRegion tests.";
      return;
    }
