...
void DxgiDuplicatorController::Release() {
  ...
  if (refcount == 0) {
    NLOG(LS_WARNING,
         "Count of references reaches zero, "
...
bool DxgiDuplicatorController::RetrieveD3dInfo(D3dInfo* info) {
  ...
  if (!result) {
    NLOG(LS_WARNING,
         "Failed to initialize DXGI components, the D3dInfo "
...
DxgiDuplicatorController::DoDuplicate(DxgiFrame* frame, int monitor_id) {
  ...
  if (!Initialize()) {
    if (succeeded_duplications_ == 0 && !IsCurrentSessionSupported()) {
      NLOG(LS_WARNING,
           "Current binary is running in session 0. DXGI "
...
bool DxgiDuplicatorController::DoInitialize() {
  ...
  if (devices.empty()) {
    NLOG(LS_WARNING, "No D3dDevice found.");
    return false;
  }
  ...
  for (size_t i = 0; i < devices.size(); i++) {
    ...
    if (!duplicator.Initialize()) {
      NLOG(LS_WARNING,
           "Failed to initialize DxgiAdapterDuplicator on "
           "adapter ",
           i);
      continue;
    }
  ...
  if (duplicators_.empty()) {
    NLOG(LS_WARNING, "Cannot initialize any DxgiAdapterDuplicator instance.");
  }
...
bool DxgiDuplicatorController::EnsureFrameCaptured(Context* context,
  ...
  while (GetNumFramesCaptured() < frames_to_skip) {
    ...
    if (rtc::TimeMillis() - start_ms > timeout_ms) {
      NLOG(LS_ERROR, "Failed to capture ", frames_to_skip,
           " frames "
           "within ",
