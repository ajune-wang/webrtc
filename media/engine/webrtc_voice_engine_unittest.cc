...
TEST_F(WebRtcVoiceEngineTestFake, TxAgcConfigViaOptions) {
  ...
  send_parameters_.options.tx_agc_target_dbov = 1;
  send_parameters_.options.tx_agc_digital_compression_gain = 5;
  send_parameters_.options.tx_agc_limiter = false;
  send_parameters_.options.auto_gain_control = true;
  SetSendParameters(send_parameters_);
  VerifyGainControlEnabledCorrectly();
  EXPECT_EQ(1, apm_config_.gain_controller1.target_level_dbfs);
  EXPECT_EQ(5, apm_config_.gain_controller1.compression_gain_db);
