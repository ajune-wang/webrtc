...
void ReceiveStatisticsProxy::OnDecodedFrame(const VideoFrame& frame,
  ...
  if (frame.packet_infos().size() > 0) {
    ...
    int64_t current_time_ms = clock_->CurrentTime().ms();
    processing_delay_ms =
        current_time_ms - first_packet->receive_time().ms_or(current_time_ms);
  }
  ...
  worker_thread_->PostTask(ToQueuedTask(task_safety_, [meta, qp, decode_time_ms,
                                                       processing_delay_ms,
                                                       content_type, this]() {
    OnDecodedFrame(meta, qp, decode_time_ms, processing_delay_ms, content_type);
  }));
}
