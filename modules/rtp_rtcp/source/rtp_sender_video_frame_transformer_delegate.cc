...
bool RTPSenderVideoFrameTransformerDelegate::TransformFrame(
  ...
  // DCHECK that the encoder queue does not change.
  RTC_DCHECK(!current || current == encoder_queue_)
      << "Current thread must either be an external thread (nullptr) or be the "
         "same as the previous encoder queue. The current thread is at "
      << current << " and the encoder queue is at " << encoder_queue_;
