...
class ChannelTest : public testing::Test, public sigslot::has_slots<> {
 public:
  ...
  void CreateChannels(std::unique_ptr<typename T::MediaChannel> ch1,
    ...
    // Enable the RTCP-MUX if it is required.
    ...
    if (!(flags1 & SDES) || !(flags1 & DTLS)) {
      channel1_->SetEncryptionDisabled(true);
    }
    if (!(flags2 & SDES) || !(flags2 & DTLS)) {
  ...
  std::unique_ptr<webrtc::RtpTransportInternal> CreateRtpTransportBasedOnFlags(
    ...
    if (flags & DTLS) {
      ...
      if (flags & RAW_PACKET_TRANSPORT) {
        return CreateSdesTransport(rtp_packet_transport, rtcp_packet_transport);
      } else {
        return CreateSdesTransport(rtp_dtls_transport, rtcp_dtls_transport);
      }
    } else {
      if (flags & RAW_PACKET_TRANSPORT) {
  ...
  std::unique_ptr<webrtc::SrtpTransport> CreateSdesTransport(
    ...
    if (rtcp_packet_transport) {
      srtp_transport->SetRtcpPacketTransport(rtcp_packet_transport);
    }
  ...
  typename T::Content* CreateMediaContentWithStream(uint32_t ssrc) {
    ...
    CreateContent(SDES, kPcmuCodec, kH264Codec, content);
  ...
  void SocketOptionsMergedOnSetTransport() {
    ...
    new_rtp_transport_ = CreateDtlsSrtpTransport(
        channel2_->rtp_dtls_transport(), channel2_->rtcp_dtls_transport());
...
std::unique_ptr<cricket::VideoChannel> ChannelTest<VideoTraits>::CreateChannel(
  ...
  auto channel = rtc::MakeUnique<cricket::VideoChannel>(
      ...
      cricket::CN_VIDEO, (flags & RTCP_MUX_REQUIRED) != 0, (flags & SDES) != 0);
  channel->Init_w(rtp_transport);
  return channel;
}
...
std::unique_ptr<cricket::RtpDataChannel> ChannelTest<DataTraits>::CreateChannel(
  ...
  auto channel = rtc::MakeUnique<cricket::RtpDataChannel>(
      worker_thread, network_thread, signaling_thread, std::move(ch),
      cricket::CN_DATA, (flags & RTCP_MUX_REQUIRED) != 0, (flags & SDES) != 0);
  channel->Init_w(rtp_transport);
  return channel;
}
