...
int32_t EncoderManager::Encode(const VideoFrame& frame,
  ...
  for (size_t i = 0; i < frame_types->size(); ++i) {
    ...
    for (const GenericFrameInfo& info : frame_infos) {
      ...
      frames.push_back(std::move(config));
    }
    ...
    encoding.encoder->Encode(
        frame, std::move(frames), [&](EncodedFrameLight encoded_frame) {
          ...
          auto it = absl::c_find_if(frame_infos, [&](const auto& info) {
            return info.frame_id == encoded_frame.id;
          });
          RTC_CHECK(it != frame_infos.end());
          codec_specific_info.generic_frame_info = *it;
