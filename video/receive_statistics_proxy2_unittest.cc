...
TEST_P(ReceiveStatisticsProxy2TestWithContent, FreezesAreReported) {
  ...
  const Frequency kExpectedNumberFreezesPerMinute = 60 / kCallDuration;
  if (videocontenttypehelpers::IsScreenshare(content_type_)) {
    EXPECT_METRIC_EQ(
    ...
    EXPECT_METRIC_EQ(
        kExpectedNumberFreezesPerMinute.hertz(),
    ...
    EXPECT_METRIC_EQ(kExpectedNumberFreezesPerMinute.hertz(),
