...
class PerfResultsLogger {
 ...
 private:
  ...
  std::vector<PlotableCounter> plottable_counters_ RTC_GUARDED_BY(&crit_);
};
...
void PrintPlotableResults(const std::vector<std::string>& desired_graphs) {
  GetPerfResultsLogger().PrintPlotableCounters(desired_graphs);
}
