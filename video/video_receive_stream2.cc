...
constexpr TimeDelta kMaxBaseMinimumDelay = TimeDelta::Millis(10000);

constexpr TimeDelta kMaxWaitForFrame = TimeDelta::Millis(3000);
...
constexpr TimeDelta kInactiveStreamThreshold =
    TimeDelta::Millis(600000);  //  10 minutes.
...
void VideoReceiveStream2::OnCompleteFrame(std::unique_ptr<EncodedFrame> frame) {
  ...
  Timestamp time_now_ms = clock_->CurrentTime();
  if (last_complete_frame_time_ &&
      time_now_ms - *last_complete_frame_time_ > kInactiveStreamThreshold) {
    frame_buffer_->Clear();
  }
  last_complete_frame_time_ = time_now_ms;
...
TimeDelta VideoReceiveStream2::GetMaxWaitMs() const {
...
void VideoReceiveStream2::UpdatePlayoutDelays() const {
  // Running on worker_sequence_checker_.
  constexpr TimeDelta kDefaultDelay = TimeDelta::Millis(-1);
  const TimeDelta minimum_delay =
      std::max({frame_minimum_playout_delay_.value_or(kDefaultDelay),
                base_minimum_playout_delay_.value_or(kDefaultDelay),
                syncable_minimum_playout_delay_.value_or(kDefaultDelay)});
  if (minimum_delay >= TimeDelta::Zero()) {
    ...
    if (frame_minimum_playout_delay_ == TimeDelta::Zero() &&
      ...
      int max_composition_delay_in_frames = std::lrint(
          static_cast<double>(frame_maximum_playout_delay_->ms() * kFrameRate) /
