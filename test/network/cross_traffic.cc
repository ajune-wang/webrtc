...
void TcpMessageRoute::SendAction(size_t message_size,
                                 std::function<void()> on_received) {
  task_queue_->PostTask(
      ToQueuedTask([this, message_size, handler = std::move(on_received)] {
        if (pending_.empty() && in_flight_.empty()) {
          cwnd_ = 10;
          ssthresh_ = INFINITY;
        }
        size_t data_left = message_size;
        size_t kMaxPacketSize = 1200;
        TcpSession session{std::move(handler)};
        while (data_left > 0) {
          size_t packet_size = std::min(data_left, kMaxPacketSize);
          int fragment_id = next_fragment_id_++;
          pending_.push_back(TcpFragment{fragment_id, packet_size});
          session.pending.insert(fragment_id);
...
void TcpMessageRoute::OnRequest(TcpPacket packet_info) {
  for (auto it = sessions_.begin(); it != sessions_.end(); ++it) {
    if (it->pending.count(packet_info.fragment.fragment_id) != 0) {
      it->pending.erase(packet_info.fragment.fragment_id);
      if (it->pending.empty()) {
        it->handler();
        sessions_.erase(it);
      }
...
void TcpMessageRoute::OnResponse(TcpPacket packet_info, Timestamp at_time) {
  ...
  if (it != in_flight_.end()) {
    last_rtt_ = at_time - packet_info.sent;
    in_flight_.erase(it);
  }
