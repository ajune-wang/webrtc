...
TEST_F(P2PTransportChannelPingTest, Forking) {
  ...
  rtc::ScopedFakeClock clock;
  FakePortAllocator pa1(rtc::Thread::Current(), nullptr);
  auto transport1 =
      std::make_unique<P2PTransportChannel>("transport1", 1, &pa1);
  PrepareChannel(transport1.get());

  FakePortAllocator pa2(rtc::Thread::Current(), nullptr);
  auto transport2 =
      std::make_unique<P2PTransportChannel>("transport2", 1, &pa2);
  PrepareChannel(transport2.get());

  auto gatherer = transport1->CreateGatherer();

  // Start the first
  EXPECT_EQ(IceGatheringState::kIceGatheringNew, transport1->gathering_state());
  transport1->StartGathering(gatherer);
  SIMULATED_WAIT(
  ...
  EXPECT_EQ(kIceParams[0].ufrag, transport1_conn->local_candidate().username());
  EXPECT_EQ(kIceParams[0].pwd, transport1_conn->local_candidate().password());
  ...
  transport2->StartGathering(gatherer);
  SIMULATED_WAIT(
      IceGatheringState::kIceGatheringComplete == transport1->gathering_state(),
      kShortTimeout, clock);
  ...
  EXPECT_EQ(kIceParams[0].ufrag, transport2_conn->local_candidate().username());
  EXPECT_EQ(kIceParams[0].pwd, transport2_conn->local_candidate().password());
  ...
  // Destroy the second and make sure things don't blow up.
  transport2.reset();
  gatherer->session()->SignalCandidatesAllocationDone(gatherer->session());
  // Make sure destroying the transport1 doesn't close the gatherer's ports.
  EXPECT_TRUE(gatherer->session()->IsGettingPorts());
}
