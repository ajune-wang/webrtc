...
class Encoder {
 public:
  ...
  bool WaitForExpectedFramesReceived(int timeout) {
    return ivf_writer_callback_->WaitForExpectedFramesReceived(timeout);
  }
...
int GetFrameCount(std::string yuv_file_name, int width, int height) {
  std::unique_ptr<FrameReader> yuv_reader =
      std::make_unique<YuvFrameReaderImpl>(yuv_file_name, width, height);
...
void WriteVideoFile(std::string input_file_name,
  ...
  RTC_CHECK(
      encoder.WaitForExpectedFramesReceived(kMaxFrameEncodeWaitTimeoutMs));
}
