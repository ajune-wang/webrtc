...
class BaseChannel : public rtc::MessageHandler,
 ...
 public:
  ...
  DtlsTransportInternal* rtp_dtls_transport() const {
    return static_cast<DtlsTransport*>(rtp_transport_->rtp_packet_transport());
  }
  DtlsTransportInternal* rtcp_dtls_transport() const {
    return static_cast<DtlsTransport*>(rtp_transport_->rtcp_packet_transport());
  }
 ...
 protected:
  ...
  void UnregisterRtpDemuxerSink();
  void RegisterRtpDemuxerSink();
  ...
  bool ParseRtpPacket(rtc::CopyOnWriteBuffer packet,
                      const rtc::PacketTime& packet_time,
 ...
 private:
  ...
  webrtc::RtpDemuxerCriteria demux_criteria_;
