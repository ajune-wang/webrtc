...
int ScaleDownResolution(int resolution,
  ...
  if (resolution <= min_resolution)
    return resolution;
  // kEpsilon is added below to compensate for possible rounding errors.
  // |resolution/scale_down_by| is supposed to be exactly equal to an
  // integer.
  const double kEpsilon = 1e-9;
  return std::max(static_cast<int>(resolution / scale_down_by + kEpsilon),
                  min_resolution);
}
