...
void VideoSendStream::UpdateActiveSpatialLayers(
    const rtc::ArrayView<const VideoStream> layers) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  RTC_LOG(LS_INFO) << "VideoSendStream::UpdateActiveSpatialLayers";
  VideoSendStreamImpl* send_stream = send_stream_.get();
  worker_queue_->PostTask([this, send_stream, layers] {
    send_stream->UpdateActiveSpatialLayers(layers);
...
void VideoSendStreamImpl::UpdateActiveSpatialLayers(
    const rtc::ArrayView<const VideoStream> layers) {
  RTC_DCHECK_RUN_ON(worker_queue_);
