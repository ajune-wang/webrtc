...
void MidToTransportMap::RegisterTransport(
    const std::string& mid,
...
std::vector<cricket::JsepTransport*> MidToTransportMap::Transports() {
  RTC_DCHECK_RUN_ON(&sequence_checker_);
...
void MidToTransportMap::DestroyAllTransports() {
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  for (const auto& jsep_transport : jsep_transports_by_name_) {
    observer_callback_(jsep_transport.first, nullptr, nullptr, nullptr);
  }
  jsep_transports_by_name_.clear();
}

const cricket::JsepTransport* MidToTransportMap::GetTransportByName(
...
cricket::JsepTransport* MidToTransportMap::GetTransportByName(
...
cricket::JsepTransport* MidToTransportMap::GetTransportForMid(
    const std::string& mid) {
  RTC_DCHECK_RUN_ON(&sequence_checker_);
...
const cricket::JsepTransport* MidToTransportMap::GetTransportForMid(
...
bool MidToTransportMap::SetTransportForMid(
    const std::string& mid,
  ...
  return observer_callback_(mid, jsep_transport->rtp_transport(),
                            jsep_transport->RtpDtlsTransport(),
                            jsep_transport->data_channel_transport());
}

void MidToTransportMap::RemoveTransportForMid(const std::string& mid) {
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  bool ret = observer_callback_(mid, nullptr, nullptr, nullptr);
...
void MidToTransportMap::RollbackTransports() {
  RTC_DCHECK_RUN_ON(&sequence_checker_);
...
void MidToTransportMap::CommitTransports() {
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  pending_mids_.clear();
}

bool MidToTransportMap::TransportInUse(
...
void MidToTransportMap::MaybeDestroyJsepTransport(const std::string& mid) {
  RTC_DCHECK_RUN_ON(&sequence_checker_);
