...
rtc::scoped_refptr<AudioDeviceModuleForTest> AudioDeviceModule::CreateForTest(
  ...
  if (audio_layer == AudioDeviceModule::kWindowsCoreAudio2) {
    RTC_DLOG(LS_ERROR) << "Use the CreateWindowsCoreAudioAudioDeviceModule() "
...
int32_t AudioDeviceModuleImpl::CheckPlatform() {
  ...
  RTC_DLOG(INFO) << "current platform is Win32";
#elif defined(WEBRTC_ANDROID)
  platform = kPlatformAndroid;
  RTC_DLOG(INFO) << "current platform is Android";
#elif defined(WEBRTC_LINUX)
  platform = kPlatformLinux;
  RTC_DLOG(INFO) << "current platform is Linux";
#elif defined(WEBRTC_IOS)
  platform = kPlatformIOS;
  RTC_DLOG(INFO) << "current platform is IOS";
#elif defined(WEBRTC_MAC)
  platform = kPlatformMac;
  RTC_DLOG(INFO) << "current platform is Mac";
#endif
...
// Windows ADM implementation.
  ...
  if ((audio_layer == kWindowsCoreAudio) ||
    ...
    RTC_DLOG(INFO) << "Attempting to use the Windows Core Audio APIs...";
    if (AudioDeviceWindowsCore::CoreAudioIsSupported()) {
      audio_device_.reset(new AudioDeviceWindowsCore());
      RTC_DLOG(INFO) << "Windows Core Audio APIs will be utilized";
    }
  }
  ...
  if (audio_layer == kAndroidJavaAudio) {
    ...
    RTC_DLOG(LS_ERROR) << "The requested audio layer is not supported";
...
// PulseAudio is the default selection.
  ...
  RTC_DLOG(WARNING) << "PulseAudio is disabled using build flag.";
  if ((audio_layer == kLinuxAlsaAudio) ||
      (audio_layer == kPlatformDefaultAudio)) {
    audio_device_.reset(new AudioDeviceLinuxALSA());
    RTC_DLOG(INFO) << "Linux ALSA APIs will be utilized.";
  }
#else
...
// iOS ADM implementation.
  ...
  if (audio_layer == kPlatformDefaultAudio) {
    ...
    RTC_DLOG(INFO) << "iPhone Audio APIs will be utilized.";
  }
...
// Mac OS X ADM implementation.
  ...
  if (audio_layer == kPlatformDefaultAudio) {
    ...
    RTC_DLOG(INFO) << "Mac OS X Audio APIs will be utilized.";
  }
#endif  // WEBRTC_MAC
...
int32_t AudioDeviceModuleImpl::Init() {
  ...
  if (status != AudioDeviceGeneric::InitStatus::OK) {
    RTC_DLOG(LS_ERROR) << "Audio device initialization failed.";
    return -1;
  }
  initialized_ = true;
  return 0;
...
int32_t AudioDeviceModuleImpl::SetStereoRecording(bool enable) {
  ...
  if (audio_device_->SetStereoRecording(enable) == -1) {
    if (enable) {
      RTC_DLOG(WARNING) << "failed to enable stereo recording";
    }
    return -1;
  }
  int8_t nChannels(1);
...
int32_t AudioDeviceModuleImpl::SetStereoPlayout(bool enable) {
  ...
  if (audio_device_->SetStereoPlayout(enable)) {
    RTC_DLOG(WARNING) << "stereo playout is not supported";
    return -1;
  }
  int8_t nChannels(1);
  if (enable) {
...
int32_t AudioDeviceModuleImpl::PlayoutDelay(uint16_t* delayMS) const {
  ...
  if (audio_device_->PlayoutDelay(delay) == -1) {
    RTC_DLOG(LERROR) << "failed to retrieve the playout delay";
    return -1;
  }
  *delayMS = delay;
  return 0;
...
int32_t AudioDeviceModuleImpl::GetPlayoutUnderrunCount() const {
  ...
  RTC_DLOG(INFO) << "output: " << underrunCount;
