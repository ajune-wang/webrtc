...
class SimulatedTimeControllerImpl : public TaskQueueFactory,
 ...
 public:
  ...
  // Runs pending tasks on all task queues but the currently executing queue if
  // called on the time controller thread.
  ...
  // Runs all runners in |runners_| that has tasks or modules pending execution.
  void RunPendingRunners();
  // Return |current_time_|.
  Timestamp CurrentTime() const;
  // Increments the time to the min of runner->GetNextRunTime() for each runner
  // in |runners_|.
 ...
 private:
  // Returns runners in |runners_| that are pending execution.
...
// Like SimulatedTimeController, but also overrides the global clock backing
// rtc::TimeMillis() and rtc::TimeMicros(). Not that this is not thread safe
