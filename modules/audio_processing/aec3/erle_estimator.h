 ...
 public:
  ...
  class ErleFreqInstantaneous {
   public:
    ...
    absl::optional<float> Update(float Y2, float E2, float X2, size_t band);
    // Reset all the member of the class.
    void Reset();
    // Returns true if the last update was done using a low energy render
    // signal.
    bool low_render_energy(size_t band) { return low_render_energy_[band]; }
