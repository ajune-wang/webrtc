...
RTCStatsCollector::CreateReportFilteredBySelector(
  ...
  if (filter_by_sender_selector) {
    ...
    if (receiver_selector) {
      RTC_LOG(LS_ERROR) << "receiver_selector: " << receiver_selector.get();
      // Find the inbound-rtp of the receiver using ssrc lookup.
      uint32_t ssrc;
      worker_thread_->BlockingCall([&] {
        ssrc = receiver_selector->ssrc();
      });
      RTC_LOG(LS_ERROR) << "INBOUND-RTPS at time of SELECTOR...";
      for (const auto* inbound_rtp :
            report->GetStatsOfType<RTCInboundRTPStreamStats>()) {
        RTC_DCHECK(inbound_rtp->ssrc.is_defined());
        RTC_LOG(LS_ERROR) << "  inbound-rtp.ssrc: "
                          << *inbound_rtp->ssrc;
      }
      RTC_LOG(LS_ERROR) << "SELECTOR HAS SSRC: " << ssrc;
      for (const auto* inbound_rtp :
            report->GetStatsOfType<RTCInboundRTPStreamStats>()) {
        RTC_DCHECK(inbound_rtp->ssrc.is_defined());
