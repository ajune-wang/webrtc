...
bool ResourceAdaptationProcessor::HasInsufficientInput() const {
  return !has_input_video_ || !manager_->last_input_frame_size_.has_value() ||
         (effective_degradation_preference_ ==
              DegradationPreference::MAINTAIN_RESOLUTION &&
...
void ResourceAdaptationProcessor::OnResourceUnderuse(
  ...
  for (const Resource* resource : resources_) {
    if (!resource->CanApplyAdaptation(restrictions_before, restrictions_after,
                                      &reason_resource)) {
...
bool VideoStreamEncoderResourceManager::
  ...
  if (!reason_resource)
    return true;
  // TODO(hbos): CanApplyAdaptation should also look at input size/fps.
  RTC_DCHECK(manager_->last_input_frame_size_.has_value());
  int input_pixels = manager_->last_input_frame_size_.value();
  AdaptationObserverInterface::AdaptReason reason =
      manager_->ReasonFromResource(*reason_resource);
...
bool VideoStreamEncoderResourceManager::PreventAdaptUpInBalancedResource::
    CanApplyAdaptation(const VideoSourceRestrictions& restrictions_before,
                       const VideoSourceRestrictions& restrictions_after,
                       const Resource* reason_resource) const {
  if (!reason_resource)
    return true;
  ...
  if (reason == AdaptationObserverInterface::AdaptReason::kQuality &&
      manager_->processor_->effective_degradation_preference_ ==
          DegradationPreference::BALANCED &&
      !manager_->balanced_settings_.CanAdaptUp(
          GetVideoCodecTypeOrGeneric(manager_->encoder_settings_), input_pixels,
...
void VideoStreamEncoderResourceManager::OnFrameDroppedDueToSize() {
  ...
  processor_->OnResourceOveruse(
      ResourceWithQuality(AdaptationObserverInterface::AdaptReason::kQuality));
  if (processor_->degradation_preference() == DegradationPreference::BALANCED &&
    ...
    processor_->OnResourceOveruse(ResourceWithQuality(
        AdaptationObserverInterface::AdaptReason::kQuality));
  }
