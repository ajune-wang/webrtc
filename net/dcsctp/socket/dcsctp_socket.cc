...
void DcSctpSocket::HandleInit(const CommonHeader& header,
  ...
  if (state_ == State::kClosed) {
    ...
    while (connect_params_.verification_tag == tcb_->my_verification_tag()) {
...
void DcSctpSocket::HandleSack(const CommonHeader& header,
  ...
  if (ValidateParseSuccess(chunk) && ValidateHasTCB()) {
    if (!tcb_->retransmission_queue().IsAcknowledgeValid(*chunk)) {
      RTC_DLOG(LS_VERBOSE) << "Dropping out-of-order SACK with TSN "
                           << *chunk->cumulative_tsn_ack();
    } else {
...
void DcSctpSocket::HandleShutdown(
  ...
  if (state_ == State::kCookieWait || state_ == State::kCookieEchoed) {
    // https://tools.ietf.org/html/rfc4960#section-9.2
