...
class JsepTransportControllerTest : public JsepTransportController::Observer,
 ...
 protected:
  void OnConnectionState(cricket::IceConnectionState state) {
    RTC_LOG(INFO) << "OnConnectionState: " << static_cast<int>(state);
...
TEST_F(JsepTransportControllerTest,
  ...
  fake_video_dtls->fake_ice_transport()->SetConnectionCount(2);
  // Setting the fake DTLS transport writable status first sets the underlying
  // ICE transport writable status. The latter fires the signal for the ICE
  // writable status, which triggers the JsepTransportController to update the
  // aggregate state and leads to an intermediate kIceConnectionConnecting
  // state. The signal of the DTLS writable status further triggers another
  // update of the aggregate state in JsepTransportController and moves the
  // state to kIceConnectionConnected. We only observe the end result
  // kIceConnectionConnected for how the waiting mechanism is implemented.
  fake_video_dtls->SetWritable(true);
