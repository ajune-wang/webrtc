...
absl::optional<uint32_t> NetEqImpl::GetPlayoutTimestamp() const {
  rtc::CritScope lock(&crit_sect_);
  if (first_packet_ || last_mode_ == Modes::kRfc3389Cng ||
      last_mode_ == Modes::kCodecInternalCng) {
...
NetEq::Operations NetEqImpl::last_operation_for_test() const {
  rtc::CritScope lock(&crit_sect_);
...
int NetEqImpl::GetAudioInternal(AudioFrame* audio_frame,
  ...
  *muted = false;
  ...
  if (enable_muted_state_ && expand_->Muted() && packet_buffer_->Empty()) {
    RTC_DCHECK_EQ(last_mode_, Modes::kExpand);
    audio_frame->Reset();
  ...
  if (last_mode_ != Modes::kRfc3389Cng) {
  ...
  if ((last_mode_ == Modes::kNormal) ||
      (last_mode_ == Modes::kAccelerateFail) ||
      (last_mode_ == Modes::kPreemptiveExpandFail) ||
      (last_mode_ == Modes::kRfc3389Cng) ||
      (last_mode_ == Modes::kCodecInternalCng)) {
  ...
  if (last_mode_ != Modes::kExpand && last_mode_ != Modes::kCodecPlc) {
  ...
  if (!(last_mode_ == Modes::kRfc3389Cng ||
        last_mode_ == Modes::kCodecInternalCng ||
        last_mode_ == Modes::kExpand || last_mode_ == Modes::kCodecPlc)) {
...
int NetEqImpl::GetDecision(Operations* operation,
  ...
  *operation = Operations::kUndefined;
  ...
  if (controller_->CngRfc3389On() || last_mode_ == Modes::kRfc3389Cng) {
  ...
  const int samples_left = static_cast<int>(sync_buffer_->FutureLength() -
                                            expand_->overlap_length());
  if (last_mode_ == Modes::kAccelerateSuccess ||
      last_mode_ == Modes::kAccelerateLowEnergy ||
      last_mode_ == Modes::kPreemptiveExpandSuccess ||
      last_mode_ == Modes::kPreemptiveExpandLowEnergy) {
      ...
      const uint64_t generated_noise_samples =
          generated_noise_stopwatch_
              ? generated_noise_stopwatch_->ElapsedTicks() *
                    ...
                    controller_->noise_fast_forward()
              : 0;
      if (generated_noise_samples > 0 && last_mode_ != Modes::kDtmf) {
...
void NetEqImpl::DoNormal(const int16_t* decoded_buffer,
  ...
  if ((speech_type == AudioDecoder::kComfortNoise) ||
      ((last_mode_ == Modes::kCodecInternalCng) && (decoded_length == 0))) {
...
bool NetEqImpl::DoCodecPlc() {
  ...
  // Update in-call and post-call statistics.
  const bool is_new_concealment_event = (last_mode_ != Modes::kCodecPlc);
...
int NetEqImpl::DoExpand(bool play_dtmf) {
  while ((sync_buffer_->FutureLength() - expand_->overlap_length()) <
    ...
    bool is_new_concealment_event = (last_mode_ != Modes::kExpand);
...
int NetEqImpl::DoAccelerate(int16_t* decoded_buffer,
  ...
  switch (return_code) {
    case Accelerate::kSuccess:
      last_mode_ = Modes::kAccelerateSuccess;
      break;
