...
TEST(RtpVideoSenderTest,
  ...
  test.AdvanceTime(TimeDelta::Millis(33));
  ASSERT_THAT(sent_packets, SizeIs(2));
  DependencyDescriptor dd_s0;
  DependencyDescriptor dd_s1;
  // Keyframe should have attached structure.
  EXPECT_TRUE(sent_packets[0].GetExtension<RtpDependencyDescriptorExtension>(
      nullptr, &dd_s0));
  EXPECT_THAT(dd_s0.attached_structure, NotNull());
  ...
  EXPECT_TRUE(sent_packets[1].GetExtension<RtpDependencyDescriptorExtension>(
      nullptr, &dd_s1));
  EXPECT_THAT(dd_s1.attached_structure, IsNull());
}
