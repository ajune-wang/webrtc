  ...
  if (required_spatial_index) {
    ...
    RTC_CHECK(mode_ != SimulcastMode::kNormal)
        << "Analyzing encoder is in kNormal "
           ...
           "stream met.";
    // RTC_CHECK(encoded_image.SpatialIndex().has_value());
    // RTC_CHECK(false);
    if (mode_ == SimulcastMode::kSimulcast) {
      ...
      std::printf("Frame: %d; %d (cur); %d (req); %d (key); %d (>); %d (!=)\n",
                  frame_id, *cur_spatial_index, *required_spatial_index,
                  encoded_image._frameType == VideoFrameType::kVideoFrameKey,
                  *cur_spatial_index > *required_spatial_index,
                  *cur_spatial_index != *required_spatial_index);
