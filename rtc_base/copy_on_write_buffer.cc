...
CopyOnWriteBuffer::CopyOnWriteBuffer(size_t size)
    : buffer_(size > 0 ? CreateBuffer(size) : nullptr),
      offset_(0),
      size_(size) {
  RTC_DCHECK(IsConsistent());
}
...
CopyOnWriteBuffer::CopyOnWriteBuffer(size_t size, size_t capacity)
    : buffer_(size > 0 || capacity > 0 ? CreateBuffer(size, capacity)
                                       : nullptr),
      offset_(0),
      size_(size) {
  RTC_DCHECK(IsConsistent());
...
void CopyOnWriteBuffer::EnsureCapacity(size_t new_capacity) {
  ...
  if (!buffer_) {
    if (new_capacity > 0) {
      buffer_ = CreateBuffer(0, new_capacity);
      offset_ = 0;
      size_ = 0;
    }
    RTC_DCHECK(IsConsistent());
...
void CopyOnWriteBuffer::Clear() {
  ...
  if (buffer_->HasOneRef()) {
    ...
    buffer_ = CreateBuffer(0, capacity());
  }
  offset_ = 0;
  size_ = 0;
  RTC_DCHECK(IsConsistent());
...
void CopyOnWriteBuffer::UnshareAndEnsureCapacity(size_t new_capacity) {
  ...
  buffer_ = CreateBuffer(buffer_->data() + offset_, size_, new_capacity);
  offset_ = 0;
  RTC_DCHECK(IsConsistent());
}
