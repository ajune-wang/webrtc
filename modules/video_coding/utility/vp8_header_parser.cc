...
bool GetQp(const uint8_t* buf, size_t length, int* qp) {
  if (length < kCommonPayloadHeaderLength) {
    NLOG(LS_WARNING, "Failed to get QP, invalid length.");
    return false;
  }
  VP8BitReader br;
  ...
  if (header_length + partition_length > length) {
    NLOG(LS_WARNING, "Failed to get QP, invalid length: ", length);
    return false;
  }
  buf += header_length;
  ...
  if (br.eof_ == 1) {
    NLOG(LS_WARNING, "Failed to get QP, end of file reached.");
    return false;
  }
  *qp = base_q0;
  return true;
