...
class AecState {
 public:
  ...
  int FilterDelayBlocks() const { return delay_state_.FilterDelayBlocks(); }
  ...
  bool TransparentMode() const { return transparent_state_.IsActive(); }
 ...
 private:
  ...
  class InitialState {
   public:
    ...
    void Reset();
    void Update(                bool active_render_block,
  ...
  class DelayState {
   public:
    explicit DelayState(const EchoCanceller3Config& config);
    void Update(const FilterAnalyzer& filter_analyzer,
                const absl::optional<DelayEstimate>& external_delay,
                size_t blocks_with_proper_filter_adaptation);
    bool ExternalDelayReported() const { return external_delay_seen_; }
    int FilterDelayBlocks() const { return filter_delay_blocks_; }
    absl::optional<int> InternalDelay() const { return internal_delay_; }
    size_t BlocksSinceDelayChange() const {
      return frames_since_external_delay_change_;
    }
  ...
  class TransparentState {
   public:
    explicit TransparentState(const EchoCanceller3Config& config);
    void Reset();
    bool IsActive() const { return transparent_mode_; }
  ...
  class FilterQualityState {
   public:
    explicit FilterQualityState(const EchoCanceller3Config& config);
    void Reset();
 ...
 explicit   SaturationDetector(const EchoCanceller3Config& config);
   ...
   private:
    ...
    size_t blocks_since_last_saturation_;
