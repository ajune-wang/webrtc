...
// clamps to the closest constraint, if |val| is outside.
// If min > max, clamps to the most distant constraint.
int ScaleDownResolution(int resolution,
  ...
  if (resolution <= min_resolution)
    return resolution;
  return std::max<int>(static_cast<int>(min_resolution / scale_down_by),
                       min_resolution);
}
