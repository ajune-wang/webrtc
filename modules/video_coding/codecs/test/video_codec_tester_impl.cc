...
class Encoder : public EncodedImageCallback {
 public:
  ...
  void Initialize(const FramesSettings& frames_settings) {
...
std::unique_ptr<VideoCodecStats> VideoCodecTesterImpl::RunDecodeTest(
  ...
  decoder.Initialize(frames_settings.begin()->second.sdp_video_format);
...
std::unique_ptr<VideoCodecStats> VideoCodecTesterImpl::RunEncodeTest(
  ...
  encoder.Initialize(frames_settings);

  for (const auto& [timestamp_rtp, encoding_settings] : frames_settings) {
    const EncodingSettings::LayerSettings& top_layer =
        encoding_settings.layers_settings.rbegin()->second;
