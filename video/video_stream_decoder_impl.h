...
class VideoStreamDecoderImpl : public VideoStreamDecoderInterface,
 ...
 private:
  ...
  // The |bookkeeping_queue_| post decode tasks to the |decode_queue_|, the
  // |decode_queue_| in turn use the |decoder_| to decode frames. The |decoder_|
  // post back to the |bookkeeping_queue_| with the decoded frames. When the
  // dtor runs we start by breaking this cycle by making sure that the
  // |bookkeeping_queue_| no longer post any tasks to the |decode_queue_|. Now
  // it is safe for the |decode_queue_| to destruct. Now that the
  // |decode_queue_| is destructed the |decoder_| can be destructed. Finally
  // the |bookkeeping_queue_| can be destructed.
