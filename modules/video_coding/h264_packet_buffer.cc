...
bool IsFirstPacketOfFragment(const rtc::CopyOnWriteBuffer& payload) {
  constexpr int kStartBit = 0x80;
  return payload.cdata()[1] & kStartBit;
}
...
bool BeginningOfIdr(const H264PacketBuffer::Packet& packet) {
  ...
  switch (h264_header.packetization_type) {
    ...
    case kH264FuA: {
      return contains_idr_nalu && IsFirstPacketOfFragment(packet.video_payload);
    }
  }
}
...
rtc::CopyOnWriteBuffer FixVideoPayload(rtc::CopyOnWriteBuffer payload,
  ...
  switch (h264_header.packetization_type) {
    ...
    case kH264FuA: {
      ...
      const int kNalTypeMask = 0x1F;

      // Only the first fragment includes the start code and nal type.
      if (IsFirstPacketOfFragment(payload)) {
        res.AppendData(kStartCode);
        uint8_t original_nal_type = payload.cdata()[1] & kNalTypeMask;
