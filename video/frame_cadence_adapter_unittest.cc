...
TEST(FrameCadenceAdapterTest, ChecksIfEncodeHappenedAlignWithMetronomeTick) {
  ZeroHertzFieldTrialEnabler enabler;
  GlobalSimulatedTimeController time_controller(Timestamp::Zero());
  // Here the metronome interval is 33ms.
  ...
  // Expect encode immediately if the next tick is infinite.
  EXPECT_CALL(callback, OnFrame(_, false, _)).Times(2);
  adapter->OnFrame(frame);
  time_controller.AdvanceTime(TimeDelta::Millis(30));
  // The next tick is still infinite now after 30ms.
  adapter->OnFrame(frame);
  time_controller.AdvanceTime(TimeDelta::Zero());
  Mock::VerifyAndClearExpectations(&callback);

  // The next tick is finite now after 36ms.
