...
bool PacketBuffer::InsertPacket(VCMPacket* packet) {
  std::vector<std::unique_ptr<RtpFrameObject>> found_frames;
  {
    ...
    if (buffer_[index] != absl::nullopt) {
      ...
      while (ExpandBufferSize() && buffer_[seq_num % buffer_.size()]) {
      }
    ...
    RTC_DCHECK(buffer_[index] == absl::nullopt);
    StoredPacket& entry = buffer_[index].emplace();

    entry.frame_begin = packet->is_first_packet_in_frame();
    entry.frame_end = packet->is_last_packet_in_frame();
    entry.seq_num = packet->seqNum;
    entry.continuous = false;
    entry.frame_created = false;
    entry.data = *packet;
    packet->dataPtr = nullptr;
...
bool PacketBuffer::ExpandBufferSize() {
  ...
  for (size_t i = 0; i < buffer_.size(); ++i) {
    if (buffer_[i]) {
      size_t index = buffer_[i]->seq_num % new_size;
      new_buffer[index] = std::move(buffer_[i]);
    }
  }
...
bool PacketBuffer::PotentialNewFrame(uint16_t seq_num) const {
  ...
  if (buffer_[index]->seq_num != seq_num)
    return false;
  if (buffer_[index]->frame_created)
    return false;
  if (buffer_[index]->frame_begin)
    return true;
  if (!buffer_[prev_index])
    return false;
  if (buffer_[prev_index]->frame_created)
    return false;
  if (buffer_[prev_index]->seq_num !=
      static_cast<uint16_t>(buffer_[index]->seq_num - 1)) {
    return false;
  }
  if (buffer_[prev_index]->data.timestamp != buffer_[index]->data.timestamp)
    return false;
  if (buffer_[prev_index]->continuous)
    return true;
  ...
  for (size_t i = 0; i < buffer_.size() && PotentialNewFrame(seq_num); ++i) {
    ...
    if (buffer_[index]->frame_end) {
      ...
      int64_t min_recv_time =
          buffer_[index]->data.packet_info.receive_time_ms();
      ...
      while (true) {
        ...
        buffer_[start_index]->frame_created = true;
      ...
      if (is_h264) {
        ...
        if (h264tid == kNoTemporalIdx && !is_h264_keyframe &&
          ...
          while (start_index != stop_index) {
            buffer_[start_index]->frame_created = false;
...
VCMPacket* PacketBuffer::GetPacket(uint16_t seq_num) {
  size_t index = seq_num % buffer_.size();
  if (!buffer_[index] || seq_num != buffer_[index]->seq_num) {
    return nullptr;
  }
