...
void AudioBuffer::CopyFrom(const int16_t* const data,
                           const StreamConfig& stream_config) {
  RTC_DCHECK_EQ(stream_config.num_channels(), input_num_channels_);
  RTC_DCHECK_EQ(stream_config.num_frames(), input_num_frames_);
  RestoreNumChannels();
  ...
  const int16_t* interleaved = data;
  if (num_channels_ == 1) {
...
void AudioBuffer::CopyTo(const StreamConfig& stream_config,
  ...
  if (num_channels_ == 1) {
    ...
    if (stream_config.num_channels() == 1) {
      ...
      for (size_t i = 0, k = 0; i < output_num_frames_; ++i) {
        ...
        for (size_t j = 0; j < stream_config.num_channels(); ++j, ++k) {
          interleaved[k] = tmp;
        }
      }
    }
    ...
    if (resampling_required) {
      for (size_t i = 0; i < num_channels_; ++i) {
        ...
        output_resamplers_[i]->Resample(data_->channels()[i],
                                        ...
                                        output_num_frames_);
        interleave_channel(i, stream_config.num_channels(), output_num_frames_,
    ...
    for (size_t i = num_channels_; i < stream_config.num_channels(); ++i) {
      for (size_t j = 0, k = i, n = num_channels_; j < output_num_frames_; ++j,
                  k += stream_config.num_channels(),
                  n += stream_config.num_channels()) {
