...
Packet CreatePacket(
  ...
  if (first_in_frame) {
    ...
    if (!is_key_frame) {
      packet.ref_frame_ids = std::move(ref_frame_ids);
    }
  }
  return packet;
}
...
class LossNotificationControllerBaseTest : public ::testing::Test,
 ...
 protected:
  ...
  void OnReceivedPacket(const Packet& packet) {
    ...
    uut_.OnReceivedPacket(packet.seq_num, packet.first_in_frame,
                          packet.frame_id, packet.ref_frame_ids);
  }
  ...
  void OnAssembledFrame(uint16_t first_seq_num,
    ...
    uut_.OnAssembledFrame(first_seq_num, frame_id, discardable,
                          previous_first_packet_in_frame_->ref_frame_ids);
  }
