...
class EchoRemoverImpl final : public EchoRemover {
 ...
 private:
  ...
  FilterBasedDelayEstimator filter_based_delay_estimator_;
...
EchoRemoverImpl::EchoRemoverImpl(const EchoCanceller3Config& config,
    ...
    : config_(config),
      ...
      aec_state_(config_),
      filter_based_delay_estimator_(config_,
                                    data_dumper_.get(),
                                    optimization_) {
...
void EchoRemoverImpl::ProcessCapture(
  ...
  data_dumper_->DumpRaw("aec3_internal_delay",
                        filter_based_delay ? *filter_based_delay : -1);
  ...
  if (!subtractor_running) {
    ...
    subtractor_output.s_main.fill(0.f);
    std::copy(y0.begin(), y0.end(), e_main.begin());
  }

  // Update the AEC state information.
  aec_state_.Update(external_delay, filter_based_delay,
