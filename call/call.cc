...
class Call final : public webrtc::Call,
 ...
 private:
  ...
  void SetAudioStreamForSyncGroup(const std::string& sync_group,
                                  AudioReceiveStream* audio_stream)
      RTC_RUN_ON(worker_thread_);
...
void Call::RemoveSyncGroup(AudioReceiveStream* audio_stream) {
  ...
  {
    RTC_DCHECK_RUN_ON(worker_thread_);
    SetAudioStreamForSyncGroup(audio_stream->config().sync_group, nullptr);
  }
}
...
// RTC_RUN_ON(worker_thread_)
void Call::SetAudioStreamForSyncGroup(const std::string& sync_group,
                                      AudioReceiveStream* audio_stream) {
  // `audio_stream` may be nullptr.
  ...
  for (VideoReceiveStream2* video_stream : video_receive_streams_) {
    ...
    // group. This is not supported in the current implementation.
...
void Call::ConfigureSync(const std::string& sync_group) {
  SetAudioStreamForSyncGroup(sync_group,
