      ...
      for (auto rate : {16000, 32000, 48000}) {
        ...
        for (size_t delay_samples : {0, 64, 150, 200, 301}) {
          ...
          std::unique_ptr<EchoRemover> remover(EchoRemover::Create(
              config, rate, num_render_channels, num_capture_channels));
          std::unique_ptr<RenderDelayBuffer> render_buffer(
              ...
              for (size_t band = 0; band < x.size(); ++band) {
                for (size_t channel = 0; channel < x[0].size(); ++channel) {
                  input_energy = std::inner_product(
                      y[band][channel].begin(), y[band][channel].end(),
                      y[band][channel].begin(), input_energy);
                }
              }
            }
              ...
              for (size_t band = 0; band < x.size(); ++band) {
                for (size_t channel = 0; channel < x[0].size(); ++channel) {
                  output_energy = std::inner_product(
                      y[band][channel].begin(), y[band][channel].end(),
                      y[band][channel].begin(), output_energy);
                }
              }
            }
          }
