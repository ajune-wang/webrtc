  ...
  for (size_t i = 0; i < size_ && PotentialNewFrame(seq_num); ++i) {
    ...
    if (sequence_buffer_[index].frame_end) {
      ...
      auto frame = std::make_unique<RtpFrameObject>(
          ...
          GetBitstream(frame_size, start_seq_num, seq_num));
...
rtc::scoped_refptr<EncodedImageBuffer> PacketBuffer::GetBitstream(
    size_t frame_size,
  ...
  do {
    ...
    RTC_DCHECK_LE(offset + length, buffer->data() + buffer->size());
    memcpy(offset, data_buffer_[index].dataPtr, length);
    offset += length;
