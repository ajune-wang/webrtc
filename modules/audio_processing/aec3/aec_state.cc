...
void AecState::Update(
  ...
  // Update counters.
  const bool active_render = DetectActiveRender(aligned_render_block);
...
AecState::IntraFilterDelay::IntraFilterDelay(const EchoCanceller3Config& config)
    : delay_headroom_blocks_(config.delay.delay_headroom_blocks) {}

void AecState::IntraFilterDelay::Update(
...
void AecState::TransparentMode::Update(int filter_delay_blocks,
  ...
  if (bounded_erl_) {
    ...
    transparent_mode_ = filter_should_have_converged;
  }
}
