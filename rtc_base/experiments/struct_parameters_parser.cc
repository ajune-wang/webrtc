...
template <typename T>
bool TypedParser<T>::Changed(const void* src, const void* base) {
...
StructParametersParser::StructParametersParser(
    ...
    : defaults_(defaults), target_(target), members_(std::move(members)) {}
...
std::string StructParametersParser::EncodeChanged() const {
  std::string res;
  bool first = true;
  for (const auto& member : members_) {
    auto member_offset = reinterpret_cast<const char*>(member.member_ptr) -
                         reinterpret_cast<const char*>(target_);
    const void* defaults_ptr =
        reinterpret_cast<const char*>(defaults_) + member_offset;
    if (member.parser.changed(member.member_ptr, defaults_ptr)) {
      if (!first)
        res += ",";
      res += member.key;
      res += ":";
...
std::string StructParametersParser::EncodeAll() const {
  std::string res;
  bool first = true;
  for (const auto& member : members_) {
    if (!first)
