...
std::unique_ptr<Port> TurnPortFactory::Create(
  ...
  TurnPort* port = TurnPort::Create(
      args.network_thread, args.socket_factory, args.resolver_factory,
      args.network, udp_socket, args.username, args.password,
      *args.server_address, args.config->credentials, args.config->priority,
...
std::unique_ptr<Port> TurnPortFactory::Create(const CreateRelayPortArgs& args,
  ...
  TurnPort* port = TurnPort::Create(
      args.network_thread, args.socket_factory, args.resolver_factory,
      args.network, min_port, max_port, args.username, args.password,
      *args.server_address, args.config->credentials, args.config->priority,
      args.origin, args.config->tls_alpn_protocols,
