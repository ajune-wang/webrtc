...
size_t ReadTrendlineFilterWindowSize() {
  ...
  if (parsed_values == 1) {
    if (window_size > 1)
      return window_size;
    NLOG(WARNING, "Window size must be greater than 1.");
  }
  NLOG(LS_WARNING,
       "Failed to parse parameters for BweTrendlineFilter "
...
DelayBasedBwe::Result DelayBasedBwe::IncomingPacketFeedbackVector(
  ...
  if (packet_feedback_vector.empty()) {
    NLOG(LS_WARNING, "Very late feedback received.");
...
DelayBasedBwe::Result DelayBasedBwe::OnLongFeedbackDelay(
  ...
  NLOG(LS_WARNING, "Long feedback delay detected, reducing BWE to ",
...
void DelayBasedBwe::SetStartBitrate(int start_bitrate_bps) {
  NLOG(LS_WARNING, "BWE Setting start bitrate to: ", start_bitrate_bps);
