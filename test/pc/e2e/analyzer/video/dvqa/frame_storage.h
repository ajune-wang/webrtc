...
#ifndef TEST_PC_E2E_ANALYZER_VIDEO_DVQA_FRAME_STORAGE_H_
#define TEST_PC_E2E_ANALYZER_VIDEO_DVQA_FRAME_STORAGE_H_
...
// Stores video frames for DefaultVideoQualityAnalyzer. Ensures cleanup when
// frames are too old.
class FrameStorage {
 public:
  FrameStorage(TimeDelta max_storage_duration, Clock* clock)
  ...
  FrameStorage(FrameStorage&&) = default;
  FrameStorage& operator=(FrameStorage&&) = default;
  ...
  // Removes frame from the storage if it is available.
  //
  // Complexity: O(log(n))
...
#endif  // TEST_PC_E2E_ANALYZER_VIDEO_DVQA_FRAME_STORAGE_H_
