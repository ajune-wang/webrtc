...
class PeerConnectionWrapper : public webrtc::PeerConnectionObserver,
 ...
 public:
  ...
  std::vector<cricket::CandidatePairChangeEvent> ice_candidate_change_history()
      const {
    return ice_candidate_change_history_;
  }
 ...
 private:
  ...
  void OnIceSelectedCandidatePairChanged(
    ...
    ice_candidate_change_history_.push_back(event);
  }
  ...
  std::vector<cricket::CandidatePairChangeEvent> ice_candidate_change_history_;
...
TEST_P(PeerConnectionIntegrationTest, MediaContinuesFlowingAfterIceRestart) {
  ...
  ASSERT_EQ(caller()->ice_candidate_change_history().size(), 1u);
  ...
  ASSERT_EQ(caller()->ice_candidate_change_history().size(), 2u);
