...
void RnnVadFeaturesExtractor::AnalyzePitch() {
  ...
  // Initial pitch search in half-band.
  const auto coarse_pitch_candidates =
      CoarsePitchSearch({lp_residual.data(), lp_residual.size()});
  for (const auto& v : coarse_pitch_candidates) {
    RTC_CHECK_LE(0, v);
    RTC_CHECK_LE(v, kBufSize);
  }

  // const auto = FindPitchCandidates(lp_residual);
