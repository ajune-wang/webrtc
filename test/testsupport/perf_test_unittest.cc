...
TEST_F(PerfTest, TestGetPerfResultsHistograms) {
  ...
  PrintResult("foo", "bar", "baz_v", 7, "sigma", true);
  // Note: the error will be ignored, not supported by histograms.
  PrintResultMeanAndError("foo", "bar", "baz_me", 1, 2000, "sigma", false);
  const double kListOfScalars[] = {1, 2, 3};
  PrintResultList("foo", "bar", "baz_vl", kListOfScalars, "sigma", false);
  ...
  ASSERT_EQ(histogram_set.histograms_size(), 2)
      << "Should be two histograms: foo and measurement";
  ...
  EXPECT_EQ(stories.generic_set().values(0), "\"baz_v\"");

  EXPECT_EQ(hist2.name(), "measurementmodifier");
