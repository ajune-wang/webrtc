...
class VideoStreamDecoderImpl : public VideoStreamDecoderInterface {
 ...
 private:
  ...
  void SaveFrameMetaInfo(const video_coding::EncodedFrame& frame)
      RTC_RUN_ON(bookkeeping_queue_);
  FrameMetaInfo* GetFrameMetaInfo(int64_t timestamp)
      RTC_RUN_ON(bookkeeping_queue_);
  ...
  static constexpr int kFrameMetaInfoMemory = 8;
  std::array<FrameMetaInfo, kFrameMetaInfoMemory> frame_meta_info_
      RTC_GUARDED_BY(bookkeeping_queue_);
  int next_frame_meta_info_index_ RTC_GUARDED_BY(bookkeeping_queue_);
