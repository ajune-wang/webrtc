...
using ReversedTree =
    flat_tree<int, webrtc::identity, std::greater<int>, std::vector<int>>;
...
TEST(FlatTree, NoExcept) {
  ...
  using MoveThrowsTree = flat_tree<MoveThrows, webrtc::identity, std::less<>,
...
TEST(FlatTree, IncompleteType) {
  struct A {
    using Tree = flat_tree<A, webrtc::identity, std::less<A>, std::vector<A>>;
    int data;
...
TEST(FlatTree, Stability) {
  ...
  using Tree =
      flat_tree<Pair, webrtc::identity, LessByFirst<Pair>, std::vector<Pair>>;
...
TYPED_TEST_P(FlatTreeTest, ErasePosition) {
  ...
  //  with a key, if key has a templated constructor.
  {
    ...
    flat_tree<T, webrtc::identity, std::less<>, std::vector<T>> cont;
    T v(0);
...
TYPED_TEST_P(FlatTreeTest, EraseIf) {
  ...
  EXPECT_EQ(0u, webrtc::EraseIf(x, [](int) { return false; }));
  EXPECT_THAT(x, ElementsAre());

  x = {1, 2, 3};
  EXPECT_EQ(1u, webrtc::EraseIf(x, [](int elem) { return !(elem & 1); }));
  EXPECT_THAT(x, ElementsAre(1, 3));

  x = {1, 2, 3, 4};
  EXPECT_EQ(2u, webrtc::EraseIf(x, [](int elem) { return elem & 1; }));
