...
class TcpMessageRoute {
 public:
  ...
  void SendAction(size_t size, std::function<void()> on_received);

 private:
  // Represents a message sent over the route. When all fragments has been
  // delivered, the message is cosidered delivered and the handler is triggered.
  ...
  std::list<ActionMessage> messages_;
