...
void AudioRtpSender::GenerateKeyFrame(const std::vector<std::string>& rids) {
  RTC_DCHECK_RUN_ON(signaling_thread_);
...
void VideoRtpSender::GenerateKeyFrame(const std::vector<std::string>& rids) {
  RTC_DCHECK_RUN_ON(signaling_thread_);
  RTC_LOG(LS_ERROR) << "GenerateKeyFrame with #" << rids.size() << " rids";
  if (video_media_channel() && ssrc_ && !stopped_) {
    worker_thread_->Invoke<void>(RTC_FROM_HERE, [&, rids] {
