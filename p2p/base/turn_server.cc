...
void TurnServer::OnInternalPacket(rtc::AsyncPacketSocket* socket,
  ...
  TurnServerConnection conn(packet.address(), iter->second, socket);
...
void TurnServerAllocation::OnExternalPacket(rtc::AsyncPacketSocket* socket,
  ...
  auto channel = FindChannel(packet.address());
  if (channel != channels_.end()) {
    ...
    server_->Send(&conn_, buf);
             ...
             HasPermission(packet.address().ipaddr())) {
    ...
    msg.AddAttribute(std::make_unique<StunXorAddressAttribute>(
        STUN_ATTR_XOR_PEER_ADDRESS, packet.address()));
    msg.AddAttribute(std::make_unique<StunByteStringAttribute>(
    ...
    RTC_LOG(LS_WARNING)
        ...
        << packet.address().ToSensitiveString();
  }
}
