  ...
  if (codec_header.ss_data_available) {
    ...
    if (frame->frame_type() == VideoFrameType::kVideoFrameKey) {
      if (frame->SpatialIndex() == 0) {
        RTC_LOG(LS_WARNING)
            << "Received keyframe without scalability structure";
        return kDrop;
      }
    }
    ...
    int64_t gof_info_idx = use_prev_gof ? unwrapped_tl0 - 1 : unwrapped_tl0;
    auto gof_info_it = gof_info_.find(gof_info_idx);
