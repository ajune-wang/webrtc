...
struct RobustThroughputEstimatorSettings {
  ...
  // The estimator keeps a window of at least `min_packets` and at least
  // the packets received during the last `min_window_duration` milliseconds.
  // (This means that it may store more than `min_packets` at high bitrates,
  ...
  TimeDelta min_window_duration = TimeDelta::Millis(750);
  ...
  // If audio packets aren't including in allocation (i.e. the
