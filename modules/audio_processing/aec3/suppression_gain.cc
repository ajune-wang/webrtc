...
void SuppressionGain::GainToNoAudibleEcho(
  ...
  const auto& p = dominant_nearend_detector_.IsNearendState() ? params_nearend_
                                                              : params_echo_;
  for (size_t k = 0; k < gain->size(); ++k) {
...
SuppressionGain::SuppressionGain(const EchoCanceller3Config& config,
    ...
    : data_dumper_(
      ...
      params_nearend_(config_.suppressor.nearend_tuning),
      params_echo_(config_.suppressor.normal_tuning),
