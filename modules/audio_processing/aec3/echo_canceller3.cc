...
// parameter set. Any the nondefault parameters are kept.
...
// new parameter set. Any the nondefault parameters are kept.
EchoCanceller3Config::Filter::ShadowConfiguration OverrideDefaultParams(
    const EchoCanceller3Config::Filter::ShadowConfiguration& config_to_override,
    const EchoCanceller3Config::Filter::ShadowConfiguration&
        overriding_config) {
...
EchoCanceller3Config AdjustConfig(const EchoCanceller3Config& config) {
  ...
  if (config.echo_removal_control.has_clock_drift) {
    if (config.ep_strength.bounded_erl) {
      custom_cfg.filter.ep_strength.default_len = 0.85f;
      custom_cfg.filter.ep_strength.lf = 0.01f;
      custom_cfg.filter.ep_strength.mf = 0.01f;
      custom_cfg.filter.ep_strength.hf = 0.01f;
      custom_cfg.filter.echo_model.render_pre_window_size = 1;
      custom_cfg.filter.echo_model.render_post_window_size = 1;
      custom_cfg.filter.echo_model.nonlinear_hold = 3;
      custom_cfg.filter.echo_model.nonlinear_release = 0.001f;
    } else {
      custom_cfg.ep_strength.bounded_erl = true;
      custom_cfg.filter.ep_strength.default_len = 0.8f;
      custom_cfg.filter.ep_strength.lf = 0.01f;
      custom_cfg.filter.ep_strength.mf = 0.01f;
      custom_cfg.filter.ep_strength.hf = 0.01f;
      custom_cfg.filter.main = {30, 0.1, 0.8, 0.001, 20075344};
      custom_cfg.filter.shadow = {30, 0.7, 20075344};
      custom_cfg.filter.main_initial = {30, 0.1, 1.5, 0.001, 20075344};
      custom_cfg.filter.shadow_initial = {30, 0.9, 20075344};

      custom_cfg.filter.echo_model.render_pre_window_size = 2;
      custom_cfg.filter.echo_model.render_post_window_size = 2;
      custom_cfg.filter.echo_model.nonlinear_hold = 3;
      custom_cfg.filter.echo_model.nonlinear_release = 0.6f;
    }
  }

  custom_cfg.filter.main =
      OverrideDefaultParams(config.filter.main, custom_cfg.filter.main);
  custom_cfg.filter.main_initial = OverrideDefaultParams(
      config.filter.main_initial, custom_cfg.filter.main_initial);
  custom_cfg.filter.shadow =
      OverrideDefaultParams(config.filter.shadow, custom_cfg.filter.shadow);
  custom_cfg.filter.shadow_initial = OverrideDefaultParams(
      config.filter.shadow_initial, custom_cfg.filter.shadow_initial);
  return custom_cfg;
}
