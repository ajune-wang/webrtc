...
TEST_F(CallStatsTest, AddAndTriggerCallback) {
  ...
  constexpr const int64_t kRtt = 25;
...
TEST_F(CallStatsTest, ProcessTime) {
  ...
  constexpr const int64_t kRtt = 100;
  constexpr const int64_t kRtt2 = 80;

  RtcpRttStats* rtcp_rtt_stats = call_stats_.rtcp_rtt_stats();

  MockStatsObserver stats_observer;

  EXPECT_CALL(stats_observer, OnRttUpdate(kRtt, kRtt)).Times(2)
  ...
  constexpr const int64_t kLastAvg = 94;
  EXPECT_CALL(stats_observer, OnRttUpdate(kLastAvg, kRtt2)).Times(1)
      .WillOnce(InvokeWithoutArgs([&event] { event.Set(); }));
...
TEST_F(CallStatsTest, MultipleObservers) {
  ...
  EXPECT_CALL(stats_observer_1, OnRttUpdate(kRtt, kRtt)).Times(AnyNumber())
      .WillOnce(InvokeWithoutArgs([&ev1] { ev1.Set(); }))
      .WillRepeatedly(Return());
  EXPECT_CALL(stats_observer_2, OnRttUpdate(kRtt, kRtt)).Times(AnyNumber())
...
TEST_F(CallStatsTest, ProducesHistogramMetrics) {
  ...
  MockStatsObserver stats_observer;
  call_stats_.RegisterStatsObserver(&stats_observer);
  EXPECT_CALL(stats_observer, OnRttUpdate(kRtt, kRtt)).Times(AnyNumber())
      .WillOnce(InvokeWithoutArgs([&event] { event.Set(); }))
      .WillRepeatedly(Return());
