...
class RenderDelayBufferImpl final : public RenderDelayBuffer {
 ...
 private:
  ...
  bool external_delay_verified_after_reset_;
...
bool RenderDelayBufferImpl::SetDelay(size_t delay) {
  if (!external_delay_verified_after_reset_ &&
    ...
    RTC_LOG(LS_WARNING) << "Difference between first estimated delay and "
                           "externally reported delay: "
                        << delay_difference << " ms.";
    external_delay_verified_after_reset_ = true;
  }
...
void RenderDelayBufferImpl::SetAudioBufferDelay(size_t delay_ms) {
  if (!external_audio_buffer_delay_ms_) {
    RTC_LOG(LS_WARNING)
        << "Receiving the intially reported externally buffer delay of "
        << delay_ms << " ms.";
  }
