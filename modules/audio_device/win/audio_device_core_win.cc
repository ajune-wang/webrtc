...
bool AudioDeviceWindowsCore::CoreAudioIsSupported() {
  ...
  if (FAILED(hr)) {
    NLOG(LS_ERROR, "AudioDeviceWindowsCore::CoreAudioIsSupported()",
         " Failed to create the required COM object (hr=", hr, ")");
    NLOG(LS_VERBOSE, "AudioDeviceWindowsCore::CoreAudioIsSupported()",
  ...
  if (MMDeviceIsAvailable) {
    ...
    for (uint16_t i = 0; i < numDevsRec; i++) {
      ...
      if (ok) {
        NLOG(LS_WARNING, "AudioDeviceWindowsCore::CoreAudioIsSupported()",
  ...
  if (coreAudioIsSupported) {
    NLOG(LS_VERBOSE, "*** Windows Core Audio is supported ***");
  } else {
    NLOG(LS_VERBOSE, "*** Windows Core Audio is NOT supported",
         " => will revert to the Wave API ***");
  }
...
AudioDeviceWindowsCore::AudioDeviceWindowsCore()
  ...
  if (!_avrtLibrary) {
    ...
    if (_avrtLibrary) {
      ...
      NLOG(LS_VERBOSE, "AudioDeviceWindowsCore::AudioDeviceWindowsCore()",
...
AudioDeviceWindowsCore::~AudioDeviceWindowsCore() {
  ...
  if (_avrtLibrary) {
    ...
    if (!freeOK) {
      NLOG(LS_WARNING, "AudioDeviceWindowsCore::~AudioDeviceWindowsCore()",
...
int32_t AudioDeviceWindowsCore::InitSpeaker() {
  ...
  if (_usingOutputDeviceIndex) {
    ...
    if (_outputDeviceIndex > (nDevices - 1)) {
      NLOG(LS_ERROR, "current device selection is invalid => unable to",
           " initialize");
      return -1;
    }
  }
  ...
  if (ret != 0 || (_ptrDeviceOut == NULL)) {
    NLOG(LS_ERROR, "failed to initialize the rendering enpoint device");
    SAFE_RELEASE(_ptrDeviceOut);
    return -1;
  }
  ...
  if (ret != 0 || pManager == NULL) {
    NLOG(LS_ERROR, "failed to initialize the render manager");
  ...
  if (ret != 0 || _ptrRenderSimpleVolume == NULL) {
    NLOG(LS_ERROR, "failed to initialize the render simple volume");
    SAFE_RELEASE(pManager);
    SAFE_RELEASE(_ptrRenderSimpleVolume);
    return -1;
  }
...
int32_t AudioDeviceWindowsCore::InitMicrophone() {
  ...
  if (ret != 0 || (_ptrDeviceIn == NULL)) {
    NLOG(LS_ERROR, "failed to initialize the capturing enpoint device");
    SAFE_RELEASE(_ptrDeviceIn);
    return -1;
  }
  ...
  if (ret != 0 || _ptrCaptureVolume == NULL) {
    NLOG(LS_ERROR, "failed to initialize the capture volume");
    SAFE_RELEASE(_ptrCaptureVolume);
    return -1;
  }
...
int32_t AudioDeviceWindowsCore::SetMicrophoneVolume(uint32_t volume) {
  NLOG(LS_VERBOSE,
       "AudioDeviceWindowsCore::SetMicrophoneVolume(volume=", volume, ")");
...
int32_t AudioDeviceWindowsCore::SetPlayoutDevice(uint16_t index) {
  ...
  if (_GetDeviceName(_ptrDeviceOut, szDeviceName, bufferLen) == 0) {
    NLOG(LS_VERBOSE, "friendly name: \"", szDeviceName, "\"");
  }
...
int32_t AudioDeviceWindowsCore::PlayoutDeviceName(
  ...
  if (index == (uint16_t)(-1)) {
    ...
    index = 0;
    NLOG(LS_VERBOSE, "Default Communication endpoint device will be used");
  }
  ...
  if (ret == 0) {
    ...
    if (WideCharToMultiByte(CP_UTF8, 0, szDeviceName, -1, name,
      ...
      NLOG(LS_ERROR, "WideCharToMultiByte(CP_UTF8) failed with error code ",
           GetLastError());
    }
  }
...
int32_t AudioDeviceWindowsCore::InitPlayout() {
  ...
  if (SUCCEEDED(hr)) {
    NLOG(LS_VERBOSE, "Audio Engine's current rendering mix format:");
    // format type
    NLOG(LS_VERBOSE, "wFormatTag     : 0x", std::hex, pWfxOut->wFormatTag,
         std::dec, " (", pWfxOut->wFormatTag, ")");
    // number of channels (i.e. mono, stereo...)
    NLOG(LS_VERBOSE, "nChannels      : ", pWfxOut->nChannels);
    // sample rate
    NLOG(LS_VERBOSE, "nSamplesPerSec : ", pWfxOut->nSamplesPerSec);
    // for buffer estimation
    NLOG(LS_VERBOSE, "nAvgBytesPerSec: ", pWfxOut->nAvgBytesPerSec);
    // block size of data
    NLOG(LS_VERBOSE, "nBlockAlign    : ", pWfxOut->nBlockAlign);
    // number of bits per sample of mono data
    NLOG(LS_VERBOSE, "wBitsPerSample : ", pWfxOut->wBitsPerSample);
    NLOG(LS_VERBOSE, "cbSize         : ", pWfxOut->cbSize);
  }
  ...
  for (unsigned int freq = 0; freq < sizeof(freqs) / sizeof(freqs[0]); freq++) {
    for (unsigned int chan = 0; chan < sizeof(_playChannelsPrioList) /
      ...
      if (hr == S_OK) {
        ...
        if (pWfxClosestMatch) {
          NLOG(INFO, "nChannels=", Wfx.nChannels,
               ", nSamplesPerSec=", Wfx.nSamplesPerSec,
               " is not supported. Closest match: ", "nChannels=",
  ...
  if (hr == S_OK) {
    ...
    NLOG(LS_VERBOSE, "VoE selected this rendering format:");
    NLOG(LS_VERBOSE, "wFormatTag         : 0x", std::hex, Wfx.wFormatTag,
         std::dec, " (", Wfx.wFormatTag, ")");
    NLOG(LS_VERBOSE, "nChannels          : ", Wfx.nChannels);
    NLOG(LS_VERBOSE, "nSamplesPerSec     : ", Wfx.nSamplesPerSec);
    NLOG(LS_VERBOSE, "nAvgBytesPerSec    : ", Wfx.nAvgBytesPerSec);
    NLOG(LS_VERBOSE, "nBlockAlign        : ", Wfx.nBlockAlign);
    NLOG(LS_VERBOSE, "wBitsPerSample     : ", Wfx.wBitsPerSample);
    NLOG(LS_VERBOSE, "cbSize             : ", Wfx.cbSize);
    NLOG(LS_VERBOSE, "Additional settings:");
    NLOG(LS_VERBOSE, "_playAudioFrameSize: ", _playAudioFrameSize);
    NLOG(LS_VERBOSE, "_playBlockSize     : ", _playBlockSize);
    NLOG(LS_VERBOSE, "_playChannels      : ", _playChannels);
  }
  ...
  if (FAILED(hr)) {
    NLOG(LS_ERROR, "IAudioClient::Initialize() failed:");
  }
  EXIT_ON_ERROR(hr);
  ...
  if (SUCCEEDED(hr)) {
    NLOG(LS_VERBOSE, "IAudioClient::GetBufferSize() => ", bufferFrameCount,
  ...
  NLOG(LS_VERBOSE, "render side is now initialized");
  return 0;
...
int32_t AudioDeviceWindowsCore::InitRecordingDMO() {
  ...
  NLOG(LS_VERBOSE, "Capture side is now initialized");
...
int32_t AudioDeviceWindowsCore::InitRecording() {
  ...
  if (SUCCEEDED(hr)) {
    NLOG(LS_VERBOSE, "Audio Engine's current capturing mix format:");
    // format type
    NLOG(LS_VERBOSE, "wFormatTag     : 0x", std::hex, pWfxIn->wFormatTag,
         std::dec, " (", pWfxIn->wFormatTag, ")");
    // number of channels (i.e. mono, stereo...)
    NLOG(LS_VERBOSE, "nChannels      : ", pWfxIn->nChannels);
    // sample rate
    NLOG(LS_VERBOSE, "nSamplesPerSec : ", pWfxIn->nSamplesPerSec);
    // for buffer estimation
    NLOG(LS_VERBOSE, "nAvgBytesPerSec: ", pWfxIn->nAvgBytesPerSec);
    // block size of data
    NLOG(LS_VERBOSE, "nBlockAlign    : ", pWfxIn->nBlockAlign);
    // number of bits per sample of mono data
    NLOG(LS_VERBOSE, "wBitsPerSample : ", pWfxIn->wBitsPerSample);
    NLOG(LS_VERBOSE, "cbSize         : ", pWfxIn->cbSize);
  }
  ...
  for (unsigned int freq = 0; freq < sizeof(freqs) / sizeof(freqs[0]); freq++) {
    for (unsigned int chan = 0;
      ...
      if (hr == S_OK) {
        ...
        if (pWfxClosestMatch) {
          NLOG(INFO, "nChannels=", Wfx.Format.nChannels,
  ...
  if (hr == S_OK) {
    ...
    NLOG(LS_VERBOSE, "VoE selected this capturing format:");
    NLOG(LS_VERBOSE, "wFormatTag        : 0x", std::hex, Wfx.Format.wFormatTag,
         std::dec, " (", Wfx.Format.wFormatTag, ")");
    NLOG(LS_VERBOSE, "nChannels         : ", Wfx.Format.nChannels);
    NLOG(LS_VERBOSE, "nSamplesPerSec    : ", Wfx.Format.nSamplesPerSec);
    NLOG(LS_VERBOSE, "nAvgBytesPerSec   : ", Wfx.Format.nAvgBytesPerSec);
    NLOG(LS_VERBOSE, "nBlockAlign       : ", Wfx.Format.nBlockAlign);
    NLOG(LS_VERBOSE, "wBitsPerSample    : ", Wfx.Format.wBitsPerSample);
    NLOG(LS_VERBOSE, "cbSize            : ", Wfx.Format.cbSize);
    NLOG(LS_VERBOSE, "Additional settings:");
    NLOG(LS_VERBOSE, "_recAudioFrameSize: ", _recAudioFrameSize);
    NLOG(LS_VERBOSE, "_recBlockSize     : ", _recBlockSize);
    NLOG(LS_VERBOSE, "_recChannels      : ", _recChannels);
  }
...
int32_t AudioDeviceWindowsCore::StartRecording() {
...

  {
    ...
    if (_builtInAecEnabled) {
      ...
      if (!_playing) {
        ...
        NLOG(LS_ERROR, "Playout must be started before recording when using",
             " the built-in AEC");
        return -1;
      }
    }
    ...
    if (_hRecThread == NULL) {
      NLOG(LS_ERROR, "failed to create the recording thread");
      return -1;
    }
  ...
  if (ret != WAIT_OBJECT_0) {
    NLOG(LS_VERBOSE, "capturing did not start up properly");
    return -1;
  }
  NLOG(LS_VERBOSE, "capture audio stream has now started...");
...
int32_t AudioDeviceWindowsCore::StopRecording() {
  ...
  if (_hRecThread == NULL) {
    NLOG(LS_VERBOSE, "no capturing stream is active => close down WASAPI only");
    SAFE_RELEASE(_ptrClientIn);
    SAFE_RELEASE(_ptrCaptureClient);
    _recIsInitialized = false;
    _recording = false;
  ...
  NLOG(LS_VERBOSE, "closing down the webrtc_core_audio_capture_thread...");
  ...
  if (ret != WAIT_OBJECT_0) {
    NLOG(LS_ERROR, "failed to close down webrtc_core_audio_capture_thread");
    err = -1;
  } else {
    NLOG(LS_VERBOSE, "webrtc_core_audio_capture_thread is now closed");
  }
  _Lock();
...
int32_t AudioDeviceWindowsCore::StartPlayout() {
...

  {
    ...
    if (_hPlayThread == NULL) {
      NLOG(LS_ERROR, "failed to create the playout thread");
      return -1;
    }
  ...
  if (ret != WAIT_OBJECT_0) {
    NLOG(LS_VERBOSE, "rendering did not start up properly");
    return -1;
  }

  _playing = true;
  NLOG(LS_VERBOSE, "rendering audio stream has now started...");
...
int32_t AudioDeviceWindowsCore::StopPlayout() {
...

  {
    ...
    // stop the driving thread...
    NLOG(LS_VERBOSE, "closing down the webrtc_core_audio_render_thread...");
  ...
  if (ret != WAIT_OBJECT_0) {
    ...
    NLOG(LS_ERROR, "failed to close down webrtc_core_audio_render_thread");
  ...
  {
    rtc::CritScope critScoped(&_critSect);
    NLOG(LS_VERBOSE, "webrtc_core_audio_render_thread is now closed");
...
DWORD AudioDeviceWindowsCore::DoRenderThread() {
  ...
  ScopedCOMInitializer comInit(ScopedCOMInitializer::kMTA);
  if (!comInit.succeeded()) {
    NLOG(LS_ERROR, "failed to initialize COM in render thread");
    return 1;
  }
  ...
  if (_winSupportAvrt) {
    ...
    if (hMmTask) {
      if (FALSE == _PAvSetMmThreadPriority(hMmTask, AVRT_PRIORITY_CRITICAL)) {
        NLOG(LS_WARNING, "failed to boost play-thread using MMCSS");
      }
      NLOG(LS_VERBOSE,
           "render thread is now registered with MMCSS (taskIndex=", taskIndex,
           ")");
    } else {
      NLOG(LS_WARNING,
           "failed to enable MMCSS on render thread (err=", GetLastError(),
           ")");
  ...
  EXIT_ON_ERROR(hr);
  NLOG(LS_VERBOSE, "[REND] size of buffer       : ", bufferLength);
  ...
  NLOG(LS_VERBOSE, "[REND] max stream latency   : ", (DWORD)latency, " (",
  ...
  NLOG(LS_VERBOSE, "[REND] device period        : ", (DWORD)devPeriod, " (",
  ...
  _writtenSamples = 0;
  NLOG(LS_VERBOSE, "[REND] initial delay        : ", playout_delay);

  double endpointBufferSizeMS =
      10.0 * ((double)bufferLength / (double)_devicePlayBlockSize);
  NLOG(LS_VERBOSE, "[REND] endpointBufferSizeMS : ", endpointBufferSizeMS);
  ...
  while (keepPlaying) {
    ...
    switch (waitResult) {
      ...
      case WAIT_TIMEOUT:  // timeout notification
        NLOG(LS_WARNING, "render event timed out after 0.5 seconds");
        goto Exit;
      default:  // unexpected error
        NLOG(LS_WARNING, "unknown wait termination on render side");
        goto Exit;
    }
    ...
    while (keepPlaying) {
      ...
      for (uint32_t n = 0; n < n10msBuffers; n++) {
        ...
        if (_ptrAudioBuffer) {
          ...
          if (nSamples == -1) {
            _UnLock();
            NLOG(LS_ERROR, "failed to read data from render client");
            goto Exit;
          }
          ...
          if (_ptrRenderClient == NULL || _ptrClientOut == NULL) {
            _UnLock();
            NLOG(LS_ERROR, "output state has been modified during unlocked",
                 " period");
            goto Exit;
          }
          if (nSamples != static_cast<int32_t>(_playBlockSize)) {
            NLOG(LS_WARNING, "nSamples(", nSamples, ") != _playBlockSize",
                 _playBlockSize, ")");
          }
...
Exit:
  ...
  if (keepPlaying) {
    ...
    NLOG(LS_ERROR, "Playout error: rendering thread has ended pre-maturely");
  } else {
    NLOG(LS_VERBOSE, "_Rendering thread is now terminated properly");
  }
  ...
  if (_winSupportAvrt) {
    ...
    if (_hMmTask) {
      if (!_PAvSetMmThreadPriority(_hMmTask, AVRT_PRIORITY_CRITICAL)) {
        NLOG(LS_WARNING, "failed to boost rec-thread using MMCSS");
      }
      NLOG(LS_VERBOSE,
           "capture thread is now registered with MMCSS (taskIndex=", taskIndex,
           ")");
    } else {
      NLOG(LS_WARNING,
           "failed to enable MMCSS on capture thread (err=", GetLastError(),
           ")");
      _TraceCOMError(GetLastError());
    }
  }
...
DWORD AudioDeviceWindowsCore::DoCaptureThreadPollDMO() {
  ...
  ScopedCOMInitializer comInit(ScopedCOMInitializer::kMTA);
  if (!comInit.succeeded()) {
    NLOG(LS_ERROR, "failed to initialize COM in polling DMO thread");
    return 1;
  }
  ...
  while (keepRecording) {
    ...
    switch (waitResult) {
      ...
      case WAIT_TIMEOUT:  // timeout notification
        break;
      default:  // unexpected error
        NLOG(LS_WARNING, "Unknown wait termination on capture side");
  ...
  if (FAILED(hr)) {
    NLOG(LS_ERROR, "Recording error: capturing thread has ended prematurely");
  } else {
    NLOG(LS_VERBOSE, "Capturing thread is now terminated properly");
  }
...
DWORD AudioDeviceWindowsCore::DoCaptureThread() {
  ...
  ScopedCOMInitializer comInit(ScopedCOMInitializer::kMTA);
  if (!comInit.succeeded()) {
    NLOG(LS_ERROR, "failed to initialize COM in capture thread");
    return 1;
  }
  ...
  if (_ptrClientIn == NULL) {
    NLOG(LS_ERROR, "input state has been modified before capture loop starts.");
    return 1;
  }
  hr = _ptrClientIn->GetBufferSize(&bufferLength);
  EXIT_ON_ERROR(hr);
  NLOG(LS_VERBOSE, "[CAPT] size of buffer       : ", bufferLength);
  ...
  NLOG(LS_VERBOSE, "[CAPT] size of sync buffer  : ", syncBufferSize,
       " [bytes]");
  ...
  NLOG(LS_VERBOSE, "[CAPT] max stream latency   : ", (DWORD)latency, " (",
       (double)(latency / 10000.0), " ms)");
  ...
  NLOG(LS_VERBOSE, "[CAPT] device period        : ", (DWORD)devPeriod, " (",
       (double)(devPeriod / 10000.0), " ms)");

  double extraDelayMS = (double)((latency + devPeriod) / 10000.0);
  NLOG(LS_VERBOSE, "[CAPT] extraDelayMS         : ", extraDelayMS);

  double endpointBufferSizeMS =
      10.0 * ((double)bufferLength / (double)_recBlockSize);
  NLOG(LS_VERBOSE, "[CAPT] endpointBufferSizeMS : ", endpointBufferSizeMS);
  ...
  while (keepRecording) {
    ...
    switch (waitResult) {
      ...
      case WAIT_OBJECT_0 + 1:  // _hCaptureSamplesReadyEvent
        break;
      case WAIT_TIMEOUT:  // timeout notification
        NLOG(LS_WARNING, "capture event timed out after 0.5 seconds");
        goto Exit;
      default:  // unexpected error
        NLOG(LS_WARNING, "unknown wait termination on capture side");
        goto Exit;
    ...
    while (keepRecording) {
      ...
      if (SUCCEEDED(hr)) {
        ...
        if (flags & AUDCLNT_BUFFERFLAGS_SILENT) {
          ...
          // actual data values.
          NLOG(LS_WARNING, "AUDCLNT_BUFFERFLAGS_SILENT");
          pData = NULL;
        }
        ...
        while (syncBufIndex >= _recBlockSize) {
          if (_ptrAudioBuffer) {
            ...
            if (_ptrCaptureClient == NULL || _ptrClientIn == NULL) {
              _UnLock();
              NLOG(LS_ERROR, "input state has been modified during",
                   " unlocked period");
              goto Exit;
            }
          }
        ...
        // client.
        NLOG(LS_ERROR, "IAudioCaptureClient::GetBuffer returned",
             " AUDCLNT_E_BUFFER_ERROR, hr = 0x", std::hex, hr, std::dec);
        goto Exit;
      }
...
Exit:
  ...
  if (keepRecording) {
    ...
    NLOG(LS_ERROR, "Recording error: capturing thread has ended pre-maturely");
  } else {
    NLOG(LS_VERBOSE, "_Capturing thread is now terminated properly");
  }
...
int AudioDeviceWindowsCore::SetDMOProperties() {
  ...
  NLOG(LS_VERBOSE, "Capture device index: ", inDevIndex,
...
int32_t AudioDeviceWindowsCore::_GetDefaultDeviceIndex(EDataFlow dir,
  ...
  if (!collection) {
    NLOG(LS_ERROR, "Device collection not valid");
    return -1;
  }
  ...
  *index = -1;
  ...
  if (*index == -1) {
    NLOG(LS_ERROR, "Unable to find collection index for default device");
    return -1;
  }
...
int32_t AudioDeviceWindowsCore::_GetDeviceName(IMMDevice* pDevice,
  ...
  if (pDevice != NULL) {
    ...
    if (FAILED(hr)) {
      NLOG(LS_ERROR, "IMMDevice::OpenPropertyStore failed, hr = 0x", std::hex,
           hr, std::dec);
    }
  }
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (FAILED(hr)) {
      NLOG(LS_ERROR, "IPropertyStore::GetValue failed, hr = 0x", std::hex, hr,
           std::dec);
    }
  }
  ...
  if ((SUCCEEDED(hr)) && (VT_EMPTY == varName.vt)) {
    hr = E_FAIL;
    NLOG(LS_ERROR, "IPropertyStore::GetValue returned no value,", " hr = 0x",
         std::hex, hr, std::dec);
  }
  ...
  if ((SUCCEEDED(hr)) && (VT_LPWSTR != varName.vt)) {
    ...
    hr = E_UNEXPECTED;
    NLOG(LS_ERROR, "IPropertyStore::GetValue returned unexpected",
...
int32_t AudioDeviceWindowsCore::_EnumerateEndpointDevicesAll(
  ...
  EXIT_ON_ERROR(hr);
  if (dataFlow == eRender)
    NLOG(LS_VERBOSE, "#rendering endpoint devices (counting all): ", count);
  else if (dataFlow == eCapture)
    NLOG(LS_VERBOSE, "#capturing endpoint devices (counting all): ", count);
  ...
  for (ULONG i = 0; i < count; i++) {
    ...
    CONTINUE_ON_ERROR(hr);
    NLOG(LS_VERBOSE, "ID string    : ", pwszID);
    ...
    hr = pProps->GetValue(PKEY_Device_FriendlyName, &varName);
    CONTINUE_ON_ERROR(hr);
    NLOG(LS_VERBOSE, "friendly name: \"", varName.pwszVal, "\"");
    ...
    if (dwState & DEVICE_STATE_ACTIVE)
      NLOG(LS_VERBOSE, "state (0x", std::hex, dwState, std::dec,
           ")  : *ACTIVE*");
    ...
    if (dwHwSupportMask & ENDPOINT_HARDWARE_SUPPORT_VOLUME)
      ...
      NLOG(LS_VERBOSE, "hwmask (0x", std::hex, dwHwSupportMask, std::dec,
    ...
    CONTINUE_ON_ERROR(hr);
    NLOG(LS_VERBOSE, "#channels    : ", nChannelCount);
    ...
    if (dwHwSupportMask & ENDPOINT_HARDWARE_SUPPORT_VOLUME) {
      ...
      hr = pEndpointVolume->GetVolumeRange(&fLevelMinDB, &fLevelMaxDB,
                                           &fVolumeIncrementDB);
      CONTINUE_ON_ERROR(hr);
      NLOG(LS_VERBOSE, "volume range : ", fLevelMinDB, " (min), ", fLevelMaxDB,
      ...
      CONTINUE_ON_ERROR(hr);
      NLOG(LS_VERBOSE, "volume steps : ", nStep, " (nStep), ", nStepCount,
           " (nStepCount)");
    }
  Next:
    if (FAILED(hr)) {
      NLOG(LS_VERBOSE, "Error when logging device information");
    }
    CoTaskMemFree(pwszID);
    pwszID = NULL;
    PropVariantClear(&varName);
...
void AudioDeviceWindowsCore::_TraceCOMError(HRESULT hr) const {
  ...
  NLOG(LS_ERROR, "Core Audio method failed (hr=", hr, ")");
  StringCchPrintf(buf, MAXERRORLENGTH, TEXT("Error details: "));
  StringCchCat(buf, MAXERRORLENGTH, errorText);
  NLOG(LS_ERROR, WideToUTF8(buf));
}
