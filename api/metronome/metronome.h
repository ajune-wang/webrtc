...
class RTC_EXPORT Metronome {
 public:
  ...
  // Requests a call to `callback` on the next tick. Scheduled
  // callbacks are executed on the same sequence as they were requested on.
  // There are no features for cancellation. When that's needed, use e.g.
  // ScopedTaskSafety from the client.
