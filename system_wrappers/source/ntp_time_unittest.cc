...
TEST(NtpTimeTest, VerifyUInt64MsToUQ32x32NearZero) {
  // Zero
  EXPECT_EQ(UInt64MsToUQ32x32(0), uint64_t{0});

  // Zero + 1 millisecond
  EXPECT_EQ(UInt64MsToUQ32x32(1), uint64_t{kOneMsQ32x32});

  // Zero + 1 second
  EXPECT_EQ(UInt64MsToUQ32x32(1000), uint64_t{kOneSecQ32x32});
}
...
TEST(NtpTimeTest, VerifyUQ32x32ToUInt64MsNearZero) {
  // Zero
  EXPECT_EQ(UQ32x32ToUInt64Ms(0), uint64_t{0});

  // Zero + 1 millisecond
  EXPECT_EQ(UQ32x32ToUInt64Ms(kOneMsQ32x32), uint64_t{1});

  // Zero + 1 second
  EXPECT_EQ(UQ32x32ToUInt64Ms(kOneSecQ32x32), uint64_t{1000});
}
...
TEST(NtpTimeTest, VerifyUInt64MsToUQ32x32NearMax) {
  constexpr uint64_t kMaxUQ32x32 = std::numeric_limits<uint64_t>::max();
  constexpr uint64_t kBoundaryMs = (kMaxUQ32x32 >> 32) * 1000 + 999;

  // Max
  const uint64_t boundary_uq32x32 = UInt64MsToUQ32x32(kBoundaryMs);
...
TEST(NtpTimeTest, VerifyUQ32x32ToUInt64MsNearMax) {
  constexpr uint64_t kMaxUQ32x32 = std::numeric_limits<uint64_t>::max();
  constexpr uint64_t kBoundaryMs = (kMaxUQ32x32 >> 32) * 1000 + 1000;

  // Max
  EXPECT_EQ(UQ32x32ToUInt64Ms(kMaxUQ32x32), kBoundaryMs);
}
...
TEST(NtpTimeTest, VerifyUInt64MsToUQ32x32RoundTrip) {
  constexpr int kIterations = 50000;

  std::mt19937 generator(123456789);
  std::uniform_int_distribution<uint64_t> distribution(
      UQ32x32ToUInt64Ms(std::numeric_limits<uint64_t>::min()),
      UQ32x32ToUInt64Ms(std::numeric_limits<uint64_t>::max()));

  for (int iteration = 0; iteration < kIterations; ++iteration) {
    uint64_t input_ms = distribution(generator);
    uint64_t transit_uq32x32 = UInt64MsToUQ32x32(input_ms);
    uint64_t output_ms = UQ32x32ToUInt64Ms(transit_uq32x32);
