...
uint32_t VideoSendStreamImpl::OnBitrateUpdated(BitrateAllocationUpdate update) {
  ...
  DataRate encoder_stable_target_rate = update.target_bitrate;
  // if (encoder_stable_target_rate > overhead) {
  encoder_stable_target_rate = encoder_stable_target_rate - overhead;
  //} else {
  fprintf(stdout,
          "=========== PLOT:{\"target\":%ld, \"stable\":%ld, "
          "\"link_allocation\":%ld, \"overhead\":%ld }\n",
          update.target_bitrate.bps(), update.stable_target_bitrate.bps(),
          link_allocation.bps(), overhead.bps());
  //}
