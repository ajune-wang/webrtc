...
class WgcCapturerWinTest : public ::testing::TestWithParam<CaptureType>,
 ...
 public:
  ...
  void CreateWindowOnSeparateThread(int window_width, int window_height) {
    RTC_DLOG(LS_INFO) << __func__ << "(" << window_width << "," << window_height
                      << ")";

    DPI_AWARENESS_CONTEXT dpi_awareness_context =
        DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2;
    if (!::SetProcessDpiAwarenessContext(dpi_awareness_context)) {
      ...
      if (!::SetProcessDpiAwareness(process_dpi_awareness)) {
        ...
        RTC_DCHECK(result) << "SetProcessDPIAware failed.";
      }
    }
  ...
  void DoStartCaptureOnWorkerThread(DesktopCapturer::Callback* callback) {
    ...
    worker_thread_->PostTask(
        [this, callback]() { capturer_->Start(callback); });
  }

  void DoCaptureOnWorkerThread() {
    EXPECT_TRUE(worker_thread_);
    worker_thread_->PostTask([this]() { capturer_->CaptureFrame(); });
  }
...
TEST_F(WgcCapturerWindowTest, IncreaseWindowSizeMidCapture) {
  ...
  SleepMs(5000);
  // Wait for callback.
  run_loop_.task_queue()->PostDelayedTask([this]() { run_loop_.Quit(); },
                                          TimeDelta::Seconds(1));
  run_loop_.Run();
