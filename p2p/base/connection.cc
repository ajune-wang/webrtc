...
void Connection::SendStunBindingResponse(const StunMessage* message) {
  ...
  RTC_LOG(LS_ERROR) << "DTLS PONG BINDING RESPONSE dtls_sz="
                    << dtls_buffer_.size() << " " << this
                    << " configured=" << dtls_in_stun_;
  if (dtls_in_stun_ && dtls_buffer_.size()) {
...
std::unique_ptr<IceMessage> Connection::BuildPingRequest(
  ...
  RTC_LOG(LS_ERROR) << "DTLS PING dtls_sz=" << dtls_buffer_.size() << " "
                    << this;
  if (dtls_in_stun_ && dtls_buffer_.size()) {
    message->AddAttribute(std::make_unique<StunByteStringAttribute>(
        STUN_ATTR_META_DTLS_IN_STUN, dtls_buffer_.data(), dtls_buffer_.size()));
