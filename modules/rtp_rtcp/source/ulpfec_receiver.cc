...
void UlpfecReceiver::ProcessReceivedFec() {
  ...
  for (const auto& received_packet : received_packets) {
    ...
    if (!received_packet->is_fec) {
      ...
      // Some header extensions need to be zeroed in `packet->data`, since they
      // are written to a packet after FEC encoding. We try to do it without a
      // copy, but if a reference is still held by
      // 'recovered_packet_callback_->OnRecoveredPacket' a copy will still be
      // made in 'rtp_packet.ZeroMutableExtensions()'.
