...
BandwidthScaler::GetBitrateLimitsForResolution(int width, int height) {
  if (width <= 0 || height <= 0) {
    return absl::nullopt;
  }
  ...
  float alpha = ((width * height) - lower_pixel_count) /
                (upper_pixel_count - lower_pixel_count);
  int min_bps = static_cast<int>(
      limits[interpolation_index].min_bitrate_bps * alpha +
      limits[interpolation_index - 1].min_bitrate_bps * (1.0 - alpha));
  int max_bps = static_cast<int>(
      limits[interpolation_index].max_bitrate_bps * alpha +
      limits[interpolation_index - 1].max_bitrate_bps * (1.0 - alpha));

  if (max_bps >= min_bps) {
    return VideoEncoder::ResolutionBitrateLimits(width * height, min_bps,
                                                 min_bps, max_bps);
  } else {
    RTC_LOG(LS_WARNING)
        ...
        << " encode_width = " << width << " encode_height = " << height
        << " min_bps = " << min_bps << " max_bps=" << max_bps;
    return absl::nullopt;
  }
}
...
BandwidthScaler::CheckBitrateResult BandwidthScaler::CheckBitrate() {
  ...
  absl::optional<VideoEncoder::ResolutionBitrateLimits> suitable_bitrate_limit =
      GetBitrateLimitsForResolution(frame_info_->encoded_width,
