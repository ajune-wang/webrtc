...
void NetworkNodeTransport::OnPacketReceived(rtc::AsyncSocket* socket) {
  ...
  int64_t timestamp;
  size_t len = socket->Recv(buffer, sizeof(buffer), &timestamp);
  receiver_(rtc::CopyOnWriteBuffer(buffer, len, len), timestamp);
}

void NetworkNodeTransport::Bind(EndpointNode* local_endpoint,
                                EndpointNode* remote_endpoint) {
  if (current_local_endpoint_ && current_remote_endpoint_ &&
      current_local_endpoint_.value() == local_endpoint &&
      current_remote_endpoint_.value() == remote_endpoint) {
    return;
  }

  rtc::AsyncSocket* socket =
      socket_factory_->CreateAsyncSocket(AF_INET, SOCK_DGRAM);
  // Bind socket to the local endpoint IP. Port equals to 0 to force endpoint
  // to pick one.
  int res = socket->Bind(
      rtc::SocketAddress(local_endpoint->GetPeerLocalAddress(), 0));
  RTC_CHECK_EQ(res, 0);

  current_local_endpoint_ = local_endpoint;
  current_remote_endpoint_ = remote_endpoint;
...
void NetworkNodeTransport::Connect(rtc::SocketAddress remote_addr,
  ...
  {
    rtc::CritScope crit(&lock_);
    socket = sockets_.back();
    // TODO(titovartem) DCHECK state of the last socket.
    int res = socket->Connect(remote_addr);
    RTC_CHECK_EQ(res, 0);
    packet_overhead_ = packet_overhead;
  }

  // We need to make this action without holding |lock_| because during connect
  // we will first acquire |lock_| and then lock inside
  // |socket->SignalReadEvent|, but data receiving thread during sending RTCP
  // feedback will first acquire lock in |socket->SignalReadEvent| and then
  // |lock_| inside SendRtcp(...) method. Hypothetically it can lead to the dead
  // lock. To avoid this situation we won't hold a lock here. Actually Bind
  // and Connect will be invoked by Scenario while network layer is idle,
  // so no packets will be delivered during this call.
  socket->SignalReadEvent.connect(this,
                                  &NetworkNodeTransport::OnPacketReceived);

  rtc::NetworkRoute route;
  route.connected = true;
  route.local_network_id = current_remote_endpoint_.value()->GetId();
  route.remote_network_id = current_remote_endpoint_.value()->GetId();
