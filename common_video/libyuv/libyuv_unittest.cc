...
TEST_F(TestLibYuv, RotateTest) {
  ...
  int y_size = width_ * height_;
  int u_size = rotated_res_i420_buffer->ChromaWidth() *
               rotated_res_i420_buffer->ChromaHeight();

  int ret =
      libyuv::I420Rotate(orig_buffer, width_, orig_buffer + y_size, width_ >> 1,
                         orig_buffer + y_size + u_size, width_ >> 1,
                         rotated_res_i420_buffer.get()->MutableDataY(),
                         rotated_res_i420_buffer.get()->StrideY(),
                         rotated_res_i420_buffer.get()->MutableDataU(),
                         rotated_res_i420_buffer.get()->StrideU(),
                         rotated_res_i420_buffer.get()->MutableDataV(),
                         rotated_res_i420_buffer.get()->StrideV(),
                         rotated_width, rotated_height, libyuv::kRotate90);
  ...
  ret =
      libyuv::I420Rotate(orig_buffer, width_, orig_buffer + y_size, width_ >> 1,
                         ...
                         rotated_res_i420_buffer.get()->MutableDataV(),
                         rotated_res_i420_buffer.get()->StrideV(),
                         rotated_width, rotated_height, libyuv::kRotate270);
  EXPECT_EQ(0, ret);
  ...
  ret =
      libyuv::I420Rotate(orig_buffer, width_, orig_buffer + y_size, width_ >> 1,
                         ...
                         rotated_res_i420_buffer.get()->StrideU(),
                         rotated_res_i420_buffer.get()->MutableDataV(),
                         rotated_res_i420_buffer.get()->StrideV(), width_,
                         height_, libyuv::kRotate180);
  EXPECT_EQ(0, ret);
}
