...
TEST_P(PeerConnectionEncodingsIntegrationParameterizedTest,
  ...
  parameters.encodings[1].active = false;
  parameters.encodings[2].active = false;
  RTC_LOG(LS_INFO) << " Disable all!";
  sender->SetParameters(parameters);

  rtc::Thread::Current()->ProcessMessages(1000);

  // Wait until the encoder has encoded another 10 frames on lower layers.
  // ASSERT_TRUE_WAIT(EncodedFrames(local_pc_wrapper, "q") > encoded_frames_q +
  // 10,
  //               kDefaultTimeout.ms());
  // ASSERT_TRUE_WAIT(EncodedFrames(local_pc_wrapper, "h") > encoded_frames_h +
  // 10,
  //              kDefaultTimeout.ms());
  EXPECT_LT(EncodedFrames(local_pc_wrapper, "f") - encoded_frames_f, 3);
  EXPECT_LT(EncodedFrames(local_pc_wrapper, "h") - encoded_frames_h, 3);
  EXPECT_LT(EncodedFrames(local_pc_wrapper, "q") - encoded_frames_q, 3);
}
