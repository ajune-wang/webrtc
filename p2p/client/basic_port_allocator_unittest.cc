...
TEST_F(BasicPortAllocatorTest, TestBasic) {
  ...
  EXPECT_EQ(RELAY_TURN, allocator().turn_servers()[0].type);
  ...
  EXPECT_TRUE(HasRelayAddress(ProtocolAddress(kTurnUdpIntAddr, PROTO_UDP)));
...
TEST_F(BasicPortAllocatorTest, TestGetAllPortsNoUdpAllowed) {
  ...
  EXPECT_TRUE(HasCandidate(candidates_, "local", "udp", kClientAddr));
  EXPECT_TRUE(HasCandidate(candidates_, "local", "tcp", kClientAddr));
  // TODO(nisse): How to update this test? What's supposed to happen after 3s?
  // Now fail because we never get more than 2 candidates and ports.

  // RelayPort connection timeout is 3sec. TCP connection with RelayServer
  // will be tried after about 3 seconds.
  EXPECT_EQ_SIMULATED_WAIT(3U, candidates_.size(), 3500, fake_clock);
  EXPECT_EQ(3U, ports_.size());
