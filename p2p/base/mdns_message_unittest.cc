...
const unsigned char kSingleQuestionForIPv4AddrWithUnicastResponse[] = {
    0x12, 0x34,                                // ID
    0x00, 0x00,                                // flags
...
const unsigned char kTwoQuestionsForIPv4AndIPv6AddrWithMulticastResponse[] = {
    0x12, 0x34,                                      // ID
    0x00, 0x00,                                      // flags
...
const unsigned char kSingleAuthoritativeAnswerWithIPv4Addr[] = {
    0x12, 0x34,                                // ID
    0x84, 0x00,                                // flags
...
const unsigned char kTwoAuthoritativeAnswersWithIPv4AndIPv6Addr[] = {
    0x12, 0x34,                                      // ID
    0x84, 0x00,                                      // flags
    0x00, 0x00,                                      // number of questions
...
bool ReadMDnsMessageTestCase(webrtc::MDnsMessage* msg,
                             ...
                             size_t size) {
  webrtc::MDnsMessageBufferReader buf(reinterpret_cast<const char*>(testcase),
                                      size);
  return msg->Read(&buf);
}
...
bool WriteMDnsMessageAndCompareWithTestCast(webrtc::MDnsMessage* msg,
  ...
  webrtc::MDnsMessageBufferReader read_buf(out);
  std::string bytes;
...
TEST_F(MDnsMessageTest, ReadSingleQuestionForIPv4Address) {
  MDnsMessage msg;
  ASSERT_TRUE(
...
TEST_F(MDnsMessageTest, ReadTwoQuestionsForIPv4AndIPv6Addr) {
  MDnsMessage msg;
...
TEST_F(MDnsMessageTest, ReadTwoQuestionsForIPv4AndIPv6AddrWithNameCompression) {
  MDnsMessage msg;
  ASSERT_TRUE(ReadMDnsMessage(
      &msg,
      kTwoQuestionsForIPv4AndIPv6AddrWithMulticastResponseAndNameCompression));
  ...
  EXPECT_TRUE(GetQueriedNames(&msg, &queried_names));
  EXPECT_TRUE(queried_names.size() == 2 &&
              queried_names.find("www.webrtc.org.") != queried_names.end() &&
              queried_names.find("mdns.webrtc.org.") != queried_names.end());
}

TEST_F(MDnsMessageTest, ReadSingleAnswerForIPv4Addr) {
  MDnsMessage msg;
...
TEST_F(MDnsMessageTest, ReadTwoAnswersForIPv4AndIPv6Addr) {
  MDnsMessage msg;
  ASSERT_TRUE(
...
TEST_F(MDnsMessageTest, ReadTwoAnswersForIPv4AndIPv6AddrWithNameCompression) {
  MDnsMessage msg;
  ASSERT_TRUE(ReadMDnsMessage(
  ...
  EXPECT_TRUE(GetResolution(&msg, &resolution));
  EXPECT_TRUE(resolution.size() == 2 &&
              resolution.find("www.webrtc.org.") != resolution.end() &&
              resolution["www.webrtc.org."].ToString() == "192.168.0.1" &&
              resolution.find("webrtc.org.") != resolution.end() &&
              resolution["webrtc.org."].ToString() == "fd12:3456:789a:1::1");
}

TEST_F(MDnsMessageTest, WriteSingleQuestionForIPv4Addr) {
  MDnsMessage msg;
  msg.SetId(0x1234);
  ...
  EXPECT_TRUE(WriteMDnsMessageAndCompare(
      &msg, kSingleQuestionForIPv4AddrWithUnicastResponse));
}

TEST_F(MDnsMessageTest, WriteTwoQuestionsForIPv4AndIPv6Addr) {
  MDnsMessage msg;
  msg.SetId(0x1234);
  msg.SetQueryOrResponse(true);
...
TEST_F(MDnsMessageTest, WriteSingleAnswerToIPv4Addr) {
  MDnsMessage msg;
  msg.SetId(0x1234);
...
TEST_F(MDnsMessageTest, WriteTwoAnswersToIPv4AndIPv6Addr) {
  MDnsMessage msg;
  msg.SetId(0x1234);
  msg.SetQueryOrResponse(false);
  msg.SetAuthoritative(true);
  ...
  EXPECT_TRUE(WriteMDnsMessageAndCompare(
      &msg, kTwoAuthoritativeAnswersWithIPv4AndIPv6Addr));
}
