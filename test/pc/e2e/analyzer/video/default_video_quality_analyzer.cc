...
void DefaultVideoQualityAnalyzer::ReportResults(
  ...
  ReportResult("psnr", stats.psnr, "dB");
  ReportResult("ssim", stats.psnr, "unitless");
  ReportResult("transport_time", stats.transport_time_ms, "ms");
  ReportResult("total_delay_incl_transport",
               stats.total_delay_incl_transport_ms, "ms");
  ReportResult("time_between_rendered_frames",
               stats.time_between_rendered_frames_ms, "ms");
  if (!stats.encode_frame_rate.IsEmpty()) {
    test::PrintResult("min_psnr", "", test_label_,
                      stats.encode_frame_rate.GetEventsPerSecond(), "unitless",
                      false);
  }
  ReportResult("encode_time", stats.encode_time_ms, "ms");
  ReportResult("time_between_freezes", stats.time_between_freezes_ms, "ms");
  ReportResult("pixels_per_frame", stats.encoded_pix, "unitless");
  if (!stats.psnr.IsEmpty()) {
    test::PrintResult("min_psnr", "", test_label_, stats.psnr.GetMin(),
                      "unitless", false);
  }
  ReportResult("decode_time", stats.decode_time_ms, "ms");
  test::PrintResult("dropped_frames", "", test_label_,
                    stream_frame_counters_.at(stream_label).dropped, "unitless",
                    false);
  test::PrintResult("freeze_200ms", "", test_label_, stats.freeze_200_ms, "ms",
                    false);
  test::PrintResult("freeze_1s", "", test_label_, stats.freeze_1s, "ms", false);
  ReportResult("max_skipped", stats.skipped_between_rendered, "unitless");
}
...
SamplesStatsCounter DefaultVideoQualityAnalyzer::GetComparisonsQueueSize()
    const {
  rtc::CritScope crit(&comparison_lock_);
  return comparisons_queue_on_add_size_;
}
...
void DefaultVideoQualityAnalyzer::AddComparison(
  ...
  rtc::CritScope crit(&comparison_lock_);
  comparisons_queue_on_add_size_.AddSample(comparisons_.size());
...
void DefaultVideoQualityAnalyzer::ReportResult(
  ...
  test::PrintResultMeanAndError(name, "", test_label_, counter.GetAverage(),
                                counter.GetStandardDeviation(), unit, false);
}
