...
// queuing delay of the network link can be estimated.
//
// The filter takes as input the frame delay delta and frame size delta, for a
// single frame. The frame delay delta is seen as the measurement, and the
// frame size delta is used in the observation model. The hidden state of the
// filter is the link bandwidth and queuing delay. The estimated state can
// be used to get the expected frame delay delta for a frame, given it's frame
// size delta. This information can then be used to estimate the frame delay
// variation coming from network jitter.
//
// Mathematical details:
//  * The state (`x` in Wikipedia notation) is a 2x1 vector comprising the
//    reciprocal of link bandwidth [1 / bytes per ms] and the
//    link queuing delay [ms].
//  * The state transition matrix (`F`) is the 2x2 identity matrix, meaning that
//    link bandwidth and link queuing delay are modeled as independent.
...
class FrameDelayDeltaKalmanFilter {
 public:
  ...
  // Given a frame size delta, returns the estimated frame delay delta explained
  // by both link bandwidth and link queuing delay.
