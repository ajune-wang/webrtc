 ...
 public:
  InitialiezedUsrSctp(rtc::Thread* thread, SctpConnOutput conn_output)
    ...
    usrsctp_thread_->Invoke<void>(
        RTC_FROM_HERE, [this, conn_output] { Initialize(conn_output); });
  }

  ~InitialiezedUsrSctp() {
    RTC_LOG(LS_INFO) << __FUNCTION__;
    usrsctp_thread_->Invoke<void>(RTC_FROM_HERE, [this] { Deinitialize(); });
  }
  ...
  int GetLAddrs(struct socket* so, sctp_assoc_t id, struct sockaddr** raddrs) {
    if (!usrsctp_thread_->IsCurrent()) {
      return usrsctp_thread_->Invoke<int>(
          RTC_FROM_HERE,
          [this, so, id, raddrs] { return GetLAddrs(so, id, raddrs); });
    }
  ...
  void FreeLAddrs(struct sockaddr* addrs) {
    if (!usrsctp_thread_->IsCurrent()) {
      return usrsctp_thread_->Invoke<void>(
          RTC_FROM_HERE, [this, addrs] { FreeLAddrs(addrs); });
    }
  ...
  int SetSockOpts(struct socket* so,
    ...
    if (!usrsctp_thread_->IsCurrent()) {
      return usrsctp_thread_->Invoke<int>(
          RTC_FROM_HERE,
          [this, so, level, option_name, option_value, option_len] {
            return SetSockOpts(so, level, option_name, option_value,
                               option_len);
          });
    }
  ...
  struct socket* Socket(int domain,
    ...
    if (!usrsctp_thread_->IsCurrent()) {
      return usrsctp_thread_->Invoke<struct socket*>(
          RTC_FROM_HERE, [this, domain, type, protocol, receive_cb, send_cb,
                          sb_threshold, ulp_info, error_number] {
            return Socket(domain, type, protocol, receive_cb, send_cb,
                          sb_threshold, ulp_info, error_number);
          });
    }
  ...
  uint32_t GetSctpSendspace() {
    if (!usrsctp_thread_->IsCurrent()) {
      return usrsctp_thread_->Invoke<uint32_t>(
          RTC_FROM_HERE, [this] { return GetSctpSendspace(); });
    }
  ...
  void RegisterAddress(void* addr) {
    if (!usrsctp_thread_->IsCurrent()) {
      return usrsctp_thread_->Invoke<void>(
          RTC_FROM_HERE, [this, addr] { RegisterAddress(addr); });
    }
  ...
  void DeRegisterAddress(void* addr) {
    if (!usrsctp_thread_->IsCurrent()) {
      return usrsctp_thread_->Invoke<void>(
          RTC_FROM_HERE, [this, addr] { DeRegisterAddress(addr); });
    }
  ...
  void ConnInput(void* addr,
    ...
    if (!usrsctp_thread_->IsCurrent()) {
      return usrsctp_thread_->Invoke<void>(
          RTC_FROM_HERE, [this, addr, buffer, length, ecn_bits] {
  ...
  int SetNonBlocking(struct socket* so, int onoff) {
    if (!usrsctp_thread_->IsCurrent()) {
      return usrsctp_thread_->Invoke<int>(RTC_FROM_HERE, [this, so, onoff] {
        return SetNonBlocking(so, onoff);
      });
    }
...
class UsrSctpManager {
 ...
 private:
  void InitializeUsrSctp() RTC_EXCLUSIVE_LOCKS_REQUIRED(lock_) {
    RTC_LOG(LS_INFO) << __FUNCTION__;
  ...
  void UninitializeUsrSctp() RTC_EXCLUSIVE_LOCKS_REQUIRED(lock_) {
    RTC_LOG(LS_INFO) << __FUNCTION__;
    RTC_DCHECK(usrsctp_ != nullptr);
    usrsctp_.reset();
    thread_.reset();
  ...
  int usrsctp_usage_count RTC_GUARDED_BY(lock_) = 0;
};
...
int SctpTransport::UsrSctpWrapper::OnSctpOutboundPacket(void* addr,
  ...
  transport->network_thread_->PostTask(ToQueuedTask(
      ...
      [transport, buf] { transport->OnPacketFromSctpToNetwork(buf); }));
  return 0;
}
...
int SctpTransport::UsrSctpWrapper::OnSctpInboundPacket(
  ...
  transport->network_thread_->PostTask(ToQueuedTask(
      transport->task_safety_, [transport, data, length, rcv, flags] {
...
int SctpTransport::UsrSctpWrapper::SendThresholdCallback(struct socket* sock,
  ...
  if (transport->network_thread_->IsCurrent()) {
    ...
    transport->network_thread_->PostTask(
        ToQueuedTask(transport->task_safety_,
                     [transport] { transport->OnSendThresholdCallback(); }));
  }
