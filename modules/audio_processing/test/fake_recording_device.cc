...
// dB-scale fake recording device. Valid levels are [0, 255]. The mic applies
// (level-100) db of gain.
...
class FakeRecordingDeviceAgc1 final : public FakeRecordingDeviceWorker {
 public:
  ...
  void ModifyBufferInt16(AudioFrame* buffer) override {
    ...
    const float factor_linear = DbToRatio(kGainMap[mic_level_ - 52] - kGainMap[undo_level - 52]);
