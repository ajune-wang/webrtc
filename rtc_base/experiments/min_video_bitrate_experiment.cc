...
DataRate GetMinVideoBitrate(VideoCodecType type) {
  const DataRate kDefaultMinVideoBitrate =
      DataRate::bps(kDefaultMinVideoBitrateBps);
  ...
  if (webrtc::field_trial::IsEnabled(kMinVideoBitrateExperiment)) {
    ...
    switch (type) {
      case kVideoCodecVP8:
        return min_bitrate_vp8 ? *min_bitrate_vp8 : kDefaultMinVideoBitrate;
      case kVideoCodecVP9:
        return min_bitrate_vp9 ? *min_bitrate_vp9 : kDefaultMinVideoBitrate;
      case kVideoCodecH264:
        return min_bitrate_h264 ? *min_bitrate_h264 : kDefaultMinVideoBitrate;
      case kVideoCodecGeneric:
      case kVideoCodecMultiplex:
        return kDefaultMinVideoBitrate;
    }
