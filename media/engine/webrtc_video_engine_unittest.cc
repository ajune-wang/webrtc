...
TEST_F(WebRtcVideoChannelTest, SetRtpParametersForH265ShouldIgnoreLevelId) {
  encoder_factory_->AddSupportedVideoCodec(
  ...
  // This will configure the negotiated level-id to 156, though supported
  // highest level-id is 180. Following level-id setting without changing other
  // params should succeed ignoring the new level-id value.
  EXPECT_TRUE(send_channel_->SetSenderParameters(send_parameters));
  FakeVideoSendStream* stream = AddSendStream();
  ASSERT_TRUE(stream);
  ...
  EXPECT_EQ(1u, parameters2.encodings.size());
  matched_codec.parameters["level-id"] = "120";
  parameters2.encodings[0].codec = matched_codec;

  EXPECT_TRUE(
      send_channel_->SetRtpSendParameters(last_ssrc_, parameters2).ok());
  webrtc::RtpParameters parameters3 =
      send_channel_->GetRtpSendParameters(last_ssrc_);

  for (const auto& codec : parameters3.codecs) {
    if (absl::EqualsIgnoreCase(codec.name, "H265")) {
      EXPECT_EQ(codec.parameters.at("level-id"), "156");
    }
  }
