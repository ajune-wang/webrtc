...
VCMDecodedFrameCallback::VCMDecodedFrameCallback(VCMTiming* timing,
    ...
    : _clock(clock),
      ...
      _include_predecode_buffer("include_predecode_buffer", true) {
  ntp_offset_ =
      _clock->CurrentNtpInMilliseconds() - _clock->TimeInMilliseconds();

  ParseFieldTrial({&_extra_decode_time},
                  field_trial::FindFullName("WebRTC-SlowDownDecoder"));
  ParseFieldTrial({&_include_predecode_buffer},
...
void VCMDecodedFrameCallback::Decoded(VideoFrame& decodedImage,
  ...
  if (low_latency_field_trial && frameInfo->playout_delay.min_ms == 0 &&
    ...
    if (!_include_predecode_buffer || *_include_predecode_buffer) {
