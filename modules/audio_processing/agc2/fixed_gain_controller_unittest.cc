...
TEST(AutomaticGainController2FixedDigital, CheckSaturationBehaviorWithLimiter) {
  ...
  for (const auto gain_db : gains_no_saturation) {
    // The positive margin |test::kLimiterMaxInputLevelDbFs| - |gain| prevents
  ...
  for (const auto gain_db : gains_saturation) {
    ...
    // |test::kLimiterMaxInputLevelDbFs| - |gain| leads  to saturation (for
    // input levels equal to or greater than the margin).
