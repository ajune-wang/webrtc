...
void GetForcedFallbackParamsFromFieldTrialGroup(int* param_min_pixels,
  ...
  if (sscanf(group.c_str(), "Enabled-%d,%d,%d", &min_pixels, &max_pixels,
             &min_bps) != 3) {
    NLOG(LS_WARNING, "Invalid number of forced fallback parameters provided.");
    return;
  }
  if (min_pixels <= 0 || max_pixels < minimum_max_pixels ||
      max_pixels < min_pixels || min_bps <= 0) {
    NLOG(LS_WARNING, "Invalid forced fallback parameter value provided.");
    return;
  }
...
bool VideoEncoderSoftwareFallbackWrapper::InitFallbackEncoder() {
  NLOG(LS_WARNING, "Encoder falling back to software encoding.");
  ...
  if (!use_fallback_encoder_) {
    NLOG(LS_ERROR, "Failed to initialize software-encoder fallback.");
...
bool VideoEncoderSoftwareFallbackWrapper::TryInitForcedFallbackEncoder() {
  ...
  NLOG(LS_INFO, "Request forced SW encoder fallback: ", codec_settings_.width,
       "x", codec_settings_.height);
  if (!InitFallbackEncoder()) {
    return false;
  }
...
bool VideoEncoderSoftwareFallbackWrapper::TryReInitForcedFallbackEncoder() {
  ...
  if (!forced_fallback_.IsValid(codec_settings_)) {
    NLOG(LS_INFO, "Stop forced SW encoder fallback, max pixels exceeded.");
    return false;
  }
  ...
  if (fallback_encoder_->InitEncode(&codec_settings_, number_of_cores_,
    ...
    NLOG(LS_ERROR, "Failed to init forced SW encoder fallback.");
    return false;
  }
  return true;
}
...
void VideoEncoderSoftwareFallbackWrapper::ValidateSettingsForForcedFallback() {
  ...
  if (!IsForcedFallbackPossible(codec_settings_)) {
    ...
    NLOG(LS_INFO, "Disable forced_fallback_possible_ due to settings.");
