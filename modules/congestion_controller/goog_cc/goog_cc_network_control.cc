...
NetworkControlUpdate UpdateFromProbes(
    std::vector<ProbeClusterConfig> probe_configs) {
  NetworkControlUpdate update;
  update.probe_cluster_configs = probe_configs;
  return update;
}

void AppendProbeConfigs(NetworkControlUpdate* update,
                        std::vector<ProbeClusterConfig> probe_configs) {
  if (update->probe_cluster_configs.empty()) {
    update->probe_cluster_configs.swap(probe_configs);
  } else {
    for (ProbeClusterConfig& probe : probe_configs)
...
NetworkControlUpdate GoogCcNetworkController::OnNetworkAvailability(
  ...
  return UpdateFromProbes(probe_controller_->OnNetworkAvailability(msg));
}
...
NetworkControlUpdate GoogCcNetworkController::OnNetworkRouteChange(
  ...
  NetworkControlUpdate update = UpdateFromProbes(probes);
...
NetworkControlUpdate GoogCcNetworkController::OnProcessInterval(
  ...
  AppendProbeConfigs(&update, probe_controller_->Process(msg.at_time.ms()));
  MaybeTriggerOnNetworkChanged(&update, msg.at_time);
  return update;
}
...
NetworkControlUpdate GoogCcNetworkController::OnTransportPacketsFeedback(
  ...
  if (result.recovered_from_overuse) {
    ...
    AppendProbeConfigs(&update, probes);
  }
