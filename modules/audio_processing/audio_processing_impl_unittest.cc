 ...
 public:
  MockEchoControlFactory() : num_created_mocks_(0) {
    for (auto& mock : mocks_) {
      mock.reset(new MockEchoControl());
    }
  }
  ...
  MockEchoControl* GetMockEchoControl() const { return mocks_.back().get(); }
  std::unique_ptr<EchoControl> Create(int sample_rate_hz) {
    RTC_DCHECK_LT(num_created_mocks_, mocks_.size())
        << "The factory cannot create too many MockEchoControl instances.";
    return std::move(mocks_[num_created_mocks_++]);
  }
