...
TEST(ByteBufferTest, TestReadWriteBuffer) {
  ByteBufferWriter buffer;
  ByteBufferReader read_buf(static_cast<const char*>(nullptr), 0);
  uint8_t ru8;
...
TEST(ByteBufferTest, TestReadWriteUVarint) {
  ...
  EXPECT_EQ(size, read_buffer.size());
