...
FullyConnectedLayer::FullyConnectedLayer(
    ...
    : input_size_(input_size),
      ...
      output_(output_size) {
  RTC_DCHECK_EQ(output_.size(), bias_.size())
      << "Mismatching output size and bias terms array size (" << layer_name
      << ").";
  RTC_DCHECK_EQ(input_size_ * output_.size(), weights_.size())
...
void FullyConnectedLayer::ComputeOutput(rtc::ArrayView<const float> input) {
  ...
  if (cpu_features_.sse2) {
    ComputeOutputSse2(input, bias_, weights_, activation_function_, output_);
    return;
  }
#endif
  ...
  for (int o = 0; o < rtc::dchecked_cast<int>(output_.size()); ++o) {
    output_[o] = bias_[o];
...
#if defined(WEBRTC_ARCH_X86_FAMILY)
void FullyConnectedLayer::ComputeOutputSse2(
  ...
  RTC_DCHECK_EQ(bias.size(), output.size());
  RTC_DCHECK_EQ(weights.size(), input.size() * output.size());
  const int input_size_by_4 = rtc::dchecked_cast<int>(input.size()) >> 2;
  const int offset = rtc::dchecked_cast<int>(input.size()) & ~3;
  // TODO(bugs.chromium.org/10480): Check if reinterpret_cast below is ok.
  __m128 sum_wx_128;
  const float* v = reinterpret_cast<const float*>(&sum_wx_128);
  for (int o = 0; o < rtc::dchecked_cast<int>(output.size()); ++o) {
    // Perform 128 bit vector operations.
    sum_wx_128 = _mm_set1_ps(0);
    const float* x_p = input.data();
    const float* w_p = weights.data() + o * input.size();
    ...
    output[o] = activation_function(
        std::inner_product(input.begin() + offset, input.end(),
                           weights.begin() + o * input.size() + offset,
