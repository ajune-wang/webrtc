...
RTCError JsepTransportController::ApplyDescription(
  ...
  for (const ContentInfo& content_info : description->contents()) {
    if (content_info.rejected ||
        (bundle_group_ && content_info.name != bundled_mid() &&
  ...
  for (size_t i = 0; i < description->contents().size(); ++i) {
    ...
    // BaseChannel/SctpTransport to change the RtpTransport/DtlsTransport first,
    // then destroy the JsepTransport2.
    if (content_info.rejected ||
        (bundle_group_ && content_info.name != bundled_mid() &&
         bundle_group_->HasContentName(content_info.name))) {
      SignalRtpTransportOrDtlsTransportChanged(content_info);
...
void JsepTransportController::SignalRtpTransportOrDtlsTransportChanged(
    const cricket::ContentInfo& content_info) {
  if (content_info.rejected) {
    if (content_info.type == cricket::MediaProtocolType::kRtp) {
      RTC_LOG(INFO) << "SignalRtpTransportChanged:" << content_info.name;
    ...
    if (content_info.type == cricket::MediaProtocolType::kRtp) {
      auto rtp_transport = transports_[bundled_mid()]->GetRtpTransport();
      SignalRtpTransportChanged(content_info.name, rtp_transport);
    } else {
      auto dtls_transport = transports_[bundled_mid()]->GetRtpDtlsTransport();
