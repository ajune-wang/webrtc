...
void SuppressionGain::GainToNoAudibleEcho(
  ...
  for (size_t k = 0; k < gain->size(); ++k) {
    ...
    float g = 1.0f;
    if (enr > params.enr_transparent_[k] && emr > params.emr_transparent_[k]) {
      g = (params.enr_suppress_[k] - enr) /
          (params.enr_suppress_[k] - params.enr_transparent_[k]);
      g = std::max(g, params.emr_transparent_[k] / emr);
    }
