...
// There are practically two tests here: when av1 is enabled via build flag
// and when it is not.
TEST(InternalDecoderFactory, Av1) {
  InternalDecoderFactory factory;
  if (kSupportsLibaomAv1Decoder) {
    ...
    EXPECT_FALSE(factory.CreateVideoDecoder(SdpVideoFormat("AV1X")));
  }
}
