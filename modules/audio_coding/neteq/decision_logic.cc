...
Operations DecisionLogic::GetDecision(
  ...
  } else if (!PacketBuffer::IsObsoleteTimestamp(next_packet->timestamp,
                                                target_timestamp,
                                                five_seconds_samples)) {
    return FuturePacketAvailable(
        decoder_frame_length, prev_mode, target_timestamp,
        next_packet->timestamp, play_dtmf, generated_noise_samples,
        packet_buffer_info.span_samples, packet_buffer_info.num_packets);
  } else {
