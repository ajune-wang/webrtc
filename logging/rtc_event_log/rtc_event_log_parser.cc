...
constexpr char Incoming::name[];
constexpr char Incoming::full_name[];
constexpr char Outgoing::name[];
constexpr char Outgoing::full_name[];
...
void ParsedRtcEventLog::StoreParsedEvent(const rtclog::Event& event) {
  ...
  switch (event.type()) {
    ...
    case rtclog::Event::RTP_EVENT: {
      ...
      if (direction == webrtc::kIncomingPacket) {
        incoming_rtp_packets_[parsed_header.ssrc].push_back(
            RtpPacketIncoming(timestamp, parsed_header, total_length));
      } else {
        outgoing_rtp_packets_[parsed_header.ssrc].push_back(
            RtpPacketOutgoing(timestamp, parsed_header, total_length));
      }
      break;
    }
    case rtclog::Event::RTCP_EVENT: {
      ...
      if (direction == webrtc::kIncomingPacket) {
        ...
        incoming_rtcp_packets_.push_back(
            RtcpPacketIncoming(timestamp, packet, total_length));
        ...
        outgoing_rtcp_packets_.push_back(
            RtcpPacketOutgoing(timestamp, packet, total_length));
      }
      ...
      for (const uint8_t* block = packet; block < packet_end;
        ...
        if (header.type() == rtcp::TransportFeedback::kPacketType &&
          ...
          if (direction == webrtc::kIncomingPacket) {
            ...
            RtcpPacketTransportFeedback& parsed_block =
          ...
          RtcpPacketSenderReport parsed_block;
          parsed_block.timestamp = GetTimestamp(event);
          ...
          RtcpPacketReceiverReport parsed_block;
          parsed_block.timestamp = GetTimestamp(event);
    ...
    case ParsedRtcEventLog::LOG_START: {
      start_log_events_.push_back(StartLogEvent(GetTimestamp(event)));
      break;
    }
    case ParsedRtcEventLog::LOG_END: {
      stop_log_events_.push_back(StopLogEvent(GetTimestamp(event)));
      break;
    }
    case ParsedRtcEventLog::AUDIO_PLAYOUT_EVENT: {
      AudioPlayoutEvent playout_event = GetAudioPlayout(event);
    ...
    case ParsedRtcEventLog::AUDIO_NETWORK_ADAPTATION_EVENT: {
      AudioNetworkAdaptationEvent ana_event = GetAudioNetworkAdaptation(event);
...
ParsedRtcEventLog::AudioPlayoutEvent ParsedRtcEventLog::GetAudioPlayout(
    size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
  const rtclog::Event& event = events_[index];
...
ParsedRtcEventLog::BweLossBasedUpdate ParsedRtcEventLog::GetLossBasedBweUpdate(
    size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
  const rtclog::Event& event = events_[index];
...
ParsedRtcEventLog::AudioNetworkAdaptationEvent
ParsedRtcEventLog::GetAudioNetworkAdaptation(size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
  const rtclog::Event& event = events_[index];
...
ParsedRtcEventLog::GetAudioNetworkAdaptation(const rtclog::Event& event) const {
  ...
  AudioNetworkAdaptationEvent res;
  res.timestamp = GetTimestamp(event);
