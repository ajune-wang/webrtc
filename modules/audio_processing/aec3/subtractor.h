 ...
 private:
  class FilterMisadjustmentEstimator {
   public:
    ...
    // microphone input.
    float GetMisadjustment() const { return adjustment_; }
    ...
    bool IsAdjustmentNeeded() {
      if (inv_misadjustment_ > 10.f) {
        // It is not aiming to adjust all the estimated mismatch. Instead,
        // it just aims to adjust half of that estimated mismatch.
        adjustment_ = 2.f / sqrtf(inv_misadjustment_);
        return true;
      } else {
        return false;
      }
