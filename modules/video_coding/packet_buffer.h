...
class PacketBuffer {
 ...
 private:
  ...
  struct StoredPacket {
    ...
    StoredPacket(StoredPacket&& other) { *this = std::move(other); }
    StoredPacket& operator=(const StoredPacket&) = delete;
    StoredPacket& operator=(StoredPacket&& other) {
      ...
      data = other.data;
      // Reset other memory to avoid double deleting it.
      other.data.dataPtr = nullptr;
      return *this;
    }

    ~StoredPacket() { delete[] data.dataPtr; }
  ...
  std::vector<absl::optional<StoredPacket>> buffer_ RTC_GUARDED_BY(crit_);
