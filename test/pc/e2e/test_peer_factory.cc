...
PeerConnectionFactoryDependencies CreatePCFDependencies(
  ...
  // There are two EnableMedia calls because of the extra functionality:
  // `WithDefaults` ensures all media related dependencies are set even when are
  // not provided by the `pcf_dependencies`.
  // `WithTimeController` inject clocks provided by the the `time_controller`.
  EnableMediaWithDefaults(pcf_deps);
  EnableMediaWithTimeController(time_controller, pcf_deps);
