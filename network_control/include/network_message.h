...
class Receiver {
 ...
 protected:
  ...
  virtual void Invoke(MSG_T msg) = 0;
...
class SameThreadReceiver : public HandlingReceiver<MSG_T> {
 ...
 protected:
  void Invoke(MSG_T msg) override { OnMessage(msg); }
...
template <typename MSG_T>
class CacheReceiver : public Receiver<MSG_T> {
 public:
  CacheReceiver() {}
...
class PendingTaskHandler {
 ...
 public:
  ...
  ~PendingTaskHandler();
  void InvokeTask(closure_t&& closure);
...
class TaskQueueReceiver : public HandlingReceiver<MSG_T> {
 ...
 protected:
  void Invoke(MSG_T msg) override {
    RTC_DCHECK(handler_);
    pending_tasks_.InvokeTask(std::bind(handler_, msg));
  }
  void OnMessage(MSG_T msg) override {
    RTC_DCHECK(handler_);
...
struct Message {
  ...
  using CacheReceiver = signal::CacheReceiver<MSG_T>;
