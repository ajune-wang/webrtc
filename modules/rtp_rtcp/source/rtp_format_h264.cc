...
RtpPacketizerH264::Fragment::Fragment(rtc::ArrayView<const uint8_t> payload) {
  ...
  switch (parse_result) {
    case SpsVuiRewriter::ParseResult::kVuiRewritten: {
      this->tmp_buffer_ = std::move(output_buffer);
      this->data_ = this->tmp_buffer_.data();
      this->size_ = this->tmp_buffer_.size();
...
RtpPacketizerH264::RtpPacketizerH264(
  ...
  for (int i = 0; i < fragmentation.fragmentationVectorSize; ++i) {
    RTC_DCHECK_LE(fragmentation.fragmentationOffset[i] +
                      fragmentation.fragmentationLength[i],
                  payload.size());
    input_fragments_.emplace_back(
...
bool RtpPacketizerH264::PacketizeSingleNalu(size_t fragment_index) {
  ...
  if (packet_capacity < fragment.size()) {
  ...
  packets_.emplace(fragment,
                   /*first_fragment=*/true, /*last_fragment=*/true,
                   /*aggregated=*/false, /*header=*/fragment.header());
  ++num_packets_left_;
  return true;
}
...
bool RtpPacketizerH264::PacketizeFuA(size_t fragment_index) {
  ...
  const Fragment& fragment = input_fragments_[fragment_index];
  rtc::ArrayView<const uint8_t> payload(fragment);
...
size_t RtpPacketizerH264::PacketizeStapA(size_t fragment_index) {
  ...
  for (size_t index = fragment_index; index < input_fragments_.size();
    ...
    RTC_CHECK_GT(fragment.size(), 0);
    int capacity_needed = fragment.size() + fragment_headers_length;
    ...
    packets_.emplace(fragment, aggregated_fragments == 0, false, true,
                     fragment.header());
    packet_capacity_left -= fragment.size();
