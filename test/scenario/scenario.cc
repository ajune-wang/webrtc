...
Scenario::Scenario(std::string log_path) : Scenario(log_path, true) {}

Scenario::Scenario(std::string log_path, bool real_time)
    : base_filename_(log_path),
      real_time_mode_(real_time),
      sim_clock_(1000 * kMicrosPerSec),
      clock_(real_time ? Clock::GetRealTimeClock() : &simulated_clock_),
...
CallClient* Scenario::CreateClient(std::string name, CallClientConfig config) {
  CallClient* client = new CallClient(clock_, name, config, base_filename_);
  if (!base_filename_.empty() && !name.empty() &&
      config.cc.log_interval.IsFinite()) {
    Every(config.cc.log_interval,
          [thia, client]() { client->LogCongestionControllerStats(Now()); });
  }
...
SimulationNode* Scenario::CreateNetworkNode(NetworkNodeConfig config) {
  ...
  Every(config.update_frequency,
        [this, network_node_ptr] { network_node_ptr->Process(Now()); });
...
void Scenario::RunUntil(TimeDelta max_duration,
  ...
  while (!exit_function() && Duration() < max_duration) {
    ...
    if (real_time_mode_) {
      ...
      sim_clock_.AdvanceTimeMilliseconds(wait_time.ms<int>());
    }
  }
  for (auto& stream_pair : video_streams_) {
