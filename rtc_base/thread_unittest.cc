...
TEST(ThreadTest, CountBlockingCalls) {
  ...
  //   (thread_unittest.cc:260): Blocking TestBody: total=2 (actual=1, would=1)
  RTC_LOG_THREAD_BLOCK_COUNT();
#if RTC_DCHECK_IS_ON
  rtc::Thread* current = rtc::Thread::Current();
  ASSERT_TRUE(current);
  rtc::Thread::ScopedCountBlockingCalls blocked_calls(
      [&](uint32_t actual_block, uint32_t would_block) {
        EXPECT_EQ(1u, actual_block);
        EXPECT_EQ(1u, would_block);
      });

  EXPECT_EQ(0u, blocked_calls.GetActualBlockedCallCount());
  EXPECT_EQ(0u, blocked_calls.GetWouldBlockCallCount());
  ...
  // invoke, but should still count as an invoke that would block since we
  ...
  EXPECT_EQ(0u, blocked_calls.GetActualBlockedCallCount());
  EXPECT_EQ(1u, blocked_calls.GetWouldBlockCallCount());
  ...
  EXPECT_EQ(1u, blocked_calls.GetActualBlockedCallCount());
  EXPECT_EQ(1u, blocked_calls.GetWouldBlockCallCount());
