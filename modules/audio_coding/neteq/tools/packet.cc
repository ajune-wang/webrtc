...
Packet::Packet(rtc::CopyOnWriteBuffer packet,
    ...
    : packet_(std::move(packet)),
      ...
      time_ms_(time_ms),
      valid_header_(ParseHeader(packet_, extension_map)) {}
...
bool Packet::ParseHeader(rtc::CopyOnWriteBuffer packet,
                         const RtpHeaderExtensionMap* extension_map) {
  // Special case for dummy packets that have padding marked in the RTP header.
  // This causes the RTP packet parser to report failure, but is fine in this
  // context.
  bool padding = (packet[0] & 0b0010'0000);
  if (padding) {
  ...
  rtp_payload_ = rtp_packet.payload();
  if (padding) {
    size_t padding_size = packet[packet.size() - 1];
    if (padding_size == 0 || padding_size > rtp_payload_.size()) {
      return false;
    }
    rtp_payload_ = rtc::MakeArrayView(rtp_payload_.data(),
                                      rtp_payload_.size() - padding_size);
  }
