...
// TODO(nisse): Either drop return value, or check and fail if payload
...
void RtpVideoStreamReceiver::OnRecoveredPacket(const uint8_t* rtp_packet,
  ...
  if (packet.PayloadType() == config_.rtp.red_payload_type) {
    RTC_LOG(LS_ERROR) << "Discarding recovered packet with RED encapsulation";
    return;
  }
...
void RtpVideoStreamReceiver::OnRtpPacket(const RtpPacketReceived& packet) {
  ...
  // TODO(nisse): We'll have a correct packet.payload_type_frequency() here only
  // if it was set at the time the packet was parsed, which currently isn't
  // done. Consider setting a 90 kHz frequancy as a construction time default
  // value in RtpPacketReceived; then we don't need to care about it for video
  // packets.
  ReceivePacket(packet);
  ...
  if (!packet.recovered()) {
    ...
    // IsPacketRetransmitted() relies on correct frequency value.
...
void RtpVideoStreamReceiver::ReceivePacket(const RtpPacketReceived& packet) {
  ...
  std::unique_ptr<RtpDepacketizer> depacketizer(
      RtpDepacketizer::Create(codec_type_it->second));
