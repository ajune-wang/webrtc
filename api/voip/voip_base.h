...
class VoipBase {
 public:
  ...
  // Note that transport must be set by application where as local_ssrc is
  // optional. When local_ssrc is not set, some random value will be used
  // by voip engine.
  struct Config {
  ...
  virtual absl::optional<ChannelId> CreateChannel(const Config& config) = 0;
  ...
  // start. Returns false if |channel_id| is invalid or initialization has
  // failed on selected microphone device. API is subject to expand to reflect
  // error condition to application later.
  ...
  // Returns false if |channel_id| is invalid or termination logic has failed
  // on selected microphone device.
  ...
  // Returns false if |channel_id| is invalid or initialization has failed
  // on selected speaker device.
  // API is subject to expand to reflect error condition to application later.
