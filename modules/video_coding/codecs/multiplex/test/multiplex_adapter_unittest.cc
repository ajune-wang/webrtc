...
class TestMultiplexAdapter
    : public VideoCodecUnitTest,
      public testing::WithParamInterface<bool /* supports_aug */> {
 public:
 ...
 protected:
  ...
  std::unique_ptr<VideoFrame> CreateInputFrame(bool contains_alpha,
    ...
    if (contains_data) {
      video_frame = CreateDataAugmentedInputFrame(video_frame.release());
    }

    return video_frame;
  }

  void CheckData(bool contains_data,
  ...
  std::unique_ptr<VideoFrame> ExtractAXXFrame(const VideoFrame& video_frame,
                                              bool is_augmented_buffer) {
    rtc::scoped_refptr<VideoFrameBuffer> video_frame_buffer =
        video_frame.video_frame_buffer();
    if (is_augmented_buffer) {
...
TEST_P(TestMultiplexAdapter, EncodeDecodeI420Frame) {
  std::unique_ptr<VideoFrame> input_frame = CreateInputFrame(false, GetParam());
  ...
  CheckData(GetParam(), decoded_frame->video_frame_buffer());
}

TEST_P(TestMultiplexAdapter, EncodeDecodeI420AFrame) {
  bool contains_data = GetParam();
  std::unique_ptr<VideoFrame> yuva_frame =
      CreateInputFrame(true, contains_data);
  EXPECT_EQ(WEBRTC_VIDEO_CODEC_OK,
  ...
  std::unique_ptr<VideoFrame> input_axx_frame =
      ExtractAXXFrame(*yuva_frame, contains_data);
  std::unique_ptr<VideoFrame> output_axx_frame =
      ExtractAXXFrame(*decoded_frame, contains_data);
...
TEST_P(TestMultiplexAdapter, CheckDoubleFramesEncodedBitstream) {
  std::unique_ptr<VideoFrame> yuva_frame = CreateInputFrame(true, GetParam());
  EXPECT_EQ(WEBRTC_VIDEO_CODEC_OK,
            encoder_->Encode(*yuva_frame, nullptr, nullptr));
  EncodedImage encoded_frame;
  CodecSpecificInfo codec_specific_info;
