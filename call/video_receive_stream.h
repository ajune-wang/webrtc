...
class VideoReceiveStream {
 public:
  using RecordableEncodedFrameCallback =
      std::function<void(const RecordableEncodedFrame&)>;

  // Class for handling moving in/out recording state.
  class RecordingState {
   public:
    RecordingState() : keyframe_generation_requested(false) {}
    explicit RecordingState(RecordableEncodedFrameCallback callback)
        : callback(std::move(callback)), keyframe_generation_requested(false) {}
    RecordableEncodedFrameCallback callback;
  ...
  virtual RecordingState SetRecordingState(RecordingState state,
