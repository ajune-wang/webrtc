...
void NullReceiver::DeliverPacket(std::unique_ptr<EmulatedIpPacket> packet) {}

ActionReceiver::ActionReceiver(std::function<void()> action)
    : action_(action) {}

void ActionReceiver::DeliverPacket(std::unique_ptr<EmulatedIpPacket> packet) {
  action_();
}
...
void NetworkNode::DeliverPacket(std::unique_ptr<EmulatedIpPacket> packet) {
  rtc::CritScope crit(&crit_sect_);
