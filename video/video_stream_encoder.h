...
class VideoStreamEncoder : public VideoStreamEncoderInterface,
 ...
 private:
  class CadenceCallbackAdapter;
  friend class CadenceCallbackAdapter;
  ...
  // interface.
  const std::unique_ptr<CadenceCallbackAdapter> cadence_callback_adapter_;
  // Frame cadence encoder adapter. Frames enter this adapter first, and it then
  // forwards them to our OnFrame method.
  const std::unique_ptr<FrameCadenceAdapterInterface> zero_hertz_adapter_;
