...
ReceiveStatisticsProxy::HistogramPercentileCounter::GetPercentile(
  ...
  if (elements_to_skip < 0)
    elements_to_skip = 0;
  if (elements_to_skip >= (signed)total_elements_)
    elements_to_skip = total_elements_ - 1;
  if (elements_to_skip < (signed)total_elements_low_) {
    for (uint32_t value = 0; value < total_elements_low_; ++value) {
