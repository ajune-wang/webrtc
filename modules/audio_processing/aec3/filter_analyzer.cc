...
void FilterAnalyzer::Update(
  ...
  DetectRefinedFilter(filter_time_domain, delay_blocks_, converged_filter);

  if (!ConsistentEstimate()) {
    delay_blocks_ = 1;
  }
}
...
void FilterAnalyzer::DetectRefinedFilter(
    rtc::ArrayView<const float> filter_time_domain,
    size_t peak_index,
    bool converged_filter) {
  ...
  refined_estimate_ = converged_filter_seen_ && h2_peak > 10 * std::max(n1, n2);
}
