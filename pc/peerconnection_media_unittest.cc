...
TEST_F(PeerConnectionMediaUnitTest, TestAVOfferWithAudioOnlyAnswer) {
  ...
  auto answer = callee->CreateAnswer(options_reject_video);
  callee->SetLocalDescription(std::move(answer));
  answer = callee->CreateAnswer(options_reject_video);
...
// TODO(steveanton): This test is hitting a DCHECK in RtpSender caused by the
// rejected voice channel being destroyed on the caller side before
// PeerConnection removes the tracks from the RtpSender. This results in the
// sink not being cleared from the RtpSender, and when it is reused in a later
// offer the sink is set again and fails because a sink already exists.
