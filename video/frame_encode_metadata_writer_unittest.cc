...
TEST(FrameEncodeMetadataWriterTest, DoesNotRewriteBitstreamWithoutCodecInfo) {
  ...
  EXPECT_EQ(encode_metadata_writer.GetUpdatedBitstream(image, nullptr,
                                                       &fragmentation),
            nullptr);
}
...
TEST(FrameEncodeMetadataWriterTest, DoesNotRewriteVp8Bitstream) {
  ...
  FakeEncodedImageCallback sink;
  FrameEncodeMetadataWriter encode_metadata_writer(&sink);
  EXPECT_EQ(encode_metadata_writer.GetUpdatedBitstream(
...
TEST(FrameEncodeMetadataWriterTest,
  ...
  FrameEncodeMetadataWriter encode_metadata_writer(&sink);
  EXPECT_EQ(encode_metadata_writer.GetUpdatedBitstream(
                image, &codec_specific_info, nullptr),
            nullptr);
}
...
TEST(FrameEncodeMetadataWriterTest, RewritesH264BitstreamWithNonOptimalSps) {
  ...
  FakeEncodedImageCallback sink;
  FrameEncodeMetadataWriter encode_metadata_writer(&sink);
  std::unique_ptr<FrameEncodeMetadataWriter::BitstreamData> buffer_data =
      encode_metadata_writer.GetUpdatedBitstream(image, &codec_specific_info,
                                                 &fragmentation);

  ASSERT_NE(buffer_data, nullptr);
  EXPECT_THAT(std::vector<uint8_t>(
                  buffer_data->buffer.data(),
                  buffer_data->buffer.data() + buffer_data->buffer.size()),
              testing::ElementsAreArray(kRewrittenSps));
  ASSERT_THAT(buffer_data->fragmentation.fragmentationVectorSize, 1U);
  EXPECT_EQ(buffer_data->fragmentation.fragmentationOffset[0], 4U);
  EXPECT_EQ(buffer_data->fragmentation.fragmentationLength[0],
