...
bool IsValid(const std::vector<BalancedDegradationSettings::Config>& configs) {
  ...
  for (size_t i = 1; i < configs.size(); ++i) {
    ...
    if (((configs[i].vp8_qp_high > 0) != (configs[i - 1].vp8_qp_high > 0)) ||
        ((configs[i].h264_qp_high > 0) != (configs[i - 1].h264_qp_high > 0)) ||
        ((configs[i].generic_qp_high > 0) !=
         (configs[i - 1].generic_qp_high > 0))) {
...
absl::optional<int> BalancedDegradationSettings::Vp8QpHigh(int pixels) const {
  ...
  int qp = configs_.back().vp8_qp_high;  // Use last above highest pixels.
  return (qp > 0) ? absl::optional<int>(qp) : absl::nullopt;
}
...
absl::optional<int> BalancedDegradationSettings::H264QpHigh(int pixels) const {
  ...
  int qp = configs_.back().h264_qp_high;  // Use last above highest pixels.
  return (qp > 0) ? absl::optional<int>(qp) : absl::nullopt;
}
...
absl::optional<int> BalancedDegradationSettings::GenericQpHigh(
  ...
  int qp = configs_.back().generic_qp_high;  // Use last above highest pixels.
  return (qp > 0) ? absl::optional<int>(qp) : absl::nullopt;
}
