      ...
      [this, adapted_source_width, adapted_source_height, incoming_frames,
       post_time_us, log_stats]() {
        ...
        if (posted_frames_waiting_for_encode == 1 && !cwnd_frame_drop) {
          MaybeEncodeVideoFrame(adapted_source_width, adapted_source_height,
                                incoming_frames, post_time_us);
        } else {
          if (cwnd_frame_drop) {
...
void VideoStreamEncoder::MaybeEncodeVideoFrame(
  ...
  RTC_DCHECK_RUN_ON(&encoder_queue_);
  input_state_provider_.OnFrameSizeObserved(adapted_source_width *
                                            adapted_source_height);

  if (!last_frame_info_ || adapted_source_width != last_frame_info_->width ||
      adapted_source_height != last_frame_info_->height ||
      video_frames[0].is_texture() != last_frame_info_->is_texture) {
    pending_encoder_reconfiguration_ = true;
    last_frame_info_ =
        VideoFrameInfo(adapted_source_width, adapted_source_height,
                       video_frames[0].is_texture());
    ...
    accumulated_update_rect_ = VideoFrame::UpdateRect{
        0, 0, video_frames[0].width(), video_frames[0].height()};
  }
  ...
  // TODO(https://crbug.com/webrtc/12469): Update this code and
  // EncodeVideoFrame() to look at all video frames.
  const VideoFrame& frame_to_encode = video_frames[0];

  if (DropDueToSize(last_frame_info_->pixel_count())) {
    RTC_LOG(LS_INFO) << "Dropping frame. Too large for target bitrate.";
    stream_resource_manager_.OnFrameDroppedDueToSize();
    // Storing references to a native buffer risks blocking frame capture.
    if (frame_to_encode.video_frame_buffer()->type() !=
        VideoFrameBuffer::Type::kNative) {
      pending_frame_ = frame_to_encode;
      pending_frame_post_time_us_ = time_when_posted_us;
    } else {
      // Ensure that any previously stored frame is dropped.
      pending_frame_.reset();
      accumulated_update_rect_.Union(frame_to_encode.update_rect());
      accumulated_update_rect_is_valid_ &= frame_to_encode.has_update_rect();
    }
    return;
  }
  ...
  EncodeVideoFrame(frame_to_encode, time_when_posted_us);
}
