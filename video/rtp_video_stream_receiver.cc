...
void RtpVideoStreamReceiver::OnReceivedPayloadData(
  ...
  VCMPacket packet(payload.data(), payload.size(), rtp_header, video_header,
                   ntp_estimator_.Estimate(rtp_header.timestamp),
                   clock_->TimeInMilliseconds());

  packet.video_header.is_last_packet_in_frame |= rtp_packet.Marker();
  if (const auto* vp9_header = absl::get_if<RTPVideoHeaderVP9>(
          &packet.video_header.video_type_header)) {
    packet.video_header.is_last_packet_in_frame |= vp9_header->end_of_frame;
    packet.video_header.is_first_packet_in_frame |=
        vp9_header->beginning_of_frame;
  }

  packet.video_header.rotation =
      rtp_packet.GetExtension<VideoOrientation>().value_or(kVideoRotation_0);
  packet.video_header.content_type =
      rtp_packet.GetExtension<VideoContentTypeExtension>().value_or(
          VideoContentType::UNSPECIFIED);
  packet.video_header.video_timing.flags = VideoSendTiming::kInvalid;
  rtp_packet.GetExtension<VideoTimingExtension>(
      &packet.video_header.video_timing);
  rtp_packet.GetExtension<PlayoutDelayLimits>(
      &packet.video_header.playout_delay);
  packet.video_header.frame_marking.temporal_id = kNoTemporalIdx;
  rtp_packet.GetExtension<FrameMarkingExtension>(
      &packet.video_header.frame_marking);

  // Color space should only be transmitted in the last packet of a frame,
  // therefore, neglect it otherwise so that last_color_space_ is not reset by
  // mistake.
  if (packet.video_header.is_last_packet_in_frame) {
    packet.video_header.color_space =
        rtp_packet.GetExtension<ColorSpaceExtension>();
    if (packet.video_header.color_space ||
      ...
      last_color_space_ = packet.video_header.color_space;
    } else if (last_color_space_) {
      packet.video_header.color_space = last_color_space_;
    }
  }
  ...
  if ((packet.generic_descriptor =
    ...
    packet.generic_descriptor->SetByteRepresentation(
  ...
  if (packet.generic_descriptor != absl::nullopt) {
    ...
    if (packet.generic_descriptor->FirstPacketInSubFrame()) {
      packet.video_header.frame_type =
          packet.generic_descriptor->FrameDependenciesDiffs().empty()
              ? VideoFrameType::kVideoFrameKey
              : VideoFrameType::kVideoFrameDelta;
    }

    packet.video_header.width = packet.generic_descriptor->Width();
    packet.video_header.height = packet.generic_descriptor->Height();
  }
