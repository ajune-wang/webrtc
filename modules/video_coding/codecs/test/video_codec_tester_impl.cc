...
class LockedRawVideoSource : public TestRawVideoSource,
 ...
 public:
  ...
  absl::optional<VideoFrame> Pull() override {
    MutexLock lock(&mutex_);
    return video_source_->Pull();
  }

  absl::optional<VideoFrame> Get(size_t frame_num) override {
    MutexLock lock(&mutex_);
    return video_source_->Get(frame_num);
  }

  absl::optional<VideoFrame> ReadFrame(size_t frame_num) override {
    return Get(frame_num);
  }

  void Close() override { RTC_UNREACHABLE_CODE_HIT; }
...
// Pacer guarantees that walltime between two consequetive events (Pace() calls)
// is at least equal to the pacing cycle.
class Pacer {
 public:
  void Pace(Timestamp event) {
    if (prev_event_.has_value()) {
      // Testing-firendly implimentation.
      int64_t left_ms;
...
std::unique_ptr<VideoCodecTestStats> VideoCodecTesterImpl::RunDecodeTest(
  ...
  while (auto frame = video_source->Pull()) {
...
std::unique_ptr<VideoCodecTestStats> VideoCodecTesterImpl::RunEncodeTest(
  ...
  while (auto frame = locked_source.Pull()) {
