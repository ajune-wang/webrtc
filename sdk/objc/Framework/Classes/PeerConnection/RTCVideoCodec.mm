...
#if defined(WEBRTC_IOS)
  profileLevelId = FindMaxSupportedProfileForDevice([UIDevice deviceType]);
#endif
  if (profileLevelId && profileLevelId->profile >= webrtc::H264::kProfileConstrainedBaseline) {
    auto profileString = webrtc::H264::ProfileLevelIdToString(webrtc::H264::ProfileLevelId(
        webrtc::H264::kProfileConstrainedBaseline, profileLevelId->level));
    if (profileString) {
...
NSString *RTCMaxSupportedProfileLevelConstrainedHigh() {
  ...
  profileLevelId = FindMaxSupportedProfileForDevice([UIDevice deviceType]);
#endif
  if (profileLevelId && profileLevelId->profile >= webrtc::H264::kProfileConstrainedHigh) {
    auto profileString = webrtc::H264::ProfileLevelIdToString(
        webrtc::H264::ProfileLevelId(webrtc::H264::kProfileConstrainedHigh, profileLevelId->level));
    if (profileString) {
      return [NSString stringForStdString:*profileString];
    }
  }
