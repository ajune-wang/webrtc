...
TEST(AudioNetworkAdaptorImplTest,
  ...
  fake_clock.AdvanceTime(TimeDelta::Milliseconds(kClockInitialTimeMs));
  auto states = CreateAudioNetworkAdaptor();
...
TEST(AudioNetworkAdaptorImplTest,
  ...
  fake_clock.AdvanceTime(TimeDelta::Milliseconds(100));
  timestamp_check += 100;
  check.uplink_packet_loss_fraction = kPacketLoss;
  EXPECT_CALL(*states.mock_debug_dump_writer,
              DumpNetworkMetrics(NetworkMetricsIs(check), timestamp_check));
  ...
  fake_clock.AdvanceTime(TimeDelta::Milliseconds(50));
  timestamp_check += 50;

  fake_clock.AdvanceTime(TimeDelta::Milliseconds(200));
  timestamp_check += 200;
  check.rtt_ms = kRtt;
  EXPECT_CALL(*states.mock_debug_dump_writer,
              DumpNetworkMetrics(NetworkMetricsIs(check), timestamp_check));
  ...
  fake_clock.AdvanceTime(TimeDelta::Milliseconds(150));
  timestamp_check += 150;
  check.target_audio_bitrate_bps = kTargetAudioBitrate;
  EXPECT_CALL(*states.mock_debug_dump_writer,
              DumpNetworkMetrics(NetworkMetricsIs(check), timestamp_check));
