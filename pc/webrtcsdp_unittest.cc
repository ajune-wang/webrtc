...
class WebRtcSdpTest : public testing::Test {
 public:
  ...
  void CompareRidDescriptions(const std::vector<RidDescription>& rids,
...
TEST_F(WebRtcSdpTest, TestDeserializeSimulcastAttribute) {
  ...
  CompareRidDescriptions(rids, {"1", "2", "3"});
  ASSERT_TRUE(media->has_receive_stream());
  CompareRidDescriptions(media->receive_stream().rids(), {"4", "5", "6"});
}
...
TEST_F(WebRtcSdpTest, TestDeserializeSimulcastAttributeRemovesUnknownRids) {
  ...
  EXPECT_FALSE(media->streams().empty());
  const std::vector<RidDescription>& rids = media->streams()[0].rids();
  CompareRidDescriptions(rids, {"1", "3"});
  ASSERT_TRUE(media->has_receive_stream());
  CompareRidDescriptions(media->receive_stream().rids(), {"4"});
}
...
TEST_F(WebRtcSdpTest, TestDeserializeIgnoresEmptyRidLines) {
  ...
  EXPECT_FALSE(media->streams().empty());
  const std::vector<RidDescription>& rids = media->streams()[0].rids();
  CompareRidDescriptions(rids, {"1", "2"});
...
TEST_F(WebRtcSdpTest, TestDeserializeRemovesRidsWithInvalidCodec) {
  ...
  EXPECT_EQ(1ul, rids[0].formats.size());
  EXPECT_EQ(120, rids[0].formats[0]);
...
TEST_F(WebRtcSdpTest, SerializeSimulcast_ComplexSerialization) {
  ...
  std::vector<RidDescription>& send_rids = send_stream.rids();
  send_rids.push_back(RidDescription("1", cricket::kRidDirectionSend));
  send_rids.push_back(RidDescription("2", cricket::kRidDirectionSend));
  send_rids.push_back(RidDescription("3", cricket::kRidDirectionSend));
  send_rids.push_back(RidDescription("4", cricket::kRidDirectionSend));

  StreamParams recv_stream;
  std::vector<RidDescription>& recv_rids = recv_stream.rids();
  recv_rids.push_back(RidDescription("6", cricket::kRidDirectionReceive));
  recv_rids.push_back(RidDescription("7", cricket::kRidDirectionReceive));
  recv_rids.push_back(RidDescription("8", cricket::kRidDirectionReceive));
  recv_rids.push_back(RidDescription("9", cricket::kRidDirectionReceive));
  recv_rids.push_back(RidDescription("10", cricket::kRidDirectionReceive));
  recv_rids.push_back(RidDescription("11", cricket::kRidDirectionReceive));
