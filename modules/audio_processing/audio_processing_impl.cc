...
int AudioProcessingImpl::InitializeLocked() {
  ...
  if (public_submodules_->echo_cancellation->is_enabled()) {
    ...
    config_.echo_canceller.enabled = true;
    config_.echo_canceller.mobile_mode = true;
  }
...
void AudioProcessingImpl::ApplyConfig(const AudioProcessing::Config& config) {
  ...
  capture_nonlocked_.echo_controller_enabled =
      config_.echo_canceller.enabled && private_submodules_->echo_controller;
  echo_cancellation()->Enable(config_.echo_canceller.enabled &&
                              !config_.echo_canceller.mobile_mode);
  echo_control_mobile()->Enable(config_.echo_canceller.enabled &&
                                config_.echo_canceller.mobile_mode);
