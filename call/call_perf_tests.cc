...
// Discovers the minimal supported audio+video bitrate. The allocator will not
// allocate less than minumum configured by media streams.
// Expected result is min audio + min video + overhead.
// The test bitrate is considered supported if RTT does not go above 400ms with
// the network constrained to the test bitrate.
//
...
void CallPerfTest::TestMinAudioVideoBitrate(
  ...
  static constexpr int kOpusBitrateBps = 32000;
   ...
   protected:
    ...
    void ModifyAudioConfigs(
      ...
      if (use_bitrate_allocation_strategy_) {
        ...
        send_config->max_bitrate_bps = kOpusBitrateBps;
      } else {
        send_config->send_codec_spec->target_bitrate_bps =
            rtc::Optional<int>(kOpusBitrateBps);
      }
    }
