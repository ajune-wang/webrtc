...
bool StationarityEstimator::Update(rtc::ArrayView<const float> spectrum) {
  ...
  data_dumper_->DumpRaw("aec3_stationarity_thr", thr_);
  data_dumper_->DumpRaw("aec3_stationarity_hangover", hangover_);
...
void StationarityEstimator::UpdateStationaryStatus() {
  for (size_t k = 0; k < smooth_spectrum_.size(); ++k) {
    ...
    float r = frame_power / noise;
    stationary_[k] = r < thr_[k];
    if (stationary_[k]) {
      if (hangover_[k]) {
        hangover_[k] = std::max(hangover_[k] - 1, 0);
      } else {
        thr_[k] = std::min(thr_[k] * 1.1f, 100.f);
      }
    } else {
      thr_[k] = thr_[k] / 1.1f;
      if (thr_[k] <= 10.f) {
        thr_[k] = 10.f;
        hangover_[k] = kNumBlocksPerSecond * 0.1;
      }
    }
  }
}
  ...
  for (size_t k = 0; k < spectrum.size(); ++k) {
    if (noise_spectrum_[k] < spectrum[k]) {
      ...
      if (block_counter_ > nBlocksInitialPhase) {
        ...
        if (100.f * noise_spectrum_[k] < spectrum[k]) {
