...
DEFINE_bool(postpone_decoding_bugfix,
            false,
            "Bugfix to postpone decoding after expansion");
...
int RunTest(int argc, char* argv[]) {
  ...
  if (FLAG_postpone_decoding_bugfix) {
    // Set field trial to enable the bugfix.
    field_trial_string =
        "WebRTC-Audio-NetEqPostponeDecodingAfterExpand/Enabled/";
  }
  ValidateFieldTrialsStringOrDie(field_trial_string);
  ScopedFieldTrials field_trials(field_trial_string);
