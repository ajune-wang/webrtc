...
// using the internal builder is deactivated, since the AudioProcessing object
// is already created and the builder is not used in the simulation. It is
// needed to pass the command line flags as |argc| and |argv|, so these can be
// interpreted properly by the utility. To get a fully-working audioproc_f
// utility, all that is needed is to write a main function, create an
// AudioProcessingBuilder, optionally set custom processing components on it,
// and pass the builder together with the command line arguments into this
// function. To see a list of all supported command line flags, run the
// executable with the '--help' flag.
