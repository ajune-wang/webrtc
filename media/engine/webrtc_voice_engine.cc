...
RtpCapabilities WebRtcVoiceEngine::GetCapabilities() const {
  ...
  if (allocation_settings_.enable_transport_sequence_number_extension()) {
...
class WebRtcVoiceMediaChannel::WebRtcAudioSendStream
 ...
 private:
  ...
  void UpdateAllowedBitrateRange() {
    ...
    const bool is_opus =
        ...
        absl::EqualsIgnoreCase(config_.send_codec_spec->format.name,
                               kOpusCodecName);
    if (is_opus && allocation_settings_.configure_audio_bitrate_allocation()) {
      config_.min_bitrate_bps = allocation_settings_.min_bitrate_bps();
      config_.max_bitrate_bps = allocation_settings_.max_bitrate_bps(
          rtp_parameters_.encodings[0].max_bitrate_bps);
    }
  }
  ...
  webrtc::AudioAllocationSettings allocation_settings_;
