...
constexpr char kNeverNackMultipleTimesFieldTrial[] =
    "WebRTC-Audio-NetEqNeverNackMultipleTimes";
...
NackTracker::NackTracker(int nack_threshold_packets)
    : nack_threshold_packets_(nack_threshold_packets),
      never_nack_multiple_times_(
          webrtc::field_trial::IsEnabled(kNeverNackMultipleTimesFieldTrial)),
...
std::vector<uint16_t> NackTracker::GetNackList(int64_t round_trip_time_ms) {
  ...
  constexpr int kLossCost = 2000;
  int max_wait_ms = kLossCost * packet_loss_rate_;
  for (NackList::const_iterator it = nack_list_.begin(); it != nack_list_.end();
       ++it) {
    int64_t time_since_packet =
    ...
    if (it->second.is_missing &&
        (it->second.time_to_play_ms > round_trip_time_ms ||
         time_since_packet + round_trip_time_ms < max_wait_ms))
      sequence_numbers.push_back(it->first);
  }
  if (never_nack_multiple_times_) {
    nack_list_.clear();
  }
...
void NackTracker::UpdatePacketLossRate(int packets_lost) {
  constexpr double alpha = 0.004;
  // Exponential filter.
  packet_loss_rate_ = (1.0 - alpha) * packet_loss_rate_;
  for (int i = 0; i < packets_lost; ++i) {
    packet_loss_rate_ = (1.0 - alpha) * packet_loss_rate_ + alpha;
  }
}
}  // namespace webrtc
