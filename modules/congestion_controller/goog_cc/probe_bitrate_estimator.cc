...
int ProbeBitrateEstimator::HandleProbeAndEstimateBitrate(
  ...
  if (send_interval_ms <= 0 || send_interval_ms > kMaxProbeIntervalMs ||
    ...
    NLOG(LS_INFO, "Probing unsuccessful, invalid send/receive interval",
         " [cluster id: ", cluster_id, "] [send interval: ", send_interval_ms,
         " ms]", " [receive interval: ", receive_interval_ms, " ms]");
    if (event_log_) {
  ...
  if (ratio > kMaxValidRatio) {
    NLOG(LS_INFO, "Probing unsuccessful, receive/send ratio too high",
         " [cluster id: ", cluster_id, "] [send: ", send_size, " bytes / ",
         send_interval_ms, " ms = ", send_bps / 1000, " kb/s]",
         " [receive: ", receive_size, " bytes / ", receive_interval_ms,
         " ms = ", receive_bps / 1000, " kb/s]",
         " [ratio: ", receive_bps / 1000, " / ", send_bps / 1000, " = ", ratio,
  ...
  NLOG(LS_INFO, "Probing successful", " [cluster id: ", cluster_id,
       "] [send: ", send_size, " bytes / ", send_interval_ms,
       " ms = ", send_bps / 1000, " kb/s]", " [receive: ", receive_size,
       " bytes / ", receive_interval_ms, " ms = ", receive_bps / 1000,
       " kb/s]");
