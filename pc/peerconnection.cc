...
RTCError PeerConnection::ApplyLocalDescription(
  ...
  error = SetCurrentOrPendingLocalDescription(std::move(desc));
  if (!error.ok()) {
    return error;
  }
  RTC_DCHECK(local_description());
...
RTCError PeerConnection::ApplyRemoteDescription(
  ...
  error = SetCurrentOrPendingRemoteDescription(std::move(desc));
  if (!error.ok()) {
    return error;
  }
...
RTCError PeerConnection::SetCurrentOrPendingLocalDescription(
    std::unique_ptr<SessionDescriptionInterface> desc) {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
RTCError PeerConnection::SetCurrentOrPendingRemoteDescription(
    std::unique_ptr<SessionDescriptionInterface> desc) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  ...
  // is called.
  RTCError error = UpdateSessionState(action, cricket::CS_REMOTE);
  if (!error.ok()) {
    return error;
  }
...
void PeerConnection::EnableMedia() {
  for (auto transceiver : transceivers_) {
    cricket::BaseChannel* channel = transceiver->internal()->channel();
