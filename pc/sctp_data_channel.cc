...
void SctpSid::set_value(int id) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  RTC_DCHECK(!IsValid()) << "leaking an id?";
  if (id >= cricket::kMinSctpSid && id <= cricket::kSpecMaxSctpSid) {
...
bool SctpSidAllocator::AllocateSid(rtc::SSLRole role, int* sid) {
  ...
  *sid = potential_sid;
  used_sids_.insert(SctpSid(potential_sid));
  return true;
}
...
bool SctpSidAllocator::ReserveSid(int sid) {
  ...
  used_sids_.insert(SctpSid(sid));
  return true;
}

void SctpSidAllocator::ReleaseSid(int sid) {
  auto it = used_sids_.find(SctpSid(sid));
