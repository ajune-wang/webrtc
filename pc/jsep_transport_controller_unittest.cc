...
TEST_F(JsepTransportControllerTest, SignalIceWritableStatus) {
  ...
  // are ICE writable only when all ICE transports are writable.
  EXPECT_FALSE(ice_writable_);
  EXPECT_EQ(0, ice_writable_status_signal_count_);

  fake_video_ice->SetWritable(true);
  EXPECT_TRUE(ice_writable_);
