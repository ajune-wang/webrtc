...
static std::string GetMediaSectionMsidLine() {
  std::stringstream ss;
  ss << "a=msid:" << kStreamLabel1 << " " << kAudioTrackId1;
  return ss.str();
}

static std::string GetSsrcAttributeMsidLine() {
  std::stringstream ss;
  ss << "a=ssrc:" << kAudioTrack1Ssrc << " msid:" << kStreamLabel1 << " "
     << kAudioTrackId1;
  return ss.str();
}
...
TEST_F(WebRtcSdpTest, SerializeOnlyMediaSectionMsid) {
  ...
  std::string a_msid_line = GetMediaSectionMsidLine();
  EXPECT_NE(std::string::npos, sdp.find(a_msid_line.c_str()));
  std::string a_ssrc_line = GetSsrcAttributeMsidLine();
  EXPECT_EQ(std::string::npos, sdp.find(a_ssrc_line.c_str()));
}
...
TEST_F(WebRtcSdpTest, SerializeOnlySsrcAttributeMsid) {
  ...
  std::string a_msid_line = GetMediaSectionMsidLine();
  EXPECT_EQ(std::string::npos, sdp.find(a_msid_line.c_str()));
  std::string a_ssrc_line = GetSsrcAttributeMsidLine();
  RTC_LOG(LS_INFO) << a_ssrc_line;
  EXPECT_NE(std::string::npos, sdp.find(a_ssrc_line.c_str()));
}
