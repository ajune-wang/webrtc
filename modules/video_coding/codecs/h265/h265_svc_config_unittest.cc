...
TEST(H265SvcConfigTest, TreatsEmptyAsL1T1) {
  ...
  EXPECT_TRUE(SetH265SvcConfig(video_codec, /*num_temporal_layers=*/kDontCare,
...
TEST(H265SvcConfigTest, ScalabilityModeFromNumberOfTemporalLayers) {
  ...
  EXPECT_TRUE(SetH265SvcConfig(video_codec, /*num_temporal_layers=*/3,
                               /*num_spatial_layers=*/1));
  EXPECT_EQ(video_codec.spatialLayers[0].numberOfTemporalLayers, 3);
}

TEST(H265SvcConfigTest, ScalabilityModeFromNumberOfSpatialLayers) {
  VideoCodec video_codec = GetDefaultVideoCodec();
...
TEST(H265SvcConfigTest, SetsActiveSpatialLayersFromScalabilityMode) {
  VideoCodec video_codec = GetDefaultVideoCodec();
...
TEST(H265SvcConfigTest, ConfiguresDobuleResolutionRatioFromScalabilityMode) {
  VideoCodec video_codec;
  video_codec.codecType = kVideoCodecH265;
  video_codec.SetScalabilityMode(ScalabilityMode::kL2T1);
...
TEST(H265SvcConfigTest, ConfiguresSmallResolutionRatioFromScalabilityMode) {
  VideoCodec video_codec;
  video_codec.codecType = kVideoCodecH265;
...
TEST(H265SvcConfigTest, SetsBitratesForMultipleSpatialLayers) {
  VideoCodec video_codec;
  video_codec.codecType = kVideoCodecH265;
