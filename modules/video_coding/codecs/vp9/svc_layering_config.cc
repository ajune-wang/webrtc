...
namespace {
const size_t kMinSpatialLayerWidth = 320;
const size_t kMinSpatialLayerHeight = 180;
}  // namespace
...
std::vector<SpatialLayer> ConfigureSvcLayering(size_t input_width,
  ...
  const float num_layers_fit_horz =
      std::floor(1 + std::log2(1.0f * input_width / kMinSpatialLayerWidth));
  const float num_layers_fit_vert =
      std::floor(1 + std::log2(1.0f * input_height / kMinSpatialLayerHeight));
