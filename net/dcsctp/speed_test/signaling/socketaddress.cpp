  ...
  switch (family) {
    case AF_INET: {
      ...
      if (isBinaryIp) {
        ...
        int err =
            inet_pton(address.sin_family, (const char*)pIp, &address.sin_addr);
        if (err != 1) {
          Reset();
          return;
        }
    ...
    case AF_INET6: {
      ...
      if (isBinaryIp) {
        ...
        int err = inet_pton(address.sin6_family, (const char*)pIp,
                            &address.sin6_addr);
        if (err != 1) {
          Reset();
          return;
...
bool SocketAddress::IsIPv6Loopback() const {
  return IsIPv6() &&
         IN6_IS_ADDR_LOOPBACK(&((sockaddr_in6*)&_address)->sin6_addr);
}

bool SocketAddress::IsIPv6IPv4Mapped() const {
  return IsIPv6() &&
         IN6_IS_ADDR_V4MAPPED(&((sockaddr_in6*)&_address)->sin6_addr);
}

bool SocketAddress::IsIPv6UniqueLocal() const {
  return IsIPv6() && ((IPV6_RAW_8(_address)[0] == 0xfc) ||
                      (IPV6_RAW_8(_address)[0] == 0xfd));
}

bool SocketAddress::IsIPv6LinkLocal() const {
  return IsIPv6() &&
         IN6_IS_ADDR_LINKLOCAL(&((sockaddr_in6*)&_address)->sin6_addr);
}
...
bool SocketAddress::IsIPv6GlobalUnicast() const {
  return IsIPv6() && ((IPV6_RAW_8(_address)[0] >> 5) == 1) &&
         (!IsIPv6Loopback()) && (!IsIPv6Loopback()) && (!IsIPv6IPv4Mapped()) &&
         (!IsIPv6UniqueLocal()) && (!IsIPv6LinkLocal()) && (!IsIPv6Teredo()) &&
         (!IsIPv6Benchmarking()) && (!IsIPv6Orchid()) && (!IsIPv6To4()) &&
         (!IsIPv6Documentation()) && (!IsIPv6Multicast());
}

bool SocketAddress::IsIPv6Multicast() const {
  return IsIPv6() &&
         IN6_IS_ADDR_MULTICAST(&((sockaddr_in6*)&_address)->sin6_addr);
}
...
std::string SocketAddress::SockaddrToString(const struct sockaddr* pAddress,
  ...
  bool addPort = ((pAddress->sa_family == AF_INET) &&
                  (((struct sockaddr_in*)pAddress)->sin_port != 0)) ||
                 ((pAddress->sa_family == AF_INET6) &&
                  (((struct sockaddr_in6*)pAddress)->sin6_port != 0));
  ...
  socklen_t addrLen = (pAddress->sa_family == AF_INET6)
                          ? sizeof(struct sockaddr_in6)
                          : sizeof(struct sockaddr_in);
#endif /* SOCKADDR_HAS_LENGTH_FIELD */
  int err = getnameinfo(pAddress, addrLen, ip, sizeof(ip), port, sizeof(port),
                        NI_NUMERICHOST | NI_NUMERICSERV);
  if (err != 0) {
    cout << "Error encountered while translating address into string: "
         << "err: " << err << "; "
  ...
  if (pIp != NULL)
    *pIp = ip;
  if (pPort != NULL)
    *pPort = ENTOHS(pAddress->sa_family == AF_INET
                        ? (((struct sockaddr_in*)pAddress)->sin_port)
                        : (((struct sockaddr_in6*)pAddress)->sin6_port));
...
#else  /* SOCKADDR_HAS_LENGTH_FIELD */
  _addressLength =
      (pAddress->sa_family == AF_INET ? sizeof(struct sockaddr_in)
                                      : sizeof(struct sockaddr_in6));
...
bool SocketAddress::Init(int af, const std::string& address) {
  ...
  _stringRepresentation =
      SockaddrToString((struct sockaddr*)&_address, NULL, NULL);
#ifdef SOCKADDR_HAS_LENGTH_FIELD
  //_crc32 = DigestCRC32Update(0, (uint8_t*) & _address, _address.ss_len);
#else  /* SOCKADDR_HAS_LENGTH_FIELD */
  //_crc32 = DigestCRC32Update(0, (uint8_t*) & _address, _addressLength);
...
bool SocketAddress::ParseAndResolve(int af, const std::string& addrStr) {
  ...
  if (((err = getaddrinfo(_host.c_str(), portString.c_str(), &hints,
                          &pTempResult)) != 0) ||
      (pTempResult == NULL)) {
    cout << "Error encountered while translating string into address: "
  ...
  socklen_t addrLen = (_address.ss_family == AF_INET6)
                          ? sizeof(struct sockaddr_in6)
                          : sizeof(struct sockaddr_in);
  char ipStr[NI_MAXHOST];
  err = getnameinfo((struct sockaddr*)&_address, addrLen, ipStr, sizeof(ipStr),
...
#ifndef SOCKADDR_HAS_LENGTH_FIELD
  _addressLength =
      (_address.ss_family == AF_INET ? sizeof(struct sockaddr_in)
                                     : sizeof(struct sockaddr_in6));
#endif /* SOCKADDR_HAS_LENGTH_FIELD */
