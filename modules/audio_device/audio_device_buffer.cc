...
int32_t AudioDeviceBuffer::SetRecordedBuffer(const void* audio_buffer,
  ...
  // If the timestamp is <= 0, it's not valid and are ignored. If we do an
  // timestamp alignment on them they might accidentally become >0, and will be
  // handled as if they were a correct timestamp.
