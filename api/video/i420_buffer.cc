...
I420Buffer::I420Buffer(int width, int height)
    ...
    : I420ABuffer(width, height, width, (width + 1) / 2, (width + 1) / 2, width) {
    }
  ...
  return buffer;
    ...
    rtc::scoped_refptr<I420ABuffer> I420ABuffer::Rotate(
        ...
        RTC_CHECK_EQ(0, libyuv::I420Rotate(
                                           ...
                                           src.width(), src.height(),
                                           static_cast<libyuv::RotationMode>(rotation)));
        libyuv::RotatePlane(src.DataA(), src.StrideA(), buffer->MutableDataA(), buffer->StrideA(),
                            src.width(), src.height(), static_cast<libyuv::RotationMode>(rotation));
...
int I420Buffer::StrideV() const {
  return stride_v_;
    ...
    const uint8_t* I420ABuffer::DataA() const {
        return data_.get() + stride_y_ * height_ + (stride_u_ + stride_v_) * ((height_ + 1) / 2);
    }
...
void I420Buffer::CropAndScaleFrom(const I420BufferInterface& src,
  ...
  RTC_DCHECK_EQ(res, 0);
    ...
    void I420ABuffer::CropAndScaleFrom(const I420ABufferInterface& src,
        ...
        src.DataA() + src.StrideA() * offset_y + offset_x;
        CropAndScaleFrom((const I420BufferInterface&)src, offset_x, offset_y, crop_width, crop_height);
        libyuv::ScalePlane(a_plane, src.StrideA(),
                           crop_width, crop_height,
                           MutableDataA(), StrideA(),
                           width(), height(), libyuv::kFilterBox);
