...
void WebRtcOpus_PacketDecodeFlags(const uint8_t* payload,
  ...
  for (int n = 0; n < channels; n++) {
    // The VAD bits for channel 1 are on the first up to three bits, followed by
    // the LBRR bit for channel 1, followed by the VAD bits for channel 2 and
    // the LBRR bit for channel 2.
    has_vad |= frame_data[0][0] & ((silk_frames) << (4 - 4 * n));
    // The LBRR bit for channel 1 is on the (|silk_frames| + 1)-th bit, and
    // that of channel 2 is on the |(|silk_frames| + 1) * 2 + 1|-th bit.
    has_fec |= frame_data[0][0] & (0x80 >> ((n + 1) * (silk_frames + 1) - 1));
  }
}
