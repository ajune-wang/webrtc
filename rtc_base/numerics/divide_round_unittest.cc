...
TYPED_TEST(DivideRoundTypedTest, RoundToNearestPreservesType) {
  TypeParam dividend = 100;
  int8_t divider_s8 = 3;
  int16_t divider_s16 = 3;
  int32_t divider_s32 = 3;
  int64_t divider_s64 = 3;
  uint8_t divider_u8 = 3;
  uint16_t divider_u16 = 3;
  uint32_t divider_u32 = 3;
  uint64_t divider_u64 = 3;
  static_assert(
      std::is_same<decltype(DivideRoundToNearest(dividend, divider_s8)),
                   decltype(dividend / divider_s8)>::value,
      "");
  static_assert(
      std::is_same<decltype(DivideRoundToNearest(dividend, divider_s16)),
                   decltype(dividend / divider_s16)>::value,
      "");
  static_assert(
      std::is_same<decltype(DivideRoundToNearest(dividend, divider_s32)),
                   decltype(dividend / divider_s32)>::value,
      "");
  static_assert(
      std::is_same<decltype(DivideRoundToNearest(dividend, divider_s64)),
                   decltype(dividend / divider_s64)>::value,
      "");
  static_assert(
      std::is_same<decltype(DivideRoundToNearest(dividend, divider_u8)),
                   decltype(dividend / divider_u8)>::value,
      "");
  static_assert(
      std::is_same<decltype(DivideRoundToNearest(dividend, divider_u16)),
                   decltype(dividend / divider_u16)>::value,
      "");
  static_assert(
      std::is_same<decltype(DivideRoundToNearest(dividend, divider_u32)),
                   decltype(dividend / divider_u32)>::value,
      "");
  static_assert(
      std::is_same<decltype(DivideRoundToNearest(dividend, divider_u64)),
                   decltype(dividend / divider_u64)>::value,
      "");
}
...
TYPED_TEST(DivideRoundTypedTest, RoundUpPreservesType) {
  ...
  uint32_t divider_u32 = 3;
  uint64_t divider_u64 = 3;
  static_assert(std::is_same<decltype(DivideRoundUp(dividend, divider_s8)),
                             decltype(dividend / divider_s8)>::value,
                "");
  static_assert(std::is_same<decltype(DivideRoundUp(dividend, divider_s16)),
                             decltype(dividend / divider_s16)>::value,
                "");
  static_assert(std::is_same<decltype(DivideRoundUp(dividend, divider_s32)),
                             decltype(dividend / divider_s32)>::value,
                "");
  static_assert(std::is_same<decltype(DivideRoundUp(dividend, divider_s64)),
                             decltype(dividend / divider_s64)>::value,
                "");
  static_assert(std::is_same<decltype(DivideRoundUp(dividend, divider_u8)),
                             decltype(dividend / divider_u8)>::value,
                "");
  static_assert(std::is_same<decltype(DivideRoundUp(dividend, divider_u16)),
                             decltype(dividend / divider_u16)>::value,
                "");
  static_assert(std::is_same<decltype(DivideRoundUp(dividend, divider_u32)),
                             decltype(dividend / divider_u32)>::value,
                "");
  static_assert(std::is_same<decltype(DivideRoundUp(dividend, divider_u64)),
                             decltype(dividend / divider_u64)>::value,
                "");
}
