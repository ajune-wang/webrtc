...
TEST(NetEq, CreateAndDestroy) {
  ...
  NetEqFactoryWithCodecs neteq_factory;
...
TEST_F(NetEqImplTest, EnableRtxHandling) {
  ...
  CreateInstance();
  EXPECT_CALL(*mock_neteq_controller_, GetDecision(_, _, _, _, _, _, _, _, _))
      .Times(1)
      .WillOnce(Return(Operations::kNormal));
...
TEST_F(NetEqImplTest120ms, CodecInternalCng) {
  ...
  bool muted;
  EXPECT_EQ(NetEq::kOK, neteq_->GetAudio(&output_, &muted));
  EXPECT_EQ(Operations::kCodecInternalCng, neteq_->last_operation_for_test());
}
...
TEST_F(NetEqImplTest120ms, Normal) {
  ...
  EXPECT_EQ(Operations::kNormal, neteq_->last_operation_for_test());
}
...
TEST_F(NetEqImplTest120ms, Merge) {
  ...
  bool muted;
  EXPECT_CALL(*mock_neteq_controller_, GetDecision(_, _, _, _, _, _, _, _, _))
      .WillOnce(Return(Operations::kExpand));
  EXPECT_EQ(NetEq::kOK, neteq_->GetAudio(&output_, &muted));

  InsertPacket(first_timestamp() + 2 * timestamp_diff_between_packets());

  EXPECT_CALL(*mock_neteq_controller_, GetDecision(_, _, _, _, _, _, _, _, _))
      .WillOnce(Return(Operations::kMerge));

  EXPECT_EQ(NetEq::kOK, neteq_->GetAudio(&output_, &muted));
  EXPECT_EQ(Operations::kMerge, neteq_->last_operation_for_test());
}
...
TEST_F(NetEqImplTest120ms, Expand) {
  ...
  bool muted;
  EXPECT_EQ(NetEq::kOK, neteq_->GetAudio(&output_, &muted));
  EXPECT_EQ(Operations::kExpand, neteq_->last_operation_for_test());
}
...
TEST_F(NetEqImplTest120ms, FastAccelerate) {
  ...
  EXPECT_CALL(*mock_neteq_controller_, GetDecision(_, _, _, _, _, _, _, _, _))
      .Times(1)
      .WillOnce(Return(Operations::kFastAccelerate));

  bool muted;
  EXPECT_EQ(NetEq::kOK, neteq_->GetAudio(&output_, &muted));
  EXPECT_EQ(Operations::kFastAccelerate, neteq_->last_operation_for_test());
}
...
TEST_F(NetEqImplTest120ms, PreemptiveExpand) {
  ...
  EXPECT_CALL(*mock_neteq_controller_, GetDecision(_, _, _, _, _, _, _, _, _))
      .Times(1)
      .WillOnce(Return(Operations::kPreemptiveExpand));

  bool muted;
  EXPECT_EQ(NetEq::kOK, neteq_->GetAudio(&output_, &muted));
  EXPECT_EQ(Operations::kPreemptiveExpand, neteq_->last_operation_for_test());
}
...
TEST_F(NetEqImplTest120ms, Accelerate) {
  ...
  EXPECT_CALL(*mock_neteq_controller_, GetDecision(_, _, _, _, _, _, _, _, _))
      .Times(1)
      .WillOnce(Return(Operations::kAccelerate));

  bool muted;
  EXPECT_EQ(NetEq::kOK, neteq_->GetAudio(&output_, &muted));
  EXPECT_EQ(Operations::kAccelerate, neteq_->last_operation_for_test());
}
