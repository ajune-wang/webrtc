 ...
 private:
  ...
  class OutgoingStream {
   ...
   private:
    ...
    // Increases or decreases `buffered_amount_`.
    ...
    // The current amount of buffered data. Don't modify directly; Use
    // `IncreaseBufferedAmount` or `DecreaseBufferedAmount`.
    size_t buffered_amount_ = 0;

    // Set to the threshold when the `on_buffered_amount_low_` callback should
    // be triggered.
    size_t buffered_amount_low_threshold_ = 0;

    // Indicates if the callback has been triggered since the threshold was last
    // reached. Will be reset when the amount goes above the threshold again.
