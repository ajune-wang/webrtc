...
std::unique_ptr<ScalableVideoController> CreateVp9ScalabilityStructure(
  ...
  const VideoCodecVP9& vp9 = codec.VP9();

  char name[20];
  rtc::SimpleStringBuilder ss(name);
  if (vp9.numberOfSpatialLayers == 1 && vp9.numberOfTemporalLayers <= 1) {
    return std::make_unique<ScalableVideoControllerNoLayering>();
  } else {
    if (vp9.interLayerPred == InterLayerPredMode::kOff &&
        vp9.numberOfSpatialLayers > 1) {
      ss << "S";
    } else {
      ss << "L";
    }
    ss << vp9.numberOfSpatialLayers << "T"
       << std::max<int>(1, vp9.numberOfTemporalLayers);
    if (vp9.interLayerPred == InterLayerPredMode::kOnKeyPic &&
        vp9.numberOfSpatialLayers > 1) {
      ss << "_KEY";
    }
  }
  // Check spatial ratio.
  if (vp9.numberOfSpatialLayers > 1 &&
      codec.spatialLayers[0].targetBitrate > 0) {
    if (codec.width !=
            codec.spatialLayers[vp9.numberOfSpatialLayers - 1].width ||
        codec.height !=
            codec.spatialLayers[vp9.numberOfSpatialLayers - 1].height) {
      RTC_LOG(LS_WARNING)
          << "Top layer resolution expected to match overall resolution";
      return nullptr;
    }
    if (codec.spatialLayers[1].width == 2 * codec.spatialLayers[0].width) {
      for (int sid = 1; sid < vp9.numberOfSpatialLayers; ++sid) {
        if (codec.spatialLayers[sid].width !=
                2 * codec.spatialLayers[sid - 1].width ||
            codec.spatialLayers[sid].height !=
                2 * codec.spatialLayers[sid - 1].height) {
          RTC_LOG(LS_WARNING) << "Inconsistent scalability ratio.";
          return nullptr;
        }
      }
      // no suffix for 2:1 ratio.
    } else if (2 * codec.spatialLayers[1].width ==
               3 * codec.spatialLayers[0].width) {
      for (int sid = 1; sid < vp9.numberOfSpatialLayers; ++sid) {
        if (2 * codec.spatialLayers[sid].width !=
                3 * codec.spatialLayers[sid - 1].width ||
            2 * codec.spatialLayers[sid].height !=
                3 * codec.spatialLayers[sid - 1].height) {
          RTC_LOG(LS_WARNING) << "Inconsistent scalability ratio.";
          return nullptr;
        }
      }
...
void VP9EncoderImpl::PopulateCodecSpecific(CodecSpecificInfo* codec_specific,
  ...
  if (svc_controller_) {
    ...
    codec_specific->generic_frame_info =
        svc_controller_->OnEncodeDone(std::move(*it));
    if (is_key_frame && codec_specific->generic_frame_info) {
