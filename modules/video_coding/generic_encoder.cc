...
void VCMEncodedFrameCallback::FillTimingInfo(size_t simulcast_svc_idx,
  ...
  uint8_t timing_flags = VideoSendTiming::TimingFrameFlags::kNotTriggered;
  {
    rtc::CritScope crit(&timing_params_lock_);
    ...
    if (outlier_frame_size && encoded_image->_length >= *outlier_frame_size) {
      timing_flags |= VideoSendTiming::TimingFrameFlags::kTriggeredBySize;
    }
    ...
    if (last_timing_frame_time_ms_ == -1 ||
      ...
      timing_flags |= VideoSendTiming::TimingFrameFlags::kTriggeredByTimer;
  ...
  if (encode_start_ms) {
    ...
    encoded_image->timing_.flags = VideoSendTiming::TimingFrameFlags::kInvalid;
  }
}
