...
// key frame request.
constexpr int kDefaultMaxFramesInFlightPerStreamCount = 270;
...
struct AnalyzerStats {
  ...
  // Amount of cpu overloaded comparisons. Comparison is cpu overloaded if it is
  // queued when there are too many not processed comparisons in the queue.
  // Overloaded comparison doesn't include metrics, that require heavy
  // computations like SSIM and PSNR.
  int64_t cpu_overloaded_comparisons_done = 0;
  // Amount of memory overloaded comparisons. Comparison is memory overloaded if
 ...
 public:
  explicit DefaultVideoQualityAnalyzer(
      ...
      int max_frames_in_flight_per_stream_count =
          kDefaultMaxFramesInFlightPerStreamCount);
