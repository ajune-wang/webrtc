...
TEST(RtcpTransceiverImplTest, SendsRembWhenSet) {
  const uint32_t kSenderSsrc = 12345;
  const uint32_t kBitrateBps = 10000;
  ...
  rtcp_transceiver.SetRemb(kBitrateBps, kMediaSsrcs);
...
TEST(RtcpTransceiverImplTest, SetRembUpdatesValuesToSend) {
  const uint32_t kSenderSsrc = 12345;
  const uint32_t kBitrateBps1 = 10000;
  const uint32_t kBitrateBps2 = 70000;
...
TEST(RtcpTransceiverImplTest, SendsRembRepeatedlyWhenSet) {
  const uint32_t kSenderSsrc = 12345;
  MockTransport outgoing_transport;
  RtcpTransceiverConfig config;
  config.feedback_ssrc = kSenderSsrc;
