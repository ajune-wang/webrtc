...
TEST(SimulatedNetworkTest, EnqueueFirstPacketOnNetworkWithMaxCapacity) {
  ...
  EXPECT_EQ(network.NextDeliveryTimeUs(), 1);
}
...
TEST(SimulatedNetworkTest,
  ...
  network.SetConfigNow({.link_capacity_kbps = 10},
...
TEST(SimulatedNetworkTest,
  ...
  network.SetConfigNow({.link_capacity_kbps = 10},
                       /*config_update_time_us*/ TimeDelta::Millis(500).us());
  network.SetConfigNow({.link_capacity_kbps = 0},
...
TEST(SimulatedNetworkTest, SetConfigUpdateQueueDelayAfterDelivery) {
  ...
  network.SetConfigNow({.queue_delay_ms = 1, .link_capacity_kbps = 100},
                       /*config_update_time_us*/ TimeDelta::Millis(500).us());
  EXPECT_EQ(network.NextDeliveryTimeUs(), TimeDelta::Millis(1000 + 1).us());
