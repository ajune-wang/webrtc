...
AudioProcessingSimulator::~AudioProcessingSimulator() {
  ...
  int above_threshold_el_counter = 0;
  for (const auto el : echo_likelihood_values_) {
    if (el > 0.55f) {
      above_threshold_el_counter++;
    }
  }
  std::cout << "<echo_likelihood_fraction_over_0.55>"
            << static_cast<float>(above_threshold_el_counter) /
                   echo_likelihood_values_.size()
            << std::endl;

  int above_threshold_elrm_counter = 0;
  for (const auto el : echo_likelihood_recent_max_values_) {
    if (el > 0.55f) {
      above_threshold_elrm_counter++;
    }
  }
  std::cout << "<echo_likelihood_recent_max_fraction_over_0.55>"
            << static_cast<float>(above_threshold_elrm_counter) /
                   echo_likelihood_recent_max_values_.size()
            << std::endl;
}
