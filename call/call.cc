...
class Call final : public webrtc::Call,
 ...
 private:
  ...
  bool IdentifyReceivedPacket(RtpPacketReceived& packet,
      ...
      RTC_RUN_ON(worker_thread_);
  void RegisterReceiveStream(uint32_t ssrc, ReceiveStream* stream)
      RTC_RUN_ON(worker_thread_);
  void UnregisterReceiveStream(uint32_t ssrc) RTC_RUN_ON(worker_thread_);
...
// RTC_RUN_ON(worker_thread_)
// TODO(bugs.webrtc.org/11993): Expect to be called on the network thread.
void Call::RegisterReceiveStream(uint32_t ssrc, ReceiveStream* stream) {
  RTC_DCHECK_RUN_ON(&receive_11993_checker_);
  RTC_DCHECK(stream);
  auto inserted = receive_rtp_config_.emplace(ssrc, stream);
  RTC_DCHECK(inserted.second) << "ssrc already registered: " << ssrc;
}

// RTC_RUN_ON(worker_thread_)
// TODO(bugs.webrtc.org/11993): Expect to be called on the network thread.
void Call::UnregisterReceiveStream(uint32_t ssrc) {
  RTC_DCHECK_RUN_ON(&receive_11993_checker_);
  auto erased = receive_rtp_config_.erase(ssrc);
  RTC_DCHECK(erased) << "ssrc wasn't registered: " << ssrc;
}
