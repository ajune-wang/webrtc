...
TEST(AdaptiveFirFilter, FilterAdaptationNeonOptimizations) {
  ...
  std::vector<std::vector<FftData>> H_C(
      kNumPartitions, std::vector<FftData>(kNumRenderChannels));
  std::vector<std::vector<FftData>> H_Neon(
      kNumPartitions, std::vector<FftData>(kNumRenderChannels));
  for (size_t p = 0; p < kNumPartitions; ++p) {
    for (size_t ch = 0; ch < kNumRenderChannels; ++ch) {
      H_C[p][ch].Clear();
      H_Neon[p][ch].Clear();
    }
  }
  ...
  AdaptPartitions_Neon(*render_buffer, G, kNumPartitions, &H_Neon);
  AdaptPartitions(*render_buffer, G, kNumPartitions, &H_C);
  AdaptPartitions_Neon(*render_buffer, G, kNumPartitions, &H_Neon);
  AdaptPartitions(*render_buffer, G, kNumPartitions, &H_C);
  ...
  ApplyFilter_Neon(*render_buffer, kNumPartitions, H_Neon, &S_Neon);
  ApplyFilter(*render_buffer, kNumPartitions, H_C, &S_C);
...
TEST(AdaptiveFirFilter, ComputeFrequencyResponseNeonOptimization) {
  ...
  const size_t kNumRenderChannels = 1;
  std::vector<std::vector<FftData>> H(kNumPartitions,
                                      std::vector<FftData>(kNumRenderChannels));
  ...
  ComputeFrequencyResponse(H, &H2);
  ComputeFrequencyResponse_Neon(H, &H2_Neon);
...
TEST(AdaptiveFirFilter, FilterAdaptationSse2Optimizations) {
  ...
  if (use_sse2) {
    ...
    for (size_t k = 0; k < 500; ++k) {
      ...
      ApplyFilter_Sse2(*render_buffer, kNumPartitions, H_Sse2, &S_Sse2);
      ApplyFilter(*render_buffer, kNumPartitions, H_C, &S_C);
      for (size_t j = 0; j < S_C.re.size(); ++j) {
      ...
      AdaptPartitions_Sse2(*render_buffer, G, kNumPartitions, &H_Sse2);
      AdaptPartitions(*render_buffer, G, kNumPartitions, &H_C);
...
TEST(AdaptiveFirFilter, ComputeFrequencyResponseSse2Optimization) {
  ...
  if (use_sse2) {
    ...
    ComputeFrequencyResponse(kNumPartitions, H, &H2);
    ComputeFrequencyResponse_Sse2(kNumPartitions, H, &H2_Sse2);
