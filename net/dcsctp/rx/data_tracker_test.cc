...
TEST_F(DataTrackerTest,
  ...
  Observer({11});
  EXPECT_EQ(
      tracker_->GetHandoverReadiness(),
      HandoverReadinessStatus(HandoverUnreadinessReason::kDataTrackerNotIdle));
  tracker_->ObservePacketEnd();
  EXPECT_EQ(
      tracker_->GetHandoverReadiness(),
      HandoverReadinessStatus(HandoverUnreadinessReason::kDataTrackerNotIdle));
...
TEST_F(DataTrackerTest, HandoverWhileSendingSackEveryPacketOnPacketLoss) {
  ...
  EXPECT_EQ(tracker_->GetHandoverReadiness(),
            HandoverReadinessStatus()
                .Add(HandoverUnreadinessReason::kDataTrackerNotIdle)
                .Add(HandoverUnreadinessReason::kDataTrackerTsnBlocksPending));
  tracker_->ObservePacketEnd();
  EXPECT_TRUE(tracker_->ShouldSendAck());
  Observer({14});
  tracker_->ObservePacketEnd();
