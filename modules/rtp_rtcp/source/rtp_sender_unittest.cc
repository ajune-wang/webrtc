...
TEST_P(RtpSenderTestWithoutPacer,
  ...
  constexpr size_t kPacketSize = 1400;
  ...
  const size_t expected_bytes = GetParam().with_overhead
                                    ? kPacketSize + kRtpOverheadBytesPerPacket
                                    : kPacketSize;
...
TEST_P(RtpSenderTestWithoutPacer, BytesReportedCorrectly) {
  ...
  EXPECT_EQ(rtp_stats.transmitted.payload_bytes, kPacketSize);
...
TEST_P(RtpSenderTestWithoutPacer, ClearHistoryOnSequenceNumberChange) {
