...
class HardwareVideoEncoder implements VideoEncoder {
  ...
  private static final int REQUESTED_RESOLUTION_ALIGNMENT = 16;
  ...
  public VideoCodecStatus initEncode(Settings settings, Callback callback) {
    ...
    if (settings.width % REQUESTED_RESOLUTION_ALIGNMENT != 0
        || settings.height % REQUESTED_RESOLUTION_ALIGNMENT != 0) {
  ...
  public EncoderInfo getEncoderInfo() {
    ...
    return new EncoderInfo(
        /* requestedResolutionAlignment= */ REQUESTED_RESOLUTION_ALIGNMENT,
  ...
  private VideoCodecStatus resetCodec(int newWidth, int newHeight, boolean newUseSurfaceMode) {
    ...
    if (newWidth % REQUESTED_RESOLUTION_ALIGNMENT != 0
        || newHeight % REQUESTED_RESOLUTION_ALIGNMENT != 0) {
      Logging.e(TAG, "MediaCodec is only tested with resolutions that are 16x16 aligned.");
      return VideoCodecStatus.ERR_SIZE;
    }
    width = newWidth;
