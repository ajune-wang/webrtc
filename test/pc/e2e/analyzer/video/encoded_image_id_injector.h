...
// Injects frame id into EncodedImage on encoder side and extract it back on
// decode side.
class EncodedImageIdInjector {
 public:
  ...
  virtual EncodedImage InjectId(uint16_t id, const EncodedImage& source) = 0;
...
class QualityAnalyzingVideoContext {
 public:
  enum CodingType { ENCODER, DECODER };

  virtual ~QualityAnalyzingVideoContext() = default;

  // Returns id injector for specified |coding_entity_id| and |type|. Coding
  // entity is either encoder or decoder.
  virtual EncodedImageIdInjector* GetIdInjector(int coding_entity_id,
                                                CodingType type) = 0;
};
