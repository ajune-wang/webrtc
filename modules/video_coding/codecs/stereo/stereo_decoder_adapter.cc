...
int32_t StereoDecoderAdapter::InitDecode(const VideoCodec* codec_settings,
  ...
  for (size_t i = 0; i < kStereoCodecStreams; ++i) {
    std::unique_ptr<VideoDecoder> decoder = CreateScopedVideoDecoder(
        factory_, cricket::VideoCodec("VP9"), cricket::VideoDecoderParams());
...
int32_t StereoDecoderAdapter::Decode(
  ...
  if (rv)
    return rv;
  int32_t rv =
      decoders_[kAXXStream]->Decode(input_image, missing_frames, nullptr,
                                    codec_specific_info, render_time_ms);
  return rv;
}
