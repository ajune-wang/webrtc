...
RtpTransceiver::RtpTransceiver(cricket::MediaType type)
    : type_(type) {
  RTC_DCHECK(type == cricket::MEDIA_TYPE_AUDIO || type == cricket::MEDIA_TYPE_VIDEO);
}
...
void RtpTransceiver::set_channel(cricket::BaseChannel* channel) {
  ...
  for (auto sender : senders_) {
    if (type() == cricket::MEDIA_TYPE_AUDIO) {
      static_cast<AudioRtpSender*>(sender->internal())->SetChannel(static_cast<cricket::VoiceChannel*>(channel));
    } else {
      static_cast<VideoRtpSender*>(sender->internal())->SetChannel(static_cast<cricket::VideoChannel*>(channel));
    }
  }
  for (auto receiver : receivers_) {
    if (type() == cricket::MEDIA_TYPE_VIDEO) {
      static_cast<AudioRtpReceiver*>(receiver->internal())->SetChannel(static_cast<cricket::VoiceChannel*>(channel));
    } else {
      static_cast<VideoRtpReceiver*>(receiver->internal())->SetChannel(static_cast<cricket::VideoChannel*>(channel));
    }
  }
}

void RtpTransceiver::AddSender(rtc::scoped_refptr<RtpSenderProxyWithInternal<RtpSenderInternal>> sender) {
  RTC_DCHECK(sender);
  RTC_DCHECK(std::find(senders_.begin(), senders_.end(), sender) == senders_.end());
...
std::vector<rtc::scoped_refptr<RtpSenderProxyWithInternal<RtpSenderInternal>>> RtpTransceiver::senders() const {
  return senders_;
}

void RtpTransceiver::AddReceiver(rtc::scoped_refptr<RtpReceiverProxyWithInternal<RtpReceiverInternal>> receiver) {
  RTC_DCHECK(receiver);
  RTC_DCHECK(std::find(receivers_.begin(), receivers_.end(), receiver) == receivers_.end());
}
...
std::vector<rtc::scoped_refptr<RtpReceiverProxyWithInternal<RtpReceiverInternal>>> RtpTransceiver::receivers() const {
  return receivers_;
}
