...
using rnn_vad::kFrameSize20ms12kHz;
using rnn_vad::kFrameSize20ms24kHz;
using rnn_vad::kNumOpusBands;
using rnn_vad::kPitchMaxPeriod12kHz;
...
TEST(RnnVad, CheckSpectralCoefficients) {
  // PCM samples reader and buffers.
  auto samples_reader = CreatePcmSamplesReader(kFrameSize10ms48kHz);
  const size_t num_frames = samples_reader.second;
  ...
  auto seq_buf_view =
      seq_buf.GetBufferView(kPitchMaxPeriod12kHz, kFrameSize20ms12kHz);
  BiQuadFilter hpf(
      {-1.98889291, 0.98895425, 0.99446179, -1.98892358, 0.99446179});
  SpectralFeaturesExtractor feature_extractor;
  ...
  size_t num_frames_without_silence = 0;
  {
    ...
    for (size_t i = 0; i < num_frames; ++i) {
      ...
      decimator.Resample(samples_48k_10ms.data(), samples_48k_10ms.size(),
                         samples_10ms_24kHz.data(), samples_10ms_24kHz.size());
      hpf.ProcessFrame(samples_10ms_24kHz_view, samples_10ms_24kHz_view);
      seq_buf.Push(samples_10ms_24kHz_view);
      // The feature extractor is only used to compute spectral coefficients;
      // therefore, it is ok to pass the same frame for both reference and
      // lagged frames below.
      const bool is_silence =
          feature_extractor.AnalyzeCheckSilence(seq_buf_view, seq_buf_view);
      if (is_silence)
        continue;
      ...
      feature_extractor.CopySpectralCoefficients({computed_spectral_coeffs});
      ExpectNearAbsolute({expected_spectral_coeffs}, {computed_spectral_coeffs},
                         11.f);
