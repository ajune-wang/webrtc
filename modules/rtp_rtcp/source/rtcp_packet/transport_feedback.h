...
class TransportFeedback : public Rtpfb {
 public:
  ...
  // For received packets pass receieve time as `delta` since the `BaseTime()`.
  // For missed packets pass PlusInfinity as the 2nd parameter.
  void HandleAllPackets(rtc::FunctionView<void(uint16_t sequence_number,
                                               TimeDelta delta)> handler) const;
