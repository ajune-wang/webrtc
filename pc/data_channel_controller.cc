...
void DataChannelController::DisconnectDataChannel(
  ...
  signaling_thread()->PostTask(ToQueuedTask([self = weak_factory_.GetWeakPtr(),
    ...
    if (self) {
      ...
      self->data_transport_writable_callbacks_.RemoveReceivers(
          webrtc_data_channel);
      self->data_channel_transport_received_data_callbacks_.RemoveReceivers(
          webrtc_data_channel);
      self->data_channel_transport_channel_closing_callbacks_.RemoveReceivers(
          webrtc_data_channel);
