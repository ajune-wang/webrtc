...
TEST(TestVp9ReferenceControl, SetFrameReferences1SL1TL) {
  ...
  for (size_t pics_since_key = 0; pics_since_key < 4; ++pics_since_key) {
    ...
    EXPECT_EQ(ref_cfg.reference_last[0], 1);
    EXPECT_EQ(ref_cfg.reference_golden[0], 0);
    EXPECT_EQ(ref_cfg.reference_alt_ref[0], 0);

    EXPECT_EQ(ref_cfg.update_buffer_slot[0], 1);

    ++layers[0].frames_since_key;
  }
}

TEST(TestVp9ReferenceControl, SetFrameReferences1SL2TL) {
  std::vector<ReferenceControl::ReferenceConfig> layers;
...
TEST(TestVp9ReferenceControl, SetFrameReferences1SL3TL) {
  std::vector<ReferenceControl::ReferenceConfig> layers;
  layers.push_back({4,
                    0ull,
                    false,
...
TEST(TestVp9ReferenceControl, SetFrameReferences3SL3TL) {
  std::vector<ReferenceControl::ReferenceConfig> layers;
  layers.push_back({4,
                    0ull,
                    false,
  ...
  for (int pics_since_key = 0; pics_since_key < 4; ++pics_since_key) {
    ...
    for (size_t i = 0; i < layers.size(); ++i) {
      ...
      EXPECT_EQ(ref_cfg.reference_last[i], 1);

      if (layers[i].inter_layer_pred) {
        const int base_ref_idx = ffs(ref_cfg.update_buffer_slot[i - 1]) - 1;
        EXPECT_EQ(ref_cfg.gld_fb_idx[i], base_ref_idx);
        EXPECT_EQ(ref_cfg.reference_golden[i], 1);
      }
      ...
      if (layers[i].ref_frame_flag[gof_idx]) {
        ...
        EXPECT_EQ(ref_cfg.update_buffer_slot[i],
                  1 << ReferenceControl::kSpatialRefFrameBufIdx);
      }
