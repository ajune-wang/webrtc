...
class SendTransport : public Transport {
 public:
  ...
  bool SendRtp(const uint8_t* data,
    ...
    last_packet_.reset(new RtpPacketReceived(&header_extensions_));
  ...
  std::unique_ptr<RtpPacketReceived> last_packet_;
};
...
TEST_P(RtpRtcpImpl2Test, AssignsAbsoluteSendTime) {
  ...
  EXPECT_TRUE(
      sender_.last_packet().GetExtension<AbsoluteSendTime>(&abs_send_time));
  EXPECT_NE(abs_send_time, 0u);
}
