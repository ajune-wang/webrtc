...
// This is a `prototype` namespace, appropriate for rapid prototyping of things
// that should eventually end up in WebRTC codebase in webrtc namespace.
// Anything in here is temporary and should be removed (will be moved to open
// source).
namespace webrtc {
...
class DatagramSinkInterface {
 public:
  ...
  virtual void OnDatagramReceived(absl::string_view data) = 0;
  ...
  // NOTE that when we move GoogCC to QUIC in datagram transport and
  // remove RTP transport sequence numbers and RTCP ACKs, we will need to
  // propagate datagram ACKs up the stack to at least NACK and FEC modules.
  // ACKs also used today to calculate loss rate at least on audio side we
  // can either take media transport approach and return loss or just keep
  // loss calculation as is if we need to provide ACKs anyway.
};

class DatagramTransportInterface {
 public:
  DatagramTransportInterface() = default;
  ...
  virtual RTCError SendDatagram(std::string data, DatagramId datagram_id) = 0;

  // Returns maximum size of datagram message, does not change.
  // TODO(b/132161793): Because value may be undefined before connection setup
