...
RTCError PeerConnection::UpdateDataChannel(
  ...
  if (content.rejected) {
    ...
    if (!rtp_data_channel_ && !data_channel_transport_.load()) {
...
bool PeerConnection::GetSctpSslRole(rtc::SSLRole* role) {
  ...
  if (!data_channel_transport_.load()) {
  ...
  if (data_channel_transport_) {
    ...
    RTCError error = network_thread()->Invoke<RTCError>(
        RTC_FROM_HERE, [this, params, send_params, payload] {
          return data_channel_transport_.load()->SendData(params.sid,
                                                          send_params, payload);
        });
...
void PeerConnection::AddSctpDataStream(int sid) {
  if (data_channel_transport_) {
    network_thread()->Invoke<void>(RTC_FROM_HERE, [this, sid] {
      if (data_channel_transport_) {
        data_channel_transport_.load()->OpenChannel(sid);
      }
    });
  }
}
...
void PeerConnection::RemoveSctpDataStream(int sid) {
  if (data_channel_transport_) {
    network_thread()->Invoke<void>(RTC_FROM_HERE, [this, sid] {
      if (data_channel_transport_) {
        data_channel_transport_.load()->CloseChannel(sid);
      }
    });
  }
}
...
void PeerConnection::OnReadyToSend() {
  ...
  data_channel_transport_invoker_->AsyncInvoke<void>(
      RTC_FROM_HERE, signaling_thread(), [this] {
        ...
        // TODO(mellem): Hack: is there a better way to get data channels to
        // open their streams again when the transport changes?
        // This is currently required to make re-offers work, because a new data
        // channel transport is created and we need to call open on each stream
        // for that transport, but we already called it (probably for the wrong
        // transport).
...
bool PeerConnection::SetupDataChannelTransport_n(const std::string& mid) {
  ...
  data_channel_transport_.load()->SetDataSink(this);
  sctp_mid_ = mid;
...
void PeerConnection::TeardownDataChannelTransport_n() {
  ...
  if (data_channel_transport_) {
    data_channel_transport_.load()->SetDataSink(nullptr);
  }
  data_channel_transport_ = nullptr;
}
