...
void RtpTransportControllerSend::OnNetworkRouteChanged(
  ...
  // Currently we only check local/remote network id (i.e IP address),
  // and do not consider if we change route due to TURN.
  //
  ...
  if (old_route.connected != network_route.connected ||
    ...
    task_queue_.PostTask([this, msg, network_route] {
      ...
      if (reset_feedback_on_route_change_) {
        // TODO(bugs.webrtc.org/11438) : should transport_feedback_adapter have
        // a real "route" rather than just local/remote network_id.
