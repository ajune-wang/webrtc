...
EncodedImage DefaultEncodedImageIdInjector::InjectId(
  ...
  if (buffer->capacity() < source._length + kEncodedImageBufferExpansion) {
...
std::pair<uint16_t, EncodedImage> DefaultEncodedImageIdInjector::ExtractId(
  ...
  EncodedImage out = source;
  std::vector<uint8_t>* buffer = NextBuffer();
  if (buffer->capacity() < source.capacity() - kEncodedImageBufferExpansion) {
...
EncodedImageIdInjector* DefaultQualityAnalyzingVideoContext::GetIdInjector(
  ...
  auto it = injectors_.find(std::pair<int, CodingType>(coding_entity_id, type));
  if (it == injectors_.end()) {
    ...
    injectors_.insert(std::pair<std::pair<int, CodingType>,
                                std::unique_ptr<EncodedImageIdInjector>>(
        std::pair<int, CodingType>(coding_entity_id, type),
        std::move(injector)));
    return out;
  }
