    ...
    .frame_settings = {
        {/*frame_num=*/0,
         CodingSettings({.scalability_mode = ScalabilityMode::kL1T1,
                         ...
                         .bitrate = {DataRate::KilobitsPerSec(2000)}})},
        {/*frame_num=*/300,
         CodingSettings({.scalability_mode = ScalabilityMode::kL1T1,
                         .width = {1280},
                         .height = {720},
...
class LocalFrameReader : public VideoCodecTester::TestRawVideoSource {
 public:
  LocalFrameReader(std::unique_ptr<YuvFrameReader> frame_reader,
  ...
  absl::optional<VideoFrame> PullFrame() override {
    ...
    int pulled_frame;
    auto buffer = frame_reader_->PullFrame(&pulled_frame, frame_settings.width[0], frame_settings.height[0], /*base_framerate=*/30, frame_settings.framerate.hertz());
    auto frame = VideoFrame::Builder().set_video_frame_buffer(buffer).set_timestamp_rtp(timestamp_rtp_).build();
  ...
  absl::optional<VideoFrame> ReadFrame(uint32_t timestamp_rtp) override {
    RTC_CHECK(pulled_frames_.find(timestamp_rtp) != pulled_frames_.end()) << "Frame with RTP timestamp " << timestamp_rtp << " was not pulled";
    auto buffer = frame_reader_->ReadFrame(pulled_frames_[timestamp_rtp]);
    return VideoFrame::Builder().set_video_frame_buffer(buffer).set_timestamp_rtp(timestamp_rtp).build();
  }
...
TEST_P(EncodeDecodeTest, TestEncodeDecode) {
  ...
  auto test_frame_reader = std::make_unique<LocalFrameReader>(
      std::move(frame_reader), test_settings);

  auto encoder = CreateEncoder(codec_info, test_settings.frame_settings);
  auto decoder = CreateDecoder(codec_info);

  VideoCodecTesterImpl tester;
  VideoCodecTester::TestSettings tester_settings;
  auto stats =
      tester.RunEncodeDecodeTest(std::move(test_frame_reader), tester_settings,
