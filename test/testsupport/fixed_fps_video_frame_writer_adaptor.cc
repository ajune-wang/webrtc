...
FixedFpsVideoFrameWriterAdaptor::FixedFpsVideoFrameWriterAdaptor(
    int fps,
    Clock* clock,
    VideoFrameWriter* video_writer)
    : inter_frame_interval_(kOneSecond / fps),
      clock_(clock),
      delegate_(video_writer) {}

FixedFpsVideoFrameWriterAdaptor::~FixedFpsVideoFrameWriterAdaptor() {
  Close();
}

void FixedFpsVideoFrameWriterAdaptor::Close() {
  if (is_closed_) {
    return;
  }
  is_closed_ = true;
  ...
  Timestamp now = Now();
  RTC_CHECK(WriteDroppedFramesExceptLast(now));
  RTC_CHECK(delegate_->WriteFrame(*last_frame_));
  delegate_->Close();
}

bool FixedFpsVideoFrameWriterAdaptor::WriteFrame(const VideoFrame& frame) {
  ...
  if (!WriteDroppedFramesExceptLast(now)) {
    return false;
  }
...
bool FixedFpsVideoFrameWriterAdaptor::WriteDroppedFramesExceptLast(
    Timestamp now) {
  RTC_CHECK(last_frame_time_.IsFinite());
...
Timestamp FixedFpsVideoFrameWriterAdaptor::Now() const {
