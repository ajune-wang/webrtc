...
class DtlsStunPiggybackController {
 public:
  ...
  // to obtain optional DTLS data or ACK.
  std::optional<absl::string_view> GetDataToPiggyback(
      StunMessageType stun_message_type);
  std::optional<uint64_t> GetAckToPiggyback(StunMessageType stun_message_type);
 ...
 private:
  ...
  std::optional<uint64_t> highest_received_dtls_sequence_number_;
};
