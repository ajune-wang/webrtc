...
TEST_F(NetEqImplTest, EnableRtxHandling) {
  ...
  EXPECT_CALL(*mock_neteq_controller_, GetDecision(_, _))
      .Times(1)
      .WillOnce(Return(NetEq::Operations::kNormal));
...
TEST_F(NetEqImplTest120ms, CodecInternalCng) {
  ...
  bool muted;
  EXPECT_EQ(NetEq::kOK, neteq_->GetAudio(&output_, &muted));
  EXPECT_EQ(NetEq::Operations::kCodecInternalCng,
...
TEST_F(NetEqImplTest120ms, Normal) {
  ...
  EXPECT_EQ(NetEq::Operations::kNormal, neteq_->last_operation_for_test());
}
...
TEST_F(NetEqImplTest120ms, Merge) {
  ...
  bool muted;
  EXPECT_CALL(*mock_neteq_controller_, GetDecision(_, _))
      .WillOnce(Return(NetEq::Operations::kExpand));
  EXPECT_EQ(NetEq::kOK, neteq_->GetAudio(&output_, &muted));

  InsertPacket(first_timestamp() + 2 * timestamp_diff_between_packets());

  EXPECT_CALL(*mock_neteq_controller_, GetDecision(_, _))
      .WillOnce(Return(NetEq::Operations::kMerge));

  EXPECT_EQ(NetEq::kOK, neteq_->GetAudio(&output_, &muted));
  EXPECT_EQ(NetEq::Operations::kMerge, neteq_->last_operation_for_test());
}
...
TEST_F(NetEqImplTest120ms, Expand) {
  ...
  bool muted;
  EXPECT_EQ(NetEq::kOK, neteq_->GetAudio(&output_, &muted));
  EXPECT_EQ(NetEq::Operations::kExpand, neteq_->last_operation_for_test());
}
...
TEST_F(NetEqImplTest120ms, FastAccelerate) {
  ...
  EXPECT_CALL(*mock_neteq_controller_, GetDecision(_, _))
      .Times(1)
      .WillOnce(Return(NetEq::Operations::kFastAccelerate));

  bool muted;
  EXPECT_EQ(NetEq::kOK, neteq_->GetAudio(&output_, &muted));
  EXPECT_EQ(NetEq::Operations::kFastAccelerate,
            neteq_->last_operation_for_test());
}
...
TEST_F(NetEqImplTest120ms, PreemptiveExpand) {
  ...
  EXPECT_CALL(*mock_neteq_controller_, GetDecision(_, _))
      .Times(1)
      .WillOnce(Return(NetEq::Operations::kPreemptiveExpand));

  bool muted;
  EXPECT_EQ(NetEq::kOK, neteq_->GetAudio(&output_, &muted));
  EXPECT_EQ(NetEq::Operations::kPreemptiveExpand,
            neteq_->last_operation_for_test());
}
...
TEST_F(NetEqImplTest120ms, Accelerate) {
  ...
  EXPECT_CALL(*mock_neteq_controller_, GetDecision(_, _))
      .Times(1)
      .WillOnce(Return(NetEq::Operations::kAccelerate));

  bool muted;
  EXPECT_EQ(NetEq::kOK, neteq_->GetAudio(&output_, &muted));
  EXPECT_EQ(NetEq::Operations::kAccelerate, neteq_->last_operation_for_test());
}
