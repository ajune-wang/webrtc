...
// backwards compatibility with rtc::CriticalSection from which this class is
// migrating users.
...
class RTC_LOCKABLE Mutex final {
 ...
 private:
  RTC_DISALLOW_COPY_AND_ASSIGN(Mutex);
...
class RTC_LOCKABLE GlobalMutex final {
 ...
 private:
  RTC_DISALLOW_COPY_AND_ASSIGN(GlobalMutex);
...
class RTC_SCOPED_LOCKABLE GlobalMutexLock final {
 ...
 private:
  RTC_DISALLOW_COPY_AND_ASSIGN(GlobalMutexLock);
