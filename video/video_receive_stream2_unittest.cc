...
using test::video_frame_matchers::Id;
...
class FakeVideoRenderer : public rtc::VideoSinkInterface<webrtc::VideoFrame> {
 public:
  ...
  absl::optional<webrtc::VideoFrame> WaitForFrame(TimeDelta wait) {
    if (!last_frame_) {
      loop_->Flush();
    ...
    using std::swap;
    absl::optional<webrtc::VideoFrame> ret;
    std::swap(last_frame_, ret);
    return ret;
  }
