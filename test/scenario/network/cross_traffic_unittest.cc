...
TEST(CrossTrafficTest, PulsedPeaksCrossTraffic) {
  ...
  EXPECT_NEAR(counter->total_packets_size(),
              config.peak_rate.bytes_per_sec() * 0.5,
              config.peak_rate.bytes_per_sec() * 0.05);
}
...
TEST(CrossTrafficTest, RandomWalkCrossTraffic) {
  ...
  RTC_LOG(INFO) << counter->packets_count() << " packets; "
                << counter->total_packets_size() << " bytes";
  EXPECT_NEAR(counter->total_packets_size(), config.peak_rate.bytes_per_sec(),
              config.peak_rate.bytes_per_sec() * 0.1);
}
