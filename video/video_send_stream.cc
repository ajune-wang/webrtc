...
std::unique_ptr<FlexfecSender> MaybeCreateFlexfecSender(
  ...
  if (config.rtp.flexfec.protected_media_ssrcs.empty()) {
    RTC_LOG(LS_WARNING) << "FlexFEC is enabled, but no protected media "
                        << "SSRC given. Therefore disabling FlexFEC.";
    return nullptr;
  }

  if (config.rtp.ssrcs.size() > 1) {
    RTC_LOG(LS_WARNING) << "Both FlexFEC and simulcast are enabled. This "
