...
TEST_F(WebRtcVideoChannelBaseTest, SetSink) {
  ...
  rtc::SetBE32(data1 + 8, kSsrc);
  webrtc::RtpPacketReceived packet1;
  RTC_CHECK(packet1.Parse(data1, sizeof(data1)));
  channel_->SetSink(kDefaultReceiveSsrc, NULL);
  EXPECT_TRUE(SetDefaultCodec());
  EXPECT_TRUE(SetSend(true));
  EXPECT_EQ(0, renderer_.num_rendered_frames());
  channel_->OnRtpPacket(packet1);
  EXPECT_TRUE(channel_->SetSink(kDefaultReceiveSsrc, &renderer_));
  EXPECT_TRUE(SendFrame());
  EXPECT_FRAME_WAIT(1, kVideoWidth, kVideoHeight, kTimeout);
}
...
TEST_F(WebRtcVideoChannelTest, DefaultReceiveStreamReconfiguresToUseRtx) {
  ...
  RTC_CHECK(packet.Parse(data, kDataLength));
...
TEST_F(WebRtcVideoChannelTest, ReceiveDifferentUnsignaledSsrc) {
  ...
  // TODO(nisse): Set attributes on RtpPacketReceived instead.
  ...
  webrtc::RtpPacketReceived packet;
  RTC_CHECK(packet.Parse(data, sizeof(data)));
  channel_->OnRtpPacket(packet);
  ...
  cricket::SetRtpHeader(data, sizeof(data), rtpHeader);
  webrtc::RtpPacketReceived packet2;
  RTC_CHECK(packet2.Parse(data, sizeof(data)));
  channel_->OnRtpPacket(packet2);
  ...
  cricket::SetRtpHeader(data, sizeof(data), rtpHeader);
  webrtc::RtpPacketReceived packet3;
  RTC_CHECK(packet3.Parse(data, sizeof(data)));
  channel_->OnRtpPacket(packet3);
