...
absl::optional<SendQueue::DataToSend> RRSendQueue::Produce(
  ...
  if (data.has_value()) {
    ...
    if (data->data.is_end) {
      // No more fragments. Continue with a different stream next time.
      RTC_LOG(LS_VERBOSE) << log_prefix_
                          << "Last chunk for stream_id=" << *next_stream_id_
                          << ", will use new next time";
...
absl::optional<SendQueue::DataToSend> RRSendQueue::Produce(TimeMs now,
  ...
  if (start_it != streams_.end()) {
    RTC_LOG(LS_VERBOSE) << log_prefix_ << "Want to use produce from stream_id="
                        << *next_stream_id_
                        << ", using stream_id=" << *start_it->first;
  }
