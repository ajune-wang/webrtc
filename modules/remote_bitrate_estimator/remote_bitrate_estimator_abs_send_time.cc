...
void RemoteBitrateEstimatorAbsSendTime::IncomingPacketInfo(
  ...
  std::vector<uint32_t> ssrcs;
  {
    ...
    if (!update_estimate) {
      ...
      // of an over-use.
      // Add an extra time delta to keep closer to behaviour of older code that
      // measured and compared time in milliseconds, and thus waited a bit more
      // to updated estimate.
      static constexpr TimeDelta kGrace = TimeDelta::Millis(1);
      if (last_update_.IsInfinite() ||
          now - last_update_ > remote_rate_.GetFeedbackInterval() + kGrace) {
        update_estimate = true;
