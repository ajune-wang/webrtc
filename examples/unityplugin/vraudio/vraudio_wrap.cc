...
bool VrAudioWrap::EnableRoomEffects(bool enable) {
  std::lock_guard<std::recursive_mutex> lock(spatializer_mutex_);

  if (spatializer_ == nullptr)
    return false;

  spatializer_->EnableRoomEffects(enable);
  return true;
}

bool VrAudioWrap::SetRoomProperties(float dimension_x,
                                    float dimension_y,
                                    float dimension_z,
                                    int material) {
  std::lock_guard<std::recursive_mutex> lock(spatializer_mutex_);
  ...
  vraudio::VrAudioApi::RoomProperties properties;
  properties.dimensions[0] = dimension_x;
  properties.dimensions[1] = dimension_y;
  properties.dimensions[2] = dimension_y;

  for (int i = 0; i < 6; i++) {
    properties.material_names[i] =
        static_cast<vraudio::VrAudioApi::MaterialName>(material);
  }

  spatializer_->SetRoomProperties(properties);
