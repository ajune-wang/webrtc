...
uint32_t VideoSendStreamImpl::OnBitrateUpdated(BitrateAllocationUpdate update) {
  ...
  double cwnd_reduce_ratio = update.cwnd_reduce_ratio;

  // If the congestion window pushback frame drop is enabled and active, we need
  // to check the reduced bitrate is not lower than encoder min bitrate
  if (cwnd_reduce_ratio > 0.01) {
    if (encoder_target_rate.bps() <= encoder_min_bitrate_bps_ ||
        encoder_target_rate.bps() <= 0) {
      // If the current target bitrate is already lower than encoder min
      // bitrate, then skip frame dropping. Target bitrate maybe 0 if encoder is
      // paused.
      cwnd_reduce_ratio = 0;
    } else if (encoder_target_rate.bps() -
                   encoder_target_rate.bps() * cwnd_reduce_ratio <
               encoder_min_bitrate_bps_) {
      // Make sure the reduced bitrate is not lower than min bitrate.
      cwnd_reduce_ratio = 1 - static_cast<double>(encoder_min_bitrate_bps_) /
