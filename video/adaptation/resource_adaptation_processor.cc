...
ResourceAdaptationProcessor::ResourceAdaptationProcessor(
    ...
    : manager_(manager),
      ...
      video_codec_type_(kVideoCodecGeneric),
...
void ResourceAdaptationProcessor::SetInput(
    absl::optional<int> input_pixels,
    absl::optional<int> input_fps,
    absl::optional<EncoderSettings> encoder_settings) {
  input_pixels_ = input_pixels;
  input_fps_ = input_fps;
  video_codec_type_ = GetVideoCodecTypeOrGeneric(encoder_settings);
  min_pixels_per_frame_ = MinPixelsPerFrame(encoder_settings);
  stream_adapter_->SetInput(
      VideoStreamInputState(input_pixels_.value(), input_fps_.value(),
                            video_codec_type_),
...
bool ResourceAdaptationProcessor::HasInsufficientInput() const {
  return !has_input_video_ || !manager_->encoder_settings_.has_value() ||
         !manager_->last_input_frame_size_.has_value() ||
         (effective_degradation_preference_ ==
              DegradationPreference::MAINTAIN_RESOLUTION &&
...
void ResourceAdaptationProcessor::OnResourceUnderuse(
  ...
  SetInput(manager_->last_input_frame_size_.value(),
           manager_->encoder_stats_observer_->GetInputFrameRate(),
           manager_->encoder_settings_);
