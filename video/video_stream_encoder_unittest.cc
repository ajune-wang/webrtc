...
class MockEncoderSelector
 ...
 public:
  MOCK_METHOD1(SetCurrentEncoder, void(const SdpVideoFormat& format));
  MOCK_METHOD1(EncodingBitrate, bool(const DataRate& rate));
  MOCK_METHOD0(EncoderBroken, bool());
  MOCK_METHOD0(SuggestedEncoder, SdpVideoFormat());
};
...
TEST_F(VideoStreamEncoderTest, EncoderSelectorCurrentEncoderIsSignaled) {
  ...
  EXPECT_CALL(encoder_selector, SetCurrentEncoder(_));
...
TEST_F(VideoStreamEncoderTest, EncoderSelectorBitrateSwitch) {
  ...
  ON_CALL(encoder_selector, EncodingBitrate(_)).WillByDefault(Return(true));
  ON_CALL(encoder_selector, SuggestedEncoder())
...
TEST_F(VideoStreamEncoderTest, EncoderSelectorBrokenEncoderSwitch) {
  ...
  ON_CALL(encoder_selector, EncoderBroken()).WillByDefault(Return(true));
  ON_CALL(encoder_selector, SuggestedEncoder())
