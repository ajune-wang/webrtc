...
struct ConfigHelper {
  ...
  std::unique_ptr<internal::AudioReceiveStream>
      CreateAudioReceiveStream(const AudioReceiveStream::Config& config) {
    std::unique_ptr<internal::AudioReceiveStream> stream(
        new internal::AudioReceiveStream(
            &rtp_stream_receiver_controller_, &packet_router_, config,
            audio_state_, &event_log_, channel_proxy_));
    return stream;
  }
...
TEST(AudioReceiveStreamTest, ConstructDestruct) {
  ConfigHelper helper;
  auto recv_stream = helper.CreateAudioReceiveStream(helper.config());
}
...
TEST(AudioReceiveStreamTest, StreamsShouldBeAddedToMixerOnceOnStart) {
  ...
  auto recv_stream1 = helper1.CreateAudioReceiveStream(helper1.config());
  auto recv_stream2 = helper2.CreateAudioReceiveStream(helper2.config());
