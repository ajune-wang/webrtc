...
class StoppableTaskWrapper : public QueuedRepeatableTask {
 public:
  explicit StoppableTaskWrapper(std::unique_ptr<QueuedRepeatableTask> task);
  // Implements QueuedRepeatableTask
 ...
 private:
  std::unique_ptr<QueuedRepeatableTask> task_;
};
...
class RTC_LOCKABLE RTC_EXPORT TaskQueue {
 ...
 private:
  webrtc::RepeatingTaskHandle CreateRepeatHandle(
      ...
      std::unique_ptr<webrtc::QueuedRepeatableTask> task) {
