...
class PeerConnectionSimulcastWithMediaFlowTests
 ...
 public:
  ...
  bool HasOutboundRtpWithRidAndScalabilityMode(
      ...
      absl::optional<uint32_t> frame_height = absl::nullopt) {
    rtc::scoped_refptr<const RTCStatsReport> report = GetStats(pc_wrapper);
    std::vector<const RTCOutboundRtpStreamStats*> outbound_rtps =
        report->GetStatsOfType<RTCOutboundRtpStreamStats>();
    ...
    if (frame_height.has_value()) {
      if (outbound_rtp->frame_height.is_defined()) {
        RTC_LOG(LS_INFO) << "Waiting for target resolution ("
                         << frame_height.value() << "p). Currently at "
                         << *outbound_rtp->frame_height << "p...";
      } else {
        RTC_LOG(LS_INFO)
            << "Waiting for target resolution. No frames encoded yet...";
      }
      if (!outbound_rtp->frame_height.is_defined() ||
          *outbound_rtp->frame_height != frame_height.value()) {
...
// the same as above except we end up with two inactive RTP streams, visible in
// GetStats() and GetParameters() returns all three encodings.
TEST_F(PeerConnectionSimulcastWithMediaFlowTests,
