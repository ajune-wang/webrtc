...
void EventLogAnalyzer::CreateMissingFeedbackGraph(Plot* plot) {
  ...
  for (auto& feedback : parsed_log_.transport_feedbacks(kIncomingPacket)) {
    auto transport_feedback = feedback.transport_feedback;
    auto received_packets = transport_feedback.GetReceivedPackets();
    TimeDelta shifted_time = TimeDelta::Zero();
    for (uint16_t seq_num = last_acked;
         seq_num < transport_feedback.GetBaseSequence(); ++seq_num) {
      missing_feedback_series.points.emplace_back(
          config_.GetCallTimeSec(feedback.timestamp + shifted_time), 1);
      shifted_time =
          std::min(shifted_time + TimeDelta::Millis(2), TimeDelta::Millis(50));
    }
    last_acked = transport_feedback.GetBaseSequence() +
                 transport_feedback.GetPacketStatusCount();
  }
  // Add the data set to the plot.
  ...
  plot->SetXAxis(config_.CallBeginTimeSec(), config_.CallEndTimeSec(),
                 "Time (s)", kLeftMargin, kRightMargin);
  plot->SetSuggestedYAxis(0, 1, "No feedback", kBottomMargin, kTopMargin);
  plot->SetTitle("Missing feedback");
  plot->SetYAxisTickLabels({{static_cast<float>(1), "No Feedback"}});
}
