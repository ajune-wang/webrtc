...
class LockRunner : public MessageHandler {
 public:
  ...
  void OnMessage(Message* msg) override {
    ...
    // implementation is faulty and the loop isn't unrolled, it would be
    // improbable that the error slips through.
...
TEST(MutexTest, ProtectsSharedResource_Mutex_RawLock) {
...
TEST(MutexTest, ProtectsSharedResource_Mutex_MutexLocker) {
  std::vector<std::unique_ptr<Thread>> threads;
...
TEST(MutexTest, ProtectsSharedResource_GlobalMutex_RawLock) {
  std::vector<std::unique_ptr<Thread>> threads;
...
TEST(MutexTest, ProtectsSharedResource_GlobalMutex_MutexLocker) {
  std::vector<std::unique_ptr<Thread>> threads;
