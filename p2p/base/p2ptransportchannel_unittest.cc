...
// stats from the gathering ICE endpoint not reveal the address of this local
...
// this remote host cadidate in stats.
TEST_F(P2PTransportChannelTest,
  ...
  ASSERT_EQ(3u, connection_infos_ep1.size());
  ASSERT_EQ(3u, candidate_stats_list_ep1.size());
  ASSERT_EQ(3u, connection_infos_ep2.size());
  ...
  for (const auto& connection_info : connection_infos_ep1) {
    ...
    if (local_candidate.type() == LOCAL_PORT_TYPE) {
      ...
      // srflx address. Note that NAT is not configure, hence the following
      // expectation.
