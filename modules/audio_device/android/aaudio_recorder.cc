...
AAudioRecorder::~AAudioRecorder() {
  ...
  Terminate();
  RTC_DLOG(INFO) << "detected owerflows: " << overflow_count_;
}
...
int AAudioRecorder::EnableBuiltInAEC(bool enable) {
  RTC_DLOG(INFO) << "EnableBuiltInAEC: " << enable;
  RTC_DLOG(LS_ERROR) << "Not implemented";
  return -1;
}

int AAudioRecorder::EnableBuiltInAGC(bool enable) {
  RTC_DLOG(INFO) << "EnableBuiltInAGC: " << enable;
  RTC_DLOG(LS_ERROR) << "Not implemented";
  return -1;
}

int AAudioRecorder::EnableBuiltInNS(bool enable) {
  RTC_DLOG(INFO) << "EnableBuiltInNS: " << enable;
  RTC_DLOG(LS_ERROR) << "Not implemented";
  return -1;
}

void AAudioRecorder::OnErrorCallback(aaudio_result_t error) {
  RTC_DLOG(LS_ERROR) << "OnErrorCallback: " << AAudio_convertResultToText(error);
  ...
  if (aaudio_.stream_state() == AAUDIO_STREAM_STATE_DISCONNECTED) {
    ...
    RTC_DLOG(WARNING) << "Input stream disconnected => restart is required";
...
aaudio_data_callback_result_t AAudioRecorder::OnDataCallback(
  ...
  // RTC_DCHECK(thread_checker_aaudio_.IsCurrent());
  // RTC_DLOG(INFO) << "OnDataCallback: " << num_frames;
  ...
  if (first_data_callback_) {
    RTC_DLOG(INFO) << "--- First input data callback: "
  ...
  if (overflow_count > overflow_count_) {
    RTC_DLOG(LS_ERROR) << "Overflow detected: " << overflow_count;
...
void AAudioRecorder::OnMessage(rtc::Message* msg) {
  ...
  switch (msg->message_id) {
    case kMessageInputStreamDisconnected:
      ...
      break;
    default:
      RTC_DLOG(LS_ERROR) << "Invalid message id: " << msg->message_id;
      break;
  }
}
