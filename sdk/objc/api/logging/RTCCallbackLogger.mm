...
class CallbackWithSeverityLogSink : public rtc::LogSink {
 public:
  ...
  void OnLogMessage(const std::string& message,
    ...
    if (callback_handler_) {
      RTCLoggingSeverity loggingSeverity = CallbackWithSeverityLogSink::Severity(severity);
      callback_handler_([NSString stringWithUTF8String:message.c_str()], loggingSeverity);
    }
  }

 private:
  static RTCLoggingSeverity Severity(rtc::LoggingSeverity severity) {
    switch (severity) {
      case rtc::LS_VERBOSE:
      ...
      case rtc::LS_SENSITIVE:
        ...
        // RTCLoggingSeverityNone is returned.
        return RTCLoggingSeverityNone;
      default:
        RTC_NOTREACHED();
    }
