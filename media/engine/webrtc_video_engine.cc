...
void WebRtcVideoChannel::FillSenderStats(VideoMediaSendInfo* video_media_info,
  ...
  for (const auto& send_stream : send_streams_) {
    auto infos = send_stream.GetPerLayerVideoSenderInfos(log_stats);
    if (infos.empty())
      continue;
    video_media_info->aggregated_senders.push_back(
        send_stream.GetAggregatedVideoSenderInfo(infos));
    for (auto&& info : infos) {
...
void WebRtcVideoChannel::FillReceiverStats(
  ...
  for (const auto& receive_stream : receive_streams_) {
    video_media_info->receivers.push_back(
        receive_stream.GetVideoReceiverInfo(log_stats));
  }
}
...
void WebRtcVideoChannel::FillBitrateInfo(BandwidthEstimationInfo* bwe_info) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  for (const auto& send_stream : send_streams_) {
    send_stream.FillBitrateInfo(bwe_info);
  }
}
...
void WebRtcVideoChannel::WebRtcVideoSendStream::FillBitrateInfo(
  ...
  for (const auto& substream : stats.substreams) {
    bwe_info->transmit_bitrate += substream.total_bitrate_bps;
    bwe_info->retransmit_bitrate += substream.retransmit_bitrate_bps;
  }
