...
const DataRate kInitialBitrate =
    DataRate::KilobitsPerSecond(kInitialBitrateKbps);
...
void UpdatesTargetRateBasedOnLinkCapacity(std::string test_name = "") {
  ...
  send_net->UpdateConfig([](NetworkSimulationConfig* c) {
    c->bandwidth = DataRate::KilobitsPerSecond(100);
  ...
  truth->PrintRow();
  EXPECT_NEAR(client->target_rate().KilobitsPerSecond(), 90, 25);
}
}  // namespace
...
class GoogCcNetworkControllerTest : public ::testing::Test {
 protected:
  GoogCcNetworkControllerTest()
      : current_time_(Timestamp::Milliseconds(123456)), factory_() {}
  ...
  PacketResult CreateResult(int64_t arrival_time_ms,
    ...
    packet_result.sent_packet.send_time = Timestamp::Milliseconds(send_time_ms);
  ...
  void AdvanceTimeMilliseconds(int timedelta_ms) {
    current_time_ += TimeDelta::Milliseconds(timedelta_ms);
  }
  ...
  void PacketTransmissionAndFeedbackBlock(int64_t runtime_ms, int64_t delay) {
    ...
    int64_t start_time_ms = current_time_.Milliseconds();
    while (current_time_.Milliseconds() - start_time_ms < runtime_ms) {
      constexpr size_t kPayloadSize = 1000;
      PacketResult packet = CreateResult(
          current_time_.Milliseconds() + delay_buildup,
          current_time_.Milliseconds(), kPayloadSize, PacedPacketInfo());
      delay_buildup += delay;
...
TEST_F(GoogCcNetworkControllerTest, CongestionWindowPushbackOnNetworkDelay) {
  ...
  auto send_net =
      s.CreateMutableSimulationNode([=](NetworkSimulationConfig* c) {
        c->bandwidth = DataRate::KilobitsPerSecond(1000);
        c->delay = TimeDelta::Milliseconds(100);
      });
  auto ret_net = s.CreateSimulationNode([](NetworkSimulationConfig* c) {
    c->delay = TimeDelta::Milliseconds(100);
  });
  CallClientConfig config;
  config.transport.cc_factory = &factory;
  ...
  config.transport.rates.start_rate = DataRate::KilobitsPerSecond(300);
  config.transport.rates.max_rate = DataRate::KilobitsPerSecond(2000);
  config.transport.rates.min_rate = DataRate::KilobitsPerSecond(10);
  ...
  EXPECT_LT(client->target_rate().KilobitsPerSecond(), 40);
}

TEST_F(GoogCcNetworkControllerTest, OnNetworkRouteChanged) {
  NetworkControlUpdate update;
  DataRate new_bitrate = DataRate::BitsPerSecond(200000);
  ...
  const DataRate kDefaultMinBitrate = DataRate::KilobitsPerSecond(5);
  update = controller_->OnNetworkRouteChange(CreateRouteChange());
  EXPECT_EQ(update.target_rate->target_rate, kDefaultMinBitrate);
  EXPECT_NEAR(update.pacer_config->data_rate().BitsPerSecond<double>(),
              kDefaultMinBitrate.BitsPerSecond<double>() * kDefaultPacingRate,
              10);
  EXPECT_EQ(update.probe_cluster_configs.size(), 2u);
}
...
TEST_F(GoogCcNetworkControllerTest,
  ...
  CallClientConfig config;
  // Start high so bandwidth drop has max effect.
  config.transport.rates.start_rate = DataRate::KilobitsPerSecond(1000);
  config.transport.rates.max_rate = DataRate::KilobitsPerSecond(2000);
  ...
  EXPECT_NEAR(client->padding_rate().KilobitsPerSecond(),
              client->target_rate().KilobitsPerSecond(), 1);
...
TEST_F(GoogCcNetworkControllerTest, LimitsToFloorIfRttIsHighInTrial) {
  ...
  // achieve.
  const DataRate kBandwidthFloor = DataRate::KilobitsPerSecond(50);
  ScopedFieldTrials trial("WebRTC-Bwe-MaxRttLimit/limit:2s,floor:" +
                          std::to_string(kBandwidthFloor.KilobitsPerSecond()) +
                          "kbps/");
  ...
  const DataRate kLinkCapacity = DataRate::KilobitsPerSecond(100);
  ...
  EXPECT_NEAR(client->target_rate().KilobitsPerSecond(),
              kBandwidthFloor.KilobitsPerSecond(), 5);
}
...
TEST_F(GoogCcNetworkControllerTest, StableEstimateDoesNotVaryInSteadyState) {
  ...
  net_conf.bandwidth = DataRate::KilobitsPerSecond(500);
  net_conf.delay = TimeDelta::Milliseconds(100);
...
TEST_F(GoogCcNetworkControllerTest,
  ...
  config.transport.rates.start_rate = DataRate::KilobitsPerSecond(300);
  auto send_net = s.CreateSimulationNode([](NetworkSimulationConfig* c) {
    c->bandwidth = DataRate::KilobitsPerSecond(2000);
    c->delay = TimeDelta::Milliseconds(200);
    c->loss_rate = 0.1;
  });
  auto ret_net = s.CreateSimulationNode([](NetworkSimulationConfig* c) {
  ...
  EXPECT_GT(client->target_rate().KilobitsPerSecond(), 100);
}

DataRate AverageBitrateAfterCrossInducedLoss(std::string name) {
  Scenario s(name, false);
  NetworkSimulationConfig net_conf;
  net_conf.bandwidth = DataRate::KilobitsPerSecond(1000);
  net_conf.delay = TimeDelta::Milliseconds(100);
...
TEST_F(GoogCcNetworkControllerTest,
  ...
  RTC_DCHECK_LE(average_bitrate, DataRate::KilobitsPerSecond(650));
}
...
TEST_F(GoogCcNetworkControllerTest,
  ...
  RTC_DCHECK_GE(average_bitrate, DataRate::KilobitsPerSecond(750));
}
...
TEST_F(GoogCcNetworkControllerTest, LossBasedEstimatorCapsRateAtModerateLoss) {
  ...
  CallClientConfig config;
  config.transport.rates.min_rate = DataRate::KilobitsPerSecond(10);
  config.transport.rates.max_rate = DataRate::KilobitsPerSecond(5000);
  config.transport.rates.start_rate = DataRate::KilobitsPerSecond(1000);

  NetworkSimulationConfig network;
  network.bandwidth = DataRate::KilobitsPerSecond(2000);
  network.delay = TimeDelta::Milliseconds(100);
...
TEST_F(GoogCcNetworkControllerTest, MaintainsLowRateInSafeResetTrial) {
  ...
  s.RunFor(TimeDelta::Milliseconds(500));
  EXPECT_NEAR(client->send_bandwidth().KilobitsPerSecond(),
              kLinkCapacity.KilobitsPerSecond(), 50);
  s.ChangeRoute(route->forward(), {send_net});
  // Allow new settings to propagate.
  s.RunFor(TimeDelta::Milliseconds(100));
...
TEST_F(GoogCcNetworkControllerTest, CutsHighRateInSafeResetTrial) {
  const DataRate kLinkCapacity = DataRate::KilobitsPerSecond(1000);
  const DataRate kStartRate = DataRate::KilobitsPerSecond(300);
  ...
  auto send_net = s.CreateSimulationNode([&](NetworkSimulationConfig* c) {
    c->bandwidth = kLinkCapacity;
    c->delay = TimeDelta::Milliseconds(50);
  });
  auto* client = s.CreateClient("send", [&](CallClientConfig* c) {
    c->transport.rates.start_rate = kStartRate;
  });
  ...
  s.RunFor(TimeDelta::Milliseconds(500));
  EXPECT_NEAR(client->send_bandwidth().KilobitsPerSecond(),
              kLinkCapacity.KilobitsPerSecond(), 300);
  s.ChangeRoute(route->forward(), {send_net});
  // Allow new settings to propagate.
  s.RunFor(TimeDelta::Milliseconds(50));
  // Under the trial, the target should be reset from high values.
  EXPECT_NEAR(client->send_bandwidth().KilobitsPerSecond(),
              kStartRate.KilobitsPerSecond(), 30);
}
...
TEST_F(GoogCcNetworkControllerTest, DetectsHighRateInSafeResetTrial) {
  ...
  const DataRate kInitialLinkCapacity = DataRate::KilobitsPerSecond(200);
  const DataRate kNewLinkCapacity = DataRate::KilobitsPerSecond(800);
  const DataRate kStartRate = DataRate::KilobitsPerSecond(300);
  ...
  s.CreateVideoStream(route->forward(), VideoStreamConfig());
  // Allow the controller to stabilize.
  s.RunFor(TimeDelta::Milliseconds(1000));
  EXPECT_NEAR(client->send_bandwidth().KilobitsPerSecond(),
              kInitialLinkCapacity.KilobitsPerSecond(), 50);
  ...
  s.RunFor(TimeDelta::Milliseconds(1000));
  EXPECT_GT(client->send_bandwidth().KilobitsPerSecond(),
            kNewLinkCapacity.KilobitsPerSecond() - 300);
}
...
TEST_F(GoogCcNetworkControllerTest,
  ...
  const DataRate kLinkCapacity = DataRate::KilobitsPerSecond(1000);
  const DataRate kStartRate = DataRate::KilobitsPerSecond(1000);
...
TEST_F(GoogCcNetworkControllerTest, NoBandwidthTogglingInLossControlTrial) {
  ...
  auto* client = s.CreateClient("send", [&](CallClientConfig* c) {
    c->transport.rates.start_rate = DataRate::KilobitsPerSecond(300);
  });
  auto* route = s.CreateRoutes(
      client, {send_net}, s.CreateClient("return", CallClientConfig()),
      {s.CreateSimulationNode(NetworkSimulationConfig())});
  ...
  s.RunFor(TimeDelta::Milliseconds(250));

  std::queue<DataRate> bandwidth_history;
  const TimeDelta step = TimeDelta::Milliseconds(50);
  for (TimeDelta time = TimeDelta::Zero(); time < TimeDelta::Milliseconds(2000);
       time += step) {
    s.RunFor(step);
    const TimeDelta window = TimeDelta::Milliseconds(500);
    if (bandwidth_history.size() >= window / step)
      bandwidth_history.pop();
    bandwidth_history.push(client->send_bandwidth());
    EXPECT_LT(
        CountBandwidthDips(bandwidth_history, DataRate::KilobitsPerSecond(100)),
        2);
  }
}
...
TEST_F(GoogCcNetworkControllerTest, NoRttBackoffCollapseWhenVideoStops) {
  ...
  auto* client = s.CreateClient("send", [&](CallClientConfig* c) {
    c->transport.rates.start_rate = DataRate::KilobitsPerSecond(1000);
  });
  auto* route = s.CreateRoutes(
      client, {send_net}, s.CreateClient("return", CallClientConfig()),
      {s.CreateSimulationNode(NetworkSimulationConfig())});
  ...
  EXPECT_GT(client->send_bandwidth().KilobitsPerSecond(), 1000);
}
...
TEST_F(GoogCcNetworkControllerTest, IsFairToTCP) {
  ...
  NetworkSimulationConfig net_conf;
  net_conf.bandwidth = DataRate::KilobitsPerSecond(1000);
  net_conf.delay = TimeDelta::Milliseconds(50);
  auto* client = s.CreateClient("send", [&](CallClientConfig* c) {
    c->transport.rates.start_rate = DataRate::KilobitsPerSecond(1000);
  });
  auto send_net = {s.CreateSimulationNode(net_conf)};
  ...
  EXPECT_LT(client->send_bandwidth().KilobitsPerSecond(), 750);
}
