...
RtpFrameReferenceFinder::FrameDecision RtpFrameReferenceFinder::ManageFrameVp8(
  ...
  last_picture_id_seq_num_.insert(
      std::make_pair(frame->picture_id, frame->last_seq_num()));
  ...
  if (AheadOf<uint16_t, kPicIdLength>(frame->picture_id, last_picture_id_)) {
    ...
    if (seq_num_it == last_picture_id_seq_num_.end() ||
        seq_num_it->second + 1 != frame->first_seq_num()) {
      do {
  ...
  for (uint8_t layer = 0; layer <= codec_header.temporalIdx; ++layer) {
    ...
    if (not_received_frame_it != not_yet_received_frames_.end() &&
      ...
      uint16_t next_seq_num = seq_num_it->second;
      ...
      if (seq_num_it == last_picture_id_seq_num_.end() ||
          seq_num_it->second + 1 != next_seq_num)
        return kStash;
    }
