...
class MockReferenceVideoSource
    : public VideoCodecAnalyser::ReferenceVideoSource {
 public:
...
TEST(PerfAnalyserScalabilityTest, EncodeFinished_updatesFrameStats) {
  ...
  settings.bitrate_kbps = 1024;
  settings.framerate_fps = 24;
  ...
  EXPECT_EQ(fs.target_bitrate_kbps, 1024u);
  EXPECT_EQ(fs.target_framerate_fps, 24);
}
...
TEST(VideoCodecAnalyserTest, DecodeFinished_computesPsnr) {
  ...
  VideoCodecAnalyser analyser(&reference_video_source);
  analyser.StartDecode(CreateEncodedImage(kTimestamp, kSpatialIdx));
