...
TEST_F(TestVideoReceiver, SetMinReceiverDelay) {
  const int kMinValidReceiverDelayMs = 0;
  const int kMaxValidReceiverDelayMs = 10000;
  EXPECT_EQ(0, receiver_->SetMinReceiverDelay(kMinValidReceiverDelayMs));
  EXPECT_EQ(0, receiver_->SetMinReceiverDelay(kMaxValidReceiverDelayMs));
  EXPECT_EQ(-1, receiver_->SetMinReceiverDelay(kMinValidReceiverDelayMs - 1));
  EXPECT_EQ(-1, receiver_->SetMinReceiverDelay(kMaxValidReceiverDelayMs + 1));
}
