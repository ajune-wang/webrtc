...
TEST_P(PeerConnectionIntegrationTest,
  ...
  webrtc::MockAsyncResolverFactory* caller_async_resolver_factory;
  webrtc::MockAsyncResolverFactory* callee_async_resolver_factory;
  auto caller_resolver_factory =
      absl::make_unique<webrtc::MockAsyncResolverFactory>();
  auto callee_resolver_factory =
      absl::make_unique<webrtc::MockAsyncResolverFactory>();
  caller_async_resolver_factory = caller_resolver_factory.get();
  webrtc::PeerConnectionDependencies caller_deps(nullptr);
  caller_deps.async_resolver_factory = std::move(caller_resolver_factory);
  callee_async_resolver_factory = callee_resolver_factory.get();
  ...
  // P2PTransportChannel.
  EXPECT_CALL(*caller_async_resolver_factory, Create())
      .WillOnce(Return(&caller_async_resolver));
  EXPECT_CALL(*callee_async_resolver_factory, Create())
