...
std::vector<webrtc::test::RateProfile> kBitRateHighLowHigh = {
...
std::vector<webrtc::test::RateProfile> kBitRateLowHighLow = {
...
std::vector<webrtc::test::RateProfile> kFrameRateHighLowHigh = {
...
std::vector<webrtc::test::RateProfile> kFrameRateLowHighLow = {
...
TEST_P(VideoCodecTestMediaCodecRateAdaptation, DISABLED_RateAdaptation) {
  ...
  config.num_frames = rate_profile.back().frame_num +
                      static_cast<size_t>(10 * rate_profile.back().input_fps);
  config.encode_in_real_time = true;
