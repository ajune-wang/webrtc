...
class DirectTransport : public Transport {
 ...
 private:
  struct TokenBase {};
  using Token = rtc::RefCountedObject<TokenBase>;
  ...
  // |process_token_| is shared with currently pending processing task, if this
  // is reset, processing is stopped. It's reset when StopSending or the
  // destructor is called. The fact that this is not a bool checked in a process
  // task means that we can cancel a pending process and immediately start a new
  // one, this allows SendPacket to be called directly after StopSending without
  // any gotchas.
  // TODO(srte): Simplify this when StopSending has been removed.
  rtc::scoped_refptr<Token> process_token_ RTC_GUARDED_BY(&process_lock_);
