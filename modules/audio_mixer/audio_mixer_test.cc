...
class FilePlayingSource : public AudioMixer::Source {
 public:
  ...
  AudioFrameInfo GetAudioFrameWithInfo(int target_rate_hz,
    ...
    file_is_playing_ = num_to_read == num_read;
    if (!file_is_playing_) {
      frame->Mute();
    }
    return file_is_playing_ ? AudioFrameInfo::kNormal : AudioFrameInfo::kMuted;
  }
  ...
  bool FileIsPlaying() const { return file_is_playing_; }
...
int main(int argc, char* argv[]) {
  ...
  std::vector<webrtc::FilePlayingSource> sources;
  ...
  while (!all_streams_finished) {
    ...
    bool some_playing_this_round = false;
    for (const auto& source : sources) {
      some_playing_this_round |= source.FileIsPlaying();
    }
    all_streams_finished = !some_playing_this_round;
  }
