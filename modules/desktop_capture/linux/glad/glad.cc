...
/*
    ...
    OpenGL loader generated by glad 0.1.34 on Mon Sep  6 06:05:36 2021.
...
#include "modules/desktop_capture/linux/glad/glad.h"
...
static void* get_proc(const char* namez) {
  ...
  if (result == NULL) {
#if defined(_WIN32) || defined(__CYGWIN__)
    result = reinterpret_cast<void*>(GetProcAddress((HMODULE)libGL, namez));
#else
...
static int get_exts(void) {
  ...
  if (max_loaded_major < 3) {
    ...
    if (num_exts_i > 0) {
      exts_i = reinterpret_cast<char**>(
          malloc((size_t)num_exts_i * (sizeof *exts_i)));
    }
    ...
    for (index = 0; index < (unsigned)num_exts_i; index++) {
      ...
      char* local_str =
          reinterpret_cast<char*>(malloc((len + 1) * sizeof(char)));
      if (local_str != NULL) {
...
static void free_exts(void) {
  if (exts_i != NULL) {
    ...
    for (index = 0; index < num_exts_i; index++) {
      free(reinterpret_cast<char*>(exts_i[index]));
    }
    free(reinterpret_cast<void*>(exts_i));
    exts_i = NULL;
  }
}
