...
void WebRtcVideoChannel::BackfillBufferedPackets(
  ...
  if (unknown_ssrc_packet_buffer_) {
    ...
    unknown_ssrc_packet_buffer_->BackfillPackets(
        ssrcs, [&receiver](uint32_t ssrc, int64_t packet_time_us,
                           rtc::CopyOnWriteBuffer packet) {
          receiver->DeliverPacket(webrtc::MediaType::VIDEO, packet,
                                  packet_time_us);
        });
  }
}
...
void WebRtcVideoChannel::WebRtcVideoReceiveStream::RecreateWebRtcVideoStream() {
  ...
  if (webrtc::field_trial::IsEnabled(
          "WebRTC-Video-DelayRecreateWebRtcStreamWoFrameDecryptor")) {
    if (stream_ == nullptr && config_.frame_decryptor == nullptr) {
      RTC_LOG(LS_INFO) << "Delay creation VideoReceiveStream, ssrc= "
                       << stream_params_.ssrcs[0]
                       << " as frame_decryptor is null";
      return;
    }
  }

  const bool first_create = stream_ == nullptr;
  if (stream_) {
