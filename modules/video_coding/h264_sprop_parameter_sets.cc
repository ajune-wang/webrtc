...
bool H264SpropParameterSets::DecodeSprop(const std::string& sprop) {
  ...
  NLOG(LS_INFO, "Parsing sprop \"", sprop, "\"");
  if ((separator_pos <= 0) || (separator_pos >= sprop.length() - 1)) {
    NLOG(LS_WARNING, "Invalid seperator position ", separator_pos, " *", sprop,
         "*");
    return false;
  }
  ...
  if (!DecodeAndConvert(sps_str, &sps_)) {
    NLOG(LS_WARNING, "Failed to decode sprop/sps *", sprop, "*");
    return false;
  }
  if (!DecodeAndConvert(pps_str, &pps_)) {
    NLOG(LS_WARNING, "Failed to decode sprop/pps *", sprop, "*");
    return false;
  }
  return true;
}
