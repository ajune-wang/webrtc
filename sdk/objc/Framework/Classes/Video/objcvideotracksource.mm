...
void ObjcVideoTrackSource::OnCapturedFrame(RTCVideoFrame* frame) {
  ...
  /*if (adapted_width == frame.width && adapted_height == frame.height) {
    ...
    if (frame.depth_buffer==nil) { // truncate alpha channel
        rtc::scoped_refptr<I420Buffer> i420_buffer = I420Buffer::Create(adapted_width, adapted_height);
        buffer = new rtc::RefCountedObject<ObjCFrameBuffer>(frame.buffer);
        i420_buffer->CropAndScaleFrom(*buffer->ToI420(), crop_x, crop_y, crop_width, crop_height);
        buffer = i420_buffer;

    } else {//crop_width=640;crop_height=480;
      ...
      rtc::scoped_refptr<I420BufferInterface> depth_i420 = depth_buffer->ToI420();
        crop_x = crop_x * depth_buffer->width() / buffer->width();
        crop_y = crop_y * depth_buffer->height() / buffer->height();
        crop_width = crop_width * depth_buffer->width() / buffer->width();
        crop_height = crop_height * depth_buffer->height() / buffer->height();
