...
FramePairCorruptionScore::FramePairCorruptionScore(
...
double FramePairCorruptionScore::CalculateScore(
    int qp,
    I420BufferInterface& raw_frame,
    I420BufferInterface& compressed_frame) {
  RTC_CHECK_GE(raw_frame.width(), compressed_frame.width());
  RTC_CHECK_GE(raw_frame.height(), compressed_frame.height());
  ...
  const int num_samples = static_cast<int>(
      (compressed_frame.width() * compressed_frame.height() * 3 / 2) *
      sample_fraction_);
  ...
  webrtc::scoped_refptr<I420Buffer> raw_i420_buffer =
      GetI420Buffers(raw_frame.ToI420());
  webrtc::scoped_refptr<I420Buffer> compressed_i420_buffer =
      GetI420Buffers(compressed_frame.ToI420());

  FilterSettings filter_settings = GetCorruptionFilterSettings(qp, codec_type_);

  const std::vector<FilteredSample> filtered_original_sample_values =
      GetSampleValuesForFrame(
          raw_i420_buffer, halton_samples, compressed_i420_buffer->width(),
          compressed_i420_buffer->height(), filter_settings.std_dev);
  const std::vector<FilteredSample> filtered_corrupted_sample_values =
      GetSampleValuesForFrame(compressed_i420_buffer, halton_samples,
                              compressed_i420_buffer->width(),
                              compressed_i420_buffer->height(),
                              filter_settings.std_dev);
  RTC_CHECK_EQ(filtered_original_sample_values.size(),
               filtered_corrupted_sample_values.size());

  return corruption_classifier_.CalculateCorruptionProbability(
      filtered_original_sample_values, filtered_corrupted_sample_values,
