...
class TesterDecoder {
        ...
        [this, input_frame] {
          ...
          decoder_->Decode(
              ...
              [this, spatial_idx = input_frame.SpatialIndex().value_or(0)](
                ...
                if (decoder_output_writer_) {
                  decoder_output_writer_->Write(output_frame, spatial_idx);
                }
              });

          if (decoder_input_writer_) {
            decoder_input_writer_->Write(input_frame);
          }
        },
 ...
 protected:
  ...
  std::unique_ptr<TesterIvfWriter> decoder_input_writer_;
  std::unique_ptr<TesterY4mWriter> decoder_output_writer_;
};
