...
PeerScenarioClient::PeerScenarioClient(NetworkEmulationManager* net,
  ...
  media_deps.adm = TestAudioDeviceModule::Create(
      task_queue_factory_,
      TestAudioDeviceModule::CreatePulsedNoiseCapturer(25000, 48000),
  ...
  peer_connection_ =
      pc_factory_->CreatePeerConnection(rtc_configuration, std::move(pc_deps));
}

EmulatedEndpoint* PeerScenarioClient::ep(int index) {
...
PeerScenarioClient::AudioSendTrack PeerScenarioClient::CreateAudio(
    std::string track_id,
    AudioSendTrackConfig config) {
  AudioSendTrack res;
  auto source = pc_factory_->CreateAudioSource(config.options);
