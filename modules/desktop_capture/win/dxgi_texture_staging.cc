...
bool DxgiTextureStaging::InitializeStage(ID3D11Texture2D* texture) {
  ...
  if (error.Error() != S_OK || !stage_) {
    NLOG(LS_ERROR, "Failed to create a new ID3D11Texture2D as stage, error ",
  ...
  if (error.Error() != S_OK || !surface_) {
    NLOG(LS_ERROR, "Failed to convert ID3D11Texture2D to IDXGISurface, error ",
         error.ErrorMessage(), ", code ", error.Error());
    return false;
  }
...
bool DxgiTextureStaging::CopyFromTexture(
  ...
  *rect() = {0};
  ...
  if (error.Error() != S_OK) {
    *rect() = {0};
    NLOG(LS_ERROR, "Failed to map the IDXGISurface to a bitmap, error ",
         error.ErrorMessage(), ", code ", error.Error());
    return false;
  }
