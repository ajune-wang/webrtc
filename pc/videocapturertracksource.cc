...
bool NewFormatWithConstraints(
  ...
  *format_out = format_in;
  ...
  NLOG(LS_WARNING,
       "Found unknown MediaStream constraint. Name:", constraint.key,
...
void VideoCapturerTrackSource::Initialize(
  ...
  if (formats.size() == 0) {
    NLOG(LS_WARNING, "Failed to find a suitable video format.");
    SetState(kEnded);
    return;
  }
  ...
  if (!ExtractOption(constraints, MediaConstraintsInterface::kNoiseReduction,
                     &needs_denoising_)) {
    NLOG(LS_WARNING, "Invalid mandatory value for",
