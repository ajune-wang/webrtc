...
class TransformableAudioFrame : public TransformableAudioFrameInterface {
 public:
  ...
  const uint8_t* GetPacket() const { return payload_.data(); }
  size_t GetPacketLength() const { return payload_.size(); }
...
void ChannelReceiveFrameTransformerDelegate::Transform(const uint8_t* packet,
...
void ChannelReceiveFrameTransformerDelegate::ReceiveFrame(
  ...
  receive_frame_callback_(transformed_frame->GetPacket(),
                          transformed_frame->GetPacketLength(),
