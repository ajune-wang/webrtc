...
std::unique_ptr<AudioEncoderOpusStates> CreateCodec(int sample_rate_hz,
  ...
  states->fake_clock->SetTime(Timestamp::Microseconds(kInitialTimeUs));
...
void TestSetPacketLossRate(const AudioEncoderOpusStates* states,
  ...
  for (float loss : losses) {
    ...
    states->fake_clock->AdvanceTime(TimeDelta::Milliseconds(kSampleIntervalMs));
...
TEST_P(AudioEncoderOpusTest,
  ...
  states->fake_clock->AdvanceTime(TimeDelta::Milliseconds(kSecondSampleTimeMs));
...
TEST_P(AudioEncoderOpusTest, UpdateUplinkBandwidthInAudioNetworkAdaptor) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    states->fake_clock->AdvanceTime(TimeDelta::Milliseconds(
    ...
    states->fake_clock->AdvanceTime(TimeDelta::Milliseconds(1));
    states->encoder->Encode(
        0, rtc::ArrayView<const int16_t>(audio.data(), audio.size()), &encoded);
  }
}
