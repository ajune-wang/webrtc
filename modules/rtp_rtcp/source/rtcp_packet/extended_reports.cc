...
bool ExtendedReports::Parse(const CommonHeader& packet) {
  ...
  if (packet.payload_size_bytes() < kXrBaseLength) {
    NLOG(LS_WARNING, "Packet is too small to be an ExtendedReports packet.");
    return false;
  }
  ...
  while (current_block + kBlockHeaderSizeBytes <= packet_end) {
    ...
    if (next_block > packet_end) {
      NLOG(LS_WARNING, "Report block in extended report packet is too big.");
      return false;
    }
    switch (block_type) {
      ...
      default:
        ...
        NLOG(LS_WARNING, "Unknown extended report block type ", block_type);
        break;
    }
...
void ExtendedReports::SetRrtr(const Rrtr& rrtr) {
  if (rrtr_block_)
    NLOG(LS_WARNING, "Rrtr already set, overwriting.");
...
void ExtendedReports::SetVoipMetric(const VoipMetric& voip_metric) {
  if (voip_metric_block_)
    NLOG(LS_WARNING, "Voip metric already set, overwriting.");
  voip_metric_block_.emplace(voip_metric);
}

void ExtendedReports::SetTargetBitrate(const TargetBitrate& bitrate) {
  if (target_bitrate_)
    NLOG(LS_WARNING, "TargetBitrate already set, overwriting.");
...
void ExtendedReports::ParseRrtrBlock(const uint8_t* block,
  ...
  if (block_length != Rrtr::kBlockLength) {
    NLOG(LS_WARNING, "Incorrect rrtr block size ", block_length, " Should be ",
         Rrtr::kBlockLength);
    return;
  }
  if (rrtr_block_) {
    NLOG(LS_WARNING, "Two rrtr blocks found in same Extended Report packet");
    return;
  }
  rrtr_block_.emplace();
...
void ExtendedReports::ParseDlrrBlock(const uint8_t* block,
                                     uint16_t block_length) {
  if (dlrr_block_) {
    NLOG(LS_WARNING, "Two Dlrr blocks found in same Extended Report packet");
    return;
  }
...
void ExtendedReports::ParseVoipMetricBlock(const uint8_t* block,
  ...
  if (block_length != VoipMetric::kBlockLength) {
    NLOG(LS_WARNING, "Incorrect voip metric block size ", block_length,
