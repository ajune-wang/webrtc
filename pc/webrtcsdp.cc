...
static void RemoveInvalidRidDescriptions(const std::vector<int>& payload_types,
  ...
  for (RidDescription& rid : *rids) {
    ...
    for (int format : rid.formats) {
      if (std::find(payload_types.begin(), payload_types.end(), format) ==
          payload_types.end()) {
        removed_formats.insert(format);
      }
    }

    rid.formats.erase(std::remove_if(rid.formats.begin(), rid.formats.end(),
...
static SimulcastLayerList RemoveRidsFromSimulcastLayerList(
  ...
  for (auto listIter = layers.begin(); listIter != layers.end(); listIter++) {
    std::vector<SimulcastLayer> new_layers;
    for (auto it = listIter->begin(); it != listIter->end(); it++) {
      if (to_remove.find(it->rid) == to_remove.end()) {
...
bool ParseContent(const std::string& message,
  ...
  if (!receive_rids.empty()) {
    ...
    receive_track.rids() = receive_rids;
