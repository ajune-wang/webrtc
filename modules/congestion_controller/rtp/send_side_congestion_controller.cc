...
void SendSideCongestionController::MaybeCreateControllers() {
  ...
  {
    rtc::CritScope cs(&control_handler_ptr_critical_section_);
    control_handler_ptr_ = control_handler_.get();
  }
  controller_ =
...
bool SendSideCongestionController::AvailableBandwidth(
  ...
  if (control_handler_ptr_->last_transfer_rate().has_value()) {
    *bandwidth = control_handler_ptr_->last_transfer_rate()
