...
void SctpTransport::UpdateInformation(SctpTransportState state) {
  ...
  // TODO(hta): Update max message size and max channels from internal SCTP
  // transport when available.
  info_ = SctpTransportInformation(state, dtls_transport_,
                                   info_.MaxMessageSize(), info_.MaxChannels());
