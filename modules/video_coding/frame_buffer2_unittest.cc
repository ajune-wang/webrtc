...
class TestFrameBuffer2 : public ::testing::Test {
 protected:
  ...
  int InsertFrame(uint16_t picture_id,
    ...
    frame->key.picture_id = picture_id;
    frame->key.spatial_layer = spatial_layer;
  ...
  void CheckFrame(size_t index, int picture_id, int spatial_layer) {
    ...
    ASSERT_EQ(picture_id, frames_[index]->key.picture_id);
    ASSERT_EQ(spatial_layer, frames_[index]->key.spatial_layer);
  }
...
TEST_F(TestFrameBuffer2, StatsCallback) {
...

  {
    ...
    frame->key.picture_id = pid;
    frame->key.spatial_layer = 0;
    frame->timestamp = ts;
