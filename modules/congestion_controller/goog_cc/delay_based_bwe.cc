...
size_t ReadTrendlineFilterWindowSize(const WebRtcConfig* webrtc_config) {
  std::string experiment_string =
      webrtc_config->Lookup(kBweWindowSizeInPacketsExperiment);
  size_t window_size;
  int parsed_values =
...
DelayBasedBwe::DelayBasedBwe(const WebRtcConfig* webrtc_config,
    ...
    : event_log_(event_log),
      ...
      trendline_window_size_(
          webrtc_config->Lookup(kBweWindowSizeInPacketsExperiment) == "Enabled"
      ...
      alr_limited_backoff_enabled_(
          webrtc_config->Lookup("WebRTC-Bwe-AlrLimitedBackoff") == "Enabled") {
  RTC_LOG(LS_INFO)
