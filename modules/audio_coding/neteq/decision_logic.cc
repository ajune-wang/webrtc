...
bool DecisionLogic::PostponeDecode(NetEqController::NetEqStatus status) const {
  ...
  // Only postpone if the mute factor is low enough (otherwise the expansion was
  // short enough to not be noticable). Note that the MuteFactor is in Q14, so a
  // value of 16384 corresponds to 1.
  if (IsExpand(status.last_mode) && status.expand_mutefactor < 16384 / 2) {
    return !status.packet_buffer_info.dtx_or_cng ||
