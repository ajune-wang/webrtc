...
class BitstreamReader {
 public:
  ...
  bool IfNextBoolean(
      ...
      std::function<bool()> f_else = [] { return true; }) {
    uint32_t val;
  ...
  // Reads a bit from the input stream, and passes it as a boolean to the
  // provided function. Returns false if bit cannot be read.
  bool ReadBoolean(std::function<void(bool)> f) {
    uint32_t val;
    if (!buffer_->ReadBits(&val, 1)) {
      return false;
    }
  ...
  bool VerifyNextBooleanIs(bool expected_val, const char* error_msg = nullptr) {
    uint32_t val;
    if (!buffer_->ReadBits(&val, 1)) {
      return false;
    }
  ...
  // integer that gets cast to the type T before being passed to f().
  // If number of bits matches size of data type, the bits parameter may be
  // omitted. Ex:
  //  ReadUnsigned<uint8_t>([](uint8_t val) { /* use two lsb of val */ }, 2);
  //  ReadUnsigned<uint8_t>([](uint8_t val) { /* use all 8 bits of val */ });
  template <typename T>
  bool ReadUnsigned(std::function<void(T)> f, int bits = sizeof(T) * 8) {
  ...
  template <typename T>
  bool ReadSigned(std::function<void(T)> f, int bits = sizeof(T) * 8) {
    uint32_t sign;
...
bool Vp9ReadColorConfig(BitstreamReader* br, FrameInfo* frame_info) {
  if (frame_info->profile == 2 || frame_info->profile == 3) {
    RETURN_FALSE_IF_ERROR(
        br->ReadBoolean([frame_info](bool ten_or_twelve_bits) {
  ...
  RETURN_FALSE_IF_ERROR(br->ReadUnsigned<uint8_t>(
      [frame_info](uint8_t color_space) {
        frame_info->color_space = static_cast<ColorSpace>(color_space);
      },
      3));

  if (frame_info->color_space != ColorSpace::CS_RGB) {
    RETURN_FALSE_IF_ERROR(br->ReadBoolean([frame_info](bool color_range) {
    ...
    if (frame_info->profile == 1 || frame_info->profile == 3) {
      ...
      RETURN_FALSE_IF_ERROR(br->VerifyNextBooleanIs(
...
bool Vp9ReadFrameSize(BitstreamReader* br, FrameInfo* frame_info) {
  ...
  RETURN_FALSE_IF_ERROR(br->ReadUnsigned<uint16_t>(
      [frame_info](uint16_t width) { frame_info->frame_width = width + 1; }));
  RETURN_FALSE_IF_ERROR(br->ReadUnsigned<uint16_t>(
      [frame_info](uint16_t height) { frame_info->frame_width = height + 1; }));
  return true;
}
...
bool Vp9ReadRenderSize(BitstreamReader* br, FrameInfo* frame_info) {
  ...
  return br->IfNextBoolean(
      [&] {
        ...
        RETURN_FALSE_IF_ERROR(
            br->ReadUnsigned<uint16_t>([frame_info](uint16_t width) {
              frame_info->render_width = width + 1;
            }));
        RETURN_FALSE_IF_ERROR(
            br->ReadUnsigned<uint16_t>([frame_info](uint16_t height) {
...
bool Vp9ReadFrameSizeFromRefs(BitstreamReader* br, FrameInfo* frame_info) {
  ...
  for (size_t i = 0; !found_ref && i < kVp9NumRefsPerFrame; i++) {
    // Size in refs.
    RETURN_FALSE_IF_ERROR(br->ReadBoolean([&](bool ref) { found_ref = ref; }));
  }
  ...
  return br->IfNextBoolean([&] {    // if mode_ref_delta_enabled
    return br->IfNextBoolean([&] {  // if mode_ref_delta_update
      for (size_t i = 0; i < kVp9MaxRefLFDeltas; i++) {
        RETURN_FALSE_IF_ERROR(
            br->IfNextBoolean([&] { return br->ConsumeBits(7); }));
      }
...
bool Vp9ReadQp(BitstreamReader* br, FrameInfo* frame_info) {
  ...
  // yuv offsets
  for (int i = 0; i < 3; ++i) {
    RETURN_FALSE_IF_ERROR(br->IfNextBoolean([br] {  // if delta_coded
...
bool Vp9ReadSegmentationParams(BitstreamReader* br) {
  ...
  return br->IfNextBoolean([&] {    // segmentation_enabled
    return br->IfNextBoolean([&] {  // update_map
      ...
      for (int i = 0; i < 7; ++i) {
        RETURN_FALSE_IF_ERROR(
            br->IfNextBoolean([br] { return br->ConsumeBits(8); }));
      }
  ...
  return br->IfNextBoolean([&] {
    RETURN_FALSE_IF_ERROR(br->ConsumeBits(1));  // abs_or_delta
    for (int i = 0; i < kVp9MaxSegments; ++i) {
      for (int j = 0; j < kVp9SegLvlMax; ++j) {
        RETURN_FALSE_IF_ERROR(br->IfNextBoolean([&] {  // feature_enabled
...
bool Vp9ReadTileInfo(BitstreamReader* br, FrameInfo* frame_info) {
  ...
  while (!done && cols_log2 < max_log2) {
    RETURN_FALSE_IF_ERROR(br->IfNextBoolean(
        [&] {
          ++cols_log2;
          return true;
        },
...
bool Parse(const uint8_t* buf, size_t length, FrameInfo* frame_info) {
  ...
  // Frame marker.
  RETURN_FALSE_IF_ERROR(br.VerifyNextUnsignedIs(
      2, 0x2, "Failed to parse header. Frame marker should be 2."));

  // Profile.
  RETURN_FALSE_IF_ERROR(br.ReadUnsigned<uint8_t>(
      [frame_info](uint8_t p) { frame_info->profile = p; }, 2));
  RETURN_FALSE_IF_ERROR(br.VerifyNextBooleanIs(
  ...
  RETURN_FALSE_IF_ERROR(br.ReadBoolean([frame_info](bool frame_type) {
    // Frame type: KEY_FRAME(0), INTER_FRAME(1).
    frame_info->is_keyframe = frame_type == 0;
  }));
  RETURN_FALSE_IF_ERROR(br.ReadBoolean(
      [frame_info](bool show_frame) { frame_info->show_frame = show_frame; }));
  RETURN_FALSE_IF_ERROR(br.ReadBoolean([frame_info](bool error_resilient) {
  ...
  if (frame_info->is_keyframe) {
    ...
    if (!frame_info->show_frame) {
      RETURN_FALSE_IF_ERROR(
          br.ReadBoolean([&](bool intra_only) { is_intra_only = intra_only; }));
    }
    ...
    if (is_intra_only) {
      ...
      RETURN_FALSE_IF_ERROR(br.IfNextBoolean(
          [] { return true; }, [&br] { return br.ConsumeBits(2); }));
    }
  }
  ...
  // Read base QP.
  RETURN_FALSE_IF_ERROR(Vp9ReadQp(&br, frame_info));

  const bool kParseFullHeader = false;
  if (kParseFullHeader) {
    // Currently not used, but will be needed when parsing beyond the
    // uncompressed header.
    RETURN_FALSE_IF_ERROR(Vp9ReadSegmentationParams(&br));

    RETURN_FALSE_IF_ERROR(Vp9ReadTileInfo(&br, frame_info));

    RETURN_FALSE_IF_ERROR(br.ConsumeBits(16));  // header_size_in_bytes
  }
