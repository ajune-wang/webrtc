...
RTCError PeerConnection::Initialize(
  ...
  sdp_handler_ = SdpOfferAnswerHandler::Create(
      this, configuration, dependencies, context_, trials());
...
bool PeerConnection::StartRtcEventLog(
  ...
  if (absl::StartsWith(trials().Lookup("WebRTC-RtcEventLogNewFormat"),
                       "Enabled")) {
...
PeerConnection::InitializePortAllocator_n(
  ...
  } else if (absl::StartsWith(trials().Lookup("WebRTC-IPv6Default"),
                              "Disabled")) {
    port_allocator_flags &= ~(cricket::PORTALLOCATOR_ENABLE_IPV6);
  }
