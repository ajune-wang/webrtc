...
void LossNotificationController::OnReceivedPacket(
  ...
  if (first_in_frame) {
    ...
    if (first_in_frame->frame_id <= last_received_frame_id_) {
      RTC_LOG(LS_WARNING) << "Repeated or reordered frame ID ("
                          << first_in_frame->frame_id << ").";
      return;
    }

    last_received_frame_id_ = first_in_frame->frame_id;

    if (first_in_frame->is_keyframe) {
      ...
      const bool all_dependencies_decodable =
          AllDependenciesDecodable(first_in_frame->ref_frame_ids);
...
bool LossNotificationController::AllDependenciesDecodable(
  ...
  for (int64_t ref_frame_id : ref_frame_ids) {
