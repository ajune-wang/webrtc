...
absl::optional<double> EncoderOvershootDetector::GetUtilizationFactor(
  ...
  if (overshoot_rate_bps) {
    double target_bps = target_bitrate.bps<double>();
    double overshoot_bps = overshoot_rate_bps.value();

    return 1.0 + overshoot_bps / target_bps;
  }
...
int64_t EncoderOvershootDetector::IdealFrameSizeBits() const {
  ...
  return (target_bitrate_updates_.back().bitrate.bps() +
...
DataRate EncoderOvershootDetector::GetAverageTargetBitrate(int64_t time_ms) {
  ...
  if ((time_ms - first.update_time_ms) > window_size_ms_) {
    ...
    // Only remove the clipped part from the sum if this update is actually
    // included in the sum. If it is the only item in the list it will be both
    // first and last and thus not included.
    if (target_bitrate_updates_.size() > 1) {
      target_bitsum_x1000_ -=
  ...
  const BitrateUpdate& last = target_bitrate_updates_.back();
  const int64_t window_size_ms =
      std::min(window_size_ms_, time_ms - first.update_time_ms);
  const int64_t last_bit_sum =
      (time_ms - last.update_time_ms) * last.bitrate.bps();
  return DataRate::bps((target_bitsum_x1000_ + last_bit_sum) / window_size_ms);
}
