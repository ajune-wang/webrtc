...
ParsedRtcEventLog2::EventType GetRuntimeEventType(
    rtclog::Event::EventType event_type) {
  switch (event_type) {
    case rtclog::Event::UNKNOWN_EVENT:
      return ParsedRtcEventLog2::EventType::UNKNOWN_EVENT;
    case rtclog::Event::LOG_START:
      return ParsedRtcEventLog2::EventType::LOG_START;
    case rtclog::Event::LOG_END:
      return ParsedRtcEventLog2::EventType::LOG_END;
    case rtclog::Event::RTP_EVENT:
      return ParsedRtcEventLog2::EventType::RTP_EVENT;
    case rtclog::Event::RTCP_EVENT:
      return ParsedRtcEventLog2::EventType::RTCP_EVENT;
    case rtclog::Event::AUDIO_PLAYOUT_EVENT:
      return ParsedRtcEventLog2::EventType::AUDIO_PLAYOUT_EVENT;
    case rtclog::Event::LOSS_BASED_BWE_UPDATE:
      return ParsedRtcEventLog2::EventType::LOSS_BASED_BWE_UPDATE;
    case rtclog::Event::DELAY_BASED_BWE_UPDATE:
      return ParsedRtcEventLog2::EventType::DELAY_BASED_BWE_UPDATE;
    case rtclog::Event::VIDEO_RECEIVER_CONFIG_EVENT:
      return ParsedRtcEventLog2::EventType::VIDEO_RECEIVER_CONFIG_EVENT;
    case rtclog::Event::VIDEO_SENDER_CONFIG_EVENT:
      return ParsedRtcEventLog2::EventType::VIDEO_SENDER_CONFIG_EVENT;
    case rtclog::Event::AUDIO_RECEIVER_CONFIG_EVENT:
      return ParsedRtcEventLog2::EventType::AUDIO_RECEIVER_CONFIG_EVENT;
    case rtclog::Event::AUDIO_SENDER_CONFIG_EVENT:
      return ParsedRtcEventLog2::EventType::AUDIO_SENDER_CONFIG_EVENT;
    case rtclog::Event::AUDIO_NETWORK_ADAPTATION_EVENT:
      return ParsedRtcEventLog2::EventType::AUDIO_NETWORK_ADAPTATION_EVENT;
    case rtclog::Event::BWE_PROBE_CLUSTER_CREATED_EVENT:
      return ParsedRtcEventLog2::EventType::BWE_PROBE_CLUSTER_CREATED_EVENT;
    case rtclog::Event::BWE_PROBE_RESULT_EVENT:
      return ParsedRtcEventLog2::EventType::BWE_PROBE_RESULT_EVENT;
    case rtclog::Event::ALR_STATE_EVENT:
      return ParsedRtcEventLog2::EventType::ALR_STATE_EVENT;
    case rtclog::Event::ICE_CANDIDATE_PAIR_CONFIG:
      return ParsedRtcEventLog2::EventType::ICE_CANDIDATE_PAIR_CONFIG;
    case rtclog::Event::ICE_CANDIDATE_PAIR_EVENT:
      return ParsedRtcEventLog2::EventType::ICE_CANDIDATE_PAIR_EVENT;
  }
  return ParsedRtcEventLog2::EventType::UNKNOWN_EVENT;
}
...
ParsedRtcEventLog2::ParsedRtcEventLog2(
...
bool ParsedRtcEventLog2::ParseFile(const std::string& filename) {
...
void ParsedRtcEventLog2::StoreParsedEvent(const rtclog::Event& event) {
  ...
  switch (event.type()) {
    ...
    case ParsedRtcEventLog2::LOSS_BASED_BWE_UPDATE: {
      bwe_loss_updates_.push_back(GetLossBasedBweUpdate(event));
      break;
    }
    case ParsedRtcEventLog2::DELAY_BASED_BWE_UPDATE: {
      bwe_delay_updates_.push_back(GetDelayBasedBweUpdate(event));
      break;
    }
    case ParsedRtcEventLog2::AUDIO_NETWORK_ADAPTATION_EVENT: {
      LoggedAudioNetworkAdaptationEvent ana_event =
          GetAudioNetworkAdaptation(event);
      audio_network_adaptation_events_.push_back(ana_event);
      break;
    }
    case ParsedRtcEventLog2::BWE_PROBE_CLUSTER_CREATED_EVENT: {
      bwe_probe_cluster_created_events_.push_back(
          GetBweProbeClusterCreated(event));
      break;
    }
    case ParsedRtcEventLog2::BWE_PROBE_RESULT_EVENT: {
      bwe_probe_result_events_.push_back(GetBweProbeResult(event));
      break;
    }
    case ParsedRtcEventLog2::ALR_STATE_EVENT: {
      alr_state_events_.push_back(GetAlrState(event));
      break;
    }
    case ParsedRtcEventLog2::ICE_CANDIDATE_PAIR_CONFIG: {
      ice_candidate_pair_configs_.push_back(GetIceCandidatePairConfig(event));
      break;
    }
    case ParsedRtcEventLog2::ICE_CANDIDATE_PAIR_EVENT: {
      ice_candidate_pair_events_.push_back(GetIceCandidatePairEvent(event));
      break;
    }
    case ParsedRtcEventLog2::UNKNOWN_EVENT: {
      break;
    }
  }
}

size_t ParsedRtcEventLog2::GetNumberOfEvents() const {
  return events_.size();
}

int64_t ParsedRtcEventLog2::GetTimestamp(size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
  const rtclog::Event& event = events_[index];
  return GetTimestamp(event);
}

int64_t ParsedRtcEventLog2::GetTimestamp(const rtclog::Event& event) const {
  RTC_CHECK(event.has_timestamp_us());
  return event.timestamp_us();
}

ParsedRtcEventLog2::EventType ParsedRtcEventLog2::GetEventType(
    size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
  const rtclog::Event& event = events_[index];
...
const webrtc::RtpHeaderExtensionMap* ParsedRtcEventLog2::GetRtpHeader(
    size_t index,
...
void ParsedRtcEventLog2::GetRtcpPacket(size_t index,
                                       PacketDirection* incoming,
                                       uint8_t* packet,
                                       size_t* length) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
...
void ParsedRtcEventLog2::GetRtcpPacket(const rtclog::Event& event,
                                       PacketDirection* incoming,
                                       uint8_t* packet,
                                       size_t* length) const {
  RTC_CHECK(event.has_type());
...
rtclog::StreamConfig ParsedRtcEventLog2::GetVideoReceiveConfig(
    size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
...
std::vector<rtclog::StreamConfig> ParsedRtcEventLog2::GetVideoSendConfig(
    size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
...
rtclog::StreamConfig ParsedRtcEventLog2::GetAudioReceiveConfig(
    size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
...
rtclog::StreamConfig ParsedRtcEventLog2::GetAudioSendConfig(
    size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
...
LoggedAudioPlayoutEvent ParsedRtcEventLog2::GetAudioPlayout(
    size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
  const rtclog::Event& event = events_[index];
...
LoggedBweLossBasedUpdate ParsedRtcEventLog2::GetLossBasedBweUpdate(
    size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
  const rtclog::Event& event = events_[index];
...
LoggedBweDelayBasedUpdate ParsedRtcEventLog2::GetDelayBasedBweUpdate(
    size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
  const rtclog::Event& event = events_[index];
...
LoggedAudioNetworkAdaptationEvent ParsedRtcEventLog2::GetAudioNetworkAdaptation(
    size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
  const rtclog::Event& event = events_[index];
...
LoggedBweProbeClusterCreatedEvent ParsedRtcEventLog2::GetBweProbeClusterCreated(
    size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
  const rtclog::Event& event = events_[index];
...
LoggedBweProbeResultEvent ParsedRtcEventLog2::GetBweProbeResult(
    size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
  const rtclog::Event& event = events_[index];
...
LoggedAlrStateEvent ParsedRtcEventLog2::GetAlrState(size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
  const rtclog::Event& event = events_[index];
  return GetAlrState(event);
}

LoggedAlrStateEvent ParsedRtcEventLog2::GetAlrState(
    const rtclog::Event& event) const {
  RTC_CHECK(event.has_type());
...
LoggedIceCandidatePairConfig ParsedRtcEventLog2::GetIceCandidatePairConfig(
    size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
...
LoggedIceCandidatePairEvent ParsedRtcEventLog2::GetIceCandidatePairEvent(
    size_t index) const {
  RTC_CHECK_LT(index, GetNumberOfEvents());
  const rtclog::Event& rtc_event = events_[index];
...
ParsedRtcEventLog2::MediaType ParsedRtcEventLog2::GetMediaType(
    uint32_t ssrc,
