...
bool IsDtlsHandshakePacket(rtc::ArrayView<const uint8_t> payload) {
  ...
  // Maybe BoringSSL provides a helper for parsing the individual
  // elements? But this is ok-ish.
  return payload.size() > 17 &&
         (u[0] == kDtlsHandshakeRecord || u[0] == kDtlsChangeCipherSpecRecord);
}
