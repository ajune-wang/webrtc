...
void ResidualEchoEstimator::Estimate(
  ...
  if (aec_state.UsableLinearEstimate()) {
    ...
    size_t window_start = std::max(0, aec_state.FilterDelayBlocks() - static_cast<int>(config_.echo_model.render_pre_window_size));
    size_t window_end = aec_state.FilterDelayBlocks() + static_cast<int>(config_.echo_model.render_post_window_size);
    ...
    std::transform(
        ...
        [&](float a, float b) { return std::max(0.f, a - config_.echo_model.stationary_gate_slope * b); });
...
void ResidualEchoEstimator::NonLinearEstimate(
  ...
  for (size_t k = 0; k < R2->size(); ++k) {
    ...
    (*R2)[k] = R2_hold_counter_[k] < config_.echo_model.nonlinear_hold
                   ? std::max((*R2)[k], R2_old_[k])
                   : std::min((*R2)[k] + R2_old_[k] * config_.echo_model.nonlinear_release, Y2[k]);
  }
}
...
void ResidualEchoEstimator::EchoGeneratingPower(const RenderBuffer& render_buffer,
                         size_t min_delay,
                         size_t max_delay,
  ...
  std::for_each(X2->begin(), X2->end(), [&](float& a) {
    if (config_.echo_model.noise_gate_power > a) {
      a = std::max(0.f, a - config_.echo_model.noise_gate_slope * (config_.echo_model.noise_gate_power - a));
    }
  });
}
...
void ResidualEchoEstimator::RenderNoisePower(
  ...
  for (size_t k = 0; k < render_power.size(); ++k) {
    ...
    if (render_power[k] < (*X2_noise_floor)[k]) {
      ...
      if ((*X2_noise_floor_counter)[k] >= static_cast<int>(config_.echo_model.noise_floor_hold)) {
        (*X2_noise_floor)[k] =
            std::max((*X2_noise_floor)[k] * 1.1f, config_.echo_model.min_noise_floor_power);
      } else {
