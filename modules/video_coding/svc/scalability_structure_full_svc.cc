...
constexpr absl::string_view kFramePatternNames[] = {
...
ScalabilityStructureFullSvc::ScalabilityStructureFullSvc(
    ...
    : num_layers_(num_layers),
      ...
      active_decode_targets_(
          (uint32_t{1} << (num_layers_.spatial * num_layers_.temporal)) - 1) {
  RTC_DCHECK_LE(num_layers_.spatial, kMaxNumSpatialLayers);
  RTC_DCHECK_LE(num_layers_.temporal, kMaxNumTemporalLayers);
}
...
ScalabilityStructureFullSvc::StreamConfig() const {
  ...
  result.num_spatial_layers = num_layers_.spatial;
  result.num_temporal_layers = num_layers_.temporal;
  result.scaling_factor_num[num_layers_.spatial - 1] = 1;
  result.scaling_factor_den[num_layers_.spatial - 1] = 1;
  for (int sid = num_layers_.spatial - 1; sid > 0; --sid) {
...
bool ScalabilityStructureFullSvc::TemporalLayerIsActive(int tid) const {
  if (tid >= num_layers_.temporal) {
    return false;
  }
  for (int sid = 0; sid < num_layers_.spatial; ++sid) {
...
ScalabilityStructureFullSvc::NextFrameConfig(bool restart) {
  ...
  configs.reserve(num_layers_.spatial);
  ...
  switch (current_pattern) {
    ...
    case kDeltaT1:
      for (int sid = 0; sid < num_layers_.spatial; ++sid) {
        ...
        if (num_layers_.temporal > 2 || sid < num_layers_.spatial - 1) {
...
GenericFrameInfo ScalabilityStructureFullSvc::OnEncodeDone(
  ...
  frame_info.decode_target_indications.reserve(num_layers_.spatial *
                                               num_layers_.temporal);
  for (int sid = 0; sid < num_layers_.spatial; ++sid) {
    for (int tid = 0; tid < num_layers_.temporal; ++tid) {
      frame_info.decode_target_indications.push_back(Dti(sid, tid, config));
    }
  }
  if (config.TemporalId() == 0) {
    frame_info.part_of_chain.resize(num_layers_.spatial);
    for (int sid = 0; sid < num_layers_.spatial; ++sid) {
      frame_info.part_of_chain[sid] = config.SpatialId() <= sid;
    }
  } else {
    frame_info.part_of_chain.assign(num_layers_.spatial, false);
  }
