...
std::string GetStreamIdsString(std::vector<std::string> stream_ids) {
  std::string stream_ids_str;
  if (stream_ids.empty()) {
    stream_ids_str.append("stream=\"-\"");
  } else if (stream_ids.size() == 1u) {
    stream_ids_str.append("stream=");
  } else {
    stream_ids_str.append("streams=");
  }
  for (std::vector<std::string>::const_iterator it = stream_ids.begin();
       it != stream_ids.end(); ++it) {
    if (it != stream_ids.begin()) {
      stream_ids_str.append(",");
    }
    stream_ids_str.append(*it);
  }
  return stream_ids_str;
}
