...
void RtpPayloadParams::Vp9ToGeneric(const CodecSpecificInfoVP9& vp9_info,
  ...
  const int num_temporal_layers = 3;
  ...
  for (int sid = 0; sid < num_spatial_layers; ++sid) {
    for (int tid = 0; tid < num_temporal_layers; ++tid) {
      ...
      if (sid < result.spatial_index || tid < result.temporal_index) {
        dti = DecodeTargetIndication::kNotPresent;
      } else if (result.spatial_index != sid &&
                 vp9_header.non_ref_for_inter_layer_pred) {
        dti = DecodeTargetIndication::kNotPresent;
      } else if (sid == result.spatial_index && tid == result.temporal_index) {
        // Assume that if frame is decodable, all of its own layer is decodable.
        dti = DecodeTargetIndication::kSwitch;
      } else if (sid == result.spatial_index && vp9_header.temporal_up_switch) {
        dti = DecodeTargetIndication::kSwitch;
  ...
  if (vp9_header.inter_pic_predicted) {
    for (size_t i = 0; i < vp9_header.num_ref_pics; ++i) {
      ...
      result.dependencies.push_back(
          last_vp9_frame_id_[depend_on % 128][result.spatial_index]);
    }
  }
  last_vp9_frame_id_[vp9_header.picture_id % 128][result.spatial_index] =
      shared_frame_id;
  ...
  if (!vp9_header.inter_pic_predicted && !vp9_header.inter_layer_predicted) {
    ...
    for (int sid = result.spatial_index; sid < num_spatial_layers; ++sid) {
  ...
  if (result.temporal_index == 0) {
    chain_last_frame_id_[result.spatial_index] = shared_frame_id;
    if (!vp9_header.non_ref_for_inter_layer_pred) {
      for (int sid = result.spatial_index + 1; sid < num_spatial_layers;
           ++sid) {
