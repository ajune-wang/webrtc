...
Packet& Packet::Idr(std::vector<uint8_t> payload) {
  ...
  nalu_payloads_.emplace_back(std::move(payload));
  return *this;
}
...
TEST(H264PacketBufferTest, IdrSpsPpsIsKeyframeSingleNalus) {
  ...
  RTC_UNUSED(packet_buffer.InsertPacket(
      Packet(kH264SingleNalu).Pps().SeqNum(1).Time(1).Build()));
  EXPECT_THAT(
      packet_buffer
          .InsertPacket(
  ...
  // No PPS
  RTC_UNUSED(packet_buffer.InsertPacket(
      Packet(kH264SingleNalu).Sps().SeqNum(3).Time(2).Build()));
  EXPECT_THAT(
      packet_buffer
          .InsertPacket(
              Packet(kH264SingleNalu).Idr().SeqNum(4).Time(2).Marker().Build())
          .packets,
      IsEmpty());

  RTC_UNUSED(packet_buffer.InsertPacket(
      Packet(kH264SingleNalu).Sps().SeqNum(5).Time(3).Build()));
  RTC_UNUSED(packet_buffer.InsertPacket(
      Packet(kH264SingleNalu).Pps().SeqNum(6).Time(3).Build()));
  EXPECT_THAT(
      packet_buffer
          .InsertPacket(
              Packet(kH264SingleNalu).Idr().SeqNum(7).Time(3).Marker().Build())
          .packets,
      SizeIs(3));
}
...
TEST(H264PacketBufferTest, IdrSpsPpsIsKeyframeStapA) {
  ...
  EXPECT_THAT(
      packet_buffer
          .InsertPacket(
              Packet(kH264StapA).Pps().Idr().SeqNum(1).Time(1).Marker().Build())
          .packets,
      IsEmpty());
...
TEST(H264PacketBufferTest, OldReinsertedPacketDoesNotCompleteFrame) {
  H264PacketBuffer packet_buffer(/*allow_idr_only_keyframes=*/false);
  ...
  EXPECT_THAT(
      packet_buffer
          .InsertPacket(
              Packet(kH264FuA).Slice().SeqNum(2).Time(1).Marker().Build())
          .packets,
      SizeIs(2));

  // Reinsert the same packet again.
  EXPECT_THAT(
      packet_buffer
          .InsertPacket(
              Packet(kH264FuA).Slice().SeqNum(2).Time(1).Marker().Build())
  ...
  // This packet will preced the old packet with sequence number 2.
  EXPECT_THAT(packet_buffer
                  .InsertPacket(Packet(kH264FuA)
                                    .Slice()
...
TEST(H264PacketBufferTest, RtpSeqNumWrap) {
  ...
  RTC_UNUSED(packet_buffer.InsertPacket(
      Packet(kH264StapA).Sps().Pps().SeqNum((1 << 16) - 1).Time(0).Build()));
...
TEST(H264PacketBufferTest, SingleNaluFixedBitstream) {
  ...
  auto res = packet_buffer.InsertPacket(Packet(kH264SingleNalu)
                                            .Idr({7, 8, 9})
                                            .SeqNum(2)
                                            .Time(0)
                                            .Marker()
...
TEST(H264PacketBufferTest, StapaAndFuaFixedBitstream) {
  ...
  auto res = packet_buffer.InsertPacket(
      Packet(kH264FuA).Idr({9, 9, 9}).SeqNum(2).Time(0).Marker().Build());

  auto expected_first =
      BuildVector({kStartCode, kSps, {1, 2, 3}, kStartCode, kPps, {4, 5, 6}});
  auto expected_second = BuildVector({kStartCode, {8, 8, 8}});
