...
NetEqInput::Event NetEqReplacementInput::PopEvent() {
  NetEqInput::Event event_to_return = std::move(event_);
  while (true) {
    event_ = source_->PopEvent();
    std::unique_ptr<PacketData>& packet = event_.packet_data;
    if (packet == nullptr) {
      break;
    }
...
void NetEqReplacementInput::ReplaceIfPacketEvent() {
  ...
  const uint32_t timestamp_diff =
      next_hdr->timestamp - packet->header.timestamp;
