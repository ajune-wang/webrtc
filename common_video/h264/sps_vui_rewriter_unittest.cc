...
TEST(SpsVuiRewriterOutgoingVuiTest, ParseOutgoingBitstreamOptimalVui) {
  ...
  absl::optional<SpsParser::SpsState> unused_sps_state;
  EXPECT_EQ(
      SpsVuiRewriter::ParseAndRewriteSps(
          optimal_sps.data(), optimal_sps.size(), &unused_sps_state, nullptr,
          &modified_buffer, SpsVuiRewriter::Direction::kOutgoing),
      SpsVuiRewriter::ParseResult::kVuiOk);
}
...
TEST(SpsVuiRewriterOutgoingVuiTest, ParseOutgoingBitstreamNoVui) {
  ...
  absl::optional<SpsParser::SpsState> unused_sps_state;
  EXPECT_EQ(SpsVuiRewriter::ParseAndRewriteSps(
                sps.data(), sps.size(), &unused_sps_state, nullptr,
                &modified_buffer, SpsVuiRewriter::Direction::kOutgoing),
            SpsVuiRewriter::ParseResult::kVuiRewritten);

  EXPECT_THAT(modified_buffer, ::testing::ElementsAreArray(optimal_sps));
}
}  // namespace webrtc
