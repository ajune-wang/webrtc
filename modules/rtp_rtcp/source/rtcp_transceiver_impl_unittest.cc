...
TEST(RtcpTransceiverImplTest,
  ...
  SimulatedClock clock(/*initial_time_us=*/1111);
  MockTransport outgoing_transport;
  ...
  // Feed in SenderReport for RemoteSsrc2, but no report for RemoteSsrc1.
  const NtpTime remote_ntp(0x9876543211);
  SenderReport sr;
  ...
  EXPECT_EQ(report_blocks[0].source_ssrc(), kRemoteSsrc1);
  // No matching Sender Report for kRemoteSsrc1, LastSR fields has to be 0.
  EXPECT_EQ(report_blocks[0].last_sr(), 0u);

  EXPECT_EQ(report_blocks[1].source_ssrc(), kRemoteSsrc2);
  EXPECT_EQ(report_blocks[1].last_sr(), CompactNtp(remote_ntp));
}
...
TEST(RtcpTransceiverImplTest,
  ...
  // Feed in SenderReport for first remote ssrc.
  SenderReport sr1;
  sr1.SetSenderSsrc(kRemoteSsrc1);
  auto raw_packet1 = sr1.Build();
  rtcp_transceiver.ReceivePacket(raw_packet1);

  clock.AdvanceTimeMilliseconds(100);

  // Feed in SenderReport for second remote ssrc1.
  SenderReport sr2;
  sr2.SetSenderSsrc(kRemoteSsrc2);
  auto raw_packet2 = sr2.Build();
  rtcp_transceiver.ReceivePacket(raw_packet2);
