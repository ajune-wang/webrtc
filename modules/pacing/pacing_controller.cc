...
const TimeDelta PacingController::kMinSleepTime = TimeDelta::Millis<2>();
...
Timestamp PacingController::NextSendTime() const {
  ...
  if (media_rate_ > DataRate::Zero() && !packet_queue_.Empty()) {
    next_send_time =
        std::min(next_send_time, last_send_time_ + media_debt_ / media_rate_);
  }
  if (padding_rate_ > DataRate::Zero() && packet_queue_.Empty()) {
    next_send_time = std::min(next_send_time,
                              last_send_time_ + padding_debt_ / padding_rate_);
  }
  return next_send_time;
}

void PacingController::ProcessPackets(
    absl::optional<Timestamp> scheduled_time) {
  Timestamp now = CurrentTime();
  TimeDelta time_since_scheduled_run = TimeDelta::Zero();
  if (scheduled_time.has_value()) {
    time_since_scheduled_run = now - *scheduled_time;
  }
