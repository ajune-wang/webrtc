...
TEST(ObjCVideoEncoderFactoryTest, EncoderFillsGenericFrameInfo) {
  ...
  EXPECT_CALL(callback, OnEncodedImage)
      .WillRepeatedly(
          [](const webrtc::EncodedImage& /*encoded_image*/, webrtc::CodecSpecificInfo* info) {
...
TEST(ObjCVideoEncoderFactoryTest, EncoderFillsTemplateStructureOnKeyFrames) {
  ...
  EXPECT_CALL(callback, OnEncodedImage)
      .WillOnce([](const webrtc::EncodedImage& /*encoded_image*/, webrtc::CodecSpecificInfo* info) {
        // Key frame.
        EXPECT_NE(info->template_structure, absl::nullopt);
        return webrtc::EncodedImageCallback::Result::OK;
      })
      .WillOnce([](const EncodedImage& /*encoded_image*/, webrtc::CodecSpecificInfo* info) {
        // Delta frame.
