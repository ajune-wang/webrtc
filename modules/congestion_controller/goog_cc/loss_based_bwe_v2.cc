...
absl::optional<LossBasedBweV2::Config> LossBasedBweV2::CreateConfig(
  ...
  FieldTrialParameter<DataRate> minimum_estimate_cap(
      "MinimumEstimateCap", DataRate::KilobitsPerSec(100.0));
  ...
  config->minimum_estimate_cap = minimum_estimate_cap.Get();
  return config;
}
...
bool LossBasedBweV2::IsConfigValid() const {
  ...
  if (config_->minimum_estimate_cap <= DataRate::Zero()) {
    RTC_LOG(LS_WARNING) << "The minimum estimate cap must be positive: "
                        << ToString(config_->minimum_estimate_cap);
    valid = false;
  }
  return valid;
}
