...
TEST_F(DefaultVideoQualityAnalyzerSimulatedTimeTest,
  ...
  // Mark stream paused for Bob, but not for Charlie, freeze 1s and continue for
  // Bob.
  ...
  EXPECT_THAT(GetTimeSortedValues(bob_stream_stats.freeze_time_ms),
              ElementsAre(0.0));
  // TODO(crbug.com/webrtc/14995): value should exclude pause
  EXPECT_THAT(GetTimeSortedValues(bob_stream_stats.time_between_freezes_ms),
              ElementsAre(2950.0));
  // TODO(crbug.com/webrtc/14995): Fix capture_frame_rate (has to be ~20.0)
  ExpectRateIs(bob_stream_stats.capture_frame_rate, 13.559322);
  // TODO(crbug.com/webrtc/14995): Fix encode_frame_rate (has to be ~20.0)
  ExpectRateIs(bob_stream_stats.encode_frame_rate, 13.559322);
  // TODO(crbug.com/webrtc/14995): Assert on harmonic fps
