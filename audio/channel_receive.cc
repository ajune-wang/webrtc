...
CallReceiveStatistics ChannelReceive::GetRTCPStatistics() const {
  ...
  stats.extendedMax = rtp_stats.extended_highest_sequence_number;
  ...
  if (statistician) {
    if (use_standard_bytes_stats_) {
      stats.bytesReceived = rtp_stats.packet_counter.transmitted.payload_bytes;
    } else {
      stats.bytesReceived = rtp_stats.packet_counter.transmitted.TotalBytes();
    }
    stats.packetsReceived = rtp_stats.packet_counter.transmitted.packets;
    stats.last_packet_received_timestamp_ms =
        rtp_stats.packet_counter.last_packet_received_timestamp_ms;
  } else {
