...
void AdaptiveModeLevelEstimator::Update(
  ...
  if (!state_.level_dbfs.has_value()) {
    state_.level_dbfs = {/*numerator=*/level_dbfs * vad_data.speech_probability,
    ...
    state_.level_dbfs->denominator =
        state_.level_dbfs->denominator * leak_factor +
        vad_data.speech_probability;
  }

  // Cache level estimation.
  RTC_DCHECK(state_.level_dbfs.has_value());
  ...
  if (use_saturation_protector_) {
    saturation_protector_.UpdateMargin(
        ...
        /*speech_level_dbfs=*/state_.level_dbfs->GetRatio());
  }
