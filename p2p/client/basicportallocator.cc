...
void RemoveCandidatesWithDuplicatedIps(
    std::vector<cricket::Candidate>* candidates,
    const std::set<std::string>& existing_ips) {
...
void BasicPortAllocatorSession::MaybeSignalCandidatesAllocationDone() {
  if (CandidatesAllocationDone()) {
    // Signaling candidates from any address ports after the allocation is done
    // could be problematic, since it could be delayed for a long time if a STUN
    // request times out. This should however suffice for the majority of
    // allocations.
    //
    // TODO(qingsi): Fix this.
...
void BasicPortAllocatorSession::MaybeSignalAnyAddressPortsAndCandidatesReady() {
  ...
  if (flags() & PORTALLOCATOR_DISABLE_ANY_ADDRESS_PORTS) {
    ...
    RTC_DCHECK(candidates_from_any_address_ports_.empty());
    return;
  }
  ...
  for (auto* port_data : signalable_any_address_ports) {
  ...
  RemoveCandidatesWithDuplicatedIps(&candidates_from_any_address_ports_,
                                    ips_from_non_any_address_ports_);
  SignalCandidatesReady(this, candidates_from_any_address_ports_);
  candidates_from_any_address_ports_.clear();
}
