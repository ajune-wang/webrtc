...
void UDPPort::AddressResolver::Resolve(
  ...
  if (field_trials.IsEnabled("WebRTC-ResolveStunHostnameForFamily")) {
    resolver_ptr->Start(address, family, callback);
  } else {
    resolver_ptr->Start(address, callback);
  }
}
...
void UDPPort::SendStunBindingRequests() {
  ...
  for (ServerAddresses::const_iterator it = server_addresses_.begin();
    ...
    // sending a STUN binding request may cause the address to be erased from
    // the set, so copy the loop iterator to avoid it being invalidated.
