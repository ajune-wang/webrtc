...
int VP9EncoderImpl::Encode(const VideoFrame& input_image,
  ...
  if (!force_key_frame_) {
    // Skip encoding low spatial layer frames if their target frame rate is
    // lower than actual input frame rate.
  ...
  // since thier frame rate can be different. For now calculate frame duration
...
void VP9EncoderImpl::DeliverBufferedFrame(bool end_of_picture) {
  if (encoded_image_._length > 0) {
    ...
    const uint8_t spatial_idx =
        codec_specific_.codecSpecific.VP9.spatial_idx == kNoSpatialIdx
            ? 0
            : codec_specific_.codecSpecific.VP9.spatial_idx;
    const uint32_t frame_timestamp_ms =
