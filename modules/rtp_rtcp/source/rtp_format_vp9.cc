...
void RtpPacketizerVp9::GeneratePackets() {
  if (max_payload_length_ < PayloadDescriptorLength(hdr_) + 1) {
    NLOG(LS_ERROR,
         "Payload header and one payload byte won't fit in the "
         "first packet.");
    return;
  }
  ...
  if (payload_size_ == 1 &&
    ...
    NLOG(LS_ERROR,
         "Can't fit header and payload into single packet, but "
...
bool RtpPacketizerVp9::WriteHeader(const PacketInfo& packet_info,
  ...
  if (i_bit && !WritePictureId(hdr_, &writer)) {
    NLOG(LS_ERROR, "Failed writing VP9 picture id.");
    return false;
  }
  if (l_bit && !WriteLayerInfo(hdr_, &writer)) {
    NLOG(LS_ERROR, "Failed writing VP9 layer info.");
    return false;
  }
  if (p_bit && f_bit && !WriteRefIndices(hdr_, &writer)) {
    NLOG(LS_ERROR, "Failed writing VP9 ref indices.");
    return false;
  }
  if (v_bit && !WriteSsData(hdr_, &writer)) {
    NLOG(LS_ERROR, "Failed writing VP9 SS data.");
    return false;
  }
...
bool RtpDepacketizerVp9::Parse(ParsedPayload* parsed_payload,
  ...
  if (payload_length == 0) {
    NLOG(LS_ERROR, "Payload length is zero.");
    return false;
  }
  ...
  if (i_bit && !ParsePictureId(&parser, vp9)) {
    NLOG(LS_ERROR, "Failed parsing VP9 picture id.");
    return false;
  }
  if (l_bit && !ParseLayerInfo(&parser, vp9)) {
    NLOG(LS_ERROR, "Failed parsing VP9 layer info.");
    return false;
  }
  if (p_bit && f_bit && !ParseRefIndices(&parser, vp9)) {
    NLOG(LS_ERROR, "Failed parsing VP9 ref indices.");
    return false;
  }
  if (v_bit) {
    if (!ParseSsData(&parser, vp9)) {
      NLOG(LS_ERROR, "Failed parsing VP9 SS data.");
      return false;
    }
    if (vp9->spatial_layer_resolution_present) {
  ...
  if (parsed_payload->payload_length == 0) {
    NLOG(LS_ERROR, "Failed parsing VP9 payload data.");
    return false;
  }
