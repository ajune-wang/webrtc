...
TEST(IForwardTsnChunkTest, FromCapture) {
  ...
  */
  ...
  EXPECT_EQ(chunk.new_cumulative_tsn(), 3094631148);
  EXPECT_THAT(chunk.skipped_streams(),
              ElementsAre(IForwardTsnChunk::SkippedStream(1, false, 2)));
}

TEST(IForwardTsnChunkTest, SerializeAndDeserialize) {
  IForwardTsnChunk chunk(/*new_cumulative_tsn=*/123,
                         {IForwardTsnChunk::SkippedStream(1, false, 23),
                          IForwardTsnChunk::SkippedStream(42, true, 99)});
  ...
  EXPECT_THAT(deserialized.skipped_streams(),
              ElementsAre(IForwardTsnChunk::SkippedStream(1, false, 23),
                          IForwardTsnChunk::SkippedStream(42, true, 99)));
