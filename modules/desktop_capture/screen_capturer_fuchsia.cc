...
bool ScreenCapturerFuchsia::CheckRequirements() {
  ...
  // TODO(fxbug.dev/100303): Remove when Flatland is the only API. At that
  // point, this function will no longer be necessary and the remaining parts
  // of it can be moved into SetupBuffers.
  ...
  // We only need to check if Scenic uses Flatland but while we have our Scenic
  // pointer, get the display dimensions.
...
void ScreenCapturerFuchsia::SetupBuffers() {
  zx_status_t status =
      component_context_->svc()->Connect(sysmem_allocator_.NewRequest());
  if (status != ZX_OK) {
    fatal_error_ = true;
