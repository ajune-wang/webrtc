...
NetEq::Operations DecisionLogic::GetDecision(const NetEqStatus& status,
                                             bool* reset_decoder) {
  ...
  if (status.last_mode == NetEq::Modes::kRfc3389Cng) {
    cng_state_ = kCngRfc3389On;
  } else if (status.last_mode == NetEq::Modes::kCodecInternalCng) {
  ...
  prev_time_scale_ =
      prev_time_scale_ &&
      (status.last_mode == NetEq::Modes::kAccelerateSuccess ||
       status.last_mode == NetEq::Modes::kAccelerateLowEnergy ||
       status.last_mode == NetEq::Modes::kPreemptiveExpandSuccess ||
       status.last_mode == NetEq::Modes::kPreemptiveExpandLowEnergy);

  // Do not update buffer history if currently playing CNG since it will bias
  // the filtered buffer level.
  if (status.last_mode != NetEq::Modes::kRfc3389Cng &&
      status.last_mode != NetEq::Modes::kCodecInternalCng &&
  ...
  if (status.last_mode == NetEq::Modes::kError) {
    if (!status.next_packet) {
      return NetEq::Operations::kExpand;
    } else {
      // Use kUndefined to flag for a reset.
      return NetEq::Operations::kUndefined;
    }
  }
  ...
  if (num_consecutive_expands_ > kReinitAfterExpands) {
    *reset_decoder = true;
    return NetEq::Operations::kNormal;
  }
  ...
  if ((status.last_mode == NetEq::Modes::kExpand ||
       status.last_mode == NetEq::Modes::kCodecPlc) &&
...
void DecisionLogic::ExpandDecision(NetEq::Operations operation) {
  if (operation == NetEq::Operations::kExpand) {
...
NetEq::Operations DecisionLogic::CngOperation(NetEq::Modes prev_mode,
  ...
  if (timestamp_diff < 0 && prev_mode == NetEq::Modes::kRfc3389Cng) {
    // Not time to play this packet yet. Wait another round before using this
    // packet. Keep on playing CNG from previous CNG parameters.
    return NetEq::Operations::kRfc3389CngNoPacket;
  } else {
    // Otherwise, go for the CNG packet now.
    noise_fast_forward_ = 0;
    return NetEq::Operations::kRfc3389Cng;
  }
}

NetEq::Operations DecisionLogic::NoPacket(bool play_dtmf) {
  if (cng_state_ == kCngRfc3389On) {
    // Keep on playing comfort noise.
    return NetEq::Operations::kRfc3389CngNoPacket;
  } else if (cng_state_ == kCngInternalOn) {
    // Keep on playing codec internal comfort noise.
    return NetEq::Operations::kCodecInternalCng;
  } else if (play_dtmf) {
    return NetEq::Operations::kDtmf;
  } else {
...
NetEq::Operations DecisionLogic::ExpectedPacketAvailable(NetEq::Modes prev_mode,
                                                         bool play_dtmf) {
  if (!disallow_time_stretching_ && prev_mode != NetEq::Modes::kExpand &&
      !play_dtmf) {
    ...
    if (buffer_level_packets >= high_limit << 2)
      return NetEq::Operations::kFastAccelerate;
    if (TimescaleAllowed()) {
      if (buffer_level_packets >= high_limit)
        return NetEq::Operations::kAccelerate;
      if (buffer_level_packets < low_limit)
        return NetEq::Operations::kPreemptiveExpand;
    }
  }
  return NetEq::Operations::kNormal;
}

NetEq::Operations DecisionLogic::FuturePacketAvailable(
    size_t decoder_frame_length,
    NetEq::Modes prev_mode,
    uint32_t target_timestamp,
    uint32_t available_timestamp,
    bool play_dtmf,
    size_t generated_noise_samples,
  ...
  if ((prev_mode == NetEq::Modes::kExpand ||
       prev_mode == NetEq::Modes::kCodecPlc) &&
  ...
  if (prev_mode == NetEq::Modes::kCodecPlc) {
    return NetEq::Operations::kNormal;
  }

  // If previous was comfort noise, then no merge is needed.
  if (prev_mode == NetEq::Modes::kRfc3389Cng ||
      prev_mode == NetEq::Modes::kCodecInternalCng) {
  ...
  if (prev_mode == NetEq::Modes::kExpand) {
    return NetEq::Operations::kMerge;
  } else if (play_dtmf) {
