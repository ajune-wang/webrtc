...
std::vector<AnalysisResult> RunAnalysis(
  ...
  for (size_t i = 0; i < test_frame_indicies.size(); ++i) {
    // Ignore duplicated frames in the test video.
    if (i > 0 && test_frame_indicies[i] == test_frame_indicies[i - 1])
      continue;
    ...
    const rtc::scoped_refptr<I420BufferInterface>& reference_frame =
        reference_video->GetFrame(test_frame_indicies[i] %
                                  reference_video->number_of_frames());

    // Fill in the result struct.
    AnalysisResult result;
    result.frame_number = test_frame_indicies[i];
...
std::vector<Cluster> CalculateFrameClusters(
  ...
  for (size_t i = 0; i < indicies.size(); ++i) {
    ...
    while (i < indicies.size() && indicies[i] == indicies[start_index])
      ++i;
    const int number_of_repeated_frames = static_cast<int>(i - start_index);
    clusters.push_back({indicies[start_index], number_of_repeated_frames});
  }
  return clusters;
}
