...
VideoRenderFrames::~VideoRenderFrames() {
  ...
  RTC_HISTOGRAM_COUNTS_1000("WebRTC.Video.DroppedFrames.RenderQueue",
                            frames_dropped_);
  RTC_DLOG(LS_INFO) << "WebRTC.Video.DroppedFrames.RenderQueue "
                   << frames_dropped_;
}
...
int32_t VideoRenderFrames::AddFrame(VideoFrame&& new_frame) {
  ...
  if (!incoming_frames_.empty() &&
    ...
    RTC_DLOG(LS_WARNING) << "Too old frame, timestamp=" << new_frame.timestamp();
    ++frames_dropped_;
    return -1;
  }

  if (new_frame.render_time_ms() > time_now + kFutureRenderTimestampMS) {
    RTC_DLOG(LS_WARNING) << "Frame too long into the future, timestamp="
                        << new_frame.timestamp();
    ++frames_dropped_;
    return -1;
  }
  ...
  if (new_frame.render_time_ms() < last_render_time_ms_) {
    RTC_DLOG(LS_WARNING) << "Frame scheduled out of order, render_time="
  ...
  if (incoming_frames_.size() > kMaxIncomingFramesBeforeLogged) {
    RTC_DLOG(LS_WARNING) << "Stored incoming frames: "
