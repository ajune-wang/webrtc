...
void FilterAnalyzer::Update(
  ...
  UpdateFilterGain(filter_time_domain, max_index);
...
void FilterAnalyzer::UpdateFilterGain(rtc::ArrayView<const float> filter_time_domain,
                                size_t max_index) {
  bool sufficient_time_to_converge = ++blocks_since_reset_ > 5;

  if (sufficient_time_to_converge) {
