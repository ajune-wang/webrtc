...
static constexpr int kAudioPrioLevel = 0;
...
PrioritizedPacketQueue::QueuedPacket::QueuedPacket(
...
bool PrioritizedPacketQueue::StreamQueue::EnqueuePacket(QueuedPacket packet,
  ...
  packets_[priority_level].emplace_back(std::move(packet));
...
void PrioritizedPacketQueue::Push(Timestamp enqueue_time,
  ...
  if (stream_it != streams_.end()) {
    stream_queue = stream_it->second.get();
  } else {
    auto kv = streams_.emplace(packet->Ssrc(),
                               std::make_unique<StreamQueue>(enqueue_time));
    stream_queue = kv.first->second.get();
  }
...
size_t PrioritizedPacketQueue::SizeInPackets() const {
