...
class PeerConnectionIceBaseTest : public ::testing::Test {
 protected:
  ...
  cricket::IceRole GetIceRole(const WrapperPtr& pc_wrapper_ptr) {
    PeerConnection* pc = GetInternalPeerConnection();
...
TEST_P(PeerConnectionIceTest, IceForking) {
  ...
  auto gatherer = pc1_internal->CreateIceGatherer();
  ASSERT_TRUE(gatherer);

  pc1_internal->SetIceGatherer(gatherer);
  ...
  pc2_internal->SetIceGatherer(gatherer);
  ...
  auto* pc2_ice = GetFirstIceTransport(pc1_internal);
  ...
  ASSERT_TRUE(pc1_ice->gatherer());
  ASSERT_TRUE(pc2_ice->gatherer());
  EXPECT_EQ(gatherer->session(), pc1_ice->gatherer()->session());
  EXPECT_EQ(gatherer->session(), pc2_ice->gatherer()->session());
