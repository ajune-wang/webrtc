...
class RtpSenderTest : public ::testing::Test {
 protected:
  ...
  std::unique_ptr<RtpPacketToSend> BuildRtpPacket(int payload_type,
    ...
    auto packet = rtp_sender_->CreatePacket();
...
TEST_F(RtpSenderTest, CreatePacketSetCsrcs) {
  // Configure rtp_sender with csrc.
  uint32_t csrcs[] = {0x23456789};

  auto packet = rtp_sender_->CreatePacket(csrcs);
...
TEST_F(RtpSenderTest, PaddingAlwaysAllowedOnAudio) {
  ...
  std::unique_ptr<RtpPacketToSend> audio_packet = rtp_sender_->CreatePacket();
...
TEST_F(RtpSenderTest, UpdatingCsrcsUpdatedOverhead) {
  ...
  uint32_t csrcs[] = {1, 2};
  rtp_sender_->CreatePacket(csrcs);
