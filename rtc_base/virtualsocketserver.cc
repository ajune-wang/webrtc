...
void VirtualSocket::OnMessage(Message* pmsg) {
  if (pmsg->message_id == MSG_ID_PACKET) {
    ...
    if (listen_queue_ != nullptr) {
      ...
      NLOG(LS_VERBOSE, "Socket at ", local_addr_, " is not listening");
      server_->Disconnect(server_->LookupBinding(data->addr));
    }
    delete data;
...
int VirtualSocketServer::Connect(VirtualSocket* socket,
  ...
  if (!CanInteractWith(socket, remote)) {
    NLOG(LS_INFO, "Address family mismatch between ", socket->GetLocalAddress(),
         " and ", remote_addr);
    return -1;
  }
  if (remote != nullptr) {
    ...
    NLOG(LS_INFO, "No one listening at ", remote_addr);
...
int VirtualSocketServer::SendUdp(VirtualSocket* socket,
  ...
  if (Random() < drop_prob_) {
    NLOG(LS_VERBOSE, "Dropping packet: bad luck");
  ...
  if (!recipient) {
    ...
    if (!CanInteractWith(socket, dummy_socket.get())) {
      NLOG(LS_VERBOSE,
           "Incompatible address families: ", socket->GetLocalAddress(),
           " and ", remote_addr);
      return -1;
    }
    NLOG(LS_VERBOSE, "No one listening at ", remote_addr);
    return static_cast<int>(data_size);
  }

  if (!CanInteractWith(socket, recipient)) {
    NLOG(LS_VERBOSE,
         "Incompatible address families: ", socket->GetLocalAddress(), " and ",
         remote_addr);
    return -1;
  }

  {
    ...
    if (socket->network_size_ + packet_size > network_capacity_) {
      NLOG(LS_VERBOSE, "Dropping packet: network capacity exceeded");
      return static_cast<int>(data_size);
    }
...
void VirtualSocketServer::SendTcp(VirtualSocket* socket) {
  ...
  VirtualSocket* recipient = LookupConnection(socket->local_addr_,
                                              socket->remote_addr_);
  if (!recipient) {
    NLOG(LS_VERBOSE, "Sending data to no one.");
    return;
  }
