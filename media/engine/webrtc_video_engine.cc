...
WebRtcVideoChannel::WebRtcVideoSendStream::GetPerLayerVideoSenderInfos(
  ...
  for (const auto& pair : outbound_rtp_substreams) {
    ...
    // TODO(hbos): Delete old implementation of `total_packet_send_delay`.
    info.legacy_packet_send_delay_ms += stream_stats.total_packet_send_delay_ms;
...
WebRtcVideoChannel::WebRtcVideoSendStream::GetAggregatedVideoSenderInfo(
  ...
  for (size_t i = 1; i < infos.size(); i++) {
    ...
    info.legacy_packet_send_delay_ms += infos[i].legacy_packet_send_delay_ms;
