...
void EchoPathStrengthDetector::Update(
  ...
  bool converged_filter_seen =
      render_activity_since_converged_filter_ < 20 * kNumBlocksPerSecond;

  if (!converged_filter_seen && !stable_delay_estimate &&
      !good_filter_estimate && capture_counter_ > 10 * kNumBlocksPerSecond &&
      render_activity_counter_ > 5 * kNumBlocksPerSecond) {
    echo_path_strength_ = Strength::kZero;
  } else if (!converged_filter_seen && !stable_delay_estimate &&
