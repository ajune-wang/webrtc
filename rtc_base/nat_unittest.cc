...
void TestPhysicalInternal(const SocketAddress& int_addr) {
  ...
  networks.erase(std::remove_if(networks.begin(), networks.end(),
                 ...
                 networks.end());
  if (networks.empty()) {
    NLOG(LS_WARNING, "Not enough network adapters for test.");
    return;
  }
  ...
  if (ext_addr2.IsNil()) {
    NLOG(LS_WARNING, "No available IP of same family as ", int_addr);
    return;
  }

  NLOG(LS_INFO, "selected ip ", ext_addr2.ipaddr());
...
TEST(NatTest, TestPhysicalIPv6) {
  if (HasIPv6Enabled()) {
    ...
    NLOG(LS_WARNING, "No IPv6, skipping");
  }
}
