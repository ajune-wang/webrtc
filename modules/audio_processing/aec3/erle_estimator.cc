...
ErleEstimator::ErleEstimator(float min_erle,
                             float max_erle_lf,
                             float max_erle_hf)
    : erle_time_estimator_(min_erle, max_erle_lf),
      erle_freq_estimator_(min_erle, max_erle_lf, max_erle_hf) {
  Reset();
}
...
void ErleEstimator::Reset() {
  erle_time_estimator_.Reset();
  erle_freq_estimator_.Reset();
}
...
void ErleEstimator::Update(rtc::ArrayView<const float> render_spectrum,
  ...
  erle_freq_estimator_.Update(X2, Y2, E2, converged_filter, onset_detection);
  erle_time_estimator_.Update(X2, Y2, E2, converged_filter);
}

void ErleEstimator::Dump(
    const std::unique_ptr<ApmDataDumper>& data_dumper) const {
  erle_time_estimator_.Dump(data_dumper);
  erle_freq_estimator_.Dump(data_dumper);
}
