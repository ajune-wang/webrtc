  ...
  for (auto& feedback : parsed_log_.transport_feedbacks(kIncomingPacket)) {
    ...
    for (uint16_t seq_num = last_acked;
      ...
      missing_feedback_series.points.emplace_back(
          config_.GetCallTimeSec(feedback.timestamp + shifted_time), 100);
      shifted_time =
          std::min(shifted_time + TimeDelta::Millis(2), TimeDelta::Millis(50));
    }
    last_acked = transport_feedback.GetBaseSequence() +
                 transport_feedback.GetPacketStatusCount() - 2;

    // Compute loss rate from the transport feedback.
    const std::vector<rtcp::TransportFeedback::ReceivedPacket>&
        received_packets = transport_feedback.GetReceivedPackets();
