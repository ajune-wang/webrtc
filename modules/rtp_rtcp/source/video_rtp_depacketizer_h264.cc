...
absl::optional<VideoRtpDepacketizer::ParsedRtpPayload> ProcessStapAOrSingleNalu(
  ...
  absl::optional<VideoRtpDepacketizer::ParsedRtpPayload> result(absl::in_place);
  result->video_payload = rtp_payload;
  result->video_header.width = 0;
  result->video_header.height = 0;
  result->video_header.codec = kVideoCodecH264;
  result->video_header.simulcastIdx = 0;
  result->video_header.is_first_packet_in_frame = true;
  auto& h264_header =
      result->video_header.video_type_header.emplace<RTPVideoHeaderH264>();
  ...
  for (size_t i = 0; i < nalu_start_offsets.size() - 1; ++i) {
    ...
    switch (nalu.type) {
      case H264::NaluType::kSps: {
        ...
        SpsVuiRewriter::ParseResult rewrote =
        ...
        if (rewrote == SpsVuiRewriter::ParseResult::kVuiRewritten) {
          if (payload_data != result->video_payload.cdata()) {
            RTC_LOG(LS_WARNING)
          ...
          if (h264_header.packetization_type == kH264StapA) {
            ...
            ByteWriter<uint16_t>::WriteBigEndian(
                output_buffer.data() + length_field_offset, rewritten_size);
          }
          ...
          result->video_payload.SetData(output_buffer.data(),
                                        output_buffer.size());
        }

        if (sps) {
          result->video_header.width = sps->width;
          result->video_header.height = sps->height;
          nalu.sps_id = sps->id;
        } else {
...
absl::optional<VideoRtpDepacketizer::ParsedRtpPayload> ParseFuaNalu(
  ...
  if (first_fragment) {
    ...
    result->video_payload = std::move(rtp_payload);
  } else {
  ...
  result->video_header.codec = kVideoCodecH264;
  result->video_header.simulcastIdx = 0;
  result->video_header.is_first_packet_in_frame = first_fragment;
  auto& h264_header =
      result->video_header.video_type_header.emplace<RTPVideoHeaderH264>();
