...
- (void)onShareableContentCreated:(SCShareableContent*)content withError:(NSError*)error;
...
void ScreenCapturerSck::OnShareableContentCreated(SCShareableContent* content, NSError* error) {
  if (!content) {
    RTC_LOG(LS_ERROR) << "ScreenCapturerSck " << this
                      << " getShareableContent failed with error code " << (error ? error.code : 0)
                      << ".";
    permanent_error_ = true;
    return;
  }
...
void ScreenCapturerSck::StartOrReconfigureCapturer() {
  ...
  auto handler = ^(SCShareableContent* content, NSError* error) {
    [local_helper onShareableContentCreated:content withError:error];
  };
