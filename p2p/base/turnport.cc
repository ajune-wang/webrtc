...
void TurnPort::OnSocketClose(rtc::AsyncPacketSocket* socket, int error) {
  LOG_J(LS_WARNING, this) << "Connection with server failed with error: "
                          << error;
...
bool TurnPort::HandleIncomingPacket(rtc::AsyncPacketSocket* socket,
  ...
  if (remote_addr != server_address_.address) {
    LOG_J(LS_WARNING, this)
        ...
        << " server_address_: " << server_address_.address.ToString();
    return false;
  }
...
void TurnPort::OnSendStunPacket(const void* data, size_t size,
  ...
  if (Send(data, size, options) < 0) {
    LOG_J(LS_ERROR, this) << "Failed to send TURN message, error: "
                          << socket_->GetError();
  }
}
...
void TurnPort::HandleDataIndication(const char* data, size_t size,
  ...
  if (!HasPermission(ext_addr.ipaddr())) {
    LOG_J(LS_WARNING, this) << "Received TURN data indication with unknown "
                               "peer address, addr: "
...
bool TurnPort::ScheduleRefresh(uint32_t lifetime) {
  ...
  if (lifetime < 2 * 60) {
    ...
    // for half lifetime.
    LOG_J(LS_WARNING, this)
        << "Received response with short lifetime: " << lifetime << " seconds.";
    ...
    LOG_J(LS_WARNING, this)
        << "Received response with long lifetime: " << lifetime << " seconds.";
...
void TurnAllocateRequest::OnErrorResponse(StunMessage* response) {
  ...
  LOG_J(LS_INFO, port_) << "Received TURN allocate error response, id="
...
void TurnRefreshRequest::OnSent() {
  LOG_J(LS_INFO, port_) << "TURN refresh request sent, id="
                        << rtc::hex_encode(id());
  StunRequest::OnSent();
}

void TurnRefreshRequest::OnResponse(StunMessage* response) {
  LOG_J(LS_INFO, port_) << "TURN refresh requested successfully, id="
                        << rtc::hex_encode(id())
                        << ", code=0"  // Makes logging easier to parse.
                           ", rtt="
                        << Elapsed();
...
void TurnRefreshRequest::OnErrorResponse(StunMessage* response) {
  ...
  if (error_code == STUN_ERROR_STALE_NONCE) {
    ...
    LOG_J(LS_WARNING, port_)
        << "Received TURN refresh error response, id=" << rtc::hex_encode(id())
...
void TurnCreatePermissionRequest::OnSent() {
  LOG_J(LS_INFO, port_) << "TURN create permission request sent, id="
                        << rtc::hex_encode(id());
  StunRequest::OnSent();
}

void TurnCreatePermissionRequest::OnResponse(StunMessage* response) {
  LOG_J(LS_INFO, port_) << "TURN permission requested successfully, id="
                        << rtc::hex_encode(id())
                        << ", code=0"  // Makes logging easier to parse.
                           ", rtt="
                        << Elapsed();
...
void TurnChannelBindRequest::OnSent() {
  LOG_J(LS_INFO, port_) << "TURN channel bind request sent, id="
                        << rtc::hex_encode(id());
  StunRequest::OnSent();
}

void TurnChannelBindRequest::OnResponse(StunMessage* response) {
  LOG_J(LS_INFO, port_) << "TURN channel bind requested successfully, id="
                        << rtc::hex_encode(id())
                        << ", code=0"  // Makes logging easier to parse.
                           ", rtt="
                        << Elapsed();
...
void TurnChannelBindRequest::OnErrorResponse(StunMessage* response) {
  int error_code = response->GetErrorCodeValue();
  LOG_J(LS_WARNING, port_) << "Received TURN channel bind error response, id="
                           << rtc::hex_encode(id()) << ", code=" << error_code
                           << ", rtt=" << Elapsed();
  if (entry_) {
...
void TurnEntry::OnChannelBindSuccess() {
  LOG_J(LS_INFO, port_) << "Successful channel bind for "
                        << ext_addr_.ToSensitiveString();
