...
void RTCStatsCollector::ProducePartialResultsOnNetworkThread(
  ...
  invoker_.AsyncInvoke<void>(
      RTC_FROM_HERE, signaling_thread_,
      rtc::Bind(&RTCStatsCollector::MergeNetworkReport_s,
...
void RTCStatsCollector::MergeNetworkReport_s() {
  ...
  // WaitForPendingRequest() is called while a request is pending, the event
  // might not be signaled here.
  network_report_event_.Wait(rtc::Event::kForever);
  if (!network_report_) {
    // Normally, MergeNetworkReport_s() is posted from the network thread. But
    // if WaitForPendingRequest() is called while a request is pending, an early
    // call to MergeNetworkReport_s() is made, merging the report and setting
    // |network_report_| to null. If so, when the previously posted
    // MergeNetworkReport_s() is executed the report is already null and nothing
