...
bool RRSendQueue::OutgoingStream::Discard(IsUnordered unordered,
                                          MID message_id) {
  RTC_DLOG(LS_VERBOSE) << "Discard from " << *stream_id();
  bool result = false;
  if (!items_.empty()) {
    Item& item = items_.front();
...
void RRSendQueue::OutgoingStream::Pause() {
  RTC_DCHECK(pause_state_ == PauseState::kNotPaused);
...
void RRSendQueue::OutgoingStream::Resume() {
  RTC_DCHECK(pause_state_ != PauseState::kNotPaused);
  if (pause_state_ == PauseState::kPaused && !items_.empty()) {
    RTC_DLOG(LS_VERBOSE) << "Stream " << *stream_id()
...
void RRSendQueue::OutgoingStream::Reset() {
  RTC_DCHECK(pause_state_ != PauseState::kPending);
  if (!items_.empty()) {
...
bool RRSendQueue::CanResetStreams() const {
  ...
  for (auto& [unused, stream] : streams_) {
    if (stream.pause_state() == OutgoingStream::PauseState::kPending) {
      return false;
    }
  }
  return true;
...
void RRSendQueue::CommitResetStreams() {
  for (auto& [unused, stream] : streams_) {
    if (stream.pause_state() == OutgoingStream::PauseState::kPaused) {
      stream.Reset();
    }
  }
  RTC_DCHECK(IsConsistent());
