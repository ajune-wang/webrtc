...
void Call::OnAllocationLimitsChanged(uint32_t min_send_bitrate_bps,
...

  {
    rtc::CritScope lock(&target_observer_crit_);
    if (media_transport_) {
      webrtc::MediaTransportAllocateddBitrateLimits limits;
      limits.min_send_bitrate = DataRate::bps(min_send_bitrate_bps);
      limits.max_padding_bitrate = DataRate::bps(max_padding_bitrate_bps);
      limits.total_bitrate = DataRate::bps(total_bitrate_bps);
