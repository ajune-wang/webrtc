...
bool AimdRateControl::TimeToReduceFurther(Timestamp at_time,
  ...
  if (ValidEstimate()) {
    ...
    const DataRate threshold = static_cast<DataRate>(0.5 * LatestEstimate());
...
DataRate AimdRateControl::GetNearMaxIncreaseRate() const {
  ...
  DataRate kMinIncreaseRate = DataRate::kbps(4);
  return std::max(kMinIncreaseRate, avg_packet_size / response_time);
}
...
TimeDelta AimdRateControl::GetExpectedBandwidthPeriod() const {
  ...
  DataRate increase_rate = GetNearMaxIncreaseRate();
  if (!last_decrease_)
    return smoothing_experiment_ ? kMinPeriod : kDefaultPeriod;

  TimeDelta period = TimeDelta::seconds(*last_decrease_ / increase_rate);
...
DataRate AimdRateControl::AdditiveRateIncrease(Timestamp at_time,
  ...
  return (at_time - last_time).seconds<double>() * GetNearMaxIncreaseRate();
}
