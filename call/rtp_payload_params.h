...
class RtpPayloadParams final {
 ...
 private:
  ...
  void SetGeneric(RTPVideoHeader* rtp_video_header,
                  int64_t frame_id,
                  bool is_keyframe);

  void Vp8ToGeneric(RTPVideoHeader* rtp_video_header,
                    int64_t shared_frame_id,
                    bool is_keyframe);

  // Holds the last shared frame id for a given (spatial, temporal) layer tuple.
  ...
  bool generic_picture_id_experiment_;
};
}  // namespace webrtc
