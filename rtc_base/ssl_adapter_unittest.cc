...
class SSLAdapterTestDummyClient : public sigslot::has_slots<> {
 public:
  ...
  int Connect(const std::string& hostname, const rtc::SocketAddress& address) {
    RTC_DLOG(LS_INFO) << "Initiating connection with " << address.ToString();

    int rv = ssl_adapter_->Connect(address);

    if (rv == 0) {
      RTC_DLOG(LS_INFO) << "Starting " << GetSSLProtocolName(ssl_mode_)
  ...
  int Send(const std::string& message) {
    RTC_DLOG(LS_INFO) << "Client sending '" << message << "'";
  ...
  void OnSSLAdapterReadEvent(rtc::AsyncSocket* socket) {
    ...
    if (read != -1) {
      ...
      RTC_DLOG(LS_INFO) << "Client received '" << buffer << "'";
...
class SSLAdapterTestDummyServer : public sigslot::has_slots<> {
 public:
  explicit SSLAdapterTestDummyServer(const rtc::SSLMode& ssl_mode,
    ...
    RTC_DLOG(LS_INFO) << ((ssl_mode_ == rtc::SSL_MODE_DTLS) ? "UDP" : "TCP")
                     << " server listening on "
  ...
  int Send(const std::string& message) {
    ...
    RTC_DLOG(LS_INFO) << "Server sending '" << message << "'";
  ...
  void OnSSLStreamAdapterEvent(rtc::StreamInterface* stream, int sig, int err) {
    if (sig & rtc::SE_READ) {
      ...
      if (r == rtc::SR_SUCCESS) {
        buffer[read] = '\0';
        RTC_DLOG(LS_INFO) << "Server received '" << buffer << "'";
        data_ += buffer;
      }
    }
  }
 ...
 public:
  ...
  void TestHandshake(bool expect_success) {
    ...
    if (expect_success) {
      ...
      RTC_DLOG(LS_INFO) << GetSSLProtocolName(ssl_mode_)
                       << " handshake complete.";
      ...
      RTC_DLOG(LS_INFO) << GetSSLProtocolName(ssl_mode_) << " handshake failed.";
    }
  }
  ...
  void TestTransfer(const std::string& message) {
    ...
    RTC_DLOG(LS_INFO) << "Transfer complete.";
  }
