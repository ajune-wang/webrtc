...
class RtpVideoFrameAssembler {
 public:
  enum PayloadFormat { kRaw, kH264, kVp8, kVp9, kAv1, kGenericPayload };

  explicit RtpVideoFrameAssembler(PayloadFormat payload_format);

  std::vector<std::unique_ptr<RtpFrameObject>> InsertPacket(
      const RtpPacketReceived& packet);

  void ClearTo(int64_t frame_id);

 private:
  std::vector<std::unique_ptr<RtpFrameObject>> AssembleFrames(
      video_coding::PacketBuffer::InsertResult insert_result);
  std::vector<std::unique_ptr<RtpFrameObject>> FindReferences(
      std::vector<std::unique_ptr<RtpFrameObject>> frames);
  std::vector<std::unique_ptr<RtpFrameObject>> UpdateWithPadding(
      uint16_t seq_num);
  void SaveFrameIdToSeqNumMapping(
