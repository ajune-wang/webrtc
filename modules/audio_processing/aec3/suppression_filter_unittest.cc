...
TEST(SuppressionFilter, ComfortNoiseInUnityGain) {
  ...
  for (size_t band = 0; band < e.NumBands(); ++band) {
    for (size_t channel = 0; channel < e.NumChannels(); ++channel) {
      const float* e_data = e.FloatArray(band, channel);
      const float* e_ref_data = e_ref.FloatArray(band, channel);
      for (size_t sample = 0; sample < kBlockSize; ++sample) {
        EXPECT_EQ(e_ref_data[sample], e_data[sample]);
      }
    }
  }
}
  ...
  for (size_t k = 0; k < 100; ++k) {
    for (size_t band = 0; band < kNumBands; ++band) {
      for (size_t channel = 0; channel < kNumChannels; ++channel) {
        float* e_data = e.FloatArray(band, channel);
        for (size_t sample = 0; sample < kBlockSize; ++sample) {
          e_data[sample] = k * kBlockSize + sample + channel;
        }
      }
    }
