...
RtpFrameReferenceFinder::FrameDecision RtpFrameReferenceFinder::ManageFrameH264(
  ...
  if (!rtp_frame_marking) {
    RTC_LOG(LS_WARNING)
        << "Failed to get frame marking extension from frame, dropping frame.";
    return kDrop;
  }

  uint8_t tid = rtp_frame_marking->temporal_id;
  uint8_t tl0id = rtp_frame_marking->tl0_pic_index;
  ...
  if (frame->frame_type() == kVideoFrameKey) {
    ...
    if (last_seq_num_gop_.empty()) {
      last_seq_num_gop_.insert(std::make_pair(
          0, std::make_pair(frame->picture_id, frame->picture_id)));
    }
  }
  ...
  // Clean up info about not yet received frames that are too old.
  uint16_t old_picture_id = frame->picture_id - kMaxNotYetReceivedFrames * 2;
  ...
  for (uint8_t layer = 0; layer <= tid; ++layer) {
    ...
    if (AheadOf<uint16_t>(layer_info_it->second[layer], frame->picture_id))
      return kDrop;

    // Stash and wait for missing frame between this frame and the reference
    auto not_received_seq_num_it =
        not_yet_received_seq_num_.upper_bound(layer_info_it->second[layer]);
    if (not_received_seq_num_it != not_yet_received_seq_num_.end() &&
        AheadOf<uint16_t>(frame->picture_id, *not_received_seq_num_it)) {
      return kStash;
    }

    if (!(AheadOf<uint16_t>(frame->picture_id, layer_info_it->second[layer]))) {
      RTC_LOG(LS_WARNING) << "Frame with picture id " << frame->picture_id
                          << " and packet range [" << frame->first_seq_num()
                          << ", " << frame->last_seq_num()
                          << "] already received, "
...
void RtpFrameReferenceFinder::UpdateLayerInfoH264(
  ...
  while (layer_info_it != layer_info_.end()) {
    ...
    layer_info_it->second[temporal_index] = frame->picture_id;
    ++tl0id;
...
void RtpFrameReferenceFinder::UpdateDataH264(
  ...
  if (AheadOf<uint16_t>(frame->picture_id, last_pic_id)) {
    seq_num_it->second.first = frame->picture_id;
    seq_num_it->second.second = frame->picture_id;
  }
