...
class RtcEventLogSession
 ...
 public:
  RtcEventLogSession()
    ...
    clock_.SetTime(Timestamp::Microseconds(prng_.Rand<uint32_t>()));
...
void RtcEventLogSession::WriteAudioRecvConfigs(size_t audio_recv_streams,
  ...
  for (size_t i = 0; i < audio_recv_streams; i++) {
    clock_.AdvanceTime(TimeDelta::Milliseconds(prng_.Rand(20)));
    do {
...
TEST_P(RtcEventLogCircularBufferTest, KeepsMostRecentEvents) {
  ...
  for (size_t i = 0; i < kNumEvents; i++) {
    ...
    fake_clock->AdvanceTime(TimeDelta::Milliseconds(10));
  }
  ...
  fake_clock->SetTime(Timestamp::Microseconds(first_timestamp_us));
