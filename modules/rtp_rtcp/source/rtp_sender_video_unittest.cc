...
TEST_P(RtpSenderVideoTest, SendsDependencyDescriptorWhenVideoStructureSet) {
  const int64_t kFrameId = 100000;
  uint8_t kFrame[100];
  rtp_module_->RegisterRtpHeaderExtension(
      RtpDependencyDescriptorExtension::kUri, kGenericDescriptorId02);
  auto video_structure = std::make_unique<FrameDependencyStructure>();
  ...
  rtp_sender_video_.SetVideoStructure(std::move(video_structure));
  ...
  generic.decode_target_indications = {DecodeTargetIndication::kRequired,
...
TEST_P(RtpSenderVideoTest,
       SetDiffentVideoStructureAvoidsCollisionWithPrevious) {
  const int64_t kFrameId = 100000;
  uint8_t kFrame[100];
  rtp_module_->RegisterRtpHeaderExtension(
      RtpDependencyDescriptorExtension::kUri, kGenericDescriptorId02);
  auto video_structure1 = std::make_unique<FrameDependencyStructure>();
  video_structure1->num_decode_targets = 2;
  video_structure1->templates = {
      GenericFrameInfo::Builder().S(0).T(0).Dtis("SS").Build(),
      GenericFrameInfo::Builder().S(0).T(1).Dtis("D-").Build(),
  };
  auto video_structure2 = std::make_unique<FrameDependencyStructure>();
  ...
  generic.decode_target_indications = {DecodeTargetIndication::kSwitch,
                                       DecodeTargetIndication::kSwitch};
  hdr.frame_type = VideoFrameType::kVideoFrameKey;
  rtp_sender_video_.SetVideoStructure(std::move(video_structure1));
  rtp_sender_video_.SendVideo(kPayload, kType, kTimestamp, 0, kFrame, nullptr,
                              hdr, kDefaultExpectedRetransmissionTimeMs);
  // Parse 1st extension.
  ASSERT_EQ(transport_.packets_sent(), 1);
  ...
  RtpPacket delta_packet1 = transport_.last_sent_packet();
  ...
  generic.decode_target_indications = {DecodeTargetIndication::kSwitch,
                                       DecodeTargetIndication::kSwitch};
  hdr.frame_type = VideoFrameType::kVideoFrameKey;
  rtp_sender_video_.SetVideoStructure(std::move(video_structure2));
  rtp_sender_video_.SendVideo(kPayload, kType, kTimestamp, 0, kFrame, nullptr,
                              hdr, kDefaultExpectedRetransmissionTimeMs);
  ...
  // Try to parse the 1st delta frame. It should parseble using structure from
  // the 1st key frame, but not using the 2nd key frame.
  DependencyDescriptor descriptor_delta1;
  EXPECT_TRUE(delta_packet1.GetExtension<RtpDependencyDescriptorExtension>(
      descriptor_key1.attached_structure.get(), &descriptor_delta1));
  DependencyDescriptor descriptor_delta2;
  EXPECT_FALSE(delta_packet1.GetExtension<RtpDependencyDescriptorExtension>(
      descriptor_key2.attached_structure.get(), &descriptor_delta2));
}
