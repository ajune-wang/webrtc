...
void VideoReceiveStream::HandleEncodedFrame(
  ...
  DispatchEncodedFrame(std::move(frame));
}
...
void VideoReceiveStream::DispatchEncodedFrame(
    std::unique_ptr<video_coding::EncodedFrame> frame) {
...
VideoReceiveStream::EncodedFrameDispatchState
VideoReceiveStream::SetEncodedFrameDispatchState(
    EncodedFrameDispatchState state,
    bool generate_key_frame) {
  rtc::Event event;
  EncodedFrameDispatchState old_state;
  decode_queue_.PostTask([this, &event, &old_state, generate_key_frame,
    ...
    old_state.keyframe_generation_requested = keyframe_generation_requested_;
    ...
    if (generate_key_frame) {
      ...
      keyframe_generation_requested_ = state.keyframe_generation_requested;
    }
    event.Set();
  });
