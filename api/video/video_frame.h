...
class RTC_EXPORT VideoFrame {
 public:
  ...
  // Deprecated.
  UpdateRect update_rect() const { return update_rect_or_full_frame(); }
 ...
 private:
  ...
  absl::optional<ColorSpace> color_space_;
  // Updated since the last frame area. If present it means that all the changes
  // are contained within the rectangular area. If absent, it means that there's
  // no informaiton about the change at all.
  absl::optional<UpdateRect> update_rect_;
