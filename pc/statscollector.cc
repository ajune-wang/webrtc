...
void SetAudioProcessingStats(
    ...
    rtc::Optional<int32_t> echo_return_loss,
    rtc::Optional<int32_t> echo_return_loss_enhancement,
  ...
  if (residual_echo_likelihood && *residual_echo_likelihood >= 0.0f) {
...
void ExtractStats(const cricket::VoiceSenderInfo& info, StatsReport* report) {
  ...
  SetAudioProcessingStats(
      ...
      rtc::Optional<int32_t>(info.echo_return_loss),
      rtc::Optional<int32_t>(info.echo_return_loss_enhancement),
      rtc::Optional<int32_t>(info.echo_delay_median_ms),
      rtc::Optional<float>(info.aec_quality_min),
      rtc::Optional<int32_t>(info.echo_delay_std_ms),
      rtc::Optional<float>(info.residual_echo_likelihood),
      rtc::Optional<float>(info.residual_echo_likelihood_recent_max));
...
void StatsCollector::UpdateStatsFromExistingLocalAudioTracks(
  ...
  for (const auto& it : local_audio_tracks_) {
    ...
    report->set_timestamp(stats_gathering_started_);
    UpdateReportFromAudioTrack(track, report, has_recv_streams);
  }
}
...
void StatsCollector::UpdateReportFromAudioTrack(AudioTrackInterface* track,
  ...
  if (audio_processor.get()) {
    AudioProcessorInterface::AudioProcessorStatistics stats =
        audio_processor->GetStats(has_recv_streams);
