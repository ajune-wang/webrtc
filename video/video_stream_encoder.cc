...
void VideoStreamEncoder::EncodeVideoFrame(const VideoFrame& video_frame,
  ...
  if (!is_buffer_type_supported) {
    ...
    RTC_LOG(LS_ERROR) << __func__ << " zlma begin one frame, encoder name:"
                      << info.implementation_name;
    if (info.implementation_name != "libvpx" ||
        !out_frame.video_frame_buffer()->ToNV12()) {
      RTC_LOG(LS_ERROR) << __func__ << " zlma i420 mode";
      ...
      RTC_LOG(LS_ERROR) << __func__ << " zlma nv12 mode";
      rtc::scoped_refptr<NV12BufferInterface> converted_nv12_buffer(
          out_frame.video_frame_buffer()->ToNV12());
      RTC_LOG(LS_ERROR) << __func__ << " zlma nv12 mode";
      if (!update_rect.IsEmpty() &&
          out_frame.video_frame_buffer()->ToNV12() == nullptr) {
        update_rect =
            VideoFrame::UpdateRect{0, 0, out_frame.width(), out_frame.height()};
      }
  ...
  RTC_LOG(LS_ERROR) << __func__ << " zlma start encode one frame";
