...
class RtpPayloadParams final {
 ...
 private:
  ...
  // TODO: !!! This comment.
  // Maps a dependency-identifier to the last shared frame ID associated with
  // that identifier. RtpPayloadParams does not care what the semantic meaning
  // of the identifier is, but at the time when this code was written,
  // software encoders used the codec buffer index as the ID, and hardware
  // encoders used a combination of the spatial and temporal layer indices.
  std::array<int64_t, RtpGenericFrameDescriptor::kMaxNumFrameDependencies>
      buffer_id_to_last_shared_frame_id_;
