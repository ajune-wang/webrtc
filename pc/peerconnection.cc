...
void PeerConnection::UpdateRemoteStreamsList(
  ...
  while (track_it != current_tracks->end()) {
    ...
    bool track_exists = params && params->id == info.sender_id;
    ...
    if ((info.stream_label == kDefaultStreamLabel && default_track_needed) ||
      ...
      OnRemoteTrackRemoved(info.stream_label, info.sender_id, media_type);
...
void PeerConnection::UpdateLocalTracks(
  ...
  while (track_it != current_tracks->end()) {
    ...
    if (!params || params->id != info.sender_id ||
      ...
      OnLocalTrackRemoved(info.stream_label, info.sender_id, info.ssrc,
                          media_type);
      track_it = current_tracks->erase(track_it);
    } else {
      ++track_it;
  ...
  for (const cricket::StreamParams& params : streams) {
    ...
    uint32_t ssrc = params.first_ssrc();
    const TrackInfo* track_info =
        FindTrackInfo(*current_tracks, stream_label, sender_id);
    if (!track_info) {
      current_tracks->push_back(TrackInfo(stream_label, sender_id, ssrc));
      OnLocalTrackSeen(stream_label, sender_id, params.first_ssrc(),
                       media_type);
    }
  }
}
