...
void StreamState::PauseForPeer(size_t peer) {
...
void StreamState::ResumeForPeer(size_t peer) {
  auto it = pausable_state_.find(peer);
  RTC_CHECK(it != pausable_state_.end())
      << "No pausable state for receiver " << peer;
  it->second.Resume();
...
bool StreamState::IsPausedForPeer(size_t peer) const {
  auto it = pausable_state_.find(peer);
  RTC_CHECK(it != pausable_state_.end())
      << "No pausable state for receiver " << peer;
...
bool StreamState::WasPausedForPeerAt(size_t peer, Timestamp time) const {
  auto it = pausable_state_.find(peer);
  RTC_CHECK(it != pausable_state_.end())
      << "No pausable state for receiver " << peer;
  return it->second.WasPausedAt(time);
}

bool StreamState::WasResumedForPeerAfter(size_t peer, Timestamp time) const {
  auto it = pausable_state_.find(peer);
  RTC_CHECK(it != pausable_state_.end())
      << "No pausable state for receiver " << peer;
  return it->second.WasResumedAfter(time);
}

TimeDelta StreamState::GetActiveDurationForPeerFrom(size_t peer,
                                                    Timestamp time) const {
  auto it = pausable_state_.find(peer);
  RTC_CHECK(it != pausable_state_.end())
      << "No pausable state for receiver " << peer;
  return it->second.GetActiveDurationFrom(time);
}

Timestamp StreamState::GetLastEventTimeForPeer(size_t peer) const {
  auto it = pausable_state_.find(peer);
  RTC_CHECK(it != pausable_state_.end())
      << "No pausable state for receiver " << peer;
  return it->second.GetLastEventTime();
}
