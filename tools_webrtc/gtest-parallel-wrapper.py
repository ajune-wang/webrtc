...
def ParseArgs(argv=None):
  ...
  parser.add_argument('--no-sandbox', help=argparse.SUPPRESS)

  args, unrecognized_args = parser.parse_known_args(argv)

  gtest_parallel_args = unrecognized_args
  if args.output_dir:
    gtest_parallel_args += ['--output_dir', args.output_dir]
    os.makedirs(args.output_dir)
  if args.isolated_script_test_output:
    gtest_parallel_args += [
        '--dump_json_test_results', args.isolated_script_test_output
    ]
...
def main():
  ...
  exit_code = subprocess.call(command, cwd=os.getcwd())
