...
TEST_F(FullStackTest, DualStreams_ModeratelyRestricted_Screenshare) {
  VideoQualityTest::Params dual_streams;
  dual_streams.call.send_side_bwe = true;
  dual_streams.call.dual_video = true;
  dual_streams.video[0] = {true,   1850,    1110,  5,     50000,
                           200000, 2000000, false, "VP8", 2,
                           1,      400000,  false, false, ""};
  dual_streams.screenshare[0] = {true, false, 10};

  dual_streams.video[1] = {true,    352,     288,   30,    30000,
                           2000000, 2000000, false, "VP8", 1,
                           0,       0,       false, false, "foreman_cif"};
  ...
  dual_streams.pipe.link_capacity_kbps = 2000;
...
// Same parameters as in DualStreams_ModeratelyRestricted_Screenshare, but
// the order of streams is reversed, so the another one is analyzed instead.
TEST_F(FullStackTest, DualStreams_ModeratelyRestricted_Video) {
  VideoQualityTest::Params dual_streams;
  dual_streams.call.send_side_bwe = true;
  dual_streams.call.dual_video = true;
  dual_streams.video[1] = {true,   1850,    1110,  5,     50000,
                           200000, 2000000, false, "VP8", 2,
                           1,      400000,  false, false, ""};
  dual_streams.screenshare[1] = {true, false, 10};

  dual_streams.video[0] = {true,    352,     288,   30,    30000,
                           2000000, 2000000, false, "VP8", 1,
                           0,       0,       false, false, "foreman_cif"};

  dual_streams.analyzer = {"dualstreams_moderately_restricted_video", 0.0, 0.0,
                           kFullStackTestDurationSecs};
  dual_streams.pipe.loss_percent = 1;
  dual_streams.pipe.link_capacity_kbps = 2000;
  dual_streams.pipe.queue_length_packets = 30;
