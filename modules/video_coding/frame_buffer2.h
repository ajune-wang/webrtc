...
class FrameBuffer {
 ...
 private:
  ...
  void HandleNewContinousFrame() RTC_RUN_ON(task_queue_);

  // Check that the references of |frame| are valid.
  static bool ValidReferences(const EncodedFrame& frame);
  ...
  static EncodedFrame* CombineAndDeleteFrames(
  ...
  Clock* const clock_;
  RepeatingTaskHandle frame_wait_task_ RTC_GUARDED_BY(task_queue_);
  int64_t latest_return_time_ms_ RTC_GUARDED_BY(task_queue_);
  bool pending_frame_ RTC_GUARDED_BY(task_queue_) = false;
