...
void PeerConnectionE2EQualityTest::SetupCallOnSignalingThread(
  ...
  ForcePeerCodecs(alice_.get(), run_params);
  ForcePeerCodecs(bob_.get(), run_params);
...
void PeerConnectionE2EQualityTest::ForcePeerCodecs(
    TestPeer* peer,
    const RunParams& run_params) {
  std::vector<RtpCodecCapability> video_capabilities = ForceVideoCodec(
      run_params.video_codec_name, run_params.video_codec_required_params,
      run_params.ulpfec, run_params.flexfec,
      peer->pc_factory()
