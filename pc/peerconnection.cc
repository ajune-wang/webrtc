...
RTCError PeerConnection::UpdateTransceiversAndDataChannels(
  ...
  for (size_t i = 0; i < new_contents.size(); ++i) {
    ...
    if (!cricket::IsMediaContent(&new_content)) {
      continue;
    }
    const auto* new_media_description =
        static_cast<const cricket::MediaContentDescription*>(
            new_content.description);
    cricket::MediaType media_type = new_media_description->type();
    ...
    if (media_type == cricket::MEDIA_TYPE_AUDIO ||
      ...
      if (type == SdpType::kPrAnswer || type == SdpType::kAnswer) {
        transceiver->internal()->set_current_direction(
            new_media_description->direction());
      }
      if (source == cricket::CS_REMOTE && new_content.rejected &&
          !transceiver->stopped()) {
        transceiver->Stop();
      }
      RTCError error =
