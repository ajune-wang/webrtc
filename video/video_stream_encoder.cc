...
VideoStreamEncoder::VideoStreamEncoder(
    ...
    : shutdown_event_(true /* manual_reset */, false),
      ...
      resource_adaptation_module_(std::make_unique<OveruseFrameDetectorResourceAdaptationModule>(std::move(overuse_detector))),
...
void VideoStreamEncoder::Stop() {
  ...
  encoder_queue_.PostTask([this] {
    ...
    resource_adaptation_module_->set_is_quality_scaler_enabled(false);
    shutdown_event_.Set();
  });
...
void VideoStreamEncoder::SetSource(
  ...
  encoder_queue_.PostTask([this, degradation_preference] {
    ...
    if (!IsFramerateScalingEnabled(degradation_preference) &&
      ...
      resource_adaptation_module_->OnTargetFramerateUpdated(max_framerate_);
    }
  });
}
...
void VideoStreamEncoder::ReconfigureEncoder() {
  ...
  resource_adaptation_module_->OnTargetFramerateUpdated(target_framerate);
...
void VideoStreamEncoder::ConfigureQualityScaler(
  ...
  const bool quality_scaling_allowed =
      IsResolutionScalingEnabled(resource_adaptation_module_->degradation_preference()) &&
    ...
    if (quality_scaler_ == nullptr) {
      ...
      quality_scaler_ = std::make_unique<QualityScaler>(
          ...
          experimental_thresholds ? *experimental_thresholds
                                  : *(scaling_settings.thresholds));
      resource_adaptation_module_->set_is_quality_scaler_enabled(true);
  ...
  if (resource_adaptation_module_->degradation_preference() == DegradationPreference::BALANCED &&
  ...
  encoder_stats_observer_->OnAdaptationChanged(
      ...
      resource_adaptation_module_->GetActiveCounts(kCpu),
      resource_adaptation_module_->GetActiveCounts(kQuality));
}
...
void VideoStreamEncoder::MaybeEncodeVideoFrame(const VideoFrame& video_frame,
  ...
  if (DropDueToSize(video_frame.size())) {
    ...
    int fps_count = resource_adaptation_module_->GetConstAdaptCounter().FramerateCount(kQuality);
    int res_count = resource_adaptation_module_->GetConstAdaptCounter().ResolutionCount(kQuality);
    AdaptDown(kQuality);
    if (resource_adaptation_module_->degradation_preference() == DegradationPreference::BALANCED &&
        resource_adaptation_module_->GetConstAdaptCounter().FramerateCount(kQuality) > fps_count) {
      // Adapt framerate in same step as resolution.
      AdaptDown(kQuality);
    }
    if (resource_adaptation_module_->GetConstAdaptCounter().ResolutionCount(kQuality) > res_count) {
  ...
  if (!quality_rampup_done_ && TryQualityRampup(now_ms) &&
      resource_adaptation_module_->GetConstAdaptCounter().ResolutionCount(kQuality) > 0 &&
      resource_adaptation_module_->GetConstAdaptCounter().TotalCount(kCpu) == 0) {
...
bool VideoStreamEncoder::AdaptDown(AdaptReason reason) {
  // TODO(hbos): This is not a valid code path, it is only exposed for the sake
  // of testing. Make sure it is never triggered outside of testing.
...
void VideoStreamEncoder::CheckForAnimatedContent(
  ...
  if (!automatic_animation_detection_experiment_.enabled ||
      ...
      resource_adaptation_module_->degradation_preference() != DegradationPreference::BALANCED) {
    return;
  }
