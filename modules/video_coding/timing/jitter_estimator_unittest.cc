...
TEST_F(JitterEstimatorTest, EmptyFieldTrialsParsesToUnsetConfig) {
  ...
  EXPECT_FALSE(config.median_frame_size);
  ...
  EXPECT_FALSE(config.kalman_observation_noise_remodel);
}
...
TEST_F(FieldTrialsOverriddenJitterEstimatorTest, FieldTrialsParsesCorrectly) {
  ...
  EXPECT_TRUE(config.median_frame_size);
  ...
  EXPECT_TRUE(config.kalman_observation_noise_remodel);
}
