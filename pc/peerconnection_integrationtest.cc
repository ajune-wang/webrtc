...
// Basic end-to-end test, without SSRC signaling. This tests that we correctly
// handle streams that are signaled with a=msid lines, but without a=ssrc lines.
TEST_F(PeerConnectionIntegrationTestUnifiedPlan,
  ...
  media_expectations.ExpectBidirectionalAudio();
  ASSERT_TRUE(ExpectNewFrames(media_expectations));
  // We also want to make sure that the stream id was signaled properly without
  // ssrc information.
  auto streams = callee()->pc()->GetReceivers()[0]->streams();
  ASSERT_TRUE(!streams.empty());
  EXPECT_EQ(streams[0]->id(), kStreamId);
}
