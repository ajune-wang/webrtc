...
DisplaySurfaceCGImage::DisplaySurfaceCGImage(rtc::ScopedCFTypeRef<CGImageRef> cg_image)
    : cg_image_(cg_image),
      width_(CGImageGetWidth(cg_image.get())),
      height_(CGImageGetHeight(cg_image.get())),
      bytes_per_pixel_(CGImageGetBitsPerPixel(cg_image.get()) / 8),
      bytes_per_row_(CGImageGetBytesPerRow(cg_image.get())) {
