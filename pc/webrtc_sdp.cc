...
static void AddAttributeLine(absl::string_view attribute,
  ...
  // XXX Get string_view from string builder?
...
static bool GetValueFromString(absl::string_view line,
                               ...
                               SdpParseError* error) {
  uint32_t value;
  // SimpleAtoi doesn't support uint16_t.
...
bool ParseCandidate(absl::string_view message,
  ...
  std::string first_line = std::string(message);  // XXX this copy needed? And
                                                  // why is return value from
                                                  // GetLine not checked?
  size_t pos = 0;
