...
RnnVadFft::RnnVadFft(size_t frame_size)
    : frame_size_(frame_size),
      ...
      fft_input_buf_(1, fft_length_, RealFourier::kFftBufferAlignment),
      fft_output_buf_(1, num_fft_points_, RealFourier::kFftBufferAlignment) {
  RTC_DCHECK_EQ(0, frame_size_ % 2);
  RTC_DCHECK_EQ(fft_length_ / 2 + 1, num_fft_points_);
  RTC_DCHECK_EQ(half_window_.size() * 2, frame_size_);
  std::memset(fft_input_buf_.Row(0), 0.f,
              fft_input_buf_.cols() * sizeof(float));
}
...
void RnnVadFft::CopyOutput(rtc::ArrayView<std::complex<float>> dst) {
  ...
  std::memcpy(dst.data(), fft_output_buf_.Row(0),
