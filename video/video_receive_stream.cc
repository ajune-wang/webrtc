 ...
 public:
  explicit EncodedFrameForMediaTransport(
    ...
    *static_cast<class EncodedImage*>(this) = frame.encoded_image();
    ...
    _length = _size;
    memcpy(_buffer, frame.encoded_image()._buffer, _size);

    // TODO(nisse): Receive pipeline needs payload type in order to look up the
    // decoder. Abuse codec-type field for this. Consider changing
    // MediaTransportEncodedVideoFrame to also carry a payload type (i.e.,
    // arbitrary id mapped to actual codec elsewhere).

    _payloadType = static_cast<uint8_t>(frame.codec_type());
...
void VideoReceiveStream::OnKeyFrameRequested(uint64_t channel_id) {
  // TODO(nisse): Unclear of this method makes sense on the receiver.
}
