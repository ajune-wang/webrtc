...
TEST_P(PeerConnectionRtpTest, AddSenderWithoutStreamAddsReceiver) {
  auto caller = CreatePeerConnection();
  auto callee = CreatePeerConnection();
  ...
  if (sdp_semantics_ == SdpSemantics::kPlanB) {
    // Since we are not supporting the no stream case with Plan B, there should
    // be a generated stream, even though we didn't set one with AddTrack.
    ASSERT_EQ(receiver_added->streams().size(), 1u);
    EXPECT_TRUE(receiver_added->streams()[0]->FindAudioTrack("audio_track"));
  }
}

TEST_P(PeerConnectionRtpTest, AddSenderWithStreamAddsReceiver) {
  auto caller = CreatePeerConnection();
  auto callee = CreatePeerConnection();
...
TEST_P(PeerConnectionRtpTest, RemoveSenderWithoutStreamRemovesReceiver) {
  auto caller = CreatePeerConnection();
  auto callee = CreatePeerConnection();
...
TEST_P(PeerConnectionRtpTest, RemoveSenderWithStreamRemovesReceiver) {
  auto caller = CreatePeerConnection();
  auto callee = CreatePeerConnection();
  ...
  if (sdp_semantics_ == SdpSemantics::kUnifiedPlan) {
    ...
    // With Plan B the receive is removed.
    EXPECT_EQ(0u, callee->pc()->GetReceivers().size());
  }
}

TEST_P(PeerConnectionRtpTest, RemoveSenderWithSharedStreamRemovesReceiver) {
  auto caller = CreatePeerConnection();
  auto callee = CreatePeerConnection();
