...
std::unique_ptr<ReceiveStatistics> ReceiveStatistics::CreateUnlocked(
    Clock* clock) {
...
ReceiveStatisticsImpl::ReceiveStatisticsImpl(
    ...
    : clock_(clock),
      stream_statitician_factory_(std::move(stream_statitician_factory)),
...
StreamStatisticianImplInterface* ReceiveStatisticsImpl::GetOrCreateStatistician(
  ...
  if (!impl) {  // new element
    impl = stream_statitician_factory_(ssrc, clock_, max_reordering_threshold_);
  }
  return impl.get();
}
