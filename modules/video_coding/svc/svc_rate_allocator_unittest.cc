...
TEST(SvcRateAllocatorTest, FindLayerTogglingThreshold) {
  ...
  // layers to be enabled respectively, using the config from
  // Configure(kVideoCodecVP9, ) with 1280x720 resolution and three spatial
  // layers.
...
TEST_P(SvcRateAllocatorTestParametrizedContentType, TwoTemporalLayersAv1) {
  VideoCodec codec = Configure(kVideoCodecAV1, 1280, 720, 1, 2,
                               /*is_screen_sharing=*/GetParam());
  SvcRateAllocator allocator = SvcRateAllocator(codec);
  VideoBitrateAllocation allocation =
      allocator.Allocate(VideoBitrateAllocationParameters(
...
TEST_P(SvcRateAllocatorTestParametrizedContentType, ThreeTemporalLayersAv1) {
  VideoCodec codec = Configure(kVideoCodecAV1, 1280, 720, 1, 3,
                               /*is_screen_sharing=*/GetParam());
  SvcRateAllocator allocator = SvcRateAllocator(codec);
  VideoBitrateAllocation allocation =
      allocator.Allocate(VideoBitrateAllocationParameters(
...
TEST(SvcRateAllocatorTestParametrizedContentType,
     MutlipleSpatialAndTemporalLayersAv1) {
  VideoCodec codec =
      Configure(kVideoCodecAV1, 1280, 720, 2, 3, /*is_screen_sharing=*/false);
  SvcRateAllocator allocator = SvcRateAllocator(codec);
  VideoBitrateAllocation allocation =
      allocator.Allocate(VideoBitrateAllocationParameters(
          /*total_bitrate_bps=*/1024'000, /*framerate=*/30));
  ...
  EXPECT_EQ(allocation.GetBitrate(/*spatial_index=*/0, /*temporal_index=*/2),
            107879u);
  EXPECT_EQ(allocation.GetBitrate(/*spatial_index=*/1, /*temporal_index=*/0),
            356623u);
  EXPECT_EQ(allocation.GetBitrate(/*spatial_index=*/1, /*temporal_index=*/1),
            107879u);
  EXPECT_EQ(allocation.GetBitrate(/*spatial_index=*/1, /*temporal_index=*/2),
            196143u);
}
