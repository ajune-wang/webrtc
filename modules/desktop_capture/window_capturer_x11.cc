...
WindowCapturerLinux::WindowCapturerLinux(const DesktopCaptureOptions& options)
  ...
  if (XCompositeQueryExtension(display(), &event_base, &error_base) &&
    ...
    NLOG(LS_INFO, "Xcomposite extension not available or too old.");
  }
...
bool WindowCapturerLinux::FocusOnSelectedSource() {
  ...
  if (status == 0) {
    NLOG(LS_ERROR, "Failed to query for the root window.");
    return false;
  }
...
void WindowCapturerLinux::CaptureFrame() {
  if (!x_server_pixel_buffer_.IsWindowValid()) {
    NLOG(LS_INFO, "The window is no longer valid.");
  ...
  if (!has_composite_extension_) {
    ...
    NLOG(LS_INFO, "No Xcomposite extension detected.");
    callback_->OnCaptureResult(Result::ERROR_PERMANENT, nullptr);
    return;
  }
    ...
    if (xce.window == selected_window_) {
      if (!DesktopRectFromXAttributes(xce).equals(
        ...
        if (!x_server_pixel_buffer_.Init(display(), selected_window_)) {
          NLOG(LS_ERROR, "Failed to initialize pixel buffer after resizing.");
        }
      }
    }
  }
