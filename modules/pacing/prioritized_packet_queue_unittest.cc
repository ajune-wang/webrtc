...
TEST(PrioritizedPacketQueue, ClearsPackets) {
  ...
  // Add one packet of each type, all using the same SSRC.
  for (size_t i = 0; i < kNumMediaTypes; ++i) {
    queue.Push(now, CreatePacket(static_cast<RtpPacketMediaType>(i), /*seq=*/i,
                                 kSsrc));
  }
  EXPECT_EQ(queue.SizeInPackets(), int{kNumMediaTypes});
