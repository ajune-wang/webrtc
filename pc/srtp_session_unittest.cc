...
TEST_F(SrtpSessionTest, ProtectUnprotectWrapAroundRocMismatch) {
  ...
  // failures when it wraps around with packet loss.
  // See packet_sequencer.cc too.
  EXPECT_TRUE(s1_.SetSend(kSrtpAes128CmSha1_80, kTestKey1, kTestKeyLen,
                          kEncryptedHeaderExtensionIds));
  EXPECT_TRUE(s2_.SetRecv(kSrtpAes128CmSha1_80, kTestKey1, kTestKeyLen,
                          kEncryptedHeaderExtensionIds));
  ...
  // The way this is encrypted means there is a sequence number rollover.
  ...
  // Pick your starting sequence number in the lower half of the range
}
