...
ParserBase::ParserBase(void* ref, std::vector<MemberParameter> members) {
  fields_.reserve(members.size());
  std::sort(members.begin(), members.end(),
            [](const MemberParameter& a, const MemberParameter& b) {
              return static_cast<bool>(a.key < b.key);
            });
  for (const MemberParameter& member : members) {
    fields_.push_back(StructFieldEntry{
        member.key,
        static_cast<size_t>(reinterpret_cast<const char*>(member.member_ptr) -
