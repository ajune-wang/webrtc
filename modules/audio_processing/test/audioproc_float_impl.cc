...
int RunSimulation(AudioProcessingProvider ap_builder, int argc, char* argv[]) {
  ...
  if constexpr (std::is_same_v<
                    ...
                    std::unique_ptr<BuiltinAudioProcessingBuilder>>) {
    PerformBasicParameterSanityChecks(settings,
                                      /*builtin_builder_provided=*/true);
    SetDependencies(settings, *ap_builder);
  } else {
    PerformBasicParameterSanityChecks(settings,
                                      /*builtin_builder_provided=*/false);
  }
