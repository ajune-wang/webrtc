...
TEST_P(RtpSenderVideoTest, AbsoluteCaptureTime) {
  ...
  EXPECT_TRUE(absolute_capture_time.has_value());
  EXPECT_EQ(
...
TEST_P(RtpSenderVideoTest, AbsoluteCaptureTime_IncludeCaptureClockOffset) {
  ...
  EXPECT_EQ(
      ...
      Int64MsToUQ32x32(fake_clock_.ConvertTimestampToNtpTimeInMilliseconds(
          kAbsoluteCaptureTimestampMs)));
  EXPECT_EQ(absolute_capture_time->estimated_capture_clock_offset,
            absl::optional<int64_t>(0));
}

TEST_P(RtpSenderVideoTest, AbsoluteCaptureTime_ExtensionProvided) {
  ...
  rtp_sender_video_->SendVideo(
      kPayload, kType, kTimestamp, /*capture_time_ms=*/789, kFrame, hdr,
      kDefaultExpectedRetransmissionTimeMs, kAbsoluteCaptureTime);
  ...
  EXPECT_EQ(absolute_capture_time,
            absl::optional<AbsoluteCaptureTime>(kAbsoluteCaptureTime));
}
