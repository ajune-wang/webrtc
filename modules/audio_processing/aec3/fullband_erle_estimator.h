...
class FullBandErleEstimator {
 public:
  ...
  float FullbandErleLog2() const { return erle_time_domain_log2_[0]; }
 ...
 private:
  ...
  class ErleInstantaneous {
   public:
    ErleInstantaneous(const EchoCanceller3Config::Erle& config);
    ~ErleInstantaneous();
  ...
  std::vector<std::unique_ptr<ErleInstantaneous>> instantaneous_erle_;
