...
class RtpSenderReceiverTest
 ...
 public:
  RtpSenderReceiverTest()
    ...
    std::unique_ptr<cricket::VoiceMediaSendChannelInterface>
        voice_media_send_channel = media_engine_->voice().CreateSendChannel(
    ...
    std::unique_ptr<cricket::VideoMediaSendChannelInterface>
        video_media_send_channel = media_engine_->video().CreateSendChannel(
    ...
    std::unique_ptr<cricket::VoiceMediaReceiveChannelInterface>
        voice_media_receive_channel =
            media_engine_->voice().CreateReceiveChannel(
                &fake_call_, cricket::MediaConfig(), cricket::AudioOptions(),
                webrtc::CryptoOptions(), webrtc::AudioCodecPairId::Create());
    std::unique_ptr<cricket::VideoMediaReceiveChannelInterface>
        video_media_receive_channel =
            media_engine_->video().CreateReceiveChannel(
                &fake_call_, cricket::MediaConfig(), cricket::VideoOptions(),
                webrtc::CryptoOptions());

    media_engine_->fake_voice_engine()->RegisterChannel(
        std::unique_ptr<cricket::FakeVoiceMediaSendChannel>(
            static_cast<cricket::FakeVoiceMediaSendChannel*>(
                voice_media_send_channel.release())));
    media_engine_->fake_video_engine()->RegisterChannel(
        std::unique_ptr<cricket::FakeVideoMediaSendChannel>(
            static_cast<cricket::FakeVideoMediaSendChannel*>(
                video_media_send_channel.release())));
    media_engine_->fake_voice_engine()->RegisterChannel(
        std::unique_ptr<cricket::FakeVoiceMediaReceiveChannel>(
            static_cast<cricket::FakeVoiceMediaReceiveChannel*>(
                voice_media_receive_channel.release())));
    media_engine_->fake_video_engine()->RegisterChannel(
        std::unique_ptr<cricket::FakeVideoMediaReceiveChannel>(
            static_cast<cricket::FakeVideoMediaReceiveChannel*>(
                video_media_receive_channel.release())));

    voice_media_send_channel_ = media_engine_->GetVoiceSendChannel(0);
    voice_media_receive_channel_ = media_engine_->GetVoiceReceiveChannel(0);
    video_media_send_channel_ = media_engine_->GetVideoSendChannel(0);
    video_media_receive_channel_ = media_engine_->GetVideoReceiveChannel(0);
  ...
  void CreateAudioRtpSender(
    ...
    audio_rtp_sender_->SetMediaChannel(voice_media_send_channel_);
 ...
 protected:
  ...
  cricket::FakeVoiceMediaSendChannel* voice_media_send_channel_;
  cricket::FakeVideoMediaSendChannel* video_media_send_channel_;
  cricket::FakeVoiceMediaReceiveChannel* voice_media_receive_channel_;
  cricket::FakeVideoMediaReceiveChannel* video_media_receive_channel_;
