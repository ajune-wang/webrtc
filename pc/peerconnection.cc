...
void PeerConnection::OnLocalTrackSeen(const std::string& stream_label,
                                      ...
                                      uint32_t ssrc,
                                      cricket::MediaType media_type) {
  // TODO(steveanton): Why is the track_id an RtpSender id?
  auto sender = FindSenderById(track_id);
  if (!sender) {
...
PeerConnection::FindSenderById(const std::string& track_id) const {
  for (auto transceiver : transceivers_) {
    for (auto sender : transceiver->internal()->senders()) {
      if (sender->id() == track_id) {
        return sender;
      }
    }
  }
