...
RtcpTransceiverImpl::CompoundPacketInfo RtcpTransceiverImpl::FillReports(
  ...
  size_t max_sender_reports = available_bytes / kFullSenderReportSizeBytes;

  std::vector<rtcp::ReportBlock> report_blocks = CreateReportBlocks(
      now, max_sender_reports * rtcp::SenderReport::kMaxNumberOfReportBlocks);
  ...
  for (auto it = local_senders_.begin(); it != local_senders_.end(); ++it) {
    LocalSenderState& rtp_sender = *it;
    RtpStreamRtcpHandler::RtpStats stats = rtp_sender.handler->SentStats();
    if (rtp_sender.last_sent_bytes > stats.num_sent_bytes()) {
      RTC_LOG(LS_ERROR) << "Inconsistent SR for ssrc " << rtp_sender.ssrc
                        << ": number of total bytes sent decreased.";
    }
    rtp_sender.last_sent_bytes = stats.num_sent_bytes();
  ...
  if (last_handled_sender_it != local_senders_.end()) {
    // rotate local_senders so that 1st unhandled sender become first in the
    // list, and thus will be first to generate rtcp sender report for on next
    // call to `CreateReport`.
  ...
  size_t num_receiver_reports =
      (report_blocks.end() - report_block_it +
       rtcp::ReceiverReport::kMaxNumberOfReportBlocks - 1) /
      rtcp::ReceiverReport::kMaxNumberOfReportBlocks;

  // In compund mode each report has to start with a sender or receiver report.
...
void RtcpTransceiverImpl::CreateCompoundPacket(Timestamp now,
  ...
  if (config_.non_sender_rtt_measurement) {
    reserved_bytes += rtcp::Rrtr::kLength;
  }
