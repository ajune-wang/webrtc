    ...
    : encoded_image_(),
      ...
      external_ref_ctrl_(
          !absl::StartsWith(trials.Lookup("WebRTC-Vp9ExternalRefCtrl"),
                            "Disabled")),
      speed_configs_(ParseSpeedConfigsFromTrials(trials)),
...
int VP9EncoderImpl::InitAndSetControlSettings(const VideoCodec* inst) {
  ...
  UpdateSpeedConfigs();
  RTC_DCHECK_EQ(speed_per_spatial_index_.size(),
                static_cast<size_t>(num_spatial_layers_));
  for (int si = 0; si < std::min<int>(num_spatial_layers_, 1); ++si) {
    svc_params_.speed_per_layer[si] = speed_per_spatial_index_[si].base_layer;
    svc_params_.loopfilter_ctrl[si] = speed_per_spatial_index_[si].deblock_mode;
  }
  ...
  if (is_svc_) {
    ...
    vpx_codec_control(encoder_, VP8E_SET_CPUUSED,
                      speed_per_spatial_index_[0].base_layer);
  }
  ...
  if (is_svc_) {
    ...
    for (int sl_idx = 0; sl_idx < num_spatial_layers_; ++sl_idx) {
      if (layer_id.temporal_layer_id_per_spatial[sl_idx] == 0) {
        svc_params_.speed_per_layer[sl_idx] =
            speed_per_spatial_index_[sl_idx].base_layer;
      } else {
        svc_params_.speed_per_layer[sl_idx] =
            speed_per_spatial_index_[sl_idx].high_layer;
      }
    }
    vpx_codec_control(encoder_, VP9E_SET_SVC_PARAMETERS, &svc_params_);
  }
...
void VP9EncoderImpl::UpdateSpeedConfigs() {
  const auto find_speed = [&](int min_pixel_count) {
    auto it = std::lower_bound(speed_configs_.begin(), speed_configs_.end(),
                               min_pixel_count,
                               [](const SpeedConfig& lhs, int min_pixel_count) {
                                 return lhs.min_pixel_count < min_pixel_count;
                               });
    return it == speed_configs_.begin() ? *it : *std::prev(it);
  };

  speed_per_spatial_index_.clear();
  if (is_svc_) {
    for (int si = 0; si < num_spatial_layers_; ++si) {
      speed_per_spatial_index_.push_back(find_speed(
          codec_.spatialLayers[si].width * codec_.spatialLayers[si].height));
    }
  } else {
    speed_per_spatial_index_.push_back(
        find_speed(codec_.width * codec_.height));
  }
}

// static
std::vector<VP9EncoderImpl::SpeedConfig>
VP9EncoderImpl::ParseSpeedConfigsFromTrials(
    const WebRtcKeyValueConfig& trials) {
  FieldTrialStructList<SpeedConfig> trials_list(
      {FieldTrialStructMember(
           "min_pixel_count",
           [](SpeedConfig* c) { return &c->min_pixel_count; }),
       FieldTrialStructMember("high_layer_speed",
                              [](SpeedConfig* c) { return &c->high_layer; }),
       FieldTrialStructMember("base_layer_speed",
                              [](SpeedConfig* c) { return &c->base_layer; }),
       FieldTrialStructMember("deblock_mode",
                              [](SpeedConfig* c) { return &c->deblock_mode; })},
      {});

  ParseFieldTrial({&trials_list}, trials.Lookup("WebRTC-VP9-SpeedConfig"));
  std::vector<VP9EncoderImpl::SpeedConfig> configs = trials_list;
  if (configs.empty()) {
    return GetDefaultSpeedConfig();
  }

  std::sort(configs.begin(), configs.end(),
            [](const SpeedConfig& lhs, const SpeedConfig& rhs) {
...
// static
std::vector<VP9EncoderImpl::SpeedConfig>
VP9EncoderImpl::GetDefaultSpeedConfig() {
  std::vector<VP9EncoderImpl::SpeedConfig> default_config;
