...
AudioMixer::Source::AudioFrameInfo ChannelReceive::GetAudioFrameWithInfo(
  ...
  // Store audio_frame->sample_rate_hz_ since it can change in the call to
  // neteq_->GetAudio.
  const int desired_sample_rate_hz = audio_frame->sample_rate_hz_;
  if ((neteq_->GetAudio(audio_frame) != NetEq::kOK) ||
      !resampler_helper_.MaybeResample(desired_sample_rate_hz, audio_frame)) {
    RTC_DLOG(LS_ERROR)
