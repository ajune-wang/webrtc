...
void AecState::Update(
  ...
  blocks_with_proper_filter_adaptation_ +=
      render_activity_.ActiveBlock() ? 1 : 0;
  initial_state_ =
      blocks_with_proper_filter_adaptation_ < 5 * kNumBlocksPerSecond;

  // Estimate the quality of the linear filter.
  echo_model_selector_.Update(echo_saturation_, render_activity_.ActiveBlock(),
                              converged_filter, diverged_filter);

  // Detect whether a headset is present.
  echo_path_strength_detector_.Update(delay_estimate,
                                      render_activity_.ActiveBlock(),
