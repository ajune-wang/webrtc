 ...
 protected:
  ...
  void CreateRtpTransports(RTPTransportType type,
    ...
    if (type == RTPTransportType::kRtp) {
      transport1_ = transport_controller_->CreateRtpTransport(transport_name,
                                                              kRtcpMuxEnabled);
      transport2_ = transport_controller_->CreateRtpTransport(transport_name,
                                                              kRtcpMuxEnabled);
    } else if (type == RTPTransportType::kSrtp) {
      transport1_ = transport_controller_->CreateSrtpTransport(transport_name,
                                                               kRtcpMuxEnabled);
      transport2_ = transport_controller_->CreateSrtpTransport(transport_name,
                                                               kRtcpMuxEnabled);
    }
    EXPECT_NE(nullptr, transport1_);
    // The TransportController is expected to return the existing one.
...
TEST_P(TransportControllerRTPTransportTest, CreateTransport) {
  ...
  CreateRtpTransports(type, transport_name);
...
// Test the RTP transport is not actually destroyed if there are still
// references exist.
TEST_P(TransportControllerRTPTransportTest, DestroyTransportWithReference) {
  const std::string transport_name = "transport";
  CreateRtpTransports(GetParam(), transport_name);
...
// Test the RTP is actually destroyed if there is no reference to it.
