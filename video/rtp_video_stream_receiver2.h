...
class RtpVideoStreamReceiver2 : public LossNotificationSender,
 ...
 private:
  ...
  void OnCompleteFrames(RtpFrameReferenceFinder::ReturnVector frame)
      RTC_RUN_ON(network_sequence_checker_);
  ...
  ParseGenericDependenciesResult ParseGenericDependenciesExtension(
      const RtpPacketReceived& rtp_packet,
      RTPVideoHeader* video_header) RTC_RUN_ON(network_sequence_checker_);
  ...
  bool receiving_ RTC_GUARDED_BY(network_sequence_checker_);
  int64_t last_packet_log_ms_ RTC_GUARDED_BY(network_sequence_checker_);
  ...
  std::map<uint8_t, std::map<std::string, std::string>> pt_codec_params_
      RTC_GUARDED_BY(network_sequence_checker_);
  int16_t last_payload_type_ RTC_GUARDED_BY(network_sequence_checker_) = -1;

  bool has_received_frame_ RTC_GUARDED_BY(network_sequence_checker_);
