...
bool ReadResolutions(rtc::BitBuffer* buffer,
  ...
  if (buffer->RemainingBitCount() < spatial_layers * 32 * 8) {
    return false;
  }
  ...
  for (int sid = 0; sid < spatial_layers; ++sid) {
    ...
    bool read_ok = buffer->ReadUInt16(&width_minus_1) &&
...
bool RtpDependencyDescriptorReader::ReadMandatoryFields(
  ...
  reading_state->template_id = (flags_and_template_id & 0b00111111);
...
bool RtpDependencyDescriptorReader::ReadExtendedFields(
  ...
  reading_state->custom_dtis = (value & 0b0100) != 0;
  reading_state->custom_fdiffs = (value & 0b0010) != 0;
  reading_state->custom_chains = (value & 0b0001) != 0;
...
bool RtpDependencyDescriptorReader::ReadTemplateDependencyStructure(
  ...
  if (frame_dependency_structure_ &&
      // Distance between last template of the new structure and first template
      // of the old structure.
      (reading_state->structure->structure_id +
       reading_state->structure->templates.size() -
       frame_dependency_structure_->structure_id + kMaxTemplates) %
              kMaxTemplates <
          // Should be large enough to include both template sets.
          frame_dependency_structure_->templates.size() +
              reading_state->structure->templates.size()) {
    // Reject new structure if template ids it uses overlaps with the old
    // template ids.
    // TODO(danilchap): Avoid just dropping new structure in this scenario.
    // It must be more robust to reset everything and either just use this
    // structure, specially if rtp sequence number looks newer, or request
    // another key frame.
    // It is up to the caller to decide what should be done in this case, but
    // this reader should communicate to the caller why descriptor was rejected.
    return false;
  }
