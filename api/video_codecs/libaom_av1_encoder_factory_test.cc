...
TEST(LibaomAv1Encoder, EncodeKeyframe) {
  ...
  enc->Encode(raw_frame, {.presentation_timestamp = Timestamp::Millis(0)},
              {Fb().Key().Rate(kCbr).Res(640, 360).Build()}, res.CallBack());
...
TEST(LibaomAv1Encoder, ResolutionSwitching) {
  ...
  enc->Encode(in0, {.presentation_timestamp = Timestamp::Millis(0)},
              {Fb().Rate(kCbr).Res(320, 180).Key().Build()}, res.CallBack());
...
TEST(LibaomAv1Encoder, InputResolutionSwitching) {
  ...
  rtc::scoped_refptr<I420Buffer> in0 = frame_reader->PullFrame();
  enc->Encode(in0, {.presentation_timestamp = Timestamp::Millis(0)},
              {Fb().Rate(kCbr).Res(160, 90).Key().Build()}, res.CallBack());
...
TEST(LibaomAv1Encoder, KeyframeAndStartrameAreApproximatelyEqual) {
  ...
  for (int sid = 0; sid < max_spatial_layers; ++sid) {
    ...
    enc_key->Encode(frame_in, {.presentation_timestamp = Timestamp::Millis(0)},
