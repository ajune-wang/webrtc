...
PerfTestResultWriter& GetPerfWriter() {
  if (absl::GetFlag(FLAGS_write_histogram_proto_json)) {
    static PerfTestResultWriter* logger_ = CreateHistogramWriter();
    return *logger_;
  } else {
    static PerfTestResultWriter* logger_ = CreateGraphJsonWriter();
    return *logger_;
  }
}
...
void SetPerfResultsOutput(FILE* output) {
  GetPerfWriter().SetOutput(output);
...
void PrintResult(const std::string& measurement,
  ...
  GetPerfWriter().LogResult(measurement + modifier, trace, value, units,
...
void PrintResultMeanAndError(const std::string& measurement,
                             ...
                             bool important,
                             ImproveDirection improve_direction) {
  GetPerfWriter().LogResultMeanAndError(measurement + modifier, trace, mean,
...
void PrintResultList(const std::string& measurement,
                     ...
                     bool important,
                     ImproveDirection improve_direction) {
  GetPerfWriter().LogResultList(measurement + modifier, trace, values, units,
                                important, improve_direction);
}
