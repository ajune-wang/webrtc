...
EncoderStreamFactory::CreateDefaultVideoStreams(
  ...
  //   RtpEncodingParamters, which is the encoding of is stream.
  // - `encoder_config.max_bitrate_bps` comes from SDP; "b=AS" or conditionally
  //   "x-google-max-bitrate".
  // If `api_max_bitrate` has a value then it is positive.
  absl::optional<int> api_max_bitrate;
  if (encoder_config.simulcast_layers[0].max_bitrate_bps > 0) {
    api_max_bitrate = encoder_config.simulcast_layers[0].max_bitrate_bps;
  }
  if (encoder_config.max_bitrate_bps > 0) {
    api_max_bitrate =
        api_max_bitrate.has_value()
            ? std::min(encoder_config.max_bitrate_bps, *api_max_bitrate)
