...
SctpPacket::Builder::Builder(uint32_t verification_tag,
...
absl::optional<SctpPacket> SctpPacket::Parse(
  ...
  if (data.size() < kHeaderSize + kChunkTlvHeaderSize ||
    ...
    RTC_LOG(LS_WARNING) << "Invalid packet size";
    return absl::nullopt;
  }
  ...
  CommonHeader common_header = {
      .source_port = reader.Load16<0>(),
      .destination_port = reader.Load16<2>(),
      .verification_tag = reader.Load32<4>(),
      .checksum = reader.Load32<8>(),
  };
  ...
  while (!descriptor_data.empty()) {
    if (descriptor_data.size() < kChunkTlvHeaderSize) {
      RTC_LOG(LS_WARNING) << "Too small chunk";
      return absl::nullopt;
    }
    ...
    if (padded_length > descriptor_data.size()) {
      RTC_LOG(LS_WARNING) << "Too large chunk. length=" << length
