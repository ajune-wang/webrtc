...
VideoQualityTest::Injectables::Injectables()
    : sender_network(nullptr), receiver_network(nullptr) {}

VideoQualityTest::Injectables::~Injectables() = default;
...
void VideoQualityTest::CheckParamsAndInjectables() {
  if (injectables_ == nullptr) {
    injectables_ = absl::make_unique<Injectables>();
  }
  if (!params_.config && injectables_->sender_network == nullptr &&
      injectables_->receiver_network == nullptr) {
    // TODO(titovartem) replace with default config creation when removing
    // pipe.
    params_.config = params_.pipe;
  }
  RTC_CHECK((params_.config && injectables_->sender_network == nullptr &&
             injectables_->receiver_network == nullptr) ||
            (!params_.config && injectables_->sender_network != nullptr &&
             injectables_->receiver_network != nullptr));
...
void VideoQualityTest::FillScalabilitySettings(
  ...
  if (params->ss[video_idx].streams.empty() &&
    ...
    // Validity checks performed in CheckParamsAndInjectables.
...
VideoQualityTest::CreateSendTransport() {
  ...
  if (injectables_->sender_network == nullptr) {
    simulated_network = absl::make_unique<SimulatedNetwork>(*params_.config);
  } else {
    simulated_network = std::move(injectables_->sender_network);
  }
...
VideoQualityTest::CreateReceiveTransport() {
  std::unique_ptr<NetworkSimulationInterface> simulated_network = nullptr;
  if (injectables_->receiver_network == nullptr) {
    simulated_network = absl::make_unique<SimulatedNetwork>(*params_.config);
  } else {
    simulated_network = std::move(injectables_->receiver_network);
  }
...
void VideoQualityTest::RunWithAnalyzer(
    const Params& params,
    std::unique_ptr<Injectables> injectables) {
  ...
  params_ = params;
  injectables_ = std::move(injectables);
  // TODO(ivica): Merge with RunWithRenderer and use a flag / argument to
  // differentiate between the analyzer and the renderer case.
  CheckParamsAndInjectables();
