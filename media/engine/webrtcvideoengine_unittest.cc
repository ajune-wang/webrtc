 ...
 protected:
  ...
  int GetMaxEncoderBitrate() {
    ...
    EXPECT_EQ(static_cast<size_t>(1),
              stream->GetEncoderConfig().number_of_streams);
...
TEST_F(WebRtcVideoChannelTest, ReconfiguresEncodersWhenNotSending) {
  ...
  EXPECT_EQ(static_cast<size_t>(capture_format.width), streams[0].width);
  EXPECT_EQ(static_cast<size_t>(capture_format.height), streams[0].height);
...
TEST_F(WebRtcVideoChannelTest, UsesCorrectSettingsForScreencast) {
  ...
  EXPECT_EQ(static_cast<size_t>(capture_format_hd.width),
            streams.front().width);
  EXPECT_EQ(static_cast<size_t>(capture_format_hd.height),
...
TEST_F(WebRtcVideoChannelTest, VerifyVp8SpecificSettings) {
  ...
  EXPECT_EQ(static_cast<size_t>(3), stream->GetVideoStreams().size());
  ASSERT_TRUE(stream->GetVp8Settings(&vp8_settings)) << "No VP8 config set.";
  ...
  EXPECT_EQ(static_cast<size_t>(1), stream->GetVideoStreams().size());
  ASSERT_TRUE(stream->GetVp8Settings(&vp8_settings)) << "No VP8 config set.";
  EXPECT_FALSE(vp8_settings.denoisingOn);
...
TEST_F(WebRtcVideoChannelFlexfecRecvTest,
  ...
  EXPECT_EQ(-1, config.rtp.flexfec.payload_type);
  EXPECT_EQ(static_cast<unsigned int>(0), config.rtp.flexfec.ssrc);
  EXPECT_TRUE(config.rtp.flexfec.protected_media_ssrcs.empty());
}
...
TEST_F(WebRtcVideoChannelTest,
  ...
  EXPECT_EQ(stats.rtcp_packet_type_counts.fir_packets,
            static_cast<unsigned int>(info.receivers[0].firs_sent));
  EXPECT_EQ(stats.rtcp_packet_type_counts.nack_packets,
            static_cast<unsigned int>(info.receivers[0].nacks_sent));
  EXPECT_EQ(stats.rtcp_packet_type_counts.pli_packets,
            static_cast<unsigned int>(info.receivers[0].plis_sent));
}
...
TEST_F(WebRtcVideoChannelTest, GetStatsTranslatesDecodeStatsCorrectly) {
  ...
  EXPECT_EQ(static_cast<unsigned int>(stats.frame_counts.key_frames +
...
TEST_F(WebRtcVideoChannelTest, GetStatsTranslatesReceivePacketStatsCorrectly) {
  ...
  EXPECT_EQ(stats.rtp_stats.transmitted.payload_bytes +
            ...
            static_cast<size_t>(info.receivers[0].bytes_rcvd));
  EXPECT_EQ(stats.rtp_stats.transmitted.packets,
            static_cast<unsigned int>(info.receivers[0].packets_rcvd));
...
TEST_F(WebRtcVideoChannelTest, SetRtpSendParametersPrioritySimulcastStreams) {
  ...
  EXPECT_EQ(static_cast<size_t>(kNumSimulcastStreams),
    ...
    if (conference_mode) {
      ...
      if (screenshare) {
        for (const webrtc::VideoStream& stream : expected_streams) {
          ...
          EXPECT_EQ(stream.width, static_cast<size_t>(capture_width));
          EXPECT_EQ(stream.height, static_cast<size_t>(capture_height));
        }
      }
    } else {
