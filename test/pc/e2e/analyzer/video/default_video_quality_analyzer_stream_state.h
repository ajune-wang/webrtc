...
class StreamState {
 public:
  ...
  // delays.
  void PausePeer(size_t peer);

  // Marks that peer `peer` is now expected to continue receive frames for this
  // stream.
  void ResumePeer(size_t peer);

  bool WasPausedAt(size_t peer, Timestamp time) const;

  bool WasResumedAfter(size_t peer, Timestamp time) const;
