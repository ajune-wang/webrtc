...
constexpr char kCappedProbingExperiment[] = "WebRTC-CappedProbingExperiment";
...
ProbeController::ProbeController()
    : enable_periodic_alr_probing_(false),
      ...
      in_capped_probing_experiment_(
          !webrtc::field_trial::IsDisabled(kCappedProbingExperiment)) {
  Reset(0);
}
...
std::vector<ProbeClusterConfig> ProbeController::InitiateProbing(
  ...
  for (int64_t bitrate : bitrates_to_probe) {
    ...
    if (in_capped_probing_experiment_ && max_total_allocated_bitrate_ > 0) {
