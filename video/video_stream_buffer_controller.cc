...
std::unique_ptr<VideoBufferController>
VideoBufferController::CreateFromFieldTrial(
    Clock* clock,
  ...
  switch (ParseFrameBufferFieldTrial(field_trials)) {
    case FrameBufferArm::kSyncDecode: {
      ...
      return std::make_unique<VideoBufferController>(
...
VideoBufferController::VideoBufferController(
    Clock* clock,
    TaskQueueBase* worker_queue,
    VCMTiming* timing,
    VCMReceiveStatisticsCallback* stats_proxy,
    ...
    : field_trials_(field_trials),
      ...
      timeout_tracker_(
          ...
          absl::bind_front(&VideoBufferController::OnTimeout, this)),
...
void VideoBufferController::StopOnWorker() {
...
void VideoBufferController::SetProtectionMode(
    VCMVideoProtection protection_mode) {
  RTC_DCHECK_RUN_ON(&worker_sequence_checker_);
  protection_mode_ = protection_mode;
}

void VideoBufferController::Clear() {
  RTC_DCHECK_RUN_ON(&worker_sequence_checker_);
...
absl::optional<int64_t> VideoBufferController::InsertFrame(
...
void VideoBufferController::UpdateRtt(int64_t max_rtt_ms) {
  RTC_DCHECK_RUN_ON(&worker_sequence_checker_);
  jitter_estimator_.UpdateRtt(TimeDelta::Millis(max_rtt_ms));
}
void VideoBufferController::SetMaxWaits(TimeDelta max_wait_for_keyframe,
                                        TimeDelta max_wait_for_frame) {
  RTC_DCHECK_RUN_ON(&worker_sequence_checker_);
  timeout_tracker_.SetTimeouts({.max_wait_for_keyframe = max_wait_for_keyframe,
                                .max_wait_for_frame = max_wait_for_frame});
}
void VideoBufferController::StartNextDecode(bool keyframe_required) {
...
int VideoBufferController::Size() {
  RTC_DCHECK_RUN_ON(&worker_sequence_checker_);
  return buffer_->CurrentSize();
}
void VideoBufferController::OnFrameReady(
...
void VideoBufferController::OnTimeout(TimeDelta delay) {
  RTC_DCHECK_RUN_ON(&worker_sequence_checker_);
...
void VideoBufferController::FrameReadyForDecode(uint32_t rtp_timestamp,
                                                Timestamp render_time) {
  RTC_DCHECK_RUN_ON(&worker_sequence_checker_);
...
void VideoBufferController::UpdateDroppedFrames()
...
void VideoBufferController::UpdateJitterDelay() {
...
void VideoBufferController::MaybeScheduleFrameForRelease()
    RTC_RUN_ON(&worker_sequence_checker_) {
    ...
    if (schedule) {
      ...
      if (frame_decode_scheduler_->ScheduledRtpTimestamp() !=
        ...
        frame_decode_scheduler_->ScheduleFrame(
            ...
            absl::bind_front(&VideoBufferController::FrameReadyForDecode,
                             this));
      }
      return;
    }
