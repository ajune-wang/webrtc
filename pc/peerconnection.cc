...
// would like to check against the local and remote descriptions stored from the
// last negotiation, because either of these could be the most up to date for
...
bool MediaSectionsInSameOrder(const SessionDescription& current_desc,
  ...
  for (size_t i = 0; i < current_desc.contents().size(); ++i) {
    if ((type == SdpType::kOffer) &&
        (current_desc.contents()[i].rejected ||
         (secondary_current_desc &&
          i < secondary_current_desc->contents().size() &&
          secondary_current_desc->contents()[i].rejected))) {
...
PeerConnection::AssociateTransceiver(cricket::ContentSource source,
  ...
  // the mapping between the transceiver and its m= section index. Since we only
  // recycle with offers, we dissociate with the RtpTransceiver with offers.
  if (type == SdpType::kOffer && !content.rejected &&
...
RTCError PeerConnection::ValidateSessionDescription(
  ...
  if (type == SdpType::kPrAnswer || type == SdpType::kAnswer) {
    ...
    // for a media section, but the other does not.
