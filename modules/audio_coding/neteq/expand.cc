...
int Expand::Process(AudioMultiVector* output) {
  ...
  if (first_expand_) {
    ...
    RTC_DCHECK(rand_length <= kMaxSampleRate / 8000 * 120 + 30);
  ...
  for (size_t channel_ix = 0; channel_ix < num_channels_; ++channel_ix) {
    ...
    if (current_lag_index_ == 0) {
      ...
      RTC_DCHECK(expansion_vector_position + temp_length <=
    ...
    if (channel_ix == 0) {
      ...
      RTC_DCHECK(output->Size() == current_lag);
    }
...
void Expand::AnalyzeSignal(int16_t* random_vector) {
  ...
  RTC_DCHECK(correlation_lags <= static_cast<size_t>(99 * fs_mult + 1));
  ...
  for (size_t channel_ix = 0; channel_ix < num_channels_; ++channel_ix) {
    ...
    if (channel_ix == 0) {
      ...
      if (noise_length <= RandomVector::kRandomTableSize) {
        ...
        RTC_DCHECK(noise_length <= kMaxSampleRate / 8000 * 120 + 30);
