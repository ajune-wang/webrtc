...
AsyncPacketSocket* BasicPacketSocketFactory::CreateClientTcpSocket(
    ...
    const SocketAddress& remote_address,
    const PacketSocketTcpOptions& tcp_options) {
  // TODO(bugs.webrtc.org/7447): remove when these are taken in the constructor.
  RTC_DCHECK(proxy_info_ && user_agent_)
      << "You can't call this "
      << "implementation without setting proxy_info and user_agent!";
  AsyncSocket* socket =
      socket_factory()->CreateAsyncSocket(local_address.family(), SOCK_STREAM);
  if (!socket) {
    return NULL;
  ...
  if (proxy_info_->type == PROXY_SOCKS5) {
    socket =
        new AsyncSocksProxySocket(socket, proxy_info_->address,
                                  proxy_info_->username, proxy_info_->password);
  } else if (proxy_info_->type == PROXY_HTTPS) {
    socket =
        new AsyncHttpsProxySocket(socket, *user_agent_, proxy_info_->address,
                                  proxy_info_->username, proxy_info_->password);
  }
