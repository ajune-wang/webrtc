...
void RtcEventLogEncoderTest::TestRtcEventAudioNetworkAdaptation(
  ...
  ParsedRtcEventLog::AudioNetworkAdaptationEvent ana_event =
      parsed_log_.GetAudioNetworkAdaptation(0);

  EXPECT_EQ(ana_event.timestamp, timestamp_us);
  EXPECT_EQ(ana_event.config, original_runtime_config);
}
...
TEST_P(RtcEventLogEncoderTest, RtcEventAudioPlayout) {
  ...
  ParsedRtcEventLog::AudioPlayoutEvent playout_event =
      parsed_log_.GetAudioPlayout(0);

  EXPECT_EQ(playout_event.timestamp, timestamp_us);
...
TEST_P(RtcEventLogEncoderTest, RtcEventBweUpdateLossBased) {
  ...
  ParsedRtcEventLog::BweLossBasedUpdate bwe_update =
      parsed_log_.GetLossBasedBweUpdate(0);

  EXPECT_EQ(bwe_update.timestamp, timestamp_us);
  EXPECT_EQ(bwe_update.new_bitrate, bitrate_bps);
