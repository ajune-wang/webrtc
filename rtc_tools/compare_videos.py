...
def _ParseArgs():
  ...
  parser.add_option('--yuv_directory', type='string',
                    help='Directory for output yuv files.')
  parser.add_option('--vmaf_directory', type='string',
                    help='Path to VMAF directory.')
...
def RunVmaf(options):
  ...
  """
  cmd = [
    options.vmaf_directory + 'wrapper/vmafossexec',
    'yuv420p',
    str(options.yuv_frame_width),
    str(options.yuv_frame_height),
    options.yuv_directory + options.label + "_ref.yuv",
    options.yuv_directory + options.label + "_dis.yuv",
    options.vmaf_directory + 'model/vmaf_v0.6.1.pkl',
    '--phone-model',
  ]
...
def main():
  ...
  --vmaf_directory, --yuv_directory, --yuv_frame_width, --yuv_frame_height
  ...
  * ffmpeg
  ...
  if RunFrameAnalyzer(options) != 0:
    print 'Failed to run frame analyzer.'
    return 1

  # Run VMAF for further video comparison and print output.
  if options.vmaf_directory:
    if not options.yuv_directory:
      print 'Failed to run VMAF: No YUV directory specified.'
      return 1
    return RunVmaf(options)
