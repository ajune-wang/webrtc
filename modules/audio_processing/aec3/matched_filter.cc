...
void FilterSymmetricOverlap_SSE2(rtc::ArrayView<const float> x,
  ...
  const int filter_size_by_2 = static_cast<int>(filter_size >> 1);
  const int x_size = static_cast<int>(x.size());
  ...
  for (size_t i = 0; i < y.size(); ++i) {
    int x_i = x0;
    const float* hc_p = &h[0];
    const float* x_p = &x[x0];
    __m128 x2_sum_128_j = _mm_set1_ps(0);
    __m128 e_128_j = _mm_set1_ps(0);

    int chunk1 = std::min(filter_size_by_2, x_size - x_i);
    int chunk2 = filter_size_by_2 - chunk1;

    for (int limit : {chunk1, chunk2}) {
      const int limit_by_4 = limit >> 2;
      for (int k = limit_by_4; k > 0; --k, hc_p += 4, x_p += 4) {
        const __m128 x_k = _mm_loadu_ps(x_p);
        const __m128 h_k = _mm_loadu_ps(hc_p);
        const __m128 xx = _mm_mul_ps(x_k, x_k);
        const __m128 hx = _mm_mul_ps(h_k, x_k);
      ...
      for (int k = limit - limit_by_4 * 4; k > 0; --k, ++hc_p, ++x_p) {
        float x_k = *x_p;
        x2_sum[0] += x_k * x_k;
        e[0] += *hc_p * x_k;
      }
    ...
    for (size_t j = 0; j < num_filters - 1; ++j) {
      ...
      for (int limit : {chunk1, chunk2}) {
        const int limit_by_4 = limit >> 2;
        for (int k = limit_by_4; k > 0; --k, hc_p += 8, x_p += 4) {
          const __m128 x_k = _mm_loadu_ps(x_p);
          const __m128 xx = _mm_mul_ps(x_k, x_k);
          x2_sum_128_j = _mm_add_ps(x2_sum_128_j, xx);
          ...
          const __m128 h_k = _mm_loadu_ps(hc_p);
          const __m128 h_k_1 = _mm_loadu_ps(hc_p + 4);
    ...
    chunk1 = std::min(filter_size_by_2, x_size - x_i);
    ...
    for (size_t j = 0; j < num_filters - 1; ++j) {
      ...
      for (int limit : {chunk1, chunk2}) {
        const int limit_by_4 = limit >> 2;
        for (int k = limit_by_4; k > 0; --k, h_p += 8, x_p += 4) {
          const __m128 x_k = _mm_loadu_ps(x_p);
          __m128 alpha_x = _mm_mul_ps(alpha_128_j, x_k);
          __m128 h_k = _mm_loadu_ps(h_p);
          h_k = _mm_add_ps(h_k, alpha_x);
...
void FindPeaksSymmetricOverlap(rtc::ArrayView<const float> h,
  ...
  const float* h_p = &h[0];
  const size_t size_by_2 = filter_size >> 1;
  const size_t size_by_8 = filter_size >> 3;

  float m0 = 0.f;
  size_t p0 = 0;
  for (size_t k = 0; k < size_by_2; ++k) {
    float tmp = fabs(*h_p++);
    if (tmp > m0) {
      m0 = tmp;
      p0 = k;
  ...
  for (size_t j = 0; j < num_filters - 1; ++j) {
    float m1 = 0.f;
    size_t p1 = 0;
    for (size_t k = 0, i = 0; k < size_by_8; ++k, i += 4) {
      for (size_t n = 0; n < 4; ++n) {
        float tmp = fabs(*h_p++);
        if (tmp > m0) {
          m0 = tmp;
...
MatchedFilter::MatchedFilter(ApmDataDumper* data_dumper,
    ...
    : data_dumper_(data_dumper),
      ...
      symmetric_overlap_(
          2 * alignment_shift_sub_blocks == window_size_sub_blocks && true),
...
void MatchedFilter::UpdateSymmetricOverlap(
  ...
  const float kMatchingFilterThreshold = 0.7f;
...
void MatchedFilter::UpdateGeneric(const DownsampledRenderBuffer& render_buffer,
  ...
  for (size_t n = 0; n < filters_generic_.size(); ++n) {
    ...
    // Update the lag estimates for the matched filter.
    const float kMatchingFilterThreshold = 0.5f;
