...
cricket::BaseChannel* PeerConnection::GetChannel(
  ...
  for (auto transceiver : transceivers_) {
    ...
    if (channel && channel->content_name() == content_name) {
      RTC_LOG(INFO) << "channel->content_name():" << channel->content_name();
      return channel;
    }
  }
  if (rtp_data_channel() &&
...
// TODO: Replicate in JsepTransportController
/*
...
const std::string PeerConnection::GetTransportName(
  ...
  if (channel) {
    RTC_LOG(INFO) << "channel->transport_name:" << channel->content_name();
