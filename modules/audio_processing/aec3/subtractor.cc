...
void Subtractor::ExitInitialState() {
  for (size_t ch = 0; ch < num_capture_channels_; ++ch) {
    ...
    refined_filters_[ch]->SetSizePartitions(config_.filter.refined.length_blocks,
                                         false);
...
void Subtractor::Process(const RenderBuffer& render_buffer,
  ...
  const bool same_filter_sizes =
      refined_filters_[0]->SizePartitions() == shadow_filter_[0]->SizePartitions();
  ...
  if (same_filter_sizes) {
    render_buffer.SpectralSum(refined_filters_[0]->SizePartitions(), &X2_refined);
    ...
    render_buffer.SpectralSums(shadow_filter_[0]->SizePartitions(),
                               refined_filters_[0]->SizePartitions(), &X2_shadow,
                               &X2_refined);
  } else {
  ...
  for (size_t ch = 0; ch < num_capture_channels_; ++ch) {
    ...
    if (!refined_filters_adjusted) {
      ...
      refined_gains_[ch]->Compute(X2_refined, render_signal_analyzer, output, erl,
    ...
    refined_filters_[ch]->Adapt(render_buffer, G, &refined_impulse_responses_[ch]);
    refined_filters_[ch]->ComputeFrequencyResponse(&refined_frequency_responses_[ch]);
    ...
    poor_shadow_filter_counters_[ch] =
        output.e2_refined < output.e2_shadow ? poor_shadow_filter_counters_[ch] + 1
                                          : 0;
    ...
    if (ch == 0) {
      data_dumper_->DumpWav("aec3_refined_filters_output", kBlockSize, &e_refined[0],
                            16000, 1);
