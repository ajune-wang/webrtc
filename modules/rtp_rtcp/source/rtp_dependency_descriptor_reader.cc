      ...
      (reading_state->structure->structure_id +
       ...
       frame_dependency_structure_->structure_id + kMaxTemplates) %
          ...
          frame_dependency_structure_->templates.size() +
              reading_state->structure->templates.size()) {
    RTC_LOG(LS_WARNING) << "Received frame dependency structure with template "
                           "ids overlapping with previous structure. "
                           "Previous: ["
                        << frame_dependency_structure_->structure_id << ","
                        << (frame_dependency_structure_->structure_id +
                            frame_dependency_structure_->templates.size() - 1) %
                               kMaxTemplates
                        << ", Received: ["
                        << reading_state->structure->structure_id << ","
                        << (reading_state->structure->structure_id +
                            reading_state->structure->templates.size() - 1) %
                               kMaxTemplates
                        << "].";
  }
