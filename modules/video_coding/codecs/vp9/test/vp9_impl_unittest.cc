...
class TestVp9Impl : public VideoCodecUnitTest {
 protected:
  ...
  void ExpectFrameWith(size_t num_spatial_layers,
                       ...
                       const std::vector<bool>& inter_layer_predicted,
    ...
    for (size_t frame_num = 0; frame_num < num_spatial_layers; ++frame_num) {
      ...
      EXPECT_EQ(vp9.inter_layer_predicted, inter_layer_predicted[frame_num]);
...
TEST_F(TestVp9Impl, FlexibleMode) {
  // In flexible mode we know actual list of references used for prediction
  // of a frame. It is expected that this list matches the predefined one,
  // which is used in non-flexible mode.
  ...
  for (uint8_t num_spatial_layers = 1; num_spatial_layers <= 3;
    ...
    for (uint8_t num_temporal_layers = 1; num_temporal_layers <= 3;
      ...
      for (size_t frame_num = 0; frame_num < gof.num_frames_in_gof + 1;
        ...
        const std::vector<bool> inter_layer_pred = {false, true, true};
        ...
        ExpectFrameWith(num_spatial_layers, gof.temporal_idx[gof_idx],
                        gof.temporal_up_switch[gof_idx], inter_layer_pred,
