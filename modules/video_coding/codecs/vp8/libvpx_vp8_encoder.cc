...
namespace Vp8VariableFpsScreenshare {
static constexpr double kMinFps = 5.0;
static constexpr int kSteadyQP = 15;
...
LibvpxVp8Encoder::LibvpxVp8Encoder(const Environment& env,
    ...
    : env_(env),
      ...
      framerate_controller_(Vp8VariableFpsScreenshare::kMinFps),
...
size_t LibvpxVp8Encoder::SteadyStateSize(int sid, int tid) {
  ...
  return static_cast<size_t>(
      bitrate_bps / (8 * fps) *
          (100 - Vp8VariableFpsScreenshare::kUndershootPct) / 100 +
      0.5);
}
  ...
  for (size_t encoder_idx = 0; encoder_idx < encoders_.size();
    ...
    if (send_stream_[stream_idx]) {
      if (encoded_images_[encoder_idx].size() > 0) {
        ...
        if (qp_128 > Vp8VariableFpsScreenshare::kSteadyQP ||
