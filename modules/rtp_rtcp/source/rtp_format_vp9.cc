...
RtpPacketizerVp9::RtpPacketizerVp9(rtc::ArrayView<const uint8_t> payload,
  ...
  while (bytes_processed < payload.size()) {
    ...
    PacketInfo& packet_info = packets_[--num_packets_left];
...
bool RtpPacketizerVp9::NextPacket(RtpPacketToSend* packet) {
  ...
  const PacketInfo& packet_info = packets_[packets_.size() - 1];

  if (!WriteHeaderAndPayload(packet_info, packet, packets_.size() == 1)) {
    return false;
  }
  packets_.SetSize(packets_.size() - 1);
