...
TEST_F(ReceiveStatisticsProxyTest, ReportsMaxTotalInterFrameDelay) {
  ...
  statistics_proxy_->OnDecodedFrame(frame, absl::nullopt, 0,
                                    VideoContentType::UNSPECIFIED);
  EXPECT_DOUBLE_EQ(expectedTotalInterFrameDelay,
                   statistics_proxy_->GetStats().total_inter_frame_delay);
  EXPECT_DOUBLE_EQ(
      expectedTotalSquaredInterFrameDelay,
      statistics_proxy_->GetStats().total_squared_inter_frame_delay);

  fake_clock_.AdvanceTime(kInterFrameDelay1);
  statistics_proxy_->OnDecodedFrame(frame, absl::nullopt, 0,
                                    VideoContentType::UNSPECIFIED);
  expectedTotalInterFrameDelay += kInterFrameDelay1.seconds<double>();
  expectedTotalSquaredInterFrameDelay +=
  ...
  statistics_proxy_->OnDecodedFrame(frame, absl::nullopt, 0,
                                    VideoContentType::UNSPECIFIED);
  expectedTotalInterFrameDelay += kInterFrameDelay2.seconds<double>();
  expectedTotalSquaredInterFrameDelay +=
  ...
  statistics_proxy_->OnDecodedFrame(frame, absl::nullopt, 0,
                                    VideoContentType::UNSPECIFIED);
  expectedTotalInterFrameDelay += kInterFrameDelay3.seconds<double>();
  expectedTotalSquaredInterFrameDelay +=
