...
// Kill-switch for bitrate interpolation for resolutions less than 180p.
bool DisableLowresBitrateInterpolation(
    const webrtc::WebRtcKeyValueConfig& trials) {
  return absl::StartsWith(
      trials.Lookup("WebRTC-LowresSimulcastBitrateInterpolation"), "Disabled");
}
...
constexpr const SimulcastFormat kSimulcastFormats[] = {
    ...
    // Interpolate target and max down all the way down to zero, but limit min
    // to 30 kbps. Any discrepancies between the three values will be handled at
    // the codec level.
...
std::vector<SimulcastFormat> GetSimulcastFormats(
    bool disable_lowres_bitrate_interpolation) {
  std::vector<SimulcastFormat> formats;
  formats.insert(formats.begin(), std::begin(kSimulcastFormats),
                 std::end(kSimulcastFormats));
  if (disable_lowres_bitrate_interpolation) {
    RTC_CHECK(!formats.empty());
    SimulcastFormat& format0x0 = formats[formats.size() - 1];
    format0x0.max_bitrate = webrtc::DataRate::KilobitsPerSec(200);
    format0x0.target_bitrate = webrtc::DataRate::KilobitsPerSec(150);
  }
  return formats;
}
...
int FindSimulcastFormatIndex(int width,
  ...
  const auto formats =
      GetSimulcastFormats(disable_lowres_bitrate_interpolation);
...
SimulcastFormat InterpolateSimulcastFormat(
  ...
  const int index = FindSimulcastFormatIndex(
      width, height, disable_lowres_bitrate_interpolation);
  if (index == 0)
  ...
  size_t max_layers = formats[index].max_layers;
...
webrtc::DataRate FindSimulcastMaxBitrate(
  ...
  return InterpolateSimulcastFormat(width, height,
                                    disable_lowres_bitrate_interpolation)
      .max_bitrate;
}
...
size_t LimitSimulcastLayerCount(int width,
  ...
  if (!absl::StartsWith(trials.Lookup(kUseLegacySimulcastLayerLimitFieldTrial),
                        "Disabled")) {
    const bool disable_lowres_bitrate_interpolation =
        DisableLowresBitrateInterpolation(trials);
    const auto formats =
        GetSimulcastFormats(disable_lowres_bitrate_interpolation);
    size_t adaptive_layer_count = std::max(
        need_layers,
        formats[FindSimulcastFormatIndex(width, height,
                                         disable_lowres_bitrate_interpolation)]
            .max_layers);
