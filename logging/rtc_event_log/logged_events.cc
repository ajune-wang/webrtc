...
LoggedPacketInfo::LoggedPacketInfo(const LoggedRtpPacket& rtp,
    ...
    : ssrc(rtp.header.ssrc),
      ...
      rtx(rtx),
      frame_marker(rtp.header.markerBit),
      included_in_feedback(rtp.header.extension.hasTransportSequenceNumber),
      transport_seq_no(static_cast<uint16_t>(
          included_in_feedback ? rtp.header.extension.transportSequenceNumber
                               : 0)),
      // TODO(srte): Estimate the audio sample rate.
      capture_time(rtp.header.timestamp /
                   (media_type == LoggedMediaType::kAudio ? 48000.0 : 90000.0)),
      send_or_recv_time(rtp.log_time_us() * 1e-6) {}
