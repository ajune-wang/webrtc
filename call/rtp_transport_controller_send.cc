...
void RtpTransportControllerSend::OnSentPacket(
  ...
  // TODO(bugs.webrtc.org/137439): Clarify other thread contexts calling in.
...
void RtpTransportControllerSend::ProcessSentPacket(
  ...
  NetworkControlUpdate control_updates;
  if (controller_)
    control_updates = controller_->OnSentPacket(*packet_msg);
  if (!congestion_update && !control_updates)
    return;
  if (posted_to_worker) {
    ProcessSentPacketUpdates(std::move(control_updates));
  } else {
    ...
    task_queue_.TaskQueueForPost()->PostTask(task_queue_.MaybeSafeTask(
        safety_.flag(),
        [this, control_updates = std::move(control_updates)]() mutable {
          RTC_DCHECK_RUN_ON(&task_queue_);
          ProcessSentPacketUpdates(std::move(control_updates));
        }));
  }
