...
bool SharedScreenCastStreamPrivate::StartScreenCastStream(
  ...
  // if (count++ % 2 == 0) {
    ...
    RTC_LOG(LS_ERROR) << ">>> pw thread loop wait on version: " << server_version_sync_;
    server_version_sync_ =
        pw_core_sync(pw_core_, PW_ID_CORE, server_version_sync_);

    pw_thread_loop_wait(pw_main_loop_);
    RTC_LOG(LS_ERROR) << ">>> pw thread loop wait done version: " << server_version_sync_;
  }
  // }
...

  {
    ...
    RTC_LOG(LS_ERROR) << ">>> PipeWire remote opened.";
  }
  return true;
}
