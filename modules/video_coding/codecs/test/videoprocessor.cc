...
void VideoProcessor::ProcessFrame() {
  ...
  const bool is_async_codec = config_.hw_encoder || config_.hw_decoder;
  if (!is_async_codec) {
    for (size_t simulcast_svc_idx = 0;
         simulcast_svc_idx < num_simulcast_or_spatial_layers_;
         ++simulcast_svc_idx) {
...
void VideoProcessor::CopyEncodedImage(const EncodedImage& encoded_image,
  ...
  const size_t copied_buffer_size = base_image._length + encoded_image._length +
  ...
  copied_image._length = base_image._length + encoded_image._length;
