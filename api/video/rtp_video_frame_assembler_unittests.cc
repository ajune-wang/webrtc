...
TEST(RtpVideoFrameAssembler, Padding) {
  ...
  AppendFrames(assembler.InsertPacket(PaddingPacket(124)), frames);
...
TEST(RtpVideoFrameAssembler, ClearOldPacketsWithPadding) {
  ...
  EXPECT_THAT(assembler.InsertPacket(PaddingPacket(2000)), SizeIs(0));
...
TEST(RtpVideoFrameAssembler, SeqNumStartAndSeqNumEndSet) {
  ...
  ASSERT_THAT(frames, SizeIs(1));
  EXPECT_THAT(frames[0].rtp_seq_num_start, Eq(123));
  EXPECT_THAT(frames[0].rtp_seq_num_end, Eq(124));
}
...
TEST(RtpVideoFrameAssembler, SeqNumStartAndSeqNumEndSetWhenPaddingReceived) {
  ...
  AppendFrames(assembler.InsertPacket(PaddingPacket(122)), frames);
