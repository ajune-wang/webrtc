...
// constructor, then will call to video quality analyzer, with correct
...
//
// Quality decoder registers its own callback in origin decoder in the same
 ...
 private:
  class DecoderCallback : public DecodedImageCallback {
   public:
    ...
    // Methods fo DecodedImageCallback interface.
  ...
  DecodedImageCallback* delegate_callback_ RTC_GUARDED_BY(lock_);
  std::map<uint32_t, uint16_t> timestamp_to_frame_id_ RTC_GUARDED_BY(lock_);
  // Stores currently decoding images by frame id. Because
