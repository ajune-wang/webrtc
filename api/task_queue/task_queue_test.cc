...
TEST_P(TaskQueueTest, PostAndCheckCurrent) {
  ...
  PostTask(queue.get(), [&event, &queue] {
...
TEST_P(TaskQueueTest, PostFromQueue) {
  ...
  PostTask(queue.get(), [&event, &queue] {
    PostTask(queue.get(), [&event] { event.Set(); });
  });
  EXPECT_TRUE(event.Wait(1000));
}
...
TEST_P(TaskQueueTest, PostDelayedAfterDestruct) {
  ...
  PostDelayedTask(
      queue.get(), [&run] { run.Set(); }, [&deleted] { deleted.Set(); }, 100);
  // Destroy the queue.
  queue = nullptr;
...
TEST_P(TaskQueueTest, PostALot) {
...

  {
    ...
    PostTask(queue.get(), [&event] { event.Wait(rtc::Event::kForever); });
    for (int i = 0; i < kTaskCount; ++i) {
      PostTask(
          queue.get(), [&tasks_executed] { ++tasks_executed; },
...
TEST_P(TaskQueueTest, PostTwoWithSharedUnprotectedState) {
  ...
  rtc::Event done;
  PostTask(queue.get(), [&state, &queue, &done] {
    // Post tasks from queue to guarantee, that 1st task won't be
    // executed before the second one will be posted.
    PostTask(queue.get(), [&state] { state.state = 1; });
    PostTask(queue.get(), [&state, &done] {
