...
VideoBitrateAllocationParameters::VideoBitrateAllocationParameters(
    ...
    uint32_t framerate)
    : total_bitrate(DataRate::BitsPerSecond(total_bitrate_bps)),
      stable_bitrate(DataRate::BitsPerSecond(total_bitrate_bps)),
...
VideoBitrateAllocation VideoBitrateAllocator::GetAllocation(
    uint32_t total_bitrate_bps,
    uint32_t framerate) {
  return Allocate({DataRate::BitsPerSecond(total_bitrate_bps),
                   DataRate::BitsPerSecond(total_bitrate_bps),
                   static_cast<double>(framerate)});
}

VideoBitrateAllocation VideoBitrateAllocator::Allocate(
    VideoBitrateAllocationParameters parameters) {
  return GetAllocation(parameters.total_bitrate.BitsPerSecond(),
