...
void AudioIngress::ReceivedRTPPacket(rtc::ArrayView<const uint8_t> rtp_packet) {
  ...
  if (rtp_packet_received.Ssrc() != remote_ssrc_.load()) {
...
void AudioIngress::ReceivedRTCPPacket(
  ...
  int64_t rtt = 0;
  if (rtp_rtcp_->RTT(remote_ssrc_.load(), &rtt, nullptr, nullptr, nullptr) ==
      0) {
...
IngressStatistics AudioIngress::GetIngressStatistics() {
  ...
  ingress_stats.total_duration = output_audio_level_.TotalDuration();
...
ChannelStatistics AudioIngress::GetChannelStatistics() {
  ...
  StreamStatistician* statistician =
      rtp_receive_statistics_->GetStatistician(remote_ssrc_.load());
  if (statistician) {
    ...
    channel_stats.remote_ssrc = remote_ssrc_.load();
    if (clockrate_hz > 0) {
  ...
  for (const ReportBlockData& block_data : report_data) {
    ...
    if (rtp_rtcp_->SSRC() != rtcp_report.source_ssrc ||
        remote_ssrc_.load() != rtcp_report.sender_ssrc) {
      continue;
    }
