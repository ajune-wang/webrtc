...
// construction.
constexpr int kPreinitCocodingEntitiesCount = 2;
...
DefaultEncodedImageIdInjector::DefaultEncodedImageIdInjector() {
  for (size_t i = 0;
       i < kPreinitCocodingEntitiesCount * kBuffersPoolPerCodingEntity; ++i) {
...
void DefaultEncodedImageIdInjector::ExtendIfRequired(int coding_entity_id) {
  ...
  if (coding_entities_.size() <= kPreinitCocodingEntitiesCount) {
    // Buffers for the first kPreinitCocodingEntitiesCount coding entities were
