...
class VideoSendStreamImplTest : public ::testing::Test {
 protected:
  VideoSendStreamImplTest()
    ...
    ON_CALL(rtp_video_sender_, SetActive(_))
        .WillByDefault(::testing::Invoke([&](bool active) {
          // SetActive is only called when stopping.
          EXPECT_FALSE(active);
...
TEST_F(VideoSendStreamImplTest, RegistersAsBitrateObserverOnStart) {
  ...
  worker_queue_.RunSynchronous([&] {
    vss_impl->SetSendStatePerRtpStream({true});
