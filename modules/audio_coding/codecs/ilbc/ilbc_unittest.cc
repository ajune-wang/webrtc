...
TEST(IlbcTest, BadPacket) {
  ...
  rtc::Buffer<uint8_t> bad_packet(packet.data(), packet.size());
...
TEST_P(SplitIlbcTest, NumFrames) {
  ...
  const auto generate_payload = [](size_t payload_length_bytes) {
    rtc::Buffer<uint8_t> payload(payload_length_bytes);
  ...
  for (const auto& result : results) {
    ...
    const rtc::Buffer<uint8_t>& payload = frame->payload();
...
TEST(IlbcTest, SplitTooLargePayload) {
  ...
  const auto results =
      decoder.ParsePayload(rtc::Buffer<uint8_t>(kPayloadLengthBytes), 0);
