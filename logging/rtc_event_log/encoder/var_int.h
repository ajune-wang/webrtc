...
// Same as other version, but uses a BitReader for input.
// If decoding is successful, the decoded varint is written into `output`.
// If not successful, `input` reader is set into the error state, return value
// is unspecified.
uint64_t DecodeVarInt(BitReader& input);
