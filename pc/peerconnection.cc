...
rtc::scoped_refptr<RtpSenderInterface> PeerConnection::AddTrack(
  ...
  if (track->kind() == MediaStreamTrackInterface::kAudioKind) {
    ...
    const SenderInfo* sender_info = FindSenderInfo(
        local_audio_senders_, new_sender->internal()->stream_id(), track->id());
    if (sender_info) {
      new_sender->internal()->SetSsrc(sender_info->ssrc);
    }
    ...
    const SenderInfo* sender_info = FindSenderInfo(
        local_video_senders_, new_sender->internal()->stream_id(), track->id());
    if (sender_info) {
      new_sender->internal()->SetSsrc(sender_info->ssrc);
    }
  } else {
...
rtc::scoped_refptr<RtpReceiverInterface> PeerConnection::RemoveAndStopReceiver(
    const std::string& track_id) {
  auto receiver = FindReceiverById(track_id);
  if (!receiver) {
...
void PeerConnection::AddAudioTrack(AudioTrackInterface* track,
  ...
  const SenderInfo* sender_info =
      FindSenderInfo(local_audio_senders_, stream->label(), track->id());
  if (sender_info) {
    new_sender->internal()->SetSsrc(sender_info->ssrc);
  }
}
...
void PeerConnection::AddVideoTrack(VideoTrackInterface* track,
  ...
  GetVideoTransceiver()->internal()->AddSender(new_sender);
  const SenderInfo* sender_info =
      FindSenderInfo(local_video_senders_, stream->label(), track->id());
  if (sender_info) {
    new_sender->internal()->SetSsrc(sender_info->ssrc);
  }
}
...
void PeerConnection::UpdateRemoteStreamsList(
  ...
  std::vector<SenderInfo>* current_senders = GetRemoteSenders(media_type);
  ...
  for (auto sender_it = current_senders->begin();
    ...
    const SenderInfo& info = *sender_it;
    ...
    if ((info.stream_label == kDefaultStreamLabel && default_sender_needed) ||
      ...
      OnRemoteSenderRemoved(info.stream_label, info.sender_id, media_type);
  ...
  for (const cricket::StreamParams& params : streams) {
    ...
    if (!sender_info) {
      current_senders->push_back(SenderInfo(stream_label, sender_id, ssrc));
      OnRemoteSenderSeen(stream_label, sender_id, ssrc, media_type);
    }
  }
  ...
  if (default_sender_needed) {
    ...
    const SenderInfo* default_sender_info = FindSenderInfo(
        *current_senders, kDefaultStreamLabel, default_sender_id);
    if (!default_sender_info) {
      current_senders->push_back(
          SenderInfo(kDefaultStreamLabel, default_sender_id, 0));
      OnRemoteSenderSeen(kDefaultStreamLabel, default_sender_id, 0, media_type);
    }
  }
}

void PeerConnection::OnRemoteSenderSeen(const std::string& stream_label,
  ...
  if (media_type == cricket::MEDIA_TYPE_AUDIO) {
    CreateAudioReceiver(stream, sender_id, ssrc);
  } else if (media_type == cricket::MEDIA_TYPE_VIDEO) {
    CreateVideoReceiver(stream, sender_id, ssrc);
  } else {
    RTC_NOTREACHED() << "Invalid media type";
  }
}

void PeerConnection::OnRemoteSenderRemoved(const std::string& stream_label,
                                           const std::string& sender_id,
                                           cricket::MediaType media_type) {
  MediaStreamInterface* stream = remote_streams_->find(stream_label);
  ...
  if (media_type == cricket::MEDIA_TYPE_AUDIO) {
    ...
    receiver = RemoveAndStopReceiver(sender_id);
    rtc::scoped_refptr<AudioTrackInterface> audio_track =
        stream->FindAudioTrack(sender_id);
    if (audio_track) {
    ...
    rtc::scoped_refptr<VideoTrackInterface> video_track =
        stream->FindVideoTrack(sender_id);
    if (video_track) {
...
void PeerConnection::UpdateLocalSenders(
    ...
    cricket::MediaType media_type) {
  std::vector<SenderInfo>* current_senders = GetLocalSenders(media_type);
  ...
  for (auto sender_it = current_senders->begin();
    ...
    if (!params || params->id != info.sender_id ||
      ...
      OnLocalSenderRemoved(info.stream_label, info.sender_id, info.ssrc,
                           media_type);
      sender_it = current_senders->erase(sender_it);
    } else {
      ++sender_it;
  ...
  for (const cricket::StreamParams& params : streams) {
    ...
    if (!sender_info) {
      current_senders->push_back(SenderInfo(stream_label, sender_id, ssrc));
      OnLocalSenderSeen(stream_label, sender_id, params.first_ssrc(),
                        media_type);
    }
  }
}

void PeerConnection::OnLocalSenderSeen(const std::string& stream_label,
                                       const std::string& sender_id,
                                       uint32_t ssrc,
                                       cricket::MediaType media_type) {
  auto sender = FindSenderById(sender_id);
  if (!sender) {
    RTC_LOG(LS_WARNING) << "An unknown RtpSender with id " << sender_id
  ...
  sender->internal()->set_stream_id(stream_label);
  sender->internal()->SetSsrc(ssrc);
}

void PeerConnection::OnLocalSenderRemoved(const std::string& stream_label,
                                          const std::string& sender_id,
                                          uint32_t ssrc,
                                          cricket::MediaType media_type) {
  auto sender = FindSenderById(sender_id);
...
std::vector<PeerConnection::SenderInfo>* PeerConnection::GetRemoteSenders(
    cricket::MediaType media_type) {
  RTC_DCHECK(media_type == cricket::MEDIA_TYPE_AUDIO ||
             media_type == cricket::MEDIA_TYPE_VIDEO);
  return (media_type == cricket::MEDIA_TYPE_AUDIO) ? &remote_audio_senders_
                                                   : &remote_video_senders_;
}

std::vector<PeerConnection::SenderInfo>* PeerConnection::GetLocalSenders(
    cricket::MediaType media_type) {
  RTC_DCHECK(media_type == cricket::MEDIA_TYPE_AUDIO ||
             media_type == cricket::MEDIA_TYPE_VIDEO);
  return (media_type == cricket::MEDIA_TYPE_AUDIO) ? &local_audio_senders_
                                                   : &local_video_senders_;
}

const PeerConnection::SenderInfo* PeerConnection::FindSenderInfo(
    const std::vector<PeerConnection::SenderInfo>& infos,
    const std::string& stream_label,
    const std::string sender_id) const {
  for (const SenderInfo& sender_info : infos) {
