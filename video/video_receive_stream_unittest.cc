...
class VideoReceiveStreamTestWithFakeDecoder : public ::testing::Test {
 public:
  ...
  void SetUp() {
    ...
    clock_ = Clock::GetRealTimeClock();
    ReCreateReceiveStream(VideoReceiveStream::EncodedFrameDispatchState());
  }

  void ReCreateReceiveStream(
      VideoReceiveStream::EncodedFrameDispatchState state) {
    constexpr int kDefaultNumCpuCores = 2;
    ...
    video_receive_stream_.reset(new webrtc::internal::VideoReceiveStream(
        ...
        kDefaultNumCpuCores, &packet_router_, config_.Copy(),
        process_thread_.get(), &call_stats_, clock_, timing_));
    video_receive_stream_->SetEncodedFrameDispatchState(std::move(state),
                                                        false);
  }
...
TEST_F(VideoReceiveStreamTestWithFakeDecoder,
  ...
  video_receive_stream_->SetEncodedFrameDispatchState(
      VideoReceiveStream::EncodedFrameDispatchState(callback.AsStdFunction()),
      true);
...
TEST_F(VideoReceiveStreamTestWithFakeDecoder,
  ...
  video_receive_stream_->SetEncodedFrameDispatchState(
      ...
      true);
  video_receive_stream_->Stop();
  VideoReceiveStream::EncodedFrameDispatchState old_state =
      video_receive_stream_->SetEncodedFrameDispatchState(
          VideoReceiveStream::EncodedFrameDispatchState(), false);
  ...
  VideoReceiveStreamTestWithSimulatedClock()
      : sim_clock_(Timestamp::ms(4711)),
        ...
        config_(GetConfig(&mock_transport_,
                          &fake_decoder_factory_,
                          &fake_renderer_)),
        call_stats_(&sim_clock_, process_thread_.get()),
        video_receive_stream_(task_queue_factory_.get(),
...
TEST_F(VideoReceiveStreamTestWithSimulatedClock,
  ...
  int tick_ms = internal::VideoReceiveStream::kMaxWaitForKeyFrameMs / 2;
  EXPECT_CALL(mock_transport_, SendRtcp).Times(1);
  video_receive_stream_.GenerateKeyFrame();
  PassEncodedFrameAndWait(MakeFrame(VideoFrameType::kVideoFrameDelta, 0));
  sim_clock_.AdvanceTimeMilliseconds(tick_ms);
  ...
  sim_clock_.AdvanceTimeMilliseconds(
      internal::VideoReceiveStream::kMaxWaitForKeyFrameMs);
