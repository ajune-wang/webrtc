...
void LossRateFilter::UpdateWithLossStatus(Timestamp send_time,
  ...
  // If the time diff is zero due to reordering or packets sent at the same time
  // the estimate will not change.
  // TODO(srte): Do something appropriate for packets sent at the same time.
  if (time_diff < 0)
    return;
