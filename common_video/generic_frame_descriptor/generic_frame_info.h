...
namespace webrtc {
// Structures to build and parsedependency descriptor as described in
// https://aomediacodec.github.io/av1-rtp-spec/#dependency-descriptor-rtp-header-extension
class DisplayResolution {
 public:
  constexpr DisplayResolution() = default;
  constexpr DisplayResolution(int width, int height)
      : width_(width), height_(height) {}
  DisplayResolution(const DisplayResolution&) = default;
  DisplayResolution& operator=(const DisplayResolution&) = default;

  friend bool operator==(const DisplayResolution& lhs,
                         const DisplayResolution& rhs) {
...
struct FrameDependencyStructure {
  ...
  absl::InlinedVector<DisplayResolution, 4> resolutions;
  std::vector<FrameDependencyTemplate> templates;
};

struct RtpDependencyDescriptor {
