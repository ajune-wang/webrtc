...
TEST(SctpSidTest, Basics) {
  ...
  EXPECT_TRUE(!StreamId(-1).IsValid());
  EXPECT_TRUE(!StreamId(-2).IsValid());
  EXPECT_TRUE(StreamId(cricket::kMinSctpSid).IsValid());
  EXPECT_TRUE(StreamId(cricket::kMinSctpSid + 1).IsValid());
  EXPECT_TRUE(StreamId(cricket::kSpecMaxSctpSid).IsValid());
  EXPECT_TRUE(StreamId(cricket::kMaxSctpSid).IsValid());
  ...
  EXPECT_EQ(StreamId(0).role(), rtc::SSL_CLIENT);
  EXPECT_EQ(StreamId(1).role(), rtc::SSL_SERVER);
  EXPECT_EQ(StreamId(2).role(), rtc::SSL_CLIENT);
  EXPECT_EQ(StreamId(3).role(), rtc::SSL_SERVER);
