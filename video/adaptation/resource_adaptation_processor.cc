...
void ResourceAdaptationProcessor::OnResourceUnderuse(
  ...
  MaybeUpdateVideoSourceRestrictions();
  // Stats and logging.
  manager_->UpdateAdaptationStats(reason);
...
void VideoStreamEncoderResourceManager::OnFrameDroppedDueToSize() {
  AdaptationCounters counters_before = processor_->stream_adapter_->adaptation_counters();
  processor_->OnResourceOveruse(AdaptationObserverInterface::AdaptReason::kQuality);
  if (degradation_preference() == DegradationPreference::BALANCED &&
      processor_->stream_adapter_->adaptation_counters().fps_adaptations >
          counters_before.fps_adaptations) {
    // Adapt framerate in same step as resolution.
    processor_->OnResourceOveruse(AdaptationObserverInterface::AdaptReason::kQuality);
  }
  if (processor_->stream_adapter_->adaptation_counters().resolution_adaptations >
...
void VideoStreamEncoderResourceManager::MaybeUpdateTargetFrameRate() {
  ...
  absl::optional<double> target_frame_rate =
      ApplyDegradationPreference(processor_->stream_adapter_->source_restrictions(),
...
void VideoStreamEncoderResourceManager::UpdateAdaptationStats(
  ...
  const AdaptationCounters total_counts =
      processor_->stream_adapter_->adaptation_counters();
