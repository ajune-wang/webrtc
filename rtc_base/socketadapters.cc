...
int AsyncHttpsProxySocket::Connect(const SocketAddress& addr) {
  int ret;
  NLOG(LS_VERBOSE, "AsyncHttpsProxySocket::Connect(",
...
void AsyncHttpsProxySocket::OnConnectEvent(AsyncSocket * socket) {
  NLOG(LS_VERBOSE, "AsyncHttpsProxySocket::OnConnectEvent");
  if (!ShouldIssueConnect()) {
    state_ = PS_TUNNEL;
...
void AsyncHttpsProxySocket::OnCloseEvent(AsyncSocket * socket, int err) {
  NLOG(LS_VERBOSE, "AsyncHttpsProxySocket::OnCloseEvent(", err, ")");
...
void AsyncHttpsProxySocket::SendRequest() {
  ...
  NLOG(LS_VERBOSE, "AsyncHttpsProxySocket >> ", str);
}

void AsyncHttpsProxySocket::ProcessLine(char * data, size_t len) {
  NLOG(LS_VERBOSE, "AsyncHttpsProxySocket , ", data);
...
#error Wrong code for HTTP_STATUS_PROXY_AUTH_REQ
    ...
    case HAR_IGNORE:
      NLOG(LS_VERBOSE, "Ignoring Proxy-Authenticate: ", auth_method);
  ...
  if (state_ == SS_HELLO) {
    ...
    if (atyp == 1) {
      ...
      if (!response.ReadUInt32(&addr) ||
        ...
        return;
      NLOG(LS_VERBOSE, "Bound on ", addr, ":", port);
    } else if (atyp == 3) {
      uint8_t len;
      std::string addr;
      ...
      if (!response.ReadString(&addr, 16) ||
          !response.ReadUInt16(&port))
        return;
      NLOG(LS_VERBOSE, "Bound on <IPV6>:", port);
    } else {
      Error(0);
      return;
    }
