...
TEST(CrossTrafficTest, PulsedPeaksCrossTraffic) {
  ...
  config.peak_rate = DataRate::KilobitsPerSecond(1000);
  ...
  while (fixture.clock.TimeInMilliseconds() < kRunTime.Milliseconds()) {
    pulsed_peaks.Process(
        Timestamp::Milliseconds(fixture.clock.TimeInMilliseconds()));
