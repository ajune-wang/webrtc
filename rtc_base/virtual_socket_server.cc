...
void VirtualSocket::SafetyBlock::SetNotAlive(VirtualSocketServer* server,
                                             const SocketAddress& local_addr) {
  MutexLock lock(&mutex);

  // Cancel pending sockets
  if (listen_queue.has_value()) {
    for (const SocketAddress& remote_addr : *listen_queue) {
...
int VirtualSocket::Close() {
  ...
  safety_->SetNotAlive(server_, local_addr_);

#if 1
  class Dummy : public rtc::MessageHandler {
   public:
    void OnMessage(rtc::Message*) override {}
  } dummy;
  // Noop Clear to hide thread race in StunServerTest.TestBad
  server_->msg_queue_->Clear(&dummy, 0, nullptr);
#endif
...
void VirtualSocket::PostSignalReadEvent() {
  if (safety_->pending_read_signal_event) {
