...
void RtpPacketizerVp9::GeneratePackets(rtc::ArrayView<const uint8_t> payload) {
  ...
  while (bytes_processed < payload.size()) {
    ...
    packets_.emplace();
    PacketInfo& info = packets_.back();
    info.payload = payload.data() + bytes_processed;
    info.size = packet_bytes;
    info.layer_begin = bytes_processed == 0;
    info.layer_end = rem_bytes == packet_bytes;
