...
EmulatedIpPacket::EmulatedIpPacket(const rtc::SocketAddress& from,
  ...
  RTC_DCHECK(from.family() == to.family());
  RTC_DCHECK(from.family() == AF_INET || from.family() == AF_INET6);
  ...
  header_size = kUdpHeaderSize +
                (from.family() == AF_INET ? kIPv4HeaderSize : kIPv6HeaderSize);
}
...
void LinkEmulation::OnPacketReceived(EmulatedIpPacket packet) {
  task_queue_->PostTask([this, packet = std::move(packet)]() mutable {
    ...
    bool sent = network_behavior_->EnqueuePacket(PacketInFlightInfo(
        packet.ip_size(), packet.arrival_time.us(), packet_id));
    if (sent) {
...
void EmulatedEndpoint::SendPacket(const rtc::SocketAddress& from,
  ...
  task_queue_->PostTask([this, packet = std::move(packet)]() mutable {
    ...
    if (stats_.first_packet_sent_time.IsInfinite()) {
      ...
      stats_.first_sent_packet_size = DataSize::bytes(packet.ip_size());
    }
    stats_.last_packet_sent_time = current_time;
    stats_.packets_sent++;
    stats_.bytes_sent += DataSize::bytes(packet.ip_size());
...
void EmulatedEndpoint::OnPacketReceived(EmulatedIpPacket packet) {
  ...
  if (it == port_to_receiver_.end()) {
    ...
    stats_.bytes_dropped += DataSize::bytes(packet.ip_size());
    return;
  }
...
void EmulatedEndpoint::UpdateReceiveStats(const EmulatedIpPacket& packet) {
  ...
  if (stats_.first_packet_received_time.IsInfinite()) {
    ...
    stats_.first_received_packet_size = DataSize::bytes(packet.ip_size());
  }
  stats_.last_packet_received_time = current_time;
  stats_.packets_received++;
  stats_.bytes_received += DataSize::bytes(packet.ip_size());
}
