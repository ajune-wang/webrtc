...
void EchoEmulatingCapturer::OnAudioRendered(
  ...
  if (!renderer_queue_.Insert(&queue_input_)) {
    // Test audio device works too slow with sanitizers and can't properly
    // process audio, so when capturer will be stopped renderer will quickly
    // overfill the queue.
