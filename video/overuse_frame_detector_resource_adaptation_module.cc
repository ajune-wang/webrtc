...
AdaptationCounters ApplyDegradationPreference(
  ...
  switch (degradation_preference) {
    ...
    case DegradationPreference::MAINTAIN_FRAMERATE:
      ...
      break;
    case DegradationPreference::MAINTAIN_RESOLUTION:
      counters.resolutions_adaptations = 0;
      break;
    case DegradationPreference::DISABLED:
      counters.resolutions_adaptations = 0;
      counters.fps_adaptations = 0;
...
OveruseFrameDetectorResourceAdaptationModule::
    ...
    : adaptation_listener_(adaptation_listener),
      ...
      stream_adaptor_(std::make_unique<VideoStreamAdaptor>()),
...
void OveruseFrameDetectorResourceAdaptationModule::SetDegradationPreference(
  ...
  if (degradation_preference_ != degradation_preference) {
    ...
    if (degradation_preference == DegradationPreference::BALANCED ||
      ...
      stream_adaptor_->ClearRestrictions();
    }
  }
...
void OveruseFrameDetectorResourceAdaptationModule::OnFrameDroppedDueToSize() {
  // TODO(https://crbug.com/webrtc/11392): Can we look at the totals instead of
  // only QP?
  AdaptationCounters qp_adaptations =
  ...
  if (degradation_preference() == DegradationPreference::BALANCED &&
      stream_adaptor_->adaptation_counters().fps_adaptations >
          qp_adaptations.fps_adaptations) {
    // Adapt framerate in same step as resolution.
    OnResourceOveruse(AdaptationObserverInterface::AdaptReason::kQuality);
  }
  if (stream_adaptor_->adaptation_counters().resolutions_adaptations >
      qp_adaptations.resolutions_adaptations) {
...
OveruseFrameDetectorResourceAdaptationModule::GetAdaptUpTarget(
  ...
  switch (EffectiveDegradationPreference()) {
    case DegradationPreference::BALANCED: {
      ...
      if (stream_adaptor_->CanIncreaseFrameRateTo(target_fps)) {
    ...
    case DegradationPreference::MAINTAIN_FRAMERATE: {
      ...
      if (stream_adaptor_->adaptation_counters().resolutions_adaptations == 1) {
        RTC_LOG(LS_INFO) << "Removing resolution down-scaling setting.";
        target_pixels = std::numeric_limits<int>::max();
      }
      target_pixels =
          VideoStreamAdaptor::GetHigherResolutionThan(target_pixels);
      if (!stream_adaptor_->CanIncreaseResolutionTo(target_pixels))
        return absl::nullopt;
    ...
    case DegradationPreference::MAINTAIN_RESOLUTION: {
      ...
      if (stream_adaptor_->adaptation_counters().fps_adaptations == 1) {
        RTC_LOG(LS_INFO) << "Removing framerate down-scaling setting.";
        target_fps = std::numeric_limits<int>::max();
      }
      target_fps = VideoStreamAdaptor::GetHigherFrameRateThan(target_fps);
      if (!stream_adaptor_->CanIncreaseFrameRateTo(target_fps))
        return absl::nullopt;
...
OveruseFrameDetectorResourceAdaptationModule::GetAdaptDownTarget(
  ...
  switch (EffectiveDegradationPreference()) {
    case DegradationPreference::BALANCED: {
      ...
      if (stream_adaptor_->CanDecreaseFrameRateTo(target_fps)) {
    ...
    case DegradationPreference::MAINTAIN_FRAMERATE: {
      // Scale down resolution.
      int target_pixels =
          VideoStreamAdaptor::GetLowerResolutionThan(input_pixels);
      ...
      if (!stream_adaptor_->CanDecreaseResolutionTo(target_pixels,
    ...
    case DegradationPreference::MAINTAIN_RESOLUTION: {
      int target_fps = VideoStreamAdaptor::GetLowerFrameRateThan(input_fps);
      if (!stream_adaptor_->CanDecreaseFrameRateTo(target_fps))
        return absl::nullopt;
...
void OveruseFrameDetectorResourceAdaptationModule::ApplyAdaptationTarget(
  ...
  switch (target.action) {
    case AdaptationAction::kIncreaseResolution:
      stream_adaptor_->IncreaseResolutionTo(target.value);
      return;
    case AdaptationAction::kDecreaseResolution:
      stream_adaptor_->DecreaseResolutionTo(target.value, min_pixels_per_frame);
      return;
    case AdaptationAction::kIncreaseFrameRate:
      stream_adaptor_->IncreaseFrameRateTo(target.value);
      ...
      if (EffectiveDegradationPreference() == DegradationPreference::BALANCED &&
          stream_adaptor_->adaptation_counters().fps_adaptations == 0 &&
          target.value != std::numeric_limits<int>::max()) {
        RTC_LOG(LS_INFO) << "Removing framerate down-scaling setting.";
        stream_adaptor_->IncreaseFrameRateTo(std::numeric_limits<int>::max());
      }
      return;
    case AdaptationAction::kDecreaseFrameRate:
      stream_adaptor_->DecreaseFrameRateTo(target.value);
      return;
  }
}
...
void OveruseFrameDetectorResourceAdaptationModule::
  ...
  VideoSourceRestrictions new_restrictions = ApplyDegradationPreference(
      stream_adaptor_->source_restrictions(), degradation_preference_);
...
void OveruseFrameDetectorResourceAdaptationModule::
  ...
  absl::optional<double> target_frame_rate =
      ApplyDegradationPreference(stream_adaptor_->source_restrictions(),
                                 degradation_preference_)
          .max_frame_rate();
...
void OveruseFrameDetectorResourceAdaptationModule::ComputeActiveCounts(
  ...
  const AdaptationCounters delta = total_counts - prev_total;

  RTC_DCHECK_EQ(
      std::abs(delta.resolutions_adaptations) + std::abs(delta.fps_adaptations),
      1)
      << "Adaptation took more than one step!";

  if (delta.resolutions_adaptations > 0) {
    ++active_count->resolutions_adaptations;
  } else if (delta.resolutions_adaptations < 0) {
    if (active_count->resolutions_adaptations == 0) {
      RTC_DCHECK_GT(active_count->fps_adaptations, 0) << "No downgrades left";
      RTC_DCHECK_GT(other_active->resolutions_adaptations, 0)
      ...
      --other_active->resolutions_adaptations;
    } else {
      --active_count->resolutions_adaptations;
    }
  }
  if (delta.fps_adaptations > 0) {
    ...
    if (active_count->fps_adaptations == 0) {
      RTC_DCHECK_GT(active_count->resolutions_adaptations, 0)
          << "No downgrades left";
  ...
  RTC_DCHECK(*active_count + *other_active == total_counts);
  RTC_DCHECK_EQ(other_active->Total(), other_total);
  RTC_DCHECK_EQ(active_count->Total(), active_total + delta.Total());
  RTC_DCHECK_GE(active_count->resolutions_adaptations, 0);
  RTC_DCHECK_GE(active_count->fps_adaptations, 0);
  RTC_DCHECK_GE(other_active->resolutions_adaptations, 0);
...
void OveruseFrameDetectorResourceAdaptationModule::UpdateAdaptationStats(
  ...
  const AdaptationCounters total_counts =
      stream_adaptor_->adaptation_counters();

  ComputeActiveCounts(&active_count, &other_active, total_counts);
...
OveruseFrameDetectorResourceAdaptationModule::GetActiveCounts(
  ...
  VideoStreamEncoderObserver::AdaptationSteps counts =
      VideoStreamEncoderObserver::AdaptationSteps();
  counts.num_resolution_reductions = counters.resolutions_adaptations;
  ...
  absl::optional<VideoEncoder::ResolutionBitrateLimits> bitrate_limits =
      encoder_settings_.has_value()
          ? GetEncoderBitrateLimits(
                encoder_settings_->encoder_info(),
                VideoStreamAdaptor::GetHigherResolutionThan(pixels))
          : absl::nullopt;
...
void OveruseFrameDetectorResourceAdaptationModule::
  ...
  if (try_quality_rampup && qp_counts.resolutions_adaptations > 0 &&
      cpu_counts.Total() == 0) {
...
std::string OveruseFrameDetectorResourceAdaptationModule::ActiveCountsToString()
  ...
  for (size_t reason = 0; reason < active_counts_.size(); ++reason) {
    ss << (reason ? " cpu" : "quality") << ":";
    ss << active_counts_[reason].resolutions_adaptations;
  }
  ss << "}";
