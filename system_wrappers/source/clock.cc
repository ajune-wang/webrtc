...
// advancement of the clock. We could then replace this compare_exchange_weak
// loop with just a thread checker. But currently, that breaks a couple of
// tests, RepeatingTaskTest.ClockIntegration and CallStatsTest.LastProcessedRtt.
void SimulatedClock::AdvanceTime(TimeDelta delta) {
  int64_t now_us = time_us_.load(std::memory_order_relaxed);
  while (!time_us_.compare_exchange_weak(now_us, now_us + delta.us(),
