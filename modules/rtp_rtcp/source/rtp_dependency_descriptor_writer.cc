...
bool WriteTemplateFdiffs(
  ...
  for (const FrameDependencyTemplate& current_template : templates) {
    for (uint32_t fdiff : current_template.frame_diffs) {
      RTC_DCHECK_GT(fdiff, 0);
      RTC_DCHECK_LT(fdiff - 1, 0b10000);
      if (!buffer->WriteBits(0b10000 | (fdiff - 1), 5))
        return false;
    }
...
bool WriteResolutions(rtc::ArrayView<const RenderResolution> resolutions,
                      rtc::BitBufferWriter* buffer) {
  if (buffer->RemainingBitCount() < resolutions.size() * 32 * 8) {
    return false;
  }
  for (const RenderResolution& resolution : resolutions) {
    bool write_ok =
        buffer->WriteUInt16(resolution.Width() - 1) &&
        buffer->WriteUInt16(resolution.Height() - 1) RTC_DCHECK(write_ok);
  }
  return true;
}
...
bool WriteTemplateDependencyStructure(
  ...
  if (!WriteTemplateChains(structure, bit_writer))
    return false;
  uint32_t has_resolutions = resolutions.empty() ? 0 : 1;
  if (!buffer->WriteBits(has_resolutions, /*bit_count=*/1))
    return false;
  if (has_resolutions) {
