...
Timestamp PacingController::NextSendTime() {
  ...
  if (media_rate_ > DataRate::Zero() && !packet_queue_->Empty()) {
    if (media_debt_.IsZero()) {
      // No debt so far. The first packet is delayed send_burst_interval_ from
      // when it was enqueued.
      TimeDelta time_to_send_next_packet =
          send_burst_interval_ - (now - OldestPacketEnqueueTime());
      // Packet may be counted to media debt from
      // `last_process_time_` ago if the call to NextProcessTime is late but not
      // earlier than that.
      next_send_time =
          std::max(last_process_time_, now + time_to_send_next_packet);
    } else {
      // We have a media debt. If packets are allowed to be sent in a burst, the
      ...
      next_send_time = last_process_time_ + ((send_burst_interval_ > drain_time)
                                                 ? TimeDelta::Zero()
                                                 : drain_time);
    }
