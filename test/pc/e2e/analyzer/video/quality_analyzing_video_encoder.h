...
class QualityAnalyzingVideoEncoder : public VideoEncoder,
 ...
 private:
  enum Mode {
    ...
    // images will be produced. It affects what encoded images will be then
    // passed to the video quality analyzer.
    kSimulcast,
    // In this mode encoder assumes that for each frame encoded images for
    // different spatial layers will be produced. It affects what encoded images
    // will be then passed to the video quality analyzer.
    kSVP
  };
  ...
  Mode mode_ RTC_GUARDED_BY(lock_);
