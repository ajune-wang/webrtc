    ...
    switch (nalu.type) {
      ...
      case H264::NaluType::kIdr: {
        ...
        if (video_header.is_first_packet_in_frame) {
          if (nalu.pps_id == -1) {
            NLOG(LS_WARNING, "No PPS id in IDR nalu.");
            return kRequestKeyframe;
          }

          pps = pps_data_.find(nalu.pps_id);
          if (pps == pps_data_.end()) {
            NLOG(LS_WARNING, "No PPS with id , ", nalu.pps_id, " received");
            return kRequestKeyframe;
          }

          sps = sps_data_.find(pps->second.sps_id);
          if (sps == sps_data_.end()) {
            NLOG(LS_WARNING, "No SPS with id , ", pps->second.sps_id,
                 " received");
            return kRequestKeyframe;
          }
...
void H264SpsPpsTracker::InsertSpsPpsNalus(const std::vector<uint8_t>& sps,
  ...
  if (sps.size() < kNaluHeaderOffset) {
    NLOG(LS_WARNING, "SPS size  ", sps.size(), " is smaller than ",
         kNaluHeaderOffset);
    return;
  }
  if ((sps[0] & 0x1f) != H264::NaluType::kSps) {
    NLOG(LS_WARNING, "SPS Nalu header missing");
    return;
  }
  if (pps.size() < kNaluHeaderOffset) {
    NLOG(LS_WARNING, "PPS size  ", pps.size(), " is smaller than ",
         kNaluHeaderOffset);
    return;
  }
  ...
  if (!parsed_sps) {
    NLOG(LS_WARNING, "Failed to parse SPS.");
  }

  if (!parsed_pps) {
    NLOG(LS_WARNING, "Failed to parse PPS.");
  }
  ...
  NLOG(LS_INFO, "Inserted SPS id ", parsed_sps->id, " and PPS id ",
