...
PeerConnection::~PeerConnection() {
  ...
  // Former WebRtcSession destruction.
  ...
  if (sctp_transport_) {
    ...
    network_thread()->Invoke<void>(
        RTC_FROM_HERE, [this] { DestroySctpTransport_n(); });
  }
  ...
  // Former WebRtcSession destruction end.
...
bool PeerConnection::Initialize(
  ...
  transport_controller_->SignalReceiving.connect(
      this, &PeerConnection::OnTransportControllerReceiving);
  ...
  // End WebRtcSession initialization.

  stats_.reset(new StatsCollector(this));
  stats_collector_ = RTCStatsCollector::Create(this);

  // Need to set configuration before initialization PeerConnection because it
  // will reach back to fetch the media config.
...
rtc::scoped_refptr<RtpSenderInterface> PeerConnection::CreateSender(
  ...
  if (kind == MediaStreamTrackInterface::kAudioKind) {
    new_sender = RtpSenderProxyWithInternal<RtpSenderInternal>::Create(
        signaling_thread(),
        new AudioRtpSender(voice_channel(), stats_.get()));
...
void PeerConnection::SetRemoteDescription(
  ...
  // would have been destroyed by PeerConnection's SetRemoteDescription method
  // above, so the cleanup that relies on them fails to run. This is hard to fix
  // with PeerConnection and PeerConnection separated, but once the classes are
...
void PeerConnection::SetIceConnectionReceiving(bool receiving) {
  RTC_DCHECK(signaling_thread()->IsCurrent());
  if (IsClosed()) {
    return;
  }
...
rtc::scoped_refptr<DataChannel> PeerConnection::InternalCreateDataChannel(
  ...
  rtc::scoped_refptr<DataChannel> channel(DataChannel::Create(
      this, data_channel_type(), label, new_config));
  if (!channel) {
...
void PeerConnection::OnVoiceChannelCreated() {
  SetChannelOnSendersAndReceivers<AudioRtpSender, AudioRtpReceiver>(
      voice_channel(), senders_, receivers_,
...
void PeerConnection::OnVideoChannelCreated() {
  SetChannelOnSendersAndReceivers<VideoRtpSender, VideoRtpReceiver>(
      video_channel(), senders_, receivers_,
...
// Former WebRtcSession code starts here.
...
static std::string GetErrorCodeString(webrtc::PeerConnection::Error err) {
  std::string result;
  switch (err) {
...
bool PeerConnection::UpdateSessionState(
...
bool PeerConnection::PushdownMediaDescription(
...
void PeerConnection::OnTransportControllerReceiving(bool receiving) {
...
bool PeerConnection::GetLocalCandidateMediaIndex(const std::string& content_name,
                                                int* sdp_mline_index) {
...
bool PeerConnection::CreateVoiceChannel(const cricket::ContentInfo* content,
  ...
  cricket::VoiceChannel* voice_channel =
      channel_manager()->CreateVoiceChannel(
          call_.get(), configuration_.media_config,
...
bool PeerConnection::CreateDataChannel(const cricket::ContentInfo* content,
  ...
  if (sctp) {
    ...
    rtp_data_channel_ = channel_manager()->CreateRtpDataChannel(
        configuration_.media_config, rtp_dtls_transport,
...
void PeerConnection::OnSctpTransportReadyToSendData_n() {
  ...
  sctp_invoker_->AsyncInvoke<void>(
      ...
      [this] { OnSctpTransportReadyToSendData_s(true); });
}
...
void PeerConnection::OnSctpTransportDataReceived_n(
  ...
  sctp_invoker_->AsyncInvoke<void>(
      RTC_FROM_HERE, signaling_thread(),
      [this, params, payload] { OnSctpTransportDataReceived_s(params, payload); });
}
...
bool PeerConnection::HasRtcpMuxEnabled(
    const cricket::ContentInfo* content) {
...
bool PeerConnection::ValidateSessionDescription(
  ...
  if (source == cricket::CS_LOCAL) {
    if (!ExpectSetLocalDescription(action))
      return BadLocalSdp(type, BadStateErrMsg(signaling_state()),
                         err_desc);
  } else {
    if (!ExpectSetRemoteDescription(action))
      return BadRemoteSdp(type, BadStateErrMsg(signaling_state()),
                          err_desc);
  }
