...
TEST(Normal, AvoidDivideByZero) {
  ...
  // Zero input length.
  EXPECT_EQ(0, normal.Process(input, 0, NetEq::Modes::kExpand, &output));
...
TEST(Normal, InputLengthAndChannelsDoNotMatch) {
  ...
  EXPECT_EQ(0,
            normal.Process(input, input_len, NetEq::Modes::kExpand, &output));
  EXPECT_EQ(0u, output.Size());
...
TEST(Normal, LastModeExpand120msPacket) {
  ...
  EXPECT_EQ(
      ...
      normal.Process(input, kPacketsizeBytes, NetEq::Modes::kExpand, &output));
