...
TEST_F(RtpDemuxerTest, CanAddSinkWhenExistingAmbigousPayloadTypeBindingExists) {
  constexpr uint32_t ssrc = 10;
  constexpr uint8_t payload_type = 30;
  ...
  MockRtpPacketSink sink2;
  RtpDemuxerCriteria ssrcWithPayloadTypeCriteria;
  ssrcWithPayloadTypeCriteria.ssrcs = {ssrc};
  ssrcWithPayloadTypeCriteria.payload_types = {payload_type};
  EXPECT_TRUE(AddSink(ssrcWithPayloadTypeCriteria, &sink2));
}
...
TEST_F(RtpDemuxerTest, SinkCouldBeAddedWhenPayloadType) {
  constexpr uint32_t ssrc = 404;
  MockRtpPacketSink sink;
  AddSinkOnlySsrc(ssrc, &sink);
