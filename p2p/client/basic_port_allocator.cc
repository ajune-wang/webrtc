...
void BasicPortAllocatorSession::SetCandidateFilter(uint32_t filter) {
  ...
  for (PortData& port : ports_) {
    if (port.error() || port.pruned()) {
      continue;
    }
    PortData::State cur_state = port.state();
    bool found_signalable_candidate = false;
    for (const auto& c : port.port()->Candidates()) {
      if (!IsAllowedByCandidateFilter(c, prev_filter) &&
        ...
        if (!found_signalable_candidate) {
          ...
          port.set_state(PortData::STATE_INPROGRESS);
        }
        // Note that firing the signal would also let the session set the port
        // ready if needed. See again OnCandidateReady below.
        port.port()->SignalCandidateReady(port.port(), c);
      }
    }
    // Restore the previous state.
    port.set_state(cur_state);
    if (found_signalable_candidate) {
      // With at least one new signalable candidate, the check below is surely
      // false.
      continue;
    }
