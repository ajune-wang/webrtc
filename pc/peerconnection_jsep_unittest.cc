...
TEST_F(PeerConnectionJsepTest,
       CreateOfferRecyclesWhenStoppingTransceiverOnAnswerer) {
  auto caller = CreatePeerConnection();
  auto first_transceiver = caller->AddTransceiver(cricket::MEDIA_TYPE_AUDIO);
  auto callee = CreatePeerConnection();
...
// Tests that when a caller offers twice after a media section is not in use,
// the media section will be properly recycled by correctly generating
// offer/answer and updating transceivers.
TEST_F(PeerConnectionJsepTest, CreateOfferRecyclesWhenOfferingTwice) {
  ...
  ASSERT_TRUE(
      caller->SetLocalDescription(CloneSessionDescription(second_offer.get())));
