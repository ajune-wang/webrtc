...
class PeerConnectionWrapperForRampUpTest : public PeerConnectionWrapper {
 ...
 private:
  ...
  // The port allocator is owned by the underlying PeerConnection. We hold a
  // reference to it to be able to set its flags for filtering ICE candidates.
...
class PeerConnectionRampUpTest : public ::testing::Test {
 public:
  ...
  bool CreatePeerConnectionWrappers(const RTCConfiguration& caller_config,
                                    ...
                                    bool fake_cert_verification = false) {
    caller_ =
        CreatePeerConnectionWrapper(caller_config, fake_cert_verification);
    callee_ =
        CreatePeerConnectionWrapper(callee_config, fake_cert_verification);
    return caller_ && callee_;
  }

  std::unique_ptr<PeerConnectionWrapperForRampUpTest>
  CreatePeerConnectionWrapper(const RTCConfiguration& config,
                              bool fake_cert_verification) {
    ...
    return rtc::MakeUnique<PeerConnectionWrapperForRampUpTest>(
        pc_factory_, pc, std::move(observer), port_allocator);
  }
  ...
  void SetPortAllocatorFlags(PeerConnectionWrapperForRampUpTest* pc_wrapper,
                             uint32_t flags) {
    network_thread()->Invoke<void>(
        RTC_FROM_HERE, rtc::Bind(&cricket::PortAllocator::set_flags,
                                 pc_wrapper->port_allocator(), flags));
  }
...
TEST_F(PeerConnectionRampUpTest, TurnOverTLS) {
  ...
  ASSERT_TRUE(
      CreatePeerConnectionWrappers(client_1_config, client_2_config, true));
...
TEST_F(PeerConnectionRampUpTest, TCPPeerToPeer) {
  ASSERT_TRUE(CreatePeerConnectionWrappers(
      ...
      PeerConnectionInterface::RTCConfiguration()));
  SetPortAllocatorFlags(caller(), cricket::PORTALLOCATOR_DISABLE_UDP);
  SetPortAllocatorFlags(callee(), cricket::PORTALLOCATOR_DISABLE_UDP);
