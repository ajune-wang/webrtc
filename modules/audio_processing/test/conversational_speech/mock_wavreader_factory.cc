...
std::unique_ptr<WavReaderInterface> MockWavReaderFactory::CreateMock(
  ...
  if (it == audiotrack_names_params_.end()) {
    NLOG(LS_VERBOSE, "using default parameters for ", filepath);
  ...
  NLOG(LS_VERBOSE, "using ad-hoc parameters for ", filepath);
  NLOG(LS_VERBOSE, "sample_rate ", it->second.sample_rate);
  NLOG(LS_VERBOSE, "num_channels ", it->second.num_channels);
  NLOG(LS_VERBOSE, "num_samples ", it->second.num_samples);
  return std::unique_ptr<WavReaderInterface>(
