...
EncodedImage VideoStreamEncoder::AugmentEncodedImage(
  ...
  const size_t spatial_or_simulcast_idx = encoded_image.SpatialIndex().value_or(
      encoded_image.SimulcastIndex().value_or(0));
  frame_encode_metadata_writer_.FillTimingInfo(spatial_or_simulcast_idx,
                                               &image_copy);
  ...
  if (image_copy.qp_ < 0 && qp_parsing_allowed_) {
    ...
    image_copy.qp_ = qp_parser_
                         .Parse(codec_type, spatial_or_simulcast_idx,
                                image_copy.data(), image_copy.size())
                         .value_or(-1);
  }
  RTC_LOG(LS_VERBOSE) << __func__ << " spatial_or_simulcast_idx "
                      << spatial_or_simulcast_idx << " qp " << image_copy.qp_;
  ...
  RTC_CHECK(videocontenttypehelpers::SetSimulcastId(
      &image_copy.content_type_,
      static_cast<uint8_t>(spatial_or_simulcast_idx + 1)));
