...
void LogBasedNetworkControllerSimulation::ProcessUntil(Timestamp to_time) {
  if (last_process_.IsInfinite()) {
    ...
    config.constraints.min_data_rate = DataRate::KilobitsPerSecond(30);
    config.constraints.starting_rate = DataRate::KilobitsPerSecond(300);
...
void LogBasedNetworkControllerSimulation::OnPacketSent(
  ...
  sent_packet.send_time_ms = packet.log_packet_time.Milliseconds();
...
void LogBasedNetworkControllerSimulation::OnFeedback(
  ...
  auto feedback_time = Timestamp::Milliseconds(feedback.log_time_ms());
...
void LogBasedNetworkControllerSimulation::OnReceiverReport(
  ...
  if (report.rr.report_blocks().empty())
    return;
  auto report_time = Timestamp::Milliseconds(report.log_time_ms());
...
void LogBasedNetworkControllerSimulation::OnIceConfig(
  ...
  if (candidate.type == IceCandidatePairConfigType::kSelected) {
    auto log_time = Timestamp::Microseconds(candidate.log_time_us());
    ProcessUntil(log_time);
    NetworkRouteChange msg;
    msg.at_time = log_time;
    msg.constraints.min_data_rate = DataRate::KilobitsPerSecond(30);
    msg.constraints.starting_rate = DataRate::KilobitsPerSecond(300);
