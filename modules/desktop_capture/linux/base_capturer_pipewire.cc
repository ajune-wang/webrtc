...
bool BaseCapturerPipeWire::Init(
  ...
  type_ = type;
  id_ = id;
  auto_close_connection_ = auto_close_connection;
  ...
  webrtc::XdgDesktopPortalBase* xdpBase = options.xdp_base();

  if (xdpBase->IsConnectionInitialized(id_)) {
  ...
  xdpBase->InitPortal(cb, requested_type, id_);
...
std::unique_ptr<DesktopCapturer> BaseCapturerPipeWire::CreateRawScreenCapturer(
  ...
  if (!capturer->Init(options, XdgDesktopPortalBase::CaptureSourceType::kScreen,
...
std::unique_ptr<DesktopCapturer> BaseCapturerPipeWire::CreateRawWindowCapturer(
  ...
  std::unique_ptr<BaseCapturerPipeWire> capturer =
      std::make_unique<BaseCapturerPipeWire>();
  if (!capturer->Init(options, XdgDesktopPortalBase::CaptureSourceType::kWindow,
...
BaseCapturerPipeWire::CreateRawPipeWireCapturer(
    const DesktopCaptureOptions& options,
    XdgDesktopPortalBase::CaptureSourceType type,
    int32_t id,
    bool auto_close_connection) {
  ...
  std::unique_ptr<BaseCapturerPipeWire> capturer =
      std::make_unique<BaseCapturerPipeWire>();
  if (!capturer->Init(options, type,
                      XdgDesktopPortalBase::CaptureSourceType::kAny, id,
