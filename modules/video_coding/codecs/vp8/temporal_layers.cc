...
bool TemporalLayersChecker::CheckAndUpdateBufferState(
  ...
  if (flags & TemporalLayers::BufferFlags::kReference) {
    ...
    if (!frame_is_keyframe && !state->is_keyframe &&
      ...
      NLOG(LS_ERROR, "Frame is referencing higher temporal layer.");
      return false;
    }
  }
...
bool TemporalLayersChecker::CheckTemporalConfig(
  ...
  if (frame_config.packetizer_temporal_idx >= num_temporal_layers_ ||
    ...
    NLOG(LS_ERROR, "Incorrect temporal layer set for frame: ",
  ...
  if (!CheckAndUpdateBufferState(
    ...
    NLOG(LS_ERROR, "Error in the Last buffer");
    return false;
  }
  if (!CheckAndUpdateBufferState(&golden_, &need_sync, frame_is_keyframe,
    ...
    NLOG(LS_ERROR, "Error in the Golden buffer");
    return false;
  }
  if (!CheckAndUpdateBufferState(
          ...
          sequence_number_, &lowest_sequence_referenced)) {
    NLOG(LS_ERROR, "Error in the Arf buffer");
    return false;
  }

  if (lowest_sequence_referenced < last_sync_sequence_number_ &&
      !frame_is_keyframe) {
    NLOG(LS_ERROR, "Reference past the last sync frame. Referenced ",
  ...
  if (need_sync != frame_config.layer_sync) {
    NLOG(LS_ERROR,
         "Sync bit is set incorrectly on a frame. Expected: ", need_sync,
         " Actual: ", frame_config.layer_sync);
    return false;
  }
  return true;
}
