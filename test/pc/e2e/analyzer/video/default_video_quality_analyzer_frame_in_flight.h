...
class FrameInFlight {
 ...
 private:
  ...
  // Set of peer's indexes who are expected to receive this frame. In some point
  // of time frame may actually be received by peers which are not in this set.
  // For example peer A was among expected receivers, it received frame and then
  // was removed. Stats for peer A will be preserved in the FrameInFlight, but
  // A won't be in the `expected_receivers_` set.
  //
