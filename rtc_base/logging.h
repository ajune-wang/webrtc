...
class LogParametersEater {
 public:
  // Pass by value.
  template <typename T, std::enable_if_t<pass_by_value<T>>* = nullptr>
  LogParametersEater& operator<<(T) {
    return *this;
  }

  // Pass by reference.
  template <typename T, std::enable_if_t<!pass_by_value<T>>* = nullptr>
  LogParametersEater& operator<<(const T&) {
    return *this;
  }
};
...
class LogMessageVoidify {
 public:
  ...
  // higher than ?:
  void operator&(LogParametersEater&) {}
};
...
#define RTC_DLOG_EAT_STREAM_PARAMS()                \
  ...
  ::rtc::webrtc_logging_impl::LogMessageVoidify() & \
      ::rtc::webrtc_logging_impl::LogParametersEater()
