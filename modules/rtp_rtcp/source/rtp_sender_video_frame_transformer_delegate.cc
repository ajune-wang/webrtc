...
class TransformableVideoSenderFrame : public TransformableVideoFrameInterface {
 ...
 private:
  ...
  // never dereferenced - there are no guarantees that it's still alive.
...
bool RTPSenderVideoFrameTransformerDelegate::TransformFrame(
  ...
  RTPVideoFrameSenderInterface* originating_sender;
  {
    // Make sure we don't hold the sender_lock_ when calling Transform().
    MutexLock lock(&sender_lock_);
    originating_sender = sender_;
  }

  frame_transformer_->Transform(std::make_unique<TransformableVideoSenderFrame>(
      encoded_image, video_header, payload_type, codec_type, rtp_timestamp,
      expected_retransmission_time, ssrc_,
      /*csrcs=*/std::vector<uint32_t>(), originating_sender));
  return true;
}
