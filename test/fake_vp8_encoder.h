...
class FakeVP8Encoder : public FakeEncoder, public EncodedImageCallback {
 ...
 private:
  ...
  EncodedImageCallback* callback_ RTC_ACCESS_ON(sequence_checker_);

  std::vector<std::unique_ptr<TemporalLayers>> temporal_layers_
      RTC_ACCESS_ON(sequence_checker_);
  std::vector<uint16_t> picture_id_ RTC_ACCESS_ON(sequence_checker_);
  std::vector<uint8_t> tl0_pic_idx_ RTC_ACCESS_ON(sequence_checker_);
};
