#include "net/dcsctp/speed_test/app.h"
...
App::App(std::string address, uint16_t port, bool offerer)
  ...
  _signaling.on_message([&](int id, const std::string& message) {
    ...
    if (v.isMember("candidate")) {
      std::cout << "########### -> RECEIVED ICE CANDIDATE [" << id
                << "] ###########" << std::endl;
      Ice ice;
...
void App::on_sdp(int id, const std::string& sdp) {
  ...
  sdp_o["id"] = id;
  if (_offerer) {
    std::cout << "########### SEND OFFER [" << id << "] -> ###########"
              << std::endl;
    sdp_o["offer"] = sdp;
  } else {
    std::cout << "########### SEND ANSWER [" << id << "] -> ###########"
              << std::endl;
    sdp_o["answer"] = sdp;
  }
...
void App::on_ice(int id, const Ice& ice) {
  std::cout << "######### SEND ICE CANDIDATE [" << id << "] -> ###########"
            << std::endl;
  Json::Value ice_o;
  ice_o["id"] = id;
...
void App::CreateOffer(int id) {
  ...
  con->_on_sdp = std::bind(&App::on_sdp, this, std::placeholders::_1,
                           std::placeholders::_2);
  con->_on_ice = std::bind(&App::on_ice, this, std::placeholders::_1,
                           std::placeholders::_2);
  con->_on_accept_ice = std::bind(&App::on_accept_ice, this);

  webrtc::PeerConnectionDependencies dependencies(con.get());

  auto result = _peer_connection_factory->CreatePeerConnectionOrError(
      _configuration, std::move(dependencies));
  if (!result.ok()) {
  ...
  con->CreateDataChannel("data_channel_" + std::to_string(con->_id));

  con->_peer_connection->CreateOffer(
      con->_csdo, webrtc::PeerConnectionInterface::RTCOfferAnswerOptions());
...
void App::OnOffer(int id, const std::string& parameter) {
  ...
  std::cout << "    App::" << __func__ << " id=" << id
            << " con->_id=" << con->_id << std::endl;
  ...
  _connections[con->_id]->_peer_connection->SetRemoteDescription(
      _connections[con->_id]->_ssdo, session_description);
  _connections[con->_id]->_peer_connection->CreateAnswer(
      _connections[con->_id]->_csdo,
...
void App::OnAnswer(int id, const std::string& parameter) {
  ...
  _connections[id]->_peer_connection->SetRemoteDescription(
      _connections[id]->_ssdo, session_description);
