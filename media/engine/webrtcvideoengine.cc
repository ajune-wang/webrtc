...
void WebRtcVideoChannel::WebRtcVideoSendStream::SetCodec(
  ...
  if (!parameters_.config.rtp.rtx.ssrcs.empty()) {
    // According to RFC 2198, the sender will send RED by default, but also may
    // send original media.  In WebRTC, we only send RED for ULPFEC and original
    // video stream, if RTX enabled we also should send RTX for RED.
    if (codec_settings.ulpfec.red_payload_type != -1 &&
        codec_settings.ulpfec.red_rtx_payload_type != -1) {
      // RED and RTX enabled, use RED RTX payload type.
      parameters_.config.rtp.rtx.payload_type =
          codec_settings.ulpfec.red_rtx_payload_type;
