...
TEST_F(TestVCMJitterEstimator, TestUpperBound) {
  ...
  // Test jitter estimate cap nullopt and non-nullopt code paths
  absl::optional<double> jitter_est_cap_ms = absl::nullopt;
  while (jitter_est_cap_ms.has_value() && jitter_est_cap_ms.value() <= 200.0) {
