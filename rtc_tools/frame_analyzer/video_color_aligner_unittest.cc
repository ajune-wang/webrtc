...
void ExpectNear(const ColorTransformationMatrix& expected,
  ...
  const float kAbsErrorYuv = 1.0e-3;
  EXPECT_NEAR(expected[0][0], actual[0][0], kAbsErrorYuv);
  EXPECT_NEAR(expected[0][1], actual[0][1], kAbsErrorYuv);
  EXPECT_NEAR(expected[0][2], actual[0][2], kAbsErrorYuv);
  EXPECT_NEAR(expected[0][3], actual[0][3], kAbsErrorOffset);

  EXPECT_NEAR(expected[1][0], actual[1][0], kAbsErrorYuv);
  EXPECT_NEAR(expected[1][1], actual[1][1], kAbsErrorYuv);
  EXPECT_NEAR(expected[1][2], actual[1][2], kAbsErrorYuv);
  EXPECT_NEAR(expected[1][3], actual[1][3], kAbsErrorOffset);

  EXPECT_NEAR(expected[2][0], actual[2][0], kAbsErrorYuv);
  EXPECT_NEAR(expected[2][1], actual[2][1], kAbsErrorYuv);
  EXPECT_NEAR(expected[2][2], actual[2][2], kAbsErrorYuv);
  EXPECT_NEAR(expected[2][3], actual[2][3], kAbsErrorOffset);
}
...
TEST_F(VideoColorAlignerTest, CalculateColorTransformationMatrix) {
  ...
  const ColorTransformationMatrix result_color_matrix =
      CalculateColorTransformationMatrix(
          AdjustColors(org_matrix, reference_video_), reference_video_);
