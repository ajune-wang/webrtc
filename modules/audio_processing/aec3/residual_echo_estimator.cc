...
void ApplyNoiseGate(const EchoCanceller3Config::EchoModel& config,
  ...
  RTC_DCHECK_EQ(X2.size(), kFftLengthBy2Plus1);
  for (size_t k = 0; k < kFftLengthBy2Plus1; ++k) {
...
void ResidualEchoEstimator::Estimate(
  ...
  if (aec_state.UsableLinearEstimate()) {
    ...
    if (aec_state.SaturatedEcho()) {
      ...
      for (size_t k = 0; k < kFftLengthBy2Plus1; ++k) {
        X2[k] = std::max(0.f, X2[k] - config_.echo_model.stationary_gate_slope *
                                          X2_noise_floor_[k]);
      }
...
void ResidualEchoEstimator::AddReverb(
  ...
  std::array<float, kFftLengthBy2Plus1> render_power_data;
  rtc::ArrayView<const float> render_power;
  const size_t delay = aec_state.FilterLengthBlocks() + 1;
  if (num_render_channels_ == 1) {
    render_power = render_buffer.Spectrum(delay, /*channel=*/0);
  } else {
    render_power_data.fill(0.f);
    for (size_t ch = 0; ch < num_render_channels_; ++ch) {
      const auto& channel_power = render_buffer.Spectrum(delay, ch);
      RTC_DCHECK_EQ(channel_power.size(), kFftLengthBy2Plus1);
      for (size_t k = 0; k < kFftLengthBy2Plus1; ++k) {
        render_power_data[k] += channel_power[k];
      }
  ...
  rtc::ArrayView<const float> reverb_power = echo_reverb_.get_reverb();
  RTC_DCHECK_EQ(reverb_power.size(), kFftLengthBy2Plus1);
  for (size_t ch = 0; ch < num_capture_channels; ++ch) {
    for (size_t k = 0; k < kFftLengthBy2Plus1; ++k) {
