...
bool JsepTransport::AddChannel(DtlsTransportInternal* dtls, int component) {
  if (channels_.find(component) != channels_.end()) {
    NLOG(LS_ERROR, "Adding channel for component ", component, " twice.");
    return false;
  }
  ...
  if (local_description_set_ || remote_description_set_) {
    NLOG(LS_WARNING,
         "Adding new transport channel after "
...
bool JsepTransport::RemoveChannel(int component) {
  ...
  if (it == channels_.end()) {
    NLOG(LS_ERROR, "Trying to remove channel for component ", component,
         ", which doesn't exist.");
    return false;
  }
