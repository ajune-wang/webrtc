...
std::vector<RtpVideoSender::RtpStreamSender> CreateRtpStreamSenders(
  ...
  std::vector<RtpVideoSender::RtpStreamSender> rtp_streams;
  ...
  for (uint32_t ssrc : rtp_config.ssrcs) {
    RtpVideoSender::RtpStreamSender stream;
    stream.playout_delay_oracle = absl::make_unique<PlayoutDelayOracle>();
    ...
    configuration.ack_observer = stream.playout_delay_oracle.get();
    ...
    stream.sender_video = absl::make_unique<RTPSenderVideo>(
        ...
        stream.playout_delay_oracle.get(), frame_encryptor,
        crypto_options.sframe.require_frame_encryption);
...
RtpVideoSender::RtpStreamSender::RtpStreamSender(RtpStreamSender&&) = default;
RtpVideoSender::RtpStreamSender& RtpVideoSender::RtpStreamSender::operator=(
    RtpStreamSender&&) = default;
RtpVideoSender::RtpStreamSender::~RtpStreamSender() = default;
