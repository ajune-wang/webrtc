...
void EventLogAnalyzer::CreateOutgoingTWCCLossRateGraph(Plot* plot) {
  ...
  std::unordered_set<int64_t> previouse_feedback_lost_sequence_numbers;
  size_t previouse_feedback_size = 0;
  for (auto& feedback : parsed_log_.transport_feedbacks(kIncomingPacket)) {
        ...
        [&](uint16_t sequence_number, TimeDelta receive_time_delta) {
          ...
          if (receive_time_delta.IsInfinite()) {
            ...
            num_reordered_packets +=
                previouse_feedback_lost_sequence_numbers.count(unwrapped_seq);
          }
        });
    ...
    auto reordered_rate =
        previouse_feedback_size == 0
            ? 0
            : static_cast<float>(num_reordered_packets * 100.0 /
                                 previouse_feedback_size);
    ...
    std::swap(previouse_feedback_lost_sequence_numbers, lost_sequence_numbers);
    previouse_feedback_size = transport_feedback.GetPacketStatusCount();
  }
