...
class RtpVideoSenderTestFixture {
 public:
  RtpVideoSenderTestFixture(
      ...
      FrameCountObserver* frame_count_observer)
      : time_controller_(Timestamp::Milliseconds(1000000)),
...
TEST(RtpVideoSenderTest, DoesNotRetrasmitAckedPackets) {
  ...
  test.AdvanceTime(TimeDelta::Milliseconds(33));
...
TEST(RtpVideoSenderTest, RetransmitsOnTransportWideLossInfo) {
  ...
  call_conf.transport.rates.max_rate = DataRate::KilobitsPerSecond(300);
  call_conf.transport.rates.start_rate = DataRate::KilobitsPerSecond(300);
  test::NetworkSimulationConfig net_conf;
  net_conf.bandwidth = DataRate::KilobitsPerSecond(300);
