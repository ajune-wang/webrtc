...
int StereoEncoderAdapter::Release() {
  ...
  while (iter != stashed_images_.end()) {
    auto inner_iter = iter->second.image_components.begin();
    while (inner_iter != iter->second.image_components.end()) {
      if (inner_iter->encoded_image._buffer) {
        delete[] inner_iter->encoded_image._buffer;
      }
      inner_iter++;
    }
    iter++;
