...
NetEqTest::SimulationStepResult NetEqTest::RunToNextGetAudio() {
  ...
  while (!input_->ended()) {
    ...
    if (event && event->type() == NetEqInput::Event::Type::kPacketData) {
    ...
    if (event && event->type() == NetEqInput::Event::Type::kSetMinimumDelay) {
      neteq_->SetBaseMinimumDelayMs(
          static_cast<NetEqInput::SetMinimumDelay&>(*event).delay_ms);
    }

    // Check if it is time to get output audio.
    if (event && event->type() == NetEqInput::Event::Type::kGetAudio) {
