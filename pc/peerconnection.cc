...
RTCError PeerConnection::UpdateDataChannel(
    ...
    const cricket::ContentInfo& content,
    const cricket::ContentGroup* bundle_group) {
  if (content.rejected && data_channel_type_ != cricket::DCT_NONE) {
    DestroyDataChannel();
  } else {
    ...
    if (source == cricket::CS_REMOTE) {
      ...
      if (data_desc && rtc::starts_with(data_desc->protocol().data(),
                                        cricket::kMediaProtocolRtpPrefix)) {
        UpdateRemoteRtpDataChannels(GetActiveStreams(data_desc));
      }
    }
...
void PeerConnection::GetOptionsForUnifiedPlanOffer(
  ...
  for (size_t i = 0;
    ...
    if (media_type == cricket::MEDIA_TYPE_AUDIO ||
      ...
      RTC_CHECK_EQ(media_type, transceiver->internal()->media_type());
