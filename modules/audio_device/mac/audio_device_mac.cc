...
void AudioDeviceMac::logCAMsg(const rtc::LoggingSeverity sev,
  ...
  switch (sev) {
    case rtc::LS_ERROR:
      NLOG(LS_ERROR, msg, ": ", err[0], err[1], err[2], err[3]);
      break;
    case rtc::LS_WARNING:
      NLOG(LS_WARNING, msg, ": ", err[0], err[1], err[2], err[3]);
      break;
    case rtc::LS_VERBOSE:
      NLOG(LS_VERBOSE, msg, ": ", err[0], err[1], err[2], err[3]);
      break;
    default:
      break;
  }
  ...
  switch (sev) {
    case rtc::LS_ERROR:
      NLOG(LS_ERROR, msg, ": ", err[3], err[2], err[1], err[0]);
      break;
    case rtc::LS_WARNING:
      NLOG(LS_WARNING, msg, ": ", err[3], err[2], err[1], err[0]);
      break;
    case rtc::LS_VERBOSE:
      NLOG(LS_VERBOSE, msg, ": ", err[3], err[2], err[1], err[0]);
      break;
    default:
      break;
  }
...
AudioDeviceMac::~AudioDeviceMac() {
  ...
  if (kernErr != KERN_SUCCESS) {
    NLOG(LS_ERROR, "semaphore_destroy() error: ", kernErr);
  }
...
AudioDeviceGeneric::InitStatus AudioDeviceMac::Init() {
  ...
  if (_paRenderBuffer == NULL) {
    ...
    if (bufSize == -1) {
      NLOG(LS_ERROR, "PaUtil_InitializeRingBuffer() error");
  ...
  kernErr = semaphore_create(mach_task_self(), &_renderSemaphore,
                             SYNC_POLICY_FIFO, 0);
  if (kernErr != KERN_SUCCESS) {
    NLOG(LS_ERROR, "semaphore_create() error: ", kernErr);
  ...
  if (intErr != 0) {
    NLOG(LS_ERROR, "Error in sysctlbyname(): ", err);
  } else {
    NLOG(LS_VERBOSE, "Hardware model: ", buf);
...
int32_t AudioDeviceMac::Terminate() {
  ...
  if (_recording) {
    NLOG(LS_ERROR, "Recording must be stopped");
    return -1;
  }

  if (_playing) {
    NLOG(LS_ERROR, "Playback must be stopped");
    return -1;
  }
...
int32_t AudioDeviceMac::SetPlayoutDevice(uint16_t index) {
  ...
  NLOG(LS_VERBOSE, "number of available waveform-audio output devices is ",
       nDevices);
...
int32_t AudioDeviceMac::SetRecordingDevice(uint16_t index) {
  ...
  NLOG(LS_VERBOSE, "number of available waveform-audio input devices is ",
       nDevices);
...
int32_t AudioDeviceMac::InitPlayout() {
  ...
  if (!MicrophoneIsInitialized()) {
    ...
    if (MicrophoneIsAvailable(available) == -1) {
      NLOG(LS_WARNING, "MicrophoneIsAvailable() failed");
    }
  }
  ...
  if (_macBookPro) {
    ...
    if (hasProperty) {
      ...
      if (dataSource == 'ispk') {
        ...
        NLOG(LS_VERBOSE, "MacBook Pro not using internal speakers");
      }
  ...
  if (_outStreamFormat.mChannelsPerFrame > N_DEVICE_CHANNELS) {
    NLOG(LS_ERROR, "Too many channels on output device (mChannelsPerFrame = ",
         _outStreamFormat.mChannelsPerFrame, ")");
    return -1;
  }

  if (_outStreamFormat.mFormatFlags & kAudioFormatFlagIsNonInterleaved) {
    NLOG(LS_ERROR, "Non-interleaved audio data is not supported.",
         "AudioHardware streams should not have this format.");
    return -1;
  }

  NLOG(LS_VERBOSE, "Ouput stream format:");
  NLOG(LS_VERBOSE, "mSampleRate = ", _outStreamFormat.mSampleRate,
       ", mChannelsPerFrame = ", _outStreamFormat.mChannelsPerFrame);
  NLOG(LS_VERBOSE, "mBytesPerPacket = ", _outStreamFormat.mBytesPerPacket,
       ", mFramesPerPacket = ", _outStreamFormat.mFramesPerPacket);
  NLOG(LS_VERBOSE, "mBytesPerFrame = ", _outStreamFormat.mBytesPerFrame,
       ", mBitsPerChannel = ", _outStreamFormat.mBitsPerChannel);
  NLOG(LS_VERBOSE, "mFormatFlags = ", _outStreamFormat.mFormatFlags);
  ...
  if (_outStreamFormat.mChannelsPerFrame < 2) {
    ...
    _playChannels = 1;
    NLOG(LS_VERBOSE, "Stereo playout unavailable on this device");
  }
...
int32_t AudioDeviceMac::InitRecording() {
  ...
  if (!SpeakerIsInitialized()) {
    ...
    if (SpeakerIsAvailable(available) == -1) {
      NLOG(LS_WARNING, "SpeakerIsAvailable() failed");
    }
  }
  ...
  if (_inStreamFormat.mChannelsPerFrame > N_DEVICE_CHANNELS) {
    NLOG(LS_ERROR, "Too many channels on input device (mChannelsPerFrame = ",
  ...
  if (io_block_size_samples > _captureBufSizeSamples) {
    NLOG(LS_ERROR, "Input IO block size (", io_block_size_samples,
         ") is larger than ring buffer (", _captureBufSizeSamples, ")");
    return -1;
  }

  NLOG(LS_VERBOSE, "Input stream format:");
  NLOG(LS_VERBOSE, "mSampleRate = ", _inStreamFormat.mSampleRate,
       ", mChannelsPerFrame = ", _inStreamFormat.mChannelsPerFrame);
  NLOG(LS_VERBOSE, "mBytesPerPacket = ", _inStreamFormat.mBytesPerPacket,
       ", mFramesPerPacket = ", _inStreamFormat.mFramesPerPacket);
  NLOG(LS_VERBOSE, "mBytesPerFrame = ", _inStreamFormat.mBytesPerFrame,
       ", mBitsPerChannel = ", _inStreamFormat.mBitsPerChannel);
  NLOG(LS_VERBOSE, "mFormatFlags = ", _inStreamFormat.mFormatFlags);
  logCAMsg(rtc::LS_VERBOSE, "mFormatID",
           (const char*)&_inStreamFormat.mFormatID);
  ...
  if (_inStreamFormat.mChannelsPerFrame >= 2 && (_recChannels == 2)) {
    ...
    _recChannels = 1;
    NLOG(LS_VERBOSE, "Stereo recording unavailable on this device");
  }
...
int32_t AudioDeviceMac::StartRecording() {
  ...
  if (!_initialized) {
    NLOG(LS_ERROR, "Recording worker thread has not been started");
    return -1;
  }
  ...
  if (_twoDevices && captureDeviceIsAlive == 1) {
    ...
    if (_recording) {
      ...
      if (kEventTimeout == _stopEventRec.Wait(2000)) {
        ...
        NLOG(LS_WARNING, "Timed out stopping the capture IOProc.",
      ...
      _critSect.Enter();
      _doStopRec = false;
      NLOG(LS_INFO, "Recording stopped (input device)");
    } else if (_recIsInitialized) {
      WEBRTC_CA_LOG_WARN(
          AudioDeviceDestroyIOProcID(_inputDeviceID, _inDeviceIOProcID));
      NLOG(LS_INFO, "Recording uninitialized (input device)");
    }
  } else {
    ...
    if (_recording && captureDeviceIsAlive == 1) {
      ...
      if (kEventTimeout == _stopEvent.Wait(2000)) {
        rtc::CritScope critScoped(&_critSect);
        NLOG(LS_WARNING, "Timed out stopping the shared IOProc.",
             "We may have failed to detect a device removal.");
      ...
      _critSect.Enter();
      _doStop = false;
      NLOG(LS_INFO, "Recording stopped (shared device)");
    } else if (_recIsInitialized && !_playing && !_playIsInitialized) {
      WEBRTC_CA_LOG_WARN(
          AudioDeviceDestroyIOProcID(_outputDeviceID, _deviceIOProcID));
      NLOG(LS_INFO, "Recording uninitialized (shared device)");
    }
  }
...
int32_t AudioDeviceMac::StopPlayout() {
  ...
  if (_playing && renderDeviceIsAlive == 1) {
    ...
    if (kEventTimeout == _stopEvent.Wait(2000)) {
      rtc::CritScope critScoped(&_critSect);
      NLOG(LS_WARNING, "Timed out stopping the render IOProc.",
           "We may have failed to detect a device removal.");
    ...
    _critSect.Enter();
    _doStop = false;
    NLOG(LS_INFO, "Playout stopped");
  } else if (_twoDevices && _playIsInitialized) {
    WEBRTC_CA_LOG_WARN(
        AudioDeviceDestroyIOProcID(_outputDeviceID, _deviceIOProcID));
    NLOG(LS_INFO, "Playout uninitialized (output device)");
  } else if (!_twoDevices && _playIsInitialized && !_recIsInitialized) {
    WEBRTC_CA_LOG_WARN(
        AudioDeviceDestroyIOProcID(_outputDeviceID, _deviceIOProcID));
    NLOG(LS_INFO, "Playout uninitialized (shared device)");
  }
...
int32_t AudioDeviceMac::GetNumberDevices(const AudioObjectPropertyScope scope,
  ...
  if (usedID != kAudioDeviceUnknown) {
    ...
    NLOG(LS_WARNING, "GetNumberDevices(): Default device unknown");
  }
    ...
    for (UInt32 i = 0; i < numberDevices; i++) {
      ...
      if (bufferList->mNumberBuffers > 0) {
        if (numberScopedDevices >= deviceListLength) {
          NLOG(LS_ERROR, "Device list is not long enough");
          listOK = false;
          break;
        }
...
int32_t AudioDeviceMac::GetDeviceName(const AudioObjectPropertyScope scope,
  ...
  if (index == 0) {
    ...
    if (usedID == kAudioDeviceUnknown) {
      NLOG(LS_WARNING, "GetDeviceName(): Default device unknown");
    } else {
...
int32_t AudioDeviceMac::InitDevice(const uint16_t userDeviceIndex,
  ...
  if (numberDevices < 0) {
    ...
    NLOG(LS_ERROR, "InitDevice(): No devices");
    return -1;
  }
  ...
  if (userDeviceIndex == 0) {
    ...
    if (deviceId == kAudioDeviceUnknown) {
      NLOG(LS_WARNING, "No default device exists");
    } else {
      isDefaultDevice = true;
    }
  }
  ...
  if (isInput) {
    NLOG(LS_INFO, "Input device: ", devManf, " ", devName);
  } else {
    NLOG(LS_INFO, "Output device: ", devManf, " ", devName);
  }
...
OSStatus AudioDeviceMac::SetDesiredPlayoutFormat() {
  ...
  NLOG(LS_VERBOSE, "initial playout status: _renderDelayOffsetSamples=",
       _renderDelayOffsetSamples, ", _renderDelayUs=", _renderDelayUs,
...
OSStatus AudioDeviceMac::implObjectListenerProc(
    ...
    const AudioObjectPropertyAddress addresses[]) {
  NLOG(LS_VERBOSE, "AudioDeviceMac::implObjectListenerProc()");
...
int32_t AudioDeviceMac::HandleDeviceChange() {
  ...
  NLOG(LS_VERBOSE, "kAudioHardwarePropertyDevices");
  ...
  if (MicrophoneIsInitialized()) {
    ...
    if (err == kAudioHardwareBadDeviceError || deviceIsAlive == 0) {
      NLOG(LS_WARNING, "Capture device is not alive (probably removed)");
      AtomicSet32(&_captureDeviceIsAlive, 0);
      _mixerManager.CloseMicrophone();
    } else if (err != noErr) {
  ...
  if (SpeakerIsInitialized()) {
    ...
    if (err == kAudioHardwareBadDeviceError || deviceIsAlive == 0) {
      NLOG(LS_WARNING, "Render device is not alive (probably removed)");
      AtomicSet32(&_renderDeviceIsAlive, 0);
      _mixerManager.CloseSpeaker();
    } else if (err != noErr) {
      logCAMsg(rtc::LS_ERROR, "Error in AudioDeviceGetProperty()",
...
int32_t AudioDeviceMac::HandleStreamFormatChange(
  ...
  NLOG(LS_VERBOSE, "Stream format changed");
  ...
  if (streamFormat.mChannelsPerFrame > N_DEVICE_CHANNELS) {
    NLOG(LS_ERROR, "Too many channels on device (mChannelsPerFrame = ",
         streamFormat.mChannelsPerFrame, ")");
    return -1;
  }

  if (_ptrAudioBuffer && streamFormat.mChannelsPerFrame != _recChannels) {
    NLOG(LS_ERROR, "Changing channels not supported (mChannelsPerFrame = ",
         streamFormat.mChannelsPerFrame, ")");
    return -1;
  }

  NLOG(LS_VERBOSE, "Stream format:");
  NLOG(LS_VERBOSE, "mSampleRate = ", streamFormat.mSampleRate,
       ", mChannelsPerFrame = ", streamFormat.mChannelsPerFrame);
  NLOG(LS_VERBOSE, "mBytesPerPacket = ", streamFormat.mBytesPerPacket,
       ", mFramesPerPacket = ", streamFormat.mFramesPerPacket);
  NLOG(LS_VERBOSE, "mBytesPerFrame = ", streamFormat.mBytesPerFrame,
       ", mBitsPerChannel = ", streamFormat.mBitsPerChannel);
  NLOG(LS_VERBOSE, "mFormatFlags = ", streamFormat.mFormatFlags);
...
int32_t AudioDeviceMac::HandleDataSourceChange(
  ...
  if (_macBookPro &&
    ...
    NLOG(LS_VERBOSE, "Data source changed");
...
int32_t AudioDeviceMac::HandleProcessorOverload(
  ...
  if (propertyAddress.mScope == kAudioDevicePropertyScopeInput) {
    // NLOG(LS_WARNING  , "Capture processor // overload");
    //_callback->ProblemIsReported(
    // SndCardStreamObserver::ERecordingProblem);
  } else {
    // NLOG(LS_WARNING  , "Render processor overload");
    //_callback->ProblemIsReported(
...
OSStatus AudioDeviceMac::implDeviceIOProc(const AudioBufferList* inputData,
    ...
    if (_doStop) {
      if (_twoDevices || (!_recording && !_playing)) {
        ...
        WEBRTC_CA_LOG_WARN(
            AudioDeviceDestroyIOProcID(_outputDeviceID, _deviceIOProcID));
        if (err == noErr) {
          NLOG(LS_VERBOSE, "Playout or shared device stopped");
        }
      }
  ...
  if (err != noErr) {
    if (err == 1) {
      ...
      NLOG(LS_ERROR, "Error in AudioConverterFillComplexBuffer()");
      return 1;
    } else {
...
OSStatus AudioDeviceMac::implOutConverterProc(UInt32* numberDataPackets,
  ...
  if (kernErr != KERN_SUCCESS) {
    NLOG(LS_ERROR, "semaphore_signal_all() error: ", kernErr);
    return 1;
  }
  ...
  if (_doStopRec) {
    ...
    if (_doStopRec) {
      ...
      WEBRTC_CA_LOG_WARN(
          AudioDeviceDestroyIOProcID(_inputDeviceID, _inDeviceIOProcID));
      if (err == noErr) {
        NLOG(LS_VERBOSE, "Recording device stopped");
      }
...
OSStatus AudioDeviceMac::implInConverterProc(UInt32* numberDataPackets,
  ...
  while (PaUtil_GetRingBufferReadAvailable(_paCaptureBuffer) < numSamples) {
    ...
    if (kernErr == KERN_OPERATION_TIMED_OUT) {
      ...
      NLOG(LS_ERROR, "semaphore_wait() error: ", kernErr);
    }
  }
...
bool AudioDeviceMac::RenderWorkerThread() {
  ...
  while (PaUtil_GetRingBufferWriteAvailable(_paRenderBuffer) -
    ...
    if (kernErr == KERN_OPERATION_TIMED_OUT) {
      ...
      NLOG(LS_ERROR, "semaphore_timedwait() error: ", kernErr);
    }
  }
  ...
  if (!_ptrAudioBuffer) {
    NLOG(LS_ERROR, "capture AudioBuffer is invalid");
    return false;
  }
