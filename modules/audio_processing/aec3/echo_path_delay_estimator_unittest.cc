...
TEST(EchoPathDelayEstimator, BasicApiCalls) {
  ...
  std::unique_ptr<RenderDelayBuffer> render_delay_buffer(
      RenderDelayBuffer::Create(
          3, config.delay_estimation.down_sampling_factor,
          GetDownSampledBufferSize(config.delay_estimation.down_sampling_factor,
                                   config.delay_estimation.num_filters),
          GetRenderDelayBufferSize(config.delay_estimation.down_sampling_factor,
                                   config.delay_estimation.num_filters)));
...
TEST(EchoPathDelayEstimator, DelayEstimation) {
  ...
  for (auto down_sampling_factor : kDownSamplingFactors) {
    EchoCanceller3Config config;
    config.delay_estimation.down_sampling_factor = down_sampling_factor;
    config.delay_estimation.num_filters = 10;
      ...
      std::unique_ptr<RenderDelayBuffer> render_delay_buffer(
          RenderDelayBuffer::Create(
              ...
              GetDownSampledBufferSize(
                  config.delay_estimation.down_sampling_factor,
                  config.delay_estimation.num_filters),
