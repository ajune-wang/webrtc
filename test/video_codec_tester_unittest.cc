...
TEST_P(VideoCodecTesterTestPacing, PaceEncode) {
  ...
  EncodingSettings encoding_settings = VideoCodecTester::CreateEncodingSettings(
      env, "VP8", {ScalabilityMode::kL1T1}, {video_source.resolution},
      {video_source.framerate}, {kBitrate});
...
struct CreatEncodingSettingsArgs {
  Environment env = CreateEnvironment();
  std::string codec_type = "VP8";
  std::vector<std::string> scalability_modes = "L1T1";
  std::vector<Resolution> resolutions = {{.width = 1280, .height = 720}};
  std::vector<Frequency> framerates = {Frequency::Hertz(30)};
  std::vector<DataRate> bitrates = {DataRate::KilobitsPerSec(2048)};
  bool screencast = false;
  bool frame_drop = true;
};

EncodingSettings CreateEncodinSetting(CreatEncodingSettingsArgs args) {
  return VideoCodecTester::CreateEncodingSettings(
      args.env, args.codec_type, args.scalability_modes, args.resolutions,
      args.framerates, args.bitrates, args.screencast, args.frame_drop);
}

TEST(VideoCodecTester, CreateEncodinSettingSimulcastDefault) {
  EncodingSettings es =
      CreateEncodinSetting({.scalability_modes = ScalabilityMode::kS3T1});
  es
}
...
TEST_P(VideoCodecTesterTestEncodingSettings, CreateEncodingSettings) {
  ...
  EncodingSettings encoding_settings =
      CreateEncodinSetting({.codec_type = test_params.codec_type,
                            .scalability_mode = test_params.scalability_mode,
                            .bitrates = test_params.requested_bitrate,
                            .framerates = test_params.requested_framerate,
                            .screencast = test_params.screencast});
...
TEST(VideoCodecTester, DISABLED_CompressedVideoSource) {
  ...
  EncodingSettings encoding_settings = VideoCodecTester::CreateEncodingSettings(
      env, "AV1", {ScalabilityMode::kL1T1}, {source_settings.resolution},
      {source_settings.framerate}, {DataRate::KilobitsPerSec(128)});
