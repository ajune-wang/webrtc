...
CombineAndScaleFactory::CombineAndScaleFactory(float algorithmic_delay,
                                               bool modifies_input_signal)
    : algorithmic_delay_(algorithmic_delay),
      modifies_input_signal_(modifies_input_signal) {}

std::unique_ptr<EchoControlEnhancer> CombineAndScaleFactory::Create(
    int sample_rate_hz,
    int num_input_channels) {
  return std::make_unique<CombineAndScale>(
      num_input_channels, algorithmic_delay_, modifies_input_signal_);
}
