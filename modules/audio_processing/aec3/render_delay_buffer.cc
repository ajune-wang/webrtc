...
void RenderDelayBufferImpl::ResetAlignment() {
  ...
  b.next_read_index =
      b.OffsetIndex(b.last_insert_index, -static_cast<int>(delay_));
  s.next_read_index =
      s.OffsetIndex(s.last_insert_index, static_cast<int>(delay_));
  f.next_read_index =
      f.OffsetIndex(f.last_insert_index, static_cast<int>(delay_));
...
RenderDelayBufferImpl::BufferingEvent RenderDelayBufferImpl::UpdateBuffers() {
  const auto increase_read = [&]() {
    downsampled_render_buffer_.UpdateNextReadIndex(
        -static_cast<int>(sub_block_size_));
    block_buffer_.IncNextReadIndex();
    spectrum_buffer_.DecNextReadIndex();
    fft_buffer_.DecNextReadIndex();
  };

  const auto increase_insert = [&]() {
    downsampled_render_buffer_.UpdateLastInsertIndex(
        -static_cast<int>(sub_block_size_));
    block_buffer_.IncLastInsertIndex();
    spectrum_buffer_.DecLastInsertIndex();
    fft_buffer_.DecLastInsertIndex();
  };
  ...
  for (size_t k = 0; k < render_blocks_available; ++k) {
    ...
    if (block_buffer_.OffsetIndex(block_buffer_.last_insert_index,
                                  kBufferHeadroom) ==
...
void RenderDelayBufferImpl::SetDelay(size_t delay) {
  ...
  block_buffer_.UpdateNextReadIndex(delta_delay);
  spectrum_buffer_.UpdateNextReadIndex(-delta_delay);
  fft_buffer_.UpdateNextReadIndex(-delta_delay);
}
