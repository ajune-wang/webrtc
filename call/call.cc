...
class Call final : public webrtc::Call,
 ...
 private:
  ...
  std::map<uint32_t, VideoSendStream*> video_send_ssrcs_
      RTC_GUARDED_BY(network_thread_);
  std::set<VideoSendStream*> video_send_streams_ RTC_GUARDED_BY(network_thread_);
...
// TODO(tommi): Move this code into the dtor to make threading clearer and allow
// some stats to be gathered on the network thread.
...
void Call::DestroyAudioReceiveStream(
  ...
  // TODO(bugs.webrtc.org/11993): Do async.
  Invoke(network_thread_, [this, audio_receive_stream](){
    RTC_DCHECK_RUN_ON(network_thread_);
    ...
    // TODO(tommi): If no stream remains - delete audio_receiver_controller_.
  });
...
void Call::UpdateAggregateNetworkState() {
  ...
  worker_thread_->PostTask(
      ToQueuedTask(task_safety_, [this, aggregate_network_up]() {
        RTC_DCHECK_RUN_ON(worker_thread_);
