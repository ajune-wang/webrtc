 ...
 public:
  ...
  void OnErrorResponse(StunMessage* response) override {
    ...
    port_->OnStunBindingOrResolveRequestFailed(server_addr_,
        attr ? attr->number() : STUN_ERROR_GLOBAL_FAILURE,
        attr ? attr->reason() :
  ...
  void OnTimeout() override {
    RTC_LOG(LS_ERROR) << "Binding request timed out from "
                      ...
                      << port_->Network()->name() << ")";
    port_->OnStunBindingOrResolveRequestFailed(server_addr_,
        SERVER_NOT_REACHABLE_ERROR, "STUN allocate request timed out");
  }
  ...
  if (stun_addr.IsUnresolvedIP()) {
    ...
    if (IsCompatibleAddress(stun_addr)) {
      ...
      OnStunBindingOrResolveRequestFailed(stun_addr,
          SERVER_NOT_REACHABLE_ERROR, reason);
    }
  }
}
...
void UDPPort::OnStunBindingOrResolveRequestFailed(
    const rtc::SocketAddress& stun_server_addr, int error_code,
