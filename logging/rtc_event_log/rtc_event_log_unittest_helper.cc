...
void VerifyLoggedAudioRecvConfig(
  ...
  EXPECT_EQ(original_event.config_->rtp_extensions.size(),
            logged_event.config.rtp_extensions.size());
  size_t recognized_extensions = 0;
  for (size_t i = 0; i < kMaxNumExtensions; i++) {
    auto original_id = GetExtensionId(original_event.config_->rtp_extensions,
                                      kExtensions[i].name);
    auto logged_id =
        GetExtensionId(logged_event.config.rtp_extensions, kExtensions[i].name);
    EXPECT_EQ(original_id, logged_id);
    if (original_id)
      recognized_extensions++;
  }
  EXPECT_EQ(recognized_extensions,
            original_event.config_->rtp_extensions.size());
}
...
void VerifyLoggedAudioSendConfig(
  ...
  EXPECT_EQ(original_event.config_->rtx_ssrc, logged_event.config.rtx_ssrc);
  EXPECT_EQ(original_event.config_->rsid, logged_event.config.rsid);
  EXPECT_EQ(original_event.config_->rtp_extensions,
