...
void AecState::Update(
  ...
  filter_has_had_time_to_converge_ =
      blocks_with_proper_filter_adaptation_ >= 0.5f * kNumBlocksPerSecond;
...
void AecState::UpdateReverb(const std::vector<float>& impulse_response) {
  ...
  if (current_reverb_decay_section_ < config_.filter.main.length_blocks) {
    ...
    float nz = accumulated_nz_;
    float n = accumulated_count_;
    const size_t start_index = current_reverb_decay_section_ * kFftLengthBy2;

    float section_energy = std::accumulate(
        matching_data.begin() + start_index,
        matching_data.begin() + start_index + kFftLengthBy2, 0.f)
        / kFftLengthBy2;

    section_energy += 1e-32; // Regularization to avoid division by 0.
    ...
    if (!found_end_of_reverb_decay_ &&
        section_energy > tail_energy_ &&
        !main_filter_is_adapting_) {
    ...
    if (num_reverb_decay_sections_ > 0) {
      ...
      for (size_t i = 0 ; i < kFftLengthBy2 ; i++) {
        auto fast_approx_log2f = [] (const float in) {
          union { float dummy; uint32_t a; } x = { in };
          float out = x.a;
          out *= 1.1920928955078125e-7f;
          out -= 126.94269504f;
          return out;
        };
      ...
      accumulated_nz_ = nz;
      accumulated_count_ = n;
    }

    num_reverb_decay_sections_ = num_reverb_decay_sections_ > 0 ?
        num_reverb_decay_sections_ - 1 : 0;
    ...
    const float nom = -accumulated_nz_;
    const float den = accumulated_nn_;

    if (den != 0.f) {
      const float exp_candidate = nom / den;
      decay = powf(2.0f, -exp_candidate * kFftLengthBy2);
      decay = fminf(decay, kMaxDecay);
      decay = fmaxf(decay, kMinDecay);
    }

    // Filter tail energy (assumed to be noise)

    constexpr size_t kTailLength = kFftLengthBy2 * 2;
    ...
    tail_energy_ = std::accumulate(matching_data.begin() + tail_index,
                                   matching_data.end(), 0.f) / kTailLength;
    ...
    const size_t peak_index = std::distance(matching_data.begin(),
        std::max_element(matching_data.begin(), matching_data.end()));

    // Make sure we're not out of bounds
    current_reverb_decay_section_ = (peak_index / kFftLengthBy2) + 3;
    ...
    float first_section_energy = std::accumulate(
        matching_data.begin() + start_index,
        matching_data.begin() + start_index + kFftLengthBy2, 0.f)
        / kFftLengthBy2;
    ...
    // from the max peak
    bool main_filter_has_reverb =
        first_section_energy > 4.f * tail_energy_;
    bool main_filter_is_sane = false;
    if (first_section_energy > 2.f * tail_energy_ &&
        matching_data[peak_index] < 100.f) {
      main_filter_is_sane = true;
    }
    ...
    if (!main_filter_is_adapting_ && main_filter_is_sane &&
        num_reverb_decay_sections_ > 0) {
      decay = fmaxf(.97f * reverb_decay_, decay);
      reverb_decay_ = .9f * reverb_decay_ + .1f * decay;
    }

    const float nn = (float)num_reverb_decay_sections_ * kFftLengthBy2;
    accumulated_nz_ = 0.f;
    accumulated_nn_ = nn * (nn * nn - 1.0f) / 12.0f; // Caluclated directly
    accumulated_count_ = - nn / 2.0f;
    // Linear regression approach assumes symmetric index around 0
    accumulated_count_ = accumulated_count_ + 0.5f;
