...
void WebRtcSpl_MinMaxW16Neon(const int16_t* data, size_t length,
  ...
  // First part, unroll the loop 8 times.
  for (i = 0; i < length - residual; i += 8) {
    int16x8_t in16x8 = vld1q_s16(&data[i]);
    min16x8 = vminq_s16(min16x8, in16x8);
    max16x8 = vmaxq_s16(max16x8, in16x8);
  }
...
// Finds both the minimum and maximum elements in an array of 32-bit integers.
void WebRtcSpl_MinMaxW32Neon(const int32_t* data, size_t length,
  ...
  const int32_t* p_start = data;
  int32x4_t min32x4 = vdupq_n_s32(WEBRTC_SPL_WORD32_MAX);
  int32x4_t max32x4 = vdupq_n_s32(WEBRTC_SPL_WORD32_MIN);

  // First part, unroll the loop 4 times.
  for (i = 0; i < length - residual; i += 4) {
    int32x4_t in = vld1q_s32(&data[i]);
    min32x4 = vminq_s32(min32x4, in);
    max32x4 = vmaxq_s32(max32x4, in);
  }
