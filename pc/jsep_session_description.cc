...
void UpdateConnectionAddress(
  ...
  if (rtc::IPIsUnspec(connection_addr.ipaddr()) && !hostname.empty()) {
    ...
    // processing by the same spec, we have observed in deployment that with a
    // FQDN in a c= line, SDP parsing could fail in other JSEP implementations.
    // Note also that the wildcard addresses  (0.0.0.0 or ::) with port 9 are
    // given the exception as the connection address that will not result in an
    // ICE mismatch. Combining the above considerations, we use 0.0.0.0 with
    // port 9 to populate the c= and the m= lines. See |BuildMediaDescription|
    // in webrtc_sdp.cc for the SDP generation with
