...
// class. They are computed from |target_level| and used for decision making.
void DelayManager::BufferLimits(int target_level,
  ...
  *lower_limit = (target_level * 3) / 4;
  ...
  if (deceleration_target_level_offset_ && packet_len_ms_ > 0) {
    ...
    *lower_limit = std::max(*lower_limit,
                            target_level - deceleration_target_level_offset_q8);
  }
  ...
  *higher_limit = std::max(target_level, *lower_limit + window_20ms_q8);
}
