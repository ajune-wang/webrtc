...
// Represents a PeerConnection simulation scenario. The main purpose is to
// maintain ownership and ensure safe destruction order of clients and network
// emulation. Additionally it reduces the amount of bolier plate requited for
// some actions. For example usage see the existing tests using this class.
// Note that it should be used from a single calling thread. This thread will
// also be assigned as the signaling thread for all peer connections that are
// created. This means that the process methods must be used when waiting to
// ensure that messages are processed on the signaling thread.
class PeerScenario {
 public:
  PeerScenario();
