...
class RTC_LOCKABLE RTC_EXPORT Thread : public webrtc::TaskQueueBase {
 public:
  ...
  class ScopedCountBlockingCalls {
   public:
    ...
    uint32_t GetWouldBeBlockingCallCount() const;
    uint32_t GetTotalBlockedCallCount() const;

   private:
    Thread* const thread_;
    const uint32_t base_blocking_call_count_;
    const uint32_t base_would_be_blocking_call_count_;
 ...
 private:
  ...
  uint32_t would_be_blocking_call_count_ RTC_GUARDED_BY(this) = 0;
