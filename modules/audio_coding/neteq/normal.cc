  ...
  if (last_mode == kModeExpand) {
    ...
    for (size_t channel_ix = 0; channel_ix < output->Channels(); ++channel_ix) {
      ...
      if ((energy != 0) &&
        ...
        local_mute_factor = WebRtcSpl_SqrtFloor(ratio << 14);
      } else {
        local_mute_factor = 16384;  // 1.0 in Q14.
      }
      if (local_mute_factor > mute_factor) {
        mute_factor = static_cast<int16_t>(std::min(local_mute_factor, 16384));
      }

      // If muted increase by 0.64 for every 20 ms (NB/WB 0.0040/0.0020 in Q14).
      int increment = 64 / fs_mult;
      const int back_to_fullscale_inc =
          (16384 - mute_factor) / length_per_channel;
      if (back_to_fullscale_inc > increment) {
        increment = back_to_fullscale_inc;
      }
