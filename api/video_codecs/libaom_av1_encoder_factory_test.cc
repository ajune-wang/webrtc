...
class Av1Decoder : public DecodedImageCallback {
 ...
 private:
  void Write(const EncodedData& ed, const VideoFrame& vf) {
    ...
    WriteFrame(name, vf);
    fwrite(ed.bitstream_data->data(), 1, ed.bitstream_data->size(),
           raw_out_file_);
  }
...
TEST(LibaomAv1Encoder, L3T1_KEY) {
  ...
  Av1Decoder decoder_s0("L3T1_KEY_S0");
  Av1Decoder decoder_s1("L3T1_KEY_S1");
  Av1Decoder decoder_s2("L3T1_KEY_S2");
  const Cbr kCbr = {.duration = TimeDelta::Millis(100),
                    .target_bitrate = DataRate::KilobitsPerSec(500)};
  ...
  ASSERT_TRUE(encoder->Encode(frame_reader->PullFrame(), {},
                              {{.rate_options = kCbr,
                               ...
                               {.rate_options = kCbr,
                                ...
                                .update_buffers = {2}}},
                              res.CallBack()));
  VideoFrame f0 = decoder_s0.Decode(*res.FrameAt(0));
  EXPECT_THAT(f0.width(), Eq(160));
  EXPECT_THAT(f0.height(), Eq(90));

  decoder_s1.Decode(*res.FrameAt(0));
  VideoFrame f1 = decoder_s1.Decode(*res.FrameAt(1));
  EXPECT_THAT(f1.width(), Eq(320));
  EXPECT_THAT(f1.height(), Eq(180));

  decoder_s2.Decode(*res.FrameAt(0));
  decoder_s2.Decode(*res.FrameAt(1));
  VideoFrame f2 = decoder_s2.Decode(*res.FrameAt(2));
  EXPECT_THAT(f2.width(), Eq(640));
  EXPECT_THAT(f2.height(), Eq(360));
  ...
  for (int i = 0; i < 24; i += 3) {
    ...
    VideoFrame f3 = decoder_s0.Decode(*res.FrameAt(3 + i));
    EXPECT_THAT(f3.width(), Eq(160));
    EXPECT_THAT(f3.height(), Eq(90));

    VideoFrame f4 = decoder_s1.Decode(*res.FrameAt(4 + i));
    EXPECT_THAT(f4.width(), Eq(320));
    EXPECT_THAT(f4.height(), Eq(180));

    VideoFrame f5 = decoder_s2.Decode(*res.FrameAt(5 + i));
    EXPECT_THAT(f5.width(), Eq(640));
    EXPECT_THAT(f5.height(), Eq(360));
  }
...
TEST(LibaomAv1Encoder, S3T1) {
  ...
  Av1Decoder decoder_s0("S3T1_S0");
  Av1Decoder decoder_s1("S3T1_S1");
  Av1Decoder decoder_s2("S3T1_S2");
  const Cbr kCbr = {.duration = TimeDelta::Millis(100),
                    .target_bitrate = DataRate::KilobitsPerSec(500)};
