...
FrameBuffer::ReturnReason FrameBuffer::NextFrame(
  ...
  rtc::Event done;
  NextFrame(max_wait_time_ms, keyframe_required,
            [frame_out, &return_reason, &done](
                std::unique_ptr<EncodedFrame> frame, ReturnReason reason) {
              frame_out->reset(frame.release());
...
void FrameBuffer::NextFrame(
  ...
  task_queue_.PostTask([&] {
    ...
    StartWaitingForFrame(max_wait_time_ms, keyframe_required,
                         std::move(handler));
  });
}

void FrameBuffer::StartWaitingForFrame(
    int64_t max_wait_time_ms,
    bool keyframe_required,
    std::function<void(std::unique_ptr<EncodedFrame>, ReturnReason)> handler) {
  TRACE_EVENT0("webrtc", "FrameBuffer::StartFrameWait");
  latest_return_time_ms_ = clock_->TimeInMilliseconds() + max_wait_time_ms;
  RTC_DCHECK(!frame_wait_task_.Running());
  frame_wait_task_ = RepeatingTaskHandle::Start(task_queue_.Get(), [=] {
    RTC_DCHECK_RUN_ON(&task_queue_);
    ...
    if (stopped_) {
      frame_handler_(nullptr, kStopped);
      frame_handler_ = {};
      frame_wait_task_.Stop();
...
void FrameBuffer::HandleNewContinousFrame() {
  if (!frame_handler_) {
    pending_frame_ = true;
    return;
  }
  pending_frame_ = false;
  RTC_DCHECK(frame_handler_);
  RTC_DCHECK(frame_wait_task_.Running());
  frame_wait_task_.Stop();
  int64_t now_ms;
  {
    ...
    if (!frames_out.empty()) {
      if (frames_out.size() == 1) {
        frame_handler_(absl::WrapUnique(frames_out[0]), kFrameFound);
        frame_handler_ = {};
      } else {
        frame_handler_(absl::WrapUnique(CombineAndDeleteFrames(frames_out)),
                       kFrameFound);
        frame_handler_ = {};
      }
      return;
  ...
  if (latest_return_time_ms_ - now_ms > 0) {
    ...
    StartWaitingForFrame(latest_return_time_ms_ - now_ms, false,
                         frame_handler_);
    return;
  }
...
void FrameBuffer::SetProtectionMode(VCMVideoProtection mode) {
  ...
  task_queue_.PostTask([this, mode] {
    RTC_DCHECK_RUN_ON(&task_queue_);
...
// static
bool FrameBuffer::ValidReferences(const EncodedFrame& frame) {
...
int64_t FrameBuffer::InsertFrame(std::unique_ptr<EncodedFrame> frame) {
  int64_t last_continuous_pid;
  rtc::Event done;
  task_queue_.PostTask([&] {
    RTC_DCHECK_RUN_ON(&task_queue_);
    last_continuous_pid = InsertFrameOnQueue(std::move(frame));
    done.Set();
  });
  done.Wait(rtc::Event::kForever);
...
void FrameBuffer::InsertFrame(std::unique_ptr<EncodedFrame> frame,
  ...
  struct InsertFrameTask {
    void operator()() {
      ...
      int64_t last_continuous_pid =
          frame_buffer->InsertFrameOnQueue(std::move(frame));
...
int64_t FrameBuffer::InsertFrameOnQueue(std::unique_ptr<EncodedFrame> frame) {
