...
CallReceiveStatistics ChannelReceive::GetRTCPStatistics() const {
  ...
  if (rtcp_sr_stats.has_value()) {
    NtpTime rtcp_sr_arrival_ntp(rtcp_sr_stats->last_arrival_timestamp_secs,
                                rtcp_sr_stats->last_arrival_timestamp_frac);
    NtpTime rtcp_sr_remote_ntp(rtcp_sr_stats->last_remote_timestamp_secs,
                               rtcp_sr_stats->last_remote_timestamp_frac);
    stats.last_sender_report_timestamp_ms = rtcp_sr_arrival_ntp.ToMs();
    stats.last_sender_report_remote_timestamp_ms = rtcp_sr_remote_ntp.ToMs();
