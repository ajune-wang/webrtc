 ...
 public:
  ...
  class TestEncoder : public test::FakeEncoder {
   public:
    ...
    int GetNumberOfInitializations() {
      rtc::CritScope lock(&local_crit_sect_);
      return number_of_initializations_;
    }
   ...
   private:
    ...
    int number_of_initializations_ RTC_GUARDED_BY(local_crit_sect_) = 0;
  };
...
TEST_F(VideoStreamEncoderTest, MaxBitrateChangeDoesNotTriggerEncoderInit) {
  VideoEncoderConfig video_encoder_config;
  test::FillEncoderConfiguration(kVideoCodecVP8, 1, &video_encoder_config);
  ...
  // received.
  EXPECT_EQ(1, sink_.number_of_reconfigurations());
  EXPECT_EQ(1, fake_encoder_.GetNumberOfInitializations());
  EXPECT_EQ(kTargetBitrateBps,
            bitrate_allocator_factory_.codec_.maxBitrate * 1000);
  ...
  WaitForEncodedFrame(2);
  EXPECT_EQ(2, sink_.number_of_reconfigurations());
  // Encoder is not supposed to be re-initialized on change of max bitrate.
  EXPECT_EQ(1, fake_encoder_.GetNumberOfInitializations());
