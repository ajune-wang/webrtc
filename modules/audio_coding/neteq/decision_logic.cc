    ...
    int low_limit =
        std::max(target_level_samples * 3 / 4,
                 target_level_samples -
                     kDecelerationTargetLevelOffsetMs * sample_rate_ / 1000);
    // |higher_limit| is equal to |target_level|, but should at
    // least be 20 ms higher than |lower_limit|.
    int high_limit =
        std::max(target_level_samples, low_limit + 20 * sample_rate_ / 1000);

    int buffer_level_samples = buffer_level_filter_.filtered_current_level();
