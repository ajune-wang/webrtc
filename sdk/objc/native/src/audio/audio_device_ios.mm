...
#define LOGI() RTC_DLOG(LS_INFO) << "AudioDeviceIOS::"

#define LOG_AND_RETURN_IF_ERROR(error, message)    \
  do {                                             \
    OSStatus err = error;                          \
    if (err) {                                     \
      RTC_DLOG(LS_ERROR) << message << ": " << err; \
      return false;                                \
    }                                              \
  } while (0)
...
static void LogDeviceInfo() {
  RTC_DLOG(LS_INFO) << "LogDeviceInfo";
  @autoreleasepool {
    RTC_DLOG(LS_INFO) << " system name: " << ios::GetSystemName();
    RTC_DLOG(LS_INFO) << " system version: " << ios::GetSystemVersionAsString();
    RTC_DLOG(LS_INFO) << " device type: " << ios::GetDeviceType();
    RTC_DLOG(LS_INFO) << " device name: " << ios::GetDeviceName();
    RTC_DLOG(LS_INFO) << " process name: " << ios::GetProcessName();
    RTC_DLOG(LS_INFO) << " process ID: " << ios::GetProcessID();
    RTC_DLOG(LS_INFO) << " OS version: " << ios::GetOSVersionString();
    RTC_DLOG(LS_INFO) << " processing cores: " << ios::GetProcessorCount();
    RTC_DLOG(LS_INFO) << " low power mode: " << ios::GetLowPowerModeEnabled();
#if TARGET_IPHONE_SIMULATOR
    RTC_DLOG(LS_INFO) << " TARGET_IPHONE_SIMULATOR is defined";
#endif
    RTC_DLOG(LS_INFO) << " DeviceIsSimulator: " << DeviceIsSimulator();
  }
}
...
int32_t AudioDeviceIOS::StartPlayout() {
  ...
  if (!recording_ && audio_unit_->GetState() == VoiceProcessingAudioUnit::kInitialized) {
    ...
    RTC_DLOG(LS_INFO) << "Voice-Processing I/O audio unit is now started";
  }
...
void AudioDeviceIOS::SetupAudioBuffersForActiveAudioSession() {
  ...
  if (sample_rate != webRTCConfig.sampleRate) {
    RTC_DLOG(LS_WARNING) << "Unable to set the preferred sample rate";
  }
  ...
  RTC_DLOG(LS_INFO) << " frames per I/O buffer: " << playout_parameters_.frames_per_buffer();
  RTC_DLOG(LS_INFO) << " bytes per I/O buffer: " << playout_parameters_.GetBytesPerBuffer();
