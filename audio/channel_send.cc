...
class ChannelSend : public ChannelSendInterface,
 ...
 private:
  ...
  const std::unique_ptr<VoERtcpObserver> report_block_observer_;
...
class VoERtcpObserver : public ReportBlockDataObserver {
 public:
  explicit VoERtcpObserver(ChannelSend* owner) : owner_(owner) {}
  ~VoERtcpObserver() override {}

  void OnReportBlockDataUpdated(ReportBlockData report_block) override {
    owner_->OnUplinkPacketLossRate(report_block.fraction_lost());
  }
...
ChannelSend::ChannelSend(
    ...
    : ssrc_(ssrc),
      ...
      report_block_observer_(new VoERtcpObserver(this)),
  ...
  configuration.report_block_data_observer = report_block_observer_.get();
