...
TimingFrameInfo::TimingFrameInfo()
    : rtp_timestamp(0),
      ...
      decode_finish_ms(-1),
      render_time_ms(-1),
      flags(VideoSendTiming::TimingFrameFlags::kNotTriggered) {}
...
bool TimingFrameInfo::IsOutlier() const {
  return !IsInvalid() &&
         (flags & VideoSendTiming::TimingFrameFlags::kTriggeredBySize);
}

bool TimingFrameInfo::IsTimerTriggered() const {
  return !IsInvalid() &&
         (flags & VideoSendTiming::TimingFrameFlags::kTriggeredByTimer);
}

bool TimingFrameInfo::IsInvalid() const {
  return flags == VideoSendTiming::TimingFrameFlags::kInvalid;
}
