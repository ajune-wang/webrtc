...
TEST_F(RtcpReceiverTest, ReportBlockDataObserverVerifyBlockAndTimestamp) {
  ...
  // The "report_block_timestamp_us" is obtained from the global UTC clock (not
  // the simulcated |system_clock_|) and requires a scoped fake clock.
  ...
  EXPECT_CALL(observer, OnReportBlockDataUpdated(_))
      .WillOnce(
          Invoke([&rtcp_block, &kUtcNowUs](ReportBlockData report_block_data) {
            ...
            EXPECT_EQ(kUtcNowUs, report_block_data.report_block_timestamp_us);
...
TEST_F(RtcpReceiverTest, ReportBlockDataObserverVerifyRTT) {
  MockReportBlockDataObserverImpl observer;
  rtcp_receiver_.SetReportBlockDataObserver(&observer);
  ...
  InSequence sequence;
  EXPECT_CALL(observer, OnReportBlockDataUpdated(_))
      .WillOnce(Invoke([&kRttMs](ReportBlockData report_block_data) {
  ...
  EXPECT_CALL(observer, OnReportBlockDataUpdated(_))
      .WillOnce(Invoke([](ReportBlockData report_block_data) {
...
TEST_F(RtcpReceiverTest, GetLatestReportBlockData) {
...

  {
    rtcp::ReportBlock rtcp_block;
    rtcp_block.SetMediaSsrc(kReceiverMainSsrc);
    rtcp_block.SetExtHighestSeqNum(kSequenceNumber1);
    ...
    EXPECT_EQ(1u, report_block_datas.size());
    EXPECT_EQ(kReceiverMainSsrc,
  ...
  // source SSRC.
  {
    rtcp::ReportBlock rtcp_block;
    rtcp_block.SetMediaSsrc(kReceiverMainSsrc);
    rtcp_block.SetExtHighestSeqNum(kSequenceNumber2);
  ...
  {
    const uint16_t kSequenceNumber3 = 42;

    rtcp::ReportBlock rtcp_block;
    rtcp_block.SetMediaSsrc(kReceiverExtraSsrc);
    rtcp_block.SetExtHighestSeqNum(kSequenceNumber3);
    ...
    auto report_block_datas = rtcp_receiver_.GetLatestReportBlockData();
    EXPECT_EQ(2u, report_block_datas.size());
    EXPECT_EQ(kReceiverMainSsrc,
              report_block_datas[0].report_block.source_ssrc);
    EXPECT_EQ(
        kSequenceNumber2,
