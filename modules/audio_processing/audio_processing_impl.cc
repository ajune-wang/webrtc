...
AudioProcessingBuilder& AudioProcessingBuilder::SetAudioEnhancerController(
    std::unique_ptr<AudioEnhancerController> audio_enhancer_controller) {
  audio_enhancer_controller_ = std::move(audio_enhancer_controller);
  return *this;
}
...
AudioProcessing* AudioProcessingBuilder::Create(const webrtc::Config& config) {
  AudioProcessingImpl* apm = new rtc::RefCountedObject<AudioProcessingImpl>(
      ...
      std::move(audio_enhancer_controller_));
...
AudioProcessingImpl::AudioProcessingImpl(const webrtc::Config& config)
    : AudioProcessingImpl(config,
                          ...
                          /*audio_enhancer_controller=*/nullptr) {}
  ...
  if (submodules_.echo_controller && !multi_channel_capture) {
    ...
    if (submodules_.audio_enhancer) {
      ...
      capture_.num_proc_channels_after_aec =
          std::min<int>(capture_.num_proc_channels_before_aec,
                        submodules_.audio_enhancer->NumOutputChannels());
      RTC_DCHECK_LE(
          static_cast<int>(submodules_.audio_enhancer->NumOutputChannels()),
          capture_.num_proc_channels_before_aec);
    } else {
      capture_.num_proc_channels_before_aec =
...
int AudioProcessingImpl::ProcessCaptureStreamLocked() {
  if (audio_enhancer_controller_) {
    audio_enhancer_controller_->UpdateEnhancementProperties();
  }
...
void AudioProcessingImpl::InitializeHighPassFilter(bool forced_reset) {
  ...
  if (submodule_states_.HighPassFilteringRequired() ||
    ...
    size_t num_channels = capture_.num_proc_channels_before_aec;
...
void AudioProcessingImpl::InitializeAudioEnhancer() {
  if (audio_enhancer_controller_) {
    submodules_.audio_enhancer = audio_enhancer_controller_->Create(
        capture_nonlocked_.capture_processing_format.sample_rate_hz(),
        capture_.num_proc_channels_before_aec);
  } else {
    submodules_.audio_enhancer = nullptr;
  }
}
  ...
  if (use_echo_controller) {
    ...
    if (echo_control_factory_) {
      submodules_.echo_controller = echo_control_factory_->Create(
          ...
          formats_.render_processing_format.num_channels(),
          capture_.num_proc_channels_before_aec,
          submodules_.audio_enhancer.get());
