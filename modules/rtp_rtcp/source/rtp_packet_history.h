...
class RtpPacketHistory {
 public:
  ...
  // that must have elapsed since the last time the packet was resent (parameter
  // is ignored if set to zero). If the packet is found but the minimum time has
  // not elapsed, returns nullptr.
  // If |update_send_state| is true, the time of last transmission is updated
  // and the number of retransmissions may be incremented. Otherwise, internal
 ...
 private:
  ...
  std::unique_ptr<RtpPacketToSend> RemovePacket(
      std::list<StoredPacket>::iterator packet)
  ...
  // requests arrive.
  std::map<uint16_t, std::list<StoredPacket>::iterator> rtp_seqno_map_
  ...
  std::map<uint16_t, std::list<StoredPacket>::iterator> tw_seqno_map_
      RTC_GUARDED_BY(lock_);
