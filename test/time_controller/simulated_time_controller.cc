...
void SimulatedSequenceRunner::PostTask(std::unique_ptr<QueuedTask> task) {
  ...
  // as the task queue but not from the task queue itself we have to assume that
...
TaskQueueBase* SimulatedTimeControllerImpl::CreateTaskQueue(
    absl::string_view name) {
  auto* task_queue = new SimulatedSequenceRunner(this, name);
  rtc::CritScope lock(&lock_);
  runners_.push_back(task_queue);
  return task_queue;  // transfers ownership to caller.
}
...
GlobalSimulatedTimeController::GlobalSimulatedTimeController(
    ...
    : global_clock_(CreateScopedClock(start_time)), impl_(start_time) {}
