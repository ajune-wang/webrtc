...
class ChannelBuffer {
 public:
  ...
  // returned and the usage becomes Usage: channels(band)[channel][sample].
  // Where:
  // 0 <= band < |num_bands_|
  // 0 <= channel < |num_allocated_channels_|
  // 0 <= sample < |num_frames_per_band_|

  // If band is not explicitly specified, full-band channels (or lower band
  // channels) and the usage becomes Usage: channels()[channel][sample]. Where:
  // 0 <= channel < |num_allocated_channels_|
