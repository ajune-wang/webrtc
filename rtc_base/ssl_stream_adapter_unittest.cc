...
class SSLStreamAdapterTestBase : public ::testing::Test,
 ...
 public:
  ...
  void SetUp() override {
    ...
    rtc::SSLIdentity* client_identity;
    if (!client_cert_pem_.empty() && !client_private_key_pem_.empty()) {
      client_identity = rtc::SSLIdentity::FromPEMStrings(
          client_private_key_pem_, client_cert_pem_);
    } else {
      client_identity = rtc::SSLIdentity::Generate("client", client_key_type_);
    }
    auto server_identity =
        rtc::SSLIdentity::Generate("server", server_key_type_);

    client_ssl_->SetIdentity(absl::WrapUnique(client_identity));
    server_ssl_->SetIdentity(absl::WrapUnique(server_identity));
  }
  ...
  void ResetIdentitiesWithValidity(int not_before, int not_after) {
    ...
    auto client_identity = rtc::SSLIdentity::GenerateForTest(client_params);
    ...
    auto server_identity = rtc::SSLIdentity::GenerateForTest(server_params);
...
TEST_F(SSLStreamAdapterTestDTLSCertChain, TwoCertHandshake) {
  auto server_identity = rtc::SSLIdentity::FromPEMChainStrings(
...
TEST_F(SSLStreamAdapterTestDTLSCertChain, TwoCertHandshakeWithCopy) {
  server_ssl_->SetIdentity(
      absl::WrapUnique(rtc::SSLIdentity::FromPEMChainStrings(
          kRSA_PRIVATE_KEY_PEM, std::string(kCERT_PEM) + kCACert)));
  TestHandshake();
  std::unique_ptr<rtc::SSLCertChain> peer_cert_chain =
      client_ssl_->GetPeerSSLCertChain();
 ...
 public:
  ...
  void ConfigureClient(std::string experiment) {
    ...
    auto client_identity =
        rtc::SSLIdentity::Generate("client", client_key_type_);
    client_ssl_->SetIdentity(absl::WrapUnique(client_identity));
  }
  ...
  void ConfigureServer(std::string experiment) {
    ...
    server_ssl_->SignalEvent.connect(
        static_cast<SSLStreamAdapterTestBase*>(this),
        &SSLStreamAdapterTestBase::OnEvent);
    server_ssl_->SetIdentity(absl::WrapUnique(
        rtc::SSLIdentity::Generate("server", server_key_type_)));
  }
};
