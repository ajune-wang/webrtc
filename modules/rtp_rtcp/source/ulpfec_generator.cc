...
void UlpfecGenerator::AddPacketAndGenerateFec(const RtpPacketToSend& packet) {
  ...
  FecProtectionParams& params = packet.is_key_frame()
                                    ? current_params_.keyframe_params
...
bool UlpfecGenerator::ExcessOverheadBelowMax() const {
  RTC_CHECK_RUNS_SERIALIZED(&race_checker_);
  const FecProtectionParams& params = keyframe_in_process_
                                          ? current_params_.keyframe_params
                                          : current_params_.delta_params;

  return ((Overhead() - params.fec_rate) < kMaxExcessOverhead);
}
...
std::vector<std::unique_ptr<RtpPacketToSend>> UlpfecGenerator::GetFecPackets() {
  ...
  for (const auto* fec_packet : generated_fec_packets_) {
    ...
    red_packet->SetPayloadSize(kRedForFecHeaderLength +
                               fec_packet->data.size());
    red_packet->SetPayloadType(red_payload_type_);
    red_packet->SetMarker(false);
    auto payload_buffer = red_packet->PayloadBuffer();
    payload_buffer[0] = ulpfec_payload_type_;  // RED header.
    memcpy(&payload_buffer.data()[1], fec_packet->data.data(),
           fec_packet->data.size());
    total_fec_size += red_packet->size();
    fec_packets.push_back(std::move(red_packet));
  }

  {
    rtc::CritScope cs(&crit_);
    fec_bitrate_.Update(total_fec_size, clock_->TimeInMilliseconds());
  }
...
int UlpfecGenerator::Overhead() const {
  ...
  const FecProtectionParams& params = keyframe_in_process_
                                          ...
                                          : current_params_.delta_params;
  int num_fec_packets =
      fec_->NumFecPackets(media_packets_.size(), params.fec_rate);
