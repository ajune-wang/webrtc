...
// Used by the class that wants tasks dropped after destruction.
...
class ScopedTaskSafety {
 public:
  ...
  ScopedTaskSafety(ScopedTaskSafety&&) = delete;
  ~ScopedTaskSafety() { flag_->SetNotAlive(); }

  // Returns a new eference to the safety flag.
  rtc::scoped_refptr<PendingTaskSafetyFlag> flag() const { return flag_; }

  rtc::scoped_refptr<PendingTaskSafetyFlag> operator()() { return flag_; }
