...
void ParsedRtcEventLogNew::StoreRtpPackets(
  ...
  if (proto.has_transport_sequence_number_deltas()) {
    absl::optional<uint64_t> transport_sequence_number;
    if (proto.has_transport_sequence_number()) {
      transport_sequence_number = proto.transport_sequence_number();
    }
    transport_sequence_number_values =
        DecodeDeltas(proto.transport_sequence_number_deltas(),
                     transport_sequence_number, number_of_deltas);
  ...
  if (proto.has_transmission_time_offset_deltas()) {
    absl::optional<uint64_t> transmission_time_offset;
    if (proto.has_transmission_time_offset()) {
      transmission_time_offset = proto.transmission_time_offset();
    }
    transmission_time_offset_values =
        DecodeDeltas(proto.transmission_time_offset_deltas(),
                     transmission_time_offset, number_of_deltas);
  ...
  if (proto.has_absolute_send_time_deltas()) {
    absl::optional<uint64_t> absolute_send_time;
    if (proto.has_absolute_send_time()) {
      absolute_send_time = proto.absolute_send_time();
    }
    absolute_send_time_values =
        DecodeDeltas(proto.absolute_send_time_deltas(), absolute_send_time,
                     number_of_deltas);
  ...
  if (proto.has_video_rotation_deltas()) {
    absl::optional<uint64_t> video_rotation;
    if (proto.has_video_rotation()) {
      video_rotation = proto.video_rotation();
    }
    video_rotation_values = DecodeDeltas(proto.video_rotation_deltas(),
                                         video_rotation, number_of_deltas);
  ...
  if (proto.has_audio_level_deltas()) {
    absl::optional<uint64_t> audio_level;
    if (proto.has_audio_level()) {
      audio_level = proto.audio_level();
    }
    audio_level_values =
        DecodeDeltas(proto.audio_level_deltas(), audio_level, number_of_deltas);
...
void ParsedRtcEventLogNew::StoreAudioNetworkAdaptationEvent(
  ...
  std::vector<absl::optional<uint64_t>> bitrate_bps_values =
      DecodeDeltas(proto.bitrate_deltas_bps(), bitrate_bps, number_of_deltas);
  RTC_CHECK_EQ(bitrate_bps_values.size(), number_of_deltas);

  // frame_length_ms
  RTC_CHECK(proto.has_frame_length_deltas_ms());
  ...
  std::vector<absl::optional<uint64_t>> frame_length_ms_values = DecodeDeltas(
      proto.frame_length_deltas_ms(), frame_length_ms, number_of_deltas);
  RTC_CHECK_EQ(frame_length_ms_values.size(), number_of_deltas);

  // uplink_packet_loss_fraction
  RTC_CHECK(proto.has_uplink_packet_loss_fraction_deltas());
  ...
  // enable_fec
  RTC_CHECK(proto.has_enable_fec_deltas());
  ...
  // enable_dtx
  RTC_CHECK(proto.has_enable_dtx_deltas());
  ...
  // num_channels
  RTC_CHECK(proto.has_num_channels_deltas());
