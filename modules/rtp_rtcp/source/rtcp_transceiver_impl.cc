...
RtcpTransceiverImpl::CompoundPacketInfo RtcpTransceiverImpl::FillReports(
  ...
  for (auto it = local_senders_.begin();
    ...
    result.sender_ssrcs.push_back(rtp_sender.ssrc);
  }
  ...
  uint32_t sender_ssrc = result.sender_ssrcs.empty()
                             ? config_.feedback_ssrc
                             : result.sender_ssrcs.front();
...
void RtcpTransceiverImpl::CreateCompoundPacket(Timestamp now,
  ...
  CompoundPacketInfo result = FillReports(now, reserved, sender);
  uint32_t sender_ssrc = result.sender_ssrcs.empty()
                             ? config_.feedback_ssrc
                             : result.sender_ssrcs.front();
  ...
  if (result.sender_ssrcs.empty() && config_.non_sender_rtt_measurement) {
  ...
  if (xr_with_dlrr.has_value()) {
    if (config_.reply_to_non_sender_rtt_mesaurments_on_all_ssrcs &&
      ...
      for (uint32_t sender_ssrc : result.sender_ssrcs) {
