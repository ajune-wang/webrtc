...
void StunRequest::SendInternal() {
  ...
  OnSent();
  QueueSend(resend_delay());
}

void StunRequest::QueueSend(int delay) {
  network_thread()->PostDelayedTask(
      webrtc::ToQueuedTask(task_safety_, [this]() { SendInternal(); }), delay);
}
...
void StunRequest::Send(webrtc::TimeDelta delay) {
  ...
  delay.IsZero() ? SendInternal() : QueueSend(delay.ms());
}
