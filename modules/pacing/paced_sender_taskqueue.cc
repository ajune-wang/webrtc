...
constexpr TimeDelta kMinTimeBetweenStatsUpdates = TimeDelta::Millis<16>();
...
void PacedSenderTaskQueue::MaybeProcessPackets(
  ...
  //  if (!scheduled_runtime && pacing_controller_.QueueSizePackets() == 0) {
  //    return;
  //  }
  ...
  if (!probe_started_) {
    time_to_next_process =
        std::max(time_to_next_process, PacingController::kMinSleepDuration);
  }
  ...
  if (probe_started_ ||  // If probing, schedule task regardless.
      ...
      (scheduled_process_times_.top() >
           (next_process_time + PacingController::kMinSleepDuration) &&
    ...
    task_queue_.PostDelayedTask(
        ...
        (time_to_next_process.us() + 999) / 1000);

    if (now - last_stats_update_ >= kMinTimeBetweenStatsUpdates) {
      rtc::CritScope cs(&crit_);
      current_stats_.expected_queue_time =
