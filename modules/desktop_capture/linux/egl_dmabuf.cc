...
static bool OpenEGL() {
  ...
  if (libEGL) {
    EglGetProcAddress =
        (eglGetProcAddress_func)dlsym(libEGL, "eglGetProcAddress");
...
static bool OpenGL() {
  ...
  for (const std::string& name : names) {
    libGL = dlopen(name.c_str(), RTLD_NOW | RTLD_GLOBAL);
    if (libGL) {
      GlXGetProcAddressARB =
          (glXGetProcAddressARB_func)dlsym(libGL, "glXGetProcAddressARB");
