...
class CaptureInputPin : public IMemInputPin, public IPin {
 ...
 private:
  ...
  // Accessed and possibly modified on the main thread as well as
  // capture thread. Never both at the same time though and we use IsStopped()
  // to know if it's safe to access this on the main thread.
...
class CaptureSinkFilter : public IBaseFilter {
 ...
 private:
  ...
  // May still be queried on the capture thread...
