...
class WebRtcVideoChannelTest : public WebRtcVideoEngineTest {
 ...
 protected:
  ...
  void SetSendCodecsShouldWorkForBitrates(const char* min_bitrate_kbps,
    ...
    EXPECT_CALL(
        *fake_call_->GetMockSendTransportController(),
        SetBitrateConfig(AllOf(
            Field(&BitrateConfig::min_bitrate_bps, expected_min_bitrate_bps),
            Field(&BitrateConfig::start_bitrate_bps,
                  expected_start_bitrate_bps),
            Field(&BitrateConfig::max_bitrate_bps, expected_max_bitrate_bps))));
...
TEST_F(WebRtcVideoChannelTest, SetSendCodecsWithBitratesAndMaxSendBandwidth) {
  ...
  EXPECT_CALL(
      *fake_call_->GetMockSendTransportController(),
      SetBitrateConfig(AllOf(Field(&BitrateConfig::min_bitrate_bps, 100000),
                             Field(&BitrateConfig::start_bitrate_bps, 200000),
                             Field(&BitrateConfig::max_bitrate_bps, 400000))));
  ...
  EXPECT_CALL(
      *fake_call_->GetMockSendTransportController(),
      SetBitrateConfig(AllOf(Field(&BitrateConfig::min_bitrate_bps, 100000),
                             Field(&BitrateConfig::start_bitrate_bps, -1),
                             Field(&BitrateConfig::max_bitrate_bps, 350000))));
  ...
  EXPECT_CALL(
      *fake_call_->GetMockSendTransportController(),
      SetBitrateConfig(AllOf(Field(&BitrateConfig::min_bitrate_bps, 100000),
                             Field(&BitrateConfig::start_bitrate_bps, 200000),
                             Field(&BitrateConfig::max_bitrate_bps, 300000))));
  EXPECT_TRUE(channel_->SetSendParameters(send_parameters_));
...
TEST_F(WebRtcVideoChannelTest, SetMaxSendBandwidthShouldBeRemovable) {
  send_parameters_.max_bandwidth_bps = 300000;
  EXPECT_CALL(*fake_call_->GetMockSendTransportController(),
              SetBitrateConfig(Field(&BitrateConfig::max_bitrate_bps, 300000)));

  EXPECT_TRUE(channel_->SetSendParameters(send_parameters_));
  // -1 means to disable max bitrate (set infinite).
  send_parameters_.max_bandwidth_bps = -1;
  EXPECT_CALL(*fake_call_->GetMockSendTransportController(),
              SetBitrateConfig(Field(&BitrateConfig::max_bitrate_bps, -1)));
  EXPECT_TRUE(channel_->SetSendParameters(send_parameters_));
}
...
TEST_F(WebRtcVideoChannelTest, SetMaxSendBandwidthAndAddSendStream) {
  ...
  FakeVideoSendStream* stream = AddSendStream();
  EXPECT_CALL(*fake_call_->GetMockSendTransportController(),
              SetBitrateConfig(Field(&BitrateConfig::max_bitrate_bps,
