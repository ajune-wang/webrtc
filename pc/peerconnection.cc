  ...
  if (IsUnifiedPlan()) {
    const ContentInfos& old_contents =
        (old_local_description
             ? old_local_description->description()->contents()
             : ContentInfos());
    const ContentInfos& contents =
        local_description()->description()->contents();
    for (size_t i = 0; i < contents.size(); ++i) {
      const cricket::ContentInfo& content = contents[i];
      ...
      used_mids_.insert(mid);
      if (media_type == cricket::MEDIA_TYPE_AUDIO ||
          media_type == cricket::MEDIA_TYPE_VIDEO) {
        auto transceiver_or_error =
            AssociateTransceiver(cricket::CS_LOCAL, i, content, old_content);
  ...
  if (IsUnifiedPlan()) {
    ...
    for (size_t i = 0; i < contents.size(); ++i) {
      ...
      if (media_type == cricket::MEDIA_TYPE_AUDIO ||
          media_type == cricket::MEDIA_TYPE_VIDEO) {
        auto transceiver_or_error =
            AssociateTransceiver(cricket::CS_REMOTE, i, content, old_content);
        if (!transceiver_or_error.ok()) {
          return transceiver_or_error.MoveError();
        }
        auto transceiver = transceiver_or_error.MoveValue();
...
GetMediaDescriptionOptionsForTransceiver(
  ...
  const std::string& mid =
      (transceiver->mid() ? *transceiver->mid() : AllocateMid(used_mids));
...
void PeerConnection::GetOptionsForUnifiedPlanOffer(
  ...
  std::set<rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>>
      found_transceivers;
  std::set<std::string> used_mids = used_mids_;
  for (size_t i = 0;
    ...
    if (had_been_rejected && transceiver->stopped()) {
      ...
      session_options->media_description_options.push_back(
          GetMediaDescriptionOptionsForTransceiver(transceiver, &used_mids));
...
PeerConnection::AssociateTransceiver(cricket::ContentSource source,
  ...
  const auto* media_desc =
      static_cast<const cricket::MediaContentDescription*>(content.description);
  auto transceiver = GetAssociatedTransceiver(content.name);
