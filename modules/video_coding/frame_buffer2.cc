    ...
    if (next_frame_it_ != frames_.end()) {
      ...
      if (last_decoded_frame_it_ != frames_.end()) {
        ...
        if (AheadOrAt(last_decoded_frame_timestamp_, frame->timestamp) &&
          ...
          // these conditions.
          NLOG(LS_WARNING, "Frame with (timestamp:picture_id:spatial_id) (",
...
bool FrameBuffer::HasBadRenderTiming(const EncodedFrame& frame,
  ...
  if (std::abs(render_time_ms - now_ms) > kMaxVideoDelayMs) {
    ...
    NLOG(LS_WARNING, "A frame about to be decoded is out of the configured ",
         "delay bounds (", frame_delay, " > ", kMaxVideoDelayMs,
         "). Resetting the video jitter buffer.");
    return true;
  }
  if (static_cast<int>(timing_->TargetVideoDelay()) > kMaxVideoDelayMs) {
    NLOG(LS_WARNING, "The video target delay has grown larger than ",
         kMaxVideoDelayMs, " ms.");
    return true;
  }
  return false;
...
int64_t FrameBuffer::InsertFrame(std::unique_ptr<EncodedFrame> frame) {
  ...
  if (!ValidReferences(*frame)) {
    NLOG(LS_WARNING, "Frame with (picture_id:spatial_id) (", key.picture_id,
  ...
  if (num_frames_buffered_ >= kMaxFramesBuffered) {
    if (frame->is_keyframe()) {
      NLOG(LS_WARNING, "Inserting keyframe (picture_id:spatial_id) (",
           key.picture_id, ":", static_cast<int>(key.spatial_layer),
  ...
  if (last_decoded_frame_it_ != frames_.end() &&
    ...
    if (AheadOf(frame->timestamp, last_decoded_frame_timestamp_) &&
      ...
      // keyframe.
      NLOG(LS_WARNING, "A jump in picture id was detected, clearing buffer.");
      ClearFramesAndHistory();
      last_continuous_picture_id = -1;
    } else {
      NLOG(LS_WARNING, "Frame with (picture_id:spatial_id) (", key.picture_id,
           ":", static_cast<int>(key.spatial_layer), ") inserted after frame (",
           last_decoded_frame_it_->first.picture_id, ":",
           static_cast<int>(last_decoded_frame_it_->first.spatial_layer),
    ...
    if (last_decoded_frame_it_ != frames_.end() &&
      ...
      if (ref_info == frames_.end()) {
        ...
        if (last_log_non_decoded_ms_ + kLogNonDecodedIntervalMs < now_ms) {
          NLOG(LS_WARNING, "Frame with (picture_id:spatial_id) (",
               key.picture_id, ":", static_cast<int>(key.spatial_layer),
