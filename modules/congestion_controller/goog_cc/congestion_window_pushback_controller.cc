...
bool ReadCongestionWindowPushbackExperimentParameter(
  ...
  std::string experiment_string =
      webrtc_config->Lookup(kCongestionPushbackExperiment);
...
CongestionWindowPushbackController::CongestionWindowPushbackController(
    const WebRtcConfig* webrtc_config)
    : add_pacing_(
          webrtc_config->Lookup("WebRTC-AddPacingToCongestionWindowPushback")
              .find("Enabled") == 0) {
  if (!ReadCongestionWindowPushbackExperimentParameter(
          webrtc_config, &min_pushback_target_bitrate_bps_)) {
