...
SuppressionFilter::SuppressionFilter(Aec3Optimization optimization,
  ...
  for (size_t k = 0; k < e_output_old_.size(); ++k) {
    for (size_t ch = 0; ch < e_output_old_[k].size(); ++ch) {
      e_output_old_[k][ch].fill(0.f);
    }
  }
}
...
void SuppressionFilter::ApplyGain(
  ...
  for (size_t ch = 0; ch < num_capture_channels_; ++ch) {
    ...
    if (e->size() > 1) {
      for (size_t i = 0; i < kFftLengthBy2Plus1; ++i) {
        E.re[i] = kIfftNormalization * comfort_noise_high_band[ch].re[i];
        E.im[i] = kIfftNormalization * comfort_noise_high_band[ch].im[i];
      }
      ...
      auto& e1 = (*e)[1][ch];
      for (size_t i = 0; i < kFftLengthBy2; ++i) {
        e1[i] += time_domain_high_band_noise[i] * high_bands_noise_scaling;
      }
    }
    ...
    for (size_t k = 1; k < e->size(); ++k) {
      auto& e_k = (*e)[k][ch];
      auto& e_k_old = e_output_old_[k][ch];
      for (size_t i = 0; i < kFftLengthBy2; ++i) {
        std::swap(e_k[i], e_k_old[i]);
      }
    }
    ...
    for (size_t k = 0; k < e->size(); ++k) {
      auto& e_k = (*e)[k][ch];
      for (size_t i = 0; i < kFftLengthBy2; ++i) {
        e_k[i] = rtc::SafeClamp(e_k[i], -32768.f, 32767.f);
      }
    }
  }
}
