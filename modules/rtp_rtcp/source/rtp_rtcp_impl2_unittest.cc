...
TEST_P(RtpRtcpImpl2Test, RtpSenderEgressTimestampOffset) {
  // RTP TimestampOffset not explicitly set, default to random value
  SetUp();
  ...
  EXPECT_EQ(recorded.size(), (uint32_t)1);
  EXPECT_EQ(recorded[0].timestamp, media_rtp_ts);

  RtpState saved_rtp_state = sender_.impl_->GetRtpState();

  // Change RTP TimestampOffset to 2000.
  ...
  EXPECT_TRUE(SendFrame(&sender_, sender_video_.get(), kBaseLayerTid, rtp_ts,
                        0 /* capture_time_ms */));
  AdvanceTime(kOneWayNetworkDelay);
  recorded = sender_.impl_->GetSentRtpPacketInfos(std::vector<uint16_t>{seqno});
  EXPECT_EQ(recorded.size(), (uint32_t)1);
  EXPECT_EQ(recorded[0].timestamp, media_rtp_ts);
}
