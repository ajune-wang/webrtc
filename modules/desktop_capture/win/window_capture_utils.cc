...
BOOL CALLBACK GetWindowListHandler(HWND hwnd, LPARAM param) {
  ...
  // Filter out windows that match the styles the caller has specified, e.g.
  // WS_EX_TOOLWINDOW for capturers that don't support overlay windows.
  if (exstyle & params->style_filters) {
    return TRUE;
  }
...
bool GetWindowList(int flags,
                   ...
                   LONG style_filters) {
  GetWindowListParams params(flags, style_filters, windows);
