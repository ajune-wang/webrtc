...
class IvfVideoFrameReaderTest : public ::testing::Test {
 protected:
  ...
  void CreateTestVideoFile(VideoCodecType video_codec_type) {
    ...
    std::unique_ptr<VideoEncoder> video_encoder = VP8Encoder::Create();
...
TEST_F(IvfVideoFrameReaderTest, Vp8) {
  CreateTestVideoFile(VideoCodecType::kVideoCodecVP8);
  ...
  for (size_t i = 0; i < reader.GetFramesCount(); ++i) {
    ...
    EXPECT_GT(I420PSNR(&expected_frame, &actual_frame.value()), 36);
  }
  reader.Close();
}
