...
size_t GetLimitedNumSpatialLayers(size_t input_width,
                                  size_t input_height,
...
// Use scalability mode to configure spatial layers.
std::vector<SpatialLayer> GetVp9SvcConfig(VideoCodec& codec) {
  ...
  if (info.num_spatial_layers !=
      static_cast<int>(GetLimitedNumSpatialLayers(codec.width, codec.height,
                                                  info.num_spatial_layers))) {
    // Layers reduced, do not use scalability mode for now.
    codec.UnsetScalabilityMode();
    codec.VP9()->interLayerPred =
        ScalabilityModeToInterLayerPredMode(*scalability_mode);
  }

  // TODO(bugs.webrtc.org/11607): Add support for VideoCodecMode::kScreensharing
  std::vector<SpatialLayer> spatial_layers = GetSvcConfig(
      codec.width, codec.height, codec.maxFramerate,
  ...
  // Do not limit bitrate of single spatial layer if spatial layering is not
  // requested.
...
std::vector<SpatialLayer> ConfigureSvcNormalVideo(
  ...
  // Limit number of layers for given resolution.
  num_spatial_layers =
      GetLimitedNumSpatialLayers(input_width, input_height, num_spatial_layers);
