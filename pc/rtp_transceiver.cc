...
RTCError RtpTransceiver::SetCodecPreferences(
  ...
  absl::c_remove_copy_if(
      codec_capabilities,
      std::back_insert_iterator<std::vector<RtpCodecCapability>>(codecs),
      [&codecs](const RtpCodecCapability& codec) {
        return absl::c_find(codecs, codec) != codecs.end();
      });
  ...
  if (media_type_ == cricket::MEDIA_TYPE_AUDIO) {
    ...
    for (const auto& codec_preference : codecs) {
      bool is_recv_codec =
          absl::c_find_if(recv_codecs, [&codec_preference](
                                           const cricket::AudioCodec& codec) {
            return codec.MatchesCapability(codec_preference);
          }) != recv_codecs.end();

      bool is_send_codec =
          absl::c_find_if(send_codecs, [&codec_preference](
                                           const cricket::AudioCodec& codec) {
            return codec.MatchesCapability(codec_preference);
          }) != send_codecs.end();
    ...
    // Validate codecs
    for (const auto& codec_preference : codecs) {
      if (absl::c_find_if(video_codecs, [&codec_preference](
