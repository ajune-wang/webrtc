...
// TEMP NOTE: This is duplicated in sdp_offer_answer_unified_plan.cc. It seems
// an Unified-only concern, so might want to get removed from here.
...
// The subclasses are at the bottom of this file, for ease of reference.
class SdpOfferAnswerHandlerUnifiedPlan;
class SdpOfferAnswerHandlerPlanB;
...
std::unique_ptr<SdpOfferAnswerHandler> SdpOfferAnswerHandler::Create(
  ...
  std::unique_ptr<SdpOfferAnswerHandler> handler;
  if (pc->IsUnifiedPlan()) {
    handler = absl::WrapUnique(new SdpOfferAnswerHandlerUnifiedPlan(pc));
  } else {
    handler = absl::WrapUnique(new SdpOfferAnswerHandlerPlanB(pc));
  }
...
// static
std::string SdpOfferAnswerHandler::GetSignalingStateString(
...
RTCError SdpOfferAnswerHandler::ApplyLocalDescription(
  ...
  error = ApplyLocalDescriptionByPlan(type, old_local_description);
  if (!error.ok()) {
    return error;
  }
...
RTCError SdpOfferAnswerHandler::ApplyRemoteDescription(
  ...
  error = UpdateChannelsByPlan(type, old_remote_description);
  ...
  error = ApplyRemoteDescriptionByPlan(type);
...
void SdpOfferAnswerHandler::DoSetLocalDescription(
  ...
  if (desc->GetType() == SdpType::kRollback) {
    SetLocalRollbackCompleteByPlan(observer, desc.get());
    return;
  }
  ...
  CheckIfNegotiationIsNeededByPlan();
...
void SdpOfferAnswerHandler::DoSetRemoteDescription(
  ...
  if (SetRemoteRollbackCompleteByPlan(observer, desc.get())) {
    return;
  }
...
RTCError SdpOfferAnswerHandler::ValidateSessionDescription(
  ...
  return ValidateSessionDescriptionByPlan(sdesc, source);
