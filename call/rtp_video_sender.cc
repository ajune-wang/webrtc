...
std::unique_ptr<VideoFecGenerator> MaybeCreateFecGenerator(
  ...
  if (rtp.flexfec.payload_type >= 0) {
    ...
    const std::vector<uint32_t>& flexfec_protected_ssrcs =
        rtp.flexfec.protected_media_ssrcs;

    if (flexfec_protected_ssrcs.size() > 1) {
      RTC_LOG(LS_WARNING)
    ...
    const bool media_ssrc_protected_by_flexfec =
        std::find(flexfec_protected_ssrcs.begin(),
                  flexfec_protected_ssrcs.end(),
                  rtp.ssrcs[simulcast_index]) == flexfec_protected_ssrcs.end();
    if (!media_ssrc_protected_by_flexfec) {
      return nullptr;
    }
  ...
  for (size_t i = 0; i < rtp_config_.ssrcs.size(); ++i) {
    ...
    if (fec_generator &&
      ...
      FlexfecSender* flexfec_sender =
          static_cast<FlexfecSender*>(fec_generator);
