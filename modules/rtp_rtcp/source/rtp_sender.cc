...
absl::optional<size_t> RTPSender::NextPaddingPacketSize() const {
  rtc::CritScope lock(&send_critsect_);
  ...
  if (rtx_ & kRtxRedundantPayloads) {
    return packet_history_.NextPaddingPacketSize().value_or(kMaxPaddingLength);
  }

  return kMaxPaddingLength;
}
