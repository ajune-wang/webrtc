...
absl::optional<DelayEstimate> EchoPathDelayEstimator::EstimateDelay(
  ...
  rtc::ArrayView<float> downsampled_capture(downsampled_capture_data.data(),
                                            sub_block_size_);
  data_dumper_->DumpWav("aec3_capture_decimator_input", capture[0].size(),
                        capture[0].data(), 16000, 1);
  capture_decimator_.Decimate(capture, downsampled_capture, downmix_);
