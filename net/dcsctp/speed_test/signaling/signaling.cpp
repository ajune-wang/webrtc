...
bool signaling::Init() {
  ...
  if (_offerer) {
    ...
    if (!TCPConnector::Connect(_address, _port, SOCKET_TOS_DSCP_EF, pProtocol,
                               this)) {
    ...
    if (!_acceptors->Bind()) {
      std::cout << format("Unable to fire up acceptor to: %s",
...
bool signaling::OnMessage(BaseProtocol* pProtocol,
                          uint8_t* buffer,
                          uint32_t size) {
