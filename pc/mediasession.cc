...
// 80-bit SHA1 HMAC unless 32 bit is requested and
// |sframe.require_frame_encryption| is set to true.
...
static bool SelectCrypto(const MediaContentDescription* offer,
                         ...
                         const webrtc::CryptoOptions& crypto_options,
                         CryptoParams* crypto_out) {
  const bool allow_hmac_32 = (offer->type() == MEDIA_TYPE_AUDIO) ||
  ...
  for (const CryptoParams& crypto : cryptos) {
    if ((crypto_options.srtp.enable_gcm_crypto_suites &&
        ...
        (rtc::CS_AES_CM_128_HMAC_SHA1_32 == crypto.cipher_suite &&
         allow_hmac_32 && !bundle &&
         crypto_options.srtp.enable_aes128_sha1_32_crypto_cipher)) {
