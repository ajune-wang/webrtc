...
@property(nonatomic) rtc::CopyOnWriteBuffer encoded_data;
@end

@implementation RTCEncodedImage (Private)

@synthesize encoded_data = _encoded_data;

- (instancetype)initWithNativeEncodedImage:(const webrtc::EncodedImage &)encodedImage {
  if (self = [super init]) {
    // Keeps a reference to the data.
    self.encoded_data = encodedImage.buffer();
    // Wrap the buffer in NSData without copying, do not take ownership.
    self.buffer = [NSData dataWithBytesNoCopy:const_cast<uint8_t *>(self.encoded_data.cdata())
...
- (webrtc::EncodedImage)nativeEncodedImage {
  ...
  RTC_DCHECK_EQ(self.buffer.bytes, self.encoded_data.cdata());
  encodedImage.set_buffer(self.encoded_data);
