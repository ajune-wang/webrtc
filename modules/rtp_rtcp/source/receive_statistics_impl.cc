...
bool StreamStatisticianImpl::UpdateOutOfOrder(const RtpPacketReceived& packet,
  ...
  if (received_seq_out_of_order_) {
    ...
    if (packet.SequenceNumber() == expected_sequence_number) {
      ...
      // calculation. Fraction loss for the nxt report may get a bit off, since
      // we don't update last_report_seq_max_ and last_report_cumulative_loss_
      // in aconsistent way.
