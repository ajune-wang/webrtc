...
void RemoteEstimatorProxy::IncomingPacket(int64_t arrival_time_ms,
  ...
  if (header.extension.hasTransportSequenceNumber) {
    ...
    if (packet_arrival_times_start_seq_.has_value()) {
      int64_t pos = seq - *packet_arrival_times_start_seq_;
      if (pos >= 0 &&
          pos < static_cast<int64_t>(packet_arrival_times_.size()) &&
          packet_arrival_times_[pos] != 0) {
        return;
      }
    }
...
void RemoteEstimatorProxy::SendFeedbackOnRequest(
  ...
  auto feedback_packet = BuildFeedbackPacket(
      ...
      sequence_number + 1, next_sequence_number);
