...
// from a Unified Plan offer. This tests for the case of the offer containing
// a=msid lines that signal no stream ids or multiple stream ids.
TEST_F(PeerConnectionMsidSignalingTest, UnifiedPlanToPlanBAnswer) {
  const std::string kAudioTrackId = "caller_audio";
  const std::string kVideoTrackId = "caller_video";
  ...
  EXPECT_EQ(1u, track_events[0].streams.size());
  EXPECT_EQ(1u, track_events[1].streams.size());
}
