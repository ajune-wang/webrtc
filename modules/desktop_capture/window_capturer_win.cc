...
void WindowCapturerWin::CaptureFrame() {
  ...
  // captures.
  //
  // For Windows 8.1 and later, we want to awlays use PrintWindow when the
  ...
  if (rtc::IsWindows8OrLater()) {
    ...
    // otherwise it either fails or it renders black frames. On Windows 8.0
    // this flag is not supported so this will fallback to the other call to
    // PrintWindow. It seems to be very tricky to detect Windows 8.0 vs 8.1
    // so a try/fallback is more approriate here.
