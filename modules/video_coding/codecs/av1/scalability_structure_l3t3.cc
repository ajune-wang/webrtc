...
FrameDependencyStructure ScalabilityStructureL3T3::DependencyStructure() const {
  ...
  t.resize(15);
  // Templates are shown in the order they appear in the first frames in the
  // the stream, but in structure.templates array templates are sorted by
  // (spatial_id, temporal_id) since that is a dependency descriptor requirement
  // Indexes are written in hex for nicer alignment.
