...
bool RtpVideoStreamReceiver2::OnReceivedPayloadData(
  ...
  if (video_header.is_last_packet_in_frame) {
    ...
    if (last_corruption_detection_index_per_spatial_layer_.find(spatial_idx) ==
      ...
      last_timestamp_per_spatial_layer_[spatial_idx] = std::nullopt;
    }
    if (message.sample_values().empty()) {
      ...
      // `OnReceivedPayloadData` might be called several times, however, we dont
      // want to increase the sequence index each time.
      if (!last_timestamp_per_spatial_layer_[spatial_idx].has_value() ||
          rtp_packet.Timestamp() !=
              last_timestamp_per_spatial_layer_[spatial_idx]) {
        video_header.frame_instrumentation_data =
            ConvertCorruptionDetectionMessageToFrameInstrumentationData(
                message, last_corruption_detection_index_per_spatial_layer_
                             [spatial_idx]);
        last_timestamp_per_spatial_layer_[spatial_idx] = rtp_packet.Timestamp();
      }
    }
