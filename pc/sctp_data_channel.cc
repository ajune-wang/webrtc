...
class CustomObserver : public DataChannelObserver {
 public:
  explicit CustomObserver(rtc::Thread* thread, DataChannelObserver* delegate)
      : thread_(thread), delegate_(delegate) {}
 ...
 private:
  void OnStateChange() override {
    thread_->PostTask(
        SafeTask(safety_.flag(), [this] { delegate_->OnStateChange(); }));
  }
...
void RegisterObserver(DataChannelObserver* observer) override {
  ...
  if (!observer->IsOkToCallOnTheNetworkThread()) {
    ...
    observer_ = std::make_unique<CustomObserver>(primary_thread_, observer);
