...
void AudioSinkWrapper::OnData(const void* audio_data,
  ...
  size_t size = bits_per_sample * number_of_channels * number_of_frames / 8;
  ...
  ScopedJavaLocalRef<jbyteArray> jaudio_data =
      ScopedJavaLocalRef<jbyteArray>(env, byte_array);
  Java_AudioSink_onData(env, j_sink_, jaudio_data, bits_per_sample, sample_rate,
