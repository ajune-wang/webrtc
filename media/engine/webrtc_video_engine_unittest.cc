...
TEST_F(WebRtcVideoChannelBaseTest, GetSources) {
  ...
  int64_t rtp_timestamp_1 = sources[0].rtp_timestamp();
  int64_t timestamp_ms_1 = sources[0].timestamp_ms();

  SendFrame();
  EXPECT_FRAME_WAIT(2, kVideoWidth, kVideoHeight, kTimeout);

  sources = channel_->GetSources(kSsrc);
  EXPECT_EQ(1u, sources.size());
  EXPECT_EQ(webrtc::RtpSourceType::SSRC, sources[0].source_type());
  int64_t rtp_timestamp_2 = sources[0].rtp_timestamp();
  int64_t timestamp_ms_2 = sources[0].timestamp_ms();
