...
// The maximum debt size, in terms of time, allowed when sending new packets.
...
RoundRobinPacketQueue::QueuedPacket* PacingController::GetPendingPacket(
  ...
  if (!unpaced_audio_packet && !is_probe) {
    ...
    if (mode_ == ProcessMode::kPeriodic) {
      ...
      if (time_since_target_send_time == TimeDelta::Zero()) {
        // We are right on time, allow sending only the buffer is empty.
        if (!media_debt_.IsZero()) {
          return nullptr;
        }
      } else if (time_since_target_send_time < TimeDelta::Zero()) {
        // We allow sending slightly early if we think that we would actually
        // be able to send if we were right on time - i.e. the current debt is
        // not more than would be reduced to zero at the target sent time.
        TimeDelta flush_time = media_debt_ / media_rate_;
        if (flush_time + time_since_target_send_time > TimeDelta::Zero()) {
          return nullptr;
        }
      } else {
        // We are sending slightly late, allow only if the debt would have
        // been reduced zero by now if we had sent at the optimal time.
        if (media_debt_ > time_since_target_send_time * media_rate_) {
          return nullptr;
        }
      }
    }
