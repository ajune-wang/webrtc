  ...
  bool IsTopWindow() {
    ...
    while (true) {
      ...
      if (lastError == ERROR_SUCCESS) {
        // The enumeration succeeded but didn't find the selected window (which
        // may have been closed).
        RTC_LOG(LS_WARNING) << "Failed to find selected window";
        return false;
      } else if (lastError == ERROR_INVALID_WINDOW_HANDLE) {
        // This error may occur if a window is closed around the time it's
        // enumerated; retry the enumeration in this case.
        continue;
      } else {
        // The enumeration failed with an unexpected error. After logging this &
        // asserting when enabled, report that the selected window isn't topmost
        // to avoid inadvertent capture of other windows.
