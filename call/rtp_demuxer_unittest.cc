...
class RtpDemuxerTest : public ::testing::Test {
 protected:
  ...
  bool AddSinkOnlyMid(const std::string& mid, RtpPacketSinkInterface* sink) {
    ...
    if (criteria.mid != mid) {
      // Special case for death tests. RtpDemuxerCriteria automatically
      // truncates mids that too long, so if that has happened, we put it
      // back. Since this is for a death test, we also don't log (can affect
      // global state).
      criteria.mid = mid;
    }
    return AddSink(criteria, sink);
