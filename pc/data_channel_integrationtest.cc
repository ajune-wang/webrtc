...
TEST_F(DataChannelIntegrationTestUnifiedPlan, DtlsRestart) {
  ...
  int caller_sent_on_dc = 0;
  caller()->set_on_connection_change_callback(
      [&](PeerConnectionInterface::PeerConnectionState new_state) {
        if (new_state ==
            PeerConnectionInterface::PeerConnectionState::kConnected) {
          caller()->data_channel()->SendAsync(
              DataBuffer("KESO"),
              [&](RTCError err) { caller_sent_on_dc = err.ok() ? 1 : -1; });
        }
      });

  int callee2_sent_on_dc = 0;
  callee2->set_on_connection_change_callback(
      [&](PeerConnectionInterface::PeerConnectionState new_state) {
        if (new_state ==
                PeerConnectionInterface::PeerConnectionState::kConnected &&
          ...
          callee2->data_channel()->SendAsync(
              DataBuffer("KENT"),
              [&](RTCError err) { callee2_sent_on_dc = err.ok() ? 1 : -1; });
        }
      });

  callee2->data_observer()->set_on_state_change_callback(
  ...
  ASSERT_TRUE_WAIT(caller_sent_on_dc != 0, kDefaultTimeout);
  ASSERT_TRUE_WAIT(callee2_sent_on_dc != 0, kDefaultTimeout);
