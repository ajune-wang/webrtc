...
template <class T>
void ToStringIfSet(std::string* result,
                   absl::string_view key,
                   const absl::optional<T>& val) {
  if (val) {
    absl::StrAppend(result, key, ": ", rtc::ToString(*val), ", ");
  }
}
...
std::string AudioOptions::ToString() const {
  std::string result;
  absl::StrAppend(&result, "AudioOptions {");
  ...
  ToStringIfSet(&result, "intelligibility_enhancer", intelligibility_enhancer);
