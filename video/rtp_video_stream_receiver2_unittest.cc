...
TEST_F(RtpVideoStreamReceiver2Test,
  ...
  // TODO(discuss): The following fails. I guess it is expected because of the
  // rounding errors. However, do we expect it to be as big as 0.6?
  // EXPECT_EQ(frame_inst_data_key_frame.std_dev, kStd);
...
TEST_F(RtpVideoStreamReceiver2Test,
  ...
  for (int i = 0; i < 10; i++) {
    ...
    if (frame_inst_data.sequence_index < 128) {
      EXPECT_EQ(frame_inst_data.sequence_index, sequence_idx);
    } else {
      EXPECT_EQ(frame_inst_data.sequence_index, sequence_idx + 128);
    }
  }
}
