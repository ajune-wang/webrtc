...
// this signal.
static const int kSignal = SIGURG;
...
// Global lock to ensure only one thread gets interruped at a time.
...
static SignalHandlerOutputState* volatile g_signal_handler_output_state;
...
_Unwind_Reason_Code UnwindBacktrace(struct _Unwind_Context* unwind_context,
  ...
  if (output_state->stack_size_cnt >= kMaxStackSize)
  ...
  // instruction after.
  output_state->addresses[output_state->stack_size_cnt] =
      _Unwind_GetIP(unwind_context) - 2;
  ++output_state->stack_size_cnt;
...
std::vector<StackTraceElement> GetStackTrace(int tid) {
  ...
  // records a raw stack trace, and record the old handler.
  ...
  sigemptyset(&act.sa_mask);
  {
    ...
    if (sigaction(kSignal, &act, &old_act) != 0) {
      RTC_LOG(LS_ERROR) << "Failed to change signal action with error "
                        << errno;
    ...
    // thread.
    if (tgkill(getpid(), tid, kSignal) != 0) {
      RTC_LOG(LS_ERROR) << "Failed to interrupt thread " << tid
                        << " with error " << errno;
      return std::vector<StackTraceElement>();
    }
  ...
  for (size_t i = 0; i < params.stack_size_cnt; ++i) {
    ...
    stack_trace_element.program_counter = static_cast<uint32_t>(
...
std::string StackTraceToString(
  ...
  for (size_t i = 0; i < stack_trace.size(); ++i) {
    ...
    string_builder.AppendFormat(
        "#%02zu pc %08x %s", i,
        static_cast<uint32_t>(stack_trace_element.program_counter),
