...
SendTimeHistory::PacketSendState SendTimeHistory::OnSentPacket(
    uint16_t sequence_number,
    int64_t send_time_ms) {
  int64_t unwrapped_seq_num = seq_num_unwrapper_.Unwrap(sequence_number);
  auto it = history_.find(unwrapped_seq_num);
  if (it == history_.end())
    return PacketSendState::kUknown;
  ...
  return packet_retransmit ? PacketSendState::kRetransmission
                           : PacketSendState::kSent;
}
