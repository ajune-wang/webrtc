...
static bool GetServiceTypeAndHostnameFromUri(const std::string& in_str,
  ...
  if (colonpos == std::string::npos) {
    NLOG(LS_WARNING, "Missing ':' in ICE URI: ", in_str);
    return false;
  }
  if ((colonpos + 1) == in_str.length()) {
    NLOG(LS_WARNING, "Empty hostname in ICE URI: ", in_str);
    return false;
  }
...
static RTCErrorType ParseIceServerUrl(
  ...
  if (tokens.size() == kTurnTransportTokensNum) {  // ?transport= is present.
    ...
    if (tokens[0] != kTransport) {
      NLOG(LS_WARNING, "Invalid transport parameter key.");
      return RTCErrorType::SYNTAX_ERROR;
    }
    if (tokens.size() < 2) {
      NLOG(LS_WARNING, "Transport parameter missing value.");
      return RTCErrorType::SYNTAX_ERROR;
    }
    if (!cricket::StringToProto(tokens[1].c_str(), &turn_transport_type) ||
        (turn_transport_type != cricket::PROTO_UDP &&
         turn_transport_type != cricket::PROTO_TCP)) {
      NLOG(LS_WARNING, "Transport parameter should always be udp or tcp.");
      return RTCErrorType::SYNTAX_ERROR;
    }
  }
  ...
  if (!GetServiceTypeAndHostnameFromUri(uri_without_transport, &service_type,
                                        &hoststring)) {
    NLOG(LS_WARNING, "Invalid transport parameter in ICE URI: ", url);
    return RTCErrorType::SYNTAX_ERROR;
  }
  ...
  if (tokens.size() > kTurnHostTokensNum) {
    NLOG(LS_WARNING, "Invalid user@hostname format: ", hoststring);
    return RTCErrorType::SYNTAX_ERROR;
  }
  if (tokens.size() == kTurnHostTokensNum) {
  ...
  if (!ParseHostnameAndPortFromString(hoststring, &address, &port)) {
    NLOG(WARNING, "Invalid hostname format: ", uri_without_transport);
    return RTCErrorType::SYNTAX_ERROR;
  }

  if (port <= 0 || port > 0xffff) {
    NLOG(WARNING, "Invalid port: ", port);
    return RTCErrorType::SYNTAX_ERROR;
  }
