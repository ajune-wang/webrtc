...
// Whether to reconfigurre the stream is ultimately up to the handler, which is
// able to respond asynchronously.
class QualityScalerQpUsageHandlerInterface {
 public:
  virtual ~QualityScalerQpUsageHandlerInterface();

  // Responds to QP usage being too high or too low. When this is done, the
  // |callback| MUST be invoked, allowing the QualityScaler to resume checking
  // for QP.
...
class QualityScalerQpUsageHandlerCallback
 ...
 private:
  friend class QualityScaler::CheckQpTask;
