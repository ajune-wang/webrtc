...
int64_t SignedDelta(uint64_t previous,
                         uint64_t current,
                         uint64_t width) {
  ...
  if (forward_delta == backward_delta) {  // Either 0 or half of max (rounded).
    ...
    if (width == 64) {
      ...
      return -rtc::dchecked_cast<int64_t>(forward_delta);  // Note minus-sign.
    }
  }
...
class FixedLengthDeltaEncoder final {
 ...
 private:
  ...
  // TODO: !!!
  uint64_t UnsignedRepresentation(int64_t val) const;
...
uint64_t FixedLengthDeltaEncoder::UnsignedRepresentation(int64_t val) const {
  return 0;  // TODO: !!!
}
