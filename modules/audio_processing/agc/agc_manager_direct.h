...
class AgcManagerDirect final {
 public:
  // Ctor.
  // `num_capture_channels` specifies the number of channels for the audio
  // passed to `AnalyzePreProcess()` and `Process()`. The operating range of
  // `startup_min_level` is [12, 255] and any input value outside that range
  // will be clamped. `clipped_level_step` is the amount the microphone level is
  // lowered with every clipping event, limited to (0, 255].
  // `clipped_ratio_threshold` is the proportion of clipped samples required to
  // declare a clipping event, limited to (0.f, 1.f). `clipped_wait_frames` is
  // the time in frames to wait after a clipping event before checking again,
