...
// Returns a positive value when the speech level is below the target range and
// a negative one when the speech level is above the target range.
int GetSpeechLevelErrorDb(float speech_level_dbfs,
  ...
  if (speech_level_dbfs > target_range_max_dbfs) {
    rms_error = std::round(target_range_max_dbfs - speech_level_dbfs);
  } else if (speech_level_dbfs < target_range_min_dbfs) {
    rms_error = std::round(target_range_min_dbfs - speech_level_dbfs);
  }
