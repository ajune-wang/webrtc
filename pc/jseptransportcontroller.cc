  ...
  for (size_t i = 0; i < description->contents().size(); ++i) {
    ...
    cricket::JsepTransportDescription jsep_description =
        CreateJsepTransportDescription(content_info, transport_info,
                                       extension_ids);
    if (local) {
      error = transport->SetLocalTransportDescription(jsep_description, type);
    } else {
      error = transport->SetRemoteTransportDescription(jsep_description, type);
    }
