...
TEST_P(RtpVideoStreamReceiver2TestH264, OutOfBandFmtpSpsPps) {
  ...
  // H264VideoDepacketizer will add the OOB sps/pps to the nal units, as
  // the OnReceivedPayloadData() is skiping the H264VideoDepacketizer::Parse()
  // we have to add them to the video header here.
  auto& h264 = absl::get<RTPVideoHeaderH264>(video_header.video_type_header);
  h264.nalus.push_back({H264::NaluType::kSps, 0, -1});
  h264.nalus.push_back({H264::NaluType::kPps, 0, 0});
  AddIdr(&video_header, 0, 0, &idr_data);
  rtp_packet.SetPayloadType(kPayloadType);
  rtp_packet.SetSequenceNumber(2);
  rtp_packet.SetMarker(true);
  video_header.is_first_packet_in_frame = true;
