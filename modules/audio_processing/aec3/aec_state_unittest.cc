...
TEST(AecState, NormalUsage) {
  ...
  state.Update(delay_estimate, refined_delay,
               diverged_filter_frequency_response, impulse_response, true,
               false, *render_delay_buffer->GetRenderBuffer(), E2_main, Y2, s);
  EXPECT_FALSE(state.UseLinearEchoModel());
  ...
  for (int k = 0; k < 3000; ++k) {
    ...
    state.Update(delay_estimate, refined_delay,
                 converged_filter_frequency_response, impulse_response, true,
                 false, *render_delay_buffer->GetRenderBuffer(), E2_main, Y2,
                 s);
  }
  EXPECT_TRUE(state.UseLinearEchoModel());
  ...
  ASSERT_TRUE(state.UseLinearEchoModel());
...
TEST(AecState, ConvergedFilterDelay) {
  ...
  for (int k = 0; k < kFilterLength; ++k) {
    ...
    state.Update(delay_estimate, refined_delay, frequency_response,
