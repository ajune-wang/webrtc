...
void WebRtcVideoChannel::OnPacketReceived(
  ...
  worker_thread_->PostTask(SafeTask(task_safety_.flag(), [this, packet] {
    ...
    webrtc::RtpPacketReceived video_packet(packet);
    video_packet.IdentifyExtensions(recv_rtp_extension_map_);
    video_packet.set_payload_type_frequency(webrtc::kVideoPayloadTypeFrequency);
    if (!video_packet.arrival_time().IsFinite()) {
      video_packet.set_arrival_time(
          webrtc::Timestamp::Micros(rtc::TimeMicros()));
    }

    call_->Receiver()->DeliverRtpPacket(
        webrtc::MediaType::VIDEO, std::move(video_packet),
