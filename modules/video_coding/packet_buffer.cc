        ...
        if (is_h264) {
          ...
          for (size_t j = 0; j < h264_header->nalus_length; ++j) {
            if (h264_header->nalus[j].type == H264::NaluType::kSps) {
              ...
              has_h264_idr = true;
            } else if ((idr_width > 0) && (idr_height > 0)) {
              // IDR frame was finalized and we have the correct resolution for
              // IDR; update any preceding packets to have same resolution as
              // IDR.
              data_buffer_[start_index].video_header.width = idr_width;
              data_buffer_[start_index].video_header.height = idr_height;
            }
          }
