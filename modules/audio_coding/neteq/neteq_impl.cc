...
int NetEqImpl::GetDecision(Operations* operation,
  ...
  *operation = kUndefined;
  ...
  if (neteq_controller_->CngRfc3389On() || last_mode_ == kModeRfc3389Cng) {
  ...
  generated_noise_samples =
      generated_noise_stopwatch_
          ? generated_noise_stopwatch_->ElapsedTicks() * output_size_samples_ +
                neteq_controller_->noise_fast_forward()
          : 0;
  NetEqController::NetEqStatus neteq_status;
  neteq_status.packet_buffer_info.dtx_or_cng =
      packet_buffer_->ContainsDtxOrCngPacket(decoder_database_.get());
  neteq_status.packet_buffer_info.num_samples =
      packet_buffer_->NumSamplesInBuffer(decoder_frame_length_);
  neteq_status.packet_buffer_info.span_samples = packet_buffer_->GetSpanSamples(
      decoder_frame_length_, last_output_sample_rate_hz_, true);
  neteq_status.packet_buffer_info.span_samples_no_dtx =
      packet_buffer_->GetSpanSamples(decoder_frame_length_,
                                     last_output_sample_rate_hz_, false);
  neteq_status.packet_buffer_info.num_packets =
      packet_buffer_->NumPacketsInBuffer();
  neteq_status.target_timestamp = sync_buffer_->end_timestamp();
  neteq_status.expand_mutefactor = expand_->MuteFactor(0);
  neteq_status.last_packet_samples = decoder_frame_length_;
  neteq_status.last_mode = last_mode_;
  neteq_status.play_dtmf = *play_dtmf;
  neteq_status.generated_noise_samples = generated_noise_samples;
  if (packet) {
    neteq_status.packet = {
        packet->timestamp, packet->frame && packet->frame->IsDtxPacket(),
        decoder_database_->IsComfortNoise(packet->payload_type)};
  }
  *operation = neteq_controller_->GetDecision(neteq_status, &reset_decoder_);
