...
constexpr char kDefaultName[] = "TaskQueuePacer";
constexpr TaskQueueFactory::Priority kDefaultPrio =
...
TEST(TaskQueuePacedSenderTest, SleepsDuringCoalscingWindow) {
  ...
  ScopedKeyValueConfig trials;
  TaskQueuePacedSender pacer(time_controller.GetClock(), &packet_router, trials,
                             task_queue.get(), kCoalescingWindow,
                             kNoPacketHoldback);
...
TEST(TaskQueuePacedSenderTest, PacketBasedCoalescing) {
  ...
  ScopedKeyValueConfig trials;
  TaskQueuePacedSender pacer(time_controller.GetClock(), &packet_router, trials,
                             task_queue.get(), kFixedCoalescingWindow,
...
TEST(TaskQueuePacedSenderTest, HighPrecisionPacingWhenSlackIsDisabled) {
  ...
  auto task_queue =
      task_queue_factory.CreateTaskQueue(kDefaultName, kDefaultPrio);

  MockPacketRouter packet_router;
  TaskQueuePacedSender pacer(
      time_controller.GetClock(), &packet_router, experiments, task_queue.get(),
