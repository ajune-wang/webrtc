...
namespace {
constexpr size_t kMaxDependencies = CodecSpecificInfoVP8::kMaxDependencies;
...
void RtpPayloadParams::SetDependenciesVp8New(
  ...
  if (is_keyframe) {
    RTC_DCHECK_EQ(vp8_info.referencedCount, 0u);
    last_shared_frame_id_.clear();
    last_key_shared_frame_id_ = shared_frame_id;
    return;
  }

  RTC_DCHECK_GT(vp8_info.referencedCount, 0u);
  RTC_DCHECK_LE(vp8_info.referencedCount, kMaxDependencies);

  // TODO: !!! Make sure that pushing the same dependency more than once is OK.

  for (size_t i = 0; i < vp8_info.referencedCount; ++i) {
    const size_t referenced_id = vp8_info.referenced[i];
    const auto it = last_shared_frame_id_.find(referenced_id);
    if (it != last_shared_frame_id_.end()) {
      const int64_t dependency_frame_id = it->second;
      RTC_DCHECK_LT(dependency_frame_id, shared_frame_id);
      generic->dependencies.push_back(dependency_frame_id);
    } else {
      // Key frames update all buffers, but we don't bother inserting
      // them into |last_shared_frame_id_|, because we don't know how
      // many dependency identifiers there will be.
      generic->dependencies.push_back(last_key_shared_frame_id_);
    }
  }

  RTC_DCHECK_LE(vp8_info.updatedCount, kMaxDependencies);
  for (size_t i = 0; i < vp8_info.updatedCount; ++i) {
    const size_t updated_id = vp8_info.updated[i];
    last_shared_frame_id_[updated_id] = shared_frame_id;
  }

  RTC_DCHECK_LE(last_shared_frame_id_.size(), kMaxDependencies);
}
