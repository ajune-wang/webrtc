...
bool RTPSenderVideo::SendEncodedImage(int payload_type,
  ...
  if (frame_transformer_delegate_) {
    ...
    // We provide a getter for video_structure_ so that the
    // frame_transformer_delegate_ can serialize access to it via its own lock,
    // to satisfy the send_checker_.
