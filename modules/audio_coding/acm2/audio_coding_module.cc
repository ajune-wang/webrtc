...
int AudioCodingModuleImpl::PreprocessToAddData(const AudioFrame& in_frame,
  ...
  auto preprocess_data = preprocess_frame_.mutable_data(
      in_frame.samples_per_channel_, in_frame.num_channels_);
  *ptr_out = &preprocess_frame_;
  ...
  if (down_mix) {
    ...
    DownMixFrame(in_frame,
                 resample ? rtc::ArrayView<int16_t>(audio) : preprocess_data);
