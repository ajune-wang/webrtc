...
class MediaTypesEnum : public IEnumMediaTypes {
 public:
  MediaTypesEnum(const VideoCaptureCapability& capability)
    ...
    // If the preferred VideoType in capability_ matches a supported type, move
    // it to the front of the list, so the camera will select it.
    auto it = std::find(format_preference_order_.begin(),
                        format_preference_order_.end(), capability_.videoType);
    // If the video type is found and not already at the front of the list, move
    // it.
 ...
 private:
  ...
  STDMETHOD(Next)(ULONG count, AM_MEDIA_TYPE** types, ULONG* fetched) {
    ...
    for (ULONG i = 0; i < count && pos_ < format_preference_order_.size();
         ++i) {
      ...
      RTC_DCHECK(pos_ <= format_preference_order_.size());
    ...
    return pos_ == format_preference_order_.size() ? S_FALSE : S_OK;
  }
