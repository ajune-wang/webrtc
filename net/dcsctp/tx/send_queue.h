...
class SendQueue {
 public:
  ...
  // prioritization, which is important for any advanced stream scheduler.

  // Produce a chunk to be sent.
  //
  // `max_size` refers to how many payload bytes that may be produced, not
  // including any headers.
  virtual absl::optional<DataToSend> Produce(int64_t now_ms,
                                             size_t max_size) = 0;

  // Discards a message identified by the parameters.
