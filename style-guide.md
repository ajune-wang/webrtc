...
* Declarations in `foo.h`, definitions in both `foo_bar.cc` and
  `foo_baz.cc`. *Fix:* The `.h` and `.cc` files should come in pairs,
  so either split `foo.h` into `foo_bar.h` and `foo_baz.h`, or merge
  `foo_bar.cc` and `foo_baz.cc` into `foo.cc`.
