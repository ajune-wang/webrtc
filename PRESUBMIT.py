...
def _ExtractWarnings(log):
  """Extract analyzer warnings from log.
...
def _CollectSourceLines(diagnostic):
  """Crudely parse and collect normalized file path and line numbers.
  ...
  Returns a dict {filepath: set of reported lines}.
  """
  result = {}
  for line in diagnostic:
    ...
    if len(fields) >= 2:
      ...
      try:
        # Get source path relatively to src/ root
        # to match AffectedFile.LocalPath() format.
        local_path = path.split('/src/')[-1]
        line_num = int(line)
        result.setdefault(local_path, set()).add(line_num)
      except ValueError:
        pass
  return result
...
def CheckStaticAnalyzer(input_api, output_api,
  ...
  if gn_errors:
    raise Exception('Cannot complete check due to gn error:\n%s' % gn_errors)
  errors = []
  file_filter = lambda x: (input_api.FilterSourceFile(x)
                           and source_file_filter(x)
                           and x.LocalPath().endswith(('.cc', '.h')))
  for f in input_api.AffectedSourceFiles(file_filter):
    ...
    for diagnostic in _ExtractWarnings(log):
      # Diagnostic may relate to another file (included header),
      # so fallback to empty set.
      warning_lines = _CollectSourceLines(diagnostic).get(f.LocalPath(), set())
...
def CheckChangeOnCommit(input_api, output_api):
  ...
  non_third_party_sources = lambda x: input_api.FilterSourceFile(
      x, black_list=third_party_filter_list)
  results.extend(CheckStaticAnalyzer(
    input_api, output_api, non_third_party_sources))
  return results
