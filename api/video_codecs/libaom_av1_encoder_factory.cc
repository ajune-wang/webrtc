...
aom_svc_ref_frame_config_t GetSvcRefFrameConfig(
  ...
  if (!settings.reference_buffers.empty()) {
    ...
    // A valid AV1 bitstream must not alias an unused buffer, and since start
    // frames only update some buffers it is not safe to leave unused aliases
    // to simply point buffer 0.
