...
class SpsVuiRewriter : private SpsParser {
 public:
  ...
  // Parses NAL units from |buffer| based on |nalus| and rewrites VUI in SPS
  // blocks if necessary. The result is written to |output_buffer| and modified
  // NAL unit offsets and lenghts are written to |output_nalus| to account for
  // any added data.
