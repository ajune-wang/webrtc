...
bool ValidateRtpExtensions(
  ...
  bool id_used[1 + webrtc::RtpExtension::kMaxId -
               webrtc::RtpExtension::kMinId] = {false};
  for (const auto& extension : extensions) {
    ...
    if (id_used[extension.id - webrtc::RtpExtension::kMinId]) {
      RTC_LOG(LS_ERROR) << "Duplicate RTP extension ID: "
                        << extension.ToString();
      return false;
    }
    id_used[extension.id - webrtc::RtpExtension::kMinId] = true;
  }
  return true;
}
