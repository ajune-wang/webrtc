...
inline int64_t DivideAndRound(int64_t numerator, int64_t denominator) {
  if (numerator >= 0) {
    ...
    return (numerator + (denominator / 2)) / denominator - 1;
  }
}
}  // namespace timedelta_impl
...
class TimeDelta {
 public:
  ...
  int64_t s() const { return timedelta_impl::DivideAndRound(us(), 1000000); }
  int64_t ms() const { return timedelta_impl::DivideAndRound(us(), 1000); }
  int64_t us() const {
    RTC_DCHECK(IsFinite());
