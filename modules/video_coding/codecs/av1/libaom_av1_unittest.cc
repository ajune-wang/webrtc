...
struct SvcTestParam {
  ...
  std::map<LayerId, DataRate> layer_rates;
};
...
TEST_P(LibaomAv1SvcTest, SetRatesMatchMeasuredBitrate) {
  ...
  for (const auto& kv : param.layer_rates) {
  ...
  EXPECT_THAT(measured_bitrates,
              Pointwise(SameLayerIdAndBitrateIsNear(), param.layer_rates));
}
