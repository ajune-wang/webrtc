...
EchoRemoverImpl::EchoRemoverImpl(const EchoCanceller3Config& config,
    ...
    : config_(config),
      ...
      aec_state_(config_),
      Y2_heap_(num_capture_channels_),
      E2_heap_(num_capture_channels_),
      R2_heap_(num_capture_channels_),
      S2_linear_heap_(num_capture_channels_),
      Y_heap_(num_capture_channels_),
      E_heap_(num_capture_channels_),
      comfort_noise_heap_(num_capture_channels_),
      high_band_comfort_noise_heap_(num_capture_channels_),
      subtractor_output_heap_(num_capture_channels_) {
...
void EchoRemoverImpl::ProcessCapture(
  ...
  if (NumChannelsOnHeap(num_capture_channels_) > 0) {
    // If the stack data is too small, use heap-allocated data for storing the
    // microphone data.
