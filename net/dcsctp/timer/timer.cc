...
namespace dcsctp {
namespace {
uint64_t MakeTimeoutId(uint32_t timer_id, uint32_t generation) {
  return static_cast<uint64_t>(timer_id) << 32 | generation;
}
...
void TimerManager::HandleTimeout(uint64_t timeout_id) {
