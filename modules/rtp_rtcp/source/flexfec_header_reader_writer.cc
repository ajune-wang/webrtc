...
bool FlexfecHeaderReader::ReadFecHeader(
  ...
  if (r_bit) {
    RTC_LOG(LS_INFO) << "FlexFEC packet with retransmission bit set. We do "
                     << "not yet support this, thus discarding the packet.";
    return false;
  }
  bool f_bit = (fec_packet->pkt->data[0] & 0x40) != 0;
  if (f_bit) {
    RTC_LOG(LS_INFO) << "FlexFEC packet with inflexible generator matrix. "
                     << "We do not yet support this, thus discarding packet.";
    return false;
  }
  uint8_t ssrc_count =
      ByteReader<uint8_t>::ReadBigEndian(&fec_packet->pkt->data[8]);
  if (ssrc_count != 1) {
    RTC_LOG(LS_INFO) << "FlexFEC packet protecting multiple media SSRCs. "
                     << "We do not yet support this, thus discarding packet.";
    return false;
  }
  ...
  if (k_bit0) {
    ...
    if (k_bit1) {
      ...
      if (k_bit2) {
        ...
        RTC_LOG(LS_WARNING) << "Discarding FlexFEC packet with malformed "
                            << "header.";
        return false;
      }
