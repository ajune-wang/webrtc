...
void RemixFrame(size_t target_number_of_channels, AudioFrame* frame) {
  ...
  // Use legacy components for the most simple cases (mono <-> stereo).
  // Saves resources since we don't have to create a channel-mixing matrix.
  ...
  if (target_number_of_channels < 3 && frame->num_channels() < 3) {
    ...
    const ChannelLayout out ChannelMixer mixer(
        GuessChannelLayout(frame->num_channels()),
        GuessChannelLayout(target_number_of_channels));
    mixer.Transform(frame);
    RTC_DCHECK_NE(frame->channel_layout(), CHANNEL_LAYOUT_UNSUPPORTED);
  }
