...
bool setFdKeepAlive(SOCKET_TYPE fd, bool isUdp) {
  if (isUdp)
    return true;
  int one = 1;
  if (setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, (const char*)&one,
                 sizeof(one)) != 0) {
    int err = SOCKET_LAST_ERROR;
#ifdef WIN32
    printf("setsockopt() with SOL_SOCKET/SO_KEEPALIVE failed. Error was: %d",
           err);
#else  /* WIN32 */
    printf(
        "setsockopt() with SOL_SOCKET/SO_KEEPALIVE failed. Error was: (%d) %s",
        err, strerror(err));
#endif /* WIN32 */
    return false;
  }
...
bool setFdNoNagle(SOCKET_TYPE fd, bool isUdp) {
  if (isUdp)
    return true;
  int one = 1;
  if (setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, (const char*)&one,
                 sizeof(one)) != 0) {
    int err = SOCKET_LAST_ERROR;
#ifdef WIN32
    printf("setsockopt() with IPPROTO_TCP/TCP_NODELAY failed. Error was: %d",
           err);
#else  /* WIN32 */
    printf(
        "setsockopt() with IPPROTO_TCP/TCP_NODELAY failed. Error was: (%d) %s",
        err, strerror(err));
#endif /* WIN32 */
    return false;
  }
...
bool setFdReuseAddress(SOCKET_TYPE fd) {
  ...
  if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (const char*)&one,
    ...
    int err = SOCKET_LAST_ERROR;
#ifdef WIN32
    printf("setsockopt() with SOL_SOCKET/SO_REUSEADDR failed. Error was: %d",
           err);
#else  /* WIN32 */
    printf(
        "setsockopt() with SOL_SOCKET/SO_REUSEADDR failed. Error was: (%d) %s",
        err, strerror(err));
#endif /* WIN32 */
    return false;
  }
#ifdef SO_REUSEPORT
  if (setsockopt(fd, SOL_SOCKET, SO_REUSEPORT, (const char*)&one,
                 sizeof(one)) != 0) {
    int err = SOCKET_LAST_ERROR;
#ifdef WIN32
    printf("setsockopt() with SOL_SOCKET/SO_REUSEPORT failed. Error was: %d",
           err);
#else  /* WIN32 */
    printf(
        "setsockopt() with SOL_SOCKET/SO_REUSEPORT failed. Error was: (%d) %s",
        err, strerror(err));
#endif /* WIN32 */
  }
#endif /* SO_REUSEPORT */
...
#else  /* WIN32 */
    printf("setsockopt() with IPPROTO_IP/IP_TTL failed. Error was: (%d) %s",
           err, strerror(err));
#endif /* WIN32 */
  }
  return true;
...
bool setFdMulticastTTL(SOCKET_TYPE fd, uint8_t ttl) {
  int temp = ttl;
  if (setsockopt(fd, IPPROTO_IP, IP_MULTICAST_TTL, (const char*)&temp,
                 sizeof(temp)) != 0) {
    int err = SOCKET_LAST_ERROR;
#ifdef WIN32
    printf(
        "setsockopt() with IPPROTO_IP/IP_MULTICAST_TTL failed. Error was: %d",
        err);
#else  /* WIN32 */
    printf(
        "setsockopt() with IPPROTO_IP/IP_MULTICAST_TTL failed. Error was: (%d) "
...
bool setIPv4TOS(SOCKET_TYPE fd, uint8_t tos) {
  int temp = tos;
  if (setsockopt(fd, IPPROTO_IP, IP_TOS, (const char*)&temp, sizeof(temp)) !=
      0) {
    int err = SOCKET_LAST_ERROR;
#ifdef WIN32
    printf("setsockopt() with IPPROTO_IP/IP_TOS failed. Error was: %d", err);
#else  /* WIN32 */
    printf("setsockopt() with IPPROTO_IP/IP_TOS failed. Error was: (%d) %s",
           err, strerror(err));
#endif /* WIN32 */
  }
  return true;
...
#else  /* WIN32 */
  int temp = tos;
  if (setsockopt(fd, IPPROTO_IPV6, IPV6_TCLASS, (const char*)&temp,
                 sizeof(temp)) != 0) {
    int err = SOCKET_LAST_ERROR;
    printf(
        "setsockopt() with IPPROTO_IPV6/IPV6_TCLASS failed. Error was: (%d) %s",
        err, strerror(err));
  }
  return true;
#endif /* WIN32 */
...
bool setFdLinger(SOCKET_TYPE fd) {
  ...
  temp.l_onoff = 0;
  temp.l_linger = 0;
  if (setsockopt(fd, SOL_SOCKET, SO_LINGER, (const char*)&temp, sizeof(temp)) !=
      0) {
    int err = SOCKET_LAST_ERROR;
#ifdef WIN32
    printf("setsockopt() with SOL_SOCKET/SO_LINGER failed. Error was: %d", err);
#else  /* WIN32 */
    printf("setsockopt() with SOL_SOCKET/SO_LINGER failed. Error was: (%d) %s",
           err, strerror(err));
#endif /* WIN32 */
  }
  return true;
...
bool setFdBuff(SOCKET_TYPE fd, int option, int size) {
  ...
  if (setsockopt(fd, SOL_SOCKET, option, (const char*)&size, sizeof(size)) !=
      0) {
    int err = SOCKET_LAST_ERROR;
#ifdef WIN32
...
bool setFdSndRcvBuff(SOCKET_TYPE fd, uint32_t sendSize, uint32_t recvSize) {
  return ((sendSize > 0) ? setFdBuff(fd, SO_SNDBUF, sendSize) : true) &&
         ((recvSize > 0) ? setFdBuff(fd, SO_RCVBUF, recvSize) : true);
}

bool setFdNoSIGPIPE(SOCKET_TYPE fd) {
  // int32_t one = 1;
  // if (setsockopt(fd, SOL_SOCKET, SO_NOSIGPIPE, (const char*) &one,
  // sizeof(one)) != 0) { 	printf("Unable to set SO_NOSIGPIPE\n");
  // return false;
  //}
  return true;
}
...
bool setFdMaxSndRcvBuff(SOCKET_TYPE fd, bool isUdp) {
  return setFdBuff(fd, SO_SNDBUF, MAX_SOCK_BUF_SIZE) &&
         setFdBuff(fd, SO_RCVBUF, MAX_SOCK_BUF_SIZE);
}
...
bool setFdOptions(SOCKET_TYPE fd, bool isUdp) {
  ...
  if ((!setFdNoSIGPIPE(fd)) || (!setFdKeepAlive(fd, isUdp)) ||
      (!setFdReuseAddress(fd)) || (!setFdMaxSndRcvBuff(fd, isUdp)))
    return false;
  return true;
}
