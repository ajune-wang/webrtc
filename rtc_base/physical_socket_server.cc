...
void PhysicalSocketServer::Add(Dispatcher* pdispatcher) {
  ...
  if (pdispatcher->key_) {
    ...
    do {
      ++last_dispatcher_key_;
    } while (dispatchers_.count(last_dispatcher_key_));
    pdispatcher->key_ = last_dispatcher_key_;
    dispatchers_.emplace(last_dispatcher_key_, pdispatcher);
  }
#if defined(WEBRTC_USE_EPOLL)
  if (epoll_fd_ != INVALID_SOCKET) {
...
void PhysicalSocketServer::Remove(Dispatcher* pdispatcher) {
  ...
  auto it = dispatchers_.find(pdispatcher->key_.value());
  if (it == dispatchers_.end()) {
    RTC_LOG(LS_ERROR)
        << "PhysicalSocketServer dispatcher lookup by key failed!";
    RTC_NOTREACHED();
    return;
  }
  dispatchers_.erase(it);
  pdispatcher->key_.reset();
#if defined(WEBRTC_USE_EPOLL)
  if (epoll_fd_ != INVALID_SOCKET) {
...
void PhysicalSocketServer::Update(Dispatcher* pdispatcher) {
  ...
  // Don't update dispatchers that are lacking a key, meeting they have not
  // been added.
  if (!pdispatcher->key_) {
    return;
  }
...
// http://crbug.com/344505
  ...
  while (fWait_) {
    ...
    int fdmax = -1;
    {
      ...
      for (auto const& pair : dispatchers_) {
        Dispatcher* pdispatcher = pair.second;
        if (!pdispatcher)
          continue;
        // Query dispatchers for read and write wait state
        if (!process_io && (pdispatcher != signal_wakeup_))
          continue;
        current_dispatcher_keys_.push_back(pair.first);
    ...
    if (n < 0) {
      ...
      for (Dispatcher::key_t key : current_dispatcher_keys_) {
        if (!dispatchers_.count(key))
          continue;
        Dispatcher* pdispatcher = dispatchers_.at(key);
...
void PhysicalSocketServer::AddEpoll(Dispatcher* pdispatcher) {
  ...
  event.data.u64 = pdispatcher->key_.value();
...
void PhysicalSocketServer::UpdateEpoll(Dispatcher* pdispatcher) {
  ...
  if (!pdispatcher->key_) {
    RTC_LOG(LS_ERROR) << "Attempt to update a dispatcher's epoll settings "
                         "before adding to the list of dispatchers";
    return;
  }
  ...
  while (fWait_) {
    ...
    if (n < 0) {
      ...
      for (int i = 0; i < n; ++i) {
        ...
        Dispatcher::key_t key = event.data.u64;
        if (!dispatchers_.count(key)) {
...
bool PhysicalSocketServer::Wait(int cmsWait, bool process_io) {
  ...
  while (fWait_) {
    ...
    std::vector<Dispatcher::key_t> event_owners;
...

    {
      ...
      for (Dispatcher::key_t key : current_dispatcher_keys_) {
        ...
        Dispatcher* disp = dispatchers_.at(key);
        if (!disp)
          continue;
    ...
    if (dw == WSA_WAIT_FAILED) {
      ...
      if (index > 0) {
        ...
        Dispatcher::key_t key = event_owners[index];
        if (!dispatchers_.count(key)) {
