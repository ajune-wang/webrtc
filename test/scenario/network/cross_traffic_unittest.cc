...
TEST(CrossTrafficTest, PulsedPeaksCrossTraffic) {
  ...
  network_manager.Start();
  rtc::Event event;
  // Wait a bit more than 1 sec to handle minor delays inside traffic generator
  // that can cause not sending the last packet
  event.Wait(1010);
  network_manager.Stop();

  uint64_t kb100 = 100 * 1024;

  RTC_LOG(INFO) << counter->packets_count() << " packets; "
                << counter->total_packets_size() << " bytes";
  // We were sending 0.5 sec with 100 kB/s. Also we could send some data in
  // extra 10 ms.
  ASSERT_NEAR(counter->total_packets_size(), 0.5 * kb100, kb100 * 0.01);
}
...
TEST(CrossTrafficTest, RandomWalkCrossTraffic) {
  ...
  RTC_LOG(INFO) << counter->packets_count() << " packets; "
                << counter->total_packets_size() << " bytes";
  ASSERT_NEAR(counter->total_packets_size(), kb100 * 1.01, kb100 * 0.01);
}
