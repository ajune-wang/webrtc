...
ProbeController::ProbeController(const WebRtcKeyValueConfig* key_value_config,
  ...
  ParseFieldTrial(
      {&first_exponential_probe_scale_, &second_exponential_probe_scale_,
       ...
       &alr_probing_interval_ms_, &alr_probe_scale_},
      key_value_config->Lookup("WebRTC-Bwe-ConfigurableProbing"));
}
...
std::vector<ProbeClusterConfig> ProbeController::InitiateExponentialProbing(
  ...
  probes.push_back(first_exponential_probe_scale_ * start_bitrate_bps_);
  if (second_exponential_probe_scale_ > 0) {
    probes.push_back(second_exponential_probe_scale_ * start_bitrate_bps_);
  }
...
std::vector<ProbeClusterConfig> ProbeController::SetEstimatedBitrate(
  ...
  if (state_ == State::kWaitingForProbingResult) {
    ...
    if (min_bitrate_to_probe_further_bps_ != kExponentialProbingDisabled &&
      ...
      pending_probes =
          InitiateProbing(at_time_ms,
                          {static_cast<int64_t>(
                              further_exponential_probe_scale_ * bitrate_bps)},
                          true);
    }
  }
  ...
  if (enable_periodic_alr_probing_ && state_ == State::kProbingComplete) {
    ...
    if (alr_start_time_ms_ && estimated_bitrate_bps_ > 0) {
      int64_t next_probe_time_ms =
          ...
          alr_probing_interval_ms_;
      if (at_time_ms >= next_probe_time_ms) {
        return InitiateProbing(
            at_time_ms,
            {static_cast<int64_t>(estimated_bitrate_bps_ * alr_probe_scale_)},
            true);
      }
    }
  }
...
std::vector<ProbeClusterConfig> ProbeController::InitiateProbing(
  ...
  if (probe_further) {
    ...
    min_bitrate_to_probe_further_bps_ =
        (*(bitrates_to_probe.end() - 1)) * further_probe_threshold;
  } else {
    state_ = State::kProbingComplete;
    min_bitrate_to_probe_further_bps_ = kExponentialProbingDisabled;
  }
