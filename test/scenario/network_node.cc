...
void NetworkNode::SetRoute(uint64_t receiver,
                           EmulatedNetworkReceiverInteface* node) {
...
void NetworkNode::Process(Timestamp at_time) {
  ...
  for (PacketDeliveryInfo& delivery_info : delivery_infos) {
    ...
    EmulatedNetworkReceiverInteface* receiver = nullptr;
    {
      rtc::CritScope crit(&crit_sect_);
...
CrossTrafficSource::CrossTrafficSource(EmulatedNetworkReceiverInteface* target,
                                       uint64_t receiver_id,
