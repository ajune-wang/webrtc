...
void EventLogAnalyzer::CreateOutgoingEcnFeedbackGraph(Plot* plot) {
  TimeSeries not_ect("Not Ect", LineStyle::kLine, PointStyle::kHighlight);
  TimeSeries ect_1("Ect(1)", LineStyle::kLine, PointStyle::kHighlight);
  TimeSeries ce("Ce", LineStyle::kLine, PointStyle::kHighlight);

  for (const LoggedRtcpCongestionControlFeedback& feedback :
       parsed_log_.congestion_feedback(kOutgoingPacket)) {
    int ect_1_count = 0;
    int not_ect_count = 0;
    int ce_count = 0;
    ...
    for (const rtcp::CongestionControlFeedback::PacketInfo& info :
      ...
      switch (info.ecn) {
        ...
        case webrtc::EcnMarking::kEct0:
          RTC_LOG(LS_ERROR) << "unexpected eto(0)";
          break;
  ...
  plot->SetSuggestedYAxis(0, 10, "Count per feedback", kBottomMargin,
                          kTopMargin);
  plot->SetTitle("Outgoing ECN feedback)");
}
