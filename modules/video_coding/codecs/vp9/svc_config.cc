...
const size_t kMinScreenSharingLayerBitrateKbps[] = {30, 30};
}  // namespace
...
std::vector<SpatialLayer> ConfigureSvcScreenSharing(size_t input_width,
  ...
  num_spatial_layers =
      std::min<size_t>(num_spatial_layers, kMaxNumLayersForScreenSharing);
  ...
  for (size_t sl_idx = 0; sl_idx < num_spatial_layers; ++sl_idx) {
    ...
    spatial_layer.minBitrate = kMinScreenSharingLayerBitrateKbps[sl_idx];
