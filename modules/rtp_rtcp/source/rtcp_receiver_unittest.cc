...
TEST_F(RtcpReceiverTest, InjectByePacket_RemovesReferenceTimeInfo) {
  ...
  std::vector<rtcp::ReceiveTimeInfo> last_xr_rtis =
      rtcp_receiver_.LastReceivedXrReferenceTimeInfo();
  ...
  EXPECT_THAT(rtcp_receiver_.LastReceivedXrReferenceTimeInfo(), IsEmpty());
}
...
TEST_F(RtcpReceiverTest, InjectExtendedReportsReceiverReferenceTimePacket) {
  ...
  last_xr_rtis = rtcp_receiver_.LastReceivedXrReferenceTimeInfo();
  ASSERT_THAT(last_xr_rtis, SizeIs(1));
  EXPECT_EQ(kSenderSsrc, last_xr_rtis[0].ssrc);
  ...
  last_xr_rtis = rtcp_receiver_.LastReceivedXrReferenceTimeInfo();
  ASSERT_THAT(last_xr_rtis, SizeIs(1));
  EXPECT_NEAR(kDelayMs, CompactNtpRttToMs(last_xr_rtis[0].delay_since_last_rr),
              1);
}
...
TEST_F(RtcpReceiverTest, LastReceivedXrReferenceTimeInfoInitiallyEmpty) {
  EXPECT_THAT(rtcp_receiver_.LastReceivedXrReferenceTimeInfo(), IsEmpty());
}
...
TEST_F(RtcpReceiverTest, GetLastReceivedXrReferenceTimeInfoUpdated) {
...
TEST_F(RtcpReceiverTest, GetLastReceivedXrReferenceTimeInfoForMultipleReports) {
  const size_t kNumReports = 2;
  ...
  ASSERT_THAT(last_xr_rtis, SizeIs(kNumReports));
  for (size_t i = 0; i < kNumReports; ++i) {
