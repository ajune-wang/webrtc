...
TEST(RtpPacketizerH264Test, StapA) {
  ...
  // 1st fragment.
  EXPECT_THAT(payload.subview(0, 2), ElementsAre(0, 2));  // Size.
  EXPECT_THAT(payload.subview(2, 2), ElementsAreArray(frame.data(), 2));
  payload = payload.subview(kLengthFieldLength + 2);
  // 2nd fragment.
  EXPECT_THAT(payload.subview(0, 2), ElementsAre(0, 2));  // Size.
  EXPECT_THAT(payload.subview(2, 2), ElementsAreArray(frame.data() + 2, 2));
  payload = payload.subview(kLengthFieldLength + 2);
  // 3rd fragment.
  EXPECT_THAT(payload.subview(0, 2), ElementsAre(0x1, 0x23));  // Size.
  EXPECT_THAT(payload.subview(2), ElementsAreArray(frame.data() + 4, 0x123));
