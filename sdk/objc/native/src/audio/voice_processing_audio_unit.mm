...
bool VoiceProcessingAudioUnit::Initialize(Float64 sample_rate) {
  ...
  RTCAudioSessionConfiguration* webRTCConfiguration =
      [RTCAudioSessionConfiguration webRTCConfiguration];
  if (webRTCConfiguration.mode != AVAudioSessionModeMoviePlayback) {
    RTCLog("@Setting the format on the output scope of the input element/bus because it's not "
           "movie mode");
    ...
    RTCLog("@NOT setting the format on the output sscope of the input element because it's movie "
           "mode");
  }
