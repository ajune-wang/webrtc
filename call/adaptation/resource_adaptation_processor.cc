...
void ResourceAdaptationProcessor::MaybeUpdateEffectiveDegradationPreference() {
  ...
  RTC_LOG(INFO) << "Update degradation preference to " << DegradationPreferenceToString(effective_degradation_preference_);
...
void ResourceAdaptationProcessor::OnVideoSourceRestrictionsUpdated(
  ...
  RTC_DCHECK_RUN_ON(resource_adaptation_queue_);
  if (reason)
    UpdateResourceLimitations(reason, {restrictions, adaptation_counters});
}
