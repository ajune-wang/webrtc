...
// Computes the speech level error to be compensated by an input volume
// adjustment. Returns zero if `speech_level_dbfs` falls in the
// [`target_range_min_dbfs`, `target_range_max_dbfs`] range, or a positive
// number computed as the difference between `target_range_min_dbfs` and
// `speech_level_dbfs` if the latter value lies under the range, or a negative
// number computed as the differente between `target_range_min_dbfs`
// and`speech_level_dbfs` if the latter value lies above the range.
