...
// A histogram is only created if a valid ApiInformation::IsPropertyPresent()
// call can be done.
...
void LogDirtyRegionSupport() {
  ...
  boolean is_dirty_region_mode_supported = api_info_statics->IsPropertyPresent(
      wgc_session_type, dirty_region_mode, &is_dirty_region_mode_supported);
