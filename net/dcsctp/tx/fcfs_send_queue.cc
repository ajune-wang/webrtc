...
namespace dcsctp {
void FCFSSendQueue::Add(int64_t now_ms,
                        DcSctpMessage message,
                        const SendOptions& send_options) {
  std::deque<Item>& queue =
      IsPaused(message.stream_id()) ? paused_items_ : items_;
  int64_t expiry_ms =
      send_options.lifetime_ms < 0 ? -1 : now_ms + send_options.lifetime_ms;
  queue.emplace_back(std::move(message), expiry_ms, send_options);
}
...
FCFSSendQueue::Item* FCFSSendQueue::GetFirstNonExpiredMessage(int64_t now_ms) {
  while (!items_.empty()) {
    ...
    if (!item.message_id.has_value() && item.expiry_ms >= 0 &&
        item.expiry_ms <= now_ms) {
...
absl::optional<SendQueue::DataToSend> FCFSSendQueue::Produce(int64_t now_ms,
                                                             size_t max_size) {
  Item* item = GetFirstNonExpiredMessage(now_ms);
  if (item == nullptr) {
    return absl::nullopt;
  }
  ...
  SendQueue::DataToSend chunk = {
      .data = Data(stream_id, item->ssn.value_or(SSN(0)),
                   item->message_id.value(), fsn, ppid, std::move(payload),
                   is_beginning, is_end, item->send_options.unordered),
      .max_retransmissions = item->send_options.max_retransmissions,
