...
void VideoStreamDecoderImpl::SaveFrameMetaInfo(
    const video_coding::EncodedFrame& frame) {
  FrameMetaInfo* frame_meta_info =
      &frame_meta_info_[next_frame_meta_info_index_];
  frame_meta_info->timestamp = frame.Timestamp();
  frame_meta_info->decode_start_time_ms = rtc::TimeMillis();
  frame_meta_info->render_time_us = frame.RenderTimeMs() * 1000;
  frame_meta_info->content_type = frame.EncodedImage().content_type_;

  next_frame_meta_info_index_ =
      Add<kFrameMetaInfoMemory>(next_frame_meta_info_index_, 1);
}
...
VideoStreamDecoderImpl::FrameMetaInfo* VideoStreamDecoderImpl::GetFrameMetaInfo(
    int64_t timestamp) {
  int start_time_index = next_frame_meta_info_index_;
  for (int i = 0; i < kFrameMetaInfoMemory; ++i) {
    start_time_index = Subtract<kFrameMetaInfoMemory>(start_time_index, 1);

    if (frame_meta_info_[start_time_index].timestamp == timestamp)
      return &frame_meta_info_[start_time_index];
  }
...
void VideoStreamDecoderImpl::OnDecodedFrameCallback(
  ...
  bookkeeping_queue_.PostTask([this, decode_stop_time_ms, decoded_image,
    ...
    FrameMetaInfo* frame_meta_info =
        GetFrameMetaInfo(decoded_image.timestamp());
    if (!frame_meta_info) {
    ...
    Callbacks::FrameMetaInfo callback_meta_info;
    callback_meta_info.content_type = frame_meta_info->content_type;

    if (qp)
      callback_meta_info.qp.emplace(*qp);

    callback_meta_info.decode_time_ms = decode_time_ms.value_or(
        decode_stop_time_ms - frame_meta_info->decode_start_time_ms);

    timing_.StopDecodeTimer(*callback_meta_info.decode_time_ms,
                            decode_stop_time_ms);

    VideoFrame copy = decoded_image;
    copy.set_timestamp_us(frame_meta_info->render_time_us);
    callbacks_->OnDecodedFrame(copy, callback_meta_info);
  });
}
