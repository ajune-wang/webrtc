...
void VideoStreamEncoder::OnVideoSourceRestrictionsUpdated(
  ...
  // TODO(hbos): FIX THIS BEFORE LANDING
  max_pixels_updated = true;
  if (max_pixels_updated) {
    // Reconfigure encoder in response to restricted frame size changing.
    // This is only needed when `requested_resolution` is used, otherwise we
    // reconfigure in response to VideoFrame being downscaled by the source.
    bool uses_requested_resolution = false;
    for (const VideoStream& simulcast_layer :
         encoder_config_.simulcast_layers) {
      if (simulcast_layer.requested_resolution.has_value()) {
        uses_requested_resolution = true;
        break;
      }
    }
    // TODO(hbos): FIX THIS BEFORE LANDING
    uses_requested_resolution = true;
    if (encoder_ && uses_requested_resolution) {
      pending_encoder_reconfiguration_ = true;
      ReconfigureEncoder();
    }
  }
