...
TEST(CpuSpeedExperimentTest, GetValue) {
  ...
  const absl::optional<std::vector<CpuSpeedExperiment::Config>> kConfigs =
      CpuSpeedExperiment::GetConfigs();
  ASSERT_TRUE(kConfigs.has_value());
...
TEST(CpuSpeedExperimentTest, GetConfigsFailsIfPixelsInDecreasingOrder) {
  webrtc::test::ScopedFieldTrials field_trials(
      "WebRTC-VP8-CpuSpeed-Arm/Enabled-1000,-1,999,-10,3000,-16/");
  EXPECT_FALSE(CpuSpeedExperiment::GetConfigs());
}

TEST(CpuSpeedExperimentTest, GetConfigsFailsSpeedInIncreasingOrder) {
  webrtc::test::ScopedFieldTrials field_trials(
