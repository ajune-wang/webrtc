...
int GetAlignment(const VideoEncoder::EncoderInfo& encoder_info,
  ...
  if (!encoder_info.apply_alignment_to_simulcast_layers) {
  ...
  // Test round scale factors to the closest value that is a multiple of
  // |multiple|.
  double diff = RoundToMultiple(multiple, config, /*update_config=*/false);
  if (diff > 0.2 && multiple <= 4) {
    multiple *= 2;  // Increase scaling steps.
  }
