...
TEST_F(PeerConnectionMultiTrackTest, RecycleMediaSectionForStoppedTransceiver) {
  auto caller = CreatePeerConnection();
  auto transceiver = caller->AddTransceiver(cricket::MEDIA_TYPE_AUDIO);
  auto callee = CreatePeerConnection();
  ...
  std::string original_mid = *transceiver->mid();
  transceiver->Stop();
  ...
  ASSERT_EQ(rtc::nullopt, transceiver->mid());
  ...
  EXPECT_EQ(
      cricket::MEDIA_TYPE_VIDEO,
      static_cast<MediaContentDescription*>(contents[0].description)->type());
  EXPECT_EQ(original_mid, contents[0].name);
}
