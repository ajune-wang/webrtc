...
// RAII type that ensures global state is consistent.
class ScopedGlobalFieldTrials {
 public:
  explicit ScopedGlobalFieldTrials(flat_set<std::string> keys) {
    InitFieldTrialsFromString(nullptr);
    RegisterFieldTrialsForTesting(std::move(keys));
  }
  ~ScopedGlobalFieldTrials() { UnregisterAllFieldTrialsForTesting(); }
};

TEST(FieldTrialsTest, EmptyStringHasNoEffect) {
  ScopedGlobalFieldTrials g({"MyCoolTrial"});
  FieldTrials f("");
...
TEST(FieldTrialsTest, FieldTrialsDoesNotReadGlobalString) {
  ScopedGlobalFieldTrials g({"MyCoolTrial", "MyUncoolTrial"});
...
TEST(FieldTrialsTest, NonGlobalFieldTrialsInstanceDoesNotModifyGlobalString) {
  ScopedGlobalFieldTrials g({"SomeString"});
...
TEST(FieldTrialsTest, GlobalAndNonGlobalFieldTrialsAreDisjoint) {
  ScopedGlobalFieldTrials g({"SomeString", "SomeOtherString"});
