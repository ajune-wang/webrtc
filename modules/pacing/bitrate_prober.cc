...
constexpr TimeDelta kProbeClusterTimeout = TimeDelta::Millis<5000>();
...
void BitrateProber::OnIncomingPacket(size_t packet_size, Timestamp now) {
...
absl::optional<Timestamp> BitrateProber::NextProbeTime(Timestamp now) const {
  // Probing is not active or probing is already complete.
  if (probing_state_ != ProbingState::kActive || clusters_.empty()) {
    return absl::nullopt;
  }

  if (next_probe_time_.IsInfinite()) {
    return absl::nullopt;
  }

  if (now - next_probe_time_ > config_.max_probe_delay.Get()) {
...
void BitrateProber::ProbeSent(Timestamp now, size_t bytes) {
  ...
  if (!clusters_.empty()) {
    ...
    next_probe_time_ = GetNextProbeTime(*cluster);
...
Timestamp BitrateProber::GetNextProbeTime(const ProbeCluster& cluster) {
  RTC_CHECK_GT(cluster.pace_info.send_bitrate_bps, 0);
  RTC_CHECK_GE(cluster.started_at, Timestamp::Zero());
