...
void VideoReceiveStream2::RequestKeyFrame(int64_t timestamp_ms) {
  ...
  webrtc::MutexLock lock(&evans_mutex_);
...
void VideoReceiveStream2::StartNextDecode() {
  ...
  RTC_DCHECK_RUN_ON(&decode_sequence_);
  ...
  RTC_DLOG(INFO) << "last decode time " << last_decoded_frame_timestamp_.ms();

  if (decoder_stopped_) {
    RTC_DLOG(INFO) << "decoder_stopped";
    return;
  }

  std::unique_ptr<EncodedFrame> next_frame =
      frame_buffer_->GetNextFrame2(keyframe_required_);
  const Timestamp encode_start = clock_->CurrentTime();
  RTC_DLOG(INFO) << "fetched frame. encode start " << encode_start.ms();
  if (!next_frame) {
    RTC_DLOG(INFO) << "no frame.";
    // Handle timeout or just skip cancel.
    if (encode_start - last_decoded_frame_timestamp_ >
        TimeDelta::Millis(GetMaxWaitMs())) {
      // TODO(bugs.webrtc.org/11993): PostTask to the network thread.
      RTC_DLOG(INFO) << "timeout. waited more than wait " << GetMaxWaitMs();
