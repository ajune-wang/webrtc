...
RTCError PeerConnection::UpdateTransceiverChannel(
  ...
  cricket::BaseChannelInterface* channel = transceiver->internal()->channel();
  if (content.rejected) {
    if (channel) {
...
cricket::BaseChannelInterface* PeerConnection::GetChannel(
    const std::string& content_name) {
  for (auto transceiver : transceivers_) {
    cricket::BaseChannelInterface* channel = transceiver->internal()->channel();
...
const std::string PeerConnection::GetTransportName(
    const std::string& content_name) {
  cricket::BaseChannelInterface* channel = GetChannel(content_name);
  if (channel) {
...
void PeerConnection::DestroyBaseChannel(
    cricket::BaseChannelInterface* channel) {
  RTC_DCHECK(channel);
