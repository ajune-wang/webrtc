...
namespace {
// MONITOR_DEFAULTTOPRIMARY from winuser.h, this avoids a header include.
const DWORD kDefaultToPrimaryMonitor = 1;
}  // namespace
...
bool DisplayConfigurationMonitor::IsChanged(
  ...
  if (!initialized_) {
    initialized_ = true;
  } else if (!rect.equals(rect_) || !source_dpis_.at(source_id).equals(dpi)) {
    has_changed = true;
  }
...
DesktopVector DisplayConfigurationMonitor::GetDpiForSourceId(
  ...
  if (source_id == kFullDesktopScreenId) {
    ...
    monitor = MonitorFromPoint({0, 0}, kDefaultToPrimaryMonitor);
