...
bool DxgiOutputDuplicator::Duplicate(Context* context,
  ...
  if (target->is_texture()) {
    DesktopFrameTexture frame(target->size());
    bool ret = DuplicateNative(context, offset, &frame);
    target->MoveFrameInfoFrom(&frame);
...
bool DxgiOutputDuplicator::CreateNV12Texture() {
  if (last_handle_) {
    CloseHandle(last_handle_);
  ...
  _com_error error = device_.d3d_device()->CreateTexture2D(
      &desc, nullptr, last_texture_.GetAddressOf());
  if (error.Error() != S_OK) {
...
bool DxgiOutputDuplicator::DuplicateNative(Context* context,
  ...
  if (error.Error() == S_OK && resource &&
    ...
    if (!CreateNV12Texture()) {
        RTC_LOG(LS_ERROR) << "Failed to create NV12 texture.";
        return ReleaseFrame();
    }
    ...
    HANDLE dup_handle;
    if (!DuplicateHandle(GetCurrentProcess(), last_handle_, GetCurrentProcess(),
  ...
  if (last_handle_) {
    ...
    if (!DuplicateHandle(GetCurrentProcess(), last_handle_, GetCurrentProcess(),
                        &dup_handle, 0, FALSE, DUPLICATE_SAME_ACCESS)) {
      RTC_LOG(LS_ERROR) << "Failed to duplicate last handle.";
    } else {
      target->Init(device_, dup_handle);
    }
  } else {
