...
bool WebRtcVideoChannel::SetBaseMinimumPlayoutDelayMs(uint32_t ssrc,
                                                      int delay_ms) {
  // Be careful here, GetDefaultReceiveStreamSsrc locks streaM_crit_ too.
  // Changing calling order may result in deadlock.
...
void WebRtcVideoChannel::WebRtcVideoReceiveStream::RecreateWebRtcVideoStream() {
  ...
  if (base_minimum_playout_delay_ms_) {
    stream_->SetBaseMinimumPlayoutDelayMs(
        base_minimum_playout_delay_ms_.value());
  }
...
bool WebRtcVideoChannel::WebRtcVideoReceiveStream::SetBaseMinimumPlayoutDelayMs(
    int delay_ms) {
  if (stream_ && stream_->SetBaseMinimumPlayoutDelayMs(delay_ms)) {
    // Memorize only valid delays.
