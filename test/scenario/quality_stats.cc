...
void VideoQualityAnalyzer::OnDecodedFrame(const VideoFrame& frame) {
  ...
  task_queue_.PostTask([this, decoded] {
    // TODO(srte): Handle the case where the first frame never is received.
    if (!first_decode_rtp_timestamp_)
      first_decode_rtp_timestamp_ = decoded.timestamp();
    RTC_CHECK(!captured_frames_.empty());
    int64_t decoded_capture_time_ms = DecodedFrameCaptureMs(decoded);
    while (CaptureFrameCaptureMs(captured_frames_.front()) <
    ...
    RTC_CHECK(!captured_frames_.empty());
    RTC_CHECK(CaptureFrameCaptureMs(captured_frames_.front()) ==
              DecodedFrameCaptureMs(decoded));
...
int64_t VideoQualityAnalyzer::DecodedFrameCaptureMs(
...
int64_t VideoQualityAnalyzer::CaptureFrameCaptureMs(
