...
//
//   // Register a callback (doesn't have to be a lambda). This can
//   // be done zero or more times. The callback must return void.
//   foo_.AddReceiver([...](int a, float b) {...});
//
...
// is protected bby a mutex, for example.
//
...
// adding them, but removal support can easily be added. There are essentially
// two good options that I can see:
//
//   * Have callbacks optionally return an enum instead of void; the two
//     possible values would be kKeepCallingMe and kDontCallMeAgain.
//
//   * Return a cookie from AddReceiver(), and add a RemoveReceiver() method
//     that accepts such cookies.
//
// The two solutions are convenient in different situations, and are not
// mutually exclusive; we could conceivably build them both.
