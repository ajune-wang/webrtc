...
template <>
struct TypedParameterParser<CustomEnum> {
 public:
  explicit TypedParameterParser(const std::string& str) {
    if (str == "default")
      value = CustomEnum::kDefault;
    else if (str == "red")
      value = CustomEnum::kRed;
    else if (str == "blue")
      value = CustomEnum::kBlue;
    else
      valid = false;
  }
  CustomEnum Get() const { return value; }
  bool valid = true;
  CustomEnum value;
};
...
TEST(FieldTrialParserTest, ParsesOptionalParameters) {
  FieldTrialParameter<rtc::Optional<int64_t>> max_count("c", rtc::nullopt);
