  ...
  for (size_t i = 0; i < streams->count(); ++i) {
    ...
    if (is_local) {
      for (auto audio_track : stream->GetAudioTracks()) {
        auto sender = track_media_info_map.GetSenderByTrack(audio_track);
        RTC_DCHECK(sender);
        stream_stats->track_ids->push_back(
            RTCMediaStreamTrackStatsIDFromTrackKindIDAndAttachment(
                is_local, MediaStreamTrackInterface::kAudioKind,
                audio_track->id(), sender->AttachmentId()));
      }
      for (auto video_track : stream->GetVideoTracks()) {
        auto sender = track_media_info_map.GetSenderByTrack(video_track);
        RTC_DCHECK(sender);
        stream_stats->track_ids->push_back(
            RTCMediaStreamTrackStatsIDFromTrackKindIDAndAttachment(
                is_local, MediaStreamTrackInterface::kVideoKind,
                video_track->id(), sender->AttachmentId()));
      }
    } else {
      for (auto audio_track : stream->GetAudioTracks()) {
        auto receiver = track_media_info_map.GetReceiverByTrack(audio_track);
        RTC_DCHECK(receiver);
        stream_stats->track_ids->push_back(
            RTCMediaStreamTrackStatsIDFromTrackKindIDAndAttachment(
                is_local, MediaStreamTrackInterface::kAudioKind,
                audio_track->id(), receiver->AttachmentId()));
      }
      for (auto video_track : stream->GetVideoTracks()) {
        auto receiver = track_media_info_map.GetReceiverByTrack(video_track);
        RTC_DCHECK(receiver);
        stream_stats->track_ids->push_back(
            RTCMediaStreamTrackStatsIDFromTrackKindIDAndAttachment(
                is_local, MediaStreamTrackInterface::kVideoKind,
                video_track->id(), receiver->AttachmentId()));
      }
    }
  ...
  if (track_media_info_map.voice_media_info()) {
    ...
    for (const cricket::VoiceReceiverInfo& voice_receiver_info :
      ...
      if (audio_track) {
        ...
        auto receiver = track_media_info_map_->GetReceiverByTrack(audio_track);
        RTC_DCHECK(receiver);
    ...
    for (const cricket::VoiceSenderInfo& voice_sender_info :
      ...
      if (audio_track) {
        RTC_DCHECK(track_to_id_.find(audio_track.get()) != track_to_id_.end());
        auto sender = track_media_info_map_->GetSenderByTrack(audio_track);
        RTC_DCHECK(sender);
        RTC_DCHECK(sender->AttachmentId());  // should be nonzero
  ...
  if (track_media_info_map.video_media_info()) {
    ...
    for (const cricket::VideoReceiverInfo& video_receiver_info :
      ...
      if (video_track) {
        ...
        auto receiver = track_media_info_map_->GetReceiverByTrack(video_track);
        RTC_DCHECK(receiver);
    ...
    for (const cricket::VideoSenderInfo& video_sender_info :
      ...
      if (video_track) {
        RTC_DCHECK(track_to_id_.find(video_track.get()) != track_to_id_.end());
        auto sender = track_media_info_map_->GetSenderByTrack(video_track);
        RTC_DCHECK(sender);
        RTC_DCHECK(sender->AttachmentId());  // should be nonzero
