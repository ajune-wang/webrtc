...
void SubtractorOutputAnalyzer::Update(
  ...
  for (size_t channel = 0; channel < subtractor_output.size(); ++channel) {
    const float y2 = subtractor_output[channel].y2;
    const float e2_main = subtractor_output[channel].e2_main;
    const float e2_shadow = subtractor_output[channel].e2_shadow;
