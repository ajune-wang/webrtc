...
#ifndef MEDIA_SCTP_DCSCTP_PACKET_BOUNDED_BYTE_READER_H_
#define MEDIA_SCTP_DCSCTP_PACKET_BOUNDED_BYTE_READER_H_
...
class BoundedByteReader {
 public:
  ...
  uint64_t Load64() const {
    static_assert(offset + sizeof(uint64_t) <= FixedSize, "Out-of-bounds");
    static_assert((offset % sizeof(uint32_t)) == 0, "Wrong alignment");
    return absl::big_endian::Load64(&data_[offset]);
  }
  ...
  uint32_t Load32() const {
    ...
    static_assert((offset % sizeof(uint32_t)) == 0, "Wrong alignment");
    return absl::big_endian::Load32(&data_[offset]);
  }

  template <size_t offset>
  uint16_t Load16() const {
    static_assert(offset + sizeof(uint16_t) <= FixedSize, "Out-of-bounds");
    static_assert((offset % sizeof(uint16_t)) == 0, "Wrong alignment");
    return absl::big_endian::Load16(&data_[offset]);
  }
...
#endif  // MEDIA_SCTP_DCSCTP_PACKET_BOUNDED_BYTE_READER_H_
