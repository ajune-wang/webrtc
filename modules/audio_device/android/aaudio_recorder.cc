...
void AAudioRecorder::OnErrorCallback(aaudio_result_t error) {
  ...
  if (aaudio_.stream_state() == AAUDIO_STREAM_STATE_DISCONNECTED) {
    ...
    if (rtc::AtomicOps::CompareAndSwap(&restart_count_, 0, 1) == 0) {
      RTC_LOG(WARNING) << "___Restarting input: " << restart_count_;
...
void AAudioRecorder::HandleStreamDisconnected() {
  ...
  InitRecording();
  StartRecording();
  RTC_LOG(INFO) << "Restarted device ID: " << aaudio_.device_id();
  int old_value = rtc::AtomicOps::CompareAndSwap(&restart_count_, 1, 0);
  RTC_DCHECK_EQ(1, old_value);
  RTC_LOG(WARNING) << "___Restart of input is done: " << restart_count_;
}
