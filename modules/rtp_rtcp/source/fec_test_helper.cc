...
RtpPacketReceived UlpfecPacketGenerator::BuildMediaRedPacket(
  ...
  uint8_t media_payload_type = red_buffer.data()[1] & 0x7F;
  red_buffer.AppendData({media_payload_type});
  // Set packet payload type to RED.
  red_buffer.MutableData()[1] = (red_buffer.data()[1] & 0x80) | kRedPayloadType;
  ...
  RTC_CHECK(red_packet.Parse(red_buffer));
