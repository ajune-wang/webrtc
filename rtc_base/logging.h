...
class LogMetadata {
 ...
 private:
  // We need this constructor in order for this class to be trivial, which is
  // necessary since we want to use it as a variadic argument.
  LogMetadata() = default;
...
// TODO(webrtc:8982): Get rid of this specialization when callers don't need it
// anymore. No in-tree caller does, but some external callers still do.
template <>
...
// TODO(webrtc:8982): Get rid of this overload when callers don't need it
// anymore. No in-tree caller does, but some external callers still do.
template <
    typename T,
    typename T1 =
...
#define RTC_DLOG_EAT_STREAM_PARAMS(sev)             \
  for (bool do_log = false; do_log; do_log = false) \
  rtc::webrtc_logging_impl::LogCall() &             \
      rtc::webrtc_logging_impl::LogStreamer<>()     \
          << rtc::webrtc_logging_impl::LogMetadata(__FILE__, __LINE__, sev)
