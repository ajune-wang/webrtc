...
int VP9EncoderImpl::Encode(const VideoFrame& input_image,
  ...
  if (!force_key_frame_) {
    ...
    if (VideoCodecMode::kScreensharing == codec_.mode) {
      ...
      if (input_image.update_rect().IsEmpty() &&
          num_steady_state_frames_ >= 5) {
        if (variable_framerate_experiment_.enabled &&
            variable_framerate_controller_.DropFrame(frame_timestamp_ms)) {
          // Skip all layers to drop the frame.
...
void VP9EncoderImpl::DeliverBufferedFrame(bool end_of_picture) {
  if (encoded_image_.size() > 0) {
    ...
    if (codec_.mode == VideoCodecMode::kScreensharing) {
      ...
      if (codec_.spatialLayers[spatial_idx].maxFramerate >
