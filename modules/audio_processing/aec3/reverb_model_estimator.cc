...
bool ReverbModelEstimator::IsAGoodFilter(int filter_delay_blocks,
...
void ReverbModelEstimator::Update(
  ...
  if (enable_smooth_freq_resp_tail_updates_) {
    if (!stationary_block) {
      if (IsAGoodFilter(filter_delay_blocks, usable_linear_estimate,
...
void ReverbModelEstimator::UpdateReverbDecay(
  ...
  std::transform(
      ...
      [](float a) {
        return a * a;
      });  // TODO(devicentepena) check if focusing on one block would be enough
