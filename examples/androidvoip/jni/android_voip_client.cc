...
void AndroidVoipClient::StartSession(JNIEnv* env) {
  ...
  Java_VoipClient_onStartSessionCompleted(env_, j_voip_client_,
                                          /*isSuccessful=*/true);
  voip_thread_->PostTask([this, env] { LogStatistics(env); });
}

void AndroidVoipClient::LogStatistics(JNIEnv* env) {
  RUN_ON_VOIP_THREAD(LogStatistics, env)
  ...
  voip_thread_->PostDelayedTask([this, env] { LogStatistics(env); },
