...
class RtpVideoFrameAssembler::Impl {
 ...
 private:
  ...
  void SaveFrameIdToSeqNum(const RtpFrameReferenceFinder::ReturnVector& frames);
...
RtpVideoFrameAssembler::Impl::FindReferences(RtpFrameVector frames) {
  ...
  for (auto& frame : frames) {
    ...
    SaveFrameIdToSeqNum(complete_frames);
...
RtpVideoFrameAssembler::Impl::UpdateWithPadding(uint16_t seq_num) {
  ...
  // Completed frames already found by updating the `packet_buffer_` and then
  // the `reference_finder_` with potentially new frames have already been saved
  // when 'FindReferences' was called above.
  SaveFrameIdToSeqNum(ref_finder_update);
...
void RtpVideoFrameAssembler::Impl::SaveFrameIdToSeqNum(
    const RtpFrameReferenceFinder::ReturnVector& frames) {
  for (auto& frame : frames) {
...
RtpVideoFrameAssembler::RtpVideoFrameAssembler(PayloadFormat payload_format)
    : impl_(std::make_unique<RtpVideoFrameAssembler::Impl>(
          CreateDepacketizer(payload_format))) {}
