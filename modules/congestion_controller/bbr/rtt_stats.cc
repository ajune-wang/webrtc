...
void RttStats::UpdateRtt(TimeDelta send_delta,
  ...
  if (smoothed_rtt_.IsZero()) {
    ...
    mean_deviation_ = TimeDelta::us(rtt_sample.us() / 2);
  } else {
    mean_deviation_ = TimeDelta::us(static_cast<int64_t>(
        kOneMinusBeta * mean_deviation_.us() +
        kBeta * std::abs((smoothed_rtt_ - rtt_sample).us())));
