...
int32_t AudioMixerManagerLinuxPulse::SetPulseAudioObjects(
  ...
  if (!mainloop || !context) {
    NLOG(LS_ERROR, "could not set PulseAudio objects for mixer");
    return -1;
  }
  ...
  NLOG(LS_VERBOSE, "the PulseAudio objects for the mixer has been set");
...
int32_t AudioMixerManagerLinuxPulse::SetPlayStream(pa_stream* playStream) {
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
  NLOG(LS_VERBOSE, "AudioMixerManagerLinuxPulse::SetPlayStream(playStream)");
...
int32_t AudioMixerManagerLinuxPulse::SetRecStream(pa_stream* recStream) {
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
  NLOG(LS_VERBOSE, "AudioMixerManagerLinuxPulse::SetRecStream(recStream)");
...
int32_t AudioMixerManagerLinuxPulse::OpenSpeaker(uint16_t deviceIndex) {
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
  NLOG(LS_VERBOSE,
       "AudioMixerManagerLinuxPulse::OpenSpeaker(deviceIndex=", deviceIndex,
       ")");

  // No point in opening the speaker
  // if PA objects have not been set
  if (!_paObjectsSet) {
    NLOG(LS_ERROR, "PulseAudio objects has not been set");
    return -1;
  }
...
int32_t AudioMixerManagerLinuxPulse::OpenMicrophone(uint16_t deviceIndex) {
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
  NLOG(LS_VERBOSE,
       "AudioMixerManagerLinuxPulse::OpenMicrophone(deviceIndex=", deviceIndex,
       ")");

  // No point in opening the microphone
  // if PA objects have not been set
  if (!_paObjectsSet) {
    NLOG(LS_ERROR, "PulseAudio objects have not been set");
    return -1;
  }
  ...
  NLOG(LS_VERBOSE, "the input mixer device is now open");
...
int32_t AudioMixerManagerLinuxPulse::SetSpeakerVolume(uint32_t volume) {
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
  NLOG(LS_VERBOSE,
       "AudioMixerManagerLinuxPulse::SetSpeakerVolume(volume=", volume, ")");

  if (_paOutputDeviceIndex == -1) {
    NLOG(LS_WARNING, "output device index has not been set");
    return -1;
  }
  ...
  if (_paPlayStream &&
    ...
    if (!spec) {
      NLOG(LS_ERROR, "could not get sample specification");
      return -1;
    }
  ...
  if (setFailed) {
    NLOG(LS_WARNING, "could not set speaker volume, error=",
...
int32_t AudioMixerManagerLinuxPulse::SpeakerVolume(uint32_t& volume) const {
  ...
  NLOG(LS_VERBOSE,
       "AudioMixerManagerLinuxPulse::SpeakerVolume() => vol=", volume);
...
int32_t AudioMixerManagerLinuxPulse::SetSpeakerMute(bool enable) {
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
  NLOG(LS_VERBOSE,
       "AudioMixerManagerLinuxPulse::SetSpeakerMute(enable=", enable, ")");
  ...
  if (setFailed) {
    NLOG(LS_WARNING,
         "could not mute speaker, error=", LATE(pa_context_errno)(_paContext));
    return -1;
  }
...
int32_t AudioMixerManagerLinuxPulse::StereoRecordingIsAvailable(
  ...
  if (_paInputDeviceIndex == -1) {
    NLOG(LS_WARNING, "input device index has not been set");
    return -1;
  }
  ...
  NLOG(LS_VERBOSE, "AudioMixerManagerLinuxPulse::StereoRecordingIsAvailable()",
       " => available=", available);
...
int32_t AudioMixerManagerLinuxPulse::SetMicrophoneMute(bool enable) {
  ...
  if (setFailed) {
    NLOG(LS_WARNING, "could not mute microphone, error=",
         LATE(pa_context_errno)(_paContext));
    return -1;
  }
...
int32_t AudioMixerManagerLinuxPulse::SetMicrophoneVolume(uint32_t volume) {
  ...
  if (setFailed) {
    NLOG(LS_WARNING, "could not set microphone volume, error=",
         LATE(pa_context_errno)(_paContext));
    return -1;
  }
...
void AudioMixerManagerLinuxPulse::PaSetVolumeCallback(pa_context* c,
                                                      ...
                                                      void* /*pThis*/) {
  if (!success) {
    NLOG(LS_ERROR, "failed to set volume");
  }
}
