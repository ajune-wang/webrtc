...
#if defined(WEBRTC_WIN)
constexpr PlatformThread::Handle PlatformThread::kNullHandle;
#else
constexpr PlatformThread::Handle PlatformThread::kNullHandle;
#endif  // defined(WEBRTC_WIN)
...
PlatformThread::Handle PlatformThread::GetHandle() const {
  return handle_;
}

#if defined(WEBRTC_WIN)
bool PlatformThread::QueueAPC(PAPCFUNC function, ULONG_PTR data) {
  return QueueUserAPC(function, handle_, data) != FALSE;
}
#endif
...
PlatformThread PlatformThread::SpawnThread(
  ...
  auto start_thread_function_ptr =
      std::make_unique<std::function<void()>>([thread_function =
  ...
  PlatformThread::Handle handle = PlatformThread::kNullHandle;
#if defined(WEBRTC_WIN)
  // See bug 2902 for background on STACK_SIZE_PARAM_IS_A_RESERVATION.
  // Set the reserved stack stack size to 1M, which is the default on Windows
  // and Linux.
  DWORD thread_id = 0;
  handle = ::CreateThread(nullptr, 1024 * 1024, &RunPlatformThread,
