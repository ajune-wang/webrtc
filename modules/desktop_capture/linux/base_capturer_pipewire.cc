...
void BaseCapturerPipeWire::HandleBuffer(pw_buffer* buffer) {
  ...
  if ((videoCrop = static_cast<struct spa_meta_video_crop*>(
    ...
    RTC_DCHECK(videoCrop->width <= desktop_size_.width() &&
               videoCrop->height <= desktop_size_.height());
    if (videoCrop->width != desktop_size_.width() ||
        videoCrop->height != desktop_size_.height()) {
  ...
  if (!current_frame_ ||
      (video_crop_size_ && !video_crop_size_->equals(prevCropSize))) {
    current_frame_ = std::make_unique<uint8_t[]>(frameSize);
  }
  ...
  const int xOffset =
      video_crop_size_ && (videoCrop->x + video_crop_size_->width() <=
                           desktop_size_.width())
...
void BaseCapturerPipeWire::CaptureFrame() {
  ...
  DesktopSize frameSize = video_crop_size_.value_or(desktop_size_);
  std::unique_ptr<DesktopFrame> result(new BasicDesktopFrame(frameSize));
  result->CopyPixelsFrom(
      current_frame_.get(), (frameSize.width() * kBytesPerPixel),
