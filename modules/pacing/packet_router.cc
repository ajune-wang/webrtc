...
PacketRouter::PacketRouter()
    : last_send_module_(nullptr), active_remb_module_(nullptr) {}
...
void PacketRouter::RegisterNotifyBweCallback(
  ...
  RTC_DCHECK_RUN_ON(&thread_checker_);
  generate_transport_sequence_number_ = std::move(callback);
}
...
void PacketRouter::SendPacket(std::unique_ptr<RtpPacketToSend> packet,
  ...
  if (generate_transport_sequence_number_) {
    absl::optional<uint> transport_sequence_number =
        generate_transport_sequence_number_(*packet, cluster_info);
    if (transport_sequence_number) {
  ...
  if (!rtp_module->TrySendPacket(std::move(packet), cluster_info)) {
    RTC_DCHECK_NOTREACHED() << "Failed to send packet, rejected by RTP module.";
    return;
  }
