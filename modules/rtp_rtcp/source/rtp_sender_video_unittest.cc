...
TEST_F(RtpSenderVideoTest, KeyFrameHasCVO) {
  ...
  rtp_sender_video_->SendVideo(
      ...
      sizeof(kFrame), hdr, kDefaultExpectedRetransmissionTime, /*csrcs=*/{},
...
TEST_F(RtpSenderVideoTest, SendGenericVideo) {
  ...
  ASSERT_TRUE(rtp_sender_video_->SendVideo(
      ...
      sizeof(kPayload), video_header, TimeDelta::PlusInfinity(), /*csrcs=*/{},
      /*externally_encoded=*/false));
...
TEST_F(RtpSenderVideoTest, HasSentExternallyEncodedMediaGetsUpdated) {
  EXPECT_FALSE(rtp_sender_video_->HasSentExternallyEncodedMedia());

  uint8_t kFrame[kMaxPacketLength];
  rtp_sender_video_->SendVideo(
      kPayload, kType, kTimestamp, fake_clock_.CurrentTime(), kFrame,
      sizeof(kFrame), RTPVideoHeader(), kDefaultExpectedRetransmissionTime,
      /*csrcs=*/{}, /*externally_encoded=*/true);

  EXPECT_TRUE(rtp_sender_video_->HasSentExternallyEncodedMedia());
}
