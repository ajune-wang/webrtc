...
TEST_F(SdpOfferAnswerTest, Woes) {
  auto pc = CreatePeerConnection();
  ...
  EXPECT_TRUE(pc->SetRemoteDescription(std::move(offer)));
  auto first_transceiver = pc->pc()->GetTransceivers()[0];
  RTC_LOG(LS_ERROR) << "DIRECTION " << first_transceiver->direction();
  ...
  answer->ToString(&sdp);
  RTC_LOG(LS_ERROR) << "Answer: " << first_ssrc << "\n" << sdp;
  EXPECT_TRUE(pc->SetLocalDescription(std::move(answer)));
  ...
  reoffer->ToString(&sdp);
  RTC_LOG(LS_ERROR) << "Reoffer: " << second_ssrc << "\n" << sdp;
