...
class VideoMediaChannelTest : public testing::Test,
 ...
 protected:
  ...
  bool SendFrame() {
    if (video_capturer_2_) {
      video_capturer_2_->CaptureSingleFrame();
    }
    return video_capturer_.get() && video_capturer_->CaptureSingleFrame();
  }
  ...
  bool SendCustomVideoFrame(int w, int h) {
    ...
    return video_capturer_->CaptureSingleCustomFrame(w, h,
                                                     cricket::FOURCC_I420);
  }
  int NumRtpBytes() {
  ...
  void GetStatsMultipleSendStreams() {
    ...
    EXPECT_TRUE(capturer->CaptureSingleCustomFrame(kTestWidth, kTestHeight,
                                                   cricket::FOURCC_I420));
  ...
  void AddRemoveCapturer() {
    ...
    EXPECT_TRUE(capturer->CaptureSingleCustomFrame(format.width, format.height,
                                                   FOURCC_I420));
    ...
    for (int iterations = 0; iterations < 2; ++iterations) {
      ...
      EXPECT_TRUE(capturer->CaptureSingleCustomFrame(
          format.width, format.height, FOURCC_I420));
      ++captured_frames;
  ...
  void AddRemoveCapturerMultipleSources() {
    ...
    const int kTestWidth = 160;
    const int kTestHeight = 120;
    EXPECT_TRUE(capturer1->CaptureSingleCustomFrame(kTestWidth, kTestHeight,
                                                    cricket::FOURCC_I420));
    EXPECT_FRAME_ON_RENDERER_WAIT(
        renderer1, 1, kTestWidth, kTestHeight, kTimeout);
    // Capture a frame with additional capturer2, frames should be received
    EXPECT_TRUE(capturer2->CaptureSingleCustomFrame(kTestWidth, kTestHeight,
                                                    cricket::FOURCC_I420));
    EXPECT_FRAME_ON_RENDERER_WAIT(
        renderer2, 1, kTestWidth, kTestHeight, kTimeout);
