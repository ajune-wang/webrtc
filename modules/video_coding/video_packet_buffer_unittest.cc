...
TEST_F(TestPacketBuffer, HasHistoryOfUniqueFrames) {
  ...
  // Very old packets should also be ignored even if with unique timestamp.
  EXPECT_TRUE(
      Insert(seq_num, kKeyFrame, kFirst, kNotLast, 0, nullptr, timestamp + 5));
  ASSERT_EQ(kNumFrames, packet_buffer_->GetUniqueFramesSeen());

  // Yet new packets within history span should be counted.
  EXPECT_TRUE(
      Insert(seq_num + kNumFrames - kRequiredHistoryLength + 1, kKeyFrame,
             kFirst, kNotLast, 0, nullptr,
             timestamp + 10 * (kNumFrames - kRequiredHistoryLength) + 5));
