...
TimeDelta RTCPSender::ComputeTimeUntiNextReport(DataRate send_bitrate) {
  /*
      ...
      For video we use a configurable interval (default: 1 second) for a BW
          smaller than 200 kbit/s, technicaly we break the max 5% RTCP BW for
          video but that should be extremely rare
...
void RTCPSender::PrepareReport(const FeedbackState& feedback_state) {
  ...
  if (generate_report) {
    ...
    SetNextRtcpSendEvaluationDuration(
        ComputeTimeUntiNextReport(feedback_state.send_bitrate));
