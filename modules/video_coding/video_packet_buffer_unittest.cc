...
class TestPacketBufferH264 : public TestPacketBuffer,
 ...
 protected:
  ...
  explicit TestPacketBufferH264(bool require_sps_pps_for_h264_keyframe)
      : require_sps_pps_for_h264_keyframe_(require_sps_pps_for_h264_keyframe),
        scoped_field_trials_(require_sps_pps_for_h264_keyframe_
  ...
  bool InsertH264(uint16_t seq_num,           // packet sequence number
    ...
    if (keyframe == kKeyFrame) {
      if (require_sps_pps_for_h264_keyframe_) {
...
class TestPacketBufferH264IdrIsKeyframe : public TestPacketBufferH264 {
 protected:
  TestPacketBufferH264IdrIsKeyframe() : TestPacketBufferH264(false) {}
};
...
TEST_F(TestPacketBufferH264IdrIsKeyframe, IdrIsKeyframe) {
  ...
  EXPECT_EQ(kVideoFrameKey, frames_from_callback_[1]->frame_type());
}
...
TEST_F(TestPacketBufferH264IdrIsKeyframe, SpsPpsIdrIsKeyframe) {
  ...
  packet.video_header.codecHeader.H264.nalus[0].type = H264::NaluType::kSps;
  packet.video_header.codecHeader.H264.nalus[1].type = H264::NaluType::kPps;
  packet.video_header.codecHeader.H264.nalus[2].type = H264::NaluType::kIdr;
  packet.video_header.codecHeader.H264.nalus_length = 3;
...
class TestPacketBufferH264SpsPpsIdrIsKeyframe : public TestPacketBufferH264 {
 protected:
  TestPacketBufferH264SpsPpsIdrIsKeyframe() : TestPacketBufferH264(true) {}
};
...
TEST_F(TestPacketBufferH264SpsPpsIdrIsKeyframe, IdrIsNotKeyframe) {
  ...
  ASSERT_EQ(1u, frames_from_callback_.size());
  EXPECT_EQ(kVideoFrameDelta, frames_from_callback_[1]->frame_type());
}
...
TEST_F(TestPacketBufferH264SpsPpsIdrIsKeyframe, SpsPpsIsNotKeyframe) {
  ...
  packet.video_header.codecHeader.H264.nalus[0].type = H264::NaluType::kSps;
  packet.video_header.codecHeader.H264.nalus[1].type = H264::NaluType::kPps;
  packet.video_header.codecHeader.H264.nalus_length = 2;
