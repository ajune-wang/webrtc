...
void RtpVideoStreamReceiver2::ReceivePacket(const RtpPacketReceived& packet) {
  ...
  if (parse_and_insert(packet)) {
    ...
    std::vector<RtpPacketReceived> restashed_packets;
    for (const RtpPacketReceived& stashed_packet : stashed_packets_) {
      if (parse_and_insert(stashed_packet)) {
        restashed_packets.push_back(stashed_packet);
      }
    }
    stashed_packets_ = std::move(restashed_packets);
  }
}
