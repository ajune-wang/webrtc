 ...
 protected:
  ...
  void Test(const Result& expected) {
    ...
    if (connect_time < expected.connect_wait) {
      RTC_DLOG(LS_INFO) << "Connect time: " << connect_time << " ms";
    } else {
      RTC_DLOG(LS_INFO) << "Connect time: TIMEOUT (" << expected.connect_wait
                       << " ms)";
    }
    ...
    if (ep1_ch1()->selected_connection() && ep2_ch1()->selected_connection()) {
      ...
      if (converge_time < converge_wait) {
        RTC_DLOG(LS_INFO) << "Converge time: " << converge_time << " ms";
      } else {
        RTC_DLOG(LS_INFO) << "Converge time: TIMEOUT (" << converge_wait
                         << " ms)";
      }
    }
    ...
    switch (msg->message_id) {
      case MSG_ADD_CANDIDATES: {
        ...
        for (auto& c : data->candidates) {
          ...
          RTC_DLOG(LS_INFO) << "Candidate(" << data->channel->component() << "->"
      ...
      case MSG_REMOVE_CANDIDATES: {
        ...
        for (Candidate& c : data->candidates) {
          RTC_DLOG(LS_INFO) << "Removed remote candidate " << c.ToString();
...
#define P2P_TEST_DECLARATION(x, y, z)                            \
  TEST_F(P2PTransportChannelTest, z##Test##x##To##y) {           \
    ...
    if (kMatrix[x][y] != NULL)                                   \
      ...
      RTC_DLOG(LS_WARNING) << "Not yet implemented";              \
  }
...
TEST_F(P2PTransportChannelMultihomedTest, TestPingBackupConnectionRate) {
  ...
  RTC_DLOG(LS_INFO) << "Time elapsed: " << time_elapsed;
...
TEST_F(P2PTransportChannelMultihomedTest,
  ...
  RTC_DLOG(LS_INFO) << "Draining again...";
