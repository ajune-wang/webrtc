...
class IsacApiTest
 ...
 private:
  ...
  int GetInputFrameLength() const {
    return std::get<0>(GetParam()) / 100;  // 10 ms.
  }
  int GetEncoderFrameLenghtMs() const {
    int frame_size_ms = std::get<1>(GetParam());
    RTC_DCHECK(frame_size_ms == 30 || frame_size_ms == 60);
...
TEST_P(IsacApiTest, EncodeDecode) {
  ...
  for (int i = 0; i < kNumFrames; ++i) {
    ...
    EXPECT_EQ(parse_result.size(), static_cast<size_t>(1));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    IsacApiTest,
    ::testing::ValuesIn({
        std::make_tuple(16000, 30, IsacImpl::kFloat, IsacImpl::kFloat),
        std::make_tuple(16000, 30, IsacImpl::kFloat, IsacImpl::kFixed),
        std::make_tuple(16000, 30, IsacImpl::kFixed, IsacImpl::kFloat),
        std::make_tuple(16000, 30, IsacImpl::kFixed, IsacImpl::kFixed),
        std::make_tuple(16000, 60, IsacImpl::kFloat, IsacImpl::kFloat),
        std::make_tuple(16000, 60, IsacImpl::kFloat, IsacImpl::kFixed),
        std::make_tuple(16000, 60, IsacImpl::kFixed, IsacImpl::kFloat),
        std::make_tuple(16000, 60, IsacImpl::kFixed, IsacImpl::kFixed),
        std::make_tuple(32000, 30, IsacImpl::kFloat, IsacImpl::kFloat),
    }));
