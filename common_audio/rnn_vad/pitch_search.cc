...
const std::array<size_t, 16> kSubHarmonicMultipliers = {
...
const std::array<size_t, 16> kInitialPitchPeriodThresholds = {
    0,  0,  // The first two are never used.
...
PitchInfo CheckLowerPitchPeriodsAndComputePitchGain(
  ...
  const float g0 =
      pitch_gain(ComputeAutoCorrelationCoeff(pitch_buf, GetInvertedLag(t0),
                                             kPitchMaxPeriod),
                 yy_values[t0]);
...
PitchInfo PitchSearch(rtc::ArrayView<const float, kBufSize> pitch_buf,
  ...
  RTC_DCHECK_LT(pitch_inv_lag, kHalfPitchMaxPeriod);
  const auto pitch_period = kHalfPitchMaxPeriod - pitch_inv_lag;
