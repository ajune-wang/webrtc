...
void VideoStreamEncoder::Stop() {
  ...
  rtc::Event cleanup_processor_event;
  resource_adaptation_queue_.PostTask([this, &cleanup_processor_event] {
    RTC_DCHECK_RUN_ON(&resource_adaptation_queue_);
    if (resource_adaptation_processor_) {
      resource_adaptation_processor_->StopResourceAdaptation();
      resource_adaptation_processor_.reset();
    }
    cleanup_processor_event.Set();
  });
  cleanup_processor_event.Wait(rtc::Event::kForever);
}
...
void VideoStreamEncoder::SetSource(
  ...
  // Set the degradation preference and QP scaler on the encoder queue.
  // TODO(hbos): Do we really need to do this? Seems like a risk of race.
...
void VideoStreamEncoder::InjectAdaptationResource(
  ...
  resource_adaptation_queue_.PostTask([this, resource, reason] {
    RTC_DCHECK_RUN_ON(&resource_adaptation_queue_);
    // Another reason to use locks I guess?
