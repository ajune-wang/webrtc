...
VideoEncoder::EncoderInfo MakeEncoderInfo(
    std::vector<ResolutionAndBitrate> resolution_bitrate) {
  VideoEncoder::EncoderInfo encoder_info;
  size_t num_layers = resolution_bitrate.size();
  for (size_t layer_idx = 0; layer_idx < num_layers; ++layer_idx) {
    int frame_size_px = resolution_bitrate[layer_idx].width_px *
                        resolution_bitrate[layer_idx].height_px;

    VideoEncoder::ResolutionBitrateLimits bitrate_limits(
        frame_size_px, resolution_bitrate[layer_idx].bitrate_bps, 0,
...
VideoEncoder::EncoderInfo kEncoderInfo = MakeEncoderInfo(
    {{/*widht_px=*/640, /*height_px=*/360, /*bitrate_bps=*/500 * 1000},
     {/*widht_px=*/1280, /*height_px=*/720, /*bitrate_bps=*/1000 * 1000},
     {/*widht_px=*/1920, /*height_px=*/1080, /*bitrate_bps=*/2000 * 1000}});
...
TEST(BitrateConstraintTest,
  ...
  EncoderSettings encoder_settings(kEncoderInfo, std::move(encoder_config),
                                   video_codec);
...
TEST(
    ...
    IsAdaptationUpAllowedReturnsTrueAtSinglecastViaLayersDeactivationUpperLayerActiveIfBitrateIsEnough) {
  VideoCodec video_codec;
  VideoEncoderConfig encoder_config;
  FillCodecConfig(&video_codec, &encoder_config,
                  /*width_px=*/640, /*height_px=*/360,
