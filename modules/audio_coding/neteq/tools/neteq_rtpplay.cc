...
int main(int argc, char* argv[]) {
  ...
  config.plot_scripts_basename = CreateOptionalOutputFileName(
      ...
      output_audio_filename, "");
  if (config.textlog_filename)
    std::cout << "DBG " << *config.textlog_filename << std::endl;
  if (config.plot_scripts_basename)
    std::cout << "DBG " << *config.plot_scripts_basename << std::endl;
