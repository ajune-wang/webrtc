...
ActiveLayers GetActiveLayers(const VideoCodec& codec,
...
VideoBitrateAllocation SvcRateAllocator::Allocate(
  ...
  const ActiveLayers active_layers =
      GetActiveLayers(codec_, num_layers_.spatial);
...
DataRate SvcRateAllocator::GetMaxBitrate(const VideoCodec& codec) {
  ...
  const ActiveLayers active_layers = GetActiveLayers(codec, num_layers.spatial);
