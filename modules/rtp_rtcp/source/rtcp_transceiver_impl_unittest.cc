...
class MockMediaReceiverRtcpObserver
    : public webrtc::MediaReceiverRtcpObserver {
 public:
...
TEST(RtcpTransceiverImplTest, DoesntCallObserverAfterRemoved) {
  const uint32_t kRemoteSsrc = 12345;
  StrictMock<MockMediaReceiverRtcpObserver> observer1;
  StrictMock<MockMediaReceiverRtcpObserver> observer2;
  RtcpTransceiverImpl rtcp_transceiver(DefaultTestConfig());
...
TEST(RtcpTransceiverImplTest, ObserverOnSenderReportBySenderSsrc) {
...
TEST(RtcpTransceiverImplTest, CallObserverOnTargetBitrateBySenderSsrc) {
  const uint32_t kRemoteSsrc1 = 12345;
  const uint32_t kRemoteSsrc2 = 22345;
  StrictMock<MockMediaReceiverRtcpObserver> observer1;
  StrictMock<MockMediaReceiverRtcpObserver> observer2;
...
TEST(RtcpTransceiverImplTest, CallObserverOnByeBehindSenderReport) {
  const uint32_t kRemoteSsrc = 12345;
  MockMediaReceiverRtcpObserver observer;
  RtcpTransceiverImpl rtcp_transceiver(DefaultTestConfig());
  rtcp_transceiver.AddMediaReceiverObserver(kRemoteSsrc, &observer);
...
TEST(RtcpTransceiverImplTest, CallObserverOnByeBehindUnknownRtcpPacket) {
  const uint32_t kRemoteSsrc = 12345;
  MockMediaReceiverRtcpObserver observer;
  RtcpTransceiverImpl rtcp_transceiver(DefaultTestConfig());
  rtcp_transceiver.AddMediaReceiverObserver(kRemoteSsrc, &observer);
