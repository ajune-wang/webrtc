...
void VideoStreamEncoder::OnFrames(
  ...
  encoder_queue_.PostTask(
      [this, adapted_source_width, adapted_source_height, incoming_frame,
...
void VideoStreamEncoder::MaybeEncodeVideoFrame(
  ...
  if (!last_frame_info_ || adapted_source_width != last_frame_info_->width ||
      ...
      frame.is_texture() != last_frame_info_->is_texture) {
    pending_encoder_reconfiguration_ = true;
    last_frame_info_ = VideoFrameInfo(
        adapted_source_width, adapted_source_height, frame.is_texture());
  ...
  const VideoFrame& frame_to_encode = frame;
