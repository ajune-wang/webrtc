...
RemoteEstimatorProxy::RemoteEstimatorProxy(
    ...
    : clock_(clock),
      ...
      send_interval_ms_(send_config.default_interval.Get().ms()),
...
void RemoteEstimatorProxy::OnBitrateChanged(int bitrate_bps) {
  ...
  const double kMinTwccRate =
      kTwccReportSize * 8.0 * 1000.0 / send_config.max_interval.Get().ms();
  const double kMaxTwccRate =
      kTwccReportSize * 8.0 * 1000.0 / send_config.min_interval.Get().ms();
  ...
  if (send_periodic_feedback_) {
    if (periodic_window_start_seq_ &&
      ...
      for (auto it = packet_arrival_times_.begin();
           ...
           arrival_time - it->second >= send_config.back_window.Get().ms();) {
