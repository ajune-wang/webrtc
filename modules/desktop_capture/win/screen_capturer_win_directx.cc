...
void ScreenCapturerWinDirectx::CaptureFrame() {
  ...
  if (result != DuplicateResult::SUCCEEDED) {
    NLOG(LS_ERROR,
         "DxgiDuplicatorController failed to capture desktop, "
         "error code ",
  ...
  switch (result) {
    ...
    case DuplicateResult::FRAME_PREPARE_FAILED: {
      NLOG(LS_ERROR, "Failed to allocate a new DesktopFrame.");
    ...
    case DuplicateResult::INVALID_MONITOR_ID: {
      NLOG(LS_ERROR, "Invalid monitor id ", current_screen_id_);
      callback_->OnCaptureResult(Result::ERROR_PERMANENT, nullptr);
      break;
    }
