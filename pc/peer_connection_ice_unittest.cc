...
TEST_P(PeerConnectionIceTest, IceForking) {
  auto ice_gatherer = new rtc::RefCountedObject<cricket::IceGatherer>(
  ...
  pc1_internal->UseSharedIceGatherer(ice_gatherer);
  ...
  pc2_internal->UseSharedIceGatherer(ice_gatherer);
  ...
  EXPECT_EQ(ice_gatherer->port_allocator_session(),
            pc1_ice->shared_gatherer()->port_allocator_session());
  EXPECT_EQ(ice_gatherer->port_allocator_session(),
            pc2_ice->shared_gatherer()->port_allocator_session());
  EXPECT_TRUE(ice_gatherer->port_allocator_session()->IsGettingPorts());
}
