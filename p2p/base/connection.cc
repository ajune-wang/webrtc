...
void ConnectionRequest::Prepare(StunMessage* request) {
  ...
  if (connection_->IsStunBindingUnchanged(request)) {
...
void Connection::SendPingResponse(const StunMessage* request) {
  ...
  response.SetTransactionID(request->transaction_id());
  response.AddMessageIntegrity32(remote_candidate_.password());
  ...
  RTC_LOG_V(sev) << ToString()
                 << ": Sent STUN PING response, to=" << addr.ToSensitiveString()
                 << ", id=" << rtc::hex_encode(response.transaction_id());
...
bool Connection::IsStunBindingUnchanged(const StunMessage* message) {
