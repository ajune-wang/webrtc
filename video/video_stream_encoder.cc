...
VideoStreamEncoder::VideoStreamEncoder(
  ...
  resource_adaptation_queue_.PostTask([this, &initialize_processor_event] {
    ...
    resource_adaptation_processor_->AddProcessorListener(
        &stream_resource_manager_);
    resource_adaptation_processor_->AddProcessorListener(this);
...
void VideoStreamEncoder::Stop() {
  ...
  resource_adaptation_queue_.PostTask([this,
    ...
    if (resource_adaptation_processor_) {
      ...
      resource_adaptation_processor_->RemoveProcessorListener(this);
      resource_adaptation_processor_->RemoveProcessorListener(
          &stream_resource_manager_);
...
void VideoStreamEncoder::AddProcessorListenerForTesting(
    ResourceAdaptationProcessorListener* processor_listener) {
  rtc::Event event;
  resource_adaptation_queue_.PostTask([this, processor_listener, &event] {
    RTC_DCHECK_RUN_ON(&resource_adaptation_queue_);
    RTC_DCHECK(resource_adaptation_processor_);
    resource_adaptation_processor_->AddProcessorListener(processor_listener);
    event.Set();
  });
  event.Wait(rtc::Event::kForever);
}

void VideoStreamEncoder::RemoveProcessorListenerForTesting(
    ResourceAdaptationProcessorListener* processor_listener) {
  rtc::Event event;
  resource_adaptation_queue_.PostTask([this, processor_listener, &event] {
    RTC_DCHECK_RUN_ON(&resource_adaptation_queue_);
    RTC_DCHECK(resource_adaptation_processor_);
    resource_adaptation_processor_->RemoveProcessorListener(processor_listener);
    event.Set();
  });
  event.Wait(rtc::Event::kForever);
}
