...
void CallTest::CreateFrameGeneratorCapturerWithDrift(Clock* clock,
  ...
  auto frame_generator_capturer = test::FrameGeneratorCapturer::Create(
      test::FrameGenerator::CreateSquareGenerator(width, height, absl::nullopt,
                                                  absl::nullopt),
      framerate * speed, clock, task_queue_factory_.get());
...
void CallTest::CreateFrameGeneratorCapturer(int framerate,
  ...
  auto frame_generator_capturer = test::FrameGeneratorCapturer::Create(
      test::FrameGenerator::CreateSquareGenerator(width, height, absl::nullopt,
                                                  absl::nullopt),
      framerate, clock_, task_queue_factory_.get());
  frame_generator_capturer_ = frame_generator_capturer.get();
  video_sources_.push_back(std::move(frame_generator_capturer));
  ConnectVideoSourcesToStreams();
}
