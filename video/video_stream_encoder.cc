...
void VideoStreamEncoder::MaybeEncodeVideoFrame(const VideoFrame& video_frame,
  ...
  if (IsResolutionScalingEnabled(degradation_preference_) && quality_scaler_ &&
      DropDueToSize(video_frame.size())) {
...
void VideoStreamEncoder::OnBitrateUpdated(uint32_t bitrate_bps,
  ...
  if (video_suspension_changed && !video_is_suspended && pending_frame_ &&
      // TODO(nisse): Should we also check
      // IsResolutionScalingEnabled(degradation_preference_) and
      // quality_scaler_?
