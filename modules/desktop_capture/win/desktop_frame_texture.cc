...
void DesktopFrameTexture::set_handle(HANDLE handle) {
  if (handle_) {
    CloseHandle(handle_);
  }
  handle_ = handle;
...
std::unique_ptr<DesktopFrameTexture> DesktopFrameTexture::Share() {
  ...
  result->MoveFrameInfoFrom(this);
  // result->d3d_device_ = d3d_device_;
  result->handle_ = handle_;
  return result;
}

TextureHandleWin::TextureHandleWin(HANDLE handle)
    : ScopedHandle(SharedMemory::Handle(handle)) {}

TextureHandleWin::~TextureHandleWin() {
  CloseHandle(HANDLE(handle_));
}

SharedMemory::Handle TextureHandleWin::Release() {
  webrtc::SharedMemory::Handle released = handle_;
  handle_ = webrtc::SharedMemory::kInvalidHandle;
  return released;
}

std::unique_ptr<TextureHandleWin> TextureHandleWin::Create(HANDLE handle) {
  return std::make_unique<TextureHandleWin>(new TextureHandleWin(handle));
}
