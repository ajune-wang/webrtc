...
EncodedImageCallback::Result QualityAnalyzingVideoEncoder::OnEncodedImage(
  ...
  bool discard = false;
  {
    ...
    if (required_spatial_index) {
      ...
      RTC_CHECK(mode_ != Mode::kNormal) << "Analyzing encoder is in kNormal "
                                           "mode, but spatial layer/simulcast "
                                           "stream met.";
      if (mode_ == Mode::kSimulcast) {
        // In simulcast mode only encoded images with required spatial index are
        // interested, so all others have to be discarded.
        discard = *encoded_image.SpatialIndex() != *required_spatial_index;
      } else if (mode_ == Mode::kSVP) {
        // In SVP mode encoded images with spatial indexes that are equal or
