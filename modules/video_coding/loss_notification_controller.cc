...
void LossNotificationController::OnReceivedPacket(
  ...
  if (first_packet_in_frame) {
    ...
    if (last_received_unwrapped_frame_id_ &&
        frame_id <= *last_received_unwrapped_frame_id_) {
      RTC_LOG(LS_WARNING) << "Repeated or reordered frame ID (" << frame_id
                          << ").";
      return;
    }

    last_received_unwrapped_frame_id_ = frame_id;

    const bool intra_frame = frame_dependencies.empty();
    ...
    if (key_frame) {
      ...
      const bool all_dependencies_decodable =
          AllDependenciesDecodable(frame_id, frame_dependencies);
...
void LossNotificationController::OnAssembledFrame(
  ...
  const auto it = decodable_unwrapped_frame_ids_.insert(frame_id);
...
bool LossNotificationController::AllDependenciesDecodable(
  ...
  for (int64_t unwrapped_ref_frame_id : frame_dependencies) {
