...
class PeerConnectionSimulcastMetricsTests
 ...
 protected:
  ...
  void ValidateLocalDescriptionSimulcastApiMetrics(
      const std::map<int, int>& expected) {
    ValidateSimulcastApiMetrics(
        "WebRTC.PeerConnection.Simulcast.ApplyLocalDescription", expected);
  }
  void ValidateRemoteDescriptionSimulcastApiMetrics(
      const std::map<int, int>& expected) {
    ValidateSimulcastApiMetrics(
        "WebRTC.PeerConnection.Simulcast.ApplyRemoteDescription", expected);
  }

 private:
  // Validates that the simulcast API metrics have the provided values.
  void ValidateSimulcastApiMetrics(const std::string& metric,
                                   const std::map<int, int>& expected) {
    int total = 0;
    for (const auto& pair : expected) {
      EXPECT_EQ(pair.second, metrics::NumEvents(metric, pair.first))
          << metric << " value: " << pair.first;
      total += pair.second;
    }
    EXPECT_EQ(total, metrics::NumSamples(metric));
  }
};
...
TEST_F(PeerConnectionSimulcastMetricsTests, NoSimulcastUsageIsLogged) {
  ...
  ValidateLocalDescriptionSimulcastApiMetrics({{kSimulcastApiVersionNone, 2}});
  ValidateRemoteDescriptionSimulcastApiMetrics({{kSimulcastApiVersionNone, 2}});
}
...
TEST_F(PeerConnectionSimulcastMetricsTests, SpecComplianceIsLogged) {
  ...
  expected[kSimulcastApiVersionNone] = 1;
  expected[kSimulcastApiVersionSpecCompliant] = 1;
  ValidateLocalDescriptionSimulcastApiMetrics(expected);
  ValidateRemoteDescriptionSimulcastApiMetrics(expected);
}
...
TEST_F(PeerConnectionSimulcastMetricsTests, IncomingSimulcastIsLogged) {
  ...
  {
    SCOPED_TRACE("");
    ValidateLocalDescriptionSimulcastApiMetrics(
        {{kSimulcastApiVersionSpecCompliant, 1}});
  }
  ...
  std::string error;
  EXPECT_TRUE(remote->SetRemoteDescription(std::move(offer), &error)) << error;
  ValidateRemoteDescriptionSimulcastApiMetrics(
      {{kSimulcastApiVersionSpecCompliant, 1}});
...
TEST_F(PeerConnectionSimulcastMetricsTests, RejectedSimulcastIsLogged) {
  ...
  auto layers = CreateLayers({"1", "2", "3"}, true);
  AddTransceiver(local.get(), layers);
  std::map<int, int> expected_set_local, expected_set_remote;
  auto offer = local->CreateOfferAndSetAsLocal();
  {
    SCOPED_TRACE("create offer");
    expected_set_local[kSimulcastApiVersionSpecCompliant] = 1;
    ValidateLocalDescriptionSimulcastApiMetrics(expected_set_local);
  }
  RemoveSimulcast(offer.get());
  std::string error;
  EXPECT_TRUE(remote->SetRemoteDescription(std::move(offer), &error)) << error;
  {
    SCOPED_TRACE("remote peer connection");
    expected_set_remote[kSimulcastApiVersionNone] = 1;
    ValidateRemoteDescriptionSimulcastApiMetrics(expected_set_remote);
  }
  auto answer = remote->CreateAnswerAndSetAsLocal();
  {
    SCOPED_TRACE("create answer");
    expected_set_local[kSimulcastApiVersionNone] = 1;
    ValidateLocalDescriptionSimulcastApiMetrics(expected_set_local);
  }
  EXPECT_TRUE(local->SetRemoteDescription(std::move(answer), &error)) << error;
  {
    SCOPED_TRACE("local peer connection");
    expected_set_remote[kSimulcastApiVersionNone] = 2;
    ValidateRemoteDescriptionSimulcastApiMetrics(expected_set_remote);
  }
}
...
TEST_F(PeerConnectionSimulcastMetricsTests, LegacySimulcastIsLogged) {
  ...
  {
    SCOPED_TRACE("create offer");
    expected_set_local[kSimulcastApiVersionLegacy] = 1;
    ValidateLocalDescriptionSimulcastApiMetrics(expected_set_local);
  }
  EXPECT_TRUE(remote->SetRemoteDescription(std::move(offer), &error)) << error;
  {
