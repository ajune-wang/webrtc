...
class AudioDeviceTemplateAndroid : public AudioDeviceModule {
 public:
  ...
  AudioDeviceTemplateAndroid(AudioDeviceModule::AudioLayer audio_layer,
                      std::unique_ptr<AudioManager> audio_manager)
      : audio_layer_(audio_layer),
        audio_manager_(std::move(audio_manager)),
        output_(audio_manager_.get()),
        input_(audio_manager_.get()),
        initialized_(false) {
  ...
  int32_t Init() override {
    ...
    RTC_LOG(INFO) << "PHEN ADTA Init succeeded!";
    if (initialized_) {
      RTC_LOG(INFO) << "PHEN ADTA Init was already initialized";
      return 0;
    }
    RTC_LOG(INFO) << "PHEN ADTA Init manager_nullptr?: ";
    InitStatus status;
    if (!audio_manager_->Init()) {
      RTC_LOG(INFO) << "PHEN ADTA Init OTHER_ERROR";
      status = InitStatus::OTHER_ERROR;
    } else if (output_.Init() != 0) {
      RTC_LOG(INFO) << "PHEN ADTA Init PLAYOUT_ERROR";
      audio_manager_->Close();
      status = InitStatus::PLAYOUT_ERROR;
    } else if (input_.Init() != 0) {
      RTC_LOG(INFO) << "PHEN ADTA Init RECORDING_ERROR";
      output_.Terminate();
      audio_manager_->Close();
      status = InitStatus::RECORDING_ERROR;
    } else {
      RTC_LOG(INFO) << "PHEN ADTA Init OK";
      initialized_ = true;
      status = InitStatus::OK;
    }
    RTC_LOG(INFO) << "PHEN ADTA Init status = " << static_cast<int>(status);
    RTC_HISTOGRAM_ENUMERATION(
        "WebRTC.Audio.InitializationResult", static_cast<int>(status),
        static_cast<int>(InitStatus::NUM_STATUSES));
    if (status != InitStatus::OK) {
      RTC_LOG(LS_ERROR) << "PHEN Audio device initialization failed.";
      return -1;
    }
    RTC_LOG(INFO) << "PHEN ADTA Init returned 0";
    return 0;
  }
  ...
  int32_t SpeakerVolumeIsAvailable(bool* available) override {
    ...
    if (output_.SpeakerVolumeIsAvailable(isAvailable) == -1) {
      return -1;
    }
  ...
  int32_t SpeakerVolume(uint32_t* volume) const override {
    ...
    CHECKinitialized_();
    uint32_t level = 0;
    if (output_.SpeakerVolume(level) == -1) {
      return -1;
    }
    *volume = level;
  ...
  int32_t MaxSpeakerVolume(uint32_t* maxVolume) const override {
    ...
    CHECKinitialized_();
    uint32_t maxVol = 0;
    if (output_.MaxSpeakerVolume(maxVol) == -1) {
      return -1;
    }
    *maxVolume = maxVol;
    return 0;
  ...
  int32_t MinSpeakerVolume(uint32_t* minVolume) const override {
    ...
    CHECKinitialized_();
    uint32_t minVol = 0;
    if (output_.MinSpeakerVolume(minVol) == -1) {
      return -1;
    }
    *minVolume = minVol;
    return 0;
  ...
  int32_t AttachAudioBuffer() {
    RTC_LOG(INFO) << __FUNCTION__;
    output_.AttachAudioBuffer(&audio_device_buffer_);
    input_.AttachAudioBuffer(&audio_device_buffer_);
    return 0;
  }
 ...
 private:
  ...
  // construction. The real object is owned by AudioDeviceModuleAndroid and the
  // life time is the same as that of the AudioDeviceModuleAndroid, hence there
  // is no risk of reading a NULL pointer at any time in this class.
  // ???? Not sure with the new implementtion.
