...
void SuppressionGain::UpdateGainIncrease(
  ...
  if (!linear_echo_estimate) {
    ...
    if (initial_state_change_counter_ > 0) {
      ...
      float one_minus_change_factor = 1.f - change_factor;

      max_inc = change_factor * p.initial.max_inc +
                one_minus_change_factor * p.normal.max_inc;
      max_dec = change_factor * p.initial.max_dec +
                one_minus_change_factor * p.normal.max_dec;
      rate_inc = change_factor * p.initial.rate_inc +
                 one_minus_change_factor * p.normal.rate_inc;
      rate_dec = change_factor * p.initial.rate_dec +
                 one_minus_change_factor * p.normal.rate_dec;
      min_inc = change_factor * p.initial.min_inc +
                one_minus_change_factor * p.normal.min_inc;
      min_dec = change_factor * p.initial.min_dec +
                one_minus_change_factor * p.normal.min_dec;
    } else {
