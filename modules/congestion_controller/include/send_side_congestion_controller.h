...
class SendSideCongestionController : public CallStatsObserver,
 ...
 private:
  ...
  // the proper handler. It also keep cached values for safe asynchronous use.
  ...
  const std::unique_ptr<EncodingRateController> encoding_rate_controller_;
  const std::unique_ptr<PacerController> pacer_controller_;
  const std::unique_ptr<ControlRouter> router_;
  ...
  // This gurantee is currently guranteed by using explicit reset in the
  // destructor of this class. It's is declared last to indicate that it's
  // lifetime is shorter than all other members.
