...
bool CheckRedParameters(
  ...
  // Additionally check the fmtp line. It should list opus as the primary
  // payload type
  auto redParameters = red_codec.params.find("");
  if (redParameters == red_codec.params.end()) {
    RTC_LOG(LS_WARNING) << "audio/RED missing fmtp parameters.";
    return false;
  }
  std::vector<std::string> redundant_payloads;
  rtc::split(redParameters->second, '/', &redundant_payloads);
  ...
  for (auto pt : redundant_payloads) {
    allMatch &= pt == rtc::ToString(send_codec_spec.payload_type);
  }
  if (!allMatch) {
    return false;
  }
