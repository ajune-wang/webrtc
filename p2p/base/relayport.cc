...
void RelayPort::AddExternalAddress(const ProtocolAddress& addr) {
  ...
  for (std::vector<ProtocolAddress>::iterator it = external_addr_.begin();
    ...
    if ((it->address == addr.address) && (it->proto == addr.proto)) {
      NLOG(INFO, "Redundant relay address: ", proto_name, " @ ",
...
void RelayConnection::OnSendPacket(const void* data, size_t size,
  ...
  if (sent <= 0) {
    NLOG(LS_VERBOSE, "OnSendPacket: failed sending to ", GetAddress(),
...
void RelayEntry::Connect() {
  ...
  if (!ra) {
    NLOG(LS_WARNING, "No more relay addresses left to try");
    return;
  }
  ...
  NLOG(LS_INFO, "Connecting to relay via ", ProtoToString(ra->proto), " @ ",
  ...
  if (ra->proto == PROTO_UDP) {
    ...
    NLOG(LS_WARNING, "Unknown protocol (", ra->proto, ")");
  }

  // If we failed to get a socket, move on to the next protocol.
  if (!socket) {
    NLOG(LS_WARNING, "Socket creation failed");
...
void RelayEntry::OnConnect(const rtc::SocketAddress& mapped_addr,
  ...
  NLOG(INFO, "Relay allocate succeeded: ", ProtoToString(proto), " @ ",
...
void RelayEntry::OnMessage(rtc::Message *pmsg) {
  ...
  if (current_connection_) {
    ...
    NLOG(LS_WARNING, "Relay ", ra->proto, " connection to ", ra->address,
         " timed out");
...
void RelayEntry::OnSocketConnect(rtc::AsyncPacketSocket* socket) {
  NLOG(INFO, "relay tcp connected to ",
...
void RelayEntry::OnReadPacket(
  ...
  if (current_connection_ == NULL || socket != current_connection_->socket()) {
    ...
    NLOG(WARNING, "Dropping packet: unknown address");
    return;
  }
  ...
  if (!port_->HasMagicCookie(data, size)) {
    if (locked_) {
      ...
      NLOG(WARNING, "Dropping packet: entry not locked");
    }
    return;
  }
  ...
  RelayMessage msg;
  if (!msg.Read(&buf)) {
    NLOG(INFO, "Incoming packet was not STUN");
    return;
  }
  ...
  if (current_connection_->CheckResponse(&msg)) {
    ...
    NLOG(INFO, "Received BAD stun type from server: ", msg.type());
    return;
  }
  ...
  if (!addr_attr) {
    NLOG(INFO, "Data indication has no source address");
    return;
  } else if (addr_attr->family() != 1) {
    NLOG(INFO, "Source address has bad family");
    return;
  }
  ...
  if (!data_attr) {
    NLOG(INFO, "Data indication has no data");
    return;
  }
...
void AllocateRequest::OnResponse(StunMessage* response) {
  ...
  if (!addr_attr) {
    NLOG(INFO, "Allocate response missing mapped address.");
  } else if (addr_attr->family() != 1) {
    NLOG(INFO, "Mapped address has bad family");
  } else {
...
void AllocateRequest::OnErrorResponse(StunMessage* response) {
  ...
  if (!attr) {
    NLOG(LS_ERROR, "Missing allocate response error code.");
  } else {
    NLOG(INFO, "Allocate error response:", " code=", attr->code(), " reason='",
         attr->reason(), "'");
  }
...
void AllocateRequest::OnTimeout() {
  NLOG(INFO, "Allocate request timed out");
