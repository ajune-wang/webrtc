...
class DefaultTemporalLayers final : public Vp8FrameBufferController {
 public:
  ...
  // must be called once for each NextFrameConfig() call (using the RTP
  // timestamp as ID), and in order.
 ...
 private:
  ...
  static std::array<bool, kNumReferenceBuffers> DetermineStaticBuffers(
  ...
  const std::array<bool, kNumReferenceBuffers> is_static_buffer_;
