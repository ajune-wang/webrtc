...
class VideoStreamEncoderTest : public ::testing::Test {
 public:
  ...
  bool WaitForEncodedFrame(int64_t expected_ntp_time) {
    auto val = sink_.WaitForEncodedFrame(expected_ntp_time);
  ...
  bool ExpectDroppedFrame() {
    bool val = sink_.ExpectDroppedFrame();
    AdvanceTime(TimeDelta::Seconds(1) / max_framerate_);
    return val;
  }
  ...
  class TestSink : public VideoStreamEncoder::EncoderSink {
   public:
    ...
    bool WaitForEncodedFrame(int64_t expected_ntp_time) {
      auto val = TimedWaitForEncodedFrame(expected_ntp_time, kDefaultTimeout);
      EXPECT_TRUE(val);
      return val;
    }
    ...
    bool ExpectDroppedFrame() {
      auto val = WaitForFrame(TimeDelta::Millis(100));
      EXPECT_FALSE(val);
      return !val;
    }
...
TEST_P(VideoStreamEncoderInitialFrameDropperTest,
  ...
  // Frame should not be dropped.
  ASSERT_TRUE(WaitForEncodedFrame(1));
  ...
  ASSERT_TRUE(WaitForEncodedFrame(2));
  ...
  ASSERT_TRUE(ExpectDroppedFrame());
