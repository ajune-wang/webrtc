...
void DefaultVideoQualityAnalyzer::OnFramePreEncode(
  ...
  RTC_CHECK_EQ(state_, State::kActive)
      << "DefaultVideoQualityAnalyzer has to be started before use";
  RTC_DCHECK(frame.id().has_value());
  uint16_t frame_id = frame.id().value();
...
void DefaultVideoQualityAnalyzer::OnFrameEncoded(
  ...
  auto it = captured_frames_in_flight_.find(frame_id.value());
  if (it == captured_frames_in_flight_.end()) {
    RTC_LOG(LS_WARNING)
        << "The encoding of video frame with id [" << *frame_id
        << "] for peer [" << peer_name
        << "] finished after all receivers rendered this frame or "
  ...
  used_encoder.first_frame_id = frame_id.value();
  used_encoder.last_frame_id = frame_id.value();
...
void DefaultVideoQualityAnalyzer::OnFrameDecoded(
  ...
  auto it = captured_frames_in_flight_.find(frame.id().value());
  if (it == captured_frames_in_flight_.end() ||
  ...
  used_decoder.first_frame_id = frame.id().value();
  used_decoder.last_frame_id = frame.id().value();
...
void DefaultVideoQualityAnalyzer::OnFrameRendered(
  ...
  auto frame_it = captured_frames_in_flight_.find(frame.id().value());
  if (frame_it == captured_frames_in_flight_.end() ||
    ...
    RTC_LOG(LS_WARNING)
        ...
        << frame.id().value() << " which was " << reason << " before";
    return;
  }
  ...
  std::optional<VideoFrame> captured_frame =
      frames_storage_.Get(frame.id().value());
  ...
  int dropped_count = ProcessNotSeenFramesBeforeRendered(
      peer_index, frame.id().value(), stats_key, *state);
...
void DefaultVideoQualityAnalyzer::OnEncoderError(
    ...
    const webrtc::VideoFrame& frame,
    int32_t error_code) {
  RTC_LOG(LS_ERROR) << "Encoder error for frame.id=" << frame.id().value_or(-1)
                    << ", code=" << error_code;
}
...
void DefaultVideoQualityAnalyzer::OnDecoderError(
  ...
  StreamCodecInfo used_decoder;
  used_decoder.codec_name = stats.decoder_name;
  used_decoder.first_frame_id = frame_id.value();
  used_decoder.last_frame_id = frame_id.value();
  used_decoder.switched_on_at = now;
  used_decoder.switched_from_at = now;
