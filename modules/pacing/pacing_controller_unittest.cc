...
TEST_F(PacingControllerTest, ProbingWithPacketExtendedSleep) {
  const size_t kPacketSize = 1200;
  const int kInitialBitrateBps = 300000;
  uint32_t ssrc = 12346;
  uint16_t sequence_number = 1234;
  ...
  EXPECT_EQ(TimeUntilNextProcess(), TimeDelta::Zero());

  // Advance to one ms past where we should be.
  clock_.AdvanceTime(TimeDelta::ms(1));
  // Still indicate "process immediately".
  EXPECT_EQ(TimeUntilNextProcess(), TimeDelta::Zero());

  // We've gone too far, the probe is considered failed, and the default 5ms
  // polling distance will be reported instead (minus the 1001us we already
  // added).
  clock_.AdvanceTime(TimeDelta::us(1));
  EXPECT_EQ(TimeUntilNextProcess(), TimeDelta::us(3999));
}
