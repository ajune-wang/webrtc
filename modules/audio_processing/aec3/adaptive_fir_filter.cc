  ...
  for (size_t p = 0; p < num_partitions; ++p) {
    ...
    for (size_t ch = 0; ch < num_render_channels; ++ch) {
      for (size_t j = 0; j < kFftLengthBy2; j += 4) {
        ...
        float32x4_t H2_new = vmlaq_f32(H2_new, im, im);
        float32x4_t H2_p_j = vld1q_f32(&(*H2)[p][j]);
        float32x4_t H2_p_j = vmaxq_f32(H2_p_j, H2_new);
        vst1q_f32(&(*H2)[p][j], H2_p_j);
      }
      float H2_new = H[p].re[kFftLengthBy2] * H[p].re[kFftLengthBy2] +
                     H[p].im[kFftLengthBy2] * H[p].im[kFftLengthBy2];
...
AdaptiveFirFilter::AdaptiveFirFilter(size_t max_size_partitions,
  ...
  static_cast<void>(num_render_channels_);
}
...
void AdaptiveFirFilter::ComputeFrequencyResponse(
  ...
  RTC_DCHECK_GE(max_size_partitions_, H2->size());
...
void AdaptiveFirFilter::ConstrainAndUpdateImpulseResponse(
  ...
  for (size_t ch = 0; ch < num_render_channels_; ++ch) {
    ...
    for (size_t k = 0, j = partition_to_constrain_ * kFftLengthBy2;
         k < kFftLengthBy2; ++k, ++j) {
      (*impulse_response)[j] = std::max((*impulse_response)[j], h[k]);
    }
...
void AdaptiveFirFilter::SetFilter(const std::vector<std::vector<FftData>>& H) {
