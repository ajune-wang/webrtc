...
CreateRTCRemoteOutboundRTPAudioStreamStats(
    const cricket::VoiceReceiverInfo& voice_receiver_info,
    const std::string& mid,
  ...
  RTC_DCHECK_GT(voice_receiver_info.num_remote_sender_reports, 0);
  ...
  stats->packets_sent = 0;  // TODO(crbug.com/webrrtc/12529): Read from RTCP SR.
  stats->bytes_sent = 0;    // TODO(crbug.com/webrrtc/12529): Read from RTCP SR.
  ...
  stats->reports_sent = voice_receiver_info.num_remote_sender_reports;
...
void RTCStatsCollector::ProduceAudioRTPStreamStats_n(
  ...
  for (const cricket::VoiceReceiverInfo& voice_receiver_info :
    ...
    // Remote-outbound.
    auto remote_outbound_audio = CreateRTCRemoteOutboundRTPAudioStreamStats(
        voice_receiver_info, mid, inbound_audio_id, transport_id);
