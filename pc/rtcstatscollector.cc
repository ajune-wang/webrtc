...
rtc::scoped_refptr<RTCStatsReport> FilterBySelector(
    rtc::scoped_refptr<const RTCStatsReport> report,
    rtc::scoped_refptr<RtpSenderInternal> sender_selector,
    rtc::scoped_refptr<RtpReceiverInternal> receiver_selector) {
  RTC_DCHECK(sender_selector || receiver_selector);
  ...
  if (sender_selector) {
    ...
    // Because we do not implement sender stats, we look at outbound-rtp(s) that
    // reference the track attachment stats for the sender instead.
...
RTCStatsCollector::RequestInfo::RequestInfo(
    rtc::scoped_refptr<RTCStatsCollectorCallback> callback_arg,
    rtc::scoped_refptr<RtpSenderInternal> sender_selector_arg,
    rtc::scoped_refptr<RtpReceiverInternal> receiver_selector_arg)
    : callback(std::move(callback_arg)),
      sender_selector(std::move(sender_selector_arg)),
      receiver_selector(std::move(receiver_selector_arg)) {
  RTC_DCHECK(callback);
  RTC_DCHECK(!sender_selector || !receiver_selector);
}
...
void RTCStatsCollector::GetStatsReport(
  ...
  GetStatsReportInternal(RequestInfo(std::move(callback), nullptr, nullptr));
}
...
void RTCStatsCollector::GetStatsReport(
  ...
  GetStatsReportInternal(
      RequestInfo(std::move(callback), std::move(selector), nullptr));
}
...
void RTCStatsCollector::GetStatsReport(
  ...
  GetStatsReportInternal(
      RequestInfo(std::move(callback), nullptr, std::move(selector)));
}
...
void RTCStatsCollector::DeliverCachedReport(
  ...
  for (const RequestInfo& request : requests) {
    if (!request.sender_selector && !request.receiver_selector) {
      request.callback->OnStatsDelivered(cached_report);
    } else {
      request.callback->OnStatsDelivered(FilterBySelector(
          cached_report, request.sender_selector, request.receiver_selector));
    }
  }
}
