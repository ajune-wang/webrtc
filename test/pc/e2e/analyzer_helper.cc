...
void AnalyzerHelper::AddTrackToStreamMapping(std::string track_id,
  ...
  track_to_stream_map_.insert(
      {std::move(track_id),
       AnalyzerHelper::StreamInfo{stream_label, stream_label}});
}
...
void AnalyzerHelper::AddTrackToStreamMapping(std::string track_id,
  ...
  track_to_stream_map_.insert(
      {std::move(track_id),
       AnalyzerHelper::StreamInfo{std::move(stream_label),
                                  std::move(sync_group_label)}});
}
...
const std::string& AnalyzerHelper::GetSyncGroupLabelFromTrackId(
    const std::string& track_id) const {
  return GetStreamInfoFromTrackId(track_id).sync_group_label;
}
