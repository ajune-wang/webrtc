...
NetworkControllerInterface::uptr GoogCCNetworkControllerFactory::Create() {
  return rtc::MakeUnique<GoogCCNetworkController>(event_log_);
}

GoogCCNetworkController::GoogCCNetworkController(RtcEventLog* event_log)
    : event_log_(event_log),
      probe_controller_(new ProbeController(&ProbeClusterConfigJunction)),
  ...
  using This = GoogCCNetworkController;
...
NetworkInformationReceivers GoogCCNetworkController::GetReceivers() {
  NetworkInformationReceivers ret;
  probe_controller_->NetworkAvailabilityReceiver.AssignReceiverTo(&ret);
...
NetworkControlProducers GoogCCNetworkController::GetProducers() {
...
TimeDelta GoogCCNetworkController::GetProcessInterval() {
...
bool GoogCCNetworkController::GetNetworkParameters(
  ...
  *estimated_bitrate_bps = std::max<int32_t>(
  ...
  BWE_TEST_LOGGING_PLOT(1, "Target_bitrate_kbps", cat_time.ms(),
...
void GoogCCNetworkController::OnRoundTripTimeReport(
