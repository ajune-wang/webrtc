...
bool DtlsEnabled(const PeerConnectionInterface::RTCConfiguration& configuration,
  ...
  if (configuration.enable_dtls_srtp)
    dtls_enabled = *(configuration.enable_dtls_srtp);
...
PeerConnection::PeerConnection(
  ...
  worker_thread()->Invoke<void>(RTC_FROM_HERE, [this] {
    ...
    call_safety_ = PendingTaskSafetyFlag::Create();
    if (!call_)
      call_safety_->SetNotAlive();
  });
}
...
RTCError PeerConnection::Initialize(
  ...
  // TODO: Return configuration instead?
...
PeerConnection::InitializeRtcpCallback() {
  RTC_DCHECK_RUN_ON(network_thread());
  return [this, flag = call_safety_](const rtc::CopyOnWriteBuffer& packet,
                                     int64_t packet_time_us) {
    RTC_DCHECK_RUN_ON(network_thread());
