...
void CallTest::CreateVideoSendConfig() {
  CreateVideoSendConfig(&video_send_config_, num_video_streams_, 0,
                        send_transport_.get());
}
...
void CallTest::CreateFrameGeneratorCapturerWithDrift(Clock* clock,
  ...
  ConnectVideoCapturersToStreams();
}
...
void CallTest::CreateVideoSendStream(const VideoEncoderConfig& encoder_config) {
  RTC_DCHECK(video_send_stream_ == nullptr);
  video_send_stream_ = sender_call_->CreateVideoSendStream(
      video_send_config_.Copy(), encoder_config.Copy());
}
...
void CallTest::ConnectVideoCapturersToStreams() {
...
void CallTest::SetVideoSource(
  ...
  if (source != nullptr) {
    source = frame_generator_capturer_.get();
  }
  if (!preference) {
