...
rtc::scoped_refptr<RtpSenderInterface> PeerConnection::AddTrack(
  ...
  if (stream_labels.empty()) {
    stream_labels.push_back(rtc::CreateRandomUuid());
  }
  auto sender_or_error = AddTrackInternal(track, stream_labels);
...
PeerConnection::AddTrackInternal(
  ...
  auto sender_or_error =
      (IsUnifiedPlan() ? AddTrackUnifiedPlan(track, stream_labels)
                       : AddTrackPlanB(track, stream_labels));
  if (sender_or_error.ok()) {
    observer_->OnRenegotiationNeeded();
  }
