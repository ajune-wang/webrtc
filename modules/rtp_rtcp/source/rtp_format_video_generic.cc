...
RtpPacketizerGeneric::RtpPacketizerGeneric(
    ...
    : picture_id_(rtp_video_header.frame_id != kNoPictureId
                      ? absl::make_optional<uint16_t>(
                            rtp_video_header.frame_id & 0x7FFF)
                      : absl::nullopt),
      use_extended_header_(picture_id_.has_value()),
      payload_data_(nullptr),
      payload_size_(0),
      max_payload_len_(max_payload_len - kGenericHeaderLength -
                       (use_extended_header_ ? kExtendedHeaderLength : 0)),
...
bool RtpDepacketizerGeneric::Parse(ParsedPayload* parsed_payload,
  ...
  if (generic_header & RtpFormatVideoGeneric::kExtendedHeaderBit) {
    if (payload_data_length < kExtendedHeaderLength) {
      RTC_LOG(LS_ERROR) << "Too short payload for generic header.";
      return false;
    }
