...
- (instancetype)initWithMediaSubtype:(FourCharCode)subtype
  ...
  self = [super init];
  if (self) {
    CMVideoFormatDescriptionCreate(nil, subtype, kFormatWidth, kFormatHeight, nil, &_format);
...
+ (instancetype)invalidMediaSubtypeFormat {
  AVCaptureDeviceFormatMock* instance =
      [[AVCaptureDeviceFormatMock alloc] initWithMediaSubtype:kCVPixelFormatType_420YpCbCr8Planar
                                                       minFps:0.0
                                                       maxFps:60.0];
  return instance;
}
...
TEST(AVFormatMapperTest, SuportedCricketFormatsWithInvalidFramerateFormats) {
  ...
  AVCaptureDeviceFormatMock* mock = [AVCaptureDeviceFormatMock invalidFpsFormat];
  OCMStub([mockDevice formats]).andReturn(@[ mock ]);

  // when
  std::set<cricket::VideoFormat> result = webrtc::GetSupportedVideoFormatsForDevice(mockDevice);
...
TEST(AVFormatMapperTest, SuportedCricketFormatsWithInvalidFormats) {
  ...
  AVCaptureDeviceFormatMock* mock = [AVCaptureDeviceFormatMock invalidMediaSubtypeFormat];
  OCMStub([mockDevice formats]).andReturn(@[ mock ]);
...
TEST(AVFormatMapperTest, MediaSubtypePreference) {
  ...
  OCMExpect([mockDevice setActiveVideoMinFrameDuration:CMTimeMake(1, kFramerate)]);

  // when
  bool resultFormat = webrtc::SetFormatForCaptureDevice(mockDevice, nil, expectedFormat);
...
TEST(AVFormatMapperTest, SetFormatWhenDeviceCannotLock) {
  // given
  id mockDevice = OCMClassMock([AVCaptureDevice class]);
  [[[mockDevice stub] andReturnValue:@(NO)] lockForConfiguration:[OCMArg setTo:nil]];
  [[[mockDevice stub] andReturn:@[]] formats];

  // when
  bool resultFormat = webrtc::SetFormatForCaptureDevice(mockDevice, nil, cricket::VideoFormat());
...
TEST(AVFormatMapperTest, SetFormatWhenFormatIsIncompatible) {
  ...
  OCMStub([mockDevice lockForConfiguration:[OCMArg setTo:nil]]).andReturn(YES);
  NSException* testException = [NSException exceptionWithName:@"Test exception"
