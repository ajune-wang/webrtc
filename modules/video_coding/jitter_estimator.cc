...
int VCMJitterEstimator::GetJitterEstimate(double rttMultiplier) {
  ...
  if (fps < kJitterScaleLowThreshold) {
    if (fps == 0.0) {
      int ret = rtc::checked_cast<int>(std::max(0.0, jitterMS) + 0.5);
      return jitter_upper_bound_ms_ ? std::min(*jitter_upper_bound_ms_, ret)
                                    : ret;
    }
    return 0;
  }
  ...
  int ret = rtc::checked_cast<int>(std::max(0.0, jitterMS) + 0.5);
  return jitter_upper_bound_ms_ ? std::min(*jitter_upper_bound_ms_, ret) : ret;
}
