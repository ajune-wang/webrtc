...
std::unique_ptr<test::RtpReplayer::StreamState> ConfigureFromFlags() {
  ...
  stream_state->receive_streams.emplace_back(
      stream_state->call->CreateVideoReceiveStream(std::move(receive_config)));
  return stream_state;
}

void RtpReplay() {
  if (flags::ConfigFile().empty()) {
    test::RtpReplayer::Replay(ConfigureFromFlags(), flags::InputFile());
  } else {
    auto stream_state =
        test::RtpReplayer::StreamState::Load(flags::ConfigFile());
    test::RtpReplayer::Replay(std::move(stream_state), flags::InputFile());
  }
}
