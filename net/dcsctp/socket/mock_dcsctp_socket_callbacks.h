...
class MockDcSctpSocketCallbacks : public DcSctpSocketCallbacks {
 public:
  MockDcSctpSocketCallbacks() : timeout_manager_([this]() { return now_; }) {
  ...
  uint32_t GetRandomInt(uint32_t low, uint32_t high) override { return 4; }
  ...
  void AdvanceTime(DurationMs duration_ms) {
    now_ = TimeMs(*now_ + *duration_ms);
  }
