...
bool ValidateEncodeParams(
  ...
  for (size_t i = 0; i < frame_settings.size(); ++i) {
    ...
    if (!settings.output_functions) {
      RTC_LOG(LS_ERROR) << "No output functions provided.";
      return false;
    }
...
void LibaomAv1Encoder::Encode(
  ...
  absl::Cleanup on_return = [&] {
    ...
    for (FrameEncodeSettings& settings : frame_settings) {
      if (settings.output_functions) {
        settings.output_functions->EncodeComplete(EncodingError());
      }
    }
  };
  ...
  for (int sid = frame_settings[0].spatial_id;
    ...
    while (const aom_codec_cx_pkt_t* pkt =
      ...
      if (pkt->kind == AOM_CODEC_CX_FRAME_PKT && pkt->data.frame.sz > 0) {
        ...
        output_buffer = settings.output_functions->GetBitstreamOutputBuffer(
    ...
    if (output_buffer.empty()) {
      ...
      RTC_CHECK(settings.output_functions);
      settings.output_functions->EncodeComplete(result);
      // To avoid invoking any callback more than once.
      settings.output_functions = nullptr;
    }
  }
}
}  // namespace
