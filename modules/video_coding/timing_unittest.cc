...
TEST(ReceiverTimingTest, MaxWaitingTimeIsZeroForZeroRenderTime) {
  ...
  constexpr int64_t kRenderTimeMs = 0;
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    EXPECT_EQ(timing.MaxWaitingTime(kRenderTimeMs, now_ms), 0);
  }
}
...
TEST(ReceiverTimingTest, MaxWaitingTimeZeroDelayPacingExperiment) {
  ...
  // trial setting.
  int64_t now_ms = clock.TimeInMilliseconds();
  EXPECT_EQ(timing.MaxWaitingTime(kRenderTimeMs, now_ms), kMinPacingMs);
  // If there's a burst of frames, the min pacing interval is summed.
  EXPECT_EQ(timing.MaxWaitingTime(kRenderTimeMs, now_ms), 2 * kMinPacingMs);
  EXPECT_EQ(timing.MaxWaitingTime(kRenderTimeMs, now_ms), 3 * kMinPacingMs);
  EXPECT_EQ(timing.MaxWaitingTime(kRenderTimeMs, now_ms), 4 * kMinPacingMs);
  // Allow a few ms to pass, this should be subtraced from the MaxWaitingTime.
  constexpr int64_t kTwoMs = 2;
  clock.AdvanceTimeMilliseconds(kTwoMs);
  now_ms = clock.TimeInMilliseconds();
  EXPECT_EQ(timing.MaxWaitingTime(kRenderTimeMs, now_ms),
