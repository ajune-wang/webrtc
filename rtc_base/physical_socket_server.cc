...
inline std::pair<PhysicalSocketServer::DispatcherSet::iterator, bool> PhysicalSocketServer::DispatcherSet::insert(Dispatcher* pdispatcher) {
...
inline PhysicalSocketServer::DispatcherSet::size_type PhysicalSocketServer::DispatcherSet::erase(Dispatcher* pdispatcher) {
  auto it = this->lower_bound(pdispatcher);
  if (it != this->end() && *it == pdispatcher) {
    disp_list_.erase(it);
    return 1u;
...
inline PhysicalSocketServer::DispatcherSet::iterator PhysicalSocketServer::DispatcherSet::find(Dispatcher* pdispatcher) {
  auto it = this->lower_bound(pdispatcher);
  if (it != this->end() && *it != pdispatcher)
    it = this->end();
  return it;
...
inline PhysicalSocketServer::DispatcherSet::iterator PhysicalSocketServer::DispatcherSet::lower_bound(Dispatcher* pdispatcher) {
  return std::lower_bound(disp_list_.begin(), disp_list_.end(), pdispatcher);
}

inline PhysicalSocketServer::DispatcherInfoSet::key_t PhysicalSocketServer::DispatcherInfoSet::insert(Dispatcher* pdispatcher) {
  auto it = this->find(pdispatcher);
  if (it != this->end())
    return it->key;

  it = std::find_if(infos_.begin(), infos_.end(), [](DispatcherInfo const& info) { return info.dispatcher == nullptr; });
  if (it == infos_.end()) {
    infos_.emplace_back();
    it = infos_.end();
    --it;
  ...
  key_t key = (static_cast< uint64_t >(last_id_) << 32) | static_cast< uint64_t >(static_cast< uint32_t >(it - infos_.begin()));
  last_id_ += 2u; // make sure the lowest bit is never set, which is a guarantee that key value of -1 is never valid
...
inline PhysicalSocketServer::DispatcherInfoSet::size_type PhysicalSocketServer::DispatcherInfoSet::erase(Dispatcher* pdispatcher) {
  auto it = this->find(pdispatcher);
  if (it != this->end()) {
    it->dispatcher = nullptr;
    pdispatcher->key_ = -1;
    // Remove any invalid entries from the tail
    auto it = std::find_if(infos_.rbegin(), infos_.rend(), [](DispatcherInfo const& info) { return info.dispatcher != nullptr; }).base();
...
inline Dispatcher* PhysicalSocketServer::DispatcherInfoSet::get_by_key(key_t key) const noexcept {
...
inline PhysicalSocketServer::DispatcherInfoSet::iterator PhysicalSocketServer::DispatcherInfoSet::find(Dispatcher* pdispatcher) noexcept {
...
inline PhysicalSocketServer::DispatcherInfoSet::const_iterator PhysicalSocketServer::DispatcherInfoSet::find(Dispatcher* pdispatcher) const noexcept {
...
static void ProcessEvents(Dispatcher* dispatcher,
  ...
  // Most often the socket is writable or readable or both, so make a single virtual call to get requested events
...
void PhysicalSocketServer::AddEpoll(Dispatcher* pdispatcher) {
  ...
  if (it == dispatchers_.end()) {
    // This should not happen since epoll wait doesn't use processing_dispatchers_, which means
    // that the dispatcher must always be added to dispatchers_ before this call.
    RTC_LOG(LS_ERROR) << "Attempt to add a dispatcher to epoll before adding to the list of dispatchers";
    return;
  }
