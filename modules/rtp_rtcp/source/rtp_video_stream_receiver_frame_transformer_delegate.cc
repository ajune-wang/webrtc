...
class TransformableVideoReceiverFrame
 ...
 public:
  ...
  void SetMetadata(const VideoFrameMetadata& metadata) override {
    VideoFrameMetadata duplicate_metadata = metadata;
    duplicate_metadata.SetFrameId(Metadata().GetFrameId());
    duplicate_metadata.SetFrameDependencies(Metadata().GetFrameDependencies());
