...
constexpr std::array<float, kFftNumCoeffs20ms48kHz> kExpectedFftCoeffsReal = {
    ...
    -0.0031f, 0.0135f};
constexpr std::array<float, kFftNumCoeffs20ms48kHz> kExpectedFftCoeffsImag = {
...
void CheckFftResult(rtc::ArrayView<const float> expected_real,
  ...
  for (size_t i = 0; i < computed.size(); ++i) {
    SCOPED_TRACE(i);
    EXPECT_NEAR(expected_real[i], computed[i].real() * scaling_factor, 1e-4f);
    EXPECT_NEAR(expected_imag[i], computed[i].imag() * scaling_factor, 1e-4f);
    break;
  }
}
...
TEST(RnnVad, ComputeForwardFftBitExactness) {
  // Read frame.
  auto samples_reader = CreatePcmSamplesReader(kFrameSize20ms48kHz);
  std::array<float, kFrameSize20ms48kHz> samples;
  samples_reader.first->SeekForward(9 * kFrameSize20ms48kHz);
  ...
  {
    rtc::ArrayView<float> frame1(samples.data(), kFrameSize10ms48kHz);
    rtc::ArrayView<float> frame2(samples.data() + kFrameSize10ms48kHz,
                                 kFrameSize10ms48kHz);
    ...
    ASSERT_NEAR(7.230698f, samples[kFrameSize10ms48kHz], 1e-6f);
  }
  // Init FFT.
