...
EncoderManager::EncoderManager(EncodedImageCallback* encoded_image_signal)
  ...
  encodings_[0].structure =
      std::make_unique<FrameDependenciesControllerSingleStream>();
}
...
int32_t EncoderManager::Encode(const VideoFrame& frame,
  ...
  for (size_t i = 0; i < frame_types->size(); ++i) {
    ...
    std::vector<GenericFrameInfo> frame_infos =
        encoding.structure->NextFrameConfig(is_keyframe);
    std::vector<FrameConfig> frame_configs;
    frame_configs.reserve(frame_infos.size());
    for (const GenericFrameInfo& info : frame_infos) {
      FrameConfig config;
      config.id = ++frame_id_;
      config.spatial_id = info.spatial_id;
      config.temporal_id = info.temporal_id;
      config.encoder_buffers.assign(info.encoder_buffers.begin(),
                                    info.encoder_buffers.end());
      frame_configs.push_back(std::move(config));
    }
    ...
    encoding.encoder->Encode(
        frame, frame_configs, [&](EncodedFrameLight encoded_frame) {
          ...
          // Typicaly frame_infos will not be more than 3, so linear search is
          // best, do not bother to optimize.
          auto it = absl::c_find_if(frame_configs, [&](const auto& info) {
            return info.id == encoded_frame.id;
          });
          RTC_CHECK(it != frame_configs.end());
          codec_specific_info.generic_frame_info =
              frame_infos[it - frame_configs.begin()];
