...
using webrtc::network::NetworkControllerWrapper;
...
bool BbrExperimentEnabled() {
  const char kBbrExperiment[] = "WebRTC-BbrExperiment";
  std::string experiment_string =
      webrtc::field_trial::FindFullName(kBbrExperiment);
  // The experiment is enabled iff the field trial string begins with "Enable".
  return experiment_string.find("Enable") == 0;
}
...
SendSideCongestionController::SendSideCongestionController(
  ...
  if (BbrExperimentEnabled())
    factory = std::make_shared<BbrNetworkControllerFactory>(clock_, event_log_);
  else
    factory =
        std::make_shared<GoccNetworkControllerFactory>(clock_, event_log_);
  controller_ = MakeUnique<NetworkControllerWrapper>(factory);

  network::signal::QueueTaskRunner* queue = nullptr;
  if (using_task_queue_) {
    task_queue_ = MakeUnique<rtc::TaskQueue>("SendSideCCQueue");
    safe_queue_ =
        MakeUnique<network::signal::QueueTaskRunner>(task_queue_.get());
    queue = safe_queue_.get();
  }
  ...
  NetworkRouteChangeJunction =
      network::NetworkRouteChange::CreateJunction(queue, lock);
  ProcessIntervalJunction =
      network::ProcessInterval::CreateJunction(queue, lock);
  RemoteBitrateReportJunction =
      network::RemoteBitrateReport::CreateJunction(queue, lock);
  RoundTripTimeReportJunction =
      network::RoundTripTimeReport::CreateJunction(queue, lock);
  SentPacketJunction = network::SentPacket::CreateJunction(queue, lock);
  StreamsConfigJunction = network::StreamsConfig::CreateJunction(queue, lock);
  TargetRateConstraintsJunction =
      network::TargetRateConstraints::CreateJunction(queue, lock);
  TransportLossReportJunction =
      network::TransportLossReport::CreateJunction(queue, lock);
  TransportPacketsFeedbackJunction =
      network::TransportPacketsFeedback::CreateJunction(queue, lock);

  OutstandingDataJunction =
      network::OutstandingData::CreateJunction(queue, lock);
  NetworkAvailabilityJunction =
      network::NetworkAvailability::CreateJunction(queue, lock);
  PacerQueueUpdateJunction =
      network::PacerQueueUpdate::CreateJunction(queue, lock);
  ...
  network::NetworkControlProducer* prods = controller_->GetProducers();
  ...
  network::NetworkInformationReceiver* recvs = controller_->GetReceivers();
...
void SendSideCongestionController::SetBweBitrates(int min_bitrate_bps,
  ...
  msg.max_data_rate = max_bitrate_bps > 0 ? DataRate::bps(max_bitrate_bps)
                                          : DataRate::Infinity();
  TargetRateConstraintsJunction->OnMessage(msg);
  WaitOnControllers();
}
