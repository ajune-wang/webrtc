...
size_t LossNotification::BlockLength() const {
  constexpr size_t kUniqueIdentifierSizeBytes = 4;
  constexpr size_t kLossNotificationBodySizeBytes = 8;
  return kHeaderLength + kCommonFeedbackLength + kUniqueIdentifierSizeBytes +
         kLossNotificationBodySizeBytes;
}
...
bool LossNotification::Parse(const CommonHeader& packet) {
  ...
  if (packet.payload_size_bytes() < BlockLength()) {
    RTC_LOG(LS_WARNING) << "Payload length (" << packet.payload_size_bytes()
                        << ") is too small for a Loss Notification packet.";
    return false;
  }
  ...
  if (ByteReader<uint32_t>::ReadBigEndian(&payload[8]) != kUniqueIdentifier) {
    RTC_LOG(LS_INFO) << "Loss Notification identifier not found.";
    return false;
  }
  ...
  last_received_ = (last_received_delta_and_decodability >> 1);
