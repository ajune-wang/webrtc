...
int FakeNetworkSocket::RecvFrom(void* pv,
  ...
  absl::optional<EmulatedIpPacket> packet = absl::nullopt;
  {
    rtc::CritScope crit(&lock_);
    ...
    packet = absl::make_optional(std::move(packet_queue_.front()));
    packet_queue_.pop_front();
  }
  *paddr = packet.value().from;
  size_t data_read = std::min(cb, packet.value().size());
  memcpy(pv, packet.value().data.data(), data_read);

  if (data_read < packet.value().data.size()) {
    // packet.value().data.erase(packet.value().data.begin(),
    // packet.value().data.begin() + data_read);
    {
      rtc::CritScope crit(&lock_);
      packet_queue_.push_front(std::move(packet.value()));
    }
    SignalReadEvent(this);
  }
