...
SendSideCongestionController::~SendSideCongestionController() {
  ...
  CongestionWindowReceiver.reset();
  TargetTransferRateReceiver.reset();
  PacerConfigReceiver.reset();
  ProbeClusterConfigReceiver.reset();

  CongestionInvalidationJunction.Disconnect(
      CongestionInvalidationReceiver.get());
  CongestionInvalidationReceiver.reset();

  NetworkInvalidationJunction.Disconnect(NetworkInvalidationReceiver.get());
  NetworkInvalidationReceiver.reset();

  PacerStateJunction.Disconnect(PacerStateReceiver.get());
  PacerStateReceiver.reset();
}
