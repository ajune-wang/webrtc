...
VideoBitrateAllocation SvcRateAllocator::GetAllocationScreenSharing(
  ...
  // Add leftover to layers from last to first, up to max bitrate.
  while (sl_idx > first_active_layer &&
         total_bitrate - allocated_rate > DataRate::Zero()) {
    DataRate old_rate =
        DataRate::bps(bitrate_allocation.GetBitrate(sl_idx - 1, 0));
    DataRate new_rate =
        std::min(old_rate + (total_bitrate - allocated_rate),
                 DataRate::kbps(codec_.spatialLayers[sl_idx - 1].maxBitrate));
    bitrate_allocation.SetBitrate(sl_idx - 1, 0, new_rate.bps());
    allocated_rate += new_rate - old_rate;
  }
