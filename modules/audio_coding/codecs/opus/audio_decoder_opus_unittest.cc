...
void EncodeSpeech(AudioEncoderOpusImpl& encoder,
...
void EncodeDecodeNoiseUntilDecoderInDtxMode(AudioEncoderOpusImpl& encoder,
  ...
  for (int i = 0; i < 50; ++i) {
    ...
    // Ignore speech payloads: they do not update the internal decode parameters
    // for comfort noise generation and they cannot be DTX packets.
    if (payload.size() == 0 || info.speech) {
      continue;
    }

    // Decode `payload`, which encodes a non-speech packet. If not a DTX packet,
    // decoding it will update the internal decoder parameters for comfort noise
    // generation.
    ...
    if (parse_results[0].frame->IsDtxPacket()) {
      // The decoder is now in DTX mode and further decode requests will
...
TEST(AudioDecoderOpusTest, MonoEncoderStereoDecoderOutputsNonTrivialStereoDtx) {
  ...
  EncodeSpeech(encoder, rtp_timestamp, /*max_frames=*/100);
  ...
  RTC_CHECK_EQ(speech_type, AudioDecoder::SpeechType::kComfortNoise);
  ...
  RTC_CHECK(!IsZeroedFrame(decoded_view));
