...
void FramerateController::SetMaxFramerate(int max_framerate) {
  max_framerate_ = max_framerate;
}

bool FramerateController::DropFrame(int64_t in_timestamp_ns) {
  if (max_framerate_ <= 0)
    return true;

  // If `max_framerate_` is not set (i.e. maxint), `frame_interval_ns` is
  // rounded to 0.
