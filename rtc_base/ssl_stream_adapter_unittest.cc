...
// Client has legacy ciphers disabled, server has DTLS 1.0 only.
...
TEST_F(SSLStreamAdapterTestDTLSLegacyProtocols,
  ...
  ConfigureServer("");
  SetupProtocolVersions(rtc::SSL_PROTOCOL_DTLS_12, rtc::SSL_PROTOCOL_DTLS_10);
  // Handshake ends up as connected, despite error messages.
  TestHandshake();
  // And protocol ends up as DTLS 1.2 despite max protocol version set to 1.0
  EXPECT_EQ(rtc::SSL_PROTOCOL_DTLS_12, GetSslVersion(true));
  EXPECT_EQ(rtc::SSL_PROTOCOL_DTLS_12, GetSslVersion(false));
}
