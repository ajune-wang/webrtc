...
void AdaptiveAgc::Analyze(AudioFrameView<const float> float_frame) {
  ...
  apm_data_dumper_->DumpRaw("agc2_vad_rms_dbfs", latest_vad_result_.speech_rms_dbfs);

  apm_data_dumper_->DumpRaw("agc2_vad_peak_dbfs", latest_vad_result_.speech_peak_dbfs);
  ...
  apm_data_dumper_->DumpRaw("agc2_noise_estimate_dbfs", latest_noise_level_dbfs_);
}

void AdaptiveAgc::Modify(AudioFrameView<float> float_frame) {
  // The gain applier applies the gain.
  gain_applier_.Process(latest_speech_level_dbfs_, latest_noise_level_dbfs_,
                        latest_vad_result_,
                        float_frame);
}
