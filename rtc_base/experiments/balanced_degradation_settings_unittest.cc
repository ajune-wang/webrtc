...
TEST(BalancedDegradationSettings, GetsDefaultConfigForVp8ZeroQpValue) {
  webrtc::test::ScopedFieldTrials field_trials(
      "WebRTC-Video-BalancedDegradationSettings/"
      "pixels:1000|2000|3000,fps:5|15|25,vp8_qp_high:0|95|100/");
  BalancedDegradationSettings settings;
  VerifyIsDefault(settings.GetConfigs());
}

TEST(BalancedDegradationSettings, GetsDefaultConfigForH264ZeroQpValue) {
  webrtc::test::ScopedFieldTrials field_trials(
      "WebRTC-Video-BalancedDegradationSettings/"
      "pixels:1000|2000|3000,fps:5|15|25,h264_qp_high:20|0|30/");
  BalancedDegradationSettings settings;
  VerifyIsDefault(settings.GetConfigs());
}

TEST(BalancedDegradationSettings, GetsDefaultConfigForGenericZeroQpValue) {
  webrtc::test::ScopedFieldTrials field_trials(
      "WebRTC-Video-BalancedDegradationSettings/"
...
TEST(BalancedDegradationSettings, GetsH264HighQpThreshold) {
  ...
  BalancedDegradationSettings settings;
  EXPECT_EQ(41, settings.QpHighThreshold(kVideoCodecH264, 1));
  EXPECT_EQ(41, settings.QpHighThreshold(kVideoCodecH264, 1000));
  ...
  EXPECT_EQ(42, settings.QpHighThreshold(kVideoCodecH264, 3001));
}
...
TEST(BalancedDegradationSettings, GetsGenericHighQpThreshold) {
  ...
  EXPECT_EQ(23, settings.QpHighThreshold(kVideoCodecGeneric, 1001));
  EXPECT_EQ(23, settings.QpHighThreshold(kVideoCodecGeneric, 2000));
  ...
  EXPECT_EQ(24, settings.QpHighThreshold(kVideoCodecGeneric, 3001));
}
