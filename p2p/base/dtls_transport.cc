...
rtc::StreamResult StreamInterfaceChannel::Write(
  ...
  RTC_LOG(LS_ERROR) << "SSL WRITE " << data.size() << " is_handshake= "
                    << (data.size() > 0 && data[0] == 0x16);
...
void DtlsTransport::MaybeStartDtls() {
  ...
  // This is the crucial change. We currently call this early
  // but since the ICE transport is not writable we do nothing.
  // This needs to be guarded by field trial for sending.
