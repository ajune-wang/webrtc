...
TEST_F(TestRtpFrameReferenceFinder, WrappingFlexReference) {
  ...
  ASSERT_EQ(1UL, frames_from_callback_.size());
  // A frame with picture id 0 is referencing a frame with picture id 0 - 1. In
  // the wrapped space -1 is represented as 32767. The unwrapper in
  // RtpFrameReferenceFinder will first unwrap 32767 to |start_at| + 32767, next
  // picture id 0 will be unwrapped to |start_at| + 32768.
