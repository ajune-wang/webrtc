...
// FFT analysis at 12 kHz.
constexpr size_t kFftLenght20ms12kHz = 256;  // Next^2 of |kFrameSize20ms12kHz|.
static_assert(kFftLenght20ms12kHz / 2 < kFrameSize20ms12kHz &&
                  kFrameSize20ms12kHz <= kFftLenght20ms12kHz,
              "FFT size and frame size are not compatible.");
static_assert((kFftLenght20ms12kHz & (kFftLenght20ms12kHz - 1)) == 0,
              "The FFT size must be a power of 2.");
constexpr size_t kFftNumCoeffs20ms12kHz = kFftLenght20ms12kHz / 2 + 1;
