...
constexpr int kSkewHistorySizeLog2 = 7;
...
class SkewEstimator {
 ...
 private:
  int skew_ = 0;
  std::array<int, 2 << kSkewHistorySizeLog2> skew_history_;
...
rtc::Optional<DelayEstimate> RenderDelayControllerImpl::GetDelay(
  ...
  if (delay_samples) {
    if (!delay_samples_ ||
        (delay_samples->delay != delay_samples_->delay &&
         delay_samples->quality == DelayEstimate::Quality::kRefined)) {
      // If a new delay estimate is avaliable, store the skew for that ((note
      // that a reliable new delay estimate cannot be made without a reliable
      // skew));
      delay_change_counter_ = 0;
    }
  ...
  if (skew_ && skew &&
    ...
    // Compute the skew offset and add a margin of offset.
    offset_blocks = *skew_ - *skew;
    if (abs(offset_blocks) <= 1) {
      offset_blocks = 0;
    }

    // Soft reset the delay estimator if there is a significant offset detected.
    if (abs(offset_blocks) > 1 &&
        soft_reset_counter_ > 10 * kNumBlocksPerSecond) {
