...
cricket::Candidate CreateUdpCandidate(CandidateType type,
                                      absl::string_view ip,
                                      int port,
                                      int priority,
...
TEST_F(P2PTransportChannelTest, TurnToTurnPresumedWritable) {
  ...
  ep1_ch1()->AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kHost, "1.1.1.1", 1, 100));
  ep1_ch1()->AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kRelay, "2.2.2.2", 2, 0));
...
TEST_F(P2PTransportChannelTest, SignalReadyToSendWithPresumedWritable) {
  ...
  EXPECT_EQ_WAIT(IceGatheringState::kIceGatheringComplete,
                 ep1_ch1()->gathering_state(), kDefaultTimeout);
  ep1_ch1()->AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kRelay, "1.1.1.1", 1, 0));
 ...
 protected:
  ...
  Connection* CreateConnectionWithCandidate(P2PTransportChannel* channel,
    ...
    channel->AddRemoteCandidate(
        CreateUdpCandidate(CandidateType::kHost, ip_addr, port, priority));
    EXPECT_TRUE_SIMULATED_WAIT(
...
TEST_F(P2PTransportChannelPingTest, TestTriggeredChecks) {
  ...
  ch.AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kHost, "1.1.1.1", 1, 1));
  ch.AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kHost, "2.2.2.2", 2, 2));
...
TEST_F(P2PTransportChannelPingTest, TestAddRemoteCandidateWithVariousUfrags) {
  ...
  ch.AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kHost, "1.1.1.1", 1, 1, kIceUfrag[2]));
  Connection* conn1 = WaitForConnectionTo(&ch, "1.1.1.1", 1);
  ASSERT_TRUE(conn1 != nullptr);
  ...
  ch.AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kHost, "2.2.2.2", 2, 2, kIceUfrag[1]));
  ...
  ch.AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kHost, "3.3.3.3", 3, 0, kIceUfrag[2]));
...
TEST_F(P2PTransportChannelPingTest, TestSelectConnectionBeforeNomination) {
  ...
  ch.AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kHost, "2.2.2.2", 2, 10));
  Connection* conn2 = WaitForConnectionTo(&ch, "2.2.2.2", 2);
  ASSERT_TRUE(conn2 != nullptr);
  conn2->ReceivedPingResponse(LOW_RTT, "id");
  ...
  EXPECT_EQ(len, SendData(&ch, data, len, ++last_packet_id));
  ch.AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kHost, "3.3.3.3", 3, 1));
  ...
  EXPECT_EQ(len, SendData(&ch, data, len, ++last_packet_id));
  ch.AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kHost, "4.4.4.4", 4, 100));
...
TEST_F(P2PTransportChannelPingTest, TestSelectConnectionFromUnknownAddress) {
  ...
  ch.AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kHost, "2.2.2.2", 2, 1));
  Connection* conn2 = WaitForConnectionTo(&ch, "2.2.2.2", 2);
  ASSERT_TRUE(conn2 != nullptr);
...
TEST_F(P2PTransportChannelPingTest, TestSelectConnectionBasedOnMediaReceived) {
  ...
  ch.MaybeStartGathering();
  ch.AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kHost, "1.1.1.1", 1, 10));
  Connection* conn1 = WaitForConnectionTo(&ch, "1.1.1.1", 1);
  ASSERT_TRUE(conn1 != nullptr);
  conn1->ReceivedPingResponse(LOW_RTT, "id");
  EXPECT_EQ_WAIT(conn1, ch.selected_connection(), kDefaultTimeout);
...
TEST_F(P2PTransportChannelPingTest, TestAddRemoteCandidateWithAddressReuse) {
  ...
  Candidate candidate = CreateUdpCandidate(CandidateType::kHost, host_address,
...
TEST_F(P2PTransportChannelPingTest, TestDeleteConnectionsIfAllWriteTimedout) {
  ...
  ch.AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kHost, "3.3.3.3", 3, 2));
  Connection* conn3 = WaitForConnectionTo(&ch, "3.3.3.3", 3, &clock);
  ASSERT_TRUE(conn3 != nullptr);
...
TEST_F(P2PTransportChannelPingTest, TestStopPortAllocatorSessions) {
  ...
  ch.AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kHost, "2.2.2.2", 2, 100));
  Connection* conn2 = WaitForConnectionTo(&ch, "2.2.2.2", 2);
  ASSERT_TRUE(conn2 != nullptr);
  conn2->ReceivedPingResponse(LOW_RTT, "id");  // Becomes writable and receiving
  EXPECT_TRUE(!ch.allocator_session()->IsGettingPorts());
...
TEST_F(P2PTransportChannelMostLikelyToWorkFirstTest,
  ...
  ch.AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kRelay, "1.1.1.1", 1, 1));
  ch.AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kHost, "2.2.2.2", 2, 2));
...
TEST_F(P2PTransportChannelMostLikelyToWorkFirstTest,
  ...
  ch.AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kRelay, "2.2.2.2", 2, 2));
  EXPECT_TRUE_WAIT(ch.connections().size() == 4, kDefaultTimeout);
...
TEST_F(P2PTransportChannelTest,
  ...
  cricket::Candidate host_candidate = CreateUdpCandidate(
      CandidateType::kHost, "1.1.1.1", 1 /* port */, 0 /* priority */);
  ep1_ch1()->AddRemoteCandidate(host_candidate);

  std::vector<cricket::Candidate> mdns_candidates;
  mdns_candidates.push_back(CreateUdpCandidate(
      CandidateType::kHost, "example.local", 2 /* port */, 0 /* priority */));
  mdns_candidates.push_back(CreateUdpCandidate(
      CandidateType::kSrflx, "example.local", 3 /* port */, 0 /* priority */));
  mdns_candidates.push_back(CreateUdpCandidate(
      CandidateType::kRelay, "example.local", 4 /* port */, 0 /* priority */));
...
TEST_F(P2PTransportChannelTest, DisableDnsLookupsWithTransportPolicyRelay) {
  ...
  ep1_ch1()->AddRemoteCandidate(
      CreateUdpCandidate(CandidateType::kHost, "hostname.test", 1, 100));
