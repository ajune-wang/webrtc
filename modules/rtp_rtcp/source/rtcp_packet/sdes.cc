...
bool Sdes::Parse(const CommonHeader& packet) {
  ...
  if (packet.payload_size_bytes() % 4 != 0) {
    RTC_DLOG(LS_WARNING) << "Invalid payload size "
  ...
  for (size_t i = 0; i < number_of_chunks;) {
    ...
    if (payload_end - looking_at < 8) {
      RTC_DLOG(LS_WARNING) << "Not enough space left for chunk #" << (i + 1);
      return false;
    }
    ...
    if (cname_found) {
      ...
      RTC_DLOG(LS_WARNING) << "CNAME not found for ssrc " << chunks[i].ssrc;
      --number_of_chunks;
      chunks.resize(number_of_chunks);
    }
...
bool Sdes::AddCName(uint32_t ssrc, std::string cname) {
  ...
  if (chunks_.size() >= kMaxNumberOfChunks) {
    RTC_DLOG(LS_WARNING) << "Max SDES chunks reached.";
    return false;
  }
  Chunk chunk;
  chunk.ssrc = ssrc;
