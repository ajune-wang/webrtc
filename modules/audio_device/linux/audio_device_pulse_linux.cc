...
int32_t AudioDeviceLinuxPulse::LatencyUsecs(pa_stream* stream) {
  ...
  if (negative) {
    RTC_LOG(LS_VERBOSE) << "warning: pa_stream_get_latency reported negative "
                        << "delay";
  ...
  if (_playing) {
    ...
    if (_playbackBufferUnused < _playbackBufferSize) {
      ...
      if (LATE(pa_stream_write)(
        ...
        _writeErrors++;
        if (_writeErrors > 10) {
          RTC_LOG(LS_ERROR) << "Playout error: _writeErrors="
                        << _writeErrors
    ...
    if (_tempBufferSpace > 0) {
      ...
      if (nSamples != numPlaySamples) {
        RTC_LOG(LS_ERROR) << "invalid number of output samples("
                          << nSamples << ")";
      }
  ...
  if (_recording) {
    ...
    while (true) {
      ...
      if (LATE(pa_stream_drop)(_recStream) != 0) {
        RTC_LOG(LS_WARNING) << "failed to drop, err="
                        << LATE(pa_context_errno)(_paContext);
      }
