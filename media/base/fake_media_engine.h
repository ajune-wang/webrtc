...
template <class Base>
class RtpReceiveChannelHelper : public Base {
 public:
  ...
  const std::string rtcp_cname() {
    if (receive_streams_.empty())
      return "";
    return receive_streams_[0].cname;
  }
...
class FakeVoiceMediaReceiveChannel
 ...
 public:
  ...
  FakeVoiceMediaReceiveChannel(FakeVoiceEngine* engine,
  ...
  cricket::MediaType media_type() const { return cricket::MEDIA_TYPE_AUDIO; }
...
class FakeVoiceMediaSendChannel
 ...
 public:
  ...
  FakeVoiceMediaSendChannel(FakeVoiceEngine* engine,
                            const AudioOptions& options,
                            webrtc::TaskQueueBase* network_thread);
  ...
  virtual VoiceMediaSendChannelInterface* AsVoiceSendChannel() { return this; }
...
class FakeVideoMediaReceiveChannel
 ...
 public:
  FakeVideoMediaReceiveChannel(FakeVideoEngine* engine,
...
class FakeVideoMediaSendChannel
 ...
 public:
  FakeVideoMediaSendChannel(FakeVideoEngine* engine,
                            const VideoOptions& options,
                            webrtc::TaskQueueBase* network_thread);

  ~FakeVideoMediaSendChannel();

  virtual VideoMediaSendChannelInterface* AsVideoSendChannel() { return this; }
...
class FakeVoiceEngine : public VoiceEngineInterface {
 public:
  ...
  FakeVoiceMediaSendChannel* GetSendChannel(size_t index);
  FakeVoiceMediaReceiveChannel* GetReceiveChannel(size_t index);

  void RegisterChannel(std::unique_ptr<FakeVoiceMediaReceiveChannel> channel);
  void RegisterChannel(std::unique_ptr<FakeVoiceMediaSendChannel> channel);
  void UnregisterChannel(VoiceMediaSendChannelInterface* channel);
  void UnregisterChannel(VoiceMediaReceiveChannelInterface* channel);
 ...
 private:
  std::vector<std::unique_ptr<FakeVoiceMediaSendChannel>> send_channels_;
  std::vector<std::unique_ptr<FakeVoiceMediaReceiveChannel>> receive_channels_;
  std::vector<AudioCodec> recv_codecs_;
  std::vector<AudioCodec> send_codecs_;
...
class FakeVideoEngine : public VideoEngineInterface {
 public:
  ...
  void RegisterChannel(std::unique_ptr<FakeVideoMediaSendChannel> channel);
  void RegisterChannel(std::unique_ptr<FakeVideoMediaReceiveChannel> channel);
  void UnregisterChannel(VideoMediaSendChannelInterface* channel);
  void UnregisterChannel(VideoMediaReceiveChannelInterface* channel);
 ...
 private:
  std::vector<std::unique_ptr<FakeVideoMediaSendChannel>> send_channels_;
  std::vector<std::unique_ptr<FakeVideoMediaReceiveChannel>> receive_channels_;
  std::vector<VideoCodec> send_codecs_;
  std::vector<VideoCodec> recv_codecs_;
  bool capture_;
  VideoOptions options_;
...
class FakeMediaEngine : public CompositeMediaEngine {
 public:
  ...
  FakeVoiceMediaSendChannel* GetVoiceSendChannel(size_t index);
  FakeVideoMediaSendChannel* GetVideoSendChannel(size_t index);
  FakeVoiceMediaReceiveChannel* GetVoiceReceiveChannel(size_t index);
  FakeVideoMediaReceiveChannel* GetVideoReceiveChannel(size_t index);
