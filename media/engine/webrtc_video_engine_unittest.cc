...
TEST_F(WebRtcVideoChannelEncodedFrameCallbackTest,
  ...
  EXPECT_TRUE(receive_channel_->AddDefaultRecvStreamForTesting(
      cricket::StreamParams::CreateLegacy(kSsrc)));
  EXPECT_TRUE(receive_channel_->SetSink(kSsrc, &renderer_));
  receive_channel_->SetRecordableEncodedFrameCallback(kSsrc, callback.AsStdFunction());
  DeliverKeyFrame(kSsrc);
  time_controller_.AdvanceTime(kFrameDuration);
  EXPECT_EQ(renderer_.num_rendered_frames(), 1);
  receive_channel_->RemoveRecvStream(kSsrc);
...
TEST_F(WebRtcVideoChannelEncodedFrameCallbackTest,
  ...
  EXPECT_CALL(callback, Call);
  EXPECT_TRUE(
      receive_channel_->AddRecvStream(cricket::StreamParams::CreateLegacy(kSsrc)));
  EXPECT_TRUE(receive_channel_->SetSink(kSsrc, &renderer_));
  receive_channel_->SetRecordableEncodedFrameCallback(kSsrc, callback.AsStdFunction());
  DeliverKeyFrame(kSsrc);
  time_controller_.AdvanceTime(kFrameDuration);
...
TEST_F(WebRtcVideoChannelEncodedFrameCallbackTest,
  ...
  testing::StrictMock<
      ...
      callback;
  EXPECT_TRUE(
      receive_channel_->AddRecvStream(cricket::StreamParams::CreateLegacy(kSsrc + 1)));
...
class WebRtcVideoChannelBaseTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    // frames become flaky.
    media_config.video.enable_cpu_adaptation = false;
    send_channel_ = engine_.CreateSendChannel( call_.get(), media_config,
    ...
    EXPECT_TRUE(send_channel_->SetVideoSend(kSsrc, nullptr, frame_forwarder_.get()));
  }
  ...
  void SetUpSecondStreamWithNoRecv() {
    ...
    EXPECT_TRUE(
        send_channel_->SetVideoSend(kSsrc + 2, nullptr, frame_forwarder_2_.get()));
  }
...
TEST_F(WebRtcVideoChannelBaseTest, SetSendSsrcAfterSetCodecs) {
  ...
  EXPECT_TRUE(send_channel_->SetVideoSend(999u, nullptr, frame_forwarder_.get()));
  EXPECT_TRUE(SetSend(true));
...
TEST_F(WebRtcVideoChannelBaseTest, AddRemoveSendStreams) {
  ...
  EXPECT_TRUE(send_channel_->SetVideoSend(789u, nullptr, frame_forwarder_.get()));
...
class WebRtcVideoChannelTest : public WebRtcVideoEngineTest {
 public:
  ...
  void SetUp() override {
    ...
    send_channel_ = engine_.CreateSendChannel(fake_call_.get(), GetMediaConfig(),
        VideoOptions(), webrtc::CryptoOptions(),
        video_bitrate_allocator_factory_.get());
    receive_channel_ = engine_.CreateReceiveChannel(fake_call_.get(), GetMediaConfig(),
        VideoOptions(), webrtc::CryptoOptions());
    ...
    send_channel_->SetSendCodecChangedCallback([this]() {
      receive_channel_->SetReceiverFeedbackParameters(
          ...
          send_channel_->SendCodecRtcpMode(), send_channel_->SendCodecRtxTime());
    });
    send_channel_->OnReadyToSend(true);
    receive_channel_->SetReceive(true);
    last_ssrc_ = 123;
 ...
 protected:
  ...
  void TestSetRecvRtpHeaderExtensions(const std::string& ext_uri) {
    ...
    EXPECT_THAT(receive_channel_->GetRtpReceiveParameters(123).header_extensions,
...
TEST_F(WebRtcVideoChannelTest, ReconfiguresEncodersWhenNotSending) {
  ...
  EXPECT_TRUE(send_channel_->SetVideoSend(last_ssrc_, nullptr, &frame_forwarder));
  frame_forwarder.IncomingCapturedFrame(frame_source.GetFrame());
...
TEST_F(WebRtcVideoChannelTest,
  ...
  cricket::FakeFrameSource frame_source(1280, 720,
                                        rtc::kNumMicrosecsPerSec / 30);
  EXPECT_TRUE(send_channel_->SetVideoSend(last_ssrc_, &options, &frame_forwarder));
  EXPECT_TRUE(send_channel_->SetSend(true));
...
TEST_F(WebRtcVideoChannelTest, SetMediaConfigSuspendBelowMinBitrate) {
  ...
  send_channel_ = engine_.CreateSendChannel( fake_call_.get(), media_config,
      VideoOptions(), webrtc::CryptoOptions(),
      video_bitrate_allocator_factory_.get());
  receive_channel_ = engine_.CreateReceiveChannel( fake_call_.get(), media_config,
      VideoOptions(), webrtc::CryptoOptions());
  send_channel_->OnReadyToSend(true);
...
TEST_F(WebRtcVideoChannelTest, PreviousAdaptationDoesNotApplyToScreenshare) {
  ...
  send_channel_->SetVideoSend(last_ssrc_, &screenshare_options, &frame_forwarder);
  ASSERT_EQ(2, fake_call_->GetNumCreatedSendStreams());
  send_stream = fake_call_->GetVideoSendStreams().front();
...
TEST_F(WebRtcVideoChannelTest,
  ...
  webrtc::test::FrameForwarder frame_forwarder;
  EXPECT_TRUE(send_channel_->SetVideoSend(kSsrcs3[0], nullptr, &frame_forwarder));
  frame_forwarder.IncomingCapturedFrame(frame_source_.GetFrame());
...
TEST_F(WebRtcVideoChannelTest, SetRtpSendParametersPrioritySimulcastStreams) {
  ...
  webrtc::test::FrameForwarder frame_forwarder;
  VideoOptions options;
  EXPECT_TRUE(send_channel_->SetVideoSend(primary_ssrc, &options, &frame_forwarder));
  send_channel_->SetSend(true);
  frame_forwarder.IncomingCapturedFrame(frame_source_.GetFrame(
      1920, 1080, webrtc::VideoRotation::kVideoRotation_0,
      rtc::kNumMicrosecsPerSec / 30));
...
TEST_F(WebRtcVideoChannelTest, SetAndGetRtpReceiveParameters) {
  ...
  EXPECT_EQ(initial_params, receive_channel_->GetRtpReceiveParameters(last_ssrc_));
}
