...
// TODO(hbos): Merge with VideoStreamInputState in video_stream_adapter.h.
// TODO(hbos): Make it a class and add IsSufficient().
class VideoStreamInputStateForProcessor {
 public:
  VideoStreamInputStateForProcessor();

  void set_input_pixels(absl::optional<int> input_pixels);
  void set_input_fps(absl::optional<int> input_fps);
  void set_video_codec_type(VideoCodecType video_codec_type);
  void set_min_pixels_per_frame(int min_pixels_per_frame);

  absl::optional<int> input_pixels() const;
  absl::optional<int> input_fps() const;
  VideoCodecType video_codec_type() const;
  int min_pixels_per_frame() const;

  bool HasPixelsAndFps() const;
  VideoStreamInputState SubsetForAdapter() const;

 private:
  // TODO(hbos): int versus size_t?
  absl::optional<int> input_pixels_;
  absl::optional<int> input_fps_;
...
class VideoStreamInputStateProvider {
 public:
  ...
  VideoStreamInputStateForProcessor InputState();

 private:
  mutable rtc::CriticalSection crit_;
  VideoStreamEncoderResourceManager* const manager_;  // No, don't
  VideoStreamEncoderObserver* const frame_rate_provider_;
  VideoStreamInputStateForProcessor input_state_;
};
