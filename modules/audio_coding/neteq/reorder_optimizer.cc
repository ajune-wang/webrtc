...
void ReorderOptimizer::Update(int relative_delay_ms, bool reordered) {
  const int index = reordered ? relative_delay_ms / kBucketSizeMs : 0;
  if (index < histogram_.NumBuckets()) {
    // Maximum delay to register is 2000 ms.
    histogram_.Add(index);
  }
  int bucket_index = MinimizeCostFunction();
...
int ReorderOptimizer::MinimizeCostFunction() const {
  ...
  for (size_t i = 0; i < buckets.size(); ++i) {
    ...
    int64_t delay_ms = static_cast<int64_t>(i << 30) * kBucketSizeMs;
