...
PacedSender::PacedSender(Clock* clock,
    ...
    : pacing_controller_(absl::make_unique<PacedSendingController>(
          clock,
          static_cast<PacedSendingController::PacketSender*>(this),
          event_log,
          field_trials)),
...
int64_t PacedSender::TimeUntilNextProcess() {
  ...
  if (pacing_controller_->IsPaused()) {
    return std::max(
               PacedSendingController::kPausedProcessInterval - elapsed_time,
               TimeDelta::Zero())
        .ms();
  }
