...
TCPAcceptor::TCPAcceptor(const ubnt::abstraction::SocketAddress& address,
                         uint8_t tos,
                         Observer* observer,
                         TCPProtocol::Observer* tcpobserver)
    : IOHandler(0, 0, IOHT_ACCEPTOR),
      _observer(observer),
      _tcpobserver(tcpobserver) {
  _address = address;
  _enabled = false;
  _acceptedCount = 0;
  _droppedCount = 0;
...
bool TCPAcceptor::Bind() {
  ...
  // bind it
  if (::bind(_inboundFd, (const sockaddr*)_address, _address.GetLength()) !=
      0) {
    int err = errno;
    printf("Unable to bind on address: tcp://%s; Error was: (%d) %s",
           (const char*)_address, err, strerror(err));
    return false;
  }
...
bool TCPAcceptor::Accept() {
  ...
  if (!_enabled) {
    ...
    printf("Acceptor is not enabled. Client dropped: %s -> %s",
