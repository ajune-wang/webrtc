...
TEST_F(VideoReceiveStreamTestWithFakeDecoder,
  ...
  EXPECT_THAT(&fake_renderer_.encoded_frame()->get(), Eq(test_frame_ptr));
}
...
TEST_F(VideoReceiveStreamTestWithRecordingRenderer,
  ...
  for (int i = 0; i < 10; ++i) {
    EXPECT_EQ(encoded_frame_ptrs[i],
              &frames_rendered[i].get_encoded_frame_source()->get());
  }
  video_receive_stream_->DoneEncodedOutput();
  for (int i = 10; i < 15; ++i) {
    auto test_frame = std::make_unique<FrameObjectFake>();
  ...
  for (int i = 10; i < 15; ++i) {
    EXPECT_EQ(frames_rendered[i].get_encoded_frame_source(), nullptr);
  }
}
