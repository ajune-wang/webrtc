...
ScopedJavaLocalRef<jobject> NativeToJavaRTCCertificate(
    JNIEnv* jni,
    const rtc::scoped_refptr<rtc::RTCCertificate>& certificate) {
  rtc::RTCCertificatePEM pem = certificate->ToPEM();
  return Java_RTCCertificate_Constructor(
      jni, NativeToJavaString(jni, pem.private_key()),
      NativeToJavaString(jni, pem.certificate()));
}

static ScopedJavaLocalRef<jobject> JNI_RTCCertificate_GenerateCertificate(
    JNIEnv* jni,
