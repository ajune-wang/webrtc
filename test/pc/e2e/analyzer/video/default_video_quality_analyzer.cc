...
void DefaultVideoQualityAnalyzer::OnFramePreDecode(
  ...
  if (stats_it == frame_stats_.end()) {
    ...
    // a key frame for one stream after key frame for another stream.
    return;
  }
...
void DefaultVideoQualityAnalyzer::OnFrameDecoded(
  ...
  if (stats_it == frame_stats_.end()) {
    ...
    // when we have multiple simulcast streams in one track and received a key
    // frame for one stream after key frame for another stream.
    return;
  }
