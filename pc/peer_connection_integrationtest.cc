...
TEST_P(PeerConnectionIntegrationTest,
  ...
  // Note that we do *not* set the datagram transport to be ready-to-send
  // before this.  This prevents the callee from observing the data channel
  // if the caller tries to use the datagram transport, so the following
  // assertion only passes if SCTP is used.
  ASSERT_TRUE_WAIT(callee()->data_channel() != nullptr, kDefaultTimeout);
  EXPECT_TRUE_WAIT(caller()->data_observer()->IsOpen(), kDefaultTimeout);
  EXPECT_TRUE_WAIT(callee()->data_observer()->IsOpen(), kDefaultTimeout);
...
TEST_P(PeerConnectionIntegrationTest,
  ...
  // Disabling DTLS precludes a fallback to SCTP, so that this test only passes
  // if the datagram transport is successfully negotiated and used.
