...
bool StreamStatisticianImpl::UpdateOutOfOrder(const RtpPacketReceived& packet,
  ...
  if (received_seq_out_of_order_) {
    // Now count the previous packet as a received.
    --cumulative_loss_;
    ...
    if (packet.SequenceNumber() == expected_sequence_number) {
      ...
      // consistent way.
      last_report_seq_max_ = received_seq_max_ = sequence_number - 2;
      return false;
    }
  }
...
void StreamStatisticianImpl::UpdateCounters(const RtpPacketReceived& packet) {
  ...
  if (!ReceivedRtpPacket()) {
    ...
    last_report_seq_max_ = received_seq_max_ = sequence_number - 1;
...
RtcpStatistics StreamStatisticianImpl::CalculateRtcpStatistics() {
  ...
  if (exp_since_last && lost_since_last > 0) {
  ...
  if (stats.packets_lost < 0) {
    ...
    cumulative_loss_rtcp_offset_ -= stats.packets_lost;
