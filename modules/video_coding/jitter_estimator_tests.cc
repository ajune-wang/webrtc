...
TEST_F(TestVCMJitterEstimator, TestUpperBound) {
  ...
  std::vector<TestContext> test_cases(2);  // CAVEAT change to appropriate value
  ...
  test_cases[0].upper_bound = 1000.0;
  test_cases[0].rtt_mult = 1.0;
  test_cases[0].jitter_est_cap_ms = 100.0;
  // Large upper bound, rtt_mult = 1, and small rtt_mult cap value.
  test_cases[1].upper_bound = 1000.0;
  test_cases[1].rtt_mult = 1.0;
  test_cases[1].jitter_est_cap_ms = 10.0;
