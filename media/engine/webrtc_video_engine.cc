...
WebRtcVideoSendChannel::WebRtcVideoSendStream::GetPerLayerVideoSenderInfos(
  ...
  for (const auto& pair : outbound_rtp_substreams) {
    ...
    info.psnr_measurements_y = stream_stats.psnr_measurements_y;
...
WebRtcVideoSendChannel::WebRtcVideoSendStream::GetAggregatedVideoSenderInfo(
  ...
  for (size_t i = 1; i < infos.size(); i++) {
    ...
    if (infos[i].psnr_measurements_y) {
      if (!info.psnr_measurements_y) {
        info.psnr_measurements_y = 0;
      }
      info.psnr_measurements_y =
          *info.psnr_measurements_y + *infos[i].psnr_measurements_y;
    }
