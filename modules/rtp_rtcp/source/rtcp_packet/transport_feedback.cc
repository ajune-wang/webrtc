...
void TransportFeedback::HandleAllPackets(
    rtc::FunctionView<void(uint16_t sequence_numbers, TimeDelta delta)> handler)
    const {
  TimeDelta packet_delta = TimeDelta::Zero();
  ...
  for (uint16_t seq_num = base_seq_no_; seq_num != last_seq_num; ++seq_num) {
    if (received_it != received_packets_.end() &&
      ...
      packet_delta += received_it->delta();
      handler(seq_num, packet_delta);
      ++received_it;
    } else {
