...
using rnn_vad::KissFft;

namespace {

const double kPi = std::acos(-1.0);

void FillFftInputBuffer(rtc::ArrayView<const float> samples,
                        rtc::ArrayView<std::complex<float>> input_buf) {
  ASSERT_EQ(samples.size(), input_buf.size());
...
TEST_P(RnnVadTest, KissFftForwardReverseCheckIdentity) {
  ...
  FillFftInputBuffer({samples}, {fft_buf_1});
  {
  ...
  CheckFftResult({samples}, {zeros}, {fft_buf_1}, tolerance);
}
...
TEST(RnnVadTest, KissFftBitExactness) {
  ...
  FillFftInputBuffer({samples}, {fft_buf_in});
  fft.ForwardFft(fft_buf_in.size(), fft_buf_in.data(), fft_buf_out.size(),
                 fft_buf_out.data());
  CheckFftResult({expected_real}, {expected_imag}, {fft_buf_out.data(), 17},
