...
- (NSInteger)encode:(RTC_OBJC_TYPE(RTCVideoFrame) *)frame
  ...
  if ([frame.buffer isKindOfClass:[RTC_OBJC_TYPE(RTCCVPixelBuffer) class]]) {
    ...
    if (![rtcPixelBuffer requiresCropping]) {
      ...
      CVPixelBufferPoolRef pbPool = VTCompressionSessionGetPixelBufferPool(_compressionSession);
      if (!pbPool) {
        return WEBRTC_VIDEO_CODEC_ERROR;
      }

      pixelBuffer = CreatePixelBuffer(pbPool);
      if (!pixelBuffer) {
        return WEBRTC_VIDEO_CODEC_ERROR;
      }
...
- (BOOL)resetCompressionSessionIfNeededWithFrame:(RTC_OBJC_TYPE(RTCVideoFrame) *)frame {
  ...
  if (_compressionSession) {
    ...
    NSDictionary *poolAttributes =
        (__bridge NSDictionary *)CVPixelBufferPoolGetPixelBufferAttributes(pbPool);
    id pixelFormats =
