...
TEST_F(PeerConnectionE2EQualityTestSmokeTest, MAYBE_HighBitrate) {
  ...
  RunTest(
      ...
      [](PeerConfigurer* alice) {
        ...
        bitrate_params.current_bitrate_bps = 8000000;
        bitrate_params.max_bitrate_bps = 10000000;
        alice->SetBitrateParameters(bitrate_params);
        VideoConfig video(1920, 1080, 30);
        video.stream_label = "alice-video";
        video.min_encode_bitrate_bps = 6000000;
        video.max_encode_bitrate_bps = 8000000;
        alice->AddVideoConfig(std::move(video));
