...
class AudioProcessingImpl : public AudioProcessing {
 public:
  ...
  int ProcessStream(const float* const* src,
                    ...
                    int& recommended_input_volume) override;
 ...
 private:
  ...
  int ProcessStreamLocked(const float* const* src, float* const* dest)
      RTC_EXCLUSIVE_LOCKS_REQUIRED(mutex_capture_);
  ...
  struct ApmCaptureState {
    ...
    // False if `set_stream_analog_level()` is not called since the last call to
    // `ProcessStream()`.
    bool applied_input_volume_updated;
