...
TEST(LoopbackMediaTransport, DataDeliveredToSink) {
  ...
  transport_pair.second()->SendData(1, params, buffer);
...
TEST(LoopbackMediaTransport, CloseDeliveredToSink) {
...

  {
    ...
    EXPECT_CALL(second_sink, OnChannelClosing(1));
    EXPECT_CALL(second_sink, OnChannelClosed(1));
    EXPECT_CALL(first_sink, OnChannelClosed(1));
  }

  transport_pair.first()->CloseChannel(1);
