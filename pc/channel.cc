...
sigslot::signal1<ChannelInterface*>& BaseChannel::SignalFirstPacketReceived() {
  ...
  // TODO(tommi): Change this to be a std::function callback that's set at
  // constructor time, remove sigslot dependency from both the channel
  // classes and Transceiver class. Move construction of the channels into
  // the Transceiver classes (where the callback can be set from the start).
  // Doing so will also move full ownership into the transceiver and remove
  // the need to keep the ownership in the channel manager (fewer raw pointers).
  // This is also the last holdout for needing MessageHandlerAutoCleanup in
  // BaseChannel.
