 ...
 public:
  ...
  void GeneratePlc(size_t requested_samples_per_channel,
    ...
    const bool last_was_plc = last_was_plc_;
      ...
      int dec_len =
          DecodeInternal(nullptr, 2 * 10 * sample_rate_hz_ / 1000,
...
class LossyInput : public NetEqInput {
 public:
  LossyInput(int loss_cadence, int burst_length,
...
TestStatistics RunTest(int loss_cadence, int burst_length,
                       std::string* checksum) {
  NetEq::Config config;
...
TEST(NetEqDecoderPlc, NoInterruptionsInSmallLosses) {
  ...
  EXPECT_EQ(stats_1.lifetime.concealed_samples, 640);  // 20ms of concealment.
  EXPECT_EQ(stats_1.lifetime.concealment_events, 1);   // in just one event.

  EXPECT_EQ(stats_4.lifetime.interruption_count, 0);
  EXPECT_EQ(stats_4.lifetime.total_interruption_duration_ms, 0);
  EXPECT_EQ(stats_4.lifetime.concealed_samples, 2560);  // 80ms of concealment.
  EXPECT_EQ(stats_4.lifetime.concealment_events, 1);    // in just one event.
}
