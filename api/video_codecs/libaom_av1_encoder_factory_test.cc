...
TEST(LibaomAv1Encoder, KeyframeUpdatesSpecifiedBuffer) {
  ...
  enc->Encode(raw_delta, {.presentation_timestamp = Timestamp::Millis(100)},
              {Fb().Rate(kCbr).Res(640, 360).Ref({5}).Build()}, res.CallBack());
  ASSERT_THAT(res.FrameAt(2), NotNull());
  VideoFrame decoded_delta = dec.Decode(*res.FrameAt(2));
  EXPECT_THAT(Resolution(decoded_delta), ResolutionIs(640, 360));
  EXPECT_THAT(Psnr(raw_delta, decoded_delta), Gt(40));
}
