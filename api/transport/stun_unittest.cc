...
// 2.1.  Sample Request
static const unsigned char kRfc5769SampleRequestMI32[] = {
...
TEST_F(StunTest, ValidateMessageIntegrity32) {
  ...
  EXPECT_TRUE(StunMessage::ValidateMessageIntegrity32(
      reinterpret_cast<const char*>(kRfc5769SampleRequestMI32),
      sizeof(kRfc5769SampleRequestMI32), kRfc5769SampleMsgPassword));
  EXPECT_FALSE(StunMessage::ValidateMessageIntegrity32(
      reinterpret_cast<const char*>(kRfc5769SampleRequestMI32),
      sizeof(kRfc5769SampleRequestMI32), "InvalidPassword"));
  ...
  // Test that munging a single bit anywhere in the message causes the
  // message-integrity check to fail, unless it is after the M-I attribute.
  char buf[sizeof(kRfc5769SampleRequestMI32)];
  memcpy(buf, kRfc5769SampleRequestMI32, sizeof(kRfc5769SampleRequestMI32));
  for (size_t i = 0; i < sizeof(buf); ++i) {
    buf[i] ^= 0x01;
    if (i > 0)
      buf[i - 1] ^= 0x01;
...
TEST_F(StunTest, AddMessageIntegrity32) {
  ...
  const StunByteStringAttribute* mi_attr =
      msg.GetByteString(STUN_ATTR_MESSAGE_INTEGRITY_32);
  ...
  const StunByteStringAttribute* mi_attr2 =
      msg2.GetByteString(STUN_ATTR_MESSAGE_INTEGRITY_32);
