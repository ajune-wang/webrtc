...
HRESULT WgcCaptureSession::ProcessFrame() {
  ...
  // ProcessFrame() is normally called from OnFrameArrived() but can also be
  // called by GetFrame() if the queue is empty to ensure that GetFrame() always
  // can deliver a valid frame. This scheme can lead to a state where the frame
  // pool is empty and the condition below determines if that is the case. If we
  // know in advance that the frame pool will be empty, we might as well return
  // already at this early stage to save resources and avoid adding logs and UMA
  // stats. Ideally, on_frame_arrived_events_ = processed_frames_ + 1.
  const bool frame_pool_is_empty =
      (on_frame_arrived_events_ > 0 &&
       on_frame_arrived_events_ <= processed_frames_);
  if (frame_pool_is_empty)
    return S_OK;
