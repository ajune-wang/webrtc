...
TEST_F(TransientFileUtilsTest, MAYBE_ReadInt16BufferFromFile) {
  ...
  FileWrapper file =
      FileWrapper::Open(test_filename.c_str(), true);  // Read only.
...
TEST_F(TransientFileUtilsTest, MAYBE_WriteInt16BufferToFile) {
  ...
  FileWrapper file =
      FileWrapper::Open(kOutFileName.c_str(), false);  // Write mode.
  ASSERT_TRUE(file.is_open()) << "File could not be opened:\n"
                              << kOutFileName.c_str();
  ...
  file = FileWrapper::Open(kOutFileName.c_str(), true);  // Read only.
  ASSERT_TRUE(file.is_open()) << "File could not be opened:\n"
                              << kOutFileName.c_str();
...
TEST_F(TransientFileUtilsTest, MAYBE_ExpectedErrorReturnValues) {
  ...
  file = FileWrapper::Open(test_filename.c_str(), true);  // Read only.
  ASSERT_TRUE(file.is_open()) << "File could not be opened:\n"
                              << kTestFileName.c_str();
