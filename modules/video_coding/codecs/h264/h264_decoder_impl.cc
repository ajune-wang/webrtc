...
int H264DecoderImpl::AVGetBuffer2(AVCodecContext* context,
  ...
  if (ret < 0) {
    RTC_DLOG(LS_ERROR) << "Invalid picture size " << width << "x" << height;
    decoder->ReportError();
    return ret;
  }
...
int32_t H264DecoderImpl::InitDecode(const VideoCodec* codec_settings,
  ...
  if (!codec) {
    ...
    RTC_DLOG(LS_ERROR) << "FFmpeg H.264 decoder not found.";
    Release();
    ReportError();
  ...
  if (res < 0) {
    RTC_DLOG(LS_ERROR) << "avcodec_open2 error: " << res;
    Release();
    ReportError();
    return WEBRTC_VIDEO_CODEC_ERROR;
  }
...
int32_t H264DecoderImpl::Decode(const EncodedImage& input_image,
  ...
  if (result < 0) {
    RTC_DLOG(LS_ERROR) << "avcodec_send_packet error: " << result;
    ReportError();
    return WEBRTC_VIDEO_CODEC_ERROR;
  }
  ...
  if (result < 0) {
    RTC_DLOG(LS_ERROR) << "avcodec_receive_frame error: " << result;
    ReportError();
    return WEBRTC_VIDEO_CODEC_ERROR;
  }
