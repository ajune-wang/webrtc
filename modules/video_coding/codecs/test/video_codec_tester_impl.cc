...
// Pacing is implemented by posting encode/decode tasks to `TaskQueue` with
// delay. Note that delay in `TaskQueue::PostDelayedTask` is time from now
// but not from previously posted task.
class Pacer {
 public:
...
// the limit is reached, posting of a new task is blocked until execution of an
// early posted task starts. This class is not thread-safe.
class LimitedTaskQueue {
 public:
  // The codec tester reads frames from video source in the main thread.
  // Encoding/decoding is done in separate threads. This lets reading to go far
  // ahead of encoding/decoding and may cause buffering of too many raw video
...
std::unique_ptr<VideoCodecTestStats> VideoCodecTesterImpl::RunDecodeTest(
  ...
  VideoCodecAnalyzer perf_analyzer(/*reference_video_source=*/nullptr);
