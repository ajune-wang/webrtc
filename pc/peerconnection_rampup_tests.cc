...
namespace {
static const int kDefaultTestTime = 1000;
...
// test, so it's set to 500 kbps.
static const int kHighNetworkBandwidth = 500000;
}  // namespace
...
class PeerConnectionRampUpTest : public ::testing::Test {
 public:
        ...
        network_thread_->Invoke<cricket::TestTurnServer*>(
            RTC_FROM_HERE, [thread, type] {
              ...
              static const rtc::SocketAddress turn_server_external_address{
                  "88.88.88.1", 0};
              return new cricket::TestTurnServer(
  ...
  // Runs the call for 5 seconds, and then records the bandwidth send
  // side estimation to perf results.
  void RunTest(const std::string& test_string) {
    rtc::Thread::Current()->ProcessMessages(kDefaultTestTime);
    double caller_outgoing_bitrate = GetCallerOutgoingBitrate();
    ASSERT_NE(0, caller_outgoing_bitrate);
    std::string value_description =
        "bwe_after_" + std::to_string(kDefaultTestTime) + "_seconds";
    test::PrintResult("peerconnection_ramp_up_", test_string, value_description,
                      caller_outgoing_bitrate, "bwe", false);
  }
