 ...
 public:
  ...
  int Connect(const SocketAddress& addr) override {
    if (type_ == SOCK_STREAM) {
      if (!server_->Check(FP_TCP, GetLocalAddress(), addr)) {
        NLOG(LS_VERBOSE, "FirewallSocket outbound TCP connection from ",
  ...
  int SendTo(const void* pv, size_t cb, const SocketAddress& addr) override {
    ...
    if (!server_->Check(protocol, GetLocalAddress(), addr)) {
      NLOG(LS_VERBOSE, "FirewallSocket outbound packet with type ", type_,
           " from ", GetLocalAddress().ToSensitiveString(), " to ",
    ...
    if (type_ == SOCK_DGRAM) {
      while (true) {
        ...
        if (server_->Check(FP_UDP, *paddr, GetLocalAddress()))
          return res;
        NLOG(LS_VERBOSE, "FirewallSocket inbound UDP packet from ",
  ...
  int Listen(int backlog) override {
    if (!server_->tcp_listen_enabled()) {
      NLOG(LS_VERBOSE, "FirewallSocket listen attempt denied");
      return -1;
    }
  ...
  AsyncSocket* Accept(SocketAddress* paddr) override {
    ...
    while (AsyncSocket* sock = AsyncSocketAdapter::Accept(&addr)) {
      ...
      sock->Close();
      delete sock;
      NLOG(LS_VERBOSE, "FirewallSocket inbound TCP connection from ",
...
AsyncSocket* FirewallSocketServer::WrapSocket(AsyncSocket* sock, int type) {
  if (!sock ||
    ...
    NLOG(LS_VERBOSE, "FirewallSocketServer socket creation denied");
    delete sock;
    return nullptr;
  }
