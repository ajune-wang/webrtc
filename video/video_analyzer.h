...
class VideoAnalyzer : public PacketReceiver,
 ...
 private:
  ...
  uint32_t pause_count_ RTC_GUARDED_BY(comparison_lock_);
  uint32_t total_freezes_duration_ms_ RTC_GUARDED_BY(comparison_lock_);
  uint32_t total_pauses_duration_ms_ RTC_GUARDED_BY(comparison_lock_);
  uint32_t total_frames_duration_ms_ RTC_GUARDED_BY(comparison_lock_);
  float sum_squared_interframe_delays_secs_ RTC_GUARDED_BY(comparison_lock_);
