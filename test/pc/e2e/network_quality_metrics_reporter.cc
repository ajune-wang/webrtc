...
void NetworkQualityMetricsReporter::OnStatsReports(
  ...
  for (const auto& stat : inbound_stats) {
    payload_received += DataSize::Bytes(
        test::GetStatOrDefault(stat->bytes_received, 0ul) +
        test::GetStatOrDefault(stat->header_bytes_received, 0ul));
  }

  auto outbound_stats = report->GetStatsOfType<RTCOutboundRTPStreamStats>();
  for (const auto& stat : outbound_stats) {
    payload_sent +=
        DataSize::Bytes(test::GetStatOrDefault(stat->bytes_sent, 0ul) +
                        test::GetStatOrDefault(stat->header_bytes_sent, 0ul));
  }
