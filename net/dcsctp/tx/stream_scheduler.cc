...
absl::optional<SendQueue::DataToSend> StreamScheduler::Produce(
  ...
  absl::optional<ProducedData> data;
  ...
  // in `active_streams`.
  size_t next_send = data->additional_pending_data;
...
absl::optional<StreamScheduler::ProducedData> StreamScheduler::Stream::Produce(
    TimeMs now,
    size_t max_size) {
  absl::optional<ProducedData> data = callback_.Produce(now, max_size);

  if (data.has_value()) {
    VirtualTime new_current =
        CalculateFinishTime(data->data.data.payload.size());
