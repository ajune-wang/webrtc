...
RouteId GetRouteId(const rtc::NetworkRoute& route,
                   bool distinguish_on_relayed) {
  return RouteId(route.local.network_id(), route.remote.network_id(),
                 distinguish_on_relayed ? absl::optional<bool>(IsRelayed(route))
                                        : absl::nullopt);
}
...
RtpTransportControllerSend::RtpTransportControllerSend(
    ...
    : clock_(clock),
      ...
      relay_bandwidth_cap_("relay_cap"),
  ...
  ParseFieldTrial(
      ...
      field_trial::FindFullName("WebRTC-Bwe-NetworkRouteConstraints"));
...
void RtpTransportControllerSend::OnNetworkRouteChanged(
  ...
  SetRelayCap(IsRelayed(network_route));
  ...
  // other conditions.
  const bool distinguish_on_relaying = relay_bandwidth_cap_ ? true : false;
  const RouteId route_id = GetRouteId(network_route, distinguish_on_relaying);
  if (GetRouteId(old_route, distinguish_on_relaying) != route_id ||
      !old_route.connected) {
    ...
    task_queue_.PostTask([this, msg, network_route, route_id] {
      RTC_DCHECK_RUN_ON(&task_queue_);
      transport_overhead_bytes_per_packet_ = network_route.packet_overhead;
      if (reset_feedback_on_route_change_) {
        transport_feedback_adapter_.SetRouteId(route_id);
      }
      if (controller_) {
...
void RtpTransportControllerSend::SetRelayCap(bool is_relayed) {
  int cap_bps =
      is_relayed && relay_bandwidth_cap_ ? relay_bandwidth_cap_->bps() : -1;
  absl::optional<BitrateConstraints> updated =
      bitrate_configurator_.UpdateWithRelayCap(cap_bps);
  if (updated.has_value()) {
    UpdateBitrateConstraints(*updated);
  } else {
    RTC_LOG(LS_VERBOSE) << "WebRTC.RtpTransportControllerSend.SetRelayCap: "
                           "nothing to update";
  }
}
