...
rtc::scoped_refptr<AudioDeviceModule> TestPeerFactory::CreateAudioDeviceModule(
  ...
  if (params->audio_config) {
    ...
    if (params->audio_config->mode == AudioConfig::Mode::kGenerated) {
      capturer = TestAudioDeviceModule::CreatePulsedNoiseCapturer(32000, 48000);
      ...
      RTC_CHECK(false) << "Unknown params->audio_config->mode";
    }
  } else {
  ...
  absl::optional<std::string> input_dump_file_name = absl::nullopt;
  if (params->audio_config) {
    input_dump_file_name = params->audio_config->input_dump_file_name;
  }

  if (input_dump_file_name) {
    capturer = absl::make_unique<CopyToFileAudioCapturer>(
        std::move(capturer), input_dump_file_name.value());
  }

  std::unique_ptr<TestAudioDeviceModule::Renderer> renderer;
  if (audio_output_file_name) {
    renderer = TestAudioDeviceModule::CreateBoundedWavFileWriter(
        audio_output_file_name.value(), 48000);
  } else {
    renderer = TestAudioDeviceModule::CreateDiscardRenderer(48000);
  }
...
std::unique_ptr<VideoEncoderFactory> TestPeerFactory::CreateVideoEncoderFactory(
  ...
  if (components->pcf_dependencies->video_encoder_factory != nullptr) {
    video_encoder_factory =
        move(components->pcf_dependencies->video_encoder_factory);
  } else {
    video_encoder_factory = CreateBuiltinVideoEncoderFactory();
  }
  video_encoder_factory = video_analyzer_holder->WrapVideoEncoderFactory(
      move(video_encoder_factory));
  return video_encoder_factory;
}
...
std::unique_ptr<VideoDecoderFactory> TestPeerFactory::CreateVideoDecoderFactory(
  ...
  if (components->pcf_dependencies->video_decoder_factory != nullptr) {
    video_decoder_factory =
        move(components->pcf_dependencies->video_decoder_factory);
  } else {
    video_decoder_factory = CreateBuiltinVideoDecoderFactory();
  }
  video_decoder_factory = video_analyzer_holder->WrapVideoDecoderFactory(
      move(video_decoder_factory));
  return video_decoder_factory;
}
