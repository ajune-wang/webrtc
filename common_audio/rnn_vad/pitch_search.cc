...
PitchInfo CheckLowerPitchPeriodsAndComputePitchGain(
  ...
  auto to_inv_lag = [](size_t period) { return kPitchMaxPeriod - 1 - period; };
  auto pitch_gain = [xx](float xy, float yy) {
    return xy / std::sqrt(1.f + xx * yy);
  };

  // Initial pitch candidate gain.
  const float g0 = pitch_gain(
      ComputeAutoCorrelationCoeff(pitch_buf, to_inv_lag(t0), kPitchMaxPeriod),
      yy_values[t0]);
  ...
  for (size_t k = 2; k < kSubHarmonicMultipliers.size(); ++k) {
    ...
    const float xy_t1 =
        ComputeAutoCorrelationCoeff(pitch_buf, to_inv_lag(t1), kPitchMaxPeriod);
    const float xy_t1_b = ComputeAutoCorrelationCoeff(
        pitch_buf, to_inv_lag(t1_b), kPitchMaxPeriod);
    ...
    const float threshold = ComputePitchGainThreshold(
        t1, t0, g0, last_pitch_2x.period / 2, last_pitch_2x.gain);
    if (g1 > threshold) {
