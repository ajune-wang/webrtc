...
bool LibaomAv1Encoder::InitEncode(
  ...
  // TODO: does 90khz timebase make sense, use microseconds instead maybe?
  cfg_.g_timebase.den = kRtpTicksPerSecond;
  cfg_.g_input_bit_depth = settings.encoding_format.bit_depth;
  cfg_.kf_mode = AOM_KF_DISABLED;
  // TODO: rc_undershoot_pct and rc_overshoot_pct should probably be removed.
...
bool ValidateEncodeParams(
  ...
  for (size_t i = 0; i < frame_settings.size(); ++i) {
    ...
    if (i > 0) {
      if (!GetScalingFactor(frame_settings[i - 1].resolution,
                            settings.resolution)) {
        RTC_LOG(LS_ERROR)
    ...
    for (size_t buffer_id = 0; buffer_id < settings.reference_buffers.size();
         ++buffer_id) {
      if (!in_range(0, kNumBuffers, settings.reference_buffers[buffer_id])) {
        RTC_LOG(LS_ERROR) << "Invalid reference buffer id.";
        return false;
      }
      for (size_t other_id = buffer_id + 1;
           other_id < settings.reference_buffers.size(); ++other_id) {
        if (settings.reference_buffers[buffer_id] ==
            settings.reference_buffers[other_id]) {
...
aom_svc_ref_frame_config_t GetSvcRefFrameConfig(
  ...
  // TODO: Automatically select LAST, GOLDEN and ALTREF depending on previous
  //       buffer usage.
...
void LibaomAv1Encoder::Encode(
  ...
  if (static_cast<int>(cfg_.g_w) != frame_buffer->width() ||
    ...
    SET_OR_DO_ERROR_CALLBACK_AND_RETURN(AV1E_SET_TILE_ROWS, ttsbi.tile_rows);
...
// TODO: it should also possible to expose SW/HW/driver version.
