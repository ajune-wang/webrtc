...
class ScalabilityStructureFullSvc : public ScalableVideoController {
 public:
  ...
  ScalabilityStructureFullSvc(NumLayers num_layers,
 ...
 private:
  ...
  int BufferIndex(int sid, int tid) const {
    return tid * num_layers_.spatial + sid;
  }
  bool DecodeTargetIsActive(int sid, int tid) const {
    return active_decode_targets_[sid * num_layers_.temporal + tid];
  }
  void SetDecodeTargetIsActive(int sid, int tid, bool value) {
    active_decode_targets_.set(sid * num_layers_.temporal + tid, value);
  }
