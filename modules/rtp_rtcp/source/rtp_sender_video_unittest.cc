...
enum ExtensionId : int {  // The first valid value is 1.
  ...
  kGenericDescriptorIdV00,
  kGenericDescriptorIdV01,
...
void RtpSenderVideoTest::PopulateGenericFrameDescriptor(int version) {
  ...
  const int ext_id =
      (version == 0) ? kGenericDescriptorIdV00 : kGenericDescriptorIdV01;
...
void RtpSenderVideoTest::
  ...
  if (version == 0) {
    ASSERT_TRUE(rtp_sender_.RegisterRtpHeaderExtension(
        RtpGenericFrameDescriptorExtension00::kUri, kGenericDescriptorIdV00));
  } else {
    ASSERT_TRUE(rtp_sender_.RegisterRtpHeaderExtension(
        RtpGenericFrameDescriptorExtension01::kUri, kGenericDescriptorIdV01));
  }
