...
void ReceiveStatisticsProxy::UpdateHistograms() {
  ...
  for (auto it : aggregated_stats) {
    ...
    if (e2e_delay_ms != -1) {
      ...
      RTC_LOG(LS_INFO) << uma_prefix << ".EndToEndDelayInMs"
                       << uma_suffix << " " << e2e_delay_ms;
    }
    ...
    if (e2e_delay_max_ms != -1 && e2e_delay_ms != -1) {
      ...
      RTC_LOG(LS_INFO) << uma_prefix << ".EndToEndDelayMaxInMs"
                       << uma_suffix << " " << e2e_delay_max_ms;
    }
    ...
    if (interframe_delay_ms != -1) {
      RTC_HISTOGRAM_COUNTS_SPARSE_10000(
          ...
          interframe_delay_ms);
      RTC_LOG(LS_INFO) << uma_prefix << ".InterframeDelayInMs"
                       << uma_suffix << " " << interframe_delay_ms;
    }
