...
@implementation RTC_OBJC_TYPE (RTCVideoTrack) {
  ...
  NSMutableArray *_adapters /* guarded by _worker_thread */;
}
...
- (instancetype)initWithFactory:(RTC_OBJC_TYPE(RTCPeerConnectionFactory) *)factory
  ...
  if (self = [super initWithFactory:factory nativeTrack:nativeMediaTrack type:type]) {
    ...
    _worker_thread = factory.workerThread;
  }
  return self;
}
...
- (void)addRenderer:(id<RTC_OBJC_TYPE(RTCVideoRenderer)>)renderer {
  if (!_worker_thread->IsCurrent()) {
    _worker_thread->Invoke<void>(RTC_FROM_HERE, [renderer, self] { [self addRenderer:renderer]; });
    return;
  }
...
- (void)removeRenderer:(id<RTC_OBJC_TYPE(RTCVideoRenderer)>)renderer {
  ...
  if (indexToRemove == NSNotFound) {
    RTC_LOG(LS_INFO) << "|removeRenderer| is called on unexpected RTC_OBJC_TYPE(RTCVideoRenderer)";
    return;
  }
