...
class TypedParser {
 public:
  ...
  static bool Changed(const void* src, const void* base);
...
// parsers as needed. Gennerally, try to use these suggested types event if the
...
void AddMembers(MemberParameter* out, const char* key, T* member) {
  *out = MemberParameter{
      key, member,
      TypedMemberParser{&TypedParser<T>::Parse, &TypedParser<T>::Changed,
...
class StructParametersParser {
 public:
  ...
  static std::unique_ptr<StructParametersParser> Create(S* target,
                                                        const char* first_key,
                                                        T* first_member,
                                                        Args... args) {
    static const S* defauts = new S();
    std::vector<struct_parser_impl::MemberParameter> members(
        sizeof...(args) / 2 + 1);
    struct_parser_impl::AddMembers(&members.front(), std::move(first_key),
                                   first_member, args...);
    return absl::WrapUnique(
        new StructParametersParser(defauts, target, std::move(members)));
  }
