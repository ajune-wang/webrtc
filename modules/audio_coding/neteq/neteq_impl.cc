...
std::unique_ptr<NetEqController> CreateNetEqController(
  ...
  config.neteq_facade = std::make_unique<NetEqFacadeImpl>(
      packet_buffer, buffer_level_filter, decoder_database,
    ...
    : clock(clock),
      ...
      neteq_controller(
          CreateNetEqController(config.min_delay_ms,
                                ...
                                tick_timer.get(),
                                packet_buffer.get(),
                                buffer_level_filter.get(),
                                decoder_database.get(),
                                stats.get())),
...
int NetEqImpl::InsertPacketInternal(const RTPHeader& rtp_header,
  ...
  neteq_controller_->LastDecodedWasCngOrDtmf(
...
int NetEqImpl::GetDecision(Operations* operation,
  ...
  *operation = kUndefined;
  ...
  if (packet) {
    packet_info = {packet->timestamp,
                   packet->frame ? packet->frame->IsDtxPacket() : false,
  ...
  *operation = neteq_controller_->GetDecision(
      ...
      decoder_frame_length_, packet_info,
      buffer_level_filter_->filtered_current_level(), last_mode_, *play_dtmf,
      generated_noise_samples, &reset_decoder_);
