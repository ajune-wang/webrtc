...
bool RtpDependencyDescriptorWriter::Write() {
  // Check is preparation was successful. It is not safe to try to write
  // if it wasn't.
  if (build_failed_)
    return false;
...
bool RtpDependencyDescriptorWriter::FindBestTemplate() {
...
void RtpDependencyDescriptorWriter::WriteBits(uint64_t value, size_t bits) {
  if (!bit_writer_.WriteBits(value, bits))
    build_failed_ = true;
}
