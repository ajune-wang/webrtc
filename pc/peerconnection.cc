...
RTCError PeerConnection::ApplyRemoteDescription(
  ...
  if (IsUnifiedPlan()) {
    ...
    for (auto transceiver : transceivers_) {
      ...
      if (!content->rejected) {
        const auto& stream = content->media_description()->streams()[0];
        transceiver->internal()->receiver_internal()->SetSsrc(
            stream.first_ssrc());
      }
    }
...
void PeerConnection::CreateAudioReceiver(
  ...
  audio_receiver->SetSsrc(remote_sender_info.first_ssrc);
...
void PeerConnection::CreateVideoReceiver(
  ...
  video_receiver->SetSsrc(remote_sender_info.first_ssrc);
  auto receiver = RtpReceiverProxyWithInternal<RtpReceiverInternal>::Create(
