...
// the message and its length, so that we can recall previously parsed data.
class MessageBufferReader : public rtc::ByteBufferReader {
 public:
  ...
  const char* MessageData() const { return message_data_; }
  ...
  size_t MessageLength() const { return message_length_; }
  // Current offset in the message.
  size_t CurrentOffset() const { return message_length_ - Length(); }
