...
WebRtcVideoChannel::WebRtcVideoSendStream::ConfigureVideoEncoderSettings(
  ...
  if (CodecNamesEq(codec.name, kVp9CodecName)) {
    ...
    if (is_screencast) {
      ...
      const size_t num_ssrcs = parameters_.config.rtp.ssrcs.size();
      size_t num_spatial_layers =
          std::max<size_t>(std::min<size_t>(num_ssrcs, 3), 1);
      size_t num_temporal_layers = num_spatial_layers > 1 ? 3 : 1;
      vp9_settings.numberOfSpatialLayers =
          GetVp9SpatialLayersFromFieldTrial(num_spatial_layers);
      vp9_settings.numberOfTemporalLayers =
          GetVp9TemporalLayersFromFieldTrial(num_temporal_layers);
    }
