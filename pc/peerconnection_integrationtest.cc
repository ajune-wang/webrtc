...
class PeerConnectionWrapper : public webrtc::PeerConnectionObserver,
 ...
 public:
  ...
  ~PeerConnectionWrapper() {
    ...
    // TODO(nisse): The OnSourceDestroyed isn't quite enough. We would
    // need to
    // 1. Tell the FakePeriodicVideoSource to stop generating OnFrame
    //    callbacks, and ignore any further AddOrUpdateSink (missing).
    // 2. Call OnSourceDestroyed on the VideoTrackSource
    // 3. Destroy the FakePeriodicVideoSource.
    // 4. Drop our references to VideoTrackSource.
