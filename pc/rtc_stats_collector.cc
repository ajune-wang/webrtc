...
void RTCStatsCollector::GetStatsReportInternal(
  ...
  if (cached_report_ &&
    ...
    signaling_thread_->PostTask(ToQueuedTask(
        [self = rtc::scoped_refptr<RTCStatsCollector>(this),
         cached_report = cached_report_, requests = std::move(requests_)] {
          self->DeliverCachedReport(cached_report, std::move(requests));
          return true;
        }));
    // Reset moved-from member to known state.
    requests_.clear();
