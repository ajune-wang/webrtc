...
TEST_P(SvcRateAllocatorTestParametrizedContentType, PaddingBitrate) {
  ...
  uint32_t min_bitrate_bps = SvcRateAllocator::GetPaddingBitrateBps(codec);

  VideoBitrateAllocation allocation =
      allocator.GetAllocation(min_bitrate_bps, 30);
  ...
  allocation = allocator.GetAllocation(min_bitrate_bps - 100, 30);
  EXPECT_GT(allocation.GetSpatialLayerSum(0), 0UL);
  EXPECT_GT(allocation.GetSpatialLayerSum(1), 0UL);
  ...
  min_bitrate_bps = SvcRateAllocator::GetPaddingBitrateBps(codec);
  allocation = allocator.GetAllocation(min_bitrate_bps, 30);
  EXPECT_GT(allocation.GetSpatialLayerSum(0), 0UL);
  EXPECT_GT(allocation.GetSpatialLayerSum(1), 0UL);
  EXPECT_EQ(allocation.GetSpatialLayerSum(2), 0UL);
