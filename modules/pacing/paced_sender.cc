...
void PacedSender::OnProbeClusterConfig(network::ProbeClusterConfig config) {
  ...
  prober_->CreateProbeCluster(bitrate_bps, config.time_created.ms());
}
...
int64_t PacedSender::ExpectedQueueTimeMs() const {
  if (queue_size_bytes_ == 0)
    return 0;
  RTC_DCHECK_GT(pacing_bitrate_kbps_.load(), 0);
...
int64_t PacedSender::TimeUntilNextProcess() {
  ...
  rtc::CritScope cs(&critsect_);
  auto ret = prober_->TimeUntilNextProbe(clock_->TimeInMilliseconds());
