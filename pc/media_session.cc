...
RTCError MediaSessionDescriptionFactory::AddAudioContentForOffer(
  ...
  if (!media_description_options.codec_preferences.empty()) {
    ...
    if (current_content && !current_content->rejected &&
      ...
      for (const Codec& codec : acd->codecs()) {
        if (FindMatchingCodec(acd->codecs(), audio_codecs, codec,
                              field_trials)) {
          filtered_codecs.push_back(codec);
        }
      }
  ...
  if (!media_description_options.codec_preferences.empty()) {
    ...
    if (current_content && !current_content->rejected &&
      ...
      for (const VideoCodec& codec : vcd->codecs()) {
        if (FindMatchingCodec(vcd->codecs(), video_codecs, codec,
                              field_trials)) {
          filtered_codecs.push_back(codec);
        }
      }
...
RTCError MediaSessionDescriptionFactory::AddVideoContentForAnswer(
  ...
  if (!media_description_options.codec_preferences.empty()) {
    ...
    if (current_content && !current_content->rejected &&
      ...
      const VideoContentDescription* vcd =
          current_content->media_description()->as_video();
      for (const Codec& codec : vcd->codecs()) {
        if (FindMatchingCodec(vcd->codecs(), video_codecs, codec,
                              field_trials)) {
          filtered_codecs.push_back(codec);
        }
