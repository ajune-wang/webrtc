...
TEST_F(TestVp8Impl, DynamicSetRates) {
  ...
  rate_settings.bandwidth_allocation = DataRate::BitsPerSecond(kBitrateBps);
  EXPECT_CALL(
      *vpx,
      codec_enc_config_set(
          _, AllOf(Field(&vpx_codec_enc_cfg_t::rc_target_bitrate,
  ...
  rate_settings.bandwidth_allocation = DataRate::BitsPerSecond(kBitrateBps * 2);
  EXPECT_CALL(
  ...
  rate_settings.bandwidth_allocation =
      DataRate::BitsPerSecond((3 * kBitrateBps) / 2);
  EXPECT_CALL(
      *vpx,
      codec_enc_config_set(
          _, AllOf(Field(&vpx_codec_enc_cfg_t::rc_target_bitrate,
