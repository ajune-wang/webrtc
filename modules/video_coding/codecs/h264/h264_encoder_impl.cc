...
static void RtpFragmentize(EncodedImage* encoded_image,
  ...
  if (encoded_image->_size < required_size) {
    ...
    if (encoded_image->_size < required_size) {
      ...
      NLOG(LS_WARNING, "Encoding produced more bytes than the original image ",
...
int32_t H264EncoderImpl::InitEncode(const VideoCodec* codec_settings,
  ...
  if (WelsCreateSVCEncoder(&openh264_encoder_) != 0) {
    ...
    NLOG(LS_ERROR, "Failed to create OpenH264 encoder");
    RTC_DCHECK(!openh264_encoder_);
    ReportError();
  ...
  if (openh264_encoder_->InitializeExt(&encoder_params) != 0) {
    NLOG(LS_ERROR, "Failed to initialize OpenH264 encoder");
    Release();
    ReportError();
    return WEBRTC_VIDEO_CODEC_ERROR;
  }
...
int32_t H264EncoderImpl::Encode(const VideoFrame& input_frame,
  ...
  if (!encoded_image_callback_) {
    NLOG(LS_WARNING, "InitEncode() has been called, but a callback function ",
  ...
  if (enc_ret != 0) {
    NLOG(LS_ERROR, "OpenH264 frame encoding failed, EncodeFrame returned ",
         enc_ret, ".");
    ReportError();
    return WEBRTC_VIDEO_CODEC_ERROR;
  }
...
SEncParamExt H264EncoderImpl::CreateEncoderParams() const {
  ...
  NLOG(INFO, "OpenH264 version is ", OPENH264_MAJOR, ".", OPENH264_MINOR);
