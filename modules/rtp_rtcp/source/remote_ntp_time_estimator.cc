...
RemoteNtpTimeEstimator::EstimateRemoteToLocalClockOffsetMs() {
  absl::optional<int64_t> offset = EstimateRemoteToLocalClockOffset();
  if (!offset.has_value()) {
    return absl::nullopt;
  }
  // DivideRoundToNearest works only for positive numbers.
  if (*offset >= 0) {
    return DivideRoundToNearest(*offset * 1'000, int64_t{1} << 32);
  } else {
    return -DivideRoundToNearest(-*offset * 1'000, int64_t{1} << 32);
  }
}
