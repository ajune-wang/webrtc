...
static void dump(const void *void_data, unsigned int size) {
	enum {
		bytes_per_line = 16
	};
	const unsigned char *data = (const unsigned char*) (void_data);
	...
	*end_ascii_dump = 0;
	...
	for (int line_bytes = 0; data < data_end; line_bytes += bytes_per_line) {
...
void DataChannel::OnMessage(const webrtc::DataBuffer &buffer) {
	...
	if (buffer.data.size() > 0) {
		...
		if (*data != _content) {
			std::cout << "    DataChannel::" << __func__ << " ################### DATA MISMATCH " << _label
			        << " EXPECTED: 0x" << std::hex << std::setfill('0') << std::setw(2) << (int) _content
					<< " RECEIVED: 0x" << std::hex << std::setfill('0') << std::setw(2) << (int) *data << " #################" << std::endl;
			dump(data, std::min(buffer.data.size(), (size_t)256));
