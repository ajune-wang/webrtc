...
ResourceListenerResponse VideoStreamAdapter::ApplyAdaptationTarget(
    ...
    int input_fps) {
  const int min_pixels_per_frame = MinPixelsPerFrame(encoder_settings);
  AdaptationRequest::Mode adaptation_direction =
  ...
  last_adaptation_request_.emplace(
      AdaptationRequest{input_pixels, input_fps, adaptation_direction});
  // Adapt!
  ...
  // instead.
  if (adaptation_direction == AdaptationRequest::Mode::kAdaptUp &&
      EffectiveDegradationPreference(input_mode) ==
          DegradationPreference::BALANCED &&
      target.action ==
