...
int32_t QualityAnalyzingVideoDecoder::Decode(
  ...
  if (out.discard) {
    // All encoded images were discarded, so black frame have to be returned.
    // Because simulcast streams will be received by receiver as 3 different
    // independent streams we don't want that irrelevant streams affect video
    // quality metrics and also we don't want to use CPU time to decode them
    // to prevent regressions on relevant streams. Also we can't just drop
    // frame, because in such case, receiving part will be confused with all
    // frames missing and will request a key frame, which will result into extra
    // load on network and sender side. Because of it, discarded image will be
    // always decoded as black frame and will be passed to callback directly
    // without reaching decoder and video quality analyzer.
    //
