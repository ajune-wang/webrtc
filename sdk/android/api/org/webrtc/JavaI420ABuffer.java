...
public class JavaI420ABuffer implements VideoFrame.I420ABuffer {
  ...
  public VideoFrame.Buffer cropAndScale(
    ...
    if (cropWidth == scaleWidth && cropHeight == scaleHeight) {
      ByteBuffer newDataY = yuvImage.getDataY().slice();
      newDataY.position(cropX + cropY * yuvImage.getStrideY());

      ByteBuffer newDataU = yuvImage.getDataU().slice();
      newDataU.position(cropX / 2 + cropY / 2 * yuvImage.getStrideU());

      ByteBuffer newDataV = yuvImage.getDataV().slice();
      newDataV.position(cropX / 2 + cropY / 2 * yuvImage.getStrideV());
