...
void RtcEventLogEncoderNewFormat::EncodeGenericAcksReceived(
  ...
  absl::optional<uint64_t> base_receive_timestamp = 0;
  if (base_event->receive_timestamp_ms()) {
    int64_t receive_timestamp_ms = base_event->receive_timestamp_ms().value();
    base_receive_timestamp = ToUnsigned(receive_timestamp_ms);
    proto_batch->set_receive_timestamp_ms(receive_timestamp_ms);
  }
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    const RtcEventGenericAckReceived* event = batch[i + 1];
    values[i] = event->receive_timestamp_ms();
  }
