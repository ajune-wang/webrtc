...
uint16_t DefaultVideoQualityAnalyzer::OnFrameCaptured(
  ...
  }
  {
    ...
    if (it != captured_frames_in_flight_.end()) {
      ...
      RTC_DCHECK(frame_id == stream_frame_id_list_[stream_label].front());
      stream_frame_id_list_[stream_label].pop_front();
...
void DefaultVideoQualityAnalyzer::ReportResults() const {
  ...
  rtc::CritScope crit2(&comparison_lock_);
  for (auto& item : stream_stats_) {
    const std::string& stream_label = item.first;
    const StreamStats& stats = item.second;
    ReportResult("psnr", stream_label, stats.psnr, "dB");
    ReportResult("ssim", stream_label, stats.ssim, "unitless");
    ...
    test::PrintResult("encode_frame_rate", "", GetTestCaseName(stream_label),
    ...
    ReportResult("pixels_per_frame", stream_label,
                 stats.resolution_of_encoded_image, "unitless");
    test::PrintResult("min_psnr", "", GetTestCaseName(stream_label),
                      stats.psnr.IsEmpty() ? 0 : stats.psnr.GetMin(), "dB",
                      /*important=*/false);
    ReportResult("decode_time", stream_label, stats.decode_time_ms, "ms");
    test::PrintResult("dropped_frames", "", GetTestCaseName(stream_label),
                      stream_frame_counters_.at(stream_label).dropped,
                      "unitless",
                      /*important=*/false);
    test::PrintResult("freeze_200ms", "", GetTestCaseName(stream_label),
                      stats.freeze_200_ms, "ms", /*important=*/false);
    test::PrintResult("freeze_1s", "", GetTestCaseName(stream_label),
                      stats.freeze_1s, "ms", /*important=*/false);
