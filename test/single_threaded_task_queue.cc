...
void DEPRECATED_SingleThreadedTaskQueueForTesting::RunLoop() {
  ...
  while (true) {
...

    {
      ...
      if (!tasks_.empty()) {
        auto first_stored_task = tasks_.begin();
        int64_t earliest_exec_time = first_stored_task->first;
        int64_t remaining_delay_ms =
            rtc::TimeDiff(earliest_exec_time, rtc::TimeMillis());
        if (remaining_delay_ms <= 0) {
          queued_task = std::move(first_stored_task->second.task);
          tasks_.erase(first_stored_task);
        } else {
