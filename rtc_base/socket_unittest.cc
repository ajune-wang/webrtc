...
void SocketTest::SocketRecvTimestamp(const IPAddress& loopback) {
  ...
  char buffer[3];
  EXPECT_EQ(3, socket->RecvFrom(buffer, 3, nullptr, &recv_timestamp_1));
  // Ensure receive timestamp has the same epoch as TimeMicros to ensure
  // there are is huge time diffs between to received packets if the connection
  ...
  EXPECT_EQ(3, socket->RecvFrom(buffer, 3, nullptr, &recv_timestamp_2));
