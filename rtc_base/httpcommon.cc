...
HttpAuthResult HttpAuthenticate(const char* challenge,
  ...
  // BASIC
  if (absl::EqualsIgnoreCase(auth_method.c_str(), "basic") == 0) {
    if (context)
  ...
  // DIGEST
  if (absl::EqualsIgnoreCase(auth_method.c_str(), "digest") == 0) {
    if (context)
      return HAR_CREDENTIALS;  // Bad credentials
    if (username.empty())
      return HAR_CREDENTIALS;  // Missing credentials
...
#if 1
  bool want_negotiate =
      (absl::EqualsIgnoreCase(auth_method.c_str(), "negotiate") == 0);
  bool want_ntlm = (absl::EqualsIgnoreCase(auth_method.c_str(), "ntlm") == 0);
  // SPNEGO & NTLM
