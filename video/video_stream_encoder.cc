...
void VideoStreamEncoder::ReconfigureEncoder() {
  ...
  // TODO(nisse): Can we disenrtangle this? Seems unfortunate that we have to
  // query the stats machinery in order to compute the value passed to same
  // machinery. Need to figure out what VideoSenderInfo::preferred_bitrate is
  // used for, and which component can be respnsible for it.

  // Get the current actual framerate, as measured by the stats proxy. This is
  // used to get the correct bitrate layer allocation.
  int current_framerate = encoder_stats_observer_->GetSendFrameRate();
