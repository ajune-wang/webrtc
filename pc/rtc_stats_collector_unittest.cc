...
TEST_P(RTCStatsCollectorTestWithParamKind,
       RTCRemoteInboundRtpStreamStatsWithEnviromentTimestampFromReportBlock) {
  const Timestamp kReportBlockTimestampUtc = Timestamp::Micros(123456789);
  const Timestamp kReportBlockTimestamp = Timestamp::Micros(12345678);
  fake_clock_.SetTime(kReportBlockTimestamp);
  pc_.GetConfiguration().set_stats_timestamp_with_enviroment_clock(true);
  ...
  // back to default
  pc_.GetConfiguration().set_stats_timestamp_with_enviroment_clock(false);

  // Check if enviroment timestamp was used to crete the
  // RTCRemoteInboundRtpStreamStats object
