...
rtc::StringBuilder& operator<<(rtc::StringBuilder& builder,
                               const std::vector<RidDescription>& list) {
  bool first = true;
  for (const RidDescription& rid : list) {
    if (!first) {
...
RTCErrorOr<SimulcastLayerList> ParseSimulcastLayerList(const std::string& str) {
  ...
  for (const auto& token : tokens) {
    ...
    result.AddLayerWithAlternatives(rids);
  }
...
RTCErrorOr<SimulcastDescription> SdpSerializer::DeserializeSimulcastDescription(
  ...
  if (list1.ok()) {
    simulcast.send_layers() = list1.value();
  }

  if (list2.ok()) {
    simulcast.receive_layers() = list2.value();
  }
