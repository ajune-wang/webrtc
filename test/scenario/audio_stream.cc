...
absl::optional<std::string> CreateAdaptationString(
  ...
  if (config.frame.max_rate_for_60_ms.IsFinite()) {
    ...
    auto min_rate_for_20_ms = config.frame.min_rate_for_20_ms;
    controller->set_fl_20ms_to_60ms_bandwidth_bps(min_rate_for_20_ms.bps());
    auto max_rate_for_60_ms = config.frame.max_rate_for_60_ms;
    controller->set_fl_60ms_to_20ms_bandwidth_bps(max_rate_for_60_ms.bps());
    if (config.frame.max_rate_for_120_ms.IsFinite()) {
      auto min_rate_for_60_ms = config.frame.min_rate_for_60_ms;
      controller->set_fl_60ms_to_120ms_bandwidth_bps(min_rate_for_60_ms.bps());
      auto max_rate_for_120_ms = config.frame.max_rate_for_120_ms;
      controller->set_fl_120ms_to_60ms_bandwidth_bps(max_rate_for_120_ms.bps());
    }
  }
  cont_conf.add_controllers()->mutable_bitrate_controller();
  std::string config_string;
  cont_conf.SerializeToString(&config_string);
