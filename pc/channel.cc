...
bool BaseChannel::ConnectToRtpTransport() {
  ...
  if (!media_transport_) {
    RTC_LOG(LS_INFO) << "media_transport_ is not present, connecting "
                        "SignalNetworkRouteChanged to RTP transport";
...
void BaseChannel::Init_w(webrtc::RtpTransportInternal* rtp_transport,
  ...
  RTC_LOG(LS_INFO) << "Called Init_w";
  if (media_transport_) {
    RTC_LOG(LS_INFO)
        << "Adding network change callback on media_transport_ interface";
    media_transport_->SetNetworkChangeCallback(this);
  } else {
    RTC_LOG(LS_INFO)
        << "NOT adding network change callback on media_transport_ interface";
  }
}
