...
void AdaptiveModeLevelEstimatorAgcInterface::Process(const int16_t* audio,
  ...
  speech_level_estimator_.UpdateEstimation(vad_prob);
}
...
bool AdaptiveModeLevelEstimatorAgcInterface::GetRmsErrorDb(int* error) {
  *error = std::floor(target_level_dbfs_ -
                      speech_level_estimator_.LatestLevelEstimate() + 0.5);
  auto result = speech_level_estimator_.CurrentMemory() >=
                kVadConfidenceThreshold / (1.f - kFullBufferLeakFactor) * 0.5;
  if (result) {
    speech_level_estimator_.ResetMemory();
  }
  return result;
}
