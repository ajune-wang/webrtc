/*
 *  Copyright 2018 The WebRTC project authors. All Rights Reserved.
 *
 *  Use of this source code is governed by a BSD-style license
 *  that can be found in the LICENSE file in the root of the source
 *  tree. An additional intellectual property rights grant can be found
 *  in the file PATENTS.  All contributing project authors may
 *  be found in the AUTHORS file in the root of the source tree.
 */
#include "modules/congestion_controller/bbr/loss_rate_filter.h"
#include "test/gtest.h"

namespace webrtc {
namespace bbr {
namespace {
const TimeDelta kTestFilterTime = TimeDelta::seconds(5);
const Timestamp kTestStartTime = Timestamp::seconds(100000);
}  // namespace
TEST(LossRateFilterTest, AccumulatesToOne) {
  LossRateFilter filter(kTestFilterTime);
  Timestamp current_time = kTestStartTime;
  for (int i = 0; i < 100; i++) {
    filter.UpdateWithLossStatus(current_time, true);
    current_time += TimeDelta::seconds(1);
  }
  EXPECT_NEAR(filter.GetLossRate(), 1.0, 0.01);
}

TEST(LossRateFilterTest, StaysAtZero) {
  LossRateFilter filter(kTestFilterTime);
  Timestamp current_time = kTestStartTime;
  for (int i = 0; i < 100; i++) {
    filter.UpdateWithLossStatus(current_time, false);
    current_time += TimeDelta::seconds(1);
  }
  EXPECT_NEAR(filter.GetLossRate(), 0.0, 0.01);
}

TEST(LossRateFilterTest, VariesWithInput) {
  LossRateFilter filter(kTestFilterTime);
  Timestamp current_time = kTestStartTime;
  for (int j = 0; j < 10; j++) {
    for (int i = 0; i < 20; i++) {
      filter.UpdateWithLossStatus(current_time, true);
      current_time += TimeDelta::seconds(1);
    }
    EXPECT_NEAR(filter.GetLossRate(), 1.0, 0.1);
    for (int i = 0; i < 20; i++) {
      filter.UpdateWithLossStatus(current_time, false);
      current_time += TimeDelta::seconds(1);
    }
    EXPECT_NEAR(filter.GetLossRate(), 0.0, 0.1);
  }
}

TEST(LossRateFilterTest, FiltersOutlier) {
  LossRateFilter filter(kTestFilterTime);
  Timestamp current_time = kTestStartTime;
  for (int i = 0; i < 10; i++) {
    current_time += TimeDelta::seconds(1);
    filter.UpdateWithLossStatus(current_time, false);
  }
  current_time += TimeDelta::ms(10);
  filter.UpdateWithLossStatus(current_time, true);
  EXPECT_NEAR(filter.GetLossRate(), 0.0, 0.01);
}

}  // namespace bbr
}  // namespace webrtc
