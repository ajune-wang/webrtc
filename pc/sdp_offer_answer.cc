...
class CreateSessionDescriptionObserverLastDescriptionWrapper
    : public CreateSessionDescriptionObserver {
 public:
  CreateSessionDescriptionObserverLastDescriptionWrapper(
...
void SdpOfferAnswerHandler::DoCreateOffer(
  ...
  auto observer_wrapper = rtc::make_ref_counted<
      ...
      [this](const SessionDescriptionInterface* desc) {
        RTC_DCHECK_RUN_ON(signaling_thread());
        if (desc) {
          last_created_offer_.reset(desc->Clone().release());
        }
      },
      std::move(observer));
...
void SdpOfferAnswerHandler::DoCreateAnswer(
  ...
  auto observer_wrapper = rtc::make_ref_counted<
      ...
      [this](const SessionDescriptionInterface* desc) {
        RTC_DCHECK_RUN_ON(signaling_thread());
        if (desc) {
          last_created_answer_.reset(desc->Clone().release());
        }
      },
      std::move(observer));
...
RTCError SdpOfferAnswerHandler::ValidateSdpMunging(
  ...
  RTC_DCHECK_EQ(sdesc->GetType(), last_created_desc->GetType());
