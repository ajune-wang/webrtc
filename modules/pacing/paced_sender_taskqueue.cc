...
void PacedSenderTaskQueue::MaybeProcessPackets(bool is_probe) {
  ...
  if (next_scheduled_process_.value_or(now) <= now) {
    next_scheduled_process_.reset();
  }

  Timestamp next_process_time = now + time_to_next_process;
  if (process_tasks_in_flight_ == 0 || probe_started_ ||
      !next_scheduled_process_ ||
      *next_scheduled_process_ > next_process_time) {
    next_scheduled_process_ = next_process_time;
