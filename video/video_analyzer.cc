...
void VideoAnalyzer::PrintResults() {
  ...
  const uint32_t sum_squared_frame_durations_ms =
      1000 * sum_squared_frame_durations_;
  if (sum_squared_frame_durations_ms > 0) {
    test::PrintResult(
        "harmonic_frame_rate", "", test_label_.c_str(),
        total_freezes_duration_ms_double / sum_squared_frame_durations_ms, "",
        false);
  }

  // test::PrintResult("xxx", "", test_label_.c_str(), xxx,
  //                   "unit", false);
  // test::PrintResult("xxx", "", test_label_.c_str(), xxx,
  //                   "unit", false);
  // test::PrintResult("xxx", "", test_label_.c_str(), xxx,
