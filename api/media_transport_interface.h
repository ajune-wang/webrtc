...
class MediaTransportEncodedAudioFrame {
 public:
  ...
  int32_t sequence_number() const { return sequence_number_; }
...
class MediaTransportInterface {
 public:
  ...
  // Start asyncronous send of audio frame.
  virtual RTCError SendAudioFrame(uint64_t channel_id,
                                  MediaTransportEncodedAudioFrame frame) = 0;

  // Registers audio sink. Only one audio sink can be registered at a time.
  // Sink should be unregistered by the time media transport is destroyed.
  virtual RTCError RegisterAudioSink(
      MediaTransportAudioSinkInterface* sink) = 0;

  // Unregisters audio sink.
