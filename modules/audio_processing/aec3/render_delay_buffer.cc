...
void RenderDelayBufferImpl::Reset() {
  ...
  ds_render_.next_read =
      ds_render_.OffsetIndex(ds_render_.last_insert, offset3);
  blocks_.next_read = blocks_.OffsetIndex(blocks_.last_insert, -offset2);
  spectra_.next_read = spectra_.OffsetIndex(spectra_.last_insert, offset2);
  ffts_.next_read = ffts_.OffsetIndex(ffts_.last_insert, offset2);
  render_surplus_ = 0;
