...
int32_t H264DecoderImpl::Decode(const EncodedImage& input_image,
  ...
  int result = avcodec_decode_video2(av_context_.get(),
                                     ...
                                     &packet);
  if (result < 0) {
    RTC_LOG(LS_ERROR) << "avcodec_decode_video2 error: " << result;
    ReportError();
    return WEBRTC_VIDEO_CODEC_ERROR;
  }
  // |result| is number of bytes used, which should be all of them.
  if (result != packet.size) {
    RTC_LOG(LS_ERROR) << "avcodec_decode_video2 consumed " << result
                      << " bytes when " << packet.size
                      << " bytes were expected.";
    ReportError();
    return WEBRTC_VIDEO_CODEC_ERROR;
  }
  ...
  if (!frame_decoded) {
    RTC_LOG(LS_WARNING) << "avcodec_decode_video2 successful but no frame was "
        "decoded.";
    return WEBRTC_VIDEO_CODEC_OK;
  }
