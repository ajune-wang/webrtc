...
TestAudioDeviceModule::CreateWavFileReader(std::string filename,
                                           int sampling_frequency_in_hz,
                                           int num_channels) {
  return std::unique_ptr<TestAudioDeviceModule::Capturer>(new WavFileReader(
      filename, sampling_frequency_in_hz, num_channels, false));
}
...
TestAudioDeviceModule::CreateWavFileReader(std::string filename, bool repeat) {
  ...
  return std::unique_ptr<TestAudioDeviceModule::Capturer>(new WavFileReader(
      filename, sampling_frequency_in_hz, num_channels, repeat));
}
...
TestAudioDeviceModule::CreateWavFileReader(rtc::PlatformFile file,
  ...
  return std::unique_ptr<TestAudioDeviceModule::Capturer>(
      new WavFileReader(file, sampling_frequency_in_hz, num_channels, false));
}
...
TestAudioDeviceModule::CreateWavFileReader(rtc::PlatformFile file,
  ...
  int num_channels = rtc::checked_cast<int>(reader.num_channels());
  return std::unique_ptr<TestAudioDeviceModule::Capturer>(
      new WavFileReader(file, sampling_frequency_in_hz, num_channels, repeat));
}
