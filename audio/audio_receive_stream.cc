...
AudioReceiveStream::AudioReceiveStream(
    ...
    : config_(config),
      ...
      audio_device_module_(static_cast<internal::AudioState*>(audio_state.get())
                               ->audio_device_module()),
      output_gain_(1.0f),
...
AudioReceiveStream::~AudioReceiveStream() {
  ...
  // Resets the delegate's callback to OnReceivedPayloadData.
...
void AudioReceiveStream::OnRtpPacket(const RtpPacketReceived& packet) {
  ...
  size_t packet_length = packet_copy.size();
  const uint8_t* payload = packet_copy.data() + header.headerLength;
