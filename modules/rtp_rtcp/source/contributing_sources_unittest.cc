...
TEST(ContributingSourcesTest, UpdateSources) {
  ContributingSources csrcs;
  // TODO(nisse): This is clumsy, we should really have a nice way to construct
  // an array view with an in-place array literal.
  const uint32_t kCsrcs1[] = {kCsrc1, kCsrc2};
  const uint32_t kCsrcs2[] = {kCsrc2, kCsrc3};
  const int64_t kTime1 = 10;
...
TEST(ContributingSourcesTest, PurgeOldSources) {
  ...
  const uint32_t kCsrcs2[] = {kCsrc2, kCsrc3};
  const int64_t kTime1 = 10;
  const int64_t kTime2 = kTime1 + 10 * rtc::kNumMillisecsPerSec;
  const int64_t kTime3 = kTime1 + 20 * rtc::kNumMillisecsPerSec;
  csrcs.Update(kTime1, kCsrcs1);
  EXPECT_THAT(
      csrcs.GetSources(kTime2),
      UnorderedElementsAre(RtpSource(kTime1, kCsrc1, RtpSourceType::CSRC),
