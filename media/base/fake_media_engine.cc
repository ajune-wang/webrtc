...
bool FakeVoiceMediaReceiveChannel::SetLocalSource(uint32_t ssrc,
                                                  AudioSource* source) {
...
bool FakeVideoMediaSendChannel::rendering() const {
  return playout();
}
...
FakeVideoMediaSendChannel::sinks() const {
  return sinks_;
}
...
FakeVoiceEngine::CreateSendChannel(webrtc::Call* call,
  ...
  std::unique_ptr<FakeVoiceMediaSendChannel> ch =
      std::make_unique<FakeVoiceMediaSendChannel>(this, options,
                                                  call->network_thread());
  return ch;
}
...
FakeVoiceEngine::CreateReceiveChannel(
  ...
  std::unique_ptr<FakeVoiceMediaReceiveChannel> ch =
      std::make_unique<FakeVoiceMediaReceiveChannel>(this, options,
                                                     call->network_thread());
  return ch;
}
FakeVoiceMediaSendChannel* FakeVoiceEngine::GetSendChannel(size_t index) {
  return (send_channels_.size() > index) ? send_channels_[index].get()
                                         : nullptr;
}
FakeVoiceMediaReceiveChannel* FakeVoiceEngine::GetReceiveChannel(size_t index) {
  return (receive_channels_.size() > index) ? receive_channels_[index].get()
                                            : nullptr;
}
void FakeVoiceEngine::RegisterChannel(
    std::unique_ptr<FakeVoiceMediaReceiveChannel> channel) {
  receive_channels_.emplace_back(std::move(channel));
}
void FakeVoiceEngine::RegisterChannel(
    std::unique_ptr<FakeVoiceMediaSendChannel> channel) {
  send_channels_.emplace_back(std::move(channel));
}
void FakeVoiceEngine::UnregisterChannel(
    VoiceMediaReceiveChannelInterface* channel) {
  receive_channels_.erase(absl::c_find_if(
      receive_channels_, [&](const auto& o) { return o.get() == channel; }));
}
void FakeVoiceEngine::UnregisterChannel(
    VoiceMediaSendChannelInterface* channel) {
  send_channels_.erase(absl::c_find_if(
      send_channels_, [&](const auto& o) { return o.get() == channel; }));
}
...
FakeVideoEngine::CreateSendChannel(
  ...
  std::unique_ptr<FakeVideoMediaSendChannel> ch =
      std::make_unique<FakeVideoMediaSendChannel>(this, options,
                                                  call->network_thread());
  return ch;
}
...
FakeVideoEngine::CreateReceiveChannel(
  ...
  std::unique_ptr<FakeVideoMediaReceiveChannel> ch =
      std::make_unique<FakeVideoMediaReceiveChannel>(this, options,
                                                     call->network_thread());
  return ch;
}
FakeVideoMediaSendChannel* FakeVideoEngine::GetSendChannel(size_t index) {
  return (send_channels_.size() > index) ? send_channels_[index].get()
                                         : nullptr;
}
FakeVideoMediaReceiveChannel* FakeVideoEngine::GetReceiveChannel(size_t index) {
  return (receive_channels_.size() > index) ? receive_channels_[index].get()
                                            : nullptr;
}
void FakeVideoEngine::RegisterChannel(
    std::unique_ptr<FakeVideoMediaSendChannel> channel) {
  send_channels_.emplace_back(std::move(channel));
}
void FakeVideoEngine::RegisterChannel(
    std::unique_ptr<FakeVideoMediaReceiveChannel> channel) {
  receive_channels_.emplace_back(std::move(channel));
}
void FakeVideoEngine::UnregisterChannel(
    VideoMediaSendChannelInterface* channel) {
  send_channels_.erase(absl::c_find_if(
      send_channels_, [&](const auto& o) { return o.get() == channel; }));
}
void FakeVideoEngine::UnregisterChannel(
    VideoMediaReceiveChannelInterface* channel) {
  receive_channels_.erase(absl::c_find_if(
      receive_channels_, [&](const auto& o) { return o.get() == channel; }));
}
...
FakeVoiceMediaSendChannel* FakeMediaEngine::GetVoiceSendChannel(size_t index) {
  return voice_->GetSendChannel(index);
}
FakeVideoMediaSendChannel* FakeMediaEngine::GetVideoSendChannel(size_t index) {
  return video_->GetSendChannel(index);
}
FakeVoiceMediaReceiveChannel* FakeMediaEngine::GetVoiceReceiveChannel(
    size_t index) {
  return voice_->GetReceiveChannel(index);
}
FakeVideoMediaReceiveChannel* FakeMediaEngine::GetVideoReceiveChannel(
    size_t index) {
