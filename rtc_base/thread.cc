...
void Thread::Send(const Location& posted_from,
...
#if RTC_DCHECK_IS_ON
  {
    CritScope cs(&current_thread->crit_);
    current_thread->send_targets_.insert(this);
  }
#endif
  bool ready = false;
  PostTask(
  ...
  crit_.Enter();
#if RTC_DCHECK_IS_ON
  // Check for send looops to avoid deadlock. As a common case optimization, we
  // only do this it this thread currently are sending to any other thread.
  if (!send_targets_.empty()) {
    RTC_DCHECK_EQ(send_targets_.count(current_thread), 0)
        << " send loop between " << current_thread->name() << " and "
        << this->name();
    std::deque<Thread*> targets(send_targets_.begin(), send_targets_.end());
    for (auto it = targets.begin(); it != targets.end(); ++it) {
      CritScope cs(&(*it)->crit_);
      RTC_DCHECK_EQ((*it)->send_targets_.count(current_thread), 0)
          << " send loop between " << current_thread->name() << " and "
          << this->name() << " via " << (*it)->name();
      targets.insert(targets.end(), (*it)->send_targets_.begin(),
                     (*it)->send_targets_.end());
    }
  }
#endif
