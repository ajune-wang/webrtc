 ...
 public:
  struct Event {
   public:
    ...
    virtual Type type() = 0;
    virtual int64_t get_timestamp_ms() = 0;
    virtual ~Event() = default;
  };

  struct PacketData : public Event {
    PacketData();
    ~PacketData();
    Type type() override { return Type::kPacketData; }
    int64_t get_timestamp_ms() override { return time_ms; }
  ...
  struct SetMinimumDelay : public Event {
    ...
    int64_t get_timestamp_ms() override { return timestamp_ms; }
    int64_t timestamp_ms;
    int delay_ms;
  };

  struct GetAudio : public Event {
  ...
  virtual const std::unique_ptr<Event>& NextEvent() const = 0;
...
class TimeLimitedNetEqInput : public NetEqInput {
 public:
  ...
  virtual std::unique_ptr<Event> PopEvent() override;
 ...
 private:
  ...
  bool ended_ = false;
  std::unique_ptr<Event> empty_event_;
};
