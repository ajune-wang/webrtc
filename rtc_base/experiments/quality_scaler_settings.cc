...
QualityScalerSettings::QualityScalerSettings(
    ...
    : min_frames_("min_frames"),
      ...
      fast_scale_factor_("fast_scale_factor") {
  ParseFieldTrial(
      {&min_frames_, &scale_factor_, &fast_scale_factor_},
...
absl::optional<double> QualityScalerSettings::FastScaleFactor() const {
  if (fast_scale_factor_ && fast_scale_factor_.Value() < kMinScaleFactor) {
    RTC_LOG(LS_WARNING) << "Unsupported fast_scale_factor value, ignored.";
    return absl::nullopt;
  }
  return fast_scale_factor_.GetOptional();
}
