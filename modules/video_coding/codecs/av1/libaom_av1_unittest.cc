...
std::unique_ptr<VideoEncoder> CreateEncoder(
  ...
  std::unique_ptr<VideoEncoder> encoder = CreateLibaomAv1Encoder(controller);
  if (encoder == nullptr) {
    return encoder;
  }
  ...
  VideoEncoder::Settings encoder_settings(
      ...
      /*number_if_cores=*/1, /*max_payload_size=*/1200);
...
TEST(LibaomAv1Test, EncodeDecode) {
  ...
  NiceMock<MockEncodedImageCallback> on_encoded;
  // enumerate frames for nicer error message when test fails.
  int64_t frame_number = 0;
  ON_CALL(on_encoded, OnEncodedImage)
      .WillByDefault(WithArgs<0>([&](const EncodedImage& image) {
        client.Decode(++frame_number, image);
        return EncodedImageCallback::Result(EncodedImageCallback::Result::OK);
      }));
  std::unique_ptr<VideoEncoder> encoder = CreateEncoder(nullptr, &on_encoded);
...
class SvcEncoderCallback : public EncodedImageCallback {
 public:
  explicit SvcEncoderCallback(rtc::ArrayView<TestDecoder> decoders)
      : decoders_(decoders) {}
  ...
  Result OnEncodedImage(
    ...
    for (size_t i = 0; i < decoders_.size(); ++i) {
      if (dtis[i] != webrtc::DecodeTargetIndication::kNotPresent) {
        decoders_[i].Decode(frame_id_, encoded_image);
      }
    }
    return Result(Result::OK);
  }
 ...
 private:
  const rtc::ArrayView<TestDecoder> decoders_;
  // Count frame_id for nicer error message when test fails.
  int64_t frame_id_ = 0;
};
...
TEST_P(LibaomAv1SvcTest, EncodeAndDecodeAllDecodeTarget) {
  ...
  SvcEncoderCallback on_encoded(clients);
  std::unique_ptr<VideoEncoder> encoder =
      CreateEncoder(Controller(), &on_encoded);
  ASSERT_THAT(encoder, NotNull());

  // Act
  std::vector<VideoFrameType> frame_types = {VideoFrameType::kVideoFrameDelta};
  for (const VideoFrame& frame : CreateFrames(12)) {
    encoder->Encode(frame, &frame_types);
  }
