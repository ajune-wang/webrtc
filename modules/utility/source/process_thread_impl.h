 ...
 private:
  ...
  struct DelayedTask {
    DelayedTask(int64_t run_at_ms,
        ...
        : run_at_ms(run_at_ms), seq_id_(seq_id), task(task.release()) {}
    friend bool operator<(const DelayedTask& lhs, const DelayedTask& rhs) {
      ...
      return lhs.run_at_ms > rhs.run_at_ms ||
             (lhs.run_at_ms == rhs.run_at_ms && lhs.seq_id_ > rhs.seq_id_);
    }
  ...
  // with the same wakeup time, use `seq_id_` to ensure FIFO ordering.
  std::priority_queue<DelayedTask> delayed_tasks_ RTC_GUARDED_BY(mutex_);
  uint32_t seq_id_ RTC_GUARDED_BY(mutex_) = 0;
