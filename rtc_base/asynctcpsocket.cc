...
AsyncSocket* AsyncTCPSocketBase::ConnectSocket(
  ...
  if (socket->Bind(bind_address) < 0) {
    NLOG(LS_ERROR, "Bind() failed with error ", socket->GetError());
    return nullptr;
  }
  if (socket->Connect(remote_address) < 0) {
    NLOG(LS_ERROR, "Connect() failed with error ", socket->GetError());
    return nullptr;
  }
...
AsyncTCPSocketBase::AsyncTCPSocketBase(AsyncSocket* socket, bool listen,
  ...
  if (listen_) {
    if (socket_->Listen(kListenBacklog) < 0) {
      NLOG(LS_ERROR, "Listen() failed with error ", socket_->GetError());
    }
  }
}
...
void AsyncTCPSocketBase::OnReadEvent(AsyncSocket* socket) {
  ...
  if (listen_) {
    ...
    if (!new_socket) {
      ...
      // to the user.
      NLOG(LS_ERROR, "TCP accept failed with error ", socket_->GetError());
      return;
    }
    ...
    while (true) {
      ...
      if (len < 0) {
        ...
        if (!socket_->IsBlocking()) {
          NLOG(LS_ERROR, "Recv() returned error: ", socket_->GetError());
        }
        break;
      }
    ...
    if (size > inbuf_.size()) {
      NLOG(LS_ERROR, "input buffer overflow");
      RTC_NOTREACHED();
      inbuf_.Clear();
    } else {
      inbuf_.SetSize(size);
