...
VideoBitrateAllocation SvcRateAllocator::Allocate(
  ...
  if (experiment_settings_.IsEnabled() &&
    ...
    size_t num_enabled_with_hysteresis =
        FindNumActiveLayers(parameters.stable_bitrate / hysteresis_factor);
    if (num_enabled_with_hysteresis >= last_active_layer_count_) {
      ...
      num_spatial_layers =
          std::min(last_active_layer_count_,
                   FindNumActiveLayers(parameters.stable_bitrate));
    }
  } else {
    num_spatial_layers = FindNumActiveLayers(parameters.total_bitrate);
  }
...
size_t SvcRateAllocator::FindNumActiveLayers(DataRate target_rate) const {
  if (cumulative_layer_start_bitrates_.empty()) {
    return 0;
  }

  // lower_bound finds the smallest element >= target_rate, but we want the
  // largest element <= target_rate. So if element is not equal, try to back
  // down one slot. If already at the first element, use that.
  auto it = cumulative_layer_start_bitrates_.lower_bound(target_rate);
  if (it == cumulative_layer_start_bitrates_.end()) {
    return cumulative_layer_start_bitrates_.rbegin()->second;
  }
  if (it != cumulative_layer_start_bitrates_.begin() &&
      it->first > target_rate) {
    --it;
  }
  return it->second;
}
...
DataRate SvcRateAllocator::GetPaddingBitrate(const VideoCodec& codec) {
  ...
  return start_bitrate.rbegin()->first;
}

std::map<DataRate, size_t> SvcRateAllocator::GetLayerStartBitrates(
    const VideoCodec& codec) {
  std::map<DataRate, size_t> start_bitrates;
  ...
  for (size_t i = 1; i <= num_layers; ++i) {
    ...
    start_bitrates[layer_toggling_rate] = i;
