...
const uint8_t kThreeQuestionsWithTwoNamePointers[] = {
    0x12, 0x34,                                // ID
    0x00, 0x00,                                // flags
...
bool GetQueriedNames(MDnsMessage* msg, std::set<std::string>* names) {
  if (!msg->IsQuery() || msg->question_section().size() == 0) {
    return false;
  }
...
bool GetResolution(MDnsMessage* msg,
  ...
  if (msg->IsQuery() || msg->answer_section().size() == 0) {
    return false;
  }
...
TEST(MDnsMessageTest, ReadThreeQuestionsWithTwoNamePointers) {
  MDnsMessage msg;
  ASSERT_TRUE(ReadMDnsMessage(&msg, kThreeQuestionsWithTwoNamePointers));
