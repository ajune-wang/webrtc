...
std::unique_ptr<StructParametersParser> DummyConfig::Parser() {
  ...
  return CreateStructParametersParser(this, "e", &enabled,  //
                                      "f", &factor,         //
                                      "r", &retries,        //
                                      "p", &ping,           //
                                      "h", &hash,           //
...
TEST(StructParametersParserTest, ParsesValidParameters) {
  DummyConfig exp;
  exp.Parser()->Parse("e:1,f:-1.7,r:2,p:1,h:x7c,d:8,l:,");
...
TEST(StructParametersParserTest, EmptyDefaults) {
  DummyConfig exp;
  auto encoded = exp.Parser()->EncodeChanged();
...
TEST(StructParametersParserTest, EncodeAll) {
  DummyConfig exp;
  auto encoded = exp.Parser()->EncodeAll();
  // All parameters are encoded.
  EXPECT_EQ(encoded, "e:false,f:0.5,r:5,p:false,h:a80,d:,l:100 ms");
}
...
TEST(StructParametersParserTest, EncodeChanged) {
  ...
  exp.retries = 4;
  auto encoded = exp.Parser()->EncodeChanged();
  // We expect the changed parameters to be encoded.
  EXPECT_EQ(encoded, "r:4,p:true");
}
