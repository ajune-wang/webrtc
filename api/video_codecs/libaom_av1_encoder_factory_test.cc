...
TEST(LibaomAv1Encoder, ReferenceOrderDoesNotMatter) {
  ...
  do {
    ...
    printf("Refs {%d, %d, %d}  PSNR %f\n", refs[0], refs[1], refs[2], psnrs.back());
...
TEST(LibaomAv1Encoder, BitrateConsistentAcrossSpatialLayers) {
  ...
  for (int sid = 0; sid < max_spatial_layers; ++sid) {
    ...
    double encode_bitrate = (total_size / total_duration).kbps<double>();
    double target_bitrate = kRate.target_bitrate.kbps<double>();

    EXPECT_NEAR(encode_bitrate, target_bitrate, target_bitrate * 0.05);
  }
}
