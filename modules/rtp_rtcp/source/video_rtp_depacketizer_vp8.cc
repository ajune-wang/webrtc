...
int VideoRtpDepacketizerVp8::ParseRtpPayload(
  ...
  int vp8_data_size = rtp_payload.size() - descriptor_size;
  if (vp8_data_size == 0) {
    RTC_LOG(LS_ERROR) << "Error parsing VP8. Empty payload!";
    return kFailedToParse;
  }
  const uint8_t* vp8_data = rtp_payload.data() + descriptor_size;

  // Read P bit from payload header (only at beginning of first partition).
  if (video_header->is_first_packet_in_frame && (*vp8_data & 0x01) == 0) {
    ...
    video_header->width = ((vp8_data[7] << 8) + vp8_data[6]) & 0x3FFF;
    video_header->height = ((vp8_data[9] << 8) + vp8_data[8]) & 0x3FFF;
  } else {
