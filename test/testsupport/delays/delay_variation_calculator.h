...
#ifndef TEST_TESTSUPPORT_DELAYS_DELAY_VARIATION_CALCULATOR_H_
#define TEST_TESTSUPPORT_DELAYS_DELAY_VARIATION_CALCULATOR_H_
...
// This class calculates inter-frame delay variations and outputs the results as
// a structured time series. This can be useful for simulation and replay of
// event logs.
class DelayVariationCalculator {
 public:
  struct TimeSeries {
    ...
    // Time series of local arrival timestamps `r(n)` for each frame `n`.
    SamplesStatsCounter arrival_times_ms;
    // Time series of sizes.
    ...
    // Time series of `d_r(n) - d_t(n) = (r(n) - r(n-1)) - (t(n) - t(n-1))` for
    // each frame.
    SamplesStatsCounter inter_delay_variations_ms;
    // Time series of `s(n) - s(n-1)`, where `s(n)` is the size (in bytes)
    // for a frame.
...
#endif  // TEST_TESTSUPPORT_DELAYS_DELAY_VARIATION_CALCULATOR_H_
