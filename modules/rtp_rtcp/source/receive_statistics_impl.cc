...
StreamDataCounters StreamStatisticianImpl::UpdateCounters(
  ...
  if (!ReceivedRtpPacket()) {
    ...
    if (!stream_restart && std::abs(sequence_number - received_seq_max_) >
                               max_reordering_threshold_) {
      // Sequence number gap looks too large, consider stream restart.
      received_seq_out_of_order_ = packet.SequenceNumber();
      // Do not yet update max sequence number received.
      return receive_counters_;
    }

    if (!stream_restart && sequence_number <= received_seq_max_) {
      // Old packet, may be restransmt.
