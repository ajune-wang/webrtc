...
void VideoSendStreamTest::TestVp9NonFlexMode(
  ...
  class NonFlexibleMode : public Vp9HeaderObserver {
   public:
    ...
    int GetRequiredDivisiblity() const {
      absl::optional<ScalabilityMode> scalability_mode =
          ScalabilityModeFromString(params_.scalability_mode);
    ...
    void ModifyVideoCaptureStartResolution(int* width,
      ...
      *height = expected_height_;
      // Top layer may be adjusted to ensure that lower layers are even.
      int divisiblity = GetRequiredDivisiblity();
      expected_width_ -= (expected_width_ % divisiblity);
      expected_height_ -= (expected_height_ % divisiblity);
    }
