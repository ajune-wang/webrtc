...
NetEqImpl::NetEqImpl(const NetEq::Config& config,
    ...
    : clock_(deps.clock),
      ...
      neteq_controller_(std::move(deps.neteq_controller)),
      last_mode_(Modes::kModeNormal),
  ...
  neteq_controller_->SetMaximumDelay(config.max_delay_ms);
  fs_hz_ = fs;
  fs_mult_ = fs / 8000;
  last_output_sample_rate_hz_ = fs;
  output_size_samples_ = static_cast<size_t>(kOutputSizeMs * 8 * fs_mult_);
  neteq_controller_->SetSampleRate(fs_hz_, output_size_samples_);
...
void NetEqImpl::InsertEmptyPacket(const RTPHeader& /*rtp_header*/) {
  ...
  rtc::CritScope lock(&crit_sect_);
  neteq_controller_->RegisterEmptyPacket();
}
...
bool NetEqImpl::SetMinimumDelay(int delay_ms) {
  ...
  if (delay_ms >= 0 && delay_ms <= 10000) {
    assert(neteq_controller_.get());
    return neteq_controller_->SetMinimumDelay(delay_ms);
  }
  return false;
}
...
bool NetEqImpl::SetMaximumDelay(int delay_ms) {
  ...
  if (delay_ms >= 0 && delay_ms <= 10000) {
    assert(neteq_controller_.get());
    return neteq_controller_->SetMaximumDelay(delay_ms);
  }
  return false;
}
...
bool NetEqImpl::SetBaseMinimumDelayMs(int delay_ms) {
  rtc::CritScope lock(&crit_sect_);
  if (delay_ms >= 0 && delay_ms <= 10000) {
    return neteq_controller_->SetBaseMinimumDelay(delay_ms);
  }
  return false;
}
...
int NetEqImpl::GetBaseMinimumDelayMs() const {
  rtc::CritScope lock(&crit_sect_);
  return neteq_controller_->GetBaseMinimumDelay();
}

int NetEqImpl::TargetDelayMs() const {
  rtc::CritScope lock(&crit_sect_);
  RTC_DCHECK(neteq_controller_.get());
  return neteq_controller_->TargetLevelMs();
}
...
int NetEqImpl::FilteredCurrentDelayMs() const {
  ...
  // buffer.
  const int delay_samples = neteq_controller_->GetFilteredBufferLevel() +
...
int NetEqImpl::NetworkStatistics(NetEqNetworkStatistics* stats) {
  ...
  const size_t total_samples_in_buffers =
      ...
      sync_buffer_->FutureLength();
  assert(neteq_controller_.get());
  stats->preferred_buffer_size_ms = neteq_controller_->TargetLevelMs();
  stats->jitter_peaks_found = neteq_controller_->PeakFound();
...
absl::optional<uint32_t> NetEqImpl::GetPlayoutTimestamp() const {
  rtc::CritScope lock(&crit_sect_);
  if (first_packet_ || last_mode_ == Modes::kModeRfc3389Cng ||
      last_mode_ == Modes::kModeCodecInternalCng) {
...
int NetEqImpl::InsertPacketInternal(const RTPHeader& rtp_header,
  ...
  auto relative_delay = neteq_controller_->PacketArrived(
...
int NetEqImpl::GetAudioInternal(AudioFrame* audio_frame,
  ...
  *muted = false;
  ...
  if (enable_muted_state_ && expand_->Muted() && packet_buffer_->Empty()) {
    RTC_DCHECK_EQ(last_mode_, Modes::kModeExpand);
    audio_frame->Reset();
  ...
  bool sid_frame_available =
      (operation == Operations::kRfc3389Cng && !packet_list.empty());
  ...
  switch (operation) {
    ...
    case Operations::kAccelerate:
    case Operations::kFastAccelerate: {
      const bool fast_accelerate =
          enable_fast_accelerate_ && (operation == Operations::kFastAccelerate);
      return_value = DoAccelerate(decoded_buffer_.get(), length, speech_type,
                                  play_dtmf, fast_accelerate);
      break;
    }
    case Operations::kPreemptiveExpand: {
      return_value = DoPreemptiveExpand(decoded_buffer_.get(), length,
                                        speech_type, play_dtmf);
      break;
    }
    case Operations::kRfc3389Cng:
    case Operations::kRfc3389CngNoPacket: {
      return_value = DoRfc3389Cng(&packet_list, play_dtmf);
      break;
    }
    case Operations::kCodecInternalCng: {
    ...
    case Operations::kDtmf: {
      // TODO(hlundin): Write test for this.
      return_value = DoDtmf(dtmf_event, &play_dtmf);
      break;
    }
    case Operations::kUndefined: {
  ...
  if (last_mode_ != Modes::kModeRfc3389Cng) {
  ...
  if ((last_mode_ == Modes::kModeNormal) ||
      (last_mode_ == Modes::kModeAccelerateFail) ||
      (last_mode_ == Modes::kModePreemptiveExpandFail) ||
      (last_mode_ == Modes::kModeRfc3389Cng) ||
      (last_mode_ == Modes::kModeCodecInternalCng)) {
    background_noise_->Update(*sync_buffer_, *vad_.get());
  }

  if (operation == Operations::kDtmf) {
  ...
  if (last_mode_ != Modes::kModeExpand && last_mode_ != Modes::kModeCodecPlc) {
  ...
  if (!(last_mode_ == Modes::kModeRfc3389Cng ||
        last_mode_ == Modes::kModeCodecInternalCng ||
        last_mode_ == Modes::kModeExpand ||
        last_mode_ == Modes::kModeCodecPlc)) {
...
int NetEqImpl::GetDecision(Operations* operation,
  ...
  *play_dtmf = false;
  *operation = Operations::kUndefined;
  ...
  uint64_t generated_noise_samples =
      generated_noise_stopwatch_ ? (generated_noise_stopwatch_->ElapsedTicks() -
                                    1) * output_size_samples_ +
                                       neteq_controller_->noise_fast_forward()
                                 : 0;

  if (neteq_controller_->CngRfc3389On() ||
      last_mode_ == Modes::kModeRfc3389Cng) {
  ...
  const int samples_left = static_cast<int>(sync_buffer_->FutureLength() -
                                            expand_->overlap_length());
  if (last_mode_ == Modes::kModeAccelerateSuccess ||
      last_mode_ == Modes::kModeAccelerateLowEnergy ||
      last_mode_ == Modes::kModePreemptiveExpandSuccess ||
      last_mode_ == Modes::kModePreemptiveExpandLowEnergy) {
    // Subtract (samples_left + output_size_samples_) from sampleMemory.
    neteq_controller_->AddSampleMemory(
  ...
  generated_noise_samples =
      generated_noise_stopwatch_
          ? generated_noise_stopwatch_->ElapsedTicks() * output_size_samples_ +
                neteq_controller_->noise_fast_forward()
          : 0;
  absl::optional<NetEqController::PacketInfo> packet_info;
  if (packet) {
    packet_info = {packet->timestamp,
                   packet->frame && packet->frame->IsDtxPacket(),
                   decoder_database_->IsComfortNoise(packet->payload_type)};
  }
  NetEqController::PacketBufferInfo packet_buffer_info;
  packet_buffer_info.dtx_or_cng =
      packet_buffer_->ContainsDtxOrCngPacket(decoder_database_.get());
  packet_buffer_info.num_samples =
      packet_buffer_->NumSamplesInBuffer(decoder_frame_length_);
  packet_buffer_info.span_samples = packet_buffer_->GetSpanSamples(
      decoder_frame_length_, last_output_sample_rate_hz_, true);
  packet_buffer_info.span_samples_no_dtx = packet_buffer_->GetSpanSamples(
      decoder_frame_length_, last_output_sample_rate_hz_, false);
  packet_buffer_info.num_packets = packet_buffer_->NumPacketsInBuffer();
  *operation = neteq_controller_->GetDecision(
      sync_buffer_->end_timestamp(), expand_->MuteFactor(0),
      decoder_frame_length_, packet_info, last_mode_, *play_dtmf,
      generated_noise_samples, std::move(packet_buffer_info), &reset_decoder_);
  ...
  if (packet && packet->frame && packet->frame->IsDtxPacket() &&
      (*operation == Operations::kMerge ||
       *operation == Operations::kAccelerate ||
       *operation == Operations::kFastAccelerate ||
       *operation == Operations::kPreemptiveExpand)) {
    *operation = Operations::kNormal;
  }
  ...
  neteq_controller_->ExpandDecision(*operation);

  // Check conditions for reset.
  if (new_codec_ || *operation == Operations::kUndefined) {
    ...
    if (*play_dtmf && !packet) {
      ...
      if (*operation == Operations::kRfc3389CngNoPacket &&
          decoder_database_->IsComfortNoise(packet->payload_type)) {
        // Change decision to CNG packet, since we do have a CNG packet, but it
        // was considered too early to use. Now, use it anyway.
        *operation = Operations::kRfc3389Cng;
      } else if (*operation != Operations::kRfc3389Cng) {
        *operation = Operations::kNormal;
      }
    }
  ...
  switch (*operation) {
      ...
      const uint64_t generated_noise_samples =
          generated_noise_stopwatch_
              ? generated_noise_stopwatch_->ElapsedTicks() *
                    ...
                    neteq_controller_->noise_fast_forward()
              : 0;
      if (generated_noise_samples > 0 && last_mode_ != Modes::kModeDtmf) {
    ...
    case Operations::kFastAccelerate: {
      ...
      if (samples_left >= static_cast<int>(samples_30_ms)) {
        ...
        neteq_controller_->set_sample_memory(samples_left);
        neteq_controller_->set_prev_time_scale(true);
        return 0;
  ...
  if (packet) {
    ...
    if (*operation != Operations::kRfc3389Cng) {
      // We are about to decode and use a non-CNG packet.
      neteq_controller_->SetCngOff();
    }
  ...
  if (*operation == Operations::kAccelerate ||
      *operation == Operations::kFastAccelerate ||
      *operation == Operations::kPreemptiveExpand) {
    neteq_controller_->set_sample_memory(samples_left + extracted_samples);
    neteq_controller_->set_prev_time_scale(true);
  }
...
int NetEqImpl::Decode(PacketList* packet_list,
  ...
  *decoded_length = 0;
  ...
  if ((*operation == Operations::kMerge) && decoder &&
      decoder->HasDecodePlc()) {
    decoder->DecodePlc(1, &decoded_buffer_[*decoded_length]);
  }

  int return_value;
  if (*operation == Operations::kCodecInternalCng) {
    RTC_DCHECK(packet_list->empty());
  ...
  if (*decoded_length < 0) {
    ...
    *operation = Operations::kExpand;  // Do expansion to get data instead.
  }
...
int NetEqImpl::DecodeLoop(PacketList* packet_list,
  ...
  while (!packet_list->empty() && !decoder_database_->IsComfortNoise(
    ...
    assert(operation == Operations::kNormal ||
           operation == Operations::kAccelerate ||
           operation == Operations::kFastAccelerate ||
           operation == Operations::kMerge ||
           operation == Operations::kPreemptiveExpand);
...
void NetEqImpl::DoNormal(const int16_t* decoded_buffer,
  ...
  if ((speech_type == AudioDecoder::kComfortNoise) ||
      ((last_mode_ == Modes::kModeCodecInternalCng) && (decoded_length == 0))) {
...
bool NetEqImpl::DoCodecPlc() {
  ...
  // Update in-call and post-call statistics.
  const bool is_new_concealment_event = (last_mode_ != Modes::kModeCodecPlc);
...
int NetEqImpl::DoExpand(bool play_dtmf) {
  while ((sync_buffer_->FutureLength() - expand_->overlap_length()) <
    ...
    bool is_new_concealment_event = (last_mode_ != Modes::kModeExpand);
...
int NetEqImpl::DoAccelerate(int16_t* decoded_buffer,
  ...
  switch (return_code) {
    case Accelerate::kSuccess:
      last_mode_ = Modes::kModeAccelerateSuccess;
      break;
...
void NetEqImpl::SetSampleRateAndChannels(int fs_hz, size_t channels) {
  ...
  RTC_CHECK(neteq_controller_) << "Unexpectedly found no NetEqController";
  neteq_controller_->SetSampleRate(fs_hz_, output_size_samples_);
}
...
NetEqImpl::OutputType NetEqImpl::LastOutputType() {
  assert(vad_.get());
  assert(expand_.get());
  if (last_mode_ == Modes::kModeCodecInternalCng ||
      last_mode_ == Modes::kModeRfc3389Cng) {
    return OutputType::kCNG;
  } else if (last_mode_ == Modes::kModeExpand && expand_->MuteFactor(0) == 0) {
    // Expand mode has faded down to background noise only (very long expand).
    return OutputType::kPLCCNG;
  } else if (last_mode_ == Modes::kModeExpand) {
    return OutputType::kPLC;
  } else if (vad_->running() && !vad_->active_speech()) {
    return OutputType::kVadPassive;
  } else if (last_mode_ == Modes::kModeCodecPlc) {
