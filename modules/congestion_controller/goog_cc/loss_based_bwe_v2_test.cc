...
class LossBasedBweV2Test : public ::testing::TestWithParam<bool> {
 protected:
  std::string Config(bool enabled,
                     bool valid,
                     bool trendline_with_max_increase_factor_enabled) {
    char buffer[1024];
    ...
    if (trendline_with_max_increase_factor_enabled) {
      config_string << ",TrendlineWithMaxIncreaseFactor:true";
    } else {
      config_string << ",TrendlineWithMaxIncreaseFactor:false";
    }
...
TEST_P(LossBasedBweV2Test, EnabledWhenGivenValidConfigurationValues) {
  ExplicitKeyValueConfig key_value_config(
      Config(/*enabled=*/true, /*valid=*/true,
             /*trendline_with_max_increase_factor_enabled=*/GetParam()));
...
TEST_P(LossBasedBweV2Test, BandwidthEstimateNotIncreaseWhenNetworkUnderusing) {
  if (!GetParam()) {
    GTEST_SKIP() << "This test should run only if "
                    "trendline_with_max_increase_factor_enabled is enabled";
  }
  std::vector<PacketResult> enough_feedback_1 =
      CreatePacketResultsWithReceivedPackets(
          /*first_packet_timestamp=*/Timestamp::Zero());
