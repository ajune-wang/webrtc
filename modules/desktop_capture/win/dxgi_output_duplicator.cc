...
bool DxgiOutputDuplicator::PrepareVideoProcessor() {
  ...
  D3D11_TEXTURE2D_DESC desc = {
    .Width = static_cast<UINT>(desktop_rect_.width()),
    .Height = static_cast<UINT>(desktop_rect_.height()),
    .MipLevels = 1,
    .ArraySize = 1,
    .Format = dxgi_format,
    .SampleDesc = {1, 0},
  ...
  error = device_.d3d_device()->CreateTexture2D(
      &desc, nullptr, &last_texture_);
  if (error.Error() != S_OK) {
  ...
  error = last_texture_.As(&dxgi_resource);
  if (error.Error() != S_OK) {
...
bool DxgiOutputDuplicator::DuplicateNative(Context* context,
  ...
  //////////////////////////////
  RTC_LOG(LS_ERROR) << "@AcquireFrame result:" << error.Error() << " resource:"
    << resource.Get() << " accum: " << frame_info.AccumulatedFrames;
  RTC_LOG(LS_ERROR) << "@AcquireFrame info LastMouseUpdateTime:"
    << frame_info.LastMouseUpdateTime.QuadPart
    << " PointerPosition:" << frame_info.PointerPosition.Position.x << ","
    << frame_info.PointerPosition.Position.y << " vis:"
  ...
  if (error.Error() == S_OK && resource &&
      (frame_info.AccumulatedFrames > 0 || !last_handle_)) {
    DetectUpdatedRegion(frame_info, &context->updated_region);
    SpreadContextChange(context);
    updated_region.AddRegion(context->updated_region);
    // Only update texture according to updated region in the furture.
    last_frame_offset_ = offset;
    updated_region.Translate(offset.x(), offset.y());
    target->mutable_updated_region()->AddRegion(updated_region);
    num_frames_captured_++;
    ...
    if (!ConvertBGRATextureToNV12(src_texture.Get(), last_texture_.Get())) {
        RTC_LOG(LS_ERROR) << "Failed to convert to NV12 texture.";
    }
    // Duplicate handle for target frame.
    HANDLE dup_handle;
    bool dret = DuplicateHandle(GetCurrentProcess(), last_handle_, GetCurrentProcess(),
                    &dup_handle, 0, FALSE, DUPLICATE_SAME_ACCESS);
    RTC_LOG(LS_ERROR) << "### dret:" << dret;
    RTC_LOG(LS_ERROR) << "### dup handle:" << last_handle_ << ", " << dup_handle;
    target->Init(device_, dup_handle);
    return ReleaseFrame();
  }
  ...
  if (last_handle_) {
    ...
    // Duplicate handle for target frame.
    HANDLE dup_handle;
    DuplicateHandle(GetCurrentProcess(), last_handle_, GetCurrentProcess(),
                    &dup_handle, 0, FALSE, DUPLICATE_SAME_ACCESS);
    target->Init(device_, dup_handle);
  } else {
    // If we were at the very first frame, and capturing failed, the
