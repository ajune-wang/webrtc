...
class TCPConnector : public IOHandler {
 ...
 public:
  TCPConnector(int fd,
               const ubnt::abstraction::SocketAddress& targetAddress,
  ...
  virtual bool OnEvent(epoll_event& event) {
    ...
    if ((event.events & EPOLLERR) != 0) {
      printf("***CONNECT ERROR: Unable to connect to: %s",
             (const char*)_targetAddress);
      _closeSocket = true;
      return false;
    }
  ...
  static bool Connect(std::string ip,
    ...
    TCPConnector* pTCPConnector =
        new TCPConnector(fd, targetAddress, pProtocol, observer);
  ...
  bool Connect() {
    ...
    if (connect(_inboundFd, (const sockaddr*)_targetAddress,
                _targetAddress.GetLength()) != 0) {
      int err = errno;
      if (err != EINPROGRESS) {
        printf("Unable to connect to %s. Error was: (%d) %s",
               (const char*)_targetAddress, err, strerror(err));
        _closeSocket = true;
        return false;
      }
    }
