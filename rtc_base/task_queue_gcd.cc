...
struct TaskQueueGcd::TaskContext {
  TaskContext(TaskQueueGcd* queue, std::unique_ptr<QueuedTask> task)
      : queue_(queue_ctx), task(std::move(task)) {}

  TaskQueueGcd* const queue_;
...
// static
void TaskQueueGcd::SetNotActive(void* context) {
  TaskQueueGcd* qc = static_cast<TaskQueueGcd*>(context);
  qc->is_active_ = false;
}

// static
void TaskQueueGcd::DeleteContext(void* context) {
  TaskQueueGcd* qc = static_cast<TaskQueueGcd*>(context);
  delete qc;
}
