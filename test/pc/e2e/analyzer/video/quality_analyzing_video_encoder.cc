...
int32_t QualityAnalyzingVideoEncoder::Encode(
  ...
  if (result != WEBRTC_VIDEO_CODEC_OK) {
    // If origin encoder failed, then cleanup data for this frame.
    {
      ...
      for (auto it = timestamp_to_frame_id_list_.end();
           it != timestamp_to_frame_id_list_.begin();) {
        --it;
...
EncodedImageCallback::Result QualityAnalyzingVideoEncoder::OnEncodedImage(
  ...
  uint16_t frame_id;
  {
    ...
    // frame id. We don't remove it from the list, because there are can be
    // multiple spatial layer for this frame, so encoder can produce more
    // encoded images with this timestamp. But it is guaranteed, that when
    // encode will produce |encoded_image| with another timestamp, the previous
    // one won't be required any more.
