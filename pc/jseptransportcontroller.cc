...
bool JsepTransportController::GetLocalCertificate(
  ...
  if (!network_thread_->IsCurrent()) {
    return network_thread_->Invoke<bool>(RTC_FROM_HERE, [=] {
      return GetLocalCertificate(transport_name, certificate);
    });
  }
  RTC_DCHECK(network_thread_->IsCurrent());
...
RTCError JsepTransportController::ApplyDescription_n(
  ...
  for (size_t i = 0; i < description->contents().size(); ++i) {
    ...
    RTCError error;
    if (local) {
      error = SetLocalTransportDescription_n(
          content_info.name, content_info, transport_info, type, extension_ids);
    } else {
      error = SetRemoteTransportDescription_n(
          content_info.name, content_info, transport_info, type, extension_ids);
    }
...
RTCError JsepTransportController::SetLocalTransportDescription_n(
    const std::string& mid,
    const cricket::ContentInfo& content_info,
    const cricket::TransportInfo& transport_info,
    SdpType type,
  ...
  return transport->SetLocalTransportDescription(
      ...
      encrypted_extension_ids, type);
}

RTCError JsepTransportController::SetRemoteTransportDescription_n(
    const std::string& mid,
    const cricket::ContentInfo& content_info,
    const cricket::TransportInfo& transport_info,
    SdpType type,
