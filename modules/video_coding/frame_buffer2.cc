...
FrameBuffer::ReturnReason FrameBuffer::NextFrame(
  ...
  fprintf(stderr, "NextFrame called with wait time = %ld\n", max_wait_time_ms);
  do {
    now_ms = clock_->TimeInMilliseconds();
    fprintf(stderr, "NextFrame in the loop at time %ld\n", now_ms);
    {
      ...
      for (; frame_it != continuous_end_it && frame_it != frames_.end();
        ...
        std::vector<VideoLayerFrameId> current_superframe;
        current_superframe.push_back(frame_it->first);
        ...
        while (true) {
          ...
          if (frame->Timestamp() != next_frame_it->second.frame->Timestamp()) {
            RTC_LOG(LS_WARNING)
                << "Frames in a single superframe have differen"
                   " timestamps. Skipping undecodable superframe.";
            break;
          }
          current_superframe.push_back(next_frame_it->first);
        ...
        frames_to_decode_ = current_superframe;
  ...
  fprintf(stderr, "After waiting loop\n");
  ...
  fprintf(stderr, "Didn't return any frames\n");
