...
void TaskQueuePacedSender::MaybeProcessPackets(
  ...
  Timestamp next_send_time =
      std::max(now, std::min(pacing_controller_.NextSendTime(),
                             last_stats_update_ + kMinTimeBetweenStatsUpdates));
  if (next_process_time_.IsMinusInfinity() ||
      next_send_time <= next_process_time_ - PacingController::kMinSleepTime) {
    next_process_time_ = next_send_time;

    uint32_t sleep_time =
        (next_send_time - now).RoundTo(TimeDelta::ms(1)).ms<uint32_t>();
