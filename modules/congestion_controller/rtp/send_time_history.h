...
class SendTimeHistory {
 public:
  enum class PacketSendState { kUknown, kSent, kRetransmission };
  ...
  // Return an PacketSendState indicating if the packet was not found, sent,
  // or if it was previously already marked as sent.
  PacketSendState OnSentPacket(uint16_t sequence_number, int64_t send_time_ms);
