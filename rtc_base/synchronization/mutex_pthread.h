...
class RTC_LOCKABLE MutexImpl final {
 ...
 private:
  class OwnerRecord {
   public:
#if RTC_DCHECK_IS_ON
    OwnerRecord() { latest_owner_ = pthread_self(); }
  ...
  //
  // The failure case is more subtle: If AssertHeld is called form some thread
  // not holding the mutex, we have a data race. It is highly likely that the
  // calling thread will see `is_owned_` false or `latest_owner_` different from
  // itself, and crash. But it may fail to crash, and invoke some other
  // undefined behavior.
