...
MultiplexImageHeader DepackHeader(uint8_t* buffer) {
...
MultiplexImageComponentHeader DepackFrameHeader(uint8_t* buffer) {
  MultiplexImageComponentHeader frame_header;
  int offset = 0;
...
MultiplexImage MultiplexEncodedImagePacker::Unpack(
  ...
  const MultiplexImageHeader& header = DepackHeader(combined_image._buffer);
