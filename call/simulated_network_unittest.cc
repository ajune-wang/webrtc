...
TEST(SimulatedNetworkTest, CodelDoesNothingAtCapacity) {
  ...
  DataRate link_capacity = DataRate::KilobitsPerSecond(1000);
  ...
  config.link_capacity_kbps = link_capacity.KilobitsPerSecond();
  ...
  Timestamp start_time = Timestamp::Milliseconds(0);
  ...
  while (current_time - start_time < kRuntime) {
    if (current_time >= next_packet_time) {
      bool success = network.EnqueuePacket(PacketInFlightInfo{
          packet_size.Bytes<size_t>(), current_time.Microseconds(), next_id});
      EXPECT_TRUE(success);
    ...
    if (network.NextDeliveryTimeUs())
      next_delivery = Timestamp::Microseconds(*network.NextDeliveryTimeUs());
    current_time = std::min(next_packet_time, next_delivery);
    if (current_time >= next_delivery) {
      for (PacketDeliveryInfo packet :
           network.DequeueDeliverablePackets(current_time.Microseconds())) {
...
TEST(SimulatedNetworkTest, CodelLimitsDelayAndDropsPacketsOnOverload) {
  ...
  while (current_time - start_time < kRuntime) {
    if (current_time >= next_packet_time) {
      bool success = network.EnqueuePacket(PacketInFlightInfo{
          packet_size.Bytes<size_t>(), current_time.Microseconds(), next_id});
      send_times_us.insert({next_id, current_time.Microseconds()});
      ++next_id;
      EXPECT_TRUE(success);
      next_packet_time += packet_inverval;
    }
    ...
    if (current_time > last_check + kCheckInterval) {
      ...
      EXPECT_LT(average_delay.Milliseconds(), 200)
          << "Time " << (current_time - start_time).Milliseconds() << "\n";
