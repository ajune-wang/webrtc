...
struct Traits<RetType (ClassType::*)() const> {
  using ret = RetType;
};

template <typename F, typename ReturnT = typename Traits<F>::ret>
ReturnT MaybeInvoke(rtc::Thread* t, const rtc::Location& l, F&& functor) {
  // Note that we do this check here rather than relying on the corresponding
  // check in rtc::Thread, since the latter will not run the functor if the
  // thread is being destroyed.
  if (t->IsCurrent()) {
    return functor();
  } else {
    return t->Invoke<ReturnT>(RTC_FROM_HERE, functor);
  }
}
}  // namespace proxy_impl
...
#define REFCOUNTED_PROXY_MAP_BOILERPLATE(c)                     \
 protected:                                                     \
  ~c##ProxyWithInternal() {                                     \
    proxy_impl::MaybeInvoke(destructor_thread(), RTC_FROM_HERE, \
                            [&] { c_ = nullptr; });             \
  }                                                             \
                                                                \
 private:                                                       \
...
#define PROXY_METHOD0_BASE(t, modifier, r, method)                \
  r method() modifier override {                                  \
    return proxy_impl::MaybeInvoke(t, RTC_FROM_HERE,              \
                                   [&] { return c_->method(); }); \
  }
