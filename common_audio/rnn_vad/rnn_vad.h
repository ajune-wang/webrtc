...
#ifndef COMMON_AUDIO_AUDIO_RNN_VAD_RNN_H_
#define COMMON_AUDIO_AUDIO_RNN_VAD_RNN_H_
...
class FullyConnectedLayer {
 ...
 private:
  FullyConnectedLayer(const FullyConnectedLayer& that) = delete;
...
class GatedRecurrentLayer {
 ...
 private:
  GatedRecurrentLayer(const GatedRecurrentLayer& that) = delete;
  const size_t input_size_;
  const size_t output_size_;
  const rtc::ArrayView<const int8_t> bias_;
  const rtc::ArrayView<const int8_t> weights_;
...
#endif  // COMMON_AUDIO_AUDIO_RNN_VAD_RNN_H_
