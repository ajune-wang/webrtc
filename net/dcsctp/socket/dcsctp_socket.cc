...
constexpr uint32_t kMaxVerificationTag = 4294967295;

// // https://tools.ietf.org/html/rfc4960#section-3.3.2
constexpr uint32_t kMinInitialTsn = 0;
constexpr uint32_t kMaxInitialTsn = 4294967295;
...
bool DcSctpSocket::IsConsistent() const {
  switch (state_) {
    case State::kClosed:
      ...
      RTC_DCHECK(!t1_init_->is_running());
      RTC_DCHECK(!t1_cookie_->is_running());
      RTC_DCHECK(!t2_shutdown_->is_running());
      break;
    ...
    case State::kCookieEchoed:
      ...
      RTC_DCHECK(t1_cookie_->is_running());
      RTC_DCHECK(!t2_shutdown_->is_running());
      RTC_DCHECK(cookie_echo_chunk_.has_value());
      break;
...
bool DcSctpSocket::HandleCookieEchoWithTCB(const CommonHeader& header,
  ...
  if (header.verification_tag != tcb_->my_verification_tag() &&
    ...
    // ACK."
    RTC_DLOG(LS_VERBOSE) << log_prefix()
                         << "Received duplicate CookieEcho. Continuing.";
  }
  return true;
}
