...
- (instancetype)initWithNativeDependencies:(webrtc::PeerConnectionFactoryDependencies)deps {
  self = [self initNative];
  if (self) {
    // Set fields that are relevant both to 'no media' and 'with media' scenarios.
    deps.network_thread = _networkThread.get();
    deps.worker_thread = _workerThread.get();
    deps.signaling_thread = _signalingThread.get();
    if (deps.trials == nullptr) {
      deps.trials = std::make_unique<webrtc::FieldTrialBasedConfig>();
    }
    if (deps.network_monitor_factory == nullptr &&
        deps.trials->IsEnabled("WebRTC-Network-UseNWPathMonitor")) {
      deps.network_monitor_factory = webrtc::CreateNetworkMonitorFactory();
    }

    _nativeFactory = webrtc::CreateModularPeerConnectionFactory(std::move(deps));
...
- (instancetype)init {
  ...
  return [self initWithMediaDependencies:std::move(dependencies)];
}
...
- (instancetype)initWithMediaDependencies:(webrtc::PeerConnectionFactoryDependencies)dependencies {
  if (dependencies.audio_processing == nullptr &&
      dependencies.audio_processing_builder == nullptr) {
