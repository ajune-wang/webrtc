...
class VideoReceiveStream2
 ...
 private:
  void StartNextDecodeOnDecoderThread() RTC_RUN_ON(decoder_queue_);
  void CreateAndRegisterExternalDecoder(const Decoder& decoder);
  int64_t GetMaxWaitMs() const RTC_RUN_ON(decoder_queue_);
  void HandleEncodedFrame(std::unique_ptr<EncodedFrame> frame)
      RTC_RUN_ON(decoder_queue_);
  ...
  bool keyframe_required_ RTC_GUARDED_BY(decoder_queue_) = true;

  // If we have successfully decoded any frame.
  bool frame_decoded_ RTC_GUARDED_BY(decoder_queue_) = false;
  ...
  Timestamp last_decoded_frame_timestamp_ RTC_GUARDED_BY(decoder_queue_) =
