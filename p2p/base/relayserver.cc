...
void Send(rtc::AsyncPacketSocket* socket, const char* bytes, size_t size,
  ...
  if (result < static_cast<int>(size)) {
    NLOG(LS_ERROR, "SendTo wrote only ", result, " of ", size, " bytes");
  } else if (result < 0) {
...
void RelayServer::OnInternalPacket(
  ...
  if (!int_conn->locked()) {
    NLOG(LS_WARNING, "Dropping packet: connection not locked");
    return;
  }
  ...
  if (ext_conn && ext_conn->locked()) {
    ...
    NLOG(LS_INFO, "Dropping packet: no external connection");
  }
}
...
void RelayServer::OnExternalPacket(
  ...
  if (!msg.Read(&buf)) {
    NLOG(LS_WARNING, "Dropping packet: first packet not STUN");
    return;
  }
  ...
  if (!username_attr) {
    NLOG(LS_WARNING, "Dropping packet: no username");
    return;
  }
  ...
  if (biter == bindings_.end()) {
    NLOG(LS_WARNING, "Dropping packet: no binding with username");
    return;
  }
...
void RelayServer::HandleStunAllocate(
  ...
  if (biter != bindings_.end()) {
    ...
    if (log_bindings_) {
      NLOG(LS_INFO, "Added new binding ", username, ", ", bindings_.size(),
           " total");
    }
  }
...
void RelayServer::RemoveBinding(RelayServerBinding* binding) {
  ...
  if (log_bindings_) {
    NLOG(LS_INFO, "Removed binding ", binding->username(), ", ",
...
void RelayServerBinding::OnMessage(rtc::Message *pmsg) {
  if (pmsg->message_id == MSG_LIFETIME_TIMER) {
    ...
    if (rtc::TimeMillis() >= last_used_ + lifetime_) {
      NLOG(LS_INFO, "Expiring binding ", username_);
      SignalTimeout(this);
    } else {
      server_->thread()->PostDelayed(RTC_FROM_HERE, lifetime_, this,
                                     MSG_LIFETIME_TIMER);
