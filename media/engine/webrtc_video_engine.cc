...
GetOutboundRtpSubstreams(
  ...
  // substream stats.
  for (const auto& pair : substreams) {
    const webrtc::VideoSendStream::StreamStats& contributing_substream =
        pair.second;
    if (contributing_substream.type !=
            webrtc::VideoSendStream::StreamStats::StreamType::kRtx &&
        contributing_substream.type !=
            webrtc::VideoSendStream::StreamStats::StreamType::kFlexfec) {
      continue;
    }
    RTC_DCHECK(contributing_substream.referenced_media_ssrc.has_value());
    uint32_t media_ssrc = contributing_substream.referenced_media_ssrc.value();
    ...
    outbound_rtp.frame_counts.key_frames +=
        contributing_substream.frame_counts.key_frames;
    outbound_rtp.frame_counts.delta_frames +=
        contributing_substream.frame_counts.delta_frames;
    outbound_rtp.total_packet_send_delay_ms +=
        contributing_substream.total_packet_send_delay_ms;
    outbound_rtp.rtp_stats.Add(contributing_substream.rtp_stats);
    outbound_rtp.rtcp_packet_type_counts.Add(
        contributing_substream.rtcp_packet_type_counts);
    outbound_rtp.rtcp_stats.fraction_lost +=
        contributing_substream.rtcp_stats.fraction_lost;
    outbound_rtp.rtcp_stats.packets_lost +=
        contributing_substream.rtcp_stats.packets_lost;
  }
