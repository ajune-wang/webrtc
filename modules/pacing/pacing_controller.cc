...
void PacingController::ProcessPacketsAt(Timestamp scheduled_time) {
  ...
  // If we're using dynamic process mode and we woke up after the desired send
  // time for a packet, send as if we were actually on time.
  Timestamp target_send_time = now;
  if (mode_ == ProcessMode::kDynamic && scheduled_time < now) {
    // Don't allow acting is if we are sending a packet prior to the last actual
    // process time.
    target_send_time = std::max(scheduled_time, time_last_process_);
  }
  ...
  while (!paused_) {
    ...
    if (mode_ == ProcessMode::kDynamic) {
      ...
      // in processing.
      target_send_time = std::min(now, NextSendTime());
    }
  }
  ...
  if (mode_ == ProcessMode::kDynamic &&
      time_since_scheduled_run > TimeDelta::Zero()) {
