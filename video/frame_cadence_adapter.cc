...
class PassthroughAdapterMode : public AdapterMode {
 public:
  ...
  void UpdateFrameRate(Timestamp frame_timestamp) override {
    ...
    RTC_LOG(LS_INFO) << " last_frame_rate_ " << last_frame_rate_.value_or(-1)
                     << "ts" << frame_timestamp.ms();
...
void FrameCadenceAdapterImpl::OnFrameOnMainQueue(Timestamp post_time,
  ...
  Timestamp update_frame_rate_timetamp =
      use_video_frame_timestamp_ ? Timestamp::Micros(frame.timestamp_us())
                                 : post_time;
  UpdateFrameRate(update_frame_rate_timetamp);
}
