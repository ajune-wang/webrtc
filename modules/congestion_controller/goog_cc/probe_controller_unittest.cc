...
LossBasedBweV2::Result CreateResultDelayBasedState() {
  LossBasedBweV2::Result result;
  result.state = LossBasedState::kDelayBasedEstimate;
  return result;
}

LossBasedBweV2::Result CreateResultIncreasingState() {
  LossBasedBweV2::Result result;
  result.state = LossBasedState::kIncreasing;
  return result;
}

LossBasedBweV2::Result CreateResultDecreasingState() {
  LossBasedBweV2::Result result;
  result.state = LossBasedState::kDecreasing;
  return result;
}
...
TEST(ProbeControllerTest, InitiatesProbingOnMaxBitrateIncrease) {
  ...
  probes = probe_controller->SetEstimatedBitrate(
      kStartBitrate, CreateResultDelayBasedState(), fixture.CurrentTime());
...
TEST(ProbeControllerTest, ProbesOnMaxBitrateIncreaseOnlyWhenInAlr) {
  ...
  auto probes = probe_controller->SetBitrates(
      kMinBitrate, kStartBitrate, kMaxBitrate, fixture.CurrentTime());
  probes = probe_controller->SetEstimatedBitrate(
      kMaxBitrate - DataRate::BitsPerSec(1), CreateResultDelayBasedState(),
      fixture.CurrentTime());
...
TEST(ProbeControllerTest,
  ...
  probes = probe_controller->SetEstimatedBitrate(
      kStartBitrate, CreateResultIncreasingState(), fixture.CurrentTime());
...
TEST(ProbeControllerTest, NotProbeFurtherWhenInAlrIfLossBasedDecreases) {
  ...
  probe_controller->EnablePeriodicAlrProbing(true);
  probes = probe_controller->SetEstimatedBitrate(
      kStartBitrate, CreateResultDecreasingState(), fixture.CurrentTime());
...
TEST(ProbeControllerTest, InitiatesProbingOnMaxBitrateIncreaseAtMaxBitrate) {
  ...
  probes = probe_controller->Process(fixture.CurrentTime());
  probes = probe_controller->SetEstimatedBitrate(
      kMaxBitrate, CreateResultDelayBasedState(), fixture.CurrentTime());
  probes = probe_controller->SetBitrates(
      kMinBitrate, kStartBitrate, kMaxBitrate + DataRate::BitsPerSec(100),
      fixture.CurrentTime());
  EXPECT_EQ(probes.size(), 1u);
...
TEST(ProbeControllerTest, TestExponentialProbing) {
  ...
  probes = probe_controller->SetEstimatedBitrate(DataRate::BitsPerSec(1000),
                                                 CreateResultDelayBasedState(),
                                                 fixture.CurrentTime());
  ASSERT_TRUE(probes.empty());

  probes = probe_controller->SetEstimatedBitrate(DataRate::BitsPerSec(1800),
                                                 CreateResultDelayBasedState(),
                                                 fixture.CurrentTime());
  EXPECT_EQ(probes.size(), 1u);
...
TEST(ProbeControllerTest, RequestProbeInAlr) {
  ...
  fixture.AdvanceTime(kAlrProbeInterval + TimeDelta::Millis(1));
  probes = probe_controller->Process(fixture.CurrentTime());
  probes = probe_controller->SetEstimatedBitrate(DataRate::BitsPerSec(250),
                                                 CreateResultDelayBasedState(),
                                                 fixture.CurrentTime());
...
TEST(ProbeControllerTest, TestAllocatedBitrateCap) {
  ...
  probes = probe_controller->SetEstimatedBitrate(
      estimated_bitrate, CreateResultDelayBasedState(), fixture.CurrentTime());
...
TEST(ProbeControllerTest, ConfigurableProbingFieldTrial) {
  ...
  probes = probe_controller->SetEstimatedBitrate(DataRate::BitsPerSec(1100),
                                                 CreateResultDelayBasedState(),
                                                 fixture.CurrentTime());
  EXPECT_EQ(probes.size(), 0u);

  probes = probe_controller->SetEstimatedBitrate(DataRate::BitsPerSec(1250),
                                                 CreateResultDelayBasedState(),
                                                 fixture.CurrentTime());
  EXPECT_EQ(probes.size(), 1u);
...
TEST(ProbeControllerTest, PeriodicProbeAtUpperNetworkStateEstimate) {
  ...
  auto probes = probe_controller->SetBitrates(
      kMinBitrate, kStartBitrate, kMaxBitrate, fixture.CurrentTime());
  probes = probe_controller->SetEstimatedBitrate(DataRate::BitsPerSec(5000),
                                                 CreateResultDelayBasedState(),
                                                 fixture.CurrentTime());
  // Expect the controller to send a new probe after 5s has passed.
...
TEST(ProbeControllerTest, AlrProbesLimitedByNetworkStateEstimate) {
  ...
  auto probes = probe_controller->SetBitrates(
      kMinBitrate, kStartBitrate, kMaxBitrate, fixture.CurrentTime());
  probes = probe_controller->SetEstimatedBitrate(DataRate::KilobitsPerSec(6),
                                                 CreateResultDelayBasedState(),
                                                 fixture.CurrentTime());
  probe_controller->SetAlrStartTimeMs(fixture.CurrentTime().ms());
...
TEST(ProbeControllerTest, CanSetLongerProbeDurationAfterNetworkStateEstimate) {
  ...
  auto probes = probe_controller->SetBitrates(
      kMinBitrate, kStartBitrate, kMaxBitrate, fixture.CurrentTime());
  probes = probe_controller->SetEstimatedBitrate(DataRate::KilobitsPerSec(5),
                                                 CreateResultDelayBasedState(),
                                                 fixture.CurrentTime());
  ASSERT_FALSE(probes.empty());
...
TEST(ProbeControllerTest, ProbeAfterLargeNetworkStateDropLossLimited) {
  ...
  // Loss limited.
  probes = probe_controller->SetEstimatedBitrate(
      kStartBitrate / 3, CreateResultDecreasingState(), fixture.CurrentTime());
  // If NetworkState decrease dramatically, expect a probe to be sent.
...
TEST(ProbeControllerTest, ProbeFurtherWhenLossBasedIncreases) {
  ...
  probes = probe_controller->SetEstimatedBitrate(
      probe_target_rate, CreateResultIncreasingState(), fixture.CurrentTime());
  EXPECT_FALSE(probes.empty());
...
TEST(ProbeControllerTest, ProbeFurtherWhenLossBasedIsSameAsDelayBasedEstimate) {
  ...
  // Expect that no more probes are sent immediately if BWE is loss limited.
  probes = probe_controller->SetEstimatedBitrate(
      probe_target_rate, CreateResultDelayBasedState(), fixture.CurrentTime());
  EXPECT_FALSE(probes.empty());
  EXPECT_EQ(probes[0].target_data_rate, 2 * probe_target_rate);
}
