...
TEST(TimestampExtrapolatorTest, EstiatedClockDriftHistogram) {
  ...
  constexpr int kMinimumSamples = 300;
  constexpr Frequency k24Fps = Frequency::Hertz(24);
  constexpr TimeDelta k24FpsDelay = 1 / k24Fps;

  // This simulates a remote clock without drift, which produces frames at 25Hz.
  // Local scope to trigger the desctrutor of TimestampExtrapolator.
  {
  ...
  // This simulates a slow remote clock, so that the RTP timestamps are
  // incremented as if the camera was 25 fps even though frames arrive at 24
  // fps. Local scope to trigger the desctrutor of TimestampExtrapolator.
  {
    // Clear all histogram data.
    metrics::Reset();
    SimulatedClock clock(Timestamp::Millis(1337));
  ...
  // This simulates a fast remote clock, so that the RTP timestamps are
