...
uint16_t NumberOf32BitWords(size_t size_in_bytes) {
  return rtc::dchecked_cast<uint16_t>((size_in_bytes + 3) /
                                      4);  // Wrap up to 32bit.
}

size_t WordAlignedLength(size_t extensions_size) {
  return 4 * NumberOf32BitWords(extensions_size);
}
...
bool RtpPacket::AllocatePaddingInHeaderExtension15(size_t length) {
  ...
  if (extensions_offset + WordAlignedLength(new_extensions_size) > capacity()) {
    RTC_LOG(LS_ERROR)
...
uint16_t RtpPacket::SetExtensionLengthMaybeAddZeroPadding(
  ...
  uint16_t extensions_words = NumberOf32BitWords(extensions_size_);
...
bool RtpPacket::SetPadding(size_t padding_bytes) {
  ...
  if (padding_bytes > std::numeric_limits<uint8_t>::max()) {
    ...
    padding_bytes -= WordAlignedLength(extensions_size_);
  }
