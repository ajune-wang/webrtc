...
uint16_t ExampleVideoQualityAnalyzer::OnFrameCaptured(
  ...
  if (it == frames_in_flight_.end()) {
    ...
    frames_to_stream_.insert({frame_id, stream_label});
  } else {
    ...
    // there.
    ++frames_dropped_;
    auto stream_it = frames_to_stream_.find(frame_id);
    RTC_CHECK(stream_it != frames_to_stream_.end());
...
std::string ExampleVideoQualityAnalyzer::GetStreamLabel(uint16_t frame_id) {
  rtc::CritScope crit(&lock_);
  auto it = frames_to_stream_.find(frame_id);
  RTC_DCHECK(it != frames_to_stream_.end()) << "Unknown frame_id=" << frame_id;
  return it->second;
}
