...
struct JsepTransportDescription {
 public:
  ...
  // TODO(zhihuang): Add the ICE and DTLS variables and methods from
...
class JsepTransport2 : public sigslot::has_slots<> {
 ...
 private:
  ...
  // Negotiates and sets the transport parameters based on the current local and
  ...
  void ApplyIceLocalTransportDescription(IceTransportInternal* ice);

  // Pushes down the ICE parameters from the remote description to the
  // transport.
  void ApplyIceRemoteTransportDescription(IceTransportInternal* ice);
