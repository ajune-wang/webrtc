...
void ErleEstimator::Update(rtc::ArrayView<const float> render_spectrum,
  ...
  constexpr int kFramesOnsetDetection = kErleHold + 150;
  ...
  auto erle_update = [&](size_t start, size_t stop, float max_erle,
    ...
    for (size_t k = start; k < stop; ++k) {
      if (converged_filter && (X2[k] > kX2Min)) {
        ...
        if (num_points_[k] == kPointsToAccumulate) {
          if (E2_acum_[k] > 0) {
            ...
            hold_counters_[k - 1] = kFramesOnsetDetection;
  ...
  erle_update(1, kFftLengthBy4, max_erle_lf_, converged_filter);
  erle_update(kFftLengthBy4, kFftLengthBy2, max_erle_hf_, converged_filter);

  for (size_t k = 0; k < hold_counters_.size(); ++k) {
    hold_counters_[k]--;
    if (hold_counters_[k] <= (kFramesOnsetDetection - kErleHold)) {
