...
PacingController::PacingController(Clock* clock,
    ...
    : mode_(mode),
      ...
      congestion_end_time_(time_last_process_),
...
void PacingController::SetCongestionWindow(DataSize congestion_window_size) {
  ...
  if (was_congested && !Congested()) {
    congestion_end_time_ = CurrentTime();
  }
}
...
TimeDelta PacingController::UpdateTimeAndGetElapsed(Timestamp now) {
  if (time_last_process_.IsMinusInfinity()) {
    return TimeDelta::Zero();
  }
...
Timestamp PacingController::NextSendTime() const {
  ...
  if (prober_.IsProbing()) {
    ...
    if (probe_time != Timestamp::PlusInfinity()) {
      // Check that don't have timeout of send failure before
      // going with the probe time.
  ...
  if (mode_ == ProcessMode::kPeriodic) {
    ...
    return time_last_process_ + min_packet_limit_;
  }

  // In dynamic mode, figure out when the next packet should be sent,
  // given the current conditions.

  // We to at least send keep-alive packets with some interval.
  ...
  // If there are pending packets, check how it will take until buffers
  // have emptied.
  ...
  // Don't schedule packets for before the last congestion end time.
  next_send_time = std::max(next_send_time, congestion_end_time_);
...
void PacingController::ProcessPackets() {
  Timestamp now = CurrentTime();
  RTC_DCHECK_GE(now, time_last_process_);
  ...
  TimeDelta time_since_target_send_time = now - target_send_time;
  ...
  while (!paused_) {
    ...
    // If |time_since_scheduled_run| is non-zero, GetPendingPacket() will return
    // new packets as long as the queue is not empty and accumulated debt is not
    // greater than |time_since_scheduled_run| * |media_rate_|.
    auto* packet = GetPendingPacket(pacing_info, time_since_target_send_time);
    if (packet == nullptr) {
  ...
  if (mode_ == ProcessMode::kDynamic &&
      time_since_target_send_time > TimeDelta::Zero()) {
    // Pay off any debt accumulated between now and when we should have sent
    // packets.
    UpdateBudgetWithElapsedTime(time_since_target_send_time);
  }
}
...
RoundRobinPacketQueue::QueuedPacket* PacingController::GetPendingPacket(
    ...
    TimeDelta time_since_target_send_time) {
  if (packet_queue_.Empty()) {
    return nullptr;
  }
  ...
  if (!(unpaced_audio_packet || is_probe)) {
    ...
    if (mode_ == ProcessMode::kPeriodic) {
      ...
      // We allow sending up |kMinSleepTime| early, if we think that we
      // would actually be able to send if we were on time - i.e. the current
      // debt is not more than would be reduced to zero.
      // Similarly, if we are late, allow sending while the debt is lower
      // than what would have been reduced to zero since the scheduled
      // send time.
      if (media_debt_ > time_since_target_send_time.Abs() * media_rate_) {
        return nullptr;
      }
    }
  }
