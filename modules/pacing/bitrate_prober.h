...
class BitrateProber {
 public:
  ...
  // with.
  void OnIncomingPacket(size_t packet_size, Timestamp now);
  ...
  absl::optional<Timestamp> NextProbeTime(Timestamp now) const;
 ...
 private:
  ...
  Timestamp GetNextProbeTime(const ProbeCluster& cluster);
