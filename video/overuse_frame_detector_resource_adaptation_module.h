...
class OveruseFrameDetectorResourceAdaptationModule
 ...
 public:
  ...
  void OnInputFrame(const VideoFrame& frame) override;
  void OnInputFrameDroppedDueToSize() override;
  void OnInputFrameEncodeStarted(const VideoFrame& cropped_frame,
                                 int64_t time_when_first_seen_us) override;
  void OnInputFrameEncodeCompleted(
      uint32_t timestamp,
