...
void LibvpxVp8Encoder::PopulateCodecSpecific(CodecSpecificInfo* codec_specific,
  ...
  if (codec_specific->encoder_buffers) {
    // Rename the encoder buffers so that different buffers from different
    // encoder instances does not collide.
    for (EncoderBuffer& buffer : *codec_specific->encoder_buffers) {
      constexpr int kNumBuffersVp8 = 3;
      buffer.id = buffer.id + kNumBuffersVp8 * stream_idx;
    }
  }
}
