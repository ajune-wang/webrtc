...
void WebRtcVideoChannel::BackfillBufferedPackets(
  ...
  out << "]";
  auto level = delivery_unknown_ssrc_cnt
                   ? rtc::LS_ERROR
                   : delivery_packet_error_cnt ? rtc::LS_WARNING : rtc::LS_INFO;
  int total =
      delivery_ok_cnt + delivery_unknown_ssrc_cnt + delivery_packet_error_cnt;
  RTC_LOG_FILE_LINE(level, __FILE__, __LINE__)
      << "Backfilled " << total << " packets for ssrcs: " << out.Release()
      << " ok: " << delivery_ok_cnt << " error: " << delivery_packet_error_cnt
