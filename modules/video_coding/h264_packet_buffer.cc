...
rtc::CopyOnWriteBuffer FixVideoPayload(rtc::ArrayView<const uint8_t> payload,
  ...
  switch (h264_header.packetization_type) {
    ...
    case kH264FuA: {
      ...
      res.AppendData(payload.data(), payload.size());
      return res;
    }
...
H264PacketBuffer::InsertResult H264PacketBuffer::InsertPacket(
  ...
  ClearOldPacketsBeforeInsertingNewPacket(unwrapped_seq_num);
  const int index = EuclideanMod(unwrapped_seq_num, kBufferSize);
  if (buffer_[index]) {
...
void H264PacketBuffer::ClearOldPacketsBeforeInsertingNewPacket(
    int64_t unwrapped_seq_num) {
  int64_t last_seq_num =
      last_cleared_unwrapped_seq_num_.value_or(unwrapped_seq_num);
  for (int64_t i = last_seq_num + 1; i <= unwrapped_seq_num; ++i) {
    GetPacket(i).reset();
  }

  if (last_cleared_unwrapped_seq_num_ < unwrapped_seq_num) {
    last_cleared_unwrapped_seq_num_ = unwrapped_seq_num;
  }
}
...
H264PacketBuffer::FindFrames(int64_t unwrapped_seq_num) {
  ...
  for (int64_t seq_num = unwrapped_seq_num;
    ...
    // In case of RTX old packets can be reinserted in the 'buffer_'. Check that
    // the packet sequence number match the expected unwrapped sequence number.
