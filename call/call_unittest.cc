...
TEST(CallTest, RegisterMediaTransportBitrateCallbacksInMediaTransportChange) {
  CallHelper call;
...
TEST(CallTest, RegisterMediaTransportBitrateIgnoredIfAlreadyRegistered) {
  CallHelper call;
  MediaTransportSettings settings;
  webrtc::FakeMediaTransport fake_media_transport(settings);
  ...
  // We already had 1 subscriber, so now just |ChannelSend| needs to subscribe.
  EXPECT_EQ(2, fake_media_transport.target_rate_observers_size());
