...
void RtpTransceiver::AddSender(
  ...
  RTC_DCHECK(sender);
  RTC_DCHECK_EQ(type(), sender->internal()->media_type());
...
bool RtpTransceiver::RemoveSender(RtpSenderInterface* sender) {
  if (sender) {
    RTC_DCHECK_EQ(type(), sender->media_type());
  }
...
void RtpTransceiver::AddReceiver(
    rtc::scoped_refptr<RtpReceiverProxyWithInternal<RtpReceiverInternal>>
        receiver) {
  RTC_DCHECK(receiver);
  RTC_DCHECK_EQ(type(), receiver->internal()->media_type());
...
bool RtpTransceiver::RemoveReceiver(RtpReceiverInterface* receiver) {
  if (receiver) {
    RTC_DCHECK_EQ(type(), receiver->media_type());
  }
