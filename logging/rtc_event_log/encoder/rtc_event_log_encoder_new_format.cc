...
void RtcEventLogEncoderNewFormat::EncodeGenericAcksReceived(
  ...
  absl::optional<uint64_t> base_timestamp = 0;
  if (base_event->receive_timestamp_ms()) {
    base_timestamp = ToUnsigned(*base_event->receive_timestamp_ms());
    proto_batch->set_receive_timestamp_ms(*base_event->receive_timestamp_ms());
  }
  ...
  encoded_deltas = EncodeDeltas(base_timestamp, values);
  if (!encoded_deltas.empty()) {
