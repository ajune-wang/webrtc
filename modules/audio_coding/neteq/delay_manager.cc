...
absl::optional<int> GetDecelerationTargetLevelOffsetQ8() {
  ...
  if (deceleration_target_level_offset >= 0) {
    RTC_LOG(LS_INFO) << "NetEq deceleration_target_level_offset "
                     << "in milliseconds " << deceleration_target_level_offset;
    // Convert into Q8.
    return deceleration_target_level_offset << 8;
  }
  return absl::nullopt;
}
...
DelayManager::DelayManager(size_t max_packets_in_buffer,
    ...
    : first_packet_received_(false),
      ...
      deceleration_target_level_offset_(GetDecelerationTargetLevelOffsetQ8()) {
...
void DelayManager::BufferLimits(int target_level,
  ...
  *lower_limit = (target_level * 3) / 4;
  ...
  if (deceleration_target_level_offset_ && packet_len_ms_ > 0) {
    *lower_limit = std::max(
        *lower_limit,
        target_level - *deceleration_target_level_offset_ / packet_len_ms_);
  }
