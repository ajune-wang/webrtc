...
class EventLogAnalyzer {
 ...
 private:
  ...
  void CreateAccumulatedPacketsTimeSeries(Plot* plot,
                                          const PacketView<T>& packets,
                                          const std::string& label);

  void CreateStreamGapAlerts(PacketDirection direction);
  void CreateTransmissionGapAlerts(PacketDirection direction);
  // void DetectSeqNumJump(PacketDirection direction, const RtpPacketView
  // packet_stream, int64_t end_time_us); void
  // DetectCaptureTimeJump(PacketDirection direction, const RtpPacketView
  // packet_stream, int64_t end_time_us);
  ...
  void Alert_RtpLogTimeGap(PacketDirection direction,
                           ...
                           int64_t duration) {
    if (direction == kIncomingPacket) {
      incoming_rtp_recv_time_gaps.emplace_back(time_seconds, duration);
    } else {
      outgoing_rtp_send_time_gaps.emplace_back(time_seconds, duration);
    }
  }
  ...
  void Alert_RtcpLogTimeGap(PacketDirection direction,
                            ...
                            int64_t duration) {
    if (direction == kIncomingPacket) {
      incoming_rtcp_recv_time_gaps.emplace_back(time_seconds, duration);
    } else {
      outgoing_rtcp_send_time_gaps.emplace_back(time_seconds, duration);
    }
  }
  ...
  void Alert_SeqNumJump(PacketDirection direction,
                        ...
                        uint32_t ssrc) {
    if (direction == kIncomingPacket) {
      incoming_seq_num_jumps.emplace_back(time_seconds, ssrc);
    } else {
      outgoing_seq_num_jumps.emplace_back(time_seconds, ssrc);
    }
  }
  ...
  void Alert_CaptureTimeJump(PacketDirection direction,
                             ...
                             uint32_t ssrc) {
    if (direction == kIncomingPacket) {
      incoming_capture_time_jumps.emplace_back(time_seconds, ssrc);
    } else {
      outgoing_capture_time_jumps.emplace_back(time_seconds, ssrc);
    }
  }

  void Alert_OutgoingHighLoss(double avg_loss_fraction) {
    outgoing_high_loss_alerts.emplace_back(avg_loss_fraction);
  }
  ...
  std::vector<IncomingRtpReceiveTimeGap> incoming_rtp_recv_time_gaps;
  std::vector<IncomingRtcpReceiveTimeGap> incoming_rtcp_recv_time_gaps;
  std::vector<OutgoingRtpSendTimeGap> outgoing_rtp_send_time_gaps;
  std::vector<OutgoingRtcpSendTimeGap> outgoing_rtcp_send_time_gaps;
  std::vector<IncomingSeqNumJump> incoming_seq_num_jumps;
  std::vector<IncomingCaptureTimeJump> incoming_capture_time_jumps;
  std::vector<OutgoingSeqNoJump> outgoing_seq_num_jumps;
  std::vector<OutgoingCaptureTimeJump> outgoing_capture_time_jumps;
  std::vector<OutgoingHighLoss> outgoing_high_loss_alerts;
