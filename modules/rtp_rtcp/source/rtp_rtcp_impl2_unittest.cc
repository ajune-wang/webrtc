...
MATCHER_P(SenderReportStatsReportsCountEq, n, "") {
  return arg.reports_count == n;
}
...
TEST_F(RtpRtcpImpl2Test, SenderReportStatsCount) {
  ...
  EXPECT_THAT(receiver_.impl_->GetSenderReportStats(),
              Optional(SenderReportStatsReportsCountEq(1u)));
  // Send the second SR.
  ASSERT_EQ(0, sender_.impl_->SendRTCP(kRtcpReport));
  EXPECT_THAT(receiver_.impl_->GetSenderReportStats(),
              Optional(SenderReportStatsReportsCountEq(2u)));
}

// Checks the arrival and remote timestamps for the last received RTCP SR.
TEST_F(RtpRtcpImpl2Test, SenderReportStatsCheckArrivalRemoteTimestamps) {
  // Send a frame in order to send an SR.
  SendFrame(&sender_, sender_video_.get(), kBaseLayerTid);
  NtpTime expected_remote_ntp = time_controller_.GetClock()->CurrentNtpTime();
  // Send an SR.
  ASSERT_EQ(0, sender_.impl_->SendRTCP(kRtcpReport));
  NtpTime expected_arrival_ntp = time_controller_.GetClock()->CurrentNtpTime();

  auto stats = receiver_.impl_->GetSenderReportStats();
  ASSERT_TRUE(stats.has_value());
  EXPECT_EQ(stats->last_arrival_timestamp_secs, expected_arrival_ntp.seconds());
  EXPECT_EQ(stats->last_arrival_timestamp_frac,
            expected_arrival_ntp.fractions());
  EXPECT_EQ(stats->last_remote_timestamp_secs, expected_remote_ntp.seconds());
  EXPECT_EQ(stats->last_remote_timestamp_frac, expected_remote_ntp.fractions());
}
