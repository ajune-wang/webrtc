...
void SimulatedThread::RunReady(Timestamp at_time) {
  ...
  auto* manager = rtc::ThreadManager::Instance();
  rtc::Thread* previous_thread = manager->CurrentThread();
  manager->SetCurrentThread(this);
  ProcessMessages(0);
  int delay_ms = GetDelay();
  manager->SetCurrentThread(previous_thread);
