...
class WrappedVideoDecoder : public VideoDecoder {
 public:
  WrappedVideoDecoder(StubVideoDecoder* decoder) : decoder_(decoder) {}
...
class FakeVideoDecoderFactory : public VideoDecoderFactory {
 ...
 private:
  ::testing::NiceMock<StubVideoDecoder> vp8_decoder_;
  ::testing::NiceMock<StubVideoDecoder> av1_decoder_;
};
...
class FakeEncodedFrameBuilder {
 public:
  FakeEncodedFrameBuilder() : frame_(std::make_unique<FakeEncodedFrame>()) {}

  FakeEncodedFrameBuilder& WithPayloadType(int payload_type) {
    frame_->SetPayloadType(payload_type);
    return *this;
  }

  FakeEncodedFrameBuilder& WithPictureId(int picture_id) {
...
static FakeEncodedFrameBuilder FrameBuilder() {
  return FakeEncodedFrameBuilder();
}
  ...
  VideoStreamDecoderImplTest()
      : sim_time_controller_(Timestamp::seconds(0)),
        video_stream_decoder_(&callbacks_,
                              &decoder_factory_,
                              sim_time_controller_.GetTaskQueueFactory(),
                              {{1, std::make_pair(SdpVideoFormat("VP8"), 1)},
                               {2, std::make_pair(SdpVideoFormat("AV1"), 1)}}) {
  }

  ::testing::NiceMock<MockVideoStreamDecoderCallbacks> callbacks_;
  FakeVideoDecoderFactory decoder_factory_;
  GlobalSimulatedTimeController sim_time_controller_;
...
TEST_F(VideoStreamDecoderImplTest, InsertAndDecodeFrame) {
  ...
  sim_time_controller_.AdvanceTime(TimeDelta::ms(1));
}

TEST_F(VideoStreamDecoderImplTest, RequestKeyframeWaitingForKeyframe) {
  EXPECT_CALL(callbacks_, OnNonDecodableState);
  sim_time_controller_.AdvanceTime(TimeDelta::ms(200));
}

TEST_F(VideoStreamDecoderImplTest, RequestKeyframeWaitingForDeltaFrame) {
  video_stream_decoder_.OnFrame(FrameBuilder().WithPayloadType(1).Build());
  EXPECT_CALL(callbacks_, OnDecodedFrame);
  sim_time_controller_.AdvanceTime(TimeDelta::ms(1));
  EXPECT_CALL(callbacks_, OnNonDecodableState);
  sim_time_controller_.AdvanceTime(TimeDelta::ms(3000));
}

TEST_F(VideoStreamDecoderImplTest, InsertAndDecodeFrameWithKeyframeRequest) {
  video_stream_decoder_.OnFrame(FrameBuilder().WithPayloadType(1).Build());
  EXPECT_CALL(decoder_factory_.Vp8Decoder(), Decode)
      .WillOnce(Return(WEBRTC_VIDEO_CODEC_OK_REQUEST_KEYFRAME));
  EXPECT_CALL(callbacks_, OnNonDecodableState);
  sim_time_controller_.AdvanceTime(TimeDelta::ms(1));
}

TEST_F(VideoStreamDecoderImplTest, InsertAndFailToDecodeFrame) {
  video_stream_decoder_.OnFrame(FrameBuilder().WithPayloadType(1).Build());
  EXPECT_CALL(decoder_factory_.Vp8Decoder(), Decode)
