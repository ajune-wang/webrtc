...
bool SctpTransport::SendData(const SendDataParams& params,
  ...
  OutgoingMessage message(payload, params, /*offset=*/0);
...
size_t SctpTransport::SendMessageInternal(const OutgoingMessage& message,
  ...
  size_t payload_size = message.buffer.size() - message.offset;
  if (!sock_) {
    RTC_LOG(LS_WARNING) << debug_name_ << "->SendMessageInternal(...): "
                        << "Not sending packet with sid="
                        << message.send_params.sid << " len=" << payload_size
                        << " before Start().";
    return 0;
  }
  ...
  if (payload_size > static_cast<size_t>(max_message_size_)) {
    RTC_LOG(LS_ERROR) << "Attempting to send message of size " << payload_size
  ...
  ssize_t send_res = usrsctp_sendv(
      sock_, message.buffer.data() + message.offset,
      static_cast<size_t>(payload_size), NULL, 0, &spa,
...
bool SctpTransport::SendBufferedMessage() {
  RTC_DCHECK_RUN_ON(network_thread_);
  RTC_DCHECK(partial_outgoing_message_.has_value());
  size_t remaining_message_size = partial_outgoing_message_->buffer.size() -
                                  partial_outgoing_message_->offset;
  ...
  if (sent_size < remaining_message_size) {
    partial_outgoing_message_->offset += sent_size;
    return false;
  }
