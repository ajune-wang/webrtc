...
// TODO(tommi): Looks like copy/paste code between this file and
...
// TODO(tommi): (looks like copy/paste code between this function and
// InputVolumeController::AnalyzeInputAudio).
void AgcManagerDirect::AnalyzePreProcess(const AudioBuffer& audio_buffer) {
  ...
  RTC_CHECK_LE(num_capture_channels_, NumChannels(audio));
  if (static_cast<size_t>(num_capture_channels_) != NumChannels(audio)) {
    // In case the `audio_buffer` represents more channels than
    // `num_capture_channels_`, clamp it down to match.
    audio = DeinterleavedView<const float>(audio.data().data(),
