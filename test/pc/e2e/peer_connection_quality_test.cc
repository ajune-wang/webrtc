...
PeerConnectionE2EQualityTest::PeerConnectionE2EQualityTest(
  ...
  stats_observers_.push_back(video_quality_analyzer.get());
  ...
  stats_observers_.push_back(audio_quality_analyzer_.get());
}
...
void PeerConnectionE2EQualityTest::Run(
  ...
  StatsPoller stats_poller(stats_observers_,
  ...
  RTC_CHECK(stats_polling_stopped.Wait(kStatsPollingStopTimeout.ms()))
      << "Failed to stop Stats polling after "
