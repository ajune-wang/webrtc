...
- (id)initWithOwner:(VideoCaptureIos*)owner {
  if (self = [super init]) {
    ...
    if ([_captureSession canAddOutput:captureOutput]) {
      ...
      NLOG(LS_ERROR, __FUNCTION__, ": Could not add output to AVCaptureSession");
    }
...
- (void)onVideoError:(NSNotification*)notification {
  ...
  NLOG(LS_ERROR, __FUNCTION__, ": [AVCaptureSession startRunning] error.");
}
...
- (BOOL)changeCaptureInputByUniqueId:(NSString*)uniqueId {
  ...
  if (!newCaptureInput) {
    ...
    NLOG(LS_ERROR, __FUNCTION__, ": deviceInputWithDevice error:", errorMessage);
