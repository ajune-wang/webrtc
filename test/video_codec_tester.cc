 ...
 private:
  ...
  void Configure(const EncodingSettings& es) {
    ...
    for (int sidx = 0; sidx < num_spatial_layers; ++sidx) {
      ...
      // In simulcastStream[] streams are ordered from low to high resolution.
      // Int spatialLayers[] layers are ordered from high to low resolution.
      SimulcastStream& ss =
          is_simulcast_codec ? vc.simulcastStream[sidx]
                             : vc.spatialLayers[num_spatial_layers - sidx - 1];
