...
enum TrackAdditionMode { kAddStream, kAddTrack };

class StatsCollectorTrackTest
    : public StatsCollectorTest,
      public ::testing::WithParamInterface<TrackAdditionMode> {
 public:
...
TEST_P(StatsCollectorTrackTest, FilterOutNegativeInitialValues) {
  ...
  EXPECT_CALL(pc_, GetLocalTrackIdBySsrc(kSsrcOfTrack, _))
      .WillOnce(DoAll(SetArgPointee<1>(kLocalTrackId), Return(true)));
  RTC_DCHECK(GetParam() == kAddStream);
  stats.AddStream(stream_);
...
INSTANTIATE_TEST_CASE_P(NoStream,
                        StatsCollectorTrackTest,
                        ::testing::Values(kAddStream, kAddTrack));
