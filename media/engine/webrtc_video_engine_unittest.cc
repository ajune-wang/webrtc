...
TEST_F(WebRtcVideoChannelTest, GetStatsTranslatesReceivePacketStatsCorrectly) {
  ...
  FakeVideoReceiveStream* stream = AddRecvStream();
  webrtc::VideoReceiveStream::Stats stats;
  stats.rtp_stats.packet_counter.transmitted.payload_bytes = 2;
  stats.rtp_stats.packet_counter.transmitted.header_bytes = 3;
  stats.rtp_stats.packet_counter.transmitted.padding_bytes = 4;
  stats.rtp_stats.packet_counter.transmitted.packets = 5;
  stats.rtp_stats.packets_lost = 6;
  stream->SetStats(stats);

  cricket::VideoMediaInfo info;
  ASSERT_TRUE(channel_->GetStats(&info));
  EXPECT_EQ(stats.rtp_stats.packet_counter.transmitted.payload_bytes,
            rtc::checked_cast<size_t>(info.receivers[0].bytes_rcvd));
  EXPECT_EQ(stats.rtp_stats.packet_counter.transmitted.packets,
