...
- (void)startCapture {
    [RTCDispatcher dispatchAsyncOnType:RTCDispatcherTypeCaptureSession
                                 block:^{
                                     ...
                                     CVPixelBufferCreate(nil, 1280, 720, kCVPixelFormatType_420YpCbCr8BiPlanarFullRange, nil, &_last_color_buffer);
                                     ...
                                     for (int i = 0; i < kNumBuffer; i++) {
                                         ...
                                         CVPixelBufferCreate(nil, 640, 480, kCVPixelFormatType_420YpCbCr8BiPlanarFullRange, nil, &_out_color_buffer_queue[i]);
                                         ...
                                         CVPixelBufferCreate(nil, 1280, 720, kCVPixelFormatType_420YpCbCr8BiPlanarFullRange, nil, &_pre_color_buffer_queue[i]);
                                     }
#endif
                                 }];
}
...
- (void)stopCapture {
    [RTCDispatcher dispatchAsyncOnType:RTCDispatcherTypeCaptureSession
                                 block:^{
                                     ...
                                     if (_last_color_buffer != NULL) {
                                         CVPixelBufferRelease(_last_color_buffer);
                                         _last_color_buffer = NULL;
                                     }
                                     ...
                                     for (int i = 0; i < kNumBuffer; i++) {
                                         ...
                                         if (_out_color_buffer_queue[index] != NULL) {
                                             CVPixelBufferRelease(_out_color_buffer_queue[index]);
                                             _out_color_buffer_queue[index] = NULL;
                                         }
                                         ...
                                         if (_pre_color_buffer_queue[index] != NULL) {
                                             CVPixelBufferRelease(_pre_color_buffer_queue[index]);
                                             _pre_color_buffer_queue[index] = NULL;
                                         }
                                     }
...
- (void)session:(ARSession *)sessionn didUpdateFrame:(ARFrame *)frame {
    ...
    [self copyColorBufferFrom: color_frame
                           To: _color_buffer_queue[buffer_index]];

    if (depth_data == NULL) {
        [self copyColorBufferFrom:_last_color_buffer To:_pre_color_buffer_queue[buffer_index]];
    }

    [self copyColorBufferFrom:color_frame To:_last_color_buffer];
    ...
    [RTCDispatcher dispatchAsyncOnType:RTCDispatcherTypeProcess
                                 block:^{
                                     [self segmentAndStreamDownAtIndex:buffer_index
                                                      ...
                                                      withCurrentDepth:(depth_data != NULL)];
                                 }];
}
