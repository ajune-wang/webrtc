...
#ifndef MODULES_VIDEO_CODING_UTILITY_FILTER_SETTINGS_GENERATOR_H_
#define MODULES_VIDEO_CODING_UTILITY_FILTER_SETTINGS_GENERATOR_H_
...
class FilterSettingsGenerator {
 public:
  // A struct with the parameters for a ration function use to determine the
  ...
  // A struct with the parameters for an exponential functionused to determine
  ...
  struct TransientParameters {
    ...
    // Don't use a filter kernel samller than this.
    double std_dev_lower_bound = 0.0;
  };

  FilterSettingsGenerator(const RationalFunctionParameters& function_params,
                          const ErrorThresholds& default_error_thresholds,
                          const TransientParameters& transient_params);
  FilterSettingsGenerator(const ExponentialFunctionParameters& function_params,
                          const ErrorThresholds& default_error_thresholds,
                          const TransientParameters& transient_params);
 ...
 private:
  ...
  std::optional<int> previoius_qp_;
};

}  // namespace webrtc

#endif  // MODULES_VIDEO_CODING_UTILITY_FILTER_SETTINGS_GENERATOR_H_
