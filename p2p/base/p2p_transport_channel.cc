...
void P2PTransportChannel::AddAllocatorSession(
  ...
  uint32_t generation = static_cast<uint32_t>(allocator_sessions_.size());
  if (shared_gatherer_) {
    // We need to always be bigger than the shared_gatherer_'s generation
...
void P2PTransportChannel::StartGatheringWithSharedGatherer(
    rtc::scoped_refptr<cricket::IceGathererInterface> shared_gatherer) {
  RTC_DCHECK_RUN_ON(network_thread_);
  if (gathering_state_ != kIceGatheringGathering) {
    gathering_state_ = kIceGatheringGathering;
    SignalGatheringState(this);
  ...
  auto* session = shared_gatherer->port_allocator_session();
  session->SignalPortReady.connect(this, &P2PTransportChannel::OnPortReady);
  session->SignalPortsPruned.connect(this, &P2PTransportChannel::OnPortsPruned);
  session->SignalCandidatesReady.connect(
      this, &P2PTransportChannel::OnCandidatesReady);
  ...
  // If that is ever not the case, we should add an IceGathererInterface::Start
  // method that makes sure StartGettingPorts is only called once and then call
  // that method here.
  session->StartGettingPorts();
  shared_gatherer_ = shared_gatherer;
}
...
void P2PTransportChannel::OnUnknownAddress(PortInterface* port,
  ...
  if (ice_param != nullptr) {
    ...
    // If we don't know the remote ufrag and the session is shared between
