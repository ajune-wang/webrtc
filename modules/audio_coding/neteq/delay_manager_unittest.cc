...
TEST_F(DelayManagerTest, UpdateNormal) {
  ...
  EXPECT_CALL(detector_, Update(1, 1, false)).WillOnce(Return(false));
  InsertNextPacket();
...
TEST_F(DelayManagerTest, UpdateLongInterArrivalTime) {
  ...
  // equal to 1 packet, and (base) target level equal to 1 as well.
  // Return false to indicate no peaks found.
  EXPECT_CALL(detector_, Update(2, 2, false)).WillOnce(Return(false));
  InsertNextPacket();
...
TEST_F(DelayManagerTest, UpdatePeakFound) {
  ...
  EXPECT_CALL(detector_, Update(1, 1, false)).WillOnce(Return(true));
...
TEST_F(DelayManagerTest, MaxDelay) {
  ...
  EXPECT_CALL(detector_, Update(kExpectedTarget, _, false))
...
TEST_F(DelayManagerTest, UpdateReorderedPacket) {
  ...
  EXPECT_CALL(detector_, Update(_, _, true));
...
TEST_F(DelayManagerTest, EmptyPacketsNotReported) {
  ...
  // equal to 1 packet, and (base) target level equal to 1 as well.
  // Return false to indicate no peaks found.
  EXPECT_CALL(detector_, Update(10, 10, false)).WillOnce(Return(false));
  InsertNextPacket();
