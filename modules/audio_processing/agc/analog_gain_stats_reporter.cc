...
void AnalogGainStatsReporter::UpdateStatistics(int analog_mic_level) {
  const int level = rtc::SafeClamp<int>(analog_mic_level, kMinGain, kMaxGain);
  const bool level_changed = (previous_analog_mic_level_.has_value())
                                 ? level != previous_analog_mic_level_.value()
                                 : false;
  if (level_changed) {
  ...
  previous_analog_mic_level_ = level;
}
