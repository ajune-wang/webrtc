...
int SimulcastEncoderAdapter::InitEncode(
  ...
  num_active_layers_ = active_streams.num_active_streams;
  first_active_layer_ = active_streams.first_active_stream_idx;
  ...
  auto spatial_layers = rtc::ArrayView<SpatialLayer>(codec_.simulcastStream,
...
int SimulcastEncoderAdapter::Encode(
  ...
  for (auto& layer : encoder_contexts_) {
    ...
    if (send_key_frame) {
      ...
      layer.OnKeyframe(frame_timestamp_ms);
    } else {
      if (layer.ShouldDropFrame(frame_timestamp_ms)) {
        continue;
      }
      std::fill(stream_frame_types.begin(), stream_frame_types.end(),
