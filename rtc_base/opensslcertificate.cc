...
//////////////////////////////////////////////////////////////////////
// OpenSSLRootCertificateLoader
//////////////////////////////////////////////////////////////////////

OpenSSLBuiltinRootCertLoader::~OpenSSLBuiltinRootCertLoader() = default;

SSLCertChain OpenSSLBuiltinRootCertLoader::Load() {
  // Add all of the SSL certificates from the sslroots header.
  std::vector<std::unique_ptr<SSLCertificate>> ssl_root_certs_added;
#ifndef WEBRTC_DISABLE_BUILT_IN_SSL_ROOT_CERTIFICATES
  const size_t num_root_certs = arraysize(kSSLCertCertificateList);
  ssl_root_certs_added.reserve(num_root_certs);
  ...
  for (size_t i = 0; i < num_root_certs; i++) {
    ...
    if (cert) {
      ssl_root_certs_added.push_back(MakeUnique<OpenSSLCertificate>(cert));
      // OpenSSLCertificate uprefs on construction, so freeing is correct.
      X509_free(cert);
    }
  }
#endif  // WEBRTC_DISABLE_BUILT_IN_SSL_ROOT_CERTIFICATES
  return SSLCertChain(std::move(ssl_root_certs_added));
}
