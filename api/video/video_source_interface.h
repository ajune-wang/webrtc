...
struct RTC_EXPORT VideoSinkWants {
  ...
  // The resolutions that the encoder is configured to encode. In singlecast we
  // only encode one resolution, but in simulcast and SVC this can mean multiple
  // resolutions per frame.
  //
  // The encoder is always configured to encode a subset of the
  // webrtc::VideoFrame's resolution. We usually encode at webrtc::VideoFrame's
  // resolution but this may not always the case due to scaleResolutionDownBy or
  // turning off simulcast or SVC layers.
  //
  ...
  // of size 480p but can optimize internal buffers for 240p, avoiding
  // downsampling to 480p if possible. Note that the |encoder_resolutions| can
  // change while frames are in flight and should only be used as a hint when
  // constructing the webrtc::VideoFrame.
  std::vector<FrameSize> encoder_resolutions;
};

bool RTC_EXPORT operator==(const VideoSinkWants::FrameSize& a,
