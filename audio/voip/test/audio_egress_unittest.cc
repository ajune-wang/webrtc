...
TEST_F(AudioEgressTest, TestSpeechInputLevelAndEnergyDuration) {
  ...
  EXPECT_EQ(egress_->GetSpeechInputLevelFullRange(), kAudioLevel);
  EXPECT_DOUBLE_EQ(egress_->GetSpeechInputEnergy(), kExpectedEnergy);
  EXPECT_DOUBLE_EQ(egress_->GetSpeechInputDuration(), kExpectedDuration);
}
