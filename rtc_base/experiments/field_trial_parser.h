...
class FieldTrialParameterInterface {
 ...
 protected:
  ...
  virtual bool TryParse(const std::string& value) = 0;
...
class FieldTrialParameter : public FieldTrialParameterInterface {
 ...
 protected:
  bool TryParse(const std::string& str_value) override {
 ...
 protected:
  bool TryParse(const std::string& str_value) override {
    if (str_value.empty()) {
      ...
      rtc::Optional<rtc::Optional<T>> value = ParseTypedParameter<T>(str_value);
      if (value.has_value())
        value_ = value.value();
      return value.has_value();
    }
