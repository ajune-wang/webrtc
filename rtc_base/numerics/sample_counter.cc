/*
 *  Copyright (c) 2018 The WebRTC project authors. All Rights Reserved.
 *
 *  Use of this source code is governed by a BSD-style license
 *  that can be found in the LICENSE file in the root of the source
 *  tree. An additional intellectual property rights grant can be found
 *  in the file PATENTS.  All contributing project authors may
 *  be found in the AUTHORS file in the root of the source tree.
 */

#include <limits>

#include "rtc_base/checks.h"
#include "rtc_base/numerics/sample_counter.h"

namespace rtc {

SampleCounter::SampleCounter() {}
SampleCounter::~SampleCounter() = default;

void SampleCounter::Add(int sample) {
  RTC_DCHECK(sample <= std::numeric_limits<int64_t>::max() - sum_);
  sum_ += sample;
  sum_squared_ += static_cast<int64_t>(sample) * sample;
  RTC_DCHECK(static_cast<int64_t>(sample) * sample <=
             std::numeric_limits<int64_t>::max() - sum_squared_);
  ++num_samples_;
  if (!max_ || sample > *max_) {
    max_ = sample;
  }
}

void SampleCounter::Add(const SampleCounter& other) {
  RTC_DCHECK(other.sum_ <= std::numeric_limits<int64_t>::max() - sum_);
  sum_ += other.sum_;
  RTC_DCHECK(other.sum_squared_ <=
             std::numeric_limits<int64_t>::max() - sum_squared_);
  sum_squared_ += other.sum_squared_;
  RTC_DCHECK(other.num_samples_ <=
             std::numeric_limits<int64_t>::max() - num_samples_);
  num_samples_ += other.num_samples_;
  if (other.max_ && (!max_ || *max_ < *other.max_))
    max_ = other.max_;
}

rtc::Optional<int> SampleCounter::Avg(int64_t min_required_samples) const {
  if (num_samples_ < min_required_samples || num_samples_ == 0)
    return -1;
  return static_cast<int>(sum_ / num_samples_);
}

rtc::Optional<int64_t> SampleCounter::Variance(
    int64_t min_required_samples) const {
  if (num_samples_ < min_required_samples || num_samples_ == 0)
    return -1;
  // E[(x-mean)^2] = E[x^2] - mean^2
  int64_t mean = sum_ / num_samples_;
  return sum_squared_ / num_samples_ - mean * mean;
}

rtc::Optional<int> SampleCounter::Max() const {
  return max_.value_or(-1);
}

void SampleCounter::Reset() {
  num_samples_ = 0;
  sum_ = 0;
  sum_squared_ = 0;
  max_.reset();
}

}  // namespace rtc
