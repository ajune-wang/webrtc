...
const Timestamp kStartTime = Timestamp::Millis(123456789);
const int kDefaultPayloadType = 100;
const uint16_t kStartSequenceNumber = 33;
...
class TestTransport : public Transport {
 public:
  ...
  bool SendRtp(const uint8_t* packet,
    ...
    last_packet_.emplace(rtc::ArrayView<const uint8_t>(packet, length), options,
                         extensions_);
    return true;
  }
...
class RtpSenderEgressTest : public ::testing::TestWithParam<TestConfig> {
 protected:
  ...
  void SetUp() override { SetUp(GetDefaultConfig()); }

  void SetUp(const RtpRtcp::Configuration& config) {
...
TEST_P(RtpSenderEgressTest, TransportFeedbackObserverGetsCorrectByteCount) {
  ...
  egress_->SendPacket(packet.get(), PacedPacketInfo());
}
