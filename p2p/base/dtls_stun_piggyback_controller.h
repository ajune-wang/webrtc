...
class DtlsStunPiggybackController {
 public:
  ...
  enum class State {
    ...
    CONFIRMED = 1,
    // We are waiting for the final ack.
    PENDING = 2,
    // We successfully negotiated Dtls in STUN.
    COMPLETE = 3,
  ...
  std::optional<uint32_t> GetAckToPiggyback(StunMessageType stun_message_type);

  // Called by Connection when receiving a STUN BINDING { REQUEST / RESPONSE }.
  // TODO: UInt16List for ack?
