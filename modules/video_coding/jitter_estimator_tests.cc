...
TEST_F(TestVCMJitterEstimator, TestUpperBound) {
  ...
  test_cases[0].upper_bound =
      100.0;  // First use essentially no upper bound and nullopt codepath.
  test_cases[0].jitter_est_cap_ms = absl::nullopt;
  test_cases[1].upper_bound =
      3.5;  // Second, reasonably small upper bound and nullopt codepath.
  test_cases[1].jitter_est_cap_ms = absl::nullopt;
  test_cases[2].upper_bound =
      100;  // Third, upper bound provided by non-nullopt, jitter_est_cap_ms.
  test_cases[2].jitter_est_cap_ms = 3.5;
  ...
  // Max should be lower when jitter_est_cap_ms-bounded case (test case 3).
  /*max_unbound = *test_cases[0].percentiles.GetPercentile(1.0);
  max_bounded = *test_cases[2].percentiles.GetPercentile(1.0);
  EXPECT_GT(max_unbound, static_cast<uint32_t>(max_bounded * 1.25));*/
}
