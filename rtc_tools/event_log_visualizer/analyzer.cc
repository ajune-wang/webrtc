...
rtc::Optional<double> NetworkDelayDiff_AbsSendTime(
  ...
  if (old_packet.header.extension.hasAbsoluteSendTime &&
    ...
    int64_t recv_time_diff = new_packet.timestamp_us - old_packet.timestamp_us;
...
rtc::Optional<double> NetworkDelayDiff_CaptureTime(
  ...
  if (delay_change < -10000 || 10000 < delay_change) {
    ...
    RTC_LOG(LS_WARNING) << "Old capture time " << old_packet.header.timestamp
                        << ", received time " << old_packet.timestamp_us;
    RTC_LOG(LS_WARNING) << "New capture time " << new_packet.header.timestamp
                        << ", received time " << new_packet.timestamp_us;
...
void ProcessPoints(rtc::FunctionView<rtc::Optional<float>(const DataType&)> f,
  ...
  for (size_t i = 0; i < data_view.size(); i++) {
    ...
    float x = static_cast<float>(elem.timestamp_us - begin_time) /
              kNumMicrosecsPerSec;
...
void ProcessPairs(
  ...
  for (size_t i = 1; i < data.size(); i++) {
    float x = static_cast<float>(data[i].timestamp_us - begin_time) /
              kNumMicrosecsPerSec;
...
void MovingAverage(
  ...
  for (int64_t t = begin_time; t < end_time + step; t += step) {
    while (window_index_end < data_view.size() &&
           data_view[window_index_end].timestamp_us < t) {
    ...
    while (window_index_begin < data_view.size() &&
           data_view[window_index_begin].timestamp_us <
...
void EventLogAnalyzer::CreatePacketGraph(PacketDirection direction,
  ...
  for (auto& kv : parsed_log_.rtp_packets(direction)) {
    ...
    const RtpPacketView packet_stream = kv.second;
    // Filter on SSRC.
    ...
    ProcessPoints<LoggedRtpPacket, RtpPacketView>(GetPacketSize, packet_stream,
...
void EventLogAnalyzer::CreateAccumulatedPacketsTimeSeries(
  ...
  for (size_t i = 0; i < packets.size(); i++) {
    float x = ToCallTime(packets[i].timestamp_us);
...
void EventLogAnalyzer::CreateAccumulatedPacketsGraph(PacketDirection direction,
  ...
  for (const auto& kv : parsed_log_.rtp_packets(direction)) {
    uint32_t ssrc = kv.first;
    const RtpPacketView packets = kv.second;
    if (!MatchingSsrc(ssrc, desired_ssrc_))
      continue;
  ...
  CreateAccumulatedPacketsTimeSeries(plot, parsed_log_.rtcp_packets(direction),
                                     label);
  ...
  for (auto& kv : parsed_log_.rtp_packets(direction)) {
    ...
    for (auto& packet : packets) {
      if (packet.header.extension.hasAudioLevel) {
        float x = ToCallTime(packet.timestamp_us);
...
void EventLogAnalyzer::CreateSequenceNumberGraph(Plot* plot) {
  for (auto& kv : parsed_log_.incoming_rtp_packets()) {
    uint32_t ssrc = kv.first;
    const std::vector<LoggedRtpPacketIncoming>& packets = kv.second;
    // Filter on direction and SSRC.
    if (!MatchingSsrc(ssrc, desired_ssrc_)) {
      continue;
    }

    TimeSeries time_series(GetStreamName(kIncomingPacket, ssrc),
                           LineStyle::kBar);
    ...
    ProcessPairs<LoggedRtpPacketIncoming, float>(GetSequenceNumberDiff, packets,
                                                 begin_time_, &time_series);
    plot->AppendTimeSeries(std::move(time_series));
  }
...
void EventLogAnalyzer::CreateIncomingPacketLossGraph(Plot* plot) {
  for (auto& kv : parsed_log_.incoming_rtp_packets()) {
    uint32_t ssrc = kv.first;
    const std::vector<LoggedRtpPacketIncoming>& packet_stream = kv.second;
    // Filter on direction and SSRC.
    ...
    for (int64_t t = begin_time_; t < end_time_ + kStep; t += kStep) {
      while (window_index_end < packet_stream.size() &&
             packet_stream[window_index_end].timestamp_us < t) {
      ...
      while (window_index_begin < packet_stream.size() &&
             packet_stream[window_index_begin].timestamp_us < t - kWindowUs) {
...
void EventLogAnalyzer::CreateIncomingDelayDeltaGraph(Plot* plot) {
  for (auto& kv : parsed_log_.incoming_rtp_packets()) {
    ...
    // Filter on direction and SSRC.
    if (!MatchingSsrc(ssrc, desired_ssrc_) ||
        IsAudioSsrc(kIncomingPacket, ssrc) ||
        !IsVideoSsrc(kIncomingPacket, ssrc) ||
        IsRtxSsrc(kIncomingPacket, ssrc)) {
      continue;
    }

    TimeSeries capture_time_data(
        GetStreamName(kIncomingPacket, ssrc) + " capture-time",
        LineStyle::kBar);
    ProcessPairs<LoggedRtpPacketIncoming, double>(
    ...
    TimeSeries send_time_data(
        GetStreamName(kIncomingPacket, ssrc) + " abs-send-time",
        LineStyle::kBar);
    ProcessPairs<LoggedRtpPacketIncoming, double>(
...
void EventLogAnalyzer::CreateFractionLossGraph(Plot* plot) {
  ...
  for (auto& bwe_update : parsed_log_.bwe_loss_updates()) {
    float x = ToCallTime(bwe_update.timestamp_us);
...
void EventLogAnalyzer::CreateTotalIncomingBitrateGraph(Plot* plot) {
  ...
  for (const auto& kv : parsed_log_.incoming_rtp_packets()) {
    for (const LoggedRtpPacketIncoming& packet : kv.second)
      packets_in_order.insert(
          std::make_pair(packet.timestamp_us, packet.total_length));
  }
  ...
  for (const auto& rtcp : parsed_log_.outgoing_rembs()) {
    float x = ToCallTime(rtcp.timestamp_us);
...
void EventLogAnalyzer::CreateTotalOutgoingBitrateGraph(Plot* plot,
  ...
  // TODO(terelius): This could be provided by the parser.
  std::multimap<int64_t, size_t> packets_in_order;
  for (const auto& kv : parsed_log_.outgoing_rtp_packets()) {
    for (const LoggedRtpPacketOutgoing& packet : kv.second)
      packets_in_order.insert(
          std::make_pair(packet.timestamp_us, packet.total_length));
  }
  ...
  for (auto& loss_update : parsed_log_.bwe_loss_updates()) {
    float x = ToCallTime(loss_update.timestamp_us);
  ...
  for (auto& delay_update : parsed_log_.bwe_delay_updates()) {
    float x = ToCallTime(delay_update.timestamp_us);
  ...
  for (auto& cluster : parsed_log_.bwe_probe_cluster_created_events()) {
    float x = ToCallTime(cluster.timestamp_us);
  ...
  for (auto& result : parsed_log_.bwe_probe_result_events()) {
    if (result.bitrate_bps) {
      float x = ToCallTime(result.timestamp_us);
  ...
  for (auto& alr : parsed_log_.alr_state_events()) {
    float y = ToCallTime(alr.timestamp_us);
    if (!previously_in_alr && alr.in_alr) {
      alr_start = alr.timestamp_us;
  ...
  TimeSeries remb_series("Remb", LineStyle::kStep);
  for (const auto& rtcp : parsed_log_.incoming_rembs()) {
    float x = ToCallTime(rtcp.timestamp_us);
    float y = static_cast<float>(rtcp.remb.bitrate_bps()) / 1000;
    remb_series.points.emplace_back(x, y);
  }
...
void EventLogAnalyzer::CreateStreamBitrateGraph(PacketDirection direction,
  ...
  for (auto& kv : parsed_log_.rtp_packets(direction)) {
    ...
    MovingAverage<LoggedRtpPacket, double, RtpPacketView>(
        GetPacketSizeKilobits, packet_stream, begin_time_, end_time_,
...
void EventLogAnalyzer::CreateSendSideBweSimulationGraph(Plot* plot) {
  ...
  for (const auto& kv : parsed_log_.outgoing_rtp_packets()) {
    for (const RtpPacketType& rtp_packet : kv.second)
      outgoing_rtp.insert(std::make_pair(rtp_packet.timestamp_us, &rtp_packet));
  }

  const std::vector<TransportFeedbackType>& incoming_rtcp =
      parsed_log_.incoming_transport_feedbacks();
  ...
  auto NextRtcpTime = [&]() {
    if (rtcp_iterator != incoming_rtcp.end())
      return static_cast<int64_t>(rtcp_iterator->timestamp_us);
    return std::numeric_limits<int64_t>::max();
  };
...
void EventLogAnalyzer::CreateReceiveSideBweSimulationGraph(Plot* plot) {
  ...
  for (const auto& kv : parsed_log_.incoming_rtp_packets()) {
    if (IsVideoSsrc(kIncomingPacket, kv.first)) {
      for (const auto& rtp_packet : kv.second)
        incoming_rtp.insert(
            std::make_pair(rtp_packet.timestamp_us, &rtp_packet));
    }
  }
  ...
  for (const auto& kv : incoming_rtp) {
    ...
    int64_t arrival_time_ms = packet.timestamp_us / 1000;
    size_t payload = packet.total_length; /*Should subtract header?*/
    clock.AdvanceTimeMicroseconds(packet.timestamp_us -
...
void EventLogAnalyzer::CreatePacerDelayGraph(Plot* plot) {
  ...
  for (const auto& kv : parsed_log_.outgoing_rtp_packets()) {
    ...
    if (!estimated_frequency)
      continue;
    if (IsVideoSsrc(kOutgoingPacket, ssrc) && *estimated_frequency != 90000) {
      RTC_LOG(LS_WARNING)
    ...
    TimeSeries pacer_delay_series(
        GetStreamName(kOutgoingPacket, ssrc) + "(" +
    ...
    uint64_t first_send_timestamp = packets.front().timestamp_us;
    for (const auto& packet : packets) {
      ...
      double send_time_ms =
          static_cast<double>(packet.timestamp_us - first_send_timestamp) /
          1000;
      float x = ToCallTime(packet.timestamp_us);
...
void EventLogAnalyzer::CreateTimestampGraph(PacketDirection direction,
  ...
  for (const auto& kv : parsed_log_.rtp_packets(direction)) {
    uint32_t ssrc = kv.first;
    const RtpPacketView rtp_packets = kv.second;
    ...
    const auto& sender_reports =
        (direction == kIncomingPacket ? parsed_log_.incoming_sender_reports()
                                      : parsed_log_.outgoing_sender_reports());
...
class NetEqStreamInput : public test::NetEqInput {
 public:
  ...
  rtc::Optional<int64_t> NextPacketTime() const override {
    ...
    if (end_time_us_ && packet_stream_it_->timestamp_us > *end_time_us_) {
      return rtc::nullopt;
    }
    // Convert from us to ms.
    return packet_stream_it_->timestamp_us / 1000;
  }
  ...
  std::unique_ptr<PacketData> PopPacket() override {
    ...
    // Convert from us to ms.
    packet_data->time_ms = packet_stream_it_->timestamp_us / 1000.0;
...
void EventLogAnalyzer::CreateAudioJitterBufferGraph(
  ...
  uint32_t ssrc;
  for (const auto& kv : parsed_log_.incoming_rtp_packets()) {
    if (IsAudioSsrc(kIncomingPacket, kv.first)) {
...
void EventLogAnalyzer::CreateIceCandidatePairConfigGraph(Plot* plot) {
  ...
  for (const auto& config : parsed_log_.ice_candidate_pair_configs()) {
    ...
    float x = ToCallTime(config.timestamp_us);
...
void EventLogAnalyzer::CreateIceConnectivityCheckGraph(Plot* plot) {
  ...
  for (const auto& event : parsed_log_.ice_candidate_pair_events()) {
    ...
    float x = ToCallTime(event.timestamp_us);
...
void EventLogAnalyzer::PrintNotifications(FILE* file) {
  ...
  for (const auto& alert : incoming_rtp_recv_time_gaps) {
    fprintf(file, "%3.3lf s : %s\n", alert.Time(), alert.ToString().c_str());
  }
  for (const auto& alert : incoming_rtcp_recv_time_gaps) {
    fprintf(file, "%3.3lf s : %s\n", alert.Time(), alert.ToString().c_str());
  }
  for (const auto& alert : outgoing_rtp_send_time_gaps) {
    fprintf(file, "%3.3lf s : %s\n", alert.Time(), alert.ToString().c_str());
  }
  for (const auto& alert : outgoing_rtcp_send_time_gaps) {
    fprintf(file, "%3.3lf s : %s\n", alert.Time(), alert.ToString().c_str());
  }
  for (const auto& alert : incoming_seq_num_jumps) {
    fprintf(file, "%3.3lf s : %s\n", alert.Time(), alert.ToString().c_str());
  }
  for (const auto& alert : incoming_capture_time_jumps) {
    fprintf(file, "%3.3lf s : %s\n", alert.Time(), alert.ToString().c_str());
  }
  for (const auto& alert : outgoing_seq_num_jumps) {
    fprintf(file, "%3.3lf s : %s\n", alert.Time(), alert.ToString().c_str());
  }
  for (const auto& alert : outgoing_capture_time_jumps) {
    fprintf(file, "%3.3lf s : %s\n", alert.Time(), alert.ToString().c_str());
  }
  for (const auto& alert : outgoing_high_loss_alerts) {
...
void EventLogAnalyzer::CreateStreamGapAlerts(PacketDirection direction) {
  ...
  for (auto& kv : parsed_log_.rtp_packets(direction)) {
    for (const auto& packet : kv.second) {
      if (packet.timestamp_us > end_time_us) {
      ...
      if (last_seq_num.has_value() &&
        ...
        Alert_SeqNumJump(direction, ToCallTime(packet.timestamp_us),
                         packet.header.ssrc);
      }
      ...
      if (last_capture_time.has_value() &&
          std::abs(capture_time - last_capture_time.value()) >
              kMaxCaptureTimeJump) {
        Alert_CaptureTimeJump(direction, ToCallTime(packet.timestamp_us),
                              packet.header.ssrc);
      }
...
void EventLogAnalyzer::CreateTransmissionGapAlerts(PacketDirection direction) {
  ...
  for (const auto& kv : parsed_log_.rtp_packets(direction)) {
    for (const LoggedRtpPacket& rtp_packet : kv.second)
      rtp_in_direction.emplace(rtp_packet.timestamp_us, &rtp_packet);
  }
  ...
  for (const LoggedRtcpPacket& rtcp : parsed_log_.rtcp_packets(direction)) {
    if (rtcp.timestamp_us > end_time_us) {
      // Only process the first (LOG_START, LOG_END) segment.
      break;
    }
    int64_t duration = rtcp.timestamp_us - last_rtcp_time.value_or(0);
    if (last_rtcp_time.has_value() && duration > kMaxRtcpTransmissionGap) {
      // No feedback sent/received for more than 2000 ms.
      Alert_RtcpLogTimeGap(direction, ToCallTime(rtcp.timestamp_us),
                           duration / 1000);
    }
    last_rtcp_time.emplace(rtcp.timestamp_us);
  }
}
...
void EventLogAnalyzer::CreateTriageNotifications() {
  ...
  for (auto& bwe_update : parsed_log_.bwe_loss_updates()) {
    if (bwe_update.timestamp_us > end_time_us) {
      // Only process the first (LOG_START, LOG_END) segment.
      break;
    }
