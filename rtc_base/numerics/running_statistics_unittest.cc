...
TEST(RunningStatistics, AddSamples) {
  constexpr int size = 5;
  int data[size] = {2, 2, -1, 5, 10};
  // Split the data in different partitions.
  // We have 5 distinct tests:
  ...
  for (int i = 0; i < size + 1; ++i) {
    ...
    for (int j = 0; j < i; ++j) {
      stats0.AddSample(data[j]);
    }
    for (int j = i; j < size; ++j) {
      stats1.AddSample(data[j]);
    }
    stats0.AddSamples(stats1);

    EXPECT_EQ(stats0.Size(), size);
