...
TEST_F(RRSendQueueTest, WillStayInStreamWhenOnlySmallFragmentRemaining) {
  ...
  // space left in the packet for new chunks. What it should NOT do right now is
  // to try to send a message from StreamID 6. And it should not try to send a
  // very small fragment from StreamID 5 either. So just skip this one.
  EXPECT_FALSE(buf_.Produce(kNow, 8).has_value());

  // And when the next produce request comes with a large buffer to fill,
  // continue sending from StreamID 5.
