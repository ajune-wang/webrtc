...
class ResourceAdaptationModuleInterface {
 public:
  ...
  // Note that if the encoder is pipelined, multiple frames may be processed in
  // parallel.
  //
  ...
  // may get dropped or it may get encoded, see OnInputFrameEncodeStarted().
  // There currently is no input signal for frame dropped.
  virtual void OnInputFrame(const VideoFrame& frame) = 0;
  // 2. An input frame is about to be encoded. It may have been cropped and have
  // different dimensions than what was observed at OnInputFrame(). Next up:
  // encoding completes or fails, see OnInputFrameEncodeCompleted(). There is
