...
class RtpVideoStreamReceiver::RtcpObserver : public MediaReceiverRtcpObserver {
 public:
  ...
  void OnSenderReport(uint32_t sender_ssrc,
    ...
    rtp_video_stream_receiver_->SetRemoveNtpRtpTimes(ntp_time, rtp_time);
  }
  void OnSdes(uint32_t sender_ssrc, const std::string& cname) /* override */ {
    receive_stats_proxy_->CNameChanged(cname.c_str(), sender_ssrc);
  }
...
void RtpVideoStreamReceiver::SetRemoveNtpRtpTimes(NtpTime ntp_time,
                                                  uint32_t rtp_time) {
...
void RtpVideoStreamReceiver::SignalNetworkState(NetworkState state) {
  rtcp_transceiver_.SignalNetworkState(state == kNetworkUp);
}
