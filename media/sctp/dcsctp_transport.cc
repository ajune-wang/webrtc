...
bool DcsctpTransport::Start(int local_sctp_port,
  ...
  // TODO(orphis): Need to allow reconfiguring the max_message_size
  if (!socket_) {
    dcsctp::DcSctpOptions options;
    options.local_port = local_sctp_port;
    options.remote_port = remote_sctp_port;
    options.mtu = dcsctp::DcSctpOptions::kMaxSafeMTUSize;
  ...
  switch (error) {
    ...
    case dcsctp::SendStatus::kErrorResourceExhaustion:
      *result = cricket::SDR_BLOCK;
      ...
      break;
    case dcsctp::SendStatus::kErrorMessageEmpty:
    case dcsctp::SendStatus::kErrorMessageTooLarge:
    case dcsctp::SendStatus::kErrorShuttingDown:
...
void DcsctpTransport::SendPacket(rtc::ArrayView<const uint8_t> data) {
  ...
  if (max_message_size > 0 && data.size() > max_message_size) {
    RTC_LOG(LS_ERROR) << debug_name_
                      << "->SendPacket(...): "
                         "Trying to send packet bigger "
                         "than the max message size: "
                      << data.size() << " vs max of " << max_message_size;
    return;
  }
