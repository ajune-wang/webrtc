...
class VideoSendStreamPeer {
 public:
  ...
  rtc::Optional<float> GetPacingFactorOverride() {
    return internal_stream_->GetPacingFactorOverride();
  }

 private:
  internal::VideoSendStream* internal_stream_;
};
}  // namespace test
...
class PacingFactorObserver : public test::SendTest {
 public:
  ...
  void OnVideoStreamsCreated(
    ...
    // Video streams created, check that paing factor is correctly configured.
