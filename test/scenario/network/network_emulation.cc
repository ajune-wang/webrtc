...
void EndpointNode::OnPacketReceived(EmulatedIpPacket packet) {
  ...
  RTC_CHECK(it != port_to_receiver_.end())
      << "No receiver registered in " << id_ << " on port " << packet.to.port();
  // Endpoint assumes often calls to bind and unbind methods, so we hold lock
  // during packet processing to ensure, that receiver won't be destructed
