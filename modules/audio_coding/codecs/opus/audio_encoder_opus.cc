...
AudioCodecInfo AudioEncoderOpusImpl::QueryAudioEncoder(
  ...
  RTC_DCHECK(config.IsOk());
  AudioCodecInfo info(config.input_sample_rate_hz, config.num_channels,
                      *config.bitrate_bps,
...
int AudioEncoderOpusImpl::SampleRateHz() const {
  return config_.input_sample_rate_hz;
}
...
size_t AudioEncoderOpusImpl::SamplesPer10msFrame() const {
  return rtc::CheckedDivExact(config_.input_sample_rate_hz, 100) *
