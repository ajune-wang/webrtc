...
MediaSessionDescriptionFactory::CreateAnswer(
  ...
  // rejected, we respond with an empty groups). |offer_bundles|,
    ...
    switch (media_description_options.type) {
      ...
      case MEDIA_TYPE_DATA:
        if (!AddDataContentForAnswer(media_description_options, session_options,
                                     ...
                                     current_description, bundle_transport,
                                     answer_rtp_data_codecs, &current_streams,
                                     answer.get(), &ice_credentials)) {
          return nullptr;
        }
        break;
  ...
  if (!offer_bundles.empty()) {
    RTC_LOG(LS_ERROR) << "=== ANSWER GROUPS ===";
    for (const ContentGroup& answer_bundle : answer_bundles) {
      std::string str;
      for (const auto& member : answer_bundle.content_names()) {
        if (!str.empty())
          str += " ";
        str += member;
      }
      RTC_LOG(LS_ERROR) << answer_bundle.semantics() << " " << str;
