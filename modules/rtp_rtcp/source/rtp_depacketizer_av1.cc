...
Leb128Value ReadLeb128(rtc::ArrayView<const uint8_t> next_bytes) {
  ...
  for (uint8_t byte : next_bytes) {
    uint64_t add_value = uint64_t{byte & 0b0111'1111u} << (result.size * 7);
    if (result.size >= 8 || add_value >= uint64_t{1} << 32) {
      // AV1 spec limits leb128 encoding to unsigned numbers with max value of
      // (1 << 32) - 1 and require max 8 bytes to store it. See
      // https://aomediacodec.github.io/av1-spec/av1-spec.pdf Section 4.10.5
      result.size = 0;
      return result;
    }
...
bool RtpDepacketizerAv1::Parse(ParsedPayload* parsed_payload,
  ...
  if (parsed_payload->video.is_first_packet_in_frame) {
    ...
    for (int obu_index = 1; !remaining.empty() && obu_index <= 2; ++obu_index) {
