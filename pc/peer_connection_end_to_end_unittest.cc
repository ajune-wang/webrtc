...
std::unique_ptr<webrtc::AudioDecoder> CreateForwardingMockDecoder(
  ...
  EXPECT_CALL(*mock_decoder, DecodeInternal(_, _, _, _, _, _))
      .Times(AtLeast(1))
      .WillRepeatedly(Invoke(
          [dec](const uint8_t* encoded, size_t encoded_len, int sample_rate_hz,
                int16_t* decoded, webrtc::AudioDecoder::SpeechType* speech_type,
                webrtc::AudioDecoder::ChannelLayout* channel_layout) {
