...
#include "modules/pacing/paced_sending_controller.h"
...
const TimeDelta PacedSendingController::kMaxExpectedQueueLength =
    TimeDelta::Millis<2000>();
const float PacedSendingController::kDefaultPaceMultiplier = 2.5f;
const TimeDelta PacedSendingController::kPausedProcessInterval =
    kCongestedPacketInterval;

PacedSendingController::PacedSendingController(
    Clock* clock,
...
PacedSendingController::~PacedSendingController() = default;

void PacedSendingController::CreateProbeCluster(DataRate bitrate,
                                                int cluster_id) {
  prober_.CreateProbeCluster(bitrate.bps(), CurrentTime().ms(), cluster_id);
}

void PacedSendingController::Pause() {
  if (!paused_)
...
void PacedSendingController::Resume() {
  if (paused_)
...
bool PacedSendingController::IsPaused() const {
  return paused_;
}

void PacedSendingController::SetCongestionWindow(
    DataSize congestion_window_size) {
  congestion_window_size_ = congestion_window_size;
}

void PacedSendingController::UpdateOutstandingData(DataSize outstanding_data) {
  outstanding_data_ = outstanding_data;
}

bool PacedSendingController::Congested() const {
...
Timestamp PacedSendingController::CurrentTime() const {
...
void PacedSendingController::SetProbingEnabled(bool enabled) {
  RTC_CHECK_EQ(0, packet_counter_);
  prober_.SetEnabled(enabled);
}

void PacedSendingController::SetPacingRates(DataRate pacing_rate,
...
void PacedSendingController::InsertPacket(RtpPacketSender::Priority priority,
                                          uint32_t ssrc,
...
void PacedSendingController::EnqueuePacket(
...
void PacedSendingController::SetAccountForAudioPackets(bool account_for_audio) {
  account_for_audio_ = account_for_audio;
}

TimeDelta PacedSendingController::ExpectedQueueTime() const {
...
size_t PacedSendingController::QueueSizePackets() const {
  return packet_queue_.SizeInPackets();
}

DataSize PacedSendingController::QueueSizeData() const {
  return packet_queue_.Size();
}

absl::optional<Timestamp> PacedSendingController::FirstSentPacketTime() const {
  return first_sent_packet_time_;
}

TimeDelta PacedSendingController::OldestPacketWaitTime() const {
...
TimeDelta PacedSendingController::UpdateTimeAndGetElapsed(Timestamp now) {
...
bool PacedSendingController::ShouldSendKeepalive(Timestamp now) const {
...
absl::optional<TimeDelta> PacedSendingController::TimeUntilNextProbe() {
...
TimeDelta PacedSendingController::TimeElapsedSinceLastProcess() const {
  return CurrentTime() - time_last_process_;
}

void PacedSendingController::ProcessPackets() {
  Timestamp now = CurrentTime();
...
DataSize PacedSendingController::PaddingToAdd(
    absl::optional<DataSize> recommended_probe_size,
    DataSize data_sent) {
...
RoundRobinPacketQueue::QueuedPacket* PacedSendingController::GetPendingPacket(
...
void PacedSendingController::OnPacketSent(
...
void PacedSendingController::OnPaddingSent(DataSize data_sent) {
