...
TEST_F(PeerConnectionJsepTest, AudioOnlyInitialOffer) {
  ...
  auto* media_description =
      static_cast<MediaContentDescription*>(content_info.description);
...
TEST_F(PeerConnectionJsepTest, MediaSectionsInInitialOfferOrderedCorrectly) {
  ...
  auto* media_description2 =
      static_cast<MediaContentDescription*>(contents[1].description);
  EXPECT_EQ(cricket::MEDIA_TYPE_AUDIO, media_description2->type());
  EXPECT_EQ(RtpTransceiverDirection::kSendRecv,
            media_description2->direction());

  auto* media_description3 =
      static_cast<MediaContentDescription*>(contents[2].description);
...
TEST_P(RecycleMediaSectionTest, VerifyOfferAnswerAndTransceivers) {
  ...
  EXPECT_EQ(second_type_, static_cast<MediaContentDescription*>(
                              answer_contents[0].description)
                              ->type());
