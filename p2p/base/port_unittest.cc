...
// This test is a bit hacky and can be deleted in case it's making things like
// refactoring difficult. The point of it is to highlight a corner case whereby
// two delete operations could be scheduled to run for the same object due to
// a combination of a timer expiring and triggering async deletion while other
// triggers for deletion could also occur.
TEST_F(PortTest, TestTcpReconnectAvoidDoubleDelete) {
  rtc::ScopedFakeClock clock;
  auto port1 = CreateTcpPort(kLocalAddr1);
  port1->SetIceRole(cricket::ICEROLE_CONTROLLING);
  auto port2 = CreateTcpPort(kLocalAddr2);
  ...
  tcp1->network_thread()->PostDelayedTask(
      webrtc::ToQueuedTask([&]() {
        if (tcp1 != ch1.conn()) {
          // If we hit this, the `tcp1` has already been deleted and there's no
          // race.
          tcp1 = static_cast<TCPConnection*>(ch1.conn());
          return;
        }
