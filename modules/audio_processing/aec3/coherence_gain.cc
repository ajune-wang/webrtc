...
void CoherenceGain::ComputeGain(rtc::ArrayView<const float> x,
  ...
  fft_.PaddedFft(e, e_old_,  Aec3Fft::Window::kSqrtHanning, &E);
  std::copy(e.begin(), e.end(), e_old_.begin());
  // for (size_t k = 0; k < 65;++k)
  //   printf("%f %f %f %f\n", E.re[k], E.im[k],E2.re[k], E2.im[k]);
  // exit(0);
...
//
void CoherenceGain::UpdateCoherenceSpectra(const FftData& E, const FftData& X, const FftData& Y) {
...
void CoherenceGain::FormSuppressionGain(
  ...
  if (h_nl_xd_avg < 0.75f && h_nl_xd_avg < h_nl_xd_avg_min_) {
    h_nl_xd_avg_min_ = h_nl_xd_avg;
  }
  ...
  if (h_nl_fb_low < 0.6f && h_nl_fb_low < h_nl_fb_local_min_) {
    h_nl_fb_local_min_ = h_nl_fb_low;
    h_nl_fb_min_ = h_nl_fb_low;
    h_nl_new_min_ = 1;
    h_nl_min_ctr_ = 0;
  }
  h_nl_fb_local_min_ =
      std::min(h_nl_fb_local_min_ + 0.0008f / sample_rate_scaler_, 1.f);
  h_nl_xd_avg_min_ =
      std::min(h_nl_xd_avg_min_ + 0.0006f / sample_rate_scaler_, 1.f);
  ...
  if (h_nl_min_ctr_ == 2) {
    ...
    overdrive_ = std::max(
        -18.4f / static_cast<float>(log(h_nl_fb_min_ + epsilon) + epsilon),
        15.f);
  }

  // Smooth the overdrive.
  if (overdrive_ < overdrive_scaling_) {
    overdrive_scaling_ = 0.99f * overdrive_scaling_ + 0.01f * overdrive_;
  } else {
    overdrive_scaling_ = 0.9f * overdrive_scaling_ + 0.1f * overdrive_;
  }
  ...
  for (size_t i = 0; i < num_bands_to_compute_; ++i) {
    ...
    gain[i] = powf(h_nl[i], overdrive_scaling_ * kOverDriveCurve[i]);
  }
}
...
void CoherenceGain::FormWindowedFft(rtc::ArrayView<const float> v,
                                    rtc::ArrayView<float> v_old,
                                    FftData* V) const {
  fft_.PaddedFft(v, v_old,  Aec3Fft::Window::kSqrtHanning, V);
