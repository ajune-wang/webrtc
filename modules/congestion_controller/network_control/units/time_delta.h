...
class TimeDelta {
 public:
  ...
  int64_t s() const { return units_impl::DivideAndRound(us(), 1000000); }
  int64_t ms() const { return units_impl::DivideAndRound(us(), 1000); }
  int64_t us() const {
    RTC_DCHECK(IsFinite());
  ...
  TimeDelta operator*(double scalar) const {
    return TimeDelta::us(units_impl::DoubleToIntRounded(us() * scalar));
  }
