...
class ResourceAdaptationModuleInterface {
 public:
  ...
  // may get dropped or it may get encoded, see OnInputFrameDroppedDueToSize()
  // and OnInputFrameEncodeStarted(). There currently is no input signal for
  // frame dropped.
  virtual void OnInputFrame(const VideoFrame& frame) = 0;
  ...
  // pipeline. The module must adapt down in resolution.
  // TODO(hbos): If we take frame rate into account perhaps it would be valid to
  // adapt down in frame rate as well.
  virtual void OnInputFrameDroppedDueToSize() = 0;
  // 2.ii) An input frame is about to be encoded. It may have been cropped and
  // have different dimensions than what was observed at OnInputFrame(). Next
  // up: encoding completes or fails, see OnInputFrameEncodeCompleted(). There
  // is currently no signal for encode failure.
  virtual void OnInputFrameEncodeStarted(const VideoFrame& cropped_frame,
  ...
  virtual void OnInputFrameEncodeCompleted(
      uint32_t timestamp,
