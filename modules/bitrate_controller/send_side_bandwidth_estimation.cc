...
void SendSideBandwidthEstimation::UpdateReceiverBlock(uint8_t fraction_loss,
  ...
  int packets_lost = (static_cast<int>(fraction_loss) * number_of_packets +
                      kRoundingConstant) >>
                     8;
  UpdateReceiverBlock(packets_lost, number_of_packets, now_ms);
  UpdateReceiverBlockRtt(rtt_ms, now_ms);
}

void SendSideBandwidthEstimation::UpdateReceiverBlock(int packets_lost,
                                                      int number_of_packets,
                                                      int64_t now_ms) {
  ...
  if (number_of_packets > 0) {
    ...
    last_fraction_loss_ = lost_Q8 / expected;
  ...
  UpdateUmaStats(now_ms, packets_lost);
}
...
void SendSideBandwidthEstimation::UpdateReceiverBlockRtt(int64_t rtt,
                                                         int64_t now_ms) {
  // Update RTT if we were able to compute an RTT based on this RTCP.
  // FlexFEC doesn't send RTCP SR, which means we won't be able to compute RTT.
  if (rtt > 0)
    last_round_trip_time_ms_ = rtt;
  UpdateUmaStats(now_ms, rtt);
}

void SendSideBandwidthEstimation::UpdateUmaStatsRtt(int64_t now_ms,
                                                    int64_t rtt) {
