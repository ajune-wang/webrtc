...
void WebRtcVideoChannel::WebRtcVideoReceiveStream::SetRecvParameters(
  ...
  if (params.flexfec_payload_type) {
    ...
    RTC_DCHECK_NE(-1, *params.flexfec_payload_type);
    ...
    // thread).
    // RTC_DCHECK(!flexfec_stream_) << "update payload";
    if (flexfec_stream_) {
      RTC_DCHECK(false) << "payload=" << *params.flexfec_payload_type;
      flexfec_stream_->SetPayloadType(*params.flexfec_payload_type);
    } else if (flexfec_config_.IsCompleteAndEnabled()) {
      flexfec_needs_recreation = true;
    }
    /*
