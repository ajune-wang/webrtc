...
void VideoRtpReceiver::SetSink(rtc::VideoSinkInterface<VideoFrame>* sink) {
  ...
  RTC_DCHECK(!stopped_);
  worker_thread_->Invoke<void>(RTC_FROM_HERE,
                               [&] { SetSinkOnWorkerThread(sink); });
}

void VideoRtpReceiver::SetSinkOnWorkerThread(
...
void VideoRtpReceiver::RestartMediaChannel(absl::optional<uint32_t> ssrc) {
  ...
  bool encoded_sink_enabled;
  worker_thread_->Invoke<void>(RTC_FROM_HERE, [&] {
    RTC_DCHECK_RUN_ON(worker_thread_);
    if (!stopped_) {
      SetSinkOnWorkerThread(nullptr);
    }
    encoded_sink_enabled = saved_encoded_sink_enabled_;
  ...
  worker_thread_->Invoke<void>(RTC_FROM_HERE, [&] {
    RTC_DCHECK_RUN_ON(worker_thread_);
    SetSinkOnWorkerThread(source_->sink());
