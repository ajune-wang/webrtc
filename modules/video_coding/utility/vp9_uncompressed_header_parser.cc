...
bool Vp9ReadProfile(rtc::BitBuffer* br, uint8_t* profile) {
  ...
  *profile = (high_bit << 1) + low_bit;
  if (*profile > 2) {
    ...
    if (reserved_bit) {
      NLOG(LS_WARNING, "Failed to get QP. Unsupported bitstream profile.");
      return false;
    }
  }
  return true;
...
bool Vp9ReadSyncCode(rtc::BitBuffer* br) {
  ...
  if (sync_code != 0x498342) {
    NLOG(LS_WARNING, "Failed to get QP. Invalid sync code.");
    return false;
  }
  return true;
}
...
bool Vp9ReadColorConfig(rtc::BitBuffer* br, uint8_t profile) {
  ...
  if (color_space != 7) {
    ...
    if (profile == 1 || profile == 3) {
      ...
      RETURN_FALSE_IF_ERROR(br->ReadBits(&reserved_bit, 1));
      if (reserved_bit) {
        NLOG(LS_WARNING, "Failed to get QP. Reserved bit set.");
        return false;
      }
    }
  } else {
    if (profile == 1 || profile == 3) {
      ...
      NLOG(LS_WARNING,
           "Failed to get QP. 4:4:4 color not supported in "
           "profile 0 or 2.");
      return false;
    }
  }
...
bool GetQp(const uint8_t* buf, size_t length, int* qp) {
  ...
  if (frame_marker != 0x2) {
    NLOG(LS_WARNING, "Failed to get QP. Frame marker should be 2.");
    return false;
  }
