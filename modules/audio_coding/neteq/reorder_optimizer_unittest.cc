...
TEST(ReorderOptimizerTest, OnlyIncreaseDelayForReorderedPackets) {
  ...
  reorder_optimizer.Update(60, /*reordered=*/false);
  EXPECT_EQ(reorder_optimizer.GetOptimalDelayMs(), 20);

  reorder_optimizer.Update(100, /*reordered=*/false);
  EXPECT_EQ(reorder_optimizer.GetOptimalDelayMs(), 20);

  reorder_optimizer.Update(80, /*reordered=*/true);
