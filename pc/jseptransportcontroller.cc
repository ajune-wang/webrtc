...
RTCError JsepTransportController::RemoveRemoteCandidates(
  ...
  for (const auto& kv : candidates_by_transport_name) {
    ...
    if (!jsep_transport) {
      RTC_LOG(LS_WARNING)
          << "Not remove candidate because the JsepTransport doesn't exist.";
      continue;
    }
    for (const cricket::Candidate& candidate : candidates) {
...
JsepTransportController::CreateSdesTransport(
  ...
  srtp_transport->SetRtpDtlsTransport(rtp_dtls_transport);
  if (rtcp_dtls_transport) {
    srtp_transport->SetRtcpDtlsTransport(rtcp_dtls_transport);
  }
