...
int LibaomAv1Encoder::GetCpuSpeed(int width, int height) {
  if (aux_config_) {
    auto it = aux_config_->max_pixel_count_to_cpu_speed.begin();
    while (it != aux_config_->max_pixel_count_to_cpu_speed.end()) {
      if (width * height <= it->first) {
        break;
      }
      ++it;
    }

    return it != aux_config_->max_pixel_count_to_cpu_speed.end() ? it->second
                                                                 : 10;
  } else {
