...
rtc::Optional<DelayEstimate> RenderDelayControllerImpl::GetDelay(
  ...
  // if (delay_samples) {
  //     printf("delay_samples_delay: %zu\n", delay_samples->delay/kBlockSize);
  //   }
  //   if (echo_remover_delay) {
  //     printf("echo_remover_delay: %d\n", *echo_remover_delay);
  //     printf("current_delay: %zu\n", current_delay);
  //   }

  if (echo_remover_delay && !delay_samples) {
    delay_samples =
        DelayEstimate(DelayEstimate::Quality::kRefined,
                      (current_delay + *echo_remover_delay) * kBlockSize);
  }
