  ...
  switch (degradation_preference_) {
    ...
    case VideoSendStream::DegradationPreference::kMaintainFramerate: {
      ...
      if (!source_proxy_->RequestResolutionLowerThan(
              adaptation_request.input_pixel_count_,
              // For initial scale down, based on DropDueToSize, this
              // method may be called before the encoder is created.
              // Don't specify any minimum in this case.
              encoder_ ? encoder_->GetScalingSettings().min_pixels_per_frame
                       : 0,
              &min_pixels_reached)) {
