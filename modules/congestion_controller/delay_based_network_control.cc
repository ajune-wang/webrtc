...
std::unique_ptr<NetworkControllerInterface> CreateDelayBasedNetworkController(
  ...
  NetworkControlJunctions::uptr junctions =
      MakeUnique<NetworkControlJunctions>();
  NetworkControllerInternalInterface::uptr controller =
      MakeUnique<DelayBasedNetworkController>(clock, event_log,
                                              junctions->GetObservers());
  NetworkControlHandlingReceivers::uptr receivers;
  if (task_queue == nullptr) {
    receivers = MakeUnique<NetworkControlHandlingReceivers>(controller.get());
  } else {
    receivers = MakeUnique<NetworkControlHandlingReceivers>(task_queue,
                                                            controller.get());
  }
  return MakeUnique<NetworkControllerWrapper>(
      std::move(junctions), std::move(controller), std::move(receivers));
}
...
DelayBasedNetworkController::DelayBasedNetworkController(
    ...
    : clock_(clock),
      ...
      probe_controller_(
          new ProbeController(clock_, observers_->ProbeClusterConfigObserver)),
...
void DelayBasedNetworkController::ConnectHandlers(
    NetworkControlHandlers::uptr receivers) {
...
void DelayBasedNetworkController::MaybeUpdateCongestionWindow() {
  ...
  observers_->CongestionWindowObserver->OnMessage(msg);
...
void DelayBasedNetworkController::OnNetworkEstimate(NetworkEstimate estimate) {
  ...
  observers_->TargetTransferRateObserver->OnMessage(target_rate);
}
...
void DelayBasedNetworkController::UpdatePacingRates() {
  ...
  observers_->PacerConfigObserver->OnMessage(msg);
}
