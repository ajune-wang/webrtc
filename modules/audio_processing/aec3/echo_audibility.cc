...
float SpectralPower(rtc::ArrayView<const float> spectrum) {
  ...
  float power = 0;
  for (size_t k = 1; k < spectrum.size() - 1; ++k) { power += spectrum[k]; }
  power *= 2;
...
EchoAudibility::EchoAudibility()
        : data_dumper_(new ApmDataDumper(rtc::AtomicOps::Increment(&instance_count_))),
...
void EchoAudibility::Update(const RenderBuffer& render_buffer,
  ...
  inaudible_blocks_[0] = stationarity_.Update(render_buffer.Spectrum(delay_blocks));
  ...
  if (++convergence_counter_ < 20) {
    ...
    for (size_t k = 1; k < inaudible_blocks_.size(); k++) {
      inaudible_blocks_[k] = stationarity_.Analyze(render_buffer.Spectrum(delay_blocks+k));
    }
  }
  ...
  data_dumper_->DumpRaw("aec3_render_stationary_power",stationarity_.StationaryPower() );
  data_dumper_->DumpRaw("aec3_num_non_audible_echo_blocks", num_nonaudible_blocks_);
...
EchoAudibility::Stationarity::NoiseSpectrum::NoiseSpectrum() : power_(0) {
...
void EchoAudibility::Stationarity::NoiseSpectrum::Update(
  ...
  if (first_update) {
    ...
    for (auto& v : noise_spectrum_) { v = std::max(v, kMinNoisePower);}
  } else {
