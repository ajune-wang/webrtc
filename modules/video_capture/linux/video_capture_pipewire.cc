...
uint32_t VideoCaptureModulePipeWire::VideoTypeToPipeWireRawFormat(
  ...
  RTC_LOG(LS_INFO) << "Unsupported pixel format: " << type;
...
static spa_pod* BuildFormat(spa_pod_builder* builder,
  ...
  uint32_t format = video_type == VideoType::kMJPEG ? SPA_MEDIA_SUBTYPE_mjpg
  ...
  if (format == SPA_MEDIA_SUBTYPE_raw) {
    spa_pod_builder_add(
        ...
        SPA_POD_Id(VideoCaptureModulePipeWire::VideoTypeToPipeWireRawFormat(
            video_type)),
        0);
  }
