...
void AudioSendStream::ConfigureStream(
  ...
  // Set currently known overhead (used in ANA, opus only).
  {
    ...
    UpdateOverheadForEncoder(first_time);
  }
...
void AudioSendStream::DeliverRtcp(const uint8_t* packet, size_t length) {
  ...
  // sending mid/rid.
  MutexLock lock(&overhead_per_packet_lock_);
  UpdateOverheadForEncoder(false);
}
...
void AudioSendStream::UpdateOverheadForEncoder(bool first_time) {
  RTC_DCHECK_RUN_ON(&worker_thread_checker_);
...
void AudioSendStream::ConfigureBitrateObserver() {
  ...
  rtc::Event thread_sync_event;
  worker_queue_->PostTask([this, &thread_sync_event, &constraints,
                           &priority_bitrate,
...
void AudioSendStream::UpdateCachedTargetAudioBitrateConstraints() {
  ...
  worker_queue_->PostTask(
      [this, new_constraints = GetMinMaxBitrateConstraints()]() {
        RTC_DCHECK_RUN_ON(worker_queue_);
