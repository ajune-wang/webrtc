...
class DataChannelProviderInterface {
 public:
  ...
  // Resets the outgoing stream with ID |sid|, as part of the closing
  // procedure.
  virtual void ResetOutgoingSctpDataStream(int sid) = 0;
...
// 3. Alice calls ResetOutgoingSctpDataStream.
// 4. Bob receives incoming stream reset; OnIncomingStreamReset called.
// 5. Bob finishes sending any queued data.
// 6. Bob calls ResetOutgoingSctpDataStream
...
class DataChannel : public DataChannelInterface,
 ...
 public:
  ...
  // stream (our incoming stream). Sets state to kClosing; afterwards we need
  // to finish sending data and then reset our outgoing stream to complete the
  // closing procedure.
  void OnIncomingStreamReset(int sid);
  ...
  // asynchronously after ResetOutgoingSctpDataStream.
