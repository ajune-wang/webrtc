...
class MockPacedSendingControllerCallback
    : public PacedSendingController::PacketSender {
 public:
...
class MockPacketSender : public PacedSendingController::PacketSender {
 public:
...
class PacedSendingControllerPadding
    : public PacedSendingController::PacketSender {
 public:
  static const size_t kPaddingPacketSize = 224;

  PacedSendingControllerPadding() : padding_sent_(0) {}
...
class PacedSendingControllerProbing
    : public PacedSendingController::PacketSender {
 public:
  PacedSendingControllerProbing() : packets_sent_(0), padding_sent_(0) {}
...
class PacedSendingControllerTest : public ::testing::TestWithParam<PacerMode> {
 protected:
  PacedSendingControllerTest()
      : clock_(123456), field_trial_(GetFieldTrialStirng(GetParam())) {
    srand(0);
    // Need to initialize PacedSendingController after we initialize clock.
    pacer_ = absl::make_unique<PacedSendingController>(&clock_, &callback_,
                                                       nullptr, nullptr);
    Init();
  }
  ...
  void Init() {
    ...
    // have to enable probing, either by creating a new PacedSendingController
  ...
  MockPacedSendingControllerCallback callback_;
  std::unique_ptr<PacedSendingController> pacer_;
};

class PacedSendingControllerFieldTrialTest
 ...
 protected:
  ...
  PacedSendingControllerFieldTrialTest() : clock_(123456) {}
  void InsertPacket(PacedSendingController* pacer, MediaStream* stream) {
    if (GetParam() == PacerMode::kReferencePackets) {
  ...
  void ProcessNext(PacedSendingController* pacer) {
...
TEST_P(PacedSendingControllerFieldTrialTest, DefaultNoPaddingInSilence) {
  PacedSendingController pacer(&clock_, &callback_, nullptr, nullptr);
...
TEST_P(PacedSendingControllerFieldTrialTest, PaddingInSilenceWithTrial) {
...
TEST_P(PacedSendingControllerFieldTrialTest,
       DefaultCongestionWindowAffectsAudio) {
  EXPECT_CALL(callback_, SendPadding).Times(0);
  PacedSendingController pacer(&clock_, &callback_, nullptr, nullptr);
...
TEST_P(PacedSendingControllerFieldTrialTest, DefaultBudgetAffectsAudio) {
  PacedSendingController pacer(&clock_, &callback_, nullptr, nullptr);
...
TEST_P(PacedSendingControllerFieldTrialTest, BudgetDoesNotAffectAudioInTrial) {
  ScopedFieldTrials trial(GetFieldTrialStirng(GetParam()) +
                          "WebRTC-Pacer-BlockAudio/Disabled/");
  EXPECT_CALL(callback_, SendPadding).Times(0);
  PacedSendingController pacer(&clock_, &callback_, nullptr, nullptr);
...
INSTANTIATE_TEST_SUITE_P(ReferencingAndOwningPackets,
                         PacedSendingControllerFieldTrialTest,
                         ::testing::Values(PacerMode::kReferencePackets,
                                           PacerMode::kOwnPackets));

TEST_P(PacedSendingControllerTest, FirstSentPacketTimeIsSet) {
...
TEST_P(PacedSendingControllerTest, QueuePacket) {
  uint32_t ssrc = 12345;
  uint16_t sequence_number = 1234;
...
TEST_P(PacedSendingControllerTest, PaceQueuedPackets) {
  uint32_t ssrc = 12345;
  uint16_t sequence_number = 1234;
...
TEST_P(PacedSendingControllerTest, RepeatedRetransmissionsAllowed) {
...
TEST_P(PacedSendingControllerTest,
...
TEST_P(PacedSendingControllerTest, Padding) {
  uint32_t ssrc = 12345;
  uint16_t sequence_number = 1234;
...
TEST_P(PacedSendingControllerTest, NoPaddingBeforeNormalPacket) {
  pacer_->SetPacingRates(kTargetRate * kPaceMultiplier, kTargetRate);
...
TEST_P(PacedSendingControllerTest, VerifyPaddingUpToBitrate) {
  uint32_t ssrc = 12345;
  uint16_t sequence_number = 1234;
  int64_t capture_time_ms = 56789;
...
TEST_P(PacedSendingControllerTest, VerifyAverageBitrateVaryingMediaPayload) {
  uint32_t ssrc = 12345;
  uint16_t sequence_number = 1234;
  int64_t capture_time_ms = 56789;
  const int kTimeStep = 5;
  const int64_t kBitrateWindow = 10000;
  PacedSendingControllerPadding callback;
  pacer_ = absl::make_unique<PacedSendingController>(&clock_, &callback,
                                                     nullptr, nullptr);
  pacer_->SetProbingEnabled(false);
  pacer_->SetPacingRates(kTargetRate * kPaceMultiplier, kTargetRate);
...
TEST_P(PacedSendingControllerTest, Priority) {
...
TEST_P(PacedSendingControllerTest, RetransmissionPriority) {
  uint32_t ssrc = 12345;
  uint16_t sequence_number = 1234;
...
TEST_P(PacedSendingControllerTest, HighPrioDoesntAffectBudget) {
  uint32_t ssrc = 12346;
  uint16_t sequence_number = 1234;
  int64_t capture_time_ms = 56789;
...
TEST_P(PacedSendingControllerTest, SendsOnlyPaddingWhenCongested) {
...
TEST_P(PacedSendingControllerTest, DoesNotAllowOveruseAfterCongestion) {
  uint32_t ssrc = 202020;
...
TEST_P(PacedSendingControllerTest, ResumesSendingWhenCongestionEnds) {
  uint32_t ssrc = 202020;
  uint16_t sequence_number = 1000;
...
TEST_P(PacedSendingControllerTest, Pause) {
  uint32_t ssrc_low_priority = 12345;
  uint32_t ssrc = 12346;
...
TEST_P(PacedSendingControllerTest, ResendPacket) {
  if (GetParam() == PacerMode::kOwnPackets) {
...
TEST_P(PacedSendingControllerTest, ExpectedQueueTimeMs) {
  uint32_t ssrc = 12346;
  uint16_t sequence_number = 1234;
  ...
  const TimeDelta deviation =
      duration - PacedSendingController::kMaxExpectedQueueLength;
  EXPECT_LT(deviation.Abs(),
            TimeDelta::ms(1000 * kPacketSize * 8 / kMaxBitrate));
}

TEST_P(PacedSendingControllerTest, QueueTimeGrowsOverTime) {
  uint32_t ssrc = 12346;
  uint16_t sequence_number = 1234;
  EXPECT_EQ(TimeDelta::Zero(), pacer_->OldestPacketWaitTime());
...
TEST_P(PacedSendingControllerTest, ProbingWithInsertedPackets) {
  const size_t kPacketSize = 1200;
  const int kInitialBitrateBps = 300000;
  uint32_t ssrc = 12346;
  uint16_t sequence_number = 1234;

  PacedSendingControllerProbing packet_sender;
  pacer_ = absl::make_unique<PacedSendingController>(&clock_, &packet_sender,
                                                     nullptr, nullptr);
...
TEST_P(PacedSendingControllerTest, ProbingWithPaddingSupport) {
  const size_t kPacketSize = 1200;
  const int kInitialBitrateBps = 300000;
  uint32_t ssrc = 12346;
  uint16_t sequence_number = 1234;
...
TEST_P(PacedSendingControllerTest, PaddingOveruse) {
  uint32_t ssrc = 12346;
  uint16_t sequence_number = 1234;
  const size_t kPacketSize = 1200;
...
TEST_P(PacedSendingControllerTest, ProbeClusterId) {
  MockPacketSender callback;
...
TEST_P(PacedSendingControllerTest, OwnedPacketPrioritizedOnType) {
  if (GetParam() != PacerMode::kOwnPackets) {
