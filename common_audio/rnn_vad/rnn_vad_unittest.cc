...
// File of PCM samples (mono, 48 kHz).
const std::string kAudioSamplesFilePath =
    test::ResourcePath("common_audio/rnn_vad/samples", "pcm");
// Features and VAD probabilities corresponding to the PCM samples file must be
// computed using the trained model (e.g., using TensorFlow code). The latter
// must match the model encoded in rnn_vad.cc - i.e., same network and same
// parameters.
// The features file must also include the silence flag for each frame.
const std::string kFeaturesFilePath =
    test::ResourcePath("common_audio/rnn_vad/features", "out");
const std::string kVadProbFilePath =
    test::ResourcePath("common_audio/rnn_vad/vad_prob", "out");
// RNN VAD parameters file.
const std::string kRnnVadParamsFilePath =
    test::ResourcePath("common_audio/rnn_vad/rnn_vad_params_20180202", "pb");
...
class BinaryFileReader {
 public:
  ...
  bool ReadChunk(rtc::ArrayView<T> buff) {
    RTC_CHECK_GT(buff.size(), 0);
    for (size_t i = 0; i < buff.size(); ++i) {
      is_.read(reinterpret_cast<char*>(&buff[i]), sizeof(T));
      if (is_.gcount() != sizeof(T))
        return false;
    }
    return true;
  }
...
void TestFullyConnectedLayer(
    FullyConnectedLayer* fc, rtc::ArrayView<const float> input_vector,
...
void TestGatedRecurrentLayer(
    GatedRecurrentLayer* gru, rtc::ArrayView<const float> input_sequence,
    rtc::ArrayView<const float> expected_output_sequence) {
  RTC_CHECK(gru);
  auto gru_output_view = gru->GetOutput();
  const size_t input_sequence_length = rtc::CheckedDivExact(
      input_sequence.size(), gru->input_size());
  const size_t output_sequence_length = rtc::CheckedDivExact(
      expected_output_sequence.size(), gru->output_size());
  ...
  for (size_t i = 0; i < input_sequence_length; ++i) {
    gru->ComputeOutput(input_sequence.subview(i * gru->input_size(),
                                              gru->input_size()));
...
TEST(RnnVad, CheckFeatureExtraction) {
  // TODO(alessiob): Add once RnnVadFeaturesExtractor is implemented.
}
...
TEST(RnnVad, CheckGatedRecurrentLayer) {
  const std::array<int8_t, 12> bias = {
      96,  -99, -81, -114, 49, 119, -118, 68, -76, 91, 121, 125};
  const std::array<int8_t, 60> weights = {
      124,    9,    1,  116,  -66,  -21, -118, -110,  104,   75,  -23,
      -51,  -72, -111,   47,   93,   77,  -98,   41,   -8,   40,  -23,
      -43, -107,    9,  -73,   30,  -32,   -2,   64,  -26,   91,  -48,
      -24,  -28, -104,   74,  -46,  116,   15,   32,   52, -126,  -38,
     -121,   12,  -16,  110,  -95,   66, -103,  -35,  -38,    3, -126,
      -61,   28,   98, -117,  -43};
  const std::array<int8_t, 60> recurrent_weights = {
      -3,   87,   50,   51,  -22,   27,  -39,   62,   31,  -83,  -52,
      -48,   -6,   83,  -19,  104,  105,   48,   23,   68,   23,   40,
        7, -120,   64,  -62,  117,   85,  -51,  -43,   54, -105,  120,
       56, -128, -107,   39,   50,  -17,  -47, -117,   14,  108,   12,
       -7,  -72,  103,  -87,  -66,   82,   84,  100,  -98,  102,  -49,
       44,  122,  106,  -20,  -69};
  ...
  // Test on different inputs.
  {
    ...
    const std::array<float, 16> expected_output_sequence = {
        0.02391230,  0.57730770,  0.00000000,  0.00000000,
        0.01282811,  0.64330572,  0.00000000,  0.04863098,
        0.00781069,  0.75267816,  0.00000000,  0.02579715,
...
TEST(RnnVad, CheckValidVadProbabilities) {
  ...
  BinaryFileReader<int16_t> samples_reader(kAudioSamplesFilePath);
  std::array<int16_t, rnn_vad::kAudioFrameSize> samples;
  rtc::ArrayView<int16_t, rnn_vad::kAudioFrameSize> samples_view(
  ...
  const size_t num_frames =
      samples_reader.data_length() / rnn_vad::kAudioFrameSize;
...
TEST(RnnVad, RnnVadBitExactness) {
  // Init.
  BinaryFileReader<float> features_reader(kFeaturesFilePath);
  BinaryFileReader<float> vad_probs_reader(kVadProbFilePath);
  const size_t number_of_frames = vad_probs_reader.data_length();
  ASSERT_EQ(features_reader.data_length(),
            number_of_frames * (rnn_vad::kFeatureVectorSize + 1))
      << "The feature matrix and VAD probabilities vector sizes mismatch.";
  // Frame-wise buffers.
  float expected_vad_probability;
  float is_silence;
  std::array<float, rnn_vad::kFeatureVectorSize + 1> features;
  rtc::ArrayView<float, rnn_vad::kFeatureVectorSize> features_view(
      features.data(), rnn_vad::kFeatureVectorSize);
  ...
  for (size_t i = 0; i < number_of_frames; ++i) {
    ...
    RTC_CHECK(features_reader.ReadChunk(features_view));
    // Skip silent frames.
    ...
    vad.ComputeVadProbability(features_view);
