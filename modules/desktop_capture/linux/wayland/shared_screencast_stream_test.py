...
# Get rid of "modules/desktop_capture/linux/wayland"
...
def _ParseArgs():
  ...
  parser.add_argument('--isolated-script-test-perf-output',
                      default=None,
                      help='Path to output an empty perf results.')
  parser.add_argument('--isolated-script-test-output',
                      default=None,
                      help='Path to output a JSON file.')
...
def _GetPipeWireDir():
  ...
  pipewire_dir = os.path.join(third_party_dir, 'pipewire/linux-amd64')
...
def main():
        ...
        json.dump(
            {
                ...
                'tests': {
                    'SharedScreencastStream': {
                        ...
                        'expected': 'PASS',
                        'artifacts': {
                            'reason': 'Failed to open PipeWIre libraries'
                        }
                    }
                }
            }, fp)
  ...
  pipewire_test_process = subprocess.run([test_command],
                                         stdout=True,
                                         check=False)
      ...
      json.dump(
          {
              ...
              'tests': {
                  'SharedScreencastStream': {
                      'actual': 'FAIL' if return_value else 'PASS',
                      'expected': 'PASS',
                  }
              }
          }, fp)
