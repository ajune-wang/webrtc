...
void ComputeCrossCorrelation(rtc::ArrayView<float> x_corr,
                             rtc::ArrayView<const float> x,
                             rtc::ArrayView<const float> y) {
  const size_t len = x.size();
  const size_t max_lag = x_corr.size();
  RTC_DCHECK_EQ(len, y.size());
  RTC_DCHECK_LT(max_lag, len);
  for (size_t lag = 0; lag < max_lag; ++lag)
    x_corr[lag] = std::inner_product(x.begin(), x.begin() + len - lag,
                                     y.begin() + lag, 0.f);
}
...
void ComputeInverseFilterCoefficients(
  ...
  // Compute auto correlation terms and denoise them assuming -40 dB white noise
  // floor.
  ComputeCrossCorrelation({auto_corr, kNumLpcCoefficients}, x, x);
  auto_corr[0] *= 1.0001f;
