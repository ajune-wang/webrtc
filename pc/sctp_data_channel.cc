...
BYPASS_PROXY_CUSTOM_METHOD2(void, SendAsync, DataBuffer, void*)
...
class SctpDataChannel::ObserverAdapter : public DataChannelObserver {
 ...
 private:
  ...
  void OnSendComplete(RTCError error, void* context) override {
    RTC_DCHECK_RUN_ON(network_thread());
    signaling_thread()->PostTask(SafeTask(
        safety_.flag(),
        [this, error, context] { delegate_->OnSendComplete(error, context); }));
  }
...
void SctpDataChannel::SendAsync(DataBuffer buffer, void* context) {
  network_thread_->PostTask(
      SafeTask(network_safety_, [this, buffer = std::move(buffer), context] {
        ...
        if (observer_)
          observer_->OnSendComplete(err, context);
      }));
}
