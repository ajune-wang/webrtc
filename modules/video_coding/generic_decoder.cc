...
void VCMDecodedFrameCallback::Decoded(VideoFrame& decodedImage,
  ...
  int dropped_frames = 0;
  {
    ...
    while (!timestamp_map_.empty()) {
      // The timestamp we are looking for is older than oldest timestamp in the
      // list and so it is not present.
      if (IsNewerTimestamp(timestamp_map_.front().rtp_timestamp,
                           decodedImage.timestamp())) {
        break;
      }
      const auto entry = timestamp_map_.front();
      timestamp_map_.pop_front();
      if (entry.rtp_timestamp == decodedImage.timestamp()) {
        frame_info = std::move(entry.frame_info);
        break;
      } else {
        ++dropped_frames;
      }
