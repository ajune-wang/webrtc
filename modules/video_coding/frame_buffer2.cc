...
FrameBuffer::ReturnReason FrameBuffer::NextFrame(
  ...
  do {
    now_ms = clock_->TimeInMilliseconds();
    {
      ...
      // |frame_it| points to the first undecoded frame.
      auto frame_it = frames_.begin();

      for (; frame_it != frames_.end(); ++frame_it) {
        if (frame_it->first > last_continuous_frame_) {
          break;
        }
...
void FrameBuffer::ClearFramesAndHistory() {
  ...
  frames_.clear();
  last_decoded_frame_ = absl::nullopt;
  last_continuous_frame_ = absl::nullopt;
  frames_to_decode_.clear();
  num_frames_history_ = 0;
  num_frames_buffered_ = 0;
}
