...
TEST_F(VideoSendStreamImplTest, RegistersAsBitrateObserverOnStart) {
  test_queue.SendTask([this] {
    ...
    const double kBitratePriority = 0.5;
    config.suspend_below_min_bitrate = kSuspend;
    auto vss_impl = CreateVideoSendStreamImpl(
        kDefaultInitialBitrateBps, kBitratePriority,
        VideoEncoderConfig::ContentType::kRealtimeVideo);
    EXPECT_CALL(bitrate_allocator,
                AddObserver(vss_impl.get(), 0, kDefaultInitialBitrateBps, 0,
                            !kSuspend, "test", kBitratePriority, false))
        .Times(1);
    vss_impl->Start();
...
TEST_F(VideoSendStreamImplTest, ReportFeedbackAvailability) {
  test_queue.SendTask([this] {
    ...
    auto vss_impl = CreateVideoSendStreamImpl(
        ...
        VideoEncoderConfig::ContentType::kRealtimeVideo);
    EXPECT_CALL(bitrate_allocator,
                AddObserver(vss_impl.get(), _, _, _, _, _, _, true))
        .Times(1);
    vss_impl->Start();
    EXPECT_CALL(bitrate_allocator, RemoveObserver(vss_impl.get())).Times(1);
    vss_impl->Stop();
...
TEST_F(VideoSendStreamImplTest, SetsScreensharePacingFactorWithFeedback) {
  ...
  test_queue.SendTask([this] {
    ...
    EXPECT_CALL(transport_controller,
        ...
        .Times(1);
    auto vss_impl = CreateVideoSendStreamImpl(
        kDefaultInitialBitrateBps, 0, VideoEncoderConfig::ContentType::kScreen);
    vss_impl->Start();
    vss_impl->Stop();
    vss_impl.reset();
  });
...
TEST_F(VideoSendStreamImplTest,
       DoesNotSetScreensharePacingFactorWithoutFeedback) {
  test::ScopedFieldTrials alr_experiment(GetAlrProbingExperimentString());
  test_queue.SendTask([this] {
