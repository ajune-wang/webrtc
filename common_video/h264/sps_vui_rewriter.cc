...
bool CopyOrRewriteVideoSignalTypeInfo(
  ...
  uint32_t colour_primaries = 3;          // H264 default: unspecified
  uint32_t transfer_characteristics = 3;  // H264 default: unspecified
  uint32_t matrix_coefficients = 3;       // H264 default: unspecified
  ...
  uint32_t colour_primaries_override = colour_primaries;
  uint32_t transfer_characteristics_override = transfer_characteristics;
  uint32_t matrix_coefficients_override = matrix_coefficients;
  if (color_space) {
  ...
  if (video_signal_type_present_flag_override) {
    ...
    if (colour_description_present_flag_override) {
      RETURN_FALSE_ON_FAIL(
          destination->WriteBits(colour_primaries_override, 8));
      RETURN_FALSE_ON_FAIL(
