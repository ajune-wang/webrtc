...
class MockAudioTransport : public webrtc::AudioTransport {
 public:
  ...
  void expectNeedMorePlayData(NeedMorePlayDataBlock block) { needMorePlayDataBlock = block; }
...
- (void)testRunPlayoutAndRecordingInFullDuplex {
  ...
  mock.expectRecordedDataIsAvailable(^(const void *audioSamples,
    ...
    NSData *data = [NSData dataWithBytes:audioSamples
  ...
  size_t fifoAverageSize = (fifoTotalWrittenElements == 0) ?
      0.0 :
