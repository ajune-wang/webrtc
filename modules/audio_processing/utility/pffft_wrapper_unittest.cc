...
void PffftValidateWrapper(size_t fft_size, bool complex_fft) {
  // Always use the same seed to avoid flackiness.
  std::srand(0);
  ...
  // Forward transform.
  pffft_transform(pffft_status, in, out, nullptr, PFFFT_FORWARD);
  ...
  // Backward transform.
  pffft_transform(pffft_status, in, out, nullptr, PFFFT_BACKWARD);
