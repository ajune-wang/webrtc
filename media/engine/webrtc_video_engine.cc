...
bool WebRtcVideoChannel::RemoveSendStream(uint32_t ssrc) {
  ...
  if (rtcp_receiver_report_ssrc_ == ssrc) {
    ...
    for (auto& kv : receive_streams_) {
      ...
      call_->OnLocalSsrcUpdated(kv.second->stream(),
                                rtcp_receiver_report_ssrc_);
      auto* flex_fec = kv.second->flexfec_stream();
      if (flex_fec)
        call_->OnLocalSsrcUpdated(*flex_fec, rtcp_receiver_report_ssrc_);
    }
  }
