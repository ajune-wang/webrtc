    ...
    case STUN_BINDING_REQUEST:
      ...
      if (remote_ufrag == remote_candidate_.username()) {
        if (dtls_stun_piggyback_consumer_) {
          ...
          const StunUInt64Attribute* dtls_piggyback_ack =
              msg->GetUInt64(STUN_ATTR_META_DTLS_IN_STUN_ACK);
...
void Connection::SendStunBindingResponse(const StunMessage* message) {
  ...
  if (dtls_stun_piggyback_data_producer_ && dtls_stun_piggyback_ack_producer_) {
    std::optional<absl::string_view> dtls_piggyback_attr =
        dtls_stun_piggyback_data_producer_(STUN_BINDING_RESPONSE);
    std::optional<uint64_t> dtls_piggyback_ack =
    ...
    size_t need_length =
        (dtls_piggyback_attr
             ...
             : 0) +
        (dtls_piggyback_ack ? 8 + kStunAttributeHeaderSize : 0);
    if (response.length() + need_length < kMaxStunBindingLength) {
      ...
      if (dtls_piggyback_ack) {
        response.AddAttribute(std::make_unique<StunUInt64Attribute>(
  ...
  if (dtls_stun_piggyback_data_producer_ && dtls_stun_piggyback_ack_producer_) {
    ...
    size_t need_length =
        (dtls_piggyback_attr
             ...
             : 0) +
        (dtls_piggyback_ack ? 8 + kStunAttributeHeaderSize : 0);
    if (message->length() + need_length < kMaxStunBindingLength) {
      if (dtls_piggyback_attr) {
        message->AddAttribute(std::make_unique<StunByteStringAttribute>(
            STUN_ATTR_META_DTLS_IN_STUN, *dtls_piggyback_attr));
      }
...
void Connection::OnConnectionRequestResponse(StunRequest* request,
  ...
  if (dtls_stun_piggyback_consumer_) {
    ...
    const StunUInt64Attribute* dtls_piggyback_ack =
        response->GetUInt64(STUN_ATTR_META_DTLS_IN_STUN_ACK);
