...
TEST(DefaultVideoQualityAnalyzerTest, FrameCanBeReceivedBySender) {
  ...
  std::map<StatsKey, StreamStats> stats = analyzer.GetStats();
  const StatsKey kSenderReceiverStats(kStreamLabel, kSenderPeerName,
                                      kReceiverPeerName);
  const StatsKey kSenderSenderStats(kStreamLabel, kSenderPeerName,
                                    kSenderPeerName);
  EXPECT_EQ(stats.size(), 2lu);
  {
    FrameCounters stream_conters =
        analyzer.GetPerStreamCounters().at(kSenderReceiverStats);
  ...
  {
    FrameCounters stream_conters =
        analyzer.GetPerStreamCounters().at(kSenderSenderStats);
    EXPECT_EQ(stream_conters.captured, 1);
    EXPECT_EQ(stream_conters.pre_encoded, 1);
    EXPECT_EQ(stream_conters.encoded, 1);
    EXPECT_EQ(stream_conters.received, 1);
