...
void TestTimestampFilter(double rel_freq_error) {
  ...
  for (int i = 0; i < kNumFrames; i++) {
    ...
    int64_t kCaptureTimeUs = kEpoch + i * (kIntervalUs + interval_error_us);
    int64_t kSystemTimeUs = system_start_us + i * kIntervalUs;
    // And system time readings are subject to jitter.
    int64_t system_measured_us = kSystemTimeUs + random.Rand(kJitterUs);

    int64_t offset_us = timestamp_aligner_for_test.UpdateOffset(
        kCaptureTimeUs, system_measured_us);

    int64_t filtered_time_us = kCaptureTimeUs + offset_us;
    ...
    if (i == 0) {
      ...
      EXPECT_NEAR(filtered_time_us, kSystemTimeUs + expected_error_us,
