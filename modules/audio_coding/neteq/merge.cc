...
size_t Merge::Process(int16_t* input,
  ...
  RTC_DCHECK(fs_hz_ == 8000 || fs_hz_ == 16000 || fs_hz_ == 32000 ||
             fs_hz_ == 48000);
  RTC_DCHECK(fs_hz_ <= kMaxSampleRate);  // Should not be possible.
  if (input_length == 0) {
    return 0;
  }
  ...
  RTC_DCHECK(input_length_per_channel == input_length / num_channels_);
  ...
  for (size_t channel = 0; channel < num_channels_; ++channel) {
    ...
    if (channel == 0) {
      ...
      RTC_DCHECK(output->Size() == output_length);
    }
...
size_t Merge::GetExpandedSignal(size_t* old_length, size_t* expand_period) {
  ...
  *old_length = sync_buffer_->FutureLength();
  ...
  RTC_DCHECK(*old_length >= expand_->overlap_length());
  ...
  RTC_DCHECK(210 * kMaxSampleRate / 8000 >= *old_length);
  ...
  *expand_period = expanded_temp.Size();  // Samples per channel.
  ...
  RTC_DCHECK(expanded_.Size() == *old_length);
  RTC_DCHECK(expanded_temp.Size() > 0);
  ...
  RTC_DCHECK(expanded_.Size() >= required_length);
...
size_t Merge::CorrelateAndPeakSearch(size_t start_position,
  ...
  while (((best_correlation_index + input_length) <
    ...
    RTC_DCHECK(false);                        // Should never happen.
