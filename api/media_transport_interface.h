...
class MediaTransportInterface {
 public:
  ...
  // Sets an observer for per packet overhead. At this time this returns
  // a fixed per packet overhead (not including transport overheads), regardless
  // if it's audio or video packet. (that is: it returns an overhead for audio
  // packet).
  virtual void SetOverheadObserver(OverheadObserver* observer);
