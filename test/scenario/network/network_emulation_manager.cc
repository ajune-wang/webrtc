...
EmulatedNetworkController*
NetworkEmulationManagerImpl::CreateEmulatedNetworkController(
    const std::vector<EmulatedEndpoint*>& endpoints) {
  auto endpoints_controller = absl::make_unique<EndpointsController>(endpoints);
  ...
  network_thread->SetName("net_thread" + std::to_string(socket_servers_.size()),
                          nullptr);
  network_thread->Start();

  auto network_manager = absl::make_unique<EmulatedNetworkManager>(
      endpoints_controller.get(), std::move(network_thread));
  ...
  EmulatedNetworkController* out = network_manager.get();
