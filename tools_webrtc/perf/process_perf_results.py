...
def main():
  ...
  exit_code = _ConfigurePythonPath(args.build_properties['outdir'])
  if exit_code != 0:
  ...
  uploader_options = catapult_uploader.UploaderOptions(
      perf_dashboard_machine_group=(
          args.build_properties['perf_dashboard_machine_group']),
      bot=args.build_properties['bot'],
      webrtc_git_hash=args.build_properties['webrtc_git_hash'],
      commit_position=args.build_properties['commit_position'],
      build_page_url=args.build_properties['build_page_url'],
      dashboard_url=args.build_properties['dashboard_url'],
  ...
  for d in directory_list:
    for f in os.listdir(d):
      ...
      if 'perftest-output' in f:
        input_results_file = os.path.join(args.task_output_dir, d, f)
