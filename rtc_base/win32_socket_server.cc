...
void ReportWSAError(LPCSTR context, int error, const SocketAddress& address) {
  ...
  RTC_DLOG(LS_INFO) << context << " = " << error << " (" << error_string << ":"
...
void Win32Socket::EventSink::OnNcDestroy() {
  if (parent_) {
    RTC_DLOG(LS_ERROR) << "EventSink hwnd is being destroyed, but the event sink"
  ...
  switch (event) {
    case FD_CONNECT:
      if (error != ERROR_SUCCESS) {
        ...
        RTC_DLOG(LS_INFO) << "WSAAsync:connect error (" << duration
                         << " ms), faking close";
#endif
        state_ = CS_CLOSED;
...
#if !defined(NDEBUG)
        int64_t duration = TimeSince(connect_time_);
        RTC_DLOG(LS_INFO) << "WSAAsync:connect (" << duration << " ms)";
#endif
        state_ = CS_CONNECTED;
