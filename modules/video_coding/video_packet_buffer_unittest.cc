...
TEST_F(TestPacketBuffer, CountsUniqueFrames) {
  ...
  uint8_t* data3 = new uint8_t[5]();
  uint8_t* data4 = new uint8_t[5]();
  uint8_t* data5 = new uint8_t[5]();
  uint8_t* data6 = new uint8_t[5]();
  uint8_t* data7 = new uint8_t[5]();

  ASSERT_EQ(0, packet_buffer_->GetUniqueFramesSeen());

  EXPECT_TRUE(Insert(seq_num, kKeyFrame, kFirst, kNotLast, 5, data1, 100));
  ASSERT_EQ(1, packet_buffer_->GetUniqueFramesSeen());
  // Still the same frame.
  EXPECT_TRUE(Insert(seq_num + 1, kKeyFrame, kNotFirst, kLast, 5, data2, 100));
  ASSERT_EQ(1, packet_buffer_->GetUniqueFramesSeen());

  // Second frame.
  EXPECT_TRUE(Insert(seq_num + 2, kKeyFrame, kFirst, kNotLast, 5, data3, 200));
  ASSERT_EQ(2, packet_buffer_->GetUniqueFramesSeen());
  EXPECT_TRUE(Insert(seq_num + 3, kKeyFrame, kNotFirst, kLast, 5, data4, 200));
  ASSERT_EQ(2, packet_buffer_->GetUniqueFramesSeen());

  // Old packet.
  EXPECT_TRUE(Insert(seq_num + 1, kKeyFrame, kNotFirst, kLast, 5, data5, 100));
  ASSERT_EQ(2, packet_buffer_->GetUniqueFramesSeen());

  // Missing middle packet.
  EXPECT_TRUE(Insert(seq_num + 4, kKeyFrame, kFirst, kNotLast, 5, data6, 300));
  EXPECT_TRUE(Insert(seq_num + 6, kKeyFrame, kNotFirst, kLast, 5, data7, 300));
