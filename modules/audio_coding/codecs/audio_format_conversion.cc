...
CodecInst SdpToCodecInst(int payload_type, const SdpAudioFormat& audio_format) {
  if (rtc::StrCaseCmp(audio_format.name.c_str(), "g722") == 0) {
  ...
  } else if (rtc::StrCaseCmp(audio_format.name.c_str(), "opus") == 0) {
