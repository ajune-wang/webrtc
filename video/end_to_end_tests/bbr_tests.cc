...
class BbrTestObserver : public test::BaseCongestionControllerTest {
 ...
 private:
  ...
  test::PacketTransport* CreateSendTransport(
    ...
    auto send_pipe = rtc::MakeUnique<webrtc::FakeNetworkPipe>(
  ...
  CreateSendCCFactory(RtcEventLog* event_log) override {
    if (conf_.ret == kBbr) {
      auto bbr_printer = rtc::MakeUnique<BbrStatePrinter>();
      auto return_factory = rtc::MakeUnique<BbrDebugFactory>(bbr_printer.get());
      return {std::move(return_factory), std::move(bbr_printer)};
    } else {
      auto goog_printer = rtc::MakeUnique<GoogCcStatePrinter>();
      auto return_factory =
          rtc::MakeUnique<GoogCcDebugFactory>(event_log, goog_printer.get());
  ...
  CreateReturnCCFactory(RtcEventLog* event_log) override {
    if (conf_.ret == kBbr) {
      auto bbr_printer = rtc::MakeUnique<BbrStatePrinter>();
      auto send_factory = rtc::MakeUnique<BbrDebugFactory>(bbr_printer.get());
      return {std::move(send_factory), std::move(bbr_printer)};
    } else {
      auto goog_printer = rtc::MakeUnique<GoogCcStatePrinter>();
      auto send_factory =
          rtc::MakeUnique<GoogCcDebugFactory>(event_log, goog_printer.get());
