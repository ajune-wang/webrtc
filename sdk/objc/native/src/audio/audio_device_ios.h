...
class AudioDeviceIOS : public AudioDeviceGeneric,
 ...
 private:
  ...
  // Updates the audio input unit state based on current state.
  void UpdateAudioInputUnit(bool can_record);

  // Creates the audio unit for playout.
  bool CreateAudioOutputUnit();

  // Updates the audio output unit state based on current state.
  void UpdateAudioOutputUnit(bool can_play);
  ...
  // Activates our audio session, creates and initializes the output audio unit
  // (Remote I/O) and verifies that we got the preferred native audio
  // parameters.
  bool InitAudioOutputUnit();

  // Activates our audio session, creates and initializes the input audio unit
  // (Voice-Processing I/O) and verifies that we got the preferred native audio
  // parameters.
  bool InitAudioInputUnit();

  // Closes and deletes the Voice-Processing I/O audio unit.
  void DeinitAudioInputUnit();

  // Closes and deletes the Remote I/O audio unit
  void DeinitAudioOutputUnit();
  ...
  // Set to true after successful call to InitRecording(),
  // false otherwise.
  bool audio_recording_is_initialized_ RTC_GUARDED_BY(thread_checker_);

  // Set to true after successful call to InitPlayout(),
  // false otherwise.
  bool audio_playout_is_initialized_ RTC_GUARDED_BY(thread_checker_);
