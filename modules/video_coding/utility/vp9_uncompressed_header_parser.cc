...
bool Vp9ReadColorConfig(BitstreamReader* br, FrameInfo* frame_info) {
  if (frame_info->profile == 2 || frame_info->profile == 3) {
    USE_OR_RETURN(br->ReadBoolean(), [frame_info](bool ten_or_twelve_bits) {
...
bool Vp9ReadFrameSize(BitstreamReader* br, FrameInfo* frame_info) {
  ...
  USE_OR_RETURN(br->ReadUnsigned<uint16_t>(), [frame_info](uint16_t width) {
    frame_info->frame_width = width + 1;
  });
  USE_OR_RETURN(br->ReadUnsigned<uint16_t>(), [frame_info](uint16_t height) {
    frame_info->frame_width = height + 1;
  });
  return true;
}
...
bool Vp9ReadRenderSize(BitstreamReader* br, FrameInfo* frame_info) {
  ...
  return br->IfNextBoolean(
      [&] {
        ...
        USE_OR_RETURN(br->ReadUnsigned<uint16_t>(),
...
bool Vp9ReadFrameSizeFromRefs(BitstreamReader* br, FrameInfo* frame_info) {
  ...
  for (size_t i = 0; !found_ref && i < kVp9NumRefsPerFrame; i++) {
    // Size in refs.
    USE_OR_RETURN(br->ReadBoolean(), [&](bool ref) { found_ref = ref; });
  }
...
bool Vp9ReadQp(BitstreamReader* br, FrameInfo* frame_info) {
  USE_OR_RETURN(br->ReadUnsigned<uint8_t>(),
...
bool Parse(const uint8_t* buf, size_t length, FrameInfo* frame_info) {
  ...
  USE_OR_RETURN(br.ReadBoolean(),
                [frame_info](bool low) { frame_info->profile = int{low}; });
  USE_OR_RETURN(br.ReadBoolean(), [frame_info](bool high) {
  ...
  // Show existing frame.
  RETURN_IF_FALSE(br.IfNextBoolean([&] {
    USE_OR_RETURN(br.ReadUnsigned<uint8_t>(3), [frame_info](uint8_t frame_idx) {
  ...
  USE_OR_RETURN(br.ReadBoolean(), [frame_info](bool frame_type) {
    // Frame type: KEY_FRAME(0), INTER_FRAME(1).
    frame_info->is_keyframe = frame_type == 0;
  });
  USE_OR_RETURN(br.ReadBoolean(), [frame_info](bool show_frame) {
    frame_info->show_frame = show_frame;
  });
  USE_OR_RETURN(br.ReadBoolean(), [frame_info](bool error_resilient) {
