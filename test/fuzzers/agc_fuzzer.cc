...
void FillAudioBuffer(test::FuzzDataHelper* fuzz_data, AudioBuffer* buffer) {
  ...
  for (size_t i = 0; i < buffer->num_channels(); ++i) {
    for (size_t j = 0; j < buffer->num_frames(); ++j) {
      channels[i][j] =
          2 * static_cast<float>(fuzz_data->ReadOrDefaultValue<int16_t>(0));
    }
  }
}

template <typename T, typename U>
T ClipValue(T x, U min, U max) {
...
void FuzzGainControllerConfig(test::FuzzDataHelper* fuzz_data,
  ...
  const int analog_level_min =
      ClipValue(fuzz_data->ReadOrDefaultValue<uint16_t>(0), 0, 65534);
  const int analog_level_max =
      ClipValue(fuzz_data->ReadOrDefaultValue<uint16_t>(65535),
                analog_level_min + 1, 65535);
  const int stream_analog_level =
      ClipValue(fuzz_data->ReadOrDefaultValue<uint16_t>(30000),
                analog_level_min, analog_level_max);
  const int gain =
      ClipValue(fuzz_data->ReadOrDefaultValue<int8_t>(30), -1, 100);
  const int target_level_dbfs =
      ClipValue(fuzz_data->ReadOrDefaultValue<int8_t>(15), -1, 35);
