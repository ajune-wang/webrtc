...
void TestInvalidParameterHandler(const wchar_t* expression,
                                 ...
                                 unsigned int line,
                                 uintptr_t pReserved) {
  NLOG(LS_ERROR, "InvalidParameter Handler called.  Exiting.");
  NLOG(LS_ERROR, expression, std::endl, function, std::endl, file, std::endl,
       line);
  exit(1);
}
void TestPureCallHandler() {
  NLOG(LS_ERROR, "Purecall Handler called.  Exiting.");
  exit(1);
}
int TestCrtReportHandler(int report_type, char* msg, int* retval) {
  NLOG(LS_ERROR, "CrtReport Handler called...");
  NLOG(LS_ERROR, msg);
