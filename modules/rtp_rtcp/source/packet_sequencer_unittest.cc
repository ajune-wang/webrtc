...
TEST_F(PacketSequencerTest, CrashOnUnknownSsrc) {
  constexpr uint32_t kSsrc = 123458;
  RtpPacketToSend packet =
      CreatePacket(RtpPacketMediaType::kForwardErrorCorrection, kSsrc);
  sequencer_.set_media_sequence_number(kMediaStartSequenceNumber);
  EXPECT_DEATH(sequencer_.Sequence(packet), "");
}
