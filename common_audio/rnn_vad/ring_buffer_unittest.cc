...
template <typename T>
void ExpectEq(rtc::ArrayView<const T> a, rtc::ArrayView<const T> b) {
...
void TestRingBuffer() {
  ...
  ss << "buffer size: " << N << ", array size: " << S;
  SCOPED_TRACE(ss.str());
  std::array<T, S> prev_pushed_array;
  std::array<T, S> pushed_array;
  rtc::ArrayView<T, S> pushed_array_view(pushed_array.data(), S);

  // Init.
  RingBuffer<T, S, N> ring_buf(0);
  ring_buf.GetArrayView(0);
  pushed_array.fill(0);
  ring_buf.Push(pushed_array_view);
  ExpectEq({pushed_array}, ring_buf.GetArrayView(0));
  ...
  for (T v = 1; v <= static_cast<T>(N); ++v) {
    ...
    if (N > 1) {
      ...
      ExpectEq({pushed_array}, ring_buf.GetArrayView(1));
    }
  }
  ...
  for (size_t delay = 2; delay < N; ++delay) {
    ...
    ExpectEq({pushed_array}, ring_buf.GetArrayView(delay));
  }
}
...
TEST(RnnVad, CheckRingBufferArrayViews) {
  constexpr size_t s = 3;
  constexpr size_t n = 4;
  RingBuffer<int, 3, n> ring_buf(0);
  ...
  for (size_t k = 0; k <= n; ++k) {  // Push data n + 1 times.
    ...
    ring_buf.Push({pushed_array.data(), s});
  }
}

TEST(RnnVad, CheckRingBufferUnsigned) {
...
TEST(RnnVad, CheckRingBufferSigned) {
...
TEST(RnnVad, CheckRingBufferFloating) {
