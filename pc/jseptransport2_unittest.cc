...
class JsepTransport2HeaderExtensionTest
 ...
 protected:
  ...
  void TestSendRecvPacketWithEncryptedHeaderExtension() {
    ...
    // decrypted.
    ASSERT_TRUE(jsep_transport1_->rtp_transport()->SendRtpPacket(
        &rtp_packet1to2, options, cricket::PF_SRTP_BYPASS));

    // Do the same thing in the opposite direction;
    ASSERT_TRUE(jsep_transport2_->rtp_transport()->SendRtpPacket(
        &rtp_packet2to1, options, cricket::PF_SRTP_BYPASS));
  }
