...
// Checks that `GainController2` crashes if it runs its internal VAD and the
// speech probability values are provided by the caller.
TEST(GainController2DeathTest, CrashIfUseInternalVadAndSpeechProbabilityGiven) {
  constexpr int kSampleRateHz = AudioProcessing::kSampleRate48kHz;
  constexpr int kStereo = 2;
  AudioBuffer audio_buffer(kSampleRateHz, kStereo, kSampleRateHz, kStereo,
                           kSampleRateHz, kStereo);
