...
int main(int argc, char* argv[]) {
  ...
  if (!parsed_log.rtp_packets_by_ssrc(webrtc::kOutgoingPacket).empty()) {
    analyzer.CreateStreamBitrateGraph(webrtc::kOutgoingPacket,
                                      collection->AppendNewPlot());
  }
  RTC_DCHECK(!parsed_log.generic_packets_sent().empty()) << "Sent!";
  RTC_DCHECK(!parsed_log.generic_packets_received().empty()) << "Received!";
  RTC_DCHECK(!parsed_log.generic_acks_received().empty()) << "Acked!";
  if (FLAG_plot_incoming_stream_bitrate &&
