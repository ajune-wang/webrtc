...
void Vp9ReadRenderSize(BitReader& br,
                       int total_buffer_size_in_bits,
                       Vp9UncompressedHeader& frame_info) {
  if (br.Read<bool>()) {
    frame_info.render_size_offset_bits =
        total_buffer_size_in_bits - br.RemainingBitCount();
...
void Vp9ReadFrameSizeFromRefs(BitReader& br,
  ...
  Vp9ReadRenderSize(br, total_buffer_size_bits, frame_info);
}
...
void Vp9ReadLoopfilter(BitReader& br) {
  ...
  if (br.Read<bool>()) {    // if mode_ref_delta_enabled
    if (br.Read<bool>()) {  // if mode_ref_delta_update
...
void Vp9ReadSegmentationParams(BitReader& br,
  ...
  if (!br.Read<bool>()) {  // segmentation_enabled
    return;
  }
...
bool Parse(rtc::ArrayView<const uint8_t> buf,
  ...
  if (frame_info.is_keyframe) {
    ...
    if (is_intra_only) {
      ...
      Vp9ReadFrameSizeFromRefs(br, total_buffer_size_bits, frame_info);
