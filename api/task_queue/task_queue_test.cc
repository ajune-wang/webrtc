...
TEST_P(TaskQueueTest, PostedExecutedDelayedClosureDestroyedOnTaskQueue) {
  std::unique_ptr<webrtc::TaskQueueFactory> factory = GetParam()(nullptr);
  auto queue = CreateTaskQueue(
      factory, "PostedExecutedDelayedClosureDestroyedOnTaskQueue");
  TaskQueueBase* queue_ptr = queue.get();
  // Ensure an executed lambda is destroyed on the task queue.
  rtc::Event finished;
  queue->PostDelayedTask([cleanup = absl::Cleanup([queue_ptr, &finished] {
...
TEST_P(TaskQueueTest,
       PostedExecutedDelayedHighPrecisionClosureDestroyedOnTaskQueue) {
  std::unique_ptr<webrtc::TaskQueueFactory> factory = GetParam()(nullptr);
  auto queue = CreateTaskQueue(
      factory, "PostedExecutedDelayedHighPrecisionClosureDestroyedOnTaskQueue");
  TaskQueueBase* queue_ptr = queue.get();
  // Ensure an executed lambda is destroyed on the task queue.
  rtc::Event finished;
  queue->PostDelayedHighPrecisionTask(
