...
std::unique_ptr<TickTimer::Countdown> GetNewCountdown(
  ...
  return tick_timer.GetNewCountdown(logging_period_ms /
...
ExpandUmaLogger::ExpandUmaLogger(std::string uma_name,
                                 ...
                                 const TickTimer* tick_timer)
    : uma_name_(uma_name),
      logging_period_ms_(logging_period_ms),
      tick_timer_(*tick_timer),
      timer_(GetNewCountdown(tick_timer_, logging_period_ms_)) {}
...
void ExpandUmaLogger::UpdateSampleCounter(uint64_t samples,
  ...
  const int rate = (100 * 1000 * diff) / (sample_rate_hz * logging_period_ms_);
  RTC_DCHECK_GE(rate, 0);
  RTC_DCHECK_LE(rate, 100);
  RTC_HISTOGRAM_PERCENTAGE(uma_name_, rate);
  timer_ = GetNewCountdown(tick_timer_, logging_period_ms_);
}
