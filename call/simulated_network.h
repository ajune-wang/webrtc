...
//
// Usage:
// Create the BuiltInNetworkBehaviorConfig that represent the network
// configuration and create an instance of SimulatedNetwork:
//
// BuiltInNetworkBehaviorConfig config;
// SimulatedNetwork network = SimulatedNetwork(config);
//
...
class SimulatedNetwork : public SimulatedNetworkInterface {
 ...
 private:
  struct PacketInfo {
    ...
    // Time when the packet starts to be sent on the wire, this can differ from
    // `send_time_us` in `packet`, which instead represents the time when the
    // packet entered the network emulation (in case of limited link capacity,
    // the packet will not be sent immediately over the wire).
    int64_t actual_transmission_time_us;
