...
class BbrNetworkController : public NetworkControllerInterface {
 public:
  ...
  explicit BbrNetworkController(NetworkControllerObserver* observer);
 ...
 private:
  ...
  // Timestamp::kEpoch if no valid timestamp is available.
  Timestamp last_acked_packet_sent_time_ = Timestamp::kEpoch;

  // The time at which the most recent packet was acknowledged.
  Timestamp last_acked_packet_ack_time_ = Timestamp::kEpoch;

  bool is_app_limited_ = false;

  // The packet that will be acknowledged after this one will cause the sampler
  // to exit the app-limited phase.
  Timestamp end_of_app_limited_phase_ = Timestamp::kEpoch;

  // The number of the round trips that have occurred during the connection.
  BbrRoundTripCount round_trip_count_ = 0;

  // The send time of the most recently sent packet.
  Timestamp last_send_time_ = Timestamp::kEpoch;

  // Acknowledgement of any packet after |current_round_trip_end_| will cause
  // the round trip counter to advance.
  Timestamp current_round_trip_end_ = Timestamp::kEpoch;
  ...
  Timestamp min_rtt_timestamp_ = Timestamp::kEpoch;
  ...
  Timestamp end_recovery_at_ = Timestamp::kEpoch;
  ...
  // NetworkControllerInterface interface
 public:
  void OnNetworkAvailability(NetworkAvailability) override;
  void OnNetworkRouteChange(NetworkRouteChange) override;
  void OnProcessInterval(ProcessInterval) override;
  void OnRemoteBitrateReport(RemoteBitrateReport) override;
  void OnRoundTripTimeUpdate(RoundTripTimeUpdate) override;
  void OnSentPacket(SentPacket) override;
  void OnStreamsConfig(StreamsConfig) override;
  void OnTargetRateConstraints(TargetRateConstraints) override;
  void OnTransportLossReport(TransportLossReport) override;
  void OnTransportPacketsFeedback(TransportPacketsFeedback) override;
};
