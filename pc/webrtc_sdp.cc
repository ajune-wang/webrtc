...
bool IsFmtpParam(const std::string& name) {
  ...
  // parameters. Only ptime, maxptime, adaptive-ptime, channels and rate are
  ...
  if (media_type == cricket::MEDIA_TYPE_VIDEO) {
    ...
    for (const cricket::AudioCodec& codec : media_desc->as_audio()->codecs()) {
      ...
      if (GetParameter(kCodecParamAdaptivePTime, codec.params,
                       &adaptive_ptime)) {
        RTC_DCHECK_EQ(adaptive_ptime, codec.id);
...
bool ParseContent(const std::string& message,
  ...
  std::vector<int> adaptive_ptime_codecs;  // Codec PTs with adaptive-ptime set.
  std::vector<std::string> stream_ids;
  std::string track_id;
