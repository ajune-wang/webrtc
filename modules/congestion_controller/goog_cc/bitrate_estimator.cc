...
int ReadInitialThroughputWindowSizeMs(const WebRtcConfig* webrtc_config) {
  std::string experiment_string =
      webrtc_config->FindFullName(kBweInitialThroughputWindowExperiment);
...
BitrateEstimator::BitrateEstimator(const WebRtcConfig* webrtc_config)
    : sum_(0),
  ...
  if (webrtc_config->IsEnabled(kBweInitialThroughputWindowExperiment)) {
    initial_window_ms_ = ReadInitialThroughputWindowSizeMs(webrtc_config);
  }
}
