...
// Updates the noise floor with instant attack and slow decay. This tuning is
// specific for AGC2, so that it can reduce the gain promptly if the noise level
// suddenly increases (instant attack) and it does not too quickly increase the
// gain because of a sudden noise level reduction (slow decay).
float SmoothNoiseFloorEstimate(float current_estimate, float new_estimate) {
  constexpr float kDecay = 0.7f;
  static_assert(kDecay > 0.0f, "");
  static_assert(kDecay < 1.0f, "");
  if (current_estimate > new_estimate) {
    // Decay phase.
    return kDecay * new_estimate + (1.0f - kDecay) * current_estimate;
  }
  // Instant attack.
  return new_estimate;
}
...
class NoiseFloorEstimator : public NoiseLevelEstimator {
 public:
  ...
  float Analyze(const AudioFrameView<const float>& frame) override {
    ...
    if (frame_energy <= min_noise_energy_) {
      ...
      const float noise_energy_dbfs =
          EnergyToDbfs(noise_energy_, frame.samples_per_channel());
      data_dumper_->DumpRaw("agc2_noise_floor_preliminary_level_dbfs",
                            noise_energy_dbfs);
      return noise_energy_dbfs;
    }
    ...
    data_dumper_->DumpRaw(
        "agc2_noise_floor_preliminary_level_dbfs",
        EnergyToDbfs(preliminary_noise_energy_, frame.samples_per_channel()));
 ...
 private:
  void Initialize(int sample_rate_hz) {
    ...
    first_period_ = true;
    preliminary_noise_energy_set_ = false;
    // Minimum frame energy based on the smallest S16 sample value.
    min_noise_energy_ = sample_rate_hz * 2.0f * 2.0f / kFramesPerSecond;
