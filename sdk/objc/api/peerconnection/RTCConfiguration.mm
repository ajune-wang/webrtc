...
- (instancetype)initWithNativeConfiguration:
  ...
  if (self = [super init]) {
    ...
    for (const webrtc::PeerConnectionInterface::IceServer& server : config.servers) {
      RTC_OBJC_TYPE(RTCIceServer) *iceServer = [[RTC_OBJC_TYPE(RTCIceServer) alloc] initWithNativeServer:server];
    ...
    if (!config.certificates.empty()) {
      ...
      _certificate =
          [[RTC_OBJC_TYPE(RTCCertificate) alloc] initWithPrivateKey:@(native_pem.private_key().c_str())
