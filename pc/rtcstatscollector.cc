...
void ProduceSenderMediaTrackStats(
  ...
  for (auto const sender : senders) {
    RTC_LOG(LS_ERROR) << "DEBUG: Starting sender";
    if (sender->media_type() == cricket::MEDIA_TYPE_AUDIO) {
      ...
      const std::vector<cricket::VoiceSenderInfo*>* voice_sender_infos =
          track_media_info_map.GetVoiceSenderInfos(*track);
      RTC_LOG(LS_ERROR) << "DEBUG: # voice sender infos "
                        << voice_sender_infos->size();
      for (const auto& voice_sender_info : *voice_sender_infos) {
        // Only use the sender info associated with this sender.
        // Looks silly, but don't see how to map this info from sender.
        if (voice_sender_info->ssrc() == sender->ssrc()) {
          std::unique_ptr<RTCMediaStreamTrackStats> audio_track_stats =
              ProduceMediaStreamTrackStatsFromVoiceSenderInfo(
                  timestamp_us, *track, *voice_sender_info);
          RTC_LOG(LS_ERROR) << "DEBUG: Adding " << audio_track_stats->id()
                            << " ssrc " << sender->ssrc();
      ...
      const std::vector<cricket::VideoSenderInfo*>* video_sender_infos =
          track_media_info_map.GetVideoSenderInfos(*track);
      if (!video_sender_infos) {
        continue;
      }
      // See comment above.
      for (const auto& video_sender_info : *video_sender_infos) {
        if (video_sender_info->ssrc() == sender->ssrc()) {
          std::unique_ptr<RTCMediaStreamTrackStats> video_track_stats =
              ProduceMediaStreamTrackStatsFromVideoSenderInfo(
                  timestamp_us, *track, *video_sender_info);
