...
template <typename T>
constexpr uint64_t MakeMask(const size_t N, const T* values) {
  return N > 1 ? (MakeMask(1, values) + MakeMask(N - 1, &values[1]))
               : (static_cast<uint8_t>(values[0]) < 64
                      ? (uint64_t{1} << static_cast<uint8_t>(values[0]))
...
constexpr uint64_t CreateEnumBitmask(const T (&values)[N]) {
  return MakeMask(N, values);
}
