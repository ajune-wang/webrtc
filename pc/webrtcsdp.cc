...
// This is a an track that does not contain ssrcs and only contains
// stream_ids/track_id if it's signaled with a=msid lines.
void CreateUnsignaledTrack(const std::vector<std::string>& msid_stream_ids,
...
bool ParseContent(const std::string& message,
  ...
  if (!ssrc_infos.empty()) {
    ...
    CreateUnsignaledTrack(stream_ids, track_id, &tracks);
  }
