...
void DefaultAudioQualityAnalyzer::OnStatsReports(
  ...
  for (auto& stat : stats) {
    ...
    sample.total_samples_received =
        GetStatOrDefault(stat->total_samples_received, 0ul);
    sample.concealed_samples = GetStatOrDefault(stat->concealed_samples, 0ul);
    sample.removed_samples_for_acceleration =
        GetStatOrDefault(stat->removed_samples_for_acceleration, 0ul);
    sample.inserted_samples_for_deceleration =
        GetStatOrDefault(stat->inserted_samples_for_deceleration, 0ul);
    sample.silent_concealed_samples =
        GetStatOrDefault(stat->silent_concealed_samples, 0ul);
    sample.jitter_buffer_target_delay = TimeDelta::Seconds(
        GetStatOrDefault(stat->jitter_buffer_target_delay, 0.));
    sample.jitter_buffer_emitted_count =
        GetStatOrDefault(stat->jitter_buffer_emitted_count, 0ul);
