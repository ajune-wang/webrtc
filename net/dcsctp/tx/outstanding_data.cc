...
bool OutstandingData::IsConsistent() const {
  ...
  for (UnwrappedTSN tsn = last_cumulative_tsn_ack_.next_value();
       tsn <= last_outstanding_tsn_; tsn = tsn.next_value()) {
...
void OutstandingData::RemoveAcked(UnwrappedTSN cumulative_tsn_ack,
                                  AckInfo& ack_info) {
  for (UnwrappedTSN tsn = last_cumulative_tsn_ack_.next_value();
       tsn <= std::min(cumulative_tsn_ack, last_outstanding_tsn_);
       tsn = tsn.next_value()) {
...
void OutstandingData::AckGapBlocks(
  ...
  for (auto& block : gap_ack_blocks) {
    ...
    for (UnwrappedTSN tsn = start; tsn <= end; tsn = tsn.next_value()) {
      if (tsn > last_cumulative_tsn_ack_ && tsn <= last_outstanding_tsn_) {
        Item& item = outstanding_data_[Index(tsn)];
        AckChunk(ack_info, tsn, item);
      }
    }
...
void OutstandingData::EnsureEnoughCapacity() {
  size_t current_size =
      UnwrappedTSN::Difference(last_outstanding_tsn_, last_cumulative_tsn_ack_);
...
bool OutstandingData::ShouldSendForwardTsn() const {
  if (last_outstanding_tsn_ == last_cumulative_tsn_ack_) {
    // No outstanding data
    return false;
  }

  return outstanding_data_[Index(last_cumulative_tsn_ack_.next_value())]
      .is_abandoned();
}
...
ForwardTsnChunk OutstandingData::CreateForwardTsn() const {
  ...
  for (UnwrappedTSN tsn = new_cumulative_ack.next_value();
       tsn <= last_outstanding_tsn_; tsn = tsn.next_value()) {
    const Item& item = outstanding_data_[Index(tsn)];
    if (!item.is_abandoned()) {
      break;
