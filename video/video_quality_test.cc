...
// An encoder wrapper hooks into the VideoAnalyzer, and in addition can writes
// the encoded frames to file, one per simulcast layer.
...
std::unique_ptr<VideoEncoder> VideoQualityTest::CreateVideoEncoder(
  ...
  if (!params_.logging.encoded_frame_base_path.empty()) {
    ...
    encoder = absl::make_unique<QualityTestVideoEncoder>(
        std::move(encoder), analyzer_.get(),
    ...
    encoder = absl::make_unique<QualityTestVideoEncoder>(
        std::move(encoder), analyzer_.get(), std::vector<rtc::PlatformFile>());
  }
