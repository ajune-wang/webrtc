...
int32_t H264DecoderImpl::Decode(const EncodedImage& input_image,
  ...
  RTC_DCHECK_GE(av_frame_->data[kYPlaneIndex], planar_yuv_buffer->DataY());
  RTC_DCHECK_LE(av_frame_->data[kYPlaneIndex] +
                    av_frame_->linesize[kYPlaneIndex] * av_frame_->height,
                planar_yuv_buffer->DataY() + planar_yuv_buffer->StrideY() *
                                                  ,
  planar_yuv_buffer->height()); RTC_DCHECK_GE(av_frame_->data[kUPlaneIndex], ,
  planar_yuv_buffer->DataU()); RTC_DCHECK_LE(av_frame_->data[kUPlaneIndex] +
                    av_frame_->linesize[kUPlaneIndex] * av_frame_->height / 2,
                planar_yuv_buffer->DataU() + planar_yuv_buffer->StrideU() *
                                                  planar_yuv_buffer->height()
  / 2); RTC_DCHECK_GE(av_frame_->data[kVPlaneIndex], ,
  planar_yuv_buffer->DataV()); RTC_DCHECK_LE(av_frame_->data[kVPlaneIndex] +
                    av_frame_->linesize[kVPlaneIndex] * av_frame_->height / 2,
                planar_yuv_buffer->DataV() + planar_yuv_buffer->StrideV() *
                                                  planar_yuv_buffer->height()
  / 2);*/
  ...
  if (preferred_output_format_ == VideoFrameBuffer::Type::kNV12) {
    ...
    const PlanarYuv8Buffer* cropped_planar_yuv_buffer = nullptr;
    switch (video_frame_buffer_type) {
      case VideoFrameBuffer::Type::kI420:
        cropped_planar_yuv_buffer = cropped_buffer->GetI420();
        libyuv::I420ToNV12(cropped_planar_yuv_buffer->DataY(),
                           ...
                           nv12_buffer->StrideUV(), planar_yuv_buffer->width(),
                           planar_yuv_buffer->height());
        cropped_buffer = nv12_buffer;
        break;
      case VideoFrameBuffer::Type::kI444:
        cropped_planar_yuv_buffer = cropped_buffer->GetI444();
        libyuv::I444ToNV12(cropped_planar_yuv_buffer->DataY(),
                           cropped_planar_yuv_buffer->StrideY(),
                           cropped_planar_yuv_buffer->DataU(),
                           cropped_planar_yuv_buffer->StrideU(),
                           cropped_planar_yuv_buffer->DataV(),
        ...
        break;
      case VideoFrameBuffer::Type::kI422:
        cropped_planar_yuv_buffer = cropped_buffer->GetI422();
        // Swap src_u and src_v to implement I422ToNV12.
        libyuv::I422ToNV21(cropped_planar_yuv_buffer->DataY(),
                           cropped_planar_yuv_buffer->StrideY(),
                           cropped_planar_yuv_buffer->DataV(),
                           cropped_planar_yuv_buffer->StrideV(),
                           cropped_planar_yuv_buffer->DataU(),
