...
const uint8_t kEncodedFrameH264InterSliceQpDelta0[] = {0x00, 0x00, 0x00, 0x01,
                                                       0x41, 0x9a, 0x39, 0xea};
...
TEST(QpParserTest, ParseQpVp8) {
  ...
  absl::optional<uint32_t> qp =
      parser.Parse(VideoCodecType::kVideoCodecVP8, 0, kCodedFrameVp8Qp25,
...
TEST(QpParserTest, ParseQpVp9) {
  QpParser parser;
  absl::optional<uint32_t> qp =
      parser.Parse(VideoCodecType::kVideoCodecVP9, 0, kCodedFrameVp9Qp96,
                   sizeof(kCodedFrameVp9Qp96));
  ASSERT_TRUE(qp.has_value());
  EXPECT_EQ(*qp, 96u);
}
...
TEST(QpParserTest, ParseQpH264) {
  ...
  qp = parser.Parse(VideoCodecType::kVideoCodecH264, 1,
                    kCodedFrameH264SpsPpsIdrQp49,
                    sizeof(kCodedFrameH264SpsPpsIdrQp49));
  ASSERT_TRUE(qp.has_value());
  EXPECT_EQ(*qp, 49u);

  qp = parser.Parse(VideoCodecType::kVideoCodecH264, 0,
