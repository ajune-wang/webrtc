...
using webrtc::SdpContentMutator;
using webrtc::SdpContentPredicate;
using webrtc::SdpContentsAll;
using webrtc::SdpContentsForEach;
using webrtc::SdpContentsNone;
...
class PeerConnectionCryptoUnitTest : public ::testing::Test {
 protected:
  ...
  WrapperPtr CreatePeerConnection(
    ...
    auto* fake_certificate_generator_ptr = fake_certificate_generator.get();
    auto observer = rtc::MakeUnique<webrtc::MockPeerConnectionObserver>();
    auto pc = pc_factory_->CreatePeerConnection(
        config, std::move(fake_port_allocator),
        std::move(fake_certificate_generator), observer.get());
    if (!pc) {
      return nullptr;
    }

    auto wrapper = rtc::MakeUnique<PeerConnectionWrapperForCryptoUnitTest>(
        pc_factory_, pc, std::move(observer));
    wrapper->fake_certificate_generator_ = fake_certificate_generator_ptr;
    return wrapper;
  }
