...
void PacketArrivalTimeMap::AddPacket(int64_t sequence_number,
  ...
  if (sequence_number < begin_sequence_number()) {
    ...
    arrival_times_[Index(sequence_number)] = arrival_time;
    Clear(sequence_number + 1, begin_sequence_number_);
    begin_sequence_number_ = sequence_number;
    return;
  }
  ...
  Clear(end_sequence_number_, sequence_number);
  end_sequence_number_ = new_end_sequence_number;
  arrival_times_[Index(sequence_number)] = arrival_time;
}

void PacketArrivalTimeMap::TrimLeadingEmptyEntries() {
  const int begin_index = Index(begin_sequence_number_);
  const Timestamp* const begin_it = arrival_times_ + begin_index;
  const Timestamp* const end_it = arrival_times_ + capacity();
  ...
  // Reached end of the buffer_ and all entries represent not received packets.
  // Remove them.
  begin_sequence_number_ += (capacity() - begin_index);
  // Continue removing entries at the beginning of the circular buffer.
  for (const Timestamp* it = arrival_times_; it != begin_it; ++it) {
    if (*it >= Timestamp::Zero()) {
      begin_sequence_number_ += (it - arrival_times_);
      return;
    }
  }
...
void PacketArrivalTimeMap::Clear(int64_t begin_sn_inclusive,
                                 int64_t end_sn_exclusive) {
  static constexpr Timestamp value = Timestamp::MinusInfinity();

  int begin_index = Index(begin_sn_inclusive);
  int end_index = Index(end_sn_exclusive);

  if (begin_index <= end_index) {
    // Entries to clear are in single block:
    // [......{-----}....]
    std::fill(arrival_times_ + begin_index, arrival_times_ + end_index, value);
  } else {
    // Entries to clear span across arrival_times_ border:
    // [--}..........{---]
    std::fill(arrival_times_ + begin_index, arrival_times_ + capacity(), value);
    std::fill(arrival_times_, arrival_times_ + end_index, value);
  }
}
...
PacketArrivalTimeMap::~PacketArrivalTimeMap() {
  delete[] arrival_times_;
}
