...
int AudioFrameOperations::StereoToMono(AudioFrame* frame) {
  ...
  return DownmixChannels(1, frame);
}
...
/*
   Goes from 4 to 2 or from X to 1. Nothing else is supported. Returns a check
   if it worked.
*/
...
/* Signal error when:
   - WRONG num_channels
   - stuff doesn't fit in array

   Fkn - called by MonoToStereo (which is deprecated)
       - called by RemixAndResample (and the result is ignored.)
       - called by the FrameCombiner (result is DCHECKed).

   Make sure it doesn't buffer-overflow; check what Oskar writes (that I
   shouldn't have a !debug-only test; should fall back on a death test), remove
   the flag (people don't want flags, and don't care about checking the result).
