...
- (UIImage *)rgbdSegmentUIImage:(UIImage *)color
                     depthImage:(UIImage *)depth
                          index:(int)index
...
// @param snapshotPath: the path for the snapshot.
// @returns the segmented color RGB image.
- (UIImage *)rgbdSegmentPixelBuffer:(CVPixelBufferRef)color
                        depthBuffer:(CVPixelBufferRef)depth
                              index:(int)index
                       snapshotPath:(NSString *)path;

- (void)rgbdSegmentPixelBuffers:(CVPixelBufferRef)color
                    depthBuffer:(CVPixelBufferRef)depth
                          index:(int)index
                   snapshotPath:(NSString *)path;

- (void)rgbdSegmentAlpha:(CVPixelBufferRef)color
             depthBuffer:(CVPixelBufferRef)depth
