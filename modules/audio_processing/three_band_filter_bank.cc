...
constexpr int kSubSampling = kNumBands;
constexpr int kDctSize = ThreeBandFilterBank::kSplitBandSize;
constexpr int kMinShiftStart = -(kStride - 1);
static_assert(kNumBands * ThreeBandFilterBank::kSplitBandSize ==
...
void FilterCore(
  ...
  RTC_DCHECK_GE(shift_start, kMinShiftStart);
  RTC_DCHECK_GE(0, shift_start);
  ...
  for (; k < ThreeBandFilterBank::kSplitBandSize; ++k, ++shift) {
    for (int i = 0, j = shift; i < kFilterSize; ++i, j -= kStride) {
      out[k] += in[j] * filter[i];
    }
  }
...
ThreeBandFilterBank::ThreeBandFilterBank(size_t length) {
  RTC_DCHECK_EQ(kNumNonZeroFilters, state_analysis_.size());
  RTC_DCHECK_EQ(kNumNonZeroFilters, state_synthesis_.size());
...
//      |kSparsity| signals with different delays.a
