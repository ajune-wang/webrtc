...
TEST(NetworkEmulationManagerTest, Run) {
  ...
  nt1->GetStats([&](rtc::scoped_refptr<EmulatedNetworkStats> st) {
    ...
    EXPECT_EQ(st->IncomingStatsPerSource()[bob_endpoint->GetPeerLocalAddress()]
                  .packets_received,
              2000l);
    EXPECT_EQ(st->IncomingStatsPerSource()[bob_endpoint->GetPeerLocalAddress()]
  ...
  nt2->GetStats([&](rtc::scoped_refptr<EmulatedNetworkStats> st) {
    EXPECT_EQ(st->PacketsSent(), 2000l);
    EXPECT_EQ(st->BytesSent().bytes(), single_packet_size * 2000l);
    EXPECT_EQ(st->PacketsReceived(), 2000l);
    EXPECT_EQ(st->BytesReceived().bytes(), single_packet_size * 2000l);
    ...
    EXPECT_EQ(
        st->IncomingStatsPerSource()[alice_endpoint->GetPeerLocalAddress()]
            .packets_received,
        2000l);
    EXPECT_EQ(
        st->IncomingStatsPerSource()[alice_endpoint->GetPeerLocalAddress()]
