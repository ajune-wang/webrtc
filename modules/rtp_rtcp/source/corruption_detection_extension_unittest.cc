...
TEST(CorruptionDetectionExtensionTest, ValueSizeIs1UnlessSamplesAreSpecified) {
  ...
  EXPECT_EQ(CorruptionDetectionExtension::ValueSize(*kMessage), 1);
}
...
TEST(CorruptionDetectionExtensionTest,
  ...
  ASSERT_NE(kMessage, absl::nullopt);
  EXPECT_EQ(CorruptionDetectionExtension::ValueSize(*kMessage), 7);
}
...
TEST(CorruptionDetectionExtensionTest,
  ...
  for (int i = 0; i < sizeof(kSampleValues); ++i) {
