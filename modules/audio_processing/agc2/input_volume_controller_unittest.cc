 ...
 public:
  ...
  InputVolumeControllerTestHelper(const InputVolumeController::Config& config =
      ...
      : audio_buffer(kSampleRateHz,
                     ...
                     kSampleRateHz,
                     kNumChannels),
        manager(/*num_capture_channels=*/1, config) {
    manager.Initialize();
  ...
  int CallAgcSequence(int applied_input_volume,
    ...
    for (int i = 0; i < num_calls; ++i) {
      ...
      manager.AnalyzePreProcess(audio_buffer);
      manager.Process(speech_probability, speech_level);
      volume = manager.recommended_analog_level();
    }
    return volume;
  }
...
TEST_P(InputVolumeControllerParametrizedTest,
  ...
  helper.manager.Initialize();

  EXPECT_FALSE(helper.manager.clipping_predictor_enabled());
  EXPECT_FALSE(helper.manager.use_clipping_predictor_step());
...
TEST_P(InputVolumeControllerParametrizedTest,
  ...
  helper_1.manager.Initialize();
  helper_2.manager.Initialize();

  EXPECT_TRUE(helper_1.manager.clipping_predictor_enabled());
  EXPECT_FALSE(helper_2.manager.clipping_predictor_enabled());
  EXPECT_TRUE(helper_1.manager.use_clipping_predictor_step());
