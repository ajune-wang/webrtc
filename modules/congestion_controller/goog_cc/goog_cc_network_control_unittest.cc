...
TEST(GoogCcScenario, FallbackToLossBasedBweV0IfNoPacketFeedback) {
  ScopedFieldTrials trial("WebRTC-Bwe-LossBasedBweV2/Enabled/");
  const DataRate kLinkCapacity = DataRate::KilobitsPerSec(1000);
  const DataRate kStartRate = DataRate::KilobitsPerSec(1000);
  ...
  // LossBasedBweV2 is not ready, but bandwidth still decreases thanks to loss based bwe v0.
