...
inline int64_t DivideAndRound(int64_t dividend, int64_t divisor) {
  return (dividend + divisor / 2) / divisor;
}
}  // namespace units_internal
namespace units_constants {
...
class TimeDelta {
 public:
  ...
  static TimeDelta ms(int64_t millis) { return TimeDelta(millis * 1000); }
...
class Timestamp {
 public:
  ...
  bool IsInfinite() const {
    return *this == kPlusInfinity && *this != kNotInitialized;
  }
  bool IsFinite() const { return microseconds_ != kPlusInfinity.microseconds_; }
  ...
  bool IsInitialized() const { return *this != kNotInitialized; }
...
class DataSize {
 public:
  ...
  static DataSize bits(int64_t bits) {
    return DataSize(bits / units_constants::kBitsPerByte);
  }
  ...
  int64_t bits() const { return bytes() * units_constants::kBitsPerByte; }
