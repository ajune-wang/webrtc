...
RTCError PeerConnection::UpdateTransceiversAndDataChannels(
  ...
  RTC_DCHECK(IsUnifiedPlan());
  SdpType type = new_session.GetType();
  const ContentInfos& old_contents =
      (old_session ? old_session->description()->contents() : ContentInfos());
  const ContentInfos& contents = new_session.description()->contents();
  for (size_t i = 0; i < contents.size(); ++i) {
    ...
    used_mids_.insert(content.name);
    if (media_type == cricket::MEDIA_TYPE_AUDIO ||
        media_type == cricket::MEDIA_TYPE_VIDEO) {
      auto transceiver_or_error =
          AssociateTransceiver(source, i, content, old_content);
      ...
      if (source == cricket::CS_REMOTE && content.rejected &&
          !transceiver->stopped()) {
        transceiver->Stop();
      }
      RTCError error =
          UpdateTransceiverChannel(transceiver, content, bundle_group);
      if (!error.ok()) {
        return error;
      }
