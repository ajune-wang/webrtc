...
CallClient::CallClient(std::string name,
  ...
  if (base_filename.empty()) {
    ...
    if (config.cc.log_interval.IsFinite()) {
      ...
      switch (config.cc.type) {
        case CallClientConfig::CongestionControl::Type::kBbr: {
        ...
        case CallClientConfig::CongestionControl::Type::kGoogCc: {
        ...
        case CallClientConfig::CongestionControl::Type::kGoogCcFeedback: {
          auto goog_printer = absl::make_unique<GoogCcStatePrinter>();
...
void CallClient::LogCongestionControllerStats(Timestamp at_time) {
