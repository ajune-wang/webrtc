...
// position in the original video. We should provide a statistics vector along
// with the test video. The stats vector contains the connection between the
// actual frames in the test file and their corresponding reference frame.
std::vector<AnalysisResult> RunAnalysis(
    ...
    const std::vector<size_t>& test_frame_indicies);
