...
void VideoStreamEncoder::MaybeEncodeVideoFrame(const VideoFrame& video_frame,
  ...
  if (frame_dropping_enabled && frame_dropper_.DropFrame()) {
    ...
    AccumulateUpdateRect(video_frame);
    return;
  }
...
// RTC_RUN_ON(&encoder_queue_)
void VideoStreamEncoder::DropFrame(
    const VideoFrame& frame,
    VideoStreamEncoderObserver::DropReason reason) {
  AccumulateUpdateRect(frame);
  encoder_stats_observer_->OnFrameDropped(reason);
}

// RTC_RUN_ON(&encoder_queue_)
void VideoStreamEncoder::AccumulateUpdateRect(const VideoFrame& frame) {
