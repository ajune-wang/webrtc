...
class ParsedRtcEventLog {
 ...
 public:
  ...
  const PacketView<const LoggedRtcpPacket> rtcp_packets(
      PacketDirection direction) const {
    if (direction == kIncomingPacket) {
      return PacketView<const LoggedRtcpPacket>::Create(
          incoming_rtcp_packets_.data(), incoming_rtcp_packets_.size(),
          offsetof(LoggedRtcpPacketIncoming, rtcp));
    } else {
      return PacketView<const LoggedRtcpPacket>::Create(
          outgoing_rtcp_packets_.data(), outgoing_rtcp_packets_.size(),
          offsetof(LoggedRtcpPacketOutgoing, rtcp));
    }
  }
