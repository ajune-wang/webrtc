...
std::set<std::string> GetKnownCodecNames() {
  return {cricket::kVp8CodecName, cricket::kVp9CodecName,
...
void ForceVideoCodec(
  ...
  for (cricket::ContentInfo& content :
    ...
    //  * known codecs
    //  * retransmission "codecs" for codecs
    //  * FEC "codecs"
    ...
    std::set<std::string> known_codecs = GetKnownCodecNames();
    codecs.erase(
        std::remove_if(
