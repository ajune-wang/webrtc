...
class DegradedCall : public Call, private PacketReceiver {
 ...
 private:
  ...
  class ScopedTransport : public Transport {
   public:
   ...
   private:
    ...
    Transport* transport_ RTC_GUARDED_BY(lock_);
  };

  class TransportAdapter : public Transport {
   public:
    TransportAdapter(FakeNetworkPipeOnTaskQueue* fake_network,
                     Call* call,
                     Clock* clock,
   ...
   private:
    ...
    Call* const call_;
    Clock* const clock_;
    ScopedTransport real_transport_;
  };
  ...
  std::map<AudioSendStream*, std::unique_ptr<TransportAdapter>>
      audio_send_transport_adapters_;
  std::map<VideoSendStream*, std::unique_ptr<TransportAdapter>>
