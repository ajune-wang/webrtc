...
void CalculateYChannel(const uint8_t* y_data,
  ...
  if (width % 2 == 1) {
    const float val = coeff[0] * y_data[width - 1] +
                      coeff[1] * u_data[(width - 1) / 2] +
                      coeff[2] * v_data[(width - 1) / 2] + coeff[3];
    output[width - 1] =
...
void CalculateUVChannel(const uint8_t* y_data,
                        const uint8_t* u_data,
                        const uint8_t* v_data,
                        const std::array<float, 4>& coeff,
                        size_t width,
...
rtc::scoped_refptr<I420BufferInterface> AdjustColors(
  ...
  for (int y = 0; y < frame->height(); ++y) {
    ...
    if (y % 2 == 0) {
      ...
      CalculateUVChannel(
          downscaled_y_row, u_row, v_row, color_matrix[1], frame->ChromaWidth(),
          adjusted_frame->MutableDataU() + adjusted_frame->StrideU() * half_y);
      CalculateUVChannel(
          downscaled_y_row, u_row, v_row, color_matrix[2], frame->ChromaWidth(),
