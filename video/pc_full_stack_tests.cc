...
class SvcVideoQualityAnalyzer : public DefaultVideoQualityAnalyzer {
 public:
  ...
  void OnFramePreDecode(absl::string_view peer_name,
    ...
    for (int i = 0; i <= spatial_id.value_or(0); ++i) {
      if (input_image.SpatialLayerFrameSize(i).has_value()) {
        decoder_layers_seen_[i]++;
        auto temporal_id = input_image.TemporalIndex();
        decoder_layers_seen2_[spatial_id.value_or(0)]
                             [temporal_id.value_or(0)]++;
      }
    }
  ...
  const SpatialTemporalLayerCounts& decoder_layers_seen2() {
    return encoder_layers_seen_;
  }
 ...
 private:
  ...
  SpatialTemporalLayerCounts decoder_layers_seen2_;
};
...
TEST(PCFullStackTest, Pc_Bllaaaaaa) {
  ...
  printf("======= decoder layers seen: %lu\n",
         analyzer_ptr->decoder_layers_seen().size());
  for (const auto& spatial_layer : analyzer_ptr->decoder_layers_seen2()) {
    int spatial_index = spatial_layer.first;
    for (const auto& temporal_layer : spatial_layer.second) {
      int temporal_index = temporal_layer.first;
      printf("======= decoder layer: %d,%d frames: %d\n", spatial_index,
             temporal_index, temporal_layer.second);
    }
  }
}
