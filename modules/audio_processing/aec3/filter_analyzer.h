...
class FilterAnalyzer {
 public:
  ...
  bool ConsistentEstimate() const { return consistent_estimate_; }
  bool RefinedEstimate() const { return refined_estimate_; }

 private:
  void UpdateFilterGain(rtc::ArrayView<const float> filter_time_domain,
                        size_t max_index);

  void DetectRefinedFilter(rtc::ArrayView<const float> filter_time_domain,
                           size_t delay_blocks,
                           bool converged_filter);
