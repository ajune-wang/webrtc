...
class EchoEmulatingCapturer : public TestAudioDeviceModule::Capturer {
 ...
 private:
  ...
  SequenceChecker renderer_input_checker_;
  std::vector<int16_t> queue_input_ RTC_GUARDED_BY(renderer_input_checker_);
  bool recording_started_ RTC_GUARDED_BY(renderer_input_checker_) = false;

  std::vector<int16_t> queue_output_;

  std::atomic<int> adds_to_full_queue{0};
};
