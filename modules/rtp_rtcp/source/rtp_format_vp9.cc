...
bool RtpPacketizerVp9::NextPacket(RtpPacketToSend* packet) {
  ...
  bool first = current_packet_ == payload_sizes_.begin();
  int packet_payload_len = *current_packet_;
  ++current_packet_;
  bool last = current_packet_ == payload_sizes_.end();

  int header_size = header_size_;
  if (first)
    header_size += first_packet_extra_header_size_;

  int rtp_payload_len = header_size + packet_payload_len;
  uint8_t* buffer = packet->AllocatePayload(rtp_payload_len);
  RTC_CHECK(buffer);

  if (!WriteHeader(first, last, rtc::MakeArrayView(buffer, header_size)))
    return false;
  ...
  packet->SetMarker(last && hdr_.end_of_picture);
  return true;
}
...
//      | ..            |
//      +-+-+-+-+-+-+-+-+
bool RtpPacketizerVp9::WriteHeader(bool first,
                                   bool last,
