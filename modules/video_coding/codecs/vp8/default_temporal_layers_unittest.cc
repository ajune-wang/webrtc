...
TEST(TemporalLayersTest, 2Layers) {
  ...
  for (int i = 0; i < 16; ++i) {
    ...
    tl.FrameEncoded(timestamp, 500 / 8 / 30, 2);
...
TEST(TemporalLayersTest, SearchOrder) {
  ...
  tl.FrameEncoded(timestamp, 500 / 8 / 30, 2);
  EXPECT_EQ(tl_config.first_search, Vp8BufferReference::kLast);
  EXPECT_EQ(tl_config.second_search, Vp8BufferReference::kNone);
  ...
  tl.PopulateCodecSpecific(false, tl_config, &vp8_info, timestamp);
  tl.FrameEncoded(timestamp, 500 / 8 / 30, 2);
  EXPECT_EQ(tl_config.first_search, Vp8BufferReference::kGolden);
  EXPECT_EQ(tl_config.second_search, Vp8BufferReference::kAltref);
}
...
TEST(TemporalLayersTest, SearchOrderWithDrop) {
  ...
  tl.PopulateCodecSpecific(false, tl_config, &vp8_info, timestamp);
  tl.FrameEncoded(timestamp, 500 / 8 / 30, 2);
  EXPECT_EQ(tl_config.first_search, Vp8BufferReference::kAltref);
  EXPECT_EQ(tl_config.second_search, Vp8BufferReference::kLast);
}
