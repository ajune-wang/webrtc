...
class SimulatedSequenceRunner : public ProcessThread, public TaskQueueBase {
 public:
  ...
  // Iterates through delayed tasks and modules and adds them to the pending set
  // if they are supposed to execute by |at time|.
  void TimeUpdate(Timestamp at_time);
  // Runs all pending tasks and modules and updates next run time.
...
void SimulatedTimeControllerImpl::RunPendingRunners() {
  RTC_DCHECK_RUN_ON(&thread_checker_);
