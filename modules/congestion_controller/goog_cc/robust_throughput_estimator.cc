...
absl::optional<DataRate> RobustThroughputEstimator::bitrate() const {
  ...
  // between the first and the last packet would only be (N-1)*T.
  // To remove the bias, we subtract half of the first and the last packet
  // from the total size.
  //  data_size -= (window_.front().sent_packet.size +
  //                window_.front().sent_packet.prior_unacked_data) /
  //               2;
  //  data_size -= (window_.back().sent_packet.size +
  //                window_.back().sent_packet.prior_unacked_data) /
  //               2;

  //  TimeDelta send_duration = max_send_time - min_send_time;
  //  TimeDelta recv_duration = (max_recv_time - min_recv_time) -
  //  largest_recv_gap +
  ...
  DataSize recv_size = data_size - window_.front().sent_packet.size -
                       window_.front().sent_packet.prior_unacked_data;
  DataSize send_size = data_size - window_.back().sent_packet.size -
                       window_.back().sent_packet.prior_unacked_data;
  TimeDelta send_duration = max_send_time - min_send_time;
