...
std::string FindFullName(const WebRtcKeyValueConfig* const key_value_config,
                         absl::string_view key) {
  std::string value = key_value_config->Lookup(key);
  if (value.empty()) {
    value = webrtc::field_trial::FindFullName(std::string(key));
  }
  return value;
}
...
VideoRateControlExperiments::VideoRateControlExperiments(
  ...
  if (!congestion_window_) {
    std::string experiment_string =
        FindFullName(key_value_config, kCongestionWindowFieldTrialName);
  ...
  if (!congestion_window_pushback_) {
    std::string experiment_string =
        FindFullName(key_value_config, kCongestionWindowPushbackFieldTrialName);
    // The experiment is enabled iff the field trial string begins with
    // "Enabled".
    if (experiment_string.find("Enabled") == 0) {
      uint32_t min_pushback_target_bitrate_bps;
