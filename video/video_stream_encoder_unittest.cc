...
TEST_P(VideoStreamEncoderWithRealEncoderTest, EncoderMapsNativeI420) {
  auto native_i420_frame = test::CreateMappableNativeFrame(
      1, VideoFrameBuffer::Type::kI420, true, codec_width_, codec_height_);
...
TEST_P(VideoStreamEncoderWithRealEncoderTest, EncoderMapsNativeNV12) {
  auto native_nv12_frame = test::CreateMappableNativeFrame(
      1, VideoFrameBuffer::Type::kNV12, true, codec_width_, codec_height_);
...
constexpr std::pair<VideoCodecType, bool> kVP9AllowConversion =
    std::make_pair(kVideoCodecVP9, /*allow_i420_conversion=*/true);
...
INSTANTIATE_TEST_SUITE_P(
    All,
    VideoStreamEncoderWithRealEncoderTest,
// The windows compiler does not play nicely with placing the #if statement
// inside ::testing::Values(), so we have to list all values twice (with and
// without H264).
