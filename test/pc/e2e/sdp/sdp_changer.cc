...
// mids.
OfferAnswerExchangeSimulcastContext PrepareSimulcastOffer(
    SessionDescriptionInterface* offer) {
  OfferAnswerExchangeSimulcastContext context;
  for (auto& content : offer->description()->contents()) {
    context.mids_order.push_back(content.mid());
...
LocalAndRemoteOfferWithContext PatchOffer(
  ...
  OfferAnswerExchangeSimulcastContext context =
      PrepareSimulcastOffer(offer.get());
  if (context.empty()) {
    auto offer_for_remote = CloneSessionDescription(offer.get());
    return LocalAndRemoteOfferWithContext(
        std::move(offer), std::move(offer_for_remote), std::move(context));
  }
  ...
  return LocalAndRemoteOfferWithContext(
      std::move(offer), std::move(patched_offer), std::move(context));
}
...
std::unique_ptr<cricket::SessionDescription> RestoreMediaSectionsOrder(
  ...
  for (auto& mid : context.mids_order) {
  ...
  for (auto& mid : context.mids_order) {
    ...
    RTC_CHECK(content);
    out->AddContent(mid, content->type, content->media_description()->Copy());
  }
  return out;
}
...
std::unique_ptr<SessionDescriptionInterface> PatchAnswer(
  ...
  desc = RestoreMediaSectionsOrder(std::move(desc), context);
