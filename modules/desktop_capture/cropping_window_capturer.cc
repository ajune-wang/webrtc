...
void CroppingWindowCapturer::OnCaptureResult(
  ...
  if (!ShouldUseScreenCapturer()) {
    NLOG(LS_INFO, "Window no longer on top when ScreenCapturer finishes");
    window_capturer_->CaptureFrame();
    return;
  }

  if (result != Result::SUCCESS) {
    NLOG(LS_WARNING, "ScreenCapturer failed to capture a frame");
  ...
  if (window_rect.is_empty()) {
    NLOG(LS_WARNING, "Window rect is empty");
