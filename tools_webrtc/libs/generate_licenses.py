...
# third_party library to licences mapping
# Following key format are supported:
# - Name of the library (right after the `third_party/` prefix), i.e.: <libname>
# - Name of the library followed by regular expression matching internal targets
#   inside that library, i.e.: `<libname>/<regex>` or `<libname>:<regex>`
...
LIB_DICT_KEY_EXTRACT_REGEX = r'^([\w\-+]+)[:/]'


class LicenseBuilder(object):

  def __init__(self, buildfile_dirs, targets, lib_to_licenses_dict):
    self.buildfile_dirs = buildfile_dirs
    self.targets = targets
    self.lib_to_licenses_dict = lib_to_licenses_dict
    self.libraries_with_regex = \
      LicenseBuilder._GetLibrariesWithRegEx(lib_to_licenses_dict)

  @staticmethod
  def _GetLibrariesWithRegEx(library_to_licenses_dict):
    """Extracts entries from LIB_TO_LICENSES_DICT that have regex in keys

    Looks through LIB_TO_LICENSES_DICT mapping, extracts those entries
    that have additional regular expressions and groups them by library name

    Outputs: Dict[library_name, Set[library_name_with_regex]]
    """
    libs_with_regex = {}
    for lib in library_to_licenses_dict:
      groups = re.search(LIB_DICT_KEY_EXTRACT_REGEX, lib)
      if groups:
        lib_key = groups.group(1)
        libs_with_regex[lib_key] = libs_with_regex.get(lib_key, set()) | {lib}

    return libs_with_regex
  ...
  def _ParseLibraryName(dep):
    ...
    """
    groups = re.match(THIRD_PARTY_LIB_REGEX, dep)
    return groups.group(1) if groups else None

  def _ParseLibrary(self, dep):
    """Returns library name with regex that matches `dep` after third_party

    This method matches `dep` dependency against LIB_TO_LICENSES_DICT keys
    that can also contain regexps.

    Outputs matched dict key or None if this is not a third_party dependency.
    """
    libname = LicenseBuilder._ParseLibraryName(dep)
    if libname and libname in self.libraries_with_regex:
      for lib_regex in self.libraries_with_regex[libname]:
  ...
  def GenerateLicenseText(self, output_dir):
    ...
    missing_licenses = third_party_libs - set(self.lib_to_licenses_dict.keys())
    if missing_licenses:
    ...
    for license_lib in license_libs:
      if len(self.lib_to_licenses_dict[license_lib]) == 0:
      ...
      for path in self.lib_to_licenses_dict[license_lib]:
