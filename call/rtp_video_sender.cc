...
struct RtpVideoSender::RtpStreamSender {
  RtpStreamSender();
  ~RtpStreamSender();
...
RtpVideoSender::RtpStreamSender::RtpStreamSender() = default;
RtpVideoSender::RtpStreamSender::~RtpStreamSender() = default;
...
std::vector<std::unique_ptr<RtpVideoSender::RtpStreamSender>>
CreateRtpStreamSenders(const RtpConfig& rtp_config,
  ...
  std::vector<std::unique_ptr<RtpVideoSender::RtpStreamSender>> rtp_streams;
  ...
  for (uint32_t ssrc : rtp_config.ssrcs) {
    auto stream = absl::make_unique<RtpVideoSender::RtpStreamSender>();
    ...
    configuration.ack_observer = &stream->playout_delay_oracle;
    stream->rtp_rtcp = absl::WrapUnique(RtpRtcp::CreateRtpRtcp(configuration));
    stream->rtp_rtcp->SetSendingStatus(false);
    stream->rtp_rtcp->SetSendingMediaStatus(false);
    stream->rtp_rtcp->SetRTCPStatus(RtcpMode::kCompound);

    stream->sender_video = absl::make_unique<RTPSenderVideo>(
        configuration.clock, stream->rtp_rtcp->RtpSender(), flexfec_sender,
        &stream->playout_delay_oracle, frame_encryptor,
        crypto_options.sframe.require_frame_encryption,
        FieldTrialBasedConfig());
    rtp_streams.push_back(std::move(stream));
  }
  return rtp_streams;
}
...
RtpVideoSender::RtpVideoSender(
  ...
  for (const std::unique_ptr<RtpStreamSender>& stream : rtp_streams_) {
...
void RtpVideoSender::ConfigureProtection(const RtpConfig& rtp_config) {
  ...
  for (const std::unique_ptr<RtpStreamSender>& stream : rtp_streams_) {
    // Set NACK.
    stream->rtp_rtcp->SetStorePacketsStatus(true,
                                            kMinSendSidePacketHistorySize);
    // Set RED/ULPFEC information.
    stream->sender_video->SetUlpfecConfig(red_payload_type,
                                          ulpfec_payload_type);
  }
}
