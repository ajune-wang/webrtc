...
TEST_F(TestVp9Impl, InsufficientBwForConfiguredNumSpatialLayers) {
  // Configure encoder to produce N spatial layers. Allocate zero bit rate
  // To the Nth spatial layer and encode several frames.
  const size_t num_spatial_layers = 3;
  ...
  VideoBitrateAllocation bitrate_allocation;
  // Allocate high bit rate to N-1 lower spatial layers to avoid frame dropping
  // due to rate control. Top-most layer has 0 bit rate.
