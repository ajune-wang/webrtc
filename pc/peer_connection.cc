...
class PeerConnection::LocalIceCredentialsToReplace {
 public:
  ...
  void ClearIceCredentials() { ice_cedentials_.clear(); }

  // Returns true if we have ICE credentials that need restarting.
  bool HasIceCredentials() const { return !ice_cedentials_.empty(); }
  ...
  bool SatisfiesIceRestart(
    ...
    for (const auto& transport_info :
      ...
      if (ice_cedentials_.find(std::make_pair(
              transport_info.description.ice_ufrag,
              transport_info.description.ice_pwd)) != ice_cedentials_.end()) {
        return false;
      }
    }
    return true;
