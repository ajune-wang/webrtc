...
int32_t MultiplexDecoderAdapter::Decode(
  ...
  if (supports_augmenting_data_) {
    ...
    decoded_augmenting_data_.emplace(input_image._timeStamp,
                                     std::move(image.augmenting_data));
    decoded_augmenting_data_length_.emplace(input_image._timeStamp,
...
void MultiplexDecoderAdapter::Decoded(AlphaCodecStream stream_idx,
  ...
  const auto& augmenting_data_length_it =
      decoded_augmenting_data_length_.find(decoded_image->timestamp());
  if (other_decoded_data_it != decoded_data_.end()) {
    uint16_t augmenting_data_size =
        augmenting_data_length_it == decoded_augmenting_data_length_.end()
            ? 0
            : augmenting_data_length_it->second;
    std::unique_ptr<uint8_t[]> augmenting_data =
        augmenting_data_size == 0 ? NULL
                                  : std::move(augmenting_data_it->second);
    ...
    if (supports_augmenting_data_) {
      decoded_augmenting_data_.erase(decoded_augmenting_data_.begin(),
                                     augmenting_data_it);
      decoded_augmenting_data_length_.erase(
          decoded_augmenting_data_length_.begin(), augmenting_data_length_it);
    }
    return;
  }
