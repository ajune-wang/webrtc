...
void ChannelReceive::InitFrameTransformerDelegate(
  ...
  ChannelReceiveFrameTransformerDelegate::ReceiveFrameCallback
      receive_audio_callback = [this](const uint8_t* packet,
                                      size_t packet_length,
                                      const RTPHeader& header) {
        ReceivePacket(packet, packet_length, header);
      };
  frame_transformer_delegate_ =
...
void ChannelReceive::OnRtpPacket(const RtpPacketReceived& packet) {
  ...
  if (frame_transformer_delegate_) {
    ...
    frame_transformer_delegate_->Transform(
        packet_copy.data(), packet_copy.size(), header, remote_ssrc_);
  } else {
...
void ChannelReceive::SetDepacketizerToDecoderFrameTransformer(
    rtc::scoped_refptr<webrtc::FrameTransformerInterface> frame_transformer) {
  RTC_DCHECK(worker_thread_checker_.IsCurrent());
  // Depending on when the channel is create, the transformer might be set
  // twice. Don't overrider the delegate if it was already initialized.
