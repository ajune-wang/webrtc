...
static constexpr fuchsia::sysmem::PixelFormatType kI420PixelFormatType =
...
void ScreenCapturerFuchsia::CaptureFrame() {
  ...
  // TODO(bugs.webrtc.org/14097): Use SharedMemoryDesktopFrame
  std::unique_ptr<BasicDesktopFrame> frame(
      new BasicDesktopFrame(DesktopSize(width_, height_)));

  status = buffer_collection_info_.buffers[buffer_index].vmo.op_range(
  ...
  MapHostPointer(buffer_collection_info_, buffer_index, frame);
...
ScreenCapturerFuchsia::GetBufferConstraints() {
  ...
  image_constraints.pixel_format.type = kI420PixelFormatType;
...
uint32_t ScreenCapturerFuchsia::GetPixelsPerRow(
  ...
  uint32_t stride = RoundUpToMultiple(
      std::max(constraints.min_bytes_per_row, width_ * kBytesPerPixel),
...
void ScreenCapturerFuchsia::MapHostPointer(
    ...
    uint32_t vmo_index,
    std::unique_ptr<BasicDesktopFrame>& frame) {
  RTC_DCHECK(vmo_index < collection_info.buffer_count);

  const zx::vmo& vmo = collection_info.buffers[vmo_index].vmo;
  ...
  auto status = zx::vmar::root_self()->map(
      ZX_VM_PERM_READ, /*vmar_offset*/ 0, vmo,
  ...
  uint32_t stride = kBytesPerPixel * pixels_per_row;
  frame->CopyPixelsFrom(vmo_host, stride, DesktopRect::MakeWH(width_, height_));
  fuchsia::ui::composition::ScreenCapture_ReleaseFrame_Result release_result;
  screen_capture_->ReleaseFrame(vmo_index, &release_result);
