...
class VCMTimingFake : public VCMTiming {
 public:
  ...
  Timestamp RenderTimeMs(uint32_t frame_timestamp,
  ...
  TimeDelta MaxWaitingTime(Timestamp render_time,
    ...
    return render_time - now_ms - kDecodeTime;
  }
  ...
  TimeDelta GetCurrentJitter() {
    ...
    TimeDelta current_delay_ms = TimeDelta::Zero();
    TimeDelta target_delay_ms = TimeDelta::Zero();
    TimeDelta jitter_buffer_ms = TimeDelta::Zero();
    TimeDelta min_playout_delay_ms = TimeDelta::Zero();
