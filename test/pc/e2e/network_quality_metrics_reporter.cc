...
void NetworkQualityMetricsReporter::Start(
  ...
  rtc::scoped_refptr<EmulatedNetworkStats> alice_stats =
      PopulateStats(alice_network_);
  RTC_CHECK_EQ(alice_stats->PacketsSent(), 0);
  RTC_CHECK_EQ(alice_stats->PacketsReceived(), 0);
  rtc::scoped_refptr<EmulatedNetworkStats> bob_stats =
      PopulateStats(bob_network_);
...
rtc::scoped_refptr<EmulatedNetworkStats>
NetworkQualityMetricsReporter::PopulateStats(
    EmulatedNetworkManagerInterface* network) {
  rtc::Event wait;
  rtc::scoped_refptr<EmulatedNetworkStats> stats;
  network->GetStats([&](rtc::scoped_refptr<EmulatedNetworkStats> s) {
    stats = s;
    wait.Set();
  });
