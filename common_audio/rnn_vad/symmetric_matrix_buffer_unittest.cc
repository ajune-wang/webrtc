...
// using rnn_vad::kSpectralCoeffsHistorySize;
...
// Checks that the symmetric matrix buffer does not contain any pair which a
// value equal to the given one.
template <size_t S>
...
TEST(RnnVadTest, TestSymmetricMatrixBufferUseCase) {
  // Instance a ring buffer which will be fed with a series of integer values.
  constexpr int ring_buf_size = 10;
  RingBuffer<int, 1, static_cast<size_t>(ring_buf_size)> ring_buf;
  ...
  SymmetricMatrixBuffer<PairType, ring_buf_size> sym_matrix_buf;
  for (int t = 1; t <= 100; ++t) {  // Evolution steps.
    SCOPED_TRACE(t);
    const int t_removed = ring_buf.GetArrayView(ring_buf_size - 1)[0];
    ring_buf.Push({&t, 1});
    ...
    // buffer.
    std::array<PairType, ring_buf_size - 1> new_comparions;
    ...
    for (size_t delay1 = 0; delay1 < ring_buf_size - 1; ++delay1) {
      for (size_t delay2 = delay1 + 1; delay2 < ring_buf_size; ++delay2) {
    ...
    for (size_t delay = 1; delay < ring_buf_size; ++delay) {
      const auto t_prev = ring_buf.GetArrayView(delay)[0];
