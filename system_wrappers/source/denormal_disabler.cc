...
#else
DenormalDisabler::DenormalDisabler(bool enabled)
    : enabled_(false), status_word_(kUnspecifiedStatusWord) {}
#endif
...
void DenormalDisabler::EnableDenormals() {
#if defined(WEBRTC_DENORMAL_DISABLER_SUPPORTED)
  const int status_word = ReadStatusWord();
  if (!DenormalsEnabled(status_word)) {
    RTC_DCHECK_NE(status_word, kUnspecifiedStatusWord);
    SetStatusWord(status_word & ~kFtzControlBits);
  }
#endif
}
