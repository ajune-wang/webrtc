...
TEST_F(VideoStreamEncoderTest,
  ...
  const uint32_t defaultMinBitrateKbps =
      bitrate_allocator_factory_.codec_config().minBitrate;
  const uint32_t defaultMaxBitrateKbps =
      bitrate_allocator_factory_.codec_config().maxBitrate;
  const uint32_t encMinBitrateKbps = defaultMinBitrateKbps * 2;
  const uint32_t encMaxBitrateKbps = defaultMaxBitrateKbps * 2;
  const uint32_t appMinBitrateKbps = defaultMinBitrateKbps * 3;
  const uint32_t appMaxBitrateKbps = defaultMaxBitrateKbps * 3;

  const VideoEncoder::ResolutionBitrateLimits encoder_bitrate_limits(
      codec_width_ * codec_height_, encMinBitrateKbps * 1000,
      encMinBitrateKbps * 1000, encMaxBitrateKbps * 1000);
  ...
  // should be used.
  EXPECT_EQ(encMaxBitrateKbps,
            bitrate_allocator_factory_.codec_config().maxBitrate);
  EXPECT_EQ(encMinBitrateKbps,
            bitrate_allocator_factory_.codec_config().minBitrate);

  video_encoder_config.max_bitrate_bps = appMaxBitrateKbps * 1000;
  video_encoder_config.simulcast_layers[0].min_bitrate_bps = 0;
  video_stream_encoder_->ConfigureEncoder(video_encoder_config.Copy(),
                                          kMaxPayloadLength);
  ...
  // not be applied.
  EXPECT_EQ(appMaxBitrateKbps,
            bitrate_allocator_factory_.codec_config().maxBitrate);
  EXPECT_EQ(defaultMinBitrateKbps,
            bitrate_allocator_factory_.codec_config().minBitrate);

  video_encoder_config.max_bitrate_bps = 0;
  video_encoder_config.simulcast_layers[0].min_bitrate_bps =
      appMinBitrateKbps * 1000;
  video_stream_encoder_->ConfigureEncoder(video_encoder_config.Copy(),
                                          kMaxPayloadLength);
  ...
  // not be applied.
  EXPECT_EQ(defaultMaxBitrateKbps,
            bitrate_allocator_factory_.codec_config().maxBitrate);
  EXPECT_EQ(appMinBitrateKbps,
            bitrate_allocator_factory_.codec_config().minBitrate);
  ...
  video_stream_encoder_->ConfigureEncoder(std::move(video_encoder_config),
                                          kMaxPayloadLength);
  video_source_.IncomingCapturedFrame(CreateFrame(7, nullptr));
...
TEST_F(VideoStreamEncoderTest, EncoderRecommendedMaxBitrateCapsTargetBitrate) {
  ...
  const VideoEncoder::ResolutionBitrateLimits encoder_bitrate_limits(
      ...
      defaultTargetBitrateFor720pKbps / 2 * 1000);
  fake_encoder_.SetResolutionBitrateLimits({encoder_bitrate_limits});
