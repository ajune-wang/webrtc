...
void VideoReceiveStream2::OnFrame(const VideoFrame& video_frame) {
  ...
  if (pending_resolution_.has_value() && pending_resolution_->empty()) {
    pending_resolution_->width = video_frame.width();
    pending_resolution_->height = video_frame.height();
  }
}
...
int VideoReceiveStream2::DecodeAndMaybeDispatchEncodedFrame(
  ...
  if (encoded_frame_output_enabled) {
    ...
    // error.
    if (buffered_encoded_frames_.size() == kBufferedEncodedFramesMaxSize - 1)
      RTC_LOG(LS_ERROR) << "About to halt encoded frame output due to too many "
                           "buffered frames. Is the decoder stuck?";
  ...
  RecordableEncodedFrame::EncodedResolution pending_resolution;
  {
    webrtc::MutexLock lock(&pending_resolution_mutex_);
    // Bail out if OnFrame hasn't filled the necessary resolution yet.
    if (pending_resolution_.has_value()) {
      pending_resolution = *pending_resolution_;
      if (pending_resolution.empty()) {
        return decode_result;
      }
    }
  }
