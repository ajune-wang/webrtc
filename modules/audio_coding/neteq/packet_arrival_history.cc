...
void PacketArrivalHistory::Insert(uint32_t rtp_timestamp,
  ...
  if (!newest_timestamp_ || unwrapped_rtp_timestamp > *newest_timestamp_) {
    newest_timestamp_ = unwrapped_rtp_timestamp;
  }
...
bool PacketArrivalHistory::IsNewestTimestamp(uint32_t rtp_timestamp) const {
  if (!newest_timestamp_) {
    return true;
  }
  int64_t unwrapped_rtp_timestamp =
      timestamp_unwrapper_.UnwrapWithoutUpdate(rtp_timestamp);
  return unwrapped_rtp_timestamp >= *newest_timestamp_;
}
