...
int SimulcastEncoderAdapter::InitEncode(const VideoCodec* inst,
  ...
  const auto lowest_resolution_stream =
      std::distance(std::begin(codec_.simulcastStream), minmax.first);
  const auto highest_resolution_stream =
      std::distance(std::begin(codec_.simulcastStream), minmax.second);

  RTC_DCHECK_LE(lowest_resolution_stream, number_of_streams);
  RTC_DCHECK_LE(highest_resolution_stream, number_of_streams);
  ...
  for (int i = 0; i < number_of_streams; ++i) {
    ...
    if (!doing_simulcast) {
      ...
      StreamResolution stream_resolution = i == highest_resolution_stream
                                               ? StreamResolution::HIGHEST
                                               : i == lowest_resolution_stream
                                                     ? StreamResolution::LOWEST
