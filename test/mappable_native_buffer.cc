...
class NV12BufferWithoutToI420Support : public NV12Buffer {
 public:
  ...
  rtc::scoped_refptr<I420BufferInterface> ToI420() override {
    RTC_NOTREACHED()
        << "NV12 -> I420 is not allowed because the MappableNativeBuffer was "
        << "configured for zero-conversion.";
    return nullptr;
  }
};
      ...
      VideoFrame::Builder()
          .set_video_frame_buffer(
              new rtc::RefCountedObject<MappableNativeBuffer>(
                  mappable_type, allow_i420_conversion, width, height))
...
MappableNativeBuffer::MappableNativeBuffer(VideoFrameBuffer::Type mappable_type,
    ...
    : mappable_type_(mappable_type),
      allow_i420_conversion_(allow_i420_conversion),
      width_(width),
      height_(height) {
  ...
  switch (mappable_type_) {
    ...
    case VideoFrameBuffer::Type::kNV12: {
      ...
      if (allow_i420_conversion_) {
        nv12_buffer = new rtc::RefCountedObject<NV12Buffer>(width, height);
      } else {
        nv12_buffer = new rtc::RefCountedObject<NV12BufferWithoutToI420Support>(
            width, height);
      }
