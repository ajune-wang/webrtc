...
class BitrateAllocator {
 ...
 private:
  ...
  virtual ObserverAllocation NormalRateAllocation(uint32_t bitrate,
...
// Bitrate allocator specifically for allocating bps to tracks based upon the
// relative bitrate priority for each track from RTCRtpEncodingParameters.
class PriorityBitrateAllocator : public BitrateAllocator {
 public:
  explicit PriorityBitrateAllocator(LimitObserver* limit_observer);
  ~PriorityBitrateAllocator();

  // Allocates the bitrate based upon the relative bitrate of each track. This
 ...
 private:
  // Given a target bitrate, allocate bps to each observer. The amount
  // allocated to each observer:
  // = min_bitrate_bps + (target_bitrate * relative_bitrate)
