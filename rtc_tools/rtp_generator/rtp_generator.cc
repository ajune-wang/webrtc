...
ParseVideoSendStreamConfig(const Json::Value& json) {
  ...
  config.rtp.ssrcs.push_back(1337);
...
void RtpGenerator::GenerateReceiverConfig(
    const std::string& receiver_config_path) {}
...
int64_t RtpGenerator::GetMaxDuration() const {
  int64_t max_end_ms = 0;
  for (size_t i = 0; i < options_.video_streams.size(); ++i) {
    const int64_t end_ms = options_.video_streams[i].duration_ms;
