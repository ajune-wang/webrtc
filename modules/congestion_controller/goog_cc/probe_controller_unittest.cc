...
TEST_F(ProbeControllerTest, InitiatesProbingAtStart) {
  ...
  EXPECT_GE(probe_controller_->PopPendingProbes().size(), 2u);
}
...
TEST_F(ProbeControllerTest, ProbeOnlyWhenNetworkIsUp) {
  ...
  probe_controller_->SetBitrates(kMinBitrateBps, kStartBitrateBps,
                                 kMaxBitrateBps, NowMs());
  EXPECT_EQ(probe_controller_->PopPendingProbes().size(), 0u);
...
TEST_F(ProbeControllerTest, InitiatesProbingOnMaxBitrateIncrease) {
  ...
  EXPECT_EQ(probe_controller_->PopPendingProbes()[0].target_data_rate.bps(),
            kMaxBitrateBps + 100);
}
...
TEST_F(ProbeControllerTest, RequestProbeWhenAlrEndedRecently) {
  probe_controller_->SetBitrates(kMinBitrateBps, kStartBitrateBps,
                                 kMaxBitrateBps, NowMs());
  probe_controller_->SetEstimatedBitrate(500, NowMs());
  EXPECT_EQ(probe_controller_->PopPendingProbes().size(), 2u);
...
TEST_F(ProbeControllerTest, PeriodicProbing) {
  ...
  probe_controller_->Process(NowMs());
  probe_controller_->SetEstimatedBitrate(500, NowMs());
  EXPECT_EQ(probe_controller_->PopPendingProbes().size(), 1u);
}
...
TEST_F(ProbeControllerTest, TestExponentialProbingOverflow) {
  ...
  EXPECT_EQ(probe_controller_->PopPendingProbes()[2].target_data_rate.bps(),
            100 * kMbpsMultiplier);
