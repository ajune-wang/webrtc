...
void ScreenCastPortal::Start(PortalCallback* callback) {
...
void ScreenCastPortal::PortalFailed(RequestResponse result) {
  callback_->OnScreenCastRequestResult(result, pw_stream_node_id_, pw_fd_);
}
...
void ScreenCastPortal::OnProxyRequested(GObject* /*object*/,
  ...
  if (!proxy) {
    ...
    RTC_LOG(LS_ERROR) << "Failed to create a proxy for the screen cast portal: "
                      << error->message;
    that->PortalFailed(RequestResponse::ERROR);
    return;
  }
  that->proxy_ = proxy;
...
// static
char* ScreenCastPortal::PrepareSignalHandle(GDBusConnection* connection,
                                            const char* token) {
  Scoped<char> sender(
...
void ScreenCastPortal::OnOpenPipeWireRemoteRequested(GDBusProxy* proxy,
  ...
  that->callback_->OnScreenCastRequestResult(
      ScreenCastPortal::RequestResponse::SUCCESS, that->pw_stream_node_id_,
      that->pw_fd_);
}
