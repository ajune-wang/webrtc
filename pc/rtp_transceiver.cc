...
void RtpTransceiver::SetChannel(cricket::ChannelInterface* channel) {
  ...
  // TODO(tommi): make this slightly prettier.
  cricket::ChannelInterface* channel_to_delete = channel_;
  ...
  if (channel_to_delete) {
    if (media_type_ == cricket::MEDIA_TYPE_AUDIO) {
      // TODO(tommi): There are tests that set their own channel pointers...
      // This check shouldn't be needed and ownership should be completely with
      // the transceiver.
      if (voice_channel_.get() == channel_to_delete) {
        RTC_DCHECK_EQ(voice_channel_.get(), channel_to_delete);
        ...
        if (!network->IsCurrent()) {
          ...
          network->PostTask(RTC_FROM_HERE,
                            [ch = std::move(voice_channel_), &event]() mutable {
