...
void RtpPacketizerH264::NextAggregatePacket(RtpPacketToSend* rtp_packet) {
  ...
  // STAP-A NALU header.
  buffer[0] = (packet->header & (static_cast<uint8_t>(NalDefs::kFBit) |
                                 static_cast<uint8_t>(NalDefs::kNriMask))) |
              H264::NaluType::kStapA;
...
void RtpPacketizerH264::NextFragmentPacket(RtpPacketToSend* rtp_packet) {
  ...
  uint8_t fu_indicator =
      (packet->header & (static_cast<uint8_t>(NalDefs::kFBit) |
                         static_cast<uint8_t>(NalDefs::kNriMask))) |
      H264::NaluType::kFuA;
  uint8_t fu_header = 0;
  ...
  // S | E | R | 5 bit type.
  fu_header |=
      (packet->first_fragment ? static_cast<uint8_t>(FuDefs::kSBit) : 0);
  fu_header |=
      (packet->last_fragment ? static_cast<uint8_t>(FuDefs::kEBit) : 0);
  uint8_t type = packet->header & static_cast<uint8_t>(NalDefs::kTypeMask);
  fu_header |= type;
  rtc::ArrayView<const uint8_t> fragment = packet->source_fragment;
  uint8_t* buffer =
