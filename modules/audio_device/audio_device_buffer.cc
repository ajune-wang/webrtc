...
void AudioDeviceBuffer::LogStats(LogState state) {
  ...
  // was set to LOG_START. Hence, first printed log will be after ~20 seconds.
  if (++num_stat_reports_ > 2 && time_since_last > 0) {
    ...
    uint32_t abs_diff_rate_in_percent = static_cast<uint32_t>(
        0.5f +
        ((100.0f * std::abs(rate - rec_sample_rate_)) / rec_sample_rate_));
    ...
    abs_diff_rate_in_percent = static_cast<uint32_t>(
        0.5f +
        ((100.0f * std::abs(rate - play_sample_rate_)) / play_sample_rate_));
