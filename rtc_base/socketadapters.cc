...
#error Wrong code for HTTP_STATUS_PROXY_AUTH_REQ
    ...
    default:
      ...
      return;
             ...
             (rtc::ascnicmp(data, "Proxy-Authenticate:", 19) == 0)) {
  ...
  } else if (ascnicmp(data, "Content-Length:", 15) == 0) {
    content_length_ = strtoul(data + 15, 0, 0);
  } else if (ascnicmp(data, "Proxy-Connection: Keep-Alive", 28) == 0) {
    expect_close_ = false;
    // } else if (ascnicmp(data, "Connection: close", 17) == 0) {
    //   expect_close_ = true;
  }
}
