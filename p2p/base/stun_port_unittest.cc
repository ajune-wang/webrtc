...
// A base class template for tests connecting a StunPort to a fake STUN server
// (cricket::StunServer). Takes a PacketSocketFactory implementation as a
// template argument.
template <typename T>
class StunPortTestBase : public ::testing::Test, public sigslot::has_slots<> {
 ...
 protected:
  ...
  std::unique_ptr<T> socket_factory_;
...
class StunPortTestWithMockDnsResolver
    : public StunPortTestBase<MockDnsResolverPacketSocketFactory> {
 public:
  ...
  void SetDnsResolverExpectations(DnsResolverExpectations expectations) {
    socket_factory_.get()->SetExpectations(expectations);
  }
};
...
TEST_F(StunPortTestWithMockDnsResolver, TestPrepareAddressHostname) {
...
class StunIPv6PortTestWithMockDnsResolver
    : public StunIPv6PortTestBase<MockDnsResolverPacketSocketFactory> {
 public:
  void SetDnsResolverExpectations(DnsResolverExpectations expectations) {
    socket_factory_.get()->SetExpectations(expectations);
  }
...
// Test that we can get an address from a STUN server specified by a hostname.
TEST_F(StunIPv6PortTestWithMockDnsResolver, IPv6TestPrepareAddressHostname) {
  SetDnsResolverExpectations(
      [](webrtc::MockAsyncDnsResolver* resolver,
         webrtc::MockAsyncDnsResolverResult* resolver_result) {
        EXPECT_CALL(*resolver, Start(kStunHostnameAddr, _))
...
TEST_F(StunIPv6PortTestWithMockDnsResolver, TestPrepareAddressHostnameFamily) {
  ...
  SetDnsResolverExpectations(
      [](webrtc::MockAsyncDnsResolver* resolver,
         webrtc::MockAsyncDnsResolverResult* resolver_result) {
        EXPECT_CALL(*resolver, Start(kStunHostnameAddr, AF_INET6, _))
