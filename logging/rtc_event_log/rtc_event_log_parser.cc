...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::ParseFile(
  ...
  if (file == NULL) {
    RTC_LOG(LS_WARNING) << "Could not open file " << filename
                        << " for reading.";
    RTC_PARSE_CHECK_OR_RETURN(file != NULL);
  }

  // Compute file size
  fseek(file, 0, SEEK_END);
  size_t filesize = rtc::checked_cast<size_t>(ftell(file));
  fseek(file, 0, SEEK_SET);

  // Read file into memory.
  std::string buffer(filesize, '\0');
  size_t bytes_read = fread(&buffer[0], sizeof(uint8_t), buffer.size(), file);
  if (bytes_read != filesize) {
    RTC_LOG(LS_WARNING) << "Failed to read file " << filename;
    RTC_PARSE_CHECK_OR_RETURN_EQ(bytes_read, filesize);
  }

  auto status = ParseStream(buffer);
