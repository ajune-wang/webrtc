...
DecodeSynchronizer::SynchronizedFrameDecodeScheduler::ScheduledRtpTimestamp() {
  return next_frame_.has_value() ? absl::make_optional(next_frame_->rtp)
                                 : absl::nullopt;
}
...
Timestamp
DecodeSynchronizer::SynchronizedFrameDecodeScheduler::MaxDecodeTime() {
  RTC_DCHECK(next_frame_);
  return next_frame_->schedule.max_decode_time;
}
...
void DecodeSynchronizer::SynchronizedFrameDecodeScheduler::ScheduleFrame(
  ...
  next_frame_ = {
      .rtp = rtp, .schedule = std::move(schedule), .callback = std::move(cb)};
...
DecodeSynchronizer::CreateSynchronizedFrameScheduler() {
  ...
  if (inserted && schedulers_.size() == 1) {
    RTC_LOG(LS_VERBOSE) << "Listening to metronome";
...
void DecodeSynchronizer::OnFrameScheduled(
  ...
  // TODO(eshr): Expose 5ms from frame buffer proxy and frame buffer 2.
  const TimeDelta decode_immediately_threshold = TimeDelta::Millis(5);

  Timestamp now = clock_->CurrentTime();
  Timestamp next_tick = expected_next_tick_;
  // If no tick has regiestered yet assume it will occur in the tick period.
  if (next_tick.IsInfinite()) {
    next_tick = now + metronome_->TickPeriod();
  }

  bool decode_before_next_tick =
      scheduler->MaxDecodeTime() < (next_tick - decode_immediately_threshold);

  bool decode_time_in_past = scheduler->MaxDecodeTime() < clock_->CurrentTime();

  // Decode immediately if the decode time is in the past, or before the next
  // tick.
  if (decode_before_next_tick || decode_time_in_past) {
    auto scheduled_frame = scheduler->ReleaseNextFrame();
    RTC_LOG(LS_VERBOSE) << "Immediate release of " << scheduled_frame.rtp
                        << " now since decode_time="
                        << scheduled_frame.schedule.max_decode_time.ms()
                        << " < next_tick=" << next_tick.ms() << "-"
                        << decode_immediately_threshold.ms() << "="
                        << (next_tick - decode_immediately_threshold).ms()
                        << " or < now=" << clock_->CurrentTime().ms();
    scheduled_frame.callback(scheduled_frame.rtp,
                             scheduled_frame.schedule.render_time);
  }
}
...
void DecodeSynchronizer::RemoveFrameScheduler(
  ...
  if (schedulers_.empty()) {
    RTC_LOG(LS_VERBOSE) << "Not listening to metronome";
...
void DecodeSynchronizer::OnTick() {
  ...
  // Find all ready schedulers and put into a temporary array, since erasing
  // invalidates iterators for sets.
  std::vector<SynchronizedFrameDecodeScheduler*> ready_schedulers;
  absl::c_copy_if(
      schedulers_, std::back_inserter(ready_schedulers),
      [next_tick = expected_next_tick_](auto* scheduler) {
        if (!scheduler->ScheduledRtpTimestamp())
          return false;
        bool decode_now = (scheduler->MaxDecodeTime() < next_tick);
        if (decode_now) {
          RTC_LOG(LS_VERBOSE)
              << "Decoding " << *scheduler->ScheduledRtpTimestamp()
              << " now since decode_time=" << scheduler->MaxDecodeTime().ms()
              << " < next_tick=" << next_tick.ms();
        } else {
          RTC_LOG(LS_VERBOSE)
              << "Skipping " << *scheduler->ScheduledRtpTimestamp()
              << " now since decode_time=" << scheduler->MaxDecodeTime().ms()
              << " >= next_tick=" << next_tick.ms();
        }
        return decode_now;
  ...
  for (auto* scheduler : ready_schedulers) {
    // ReleaseNextFrame removes `scheduler` from `schedulers_`.
    auto scheduled_frame = scheduler->ReleaseNextFrame();
    scheduled_frame.callback(scheduled_frame.rtp,
                             scheduled_frame.schedule.render_time);
  }
