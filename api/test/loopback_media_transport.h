  ...
  class LoopbackMediaTransport : public MediaTransportInterface {
   public:
    ...
    RTCError SendData(int channel_id,
      ...
      // Ensure we have a copy of the data before doing an async invoke.
      rtc::CopyOnWriteBuffer buffer_copy(buffer);
      buffer_copy.data();  // Ensure the underlying data is copied.
      invoker_.AsyncInvoke<void>(
          RTC_FROM_HERE, thread_, [this, channel_id, params, buffer_copy] {
            other_->OnData(channel_id, params.type, buffer_copy);
          });
      return RTCError::OK();
    }
