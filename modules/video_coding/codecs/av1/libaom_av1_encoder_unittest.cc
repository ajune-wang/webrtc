...
TEST(LibaomAv1EncoderTest, PopulatesEncodedFrameSize) {
  ...
  EXPECT_THAT(
      encoded_frames,
      ElementsAre(Field(&Frame::encoded_image,
                        AllOf(Field(&EncodedImage::_encodedWidth,
                                    codec_settings.spatialLayers[2].width),
                              Field(&EncodedImage::_encodedHeight,
                                    codec_settings.spatialLayers[2].height))),
                  Field(&Frame::encoded_image,
                        AllOf(Field(&EncodedImage::_encodedWidth,
                                    codec_settings.spatialLayers[1].width),
                              Field(&EncodedImage::_encodedHeight,
                                    codec_settings.spatialLayers[1].height))),
                  Field(&Frame::encoded_image,
                        AllOf(Field(&EncodedImage::_encodedWidth,
                                    codec_settings.spatialLayers[0].width),
                              Field(&EncodedImage::_encodedHeight,
                                    codec_settings.spatialLayers[0].height)))));
}
