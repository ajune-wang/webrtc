...
TEST(RtpPacketizerH264Test, MixedStapAFUA) {
  ...
  EXPECT_THAT(
      packets[0].payload().subview(0, kFuAHeaderSize),
      ElementsAre(kFuA, static_cast<uint8_t>(FuDefs::kSBit) | nalus[0][0]));
  EXPECT_THAT(
  ...
  EXPECT_THAT(
      ...
      ElementsAre(kFuA, static_cast<uint8_t>(FuDefs::kEBit) | nalus[0][0]));
  EXPECT_THAT(
...
std::vector<int> TestFua(size_t frame_payload_size,
  ...
  EXPECT_TRUE(fua_header.front() & static_cast<uint8_t>(FuDefs::kSBit));
  EXPECT_TRUE(fua_header.back() & static_cast<uint8_t>(FuDefs::kEBit));
  // Clear S and E bits before testing all are duplicating same original header.
  fua_header.front() &= ~static_cast<uint8_t>(FuDefs::kSBit);
  fua_header.back() &= ~static_cast<uint8_t>(FuDefs::kEBit);
