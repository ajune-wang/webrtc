...
TEST(RtcpTransceiverImplTest, DelaysSendingFirstCompondPacket) {
  ...
  EXPECT_GE(rtc::TimeMillis() - started_ms, config.initial_delay_ms);
...
TEST(RtcpTransceiverImplTest, SendCompoundPacketDelaysPeriodicSendPackets) {
  ...
  EXPECT_FALSE(transport.WaitPacket(config.period_report_ms / 2));
  // Send packet now.
