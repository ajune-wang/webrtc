...
TEST(RtcpTransceiverImplTest, MaySendsMultipleReceiverReportInSinglePacket) {
  ...
  // Expect single rtcp packet with multiple receiver reports in it.
  EXPECT_EQ(transport.num_packets(), 1);
  // Receiver report may contain up to 31 report block, thus 2 is needed
  // to carry 40 blocks: 31 in the first, 9 - in the last.
  EXPECT_EQ(rtcp_parser.receiver_report()->num_packets(), 2);
  // RtcpParser remembers just the last receiver report, thus may check
  // only it's size.
...
TEST(RtcpTransceiverImplTest, AttachMaxNumberOfReportBlocksToCompoundPacket) {
  ...
  // Expect single rtcp packet with multiple receiver reports and fir in it.
  EXPECT_EQ(transport.num_packets(), 1);
  EXPECT_GT(rtcp_parser.receiver_report()->num_packets(), 0);
