  ...
  PacketOptions options;
  {
    ...
    if (fec_generator_ && packet->fec_protect_packet()) {
      ...
      if (packet->is_red()) {
        ...
        rtc::CopyOnWriteBuffer unpacked_buffer = unpacked_packet.Buffer();
        ...
        // Crop buffer to just the headers.
        unpacked_buffer.SetSize(headers_size);

        // Append the original payload data, after the RED header.
        unpacked_buffer.AppendData(
            buffer.Slice(headers_size + 1, packet->payload_size() - 1));
