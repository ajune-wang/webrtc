...
TEST_F(ApmTest, Process) {
  ...
  size_t delimiter =
      ref_filename_.find_last_of("/\\");  // Either windows or posix
  std::string filename =
      ref_filename_.substr(delimiter == std::string::npos ? 0 : delimiter + 1);
  std::string output_path = test::JoinFilename(output_dir, filename);
  printf("Writing to %s\n", output_path.c_str());
  if (absl::GetFlag(FLAGS_write_apm_ref_data)) {
