...
webrtc::RTCError JsepTransport::SetLocalJsepTransportDescription(
  ...
  if (sdes_transport_.get()) {
    RTC_DCHECK(!unencrypted_rtp_transport_.get());
    RTC_DCHECK(!dtls_srtp_transport_.get());
  ...
  } else if (dtls_srtp_transport_.get()) {
    RTC_DCHECK(!unencrypted_rtp_transport_.get());
    RTC_DCHECK(!sdes_transport_.get());
...
void JsepTransport::ActivateRtcpMux() {
  ...
  if (unencrypted_rtp_transport_.get()) {
    RTC_DCHECK(!sdes_transport_.get());
    RTC_DCHECK(!dtls_srtp_transport_.get());
    unencrypted_rtp_transport_->SetRtcpPacketTransport(nullptr);
  } else if (sdes_transport_.get()) {
    RTC_DCHECK(!unencrypted_rtp_transport_.get());
    RTC_DCHECK(!dtls_srtp_transport_.get());
