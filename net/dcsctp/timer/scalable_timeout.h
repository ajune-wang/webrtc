...
// should, use the same underlying `ScalableTimeoutDriver` object.
...
class ScalableTimeout : public Timeout {
 ...
 private:
  ...
  // A traditional "asymmetric doubly linked list", where `prevs_next_` points
  // to the previous element's `next_` element, for easier removal without
  // having to know the list the element is contained in.
