...
PeerConnectionFactory::CreatePeerConnection(
  ...
  RTC_DCHECK(call) << "Call creation in CreatePeerConnection";
...
std::unique_ptr<Call> PeerConnectionFactory::CreateCall_w(
  ...
  if (!channel_manager()->media_engine() || !context_->call_factory()) {
    RTC_LOG(LS_ERROR) << "DEBUG: Not creating call, media engine "
                       << channel_manager()->media_engine()
                       << ", call factory " << context_->call_factory();
    return nullptr;
  }
