...
NetEqInput::Event EncodeNetEqInput::PopEvent() {
...
NetEqInput::Event EncodeNetEqInput::GetNextEvent() {
  NetEqInput::Event event;
  if (packet_data_ == nullptr) {
    CreatePacket();
  }
  if (next_output_event_ms_ < packet_data_->time_ms) {
    event.audio_output = std::make_unique<GetAudio>(next_output_event_ms_);
    next_output_event_ms_ += kOutputPeriodMs;
    return event;
  }
  event.packet_data = std::move(packet_data_);
  CreatePacket();
  return event;
}
