...
bool EnableAdaptErleLowRender() {
  return !field_trial::IsEnabled("WebRTC-Aec3AdaptErleLowRenderKillSwitch");
}
  ...
  for (size_t k = start; k < stop; ++k) {
    if (adapt_low_render_ || (X2[k] > kX2BandEnergyThreshold)) {
      ...
      absl::optional<float> new_erle =
          erle_freq_inst_.Update(X2[k], Y2[k], E2[k], k, &low_render_energy);
      if (new_erle) {
        RTC_DCHECK(adapt_low_render_ || !low_render_energy);
