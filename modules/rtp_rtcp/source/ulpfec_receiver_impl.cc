...
int32_t UlpfecReceiverImpl::ProcessReceivedFec() {
  ...
  for (const auto& received_packet : received_packets) {
    ...
    if (!received_packet->is_recovered) {
      // We shouldn't pass recovered packets to FEC. Here it can be RTX packets
      // for example. They can have different RTP extensions which will lead
      // to the wrong FEC packet recovery and corrupted packet.
