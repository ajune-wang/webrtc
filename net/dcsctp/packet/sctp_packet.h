...
class SctpPacket {
 public:
  ...
  class Builder {
   public:
    Builder(uint32_t verification_tag, const DcSctpOptions& options);
    ...
    // Returns the payload of the build SCTP packet. The build remains valid
    // after having called this method, but it's cleared as a side-effect.
    std::vector<uint8_t> Build();

   private:
    void WritePacketHeader();
    const uint32_t verification_tag_;
    const uint16_t source_port_;
    const uint16_t dest_port_;
    const size_t max_mtu_;
