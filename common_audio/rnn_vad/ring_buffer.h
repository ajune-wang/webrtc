...
#ifndef COMMON_AUDIO_RNN_VAD_RING_BUFFER_H_
#define COMMON_AUDIO_RNN_VAD_RING_BUFFER_H_
...
class RingBuffer {
 ...
 public:
  explicit RingBuffer(const T init_value) : tail_(0) {
...
template <typename T, size_t S, size_t N>
std::ostream& operator<<(std::ostream& os, const RingBuffer<T, S, N>& t) {
  os << "[" << S << ", " << N << "]";
  for (size_t delay = 0; delay < N; ++delay) {
    os << std::endl << "  #" << delay << ":";
    auto view = t.GetArrayView(delay);
...
#endif  // COMMON_AUDIO_RNN_VAD_RING_BUFFER_H_
