...
void TimestampExtrapolator::Reset(Timestamp start) {
  start_ = start;
  prev_ = start_;
  first_wrapped_timestamp_ = absl::nullopt;
  w_[0] = 90.0;
  w_[1] = 0;
  pP_[0][0] = 1;
  pP_[1][1] = kPP11;
...
void TimestampExtrapolator::Update(Timestamp now, uint32_t ts90khz) {
  ...
  if (!first_wrapped_timestamp_) {
    // Make an initial guess of the offset,
    // should be almost correct since t_ms - start
    // should about zero at this time.
    w_[1] = -w_[0] * t_ms;
    first_wrapped_timestamp_ = unwrapped_ts90khz;
  }

  double residual =
      (static_cast<double>(unwrapped_ts90khz) - *first_wrapped_timestamp_) -
      t_ms * w_[0] - w_[1];
  ...
  // K = P*T/(lambda + T'*P*T);
  double K[2];
  K[0] = pP_[0][0] * t_ms + pP_[0][1];
  K[1] = pP_[1][0] * t_ms + pP_[1][1];
  ...
  // P = 1/lambda*(P - K*T'*P);
  double p00 =
      1 / kLambda * (pP_[0][0] - (K[0] * t_ms * pP_[0][0] + K[0] * pP_[1][0]));
  double p01 =
      1 / kLambda * (pP_[0][1] - (K[0] * t_ms * pP_[0][1] + K[0] * pP_[1][1]));
  pP_[1][0] =
      1 / kLambda * (pP_[1][0] - (K[1] * t_ms * pP_[0][0] + K[1] * pP_[1][0]));
  pP_[1][1] =
      1 / kLambda * (pP_[1][1] - (K[1] * t_ms * pP_[0][1] + K[1] * pP_[1][1]));
  pP_[0][0] = p00;
  pP_[0][1] = p01;
...
absl::optional<Timestamp> TimestampExtrapolator::ExtrapolateLocalTime(
  ...
  if (!first_wrapped_timestamp_) {
    ...
    double timestampDiff = unwrapped_ts90khz - *first_wrapped_timestamp_;
