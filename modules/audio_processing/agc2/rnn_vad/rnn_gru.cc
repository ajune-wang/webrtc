...
void ComputeGruUpdateResetGates(int input_size,
  ...
  const int output_size = rtc::dchecked_cast<int>(gate.size());
  for (int o = 0; o < output_size; ++o) {
    gate[o] = bias[o];
...
GatedRecurrentLayer::GatedRecurrentLayer(
    ...
    : input_size_(input_size),
      ...
      reset_(output_size),
      output_(output_size) {
  RTC_DCHECK_EQ(kNumGruGates * state_.size(), bias_.size())
      << "Mismatching output size and bias terms array size (" << layer_name
      << ").";
  RTC_DCHECK_EQ(kNumGruGates * input_size_ * state_.size(), weights_.size())
      << "Mismatching input-output size and weight coefficients array size ("
      << layer_name << ").";
  RTC_DCHECK_EQ(kNumGruGates * state_.size() * state_.size(),
...
void GatedRecurrentLayer::ComputeOutput(rtc::ArrayView<const float> input) {
  ...
  const int output_size = rtc::dchecked_cast<int>(state_.size());
  const int stride_in = input_size_ * output_size;
  ...
  // Update gate.
  ComputeGruUpdateResetGates(input_size_, weights.subview(0, stride_in),
                             recurrent_weights.subview(0, stride_out),
                             bias.subview(0, output_size), input, state_,
                             update_);

  // Reset gate.
  ComputeGruUpdateResetGates(input_size_, weights.subview(stride_in, stride_in),
                             recurrent_weights.subview(stride_out, stride_out),
                             bias.subview(output_size, output_size), input,
                             state_, reset_);

  // Output gate.
  ComputeGruOutputGate(input_size_, weights.subview(2 * stride_in, stride_in),
                       recurrent_weights.subview(2 * stride_out, stride_out),
                       bias.subview(2 * output_size, output_size), input,
                       state_, reset_, output_);

  // Update output through the update gates and update the state.
  for (int o = 0; o < output_size; ++o) {
    output_[o] = update_[o] * state_[o] + (1.f - update_[o]) * output_[o];
    state_[o] = output_[o];
  }
}
