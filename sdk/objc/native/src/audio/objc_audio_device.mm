...
int32_t ObjCAudioDevice::InitPlayout() {
  ...
  SetupOutputAudioBuffersForAudioDevice();
...
int32_t ObjCAudioDevice::InitRecording() {
  ...
  SetupInputAudioBuffersForAudioDevice();
...
OSStatus ObjCAudioDevice::OnDeliverRecordedData(AudioUnitRenderActionFlags* flags,
  ...
  // at each input callback when calling `renderBlock`.
...
void ObjCAudioDevice::HandleAudioInputInterrupted() {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  io_record_thread_checker_.Detach();
}

void ObjCAudioDevice::HandleAudioOutputInterrupted() {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  io_playout_thread_checker_.Detach();
}

void ObjCAudioDevice::HandleAudioInputParametersChange() {
  RTC_LOG_F(LS_INFO) << "";
  const double input_sample_rate = [audio_device_ inputSampleRate];

  const NSTimeInterval input_io_buffer_duration = [audio_device_ inputIOBufferDuration];
  const size_t input_frames_per_buffer =
      static_cast<size_t>(input_sample_rate * input_io_buffer_duration + .5);

  const double current_sample_rate = record_parameters_.sample_rate();
  const size_t current_frames_per_buffer = record_parameters_.frames_per_buffer();

  RTC_LOG(LS_INFO) << "Handling input sample rate change"
                   << ". Input sample rate: " << input_sample_rate
                   << " frames_per_buffer: " << input_frames_per_buffer
                   << ". ADM sample rate: " << current_sample_rate
                   << " frames_per_buffer: " << current_frames_per_buffer;

  // Sample rate and buffer size are the same, no work to do.
  if (std::abs(current_sample_rate - input_sample_rate) <= DBL_EPSILON &&
      current_frames_per_buffer == input_frames_per_buffer) {
    RTC_LOG(LS_INFO) << "Ignoring sample rate change since audio input parameters are intact.";
    return;
  }

  // Extra sanity check to ensure that the new sample rate is valid.
  if (input_sample_rate <= 0.0) {
    RTC_LOG(LS_ERROR) << "Input sample rate is invalid: " << input_sample_rate;
    return;
  }
  ...
  RTC_DCHECK_EQ(record_parameters_.sample_rate(), input_sample_rate);
}

void ObjCAudioDevice::HandleAudioOutputParametersChange() {
  RTC_LOG_F(LS_INFO) << "";
  const double output_sample_rate = [audio_device_ outputSampleRate];

  const NSTimeInterval output_io_buffer_duration = [audio_device_ outputIOBufferDuration];
  const size_t output_frames_per_buffer =
      static_cast<size_t>(output_sample_rate * output_io_buffer_duration + .5);
  ...
  RTC_LOG(LS_INFO) << "Handling output sample rate change"
  ...
  // Sample rate and buffer size are the same, no work to do.
  if (std::abs(current_sample_rate - output_sample_rate) <= DBL_EPSILON &&
      current_frames_per_buffer == output_frames_per_buffer) {
    RTC_LOG(LS_INFO) << "Ignoring sample rate change since audio output parameters are intact.";
    return;
  }

  // Extra sanity check to ensure that the new sample rate is valid.
  if (output_sample_rate <= 0.0) {
    RTC_LOG(LS_ERROR) << "Output sample rate is invalid: " << output_sample_rate;
    return;
  }
  ...
  RTC_DCHECK_EQ(playout_parameters_.sample_rate(), output_sample_rate);
}

void ObjCAudioDevice::UpdateOutputAudioDeviceBuffer() {
  RTC_LOG_F(LS_INFO) << "";
...
void ObjCAudioDevice::UpdateInputAudioDeviceBuffer() {
  RTC_LOG_F(LS_INFO) << "";
  RTC_DCHECK(audio_device_buffer_) << "AttachAudioBuffer must be called first";
...
void ObjCAudioDevice::SetupOutputAudioBuffersForAudioDevice() {
  RTC_LOG_F(LS_INFO) << "";

  double sample_rate = [audio_device_ outputSampleRate];
  const NSTimeInterval io_buffer_duration = [audio_device_ outputIOBufferDuration];

  AudioParameters& parameters = playout_parameters_;
  ...
  if (sample_rate <= DBL_EPSILON && parameters.sample_rate() > 0) {
    RTC_LOG(LS_ERROR) << "Reported rate is invalid: " << sample_rate << ". Using "
                      << parameters.sample_rate() << " as sample rate instead.";
...
void ObjCAudioDevice::SetupInputAudioBuffersForAudioDevice() {
  RTC_LOG_F(LS_INFO) << "";

  double sample_rate = [audio_device_ inputSampleRate];
  const NSTimeInterval io_buffer_duration = [audio_device_ inputIOBufferDuration];

  AudioParameters& parameters = record_parameters_;
  RTC_LOG_F(LS_INFO) << "sample_rate = " << sample_rate
                     << ", io_buffer_duration = " << io_buffer_duration
                     << ", parameters = " << parameters.ToString();
