...
TEST(WPDTreeTest, CorrectnessBasedOnMatlabFiles) {
  ...
  for (int i = 0; i < kLeaves; ++i) {
    ...
    matlab_files_data[i] =
        FileWrapper::Open(matlab_string.c_str(), true);  // Read only.
    ...
    out_files_data[i] =
        FileWrapper::Open(out_string.c_str(), false);  // Write mode.
  ...
  FileWrapper test_file =
      FileWrapper::Open(test_file_name.c_str(), true);  // Read only.
