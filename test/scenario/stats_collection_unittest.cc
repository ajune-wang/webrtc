...
TEST(ScenarioAnalyzerTest, PsnrIsHighWhenNetworkIsGood) {
  ...
  CallStatsCollectors stats;
  {
    ...
    good_network.bandwidth = DataRate::KilobitsPerSecond(1000);
  ...
  EXPECT_NEAR(stats.call.stats().target_rate.Mean().KilobitsPerSecond(), 700,
              300);
  EXPECT_NEAR(stats.video_send.stats().media_bitrate.Mean().KilobitsPerSecond(),
              500, 200);
  EXPECT_NEAR(stats.video_receive.stats().resolution.Mean(), 180, 10);
  EXPECT_NEAR(stats.audio_receive.stats().jitter_buffer.Mean().Milliseconds(),
              40, 20);
}
...
TEST(ScenarioAnalyzerTest, PsnrIsLowWhenNetworkIsBad) {
  ...
  CallStatsCollectors stats;
  {
    ...
    bad_network.bandwidth = DataRate::KilobitsPerSecond(100);
  ...
  EXPECT_NEAR(stats.call.stats().target_rate.Mean().KilobitsPerSecond(), 75,
              50);
  EXPECT_NEAR(stats.video_send.stats().media_bitrate.Mean().KilobitsPerSecond(),
              100, 50);
  EXPECT_NEAR(stats.video_receive.stats().resolution.Mean(), 180, 10);
