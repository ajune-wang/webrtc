...
class NetworkEmulationManager3NodesRoutingTest : public ::testing::Test {
 public:
  NetworkEmulationManager3NodesRoutingTest() {
  ...
  void SendPacketsAndValidateDelivery() {
    ...
    EXPECT_CALL(r_e1_e2, OnPacketReceived(testing::_)).Times(1);
    EXPECT_CALL(r_e2_e1, OnPacketReceived(testing::_)).Times(1);
    EXPECT_CALL(r_e1_e3, OnPacketReceived(testing::_)).Times(1);
    EXPECT_CALL(r_e3_e1, OnPacketReceived(testing::_)).Times(1);

    uint16_t common_send_port = 80;
    uint16_t r_e1_e2_port = e2_->BindReceiver(0, &r_e1_e2).value();
    uint16_t r_e2_e1_port = e1_->BindReceiver(0, &r_e2_e1).value();
    uint16_t r_e1_e3_port = e3_->BindReceiver(0, &r_e1_e3).value();
    uint16_t r_e3_e1_port = e1_->BindReceiver(0, &r_e3_e1).value();
...
//  * e1 -> n3 -> e3
//  * e3 -> n4 -> e1
TEST_F(NetworkEmulationManager3NodesRoutingTest, ComplexRouting) {
...
//  * e1 -> n1 -> e3
//  * e3 -> n4 -> e1
TEST_F(NetworkEmulationManager3NodesRoutingTest, ComplexRoutingReuse) {
  SetupRouting([](EmulatedEndpoint* e1, EmulatedEndpoint* e2,
                  EmulatedEndpoint* e3, NetworkEmulationManager* emulation) {
    auto* node1 = CreateEmulatedNodeWithDefaultBuiltInConfig(emulation);
    auto* node2 = CreateEmulatedNodeWithDefaultBuiltInConfig(emulation);
