...
#ifndef RTC_BASE_REPEATING_TASK_H_
#define RTC_BASE_REPEATING_TASK_H_
...
// Allows starting tasks that repeat themeself on a TaskQueue indefinately until
// they are stopped or the TaskQueue is destroyed. It allows starting and
// stopping multiple times, but only one repeated task can be started and it can
// only be stopped when in the running state. The public interface is not thread
// safe.
class RepeatingTaskHandle {
 public:
  using IntervalMode = webrtc_repeating_task_impl::IntervalMode;
  // |intevar_mode| specifies how the returned interval should be interpreted.
  ...
  // closure will be dealyd by the given amount.
  template <class Closure>
...
#endif  // RTC_BASE_REPEATING_TASK_H_
