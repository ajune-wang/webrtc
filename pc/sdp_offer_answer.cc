...
void SdpOfferAnswerHandler::ApplyRemoteDescriptionUpdateTransceiverState(
  ...
  for (const auto& transceiver_ext : transceivers()->List()) {
    ...
    if (sdp_type == SdpType::kPrAnswer || sdp_type == SdpType::kAnswer) {
      ...
      if (transceiver->mid()) {
        // TODO(tommi): `LookupDtlsTransportByMid` will hop over to the network
        // thread and return the transport (these details could be inside of
        // the transceiver implementation btw). We'll then get back here on
        // the signaling thread to call `set_transport`. There, a reference
        // to the transport will be set and held on the signaling thread. It
        // doesn't look like there's any need to keep that reference except for
        // the purposes of the public dtls_transport() API.
        // Q: Could we skip caching this pointer here and instead look it up
        //    on demand?
        // Q: Does the returned transport object, go to an API proxy?
