...
class LossNotificationControllerTestDecodabilityFlag
 ...
 protected:
  ...
  void ReceiveKeyFrame() {
    RTC_DCHECK_NE(key_frame_frame_id_, kReservedFrameId);
  ...
  void ReceivePacket(bool first_packet_in_frame,
    ...
    RTC_DCHECK_NE(frame_id_, kReservedFrameId);
  ...
  static constexpr uint16_t kReservedFrameId = 123;  // Never receive this.
...
TEST_F(LossNotificationControllerTestDecodabilityFlag,
  ...
  const uint16_t never_received_frame_id = key_frame_frame_id_ + 1;
  const std::vector<uint16_t> ref_frame_ids = {never_received_frame_id};
  ReceivePacket(true, true, ref_frame_ids);
