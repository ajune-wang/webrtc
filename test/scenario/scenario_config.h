...
struct TransportControllerConfig {
  struct Rates {
    ...
    Rates(const Rates&);
    ~Rates();
    DataRate min_rate = DataRate::KilobitsPerSecond(30);
    DataRate max_rate = DataRate::KilobitsPerSecond(3000);
    DataRate start_rate = DataRate::KilobitsPerSecond(300);
  } rates;
  NetworkControllerFactoryInterface* cc_factory = nullptr;
  TimeDelta state_log_interval = TimeDelta::Milliseconds(100);
};
...
struct VideoStreamConfig {
  ...
  struct Stream {
    ...
    TimeDelta nack_history_time = TimeDelta::Milliseconds(1000);
...
struct AudioStreamConfig {
  ...
  struct Encoder {
    ...
    TimeDelta initial_frame_length = TimeDelta::Milliseconds(20);
  } encoder;
  struct Stream {
    Stream();
    Stream(const Stream&);
