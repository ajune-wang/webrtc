...
class PeerConnectionWrapper : public webrtc::PeerConnectionObserver,
 ...
 private:
  ...
  void OnAddStream(
    ...
    RTC_LOG(INFO) << "___OnAddStream";
    media_stream->RegisterObserver(this);
    for (size_t i = 0; i < media_stream->GetAudioTracks().size(); ++i) {
      const std::string id = media_stream->GetAudioTracks()[i]->id();
      RTC_LOG(INFO) << "___id: " << id;
    }
