...
webrtc::RTCError JsepTransport2::SetLocalTransportDescription(
  ...
  ret = SetRtcpMux(jsep_description.rtcp_mux_enabled, type,
  ...
  if (sdes_transport_) {
    ...
    ret = SetSdes(jsep_description.cryptos,
  ...
  ApplyIceLocalTransportDescription(rtp_dtls_transport_->ice_transport());

  if (rtcp_dtls_transport_) {
    ApplyIceLocalTransportDescription(rtcp_dtls_transport_->ice_transport());
  }
...
webrtc::RTCError JsepTransport2::SetRemoteTransportDescription(
    const JsepTransportDescription& jsep_description,
    webrtc::SdpType type) {
  bool ret = true;
  webrtc::RTCError error;
  ...
  ApplyIceRemoteTransportDescription(rtp_dtls_transport_->ice_transport());

  if (rtcp_dtls_transport_) {
    ApplyIceRemoteTransportDescription(rtcp_dtls_transport_->ice_transport());
  }
...
webrtc::RTCError JsepTransport2::AddRemoteCandidates(
  ...
  if (!local_description_ || !remote_description_) {
    return webrtc::RTCError(
        ...
        mid() +
            " is not ready to use the remote candidate. Local or "
...
rtc::Optional<rtc::SSLRole> JsepTransport2::GetDtlsRole() const {
  ...
  if (!rtp_dtls_transport_->GetSslRole(&dtls_role)) {
...
void JsepTransport2::ApplyIceLocalTransportDescription(
...
void JsepTransport2::ApplyIceRemoteTransportDescription(
    IceTransportInternal* ice_transport) {
  RTC_DCHECK(ice_transport);
...
webrtc::RTCError JsepTransport2::SetNegotiatedDtlsParameters(
  ...
  if (!dtls_transport->SetSslRole(dtls_role)) {
    return webrtc::RTCError(webrtc::RTCErrorType::INVALID_PARAMETER,
...
webrtc::RTCError JsepTransport2::NegotiateAndSetDtlsParameters(
  ...
  if (remote_fp && local_fp) {
    ...
    webrtc::RTCError error =
        NegotiateDtlsRole(local_description_type, local_connection_role,
                          remote_connection_role, &negotiated_dtls_role);
    if (!error.ok()) {
      return error;
    }
