...
TEST_F(VideoCodecInitializerTest,
       Vp9LimitNumberOfSpatialLayersKeepBitrateLimits) {
  // Request 3 spatial layers for 320x180 input. Actual layering will be reduced
  // to 1 due to low input resolution but SVC bitrate limits should be applied.
  SetUpFor(VideoCodecType::kVideoCodecVP9, 3, 3, false);
  VideoStream stream = DefaultStream();
  stream.width = 320;
  stream.height = 180;
