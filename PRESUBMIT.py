...
def _ExtractClangAnalyzerWarnings(log):
  """Extract clang analyzer warnings from log.

  This is ultimately needed to report the isolated section
  related to current change rather than the whole log.

    Each diagnostic spans across several lines.
    First line is the warning itself, in the form:
      "$filefile:$line:$col: warning: $message"
    Warning bloc ends when another starts or when summary is displayed:
      "$count warning$s generated."
  """
  current = []
  in_warning_block = False
  for line in log.split('\n'):
    is_start = 'warning:' in line
    is_end = 'warning generated.' in line or 'warnings generated.' in line
    if is_start or is_end:
      # Store previous warning
      if current:
        yield current[:]
        current = []
    if is_start:
...
def _CollectReportedSourceLines(diagnostic, source, normalize_path):
  """Crudely parse diagnostic to collect reported line numbers of given source.

  We collect line numbers from "warning:" and "note:" log entries,
  since both might explain where the detected issue comes from.

  Assume diagnostic lines follow "$filepath:$line:$whatever" format. E.g.:
    "../../src/path/dummy.cc:159:3: warning: Called C++ object pointer is null"
  NB: That format is the same than for usual warnings.

  Args:
    diagnostic:      Clang log section as list of lines.
    source:          Local path of source we want to collect the lines for.
    normalize_path:  Convert path as appearing in log to local path.

  Returns:
    Set of lines reported for given source.
  """
  result = set()
  for line in diagnostic:
    ...
    if len(fields) < 2:
      continue
    path, line = fields[:2]
    # Remove ANSI escape sequence. We use the fact path starts by '../'.
    path = path[path.find('.'):]
    if normalize_path(path) != source:
      continue
    # Extra safety in case of unrelated log line (with '::' operator).
    try:
      result.add(int(line))
    except ValueError:
      pass
...
def CheckStaticAnalyzer(input_api, output_api):
  """Run changed files through clang static analyzer."""
  msg = ('Clang static analyser reported issues. '
         'Either fix or suppress them.\n'
         'See https://chromium.googlesource.com/chromium/src.git/+/HEAD/'
         'docs/clang_static_analyzer.md#addressing-false-positives\n')

  with _AddToPath(input_api.os_path.join(
      input_api.PresubmitLocalPath(), 'tools_webrtc', 'presubmit_checks_lib')):
    from build_helpers import RunGnCommand, \
                              RunClangStaticAnalyzer, GetCompilationCommands

  src_dir = FindSrcDirPath(input_api.PresubmitLocalPath())
  # Create work dir now so we don't start from scratch for each file.
  out_dir = tempfile.mkdtemp('static_analyzer')

  try:
    gn_errors = RunGnCommand([
      'gen', '--args=use_clang_static_analyzer=true', out_dir
    ], src_dir)
    if gn_errors:
    ...
    commands = GetCompilationCommands(out_dir)

    # Only process cpp compilation units.
    # Nb: headers cannot be built and won't be checked.
    def FileFilter(path):
      return (input_api.FilterSourceFile(path)
              and path.LocalPath().endswith(('.cc')))

    # Get source path relatively to root dir (src/ typically)
    # in order to match AffectedFile.LocalPath().
    def GetLocalPath(path):
      return os.path.relpath(os.path.join(out_dir, path), src_dir)

    errors = []

    for f in input_api.AffectedSourceFiles(FileFilter):
      # Build single file. NB: We don't use ninja syntax: path/file.cc^
      # since it can lead to hundreds of steps.
      rel_path = os.path.relpath(f.LocalPath(), out_dir)  # As generated by gn.
      if rel_path not in commands:
        continue

      # Only keep flags, defines and include path.
      args = commands[rel_path]['command'].split()[1:]
      log = RunClangStaticAnalyzer(args, out_dir)

      # Check whether analyzer warnings are related to the change.
      changed_lines = set(line_num for line_num, _ in f.ChangedContents())
      diagnostics = list(_ExtractClangAnalyzerWarnings(log))
      for diagnostic in diagnostics:
        warning_lines = _CollectReportedSourceLines(
          diagnostic, f.LocalPath(), GetLocalPath)
        if warning_lines.intersection(changed_lines):
          errors.extend(diagnostic)

      if log and not diagnostics:
        # Don't mute other errors, so this check is easier to maintain.
        return [output_api.PresubmitPromptWarning(
          'The analyzer returned an error we were unable to parse:\n%s\n'
          'Please open a bug.' % log)]
  finally:
    shutil.rmtree(out_dir, ignore_errors=True)

  # Use long_text instead of items, which would add a blackslash to each line.
  long_text = '\n'.join(errors)
  return [output_api.PresubmitError(msg, long_text=long_text)] if errors else []
...
def CheckChangeOnCommit(input_api, output_api):
  ...
  results.extend(CheckStaticAnalyzer(input_api, output_api))
  return results
