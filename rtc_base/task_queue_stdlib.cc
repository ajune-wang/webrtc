...
rtc::PlatformThread TaskQueueStdlib::InitializeThread(
    ...
    rtc::ThreadPriority priority) {
  struct InitData {
    TaskQueueStdlib* const task_queue;
    rtc::Event started;
  } init_data{me};

  auto thread = rtc::PlatformThread::SpawnJoinable(
      [&init_data] {
        CurrentTaskQueueSetter set_current(init_data.task_queue);
        init_data.started.Set();
        init_data.task_queue->ProcessTasks();
      },
      queue_name, rtc::ThreadAttributes().SetPriority(priority));
  init_data.started.Wait(rtc::Event::kForever);
  return thread;
}
