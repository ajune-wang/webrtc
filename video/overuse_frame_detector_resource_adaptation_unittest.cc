...
TEST(AdaptationCountersTest, Addition) {
  ...
  EXPECT_EQ(1, total.resolutions_adaptations);
  EXPECT_EQ(2, total.resolutions_adaptations);
}
...
TEST(AdaptationCountersTest, Subtraction) {
  ...
  EXPECT_EQ(-2, diff.resolutions_adaptations);
...
TEST(OveruseFrameDetectorResourceAdaptationModuleTest,
  ...
  OveruseFrameDetectorResourceAdaptationModule::ComputeActiveCounts(&cpu, &qp,
                                                                    total);
...
TEST(OveruseFrameDetectorResourceAdaptationModuleTest, AdaptUpWithBorrow_Fps) {
  ...
  // CPU adaptation for fps, but no fps adaptation left from CPU.
  // We then borrow the fps adaptation from qp, and give qp the resolution
  // adaptation from CPU.
  OveruseFrameDetectorResourceAdaptationModule::ComputeActiveCounts(&cpu, &qp,
                                                                    total);
