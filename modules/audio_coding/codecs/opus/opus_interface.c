...
int WebRtcOpus_DurationEst(OpusDecInst* inst,
                           ...
                           size_t payload_length_bytes) {
  int new, old;
  new = WebRtcOpus_DurationEst_New(inst, payload, payload_length_bytes);
  old = WebRtcOpus_DurationEst_Legacy(inst, payload, payload_length_bytes);
  assert(new == old);
  return new;
}
...
int WebRtcOpus_PacketHasFec_New(const uint8_t* payload,
                                size_t payload_length_bytes) {
  // TODO(minyue): remove after verified.
  int check = WebRtcOpus_PacketHasFec_Legacy(payload, payload_length_bytes);
...
int WebRtcOpus_PacketHasFec(const uint8_t* payload,
                            size_t payload_length_bytes) {
  int new, old;
  new = WebRtcOpus_PacketHasFec_New(payload, payload_length_bytes);
  old = WebRtcOpus_PacketHasFec_Legacy(payload, payload_length_bytes);
  assert(new == old);
  return new;
}
