...
std::map<std::string, std::string> VideoCodecStats::Frame::ToMap() const {
  ...
  map["target_spatial_idxs"] = ?
  map["target_temporal_idxs"] = ?
  return map;
}
...
void VideoCodecStats::LogMetrics(
    ...
    std::map<std::string, std::string> metadata = {}) const {
  FILE* csv_file = fopen(csv_file_path.c_str(), "w");
  ...
  std::copy(metric_names.begin(), metric_names.end(),
            std::string(header, ","));
  fwrite(header.str().c_str(), 1, header.str().size(), csv_file);

  for (const auto& f : frames) {
    const auto& metrics = f.ToMap();
    std::vector<std::string> values;
    std::transform(metrics.begin(), metrics.end(), std::back_inserter(values),
