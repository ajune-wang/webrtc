...
TEST_P(RnnGruParametrization, CheckGatedRecurrentLayer) {
  GatedRecurrentLayer gru(kGruInputSize, kGruOutputSize, kGruBias, kGruWeights,
                          kGruRecurrentWeights, /*cpu_features=*/GetParam(),
                          /*layer_name=*/"GRU");
...
TEST_P(RnnGruParametrization, DISABLED_BenchmarkGatedRecurrentLayer) {
  // Prefetch test data.
  auto gru_input_reader = CreateGruInputReader();
  std::vector<float> gru_input_sequence(gru_input_reader.second);
  gru_input_reader.first->ReadChunk(gru_input_sequence);
