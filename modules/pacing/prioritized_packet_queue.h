...
class PrioritizedPacketQueue : public PacingController::PacketQueue {
 ...
 private:
  ...
  class QueuedPacket {
   public:
    QueuedPacket(std::unique_ptr<RtpPacketToSend> packet,
  ...
  class StreamQueue {
   public:
    ...
    explicit StreamQueue(StreamQueue&&) = default;
    explicit StreamQueue(const StreamQueue&) = delete;
  ...
  // Cumulative sum, over all packet, of time spent in the queue.
