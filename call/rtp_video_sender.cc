...
void RtpVideoSender::ConfigureSsrcs(const RtpConfig& rtp_config) {
  ...
  packet_receive_observers_.clear();
  for (size_t i = 0; i < rtp_config.ssrcs.size(); ++i) {
    ...
    PacketsReceviedObserver* receive_observer =
        rtp_rtcp->GetPacketReceivedObserver();
    if (receive_observer != nullptr) {
      packet_receive_observers_[ssrc] = receive_observer;
    }
  }
...
void RtpVideoSender::OnPacketFeedbackVector(
  ...
  std::map<uint32_t, std::vector<uint16_t>> acked_packets;
  {
    ...
    const bool use_fec_loss_vector_mask = fec_controller_->UseLossVectorMask();
    for (const PacketFeedback& packet : packet_feedback_vector) {
      if (use_fec_loss_vector_mask) {
      ...
      if (packet.ssrc != 0) {
        if (packet_receive_observers_.find(packet.ssrc) !=
            packet_receive_observers_.end()) {
  ...
  for (auto it : acked_packets) {
    RTC_DCHECK(packet_receive_observers_.find(it.first) !=
               packet_receive_observers_.end());
    packet_receive_observers_[it.first]->OnPacketsReceived(it.second);
  }
}
