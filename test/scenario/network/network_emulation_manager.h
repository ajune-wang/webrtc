...
class NetworkEmulationManagerImpl : public NetworkEmulationManager {
 public:
  ...
  EmulatedNetworkController* CreateEmulatedNetworkController(
 ...
 private:
  ...
  std::vector<std::unique_ptr<EndpointsController>> endpoints_controllers_;
  std::vector<std::unique_ptr<EmulatedNetworkManager>> network_managers_;

  std::map<uint64_t, EmulatedNetworkManager*> endpoint_id_to_network_manager_;
