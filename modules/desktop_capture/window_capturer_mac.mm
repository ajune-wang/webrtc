...
void WindowCapturerMac::CaptureFrame() {
  ...
  if (cg_color_space) {
    base::ScopedCFTypeRef<CFDataRef> cf_icc_profile(CGColorSpaceCopyICCProfile(cg_color_space));
    const char* data_as_byte = reinterpret_cast<const char*>(CFDataGetBytePtr(cf_icc_profile));
    const size_t data_size = CFDataGetLength(cf_icc_profile);
