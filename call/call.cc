...
Call::Call(Clock* clock,
           ...
           std::unique_ptr<RtpTransportControllerSendInterface> transport_send)
    : clock_(clock),
      task_queue_factory_(&config.env.task_queue_factory()),
      ...
      config_(config),
      trials_(config.env.field_trials()),
      audio_network_state_(kNetworkDown),
      video_network_state_(kNetworkDown),
      aggregate_network_up_(false),
      event_log_(&config.env.event_log()),
      ...
      receive_time_calculator_(
          ReceiveTimeCalculator::CreateFromFieldTrial(trials_)),
...
webrtc::VideoSendStream* Call::CreateVideoSendStream(
  ...
  VideoSendStream* send_stream = new VideoSendStream(
      ...
      suspended_video_payload_states_, std::move(fec_controller), trials_);
