...
namespace {
constexpr const int kFramerateFps = 30;
constexpr const int kDefaultBitrateStateUpdateIntervalSeconds = 5;
constexpr const int kDefaultEncodeDeltaTimeMs = 33;  // 1/30(s) => 33(ms)
...
TEST_P(BandwidthQualityScalerTest, AllNormalFrame_640x360) {
  ...
  // [500000,800000] BandwidthQualityScaler calculates actual encode bps is
  // 654253, so it falls in the range without any operation(up/down)
...
TEST_P(BandwidthQualityScalerTest, AllNoramlFrame_AboveMaxBandwidth_640x360) {
  ...
  // When resolution is 640*360, bps experimental value interval is
  // [500000,800000] BandwidthQualityScaler calculates actual encode bps is
  // 1208000 > 800000 * 1.2, so it triggers adapt_up_event_count_
...
TEST_P(BandwidthQualityScalerTest, AllNormalFrame_Underuse_640x360) {
  ...
  // When resolution is 640*360, bps experimental value interval is
  // [500000,800000] BandwidthQualityScaler calculates actual encode bps is
  // 377379 < 500000 * 0.8, so it triggers adapt_down_event_count_
  EXPECT_TRUE(handler_->event_.Wait(
      bandwidth_quality_scaler_->GetBitrateStateUpdateIntervalMs()));
...
TEST_P(BandwidthQualityScalerTest, FixedFrameTypeTest1_640x360) {
  ...
  // When resolution is 640*360, bps experimental value interval is
  // [500000,800000] BandwidthQualityScaler calculates actual encode bps
  // is 1059462 > 800000 * 1.2, so it triggers adapt_up_event_count_
  EXPECT_TRUE(handler_->event_.Wait(
      bandwidth_quality_scaler_->GetBitrateStateUpdateIntervalMs()));
  EXPECT_EQ(0, handler_->adapt_down_event_count_);
  EXPECT_EQ(1, handler_->adapt_up_event_count_);
