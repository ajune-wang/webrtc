  ...
  for (int i = 0; i < ref_data.test_size(); i++) {
    ...
    while (ReadFrame(far_file_, &revframe_) && ReadFrame(near_file_, &frame_)) {
      ...
      if (frame_count % kStatsAggregationFrameNum == 0) {
        ...
        AudioProcessingStats stats1 = apm_->GetStatistics();

        // Echo metrics.
        const float echo_return_loss = stats1.echo_return_loss.value_or(-1.0f);
        const float echo_return_loss_enhancement =
            stats1.echo_return_loss_enhancement.value_or(-1.0f);
        const float residual_echo_likelihood =
            stats1.residual_echo_likelihood.value_or(-1.0f);
        const float residual_echo_likelihood_recent_max =
            stats1.residual_echo_likelihood_recent_max.value_or(-1.0f);
