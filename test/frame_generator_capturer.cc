 ...
 private:
  bool Run() override {
    ...
    if (repeat_interval_ms_ != current_interval_ms) {
      ...
      intended_run_time_ms_ = rtc::TimeMillis();
    }
    repeat_interval_ms_ = current_interval_ms;
    ...
    RTC_DCHECK_GT(intended_run_time_ms_, 0);
    intended_run_time_ms_ += current_interval_ms;
    if (intended_run_time_ms_ < now_ms) {
      int skipped_frames =
          1 + (now_ms - intended_run_time_ms_) / current_interval_ms;
      intended_run_time_ms_ += skipped_frames * current_interval_ms;
