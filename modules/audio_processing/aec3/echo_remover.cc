...
void EchoRemoverImpl::ProcessCapture(
  ...
  FftData X3;
  FftData Y3;
  fft_.PaddedFft(x_aligned, x_old_, Aec3Fft::Window::kSqrtHanning, &X3);
  std::copy(x_aligned.begin(), x_aligned.end(), x_old_.begin());

  fft_.PaddedFft(y0, y_old_, Aec3Fft::Window::kSqrtHanning, &Y3);
  std::copy(y0.begin(), y0.end(), y_old_.begin());

  fft_.PaddedFft(e_main, e_old_, Aec3Fft::Window::kSqrtHanning, &E3);
  std::copy(e_main.begin(), e_main.end(), e_old_.begin());

  coherence_gain_.ComputeGain(E3, X3, Y3, x_aligned, y0, e_main, G_coherence);
