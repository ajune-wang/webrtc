...
std::unique_ptr<AdaptiveAgc> CreateAdaptiveDigitalController(
  ...
  if (config.enabled) {
    return std::make_unique<AdaptiveAgc>(data_dumper, config, sample_rate_hz,
                                         num_channels);
  }
  return nullptr;
}
...
GainController2::GainController2(const Agc2Config& config,
    ...
    : data_dumper_(rtc::AtomicOps::Increment(&instance_count_)),
      fixed_gain_applier_(
          /*gain_db=*/config.fixed_digital.gain_db,
...
void GainController2::Initialize(int sample_rate_hz, int num_channels) {
  ...
  fixed_gain_applier_.Initialize(sample_rate_hz);
...
void GainController2::SetFixedGainDb(float gain_db) {
  if (fixed_gain_applier_.gain_db() != gain_db) {
    // If the gain has changed, reset the limiter to quickly react on any abrupt
    // level changes caused by the fixed gain change.
    limiter_.Reset();
  }
  fixed_gain_applier_.SetGainDb(gain_db);
}
