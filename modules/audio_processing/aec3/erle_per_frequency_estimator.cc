...
ErlePerFrequencyEstimator::ErlePerFrequencyEstimator(float min_erle,
                                                     float max_erle_lf,
                                                     float max_erle_hf)
    : min_erle_(min_erle),
      max_erle_lf_(max_erle_lf),
      max_erle_hf_(max_erle_hf),
      adapt_on_low_render_(EnableAdaptErleLowRender()) {
  Reset();
}

ErlePerFrequencyEstimator::~ErlePerFrequencyEstimator() = default;

void ErlePerFrequencyEstimator::Reset() {
...
void ErlePerFrequencyEstimator::Update(rtc::ArrayView<const float> X2,
...
void ErlePerFrequencyEstimator::Dump(
    const std::unique_ptr<ApmDataDumper>& data_dumper) const {
  data_dumper->DumpRaw("aec3_erle", Erle());
  data_dumper->DumpRaw("aec3_erle_onset", ErleOnsets());
}

void ErlePerFrequencyEstimator::UpdateBands(rtc::ArrayView<const float> X2,
                                            rtc::ArrayView<const float> Y2,
                                            rtc::ArrayView<const float> E2,
                                            size_t start,
                                            size_t stop,
...
void ErlePerFrequencyEstimator::DecreaseErlePerBandForLowRenderSignals() {
...
ErlePerFrequencyEstimator::ErleFreqInstantaneous::ErleFreqInstantaneous() {
  Reset();
}
...
absl::optional<float> ErlePerFrequencyEstimator::ErleFreqInstantaneous::Update(
    float X2,
    float Y2,
    float E2,
    size_t band,
...
void ErlePerFrequencyEstimator::ErleFreqInstantaneous::Reset() {
  Y2_acum_.fill(0.f);
  E2_acum_.fill(0.f);
  X2_acum_.fill(0.0f);
  num_points_.fill(0);
