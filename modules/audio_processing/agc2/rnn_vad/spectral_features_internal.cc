...
void ComputeBandCoefficients(
  ...
  // Fix the first and the last bands (they only got half contribution in the
  // loop above).
  coefficients[0] *= 2.f;
  coefficients[coefficients.size() - 1] *= 2.f;
  // TODO(bugs.webrtc.org/9076): Check if replacing the line above with
  // "coefficients[i] *= 2.f" (*) leads to better VAD accuracy. This alternative
  // works with any sample rate, it accounts for the actual last non-empty band.
  // As it is now, it is assumed that the last band is always |kNumBands| - 1.
