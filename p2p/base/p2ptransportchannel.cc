...
void P2PTransportChannel::AddRemoteCandidate(const Candidate& candidate) {
  ...
  if (candidate.address().IsUnresolvedIP()) {
    rtc::AsyncResolverInterface* resolver = CreateResolver();
...
rtc::AsyncResolverInterface* P2PTransportChannel::CreateResolver() {
...
void P2PTransportChannel::OnCandidateResolved(
  ...
  if (resolver->GetError()) {
    RTC_LOG(LS_WARNING) << "Failed to resolve ICE candidate hostname "
                        << p->candidate_.address().ToString() << " with error "
                        << resolver->GetError();
    resolvers_.erase(p);
    return;
  }
  ...
  RTC_LOG(LS_INFO) << "Resolved ICE candidate hostname "
                   << p->candidate_.address().ToString() << " to "
