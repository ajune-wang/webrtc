...
const cricket::ContentInfo* PeerConnection::FindContentInfo(
  ...
  if (candidate->sdp_mline_index() >= 0) {
    ...
    auto it =
        std::find_if(contents.begin(), contents.end(),
                     [candidate](const cricket::ContentInfo& content_info) {
...
bool PeerConnection::ReadyToUseRemoteCandidate(
  ...
  *valid = true;
  ...
  std::string transport_name = GetTransportName(content->name);
