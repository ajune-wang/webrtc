...
TEST(BitReaderTest, ReadBytesAligned) {
  ...
  BitReader reader(bytes);
  EXPECT_EQ(reader.ReadUInt8(), 0x0Au);
  EXPECT_EQ(reader.ReadUInt8(), 0xBCu);
  EXPECT_EQ(reader.ReadUInt16(), 0xDEF1u);
  EXPECT_EQ(reader.ReadUInt32(), 0x23456789u);
  EXPECT_TRUE(reader.Ok());
}
...
TEST(BitReaderTest, ReadBytesOffset4) {
  ...
  EXPECT_EQ(reader.ReadUInt8(), 0xABu);
  EXPECT_EQ(reader.ReadUInt8(), 0xCDu);
  EXPECT_EQ(reader.ReadUInt16(), 0xEF12u);
  EXPECT_EQ(reader.ReadUInt32(), 0x34567890u);
  EXPECT_TRUE(reader.Ok());
}
...
TEST(BitReaderTest, ReadBytesOffset3) {
  ...
  BitReader reader(bytes);
  reader.Consume(3);
  EXPECT_EQ(reader.ReadUInt8(), 0xFEu);
  EXPECT_EQ(reader.ReadUInt16(), 0xDCBAu);
  EXPECT_EQ(reader.ReadUInt32(), 0x98765432u);
  EXPECT_TRUE(reader.Ok());
...
TEST(BitReaderTest, ReadBits) {
  ...
  BitReader reader(bytes);
  EXPECT_EQ(reader.Read(3), 0b010u);
  EXPECT_EQ(reader.Read(2), 0b01u);
  EXPECT_EQ(reader.Read(7), 0b101'0011u);
  EXPECT_EQ(reader.Read(2), 0b00u);
  EXPECT_EQ(reader.Read(1), 0b1u);
  EXPECT_EQ(reader.Read(1), 0b0u);
  EXPECT_TRUE(reader.Ok());
...
TEST(BitReaderTest, ReadZeroBits) {
  ...
  EXPECT_EQ(reader.Read(0), 0u);
  EXPECT_TRUE(reader.Ok());
}

TEST(BitReaderTest, ReadFromEmptyArray) {
  BitReader reader(rtc::ArrayView<const uint8_t>(nullptr, 0));
...
TEST(BitReaderTest, ReadBits64) {
  ...
  EXPECT_EQ(reader.Read(33), 0x4D32AB5400FFFE01u >> (64 - 33));

  constexpr uint64_t kMask31Bits = (1ull << 32) - 1;
  EXPECT_EQ(reader.Read(31), 0x4D32AB5400FFFE01ull & kMask31Bits);

  EXPECT_EQ(reader.Read(64), 0xABCDEF0123456789ull);
  EXPECT_TRUE(reader.Ok());
