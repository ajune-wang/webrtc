...
class RtpTransportControllerSend final
 ...
 public:
  ...
  // void OnAddPacket(const RtpPacketSendInfo& packet_info);
 ...
 private:
  ...
  // Sets transport sequence number, if transport sequence number header
  // extension is used.
  void ModifyPacketForBweBeforeSend(RtpPacketToSend& packet,
  ...
  uint64_t transport_seq_ RTC_GUARDED_BY(sequence_checker_) = 0;
