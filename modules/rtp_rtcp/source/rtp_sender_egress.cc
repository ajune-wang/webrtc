...
webrtc::RtpSenderEgress::RtpSenderEgress(const RtpRtcp::Configuration& config,
                                         RtpPacketHistory* packet_history,
                                         Clock* clock)
    : ssrc_(config.local_media_ssrc),
      rtx_ssrc_(config.rtx_send_ssrc),
      flexfec_ssrc_(config.flexfec_sender ? absl::optional<uint32_t>(
                                                config.flexfec_sender->ssrc())
                                          : absl::nullopt),
...
void webrtc::RtpSenderEgress::SendPacket(RtpPacketToSend* packet,
...
void webrtc::RtpSenderEgress::ProcessBitrateAndNotifyObservers() {
...
DataRate webrtc::RtpSenderEgress::SendBitrate() const {
  rtc::CritScope cs(&lock_);
  return DataRate::bps(
      total_bitrate_sent_.Rate(clock_->TimeInMilliseconds()).value_or(0));
}

DataRate webrtc::RtpSenderEgress::NackOverheadRate() const {
  rtc::CritScope cs(&lock_);
  return DataRate::bps(
      nack_bitrate_sent_.Rate(clock_->TimeInMilliseconds()).value_or(0));
}

void webrtc::RtpSenderEgress::GetDataCounters(
...
void webrtc::RtpSenderEgress::ForceIncludeSendPacketsInAllocation(
...
bool RtpSenderEgress::HasCorrectSsrc(const RtpPacketToSend& packet) const {
  switch (*packet.packet_type()) {
    ...
    case RtpPacketToSend::Type::kPadding:
      ...
      // RTX stream.
      if (packet.Ssrc() != rtx_ssrc_ && packet.Ssrc() != ssrc_) {
        return false;
      }
      break;
    case RtpPacketToSend::Type::kForwardErrorCorrection:
      // FlexFEC is on separate SSRC, ULPFEC uses media SSRC.
      if (packet.Ssrc() != ssrc_ && packet.Ssrc() != flexfec_ssrc_) {
        return false;
      }
      break;
  }
