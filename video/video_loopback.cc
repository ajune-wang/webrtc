...
void Loopback() {
  ...
  pipe_config.loss_percent = absl::GetFlag(FLAGS_loss_percent);
  pipe_config.avg_burst_loss_length =
      absl::GetFlag(FLAGS_avg_burst_loss_length);
  pipe_config.link_capacity_kbps = absl::GetFlag(FLAGS_link_capacity);
  pipe_config.queue_length_packets = absl::GetFlag(FLAGS_queue_size);
  pipe_config.queue_delay_ms = absl::GetFlag(FLAGS_avg_propagation_delay_ms);
  pipe_config.delay_standard_deviation_ms =
      absl::GetFlag(FLAGS_std_propagation_delay_ms);
  pipe_config.allow_reordering = absl::GetFlag(FLAGS_allow_reordering);

  BitrateConstraints call_bitrate_config;
  call_bitrate_config.min_bitrate_bps = absl::GetFlag(FLAGS_min_bitrate) * 1000;
  call_bitrate_config.start_bitrate_bps =
      absl::GetFlag(FLAGS_start_bitrate) * 1000;
  ...
  params.call = {absl::GetFlag(FLAGS_send_side_bwe),
                 ...
                 0};
  params.video[0] = {
      absl::GetFlag(FLAGS_video),
      static_cast<size_t>(absl::GetFlag(FLAGS_width)),
      static_cast<size_t>(absl::GetFlag(FLAGS_height)),
      absl::GetFlag(FLAGS_fps),
      absl::GetFlag(FLAGS_min_bitrate) * 1000,
      absl::GetFlag(FLAGS_target_bitrate) * 1000,
      absl::GetFlag(FLAGS_min_bitrate) * 1000,
      absl::GetFlag(FLAGS_suspend_below_min_bitrate),
      absl::GetFlag(FLAGS_codec),
      absl::GetFlag(FLAGS_num_temporal_layers),
      absl::GetFlag(FLAGS_selected_tl),
      0,  // No min transmit bitrate.
      absl::GetFlag(FLAGS_use_ulpfec),
      absl::GetFlag(FLAGS_use_flexfec),
      absl::GetFlag(FLAGS_num_streams) < 2,  // Automatic quality scaling.
      absl::GetFlag(FLAGS_clip),
      static_cast<size_t>(absl::GetFlag(FLAGS_capture_device_index))};
  params.audio = {
  ...
  params.analyzer = {"video",
                     ...
                     absl::GetFlag(FLAGS_graph_title)};
  params.config = pipe_config;

  if (absl::GetFlag(FLAGS_num_streams) > 1 &&
      absl::GetFlag(FLAGS_stream0).empty() &&
      absl::GetFlag(FLAGS_stream1).empty()) {
    params.ss[0].infer_streams = true;
  }

  std::vector<std::string> stream_descriptors;
  stream_descriptors.push_back(absl::GetFlag(FLAGS_stream0));
  stream_descriptors.push_back(absl::GetFlag(FLAGS_stream1));
  std::vector<std::string> SL_descriptors;
  SL_descriptors.push_back(absl::GetFlag(FLAGS_sl0));
  SL_descriptors.push_back(absl::GetFlag(FLAGS_sl1));
  SL_descriptors.push_back(absl::GetFlag(FLAGS_sl2));
  VideoQualityTest::FillScalabilitySettings(
      &params, 0, stream_descriptors, absl::GetFlag(FLAGS_num_streams),
      absl::GetFlag(FLAGS_selected_stream),
      absl::GetFlag(FLAGS_num_spatial_layers), absl::GetFlag(FLAGS_selected_sl),
      InterLayerPred(), SL_descriptors);
