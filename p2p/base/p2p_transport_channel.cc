...
void P2PTransportChannel::SetIceConfig(const IceConfig& config) {
  ...
  webrtc::StructParametersParser::Create(
      ...
      //
      "max_initial_pings", &field_trials_.max_initial_pings,
      //
      "initial_pings_before_sort_last",
      &field_trials_.initial_pings_before_sort_last)
  ...
  if (field_trials_.max_initial_pings.has_value()) {
    RTC_LOG(LS_INFO) << "Set max_initial_pings: "
                     << *field_trials_.max_initial_pings;
  }

  if (field_trials_.initial_pings_before_sort_last.has_value()) {
    RTC_LOG(LS_INFO) << "Set initial_pings_before_sort_last: "
                     << *field_trials_.initial_pings_before_sort_last;
  }
...
int P2PTransportChannel::CompareConnectionStates(
  ...
  bool a_too_many_initial_pings = a->check_initial_pings_wo_reponse(
      field_trials_.initial_pings_before_sort_last);
  bool b_too_many_initial_pings = b->check_initial_pings_wo_reponse(
      field_trials_.initial_pings_before_sort_last);

  if (!a_too_many_initial_pings && b_too_many_initial_pings) {
    return a_is_better;
  }
  if (a_too_many_initial_pings && !b_too_many_initial_pings) {
    return b_is_better;
...
bool P2PTransportChannel::IsPingable(const Connection* conn,
  ...
  // until we get one.
  if (conn->check_initial_pings_wo_reponse(field_trials_.max_initial_pings)) {
    return false;
  }
