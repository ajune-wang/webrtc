...
class WebRtcVideoChannel : public VideoMediaChannel,
 ...
 private:
  ...
  std::map<uint32_t, WebRtcVideoReceiveStream*> receive_streams_
      RTC_GUARDED_BY(thread_checker_);
  // We have to be prepared for packets with these ssrcs to arrive after removal
  // of their receive streams because the demuxer lives on the network thread
  // and receive streams are added/removed on on the worker thread. The demuxer
  // criteria id is the key and this id is incremented every time the demuxer
  // criteria is updated.
  std::map<uint32_t, std::set<uint32_t>> recently_removed_receive_streams_
      RTC_GUARDED_BY(thread_checker_);
