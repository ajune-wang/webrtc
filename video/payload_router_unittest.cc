...
TEST(PayloadRouterTest, SendOnOneModule) {
  ...
  EXPECT_CALL(rtp, Sending()).WillOnce(Return(false));
...
TEST(PayloadRouterTest, SendSimulcastSetActive) {
  ...
  EXPECT_CALL(rtp_1, Sending()).WillOnce(Return(false));
  EXPECT_CALL(rtp_2, Sending()).WillOnce(Return(false));
  payload_router.SetActive(true);
...
// Tests how setting individual streams to active affects the overall behavior
// of the payload router. First sets one stream to active and checks that
// outgoing data can be sent on this stream, and checks that no data can be sent
// if both streams are inactive.
TEST(PayloadRouterTest, SendSimulcastSetActiveStreams) {
  NiceMock<MockRtpRtcp> rtp_1;
  NiceMock<MockRtpRtcp> rtp_2;
  std::vector<RtpRtcp*> modules = {&rtp_1, &rtp_2};
  ...
  std::vector<bool> active_streams({true, false});
  // Each module initially isn't be sending.
  EXPECT_CALL(rtp_1, Sending()).WillOnce(Return(false));
  EXPECT_CALL(rtp_2, Sending()).WillOnce(Return(false));
  EXPECT_CALL(rtp_1, SetSendingStatus(true));
  EXPECT_CALL(rtp_1, SetSendingMediaStatus(true));
  EXPECT_CALL(rtp_2, SetSendingStatus(false));
  EXPECT_CALL(rtp_2, SetSendingMediaStatus(false));
  EXPECT_TRUE(payload_router.SetActiveStreams(active_streams));
  ...
  // Setting the streams to the same values returns false. Note that this is
  // dependent on mocking the correct behavior from the rtp modules.
  EXPECT_CALL(rtp_1, Sending()).WillOnce(Return(true));
  EXPECT_CALL(rtp_2, Sending()).WillOnce(Return(false));
  EXPECT_CALL(rtp_1, SetSendingStatus(true));
  EXPECT_CALL(rtp_1, SetSendingMediaStatus(true));
  EXPECT_CALL(rtp_2, SetSendingStatus(false));
  EXPECT_CALL(rtp_2, SetSendingMediaStatus(false));
  EXPECT_FALSE(payload_router.SetActiveStreams(active_streams));

  // Setting both streams to inactive will turn the payload router to inactive.
  active_streams = {false, false};
  EXPECT_CALL(rtp_1, SetSendingStatus(false));
  EXPECT_CALL(rtp_1, SetSendingMediaStatus(false));
  EXPECT_CALL(rtp_2, SetSendingStatus(false));
  EXPECT_CALL(rtp_2, SetSendingMediaStatus(false));
  EXPECT_CALL(rtp_1, Sending()).WillOnce(Return(true));
  EXPECT_CALL(rtp_2, Sending()).WillOnce(Return(false));
  EXPECT_TRUE(payload_router.SetActiveStreams(active_streams));
  // An incoming encoded image will not ask the module if it's sending or to
  // send outgoing data because the payload router is inactive.
...
TEST_F(TestWithForcedFallbackEnabled, PictureIdIsSetForVp8) {
  ...
  EXPECT_CALL(rtp1, Sending()).WillOnce(Return(false));
  EXPECT_CALL(rtp2, Sending()).WillOnce(Return(false));
  router.SetActive(true);
