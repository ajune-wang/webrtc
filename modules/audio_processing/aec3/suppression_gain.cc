...
SuppressionGain::SuppressionGain(const EchoCanceller3Config& config,
  ...
  last_gain_.fill(1.f);
  if (config_.suppressor.use_power_based_nearend_detection) {
    dominant_nearend_detector_ = std::make_unique<PowerBasedNearendDetector>(
        config_.suppressor.power_based_nearend_detection,
        num_capture_channels_);
  } else {
