...
void PacketRouter::RegisterModifyPacketBeforeSendCallback(
    absl::AnyInvocable<void(RtpPacketToSend& packet,
                            const PacedPacketInfo& pacing_info)> callback) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  before_send_callback_ = std::move(callback);
}
...
void PacketRouter::SendPacket(std::unique_ptr<RtpPacketToSend> packet,
  ...
  if (before_send_callback_) {
    before_send_callback_(*packet, cluster_info);
  }
