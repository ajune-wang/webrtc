...
class SSLAdapterTestDummyClient : public sigslot::has_slots<> {
 public:
  ...
  int Connect(const std::string& hostname, const rtc::SocketAddress& address) {
    NLOG(LS_INFO, "Initiating connection with ", address);

    int rv = ssl_adapter_->Connect(address);

    if (rv == 0) {
      NLOG(LS_INFO, "Starting ", GetSSLProtocolName(ssl_mode_),
  ...
  int Send(const std::string& message) {
    NLOG(LS_INFO, "Client sending '", message, "'");
  ...
  void OnSSLAdapterReadEvent(rtc::AsyncSocket* socket) {
    ...
    if (read != -1) {
      ...
      NLOG(LS_INFO, "Client received '", buffer, "'");
...
class SSLAdapterTestDummyServer : public sigslot::has_slots<> {
 public:
  explicit SSLAdapterTestDummyServer(const rtc::SSLMode& ssl_mode,
    ...
    NLOG(LS_INFO, ((ssl_mode_ == rtc::SSL_MODE_DTLS) ? "UDP" : "TCP"),
  ...
  int Send(const std::string& message) {
    ...
    NLOG(LS_INFO, "Server sending '", message, "'");
  ...
  void OnSSLStreamAdapterEvent(rtc::StreamInterface* stream, int sig, int err) {
    if (sig & rtc::SE_READ) {
      ...
      if (r == rtc::SR_SUCCESS) {
        buffer[read] = '\0';
        NLOG(LS_INFO, "Server received '", buffer, "'");
        data_ += buffer;
      }
    }
  }
 ...
 public:
  ...
  void TestHandshake(bool expect_success) {
    ...
    if (expect_success) {
      ...
      NLOG(LS_INFO, GetSSLProtocolName(ssl_mode_), " handshake complete.");
      ...
      NLOG(LS_INFO, GetSSLProtocolName(ssl_mode_), " handshake failed.");
    }
  }
  ...
  void TestTransfer(const std::string& message) {
    ...
    NLOG(LS_INFO, "Transfer complete.");
  }
