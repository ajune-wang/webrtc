...
TEST_P(RtcEventLogEncoderTest, RtcEventAudioPlayout) {
  ...
  // TODO: !!! Avoid repeated SSRCs? Intentionally repeat SSRCs?
  for (size_t i = 0; i < event_count_; ++i) {
...
TEST_P(RtcEventLogEncoderTest, RtcEventBweUpdateDelayBased) {
  std::vector<std::unique_ptr<RtcEventBweUpdateDelayBased>> events(
      event_count_);
  for (size_t i = 0; i < event_count_; ++i) {
    events[i] = gen_.NewBweUpdateDelayBased();
    history_.push_back(events[i]->Copy());
  }
...
TEST_P(RtcEventLogEncoderTest, RtcEventBweUpdateLossBased) {
  ...
  for (size_t i = 0; i < event_count_; ++i) {
    events[i] = gen_.NewBweUpdateLossBased();
    history_.push_back(events[i]->Copy());
  }
...
TEST_P(RtcEventLogEncoderTest, RtcEventRtcpPacketIncoming) {
  ...
  for (size_t i = 0; i < event_count_; ++i) {
    events[i] = gen_.NewRtcpPacketIncoming();
    history_.push_back(events[i]->Copy());
  }
...
TEST_P(RtcEventLogEncoderTest, RtcEventRtcpPacketOutgoing) {
  ...
  for (size_t i = 0; i < event_count_; ++i) {
    events[i] = gen_.NewRtcpPacketOutgoing();
    history_.push_back(events[i]->Copy());
  }
...
TEST_P(RtcEventLogEncoderTest, RtcEventRtpPacketIncoming) {
  using LoggedRtpStreamIncoming = ParsedRtcEventLogNew::LoggedRtpStreamIncoming;
  ...
  // TODO: !!!
  // RtpHeaderExtensionMap extension_map = gen_.NewRtpHeaderExtensionMap();
  RtpHeaderExtensionMap extension_map = gen_.NewRtpHeaderExtensionMap(true);

  // Simulate |event_count_| incoming RTP packets, with SSRCs assigned randomly
  ...
  for (size_t i = 0; i < event_count_; ++i) {
    ...
    // TODO: !!!
    // gen_.NewRtpPacketIncoming(ssrc, extension_map, true);
    history_.push_back(event->Copy());
  ...
  const std::vector<LoggedRtpStreamIncoming>& parsed_incoming_rtp =
      parsed_log_.incoming_rtp_packets_by_ssrc();
  ASSERT_EQ(parsed_incoming_rtp.size(), events_by_ssrc.size());
  ...
  for (auto it = events_by_ssrc.begin(); it != events_by_ssrc.end(); ++it) {
    ...
    auto parsed =
        std::find_if(parsed_incoming_rtp.begin(), parsed_incoming_rtp.end(),
                     [ssrc](const LoggedRtpStreamIncoming& incoming) {
                       return incoming.ssrc == ssrc;
                     });
    ASSERT_NE(parsed, parsed_incoming_rtp.end());
    ASSERT_EQ(parsed->incoming_packets.size(), it->second.size());
    for (size_t i = 0; i < it->second.size(); ++i) {
      test::VerifyLoggedRtpPacketIncoming(*it->second[i],
                                          parsed->incoming_packets[i]);
    }
  }
}

// TODO: !!! Copy latest version from incoming.
// TODO: !!! Dedup.
TEST_P(RtcEventLogEncoderTest, RtcEventRtpPacketOutgoing) {
  using LoggedRtpStreamOutgoing = ParsedRtcEventLogNew::LoggedRtpStreamOutgoing;
  ...
  // Simulate |event_count_| outgoing RTP packets, with SSRCs assigned randomly
  // out of the small pool above.
  std::map<uint32_t, std::vector<std::unique_ptr<RtcEventRtpPacketOutgoing>>>
      events_by_ssrc;
  for (size_t i = 0; i < event_count_; ++i) {
    const uint32_t ssrc = kSsrcPool[prng_.Rand(kSsrcPool.size() - 1)];
    // TODO(terelius): Test extensions too.
  ...
  // Expect as many distinct SSRCs to be parsed, as were simulated.
  const std::vector<LoggedRtpStreamOutgoing>& parsed_outgoing_rtp =
      parsed_log_.outgoing_rtp_packets_by_ssrc();
  ASSERT_EQ(parsed_outgoing_rtp.size(), events_by_ssrc.size());
  ...
  for (auto it = events_by_ssrc.begin(); it != events_by_ssrc.end(); ++it) {
    const uint32_t ssrc = it->first;
    auto parsed =
        std::find_if(parsed_outgoing_rtp.begin(), parsed_outgoing_rtp.end(),
                     [ssrc](const LoggedRtpStreamOutgoing& outgoing) {
                       return outgoing.ssrc == ssrc;
                     });
    ASSERT_NE(parsed, parsed_outgoing_rtp.end());
    ASSERT_EQ(parsed->outgoing_packets.size(), it->second.size());
    for (size_t i = 0; i < it->second.size(); ++i) {
      test::VerifyLoggedRtpPacketOutgoing(*it->second[i],
