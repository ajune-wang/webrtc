...
// Returns a modified version of `config` based on the overriding parameters.
// Disables transient suppression (TS) if `allow_transient_supporessor_usage` is
// false. When `input_volume_controller_config_override` is specified, modifies
// the auto gain control (AGC) configuration by enabling AGC2 and disabling
// AGC1.
AudioProcessing::Config AdjustConfig(
    const AudioProcessing::Config& config,
    bool allow_transient_supporessor_usage,
    const absl::optional<InputVolumeController::Config>&
  ...
  if (!allow_transient_supporessor_usage) {
    adjusted_config.transient_suppression.enabled = false;
  }

  // Override the auto gain control configuration if the AGC1 input volume
  // controller is active and `input_volume_controller_config_override` is
  // specified.
  const bool agc1_input_volume_controller_enabled =
      config.gain_controller1.enabled &&
      (config.gain_controller1.mode ==
           AudioProcessing::Config::GainController1::kAdaptiveAnalog ||
       config.gain_controller1.analog_gain_controller.enabled);
  if (agc1_input_volume_controller_enabled &&
    ...
    if (hybrid_agc_config_detected == full_agc1_config_detected) {
      RTC_LOG(LS_ERROR)
          << "Unexpected AGC config: one and only one input volume controller "
             "must be enabled. Using the unadjusted config.";
    } else if (config.gain_controller2.input_volume_controller.enabled) {
      RTC_LOG(LS_ERROR)
          << "Unexpected AGC config: the AGC2 input volume controller must be "
             "disabled. Using the unadjusted config.";
    } else {
...
AudioProcessingImpl::AudioProcessingImpl(
    ...
    : data_dumper_(new ApmDataDumper(instance_count_.fetch_add(1) + 1)),
      ...
      allow_transient_supporessor_usage_(
          field_trial::IsEnabled("WebRTC-ApmTransientSuppressorUnkillSwitch")),
...
void AudioProcessingImpl::ApplyConfig(const AudioProcessing::Config& config) {
  ...
  const auto adjusted_config =
      AdjustConfig(config, allow_transient_supporessor_usage_,
                   input_volume_controller_config_override_);
