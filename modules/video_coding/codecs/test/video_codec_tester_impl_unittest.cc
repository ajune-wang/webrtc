...
TEST(VideoCodecTesterImplTest, LimitedTaskQueue) {
  // TODO(ssilkin): Implement.
}
...
TEST_P(VideoCodecTesterImplPacingTest, PaceEncode) {
  ...
  std::vector<int> frame_capture_delay_ms = std::get<2>(GetParam());
  std::vector<int> expected_frame_start_ms = std::get<3>(GetParam());

  size_t num_frames = frame_timestamp_ms.size();
  size_t frame_num = 0;
  ...
  EXPECT_CALL(*video_source, PullFrame).WillRepeatedly(Invoke([&]() mutable {
    ...
    SleepMs(frame_capture_delay_ms[frame_num]);
    uint32_t timestamp_rtp = frame_timestamp_ms[frame_num] * k90kHz.hertz() /
  ...
  for (size_t i = 0; i < fs.size(); ++i) {
    ...
    EXPECT_NEAR(encode_start_ms, expected_frame_start_ms[i], 10);
  }
}
...
TEST_P(VideoCodecTesterImplPacingTest, PaceDecode) {
  ...
  for (size_t i = 0; i < fs.size(); ++i) {
    int decode_start_ms = (fs[i].decode_start_ns - fs[0].decode_start_ns) /
                          rtc::kNumNanosecsPerMillisec;
    EXPECT_NEAR(decode_start_ms, expected_frame_start_ms[i], 10);
  }
}
