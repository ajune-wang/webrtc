...
CaptureInputPin::CaptureInputPin(CaptureSinkFilter* filter, LPCWSTR name) {
...
const wchar_t* CaptureInputPin::Name() const {
  RTC_DCHECK_RUN_ON(&main_checker_);
  return info_.achName;
}
...
STDMETHODIMP_(ULONG) CaptureInputPin::AddRef() {
  return 1;  // BUG - the filter currently uses new/delete.
}
...
CaptureSinkFilter::CaptureSinkFilter(const TCHAR* object_name,
    ...
    : CBaseFilter(object_name, unknown, nullptr /*&lock_*/, CLSID_SINKFILTER),
      input_pin_(new CaptureInputPin(this, L"VideoCapture")),
...
STDMETHODIMP CaptureSinkFilter::JoinFilterGraph(IFilterGraph* graph,
  ...
  if (graph) {
    HRESULT hr = graph->QueryInterface(IID_IMediaEventSink,
