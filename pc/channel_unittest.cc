...
TEST_F(VideoChannelSingleThreadTest, TestSetLocalOfferWithPacketization) {
  ...
  cricket::VideoCodec vp9Codec(98, "VP9");
  vp9Codec.packetization = cricket::kPacketizationParamRaw;
  cricket::VideoContentDescription video;
  video.set_codecs({kVp8Codec, vp9Codec});
  ...
  EXPECT_TRUE(media_channel1_->recv_codecs()[1].Matches(vp9Codec));
...
TEST_F(VideoChannelSingleThreadTest, TestSetRemoteOfferWithPacketization) {
  ...
  EXPECT_TRUE(media_channel1_->send_codecs()[1].Matches(vp9Codec));
