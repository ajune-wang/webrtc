...
class AecState {
 public:
  ...
  absl::optional<int> InternalDelay() const {
    return delay_state_.InternalDelay();
  }
 ...
 private:
  ...
  // Class for choosing the direct-path delay relative to the beginning of the filter, as well as any other data related to the delay used within AecState.
  class FilterDelay {
   public:
    explicit FilterDelay(const EchoCanceller3Config& config);

    // Returns whether an external delay has been reported to the AecState (from the delay estimator).
  ...
  // Class for detecting and toggling the transparent mode which causes the suppressor to apply no suppression.
  class TransparentMode {
   public:
    ...
    bool Active() const { return transparent_mode_; }

    // Resets the state of the detector.
    void Reset();

    // Updates the detection descision based on new data.
  ...
  // Class for analyzing how well the linear filter is, and can be expected to, perform on the current signals. The intention of this is for using to select the echo suppression functionality as well as the input to the echo suppressor.
  class FilteringQualityAnalyzer {
   public:
    explicit FilteringQualityAnalyzer(const EchoCanceller3Config& config);

    // Returns whether the the linear filter is can be used for the echo canceller output.
  ...
  // Class for detecting whether the echo is to be consideret to be saturated. The purpose of this is to allow customized behavior in the echo suppressor for when the echo is saturated.
