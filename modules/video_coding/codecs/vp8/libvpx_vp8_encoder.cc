...
bool LibvpxVp8Encoder::PrepareImage(
  ...
  if (frame->type() == VideoFrameBuffer::Type::kNative) {
    ...
    if (!mapped_frame) {
      // |frame| is not mappable to a supported format, convert to I420 and
  ...
  for (size_t i = 1; i < encoders_.size(); ++i) {
    const int kRawImageIndex = i - 1;

    auto scaled_frame = frame->Scale(raw_images_[kRawImageIndex].d_w,
                                     raw_images_[kRawImageIndex].d_h);
    ...
    switch (scaled_frame->type()) {
      case VideoFrameBuffer::Type::kI420: {
        const I420BufferInterface* i420_buffer = scaled_frame->GetI420();
        RTC_DCHECK(i420_buffer);

        raw_images_[kRawImageIndex].planes[VPX_PLANE_Y] =
            const_cast<uint8_t*>(i420_buffer->DataY());
        raw_images_[kRawImageIndex].planes[VPX_PLANE_U] =
            const_cast<uint8_t*>(i420_buffer->DataU());
        raw_images_[kRawImageIndex].planes[VPX_PLANE_V] =
            const_cast<uint8_t*>(i420_buffer->DataV());

        raw_images_[kRawImageIndex].stride[VPX_PLANE_Y] =
            i420_buffer->StrideY();
        raw_images_[kRawImageIndex].stride[VPX_PLANE_U] =
            i420_buffer->StrideU();
        raw_images_[kRawImageIndex].stride[VPX_PLANE_V] =
            i420_buffer->StrideV();
        break;
      }
      case VideoFrameBuffer::Type::kNV12: {
        const NV12BufferInterface* nv12_buffer = scaled_frame->GetNV12();
        ...
        raw_images_[kRawImageIndex].planes[VPX_PLANE_V] =
            raw_images_[kRawImageIndex].planes[VPX_PLANE_U] + 1;
        raw_images_[kRawImageIndex].stride[VPX_PLANE_Y] =
            nv12_buffer->StrideY();
        raw_images_[kRawImageIndex].stride[VPX_PLANE_U] =
