...
TEST(VideoCodecAnalyzerTest, EncodeStarted_createsFrameStats) {
...
TEST(PerfAnalyzerScalabilityTest, EncodeFinished_updatesFrameStats) {
  VideoCodecAnalyzer analyzer(
      /*frame_reader=*/nullptr);
  analyzer.StartEncode(CreateVideoFrame(kTimestamp));
  ...
  EXPECT_TRUE(fs[0].encoding_successful);
  EXPECT_EQ(fs[0].target_bitrate_kbps, 1024u);
  EXPECT_EQ(fs[0].target_framerate_fps, 24);
}

TEST(VideoCodecAnalyzerTest, DecodeStarted_noFrameStats_createsFrameStats) {
  VideoCodecAnalyzer analyzer(
      /*frame_reader=*/nullptr);
...
TEST(VideoCodecAnalyzerTest, DecodeStarted_frameStatsExists_reusesFrameStats) {
  VideoCodecAnalyzer analyzer(
      /*frame_reader=*/nullptr);
  analyzer.StartEncode(CreateVideoFrame(kTimestamp));
...
TEST(VideoCodecAnalyzerTest, DecodeFinished_updatesFrameStats) {
  VideoCodecAnalyzer analyzer(
      /*frame_reader=*/nullptr);
  analyzer.StartDecode(CreateEncodedImage(kTimestamp, kSpatialIdx));
...
TEST(VideoCodecAnalyzerTest, DecodeFinished_computesPsnr) {
