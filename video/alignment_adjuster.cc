...
double RoundToMultiple(int multiple,
  ...
  for (auto& layer : config->simulcast_layers) {
    ...
    for (int i = std::max(
             static_cast<int>(multiple / layer.scale_resolution_down_by), 1);
         i <= multiple; ++i) {
...
int GetAdjustedAlignment(int requested_alignment,
                         VideoEncoderConfig* config,
                         int best_multiple) {
  for (const auto& layer : config->simulcast_layers) {
    int i = best_multiple / layer.scale_resolution_down_by;
    if (i % requested_alignment != 0)
      return requested_alignment * best_multiple;
  }
...
// A / K integer <= 16
//
// Solution forces X = A/K then chooses closest S'[i] in a form X/j for some j.
...
int AlignmentAdjuster::GetAlignmentAndMaybeAdjustScaleFactors(
  ...
  const int kMaxMultiple = std::max(16 / requested_alignment, 1);
  ...
  // Decide on common multiple (range: [1, kMaxMultiple]) to use.
  double min_diff = std::numeric_limits<double>::max();
  int best_multiple = 1;
  for (int multiple = 1; multiple <= kMaxMultiple; ++multiple) {
  ...
  RoundToMultiple(best_multiple, config, /*update_config=*/true);

  return GetAdjustedAlignment(requested_alignment, config, best_multiple);
}
}  // namespace webrtc
