...
class SimulatedTimeControllerImpl : public TaskQueueFactory,
 ...
 public:
  ...
  // Runs pending tasks on all task queues but the currently executing queue.
  ...
  void SendTask(TaskQueueBase* task_queue, std::unique_ptr<QueuedTask> task);

 private:
  std::vector<SimulatedSequenceRunner*> GetPending(Timestamp current_time);
  bool HasRunner(SimulatedSequenceRunner* runner);
