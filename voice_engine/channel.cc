...
bool Channel::SendRtp(const uint8_t* data,
  ...
  if (_transportPtr == NULL) {
    RTC_LOG(LS_ERROR) << "Channel::SendPacket() failed to send RTP packet "
...
int32_t Channel::Init() {
  ...
  if (_moduleProcessThreadPtr == NULL) {
    RTC_LOG(LS_ERROR) << "Channel::Init() must call SetEngineInformation() "
                      << "first";
    return -1;
  }
...
void Channel::Terminate() {
  ...
  if (audio_coding_->RegisterTransportCallback(NULL) == -1) {
    RTC_LOG(LS_WARNING) << "Terminate() failed to de-register transport "
...
int Channel::SetSendTelephoneEventPayloadType(int payload_type,
  ...
  if (_rtpRtcpModule->RegisterSendPayload(codec) != 0) {
    ...
    if (_rtpRtcpModule->RegisterSendPayload(codec) != 0) {
      RTC_LOG(LS_ERROR) << "SetSendTelephoneEventPayloadType() failed to "
                        << "register send payload type";
      return -1;
    }
  }
  return 0;
...
int Channel::GetRTPStatistics(CallStatistics& stats) {
  ...
  if (_rtpRtcpModule->DataCountersRTP(&bytesSent, &packetsSent) != 0) {
    RTC_LOG(LS_WARNING) << "GetRTPStatistics() failed to retrieve RTP "
                        << "datacounters => output will not be complete";
  }
...
int Channel::SetMinimumPlayoutDelay(int delayMs) {
  ...
  if (audio_coding_->SetMinimumPlayoutDelay(delayMs) != 0) {
    RTC_LOG(LS_ERROR) << "SetMinimumPlayoutDelay() failed to set min "
                      << "playout delay";
    return -1;
  }
  return 0;
