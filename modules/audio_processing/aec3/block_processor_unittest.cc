...
// Verifies that external delay estimator delays are applied correctly to the
// render delay buffer. Verification is done by checking the input to the echo
// remover.
TEST(BlockProcessor, ExternalDelayEstimatesMatchTrueDelay) {
  ...
  // - 10 render calls, beginning to populate the buffer,
