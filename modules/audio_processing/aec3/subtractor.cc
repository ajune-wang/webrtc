...
void Subtractor::Process(const RenderBuffer& render_buffer,
  ...
  constexpr float kConvergenceThreshold = 100 * 100 * kBlockSize;
  main_filter_converged_ = e2_main < 0.1 * y2 && y2 > kConvergenceThreshold;
  shadow_filter_converged_ = e2_shadow < 0.3 * y2 && y2 > kConvergenceThreshold;
  main_filter_once_converged_ =
      main_filter_once_converged_ || main_filter_converged_;
  main_filter_diverged_ = e2_main > 2 * y2 && y2 > 100 * 100 * kBlockSize;
