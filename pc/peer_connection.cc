...
void PeerConnection::LocalUfragsToReplace::SetRestartIceDescriptions(
  ...
  if (current_local_description) {
    current_local_description_ = CreateSessionDescription(
        current_local_description->GetType(),
        current_local_description->session_id(),
        current_local_description->session_version(),
        current_local_description->description()->Clone());
  } else {
    current_local_description_.reset();
  }
  if (pending_local_description) {
    pending_local_description_ = CreateSessionDescription(
        pending_local_description->GetType(),
        pending_local_description->session_id(),
        pending_local_description->session_version(),
        pending_local_description->description()->Clone());
  } else {
    pending_local_description_.reset();
  }
}

bool PeerConnection::LocalUfragsToReplace::HasRestartIceDescriptions() const {
  return current_local_description_ || pending_local_description_;
}

bool PeerConnection::LocalUfragsToReplace::SatisfiesIceRestart(
    const SessionDescriptionInterface& description) {
  return (current_local_description_ &&
          ContainsSharedIceCredentials(current_local_description_.get(),
                                       &description)) ||
         (pending_local_description_ &&
          ContainsSharedIceCredentials(pending_local_description_.get(),
                                       &description));
}
...
void PeerConnection::RestartIce() {
  RTC_DCHECK_RUN_ON(signaling_thread());
  local_ufrags_to_replace_.SetRestartIceDescriptions(
...
RTCError PeerConnection::ApplyLocalDescription(
  ...
  bool satisfies_ice_restart =
      local_ufrags_to_replace_.SatisfiesIceRestart(*desc);
  ...
  if (type == SdpType::kAnswer && satisfies_ice_restart) {
    local_ufrags_to_replace_.SetRestartIceDescriptions(nullptr, nullptr);
  }
...
void PeerConnection::GetOptionsForOffer(
  ...
  bool ice_restart = offer_answer_options.ice_restart ||
                     local_ufrags_to_replace_.HasRestartIceDescriptions();
...
bool PeerConnection::CheckIfNegotiationIsNeeded() {
  ...
  if (local_ufrags_to_replace_.HasRestartIceDescriptions()) {
    return true;
  }
