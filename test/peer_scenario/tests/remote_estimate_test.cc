...
TEST(RemoteEstimateEndToEnd, OfferedCapabilityIsInAnswer) {
  ...
  auto* send = s.CreateClient(PeerScenarioClient::Config());
  auto* ret = s.CreateClient(PeerScenarioClient::Config());

  auto send_link = {s.net()->NodeBuilder().Build().node};
  auto ret_link = {s.net()->NodeBuilder().Build().node};

  s.net()->CreateRoute(send->ep(), send_link, ret->ep());
  s.net()->CreateRoute(ret->ep(), ret_link, send->ep());

  auto signaling = s.ConnectSignaling(send, ret, send_link, ret_link);
  signaling.StartIceSignaling();

  send->CreateVideo("VIDEO", PeerScenarioClient::VideoSendTrackConfig());
  ...
  EXPECT_TRUE(s.Wait(&offer_exchange_done));
}
