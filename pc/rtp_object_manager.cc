...
#include "pc/rtp_object_manager.h"
...
RtpObjectManager::RtpObjectManager(
    std::function<bool()> is_unified_plan,
    rtc::Thread* signaling_thread,
    rtc::Thread* worker_thread,
    cricket::ChannelManager* channel_manager,
    UsagePattern* usage_pattern,
    std::function<PeerConnectionObserver*()> observer_getter,
    std::function<StatsCollectorInterface*()> stats_getter,
...
void RtpObjectManager::OnNegotiationNeeded() {
  on_negotiation_needed_();
}

// Function that returns the currently valid observer
PeerConnectionObserver* RtpObjectManager::Observer() const {
  // Missing
  return observer_getter_();
}

cricket::VoiceMediaChannel* RtpObjectManager::voice_media_channel() const {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
cricket::VideoMediaChannel* RtpObjectManager::video_media_channel() const {
  RTC_DCHECK_RUN_ON(signaling_thread());
  RTC_DCHECK(!IsUnifiedPlan());
...
RtpObjectManager::AddTrackPlanB(
    rtc::scoped_refptr<MediaStreamTrackInterface> track,
    const std::vector<std::string>& stream_ids) {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
RTCErrorOr<rtc::scoped_refptr<RtpSenderInterface>>
RtpObjectManager::AddTrackUnifiedPlan(
    rtc::scoped_refptr<MediaStreamTrackInterface> track,
    const std::vector<std::string>& stream_ids) {
...
RtpObjectManager::CreateSender(
  ...
  if (media_type == cricket::MEDIA_TYPE_AUDIO) {
    ...
    sender = RtpSenderProxyWithInternal<RtpSenderInternal>::Create(
        signaling_thread(),
        AudioRtpSender::Create(worker_thread(), id, stats_getter_(), this));
...
RtpObjectManager::CreateReceiver(cricket::MediaType media_type,
...
RtpObjectManager::CreateAndAddTransceiver(
    rtc::scoped_refptr<RtpSenderProxyWithInternal<RtpSenderInternal>> sender,
    rtc::scoped_refptr<RtpReceiverProxyWithInternal<RtpReceiverInternal>>
        receiver) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  ...
  transceiver->internal()->SignalNegotiationNeeded.connect(
      this, &RtpObjectManager::OnNegotiationNeeded);
  return transceiver;
}

rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>
RtpObjectManager::FindFirstTransceiverForAddedTrack(
    rtc::scoped_refptr<MediaStreamTrackInterface> track) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  RTC_DCHECK(track);
...
RtpObjectManager::GetSendersInternal() const {
  RTC_DCHECK_RUN_ON(signaling_thread());
  std::vector<rtc::scoped_refptr<RtpSenderProxyWithInternal<RtpSenderInternal>>>
      all_senders;
...
std::vector<
    rtc::scoped_refptr<RtpReceiverProxyWithInternal<RtpReceiverInternal>>>
RtpObjectManager::GetReceiversInternal() const {
  RTC_DCHECK_RUN_ON(signaling_thread());
  std::vector<
      rtc::scoped_refptr<RtpReceiverProxyWithInternal<RtpReceiverInternal>>>
      all_receivers;
...
rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>
RtpObjectManager::GetAudioTransceiver() const {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>
RtpObjectManager::GetVideoTransceiver() const {
  RTC_DCHECK_RUN_ON(signaling_thread());
  // This method only works with Plan B SDP, where there is a single
  // audio/video transceiver.
  RTC_DCHECK(!IsUnifiedPlan());
...
void RtpObjectManager::AddAudioTrack(AudioTrackInterface* track,
...
void RtpObjectManager::RemoveAudioTrack(AudioTrackInterface* track,
                                        MediaStreamInterface* stream) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  RTC_DCHECK(!IsUnifiedPlan());
  auto sender = FindSenderForTrack(track);
...
void RtpObjectManager::AddVideoTrack(VideoTrackInterface* track,
                                     MediaStreamInterface* stream) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  RTC_DCHECK(track);
  RTC_DCHECK(stream);
...
void RtpObjectManager::RemoveVideoTrack(VideoTrackInterface* track,
                                        MediaStreamInterface* stream) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  RTC_DCHECK(!IsUnifiedPlan());
  auto sender = FindSenderForTrack(track);
...
rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>
RtpObjectManager::GetFirstAudioTransceiver() const {
  RTC_DCHECK_RUN_ON(signaling_thread());
  for (auto transceiver : transceivers_.List()) {
    if (transceiver->media_type() == cricket::MEDIA_TYPE_AUDIO) {
      return transceiver;
...
void RtpObjectManager::CreateAudioReceiver(
    MediaStreamInterface* stream,
...
void RtpObjectManager::CreateVideoReceiver(
    MediaStreamInterface* stream,
    const RtpSenderInfo& remote_sender_info) {
  std::vector<rtc::scoped_refptr<MediaStreamInterface>> streams;
  streams.push_back(rtc::scoped_refptr<MediaStreamInterface>(stream));
...
RtpObjectManager::RemoveAndStopReceiver(
    const RtpSenderInfo& remote_sender_info) {
...
void RtpObjectManager::OnRemoteSenderAdded(const RtpSenderInfo& sender_info,
                                           MediaStreamInterface* stream,
                                           cricket::MediaType media_type) {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
void RtpObjectManager::OnRemoteSenderRemoved(const RtpSenderInfo& sender_info,
                                             MediaStreamInterface* stream,
                                             cricket::MediaType media_type) {
  RTC_DCHECK_RUN_ON(signaling_thread());
...
void RtpObjectManager::OnLocalSenderAdded(const RtpSenderInfo& sender_info,
                                          cricket::MediaType media_type) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  RTC_DCHECK(!IsUnifiedPlan());
...
void RtpObjectManager::OnLocalSenderRemoved(const RtpSenderInfo& sender_info,
                                            cricket::MediaType media_type) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  auto sender = FindSenderById(sender_info.sender_id);
  if (!sender) {
...
std::vector<RtpSenderInfo>* RtpObjectManager::GetRemoteSenderInfos(
    cricket::MediaType media_type) {
...
std::vector<RtpSenderInfo>* RtpObjectManager::GetLocalSenderInfos(
    cricket::MediaType media_type) {
  RTC_DCHECK(media_type == cricket::MEDIA_TYPE_AUDIO ||
             media_type == cricket::MEDIA_TYPE_VIDEO);
...
const RtpSenderInfo* RtpObjectManager::FindSenderInfo(
...
rtc::scoped_refptr<RtpSenderProxyWithInternal<RtpSenderInternal>>
RtpObjectManager::FindSenderForTrack(MediaStreamTrackInterface* track) const {
  RTC_DCHECK_RUN_ON(signaling_thread());
  for (const auto& transceiver : transceivers_.List()) {
...
rtc::scoped_refptr<RtpSenderProxyWithInternal<RtpSenderInternal>>
RtpObjectManager::FindSenderById(const std::string& sender_id) const {
  RTC_DCHECK_RUN_ON(signaling_thread());
  for (const auto& transceiver : transceivers_.List()) {
    for (auto sender : transceiver->internal()->senders()) {
...
rtc::scoped_refptr<RtpReceiverProxyWithInternal<RtpReceiverInternal>>
RtpObjectManager::FindReceiverById(const std::string& receiver_id) const {
  RTC_DCHECK_RUN_ON(signaling_thread());
  for (const auto& transceiver : transceivers_.List()) {
