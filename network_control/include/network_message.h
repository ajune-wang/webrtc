 ...
 public:
  ...
  class Receiver {
   public:
    ...
    void ReceiveFrom(Producer* prod) { prod->Connect(this); }
    void EndReceiveFrom(Producer* prod) { prod->Disconnect(this); }
  ...
  // Helper functions, useful since they will coerce the type
  static void SendMessage(MSG_T msg, Observer* o) { o->OnMessage(msg); }

  static void Connect(Producer* p, Receiver* r) { p->Connect(r); }
 ...
 public:
  ...
  class SimpleJunction : public Junction {
   public:
    ...
    void Disconnect() { broadcaster_.DisconnectAll(); }
  ...
  static std::unique_ptr<Junction> CreateJunction(
      QueueTaskRunner* optional_queue,
      rtc::CriticalSection* alternative_lock) {
    if (optional_queue != nullptr)
      return rtc::MakeUnique<TaskQueueJunction>(optional_queue);
    else if (alternative_lock != nullptr)
      return rtc::MakeUnique<LockedJunction>(alternative_lock);
    else
      return rtc::MakeUnique<SimpleJunction>();
  }
