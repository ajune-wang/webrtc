...
class VideoDecoderWrapper : public VideoDecoder {
 ...
 private:
  ...
  bool InitDecodeInternal(JNIEnv* jni) RTC_RUN_ON(decoder_thread_checker_);
  ...
  // Initialized on Init and immutable after that.
  Config codec_settings_ RTC_GUARDED_BY(decoder_thread_checker_);
