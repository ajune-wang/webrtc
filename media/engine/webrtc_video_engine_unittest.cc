...
#include "media/base/fake_video_capturer.h"  // TODO(nisse): Delete.
...
using cricket::FakeVideoCapturerWithTaskQueue;  // TODO(nisse): Delete.
using testing::Field;
...
#if 0
  webrtc::test::FrameForwarder frame_forwarder;
  ASSERT_TRUE(channel_->SetVideoSend(last_ssrc_, nullptr, &frame_forwarder));
#endif
...
#if 0
  frame_forwarder.IncomingCapturedFrame(frame_source_.GetFrame(
      1284, 724, webrtc::VideoRotation::kVideoRotation_0,
      rtc::kNumMicrosecsPerSec / 30));
#endif
...
TEST_F(WebRtcVideoChannelTest, PreviousAdaptationDoesNotApplyToScreenshare) {
  ...
  frame_forwarder.IncomingCapturedFrame(
      frame_source_.GetFrame(1284, 724, webrtc::VideoRotation::kVideoRotation_0,
                             rtc::kNumMicrosecsPerSec / 30));
#endif
...
#if 0
    wants.max_pixel_count = 1280 * 720 - 1;
#endif
...
#if 0
    EXPECT_EQ(send_stream->GetLastWidth(), 1280);
    EXPECT_EQ(send_stream->GetLastHeight(), 720);
#endif
...
#if 0
    EXPECT_LT(send_stream->GetLastWidth(), 1280);
    EXPECT_LT(send_stream->GetLastHeight(), 720);
#endif
