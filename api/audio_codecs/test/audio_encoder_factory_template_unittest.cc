...
TEST(AudioEncoderFactoryTemplateTest, NoEncoderTypes) {
  ...
  EXPECT_THAT(
      factory->Create(env, /*payload_type=*/17, /*format=*/{"bar", 16'000, 1},
...
TEST(AudioEncoderFactoryTemplateTest, OneEncoderType) {
  ...
  EXPECT_THAT(factory->Create(env, 17, {"bar", 16'000, 1}, absl::nullopt),
              IsNull());
  EXPECT_THAT(factory->Create(env, 17, {"bogus", 8'000, 1}, absl::nullopt),
