...
class TaskQueueFactory {
 public:
  ...
  // TODO(danilchap): Move Priority defintion to this class when it doesn't need
  // to depend on the rtc::TaskQueue.
  using Priority = rtc::TaskQueue::Priority;

  virtual ~TaskQueueFactory() = default;
  TaskQueuePtr CreateTaskQueue(const char* name) const {
    return CreateTaskQueue(name, Priority::NORMAL);
  }
