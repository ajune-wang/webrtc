...
void AudioDeviceIOS::SetupAudioBuffersForActiveAudioSession() {
  ...
  // Crash reports indicates that oit can happen in rare cases that the reported
  // sample rate is less than or equal to zero. If that happens and if a valid
  // sample rate has already be set during initilization, the best guess we can
  // do is to reuse the current sample rate.
  if (sample_rate <= 0.0 && playout_parameters_.sample_rate() > 0) {
