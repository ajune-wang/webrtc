...
AdaptiveFirFilter::AdaptiveFirFilter(size_t max_size_partitions,
    ...
    : data_dumper_(data_dumper),
      ...
      last_size_partitions_(initial_size_partitions),
...
void AdaptiveFirFilter::SetSizePartitions(size_t size, bool immediate_effect) {
  ...
  if (immediate_effect) {
    current_size_partitions_ = last_size_partitions_ = target_size_partitions_;
...
void AdaptiveFirFilter::UpdateSize() {
  if (size_change_counter_ > 0) {
    ...
    current_size_partitions_ =
        change_factor * last_size_partitions_ +
        one_minus_change_factory * target_size_partitions_;
    ResetFilterBuffersToCurrentSize();
  } else {
    current_size_partitions_ = last_size_partitions_ = target_size_partitions_;
  }
