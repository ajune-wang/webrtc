...
uint32_t AudioSendStream::OnBitrateUpdated(BitrateAllocationUpdate update) {
  ...
  // overhead. It's hard to write DCHECk for overhead, because even if we store
  // it, it maybe reconfigured before we get callback.
  RTC_DCHECK_GE(update.target_bitrate.bps<int>(), config_.min_bitrate_bps);

  // Bitrate allocator must be already configured.
  RTC_DCHECK_GT(max_bitrate_with_overhead_bps_, 0);

  // The bitrate allocator might allocate an higher than max configured bitrate
  // if there is room, to allow for, as example, extra FEC. Ignore that for now.
  const DataRate max_bitrate = DataRate::bps(max_bitrate_with_overhead_bps_);
...
void AudioSendStream::ConfigureBitrateObserver(
  ...
  worker_queue_->PostTask([&] {
    ...
    RTC_DCHECK_GT(transport_overhead_per_packet_bytes, 0);

    if (send_side_bwe_with_overhead_ && encoder_min_frame_length_ms_ > 0 &&
        encoder_max_frame_length_ms_ > 0) {
      // Field trial SendSideBweWithOverhead.
      //
      ...
      if (send_side_bwe_with_overhead_option_min_max_) {
        ...
        const int min_overhead_bps = transport_overhead_per_packet_bytes * 8 *
                                     1000 / encoder_max_frame_length_ms_;

        const int max_overhead_bps = transport_overhead_per_packet_bytes * 8 *
        ...
        constexpr int kIpV4OverheadBytes = 20;
        constexpr int kUdpOverheadBytes = 8;
        constexpr int kSrtpOverheadBytes = 10;
        constexpr int kRtpOverheadBytes = 12;
    ...
    max_bitrate_with_overhead_bps_ = max_bitrate_bps;
