...
MATCHER(IsNullopt, "") {
  return Matches(Eq(absl::nullopt))(arg);
}
...
TEST(FrameBuffer3Test, RejectInvalidRefs) {
  ...
  EXPECT_THAT(buffer.LastContinuousFrameId(), IsNullopt());
  ...
  EXPECT_THAT(buffer.LastContinuousFrameId(), Optional(1));
}

TEST(FrameBuffer3Test, LastContinuous) {
  FrameBuffer buffer(/*max_frame_slots=*/10, /*max_decode_history=*/100);
  EXPECT_THAT(buffer.LastContinuousFrameId(), Eq(absl::nullopt));
  EXPECT_THAT(buffer.LastContinuousTemporalUnitFrameId(), IsNullopt());
  ...
  EXPECT_THAT(buffer.LastContinuousFrameId(), Optional(2));
  EXPECT_THAT(buffer.LastContinuousTemporalUnitFrameId(), Optional(2));
}
...
TEST(FrameBuffer3Test, LastContinuousFrameReordering) {
  ...
  EXPECT_THAT(buffer.LastContinuousFrameId(), Optional(3));
}
...
TEST(FrameBuffer3Test, LastContinuousTemporalUnitReordering) {
  ...
  buffer.InsertFrame(Builder().Time(10).Id(2).Refs({1}).AsLast().Build());
  EXPECT_THAT(buffer.LastContinuousTemporalUnitFrameId(), Optional(4));
}
...
TEST(FrameBuffer3Test, NextDecodable) {
  ...
  EXPECT_THAT(buffer.NextDecodableTemporalUnitRtpTimestamp(),
              Eq(absl::nullopt));
  buffer.InsertFrame(Builder().Time(10).Id(1).AsLast().Build());
  EXPECT_THAT(buffer.NextDecodableTemporalUnitRtpTimestamp(), Optional(10));
}
...
TEST(FrameBuffer3Test, AdvanceNextDecodableOnExtraction) {
  ...
  EXPECT_THAT(buffer.ExtractNextTemporalUnit(), ElementsAre(FrameWithId(1)));
  EXPECT_THAT(buffer.NextDecodableTemporalUnitRtpTimestamp(), Optional(20));
  EXPECT_THAT(buffer.ExtractNextTemporalUnit(), ElementsAre(FrameWithId(2)));
  EXPECT_THAT(buffer.NextDecodableTemporalUnitRtpTimestamp(), Optional(30));
  EXPECT_THAT(buffer.ExtractNextTemporalUnit(), ElementsAre(FrameWithId(3)));
}
...
TEST(FrameBuffer3Test, AdvanceLastDecodableOnExtraction) {
  ...
  EXPECT_THAT(buffer.LastDecodableTemporalUnitRtpTimestamp(), Optional(10));

  EXPECT_THAT(buffer.ExtractNextTemporalUnit(), ElementsAre(FrameWithId(1)));
  EXPECT_THAT(buffer.LastDecodableTemporalUnitRtpTimestamp(), Optional(30));
}
...
TEST(FrameBuffer3Test, KeyframeClearsFullBuffer) {
  ...
  EXPECT_THAT(buffer.LastContinuousFrameId(), Optional(5));
  ...
  EXPECT_THAT(buffer.LastContinuousFrameId(), Optional(7));
}

TEST(FrameBuffer3Test, DropNextTemporalUnit) {
  FrameBuffer buffer(/*max_frame_slots=*/10, /*max_decode_history=*/100);
  buffer.InsertFrame(Builder().Time(10).Id(1).AsLast().Build());
  buffer.InsertFrame(Builder().Time(20).Id(2).Refs({1}).AsLast().Build());
  buffer.InsertFrame(Builder().Time(30).Id(3).Refs({1}).AsLast().Build());
...
TEST(FrameBuffer3Test, InterleavedStream) {
  ...
  EXPECT_THAT(buffer.ExtractNextTemporalUnit(), ElementsAre(FrameWithId(3)));
  EXPECT_THAT(buffer.ExtractNextTemporalUnit(), ElementsAre(FrameWithId(4)));
  EXPECT_THAT(buffer.ExtractNextTemporalUnit(), ElementsAre(FrameWithId(5)));

  buffer.InsertFrame(Builder().Time(70).Id(7).Refs({5}).AsLast().Build());
  EXPECT_THAT(buffer.ExtractNextTemporalUnit(), ElementsAre(FrameWithId(7)));
  buffer.InsertFrame(Builder().Time(60).Id(6).Refs({4}).AsLast().Build());
  EXPECT_THAT(buffer.ExtractNextTemporalUnit(), IsEmpty());
  buffer.InsertFrame(Builder().Time(90).Id(9).Refs({7}).AsLast().Build());
  EXPECT_THAT(buffer.ExtractNextTemporalUnit(), ElementsAre(FrameWithId(9)));
}

TEST(FrameBuffer3Test, MultipleLastFramesInTemporalUnit) {
  FrameBuffer buffer(/*max_frame_slots=*/10, /*max_decode_history=*/100);
  // Invalid stream, but should still be handeled in a sensible way.
  buffer.InsertFrame(Builder().Time(10).Id(1).AsLast().Build());
  buffer.InsertFrame(Builder().Time(10).Id(2).AsLast().Build());
  EXPECT_THAT(buffer.ExtractNextTemporalUnit(), ElementsAre(FrameWithId(1)));
  EXPECT_THAT(buffer.ExtractNextTemporalUnit(), IsEmpty());
}
