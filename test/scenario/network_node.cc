...
void NullReceiver::DeliverPacket(EmulatedIpPacket packet) {}

ActionReceiver::ActionReceiver(std::function<void()> action)
    : action_(action) {}

void ActionReceiver::DeliverPacket(EmulatedIpPacket packet) {
  action_();
}
...
void NetworkNode::DeliverPacket(EmulatedIpPacket packet) {
  rtc::CritScope crit(&crit_sect_);
...
bool NetworkNodeTransport::SendRtp(const uint8_t* packet,
  ...
  send_net_->DeliverPacket(EmulatedIpPacket(rtc::SocketAddress() /*from*/,
...
void CrossTrafficSource::Process(Timestamp at_time, TimeDelta delta) {
  ...
  if (pending_size_ > config_.min_packet_size) {
    target_->DeliverPacket(EmulatedIpPacket(
