...
#ifndef TEST_TESTSUPPORT_FIXED_FPS_VIDEO_FRAME_WRITER_ADAPTOR_H_
#define TEST_TESTSUPPORT_FIXED_FPS_VIDEO_FRAME_WRITER_ADAPTOR_H_
...
//
// For example if frames A-F were received, then next output sequence will be
// generated:
...
class FixedFpsVideoFrameWriterAdaptor : public VideoFrameWriter {
 public:
  FixedFpsVideoFrameWriterAdaptor(int fps,
                                  Clock* clock,
                                  VideoFrameWriter* delegate);
  ~FixedFpsVideoFrameWriterAdaptor() override;

  bool WriteFrame(const webrtc::VideoFrame& frame) override;

  // Closes adaptor and underlying delegate. User mustn't call to the WriteFrame
 ...
 private:
  ...
  bool WriteDroppedFramesExceptLast(Timestamp now);
  Timestamp Now() const;

  const TimeDelta inter_frame_interval_;
  Clock* const clock_;
  test::VideoFrameWriter* const delegate_;
...
#endif  // TEST_TESTSUPPORT_FIXED_FPS_VIDEO_FRAME_WRITER_ADAPTOR_H_
