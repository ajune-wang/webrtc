...
void TurnServerAllocation::RefreshChannel(ChannelList::iterator it) {
  it->pending_delete.reset();
  thread_->PostDelayedTask(
      SafeTask(it->pending_delete.flag(), [this, it] { channels_.erase(it); }),
      kChannelTimeout);
}

void TurnServerAllocation::RefreshPermission(PermissionList::iterator it) {
  it->pending_delete.reset();
  thread_->PostDelayedTask(
      SafeTask(it->pending_delete.flag(), [this, it] { perms_.erase(it); }),
      kPermissionTimeout);
}
