...
std::vector<std::string> SimpleEncoderWrapper::SupportedWebrtcSvcModes(
  ...
  const bool inter_layer = prediction_constraints.max_references > 1 &&
                           prediction_constraints.shared_buffer_space;
...
void SimpleEncoderWrapper::Encode(
  ...
  for (size_t s = 0; s < configs.size(); ++s) {
    ...
    for (const CodecBufferUsage& buffer : config.Buffers()) {
      ...
      if (buffer.updated) {
        settings.update_buffers.push_back(buffer.id);
      }
    }
  ...
  encoder_->Encode(std::move(frame_buffer), {}, encode_settings,
                   std::move(callback_internal));
}
