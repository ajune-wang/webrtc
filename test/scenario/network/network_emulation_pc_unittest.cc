...
TEST(NetworkEmulationManagerPCTest, Run) {
  ...
  EmulatedNetworkController* alice_network_controller =
      emulation.CreateNetworkController({alice_endpoint});
  EmulatedNetworkController* bob_network_controller =
      emulation.CreateNetworkController({bob_endpoint});
  ...
  signaling_thread->Invoke<void>(RTC_FROM_HERE, [&]() {
    alice_pcf = CreatePeerConnectionFactory(
        signaling_thread.get(), alice_network_controller->network_thread());
    alice_pc =
        CreatePeerConnection(alice_pcf, alice_observer.get(),
                             alice_network_controller->network_manager());

    bob_pcf = CreatePeerConnectionFactory(
        signaling_thread.get(), bob_network_controller->network_thread());
    bob_pc = CreatePeerConnection(bob_pcf, bob_observer.get(),
                                  bob_network_controller->network_manager());
  });
