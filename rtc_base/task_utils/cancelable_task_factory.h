 ...
 private:
  class ControlBlock {
   public:
    ...
    // Reference counting to use ControlBlock in the scoped_refptr
    void AddRef();
    void Release();

    // Disallow new tasks to start and block until already started tasks are
    // complete.
    void CancelAll();
    // Returns true iff new tasks allows to to start.
    // StartTask and CompleteTask are also keep track of number of currently
    // running tasks.
    bool StartTask();
    void CompleteTask();
