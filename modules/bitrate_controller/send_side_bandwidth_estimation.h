...
class SendSideBandwidthEstimation {
 public:
  ...
  // Call when we receive a RTCP message with a ReceiveBlock.
  void UpdateReceiverBlock(int packets_lost,
                           int number_of_packets,
                           int64_t now_ms);

  // Call when we receive a RTCP message with a ReceiveBlock.
  void UpdateReceiverBlockRtt(int64_t rtt, int64_t now_ms);
 ...
 private:
  ...
  void UpdateUmaStatsPacketsLost(int64_t now_ms, int lost_packets);
  void UpdateUmaStatsRtt(int64_t now_ms, int64_t rtt);
