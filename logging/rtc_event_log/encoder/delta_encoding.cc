...
class FixedLengthDeltaDecoder final {
 ...
 private:
  ...
  FixedLengthDeltaDecoder(BitReader reader,
                          const FixedLengthEncodingParameters& params,
                          absl::optional<uint64_t> base,
                          size_t num_of_deltas);
...
std::unique_ptr<FixedLengthDeltaDecoder> FixedLengthDeltaDecoder::Create(
  ...
  // delta_width_bits
  const uint64_t delta_width_bits =
      reader.ReadBits(kBitsInHeaderForDeltaWidthBits) +
      1;                                // See encoding for +1's rationale.
  RTC_DCHECK_LE(delta_width_bits, 64);  // See encoding for -1's rationale.
