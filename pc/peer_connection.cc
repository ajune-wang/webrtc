...
bool PeerConnection::SendData(const cricket::SendDataParams& params,
  ...
  RTC_DCHECK_RUN_ON(signaling_thread());
  DataChannelTransportInterface* transport = data_channel_transport_.load();
  if (transport) {
    ...
    RTCError error = network_thread()->Invoke<RTCError>(
        RTC_FROM_HERE, [transport, params, send_params, payload] {
          return transport->SendData(params.sid, send_params, payload);
        });
...
RTCError PeerConnection::CreateChannels(const SessionDescription& desc) {
  ...
  if (data_channel_type_ != cricket::DCT_NONE && data && !data->rejected &&
      !rtp_data_channel_ && !data_channel_transport_.load()) {
...
bool PeerConnection::OnTransportChanged(
  ...
  DataChannelTransportInterface* old_transport = data_channel_transport_.load();
  if (old_transport && mid == sctp_mid_ &&
      old_transport != data_channel_transport) {
    // Changed which data channel transport is used for |sctp_mid_| (eg. now
    // it's bundled).
    old_transport->SetDataSink(nullptr);
