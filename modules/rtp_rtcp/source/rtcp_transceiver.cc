...
void RtcpTransceiver::SendNack(uint32_t media_ssrc,
                               std::vector<uint16_t> sequence_numbers) {
  // TODO(danilchap): Replace with lambda with move capture when available.
  struct Closure {
    void operator()() {
      if (ptr)
        ptr->SendNack(media_ssrc, std::move(sequence_numbers));
    }
  ...
  task_queue_->PostTask(Closure{ptr_, media_ssrc, std::move(sequence_numbers)});
}
