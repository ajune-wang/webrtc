 ...
 public:
  ...
  void RunTest(const std::string& test_case_name,
    ...
    for (auto stream_label : video_analyzer_ptr->GetKnownVideoStreams()) {
      ...
      // 60 = 30fps * runtime_secs. On some devices pipeline can be too slow, so
      // it can happen, that frames will stuck in the middle, so we actually
      // can't force real constraints here, so lets just check, that at least 1
