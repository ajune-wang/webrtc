...
// - both parties loosing candidate filter at the same time (approx.).
//
...
TEST_F(P2PTransportChannelTest, SurfaceRequiresCoordination) {
  ...
  // On caller we only have relay,
  // on callee we have host, srflx and relay.
  ...
  // And then loosing caller candidate filter.
  ep1->allocator_->SetCandidateFilter(CF_ALL);

  // And no p2p connection will be made,
  // it will remain on relay.
  SIMULATED_WAIT(false, kDefaultTimeout, clock);
