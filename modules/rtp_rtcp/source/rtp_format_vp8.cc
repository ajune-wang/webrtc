...
bool RtpPacketizerVp8::NextPacket(RtpPacketToSend* packet) {
  ...
  hdr_[0] &= (~kSBit);  //  Clear first packet bit.
...
RtpPacketizerVp8::RawHeader RtpPacketizerVp8::BuildHeader(
  ...
  bool x_field_present =
      tid_present || keyid_present || tl0_pid_present || pid_present;
  ...
  if (header.nonReference)
    flags |= kNBit;
  // Create header as first packet in the frame.
  flags |= kSBit;
  result.push_back(flags);
  if (!x_field_present) {
    return result;
  }
  result.push_back(0);  // Rerver place for the x_field, set it later.
  uint8_t x_field = 0;
  if (pid_present) {
    x_field |= kIBit;
