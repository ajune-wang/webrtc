...
TEST(GoogCcNetworkControllerTest, MaintainsLowRateInSafeResetTrial) {
  ...
  auto* receiver = s.CreateClient(
      "return", [&](CallClientConfig* c) { c->endpoints_count = 2; });
  auto* route =
      s.CreateRoutes(client, {send_net->node()}, receiver,
                     {s.CreateSimulationNode(NetworkNodeConfig())->node()});
  ...
  receiver->SwitchToEndpoint(receiver->GetEndpoints()[1]);
  s.ChangeRoute(route->forward(), {send_net->node()});
...
TEST(GoogCcNetworkControllerTest, DetectsHighRateInSafeResetTrial) {
  ...
  auto* receiver = s.CreateClient(
      "return", [&](CallClientConfig* c) { c->endpoints_count = 2; });
  auto* route =
      s.CreateRoutes(client, {initial_net->node()}, receiver,
                     {s.CreateSimulationNode(NetworkNodeConfig())->node()});
  s.CreateVideoStream(route->forward(), VideoStreamConfig());
  // Allow the controller to stabilize.
  ...
  receiver->SwitchToEndpoint(receiver->GetEndpoints()[1]);
  s.ChangeRoute(route->forward(), {new_net->node()});
  s.ChangeRoute(route->reverse(), {new_net->node()});
...
TEST(GoogCcNetworkControllerTest, NoBandwidthTogglingInLossControlTrial) {
  ...
  auto* route = s.CreateRoutes(
      client, {send_net->node()}, s.CreateClient("return", CallClientConfig()),
      {s.CreateSimulationNode(NetworkNodeConfig())->node()});
  s.CreateVideoStream(route->forward(), VideoStreamConfig());
