...
bool TimeToReportMetrics(int blocks_since_last_report) {
  constexpr int kReportingIntervalBlocks = 10 * kNumBlocksPerSecond;
  return blocks_since_last_report == kReportingIntervalBlocks;
}
...
void ApiCallJitterMetrics::ReportCaptureCall() {
  ...
  if (proper_call_observed_ &&
      TimeToReportMetrics(++blocks_since_last_report_)) {
    // Report jitter, where the base basic unit is blocks.
    constexpr int kMaxJitterToReport = 125;
    constexpr int kBinSizeLog2 = 1;
    constexpr int kMaxJitterBins = kMaxJitterToReport >> kBinSizeLog2;

    // Ensures that the range and granularityof the reported jitter is proper.
    auto form_value_to_report = [&](int jitter) {
      RTC_DCHECK_LT(0, jitter);
      return std::min(kMaxJitterToReport, jitter) >> kBinSizeLog2;
    };

    // Report max and min jitter for render and capture, in units of 16 ms.
    RTC_HISTOGRAM_COUNTS_LINEAR("WebRTC.Audio.EchoCanceller.MaxRenderJitter",
                                form_value_to_report(render_jitter().max()), 1,
                                kMaxJitterBins, kMaxJitterBins);
    RTC_HISTOGRAM_COUNTS_LINEAR("WebRTC.Audio.EchoCanceller.MinRenderJitter",
                                form_value_to_report(render_jitter().min()), 1,
                                kMaxJitterBins, kMaxJitterBins);

    RTC_HISTOGRAM_COUNTS_LINEAR("WebRTC.Audio.EchoCanceller.MaxCaptureJitter",
                                form_value_to_report(capture_jitter().max()), 1,
                                kMaxJitterBins, kMaxJitterBins);
    RTC_HISTOGRAM_COUNTS_LINEAR("WebRTC.Audio.EchoCanceller.MinCaptureJitter",
                                form_value_to_report(capture_jitter().min()), 1,
                                kMaxJitterBins, kMaxJitterBins);
...
bool ApiCallJitterMetrics::WillReportMetricsAtNextCapture() const {
  return TimeToReportMetrics(blocks_since_last_report_ + 1);
}
