...
void Thread::Send(const Location& posted_from,
  ...
  RTC_DCHECK(current_thread != nullptr);  // AutoThread ensures this
  {
    ...
    RTC_DCHECK(current_thread->send_target_ == nullptr);
    current_thread->send_target_ = this;
  }
  bool ready = false;
  PostTask(
  ...
  bool waited = false;
  crit_.Enter();
  RTC_DCHECK_NE(send_target_, current_thread);
  while (!ready) {
    crit_.Leave();
  ...
  {
    CritScope cs(&current_thread->crit_);
    current_thread->send_target_ = nullptr;
  }
