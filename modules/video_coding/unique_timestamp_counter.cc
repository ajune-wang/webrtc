...
void UniqueTimestampCounter::Add(uint32_t value) {
  // Fast check if it is same value as the last one inserted.
  if (unique_seen_ > 0 && latest_[(unique_seen_ - 1) % kMaxHistory] == value) {
    // Same value as last one.
    return;
  }
  if (!search_index_.insert(value).second) {
    // Already known.
    return;
  }
