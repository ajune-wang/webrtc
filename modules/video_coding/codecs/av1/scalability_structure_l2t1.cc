...
constexpr std::array<DecodeTargetIndication, 2> kSR = {kSwitch, kRequired};
constexpr std::array<DecodeTargetIndication, 2> kNR = {kNotPresent, kRequired};
constexpr std::array<DecodeTargetIndication, 2> kDtis[4] = {kSS, kNS, kSR, kNR};
...
FrameDependencyStructure ScalabilityStructureL2T1::DependencyStructure() const {
  ...
  structure.templates[0].S(0).Dtis(kSR).ChainDiffs({2, 1}).FrameDiffs({2});
  structure.templates[1].S(0).Dtis(kSS).ChainDiffs({0, 0});
  structure.templates[2].S(1).Dtis(kNR).ChainDiffs({1, 1}).FrameDiffs({2, 1});
