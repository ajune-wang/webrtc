...
bool PhysicalSocketServer::Wait(int cmsWait, bool process_io) {
  ...
  while (fWait_) {
...

    {
      ...
      // ABA problem (see later comment) and avoids the dispatchers_ iterator
      // being invalidated by calling CheckSignalClose, which may remove the
      // dispatcher from the list.
      current_dispatcher_keys_.clear();
      for (auto const& info : dispatchers_) {
        if (info.dispatcher)
          current_dispatcher_keys_.push_back(info.key);
      }
      for (auto it = current_dispatcher_keys_.begin();
           it != current_dispatcher_keys_.end(); ++it) {
        Dispatcher* disp = dispatchers_.get_by_key(*it);
        if (!disp)
          continue;
        ...
        if (disp->CheckSignalClose()) {
          ...
          it = current_dispatcher_keys_.erase(it);
        } else if (s != INVALID_SOCKET) {
          WSAEventSelect(s, events[0],
                         FlagsToEvents(disp->GetRequestedEvents()));
        } else {
          events.push_back(disp->GetWSAEvent());
          event_owners.push_back(*it);
          // Don't need to iterat over dispatchers without a socket (which are
          // expected to Signalers).
          it = current_dispatcher_keys_.erase(it);
        }
      }
    }
