  ...
  while (next < kNumPackets) {
    while (next < kNumPackets && arrival_times_ms[next] <= time) {
      ...
      if (time < peak_mode_start_ms || time > peak_mode_end_ms) {
        EXPECT_FALSE(detector.Update(iat_packets, kTargetBufferLevel, false));
      } else {
        EXPECT_TRUE(detector.Update(iat_packets, kTargetBufferLevel, false));
...
TEST(DelayPeakDetector, ZeroDistancePeaks) {
  ...
  EXPECT_FALSE(detector.Update(kInterArrivalTime, kTargetBufferLevel, false));
  EXPECT_FALSE(detector.Update(kInterArrivalTime, kTargetBufferLevel, false));
  EXPECT_FALSE(detector.Update(kInterArrivalTime, kTargetBufferLevel, false));
}
...
TEST(DelayPeakDetector, IgnoreReorderedPacket) {
  ...
  EXPECT_FALSE(detector.Update(kInterArrivalTime, kTargetBufferLevel, false));
  tick_timer.Increment();
  EXPECT_FALSE(detector.Update(kInterArrivalTime, kTargetBufferLevel, true));
}
