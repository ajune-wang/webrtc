...
void RtpGenericFrameDescriptor::SetFirstSubFrameInFrame(bool first) {
  RTC_DCHECK(!use_discardable_);
  beginning_of_frame_ = first;
}

void RtpGenericFrameDescriptor::SetLastSubFrameInFrame(bool last) {
  RTC_DCHECK(!use_discardable_);
  end_of_frame_ = last;
}
