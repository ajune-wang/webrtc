...
class RtpVideoStreamReceiver2 : public LossNotificationSender,
 ...
 private:
  ...
  void ReceivePacket(const RtpPacketReceived& packet)
      RTC_RUN_ON(network_task_checker_);
  ...
  ParseGenericDependenciesResult ParseGenericDependenciesExtension(
      const RtpPacketReceived& rtp_packet,
      RTPVideoHeader* video_header) RTC_RUN_ON(network_task_checker_);
  ...
  int64_t last_packet_log_ms_ RTC_GUARDED_BY(network_task_checker_);
  ...
  UniqueTimestampCounter frame_counter_ RTC_GUARDED_BY(network_task_checker_);
  SeqNumUnwrapper<uint16_t> frame_id_unwrapper_
      RTC_GUARDED_BY(network_task_checker_);
  ...
  std::map<uint8_t, std::map<std::string, std::string>> pt_codec_params_
      RTC_GUARDED_BY(network_task_checker_);
  int16_t last_payload_type_ RTC_GUARDED_BY(network_task_checker_) = -1;
