...
void LossNotificationController::OnReceivedPacket(
  ...
  if (last_received_seq_num_ &&
      !AheadOf(sequence_number, *last_received_seq_num_)) {
    return;
  }
  ...
  const bool seq_num_gap =
      ...
      sequence_number != static_cast<uint16_t>(*last_received_seq_num_ + 1u);

  last_received_seq_num_ = sequence_number;
  ...
  if (generic_descriptor.FirstPacketInSubFrame()) {
    ...
    if (key_frame) {
      ...
      if (seq_num_gap || !current_frame_potentially_decodable_) {
        HandleLoss(sequence_number, current_frame_potentially_decodable_);
      }
    }
    ...
    HandleLoss(sequence_number, false);
  }
}
