...
void RtpDemuxer::RemoveAmbiguousSsrcBindingsResolvedByPayloadType() {
  RemoveFromMapIf(&sink_by_ssrc_, [this](const auto& sink_by_ssrc) {
    if (sink_by_ssrc.second.origin !=
        SsrcBinding::Origin::kResolvedByPayloadType) {
      return false;
    }
...
bool RtpDemuxer::RemoveSink(const RtpPacketSinkInterface* sink) {
  ...
  size_t num_removed =
      RemoveFromMapIf(
          &sink_by_mid_,
          [sink](const auto& item) { return item.second == sink; }) +
      RemoveFromMapIf(
          &sink_by_ssrc_,
          [sink](const auto& item) { return item.second.sink == sink; }) +
      RemoveFromMapIf(
          &sinks_by_pt_,
          [sink](const auto& item) { return item.second == sink; }) +
      RemoveFromMapIf(
