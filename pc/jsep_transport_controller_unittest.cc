...
TEST_F(JsepTransportControllerTest,
  ...
  // Note: we need to create a description without bundle and add a bundle
  // manually in order to get data in the bundle group.  The helper for creating
  // a description with bundle only adds audio and video.
  auto description = CreateSessionDescriptionWithoutBundle();
...
TEST_F(JsepTransportControllerTest, CannotBundleDifferentAltProtocols) {
  ...
  for (cricket::ContentInfo& content_info : description->contents()) {
    content_info.media_description()->set_alt_protocol(
        params->protocol + "-" + content_info.name);
  }
