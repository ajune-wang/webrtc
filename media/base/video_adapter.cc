...
bool VideoAdapter::DropFrame(int64_t in_timestamp_ns) {  // change to Timestamp
  int max_fps = max_framerate_request_;
  if (max_fps_)
    max_fps = std::min(max_fps, *max_fps_);

  framerate_controller_.SetMaxFramerate(max_fps);
  return framerate_controller_.ShouldDropFrame(webrtc::Timestamp::Micros(
      in_timestamp_ns / rtc::kNumNanosecsPerMicrosec));
}

bool VideoAdapter::AdaptFrameResolution(
    int in_width,
    int in_height,
    int64_t in_timestamp_ns,  // change to Timestamp
    int* cropped_width,
    int* cropped_height,
    int* out_width,
    int* out_height) {
