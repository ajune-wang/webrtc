...
bool DefaultTemporalLayersChecker::CheckTemporalConfig(
  ...
  if (pattern_idx_ == temporal_ids_.size()) {
    ...
    if (!last_.is_keyframe && !last_.is_updated_this_cycle) {
      NLOG(LS_ERROR, "Last buffer was not updated during pattern cycle.");
      return false;
    }
    if (!arf_.is_keyframe && !arf_.is_updated_this_cycle) {
      NLOG(LS_ERROR, "Arf buffer was not updated during pattern cycle.");
      return false;
    }
    if (!golden_.is_keyframe && !golden_.is_updated_this_cycle) {
      NLOG(LS_ERROR, "Golden buffer was not updated during pattern cycle.");
      return false;
    }
  ...
  if (frame_config.packetizer_temporal_idx != expected_tl_idx) {
    NLOG(LS_ERROR, "Frame has an incorrect temporal index. Expected: ",
         static_cast<int>(expected_tl_idx),
         " Actual: ", static_cast<int>(frame_config.packetizer_temporal_idx));
    return false;
  }
