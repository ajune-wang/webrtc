...
TEST(RtcEventProcessor, EmptyList) {
  ...
  processor.AddEvents(events.begin(), events.end(), not_called);
  processor.ProcessEventsInOrder();  // Don't crash but do nothing.
}
...
TEST(RtcEventProcessor, OneList) {
  ...
  RtcEventProcessor processor;
  processor.AddEvents(events.begin(), events.end(), f);
...
TEST(RtcEventProcessor, MergeTwoLists) {
  ...
  RtcEventProcessor processor;
  processor.AddEvents(events1.begin(), events1.end(), f);
  processor.AddEvents(events2.begin(), events2.end(), f);
  processor.ProcessEventsInOrder();
...
TEST(RtcEventProcessor, MergeManyLists) {
  ...
  for (const auto& list : lists) {
    processor.AddEvents(list.begin(), list.end(), f);
  }
  processor.ProcessEventsInOrder();
...
TEST(RtcEventProcessor, DifferentTypes) {
  ...
  RtcEventProcessor processor;
  processor.AddEvents(events1.begin(), events1.end(), f1);
  processor.AddEvents(events2.begin(), events2.end(), f2);
  processor.ProcessEventsInOrder();
