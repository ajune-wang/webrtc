...
class SctpTransport : public SctpTransportInternal,
 ...
 private:
  ...
  // This is a map to search the closing status of a sream. A stream is
  // considered as fully freed when these 3 conditions are met: 1. Received
  // SCTP_STREAM_RESET_INCOMING 2. Received SCTP_STREAM_RESET_OUTGOING 3. Sent
  // SCTP_STREAM_RESET_OUTGOING The order in which those 3 conditions are
  // fulfilled is irrelevant. However, for the sake of clarity, we have 2
  // possible scenarios exemplified as follows: 1. Near side closing the
  // channel: When the near side wants to close a channel,
  //    we will only send SCTP_STREAM_RESET_OUTGOING. Than, wait for both
  //    SCTP_STREAM_RESET_INCOMING and SCTP_STREAM_RESET_OUTGOING to arrive back
  //    via sctp notifications backets. We will treat them as ACK for the
  //    SCTP_STREAM_RESET_OUTGOING. Once that is completed, than the channel can
  //    be safely discarded/discontinued, and the SID reused.
  // 2. Far side closing the channel: SCTP_STREAM_RESET_INCOMING will be
  // received as
  //    notification. That moment, SCTP_STREAM_RESET_OUTGOING should be sent,
  //    and we will wait for the SCTP_STREAM_RESET_OUTGOING to be received as
  //    well via notifications. Once that is completed, than the channel can be
  //    safely discarded/discontinued, and the SID reused.
  // When a SID is present in the map, we will consider that SID as in-flight
  // closing, so it will not be reused. When the closing is complete by
  // fulfilling the 3 conditions enumerated above (status ==
  // kFullResetCompleted), than the SID will be removed from the map and the SID
  // can than be reused
  typedef std::map<uint32_t, uint8_t> ResetStreamsStatus;

  // open_streams_ will tell us the list of opened streams. The moment a reset
  // event happens because remote sent a reset, or the local side decided to
  // close the channel, it will be removed from open_streams_ and added to
  // reset_streams_status_ with the appropriate status. The presence of a stream
  // in one of these 2 places marks the stream ID as non-usable ID in bringing
  // up new streams
