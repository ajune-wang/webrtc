 ...
 * coefficients used for describing the
 * tail (kFftLengthBy2 in this case). */

void FilterAnalyzer::UpdateFilterTailGain(
    rtc::ArrayView<const float> filter_time_domain) {
  float tail_max_energy = 0.f;
  for (auto* s = filter_time_domain.cend() - kFftLengthBy2;
       s != filter_time_domain.cend(); ++s) {
    tail_max_energy = std::max(tail_max_energy, (*s) * (*s));
  }
