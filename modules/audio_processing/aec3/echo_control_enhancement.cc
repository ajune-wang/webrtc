...
std::unique_ptr<SuppressorGainDelayBuffer> ConditionallyCreateGainDelayBuffer(
    const EchoControlEnhancer* enhancer) {
...
void EchoControlEnhancement::Enhance(
    ...
    rtc::ArrayView<float, kFftLengthBy2Plus1> low_band_suppression_gain,
    float* high_bands_suppressor_gain) {
  RTC_DCHECK(enhancer_);
  RTC_DCHECK(high_bands_suppressor_gain);

  if (use_linear_filter_output) {
    for (size_t ch = 0; ch < num_capture_channels_; ++ch) {
      std::copy(linear_filter_output[ch].begin(),
                linear_filter_output[ch].end(), (*y)[ch][0].begin());
    }
  }
  ...
  enhancer_->Process(Y_re_, Y_im_, y);
  ...
  if (enhancer_->AlgorithmicDelayInMs() != 0.f) {
    ...
    gain_delay_buffer_->Delay(low_band_suppression_gain,
                              high_bands_suppressor_gain);
  }
}
