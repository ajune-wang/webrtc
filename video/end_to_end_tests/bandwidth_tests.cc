...
class BandwidthStatsTest : public test::EndToEndTest {
 public:
  BandwidthStatsTest(bool send_side_bwe, test::SingleThreadedTaskQueueForTesting* task_queue)
  ...
  class BweObserver : public test::EndToEndTest {
   public:
    ...
    void OnCallsCreated(Call* sender_call, Call* receiver_call) override {
      ...
      sender_call_ = sender_call;
      pending_task_ = task_queue_->PostTask([this](){
        PollStats();
      });
    }
    ...
    void PollStats() {
      ...
      pending_task_ = task_queue_->PostDelayedTask([this](){
        PollStats();
      }, 1000);
    }
