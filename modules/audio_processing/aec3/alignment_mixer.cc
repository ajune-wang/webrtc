...
AlignmentMixer::AlignmentMixer(int num_channels,
                               bool downmix,
                               bool adaptive_selection,
...
int AlignmentMixer::SelectChannel(
    const rtc::ArrayView<const std::vector<float>> x) {
  RTC_DCHECK_EQ(x.size(), num_channels_);
  RTC_DCHECK_GE(num_channels_, 2);
  ...
  if (cumulative_energies_[strongest_ch] >
