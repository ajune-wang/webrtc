...
int32_t VoEBaseImpl::EnablePlayout(bool enabled) {
  ...
  if (enabled) {
    ...
    if (ret != 0) {
      LOG(LS_ERROR) << "EnablePlayout(true) failed to start playout";
    }
  } else {
    ret = shared_->audio_device()->StopPlayout();
    if (ret != 0) {
      LOG(LS_ERROR) << "EnablePlayout(false) failed to stop playout";
    }
  }
  return ret;
}
