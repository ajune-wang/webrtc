...
class FakeTaskQueue : public TaskQueueBase {
 public:
  ...
  void PostTask(const Location&, absl::AnyInvocable<void() &&> task) override {
...
TEST(RepeatingTaskTest, CanBeStoppedAfterTaskQueueDeletedTheRepeatingTask) {
  ...
  EXPECT_CALL(task_queue, PostDelayedTask)
      .WillOnce([&](const Location&, absl::AnyInvocable<void() &&> task,
                    TimeDelta delay) { repeating_task = std::move(task); });
