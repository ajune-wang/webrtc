...
class FakeBandWidthHndler : public BandwidthScalerUsageHandlerInterface {
 public:
  ~FakeBandWidthHndler() override = default;
 ...
 protected:
  ...
  BandwidthScalerTest()
      : task_queue_("BandwidthScalerTestQueue"),
        handler_(std::make_unique<FakeBandWidthHndler>()) {
  ...
  std::unique_ptr<FakeBandWidthHndler> handler_;
};
...
TEST_F(BandwidthScalerTest, KeepScaleNormalBandwidth_640X360) {
  task_queue_.SendTask(
      ...
      RTC_FROM_HERE);
  EXPECT_FALSE(handler_->event.Wait(6000));
...
TEST_F(BandwidthScalerTest, KeepScaleAboveMaxBandwidth_640X360) {
  task_queue_.SendTask(
      ...
      RTC_FROM_HERE);
  EXPECT_FALSE(handler_->event.Wait(5000 + 1000));
  task_queue_.SendTask(
      [this] {
        TriggerBandwidthScalerTest(kKeepScaleAboveMaxBandwidth, 640, 360);
      },
      RTC_FROM_HERE);
  EXPECT_TRUE(handler_->event.Wait(5000 + 1000));
  EXPECT_EQ(0, handler_->adapt_down_events_);
