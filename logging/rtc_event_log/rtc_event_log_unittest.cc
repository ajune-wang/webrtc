...
class RtcEventLogSession
 ...
 private:
  ...
  std::vector<RtcEventGenericPacketSent> generic_packets_sent_;
  std::vector<RtcEventGenericPacketReceived> generic_packets_received_;
  std::vector<RtcEventGenericAckReceived> generic_acks_received_;
...
void RtcEventLogSession::WriteLog(EventCounts count,
  ...
  for (; remaining_events > 0; remaining_events--) {
    ...
    if (selection < count.generic_packets_sent) {
      ...
      generic_packets_sent_.push_back(*event);
    ...
    if (selection < count.generic_packets_received) {
      ...
      generic_packets_received_.push_back(*event);
      event_log->Log(std::move(event));
    ...
    if (selection < count.generic_acks_received) {
      ...
      generic_acks_received_.push_back(*event);
      event_log->Log(std::move(event));
