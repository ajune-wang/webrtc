...
class WgcCaptureSession final {
 ...
 private:
  ...
  // Keeps track of the number of FrameArrived events or the number of times
  // OnFrameArrived() has been called.
  uint64_t on_frame_arrived_events_ = 0;

  // Counts number of times a valid frame has been acquired from the frame pool
  // and then successfully processed and finally copied into the queue.
