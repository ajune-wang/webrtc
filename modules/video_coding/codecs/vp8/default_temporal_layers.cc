...
bool DefaultTemporalLayers::IsSyncFrame(const FrameConfig& config) const {
  ...
  if (config.golden_buffer_flags & BufferFlags::kReference) {
    size_t frame_age =
        frames_since_buffer_refresh_.find(Vp8BufferReference::kGolden)->second;
  ...
  if (config.arf_buffer_flags & BufferFlags::kReference) {
    size_t frame_age =
        frames_since_buffer_refresh_.find(Vp8BufferReference::kAltref)->second;
    //    auto it =
    //    frames_since_buffer_refresh_.find(Vp8BufferReference::kAltref); if (it
    //    != frames_since_buffer_refresh_.end() &&
    //        it->second + 1 < pattern_idx_) {
    if (frame_age + 1 < pattern_idx_) {
      // Referencing an altref frame that has been updated this iteration.
      return false;
    }
  }
...
TemporalLayers::FrameConfig DefaultTemporalLayers::UpdateLayerConfig(
  ...
  // Update search order to let the encoder know which buffers contains to most
  // recent data.
