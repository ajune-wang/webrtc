...
class LibaomAv1Encoder final : public VideoEncoder {
 public:
  explicit LibaomAv1Encoder(
      std::unique_ptr<FrameDependenciesController> svc_controller);
  ~LibaomAv1Encoder();
 ...
 private:
  const std::unique_ptr<FrameDependenciesController> svc_controller_;
  bool inited_;
...
int32_t LibaomAv1Encoder::Encode(
  ...
  // TODO(danilchap): configure SVC when frame_infos.front().is_keyframe == true
  // and svc_controller_->DependencyStructure().num_decode_targets > 0.
  ...
  // TODO(danilchap): Remove this checks when svc is implemented.
  RTC_DCHECK_EQ(frame_infos.size(), 1);
  for (GenericFrameInfo& frame_info : frame_infos) {
    aom_enc_frame_flags_t flags =
        frame_info.is_keyframe ? AOM_EFLAG_FORCE_KF : 0;

    // TODO(danilchap): configure buffers and layers based on `frame_info` when
    // svc_ == true.
    ...
    while (const aom_codec_cx_pkt_t* pkt =
      ...
      if (pkt->kind == AOM_CODEC_CX_FRAME_PKT && pkt->data.frame.sz > 0) {
        ...
        frame_info.is_keyframe =
            ((pkt->data.frame.flags & AOM_EFLAG_FORCE_KF) != 0);
        encoded_image._frameType = frame_info.is_keyframe
    ...
    if (encoded_image.size() > 0) {
      ...
      // TODO(danilchap): Propagate frame_info when svc mode is enabled.
...
std::unique_ptr<VideoEncoder> CreateLibaomAv1Encoder() {
  return std::make_unique<LibaomAv1Encoder>(
      std::make_unique<FrameDependenciesControllerNoLayering>());
}
