...
class CallStatsTest : public ::testing::Test {
 public:
  ...
  void TerminateTaskQueue() { task_queue_.reset(); }

 protected:
  std::unique_ptr<rtc::test::TaskQueueForTest> task_queue_{
      new rtc::test::TaskQueueForTest("VideoReceiveStreamTest")};
  SimulatedClock fake_clock_{12345};
};

TEST_F(CallStatsTest, AddAndTriggerCallback) {
  CallStats call_stats{&fake_clock_, task_queue_.get()};
...
TEST_F(CallStatsTest, ProcessTime) {
  CallStats call_stats{&fake_clock_, task_queue_.get(), 1};
...
TEST_F(CallStatsTest, MultipleObservers) {
  ...
  // hmm... this isn't good since the CallStats object still has a pointer
  // to the TaskQueue...
  TerminateTaskQueue();
}
