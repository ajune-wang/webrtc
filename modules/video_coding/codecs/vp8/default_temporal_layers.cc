...
std::unique_ptr<TemporalLayersChecker> CreateChecker(int num_layers) {
#if !defined(NDEBUG)
  return TemporalLayersChecker::CreateTemporalLayersChecker(
      TemporalLayersType::kFixedPattern, num_layers);
#else
  return std::unique_ptr<TemporalLayersChecker>();
#endif
}
}  // namespace
...
DefaultTemporalLayers::DefaultTemporalLayers(int number_of_temporal_layers)
    : num_layers_(std::max(1, number_of_temporal_layers)),
      ...
      checker_(CreateChecker(number_of_temporal_layers)) {
...
void DefaultTemporalLayers::OnEncodeDone(uint32_t rtp_timestamp,
  ...
  if (num_layers_ == 1) {
    ...
    if (checker_) {
      RTC_DCHECK(
          checker_->CheckTemporalConfig(is_keyframe, frame.frame_config));
    }
  }
