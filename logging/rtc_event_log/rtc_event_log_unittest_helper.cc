...
void VerifyLoggedAudioNetworkAdaptationEvent(
  ...
  // TODO: !!! 1. Explain or 2. log a fraction from the get-go.
  // const float uplink_packet_loss_fraction_delta =
  //     std::max(original_event.config_->uplink_packet_loss_fraction,
  //              logged_event.config.uplink_packet_loss_fraction) -
  //     std::min(original_event.config_->uplink_packet_loss_fraction,
  //              logged_event.config.uplink_packet_loss_fraction);
  // EXPECT_LE(uplink_packet_loss_fraction_delta, 0.001f);
}
