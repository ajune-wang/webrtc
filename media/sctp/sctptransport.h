...
class SctpTransport : public SctpTransportInternal,
 ...
 private:
  ...
  struct StreamStatus {
    // Reset initiated by application via ResetStream? Note that this may be
    // true while outgoing_reset_initiated is false if the outgoing reset
    // needed to be queued.
    ...
    bool is_open() const {
      return !reset_initiated && !incoming_reset_complete &&
    ...
    bool need_outgoing_reset() const {
      return (incoming_reset_complete || reset_initiated) &&
