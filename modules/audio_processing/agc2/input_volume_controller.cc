...
using InputVolumeControllerConfig =
    AudioProcessing::Config::GainController2::InputVolumeController;
...
Agc1ClippingPredictorConfig CreateClippingPredictorConfig(bool enabled) {
  Agc1ClippingPredictorConfig config{
      .enabled = enabled,
      .mode = Agc1ClippingPredictorConfig::kClippingEventPrediction,
      .window_length = 5,
...
RecommendedInputVolumeEstimator::RecommendedInputVolumeEstimator(
  ...
  RTC_DCHECK_GE(max_digital_gain_db, 0);
  RTC_DCHECK_LE(max_digital_gain_db, 30);
  RTC_DCHECK_GE(min_digital_gain_db, 0);
  RTC_DCHECK_LE(min_digital_gain_db, 30);
}
...
InputVolumeController::InputVolumeController(
    ...
    : InputVolumeController(/*num_capture_channels=*/1, config) {
  RTC_DCHECK(channel_estimators_[0]);
}
...
InputVolumeController::InputVolumeController(
    ...
    : analog_controller_enabled_(config.enabled),
      ...
      disable_digital_adaptive_(!config.enable_digital_adaptive),
      ...
      channel_estimators_(num_capture_channels),
  ...
  for (size_t ch = 0; ch < channel_estimators_.size(); ++ch) {
    channel_estimators_[ch] = std::make_unique<RecommendedInputVolumeEstimator>(
  ...
  channel_estimators_[0]->ActivateLogging();
}
...
void InputVolumeController::Initialize() {
  ...
  for (size_t ch = 0; ch < channel_estimators_.size(); ++ch) {
    channel_estimators_[ch]->Initialize();
  }
  capture_output_used_ = true;
...
void InputVolumeController::AnalyzePreProcess(const AudioBuffer& audio_buffer) {
  ...
  if (!!clipping_predictor_) {
    for (int channel = 0; channel < num_capture_channels_; ++channel) {
      const auto step = clipping_predictor_->EstimateClippedLevelStep(
          ...
          channel_estimators_[channel]->min_mic_level(), kMaxMicLevel);
      if (step.has_value()) {
  ...
  if (clipping_detected ||
    ...
    for (auto& state_ch : channel_estimators_) {
...
void InputVolumeController::Process(const AudioBuffer& audio_buffer,
  ...
  for (size_t ch = 0; ch < channel_estimators_.size(); ++ch) {
    ...
    FloatS16ToS16(audio_buffer.split_bands_const_f(ch)[0], num_frames_per_band,
                  audio_use);
    channel_estimators_[ch]->Process({audio_use, num_frames_per_band},
                                     rms_error_override);
  }
...
void InputVolumeController::HandleCaptureOutputUsedChange(
    bool capture_output_used) {
  for (size_t ch = 0; ch < channel_estimators_.size(); ++ch) {
    channel_estimators_[ch]->HandleCaptureOutputUsedChange(capture_output_used);
  }
  capture_output_used_ = capture_output_used;
}
...
void InputVolumeController::set_stream_analog_level(int level) {
  ...
  for (size_t ch = 0; ch < channel_estimators_.size(); ++ch) {
    channel_estimators_[ch]->set_stream_analog_level(level);
  }
...
void InputVolumeController::AggregateChannelLevels() {
  int new_recommended_input_volume =
      channel_estimators_[0]->recommended_analog_level();
  channel_controlling_gain_ = 0;
  if (use_min_channel_level_) {
    for (size_t ch = 1; ch < channel_estimators_.size(); ++ch) {
      int level = channel_estimators_[ch]->recommended_analog_level();
