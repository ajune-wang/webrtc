...
- (RTCRtpTransceiverDirection)direction {
  return [RTCRtpTransceiver rtpTransceiverDirectionFromNativeDirection:_nativeRtpTransceiver->direction()];
}

- (void)setDirection:(RTCRtpTransceiverDirection)direction {
  _nativeRtpTransceiver->SetDirection([RTCRtpTransceiver nativeRtpTransceiverDirectionFromDirection:direction]);
}

- (RTCRtpTransceiverDirection *)currentDirection {
  if (_nativeRtpTransceiver->current_direction()) {
    return [RTCRtpTransceiver rtpTransceiverDirectionFromNativeDirection:*_nativeRtpTransceiver->current_direction()];
  } else {
    return nil;
  }
}
...
+ (webrtc::RtpTransceiverDirection)nativeRtpTransceiverDirectionFromDirection:(RTCRtpTransceiverDirection)direction {
  switch (direction) {
...
+ (RTCRtpTransceiverDirection)rtpTransceiverDirectionFromNativeDirection:(webrtc::RtpTransceiverDirection)nativeDirection {
