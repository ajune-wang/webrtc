...
BbrNetworkControllerFactory::BbrNetworkControllerFactory(const Clock* clock,
...
NetworkControllerInterface::uptr BbrNetworkControllerFactory::Create(
  ...
  return rtc::MakeUnique<bbr::BbrNetworkController>(observers);
}
...
const float kPacingGain[] = {1.25, 0.75, 1, 1, 1, 1, 1, 0.85};
...
const BbrRoundTripCount kRoundTripsWithoutGrowthBeforeExitingStartup = 3;
...
const TimeDelta kInitialRtt = TimeDelta::ms(100);
...
const float kEncoderRateGain = 0.9f;
// Encoder rate gain value for PROBE_RTT mode.
const float kEncoderRateGainForProbeRtt = 0.5f;
}  // namespace
...
BbrNetworkController::BbrNetworkController(NetworkControlObserver* observers)
...
void BbrNetworkController::ConnectHandlers(
...
void BbrNetworkController::SignalUpdatedRates(Timestamp at_time) {
  ...
  if (mode_ == PROBE_RTT)
    target_rate = bandwidth * kEncoderRateGainForProbeRtt;
  else
    target_rate = bandwidth * kEncoderRateGain;
  if (constraints_) {
  ...
  RTC_LOG(LS_VERBOSE) << "RateUpdate, mode: " << mode_ << ", bw: " << bandwidth
  ...
  TargetTransferRate::SendMessage(target_rate_msg, observers_);
  ...
  PacerConfig::SendMessage(pacer_config, observers_);

  CongestionWindow congestion_window;
  congestion_window.data_window = congestion_window_;
  CongestionWindow::SendMessage(congestion_window, observers_);
}
...
void BbrNetworkController::OnNetworkRouteChange(NetworkRouteChange msg) {
  ...
  SignalUpdatedRates(msg.constraints.at_time);
}
...
/*
void BbrNetworkController::SetFromConfig() {
...
void BbrNetworkController::OnTransportPacketsFeedback(
  ...
  if (mode_ == PROBE_BW) {
    UpdateGainCyclePhase(msg.feedback_time, prior_in_flight_,
                         !lost_packets.empty());
  }
  ...
  MaybeExitStartupOrDrain(msg.feedback_time, bytes_in_flight);

  // Handle logic specific to PROBE_RTT.
  MaybeEnterOrExitProbeRtt(msg.feedback_time, bytes_in_flight, is_round_start,
                           min_rtt_expired);
  ...
  prior_in_flight_ = bytes_in_flight;
...
bool BbrNetworkController::UpdateRoundTripCounter(
  ...
  if (last_acked_send_time > current_round_trip_end_) {
    round_trip_count_++;
    current_round_trip_end_ = last_acked_send_time;
    return true;
  }
...
void BbrNetworkController::MaybeExitStartupOrDrain(Timestamp now,
  ...
  if (mode_ == DRAIN && bytes_in_flight <= GetTargetCongestionWindow(1)) {
    EnterProbeBandwidthMode(now);
  }
}

void BbrNetworkController::MaybeEnterOrExitProbeRtt(Timestamp now,
                                                    DataSize bytes_in_flight,
                                                    bool is_round_start,
                                                    bool min_rtt_expired) {
  ...
  if (mode_ == PROBE_RTT) {
    ...
    if (!exit_probe_rtt_at_.IsInitialized()) {
      ...
      // packet.
      if (bytes_in_flight < ProbeRttCongestionWindow() + kMaxPacketSize) {
