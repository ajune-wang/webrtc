...
TEST(RepeatingTaskTest, CompensatesForLongRunTimeInInclusiveMode) {
  ...
  rtc::TaskQueue task_queue("TestQueue");
  RepeatingTaskHandle handle(
      RepeatingTaskHandle::IntervalMode::kIncludingExecution);
  handle.Start(&task_queue, [&] {
...
TEST(RepeatingTaskTest, CompensatesForShortRunTimeInInclusiveMode) {
  std::atomic_int counter(0);
  rtc::TaskQueue task_queue("TestQueue");
  RepeatingTaskHandle handle(
      RepeatingTaskHandle::IntervalMode::kIncludingExecution);
...
TEST(RepeatingTaskTest, NoCompensationForRunTimeInExclusiveMode) {
  ...
  rtc::TaskQueue task_queue("TestQueue");
  RepeatingTaskHandle handle(
      RepeatingTaskHandle::IntervalMode::kExcludingExecution);
  handle.Start(&task_queue, [&] {
    if (++counter == kSleepAtCount)
      Sleep(kSleepDuration);
    return kRepeatInterval;
