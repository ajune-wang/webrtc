...
TEST_P(BbrScenarioTest, ReceivesVideo) {
  ...
  SendVideoStream* alice_video_send;
  ReceiveVideoStream* bob_video_recv;
  std::tie(alice_video_send, bob_video_recv) = s.CreateVideoStreams(
  ...
  SendVideoStream* bob_video_send = nullptr;
  ReceiveVideoStream* alice_video_recv = nullptr;
  if (conf_.scenario.return_traffic) {
    std::tie(bob_video_send, alice_video_recv) = s.CreateVideoStreams(
        bob, {ret_net}, alice, {send_net}, [&](VideoStreamConfig* c) {
          c->encoder.fake.max_rate = DataRate::kbps(1800);
        });
    s.CreateAudioStreams(bob, {ret_net}, alice, {send_net},
                         [&](AudioStreamConfig* c) {
                           if (conf_.tuning.use_bbr) {
                             c->stream.bitrate_tracking = true;
                             c->encoder.target_rate = DataRate::kbps(31);
  ...
  ColumnsPrinter send_stats_printer(
      ...
      {s.TimePrinter(), alice->StatsPrinter(), alice_video_send->StatsPrinter(),
  ...
  if (bob_video_send)
    return_printers.push_back(bob_video_send->StatsPrinter());
...
INSTANTIATE_TEST_CASE_P(Selected,
                        BbrScenarioTest,
                        Values(make_tuple("rs:1,bw:150,dl:100", "bbr")));
...
INSTANTIATE_TEST_CASE_P(OneWayDegraded,
                        ...
                        Values(make_tuple("bw:150,dl:100,dn:30,pl:5%", "bbr"),
                               ...
                               make_tuple("bw:150,dl:100,ct:100", "bbr"),
                               make_tuple("bw:150,dl:100,ct:100", ""),
