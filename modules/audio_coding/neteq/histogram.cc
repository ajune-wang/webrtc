...
absl::optional<double> GetConvergeFactor() {
  ...
  if (sscanf(field_trial_string.c_str(), "Enabled-%lf", &converge_factor) ==
          1 &&
      converge_factor >= 1.0 && converge_factor <= 3) {
    RTC_LOG(LS_INFO) << "Histogram converge factor: " << converge_factor;
    return absl::make_optional(converge_factor);
  }
  return absl::nullopt;
}
...
void Histogram::Add(int value) {
  ...
  if (converge_factor_) {
    ++count_;
    int conv = (1 << 15) * (1 - *converge_factor_ / count_);
    forget_factor_ = std::min(base_forget_factor_, conv);
  } else {
