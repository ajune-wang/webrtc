...
class QualityAnalyzingVideoEncoder : public VideoEncoder,
 ...
 private:
  enum SimulcastMode {
    ...
    // Next modes are to test video conference behavior. It is assumed, that
    // sender sends multiple simulcast streams/spatial layers for single track
    // and there is some Selective Forwarding Unit (SFU), that forward only best
    // one, that will pass through uplink to the receiver. To emulate this
    // behavior in our case sender will always send all simulcast
    // streams/spatial layers and receiver will filter out unwanted streams in
    // the quality analyzing video decoder.
