...
void AecState::UpdateReverb(const std::vector<float>& impulse_response) {
  ...
  if (current_reverb_decay_section_ < config_.filter.main.length_blocks) {
    ...
    RTC_DCHECK_LT(current_reverb_decay_section_, kMaxAdaptiveFilterLength);
    ...
    if (num_reverb_decay_sections_ > 0) {
      ...
      for (size_t i = 0; i < kFftLengthBy2; i++) {
        auto fast_approx_log2f = [](const float in) {
          // Cast to float to uint32_t and then convert to float
          union {
            float dummy;
            uint32_t a;
          } x = {in};
    ...
    const float N =
        static_cast<float>(num_reverb_decay_sections_) * kFftLengthBy2;
    accumulated_nz_ = 0.f;
