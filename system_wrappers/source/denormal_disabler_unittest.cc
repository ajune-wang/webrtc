...
// Returns a few float values greater than one meant to be divisors of the
// smallest float in order to forge denormals.
...
TEST(DenormalDisabler, DoNotZeroDenormalsIfDisabled) {
  ...
  const float kDenormalDivisor = GetDenormalDivisors().front();

  // Force the CPU to use denormals.
  DenormalDisabler::EnableDenormals();
  ASSERT_GT(kSmallest / kDenormalDivisor, 0.0f);
...
TEST(DenormalDisabler, RestoreDenormalsEnabled) {
  ...
  {
    DenormalDisabler denormal_disabler(/*enabled=*/true);
    ASSERT_FLOAT_EQ(kSmallest / kDenormalDivisor, 0.0f);
  }
  EXPECT_GT(kSmallest / kDenormalDivisor, 0.0f);
}
...
TEST(DenormalDisabler, ZeroDenormalsNested) {
  ...
  EXPECT_FLOAT_EQ(kSmallest / kDenormalDivisor, 0.0f);
}
