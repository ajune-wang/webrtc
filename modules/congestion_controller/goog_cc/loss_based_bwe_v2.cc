...
absl::optional<LossBasedBweV2::Config> LossBasedBweV2::CreateConfig(
  ...
  FieldTrialParameter<bool> backoff_when_overusing_enabled(
      "BackoffWhenOverusingEnabled", false);
  ...
  if (key_value_config) {
    ParseFieldTrial({&enabled,
                     ...
                     &backoff_when_overusing_enabled},
  ...
  config->backoff_when_overusing_enabled = backoff_when_overusing_enabled.Get();
  return config;
}
...
std::vector<LossBasedBweV2::ChannelParameters> LossBasedBweV2::GetCandidates(
  ...
  for (double candidate_factor : config_->candidate_factors) {
    if (!can_increase_bitrate && candidate_factor >= 1.0) {
      ...
      if (candidate_factor > 1 || config_->backoff_when_overusing_enabled) {
        continue;
      }
    }
