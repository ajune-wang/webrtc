...
TEST_P(RtcEventLogEncoderTest, RtcEventLoggingStopped) {
  const int64_t start_timestamp_us = rtc::TimeMillis() * 1000;
  const int64_t start_utc_time_us = rtc::TimeUTCMillis() * 1000;
  std::string encoded =
      encoder_->EncodeLogStart(start_timestamp_us, start_utc_time_us);

  const int64_t stop_timestamp_us = rtc::TimeMillis() * 1000;
