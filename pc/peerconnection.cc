...
RTCError PeerConnection::ApplyRemoteDescription(
  ...
  if (IsUnifiedPlan()) {
    ...
    for (auto transceiver : transceivers_) {
      ...
      // Due to the SetAssociatedRemoteStreams() or
      // ProcessRemovalOfRemoteTrack() steps above, streams previously
      // associated with the receiver may have been disassociated and emptied of
      // tracks. If a stream is now empty (and thus not associated with any
      // other receiver), treat it as a removed stream.
