...
int32_t RTPSender::DeregisterRtpHeaderExtension(RTPExtensionType type) {
  rtc::CritScope lock(&send_critsect_);
  bool deregistered = rtp_header_extension_map_.Deregister(type);
  supports_bwe_extension_ = HasBweExtension(rtp_header_extension_map_);
  return deregistered;
}
...
bool RTPSender::SupportsPadding(bool with_redundant_payload) const {
  rtc::CritScope lock(&send_critsect_);
  if (!sending_media_ || !supports_bwe_extension_) {
    return false;
  }

  if (with_redundant_payload && !(rtx_ & kRtxRedundantPayloads)) {
    // Redundant RTX payload packets requested but not supported.
    return false;
  }
