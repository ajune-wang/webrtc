...
absl::optional<CryptoOptions> JavaToNativeOptionalCryptoOptions(
  ...
  if (!j_srtp.is_null()) {
    native_crypto_options.srtp.enable_gcm_crypto_suites =
        Java_CryptoOptions_Srtp_getEnableGcmCryptoSuites(jni, j_srtp);
    native_crypto_options.srtp.enable_aes128_sha1_32_crypto_cipher =
        Java_CryptoOptions_Srtp_getEnableAes128Sha1_32CryptoCipher(jni, j_srtp);
    native_crypto_options.srtp.enable_encrypted_rtp_header_extensions =
        Java_CryptoOptions_Srtp_getEnableEncryptedRtpHeaderExtensions(jni,
                                                                      j_srtp);
  }
  if (!j_sframe.is_null()) {
    native_crypto_options.sframe.require_frame_encryption =
        Java_CryptoOptions_SFrame_getRequireFrameEncryption(jni, j_sframe);
  }
