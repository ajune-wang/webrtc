...
void AecState::FilteringQualityAnalyzer::Update(
  ...
  // Check whether sufficient time has passed since the convergence for the
  // filter to possibly have converged.
  const bool convergence_feasible =
      filter_update_blocks_since_reset_ > kNumBlocksPerSecond * 0.2f;
  const bool convergence_possible =
      filter_update_blocks_since_start_ > kNumBlocksPerSecond * 0.4f;

  // The filter can only be used it has not had time to converge.
  usable_linear_estimate_ = convergence_possible;

  // If the filter has had time to converge, and an external delay or
  // convergence have been identified, the linear filter is ok to use.
  usable_linear_estimate_ =
      usable_linear_estimate_ && ((convergence_feasible && external_delay) ||
                                  (convergence_feasible && convergence_seen_));
...
void AecState::SaturationDetector::Update(
  ...
  if (usable_linear_estimate) {
    ...
    saturated_echo_ = saturated_echo_ && max_sample > 1000;
  }
}
