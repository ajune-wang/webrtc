...
bool ValidateEncodeParams(
  ...
  for (size_t i = 0; i < frame_settings.size(); ++i) {
    ...
    for (size_t j = 0; j < settings.reference_buffers.size(); ++j) {
      ...
      // Will some other frame in the temporal unit update the buffer?
      ...
      // Not updated by anoter frame in the temporal unit, what is the
      // resolution of the last frame put into that buffer?
      ...
      if (!referenced_resolution) {
        RTC_LOG(LS_ERROR) << "Referenced buffer not populated.";
        return false;
      }
