...
TEST(FlexfecSenderTest, Ssrc) {
  GlobalSimulatedTimeController time_controller(
      Timestamp::Micros(kInitialSimulatedClockTime));
  const Environment env = CreateEnvironment(time_controller.GetClock());
...
TEST(FlexfecSenderTest, SetsAndGetsRtpState) {
  ...
  time_controller.AdvanceTime(TimeDelta::Millis(1000));
