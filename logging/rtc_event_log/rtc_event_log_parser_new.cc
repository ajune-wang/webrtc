...
void ParsedRtcEventLogNew::StoreParsedEvent(const rtclog::Event& event) {
  ...
  switch (GetEventType(event)) {
    ...
    case ParsedRtcEventLogNew::EventType::RTP_EVENT: {
      ...
      // Since we give the parser only a header, there should be no way for it
      // to know the padding length. The best solution would be to log the
      // padding length in RTC event log. In absence of it, we assume the RTP
      // packet to contain only padding, if the padding bit is set.
      if ((header[0] & 0x20) != 0)
