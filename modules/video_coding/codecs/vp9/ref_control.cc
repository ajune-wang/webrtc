...
vpx_svc_ref_frame_config_t ReferenceControl::SetFrameReferences(
  ...
  for (size_t i = 0; i < layers.size(); ++i) {
    ...
    enc_layer_conf.reference_last[sl_idx] = 1;

    if (layer.inter_layer_pred) {
      // Ensure previous spatial layer frame was stored in frame buffer.
      RTC_DCHECK_NE(
          enc_layer_conf.update_buffer_slot[layers[i - 1].spatial_idx], 0);
      enc_layer_conf.gld_fb_idx[sl_idx] =
          ffs(enc_layer_conf.update_buffer_slot[layers[i - 1].spatial_idx]) - 1;

      enc_layer_conf.reference_golden[sl_idx] = 1;
    }
    ...
    if (layer.ref_frame_flag[gof_idx]) {
      ...
      // Next spatial layer will use current one for prediction. Store
      // current frame in frame buffer.
      enc_layer_conf.update_buffer_slot[sl_idx] = 1 << kSpatialRefFrameBufIdx;
    }
  }
