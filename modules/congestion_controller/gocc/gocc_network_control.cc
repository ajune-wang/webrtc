...
NetworkControllerInterface::uptr GoccNetworkControllerFactory::Create() {
  return rtc::MakeUnique<GoccNetworkController>(clock_, event_log_);
}
...
GoccNetworkController::GoccNetworkController(const Clock* clock,
    ...
    : clock_(clock),
      ...
      probe_controller_(
          new ProbeController(clock_, &ProbeClusterConfigJunction)),
...
NetworkInformationReceivers GoccNetworkController::GetReceivers() {
...
NetworkControlProducers GoccNetworkController::GetProducers() {
...
void GoccNetworkController::MaybeUpdateCongestionWindow() {
  ...
  CongestionWindowJunction.OnMessage(msg);
...
void GoccNetworkController::OnNetworkEstimate(NetworkEstimate estimate) {
  ...
  TargetTransferRateJunction.OnMessage(target_rate);
}
