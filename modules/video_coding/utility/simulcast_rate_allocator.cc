...
void SimulcastRateAllocator::DistributeAllocationToSimulcastLayers(
  ...
  if (codec_.maxBitrate) {
    DataRate max_rate = DataRate::KilobitsPerSecond(codec_.maxBitrate);
  ...
  if (codec_.numberOfSimulcastStreams == 0) {
    ...
    if (codec_.active) {
      allocated_bitrates->SetBitrate(
          0, 0,
          std::max(DataRate::KilobitsPerSecond(codec_.minBitrate),
  ...
  for (; active_layer < codec_.numberOfSimulcastStreams; ++active_layer) {
    ...
    DataRate min_bitrate = DataRate::KilobitsPerSecond(stream.minBitrate);
    DataRate target_bitrate = DataRate::KilobitsPerSecond(stream.targetBitrate);
  ...
  if (left_in_total_allocation > DataRate::Zero()) {
    ...
    DataRate initial_layer_rate = DataRate::BitsPerSecond(
        allocated_bitrates->GetSpatialLayerSum(top_active_layer));
    DataRate additional_allocation = std::min(
        left_in_total_allocation,
        DataRate::KilobitsPerSecond(stream.maxBitrate) - initial_layer_rate);
    allocated_bitrates->SetBitrate(
        top_active_layer, 0,
        (initial_layer_rate + additional_allocation).BitsPerSecond());
  }
}
