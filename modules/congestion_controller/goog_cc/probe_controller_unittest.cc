...
TEST(ProbeControllerTest, InitiatesProbingOnMaxBitrateIncrease) {
  ...
  probes = probe_controller->SetEstimatedBitrate(
      kStartBitrate, BandwidthEstimateState::kDelayBasedEstimate,
      fixture.CurrentTime());
...
TEST(ProbeControllerTest, ProbesOnMaxBitrateIncreaseOnlyWhenInAlr) {
  ...
  probes = probe_controller->SetEstimatedBitrate(
      ...
      BandwidthEstimateState::kDelayBasedEstimate, fixture.CurrentTime());
...
TEST(ProbeControllerTest,
  ...
  probes = probe_controller->SetEstimatedBitrate(
      kStartBitrate, BandwidthEstimateState::kIncreasing,
      fixture.CurrentTime());
...
TEST(ProbeControllerTest, NotProbeFurtherWhenInAlrIfLossBasedDecreases) {
  ...
  probe_controller->EnablePeriodicAlrProbing(true);
  probes = probe_controller->SetEstimatedBitrate(
      kStartBitrate, BandwidthEstimateState::kDecreasing,
      fixture.CurrentTime());
...
TEST(ProbeControllerTest, InitiatesProbingOnMaxBitrateIncreaseAtMaxBitrate) {
  ...
  probes = probe_controller->Process(fixture.CurrentTime());
  probes = probe_controller->SetEstimatedBitrate(
      kMaxBitrate, BandwidthEstimateState::kDelayBasedEstimate,
      fixture.CurrentTime());
  probes = probe_controller->SetBitrates(
      kMinBitrate, kStartBitrate, kMaxBitrate + DataRate::BitsPerSec(100),
      fixture.CurrentTime());
...
TEST(ProbeControllerTest, TestExponentialProbing) {
  ...
  probes = probe_controller->SetEstimatedBitrate(
      DataRate::BitsPerSec(1000), BandwidthEstimateState::kDelayBasedEstimate,
      fixture.CurrentTime());
  EXPECT_TRUE(probes.empty());

  probes = probe_controller->SetEstimatedBitrate(
      DataRate::BitsPerSec(1800), BandwidthEstimateState::kDelayBasedEstimate,
      fixture.CurrentTime());
  EXPECT_EQ(probes.size(), 1u);
...
TEST(ProbeControllerTest, RequestProbeInAlr) {
  ...
  EXPECT_GE(probes.size(), 2u);
  probes = probe_controller->SetEstimatedBitrate(
      DataRate::BitsPerSec(500), BandwidthEstimateState::kDelayBasedEstimate,
      fixture.CurrentTime());
  ...
  probes = probe_controller->Process(fixture.CurrentTime());
  probes = probe_controller->SetEstimatedBitrate(
      DataRate::BitsPerSec(250), BandwidthEstimateState::kDelayBasedEstimate,
      fixture.CurrentTime());
...
TEST(ProbeControllerTest, TestExponentialProbingOverflow) {
  ...
  probes = probe_controller->SetEstimatedBitrate(
      60 * kMbpsMultiplier, BandwidthEstimateState::kDelayBasedEstimate,
      fixture.CurrentTime());
  EXPECT_EQ(probes.size(), 1u);
  EXPECT_EQ(probes[0].target_data_rate, 100 * kMbpsMultiplier);
  // Verify that repeated probes aren't sent.
  probes = probe_controller->SetEstimatedBitrate(
      100 * kMbpsMultiplier, BandwidthEstimateState::kDelayBasedEstimate,
      fixture.CurrentTime());
  EXPECT_TRUE(probes.empty());
}
...
TEST(ProbeControllerTest, TestAllocatedBitrateCap) {
  ...
  probes = probe_controller->SetEstimatedBitrate(
      estimated_bitrate, BandwidthEstimateState::kDelayBasedEstimate,
      fixture.CurrentTime());
...
TEST(ProbeControllerTest, ConfigurableProbingFieldTrial) {
  ...
  probes = probe_controller->SetEstimatedBitrate(
      DataRate::BitsPerSec(1100), BandwidthEstimateState::kDelayBasedEstimate,
      fixture.CurrentTime());
  EXPECT_EQ(probes.size(), 0u);

  probes = probe_controller->SetEstimatedBitrate(
      DataRate::BitsPerSec(1250), BandwidthEstimateState::kDelayBasedEstimate,
      fixture.CurrentTime());
  EXPECT_EQ(probes.size(), 1u);
...
TEST(ProbeControllerTest, LimitAlrProbeWhenLossBasedBweLimited) {
  ...
  probes = probe_controller->SetEstimatedBitrate(
      DataRate::BitsPerSec(500), BandwidthEstimateState::kDecreasing,
      fixture.CurrentTime());
...
TEST(ProbeControllerTest, PeriodicProbeAtUpperNetworkStateEstimate) {
  ...
  auto probes = probe_controller->SetBitrates(
      kMinBitrate, kStartBitrate, kMaxBitrate, fixture.CurrentTime());
  probes = probe_controller->SetEstimatedBitrate(
      DataRate::BitsPerSec(5000), BandwidthEstimateState::kDelayBasedEstimate,
      fixture.CurrentTime());
  // Expect the controller to send a new probe after 5s has passed.
...
TEST(ProbeControllerTest, AlrProbesLimitedByNetworkStateEstimate) {
  ...
  auto probes = probe_controller->SetBitrates(
      kMinBitrate, kStartBitrate, kMaxBitrate, fixture.CurrentTime());
  probes = probe_controller->SetEstimatedBitrate(
      DataRate::KilobitsPerSec(6), BandwidthEstimateState::kDelayBasedEstimate,
      fixture.CurrentTime());
  probe_controller->SetAlrStartTimeMs(fixture.CurrentTime().ms());
...
TEST(ProbeControllerTest, CanSetLongerProbeDurationAfterNetworkStateEstimate) {
  ...
  auto probes = probe_controller->SetBitrates(
      kMinBitrate, kStartBitrate, kMaxBitrate, fixture.CurrentTime());
  probes = probe_controller->SetEstimatedBitrate(
      DataRate::KilobitsPerSec(5), BandwidthEstimateState::kDelayBasedEstimate,
      fixture.CurrentTime());
  ASSERT_FALSE(probes.empty());
...
TEST(ProbeControllerTest, ProbeAfterLargeNetworkStateDropLossLimited) {
  ...
  // Loss limited.
  probes = probe_controller->SetEstimatedBitrate(
      kStartBitrate / 3, BandwidthEstimateState::kDecreasing,
      fixture.CurrentTime());
  // If NetworkState decrease dramatically, expect a probe to be sent.
...
TEST(ProbeControllerTest, ProbeFurtherWhenLossBasedIncreases) {
  ...
  probes = probe_controller->SetEstimatedBitrate(
      probe_target_rate, BandwidthEstimateState::kIncreasing,
      fixture.CurrentTime());
  EXPECT_FALSE(probes.empty());
...
TEST(ProbeControllerTest, ProbeFurtherWhenLossBasedIsSameAsDelayBasedEstimate) {
  ...
  // Expect that no more probes are sent immediately if BWE is loss limited.
  probes = probe_controller->SetEstimatedBitrate(
      probe_target_rate, BandwidthEstimateState::kDelayBasedEstimate,
      fixture.CurrentTime());
  EXPECT_FALSE(probes.empty());
  EXPECT_EQ(probes[0].target_data_rate, 2 * probe_target_rate);
}
...
TEST(ProbeControllerTest, DontProbeFurtherWhenLossBasedDecreases) {
  ...
  // Expect that no more probes are sent immediately if BWE is loss limited.
  probes = probe_controller->SetEstimatedBitrate(
      probes[0].target_data_rate, BandwidthEstimateState::kDecreasing,
      fixture.CurrentTime());
  EXPECT_TRUE(probes.empty());
}
...
TEST(ProbeControllerTest, ProbeFurtherWhenDelayBasedLimited) {
  ...
  probes = probe_controller->SetEstimatedBitrate(
      probes[0].target_data_rate, BandwidthEstimateState::kDelayBasedEstimate,
      fixture.CurrentTime());
  EXPECT_FALSE(probes.empty());
  EXPECT_EQ(probes[0].target_data_rate, state_estimate.link_capacity_upper);
}
