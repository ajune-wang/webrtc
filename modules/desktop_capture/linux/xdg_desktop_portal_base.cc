...
void XdgDesktopPortalBase::InitPortal(
  ...
  auto data = GetConnectionData(web_id);
...
bool XdgDesktopPortalBase::IsConnectionInitialized(int32_t web_id) const {
  auto connection_data = GetConnectionData(web_id);
...
PipeWireBase* XdgDesktopPortalBase::GetPipeWireBase(int32_t id) const {
  ...
  return connection_data->pw_base_.get();
}
