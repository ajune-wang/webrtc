...
absl::optional<VideoRtpDepacketizer::ParsedRtpPayload> ProcessStapAOrSingleNalu(
  ...
  uint8_t nal_type = payload_data[0] & static_cast<uint8_t>(NalDefs::kTypeMask);
  ...
  if (nal_type == H264::NaluType::kStapA) {
    ...
    nal_type = payload_data[kStapAHeaderSize] &
               static_cast<uint8_t>(NalDefs::kTypeMask);
  } else {
  ...
  for (size_t i = 0; i < nalu_start_offsets.size() - 1; ++i) {
    ...
    NaluInfo nalu;
    nalu.type =
        payload_data[start_offset] & static_cast<uint8_t>(NalDefs::kTypeMask);
    nalu.sps_id = -1;
    nalu.pps_id = -1;
...
absl::optional<VideoRtpDepacketizer::ParsedRtpPayload> ParseFuaNalu(
  ...
  absl::optional<VideoRtpDepacketizer::ParsedRtpPayload> parsed_payload(
      absl::in_place);
  uint8_t fnri =
      rtp_payload.cdata()[0] & (static_cast<uint8_t>(NalDefs::kFBit) |
                                static_cast<uint8_t>(NalDefs::kNriMask));
  uint8_t original_nal_type =
      rtp_payload.cdata()[1] & static_cast<uint8_t>(NalDefs::kTypeMask);
  bool first_fragment =
      (rtp_payload.cdata()[1] & static_cast<uint8_t>(FuDefs::kSBit)) > 0;
  NaluInfo nalu;
...
VideoRtpDepacketizerH264::Parse(rtc::CopyOnWriteBuffer rtp_payload) {
  ...
  uint8_t nal_type =
      rtp_payload.cdata()[0] & static_cast<uint8_t>(NalDefs::kTypeMask);
