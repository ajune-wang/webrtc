...
TEST_F(RtpPayloadParamsVp9ToGenericTest, SpatialScalabilityKSvc) {
  ...
  ASSERT_TRUE(headers[0].generic);
  int num_decode_targets = headers[0].generic->decode_target_indications.size();
  // Rely on implementation detail there are always 3 temporal layers assumed,
  // in particular assume Decode Target#0 matches layer S0T0,
  // and Decode Target#3 matches layer S1T0.
  ASSERT_EQ(num_decode_targets, 6);
