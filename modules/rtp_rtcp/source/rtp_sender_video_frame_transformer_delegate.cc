...
class TransformableVideoSenderFrame : public TransformableVideoFrameInterface {
 public:
  ...
  const VideoFrameMetadata& GetMetadata() const override {
    metadata_ = GetCopyOfMetadata();
    return metadata_;
  }

  VideoFrameMetadata GetCopyOfMetadata() const override {
    VideoFrameMetadata metadata = header_.GetAsMetadata();
    metadata.SetSsrc(ssrc);
    metadata.SetCsrcs(std::move(csrcs));
 ...
 private:
  ...
  // This is a copy of `GetCopyOfMetadata()`, only needed because the
