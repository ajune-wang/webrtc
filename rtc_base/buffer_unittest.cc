...
TEST(ZeroOnFreeBufferTest, TestZeroOnSetDataFromSetter) {
  ...
  const auto setter = [](rtc::ArrayView<uint8_t> av) {
    // "offset" is copied in the lambda body because Linux / Windows compilers
    // handle capturing differently: Linux captures implicitly and doesn't allow
    // explicit capturing, which Windows would require.
    constexpr size_t offset = 1;
    for (int i = 0; i != 2; ++i)
