...
TEST(CrossTrafficTest, TriggerPacketBurst) {
  NetworkEmulationManager network_manager(webrtc::Clock::GetRealTimeClock());
  ...
  CrossTraffic* traffic = network_manager.CreateCrossTraffic(
      {node_a, node_b}, absl::make_unique<IdleSendStrategy>());
...
TEST(CrossTrafficTest, PulsedPeaksSendStrategy) {
  ...
  CrossTraffic* traffic = network_manager.CreateCrossTraffic(
      {node_a, node_b}, absl::make_unique<PulsedPeaksSendStrategy>(cfg));
  ...
  ASSERT_NEAR(counter.total_packets_size(), 50 * 1024ul, 100);
}

// TEST(CrossTrafficTest, RandomWalkSendStrategy) {
//   NetworkEmulationManager network_manager(webrtc::Clock::GetRealTimeClock());
...
//   cfg.random_seed = 1;
//   cfg.peak_rate = DataRate::kbps(100);
//   cfg.min_packet_size = DataSize::bytes(200);
//   cfg.min_packet_interval = TimeDelta::ms(1);
//   cfg.update_interval = TimeDelta::ms(200);
//   cfg.variance = 0.6;
//   cfg.bias = -0.1;
//   CrossTraffic* traffic = network_manager.CreateCrossTraffic(
//       {node_a, node_b}, absl::make_unique<RandomWalkSendStrategy>(cfg));

//   CountingReceiver counter;
//   // TODO(titovartem) think how to specify cross traffic receiver on
//   creation. node_b->RemoveReceiver(traffic->GetDestinationId());
//   node_b->SetReceiver(traffic->GetDestinationId(), &counter);
