...
void WebRtcVideoSendChannel::WebRtcVideoSendStream::SetCodec(
  ...
  if (!codec_settings_list.empty()) {
    ...
    parameters_.config.rtp.streams.resize(parameters_.config.rtp.ssrcs.size());
    for (size_t i = 0; i < codec_settings_list.size(); i++) {
      auto& stream = parameters_.config.rtp.streams[i];
      const auto& cs = codec_settings_list[i];
      stream.ssrc = parameters_.config.rtp.ssrcs[i];
      if (i < parameters_.config.rtp.rids.size()) {
        stream.rid = parameters_.config.rtp.rids[i];
      }
      stream.payload_name = cs.codec.name;
      stream.payload_type = cs.codec.id;
      stream.raw_payload = cs.codec.packetization == kPacketizationParamRaw;
      if (i < parameters_.config.rtp.rtx.ssrcs.size()) {
        stream.rtx.ssrc = parameters_.config.rtp.rtx.ssrcs[i];
        stream.rtx.payload_type = cs.rtx_payload_type;
      }
    }
  }
