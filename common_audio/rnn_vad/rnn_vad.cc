...
constexpr float kWeightsScale = 1.f / 256.f;

// Input layer: fully connected.
constexpr size_t kInputLayerInputSize = rnn_vad::kFeatureVectorSize;
constexpr size_t kInputLayerOutputSize = 24;
constexpr size_t kInputLayerConnections =
    kInputLayerInputSize * kInputLayerOutputSize;
const std::array<int8_t, kInputLayerOutputSize> kInputLayerBias{
    38,  -6,  127,  127, 127,  -43, -127, 78,  127, 5,   127, 123,
    127, 127, -128, -76, -126, 28,  127,  125, -30, 127, -89, -20};
const std::array<int8_t, kInputLayerConnections> kInputLayerWeights{
...
// Hidden layer: recurrent (GRU units).
// Both the weights and the recurrent weights arrays below are structured as
// follows. Let M be the number of input units and N the number of output units.
// Each array concatenates M sub-arrays, each having size 3 x N and containing
// N update coefficients, N reset coefficients and N output coefficients.
// Hence, the stride between sub-arrays is 3 x N.
// Similarly, the bias terms array has length 3 x N with N update terms, N reset
// terms and N output terms.
constexpr size_t kHiddenLayerInputSize = kInputLayerOutputSize;
constexpr size_t kHiddenLayerOutputSize = 24;
constexpr size_t kHiddenLayerConnections =
    kHiddenLayerInputSize * kHiddenLayerOutputSize;
const std::array<int8_t, kHiddenLayerOutputSize * 3> kHiddenLayerBias{
...
const std::array<int8_t, kHiddenLayerConnections * 3> kHiddenLayerWeights{
...
const std::array<int8_t, kHiddenLayerConnections * 3>
...
constexpr size_t kOutputLayerInputSize = kHiddenLayerOutputSize;
constexpr size_t kOutputLayerOutputSize = 1;
constexpr size_t kOutputLayerConnections =
    kOutputLayerInputSize * kOutputLayerOutputSize;
const std::array<int8_t, kOutputLayerOutputSize> kOutputLayerBias{-50};
const std::array<int8_t, kOutputLayerConnections> kOutputLayerWeights{
    127,  127,  127, 127,  127,  20,  127,  -126, -126, -54, 14,  125,
    -126, -126, 127, -125, -126, 127, -127, -127, -57,  -30, 127, 80};

}  // namespace

using rnn_vad::kFullyConnectedLayersMaxUnits;
using rnn_vad::kRecurrentLayersMaxUnits;
