...
class StreamState {
 public:
  ...
  // Marks that peer `peer` stopped receiving this stream intentionally. In such
  // case related frames are still stored in the state, but are known as paused.
  // It is done because peer `peer` still can receive frames due to various
  // delays.
  void PauseForPeer(size_t peer);

  // Marks that peer `peer` is now expected to continue receive frames for this
  // stream.
  void ResumeForPeer(size_t peer);

  bool IsPausedForPeer(size_t peer) const;

  bool WasPausedForPeerAt(size_t peer, Timestamp time) const;

  bool WasResumedForPeerAfter(size_t peer, Timestamp time) const;

  TimeDelta GetActiveDurationForPeerFrom(size_t peer, Timestamp time) const;

  Timestamp GetLastEventTimeForPeer(size_t peer) const;
