...
int32_t IvfVideoFrameReader::DecodedCallback::Decoded(
    VideoFrame& decoded_image) {
  Decoded(decoded_image, 0, 0);
  return WEBRTC_VIDEO_CODEC_OK;
}
int32_t IvfVideoFrameReader::DecodedCallback::Decoded(VideoFrame& decoded_image,
                                                      int64_t decode_time_ms) {
  Decoded(decoded_image, decode_time_ms, 0);
  return WEBRTC_VIDEO_CODEC_OK;
}
void IvfVideoFrameReader::DecodedCallback::Decoded(
    VideoFrame& decoded_image,
...
void IvfVideoFrameReader::OnFrameDecoded(const VideoFrame& decoded_frame) {
  rtc::CritScope crit(&lock_);
