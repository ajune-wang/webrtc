...
RtpFrameReferenceFinder::ManageFrameInternal(RtpFrameObject* frame) {
  ...
  switch (frame->codec_type()) {
    ...
    default: {
      ...
      if (video_header.picture_id)
        picture_id = *video_header.picture_id & 0x7fff;
...
RtpFrameReferenceFinder::ManageFrameGeneric(RtpFrameObject* frame) {
  const RTPVideoHeader& video_header = frame->GetRtpVideoHeader();
  frame->id.picture_id = *video_header.frame_id;
  frame->id.spatial_layer = video_header.spatial_index;

  const auto& deps = video_header.frame_dependencies;
  if (EncodedFrame::kMaxFrameReferences < deps.size()) {
    RTC_LOG(LS_WARNING) << "Too many dependencies in generic descriptor.";
    return kDrop;
  }

  frame->num_references = deps.size();
  for (size_t i = 0; i < deps.size(); ++i)
    frame->references[i] = deps[i];
