...
VideoDecoderFactoryWrapper::VideoDecoderFactoryWrapper(JNIEnv* jni,
    ...
    : decoder_factory_(jni, decoder_factory),
      wrapped_native_decoder_class_(
          jni,
          jni->FindClass("org/webrtc/WrappedNativeVideoDecoder")) {
  ...
  get_native_decoder_method_ = jni->GetMethodID(*wrapped_native_decoder_class_,
                                                "getNativeDecoder", "()J");
}
...
std::unique_ptr<VideoDecoder> VideoDecoderFactoryWrapper::CreateVideoDecoder(
  ...
  if (decoder != nullptr) {
    if (jni->IsInstanceOf(decoder, *wrapped_native_decoder_class_)) {
      jlong native_decoder =
          jni->CallLongMethod(decoder, get_native_decoder_method_);
      return std::unique_ptr<VideoDecoder>((VideoDecoder*)native_decoder);
    }
    return std::unique_ptr<VideoDecoder>(new VideoDecoderWrapper(jni, decoder));
  }
  return nullptr;
}
