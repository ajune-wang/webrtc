...
TEST_F(PeerConnectionBundleUnitTest,
  ...
  // Wait at least 50ms to ensure that the next call to GetStats is not stale.
  rtc::Thread::Current()->SleepMs(50);
  EXPECT_TRUE(caller->HasConnectionWithRemoteAddress(kAudioAddress1));
  EXPECT_FALSE(caller->HasConnectionWithRemoteAddress(kVideoAddress));
  EXPECT_TRUE(caller->HasConnectionWithRemoteAddress(kAudioAddress2));
}
