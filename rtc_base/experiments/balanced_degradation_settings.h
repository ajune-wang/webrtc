...
class BalancedDegradationSettings {
 public:
  ...
  struct CodecTypeSpecific {
    CodecTypeSpecific() {}
    CodecTypeSpecific(int low, int high, int fps)
        : low(low), high(high), fps(fps) {}

    bool operator==(const CodecTypeSpecific& o) const {
      return low == o.low && high == o.high && fps == o.fps;
    }
  ...
  struct Config {
    ...
    CodecTypeSpecific vp8;  // size is less than or equal to |pixels|.
    CodecTypeSpecific vp9;
    CodecTypeSpecific h264;
    CodecTypeSpecific generic;
  };
