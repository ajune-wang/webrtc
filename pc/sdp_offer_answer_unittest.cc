...
TEST_F(SdpOfferAnswerTest,
       InitialSetLocalDescriptionWithoutCreateOfferRejected) {
  RTCConfiguration config;
  ...
  auto offer = CreateSessionDescription(SdpType::kOffer, sdp);
  RTCError error;
  // TODO: crbug.com/40567530 - this should reject with INVALID_PARAMETER.
  EXPECT_TRUE(pc->SetLocalDescription(std::move(offer), &error));
}

TEST_F(SdpOfferAnswerTest,
       InitialSetLocalDescriptionWithoutCreateAnswerIsRejected) {
  RTCConfiguration config;
  config.certificates.push_back(
      FakeRTCCertificateGenerator::GenerateCertificate());
  auto pc = CreatePeerConnection(config);
