...
void VideoAnalyzer::PrintResults() {
  ...
  PrintResult("media_bitrate", media_bitrate_bps_, 0.001, "bytesPerSecond");
  PrintResult("fec_bitrate", fec_bitrate_bps_, 0.001, "bytesPerSecond");
  PrintResult("send_bandwidth", send_bandwidth_bps_, 0.001, "bytesPerSecond");
  PrintResult("pixels_per_frame", pixels_, "unitless");
...
void VideoAnalyzer::PrintResult(const char* result_type,
  ...
  test::PrintResultMeanAndError(
      ...
      stats.GetMean().value_or(0) * multiplier,
      stats.GetStandardDeviation().value_or(0) * multiplier, unit, false);
}
