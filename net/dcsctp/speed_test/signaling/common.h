...
#define MAP_HAS2(m, k1, k2) \
  ((MAP_HAS1((m), (k1)) == true) ? MAP_HAS1((m)[(k1)], (k2)) : false)
#define MAP_HAS3(m, k1, k2, k3) \
  ((MAP_HAS1((m), (k1))) ? MAP_HAS2((m)[(k1)], (k2), (k3)) : false)
#define FOR_MAP(m, k, v, i) \
  for (std::map<k, v>::iterator i = (m).begin(); i != (m).end(); i++)
#define MAP_KEY(i) ((i)->first)
#define MAP_VAL(i) ((i)->second)
#define MAP_ERASE1(m, k)  \
  if (MAP_HAS1((m), (k))) \
    (m).erase((k));
...
#define FOR_VECTOR_ITERATOR(e, v, i) \
  for (vector<e>::iterator i = (v).begin(); i != (v).end(); i++)
...
#define EHTONLP(pNetworkPointer, hostLongValue) \
  (*((uint32_t*)(pNetworkPointer)) = EHTONL(hostLongValue))
...
bool setFdJoinIPv4Multicast(SOCKET_TYPE sock,
                            std::string bindIp,
                            uint16_t bindPort,
                            const std::string& ssmIp);
...
#define MAKE_TAG8(a, b, c, d, e, f, g, h)                        \
  ((uint64_t)(((uint64_t)(a)) << 56) | (((uint64_t)(b)) << 48) | \
