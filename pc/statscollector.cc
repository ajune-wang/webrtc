...
void ExtractStats(const cricket::VideoReceiverInfo& info, StatsReport* report) {
  ...
  if (info.first_frame_received_to_decoded_delay_ms >= 0) {
    report->AddInt64(
        StatsReport::kStatsValueNameFirstFrameReceivedToDecodedDelayMs,
        info.first_frame_received_to_decoded_delay_ms);
  }
  if (info.qp_sum)
