...
absl::optional<DataRate> SimplifiedAcknowledgedBitrateEstimator::bitrate()
  ...
  return data_size / std::max(send_duration, recv_duration);
  /*
  DataRate send_rate(DataRate::Infinity());
  if (send_duration > TimeDelta::ms(10))
          send_rate = data_size / send_duration;
  DataRate recv_rate(DataRate::Infinity());
  if (recv_duration > TimeDelta::ms(10))
           recv_rate = data_size / recv_duration;
  DataRate rate = std::min(send_rate, recv_rate);
  if (rate.IsFinite())
    return rate;
  return absl::nullopt;
  */
