...
bool D3dDevice::Initialize(const ComPtr<IDXGIAdapter>& adapter) {
  ...
  if (feature_level < D3D_FEATURE_LEVEL_11_0) {
    RTC_LOG(LS_WARNING) << "D3D11CreateDevice returns an instance without "
                        << "DirectX 11 support, level " << feature_level
  ...
  if (error.Error() != S_OK || !dxgi_device_) {
    RTC_LOG(LS_WARNING) << "ID3D11Device is not an implementation of "
                        << "IDXGIDevice, this usually means the system does "
                        << "not support DirectX 11. Error "
...
std::vector<D3dDevice> D3dDevice::EnumDevices() {
  ...
  for (int i = 0;; i++) {
    ...
    if (error.Error() == S_OK) {
      ...
      RTC_LOG(LS_WARNING) << "IDXGIFactory1::EnumAdapters returns an "
                          << "unexpected error "
                          << error.ErrorMessage() << " with code "
                          << error.Error();
    }
