...
static const char kDefaultEmptyMsid[] = "-";
...
void CreateTracksFromSsrcInfos(const SsrcInfoVec& ssrc_infos,
  ...
  for (SsrcInfoVec::const_iterator ssrc_info = ssrc_infos.begin();
    ...
    if (msid_signaling & cricket::kMsidSignalingMediaSection) {
      ...
      stream_ids.insert(stream_ids.begin(), msid_stream_ids.begin(),
                        msid_stream_ids.end());
...
void BuildRtpContentAttributes(const MediaContentDescription* media_desc,
  ...
  // Specified in https://datatracker.ietf.org/doc/draft-ietf-mmusic-msid/
  ...
  if (msid_signaling & cricket::kMsidSignalingMediaSection) {
    ...
    if (streams.size() == 1u) {
      ...
      if (track.stream_ids().empty()) {
        stream_ids.push_back(kDefaultEmptyMsid);
      }
...
static bool ParseMsidAttribute(const std::string& line,
  ...
  *track_id = new_track_id;
  ...
  if (new_stream_id.compare(kDefaultEmptyMsid) != 0) {
