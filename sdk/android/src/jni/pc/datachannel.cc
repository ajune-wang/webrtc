...
DataChannelInit JavaToNativeDataChannelInit(JNIEnv* env, jobject j_init) {
  ...
  init.ordered = Java_DataChannel_Init_getOrdered(env, j_init);
  init.maxRetransmitTime =
      Java_DataChannel_Init_getMaxRetransmitTimeMs(env, j_init);
  init.maxRetransmits = Java_DataChannel_Init_getMaxRetransmits(env, j_init);
  init.protocol =
      JavaToStdString(env, Java_DataChannel_Init_getProtocol(env, j_init));
  init.negotiated = Java_DataChannel_Init_getNegotiated(env, j_init);
  init.id = Java_DataChannel_Init_getId(env, j_init);
  return init;
}
...
jobject WrapNativeDataChannel(
  ...
  return Java_DataChannel_create(env, jlongFromPointer(channel.release()));
}
