...
/*
    RtcpMode GetRuntimeRtcpMode(rtclog2::VideoReceiveConfig::RtcpMode rtcp_mode)
   { switch (rtcp_mode) { case rtclog2::VideoReceiveConfig::RTCP_COMPOUND:
                return RtcpMode::kCompound;
            case rtclog2::VideoReceiveConfig::RTCP_REDUCEDSIZE:
                return RtcpMode::kReducedSize;
        }
        RTC_NOTREACHED();
        return RtcpMode::kOff;
...
bool ParsedRtcEventLogNew::ParseStreamInternal(
    std::istream& stream) {  // no-presubmit-check TODO(webrtc:8982)
  const uint64_t kMaxEventSize = 10000000;  // Sanity check.
...
void ParsedRtcEventLogNew::StoreAlrStateEvent(const rtclog2::AlrState& proto) {
  RTC_DCHECK(proto.has_timestamp_ms());
  RTC_DCHECK(proto.has_in_alr());
...
void ParsedRtcEventLogNew::StoreAudioPlayoutEvent(
  ...
  RTC_DCHECK(proto.has_timestamp_ms());
  RTC_DCHECK(proto.has_local_ssrc());
...
void ParsedRtcEventLogNew::StoreIncomingRtpPackets(
  ...
  RTC_DCHECK(proto.has_packet_size());
  size_t total_length = proto.packet_size();

  RTC_DCHECK(proto.has_header_size());
  size_t header_length = proto.header_size();

  RTC_DCHECK(proto.has_padding_size());
  size_t padding_length = proto.padding_size();

  RTPHeader header;
  RTC_DCHECK(proto.has_marker());
  header.markerBit = proto.marker();
  RTC_DCHECK(proto.has_payload_type());
  header.payloadType = proto.payload_type();
  RTC_DCHECK(proto.has_sequence_number());
  header.sequenceNumber = proto.sequence_number();
  RTC_DCHECK(proto.has_rtp_timestamp());
  header.timestamp = proto.rtp_timestamp();
  RTC_DCHECK(proto.has_ssrc());
...
void ParsedRtcEventLogNew::StoreIncomingRtcpPackets(
  ...
  RTC_DCHECK(proto.has_raw_packet());
  incoming_rtcp_packets_.push_back(
...
void ParsedRtcEventLogNew::StoreBweLossBasedUpdate(
  ...
  RTC_DCHECK(proto.has_timestamp_ms());
  RTC_DCHECK(proto.has_bitrate_bps());
  RTC_DCHECK(proto.has_fraction_loss());
  RTC_DCHECK(proto.has_total_packets());
...
void ParsedRtcEventLogNew::StoreBweDelayBasedUpdate(
  ...
  RTC_DCHECK(proto.has_timestamp_ms());
  RTC_DCHECK(proto.has_bitrate_bps());
  RTC_DCHECK(proto.has_detector_state());
...
void ParsedRtcEventLogNew::StoreIceCandidateEvent(
  ...
  RTC_DCHECK(proto.has_event_type());
...
void ParsedRtcEventLogNew::StoreVideoRecvConfig(
  ...
  RTC_DCHECK(proto.has_remote_ssrc());  // TODO(terelius): DON'T FORGET TO
                                        // CHANGE DCHECKS TO CHECKS.
...
void ParsedRtcEventLogNew::StoreVideoSendConfig(
  ...
  stream.timestamp_us = proto.timestamp_ms() * 1000;
  rtclog::StreamConfig config;
  RTC_DCHECK(proto.has_ssrc());  // TODO(terelius): DON'T FORGET TO CHANGE
                                 // DCHECKS TO CHECKS.
