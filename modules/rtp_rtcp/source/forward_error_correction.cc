...
int ForwardErrorCorrection::EncodeFec(const PacketList& media_packets,
  ...
  if (num_media_packets > max_media_packets) {
    NLOG(LS_WARNING, "Can't protect ", num_media_packets,
  ...
  for (const auto& media_packet : media_packets) {
    ...
    if (media_packet->length < kRtpHeaderSize) {
      NLOG(LS_WARNING, "Media packet ", media_packet->length, " bytes ",
    ...
    if (media_packet->length + MaxPacketOverhead() + kTransportOverhead >
        IP_PACKET_SIZE) {
      NLOG(LS_WARNING, "Media packet ", media_packet->length, " bytes ",
           "with overhead is larger than ", IP_PACKET_SIZE, " bytes.");
    }
  }
  ...
  if (num_mask_bits < 0) {
    NLOG(LS_INFO,
         "Due to sequence number gaps, cannot protect media "
...
void ForwardErrorCorrection::InsertFecPacket(
  ...
  if (fec_packet->protected_packets.empty()) {
    ...
    NLOG(LS_WARNING, "Received FEC packet has an all-zero packet mask.");
  } else {
...
bool ForwardErrorCorrection::StartPacketRecovery(
  ...
  if (fec_packet.protection_length >
    ...
    NLOG(LS_WARNING, "Incorrect protection length, dropping FEC packet.");
    return false;
  }
...
bool ForwardErrorCorrection::FinishPacketRecovery(
  ...
  if (recovered_packet->pkt->length >
    ...
    NLOG(LS_WARNING, "The recovered packet had a length larger than a ",
  ...
  if (recovered_packets->size() == max_media_packets) {
    ...
    if (received_packet.ssrc == back_recovered_packet->ssrc) {
      ...
      if (seq_num_diff > max_media_packets) {
        ...
        NLOG(LS_INFO,
             "Big gap in media/ULPFEC sequence numbers. No need "
