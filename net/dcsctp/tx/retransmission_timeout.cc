...
void RetransmissionTimeout::ObserveRTT(DurationMs measured_rtt) {
  ...
  if (first_measurement_) {
    shifted_srtt_ = rtt << kRttShift;
    shifted_rtt_var_ = (rtt / 2) << kRttVarShift;
    first_measurement_ = false;
  } else {
    rtt -= (shifted_srtt_ >> kRttShift);
    shifted_srtt_ += rtt;
    if (rtt < 0) {
      rtt = -rtt;
    }
    rtt -= (shifted_rtt_var_ >> kRttVarShift);
    shifted_rtt_var_ += rtt;
  }
  rto_ = (shifted_srtt_ >> kRttShift) + shifted_rtt_var_;
