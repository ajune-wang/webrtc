...
NullReceiver::NullReceiver(std::string id) : NetworkReceiverInterface(id) {}

void NullReceiver::DeliverPacketInternal(
    std::unique_ptr<EmulatedIpPacket> packet) {}

ActionReceiver::ActionReceiver(std::string id, std::function<void()> action)
    : NetworkReceiverInterface(id), action_(action) {}

void ActionReceiver::DeliverPacketInternal(
    std::unique_ptr<EmulatedIpPacket> packet) {
  action_();
}
...
NetworkNode::NetworkNode(std::string id,
                         NetworkNodeConfig config,
                         std::unique_ptr<NetworkBehaviorInterface> behavior)
    : NetworkReceiverInterface(id),
      packet_overhead_(config.packet_overhead.bytes()),
      behavior_(std::move(behavior)) {}

void NetworkNode::SetRoute(std::string receiver,
                           NetworkReceiverInterface* node) {
  rtc::CritScope crit(&crit_sect_);
  routing_[receiver] = node;
}

void NetworkNode::ClearRoute(std::string receiver_id) {
  rtc::CritScope crit(&crit_sect_);
  auto it = routing_.find(receiver_id);
  routing_.erase(it);
}

void NetworkNode::DeliverPacketInternal(
    std::unique_ptr<EmulatedIpPacket> packet) {
  rtc::CritScope crit(&crit_sect_);
...
void NetworkNode::Process(Timestamp at_time) {
  ...
  for (PacketDeliveryInfo& delivery_info : delivery_infos) {
    ...
    receiver->DeliverPacket(std::move(packet->packet));
    {
      rtc::CritScope crit(&crit_sect_);
...
void NetworkNode::Route(std::string receiver_id,
...
std::unique_ptr<SimulationNode> SimulationNode::Create(
  ...
  return std::unique_ptr<SimulationNode>(
      new SimulationNode(id, config, std::move(network), simulation_ptr));
}
...
SimulationNode::SimulationNode(
    ...
    : NetworkNode(id, config, std::move(behavior)),
...
bool NetworkNodeTransport::SendRtp(const uint8_t* packet,
  ...
  send_net_->DeliverPacket(absl::make_unique<EmulatedIpPacket>(
...
void NetworkNodeTransport::Connect(NetworkNode* send_node,
  ...
  route.local_network_id = std::stoi(receiver_id);
  route.remote_network_id = std::stoi(receiver_id);
...
void CrossTrafficSource::Process(Timestamp at_time, TimeDelta delta) {
  ...
  if (pending_size_ > config_.min_packet_size) {
    target_->DeliverPacket(absl::make_unique<EmulatedIpPacket>(
