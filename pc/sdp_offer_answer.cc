...
bool SdpOfferAnswerHandler::CreateDataChannel(const std::string& mid) {
  ...
  switch (pc_->data_channel_type()) {
    ...
    default:
      ...
      auto* data_channel =
          pc_->worker_thread()->Invoke<cricket::RtpDataChannel*>(
              RTC_FROM_HERE,
              [this, mid, config = pc_->configuration()->media_config,
               srtp = pc_->SrtpRequired(), crypto = pc_->GetCryptoOptions(),
               options = video_options(), rtp_transport,
               ssrc_generator = &ssrc_generator_]() {
                return channel_manager()->CreateRtpDataChannel(
                    config, rtp_transport, signaling_thread(), mid, srtp,
                    crypto, ssrc_generator);
              });
      if (!data_channel)
        return false;
