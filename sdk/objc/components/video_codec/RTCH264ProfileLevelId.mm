...
NSString *MaxSupportedLevelForProfile(H264Profile profile) {
  const absl::optional<H264ProfileLevelId> profileLevelId = [UIDevice maxSupportedH264Profile];
  if (profileLevelId && profileLevelId->profile >= profile) {
    const absl::optional<std::string> profileString =
        H264ProfileLevelIdToString(H264ProfileLevelId(profile, profileLevelId->level));
    if (profileString) {
