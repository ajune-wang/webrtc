  ...
  for (size_t i = 0; i < parsed_stream.GetNumberOfEvents(); i++) {
    ...
    switch (parsed_stream.GetEventType(i)) {
      ...
      case webrtc::rtceventlog::ParsedRtcEventLog::
        ...
        if (FLAG_ana) {
          ...
          if (ana_event.config.bitrate_bps) {
            ss << "\tbitrate_bps=" << *ana_event.config.bitrate_bps;
          }
          if (ana_event.config.frame_length_ms) {
            ss << "\tframe_length_ms=" << *ana_event.config.frame_length_ms;
          }
          ...
          if (ana_event.config.enable_fec) {
            ss << "\tenable_fec=" << *ana_event.config.enable_fec;
          }
          if (ana_event.config.enable_dtx) {
            ss << "\tenable_dtx=" << *ana_event.config.enable_dtx;
          }
          if (ana_event.config.num_channels) {
            ss << "\tnum_channels=" << *ana_event.config.num_channels;
          }
      ...
      case webrtc::rtceventlog::ParsedRtcEventLog::BWE_PROBE_RESULT_EVENT: {
        if (FLAG_probe) {
          webrtc::rtceventlog::ParsedRtcEventLog::BweProbeResultEvent
              probe_result = parsed_stream.GetBweProbeResult(i);
      ...
      case webrtc::rtceventlog::ParsedRtcEventLog::ALR_STATE_EVENT: {
        if (FLAG_bwe) {
          webrtc::rtceventlog::ParsedRtcEventLog::AlrStateEvent alr_state =
      ...
      case webrtc::rtceventlog::ParsedRtcEventLog::ICE_CANDIDATE_PAIR_CONFIG: {
        if (FLAG_ice) {
          webrtc::rtceventlog::ParsedRtcEventLog::IceCandidatePairConfig
              ice_cp_config = parsed_stream.GetIceCandidatePairConfig(i);
      ...
      case webrtc::rtceventlog::ParsedRtcEventLog::ICE_CANDIDATE_PAIR_EVENT: {
        if (FLAG_ice) {
          webrtc::rtceventlog::ParsedRtcEventLog::IceCandidatePairEvent
              ice_cp_event = parsed_stream.GetIceCandidatePairEvent(i);
          // TODO(qingsi): convert the numeric representation of states to text
          std::cout << parsed_stream.GetTimestamp(i)
