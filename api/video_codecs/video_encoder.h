...
class RTC_EXPORT VideoEncoder {
 public:
  ...
  //
  // Input:
  //          - packet_loss_rate  : The packet loss rate (0.0 to 1.0).
  //
  // Return value                 : WEBRTC_VIDEO_CODEC_OK if OK, < 0 otherwise.
  virtual int32_t OnPacketLossRateUpdate(float packet_loss_rate);

  // Inform the encoder when the round trip time changes.
  //
  // Input:
  //          - rtt_ms            : The new RTT, in milliseconds.
  //
  // Return value                 : WEBRTC_VIDEO_CODEC_OK if OK, < 0 otherwise.
  virtual int32_t OnRttUpdate(int64_t rtt_ms);
