...
uint32_t RtpConfig::GetMediaSsrcAssociatedWithFlexfecSsrc(
  ...
  //
  // The current implementation only supports an association with a single media
  // ssrc. If multiple ssrcs are to be supported in the future, in order not to
  // break GetStats()'s packet and byte counters, we must be able to tell how
  // many packets and bytes have contributed to which SSRC.
