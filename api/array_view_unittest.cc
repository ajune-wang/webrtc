...
TEST(ArrayViewTest, TestArrayView) {
  ...
  // Call with const float input of nonfixed size using ArrayView from maker
  // method.
  ...
  EXPECT_EQ(kArrSize, len);
  len = CallFixed<const float, kArrSize>(MakeArrayViewFixed(&arr));
  EXPECT_EQ(kArrSize, len);
  len = CallFixed<const float, kArrSize>(MakeArrayViewFixedConst(arr));
  EXPECT_EQ(kArrSize, len);

  // Call with const float input of fixed size using ArrayView from maker
  // method.
