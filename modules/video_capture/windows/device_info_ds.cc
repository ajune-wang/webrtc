...
int32_t DeviceInfoDS::Init() {
  ...
  if (hr != NOERROR) {
    NLOG(LS_INFO, "Failed to create CLSID_SystemDeviceEnum, error 0x", std::hex,
         hr);
    return -1;
  }
  return 0;
...

{
  ...
  HRESULT hr = _dsDevEnum->CreateClassEnumerator(CLSID_VideoInputDeviceCategory,
                                                 &_dsMonikerDevEnum, 0);
  if (hr != NOERROR) {
    NLOG(LS_INFO, "Failed to enumerate CLSID_SystemDeviceEnum, error 0x",
        ...
        if ((wcsstr(varName.bstrVal, (L"(VFW)")) == NULL) &&
          ...
          if (index == static_cast<int>(deviceNumber)) {
            ...
            if (deviceNameLength > 0) {
              ...
              if (convResult == 0) {
                NLOG(LS_INFO, "Failed to convert device name to UTF8, ",
                     "error = ", GetLastError());
                return -1;
              }
            }
            if (deviceUniqueIdUTF8Length > 0) {
              ...
              if (FAILED(hr)) {
                ...
                NLOG(LS_INFO, "Failed to get ", "deviceUniqueIdUTF8 using ",
                     "deviceNameUTF8");
              } else {
                ...
                if (convResult == 0) {
                  NLOG(LS_INFO, "Failed to convert device ",
  ...
  if (deviceNameLength) {
    NLOG(LS_INFO, __FUNCTION__, " ", deviceNameUTF8);
  }
  return index;
}
      ...
      if (deviceUniqueIdUTF8Length > 0) {
        ...
        if (SUCCEEDED(hr)) {
          ...
          if (strncmp(tempDevicePathUTF8, (const char*)deviceUniqueIdUTF8,
            ...
            if
              FAILED(hr) {
                NLOG(LS_ERROR, "Failed to bind to the selected ",
                     "capture device ", hr);
              }
...

{
  ...
  if (!outputCapturePin) {
    NLOG(LS_INFO, "Failed to get capture device output pin");
  ...
  if (SUCCEEDED(hr) && extDevice) {
    NLOG(LS_INFO, "This is an external device");
    extDevice->Release();
  }
  ...
  hr = outputCapturePin->QueryInterface(IID_IAMStreamConfig,
                                        (void**)&streamConfig);
  if (FAILED(hr)) {
    NLOG(LS_INFO, "Failed to get IID_IAMStreamConfig interface ",
         "from capture device");
    return -1;
  }
  ...
  if (FAILED(hrVC)) {
    NLOG(LS_INFO, "IID_IAMVideoControl Interface NOT SUPPORTED");
  }
  ...
  if (FAILED(hr)) {
    NLOG(LS_INFO, "Failed to GetNumberOfCapabilities");
  ...
  for (int32_t tmp = 0; tmp < count; ++tmp) {
    ...
    if (!FAILED(hr)) {
      if (pmt->majortype == MEDIATYPE_Video &&
        ...
        NLOG(LS_INFO, "Device support FORMAT_VideoInfo2");
  ...
  for (int32_t tmp = 0; tmp < count; ++tmp) {
    hr = streamConfig->GetStreamCaps(tmp, &pmt, reinterpret_cast<BYTE*>(&caps));
    if (FAILED(hr)) {
      NLOG(LS_INFO, "Failed to GetStreamCaps");
      RELEASE_AND_CLEAR(videoControlConfig);
      RELEASE_AND_CLEAR(streamConfig);
      RELEASE_AND_CLEAR(outputCapturePin);
      RELEASE_AND_CLEAR(captureDevice);
    ...
    if (pmt->majortype == MEDIATYPE_Video &&
      ...
      if (hrVC == S_OK) {
        ...
        {
          NLOG(LS_INFO, "GetMaxAvailableFrameRate NOT SUPPORTED");
      ...
      {
        NLOG(LS_INFO, "Device support HDYC.");
        capability.videoType = VideoType::kUYVY;
      } else {
        WCHAR strGuid[39];
        StringFromGUID2(pmt->subtype, strGuid, 39);
        NLOG(LS_WARNING, "Device support unknown media type ", strGuid,
      ...
      NLOG(LS_INFO, "Camera capability, width:", capability.width,
...
void DeviceInfoDS::GetProductId(const char* devicePath,
  ...
  *productUniqueIdUTF8 = '\0';
  ...
  if (!startPos) {
    ...
    NLOG(LS_INFO, "Failed to get the product Id");
    return;
  }
  startPos += 4;
