...
namespace webrtc {
namespace rnn_vad {
namespace pitch_search_internal {
namespace {
...
size_t PitchPseudoInterpolationLagPitchBuf(
  ...
  if (lag > 0 && lag < kPitchMaxPeriod24kHz) {
...
// checked T/4 before. Instead, we look at T*3/8. The values are chosen in order
// to look at peaks that we would not expect for a different pitch.
...
PitchInfo CheckLowerPitchPeriodsAndComputePitchGain(
  ...
  best_pitch.xy = ComputeAutoCorrelationCoeff(
      pitch_buf, GetInvertedLag(best_pitch.period_24kHz), kPitchMaxPeriod24kHz);
