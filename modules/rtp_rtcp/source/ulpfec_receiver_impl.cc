...
bool UlpfecReceiverImpl::AddReceivedRedPacket(const RtpPacket& rtp_packet,
  ...
  if (received_packet->is_fec) {
    ...
    received_packet->pkt->data.SetSize(
        rtp_packet.size() - rtp_packet.padding_size() - kRedHeaderLength);
    // Copy RTP header.
    memcpy(received_packet->pkt->data.data(), rtp_packet.data(),
    ...
    // Copy payload data.
    if (rtp_packet.payload_size() > kRedHeaderLength) {
      memcpy(received_packet->pkt->data.data() + rtp_packet.headers_size(),
             red_payload.data(), red_payload.size());
    }
  }
