...
- (void)connectToRoomWithId:(NSString *)roomId
  ...
  _factory = [[RTC_OBJC_TYPE(RTCPeerConnectionFactory) alloc] initWithEncoderFactory:encoderFactory
...
- (void)startSignalingIfReady {
  ...
  RTC_OBJC_TYPE(RTCCertificate) *pcert = [RTC_OBJC_TYPE(RTCCertificate) generateCertificateWithParams:@{
    @"expires" : @100000,
...
- (void)createMediaSenders {
  ...
  if (_localVideoTrack) {
    ...
    // RTC_OBJC_TYPE(RTCRtpReceiver) with a track. The track will automatically get unmuted and produce frames
    // once RTP is received.
    RTC_OBJC_TYPE(RTCVideoTrack) *track = (RTC_OBJC_TYPE(RTCVideoTrack) *)([self videoTransceiver].receiver.track);
...
- (RTC_OBJC_TYPE(RTCVideoTrack) *)createLocalVideoTrack {
  ...
  if (self.isBroadcast) {
    ...
    RTC_OBJC_TYPE(RTCCameraVideoCapturer) *capturer = [[RTC_OBJC_TYPE(RTCCameraVideoCapturer) alloc] initWithDelegate:source];
    [_delegate appClient:self didCreateLocalCapturer:capturer];
  }
#else
#if defined(__IPHONE_11_0) && (__IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_11_0)
  if (@available(iOS 10, *)) {
    RTC_OBJC_TYPE(RTCFileVideoCapturer) *fileCapturer = [[RTC_OBJC_TYPE(RTCFileVideoCapturer) alloc] initWithDelegate:source];
