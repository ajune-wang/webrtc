...
TEST(AudioProcessingImplTest, PreAmplifierAndGainController2MutualExclusive) {
  // Enabling at the same time the pre-amplifier and AGC2 is not allowed even if
  // the gain factor is the same.
  constexpr float kGainFactor = 2.f;
  webrtc::AudioProcessing::Config apm_config;
  ...
  EXPECT_DEATH(apm->ProcessStream(&frame), "");
}
#endif
