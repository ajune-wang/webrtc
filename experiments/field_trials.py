...
def validate_field_trials(
    ...
    """
    invalid = []
    for f in field_trials:
        if not f.key.startswith('WebRTC-'):
            invalid.append(f'{f.key} does not start with "WebRTC-".')
        if len(f.bug) <= 0:
            invalid.append(f'{f.key} must have an associated bug.')
    return invalid
