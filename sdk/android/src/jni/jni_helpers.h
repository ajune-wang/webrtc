...
ScopedJavaLocalRef<jobject> JavaIntegerFromOptionalInt(
    JNIEnv* jni,
...
class Iterable {
 public:
  ...
  class Iterator {
   ...
   private:
    ...
    JNIEnv* jni_ = nullptr;
    ScopedJavaLocalRef<jobject> iterator_ = nullptr;
    ScopedJavaLocalRef<jobject> value_ = nullptr;
 ...
 private:
  JNIEnv* jni_;
  // Raw jobject. Requires the caller of the ctor to keep it alive during the
  // lifetime of the Iterable. This is valid when the Iterable is used in a
  // for-each loop.
...
std::vector<T> JavaToNativeVector(JNIEnv* env,
  ...
  for (size_t i = 0; i < size; ++i) {
    // TODO(magjed): Param or scoped java local ref?
