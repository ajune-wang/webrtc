...
TEST_F(VideoStreamEncoderTest, EncoderResetAccordingToParameterChange) {
  const float downscaleFactors[] = {4.0, 2.0, 1.0};
  const int numberLayers = 3;
  VideoEncoderConfig config;
  test::FillEncoderConfiguration(kVideoCodecVP8, numberLayers, &config);
  for (int i = 0; i < numberLayers; ++i) {
    config.simulcast_layers[i].scale_resolution_down_by = downscaleFactors[i];
  ...
  sink_.SetNumExpectedLayers(numberLayers);
  int64_t timestamp_ms = kFrameIntervalMs;
  video_source_.IncomingCapturedFrame(CreateFrame(timestamp_ms, 1280, 720));
  WaitForEncodedFrame(timestamp_ms);
  EXPECT_EQ(1, fake_encoder_.GetNumEncoderInitializations());
  ...
  config.simulcast_layers[numberLayers - 1].active = false;
  video_stream_encoder_->ConfigureEncoder(config.Copy(), kMaxPayloadLength);
  sink_.SetNumExpectedLayers(numberLayers - 1);
  timestamp_ms += kFrameIntervalMs;
  video_source_.IncomingCapturedFrame(CreateFrame(timestamp_ms, 1280, 720));
  WaitForEncodedFrame(timestamp_ms);
  EXPECT_EQ(1, fake_encoder_.GetNumEncoderInitializations());
  ...
  config.simulcast_layers[numberLayers - 1].active = true;
  video_stream_encoder_->ConfigureEncoder(config.Copy(), kMaxPayloadLength);
  sink_.SetNumExpectedLayers(numberLayers);
  timestamp_ms += kFrameIntervalMs;
  video_source_.IncomingCapturedFrame(CreateFrame(timestamp_ms, 1280, 720));
  ...
  // Encoder shouldn't be re-initialized. Next frame should be delta frame.
  config.simulcast_layers[numberLayers - 1].max_bitrate_bps -= 100;
  video_stream_encoder_->ConfigureEncoder(config.Copy(), kMaxPayloadLength);
  sink_.SetNumExpectedLayers(numberLayers);
  timestamp_ms += kFrameIntervalMs;
  video_source_.IncomingCapturedFrame(CreateFrame(timestamp_ms, 1280, 720));
  ...
  // Encoder should be re-initialized. Next frame should be key frame.
  config.simulcast_layers[numberLayers - 1].scale_resolution_down_by += 0.1;
  video_stream_encoder_->ConfigureEncoder(config.Copy(), kMaxPayloadLength);
  sink_.SetNumExpectedLayers(numberLayers);
  timestamp_ms += kFrameIntervalMs;
  video_source_.IncomingCapturedFrame(CreateFrame(timestamp_ms, 1280, 720));
