...
TEST(RtpPacketizerSplitAboutEqualy, AllPacketsAreEqualSumToPayloadLen) {
  RtpPacketizer::PayloadSizeLimits limits;
  limits.max_payload_len = 5;
  limits.last_packet_reduction_len = 2;

  std::vector<size_t> payload_sizes =
      RtpPacketizer::SplitAboutEqualy(13, limits);

  EXPECT_THAT(Sum(payload_sizes), 13);
}

TEST(RtpPacketizerSplitAboutEqualy, AllPacketsAreEqualRespectsMaxPayloadSize) {
  RtpPacketizer::PayloadSizeLimits limits;
  limits.max_payload_len = 5;
  limits.last_packet_reduction_len = 2;
...
TEST(RtpPacketizerSplitAboutEqualy,
...
TEST(RtpPacketizerSplitAboutEqualy, AllPacketsAreEqualInSize) {
  RtpPacketizer::PayloadSizeLimits limits;
  limits.max_payload_len = 5;
  limits.last_packet_reduction_len = 2;
...
TEST(RtpPacketizerSplitAboutEqualy,
  ...
  std::vector<size_t> payload_sizes =
      RtpPacketizer::SplitAboutEqualy(13, limits);
  // Computed by hand. 3 packets would have exactly capacity 3*5-3=13
  // (max length - for each packet minus last packet reduction).
  EXPECT_THAT(payload_sizes, SizeIs(3));
}

TEST(RtpPacketizerSplitAboutEqualy, SomePacketsAreSmallerSumToPayloadLen) {
  RtpPacketizer::PayloadSizeLimits limits;
  limits.max_payload_len = 7;
  limits.last_packet_reduction_len = 5;

  std::vector<size_t> payload_sizes =
      RtpPacketizer::SplitAboutEqualy(28, limits);
...
TEST(RtpPacketizerVideoGeneric,
  ...
  std::vector<size_t> payload_sizes =
      RtpPacketizer::SplitAboutEqualy(24, limits);
