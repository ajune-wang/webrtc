...
bool RTCPReceiver::ParseCompoundPacket(rtc::ArrayView<const uint8_t> packet,
  ...
  bool contains_sr = false;
  bool contains_xr = false;
  ...
  if (contains_sr && !contains_xr) {
    auto rtt_stats = non_sender_rtts_.find(packet_information->remote_ssrc);
...
void RTCPReceiver::HandleXr(const CommonHeader& rtcp_block,
  ...
  if (xr.dlrr().sub_blocks().empty()) {
    auto rtt_stats = non_sender_rtts_.find(xr.sender_ssrc());
    if (rtt_stats != non_sender_rtts_.end()) {
      rtt_stats->second.Invalidate();
    }
  }
...
void RTCPReceiver::HandleXrDlrrReportBlock(uint32_t sender_ssrc,
  ...
  double non_sender_rtt_seconds = xr_rr_rtt_ms_ * 0.001;
  non_sender_rtts_[sender_ssrc].Update(non_sender_rtt_seconds);
}
