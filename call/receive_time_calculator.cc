...
ReceiveTimeCalculator::ReceiveTimeCalculator(int64_t min_delta_diff_ms,
                                             int64_t max_delta_diff_ms)
    : min_delta_diff_us_(min_delta_diff_ms * 1000),
...
int64_t ReceiveTimeCalculator::ReconcileReceiveTimes(int64_t packet_time_us_,
  ...
  if (!receive_time_offset_set_) {
    receive_time_offset_set_ = true;
    ...
    int64_t delta_diff = safe_delta_us - packet_delta_us_;
    if (delta_diff < min_delta_diff_us_ || delta_diff > max_delta_diff_us_) {
