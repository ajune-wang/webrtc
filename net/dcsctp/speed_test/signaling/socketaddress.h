...
class SocketAddress {
  ...
  struct is_one_of<T, U, Rs...> {
    static constexpr bool value =
        std::is_same<T, U>::value || is_one_of<T, Rs...>::value;
  };
 ...
 public:
  ...
  /**
   * Constructs a SocketAddress object and initializes it with the provided
   * value.
   ...
   * @param pIp the ip in binary network order representation. The length of
   * this is deducted from the family parameter
   * @param isBinaryIp if true, than pIp will be treated as binary
   * representation of the address (4 or 16 bytes) in network order. If false,
   * than pIp is treated as the literal/text representation of the address.
  ...
  inline SocketAddress(const std::string& ip, uint16_t port)
      : SocketAddress(ip.find(':') == std::string::npos ? AF_INET : AF_INET6,
                      ip.c_str(),
                      false,
                      port) {}
  ...
  inline bool operator==(const char* pAddress) const {
    return pAddress == nullptr ? (!IsValid())
                               : (*this == std::string(pAddress));
  }

  inline bool operator==(const std::string& address) const {
    return address.empty() ? (!IsValid())
                           : (*this == SocketAddress(GetFamily(), address));
  }

  inline bool operator==(const SocketAddress& address) const {
    if ((this == &address) || ((!IsValid()) && (!address.IsValid())))
      return true;
    return IsValid() && address.IsValid() &&
           (GetLength() == address.GetLength()) && (_crc32 == address._crc32) &&
  ...
  operator==(const AddressType* pAddress) const {
    ...
    return (_address.ss_family == pAddressStorage->ss_family)
           ...
           && (_addressLength == (_address.ss_family == AF_INET
   ...
   */
  inline operator const char*() const { return _stringRepresentation.c_str(); }

  inline operator const std::string&() const { return _stringRepresentation; }

  inline operator const struct sockaddr *() const {
    return ((_address.ss_family == AF_INET) || (_address.ss_family == AF_INET6))
               ? (struct sockaddr*)&_address
               : NULL;
  }

  inline operator const struct sockaddr_in *() const {
    return _address.ss_family == AF_INET ? ((struct sockaddr_in*)&_address)
                                         : NULL;
  }

  inline operator const struct sockaddr_in6 *() const {
    return _address.ss_family == AF_INET6 ? ((struct sockaddr_in6*)&_address)
                                          : NULL;
  }
...
#ifdef SOCKADDR_HAS_LENGTH_FIELD
    return (socklen_t)(
        ((_address.ss_family == AF_INET) || (_address.ss_family == AF_INET6))
            ? _address.ss_len
            : 0);
#else  /* SOCKADDR_HAS_LENGTH_FIELD */
    return (socklen_t)(
  ...
  /**
   * Gets the CRC32 value which is computed on top of the backend struct
   * sockaddr storage
   * @return the CRC32 value which is computed on top of the backend struct
   * sockaddr storage
   */
  inline uint32_t GetCRC32() const { return _crc32; }

  /**
   * Checks if the address is initialized
   * @return true if initialized, false otherwise
   */
  inline bool IsValid() const { return IsIPv4() || IsIPv6(); }

  /**
   * Checks if the address is ipv4
   * @return true if the address is ipv4, false otherwise
   */
  inline bool IsIPv4() const { return _address.ss_family == AF_INET; }
   ...
   */
  inline bool IsIPv6() const { return _address.ss_family == AF_INET6; }
  ...
  inline bool IsAny() const {
    ...
    switch (_address.ss_family) {
      case AF_INET:
        return memcmp(
                   (const void*)(&(((struct sockaddr_in*)&_address)->sin_addr)),
                   zeroBlock, 4) == 0;
      case AF_INET6:
        return memcmp((const void*)(&(
                          ((struct sockaddr_in6*)&_address)->sin6_addr)),
                      zeroBlock, 16) == 0;
      default:
        return false;
    }
   ...
   * @return true if the the address is a ipv4 mapped onto a ipv6, false
   * otherwise
   * @referenceDoc: <root>/docs/ipv6_reference_card.pdf
   */
   ...
   * @return true if the address is a ipv6 documentation/benchmarking, false
   * otherwise
   * @referenceDoc: <root>/docs/ipv6_reference_card.pdf
   */
   ...
   * @return true if the address is a ipv6 orchid  (fixed term experiment),
   * false otherwise
   * @referenceDoc: <root>/docs/ipv6_reference_card.pdf
   */
   ...
   * the form of <ip>:<port>. If the port is 0 inside the provideod sockaddr
   * object, than the :<port> part from the end result will be omitted.
   ...
   * @param pPort output variable which will be set to the numerical
   * representation of the port. It will be ignored if NULL.
   * @return the canonical string representation of the ip/port pair
   */
  static std::string SockaddrToString(const struct sockaddr* pAddress,
                                      std::string* pIp,
                                      uint16_t* pPort);
