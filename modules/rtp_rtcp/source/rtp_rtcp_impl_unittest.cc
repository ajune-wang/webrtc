...
TEST_F(RtpRtcpImplTest, SenderReportStatsCheckArrivalRemoteTimestamps) {
  // Send a frame in order to send an SR.
  SendFrame(&sender_, sender_video_.get(), kBaseLayerTid);
  NtpTime expected_remote_ntp = clock_.CurrentNtpTime();
  // Send an SR.
  ASSERT_EQ(0, sender_.impl_->SendRTCP(kRtcpReport));
  NtpTime expected_arrival_ntp = clock_.CurrentNtpTime();
