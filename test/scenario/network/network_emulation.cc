...
void NetworkNode::CreateRoute(uint64_t receiver_id,
...
PassThroughNetworkNode::PassThroughNetworkNode() = default;
PassThroughNetworkNode::~PassThroughNetworkNode() = default;

void PassThroughNetworkNode::OnPacketReceived(EmulatedIpPacket packet) {
  rtc::CritScope crit(&lock_);
  if (routing_.find(packet.dest_endpoint_id) == routing_.end()) {
    RTC_LOG(INFO) << "PACKET DROPPED: No route for endpoint "
                  << packet.dest_endpoint_id;
  }
  packets_.emplace_back(std::move(packet));
}

void PassThroughNetworkNode::Process(Timestamp cur_time) {
  std::vector<EmulatedIpPacket> to_send;
  {
    rtc::CritScope crit(&lock_);
  ...
  for (auto& packet : to_send) {
    ...
    // We don't want to keep the lock here. Otherwise we would get a deadlock if
    // the receiver tries to push a new packet.
    receiver->OnPacketReceived(std::move(packet));
  }
}
