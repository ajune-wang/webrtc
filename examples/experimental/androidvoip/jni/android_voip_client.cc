...
int GetPayloadType(std::string codec_name) {
  ...
  if (codec_name == "PCMU") {
    return static_cast<int>(PayloadType::PCMU);
  } else if (codec_name == "PCMA") {
    return static_cast<int>(PayloadType::PCMA);
  } else if (codec_name == "G722") {
    return static_cast<int>(PayloadType::G722);
  } else if (codec_name == "opus") {
    return static_cast<int>(PayloadType::OPUS);
  } else if (codec_name == "ISAC") {
    return static_cast<int>(PayloadType::ISAC);
  } else if (codec_name == "ILBC") {
    return static_cast<int>(PayloadType::ILBC);
  }
