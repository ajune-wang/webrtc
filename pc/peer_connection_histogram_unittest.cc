...
static const rtc::SocketAddress kPrivateLocalAddressIPv6("fd12:3456:789a:1::1",
                                                         0);
...
class PeerConnectionUsageHistogramTest : public ::testing::Test {
 protected:
  ...
  WrapperPtr CreatePeerConnectionWithIpv6PrivateLocalAddresses() {
    auto* fake_network = NewFakeNetwork();
    fake_network->AddInterface(NextLocalAddress());
    fake_network->AddInterface(kPrivateLocalAddressIPv6);
...
TEST_F(PeerConnectionUsageHistogramTest, FingerprintWithMdnsCaller) {
  ...
  // the bit for the direct connection should be logged.
...
#ifndef WEBRTC_ANDROID
#ifdef HAVE_SCTP
// Test that the usage pattern bits for adding remote private (IPv6) or mDNS
// candidates are set when the remote candidates are retrieved from Offer/Answer
// SDP instead of trickled ICE messages.
TEST_F(PeerConnectionUsageHistogramTest,
  ...
  // non-trickled sessions. The callee collects mDNS candidates. Only Offer is
  // signaled to the callee and we expect a connection with prflx candidates.
  auto caller = CreatePeerConnectionWithIpv6PrivateLocalAddresses();
  ...
  int expected_fingerprint_caller = MakeUsageFingerprint(
      {PeerConnection::UsageEvent::DATA_ADDED,
       ...
       PeerConnection::UsageEvent::SET_REMOTE_DESCRIPTION_CALLED,
       PeerConnection::UsageEvent::CANDIDATE_COLLECTED,
       PeerConnection::UsageEvent::IPV6_HOST_CANDIDATE_COLLECTED,
       PeerConnection::UsageEvent::PRIVATE_CANDIDATE_COLLECTED,
       PeerConnection::UsageEvent::ICE_STATE_CONNECTED,
       PeerConnection::UsageEvent::CLOSE_CALLED});
