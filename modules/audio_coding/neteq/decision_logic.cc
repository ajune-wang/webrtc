...
void DecisionLogic::FilterBufferLevel(size_t buffer_size_samples) {
  ...
  int time_stretched_samples = time_stretched_samples_;
  if (prev_time_scale_) {
...
Operations DecisionLogic::FuturePacketAvailable(
  ...
  if (prev_mode == kModeRfc3389Cng || prev_mode == kModeCodecInternalCng) {
    ...
    if ((static_cast<uint32_t>(generated_noise_samples + target_timestamp) >=
         ...
         above_target_window) &&
        !below_target_window) {
      time_stretched_samples_ = timestamp_leap - generated_noise_samples;
