...
TEST(FixedFpsVideoFrameWriterAdaptorTest,
  ...
  auto inmemory_writer =
      std::make_unique<InMemoryVideoWriter>(time_controller->GetClock());
  InMemoryVideoWriter* inmemory_writer_ref = inmemory_writer.get();

  FixedFpsVideoFrameWriterAdaptor video_writer(fps, time_controller->GetClock(),
  ...
  std::vector<TimedFrame> received_frames =
...
TEST(FixedFpsVideoFrameWriterAdaptorTest, InterFrameIntervalsAreEqual) {
  ...
  // Last interval is shorter, because `video_writer` was destroyed
  // after 0.2 * kInterval and it lead to the flush of frames.
  EXPECT_THAT(
