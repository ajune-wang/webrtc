...
TEST(RtcpPacketNackTest, CreateFragmented) {
  ...
  ::testing::MockFunction<void(size_t)> callback;
  EXPECT_CALL(callback, Call(_))
      .WillOnce(Invoke([&](size_t length) {
        Nack nack;
        EXPECT_TRUE(test::ParseSinglePacket(buffer, length, &nack));
  ...
  EXPECT_TRUE(
      nack.BuildExternalBuffer(buffer, kBufferSize, callback.AsStdFunction()));
}
...
TEST(RtcpPacketNackTest, CreateFailsWithTooSmallBuffer) {
  ...
  MockFunction<void(size_t)> callback;
  EXPECT_CALL(callback, Call(_)).Times(0);
  EXPECT_FALSE(nack.BuildExternalBuffer(buffer, kMinNackBlockSize - 1,
