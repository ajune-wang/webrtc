...
ResidualEchoEstimator::ResidualEchoEstimator(const EchoCanceller3Config& config)
  ...
  if (config_.ep_strength.reverb_based_on_render) {
    echo_reverb_ = std::unique_ptr<ReverbModel>(new ReverbModel());
  } else {
    echo_reverb_fallback = std::unique_ptr<ReverbModelFallback>(
  ...
  if (aec_state.UsableLinearEstimate()) {
    ...
    if (echo_reverb_) {
      echo_reverb_->AddReverb(
          ...
          aec_state.GetEstimatedTailGain(), aec_state.ReverbDecay(), *R2);
    } else {
    ...
    if (echo_reverb_) {
      echo_reverb_->AddReverb(render_buffer.Spectrum(1),
                              echo_path_gain * echo_path_gain,
                              aec_state.ReverbDecay(), *R2);
    } else {
      RTC_DCHECK(echo_reverb_fallback);
      echo_reverb_fallback->AddEchoReverb(
          *R2, config_.filter.main.length_blocks, aec_state.ReverbDecay(), R2);
    }
  }
