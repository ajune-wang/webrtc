...
RTCError JsepTransportController::ApplyDescription_n(
  ...
  for (size_t i = 0; i < description->contents().size(); ++i) {
    ...
    if (content_bundle_group &&
        content_info.name == *content_bundle_group->FirstContentName()) {
      auto it =
          merged_encrypted_extension_ids_by_bundle.find(content_bundle_group);
...
JsepTransportController::MergeEncryptedHeaderExtensionIdsForBundles(
  ...
  for (const cricket::ContentGroup& bundle_group : bundle_groups_) {
    ...
    merged_encrypted_extension_ids_by_bundle.insert(
        std::make_pair(&bundle_group, merged_ids));
  }
