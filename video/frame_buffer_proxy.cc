...
class FrameBuffer3Proxy : public FrameBufferProxy {
 public:
  ...
  void StopOnWorker() override {
    ...
    encoder_waiting_for_frame_ = false;
    decode_queue_->PostTask([this] {
      RTC_DCHECK_RUN_ON(decode_queue_);
      decode_safety_->SetNotAlive();
    });
  ...
  void StartNextDecode(bool keyframe_required) override {
    ...
    encoder_waiting_for_frame_ = true;
    MaybeScheduleFrameForRelease();
  }
  ...
  void OnFrameReady(
    ...
    encoder_waiting_for_frame_ = false;
    last_released_frame_rtp_ = frame->Timestamp();
 ...
 private:
  void OnFrameReadyForExtraction(uint32_t rtp_timestamp,
                                 Timestamp render_time) {
    RTC_DCHECK_RUN_ON(&worker_sequence_checker_);
    RTC_DCHECK(buffer_.NextDecodableTemporalUnitRtpTimestamp() == rtp_timestamp)
  ...
  void MaybeScheduleFrameForRelease() RTC_RUN_ON(&worker_sequence_checker_) {
    ...
    if (!buffer_.LastDecodableTemporalUnitRtpTimestamp()) {
      RTC_DCHECK_NOTREACHED();
      return;
    }
  ...
  FrameDecodeTiming decode_timing_;
  ...
  bool encoder_waiting_for_frame_ RTC_GUARDED_BY(&worker_sequence_checker_) =
      false;
