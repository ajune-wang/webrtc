...
// Meant to be injected into an APM to modify samples in a known way.
...
TEST(AudioProcessingImplTest, UpdateCapturePreGainRuntimeSetting) {
  ...
  EXPECT_EQ(frame.data()[100], gain_factor * kAudioLevel)
...
TEST(AudioProcessingImplTest, RenderPreProcessorBeforeEchoDetector) {
  ...
  InitializeAudioFrame(kSampleRateHz, kNumChannels, &capture_frame);
  AudioFrame render_frame;
  InitializeAudioFrame(kSampleRateHz, kNumChannels, &render_frame);
  ...
  ASSERT_NE(kAudioLevelFloat, expected_preprocessed_audio_level);

  // Analyze a reverse stream frame.
  FillFixedFrame(kAudioLevel, &render_frame);
  ASSERT_EQ(AudioProcessing::Error::kNoError,
            apm->ProcessReverseStream(&render_frame));
  // Trigger a call to in EchoDetector::AnalyzeRenderAudio() via
  // ProcessStream(). An APM implementation agnostic alternative would be to
  ...
  // requires a loop.
  FillFixedFrame(kAudioLevel, &capture_frame);
  ASSERT_EQ(AudioProcessing::Error::kNoError,
            apm->ProcessStream(&capture_frame));
  // Regardless of how the call to in EchoDetector::AnalyzeRenderAudio() is
  // trigger, the line below checks that the call has occurred. If not, the APM
  // implementation may have changed and this test might need to be adapted.
