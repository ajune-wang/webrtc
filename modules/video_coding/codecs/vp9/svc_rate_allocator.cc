...
BitrateAllocation SvcRateAllocator::GetAllocation(uint32_t total_bitrate_bps,
  ...
  // Distribite total bitrate across spatial layers. If there is no enough
  // bitrate to provide all layers with at least minumum required bitrate
  ...
  do {
    ...
    for (size_t sl_idx = 0; sl_idx < num_spatial_layers; ++sl_idx) {
      ...
      // If codec is used directly spatial layer parameters might not be set.
      // Use max bitrate threshold from codec parameters in that case.
      RTC_DCHECK(codec_.spatialLayers[sl_idx].maxBitrate != 0 ||
                 codec_.VP9().numberOfTemporalLayers == 1);
      const size_t max_bitrate_bps =
          1000 * (codec_.spatialLayers[sl_idx].maxBitrate != 0
                      ? codec_.spatialLayers[sl_idx].maxBitrate
                      : codec_.maxBitrate);
      ...
      if (spatial_layer_bitrate_bps[sl_idx] < min_bitrate_bps) {
        layer_bitrate_is_less_than_required_minimum = true;
      }
    }
  } while (layer_bitrate_is_less_than_required_minimum &&
           num_spatial_layers > 1 && num_spatial_layers--);
