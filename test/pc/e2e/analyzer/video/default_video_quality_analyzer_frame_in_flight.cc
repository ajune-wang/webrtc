...
void FrameInFlight::OnFrameEncoded(webrtc::Timestamp time,
  ...
  qp_per_spatial_layer_[spatial_layer].AddSample(
      SamplesStatsCounter::StatsSample{.value = static_cast<double>(qp),
                                       .time = time});
...
FrameStats FrameInFlight::GetStatsForPeer(size_t peer) const {
  ...
  stats.qp_per_spatial_layer = qp_per_spatial_layer_;
