...
int LibvpxVp8Encoder::InitEncode(const VideoCodec* inst,
  ...
  configurations_[0].rc_dropframe_thresh = FrameDropThresh(0);
  ...
  UpdateVpxConfiguration(temporal_layers_[stream_idx].get(),
                         &configurations_[0]);
  configurations_[0].rc_dropframe_thresh = FrameDropThresh(stream_idx);
  ...
  for (size_t i = 1; i < encoders_.size(); ++i, --stream_idx) {
    ...
    configurations_[i].rc_dropframe_thresh = FrameDropThresh(stream_idx);
...
uint32_t LibvpxVp8Encoder::FrameDropThresh(size_t spatial_idx) const {
