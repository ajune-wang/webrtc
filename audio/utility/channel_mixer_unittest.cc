...
void SetTwoOneData(int16_t front_left,
                   int16_t front_right,
                   int16_t back_center,
                   AudioFrame* frame) {
  ASSERT_LE(3 * frame->samples_per_channel(), frame->max_16bit_samples());
  frame->num_channels_ = 3;
  int16_t* frame_data = frame->mutable_data();
  for (size_t i = 0; i < frame->samples_per_channel() * 3; i += 3) {
    frame_data[i] = front_left;
    frame_data[i + 1] = front_right;
    frame_data[i + 2] = back_center;
  }
  EXPECT_FALSE(frame->muted());
}

void SetQuadData(int16_t front_left,
                 int16_t front_right,
                 int16_t back_left,
                 int16_t back_right,
                 AudioFrame* frame) {
  ASSERT_LE(4 * frame->samples_per_channel(), frame->max_16bit_samples());
  frame->num_channels_ = 4;
  int16_t* frame_data = frame->mutable_data();
  for (size_t i = 0; i < frame->samples_per_channel() * 4; i += 4) {
    frame_data[i] = front_left;
    frame_data[i + 1] = front_right;
    frame_data[i + 2] = back_left;
    frame_data[i + 3] = back_right;
  }
  EXPECT_FALSE(frame->muted());
}
...
void VerifyFramesAreEqual(const AudioFrame& frame1, const AudioFrame& frame2) {
  EXPECT_EQ(frame1.channels(), frame2.channels());
  ...
  for (size_t i = 0; i < frame1.samples_per_channel() * frame1.channels();
       i++) {
...
// TODO(henrika): improve this test...
  ...
  for (ChannelLayout input_layout = CHANNEL_LAYOUT_MONO;
    ...
    for (ChannelLayout output_layout = CHANNEL_LAYOUT_MONO;
      ...
      EXPECT_EQ(ChannelLayoutToChannelCount(input_layout),
                static_cast<int>(frame_.channels()));
...
TEST_F(ChannelMixerTest, StereoToMono) {
  ...
  EXPECT_EQ(2u, frame_.channels());
  mixer.Transform(&frame_);
  EXPECT_EQ(1u, frame_.channels());
...
TEST_F(ChannelMixerTest, MonoToStereo) {
  ...
  // Perform a second test of upmixing mono to stereo but this time ensure that
  // not all samples are equal. We use SetStereoData() and SetQuadData() to
  // create the input and reference output frames and tweak the channel count
  // after each setting. As an example, the two samples 50, 60 used as input
  // will be seen as two mono samples and not one stereo sample. In addition,
  // SetQuadData() is used to create two identical stereo samples and not one
  // quad sample. We use in-place upmixing and must therefore work through the
  // audio frame starting from the end (largest index). Hence, in this case,
  // the first generated output will be [55, 55] and the second output is
  // [87, 87].

  // Use two different mono samples as input.
  SetStereoData(55, 87, &frame_);
  frame_.num_channels_ = 1;
  EXPECT_EQ(1u, frame_.channels());
  mixer.Transform(&frame_);
  EXPECT_EQ(2u, frame_.channels());
  // We expect two different stereo samples as output where left and right
  // are identical within each sample.
  SetQuadData(55, 55, 87, 87, &stereo_frame);
...
TEST_F(ChannelMixerTest, MonoToTwoOne) {
  ChannelMixer mixer(CHANNEL_LAYOUT_MONO, CHANNEL_LAYOUT_2_1);
  //
  //                         Input: mono
  //                         CENTER
  ...
  //             BACK CENTER 0
  //
  SetMonoData(67, &frame_);
  EXPECT_EQ(1u, frame_.channels());
  mixer.Transform(&frame_);
  EXPECT_EQ(3u, frame_.channels());

  AudioFrame two_one_frame;
  two_one_frame.samples_per_channel_ = frame_.samples_per_channel();
  SetTwoOneData(67, 67, 0, &two_one_frame);
  VerifyFramesAreEqual(two_one_frame, frame_);
}

TEST_F(ChannelMixerTest, TwoOneToTwoTwo) {
  ChannelMixer mixer(CHANNEL_LAYOUT_2_1, CHANNEL_LAYOUT_2_2);
  //
  //                         Input: 2.1
  //                         FRONT_LEFT FRONT_RIGHT BACK_CENTER
  // Output: 2.2 FRONT_LEFT  1          0           0
  //             FRONT_RIGHT 0          1           0
  //             SIDE_LEFT   0          0           1/sqrt(2)
  //             SIDE_RIGHT  0          0           1/sqrt(2)
  //
  SetTwoOneData(67, 73, 10, &frame_);
  EXPECT_EQ(3u, frame_.channels());
  mixer.Transform(&frame_);
  EXPECT_EQ(4u, frame_.channels());

  AudioFrame two_two_frame;
  two_two_frame.samples_per_channel_ = frame_.samples_per_channel();
  SetQuadData(67, 73, 7, 7, &two_two_frame);
  VerifyFramesAreEqual(two_two_frame, frame_);
}
