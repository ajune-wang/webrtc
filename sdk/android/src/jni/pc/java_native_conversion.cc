...
void JavaToNativeRTCConfiguration(
  ...
  jfieldID j_turn_customizer_type_id =
      GetFieldID(jni, j_rtc_config_class, "turnCustomizer",
                 "Lorg/webrtc/TurnCustomizer;");
  jobject j_turn_customizer_obj =
  ...
  if (!IsNull(jni, j_turn_customizer_obj)) {
    rtc_config->turn_customizer = reinterpret_cast<webrtc::TurnCustomizer*>(
        GetLongField(jni, j_turn_customizer_obj, j_native_turn_customizer_id));
  }
}
