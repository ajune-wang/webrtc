...
def CheckStaticAnalyzer(input_api, output_api):
  ...
  with _AddToPath(input_api.os_path.join(
    ...
    from gn_check import RunGnCommand, RunNinjaCommand
  ...
  try:
    ...
    for f in input_api.AffectedSourceFiles(FileFilter):
      # Build single file using ninja syntax: path/file.cc^
      # where we have to provide the path as generated by gn.
      rel_path = os.path.relpath(f.LocalPath(), out_dir)
      log = RunNinjaCommand(['%s^' % rel_path], out_dir)
  ...
  return [output_api.PresubmitError(msg, errors)] if errors else []
