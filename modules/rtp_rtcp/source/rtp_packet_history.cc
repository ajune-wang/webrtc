...
void RtpPacketHistory::PutRtpPacket(std::unique_ptr<RtpPacketToSend> packet,
  ...
  RTC_DCHECK(rtp_seqno_map_.find(stored_packet->packet->SequenceNumber()) ==
             rtp_seqno_map_.end());
  rtp_seqno_map_.emplace(stored_packet->packet->SequenceNumber(),
                         stored_packet);
}
...
rtc::Optional<RtpPacketHistory::PacketState> RtpPacketHistory::GetPacketState(
  ...
  return (*packet_it)->AsPacketState();
}
...
std::unique_ptr<RtpPacketToSend> RtpPacketHistory::RemovePacket(
  ...
  size_t erased = rtp_seqno_map_.erase(rtp_packet->SequenceNumber());
  RTC_DCHECK_EQ(erased, 1u);
