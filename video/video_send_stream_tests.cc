...
void VideoSendStreamTest::TestTemporalLayers(
  ...
  static constexpr int64_t kMinRunTimeMs = 1000;
  ...
  class TemporalLayerObserver
   ...
   private:
    ...
    Action OnSendRtp(const uint8_t* packet, size_t length) override {
      ...
      if (max_observed_tl_idxs_.count(ssrc) == 0) {
        max_observed_tl_idxs_[ssrc] = temporal_idx;
      }
      max_observed_tl_idxs_[ssrc] =
          std::max(temporal_idx, max_observed_tl_idxs_[ssrc]);

      if (MaxTemporalLayerSentPerStream())
        observation_complete_.Set();

      return SEND_PACKET;
    }

    bool MaxTemporalLayerSentPerStream() const {
      int64_t now_ms = Clock::GetRealTimeClock()->TimeInMilliseconds();
      if (now_ms - start_ms_ < kMinRunTimeMs) {
        return false;
      }
