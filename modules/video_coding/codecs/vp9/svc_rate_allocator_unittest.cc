...
TEST(SvcRateAllocatorTest, FindLayerTogglingThreshold) {
  ...
  VideoCodec codec = Configure(1280, 720, 3, 1, false);
  std::map<DataRate, size_t> layer_start_bitrates =
      SvcRateAllocator::GetLayerStartBitrates(codec);
  EXPECT_TRUE(layer_start_bitrates.find(kTwoLayerMinRate) !=
              layer_start_bitrates.end());
  EXPECT_EQ(layer_start_bitrates[kTwoLayerMinRate], 2u);
  EXPECT_TRUE(layer_start_bitrates.find(kThreeLayerMinRate) !=
              layer_start_bitrates.end());
  EXPECT_EQ(layer_start_bitrates[kTwoLayerMinRate], 2u);
}
...
TEST_P(SvcRateAllocatorTestParametrizedContentType, StableBitrate) {
  ...
  DataRate min_rate_two_layers = DataRate::Zero();
  DataRate min_rate_three_layers = DataRate::Zero();
  for (auto it : SvcRateAllocator::GetLayerStartBitrates(codec)) {
    if (it.second == 2) {
      min_rate_two_layers = it.first;
    } else if (it.second == 3) {
      min_rate_three_layers = it.first;
    }
  }
...
TEST_P(SvcRateAllocatorTestParametrizedContentType,
  ...
  const VideoCodec codec = Configure(1280, 720, 3, 1, is_screen_sharing_);
  DataRate min_rate_single_layer =
      DataRate::kbps(codec.spatialLayers[0].minBitrate);
  DataRate min_rate_two_layers = DataRate::Zero();
  DataRate min_rate_three_layers = DataRate::Zero();
  for (auto it : SvcRateAllocator::GetLayerStartBitrates(codec)) {
    if (it.second == 2) {
