...
TEST(NetworkEmulationManagerTest, EcnMarkingIsPropagated) {
  ...
  nt1->GetStats([&](EmulatedNetworkStats st) {
    ...
    EXPECT_EQ(st.overall_outgoing_stats.ecn_counter.ect_1(), 1);
    EXPECT_EQ(st.overall_outgoing_stats.ecn_counter.ce(), 0);
    EXPECT_EQ(st.overall_outgoing_stats.ecn_counter.not_ect(), 0);
    ++received_stats_count;
  });
  nt2->GetStats([&](EmulatedNetworkStats st) {
    EXPECT_EQ(st.overall_incoming_stats.packets_received, 1);
    EXPECT_EQ(st.overall_outgoing_stats.packets_sent, 0);
    EXPECT_EQ(st.overall_incoming_stats.ecn_counter.ect_1(), 1);
    EXPECT_EQ(st.overall_incoming_stats.ecn_counter.ce(), 0);
    EXPECT_EQ(st.overall_incoming_stats.ecn_counter.not_ect(), 0);
    ++received_stats_count;
  });
  ASSERT_EQ_SIMULATED_WAIT(received_stats_count.load(), 2,
                           kStatsWaitTimeout.ms(),
