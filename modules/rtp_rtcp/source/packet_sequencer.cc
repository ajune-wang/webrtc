...
void PacketSequencer::PopulatePaddingFields(RtpPacketToSend& packet) {
  ...
  if (last_timestamp_time_.IsFinite()) {
    TimeDelta since_last_timestamp = now - last_timestamp_time_;
    packet.SetTimestamp(packet.Timestamp() +
                        since_last_timestamp.ms() * kTimestampTicksPerMs);
    if (packet.capture_time() > Timestamp::Zero()) {
      packet.set_capture_time(packet.capture_time() + since_last_timestamp);
    }
  }
}
