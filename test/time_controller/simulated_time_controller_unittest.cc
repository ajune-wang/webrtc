...
TEST(SimulatedTimeControllerTest, TaskIsStoppedOnStop) {
  const TimeDelta kShortInterval = TimeDelta::Milliseconds(5);
  const TimeDelta kLongInterval = TimeDelta::Milliseconds(20);
...
TEST(SimulatedTimeControllerTest, TaskCanStopItself) {
  ...
  time_simulation.AdvanceTime(TimeDelta::Milliseconds(10));
...
TEST(SimulatedTimeControllerTest, Example) {
  class ObjectOnTaskQueue {
   public:
    void DoPeriodicTask() {}
    TimeDelta TimeUntilNextRun() { return TimeDelta::Milliseconds(100); }
