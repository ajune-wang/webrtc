...
class QualityScalingTest : public test::CallTest {
 protected:
  ...
  const absl::optional<VideoEncoder::ResolutionBitrateLimits> kLimits720p =
      EncoderInfoSettings::GetDefaultBitrateLimitsForResolution(1280 * 720);
};
...
TEST_F(QualityScalingTest,
  ...
  RunTest("VP8", {false, false, true}, kLimits720p->min_start_bitrate_bps - 1,
          /*automatic_resize=*/true, /*expect_adaptation=*/true);
}
...
TEST_F(QualityScalingTest, NoAdaptDownForLowStartBitrateIfBitrateEnough_Vp8) {
  ...
  RunTest("VP8", {false, false, true}, kLimits720p->min_start_bitrate_bps,
          /*automatic_resize=*/true, /*expect_adaptation=*/false);
}
...
TEST_F(QualityScalingTest,
       NoAdaptDownForLowStartBitrate_OneStreamDefaultLimitsNotUsed_Vp8) {
  // qp_low:1, qp_high:127 -> kNormalQp
  test::ScopedFieldTrials field_trials(kPrefix + "1,127,0,0,0,0" + kEnd);

  RunTest("VP8", {true}, kLimits720p->min_start_bitrate_bps - 1,
          /*automatic_resize=*/true, /*expect_adaptation=*/false);
}
