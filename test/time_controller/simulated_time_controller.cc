...
void SimulatedSequenceRunner::SendTask(std::unique_ptr<QueuedTask> task) {
  CurrentTaskQueueSetter set_current(this);
...
void SimulatedTimeControllerImpl::Yield() {
...
void SimulatedTimeControllerImpl::SendTask(TaskQueueBase* task_queue,
                                           std::unique_ptr<QueuedTask> task) {
  rtc::ThreadScopedEventSync sync_override(this);
  auto* runner = static_cast<SimulatedSequenceRunner*>(task_queue);
  RTC_DCHECK(HasRunner(runner));
  runner->SendTask(std::move(task));
}
...
void SimulatedTimeController::Sleep(TimeDelta duration) {
  rtc::ThreadScopedEventSync sync_override(&impl_);
...
void SimulatedTimeController::SendTask(TaskQueueBase* task_queue,
                                       std::unique_ptr<QueuedTask> task) {
  impl_.SendTask(task_queue, std::move(task));
}

void SimulatedTimeController::RunInEventScope(std::function<void()> closure) {
  rtc::ThreadScopedEventSync sync_override(&impl_);
  closure();
}
...
void GlobalSimulatedTimeController::SendTask(TaskQueueBase* task_queue,
                                             std::unique_ptr<QueuedTask> task) {
  impl_.SendTask(task_queue, std::move(task));
}

void GlobalSimulatedTimeController::RunInEventScope(
