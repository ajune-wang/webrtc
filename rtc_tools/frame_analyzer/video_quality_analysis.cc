...
void PrintAnalysisResults(FILE* output, const std::string& label,
  ...
  PrintResult(label, "", "Unique_frames_count", results->frames.size(), "score",
              false);
  ...
  if (results->frames.size() > 0u) {
    ...
    PrintResultList(label, "", "PSNR", psnr_values, "dB", false);
    PrintResultList(label, "", "SSIM", ssim_values, "score", false);
  }

  PrintResult(label, "", "Max_repeated", results->max_repeated_frames, "",
              false);
  PrintResult(label, "", "Max_skipped", results->max_skipped_frames, "", false);
  PrintResult(label, "", "Total_skipped", results->total_skipped_frames, "",
              false);
  PrintResult(label, "", "Decode_errors_reference", results->decode_errors_ref,
              "", false);
  PrintResult(label, "", "Decode_errors_test", results->decode_errors_test, "",
              false);
}
