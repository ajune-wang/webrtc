...
STDMETHODIMP CaptureInputPin::Connect(IPin* receive_pin,
  ...
  // If it is partially specified, determine potential candidates and try each
  // one until we can connect.
  // If the type is fully specified, we will try to connect directly.
...
void CaptureSinkFilter::NotifyEvent(long code,
                                    LONG_PTR param1,
                                    LONG_PTR param2) {
  // Called on the capture thread currently.
  if (!sink_)
    return;
