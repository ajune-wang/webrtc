...
class RTCStatsReportVerifier {
 public:
    ...
    if (*media_stream_track.kind == RTCMediaStreamTrackKind::kVideo) {
      ...
      if (*media_stream_track.remote_source) {
        ...
        verifier.TestMemberIsNonNegative<uint64_t>(
            media_stream_track.jitter_buffer_emitted_count);
        verifier.TestMemberIsNonNegative<double>(
            media_stream_track.audio_level);
        verifier.TestMemberIsNonNegative<double>(
  ...
  bool VerifyRTCAudioSourceStats(const RTCAudioSourceStats& audio_source) {
    ...
    verifier.TestMemberIsNonNegative<double>(audio_source.total_audio_energy);
    verifier.TestMemberIsNonNegative<double>(
        audio_source.total_samples_duration);
    printf("====================================\n");
    printf("\n");
    printf("\n");
    printf("audio_level: %f\n", *audio_source.audio_level);
    printf("total_audio_energy: %f\n", *audio_source.total_audio_energy);
    printf("total_samples_duration: %f\n", *audio_source.total_samples_duration);
    printf("\n");
    printf("\n");
    printf("====================================\n");
    return verifier.ExpectAllMembersSuccessfullyTested();
