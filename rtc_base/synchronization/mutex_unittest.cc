...
class RTC_LOCKABLE ScopedLock {
 public:
  void Lock() RTC_EXCLUSIVE_LOCK_FUNCTION() { mu_.Lock(); }
  void Unlock() RTC_UNLOCK_FUNCTION() { mu_.Unlock(); }
 ...
 public:
  ...
  void OnMessage(Message* msg) override {
    ...
    {  // Add them all to the set.
      MutexLock mu(&all_values_mutex_);
...
TEST(GlobalMutexTest, CanHaveStaticStorageDuration) {
  static_assert(std::is_trivially_destructible<GlobalMutex>::value, "");
  ABSL_CONST_INIT static GlobalMutex global_lock;
  global_lock.Lock();
  global_lock.Unlock();
}

TEST(GlobalMutexTest, Basic) {
  // Create and start lots of threads.
  LockRunner<GlobalMutex> runner;
  std::vector<std::unique_ptr<Thread>> threads;
  StartThreads(&threads, &runner);
  runner.SetExpectedThreadCount(kNumThreads);
...
// sys     3m48.872s
// Unit test output:
// [       OK ] MutexTest.Performance (294375 ms)
//
...
// sys     0m31.207s
// Unit test output:
// [       OK ] MutexTest.Performance (11444 ms)
//
...
// sys     0m4.495s
// Unit test output:
// [       OK ] MutexTest.Performance (1885 ms)
//
// The test is disabled by default to avoid unecessarily loading the bots.
TEST(MutexTest, DISABLED_PerformanceHighContention) {
...
TEST(MutexTest, DISABLED_PerformanceLowContention) {
  PerfTestThread thread;
  Event event;
  static const int kThreadRepeats = 100000000;
  static const int kExpectedCount = kThreadRepeats;
  PerfTestData test_data(kExpectedCount, &event);
  thread.Start(&test_data, kThreadRepeats, 1);
  event.Wait(Event::kForever);
  thread.Stop();
}
