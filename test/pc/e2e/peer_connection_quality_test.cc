...
constexpr uint32_t kPeerConnectionUsedThreads = 7;
// Framework has extra thread for network layer and extra thread for peer
// connection stats polling.
constexpr uint32_t kFrameworkUsedThreads = 2;
constexpr uint32_t kMaxVideoAnalyzerThreads = 8;
...
void PeerConnectionE2EQualityTest::Run(RunParams run_params) {
  ...
  uint32_t video_analyzer_threads =
