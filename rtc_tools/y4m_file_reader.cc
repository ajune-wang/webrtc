...
size_t VideoFile::number_of_frames() const {
...
rtc::scoped_refptr<I420BufferInterface> VideoFile::GetFrame(
...
Y4mFile::Y4mFile(int width,
                 ...
                 const std::vector<fpos_t>& frame_positions,
                 FILE* file)
    : VideoFile(width, height, frame_positions, file), fps_(fps) {}

float Y4mFile::fps() const {
  return fps_;
}

rtc::scoped_refptr<VideoFile> OpenYuvFile(const std::string& file_name,
                                          int width,
                                          int height) {
  FILE* file = fopen(file_name.c_str(), "rb");
  if (file == nullptr) {
