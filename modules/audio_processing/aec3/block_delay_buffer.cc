...
BlockDelayBuffer::BlockDelayBuffer(size_t num_bands,
    ...
    : frame_length_(frame_length),
      delay_(delay),
      buf_(num_bands, std::vector<float>(delay_ > 0 ? delay_ : 1, 0.f)) {}
...
void BlockDelayBuffer::DelaySignal(AudioBuffer* frame) {
  ...
  // const size_t i_start = (last_insert_ + 1) % buf_[0].size();
  const size_t i_start =
      last_insert_ < buf_[0].size() - 1 ? last_insert_ + 1 : 0;
  ...
  last_insert_ = (last_insert_ + frame_length_) % buf_[0].size();
}
