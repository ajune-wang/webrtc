...
class VideoCodecAnalyzer : public VideoCodecTester::VideoCodecStats {
 public:
        ...
        [this, timestamp_rtp = encoded_frame.RtpTimestamp(),
         ...
         encode_finished_us]() {
          if (spatial_idx > 0) {
            ...
            frames_.emplace(timestamp_rtp,
                            std::map<int, Frame>{{spatial_idx, base_frame}});
          }
  ...
  std::vector<Frame> Slice(Filter filter, bool merge) const {
    ...
    for (const auto& [timestamp_rtp, temporal_unit_frames] : frames_) {
      ...
      for (const auto& [spatial_idx, frame] : temporal_unit_frames) {
        ...
        if (filter.layer_id) {
          if (frame.layer_id.spatial_idx != filter.layer_id->spatial_idx) {
            continue;
          }
      ...
      for (const Frame& frame : subframes) {
  ...
  Stream Aggregate(Filter filter) const {
    ...
    TimeDelta duration =
        (frames.back().timestamp_rtp - frames.front().timestamp_rtp) / k90kHz;
    if (frames.back().target_framerate) {
      duration += 1 / *frames.back().target_framerate;
    }
    ...
    if (const auto& target_bitrate = frames.front().target_bitrate;
        target_bitrate) {
      bitrate_mismatch_pct =
          100.0 * (encoded_bitrate - *target_bitrate) / *target_bitrate;
    }
    double framerate_mismatch_pct = 0.0;
    if (const auto& target_framerate = frames.front().target_framerate;
        target_framerate) {
      framerate_mismatch_pct =
          100.0 * (encoded_framerate - *target_framerate) / *target_framerate;
    }
 ...
 private:
  ...
  DataRate GetTargetBitrate(const EncodingSettings& encoding_settings,
    ...
    if (layer_id.has_value()) {
      ...
      int max_temporal_idx = ScalabilityModeToNumTemporalLayers(
