...
struct FrameStats {
  ...
  // Sender side qp value per spatial layer. The spatial layer must be set in
  // `webrtc::EncodedImage` in order to aggregate `spatial_layers_qp`. Each
  // spatial layer can only have 1 QP value. Reason for using
  // `SamplesStatsCounter` is because `SamplesStatsCounter::StatsSample` not
  // having a default ctor and `Timestamp` is of interest.
  // TODO(b/bug_num): Replace `SamplesStatsCounter` with optional<int>
