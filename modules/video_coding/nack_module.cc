...
const int kSendNackDelayMs = 0;

bool GetSendNackDelayMs() {
  int delay_ms = strtoul(
      webrtc::field_trial::FindFullName("WebRTC-SendNackDelayMs").c_str(),
      nullptr, 10);
  if (delay_ms > 0 && delay_ms <= kSendNackDelayMs) {
    RTC_LOG(LS_INFO) << "SendNackDelay is set to " << delay_ms;
    return delay_ms;
  }
  return kSendNackDelayMs;
}
...
NackModule::NackModule(Clock* clock,
    ...
    : clock_(clock),
      ...
      send_nack_delay_ms_(GetSendNackDelayMs()) {
  RTC_DCHECK(clock_);
...
int NackModule::OnRecoveredPacket(uint16_t seq_num, bool is_keyframe) {
  rtc::CritScope lock(&crit_);
  recovered_list_.insert(seq_num);
  // And remove old ones so we don't accumulate recovered packets.
  ...
  // And remove old ones so we don't accumulate keyframes.
  it = keyframe_list_.lower_bound(seq_num - kMaxPacketAge);
  if (it != keyframe_list_.begin())
    keyframe_list_.erase(keyframe_list_.begin(), it);
  return 0;
}
...
std::vector<uint16_t> NackModule::GetNackBatch(NackFilterOptions options) {
  ...
  while (it != nack_list_.end()) {
    // Do not send nack for FEC recovered packets
    if (recovered_list_.find(it->second.seq_num) != recovered_list_.end()) {
      ++it;
      continue;
    }
