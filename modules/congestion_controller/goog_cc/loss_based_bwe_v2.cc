...
void LossBasedBweV2::UpdateBandwidthEstimate(
  ...
  // Ingore the estimate increasement if:
  // 1. The estimate is limited due to loss, and
  // 2. The estimate has been increased by `max_increase_factor` for less than
  // `delayed_increase_window` ago, and
  // 3. The new estimate is about to increase again.
  if (limited_due_to_loss_candidate_ &&
      increase_by_max_factor_timestamp_.IsFinite() &&
      increase_by_max_factor_timestamp_ + config_->delayed_increase_window >
  ...
  if (limited_due_to_loss_candidate_ &&
    ...
    increase_by_max_factor_timestamp_ = last_send_time_most_recent_observation_;
  }
