...
class MockEchoControlFactory : public EchoControlFactory {
 public:
  MockEchoControlFactory() : num_created_mocks_(0) {
    mocks_[0] = new MockEchoControl();
    mocks_[1] = new MockEchoControl();
  }
  ~MockEchoControlFactory() {
    // Delete instances without an owner.
    if (num_created_mocks_ < 2) {
      delete mocks_[1];
    }
    if (num_created_mocks_ < 1) {
      delete mocks_[0];
    }
  }
  // Returns a pointer to the last MockEchoControl allocated by this factory.
  // Note that this pointer is in NO WAY guaranteed to be valid.
  MockEchoControl* GetMockEchoControl() { return mocks_[1]; }
  std::unique_ptr<EchoControl> Create(int sample_rate_hz) {
    RTC_DCHECK_LT(num_created_mocks_, 2)
        << "The factory cannot create too many MockEchoControl instances.";
    return absl::WrapUnique(mocks_[num_created_mocks_++]);
  }

 private:
  size_t num_created_mocks_;
  // Two mocks are needed because the factory will generate a second one when
  // the sample rate changes.
  MockEchoControl* mocks_[2];
};
