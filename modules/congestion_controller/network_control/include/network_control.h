...
class FeedbackBasedNetworkControllerInterface
 ...
 public:
  using uptr = std::unique_ptr<FeedbackBasedNetworkControllerInterface>;
  // Called with per packet feedback regarding receive time.
  virtual void OnTransportPacketsFeedback(TransportPacketsFeedback) = 0;
};

// FullNetworkControllerFactoryInterface is an interface for creating a network
// controller.
class FeedbackBasedNetworkControllerFactoryInterface {
 public:
  using uptr = std::unique_ptr<FeedbackBasedNetworkControllerFactoryInterface>;
  // Used to create a new network controller, requires an observer to be
  // provided to handle callbacks.
  virtual FeedbackBasedNetworkControllerInterface::uptr Create(
...
class SimplifiedNetworkControllerInterface
    : public virtual GenericNetworkControllerInterface {
 public:
  using uptr = std::unique_ptr<SimplifiedNetworkControllerInterface>;
...
class CombinedNetworkControllerInterface
 ...
 public:
  using uptr = std::unique_ptr<CombinedNetworkControllerInterface>;
};
...
class CombinedNetworkControllerFactoryInterface {
 public:
  using uptr = std::unique_ptr<CombinedNetworkControllerFactoryInterface>;
  // Used to create a new network controller, requires an observer to be
  // provided to handle callbacks.
  virtual CombinedNetworkControllerInterface::uptr Create(
      NetworkControllerObserver* observer) = 0;
  // Returns the interval by which the network controller expects
  // OnProcessInterval calls.
  virtual TimeDelta GetProcessInterval() const = 0;
  virtual ~CombinedNetworkControllerFactoryInterface() = default;
};
}  // namespace webrtc
