...
class AAudioPlayer {
 ...
 private:
  ...
  int32_t buffer_size_in_frames() const { return buffer_size_in_frames_; }

  // Ensures that methods are called from the same thread as this object is
  // created on.
  rtc::ThreadChecker thread_checker_;

  // Stores thread ID in first call to AAudioPlayer::OnErrorCallback from a
  ...
  int32_t buffer_size_in_frames_ = 0;
