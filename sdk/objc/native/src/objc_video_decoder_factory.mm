...
std::unique_ptr<VideoDecoder> ObjCVideoDecoderFactory::Create(const Environment &env,
  ...
  for (RTC_OBJC_TYPE(RTCVideoCodecInfo) * codecInfo in decoder_factory_.supportedCodecs) {
    if ([codecName isEqualToString:codecInfo.name]) {
      ...
      if ([decoder_factory_ respondsToSelector:@selector(createDecoder:info:)]) {
        decoder = [decoder_factory_ createDecoder:env info:codecInfo];
      } else {
        decoder = [decoder_factory_ createDecoder:codecInfo];
      }
