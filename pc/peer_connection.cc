...
void ReportSimulcastApiVersion(const char* name,
  ...
  bool has_legacy =
      absl::c_any_of(session.contents(), [](const ContentInfo& content) {
        return content.description &&
               absl::c_any_of(
                   content.description->streams(), [](const StreamParams& sp) {
                     return sp.has_ssrc_group(cricket::kSimSsrcGroupSemantics);
                   });
      });
  bool has_spec_compliant =
      absl::c_any_of(session.contents(), [](const ContentInfo& content) {
        return content.description && content.description->HasSimulcast();
      });
