...
TEST(ProbingTest, InitialProbingRampsUpTargetRate) {
  Scenario s;
  ...
  video_config.encoder.implementation =
      VideoStreamConfig::Encoder::Implementation::kSoftware;
  CallClientConfig send_call_config;
  send_call_config.transport.rates.start_rate = DataRate::KilobitsPerSec(300);
  ...
  bool success = false;
  s.Every(TimeDelta::Millis(20), [&] {
    if (DataRate::BitsPerSec(caller->GetStats().send_bandwidth_bps) >=
        3 * send_call_config.transport.rates.start_rate) {
      success = true;
    }
  });

  s.RunUntil(TimeDelta::Seconds(1), TimeDelta::Millis(20),
             [&]() { return success; });

  EXPECT_TRUE(success);
}

TEST(ProbingTest, TriggerMidCallProbingTest) {
  Scenario s;

  static const DataRate kStartRate = DataRate::KilobitsPerSec(300);
  static const DataRate kConstrainedRate = DataRate::KilobitsPerSec(100);
  static const DataRate kHighRate = DataRate::KilobitsPerSec(2500);
  ...
  enum class TestState {
    ...
    kRelaxedConstraints,
    kCompleted
  } state = TestState::kInitialRampup;
  ...
  s.Every(TimeDelta::Millis(20), [&] {
    if (DataRate::BitsPerSec(caller->GetStats().send_bandwidth_bps) >=
      ...
      state = TestState::kConstrainedSend;
    }
  });
  s.RunUntil(TimeDelta::Seconds(1), TimeDelta::Millis(20),
             [&]() { return state == TestState::kConstrainedSend; });
  EXPECT_EQ(state, TestState::kConstrainedSend);
  ...
  s.Every(TimeDelta::Millis(20), [&] {
    if (DataRate::BitsPerSec(caller->GetStats().send_bandwidth_bps) <
      ...
      caller->UpdateBitrateConstraints(bitrate_config);
      state = TestState::kRelaxedConstraints;
    }
  });
  s.RunUntil(TimeDelta::Seconds(2), TimeDelta::Millis(20),
             [&]() { return state == TestState::kRelaxedConstraints; });
  EXPECT_EQ(state, TestState::kRelaxedConstraints);
  ...
  s.Every(TimeDelta::Millis(20), [&] {
    if (DataRate::BitsPerSec(caller->GetStats().send_bandwidth_bps) >
        kHighRate) {
      state = TestState::kCompleted;
    }
  });
  s.RunUntil(TimeDelta::Seconds(3), TimeDelta::Millis(20),
             [&]() { return state == TestState::kCompleted; });
  EXPECT_EQ(state, TestState::kCompleted);
}

TEST(ProbingTest, ProbeOnVideoEncoderReconfiguration) {
  Scenario s;
  static const DataRate kStartRate = DataRate::KilobitsPerSec(50);
  static const DataRate kHdRate = DataRate::KilobitsPerSec(3250);

  // Set up 3-layer simulcast, but with a lower than normal max bitrate.
  VideoStreamConfig video_config;
  video_config.encoder.codec =
      VideoStreamConfig::Encoder::Codec::kVideoCodecVP8;
  ...
  video_config.encoder.max_data_rate = DataRate::KilobitsPerSec(150);
