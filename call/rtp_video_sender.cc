...
void RtpVideoSender::OnBitrateUpdated(uint32_t bitrate_bps,
  ...
  {
    rtc::CritScope lock(&loss_mask_lock_);
    loss_mask_vector.swap(loss_mask_vector_);
  }
  // Get the encoder target rate. It is the estimated network rate -
  // protection overhead.
  encoder_target_rate_bps_ = fec_controller_->UpdateFecRates(
      payload_bitrate_bps, framerate, fraction_loss, loss_mask_vector, rtt);
...
void RtpVideoSender::OnPacketFeedbackVector(
  ...
  {
    rtc::CritScope cs(&loss_mask_lock_);
