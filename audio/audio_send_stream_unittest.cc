...
struct ConfigHelper {
  ...
  std::unique_ptr<internal::AudioSendStream>
      CreateAudioSendStream(const AudioSendStream::Config& config) {
    std::unique_ptr<internal::AudioSendStream> stream(
        new internal::AudioSendStream(
            config, audio_state_, &worker_queue_, &fake_transport_,
            &bitrate_allocator_, &event_log_, &rtcp_rtt_stats_,
            rtc::nullopt, channel_proxy_));
    return stream;
  }
...
TEST(AudioSendStreamTest, ConstructDestruct) {
  ...
  auto send_stream = helper.CreateAudioSendStream(helper.config());
}
...
TEST(AudioSendStreamTest, SendCodecAppliesAudioNetworkAdaptor) {
  ...
  auto send_stream = helper.CreateAudioSendStream(stream_config);
