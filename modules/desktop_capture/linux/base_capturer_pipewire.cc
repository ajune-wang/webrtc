...
void BaseCapturerPipeWire::HandleBuffer(pw_buffer* buffer) {
  ...
  _current_frame.reset(new BasicDesktopFrame(
      DesktopSize(video_size_.width(), video_size_.height())));
  _current_frame->CopyPixelsFrom(
      updated_src,
  ...
  if (spa_video_format_.format == SPA_VIDEO_FORMAT_RGBx ||
    ...
    uint8_t* tmp_src = _current_frame->data();
    for (int i = 0; i < video_size_.height(); ++i) {
      ...
      ConvertRGBxToBGRx(tmp_src, _current_frame->stride());
      tmp_src += _current_frame->stride();
    }
  }
}
...
void BaseCapturerPipeWire::CaptureFrame() {
  ...
  webrtc::MutexLock lock(&current_frame_lock_);
  if (!_current_frame || !_current_frame->data()) {
  ...
  callback_->OnCaptureResult(Result::SUCCESS, std::move(_current_frame));
}
