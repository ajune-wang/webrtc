...
bool SctpTransport::SendQueuedStreamResets() {
  ...
  // that will be how many entries we will have in the sctp_reset_streams
  // structure
  size_t num_streams = 0;
  for (ResetStreamsStatus::iterator it = reset_streams_status_.begin();
       it != reset_streams_status_.end(); ++it) {
    num_streams +=
        ((it->second & kStreamResetOutgoingSent) == kStreamResetNone) ? 1 : 0;
  }
  ...
  // compute the space required to send all necessary SCTP_STREAM_RESET_OUTGOING
  // messages
...
void SctpTransport::OnStreamResetEvent(
  ...
  if (evt->strreset_flags & SCTP_STREAM_RESET_FAILED) {
    ...
    // The SCTP session should be dismantled, and the WebRTC connectivity
    // errored because is clear that the distant party is not playing ball:
    // malforms the transported data
    return;
  }
  ...
  for (int i = 0; i < num_sids; i++) {
    ...
    ResetStreamsStatus::iterator foundInResetState =
        reset_streams_status_.find(stream_id);
    uint8_t status = (foundInResetState == reset_streams_status_.end())
                         ? kStreamResetNone
                         : foundInResetState->second;

    // update the status
    status |= ((evt->strreset_flags & SCTP_STREAM_RESET_INCOMING_SSN)
                   ? kStreamResetIncomingReceived
                   : kStreamResetNone);
    status |= ((evt->strreset_flags & SCTP_STREAM_RESET_OUTGOING_SSN)
                   ? kStreamResetOutgoingReceived
                   : kStreamResetNone);

    // inspect the status. If this is a complete reset, simply remove it from
    // the reset_streams_status_  thus making the SID available for reuse
    ...
    // ok, the stream reset still needs some more work. See if we need to send
    // the  SCTP_STREAM_RESET_OUTGOING_SSN and update the
    // callSendQueuedStreamResets flag
    callSendQueuedStreamResets |=
        ((status & kStreamResetOutgoingSent) == kStreamResetNone);
  }
