...
namespace {
constexpr size_t kMaxDependenciesVp8 = CodecSpecificInfoVP8::kMaxDependencies;
...
RtpPayloadParams::RtpPayloadParams(const uint32_t ssrc,
  ...
  buffer_id_to_last_shared_frame_id_.fill(-1);
...
void RtpPayloadParams::SetDependenciesVp8New(
  ...
  if (is_keyframe) {
    ...
    buffer_id_to_last_shared_frame_id_.fill(shared_frame_id);
    return;
  }

  RTC_DCHECK_GT(vp8_info.referencedBuffersCount, 0u);
  RTC_DCHECK_LE(vp8_info.referencedBuffersCount, kMaxDependenciesVp8);

  for (size_t i = 0; i < vp8_info.referencedBuffersCount; ++i) {
    const size_t referenced_buffer = vp8_info.referencedBuffers[i];
    RTC_DCHECK_LT(referenced_buffer, buffer_id_to_last_shared_frame_id_.size());
    RTC_DCHECK_LT(referenced_buffer, 3);  // VP8 imposes an even stricter limit.
    const int64_t dependency_frame_id =
        buffer_id_to_last_shared_frame_id_[referenced_buffer];
  ...
  RTC_DCHECK_LE(vp8_info.updatedBuffersCount, kMaxDependenciesVp8);
  for (size_t i = 0; i < vp8_info.updatedBuffersCount; ++i) {
    const size_t updated_id = vp8_info.updatedBuffers[i];
    buffer_id_to_last_shared_frame_id_[updated_id] = shared_frame_id;
  }

  RTC_DCHECK_LE(buffer_id_to_last_shared_frame_id_.size(), kMaxDependenciesVp8);
}
