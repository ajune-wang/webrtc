...
struct DataReader {
  ...
  T GetNum() {
    if (offset_ + sizeof(T) < size_) {
      T res = *reinterpret_cast<const T*>(data_ + offset_);
      offset_ += sizeof(T);
      return res;
    }
...
struct FuzzyPacketBuffer : public video_coding::PacketBuffer {
  ...
  VCMPacket* GetPacket(uint16_t seq_num) override {
    ...
    VCMPacket* packet = &packets.emplace(seq_num, VCMPacket()).first->second;
    packet->codec = codec;
