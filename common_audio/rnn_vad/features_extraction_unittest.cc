...
TEST(RnnVad, CheckComputeBandEnergiesPipeline) {
  ...
  // Process frames.
  {
    ...
    for (size_t i = 0; i < num_frames; ++i) {
      ...
      CheckBandEnergies({expected_band_energies}, {computed_band_energies},
                        0.1f, 1.5f);
    }
  }
}
...
TEST(RnnVad, CheckComputeBandEnergiesPipelineLowerSampleRate) {
  ...
  std::array<float, kFrameSize10ms24kHz> samples_10ms;
  rtc::ArrayView<float, kFrameSize10ms24kHz> samples_10ms_view(
      samples_10ms.data(), samples_10ms.size());
  SequenceBuffer<float, kFrameSize20ms24kHz, kFrameSize10ms24kHz> seq_buf(0.f);
  auto seq_buf_view = seq_buf.GetBufferView();
  BiQuadFilter hpf(
      {-1.98889291, 0.98895425, 0.99446179, -1.98892358, 0.99446179});
  RnnVadFft fft(kFrameSize20ms24kHz);
  auto fft_output_buf_view = fft.GetFftOutputBufferView();
  const auto band_boundary_indexes =
      ComputeOpusBandBoundaries(kSampleRate24kHz, kFrameSize20ms24kHz);
  std::array<float, kNumOpusBands> computed_band_energies;
