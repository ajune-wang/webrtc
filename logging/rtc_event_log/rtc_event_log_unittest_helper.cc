...
constexpr int kMaxExtensionSizeBytes =
    4;  // Serialized size including 1 byte ID.
...
// const std::pair<RTPExtensionType, char*> kExtensionTypes[] = {
//     RTPExtensionType::kRtpExtensionTransmissionTimeOffset,
//     RTPExtensionType::kRtpExtensionAbsoluteSendTime,
//     RTPExtensionType::kRtpExtensionTransportSequenceNumber,
...
EventGenerator::NewBweUpdateDelayBased() {
  ...
  BandwidthUsage state = states[prng_.Rand(0, kNumBwStates - 1)];
...
EventGenerator::NewIceCandidatePairConfig() {
  ...
  IceCandidatePairConfigType type =
      event_types[prng_.Rand(0, kNumIceConfigTypes - 1)];
  uint32_t pair_id = prng_.Rand<uint32_t>();

  IceCandidatePairDescription desc;

  desc.local_candidate_type =
      candidate_types[prng_.Rand(0, kNumCandidateTypes - 1)];
  desc.local_relay_protocol = protocol_types[prng_.Rand(0, kNumProtocols - 1)];
  desc.local_network_type = network_types[prng_.Rand(0, kNumNetworkTypes - 1)];
  desc.local_address_family =
      address_families[prng_.Rand(0, kNumAddressFamilies - 1)];
  desc.remote_candidate_type =
      candidate_types[prng_.Rand(0, kNumCandidateTypes - 1)];
  desc.remote_address_family =
      address_families[prng_.Rand(0, kNumAddressFamilies - 1)];
  desc.candidate_pair_protocol =
      protocol_types[prng_.Rand(0, kNumProtocols - 1)];
...
EventGenerator::NewIceCandidatePair() {
  ...
  IceCandidatePairEventType type =
      event_types[prng_.Rand(0, kNumIceCheckTypes - 1)];
  uint32_t pair_id = prng_.Rand<uint32_t>();
...
EventGenerator::NewRtcpPacketIncoming() {
  ...
  report_block.SetMediaSsrc(prng_.Rand<uint32_t>());  // Remote SSRC.
  report_block.SetFractionLost(prng_.Rand(50));
...
std::unique_ptr<RtcEventRtpPacketIncoming> EventGenerator::NewRtpPacketIncoming(
  ...
  size_t packet_size = prng_.Rand(kMaxHeaderSize, IP_PACKET_SIZE - 1);
...
RtpHeaderExtensionMap EventGenerator::NewRtpHeaderExtensionMap() {
  ...
  if (prng_.Rand<bool>()) {
    extension_map.Register<AudioLevel>(prng_.Rand(1, 2));
  }
  if (prng_.Rand<bool>()) {
    extension_map.Register<TransmissionOffset>(prng_.Rand(3, 4));
  }
  if (prng_.Rand<bool>()) {
    extension_map.Register<AbsoluteSendTime>(prng_.Rand(5, 6));
  }
  if (prng_.Rand<bool>()) {
    extension_map.Register<VideoOrientation>(prng_.Rand(7, 8));
  }
  if (prng_.Rand<bool>()) {
    extension_map.Register<TransportSequenceNumber>(prng_.Rand(9, 10));
  }
...
EventGenerator::NewAudioReceiveStreamConfig(
  ...
  for (unsigned i = 0; i < kMaxNumExtensions; i++) {
...
EventGenerator::NewVideoReceiveStreamConfig(
  ...
  config->codecs.emplace_back(prng_.Rand<bool>() ? "VP8" : "H264",
                              prng_.Rand(1, 127), prng_.Rand(1, 127));
  // Add header extensions.
  for (unsigned i = 0; i < kMaxNumExtensions; i++) {
    uint8_t id = extensions.GetId(kExtensions[i].type);
    if (id != RtpHeaderExtensionMap::kInvalidId) {
