...
struct AudioProcessingImpl::ApmPrivateSubmodules {
  ...
  std::unique_ptr<CustomAudioAnalyzer> capture_analyzer;
  std::unique_ptr<RmsLevel> rms_level_estimator;
};
...
void AudioProcessingImpl::ApplyConfig(const AudioProcessing::Config& config) {
  ...
  if (config_.level_estimation.enabled &&
      !private_submodules_->rms_level_estimator) {
    private_submodules_->rms_level_estimator.reset(new RmsLevel());
  }
}
...
int AudioProcessingImpl::ProcessCaptureStreamLocked() {
  ...
  if (config_.level_estimation.enabled) {
    for (size_t i = 0; i < capture_buffer->num_channels(); i++) {
      private_submodules_->rms_level_estimator->Analyze(
          rtc::ArrayView<const int16_t>(capture_buffer->channels_const()[i],
                                        capture_buffer->num_frames()));
    }
    capture_.stats.output_rms_dbfs =
        private_submodules_->rms_level_estimator->Average();
  } else {
