...
namespace test {
namespace {
constexpr DataRate kFirstClusterRate = DataRate::KilobitsPerSecond(900);
constexpr DataRate kSecondClusterRate = DataRate::KilobitsPerSecond(1800);
...
constexpr DataRate kProbingErrorMargin = DataRate::KilobitsPerSecond(150);
...
constexpr DataRate kTargetRate = DataRate::KilobitsPerSecond(800);
 ...
 protected:
  ...
  void ConsumeInitialBudget() {
    ...
    const size_t packets_to_send_per_interval =
        kTargetRate.BitsPerSecond() * kPaceMultiplier / (8 * kPacketSize * 200);
...
TEST_P(PacingControllerFieldTrialTest, CongestionWindowAffectsAudioInTrial) {
  ...
  EXPECT_CALL(callback_, SendPadding).Times(0);
  PacingController pacer(&clock_, &callback_, nullptr, nullptr, GetParam());
  pacer.SetPacingRates(DataRate::KilobitsPerSecond(10000), DataRate::Zero());
...
TEST_P(PacingControllerFieldTrialTest,
  ...
  EXPECT_CALL(callback_, SendPadding).Times(0);
  PacingController pacer(&clock_, &callback_, nullptr, nullptr, GetParam());
  pacer.SetPacingRates(DataRate::BitsPerSecond(10000000), DataRate::Zero());
...
TEST_P(PacingControllerFieldTrialTest, BudgetAffectsAudioInTrial) {
  ScopedFieldTrials trial("WebRTC-Pacer-BlockAudio/Enabled/");
  PacingController pacer(&clock_, &callback_, nullptr, nullptr, GetParam());
  DataRate pacing_rate = DataRate::BitsPerSecond(video.packet_size / 3 * 8 *
...
TEST_P(PacingControllerFieldTrialTest, DefaultBudgetDoesNotAffectAudio) {
  EXPECT_CALL(callback_, SendPadding).Times(0);
  PacingController pacer(&clock_, &callback_, nullptr, nullptr, GetParam());
  pacer.SetPacingRates(DataRate::BitsPerSecond(video.packet_size / 3 * 8 *
                                               kProcessIntervalsPerSecond),
                       DataRate::Zero());
  // Video fills budget for following process periods.
  InsertPacket(&pacer, &video);
...
TEST_P(PacingControllerTest, QueuePacket) {
  ...
  const size_t kPacketsToSend =
      kTargetRate.BitsPerSecond() * kPaceMultiplier / (8 * 250 * 200);
...
TEST_P(PacingControllerTest, VerifyAverageBitrateVaryingMediaPayload) {
  ...
  EXPECT_NEAR(
      kTargetRate.BitsPerSecond(),
      (DataSize::Bytes(callback.total_bytes_sent()) / kAveragingWindowLength)
          .BitsPerSecond(),
      (kTargetRate * 0.01 /* 1% error marging */).BitsPerSecond());
}
...
TEST_P(PacingControllerTest, HighPrioDoesntAffectBudget) {
  ...
  EXPECT_NEAR(pacing_time.Microseconds<double>(),
              expected_pacing_time.Microseconds<double>(),
              PeriodicProcess()
                  ? 5000.0
                  : PacingController::kMinSleepTime.Microseconds<double>());
}
...
TEST_P(PacingControllerTest, DoesNotAllowOveruseAfterCongestion) {
  ...
  // to be sent in a row.
  pacer_->SetPacingRates(DataRate::BitsPerSecond(400 * 8 * 1000 / 5),
                         DataRate::Zero());
...
TEST_P(PacingControllerTest, ExpectedQueueTimeMs) {
  ...
  pacer_->SetPacingRates(DataRate::BitsPerSecond(30000 * kPaceMultiplier),
                         DataRate::Zero());
...
TEST_P(PacingControllerTest, ProbingWithInsertedPackets) {
  ...
  pacer_->SetPacingRates(
      DataRate::BitsPerSecond(kInitialBitrateBps * kPaceMultiplier),
      DataRate::Zero());
  ...
  EXPECT_NEAR((packets_sent - 1) * kPacketSize * 8000 /
              ...
              kFirstClusterRate.BitsPerSecond(),
              kProbingErrorMargin.BitsPerSecond());
  ...
  EXPECT_NEAR((packets_sent - 1) * kPacketSize * 8000 /
                  (clock_.TimeInMilliseconds() - start),
              kSecondClusterRate.BitsPerSecond(),
              kProbingErrorMargin.BitsPerSecond());
}
...
TEST_P(PacingControllerTest, SkipsProbesWhenProcessIntervalTooLarge) {
  ...
  pacer_->CreateProbeCluster(DataRate::KilobitsPerSecond(10000),  // 10 Mbps.
                             /*cluster_id=*/3);
...
TEST_P(PacingControllerTest, PaddingOveruse) {
  ...
  pacer_->ProcessPackets();
  pacer_->SetPacingRates(DataRate::BitsPerSecond(60000 * kPaceMultiplier),
                         DataRate::Zero());
...
TEST_P(PacingControllerTest, TaskLate) {
  ...
  DataRate kSendRate = DataRate::KilobitsPerSecond(30);
