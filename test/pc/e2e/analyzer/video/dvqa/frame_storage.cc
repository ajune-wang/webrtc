...
#include "test/pc/e2e/analyzer/video/dvqa/frame_storage.h"
...
void FrameStorage::Add(const VideoFrame& frame, Timestamp captured_time) {
...
absl::optional<VideoFrame> FrameStorage::Get(uint16_t frame_id) {
...
void FrameStorage::Remove(uint16_t frame_id) {
  RemoveTooOldFrames();
...
void FrameStorage::Heapify(size_t index) {
  HeapifyUp(index);
  HeapifyDown(index);
}

void FrameStorage::HeapifyUp(size_t index) {
...
void FrameStorage::HeapifyDown(size_t index) {
...
void FrameStorage::RemoveTooOldFrames() {
  Timestamp now = clock_->CurrentTime();
  while (!heap_.empty() &&
         (heap_[0].captured_time + max_storage_duration_) < now) {
    frame_id_index_.erase(heap_[0].frame.id());
    if (heap_.size() != 1) {
      heap_[0] = std::move(heap_[heap_.size() - 1]);
      frame_id_index_[heap_[0].frame.id()] = 0;
    }
    heap_.pop_back();
    Heapify(0);
  }
