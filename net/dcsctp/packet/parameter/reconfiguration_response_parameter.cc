...
ReconfigurationResponseParameter::Parse(rtc::ArrayView<const uint8_t> data) {
  ...
  uint32_t response_sequence_number = reader->Load32<4>();
  Result result;
  switch (reader->Load32<8>()) {
    case 0:
  ...
  if (reader->variable_data().empty()) {
    ...
    RTC_LOG(LS_WARNING) << "Invalid parameter size";
    return absl::nullopt;
  }
  ...
  uint32_t sender_next_tsn = sub_reader.Load32<0>();
  uint32_t receiver_next_tsn = sub_reader.Load32<4>();
...
void ReconfigurationResponseParameter::SerializeTo(
  ...
  if (sender_next_tsn().has_value()) {
    ...
    sub_writer.Store32<0>(sender_next_tsn_.value_or(0));
    sub_writer.Store32<4>(receiver_next_tsn_.value_or(0));
  }
}
