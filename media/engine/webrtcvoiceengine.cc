...
// Set and adjust gain control options.
  ...
  if (webrtc::field_trial::IsEnabled(
    ...
    if (!(options.noise_suppression.value_or(false) or
      ...
      RTC_LOG(LS_INFO) << "Disable high-pass filter in response to field "
                       << "trial.";
  ...
  if (options.echo_cancellation) {
    ...
    if (built_in_aec) {
      ...
      if (adm()->EnableBuiltInAEC(enable_built_in_aec) == 0 &&
        ...
        options.echo_cancellation = rtc::Optional<bool>(false);
        RTC_LOG(LS_INFO) << "Disabling EC since built-in EC will be used "
                         << "instead";
      }
    }
  ...
  if (options.auto_gain_control) {
    ...
    if (built_in_agc_avaliable) {
      if (adm()->EnableBuiltInAGC(*options.auto_gain_control) == 0 &&
        ...
        options.auto_gain_control = rtc::Optional<bool>(false);
        RTC_LOG(LS_INFO) << "Disabling AGC since built-in AGC will be used "
                         << "instead";
      }
    }
  ...
  if (options.noise_suppression) {
    if (adm()->BuiltInNSIsAvailable()) {
      ...
      if (adm()->EnableBuiltInNS(builtin_ns) == 0 && builtin_ns) {
        ...
        options.noise_suppression = rtc::Optional<bool>(false);
        RTC_LOG(LS_INFO) << "Disabling NS since built-in NS will be used "
                         << "instead";
      }
    }
  ...
  if (ssrc == 0) {
    if (!default_sink_) {
      RTC_LOG(LS_WARNING) << "Attempting to get RTP parameters for the "
                          << "default, unsignaled audio receive stream, but "
...
bool WebRtcVoiceMediaChannel::SetOptions(const AudioOptions& options) {
  RTC_DCHECK(worker_thread_checker_.CalledOnValidThread());
  RTC_LOG(LS_INFO) << "Setting voice channel options: "
               << options.ToString();
      ...
      if (IsCodec(cn_codec, kCnCodecName) &&
        ...
        switch (cn_codec.clockrate) {
          ...
          case 32000:
            ...
            break;
          default:
            RTC_LOG(LS_WARNING) << "CN frequency " << cn_codec.clockrate
                            << " not supported.";
            break;
        }
        break;
