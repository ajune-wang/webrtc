...
class H264PacketBuffer {
 ...
 private:
  ...
  const std::unique_ptr<Packet>& GetPacket(int64_t unwrapped_seq_num) const;
  std::unique_ptr<Packet>& GetPacket(int64_t unwrapped_seq_num);
  bool BeginningOfStream(const Packet& packet) const;
  bool ContinuousWithLastPacket(int64_t unwrapped_seq_num) const;
  bool ContinuousWithLastMarkerBit(int64_t unwrapped_seq_num) const;

  void ClearOldPackets(int64_t unwrapped_seq_num);
  ...
  absl::optional<int64_t> last_marker_bit_unwrapped_;
  absl::optional<int64_t> last_seq_num_unwrapped_;
