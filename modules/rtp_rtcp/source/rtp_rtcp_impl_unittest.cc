...
// send.
TEST_F(RtpRtcpImplTest, NoSenderReportStats) {
...
TEST_F(RtpRtcpImplTest, SenderReportStatsTimestampsAvailable) {
  absl::optional<int64_t> last_arrival_timestamp_ms;
  absl::optional<int64_t> last_remote_timestamp_ms;
  uint32_t packets_sent;
  uint64_t bytes_sent;
...
TEST_F(RtpRtcpImplTest, SenderReportStatsArrivalAfterRemote) {
  // Simulate network delay so that the RTCP SR remote timestamp will be less
  // than the SR arrival timestamp.
  sender_.transport_.SimulateNetworkDelay(1, &clock_);
...
// has been received.
TEST_F(RtpRtcpImplTest, SenderReportStatsSentPacketsAndBytesSet) {
  absl::optional<int64_t> last_arrival_timestamp_ms;
  absl::optional<int64_t> last_remote_timestamp_ms;
  uint32_t packets_sent;
  uint64_t bytes_sent;
