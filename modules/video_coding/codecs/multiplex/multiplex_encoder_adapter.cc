...
int MultiplexEncoderAdapter::Encode(
  ...
  MultiplexVideoFrameBuffer* multiplex_video_frame_buffer = static_cast<MultiplexVideoFrameBuffer *>(input_image.video_frame_buffer().get());
  uint8_t* data = nullptr;
  uint16_t data_length = 0;
  if (multiplex_video_frame_buffer != nullptr) {
    data = multiplex_video_frame_buffer->GetAndReleaseAugmentingData();
    data_length = multiplex_video_frame_buffer->GetAugmentingDataSize();
  }
  ...
  const I420ABufferInterface* yuva_buffer =
    supports_augmented_data_ ? multiplex_video_frame_buffer->GetVideoFrameBuffer()->GetI420A() : input_image.video_frame_buffer()->GetI420A();
...
int MultiplexEncoderAdapter::SetRateAllocation(
    ...
    uint32_t framerate) {
  BitrateAllocation bitrateAllocation(bitrate);
  uint32_t rate = bitrate.GetBitrate(0, 0);
  bitrateAllocation.SetBitrate(0, 0, rate - data_size_);
  for (auto& encoder : encoders_) {
    // TODO(emircan): |framerate| is used to calculate duration in encoder
    // instances. We report the total frame rate to keep real time for now.
    // Remove this after refactoring duration logic.
    const int rv = encoder->SetRateAllocation(
        bitrateAllocation, static_cast<uint32_t>(encoders_.size()) * framerate);
    if (rv)
      return rv;
  }
  return WEBRTC_VIDEO_CODEC_OK;
