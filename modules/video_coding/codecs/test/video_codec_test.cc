...
std::unique_ptr<VideoCodecStats> RunEncodeDecodeTest(
    ...
    const FramesSettings& frames_settings) {
  ...
  const EncodingSettings& encoding_settings = frames_settings.begin()->second;
  if (!decoder_factory
...
std::unique_ptr<VideoCodecStats> RunEncodeTest(
  ...
  return tester->RunEncodeTest(source_settings, encoder_factory.get(),
                               encoder_settings, frames_settings);
}
...
TEST_P(SpatialQualityTest, SpatialQuality) {
  ...
  FramesSettings frames_settings = VideoCodecTester::CreateFramesSettings(
      codec_type, /*scalability_mode=*/"L1T1", width, height, {bitrate_kbps},
      framerate_fps, num_frames);
  ...
  if (absl::GetFlag(FLAGS_webrtc_quick_perf_test)) {
    VideoCodecStats::Stream stream = stats->Aggregate();
...
TEST_P(BitrateAdaptationTest, BitrateAdaptation) {
  ...
  uint32_t initial_timestamp_rtp =
      frames_settings.rbegin()->first + k90kHz / Frequency::Hertz(30);
  FramesSettings frames_settings2 = VideoCodecTester::CreateFramesSettings(
      codec_type, /*scalability_mode=*/"L1T1", /*width=*/640, /*height=*/360,
      {bitrate_kbps.first}, 30.0, num_frames / 2, initial_timestamp_rtp);

  frames_settings.merge(frames_settings2);

  std::unique_ptr<VideoCodecStats> stats =
      RunEncodeTest(codec_type, codec_impl, video_info, frames_settings);
...
TEST_P(FramerateAdaptationTest, FramerateAdaptation) {
  ...
  FramesSettings frames_settings = VideoCodecTester::CreateFramesSettings(
      codec_type, /*scalability_mode=*/"L1T1", /*width=*/640, /*height=*/360,
      /*layer_bitrates_kbps=*/{512}, framerate_fps.first, num_frames);

  uint32_t initial_timestamp_rtp =
      frames_settings.rbegin()->first +
  ...
  VideoCodecStats::Stream stream;
  if (stats != nullptr) {
    stream = stats->Aggregate({.first_frame = num_frames});
    if (absl::GetFlag(FLAGS_webrtc_quick_perf_test)) {
      EXPECT_NEAR(stream.bitrate_mismatch_pct.GetAverage(), 0, 10);
      EXPECT_NEAR(stream.framerate_mismatch_pct.GetAverage(), 0, 10);
    }
