...
JsepTransportController::MaybeCreateMediaTransport(
  ...
  if (local) {
    if (offer_media_transport_) {
      RTC_LOG(LS_INFO) << "Returning pre-heated media transport";
      return std::move(offer_media_transport_);
    } else {
      RTC_LOG(LS_INFO) << "Not returning subsequent media transport.";
  ...
  RTC_DCHECK(!local)
      << "If media transport is used, you must call "
         "GenerateMediaTransportOffer before SetLocalDescription. You also "
...
RTCError JsepTransportController::MaybeCreateJsepTransport(
    ...
    const cricket::ContentInfo& content_info,
    const cricket::SessionDescription& description) {
  RTC_LOG(LS_INFO) << "content_info=" << content_info.name;
  RTC_DCHECK(network_thread_->IsCurrent());
  ...
  RTC_LOG(LS_INFO) << "Jsep transport created, name=" << content_info.name
                   << ", media_transport="
                   << (jsep_transport->media_transport() != nullptr);
...
JsepTransportController::GenerateMediaTransportOffer() {
  ...
  if (config_.use_media_transport_for_media ||
    ...
    if (media_transport_or_error.ok()) {
      RTC_DCHECK(media_transport_or_error.value());
