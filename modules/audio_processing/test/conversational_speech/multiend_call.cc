...
bool MultiEndCall::CreateAudioTrackReaders() {
  ...
  for (const Turn& turn : timing_) {
    ...
    if (wavreader->NumChannels() != 1) {
      RTC_DLOG(LS_ERROR) << "Only mono audio tracks supported.";
      return false;
    }
...
bool MultiEndCall::CheckTiming() {
  ...
  for (size_t turn_index = 0; turn_index < number_of_turns; ++turn_index) {
    ...
    RTC_DLOG(LS_INFO) << "turn #" << turn_index << " " << begin_timestamp << "-"
                     << end_timestamp << " ms";
    ...
    if (offset_samples < 0 &&
      ...
      RTC_DLOG(LS_ERROR) << "invalid order";
      return false;
    }
    ...
    if (turn_index > 1 && in_interval(begin_timestamp, last_turn) &&
      ...
      RTC_DLOG(LS_ERROR) << "cross-talk with 3+ speakers";
      return false;
    }
  ...
  // Detect self cross-talk.
  for (const std::string& speaker_name : speaker_names_) {
    RTC_DLOG(LS_INFO) << "checking self cross-talk for <" << speaker_name << ">";
    ...
    if (overlap != speaking_turns_for_name.end()) {
      RTC_DLOG(LS_ERROR) << "Self cross-talk detected";
      return false;
    }
  }
