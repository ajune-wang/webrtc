...
uint32_t get_ssrc(SessionDescriptionInterface* offer, size_t index) {
...
TEST(UnsignaledStreamTest, ReplacedUnsignaledStreamOnCompletedSignaling) {
  // This test covers a scenarion that might occur if a remote client starts
  ...
  // In some edge cases the default stream is create in a different
  // transciever to where the media SSRC will actually be used.
  // This test verifies that the default stream is removed properly, and that
  // packets are demuxed properly and video frames reach the desired sink.
