...
TEST_F(BasicPortAllocatorTest, HostCandidateAddressIsReplacedByHostname) {
  ...
  // Start the mDNS responder before adding any interface so that the responder
  // is aware of the new network.
  //
  // This method is only defined for FakeManagerNetwork.
  network_manager_.StartMDnsResponder();
  AddInterface(kClientAddr);
  ASSERT_TRUE(CreateSession(ICE_CANDIDATE_COMPONENT_RTP));
  session_->StartGettingPorts();
  ASSERT_TRUE_SIMULATED_WAIT(candidate_allocation_done_,
