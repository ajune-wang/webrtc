...
const std::map<uint8_t, std::string> kExceptionCodes = {
...
TEST(FloatingPointExceptionObserver, DISABLED_CheckTestConstants) {
  ...
  for (const auto v : kExceptionCodes)
    all_flags |= v.first;
...
TEST(FloatingPointExceptionObserver, CheckNoFalsePositives) {
  for (const auto exception_code : kExceptionCodes) {
    SCOPED_TRACE(exception_code.second);
    TriggerObserveFloatingPointExceptions(
        exception_code.first, FE_ALL_EXCEPT & ~exception_code.first);
  }
}
...
TEST(FloatingPointExceptionObserver, CheckNoFalseNegatives) {
  for (const auto exception_code : kExceptionCodes) {
    ...
    if (exception_code.first == FE_INEXACT)
      continue;
    SCOPED_TRACE(exception_code.second);
    EXPECT_NONFATAL_FAILURE(TriggerObserveFloatingPointExceptions(
                                exception_code.first, exception_code.first),
                            "");
  }
}
#endif
