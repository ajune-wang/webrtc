...
RtpFrameReferenceFinder::ManageFrameGeneric(RtpFrameObject* frame,
  ...
  if (picture_id != kNoPictureId) {
    ...
    frame->key.picture_id = unwrapper_.Unwrap(picture_id);
    frame->num_references = frame->frame_type() == kVideoFrameKey ? 0 : 1;
    frame->references[0] = frame->key.picture_id - 1;
    return kHandOff;
  }
  ...
  frame->key.picture_id = frame->last_seq_num();
  frame->num_references = frame->frame_type() == kVideoFrameDelta;
  frame->references[0] = generic_unwrapper_.Unwrap(last_picture_id_gop);
  if (AheadOf<uint16_t>(frame->key.picture_id, last_picture_id_gop)) {
    seq_num_it->second.first = frame->key.picture_id;
    seq_num_it->second.second = frame->key.picture_id;
  }

  last_picture_id_ = frame->key.picture_id;
  UpdateLastPictureIdWithPadding(frame->key.picture_id);
  frame->key.picture_id = generic_unwrapper_.Unwrap(frame->key.picture_id);
  return kHandOff;
}
...
RtpFrameReferenceFinder::FrameDecision RtpFrameReferenceFinder::ManageFrameVp8(
  ...
  frame->key.picture_id = codec_header.pictureId % kPicIdLength;
  ...
  if (AheadOf<uint16_t, kPicIdLength>(frame->key.picture_id,
                                      last_picture_id_)) {
    do {
      last_picture_id_ = Add<kPicIdLength>(last_picture_id_, 1);
      not_yet_received_frames_.insert(last_picture_id_);
    } while (last_picture_id_ != frame->key.picture_id);
  }
  ...
  uint16_t old_picture_id =
      Subtract<kPicIdLength>(frame->key.picture_id, kMaxNotYetReceivedFrames);
  ...
  for (uint8_t layer = 0; layer <= codec_header.temporalIdx; ++layer) {
    ...
    if (not_received_frame_it != not_yet_received_frames_.end() &&
        AheadOf<uint16_t, kPicIdLength>(frame->key.picture_id,
    ...
    if (!(AheadOf<uint16_t, kPicIdLength>(frame->key.picture_id,
      ...
      RTC_LOG(LS_WARNING) << "Frame with picture id " << frame->key.picture_id
...
void RtpFrameReferenceFinder::UpdateLayerInfoVp8(
  ...
  while (layer_info_it != layer_info_.end()) {
    ...
    layer_info_it->second[codec_header.temporalIdx] = frame->key.picture_id;
    ++tl0_pic_idx;
    layer_info_it = layer_info_.find(tl0_pic_idx);
  }
  not_yet_received_frames_.erase(frame->key.picture_id);
...
RtpFrameReferenceFinder::FrameDecision RtpFrameReferenceFinder::ManageFrameVp9(
  ...
  frame->key.spatial_layer = codec_header.spatial_idx;
  frame->inter_layer_predicted = codec_header.inter_layer_predicted;
  frame->key.picture_id = codec_header.picture_id % kPicIdLength;
  ...
  if (codec_header.flexible_mode) {
    ...
    for (size_t i = 0; i < frame->num_references; ++i) {
      frame->references[i] = Subtract<kPicIdLength>(frame->key.picture_id,
  ...
  if (codec_header.ss_data_available) {
    ...
    if (codec_header.temporal_idx != 0) {
      ...
      scalability_structures_[current_ss_idx_].pid_start =
          frame->key.picture_id;
  ...
  if (frame->frame_type() == kVideoFrameKey) {
    ...
    FrameReceivedVp9(frame->key.picture_id, &info);
    UnwrapPictureIds(frame);
    return kHandOff;
  }
  ...
  // up switch flag set.
  if (MissingRequiredFrameVp9(frame->key.picture_id, *info))
    return kStash;

  if (codec_header.temporal_up_switch) {
    auto pid_tidx =
        std::make_pair(frame->key.picture_id, codec_header.temporal_idx);
  ...
  if (codec_header.temporal_idx == 0 && !codec_header.ss_data_available) {
    GofInfo new_info(info->gof, frame->key.picture_id);
    gof_info_.insert(std::make_pair(codec_header.tl0_pic_idx, new_info));
  }

  // Clean out old info about up switch frames.
  uint16_t old_picture_id = Subtract<kPicIdLength>(frame->key.picture_id, 50);
  ...
  for (size_t i = 0; i < frame->num_references; ++i) {
    frame->references[i] = Subtract<kPicIdLength>(
        frame->key.picture_id, info->gof->pid_diff[gof_idx][i]);

    // If this is a reference to a frame earlier than the last up switch point,
    // then ignore this reference.
    if (UpSwitchInIntervalVp9(frame->key.picture_id, codec_header.temporal_idx,
                              frame->references[i])) {
      --frame->num_references;
    }
  }
...
void RtpFrameReferenceFinder::UnwrapPictureIds(RtpFrameObject* frame) {
  ...
  frame->key.picture_id = unwrapper_.Unwrap(frame->key.picture_id);
}
