...
class RTC_EXPORT DesktopCaptureOptions {
 public:
  ...
  // deadlock. Consumers can either ensure that the thread that runs their
  // message loop never waits on |GetSourceList()|, or they can set this flag to
  // false which will prevent windows running in the current process from being
  // enumerated and included in the results. Consumers can still provide the
  // WindowId for their own windows to |SelectSource()| and capture them.
