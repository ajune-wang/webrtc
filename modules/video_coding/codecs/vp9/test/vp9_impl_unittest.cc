 ...
 protected:
  void SetUp() override {
    ...
    input_frame_generator_ = test::FrameGenerator::CreateSquareGenerator(
        ...
        test::FrameGenerator::OutputType::I420_BITDEPTH10,
        rtc::Optional<int>());
  }

  std::unique_ptr<VideoEncoder> CreateEncoder() override {
    cricket::VideoCodec profile2_codec;
    profile2_codec.SetParam(webrtc::VP9::kVP9Profile, webrtc::VP9::kProfile2);
...
rtc::scoped_refptr<I420Buffer> BitDepth10FrameToBitDepth8(VideoFrame* input) {
  rtc::scoped_refptr<I420BufferInterface> input_frame =
      input->video_frame_buffer()->ToI420();
  rtc::scoped_refptr<I420Buffer> input_frame_eq(
      I420Buffer::Create(input_frame->width(), input_frame->height()));
  libyuv::I010ToI420(reinterpret_cast<const uint16_t*>(input_frame->DataY()),
                     input_frame->StrideY() / 2,
                     reinterpret_cast<const uint16_t*>(input_frame->DataU()),
                     input_frame->StrideU() / 2,
                     reinterpret_cast<const uint16_t*>(input_frame->DataV()),
                     input_frame->StrideV() / 2, input_frame_eq->MutableDataY(),
                     input_frame_eq->StrideY(), input_frame_eq->MutableDataU(),
                     input_frame_eq->StrideU(), input_frame_eq->MutableDataV(),
                     input_frame_eq->StrideV(), input_frame->width(),
                     input_frame->height());
  return input_frame_eq;
}
...
TEST_F(TestVp9ImplProfile2, EncodeDecode) {
  ...
  // TODO(emircan): Add PSNR for different bitrates.
  EXPECT_GT(I420PSNR(*BitDepth10FrameToBitDepth8(input_frame),
                     *BitDepth10FrameToBitDepth8(decoded_frame.get())),
            10);
}
