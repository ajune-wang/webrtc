...
void CallClient::SwitchToEndpoint(size_t endpoint_idx) {
  RTC_DCHECK_GE(endpoint_idx, 0);
  RTC_DCHECK_LT(endpoint_idx, endpoints_.size());
  current_endpoint_ = endpoint_idx;
}
...
void CallClient::OnPacketReceived(rtc::CopyOnWriteBuffer buffer,
  ...
  RTC_DCHECK_GE(
      buffer.size(),
      route_overhead_.at(endpoints_[current_endpoint_]->GetId()).bytes());
  buffer.SetSize(
      buffer.size() -
      route_overhead_.at(endpoints_[current_endpoint_]->GetId()).bytes());
