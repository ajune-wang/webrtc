...
std::unique_ptr<RtpPacketToSend> RTPSender::AllocatePacket(
  ...
  MutexLock lock(&send_mutex_);
  // TODO(danilchap): Remove this fallback together with SetCsrcs member
  if (csrcs.empty()) {
    // New code shouldn't set csrcs_, old code would pass default value for
    // csrcs, which is empty.
    csrcs = csrcs_;
  }
