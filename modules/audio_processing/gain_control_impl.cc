...
void ApplyDigitalGain(const int32_t gains[11],
                      size_t num_bands,
                      float* const* out) {
  constexpr float kScaling = 1.f / 1048576.f;
  ...
  for (size_t b = 0; b < num_bands; ++b) {
    ...
    for (int k = 0, sample = 0; k < 10; ++k) {
      const float delta = gains_scaled[k + 1] - gains_scaled[k];
      float gain = gains_scaled[k] * 16.f;
      for (int n = 0; n < 16; ++n, ++sample) {
        RTC_DCHECK_EQ(k * 16 + n, sample);
