...
DecisionLogic::DecisionLogic(int fs_hz,
    ...
    : decoder_database_(decoder_database),
      ...
      time_stretched_cng_samples_(0),
      dtx_delay_("dtx_delay", false),
      cng_time_stretching_("cng_time_stretch", false),
  ...
  ParseFieldTrial(
      {&dtx_delay_, &cng_time_stretching_, &target_level_window_ms_},
      field_trial_name);
  RTC_LOG(LS_INFO) << "NetEq decision logic settings:"
                   << " dtx_delay=" << dtx_delay_
                   << " cng_time_stretching=" << cng_time_stretching_
...
Operations DecisionLogic::GetDecision(const SyncBuffer& sync_buffer,
  ...
  size_t cur_size_samples =
      dtx_delay_ ? packet_buffer_.GetSpanSamples(decoder_frame_length,
                                                 sample_rate_, true)
  ...
  if (prev_mode != kModeRfc3389Cng && prev_mode != kModeCodecInternalCng &&
      !(next_packet && next_packet->frame &&
        next_packet->frame->IsDtxPacket() && !dtx_delay_)) {
...
void DecisionLogic::FilterBufferLevel(size_t buffer_size_samples) {
  ...
  int time_stretched_samples = time_stretched_cng_samples_;
  if (prev_time_scale_) {
...
Operations DecisionLogic::FuturePacketAvailable(
  ...
  if (prev_mode == kModeRfc3389Cng || prev_mode == kModeCodecInternalCng) {
    size_t cur_size_samples =
        dtx_delay_ ? cur_size_samples = packet_buffer_.GetSpanSamples(
    ...
    if ((static_cast<uint32_t>(generated_noise_samples + target_timestamp) >=
         ...
         above_target_window) &&
        !below_target_window) {
      if (cng_time_stretching_) {
        time_stretched_cng_samples_ = timestamp_leap - generated_noise_samples;
      }
