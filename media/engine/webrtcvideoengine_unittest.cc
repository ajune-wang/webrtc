...
TEST_F(WebRtcVideoChannelTest, TestSetDscpOptions) {
  ...
  channel_->OnReadyToSend(true);
  send_parameters.encodings[0].network_priority = 4.0;
  ASSERT_TRUE(channel_->SetSendParameters(send_parameters_));
  EXPECT_EQ(RTC::DSCP_AF41, network_interface->dscp());
  send_parameters.encodings[0].network_priority = 0.5;
  ASSERT_TRUE(channel_->SetSendParameters(send_parameters_));
  EXPECT_EQ(RTC::DSCP_CS1, network_interface->dscp());

  // A bad priority does not change the dscp value.
  send_parameters.encodings[0].network_priority = 0.0;
  ASSERT_FALSE(channel_->SetSendParameters(send_parameters_));
  EXPECT_EQ(RTC::DSCP_CS1, network_interface->dscp());
...
TEST_F(WebRtcVideoChannelTest, SetRtpSendParametersInvalidNetworkPriority) {
  ...
  int good_values[] = {0.5, 1.0, 2.0, 4.0};
  int bad_values[] = {-1.0, 0.0, 0.49, 0.51, 1.1, 3.99, 4.1, 5.0};
