...
VideoStreamInputState::VideoStreamInputState(int frame_size_pixels,
...
VideoStreamAdapter::VideoStreamAdapter()
    : source_restrictor_(std::make_unique<VideoSourceRestrictor>()),
      ...
      input_(0, 0, VideoCodecType::kVideoCodecGeneric),
      last_adaptation_request_(absl::nullopt) {}

VideoStreamAdapter::~VideoStreamAdapter() {}

const VideoStreamInputState& VideoStreamAdapter::input_state() const {
  return input_;
}
...
VideoSourceRestrictions VideoStreamAdapter::filtered_source_restrictions()
    const {
...
void VideoStreamAdapter::SetInput(VideoStreamInputState input,
                                  int min_pixels_per_frame) {
  RTC_DCHECK_GE(input_.frame_size_pixels, 0);
  RTC_DCHECK_GE(input_.frames_per_second, 0);
  RTC_DCHECK_GE(min_pixels_per_frame, 0);
  // Invalidate any previously returned Adaptation.
  ++adaptation_validation_id_;
  input_ = input;
  source_restrictor_->set_min_pixels_per_frame(min_pixels_per_frame);
}
void VideoStreamAdapter::SetInput(
    int input_pixels,
    int input_fps,
    absl::optional<EncoderSettings> encoder_settings) {
  SetInput(VideoStreamInputState(input_pixels, input_fps,
                                 GetVideoCodecTypeOrGeneric(encoder_settings)),
           MinPixelsPerFrame(encoder_settings));
}
...
Adaptation VideoStreamAdapter::GetAdaptationUp() const {
  ...
  switch (degradation_preference_) {
    case DegradationPreference::BALANCED: {
      ...
      int target_fps = balanced_settings_.MaxFps(input_.video_codec_type,
                                                 input_.frame_size_pixels);
    ...
    case DegradationPreference::MAINTAIN_FRAMERATE: {
      ...
      int target_pixels = input_.frame_size_pixels;
    ...
    case DegradationPreference::MAINTAIN_RESOLUTION: {
      // Scale up framerate.
      int target_fps = input_.frames_per_second;
...
Adaptation VideoStreamAdapter::GetAdaptationDown() const {
  ...
  switch (degradation_preference_) {
    case DegradationPreference::BALANCED: {
      ...
      int target_fps = balanced_settings_.MinFps(input_.video_codec_type,
                                                 input_.frame_size_pixels);
    ...
    case DegradationPreference::MAINTAIN_FRAMERATE: {
      // Scale down resolution.
      int target_pixels = GetLowerResolutionThan(input_.frame_size_pixels);
      bool min_pixel_limit_reached =
    ...
    case DegradationPreference::MAINTAIN_RESOLUTION: {
      int target_fps = GetLowerFrameRateThan(input_.frames_per_second);
      if (!source_restrictor_->CanDecreaseFrameRateTo(target_fps)) {
        return Adaptation(adaptation_validation_id_,
                          Adaptation::Status::kLimitReached);
      }
...
ResourceListenerResponse VideoStreamAdapter::ApplyAdaptation(
  ...
  last_adaptation_request_.emplace(AdaptationRequest{
      input_.frame_size_pixels, input_.frames_per_second,
  ...
  if (degradation_preference_ == DegradationPreference::BALANCED &&
    ...
    absl::optional<int> min_diff =
        balanced_settings_.MinFpsDiff(input_.frame_size_pixels);
    if (min_diff && input_.frames_per_second > 0) {
      int fps_diff = input_.frames_per_second - adaptation.step().target;
