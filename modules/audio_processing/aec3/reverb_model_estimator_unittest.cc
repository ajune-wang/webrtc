...
class ReverbModelEstimatorTest {
 public:
  ...
  void RunEstimator(size_t num_capture_channels);
 ...
 private:
  ...
  std::vector<float> quality_linear_;
};
...
void ReverbModelEstimatorTest::CreateImpulseResponseWithDecay() {
  ...
  for (size_t i = 0; i < H2_.size(); ++i) {
    for (size_t j = 0, k = 0; j < H2_[i].size(); ++j, k += kBlockSize) {
      std::array<float, kFftLength> fft_data;
      fft_data.fill(0.f);
      std::copy(h_[i].begin() + k, h_[i].begin() + k + kBlockSize,
                fft_data.begin());
      FftData H_j;
      fft.Fft(&fft_data, &H_j);
      H_j.Spectrum(Aec3Optimization::kNone, H2_[i][j]);
    }
  }
  rtc::ArrayView<float> H2_tail(H2_[0][H2_[0].size() - 1]);
  true_power_tail_ = std::accumulate(H2_tail.begin(), H2_tail.end(), 0.f);
}
void ReverbModelEstimatorTest::RunEstimator(size_t num_capture_channels) {
...
TEST(ReverbModelEstimatorTests, NotChangingDecay) {
  ...
  ReverbModelEstimatorTest test(kDefaultDecay, kNumCaptureChannels);
  test.RunEstimator(kNumCaptureChannels);
