...
void LoadClasses(JNIEnv* jni) {
  RTC_DLOG(INFO) << "LoadClasses:";
  for (auto& c : loaded_classes) {
    jclass localRef = FindClass(jni, c.name);
    RTC_DLOG(INFO) << "name: " << c.name;
...
JvmThreadConnector::JvmThreadConnector() : attached_(false) {
  RTC_DLOG(INFO) << "JvmThreadConnector::ctor";
  JavaVM* jvm = JVM::GetInstance()->jvm();
  RTC_CHECK(jvm);
  JNIEnv* jni = GetEnv(jvm);
  if (!jni) {
    RTC_DLOG(INFO) << "Attaching thread to JVM";
    JNIEnv* env = nullptr;
...
JvmThreadConnector::~JvmThreadConnector() {
  RTC_DLOG(INFO) << "JvmThreadConnector::dtor";
  RTC_DCHECK(thread_checker_.IsCurrent());
  if (attached_) {
    RTC_DLOG(INFO) << "Detaching thread from JVM";
...
GlobalRef::GlobalRef(JNIEnv* jni, jobject object)
  ...
  RTC_DLOG(INFO) << "GlobalRef::ctor";
}

GlobalRef::~GlobalRef() {
  RTC_DLOG(INFO) << "GlobalRef::dtor";
...
NativeRegistration::NativeRegistration(JNIEnv* jni, jclass clazz)
  ...
  RTC_DLOG(INFO) << "NativeRegistration::ctor";
}

NativeRegistration::~NativeRegistration() {
  RTC_DLOG(INFO) << "NativeRegistration::dtor";
...
std::unique_ptr<GlobalRef> NativeRegistration::NewObject(const char* name,
                                                         ...
                                                         ...) {
  RTC_DLOG(INFO) << "NativeRegistration::NewObject";
  va_list args;
...
JNIEnvironment::JNIEnvironment(JNIEnv* jni) : jni_(jni) {
  RTC_DLOG(INFO) << "JNIEnvironment::ctor";
}

JNIEnvironment::~JNIEnvironment() {
  RTC_DLOG(INFO) << "JNIEnvironment::dtor";
  RTC_DCHECK(thread_checker_.IsCurrent());
}
...
std::unique_ptr<NativeRegistration> JNIEnvironment::RegisterNatives(
    ...
    int num_methods) {
  RTC_DLOG(INFO) << "JNIEnvironment::RegisterNatives: " << name;
  RTC_DCHECK(thread_checker_.IsCurrent());
...
void JVM::Uninitialize() {
  RTC_DLOG(INFO) << "JVM::Uninitialize";
  RTC_DCHECK(g_jvm);
  delete g_jvm;
  g_jvm = nullptr;
}
...
JavaClass JVM::GetClass(const char* name) {
  RTC_DLOG(INFO) << "JVM::GetClass: " << name;
  RTC_DCHECK(thread_checker_.IsCurrent());
