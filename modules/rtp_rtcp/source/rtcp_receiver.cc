  ...
  for (const rtcp::Fir::Request& fir_request : fir.requests()) {
    ...
    if (!inserted) {  // There was already an entry.
      ...
      if (fir_request.seq_nr == last_fir->sequence_number &&
        ...
        int64_t rtt;
        auto success = RTT(RemoteSSRC(), &rtt, nullptr, nullptr, nullptr);
        int64_t fir_timeout = success == -1 ? kDefaultFirTimeout : 2 * rtt;
        RTC_LOG(LS_INFO) << "now_ms " << now_ms;
        RTC_LOG(LS_INFO) << "last_fir->request_ms " << last_fir->request_ms;
        RTC_LOG(LS_INFO) << "fir_timeout " << fir_timeout;
...
void RTCPReceiver::TriggerCallbacksFromRtcpPacket(
  ...
  if (rtcp_intra_frame_observer_) {
    ...
    if ((packet_information.packet_type_flags & kRtcpPli) ||
      ...
      RTC_LOG(LS_INFO) << "Send IntraRequest " << key_frame_generated;
