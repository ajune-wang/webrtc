 ...
 *
 * Although there is no limit on the number of dimensions, WebRtcIsac_Fftradix()
 *must be called once for each dimension, but the calls may be in any order.
 *
 ...
 * example:
 * REAL data [2*NTOTAL];
 * WebRtcIsac_Fftradix ( &data[0], &data[1], NTOTAL, nPass, nSpan, 2, maxf,
 *maxp);
 *
...
static int WebRtcIsac_Fftradix(double Re[],
                               double Im[],
                               size_t nTotal,
                               size_t nPass,
                               size_t nSpan,
...
int WebRtcIsac_Fftns(unsigned int ndim,
  ...
  for (i = 0; i < ndim; i++) {
    nSpan = dims[i];
    nPass *= nSpan;
    ret = FFTRADIX(Re, Im, nTotal, nSpan, nPass, iSign, max_factors, max_perm,
                   fftstate);
 ...
 *
 * could move allocation out to WebRtcIsac_Fftn(), but leave it here so that
 * it's possible to make this a standalone function
 */
