...
void EmulatedNetworkManager::TurnOn(EmulatedEndpoint* endpoint) {
  {
    rtc::CritScope crit(&endpoints_lock_);
    auto it = std::find_if(endpoints_.begin(), endpoints_.end(),
                           [endpoint](EmulatedEndpoint* e) {
                             return e->GetId() == endpoint->GetId();
                           });
    RTC_CHECK(it != endpoints_.end())
...
void EmulatedNetworkManager::TurnOff(EmulatedEndpoint* endpoint) {
  {
    rtc::CritScope crit(&endpoints_lock_);
    auto it = std::find_if(endpoints_.begin(), endpoints_.end(),
                           [endpoint](EmulatedEndpoint* e) {
