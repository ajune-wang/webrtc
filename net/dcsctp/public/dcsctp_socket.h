...
class DcSctpSocketInterface {
 public:
  ...
  // supports stream resetting, meaning that `SupportStreamReset` must return
  // true. Calling this method on e.g. a closed association will not perform any
  // operation.
  virtual void ResetStreams(
      rtc::ArrayView<const StreamID> outgoing_streams) = 0;

  // Indicates if the peer supports resetting streams (RFC6525). Please note
  // that the connection must be establish for support to be known.
