...
class RTCStatsVerifier {
 public:
  ...
  void TestAttributeIsDefined(const RTCStatsMemberInterface& attribute) {
  ...
  void TestAttributeIsUndefined(const RTCStatsMemberInterface& attribute) {
  ...
  template <typename T>
  void TestAttributeIsPositive(const RTCStatsMemberInterface& attribute) {
    EXPECT_TRUE(attribute.is_defined())
        << stats_->type() << "." << attribute.name() << "[" << stats_->id()
        << "] was undefined.";
  ...
  template <typename T>
  void TestAttributeIsNonNegative(const RTCStatsMemberInterface& attribute) {
    EXPECT_TRUE(attribute.is_defined())
        << stats_->type() << "." << attribute.name() << "[" << stats_->id()
        << "] was undefined.";
    if (!attribute.is_defined()) {
  ...
  void TestAttributeIsIDReference(const RTCStatsMemberInterface& attribute,
                                  const char* expected_type) {
    TestAttributeIsIDReference(attribute, expected_type, false);
  }

  void TestAttributeIsOptionalIDReference(
      const RTCStatsMemberInterface& attribute,
      const char* expected_type) {
    TestAttributeIsIDReference(attribute, expected_type, true);
  }
 ...
 private:
  void TestAttributeIsIDReference(const RTCStatsMemberInterface& member,
                                  const char* expected_type,
                                  bool optional) {
    ...
    for (const Attribute& attribute : stats.Attributes()) {
      ...
      if (attribute.holds_alternative<std::string>()) {
        ...
        if (absl::EndsWith(attribute.name(), "Ids")) {
          const auto& ids = attribute.get<std::vector<std::string>>();
          for (const std::string& id : ids) {
