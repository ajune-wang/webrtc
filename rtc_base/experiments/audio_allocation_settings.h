...
class AudioAllocationSettings {
 public:
  ...
  // packets.
  bool IncludeAudioInFeedback(int transport_sequence_number_id) const;
  // Returns true if target bitrate for audio streams should be updated.
  bool UpdateAudioTargetBitrate(int transport_sequence_number_id) const;
  // Returns true if audio should be added to rate allocation when the audio
  // stream is started.
  bool IncludeAudioInAllocationOnStart(int min_bitrate_bps,
                                       int max_bitrate_bps,
                                       bool has_dscp,
                                       int transport_sequence_number_id) const;
  // Returns true if audio should be added to rate allocation when the audio
  ...
  int MaxBitrateBps(absl::optional<int> max_override_bps) const;
