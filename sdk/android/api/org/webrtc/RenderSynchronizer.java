...
public final class RenderSynchronizer {
  ...
  private final long targetFrameInterval;
  ...
  public RenderSynchronizer(float targetFrameRateFps) {
    this.targetFrameInterval = Math.round(TimeUnit.SECONDS.toNanos(1) / targetFrameRateFps);
  ...
  private void onDisplayRefreshCycleBegin(long refreshTime) {
    synchronized (lock) {
    ...
    long lastOpenDelta = refreshTime - lastOpenedTime;
    long refreshDelta = refreshTime - lastRefreshTime;
    ...
    if (Math.abs(lastOpenDelta - targetFrameInterval)
        < Math.abs(lastOpenDelta - targetFrameInterval + refreshDelta)) {
      lastOpenedTime = refreshTime;
      openRenderWindow();
