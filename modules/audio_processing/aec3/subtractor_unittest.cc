...
#if defined(NDEBUG)
  const auto num_render_channels_to_test = {1, 2, 8};
  const auto num_capture_channels_to_test = {1, 2, 4};
#else
  const auto num_render_channels_to_test = {1, 2};
  const auto num_capture_channels_to_test = {1, 2};
#endif
