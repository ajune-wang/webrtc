...
void BitrateAllocator::AddObserver(BitrateAllocatorObserver* observer,
  ...
  RTC_DCHECK(isnormal(bitrate_priority));
...
// Allocates the bitrate based on the bitrate priority of each track. This
...
// 1 had a bitrate_priority = 1.0, and track 2 has a bitrate_priority = 2.0,
...
BitrateAllocator::ObserverAllocation BitrateAllocator::NormalRateAllocation(
  ...
  // its bitrate priority above the min bitrate already allocated.
  if (bitrate > 0)
...
void BitrateAllocator::DistributeBitrateRelatively(
  ...
  struct PriorityRateObserverConfig {
    ...
    // first. If we divide each observer's capacity by its bitrate
    // priority, the "normalized" capacity will be filled first. This is
    // because the amount allocated is based upon bitrate priority. An
    // observer with twice the bitrate priority of another would need twice the
