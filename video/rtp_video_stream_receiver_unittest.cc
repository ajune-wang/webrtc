...
TEST_F(RtpVideoStreamReceiverGenericDescriptorTest,
       DropPacketsWithBothVersionsOfExtension) {
  const std::vector<uint8_t> data = {0, 1, 2, 3, 4};
  const int kPayloadType = 123;
  const int kSpatialIndex = 1;
  ...
  for (size_t i = 0; i < 2; ++i) {
    ...
    generic_descriptors[i].SetSpatialLayersBitmask(1 << kSpatialIndex);
    generic_descriptors[i].AddFrameDependencyDiff(90);
    generic_descriptors[i].AddFrameDependencyDiff(80);
    ASSERT_TRUE(SetExtensionRtpGenericFrameDescriptorExtension(
