...
const uint32_t kFreezeDetectionFrameDurationMs = 1;
const uint32_t kFreezeDetectionMovAvgWindowFrames = 30;
const uint32_t kFreezeDetectionMinIncreaseForFreezeMs = 150;
const uint32_t KFreezeDetectionMinFreezeDurationMs =
    (kFreezeDetectionMovAvgWindowFrames *
         kFreezeDetectionMinIncreaseForFreezeMs +
     (kFreezeDetectionMovAvgWindowFrames - 1) *
         kFreezeDetectionFrameDurationMs) /
    (kFreezeDetectionMovAvgWindowFrames - 1);
}  // namespace
...
class ReceiveStatisticsProxyTestWithFreezeDuration
 ...
 protected:
  const uint32_t freeze_duration_ms_ = {std::get<0>(GetParam())};
  const uint32_t expected_freeze_count_ = {std::get<1>(GetParam())};
};

INSTANTIATE_TEST_CASE_P(
    _,
    ReceiveStatisticsProxyTestWithFreezeDuration,
    ::testing::Values(std::make_pair(KFreezeDetectionMinFreezeDurationMs - 1,
                                     0),
                      std::make_pair(KFreezeDetectionMinFreezeDurationMs, 1)));

TEST_P(ReceiveStatisticsProxyTestWithFreezeDuration, TestFreezeDetection) {
  VideoReceiveStream::Stats stats = statistics_proxy_->GetStats();
  EXPECT_EQ(0u, stats.freeze_count);
  webrtc::VideoFrame frame = CreateFrame(kWidth, kHeight);
  ...
  for (uint32_t i = 0; i <= kFreezeDetectionMovAvgWindowFrames; ++i) {
    fake_clock_.AdvanceTimeMilliseconds(kFreezeDetectionFrameDurationMs);
    statistics_proxy_->OnRenderedFrame(frame);
  }
