...
TEST(Normal, AvoidDivideByZero) {
  ...
  // Zero input length.
  EXPECT_EQ(0, normal.Process(input, 0, Modes::kModeExpand, &output));
...
TEST(Normal, InputLengthAndChannelsDoNotMatch) {
  ...
  EXPECT_EQ(0, normal.Process(input, input_len, Modes::kModeExpand, &output));
  EXPECT_EQ(0u, output.Size());
...
TEST(Normal, LastModeExpand120msPacket) {
  ...
  EXPECT_EQ(
      ...
      normal.Process(input, kPacketsizeBytes, Modes::kModeExpand, &output));
