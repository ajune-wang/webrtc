...
bool FakeVideoCapturerWithTaskQueue::CaptureSingleFrame() {
  bool ret = false;
  RunSynchronouslyOnTaskQueue([this, &ret]() { ret = CaptureFrame(); });
  return ret;
}

bool FakeVideoCapturerWithTaskQueue::CaptureSingleCustomFrame(int width,
                                                              int height,
                                                              uint32_t fourcc) {
  bool ret = false;
  RunSynchronouslyOnTaskQueue([this, &ret, width, height, fourcc]() {
    ret = CaptureCustomFrame(width, height, fourcc);
  });
  return ret;
}

bool FakeVideoCapturerWithTaskQueue::CaptureSingleCustomFrame(
    int width,
    int height,
    int64_t timestamp_interval,
    uint32_t fourcc) {
  ...
  RunSynchronouslyOnTaskQueue(
      [this, &ret, width, height, timestamp_interval, fourcc]() {
        ret = CaptureCustomFrame(width, height, timestamp_interval, fourcc);
      });
  return ret;
}
