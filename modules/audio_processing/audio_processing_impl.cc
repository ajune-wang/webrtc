...
int AudioProcessingImpl::ProcessCaptureStreamLocked() {
  ...
  if (capture_.capture_output_used) {
    ...
    // TODO(tommi): When we get here, `capture_buffer` might have been changed
    // to point to something else than it did at the top of the function and
    // `capture_nonlocked_.capture_processing_format.num_frames()` may no
    // longer match with the size of the capture buffer.
    //
    // Therefore, we can't just do this and expect that `num_frames()` is still
    ...
    RTC_DCHECK_EQ(capture_nonlocked_.capture_processing_format.num_frames(),
                  SamplesPerChannel(ch));
    // TODO(tommi): This is probably incorrect. `ch` may point to a different
    // buffer.
