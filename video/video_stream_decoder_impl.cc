...
VideoStreamDecoderImpl::VideoStreamDecoderImpl(
  ...
  bookkeeping_queue_.PostTask([this]() { StartNextDecode(); });
}
...
VideoStreamDecoderImpl::~VideoStreamDecoderImpl() {
  ...
  {
    rtc::Event event;
    decode_queue_.PostTask([this, &event]() {
  ...
  {
    rtc::Event event;
    bookkeeping_queue_.PostTask([this, &event]() {
...
void VideoStreamDecoderImpl::StartNextDecode() {
  // TODO: Check if field trial is used to ajust these values.
  ...
  switch (result) {
    case video_coding::FrameBuffer::kFrameFound: {
      ...
      decode_queue_.PostTask([this, frame = std::move(frame)]() mutable {
        ...
        switch (DecodeFrame(std::move(frame))) {
          case kOk: {
    ...
    case video_coding::FrameBuffer::kTimeout: {
      RequestKeyframe();
      // We can't call StartNextDecode until this task has completed. Instead
      // call it from a later task.
      bookkeeping_queue_.PostTask([this]() { StartNextDecode(); });
      break;
    }
...
void VideoStreamDecoderImpl::RequestKeyframe() {
  if (!bookkeeping_queue_.IsCurrent()) {
    bookkeeping_queue_.PostTask([this] { RequestKeyframe(); });
    return;
  }
