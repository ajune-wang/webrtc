...
class PeerConnectionWrapper : public webrtc::PeerConnectionObserver,
 ...
 private:
  ...
  void OnIceCandidateError(const webrtc::IceCandidateInterface* candidate,
                           int error_code,
                           const std::string& error_text) override {
    error_event_ = cricket::IceCandidateErrorEvent(
        candidate->candidate().address().ToSensitiveString(),
        candidate->candidate().url(), error_code, error_text);
  }
  void OnDataChannel(
