...
void RTCStatsCollector::ProduceAudioRTPStreamStats_n(
  ...
  for (const cricket::VoiceReceiverInfo& voice_receiver_info :
    ...
    auto* inbound_audio_ptr = inbound_audio.get();
    if (!report->TryAddStats(std::move(inbound_audio))) {
      // When failing to add, `inbound_audio_ptr` becomes a dangling pointer.
      RTC_LOG(LS_ERROR)
    ...
    if (remote_outbound_audio) {
      ...
      auto* remote_outbound_audio_ptr = remote_outbound_audio.get();
      if (report->TryAddStats(std::move(remote_outbound_audio))) {
        inbound_audio_ptr->remote_id = remote_outbound_audio_ptr->id();
      } else {
        // When failing to add, `remote_outbound_audio_ptr` becomes a dangling
        // pointer.
  ...
  for (const cricket::VoiceSenderInfo& voice_sender_info :
    ...
    auto* outbound_audio_ptr = outbound_audio.get();
    if (report->TryAddStats(std::move(outbound_audio))) {
      audio_outbound_rtps.insert(
          std::make_pair(outbound_audio_ptr->id(), outbound_audio_ptr));
    } else {
      // When failing to add, `outbound_audio_ptr` becomes a dangling pointer.
      RTC_LOG(LS_ERROR)
...
void RTCStatsCollector::ProduceVideoRTPStreamStats_n(
  ...
  for (const cricket::VideoSenderInfo& video_sender_info :
    ...
    auto* outbound_video_ptr = outbound_video.get();
    if (report->TryAddStats(std::move(outbound_video))) {
      video_outbound_rtps.insert(
          std::make_pair(outbound_video_ptr->id(), outbound_video_ptr));
    } else {
      // When failing to add, `outbound_video_ptr` becomes a dangling pointer.
      RTC_LOG(LS_ERROR)
