...
class SendSideCongestionController
 ...
 private:
  void MaybeCreateControllers() RTC_RUN_ON(task_queue_raw_);
  void StartProcessPeriodicTasks() RTC_RUN_ON(task_queue_raw_);
  void UpdateControllerWithTimeInterval() RTC_RUN_ON(task_queue_raw_);
  void UpdatePacerQueue() RTC_RUN_ON(task_queue_raw_);

  void UpdateStreamsConfig() RTC_RUN_ON(task_queue_raw_);
  void MaybeUpdateOutstandingData();
  void OnReceivedRtcpReceiverReportBlocks(const ReportBlockList& report_blocks,
                                          int64_t now_ms)
      RTC_RUN_ON(task_queue_raw_);
  ...
  const std::unique_ptr<PacerController> pacer_controller_
      RTC_GUARDED_BY(task_queue_raw_);
  ...
  TimeDelta process_interval_ RTC_GUARDED_BY(task_queue_raw_);

  std::map<uint32_t, RTCPReportBlock> last_report_blocks_
      RTC_GUARDED_BY(task_queue_raw_);
  Timestamp last_report_block_time_ RTC_GUARDED_BY(task_queue_raw_);

  NetworkChangedObserver* observer_ RTC_GUARDED_BY(task_queue_raw_);
  NetworkControllerConfig initial_config_ RTC_GUARDED_BY(task_queue_raw_);
  StreamsConfig streams_config_ RTC_GUARDED_BY(task_queue_raw_);
  ...
  bool network_available_ RTC_GUARDED_BY(task_queue_raw_);
