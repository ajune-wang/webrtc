...
static void JNI_VideoFrame_CropAndScaleI420A(
    JNIEnv* jni,
    const JavaParamRef<jclass>&,
    const JavaParamRef<jobject>& j_src_y,
    jint src_stride_y,
    ...
    const JavaParamRef<jobject>& j_src_v,
    jint src_stride_v,
    const JavaParamRef<jobject>& j_src_a,
    jint src_stride_a,
    jint crop_x,
    jint crop_y,
    jint crop_width,
    ...
    const JavaParamRef<jobject>& j_dst_v,
    jint dst_stride_v,
    const JavaParamRef<jobject>& j_dst_a,
    jint dst_stride_a,
    jint scale_width,
    jint scale_height) {
  uint8_t const* src_y =
  ...
  uint8_t const* src_v =
      static_cast<uint8_t*>(jni->GetDirectBufferAddress(j_src_v.obj()));
  uint8_t const* src_a =
      static_cast<uint8_t*>(jni->GetDirectBufferAddress(j_src_a.obj()));
  uint8_t* dst_y =
      static_cast<uint8_t*>(jni->GetDirectBufferAddress(j_dst_y.obj()));
  uint8_t* dst_u =
      static_cast<uint8_t*>(jni->GetDirectBufferAddress(j_dst_u.obj()));
  uint8_t* dst_v =
      static_cast<uint8_t*>(jni->GetDirectBufferAddress(j_dst_v.obj()));
  uint8_t* dst_a =
      static_cast<uint8_t*>(jni->GetDirectBufferAddress(j_dst_a.obj()));
