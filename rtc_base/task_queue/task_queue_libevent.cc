...
#include "rtc_base/task_queue/task_queue_libevent.h"
...
void EventAssign(struct event* ev,
                 ...
                 int fd,
                 short events,                         // NOLINT
                 void (*callback)(int, short, void*),  // NOLINT
                 void* arg) {
...
class TaskQueueLibevent : public TaskQueueBase {
 public:
  explicit TaskQueueLibevent(absl::string_view queue_name,
...
class LibeventTaskQueueFactory : public TaskQueueFactory {
 public:
...
std::unique_ptr<TaskQueueFactory> CreateLibeventTaskQueueFactory() {
  return absl::make_unique<LibeventTaskQueueFactory>();
}
