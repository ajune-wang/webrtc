...
void AgcManagerDirect::AnalyzePreProcess(float* audio_flt,
                                         int num_channels,
  ...
  if (audio_flt) {
    ...
    FloatS16ToS16(audio_flt, length, audio);
  } else {
    audio = nullptr;
    safe_length = length;
  }

  file_preproc_->Write(audio, safe_length);
  ...
  float clipped_ratio = agc_->AnalyzePreproc(audio, safe_length);
  ...
  if (audio_flt) {
    S16ToFloatS16(audio, safe_length, audio_flt);
  }
}

void AgcManagerDirect::Process(const float* audio_flt,
                               size_t length,
                               int sample_rate_hz) {
  if (capture_muted_) {
    return;
  ...
  if (audio_flt) {
    audio = audio_data.data();
    safe_length = std::min(audio_data.size(), length);
    FloatS16ToS16(audio_flt, length, audio_data.data());
  } else {
    audio = nullptr;
    safe_length = length;
  }
  ...
  agc_->Process(audio, safe_length, sample_rate_hz);
  ...
  file_postproc_->Write(audio, safe_length);
