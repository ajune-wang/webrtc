...
bool RtpDataMediaChannel::SetRecvCodecs(const std::vector<DataCodec>& codecs) {
  ...
  if (unknown_codec) {
    NLOG(LS_WARNING, "Failed to SetRecvCodecs because of unknown codec: ",
...
bool RtpDataMediaChannel::AddSendStream(const StreamParams& stream) {
  ...
  if (GetStreamBySsrc(send_streams_, stream.first_ssrc())) {
    NLOG(LS_WARNING, "Not adding data send stream '", stream.id,
  ...
  NLOG(LS_INFO, "Added data send stream '", stream.id,
       "' with ssrc=", stream.first_ssrc());
  return true;
}
...
bool RtpDataMediaChannel::AddRecvStream(const StreamParams& stream) {
  ...
  if (GetStreamBySsrc(recv_streams_, stream.first_ssrc())) {
    NLOG(LS_WARNING, "Not adding data recv stream '", stream.id,
         "' with ssrc=", stream.first_ssrc(),
         " because stream already exists.");
    return false;
  }
  ...
  NLOG(LS_INFO, "Added data recv stream '", stream.id,
       "' with ssrc=", stream.first_ssrc());
  return true;
}
...
void RtpDataMediaChannel::OnPacketReceived(
  ...
  if (!receiving_) {
    NLOG(LS_WARNING, "Not receiving packet ", header.ssrc, ":", header.seq_num,
...
bool RtpDataMediaChannel::SetMaxSendBandwidth(int bps) {
  ...
  NLOG(LS_INFO, "RtpDataMediaChannel::SetSendBandwidth to ", bps, "bps.");
  return true;
}
...
bool RtpDataMediaChannel::SendData(
  ...
  if (!sending_) {
    NLOG(LS_WARNING, "Not sending packet with ssrc=", params.ssrc,
         " len=", payload.size(), " before SetSend(true).");
    return false;
  }

  if (params.type != cricket::DMT_TEXT) {
    NLOG(LS_WARNING, "Not sending data because binary type is unsupported.");
    return false;
  }
  ...
  if (!found_stream) {
    NLOG(LS_WARNING, "Not sending data because ssrc is unknown: ", params.ssrc);
    return false;
  }
  ...
  if (!found_codec) {
    NLOG(LS_WARNING, "Not sending data because codec is unknown: ",
  ...
  NLOG(LS_VERBOSE, "Sent RTP data packet: ", " stream=", found_stream->id,
       " ssrc=", header.ssrc, ", seqnum=", header.seq_num,
       ", timestamp=", header.timestamp, ", len=", payload.size());
