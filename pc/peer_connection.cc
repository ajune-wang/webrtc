...
PeerConnection::~PeerConnection() {
  ...
  if (!IsClosed()) {
    // TODO(tommi): In practice, this is a workaround for tests that
    // should be destroyed there.
    RTC_LOG(LS_ERROR) << "*******  NOT CLOSED ************";
    network_thread()->Invoke<void>(RTC_FROM_HERE, [this] {
      RTC_DCHECK_RUN_ON(network_thread());
      if (network_thread_safety_) {
        RTC_LOG(LS_ERROR) << "*******  NOT ALIVE ************";
...
void PeerConnection::ReportTransportStats() {
  ...
  network_thread()->PostTask(ToQueuedTask(network_thread_safety_, [this]() {
    RTC_DCHECK_RUN_ON(network_thread());
    RTC_LOG(LS_ERROR) << "*******  ReportTransportStats ************";
    rtc::Thread::ScopedDisallowBlockingCalls no_blocking_calls;
    std::map<std::string, std::set<cricket::MediaType>>*
        media_types_by_transport_name =
            new std::map<std::string, std::set<cricket::MediaType>>();
    for (const auto& transceiver : rtp_manager()->transceivers()->List()) {
    ...
    network_thread()->PostTask(ToQueuedTask(
        network_thread_safety_, [this, hack = media_types_by_transport_name]() {
          RTC_DCHECK_RUN_ON(network_thread());
          RTC_LOG(LS_ERROR) << "*******  rtp_data_channel() ************";
          if (rtp_data_channel()) {
            (*hack)[rtp_data_channel()->transport_name()].insert(
                cricket::MEDIA_TYPE_DATA);
          }
          network_thread()->PostTask(
              ToQueuedTask(network_thread_safety_, [this, hack]() {
                RTC_DCHECK_RUN_ON(network_thread());
                if (sctp_mid_n_) {
                  auto dtls_transport =
                      transport_controller_->GetDtlsTransport(*sctp_mid_n_);
                  if (dtls_transport) {
                    (*hack)[dtls_transport->transport_name()].insert(
                        cricket::MEDIA_TYPE_DATA);
                  }
                }
                network_thread()->PostTask(
                    ToQueuedTask(network_thread_safety_, [this, hack]() {
                      ...
                      for (const auto& entry : *hack) {
                        ...
                        if (transport_controller_->GetStats(transport_name,
                          ...
                          ReportNegotiatedCiphers(dtls_enabled_, stats,
                                                  media_types);
                        }
                      }
                      delete hack;
