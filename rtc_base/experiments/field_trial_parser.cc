...
void ParseFieldTrial(
  ...
  while (i < trial_string.length()) {
    ...
    if (field != field_map.end()) {
      if (!field->second->Parse(opt_value)) {
        RTC_LOG(LS_WARNING) << "Failed to read field with key: '" << key
                            << "' from \"" << opt_value.value_or("")
...
FieldTrialFlag::FieldTrialFlag(std::string key)
    : FieldTrialParameterInterface(key), value_(false) {}

bool FieldTrialFlag::Get() const {
  return value_;
}

bool FieldTrialFlag::Parse(const rtc::Optional<std::string>& str_value) {
...
template class FieldTrialParameter<rtc::Optional<std::string>>;
