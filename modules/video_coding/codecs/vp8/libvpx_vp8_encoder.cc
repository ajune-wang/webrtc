...
int LibvpxVp8Encoder::InitEncode(const VideoCodec* inst,
  ...
  filter_settings_generator_ = std::make_unique<FilterSettingsGenerator>(
      ...
      // On large changes, increase error threshold by one and std_dev by 2.0.
      // Trigger on qp changes larger than 30, and fade down the adjusted value
      // over 4 * num_temporal_layers to allow the base layer to converge
      // somewhat. Set a minim filter size of 1.25 since some outlier pixels
      // deviate a bit from truth even at very low QP, seeminly by bleeding into
      // neighbours.
      webrtc::FilterSettingsGenerator::TransientParameters{
          ...
          .keyframe_offset_duration_frames =
              std::max(1, SimulcastUtility::NumberOfTemporalLayers(*inst, 0)) *
              4,
    ...
    if (send_stream_[stream_idx]) {
      if (encoded_images_[encoder_idx].size() > 0) {
        ...
        encoded_images_[encoder_idx].set_corruption_detection_filter_settings(
            filter_settings_generator_->OnFrame(
