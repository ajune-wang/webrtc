...
#include "modules/congestion_controller/gocc/gocc_network_control.h"
...
GoccNetworkControllerFactory::GoccNetworkControllerFactory(
    const Clock* clock,
    RtcEventLog* event_log)
    : clock_(clock), event_log_(event_log) {}

NetworkControllerInterface::uptr GoccNetworkControllerFactory::Create(
    NetworkControlObserver* observers) {
  return rtc::MakeUnique<GoccNetworkController>(clock_, event_log_, observers);
}

GoccNetworkController::GoccNetworkController(const Clock* clock,
                                             RtcEventLog* event_log,
                                             NetworkControlObserver* observers)
    : clock_(clock),
      event_log_(event_log),
      observers_(observers),
      probe_controller_(new ProbeController(clock_, observers_)),
  ...
  using This = GoccNetworkController;
...
GoccNetworkController::~GoccNetworkController() {}

void GoccNetworkController::ConnectHandlers(
    NetworkInformationProducer* producer) {
  NetworkAvailabilityHandler.ReceiveFrom(producer);
  NetworkRouteChangeHandler.ReceiveFrom(producer);
  ProcessIntervalHandler.ReceiveFrom(producer);
  RemoteBitrateReportHandler.ReceiveFrom(producer);
  RoundTripTimeReportHandler.ReceiveFrom(producer);
  SentPacketHandler.ReceiveFrom(producer);
  StreamsConfigHandler.ReceiveFrom(producer);
  TargetRateConstraintsHandler.ReceiveFrom(producer);
  TransportLossReportHandler.ReceiveFrom(producer);
  TransportPacketsFeedbackHandler.ReceiveFrom(producer);
}

void GoccNetworkController::OnTargetRateConstraints(
...
void GoccNetworkController::OnNetworkRouteChange(NetworkRouteChange msg) {
...
void GoccNetworkController::OnStreamsConfig(StreamsConfig msg) {
  bool pacing_changed =
...
void GoccNetworkController::OnNetworkAvailability(NetworkAvailability msg) {
  NetworkState state = msg.network_available ? kNetworkUp : kNetworkDown;
  probe_controller_->OnNetworkStateChanged(state);
}

void GoccNetworkController::OnSentPacket(SentPacket sent_packet) {
  alr_detector_->OnBytesSent(sent_packet.size.bytes(),
                             sent_packet.send_time.ms());
}

units::TimeDelta GoccNetworkController::GetProcessInterval() {
  const int64_t kUpdateIntervalMs = 25;
  return TimeDelta::ms(kUpdateIntervalMs);
}

void GoccNetworkController::OnProcessInterval(ProcessInterval msg) {
...
void GoccNetworkController::OnTransportPacketsFeedback(
...
void GoccNetworkController::MaybeUpdateCongestionWindow() {
  if (!in_cwnd_experiment_)
    return;
  ...
  CongestionWindow::SendMessage(msg, observers_);
  RTC_LOG(LS_INFO) << "Feedback rtt: " << *min_feedback_rtt_ms_
                   << " Bitrate: " << last_estimate_->bandwidth.bps();
}

void GoccNetworkController::MaybeTriggerOnNetworkChanged() {
...
void GoccNetworkController::OnNetworkEstimate(NetworkEstimate estimate) {
  if (!estimate.changed)
    return;
  ...
  TargetTransferRate::SendMessage(target_rate, observers_);
}

bool GoccNetworkController::GetNetworkParameters(int32_t* estimated_bitrate_bps,
                                                 uint8_t* fraction_loss,
                                                 int64_t* rtt_ms) {
...
void GoccNetworkController::UpdatePacingRates() {
  if (!last_estimate_)
    return;
  ...
  PacerConfig::SendMessage(msg, observers_);
}

void GoccNetworkController::OnRemoteBitrateReport(RemoteBitrateReport msg) {
...
void GoccNetworkController::OnRoundTripTimeReport(RoundTripTimeReport report) {
...
void GoccNetworkController::OnTransportLossReport(TransportLossReport msg) {
