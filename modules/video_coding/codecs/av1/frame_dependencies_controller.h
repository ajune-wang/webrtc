...
class FrameDependenciesController {
 public:
  ...
  virtual std::vector<GenericFrameInfo> NextFrameConfig(
      uint32_t rtp_timestamp) = 0;
  // Updates frame dependencies after frame is encoded, if necessary.
  // Expects previous call to NextFrameConfig with the same |rtp_timestamp|.
  // Returns false on error.
  virtual bool OnEncodeDone(uint32_t rtp_timestamp,
                            bool is_keyframe,
