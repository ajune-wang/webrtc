...
class TestMultiplexWithDataAdapter : public VideoCodecUnitTest {
 ...
 protected:
  ...
  std::unique_ptr<VideoFrame> CreateDataAugmentedMultiplexInputFrame(
      rtc::scoped_refptr<VideoFrameBuffer> yuv_buffer) {
    rtc::scoped_refptr<I420ABufferInterface> yuva_buffer =
        CreateI420AFrameBuffer();
    uint8_t* data = new uint8_t[16];
    for (int i = 0; i < 16; i++) {
      data[i] = i;
    }
    rtc::scoped_refptr<MultiplexVideoFrameBuffer> multiplex_video_buffer =
        new rtc::RefCountedObject<MultiplexVideoFrameBuffer>(yuv_buffer, data,
                                                             16);
  ...
  std::unique_ptr<VideoFrame> ExtractAXXFrame(
      rtc::scoped_refptr<VideoFrameBuffer> video_buffer) {
    MultiplexVideoFrameBuffer* multiplexBuffer =
        static_cast<MultiplexVideoFrameBuffer*>(video_buffer.get());
...
TEST_F(TestMultiplexWithDataAdapter, EncodeDecodeDataAugmentedAlphaFrame) {
  rtc::scoped_refptr<I420ABufferInterface> yuva_buffer =
      CreateI420AFrameBuffer();
  std::unique_ptr<VideoFrame> data_augmented_frame =
      CreateDataAugmentedMultiplexInputFrame(yuva_buffer);
  ...
  MultiplexVideoFrameBuffer* multiplex_buffer =
      static_cast<MultiplexVideoFrameBuffer*>(
...
TEST_F(TestMultiplexWithDataAdapter, EncodeDecodeDataAugmentedFrame) {
  ...
  std::unique_ptr<VideoFrame> data_augmented_frame =
      CreateDataAugmentedMultiplexInputFrame(yuv_buffer);
  EXPECT_EQ(WEBRTC_VIDEO_CODEC_OK,
            encoder_->Encode(*data_augmented_frame, nullptr, nullptr));
  EncodedImage encoded_frame;
  CodecSpecificInfo codec_specific_info;
