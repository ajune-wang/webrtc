...
NetworkNode::NetworkNode(Clock* clock,
    ...
    : clock_(clock), simulation_(std::move(simulation)) {}
...
bool NetworkNode::TryDeliverPacket(rtc::CopyOnWriteBuffer packet,
  ...
  bool sent = simulation_->EnqueuePacket(PacketInFlightInfo(
      packet.size(), clock_->TimeInMicroseconds(), packet_id));
  if (sent) {
...
NetworkNodeTransport::NetworkNodeTransport(CallClient* sender,
                                           ...
                                           uint64_t receiver)
    : sender_(sender), send_net_(send_net), receiver_id_(receiver) {}
...
bool NetworkNodeTransport::SendRtp(const uint8_t* packet,
  ...
  return send_net_->TryDeliverPacket(rtc::CopyOnWriteBuffer(packet, length),
                                     receiver_id_);
}
...
LambdaPrinter CrossTrafficSource::StatsPrinter() {
  return {"cross_traffic_rate",
