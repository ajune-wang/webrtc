...
DefaultEncodedImageIdInjector::DefaultEncodedImageIdInjector() {
  for (size_t i = 0; i < 2 * kBuffersPoolPerCodingEntity; i++) {
...
EncodedImage DefaultEncodedImageIdInjector::InjectId(uint16_t id,
                                                     ...
                                                     CodingType type) {
  ExtendIfRequired(coding_entity_id, type);
  ...
  out._buffer[2] = source.size() & 0x00ff;
  out._buffer[3] = (source.size() & 0xff00) >> 8;
  return out;
}
...
void DefaultEncodedImageIdInjector::ExtendIfRequired(int coding_entity_id,
  ...
  if (coding_entities_.find({coding_entity_id, type}) !=
      coding_entities_.end()) {
  ...
  coding_entities_.insert({coding_entity_id, type});
  if (coding_entities_.size() <= 2) {
    // Buffers for the first 2 coding entities were allocated during
    // construction.
    return;
  }
