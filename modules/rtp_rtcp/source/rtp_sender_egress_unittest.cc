...
TEST_P(RtpSenderEgressTest, UpdatesDataCounters) {
  ...
  const RtpPacketCounter kEmptyCounter;
  RtpPacketCounter expected_media_counter;
  RtpPacketCounter expected_rtx_counter;

  // Send a media packet.
  std::unique_ptr<RtpPacketToSend> media_packet = BuildRtpPacket();
  media_packet->SetPayloadSize(6);
  expected_media_counter.packets += 1;
  expected_media_counter.payload_bytes += media_packet->payload_size();
  expected_media_counter.header_bytes += media_packet->headers_size();
  sender->SendPacket(media_packet.get(), PacedPacketInfo());
  time_controller_.AdvanceTime(TimeDelta::Zero());
  ...
  expected_rtx_counter.packets += 1;
  expected_rtx_counter.payload_bytes += rtx_packet->payload_size();
  expected_rtx_counter.header_bytes += rtx_packet->headers_size();
  ...
  EXPECT_EQ(rtp_stats.transmitted, expected_media_counter);
  ...
  // packet counts.
  EXPECT_EQ(rtx_stats.transmitted, expected_rtx_counter);
  EXPECT_EQ(rtx_stats.retransmitted, expected_rtx_counter);
