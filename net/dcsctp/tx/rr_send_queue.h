 ...
 private:
  ...
  class OutgoingStream : public StreamScheduler::StreamCallback {
   public:
    ...
    absl::optional<StreamScheduler::ProducedData> Produce(
        TimeMs now,
   ...
   private:
    ...
    struct Item {
      ...
      // The remaining payload (offset and size) to be sent, when it has
      // been fragmented.
      ...
      // The current Fragment Sequence Number, incremented for each
      // fragment.
    ...
    // Reference to the total buffered amount, which is updated directly by
    // each stream.
