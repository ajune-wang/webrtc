...
class RTC_EXPORT EncodedImage {
 public:
  ...
  absl::optional<int> SimulcastIndex() const {
    ...
    // that still does SetSpatialIndex() instead of SetSimulcastIndex() we fall
    // back to `spatial_index_` if `simulcast_index_` is not set.
  ...
  absl::optional<int> SpatialIndex() const {
    ...
    return spatial_index_.has_value() ? spatial_index_.value()
                                      : simulcast_index_;
  }
