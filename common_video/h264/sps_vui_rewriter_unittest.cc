  ...
  if (vui.vui_parameters_present_flag) {
    ...
    if (vui.video_signal_type_present_flag) {
      ...
      if (vui.colour_description_present_flag) {
        writer.WriteBits(vui.colour_primaries, 8);
        writer.WriteBits(vui.transfer_characteristics, 8);
        writer.WriteBits(vui.matrix_coefficients, 8);
      }
    }
...
TEST(SpsVuiRewriterOutgoingVuiTest, ParseOutgoingBitstreamOptimalVui) {
  ...
  SpsVuiRewriter::ParseOutgoingBitstreamAndRewriteSps(
      ...
      modified_nalu_offsets, modified_nalu_lengths, nullptr);
