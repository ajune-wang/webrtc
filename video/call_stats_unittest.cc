...
class CallStatsTest : public ::testing::Test {
 public:
  CallStatsTest() {
    // TODO(tommi): Make CallStats do this internally.
    //process_thread_->RegisterModule();
  }
  ~CallStatsTest() override { process_thread_->Stop(); }

  // TODO(tommi): The test is "cheating" since we're pretending to be a
  // ProcessThread and calling Process() from the wrong thread.
  // We need to change the test to reflect reality.
  void TriggerProcess() {
    Module* module = &call_stats_;
    module->Process();
  }
