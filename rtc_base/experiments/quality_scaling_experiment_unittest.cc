...
TEST(QualityScalingExperimentTest, ParseSettings) {
  QualityScalingExperiment::Settings kExpected = {1, 2, 3,   4,    5, 6,
                                                  7, 8, 0.9, 0.99, 1};
  webrtc::test::ScopedFieldTrials field_trials(
      "WebRTC-Video-QualityScaling/Enabled-1,2,3,4,5,6,7,8,0.9,0.99,1/");
  auto settings = QualityScalingExperiment::ParseSettings();
...
TEST(QualityScalingExperimentTest, GetConfig) {
  ...
  auto config = QualityScalingExperiment::GetConfig();
...
TEST(QualityScalingExperimentTest, GetsDefaultConfigForInvalidValues) {
...
TEST(QualityScalingExperimentTest, GetVp8Thresholds) {
  ...
  auto thresholds = QualityScalingExperiment::GetQpThresholds(kVideoCodecVP8);
...
TEST(QualityScalingExperimentTest, GetVp9Thresholds) {
  webrtc::test::ScopedFieldTrials field_trials(
      "WebRTC-Video-QualityScaling/Enabled-1,2,3,4,5,6,0,0,0.9,0.99,1/");
  auto thresholds = QualityScalingExperiment::GetQpThresholds(kVideoCodecVP9);
  EXPECT_TRUE(thresholds);
...
TEST(QualityScalingExperimentTest, GetH264Thresholds) {
  webrtc::test::ScopedFieldTrials field_trials(
      "WebRTC-Video-QualityScaling/Enabled-1,2,3,4,5,6,0,0,0.9,0.99,1/");
  auto thresholds = QualityScalingExperiment::GetQpThresholds(kVideoCodecH264);
  EXPECT_TRUE(thresholds);
