...
TEST(ApmStatistics, AEC2EnabledTest) {
  ...
  ProcessingConfig processing_config = {
      {{48000, 1}, {48000, 1}, {48000, 1}, {48000, 1}}};

  ASSERT_EQ(apm->Initialize(processing_config), 0);

  // Disable all components except for AEC2 and the residual echo detector.
  AudioProcessing::Config config;
  ...
  ASSERT_EQ(apm->echo_cancellation()->Enable(true), 0);
  ...
  EXPECT_TRUE(stats.echo_return_loss);
...
TEST(ApmStatistics, AECMEnabledTest) {
  ...
  // Disable all components except for AECM and the residual echo detector.
  AudioProcessing::Config config;
  config.residual_echo_detector.enabled = true;
  config.echo_canceller3.enabled = false;
  config.high_pass_filter.enabled = false;
  ...
  ASSERT_EQ(apm->echo_control_mobile()->Enable(true), 0);
