...
struct LoggedRtpPacket {
  LoggedRtpPacket(uint64_t timestamp_us, RTPHeader header, size_t total_length)
...
struct LoggedRtpPacketIncoming : public LoggedRtpPacket {
  LoggedRtpPacketIncoming(uint64_t timestamp_us,
                          RTPHeader header,
                          size_t total_length)
      : LoggedRtpPacket(timestamp_us, header, total_length) {}
};
...
struct LoggedRtcpPacketIncoming : public LoggedRtcpPacket {
  LoggedRtcpPacketIncoming(uint64_t timestamp_us,
                           const uint8_t* packet,
                           size_t total_length)
      : LoggedRtcpPacket(timestamp_us, packet, total_length) {}
};

struct LoggedRtcpPacketOutgoing : public LoggedRtcpPacket {
...
class PacketIterator {
 public:
  PacketIterator() = delete;
  PacketIterator(typename std::conditional<std::is_const<T>::value,
                                           const void*,
                                           void*>::type p,
                 size_t s)
      : ptr(reinterpret_cast<decltype(ptr)>(p)), element_size(s) {}
  ...
  bool operator==(const PacketIterator<T>& other) const {
    RTC_DCHECK_EQ(element_size, other.element_size);
  ...
  PacketIterator operator++(int) {
    PacketIterator iter_copy(ptr, element_size);
    ptr += element_size;
    return iter_copy;
  }
  PacketIterator operator--(int) {
...
class PacketView {
 public:
  PacketView(typename std::conditional<std::is_const<T>::value,
             ...
             size_t size,
             size_t element_size)
      : data_(data), size_(size), element_size_(element_size) {}
  using iterator = PacketIterator<T>;
  using const_iterator = PacketIterator<const T>;

  // iterator begin() {
  //   return iterator(data_, element_size_);
  // }
  // iterator end() {
  //   return iterator(data_ + size_, element_size_);
  // }
  const_iterator begin() const { return const_iterator(data_, element_size_); }
  const_iterator end() const {
    const void* end_ptr =
        static_cast<const char*>(data_) + size_ * element_size_;
  ...
  T& operator[](size_t i) {
    void* elem_ptr = static_cast<char*>(data_) + i * element_size_;
    return *static_cast<T*>(elem_ptr);
  }

  const T& operator[](size_t i) const {
    const void* elem_ptr = static_cast<const char*>(data_) + i * element_size_;
    return *static_cast<const T*>(elem_ptr);
  }
...
using RtpPacketView = PacketView<const LoggedRtpPacket>;
using RtcpPacketView = PacketView<const LoggedRtcpPacket>;
...
class ParsedRtcEventLog {
 ...
 public:
  ...
  const std::map<uint32_t, std::vector<LoggedRtpPacketIncoming>>&
  incoming_rtp_packets() const {
    return incoming_rtp_packets_;
  }

  const std::map<uint32_t, std::vector<LoggedRtpPacketOutgoing>>&
  outgoing_rtp_packets() const {
    return outgoing_rtp_packets_;
  }

  const std::map<uint32_t, const RtpPacketView>& rtp_packets(
      PacketDirection direction) const {
    if (direction == kIncomingPacket)
      return incoming_rtp_packet_views_;
    else
      return outgoing_rtp_packet_views_;
  }

  const RtcpPacketView rtcp_packets(PacketDirection direction) const {
    if (direction == kIncomingPacket) {
      return RtcpPacketView(incoming_rtcp_packets_.data(),
                            incoming_rtcp_packets_.size(),
                            sizeof(LoggedRtcpPacketIncoming));
    } else {
      return RtcpPacketView(outgoing_rtcp_packets_.data(),
                            outgoing_rtcp_packets_.size(),
                            sizeof(LoggedRtcpPacketOutgoing));
    }
  }

  const std::vector<LoggedRtcpPacketReceiverReport>& incoming_receiver_reports()
      const {
    return incoming_rr_;
  }

  const std::vector<LoggedRtcpPacketReceiverReport>& outgoing_receiver_reports()
      const {
    return outgoing_rr_;
  }

  const std::vector<LoggedRtcpPacketSenderReport>& incoming_sender_reports()
      const {
    return incoming_sr_;
  }

  const std::vector<LoggedRtcpPacketSenderReport>& outgoing_sender_reports()
      const {
    return outgoing_sr_;
  }

  const std::vector<LoggedRtcpPacketNack>& incoming_nacks() const {
    return incoming_nack_;
  }

  const std::vector<LoggedRtcpPacketNack>& outgoing_nacks() const {
    return outgoing_nack_;
  }

  const std::vector<LoggedRtcpPacketRemb>& incoming_rembs() const {
    return incoming_remb_;
  }

  const std::vector<LoggedRtcpPacketRemb>& outgoing_rembs() const {
    return outgoing_remb_;
  }

  const std::vector<LoggedRtcpPacketTransportFeedback>&
  incoming_transport_feedbacks() const {
    return incoming_transport_feedback_;
  }

  const std::vector<LoggedRtcpPacketTransportFeedback>&
  outgoing_transport_feedbacks() const {
 ...
 private:
  ...
  std::map<uint32_t, const RtpPacketView> incoming_rtp_packet_views_;
  std::map<uint32_t, const RtpPacketView> outgoing_rtp_packet_views_;
  // Raw RTCP packets.
