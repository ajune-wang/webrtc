...
bool AddOrModifySctpDataCodecPort(DataContentDescription* media_desc,
  ...
  RTC_LOG(INFO) << "AddSctpDataCodec: Set SCTP Port Number " << sctp_port;
...
bool AddOrModifySctpDataMaxMessageSize(DataContentDescription* media_desc,
                                       int max_message_size) {
  // Add the SCTP Port number as a pseudo-codec "max-message-size" parameter
  auto codec = FindOrMakeSctpDataCodec(media_desc);
  int dummy;
  ...
  RTC_LOG(INFO) << "AddSctpDataCodec: Set SCTP Max Message Size "
                << max_message_size;
  media_desc->AddOrReplaceCodec(codec);
  return true;
}
