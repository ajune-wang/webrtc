...
constexpr char kVp8ThenVp9SdpVideoString[] =
    "m=video 9 UDP/TLS/RTP/SAVPF 96 98 97 99 100 101 102\r\n";
...
TEST(SdpChangerTest, ForceVideoCodecVP8WhenItIsAlreadySelected) {
  ...
  std::unique_ptr<SessionDescriptionInterface> actual_sdp =
      changer.ReleaseSessionDescription();

  std::string actual_sdp_string = SdpToString(actual_sdp.get());
  // VP8 is already on the first place, so order have to be the same.
  std::string expected_video_line = kVp8ThenVp9SdpVideoString;
  std::size_t found = actual_sdp_string.find(expected_video_line);
  EXPECT_NE(found, std::string::npos) << actual_sdp_string;
}
...
TEST(SdpChangerTest, ForceVideoCodecVP9) {
  ...
  std::string expected_video_line = kVp9ThenVp8SdpVideoString;
  std::size_t found = actual_sdp_string.find(expected_video_line);
  EXPECT_NE(found, std::string::npos) << actual_sdp_string;
}
