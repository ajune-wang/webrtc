...
bool WgcCaptureSession::GetFrame(std::unique_ptr<DesktopFrame>* output_frame,
  ...
  // if we know that the source is not capturable. This can happen e.g. when
  // captured window is minimized and if EnsureFrame() was called in this state
  // a large amount of kFrameDropped errors would be logged.
  if (source_is_capturable)
    EnsureFrame();
