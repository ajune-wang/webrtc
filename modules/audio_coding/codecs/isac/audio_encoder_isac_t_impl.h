...
void AudioEncoderIsacT<T>::SetTargetBitrate(int target_bps) {
  ...
  // because that's what webrtc::AudioEncoderOpus does.
...
void AudioEncoderIsacT<T>::OnReceivedOverhead(
  ...
  overhead_per_packet_ = webrtc::DataSize::Bytes(overhead_bytes_per_packet);
}
...
void AudioEncoderIsacT<T>::SetTargetBitrate(int target_bps,
  ...
  if (subtract_per_packet_overhead) {
    const webrtc::DataRate overhead_rate =
        overhead_per_packet_ / TimeDelta::Millis(config_.frame_size_ms);
    target_bps -= overhead_rate.bps();
  }
  target_bps = rtc::SafeClamp(
      target_bps, kMinBitrateBps,
      MaxBitrateBps(config_.sample_rate_hz, config_.frame_size_ms));
  RTC_CHECK_EQ(0, T::Control(isac_state_, target_bps, config_.frame_size_ms));
