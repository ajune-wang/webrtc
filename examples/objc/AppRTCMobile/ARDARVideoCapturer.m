...
@implementation ARDARVideoCapturer {
    ...
    CVPixelBufferRef _last_depth_buffer;
#endif
}
...
- (void)session:(ARSession *)sessionn didUpdateFrame:(ARFrame *)frame {
    ...
    if (!depth_data && !_last_depth_buffer) return;
    ...
    CVPixelBufferRef depth_buffer = depth_data?[depth_data depthDataMap]:_last_depth_buffer;
    if (_last_depth_buffer == NULL)
        CVPixelBufferCreate(nil, CVPixelBufferGetWidth(depth_buffer),CVPixelBufferGetHeight(depth_buffer),
                            kCVPixelFormatType_DepthFloat32, nil, &_last_depth_buffer);
    
    if (depth_buffer != _last_depth_buffer){
