...
SendSideCongestionController::SendSideCongestionController(
  ...
  QueueTaskRunner* queue = safe_queue_.get();
  ...
  NetworkControlProducers prods = controller_->GetProducers();
  ...
  NetworkInformationReceivers recvs = controller_->GetReceivers();
...
void SendSideCongestionController::OnTransportFeedback(
  ...
  if (feedback_vector.size() > 0) {
    TransportPacketsFeedback msg =
        TransportPacketsFeedbackFromRtpFeedbackVector(feedback_vector,
                                                      feedback_time_ms);
