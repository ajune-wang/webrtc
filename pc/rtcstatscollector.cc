...
void RTCStatsCollector::ProduceRTPStreamStats_n(
  ...
  // Audio
  if (track_media_info_map.voice_media_info()) {
    const std::string& mid = *track_media_info_map.voice_mid();
    const std::string& transport_name = transport_names_by_mid.at(mid);
    std::string transport_id = RTCTransportStatsIDFromTransportChannel(
  ...
  // Video
  if (track_media_info_map.video_media_info()) {
    const std::string& mid = *track_media_info_map.video_mid();
    const std::string& transport_name = transport_names_by_mid.at(mid);
    std::string transport_id = RTCTransportStatsIDFromTransportChannel(
        transport_name, cricket::ICE_CANDIDATE_COMPONENT_RTP);
    // Inbound
...
RTCStatsCollector::PrepareTrackMediaInfoMap_s() const {
  RTC_DCHECK(signaling_thread_->IsCurrent());
  rtc::Optional<std::string> voice_mid;
  std::unique_ptr<cricket::VoiceMediaInfo> voice_media_info;
  if (pc_->voice_channel()) {
    voice_mid = pc_->voice_channel()->content_name();
  ...
  rtc::Optional<std::string> video_mid;
  std::unique_ptr<cricket::VideoMediaInfo> video_media_info;
  if (pc_->video_channel()) {
    video_mid = pc_->video_channel()->content_name();
  ...
  std::unique_ptr<TrackMediaInfoMap> track_media_info_map(new TrackMediaInfoMap(
      voice_mid, video_mid, std::move(voice_media_info),
      std::move(video_media_info), pc_->GetSenders(), pc_->GetReceivers()));
