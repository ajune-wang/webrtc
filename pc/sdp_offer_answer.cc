...
class SdpOfferAnswerHandler::ApplyingRemoteDescription {
 public:
  ...
  bool IsRollback() {
    ...
    if (handler_->IsUnifiedPlan()) {
      if (handler_->pc_->configuration()->enable_implicit_rollback) {
      ...
      [this_weak_ptr = weak_ptr_factory_.GetWeakPtr(),
       ...
       desc = std::unique_ptr<SessionDescriptionInterface>(desc_ptr)](
        ...
        this_weak_ptr->DoSetRemoteDescription(
            std::make_unique<ApplyingRemoteDescription>(
...
void SdpOfferAnswerHandler::DoSetRemoteDescription(
    std::unique_ptr<SdpOfferAnswerHandler::ApplyingRemoteDescription>
        operation) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  ...
  // Stash away whether or not we're dealing with an answer, for use later.
  const bool is_answer = (operation->type() == SdpType::kAnswer);
  ...
  // Consider the operation complete at this point, so delete `operation` which
  // will notify the observer and operations chain.
  operation.reset();

  SetRemoteDescriptionPostProcess(is_answer);
}
