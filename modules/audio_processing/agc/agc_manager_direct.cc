...
void MonoAgc::SetLevel(int new_level) {
  int voe_level = recommended_analog_level_;
  if (voe_level == 0) {
    RTC_DLOG(LS_INFO)
  ...
  recommended_analog_level_ = new_level;
...
int MonoAgc::CheckVolumeAndReset() {
  int level = recommended_analog_level_;
  ...
  if (level < minLevel) {
    ...
    recommended_analog_level_ = level;
  }
  agc_->Reset();
  level_ = level;
  startup_ = false;
  ...
  if (!!clipping_predictor_) {
    for (int channel = 0; channel < num_capture_channels_; ++channel) {
      const auto step = clipping_predictor_->EstimateClippedLevelStep(
          channel, recommended_analog_level_, clipped_level_step_,
...
void AgcManagerDirect::AggregateChannelLevels() {
  int new_recommended_analog_level = recommended_analog_level_;

  new_recommended_analog_level = channel_agcs_[0]->recommended_analog_level();
  ...
  if (use_min_channel_level_) {
    for (size_t ch = 1; ch < channel_agcs_.size(); ++ch) {
      ...
      if (level < new_recommended_analog_level) {
        new_recommended_analog_level = level;
  ...
  if (min_mic_level_override_.has_value() && new_recommended_analog_level > 0) {
    new_recommended_analog_level =
        std::max(new_recommended_analog_level, *min_mic_level_override_);
  }

  if (analog_controller_enabled_) {
    recommended_analog_level_ = new_recommended_analog_level;
  }
}
