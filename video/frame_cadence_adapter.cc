...
// Abstracts the current mode of the cadence adapter.
class Adapter {
 public:
...
class PassthroughAdapter : public Adapter {
 public:
  PassthroughAdapter(Clock* clock,
                     FrameCadenceAdapterInterface::Callback* callback)
      : clock_(clock),
        callback_(callback),
        input_framerate_(
            FrameCadenceAdapterInterface::kFrameRateAvergingWindowSizeMs,
            1000) {
...
class ZeroHertzAdapter : public Adapter {
 public:
  ZeroHertzAdapter(FrameCadenceAdapterInterface::Callback* callback,
                   double max_fps);
...
class FrameCadenceAdapterImpl : public FrameCadenceAdapterInterface {
 ...
 private:
  ...
  // Returns true if constraints min fps = 0, max fps > 0, field trial enabled,
  // and zero-hertz mode enabled.
  ...
  absl::optional<PassthroughAdapter> passthrough_adapter_;
  absl::optional<ZeroHertzAdapter> zero_hertz_adapter_;
  // Cache for the current adapter.
  Adapter* current_adapter_ = nullptr;
...
ZeroHertzAdapter::ZeroHertzAdapter(
    FrameCadenceAdapterInterface::Callback* callback,
    double max_fps)
...
void ZeroHertzAdapter::OnFrame(Timestamp post_time,
                               int frames_scheduled_for_processing,
...
absl::optional<uint32_t> ZeroHertzAdapter::GetInputFramerateFps() {
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  return max_fps_;
}
...
void FrameCadenceAdapterImpl::Initialize(Callback* callback) {
  ...
  current_adapter_ = &passthrough_adapter_.value();
}
...
absl::optional<uint32_t> FrameCadenceAdapterImpl::GetInputFramerateFps() {
  RTC_DCHECK_RUN_ON(queue_);
  return current_adapter_->GetInputFramerateFps();
}
...
void FrameCadenceAdapterImpl::OnFrameOnMainQueue(
    ...
    int frames_scheduled_for_processing,
    const VideoFrame& frame) {
  current_adapter_->OnFrame(post_time, frames_scheduled_for_processing, frame);
}
...
bool FrameCadenceAdapterImpl::IsZeroHertzScreenshareEnabled() const {
  return zero_hertz_screenshare_enabled_ && source_constraints_.has_value() &&
         ...
         *source_constraints_->max_fps > 0 &&
         source_constraints_->min_fps.has_value() &&
         *source_constraints_->min_fps == 0 &&
...
void FrameCadenceAdapterImpl::MaybeReconfigureAdapters(
  ...
  if (is_zero_hertz_enabled) {
    ...
    current_adapter_ = &zero_hertz_adapter_.value();
  } else {
    if (was_zero_hertz_enabled)
