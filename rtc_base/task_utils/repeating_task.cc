...
RepeatingTaskBase::RepeatingTaskBase(TaskQueueBase* task_queue,
    ...
    : task_queue_(task_queue),
      next_run_time_(Timestamp::Microseconds(rtc::TimeMicros()) + first_delay) {
}
...
bool RepeatingTaskBase::Run() {
  ...
  TimeDelta lost_time =
      Timestamp::Microseconds(rtc::TimeMicros()) - next_run_time_;
  next_run_time_ += delay;
  delay -= lost_time;
  delay = std::max(delay, TimeDelta::Zero());

  task_queue_->PostDelayedTask(absl::WrapUnique(this), delay.Milliseconds());
