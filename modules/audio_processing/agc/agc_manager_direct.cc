...
int InitializeGainControl(GainControl* gain_control) {
  ...
  if (gain_control->set_target_level_dbfs(2) != 0) {
    RTC_LOG(LS_ERROR) << "set_target_level_dbfs(2) failed.";
    return -1;
  }
  if (gain_control->set_compression_gain_db(kDefaultCompressionGain) != 0) {
    RTC_LOG(LS_ERROR)
        << "set_compression_gain_db(kDefaultCompressionGain) failed.";
    return -1;
  }
  if (gain_control->enable_limiter(true) != 0) {
...
int InitializeDisabledGainControl(GainControl* gain_control) {
  // Make sure to initialize the gain control to not apply a gain in the
  // ProcessCaptureAudio step.
  if (gain_control->set_mode(GainControl::kFixedDigital) != 0) {
    RTC_LOG(LS_ERROR) << "set_mode(GainControl::kFixedDigital) failed.";
    return -1;
  }
  if (gain_control->set_target_level_dbfs(0) != 0) {
    RTC_LOG(LS_ERROR) << "set_target_level_dbfs(0) failed.";
    return -1;
  }
  if (gain_control->set_compression_gain_db(0) != 0) {
    RTC_LOG(LS_ERROR) << "set_compression_gain_db(0) failed.";
    return -1;
  }
  if (gain_control->enable_limiter(false) != 0) {
    RTC_LOG(LS_ERROR) << "enable_limiter(false) failed.";
    return -1;
  }
  return 0;
}
...
int AgcManagerDirect::Initialize() {
  ...
  if (disable_digital_adaptive_) {
    return InitializeDisabledGainControl(gctrl_);
  } else {
    return InitializeGainControl(gctrl_);
  }
}
