...
class AudioSendStream final : public webrtc::AudioSendStream,
 ...
 private:
  ...
  // TODO(sukhanov): Consider some reasonable non-zero default
  // overhead instead of zero, unless overhead is always known
  // before we start sending audio. For default overhead we can
  // use use 70 bytes = Ipv6 + udp + srtp + rtp overhead.
  int transport_overhead_per_packet_bytes_ = 0;

  // Maximum bitrate from config_ plus estimated overhead,
  // which was used to configure bitrate_allocator.
  int max_bitrate_with_overhead_bps_ = 0;
