...
void DecodedFramesHistory::InsertDecoded(int64_t picture_id, int spatial_id) {
  if (static_cast<int>(layers_.size()) < spatial_id + 1) {
    layers_.resize(spatial_id + 1);
    layers_[spatial_id].buffer.resize(window_size_);
    layers_[spatial_id].last_stored_index = picture_id;
    layers_[spatial_id].buffer[picture_id % window_size_] = true;
    return;
  }

  LayerHistory& history = layers_[spatial_id];

  RTC_DCHECK_LT(history.last_stored_index, picture_id);

  int64_t id_jump = picture_id - history.last_stored_index;
  int last_index = history.last_stored_index % window_size_;
  int new_index = picture_id % window_size_;
  ...
  history.last_stored_index = picture_id;
}

bool DecodedFramesHistory::WasDecoded(int64_t picture_id, int spatial_id) {
  ...
  if (picture_id <= history.last_stored_index - window_size_) {
    RTC_LOG(LS_WARNING) << "Referencing a frame out of the history window. "
                           "Assuming it was undecoded to avoid artifacts.";
    return false;
  }

  if (picture_id > history.last_stored_index)
    return false;

  return history.buffer[picture_id % window_size_];
}
