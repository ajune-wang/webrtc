...
CreateRemoteOutboundAudioStreamStats(
    ...
    const std::string& transport_id) {
  if (!voice_receiver_info.last_sender_report_timestamp.has_value()) {
  ...
  auto stats = std::make_unique<RTCRemoteOutboundRtpStreamStats>(
      ...
      /*timestamp_us=*/voice_receiver_info.last_sender_report_timestamp->us());
  ...
  RTC_DCHECK(
      voice_receiver_info.last_sender_report_remote_timestamp.has_value());
  stats->remote_timestamp = static_cast<double>(
      voice_receiver_info.last_sender_report_remote_timestamp->ms());
...
CreateRemoteOutboundVideoStreamStats(
    ...
    const std::string& transport_id) {
  if (!video_receiver_info.last_sender_report_timestamp.has_value()) {
    // Cannot create `RTCRemoteOutboundRtpStreamStats` when the RTCP SR arrival
    // timestamp is not available - i.e., until the first sender report is
    // received.
    return nullptr;
  ...
  auto stats = std::make_unique<RTCRemoteOutboundRtpStreamStats>(
      ...
      /*timestamp_us=*/video_receiver_info.last_sender_report_timestamp->us());
  ...
  stats->kind = "video";
  stats->transport_id = transport_id;
  if (inbound_video_stats.codec_id.is_defined()) {
  ...
  RTC_DCHECK(
      video_receiver_info.last_sender_report_remote_timestamp.has_value());
  stats->remote_timestamp = static_cast<double>(
      video_receiver_info.last_sender_report_remote_timestamp->ms());
...
void RTCStatsCollector::ProduceVideoRTPStreamStats_n(
  ...
  for (const cricket::VideoReceiverInfo& video_receiver_info :
    ...
    auto remote_outbound_video = CreateRemoteOutboundVideoStreamStats(
        video_receiver_info, mid, *inbound_video, transport_id);
    // Add stats.
    if (remote_outbound_video) {
      ...
      if (remote_outbound_video_ptr) {
        inbound_video->remote_id = remote_outbound_video_ptr->id();
      } else {
