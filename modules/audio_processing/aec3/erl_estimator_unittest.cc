...
void VerifyErl(const std::array<float, kFftLengthBy2Plus1>& erl,
  ...
  EXPECT_NEAR(reference, erl_all_bands, 0.001);
}
...
TEST(ErlEstimator, Estimates) {
  ...
  VerifyErl(estimator.Erl(), estimator.ErlAllBands(), 10.f);
  ...
  estimator.Update(X2, Y2);
  VerifyErl(estimator.Erl(), estimator.ErlAllBands(), 20.f);
  ...
  VerifyErl(estimator.Erl(), estimator.ErlAllBands(), 1000.f);
