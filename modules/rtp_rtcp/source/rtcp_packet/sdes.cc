...
bool Sdes::Parse(const CommonHeader& packet) {
  ...
  if (packet.payload_size_bytes() % 4 != 0) {
    NLOG(LS_WARNING, "Invalid payload size ", packet.payload_size_bytes(),
  ...
  for (size_t i = 0; i < number_of_chunks;) {
    ...
    if (payload_end - looking_at < 8) {
      NLOG(LS_WARNING, "Not enough space left for chunk #", (i + 1));
      return false;
    }
    ...
    while ((item_type = *(looking_at++)) != kTerminatorTag) {
      if (looking_at >= payload_end) {
        NLOG(LS_WARNING, "Unexpected end of packet while reading chunk #",
      ...
      if (looking_at + item_length + kTerminatorSize > payload_end) {
        NLOG(LS_WARNING, "Unexpected end of packet while reading chunk #",
             (i + 1), ". Expected to find text of size ", item_length);
        return false;
      }
      if (item_type == kCnameTag) {
        if (cname_found) {
          NLOG(LS_WARNING, "Found extra CNAME for same ssrc in chunk #",
               (i + 1));
          return false;
        }
        cname_found = true;
    ...
    if (cname_found) {
      ...
      NLOG(LS_WARNING, "CNAME not found for ssrc ", chunks[i].ssrc);
      --number_of_chunks;
      chunks.resize(number_of_chunks);
    }
...
bool Sdes::AddCName(uint32_t ssrc, std::string cname) {
  ...
  if (chunks_.size() >= kMaxNumberOfChunks) {
    NLOG(LS_WARNING, "Max SDES chunks reached.");
    return false;
  }
  Chunk chunk;
  chunk.ssrc = ssrc;
