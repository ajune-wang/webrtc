...
class AcmChangeBitRateOldApi : public AcmSetBitRateOldApi {
 protected:
  ...
  void Run(int target_bitrate_bps,
    ...
    EXPECT_LE(expected_before_switch_bits, nr_bytes_before * 8 * 8 / 10);
    EXPECT_GE(expected_before_switch_bits, nr_bytes_before * 8 * 12 / 10);
    EXPECT_LE(expected_after_switch_bits, nr_bytes_after * 8 * 8 / 10);
    EXPECT_GE(expected_after_switch_bits, nr_bytes_after * 8 * 12 / 10);
  }
