...
struct RTC_EXPORT RtpEncodingParameters {
  ...
  // video source using `OnOutputFormatRequest`, since the
  // `requested_resolution` will be propagated to the video source and the
  // behaviour will be equivalent.
  //
  // The `requested_resolution` is subject to resource adaptation.
  //
  // It is an error to set both `requested_resolution` and
  // `scale_resolution_down_by`-
  absl::optional<RenderResolution> requested_resolution;
