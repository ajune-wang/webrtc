...
TEST_F(PacingControllerTest, DoesNotPadIfProcessThreadIsBorked) {
  ...
  // Process packets should send the delayed packets, followed by a small amount
  // of missed padding.
  pacer.ProcessPackets();

  // The max padding window is the max replay duration + the future padding
  // target size (5ms).
  const DataSize kMaxPadding =
      (PacingController::kMaxPaddingReplayDuration + TimeDelta::Millis(5)) *
      kTargetRate;
