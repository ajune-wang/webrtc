...
int32_t AudioDeviceModuleImpl::CreatePlatformSpecificObjects() {
  ...
  std::unique_ptr<AudioDeviceFactory> audio_device_factory =
      CreateAudioDeviceFactory();

  AudioLayer audio_layer(PlatformAudioLayer());
#if defined(WEBRTC_ANDROID)
  // Create an Android audio manager.
  audio_manager_android_.reset(new AudioManager());
  audio_device_ = audio_device_factory->CreateAudioDevice(
      audio_layer, audio_manager_android_.get());
#else
  audio_device_ = audio_device_factory->CreateAudioDevice(
      audio_layer, /*android_audio_manager=*/nullptr);
#endif
