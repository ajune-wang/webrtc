...
int64_t CalculateArrivalTimeUs(int64_t start_time_us,
  ...
  // Adding `capacity - 1` to the numerator allows some rounding of the extra
  // time calculation. Sending 0 bits takes 0 extra time, while sending 1 bit
  // gets rounded up to 1 (the multiplication by 1000 is because capacity is in
  // kbps).
...
bool SimulatedNetwork::EnqueuePacket(PacketInFlightInfo packet) {
  ...
  // network after the last bit of the previoius packet has left it.
  ...
  capacity_link_.push({.packet = packet,
                       .actual_transmission_time_us = packet_send_time_us,
...
void SimulatedNetwork::UpdateCapacityQueue(ConfigState state,
  ...
  if (!capacity_link_.empty()) {
    capacity_link_.front().arrival_time_us = CalculateArrivalTimeUs(
        capacity_link_.front().actual_transmission_time_us,
  ...
  do {
    ...
    int64_t arrival_time_us_before_delay = packet.arrival_time_us;
    ...
    int64_t next_start = std::max(arrival_time_us_before_delay,
                                  capacity_link_.front().packet.send_time_us);
    capacity_link_.front().actual_transmission_time_us = next_start;
    capacity_link_.front().arrival_time_us = CalculateArrivalTimeUs(
