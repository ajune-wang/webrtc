...
class RtpRtcpObserver {
 public:
  ...
  // TODO: !!! Used?
  void RegisterRtpHeaderExtension(RTPExtensionType type, uint8_t id) {
    parser_->RegisterRtpHeaderExtension(type, id);
  }

  // TODO: !!!
  void RegisterRtpHeaderExtensions(
      const std::vector<std::pair<RTPExtensionType, uint8_t>>& extensions) {
    for (const auto& extension : extensions) {
      RegisterRtpHeaderExtension(extension.first, extension.second);
    }
  }
