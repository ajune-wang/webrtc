...
class DataChannelObserverImpl : public webrtc::DataChannelObserver {
 public:
  explicit DataChannelObserverImpl(webrtc::DataChannelInterface* dc,
                                   bool network_ok,
                                   bool server)
      : dc_(dc), network_ok_(network_ok), server_(server) {}
  ...
  void OnMessage(const webrtc::DataBuffer& buffer) override {
    if (server_) {
      ...
      if (bytes_received_threshold_ &&
          bytes_received_ >= bytes_received_threshold_.value()) {
  ...
  bool IsOkToCallOnTheNetworkThread() override { return network_ok_; }

  bool WaitForOpenState() {
    RTC_CHECK(!server_);
    return open_event_.Wait(rtc::Event::kForever);
  }

  bool WaitForClosedState() {
    RTC_CHECK(server_);
    return closed_event_.Wait(rtc::Event::kForever);
  }
  ...
  signaling_thread->Start();
  {
    ...
    client.SetOnDataChannel(
        [&](rtc::scoped_refptr<webrtc::DataChannelInterface> channel) {
          ...
          observer = std::make_unique<DataChannelObserverImpl>(
              data_channel.get(), true, false);
