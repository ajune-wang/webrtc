...
bool ScreenCapturerLinux::Init(const DesktopCaptureOptions& options) {
  ...
  if (root_window_ == BadValue) {
    NLOG(LS_ERROR, "Unable to get the root window");
    DeinitXlib();
    return false;
  }
  ...
  if (gc_ == NULL) {
    NLOG(LS_ERROR, "Unable to get graphics context");
    DeinitXlib();
    return false;
  }
  ...
  if (!x_server_pixel_buffer_.Init(display(), DefaultRootWindow(display()))) {
    NLOG(LS_ERROR, "Failed to initialize pixel buffer.");
    return false;
  }
...
void ScreenCapturerLinux::InitXDamage() {
  ...
  if (!XDamageQueryExtension(display(), &damage_event_base_,
                             &damage_error_base_)) {
    NLOG(LS_INFO, "X server does not support XDamage.");
    return;
  }
  ...
  if (!damage_handle_) {
    NLOG(LS_ERROR, "Unable to initialize XDamage.");
    return;
  }
  ...
  if (!damage_region_) {
    ...
    NLOG(LS_ERROR, "Unable to create XFixes region.");
    return;
  }
  ...
  use_damage_ = true;
  NLOG(LS_INFO, "Using XDamage extension.");
}
