...
class SctpTransportInformation {
 public:
  explicit SctpTransportInformation(SctpTransportState state) : state_(state) {}
  ...
  absl::optional<double> maxMessageSize() const { return maxMessageSize_; }
  absl::optional<int> maxChannels() const { return maxChannels_; }

 private:
  rtc::scoped_refptr<DtlsTransportInterface> dtls_transport_;
  SctpTransportState state_;
  absl::optional<double> maxMessageSize_;
  absl::optional<int> maxChannels_;
};
...
class SctpTransportInterface : public rtc::RefCountInterface {
 public:
  ...
  virtual rtc::scoped_refptr<DtlsTransportInterface> dtls_transport() = 0;
  // Returns information on the state of the SctpTransport.
  // This function can be called from other threads.
  virtual SctpTransportInformation Information() = 0;
  // Observer management.
