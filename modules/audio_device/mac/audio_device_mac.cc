...
void AudioDeviceMac::logCAMsg(const rtc::LoggingSeverity sev,
  ...
  switch (sev) {
    case rtc::LS_ERROR:
      RTC_DLOG(LS_ERROR) << msg << ": " << err[0] << err[1] << err[2] << err[3];
      break;
    case rtc::LS_WARNING:
      RTC_DLOG(LS_WARNING) << msg << ": " << err[0] << err[1] << err[2]
                          << err[3];
      break;
    case rtc::LS_VERBOSE:
  ...
  switch (sev) {
    case rtc::LS_ERROR:
      RTC_DLOG(LS_ERROR) << msg << ": " << err[3] << err[2] << err[1] << err[0];
      break;
    case rtc::LS_WARNING:
      RTC_DLOG(LS_WARNING) << msg << ": " << err[3] << err[2] << err[1]
                          << err[0];
      break;
    case rtc::LS_VERBOSE:
...
AudioDeviceMac::~AudioDeviceMac() {
  ...
  if (kernErr != KERN_SUCCESS) {
    RTC_DLOG(LS_ERROR) << "semaphore_destroy() error: " << kernErr;
  }
...
AudioDeviceGeneric::InitStatus AudioDeviceMac::Init() {
  ...
  if (_paRenderBuffer == NULL) {
    ...
    if (bufSize == -1) {
      RTC_DLOG(LS_ERROR) << "PaUtil_InitializeRingBuffer() error";
  ...
  kernErr = semaphore_create(mach_task_self(), &_renderSemaphore,
                             SYNC_POLICY_FIFO, 0);
  if (kernErr != KERN_SUCCESS) {
    RTC_DLOG(LS_ERROR) << "semaphore_create() error: " << kernErr;
  ...
  if (aoerr != noErr) {
    RTC_DLOG(LS_ERROR) << "Error in AudioObjectSetPropertyData: "
                      << (const char*)&aoerr;
    return InitStatus::OTHER_ERROR;
  }
  ...
  if (intErr != 0) {
    RTC_DLOG(LS_ERROR) << "Error in sysctlbyname(): " << err;
  } else {
...
int32_t AudioDeviceMac::Terminate() {
  ...
  if (_recording) {
    RTC_DLOG(LS_ERROR) << "Recording must be stopped";
    return -1;
  }

  if (_playing) {
    RTC_DLOG(LS_ERROR) << "Playback must be stopped";
    return -1;
  }
...
int32_t AudioDeviceMac::InitPlayout() {
  ...
  if (!MicrophoneIsInitialized()) {
    ...
    if (MicrophoneIsAvailable(available) == -1) {
      RTC_DLOG(LS_WARNING) << "MicrophoneIsAvailable() failed";
    }
  }
  ...
  if (_outStreamFormat.mFormatFlags & kAudioFormatFlagIsNonInterleaved) {
    RTC_DLOG(LS_ERROR) << "Non-interleaved audio data is not supported."
...
int32_t AudioDeviceMac::InitRecording() {
  RTC_DLOG(LS_INFO) << "InitRecording";
  rtc::CritScope lock(&_critSect);
  ...
  if (!SpeakerIsInitialized()) {
    ...
    if (SpeakerIsAvailable(available) == -1) {
      RTC_DLOG(LS_WARNING) << "SpeakerIsAvailable() failed";
    }
  }
  ...
  if (io_block_size_samples > _captureBufSizeSamples) {
    RTC_DLOG(LS_ERROR) << "Input IO block size (" << io_block_size_samples
...
int32_t AudioDeviceMac::StartRecording() {
  RTC_DLOG(LS_INFO) << "StartRecording";
  rtc::CritScope lock(&_critSect);
  ...
  if (!_initialized) {
    RTC_DLOG(LS_ERROR) << "Recording worker thread has not been started";
    return -1;
  }
...
int32_t AudioDeviceMac::StopRecording() {
  RTC_DLOG(LS_INFO) << "StopRecording";
  rtc::CritScope lock(&_critSect);
  ...
  if (_twoDevices && captureDeviceIsAlive == 1) {
    ...
    if (_recording) {
      ...
      if (!_stopEventRec.Wait(2000)) {
        ...
        RTC_DLOG(LS_WARNING) << "Timed out stopping the capture IOProc."
      ...
      _critSect.Enter();
      _doStopRec = false;
      RTC_DLOG(LS_INFO) << "Recording stopped (input device)";
    } else if (_recIsInitialized) {
      WEBRTC_CA_LOG_WARN(
          AudioDeviceDestroyIOProcID(_inputDeviceID, _inDeviceIOProcID));
      RTC_DLOG(LS_INFO) << "Recording uninitialized (input device)";
    }
  } else {
    ...
    if (_recording && captureDeviceIsAlive == 1) {
      ...
      if (!_stopEvent.Wait(2000)) {
        rtc::CritScope critScoped(&_critSect);
        RTC_DLOG(LS_WARNING) << "Timed out stopping the shared IOProc."
                            << "We may have failed to detect a device removal.";
      ...
      _critSect.Enter();
      _doStop = false;
      RTC_DLOG(LS_INFO) << "Recording stopped (shared device)";
    } else if (_recIsInitialized && !_playing && !_playIsInitialized) {
      WEBRTC_CA_LOG_WARN(
          AudioDeviceDestroyIOProcID(_outputDeviceID, _deviceIOProcID));
      RTC_DLOG(LS_INFO) << "Recording uninitialized (shared device)";
    }
  }
...
int32_t AudioDeviceMac::StopPlayout() {
  ...
  if (_playing && renderDeviceIsAlive == 1) {
    ...
    if (!_stopEvent.Wait(2000)) {
      rtc::CritScope critScoped(&_critSect);
      RTC_DLOG(LS_WARNING) << "Timed out stopping the render IOProc."
                          << "We may have failed to detect a device removal.";
    ...
    _critSect.Enter();
    _doStop = false;
    RTC_DLOG(LS_INFO) << "Playout stopped";
  } else if (_twoDevices && _playIsInitialized) {
    WEBRTC_CA_LOG_WARN(
        AudioDeviceDestroyIOProcID(_outputDeviceID, _deviceIOProcID));
    RTC_DLOG(LS_INFO) << "Playout uninitialized (output device)";
  } else if (!_twoDevices && _playIsInitialized && !_recIsInitialized) {
    WEBRTC_CA_LOG_WARN(
        AudioDeviceDestroyIOProcID(_outputDeviceID, _deviceIOProcID));
    RTC_DLOG(LS_INFO) << "Playout uninitialized (shared device)";
  }
...
int32_t AudioDeviceMac::GetNumberDevices(const AudioObjectPropertyScope scope,
  ...
  if (size == 0) {
    RTC_DLOG(LS_WARNING) << "No devices";
    return 0;
  }
  ...
  if (usedID != kAudioDeviceUnknown) {
    ...
    RTC_DLOG(LS_WARNING) << "GetNumberDevices(): Default device unknown";
  }
    ...
    for (UInt32 i = 0; i < numberDevices; i++) {
      ...
      if (bufferList->mNumberBuffers > 0) {
        if (numberScopedDevices >= deviceListLength) {
          RTC_DLOG(LS_ERROR) << "Device list is not long enough";
          listOK = false;
          break;
        }
...
int32_t AudioDeviceMac::GetDeviceName(const AudioObjectPropertyScope scope,
  ...
  if (index == 0) {
    ...
    if (usedID == kAudioDeviceUnknown) {
      RTC_DLOG(LS_WARNING) << "GetDeviceName(): Default device unknown";
    } else {
...
int32_t AudioDeviceMac::InitDevice(const uint16_t userDeviceIndex,
  ...
  if (numberDevices < 0) {
    ...
    RTC_DLOG(LS_ERROR) << "InitDevice(): No devices";
    return -1;
  }
  ...
  if (userDeviceIndex == 0) {
    ...
    if (deviceId == kAudioDeviceUnknown) {
      RTC_DLOG(LS_WARNING) << "No default device exists";
    } else {
      isDefaultDevice = true;
    }
  }
  ...
  if (isInput) {
    RTC_DLOG(LS_INFO) << "Input device: " << devManf << " " << devName;
  } else {
    RTC_DLOG(LS_INFO) << "Output device: " << devManf << " " << devName;
  }
...
int32_t AudioDeviceMac::HandleDeviceChange() {
  ...
  if (MicrophoneIsInitialized()) {
    ...
    if (err == kAudioHardwareBadDeviceError || deviceIsAlive == 0) {
      RTC_DLOG(LS_WARNING) << "Capture device is not alive (probably removed)";
      AtomicSet32(&_captureDeviceIsAlive, 0);
      _mixerManager.CloseMicrophone();
    } else if (err != noErr) {
  ...
  if (SpeakerIsInitialized()) {
    ...
    if (err == kAudioHardwareBadDeviceError || deviceIsAlive == 0) {
      RTC_DLOG(LS_WARNING) << "Render device is not alive (probably removed)";
      AtomicSet32(&_renderDeviceIsAlive, 0);
      _mixerManager.CloseSpeaker();
    } else if (err != noErr) {
      logCAMsg(rtc::LS_ERROR, "Error in AudioDeviceGetProperty()",
...
int32_t AudioDeviceMac::HandleStreamFormatChange(
  ...
  if (streamFormat.mChannelsPerFrame > N_DEVICE_CHANNELS) {
    RTC_DLOG(LS_ERROR) << "Too many channels on device (mChannelsPerFrame = "
                      << streamFormat.mChannelsPerFrame << ")";
    return -1;
  }

  if (_ptrAudioBuffer && streamFormat.mChannelsPerFrame != _recChannels) {
    RTC_DLOG(LS_ERROR) << "Changing channels not supported (mChannelsPerFrame = "
                      << streamFormat.mChannelsPerFrame << ")";
    return -1;
  }
...
int32_t AudioDeviceMac::HandleProcessorOverload(
  ...
  if (propertyAddress.mScope == kAudioDevicePropertyScopeInput) {
    // RTC_DLOG(LS_WARNING) << "Capture processor // overload";
    //_callback->ProblemIsReported(
    // SndCardStreamObserver::ERecordingProblem);
  } else {
    // RTC_DLOG(LS_WARNING) << "Render processor overload";
    //_callback->ProblemIsReported(
...
OSStatus AudioDeviceMac::implDeviceIOProc(const AudioBufferList* inputData,
  ...
  if (err != noErr) {
    if (err == 1) {
      ...
      RTC_DLOG(LS_ERROR) << "Error in AudioConverterFillComplexBuffer()";
      return 1;
    } else {
...
OSStatus AudioDeviceMac::implOutConverterProc(UInt32* numberDataPackets,
  ...
  if (kernErr != KERN_SUCCESS) {
    RTC_DLOG(LS_ERROR) << "semaphore_signal_all() error: " << kernErr;
    return 1;
  }
...
OSStatus AudioDeviceMac::implInConverterProc(UInt32* numberDataPackets,
  ...
  while (PaUtil_GetRingBufferReadAvailable(_paCaptureBuffer) < numSamples) {
    ...
    if (kernErr == KERN_OPERATION_TIMED_OUT) {
      ...
      RTC_DLOG(LS_ERROR) << "semaphore_wait() error: " << kernErr;
    }
  }
...
bool AudioDeviceMac::RenderWorkerThread() {
  ...
  while (PaUtil_GetRingBufferWriteAvailable(_paRenderBuffer) -
    ...
    if (kernErr == KERN_OPERATION_TIMED_OUT) {
      ...
      RTC_DLOG(LS_ERROR) << "semaphore_timedwait() error: " << kernErr;
    }
  }
  ...
  if (!_ptrAudioBuffer) {
    RTC_DLOG(LS_ERROR) << "capture AudioBuffer is invalid";
    return false;
  }
