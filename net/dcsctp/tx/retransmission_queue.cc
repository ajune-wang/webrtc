...
size_t CalculateInitialCwnd(size_t mtu) {
...
RetransmissionQueue::RetransmissionQueue(
    ...
    size_t a_rwnd,
    SendQueue* producer,
    std::function<void(int64_t rtt_ms)> on_new_rtt,
    ...
    : options_(options),
      ...
      t3_rtx_(*t3_rtx),
      cwnd_(CalculateInitialCwnd(options.mtu)),
      rwnd_(a_rwnd),
      // https://tools.ietf.org/html/rfc4960#section-7.2.1
...
size_t RetransmissionQueue::RemoveAcked(UnwrappedTSN cumulative_tsn_ack,
  ...
  for (auto it = outstanding_data_.begin(); it != first_unacked; ++it) {
    bytes_acked += it->second.data.size();
    acked_tsns.push_back(it->first.Wrap());
  }
...
RetransmissionQueue::AckGapBlockInfo RetransmissionQueue::AckGapBlocks(
    UnwrappedTSN cumulative_tsn_ack,
    rtc::ArrayView<const SackChunk::GapAckBlock> gap_ack_blocks,
    std::vector<TSN>& acked_tsns) {
  AckGapBlockInfo ret(cumulative_tsn_ack);
  ...
  // SACK chunk as advisory."

  for (auto& block : gap_ack_blocks) {
    auto start = outstanding_data_.find(cumulative_tsn_ack.AddTo(block.start));
    auto end =
        outstanding_data_.upper_bound(cumulative_tsn_ack.AddTo(block.end));
    for (auto iter = start; iter != end; ++iter) {
      if (iter->second.state != State::kAcked) {
        ret.bytes_acked_by_new_gap_ack_blocks += iter->second.data.size();
        iter->second.state = State::kAcked;
        ret.htna = std::max(ret.htna, iter->first);
        acked_tsns.push_back(iter->first.Wrap());
      }
    }
  }
  return ret;
...
bool RetransmissionQueue::NackBetweenAckBlocks(
    UnwrappedTSN cumulative_tsn_ack,
    UnwrappedTSN htna,
    rtc::ArrayView<const SackChunk::GapAckBlock> gap_ack_blocks) {
  ...
  UnwrappedTSN max_tsn_to_nack = htna;
  ...
  // TODO(boivie): Must mark everything above last gap_ack_block as NACKED.
  return has_packet_loss;
}
...
void RetransmissionQueue::HandleAcknowledge(int64_t now_ms,
                                            const SackChunk& sack) {
  ...
  if (sack.gap_ack_blocks().empty()) {
    UpdateRTT(now_ms, cumulative_tsn_ack);
  }

  // Erase all items up to cumulative_tsn_ack.
  size_t bytes_acked_by_cumulative_tsn_ack =
      RemoveAcked(cumulative_tsn_ack, acked_tsns);

  // ACK packets reported in the gap ack blocks
  AckGapBlockInfo ack_info =
      AckGapBlocks(cumulative_tsn_ack, sack.gap_ack_blocks(), acked_tsns);

  // NACK and possibly mark for retransmit chunks that weren't acked.
  bool has_packet_loss = NackBetweenAckBlocks(cumulative_tsn_ack, ack_info.htna,
                                              sack.gap_ack_blocks());
  ...
  if (cumulative_tsn_ack > last_cumulative_tsn_ack_) {
    HandleIncreasedCumulativeTsnAck(
        old_outstanding_bytes, bytes_acked_by_cumulative_tsn_ack +
  ...
  // the error counter ..."
  if (bytes_acked_by_cumulative_tsn_ack > 0 ||
...
void RetransmissionQueue::UpdateRTT(int64_t now_ms,
                                    UnwrappedTSN cumulative_tsn_ack) {
  ...
  if (it != outstanding_data_.end()) {
    if (it->second.num_retransmissions == 0) {
      ...
      // later instance)"
      int64_t rtt_ms = now_ms - it->second.sent_ms;
      on_new_rtt_(rtt_ms);
    }
  }
}
  ...
  if (is_in_fast_retransmit()) {
    ...
    while (max_bytes > data_chunk_header_size_) {
      absl::optional<SendQueue::DataToSend> chunk_opt =
          producer_.Produce(now_ms, max_bytes - data_chunk_header_size_);
      ...
      outstanding_data_.emplace(
          tsn, RetransmissionQueue::TxData(std::move(chunk_opt->data),
                                           chunk_opt->max_retransmissions,
                                           now_ms, chunk_opt->expiry_ms));
    }
  }
...
bool RetransmissionQueue::ShouldSendForwardTsn(int64_t now_ms) {
...
void RetransmissionQueue::ExpireChunks(int64_t now_ms) {
  for (auto& elem : outstanding_data_) {
    UnwrappedTSN tsn = elem.first;
    TxData& item = elem.second;
    ...
    if (item.state != State::kAcked && item.state != State::kAbandoned) {
      if (item.max_retransmissions >= 0 &&
        ...
        ExpireAllFor(item);
      } else if (item.expiry_ms >= 0 && item.expiry_ms <= now_ms) {
        RTC_DLOG(LS_VERBOSE)
            << "Marking chunk " << *tsn.Wrap() << " and message "
