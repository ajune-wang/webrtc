...
class MockOutput : public RtcEventLogOutput {
 public:
  bool IsActive() const { return true; }
  MOCK_METHOD(void, OnWrite, (absl::string_view), ());
  bool Write(absl::string_view data) {
    // We don't care about empty outputs.
    if (!data.empty()) {
      OnWrite(data);
    }
    return true;
...
class FakeEvent : public RtcEvent {
 public:
  Type GetType() const override { return RtcEvent::Type::FrameDecoded; }
...
class RtcEventLogImplTest : public ::testing::Test {
 public:
  ...
  std::unique_ptr<MockOutput> output_ = std::make_unique<MockOutput>();
  MockOutput* output_ptr_ = output_.get();
...
TEST_F(RtcEventLogImplTest, WritesHeaderAndEventsAndTrailer) {
  ...
  InSequence s;
  EXPECT_CALL(*output_ptr_, OnWrite(StrEq("start")));
  EXPECT_CALL(*output_ptr_, OnWrite(StrEq("configevent")));
  EXPECT_CALL(*output_ptr_, OnWrite(StrEq("stop")));
  ...
  Mock::VerifyAndClearExpectations(output_ptr_);
}
...
TEST_F(RtcEventLogImplTest, RewritesConfigEventsOnlyOnRestart) {
  event_log_.StartLogging(std::make_unique<MockOutput>(), kOutputPeriod.ms());
  event_log_.Log(std::make_unique<FakeConfigEvent>());
  event_log_.Log(std::make_unique<FakeEvent>());
  event_log_.StopLogging();
  EXPECT_CALL(*encoder_ptr_,
