...
Call::Stats Call::GetStats() const {
  ...
  // TODO(tommi): These stats seem to most be managed on the process and worker
  // threads. Should the call be forced to be made on either of those?
  ...
  // TODO(tommi): This synchronizes with process thread.
  stats.rtt_ms = call_stats_->LastProcessedRtt();

  // Fetch available send/receive bitrates.
  std::vector<unsigned int> ssrcs;
  uint32_t recv_bandwidth = 0;
  // TODO(tommi): This syncs with process thread (fwict).
  receive_side_cc_.GetRemoteBitrateEstimator(false)->LatestEstimate(
      &ssrcs, &recv_bandwidth);
  stats.recv_bandwidth_bps = recv_bandwidth;

  {
    // TODO(tommi): This syncs with worker thread... perhaps that could be done
    // on the process thread? (See OnTargetTransferRate)
