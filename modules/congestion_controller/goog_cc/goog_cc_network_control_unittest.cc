...
TransportPacketsFeedback CreateTransportPacketsFeedback(
  ...
  for (int i = 0; i < kFeedbackSize; ++i) {
    PacketResult packet = CreatePacketResult(
        send_time + delay_buildup, send_time, kPayloadSize, PacedPacketInfo());
...
class GoogCcFactoryConfigTest : public ::testing::TestWithParam<bool> {
 protected:
...
TEST_P(GoogCcFactoryConfigTest, CalculatesRttFromTransporFeedback) {
  ...
  TransportPacketsFeedback feedback = CreateTransportPacketsFeedback(
      /*one_way_delay=*/TimeDelta::Millis(50), one_way_delay,
...
INSTANTIATE_TEST_SUITE_P(GoogCcFactoryConfigTests,
                         GoogCcFactoryConfigTest,
                         ::testing::Bool());
