...
SpectralFeaturesExtractor::RingBuffer2D::RingBuffer2D(const float init_value)
    : tail_(0) {
  buf_.fill(init_value);
}

SpectralFeaturesExtractor::RingBuffer2D::~RingBuffer2D() = default;

void SpectralFeaturesExtractor::RingBuffer2D::Push(
    rtc::ArrayView<float, kBandEnergyCoeffsHistorySize - 1> spectral_diffs) {
  // TODO(alessiob): Implement.
  tail_++;
  if (tail_ == kBandEnergyCoeffsHistorySize)
    tail_ = 0;
}
...
SpectralFeaturesExtractor::SpectralFeaturesExtractor()
    : fft_(kFrameSize20ms12kHz),
      ...
      spectral_coeffs_ring_buf_(0.f),
      spectral_variability_ring_buf_2d_(0.f) {
...
bool SpectralFeaturesExtractor::AnalyzeCheckSilence(
    rtc::ArrayView<const float> reference_frame,
  ...
  if (tot_energy < 0.04f)  // Check if silence.
    return true;
  ...
  // Push current frame data for the spectral features computation.
  spectral_coeffs_ring_buf_.Push({log_band_energy_coeffs_decorrelated.data(),
                                  log_band_energy_coeffs_decorrelated.size()});
  return false;
}

void SpectralFeaturesExtractor::ComputeAverage(
    rtc::ArrayView<float, kNumOpusBands> dst) const {
  ...
  RTC_DCHECK_LE(dst.size(), curr.size());
  RTC_DCHECK_LE(dst.size(), prev1.size());
  RTC_DCHECK_LE(dst.size(), prev2.size());
  for (size_t i = 0; i < dst.size(); ++i)
    dst[i] = curr[i] + prev1[i] + prev2[i];
}

void SpectralFeaturesExtractor::ComputeDelta1(
  ...
  RTC_DCHECK_LE(dst.size(), prev2.size());
  for (size_t i = 0; i < dst.size(); ++i)
    dst[i] = curr[i] - prev2[i];
}

void SpectralFeaturesExtractor::ComputeDelta2(
    rtc::ArrayView<float, kNumBandEnergyCoeffDeltas> dst) const {
  ...
  RTC_DCHECK_LE(dst.size(), prev2.size());
  for (size_t i = 0; i < dst.size(); ++i)
    dst[i] = curr[i] - 2 * prev1[i] + prev2[i];
}
...
void SpectralFeaturesExtractor::ComputeCorrelation(
  ...
  auto functor = [x, y](const size_t freq_bin_index) {
    return ((*x)[freq_bin_index].real() * (*y)[freq_bin_index].real() +
           ...
           kFftLenght20ms12kHz * kFftLenght20ms12kHz;  // Scaling.
  };
  ...
  // Decorrelate.
  ComputeDct({band_corr_coeffs}, {dct_table_}, dct_scaling_factor_, {dst});
}

float SpectralFeaturesExtractor::ComputeSpectralVariability() const {
  // TODO(alessiob): Reduce complexity using spectral_variability_ring_buf_2d_.
  float spec_variability = 0.f;
  for (size_t i = 0; i < kBandEnergyCoeffsHistorySize; ++i) {
    auto view_i = spectral_coeffs_ring_buf_.GetArrayView(i);
    float min_dist = 1e15f;
    for (size_t j = 0; j < kBandEnergyCoeffsHistorySize; ++j) {
      auto view_j = spectral_coeffs_ring_buf_.GetArrayView(j);
      float dist = 0.f;
      for (size_t k = 0; k < kNumOpusBands; ++k) {
        const float c = view_i[k] - view_j[k];
        dist += c * c;
      }
      if (j != i)
