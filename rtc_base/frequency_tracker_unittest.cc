...
TEST(FrequencyTrackerTest,
     ReturnsRateAfterOneMeasurementsOverExactlyFullWindow) {
  const Timestamp start = Timestamp::Seconds(12'345);
  FrequencyTracker stats(kWindow);

  stats.Update(100, start);
  ...
  EXPECT_EQ(stats.Rate(start + kWindow - kEpsilon), 100 / kWindow);
