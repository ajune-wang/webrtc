    ...
    if (alr_probing) {
      ...
      const double hysteresis_factor =
          RateControlSettings::ParseFromFieldTrials()
              .GetSimulcastHysteresisFactor(mode);
...
void VideoSendStreamImpl::OnEncoderConfigurationChanged(
  ...
  if (!worker_queue_->IsCurrent()) {
    ...
    worker_queue_->PostTask(
        [send_stream, streams, mode, min_transmit_bitrate_bps]() {
          if (send_stream) {
            send_stream->OnEncoderConfigurationChanged(
                std::move(streams), mode, min_transmit_bitrate_bps);
          }
        });
    return;
  }
  ...
  if (codec_type == kVideoCodecVP9) {
    ...
    max_padding_bitrate_ = CalculateMaxPadBitrateBps(
        streams, mode, min_transmit_bitrate_bps,
