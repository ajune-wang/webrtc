...
TEST_F(TestFrameBuffer2, DontDropSpatialLayerSlowDecoder) {
  uint16_t pid = Rand();
  uint32_t ts = Rand();

  InsertFrame(pid, 0, ts, false, false);
  InsertFrame(pid, 1, ts, false, true);
  for (int i = 1; i < 6; ++i) {
    uint32_t ts_tl0 = ts + i * kFps10;
    InsertFrame(pid + i, 0, ts_tl0, false, false, pid + i - 1);
    InsertFrame(pid + i, 1, ts_tl0, false, true, pid + i - 1);
  }
  ...
  CheckFrame(1, pid + 1, 0);
  CheckFrame(2, pid + 2, 0);
  CheckFrame(3, pid + 3, 0);
  CheckFrame(4, pid + 4, 0);
  CheckFrame(5, pid + 5, 0);
  CheckNoFrame(6);
}
