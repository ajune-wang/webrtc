...
namespace webrtc {
namespace {
// Maximum time to elapse before checking if we need to call SendRTCP or
// call NotifyTmmbrUpdated.
const int64_t kRtpRtcpMaxIdleTimeProcessMs = 100;
// For updating the bitrate when using TMMBR, we update more frequently, or
// once every 20ms (somewhat arbitrarily chosen).
const int64_t kRtpRtcpTimeProcessMs = 20;
...
void ModuleRtpRtcpImpl2::Process() {
  ...
  const bool tmmbr = rtcp_sender_.TMMBR();

  if (rtcp_sender_.Sending()) {
    if (remote_bitrate_ && tmmbr) {
      next_process_time_ = now + kRtpRtcpTimeProcessMs;
  ...
  // The timer granularity for TimeToSendRTCPReport is 'seconds'
  // (not milliseconds).
  if (rtcp_sender_.TimeToSendRTCPReport())
    rtcp_sender_.SendRTCP(GetFeedbackState(), kRtcpReport);

  // kTmmbrTimeoutIntervalMs is 25 seconds, so an order of seconds again.
