...
int main(int argc, char* argv[]) {
  ...
  if (args.size() != 1 ||
      args.size() != 2) {  // The output audio file is optional.
    // Print usage information.
    std::cout << usage;
    exit(0);
  }
  const std::string output_audio_filename((args.size() == 2) ? args[1] : "");
  ...
  std::unique_ptr<webrtc::test::NetEqTest> test =
      factory.InitializeTestFromFile(/*input_filename=*/args[0], config);
