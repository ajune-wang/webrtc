...
ChannelMember::ChannelMember(DataSocket* socket)
  ...
  RTC_DCHECK(socket);
  RTC_DCHECK(socket->method() == DataSocket::GET);
...
void ChannelMember::QueueResponse(const std::string& status,
  ...
  if (waiting_socket_) {
    ...
    RTC_DCHECK(waiting_socket_->method() == DataSocket::GET);
    bool ok =
...
void ChannelMember::SetWaitingSocket(DataSocket* ds) {
  RTC_DCHECK(ds->method() == DataSocket::GET);
  if (ds && !queue_.empty()) {
    RTC_DCHECK(waiting_socket_ == NULL);
