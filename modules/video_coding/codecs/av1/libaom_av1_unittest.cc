...
class TestAv1Decoder {
 public:
  ...
  // Id to distinguish failure message for different decoders.
  void SetDecoderId(int id) { decoder_id_ = id; }
...
class TestAv1Encoder {
 public:
  ...
  explicit TestAv1Encoder(std::unique_ptr<ScalableVideoController> controller)
      : encoder_(CreateLibaomAv1Encoder(std::move(controller))) {
    RTC_CHECK(encoder_);
...
TEST(LibaomAv1Test, EncodeDecode) {
  ...
  TestAv1Encoder encoder(std::make_unique<ScalableVideoControllerNoLayering>());
...
TEST_P(LibaomAv1SvcTest, EncodeAndDecodeAllDecodeTarget) {
  std::unique_ptr<ScalableVideoController> controller = GetParam()();
  ...
  std::vector<TestAv1Decoder> clients(num_decode_targets);
  for (size_t i = 0; i < num_decode_targets; ++i) {
    clients[i].SetDecoderId(i);
  }
  TestAv1Encoder encoder(std::move(controller));

  std::vector<TestAv1Encoder::Encoded> encoded_frames;
  for (const VideoFrame& frame : GenerateFrames(/*num_frames=*/12)) {
    encoder.EncodeAndAppend(frame, &encoded_frames);
  }
  ...
  for (size_t i = 0; i < clients.size(); ++i) {
    const TestAv1Decoder& client = clients[i];
    ASSERT_THAT(client.requested_frame_ids(), Not(IsEmpty()))
        << " Decoder#" << i;
    EXPECT_THAT(client.decoded_frame_ids(),
                ElementsAreArray(client.requested_frame_ids()))
        << " Decoder#" << i;
    EXPECT_EQ(client.num_output_frames(), client.decoded_frame_ids().size())
        << " Decoder#" << i;
  }
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    Values(std::make_unique<ScalableVideoControllerNoLayering>,
           std::make_unique<ScalableVideoControllerL2T2>));
