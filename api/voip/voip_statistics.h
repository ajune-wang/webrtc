...
// Remote statistics obtained via remote RTCP received.
struct RemoteRtcpStatistics {
  // Jitter as defined in RFC 3550 [6.4.1] expressed in second.
  double jitter = 0.0;

  // Cumulative packets lost as defined in RFC 3550 [6.4.1]
  int64_t packets_lost = 0;

  // Fraction lost as defined in RFC 3550 [6.4.1] converted from Q8.
...
struct ChannelStatistics {
  ...
  // SSRC from remote media endpoint.
...
class VoipStatistics {
 public:
  ...
  //  kInvalidArgument - |channel_id| is invalid.
  virtual VoipResult GetChannelStatistics(ChannelId channe_id,
