...
size_t Merge::Process(int16_t* input, size_t input_length,
  ...
  for (size_t channel = 0; channel < num_channels_; ++channel) {
    ...
    int16_t mute_factor = expand_->MuteFactor(channel);
    // Update |mute_factor| if it is lower than |new_mute_factor|.
    if (new_mute_factor > mute_factor) {
      mute_factor = std::min(new_mute_factor, static_cast<int16_t>(16384));
    }
