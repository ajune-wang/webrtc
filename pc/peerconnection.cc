...
bool PeerConnection::Initialize(
  ...
  if (configuration.use_media_transport_for_data_channels) {
    RTC_DCHECK(!configuration.enable_rtp_data_channel)
        << "enable_rtp_data_channel and use_media_transport_for_data_channels "
           "are not compatible";
...
RTCError PeerConnection::ApplyLocalDescription(
  ...
  // Local description applied first, so this PC is the caller.
  if (!remote_description()) {
    is_caller_ = true;
  }

  // TODO(mellem): Push down |is_caller_| from here.
  ...
  if ((data_channel_type() == cricket::DCT_SCTP ||
       data_channel_type() == cricket::DCT_MEDIA_TRANSPORT) &&
      GetSctpSslRole(&role)) {
...
rtc::scoped_refptr<DataChannel> PeerConnection::InternalCreateDataChannel(
  ...
  if (channel->data_channel_type() == cricket::DCT_RTP) {
    ...
    RTC_DCHECK(channel->data_channel_type() == cricket::DCT_SCTP ||
               channel->data_channel_type() == cricket::DCT_MEDIA_TRANSPORT);
...
bool PeerConnection::GetSctpSslRole(rtc::SSLRole* role) {
  ...
  } else if (data_channel_mid_ && media_transport_) {
...
void PeerConnection::OnMediaTransportStateChanged_n() {
  if (!data_channel_mid_ ||
      transport_controller_->GetMediaTransportState(*data_channel_mid_) !=
