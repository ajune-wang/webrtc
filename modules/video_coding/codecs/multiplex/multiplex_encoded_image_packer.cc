...
MultiplexImage::MultiplexImage(uint16_t picture_index, uint8_t frame_count, uint8_t* augmenting_data, uint16_t augmenting_data_size)
    : image_index(picture_index), component_count(frame_count), augmenting_data_size(augmenting_data_size), augmenting_data(augmenting_data) {}
...
EncodedImage MultiplexEncodedImagePacker::PackAndRelease(
  ...
  int bitstream_offset = header_offset + kMultiplexImageComponentHeaderSize *
                                          ...
                                          2 + multiplex_image.augmenting_data_size;
  ...
  // Augmenting Data
  int data_offset = kMultiplexImageHeaderSize + kMultiplexImageComponentHeaderSize * header.component_count;
  ByteWriter<uint16_t>::WriteBigEndian(combined_image._buffer + data_offset, multiplex_image.augmenting_data_size);
  if (multiplex_image.augmenting_data_size != 0) {
    memcpy(combined_image._buffer + data_offset + 2, multiplex_image.augmenting_data, multiplex_image.augmenting_data_size);
...
MultiplexImage MultiplexEncodedImagePacker::Unpack(
  ...
  int16_t augmenting_data_size = ByteReader<uint16_t>::ReadBigEndian(combined_image._buffer +
                                                                     kMultiplexImageHeaderSize +
                                                                     kMultiplexImageComponentHeaderSize * header.component_count);
  uint8_t * augmenting_data = augmenting_data_size != 0 ?
    combined_image._buffer + kMultiplexImageHeaderSize + kMultiplexImageComponentHeaderSize * header.component_count + 2 : NULL;
  MultiplexImage multiplex_image(header.image_index, header.component_count, augmenting_data, augmenting_data_size);
