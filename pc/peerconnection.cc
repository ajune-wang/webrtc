      ...
      if (media_type == cricket::MEDIA_TYPE_AUDIO ||
        ...
        if (content_info.rejected) {
          ...
          if (!transceiver->internal()->channel()) {
            ...
            if (!channel) {
              // TODO(bugs.webrtc.org/4676): handle CreateChannel failure by
              // restoring back to old description.
...
RTCError PeerConnection::ApplyRemoteDescription(
  ...
  if (configuration_.bundle_policy ==
      PeerConnectionInterface::kBundlePolicyMaxBundle) {
    bundle_group = remote_description()->description()->GetGroupByName(
        cricket::GROUP_TYPE_BUNDLE);
    if (!bundle_group) {
      LOG_AND_RETURN_ERROR(RTCErrorType::INVALID_PARAMETER,
                           "max-bundle configured but session description "
  ...
  if (IsUnifiedPlan()) {
    const ContentInfos& old_contents =
        (replaced_remote_description
             ? replaced_remote_description->description()->contents()
             : ContentInfos());
    const ContentInfos& contents =
        remote_description()->description()->contents();
    for (size_t i = 0; i < contents.size(); ++i) {
      const ContentInfo& content = contents[i];
      if (!cricket::IsMediaContent(&content)) {
        continue;
      }
      const std::string& mid = content.name;
      ...
      if (media_type == cricket::MEDIA_TYPE_AUDIO ||
        ...
        // description, not rejected in current description), dissociate the
        // currently associated RtpTransceiver by setting its mid property to
        // null, and discard the mapping between the transceiver and its m=
        // section index.
        if (i < old_contents.size()) {
          const ContentInfo& old_content = old_contents[i];
          if (old_content.rejected && !content.rejected) {
        ...
        auto transceiver_or_error =
            AssociateTransceiver(cricket::CS_REMOTE, i, content);
        if (!transceiver_or_error.ok()) {
          return transceiver_or_error.MoveError();
        }
        auto transceiver = transceiver_or_error.MoveValue();
...
void PeerConnection::GetOptionsForUnifiedPlanOffer(
  ...
  std::set<rtc::scoped_refptr<RtpTransceiverProxyWithInternal<RtpTransceiver>>>
      found_transceivers;
  ...
  for (size_t i = 0;
    ...
    const ContentInfo& content =
        (i < local_contents.size() ? local_contents[i] : remote_contents[i]);
    const auto* media_desc =
        static_cast<const MediaContentDescription*>(content.description);
    bool had_been_rejected = false;
    if (i < local_contents.size()) {
      had_been_rejected |= local_contents[i].rejected;
    }
    if (i < remote_contents.size()) {
      had_been_rejected |= remote_contents[i].rejected;
    }
    auto transceiver = GetAssociatedTransceiver(content.name);
    RTC_DCHECK(transceiver);
    found_transceivers.insert(transceiver);
    if (had_been_rejected && transceiver->stopped()) {
      session_options->media_description_options.push_back(
          cricket::MediaDescriptionOptions(media_desc->type(), content.name,
                                           RtpTransceiverDirection::kInactive,
                                           /*stopped=*/true));
      recycleable_mline_indices.push(i);
    } else {
      session_options->media_description_options.push_back(
          GetMediaDescriptionOptionsForTransceiver(transceiver, i));
      transceiver->internal()->set_mline_index(i);
    }
  }
  for (auto transceiver : transceivers_) {
    if (found_transceivers.find(transceiver) != found_transceivers.end()) {
      continue;
    }
    ...
    if (transceiver->stopped() && !transceiver->mid()) {
      continue;
    }
    size_t mline_index;
    if (!recycleable_mline_indices.empty()) {
      ...
      session_options->media_description_options[mline_index] =
          GetMediaDescriptionOptionsForTransceiver(transceiver, mline_index);
    } else {
...
void PeerConnection::GetOptionsForUnifiedPlanAnswer(
  ...
  for (const ContentInfo& content :
    ...
    cricket::MediaDescriptionOptions media_description_options(
        transceiver->internal()->media_type(), *transceiver->mid(),
        transceiver->direction(), transceiver->stopped());
    cricket::SenderOptions sender_options;
    sender_options.track_id = transceiver->sender()->id();
    sender_options.stream_ids = transceiver->sender()->stream_ids();
    sender_options.num_sim_layers = 1;
...
PeerConnection::AssociateTransceiver(cricket::ContentSource source,
                                     int mline_index,
                                     const ContentInfo& section) {
  const std::string& mid = section.name;
  auto media_desc =
      static_cast<const cricket::MediaContentDescription*>(section.description);
  ...
  if (transceiver->internal()->media_type() != media_desc->type()) {
    RTC_LOG(LS_ERROR) << "Transceiver media type: "
                      << transceiver->internal()->media_type();
    RTC_LOG(LS_ERROR) << "Media desc type: " << media_desc->type();
    LOG_AND_RETURN_ERROR(
        RTCErrorType::INVALID_PARAMETER,
...
PeerConnection::FindAvailableTransceiverToReceive(
  ...
  // Implements the transceiver receive logic for receiving transceivers.
