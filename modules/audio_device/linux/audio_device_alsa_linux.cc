...
int32_t AudioDeviceLinuxALSA::InitPlayout()
{
    ...
    if (errVal < 0)
    {
        RTC_LOG(LS_ERROR) << "snd_pcm_get_params: "
                          << LATE(snd_strerror)(errVal)
                          << " (" << errVal << ")";
        _playoutBufferSizeInFrame = 0;
        _playoutPeriodSizeInFrame = 0;
...
int32_t AudioDeviceLinuxALSA::StopPlayout()
{
    ...
    if (errVal < 0)
    {
        RTC_LOG(LS_ERROR) << "Error stop playing: "
                          << LATE(snd_strerror)(errVal);
    }
    ...
    while (!(LATE(snd_card_next)(&card)) && (card >= 0) && keepSearching) {
        ...
        for (void **list = hints; *list != NULL; ++list)
        {
                ...
                strncmp(name, ignorePrefix, strlen(ignorePrefix)) != 0)
            {
                ...
                {
                    RTC_LOG(LS_VERBOSE) << "Enum device " << enumCount << " - "
                                    << name;
        ...
        if (err != 0)
        {
            RTC_LOG(LS_ERROR) << "GetDevicesInfo - device name free hint "
                              << "error: " << LATE(snd_strerror)(err);
...
int32_t AudioDeviceLinuxALSA::InputSanityCheckAfterUnlockedPeriod() const
{
    if (_handleRecord == NULL)
    {
        RTC_LOG(LS_ERROR) << "input state has been modified during unlocked "
                          << "period";
        return -1;
    }
    return 0;
        ...
        { // buf is full
            ...
            if (_handlePlayout)
            {
                ...
                if (err < 0)
                {
                    ...
                    _playoutDelay = 0;
                    RTC_LOG(LS_ERROR) << "playout snd_pcm_delay: "
                                  << LATE(snd_strerror)(err);
                }
            }
           ...
           // TODO(xians): Shall we add 10ms buffer delay to the record delay?
            ...
            if (AGC())
            {
                ...
                if (newMicLevel != 0)
                {
                    ...
                    // Set this new mic level (received from the
