...
RtpPacketizerVp9::RtpPacketizerVp9(rtc::ArrayView<const uint8_t> payload,
    ...
    : hdr_(hdr), payload_(payload.data()), limits_(limits) {
...
void RtpPacketizerVp9::GeneratePackets(size_t payload_size) {
  ...
  size_t total_bytes =
      ss_data_len + payload_size + limits_.last_packet_reduction_len;
  ...
  while (bytes_processed < payload_size) {
    ...
    size_t rem_bytes = payload_size - bytes_processed;
  ...
  RTC_CHECK_EQ(bytes_processed, payload_size);
}
