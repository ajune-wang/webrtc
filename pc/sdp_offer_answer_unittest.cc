...
TEST_F(SdpOfferAnswerTest, MidBackfill) {
  auto pc = CreatePeerConnection();
  ...
  pc->SetRemoteDescription(std::move(desc), &error);
  EXPECT_TRUE(error.ok());
  auto contents = pc->pc()->remote_description()->description()->contents();
  ASSERT_EQ(contents.size(), 1u);
  EXPECT_EQ(contents[0].mid(), "0");
}

TEST_F(SdpOfferAnswerTest, MidBackfillAnswer) {
  auto pc = CreatePeerConnection();
  // JSEP says to only include the mid in the answer if it was in the offer
  // but do to backfill it is always present. The offerer should ignore it.
  std::string sdp =
      "v=0\r\n"
      "o=- 0 3 IN IP4 127.0.0.1\r\n"
      "s=-\r\n"
...
TEST_F(SdpOfferAnswerTest, MidBackfillDoesNotCheckAgainstBundleGroup) {
  ...
  // in the offer but that gets backfilled.
  std::string sdp =
      "v=0\r\n"
      "o=- 0 3 IN IP4 127.0.0.1\r\n"
      "s=-\r\n"
