...
class SrtpSessionTest : public ::testing::Test {
 ...
 protected:
  ...
  void TestProtectRtcp(int crypto_suite) {
    ...
    EXPECT_NE(0, std::memcmp(rtcp_packet_, kRtcpReport, rtcp_len_));
    rtcp_len_ = out_len;
  }
  ...
  void TestUnprotectRtcp(int crypto_suite) {
    rtc::CopyOnWriteBuffer buffer(rtcp_packet_, rtcp_len_);
    EXPECT_TRUE(s2_.UnprotectRtcp(buffer));
    EXPECT_EQ(buffer.size(), sizeof(kRtcpReport));
    EXPECT_EQ(0, std::memcmp(kRtcpReport, buffer.data(), buffer.size()));
  }
...
TEST_F(SrtpSessionTest, TestTamperReject) {
  ...
  buffer.SetData(rtcp_packet_, rtcp_len_);
  EXPECT_FALSE(s2_.UnprotectRtcp(buffer));
  EXPECT_METRIC_THAT(
