...
BalancedDegradationSettings::GetQpThresholds(VideoCodecType type,
                                             int pixels) const {
  absl::optional<int> low = GetLowThreshold(type, GetConfig(pixels));
  absl::optional<int> high = GetHighThreshold(type, GetConfig(pixels));
