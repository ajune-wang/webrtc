...
std::vector<std::unique_ptr<RtpFrameObject>> PacketBuffer::FindFrames(
    uint16_t seq_num) {
  std::vector<std::unique_ptr<RtpFrameObject>> found_frames;
  for (size_t j = 0; j < buffer_.size() && PotentialNewFrame(seq_num);
       ++j, ++seq_num) {
    size_t index = seq_num % buffer_.size();
    ...
    int64_t frame_timestamp = buffer_[index].data.timestamp;
    bool is_h264 = buffer_[index].data.codec() == kVideoCodecH264;
    ...
    while (true) {
      int start_index = start_seq_num % buffer_.size();
      if (!is_h264) {
        if (buffer_[start_index].frame_begin())
          break;
      } else {
    ...
    auto frame = AssembleFrame(start_seq_num, seq_num);
    if (frame == nullptr) {
      break;
    }
...
std::unique_ptr<RtpFrameObject> PacketBuffer::AssembleFrame(
  ...
  size_t num_packets = ForwardDiff<uint16_t>(start_seq_num, end_seq_num + 1);
  ...
  VCMPacket* last_packet = GetPacket(end_seq_num);
  ...
  for (uint16_t seq_num = start_seq_num;; ++seq_num) {
    ...
    if (is_h264) {
      ...
      RTC_DCHECK_LT(h264_header.nalus_length, kMaxNalusPerPacket);
  ...
  if (is_h264) {
    ...
    if ((sps_pps_idr_is_h264_keyframe_ && has_h264_idr && has_h264_sps &&
      ...
      const uint8_t h264tid = video_header.frame_marking.temporal_id;
      if (h264tid == kNoTemporalIdx &&
  ...
  return std::make_unique<RtpFrameObject>(
      start_seq_num, end_seq_num, last_packet->markerBit, max_nack_count,
