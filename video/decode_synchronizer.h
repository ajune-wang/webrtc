...
// DecodeSynchronizer synchronizes the frame scheduling by coalesing decoding
// on the metronome.
//
...
// instance when `FrameDecodeScheduer::Stop()` is called, after which the
// scheduler may be destroyed by the recieve stream.
//
...
//
// In the case where the deocde time is in the past, or must occur before the
 ...
 private:
  ...
  class SynchronizedFrameDecodeScheduler : public FrameDecodeScheduler {
   public:
    ...
    // Releases the outsatinding frame for decoding. This invalidates
