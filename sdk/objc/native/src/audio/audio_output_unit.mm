...
AudioOutputUnit::AudioOutputUnit(AudioOutputUnitObserver* observer) : observer_(observer) {
  RTC_DCHECK(observer);
}
...
BaseAudioUnit::OwnedAudioUnit AudioOutputUnit::InstantiateAudioUnit() {
  ...
  AudioComponent found_remoteio_unit_ref =
      AudioComponentFindNext(nullptr, &audio_unit_description);

  AudioUnit unit = nullptr;
  // Create an audio unit.
  OSStatus result = noErr;
  result = AudioComponentInstanceNew(found_remoteio_unit_ref, &unit);
  if (result != noErr) {
