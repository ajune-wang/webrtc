...
PeerConnectionInterface::IceServer JavaToNativeIceServer(JNIEnv* env,
                                                         jobject j_ice_server) {
  jobject j_ice_server_tls_cert_policy =
      Java_IceServer_getTlsCertPolicy(env, j_ice_server);
  jobject urls = Java_IceServer_getUrls(env, j_ice_server);
  jstring username = Java_IceServer_getUsername(env, j_ice_server);
  jstring password = Java_IceServer_getPassword(env, j_ice_server);
  PeerConnectionInterface::TlsCertPolicy tls_cert_policy =
      JavaToNativeTlsCertPolicy(env, j_ice_server_tls_cert_policy);
  jstring hostname = Java_IceServer_getHostname(env, j_ice_server);
  jobject tls_alpn_protocols =
      Java_IceServer_getTlsAlpnProtocols(env, j_ice_server);
  jobject tls_elliptic_curves =
      Java_IceServer_getTlsEllipticCurves(env, j_ice_server);
  PeerConnectionInterface::IceServer server;
  server.urls = JavaToStdVectorStrings(env, urls);
  server.username = JavaToStdString(env, username);
  server.password = JavaToStdString(env, password);
  server.tls_cert_policy = tls_cert_policy;
  server.hostname = JavaToStdString(env, hostname);
  server.tls_alpn_protocols = JavaToStdVectorStrings(env, tls_alpn_protocols);
  server.tls_elliptic_curves = JavaToStdVectorStrings(env, tls_elliptic_curves);
  return server;
}
...
PeerConnectionInterface::IceServers JavaToNativeIceServers(
  ...
  for (jobject j_ice_server : Iterable(jni, j_ice_servers))
    ice_servers.push_back(JavaToNativeIceServer(jni, j_ice_server));
  return ice_servers;
}
