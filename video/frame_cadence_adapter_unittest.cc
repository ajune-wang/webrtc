...
TEST(FrameCadenceAdapterTest, ChecksIfEncodeHappenedAlignWithMetronomeTick) {
  ZeroHertzFieldTrialEnabler enabler;
  GlobalSimulatedTimeController time_controller(Timestamp::Zero());
  ...
  // `callback->OnFrame()` would not be called if only 30ms went by after
  // `adapter->OnFrame()`.
  ...
  // `callback->OnFrame()` should be called if 36ms went by after
  // `adapter->OnFrame()`.
  EXPECT_CALL(callback, OnFrame(_, false, _)).Times(1);
  time_controller.AdvanceTime(TimeDelta::Millis(6));
  Mock::VerifyAndClearExpectations(&callback);
  ...
  // Expect the encode would not happen if only 13ms went by after
  // `adapter->OnFrame()`.
  EXPECT_CALL(callback, OnFrame(_, false, _)).Times(0);
  adapter->OnFrame(frame);
  time_controller.AdvanceTime(TimeDelta::Millis(13));
  Mock::VerifyAndClearExpectations(&callback);
  // `callback->OnFrame()` should be called if 19ms went by after
  // `adapter->OnFrame()`.
  EXPECT_CALL(callback, OnFrame(_, false, _)).Times(1);
  time_controller.AdvanceTime(TimeDelta::Millis(6));
  Mock::VerifyAndClearExpectations(&callback);

  // We need to wait the worker queue release resource done then destruct the
  // adapter.
  rtc::Event destroy;
  worker_queue->PostTask([&] {
    adapter->ReleaseResourceOnWorkerQueue();
    destroy.Set();
  });
  destroy.Wait(rtc::Event::kForever);
