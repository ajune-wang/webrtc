...
std::vector<webrtc::VideoStream> EncoderStreamFactory::CreateEncoderStreams(
    const VideoStreamFactoryInterface::Args& args) {
...
void EncoderStreamFactory::SetLayerResolutionFromRequestedResolution(
  ...
  RTC_CHECK(adapter.AdaptFrameResolution(width, height, 0, &cropped_width,
                                         &cropped_height, &out_width,
                                         &out_height));
  layer.width = out_width;
  layer.height = out_height;
  RTC_LOG(LS_INFO) << "KESO: "
                   << " requested: " << requested_resolution.width << "x"
                   << requested_resolution.height << " in: " << width << "x"
                   << height << " out: " << layer.width << "x" << layer.height;
}
