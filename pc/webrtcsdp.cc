...
static bool ParseFailed(const std::string& message,
  ...
  if (line_end != std::string::npos) {
    if (line_end > 0 && (message.at(line_end - 1) == kReturn[0])) {
      --line_end;
    }
...
static bool GetLine(const std::string& message,
  ...
  *line = message.substr(line_begin, (line_end - line_begin));
  ...
  if (line->length() < 3 || !islower(cline[0]) ||
      cline[1] != kSdpDelimiterEqual[0] ||
      (cline[0] != kLineTypeSessionName && cline[2] == kSdpDelimiterSpace[0])) {
    *pos = line_begin;
    return false;
  }
  return true;
...
static bool IsLineType(const std::string& message,
  ...
  return (cmessage[line_start] == type &&
          cmessage[line_start + 1] == kSdpDelimiterEqual[0]);
}
...
static bool GetValue(const std::string& message,
                     ...
                     SdpParseError* error) {
  std::string leftpart;
  if (!rtc::tokenize_first(message, kSdpDelimiterColon[0], &leftpart, value)) {
...
bool ParseCandidate(const std::string& message,
  ...
  if (message.size() > first_line.size()) {
    ...
    if (rtc::tokenize_first(message, kNewLine[0], &left, &right) &&
        !right.empty()) {
  ...
  if (!rtc::tokenize_first(first_line, kSdpDelimiterColon[0],
  ...
  rtc::split(candidate_value, kSdpDelimiterSpace[0], &fields);
...
bool ParseIceOptions(const std::string& line,
  ...
  std::vector<std::string> fields;
  rtc::split(ice_options, kSdpDelimiterSpace[0], &fields);
...
bool ParseSctpPort(const std::string& line,
  ...
  rtc::split(line.substr(kLinePrefixLength), kSdpDelimiterColon[0], &fields);
  if (fields.size() < expected_min_fields) {
    fields.resize(0);
    rtc::split(line.substr(kLinePrefixLength), kSdpDelimiterSpace[0], &fields);
  }
  if (fields.size() < expected_min_fields) {
...
bool ParseExtmap(const std::string& line,
  ...
  rtc::split(value_direction, kSdpDelimiterSlash[0], &sub_fields);
  int value = 0;
...
bool ParseConnectionData(const std::string& line,
  ...
  // Skip the "c="
  if (!rtc::tokenize_first(line, kSdpDelimiterEqual[0], &token, &rightpart)) {
    return ParseFailed(line, "Failed to parse the network type.", error);
  }

  // Extract and verify the <nettype>
  if (!rtc::tokenize_first(rightpart, kSdpDelimiterSpace[0], &token,
                           &rightpart) ||
...
static bool ParseMsidAttribute(const std::string& line,
  ...
  if (!rtc::tokenize_first(line.substr(kLinePrefixLength),
                           kSdpDelimiterSpace[0], &field1, &new_track_id)) {
    const size_t expected_fields = 2;
    return ParseFailedExpectFieldNum(line, expected_fields, error);
  }
...
bool ParseSsrcAttribute(const std::string& line,
  ...
  if (!rtc::tokenize_first(line.substr(kLinePrefixLength),
                           kSdpDelimiterSpace[0], &field1, &field2)) {
    const size_t expected_fields = 2;
    return ParseFailedExpectFieldNum(line, expected_fields, error);
  }
  ...
  std::string value;
  if (!rtc::tokenize_first(field2, kSdpDelimiterColon[0], &attribute, &value)) {
    rtc::StringBuilder description;
  ...
  if (attribute == kSsrcAttributeCname) {
    ...
    std::vector<std::string> fields;
    rtc::split(value, kSdpDelimiterSpace[0], &fields);
...
bool ParseFmtpParam(const std::string& line,
                    ...
                    std::string* value,
                    SdpParseError* error) {
  if (!rtc::tokenize_first(line, kSdpDelimiterEqual[0], parameter, value)) {
...
bool ParseFmtpAttributes(const std::string& line,
  ...
  // parameters.
  if (!rtc::tokenize_first(line.substr(kLinePrefixLength),
                           kSdpDelimiterSpace[0], &line_payload,
                           &line_params)) {
  ...
  std::vector<std::string> fields;
  rtc::split(line_params, kSdpDelimiterSemicolon[0], &fields);
...
bool ParseRtcpFbAttribute(const std::string& line,
  ...
  rtc::split(line.c_str(), kSdpDelimiterSpace[0], &rtcp_fb_fields);
