...
const BaseCapturerPipeWire::pw_version kDmaBufMinVersion = {0, 3, 24};
const BaseCapturerPipeWire::pw_version kDmaBufModifierMinVersion = {0, 3, 33};
...
bool PipeWireVersionIsAtLeast(
    BaseCapturerPipeWire::pw_version current_version,
    BaseCapturerPipeWire::pw_version required_version) {
  if (!current_version.major && !current_version.minor &&
...
class PipeWireThreadLoopLock {
 public:
  PipeWireThreadLoopLock(pw_thread_loop* loop) : loop_(loop) {
...
void BaseCapturerPipeWire::OnStreamParamChanged(void* data,
  ...
  const int buffer_types =
      has_modifier || PipeWireVersionIsAtLeast(that->pw_server_version_,
                                               kDmaBufMinVersion)
...
pw_stream* BaseCapturerPipeWire::CreateReceivingStream() {
  ...
  const bool has_required_pw_client_version =
      PipeWireVersionIsAtLeast(pw_client_version_, kDmaBufModifierMinVersion);
  const bool has_required_pw_server_version =
      PipeWireVersionIsAtLeast(pw_server_version_, kDmaBufModifierMinVersion);
