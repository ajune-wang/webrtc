...
TEST_F(ApmTest, GainControlDiesOnLowTargetLevelDbfs) {
  EXPECT_DEATH(apm_->gain_control()->set_target_level_dbfs(-1), "");
}

TEST_F(ApmTest, GainControlDiesOnHighTargetLevelDbfs) {
  EXPECT_DEATH(apm_->gain_control()->set_target_level_dbfs(32), "");
}

TEST_F(ApmTest, GainControlDiesOnLowCompressionGainDb) {
  EXPECT_DEATH(apm_->gain_control()->set_compression_gain_db(-1), "");
}

TEST_F(ApmTest, GainControlDiesOnHighCompressionGainDb) {
  EXPECT_DEATH(apm_->gain_control()->set_compression_gain_db(91), "");
}

TEST_F(ApmTest, GainControlDiesOnLowAnalogLevelLowerLimit) {
  EXPECT_DEATH(apm_->gain_control()->set_analog_level_limits(-1, 512), "");
}

TEST_F(ApmTest, GainControlDiesOnHighAnalogLevelUpperLimit) {
