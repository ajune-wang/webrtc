...
template <typename T>
VectorMatrix<T> ToVectorMatrix(const Matrix<T>& m) {
  VectorMatrix<T> result;
...
template <typename T>
Matrix<T> FromVectorMatrix(const VectorMatrix<T>& mat) {
...
Matrix<double> GaussianElimination(Matrix<double> matrix_to_invert,
  ...
  for (size_t diagonal_index = 0; diagonal_index < n; ++diagonal_index) {
    ...
    for (size_t column = diagonal_index + 1; column < n; ++column) {
      if (abs(matrix_to_invert[column][diagonal_index]) >
          abs(matrix_to_invert[diagonal_index][diagonal_index])) {
    ...
    const double alpha = matrix_to_invert[diagonal_index][diagonal_index];
    matrix_to_invert[diagonal_index] /= alpha;
    right_hand_matrix[diagonal_index] /= alpha;

    // Eliminate the other entries in row |diagonal_index| by making them zero.
    for (size_t column = 0; column < n; ++column) {
      if (column == diagonal_index)
        continue;
      const double alpha = matrix_to_invert[column][diagonal_index];
      matrix_to_invert[column] -= alpha * matrix_to_invert[diagonal_index];
      right_hand_matrix[column] -= alpha * right_hand_matrix[diagonal_index];
    }
  }
...
void IncrementalLinearLeastSquares::AddObservations(
  ...
  const Matrix<int64_t> xx = MatrixMultiply<int64_t>(unpacked_x, unpacked_x);
  const Matrix<int64_t> xy = MatrixMultiply<int64_t>(unpacked_x, unpacked_y);
  if (sum_xx && sum_xy) {
    *sum_xx += xx;
    *sum_xy += xy;
  } else {
...
void IncrementalLinearLeastSquares::AddObservations2(
    const VectorMatrix<int16_t>& x,
  ...
  for (const auto& column : x)
    RTC_CHECK_EQ(n, column.size());
  for (const auto& column : y)
    RTC_CHECK_EQ(n, column.size());

  // We will multiply the int16_t values together, so we need to expand to a
  // type that can safely store those values, i.e. int32_t.
  const Matrix<int32_t> unpacked_x = ConvertTo<int32_t>(FromVectorMatrix(x));
  const Matrix<int32_t> unpacked_y = ConvertTo<int32_t>(FromVectorMatrix(y));
...
VectorMatrix<double> IncrementalLinearLeastSquares::GetBestSolution() const {
