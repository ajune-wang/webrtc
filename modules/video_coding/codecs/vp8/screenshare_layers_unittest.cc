...
class ScreenshareLayerTest : public ::testing::Test {
 protected:
  ...
  int ConfigureFrame(bool key_frame) {
    ...
    int flags = WebMVp8Encoder::EncodeFlags(tl_config_);
...
TEST_F(ScreenshareLayerTest, RespectsMaxIntervalBetweenFrames) {
  ...
  EXPECT_EQ(kTl0Flags,
            WebMVp8Encoder::EncodeFlags(UpdateLayerConfig(kStartTimestamp)));
  ...
  EXPECT_EQ(kTl0Flags, WebMVp8Encoder::EncodeFlags(
...
TEST_F(ScreenshareLayerTest, 2LayersSyncAtOvershootDrop) {
  ...
  config_updated_ = layers_->UpdateConfiguration(&cfg_);
  EXPECT_EQ(kTl1SyncFlags, WebMVp8Encoder::EncodeFlags(tl_config_));
