...
// - WebRTC-Audio-TransientSuppressorVadMode/Enabled-TsVad returns `kTsVad`;
...
TransientSuppressor::VadMode GetTransientSuppressorVadMode() {
  ...
  if (absl::EndsWith(full_name, "-TsVad")) {
    return TransientSuppressor::VadMode::kTsVad;
  }
...
int AudioProcessingImpl::ProcessCaptureStreamLocked() {
  ...
  if (submodules_.transient_suppressor) {
    submodules_.transient_suppressor->Analyze(*capture_buffer);
  }
    ...
    if (submodules_.transient_suppressor) {
      ...
      switch (transient_suppressor_vad_mode_) {
        ...
        case TransientSuppressor::VadMode::kRnnVad:
          // TODO(bugs.webrtc.org/13601): Use RNN VAD.
          break;
