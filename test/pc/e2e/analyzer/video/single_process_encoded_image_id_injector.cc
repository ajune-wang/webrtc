...
EncodedImage SingleProcessEncodedImageIdInjector::InjectId(
  ...
  RTC_CHECK(source._length >= 3);

  ExtractionInfo info;
  info.length = source._length;
  memcpy(info.origin_data, source._buffer, 3);
  {
...
SingleProcessEncodedImageIdInjector::ExtractId(const EncodedImage& source) {
  ...
  while (pos < source._length) {
    ...
    uint16_t sub_id = source._buffer[pos + 2];
    ...
    memcpy(&out._buffer[pos], info.origin_data, 3);
    pos += info.length;
  }
  out._length = pos;
