...
TEST(VoipEngineFactoryTest, CreateEngineWithMockModules) {
  ...
  config.audio_processing = std::unique_ptr<AudioProcessing>(
      new rtc::RefCountedObject<test::MockAudioProcessing>());
  config.audio_device = test::MockAudioDeviceModule::CreateNice();
...
TEST(VoipEngineFactoryTest, UseNoAudioProcessing) {
  ...
  config.decoder_factory = new rtc::RefCountedObject<MockAudioDecoderFactory>();
  config.task_queue_factory = CreateDefaultTaskQueueFactory();
  config.audio_processing = std::unique_ptr<AudioProcessing>(nullptr);
  config.audio_device = test::MockAudioDeviceModule::CreateNice();
...
TEST(VoipEngineFactoryTest, UseDefaultAudioProcessing) {
  VoipEngineConfig config;
  config.encoder_factory = new rtc::RefCountedObject<MockAudioEncoderFactory>();
  config.decoder_factory = new rtc::RefCountedObject<MockAudioDecoderFactory>();
  config.task_queue_factory = CreateDefaultTaskQueueFactory();
...
TEST(VoipEngineFactoryTest, ExpectFailureWithoutEncoderFactory) {
  VoipEngineConfig config;
  config.decoder_factory = new rtc::RefCountedObject<MockAudioDecoderFactory>();
  config.task_queue_factory = CreateDefaultTaskQueueFactory();
  config.audio_device = test::MockAudioDeviceModule::CreateNice();
  ...
  auto voip_engine = CreateVoipEngine(std::move(config));
  EXPECT_EQ(voip_engine, nullptr);
}

TEST(VoipEngineFactoryTest, ExpectFailureWithoutDecoderFactory) {
  VoipEngineConfig config;
  config.decoder_factory = new rtc::RefCountedObject<MockAudioDecoderFactory>();
  config.task_queue_factory = CreateDefaultTaskQueueFactory();
  config.audio_device = test::MockAudioDeviceModule::CreateNice();
...
TEST(VoipEngineFactoryTest, ExpectFailureWithoutTaskQueueFactory) {
  VoipEngineConfig config;
  config.encoder_factory = new rtc::RefCountedObject<MockAudioEncoderFactory>();
  config.decoder_factory = new rtc::RefCountedObject<MockAudioDecoderFactory>();
  config.audio_device = test::MockAudioDeviceModule::CreateNice();
