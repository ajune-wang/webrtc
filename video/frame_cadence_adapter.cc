...
void VSyncEncodeAdapterMode::OnFrame(Timestamp post_time,
  ...
  if (!worker_queue_->IsCurrent()) {
    worker_queue_->PostTask([this, post_time, queue_overload, frame] {
  ...
  // Encode immediately since the next tick is unknown.
  ...
  // The `metronome_` interval maybe throttled in some case, so here limit the
  // maximum frame delay to 33ms or the `metronome_` interval if it is less than
  // 33ms.
  TimeDelta max_allowed_frame_delay =
      std::min(metronome_->TickPeriod(), TimeDelta::Millis(33));
...
void VSyncEncodeAdapterMode::OnReadyToEncode() {
  ...
  queue_->PostTask(SafeTask(safety_.flag(), [this, post_time, frame] {
    ...
    callback_->OnFrame(post_time, /*queue_overload=*/0, frame);
  }));
}
