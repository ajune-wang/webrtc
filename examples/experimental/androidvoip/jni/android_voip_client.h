...
#ifndef EXAMPLES_EXPERIMENTAL_ANDROIDVOIP_JNI_ANDROID_VOIP_CLIENT_H_
#define EXAMPLES_EXPERIMENTAL_ANDROIDVOIP_JNI_ANDROID_VOIP_CLIENT_H_
...
// voip_engine.h. One instance of AndroidVoipClient should suffice
// for most VoIP applications. AndroidVoipClient implements
...
// VoipBase related operations with socket_thread_, this is to
// avoid multiple threads performing VoipBase operations which would
// lead to runtime errors. AndroidVoipClient is meant to be used by
// Java through JNI.
class AndroidVoipClient : public webrtc::Transport,
 ...
 public:
  ...
  // Sets two local addresses, one for RTP packets, and another for
  ...
  // Sets two remote addresses, one for RTP packets, and another for
  // RTCP packets. The RTP address will have IP address j_ip_address_string
  // and port number j_port_number_int, the RTCP address will have IP address
  // j_ip_address_string and port number j_port_number_int+1.
 ...
 private:
  ...
  // RTP/RTCP packets.
  std::unique_ptr<rtc::Thread> socket_thread_;
  ...
  // Used to send/receive RTP packets.
  std::unique_ptr<rtc::AsyncUDPSocket> rtp_socket_;
  // Used to send/receive RTCP packets.
  std::unique_ptr<rtc::AsyncUDPSocket> rtcp_socket_;
  // Address used by rtp_socket_.
  rtc::SocketAddress rtp_local_address_;
  // Address used by rtcp_socket_.
  rtc::SocketAddress rtcp_local_address_;
  // Address used by the remote endpoint's rtp_socket_.
  rtc::SocketAddress rtp_remote_address_;
  // Address used by the remote endpoint's rtcp_socket_.
  rtc::SocketAddress rtcp_remote_address_;
};

}  // namespace webrtc_examples

#endif  // EXAMPLES_EXPERIMENTAL_ANDROIDVOIP_JNI_ANDROID_VOIP_CLIENT_H_
