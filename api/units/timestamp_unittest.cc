...
TEST(TimestampTest, ConvertsToAndFromDouble) {
  ...
  EXPECT_NEAR(Timestamp::ms(kMillis).ToSecondsAsDouble(), kSecondsDouble, 1e-9);
  EXPECT_EQ(Timestamp::FromSecondsAsDouble(kSecondsDouble).ms(), kMillis);

  const double kPlusInfinity = std::numeric_limits<double>::infinity();

  EXPECT_EQ(Timestamp::Infinity().ToSecondsAsDouble(), kPlusInfinity);
  EXPECT_TRUE(Timestamp::FromSecondsAsDouble(kPlusInfinity).IsInfinite());
}
