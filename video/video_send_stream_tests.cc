...
// Tests that when the encoder uses an internal source, the Video Encoder will
// be updated with a new bitrate when turning the VideoSendStream on/off with
// VideoSendStream::UpdateActiveStreams, and when the VideoStreamEncoder is
// reconfigured with new active layers.
TEST_F(VideoSendStreamTest, VideoSendStreamUpdateActiveSpatialLayers) {
  test::NullTransport transport;
  StartStopBitrateObserver encoder;
  ...
  task_queue_.SendTask([this]() {
    video_send_stream_->UpdateActiveSimulcastLayers(
        video_encoder_config_.simulcast_layers);
  });
  EXPECT_TRUE(encoder.WaitBitrateChanged(true));
