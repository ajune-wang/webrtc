...
// A wrapper over AudioDeviceModule.
class ADMProxy : public AudioDeviceModule, public AudioTransport {
 public:
  ADMProxy(const int32_t id, const AudioLayer audio_layer)
      : impl_(AudioDeviceModule::Create(id, audio_layer)) {
    // Register self as the audio transport callback for underlying ADM impl.
    // Callbacks will also be proxied to other external audio transports (e.g.
    // VoiceEngine) if one has been registered.
    auto res = impl_->RegisterAudioCallback(this);
    is_valid_ = (impl_.get() != nullptr) && (res == 0);
  }
  virtual ~ADMProxy() { audio_transport_ = nullptr; }
...
rtc::scoped_refptr<AudioDeviceModule> AudioDeviceProxy::Create(
    const int32_t id,
    const AudioDeviceModule::AudioLayer audio_layer) {
  rtc::scoped_refptr<ADMProxy> audio_device(
      new rtc::RefCountedObject<ADMProxy>(id, audio_layer));
...
rtc::scoped_refptr<AudioDeviceModule> AudioDeviceProxy::Create(
    const int32_t id,
    const AudioDeviceModule::AudioLayer audio_layer) {
  return webrtc::CreateAudioDeviceWithDataObserver(id, audio_layer, nullptr);
}
*/
