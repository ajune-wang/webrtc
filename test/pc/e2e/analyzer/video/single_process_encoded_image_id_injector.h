...
// EncodedImage payload. Then it uses second 2 bytes for frame sub id, that is
// required to distinguish different spatial layers. The origin data from these
// 4 bytes will be stored inside injector's internal storage and then will be
...
// payload size is greater or equals to 4 bytes
class SingleProcessEncodedImageIdInjector : public EncodedImageIdInjector {
 ...
 private:
  ...
  struct ExtractionInfo {
    ...
    size_t length;
    // Data from first 4 bytes of origin encoded image's payload.
...
// Uses single SingleProcessEncodedImageIdInjector for all coding entities.
class SingleProcessQualityAnalyzingVideoContext
    : public QualityAnalyzingVideoContext {
 public:
  SingleProcessQualityAnalyzingVideoContext();
  ~SingleProcessQualityAnalyzingVideoContext() override;

  EncodedImageIdInjector* GetIdInjector(int coding_entity_id,
                                        CodingType type) override;

 private:
  std::unique_ptr<SingleProcessEncodedImageIdInjector> injector_;
};
