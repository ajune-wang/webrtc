...
The virtual method approach has the advantage that it gives the
subclass more flexibility in how it implements the signal. It may:

* Have its own signal as a member variable.
* Use a sigslot::repeater, to repeat a signal of another class:
  ...
  ```
* Just return another class's signal directly, if safe to do so:
