...
- (void)_addConsumerWithDataLength:(size_t)dataLength
                ...
                readToCurrentFrame:(BOOL)readToCurrentFrame
                       unmaskBytes:(BOOL)unmaskBytes;
- (void)_addConsumerWithScanner:(stream_scanner)consumer
                       callback:(data_callback)callback
                     dataLength:(size_t)dataLength;
- (void)_readUntilBytes:(const void *)bytes
                 length:(size_t)length
               callback:(data_callback)dataHandler;
...
- (void)_SR_commonInit;
{
  ...
  assert([scheme isEqualToString:@"ws"] || [scheme isEqualToString:@"http"] ||
...
- (BOOL)_checkHandshake:(CFHTTPMessageRef)httpMessage;
{
  NSString *acceptHeader = CFBridgingRelease(
      CFHTTPMessageCopyHeaderFieldValue(httpMessage, CFSTR("Sec-WebSocket-Accept")));
...
- (void)_HTTPHeadersDidFinish;
{
              ...
              [NSError errorWithDomain:SRWebSocketErrorDomain
                              ...
                              userInfo:@{
                                NSLocalizedDescriptionKey : [NSString
                                    stringWithFormat:@"received bad response code from server %ld",
                                                     (long)responseCode],
                                SRHTTPResponseErrorKey : @(responseCode)
                              }]];
    return;
  }
  ...
  if (![self _checkHandshake:_receivedHTTPHeaders]) {
    [self
        _failWithError:[NSError
                           errorWithDomain:SRWebSocketErrorDomain
                                      code:2133
  ...
  NSString *negotiatedProtocol = CFBridgingRelease(
      CFHTTPMessageCopyHeaderFieldValue(_receivedHTTPHeaders, CFSTR("Sec-WebSocket-Protocol")));
          ...
          _failWithError:[NSError
                             errorWithDomain:SRWebSocketErrorDomain
                                    ...
                                    userInfo:[NSDictionary
                                                 dictionaryWithObject:
                                                     [NSString
                                                         stringWithFormat:@"Server specified "
...
- (void)didConnect {
  ...
  CFHTTPMessageSetHeaderFieldValue(
      ...
      (__bridge CFStringRef)(
          _url.port ? [NSString stringWithFormat:@"%@:%@", _url.host, _url.port] : _url.host));
  ...
  CFHTTPMessageSetHeaderFieldValue(
      request, CFSTR("Sec-WebSocket-Key"), (__bridge CFStringRef)_secKey);
  CFHTTPMessageSetHeaderFieldValue(
      request,
...
- (void)_initializeStreams;
{
  ...
  CFStreamCreatePairWithSocketToHost(
      NULL, (__bridge CFStringRef)host, port, &readStream, &writeStream);
...
- (void)closeWithCode:(NSInteger)code reason:(NSString *)reason;
{
  ...
  dispatch_async(_workQueue, ^{
    ...
    NSMutableData *mutablePayload =
        [[NSMutableData alloc] initWithLength:sizeof(uint16_t) + maxMsgSize];
...
- (void)send:(id)data;
{
  NSAssert(self.readyState != SR_CONNECTING,
           @"Invalid State: Cannot call send: until connection is open");
  // TODO: maybe not copy this for performance
  data = [data copy];
  dispatch_async(_workQueue, ^{
    if ([data isKindOfClass:[NSString class]]) {
      [self _sendFrameWithOpcode:SROpCodeTextFrame
...
{
  NSAssert(self.readyState == SR_OPEN,
           @"Invalid State: Cannot call send: until connection is open");
  // TODO: maybe not copy this for performance
...
- (void)handleCloseWithData:(NSData *)data;
{
  ...
  if (dataSize == 1) {
    ...
    if (!closeCodeIsValid(_closeCode)) {
      [self _closeWithProtocolError:[NSString stringWithFormat:@"Cannot have close code of %d",
                                                               _closeCode]];
      return;
    }
    if (dataSize > 2) {
      _closeReason =
          [[NSString alloc] initWithData:[data subdataWithRange:NSMakeRange(2, dataSize - 2)]
                                encoding:NSUTF8StringEncoding];
      if (!_closeReason) {
...
- (void)_handleFrameWithData:(NSData *)frameData opCode:(NSInteger)opcode;
{
  ...
  BOOL isControlFrame =
      (opcode == SROpCodePing || opcode == SROpCodePong || opcode == SROpCodeConnectionClose);
  if (!isControlFrame) {
    [self _readFrameNew];
  } else {
    dispatch_async(_workQueue, ^{
  ...
  switch (opcode) {
    ...
    default:
      [self
          _closeWithProtocolError:[NSString stringWithFormat:@"Unknown opcode %ld", (long)opcode]];
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
  ...
  BOOL isControlFrame =
      (frame_header.opcode == SROpCodePing || frame_header.opcode == SROpCodePong ||
       frame_header.opcode == SROpCodeConnectionClose);
  ...
  if (frame_header.payload_length == 0) {
    ...
    [self _addConsumerWithDataLength:(size_t)frame_header.payload_length
                            callback:^(SRWebSocket *self, NSData *newData) {
                              if (isControlFrame) {
...
- (void)_readFrameContinue;
{
  ...
  [self
      _addConsumerWithDataLength:2
                        callback:^(SRWebSocket *self, NSData *data) {
                          ...
                          BOOL isControlFrame =
                              (receivedOpcode == SROpCodePing || receivedOpcode == SROpCodePong ||
                               receivedOpcode == SROpCodeConnectionClose);

                          if (!isControlFrame && receivedOpcode != 0 &&
                              self->_currentFrameCount > 0) {
                            [self _closeWithProtocolError:@"all data frames after the initial data "
                                                          @"frame must have opcode 0"];
                            return;
                          }
                          ...
                          header.opcode =
                              receivedOpcode == 0 ? self->_currentFrameOpcode : receivedOpcode;
                          ...
                          size_t extra_bytes_needed =
                              header.masked ? sizeof(self->_currentReadMaskKey) : 0;
                                ...
                                _addConsumerWithDataLength:extra_bytes_needed
                                                  callback:^(SRWebSocket *self, NSData *data) {
                                                    ...
                                                    if (header.payload_length == 126) {
                                                      ...
                                                      uint16_t newLen = EndianU16_BtoN(
                                                          *(uint16_t *)(mapped_buffer));
                                                      header.payload_length = newLen;
                                                      offset += sizeof(uint16_t);
                                                    } else if (header.payload_length == 127) {
                                                      assert(mapped_size >= sizeof(uint64_t));
                                                      header.payload_length = EndianU64_BtoN(
                                                    ...
                                                    if (header.masked) {
                                                      assert(mapped_size >=
                                                             sizeof(self->_currentReadMaskOffset) +
                                                                 offset);
...
- (void)_pumpWriting;
{
  ...
  if (dataLength - _outputBufferOffset > 0 && _outputStream.hasSpaceAvailable) {
    ...
    if (_outputBufferOffset > 4096 && _outputBufferOffset > (_outputBuffer.length >> 1)) {
      _outputBuffer =
          [[NSMutableData alloc] initWithBytes:(char *)_outputBuffer.bytes + _outputBufferOffset
  ...
  if (consumer.readToCurrentFrame || foundSize) {
    ...
    if (consumer.unmaskBytes) {
      ...
      for (NSUInteger i = 0; i < len; i++) {
        bytes[i] =
            bytes[i] ^ _currentReadMaskKey[_currentReadMaskOffset % sizeof(_currentReadMaskKey)];
    ...
    if (consumer.readToCurrentFrame) {
      ...
      if (_currentFrameOpcode == SROpCodeTextFrame) {
        ...
        if (_currentFrameOpcode == SROpCodeTextFrame && currentDataSize > 0) {
          ...
          NSData *scan_data = [_currentFrameData
              subdataWithRange:NSMakeRange(_currentStringScanPosition, scanSize)];
...
- (void)_sendFrameWithOpcode:(SROpCode)opcode data:(id)data;
{
  ...
  NSAssert([data isKindOfClass:[NSData class]] || [data isKindOfClass:[NSString class]],
           @"NSString or NSData");
  ...
  NSMutableData *frame =
      [[NSMutableData alloc] initWithLength:payloadLength + SRFrameHeaderOverhead];
  if (!frame) {
...
{
  [NSURLProtocol setProperty:SR_SSLPinnedCertificates
                      forKey:@"SR_SSLPinnedCertificates"
                   inRequest:self];
}
...
static inline int32_t validate_dispatch_data_partial_string(NSData *data) {
  ...
  if (codepoint == -1) {
    ...
    if (!U8_IS_LEAD(str[lastOffset]) ||
        U8_COUNT_TRAIL_BYTES(str[lastOffset]) + lastOffset < (int32_t)size) {
      size = -1;
    } else {
  ...
  if (size != -1 &&
      ![[NSString alloc] initWithBytesNoCopy:(char *)[data bytes]
                                      length:size
                                    encoding:NSUTF8StringEncoding
                                freeWhenDone:NO]) {
    size = -1;
