...
GetGainController2ConfigOverride() {
  constexpr char kGainController2FieldTrial[] = "WebRTC-Audio-GainController2";

  if (!field_trial::IsEnabled(kGainController2FieldTrial)) {
    return absl::nullopt;
  }
  ...
  FieldTrialParameter<bool> dry_run("dry_run",
                                    kDefaultAdaptiveDigitalConfig.dry_run);
  ...
  FieldTrialConstrained<double> max_gain_db(
      ...
      absl::nullopt);
  FieldTrialConstrained<int> vad_reset_period_ms(
      "vad_reset_period_ms", kDefaultAdaptiveDigitalConfig.vad_reset_period_ms,
      0, absl::nullopt);
  FieldTrialConstrained<int> adjacent_speech_frames_threshold(
      "adjacent_speech_frames_threshold",
      kDefaultAdaptiveDigitalConfig.adjacent_speech_frames_threshold, 0,
      absl::nullopt);
  ...
  const std::string field_trial_name =
      field_trial::FindFullName(kGainController2FieldTrial);
  ...
  ParseFieldTrial(
      {&enabled, &clipped_level_min, &clipped_level_step,
       ...
       &speech_probability_threshold, &speech_ratio_threshold, &dry_run,
       &headroom_db, &max_gain_db, &vad_reset_period_ms,
       &adjacent_speech_frames_threshold, &max_gain_change_db_per_second,
       &max_output_noise_level_dbfs},
      field_trial_name);
  ...
  return AudioProcessingImpl::GainController2ConfigOverride{
      ...
      AudioProcessing::Config::GainController2::AdaptiveDigital{
          .enabled = true,
          .dry_run = static_cast<bool>(dry_run.Get()),
          .headroom_db = static_cast<float>(headroom_db.Get()),
          .max_gain_db = static_cast<float>(max_gain_db.Get()),
          .vad_reset_period_ms = static_cast<int>(vad_reset_period_ms.Get()),
          .adjacent_speech_frames_threshold =
              static_cast<int>(adjacent_speech_frames_threshold.Get()),
          .max_gain_change_db_per_second =
