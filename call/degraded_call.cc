...
FakeNetworkPipeTaskQueue::FakeNetworkPipeTaskQueue(
...
void FakeNetworkPipeTaskQueue::SendRtp(const uint8_t* packet,
                                       size_t length,
...
void FakeNetworkPipeTaskQueue::SendRtcp(const uint8_t* packet, size_t length) {
  pipe_.SendRtcp(packet, length);
  Process();
}

bool FakeNetworkPipeTaskQueue::Process() {
  pipe_.Process();
...
VideoSendStream* DegradedCall::CreateVideoSendStream(
  ...
  if (send_config_ && !send_pipe_) {
    ...
    send_pipe_ = absl::make_unique<FakeNetworkPipeTaskQueue>(
