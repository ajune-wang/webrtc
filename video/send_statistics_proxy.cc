...
void SendStatisticsProxy::OnSuspendChange(bool is_suspended) {
  ...
  if (is_suspended) {
    ...
    if (adaptations_.MaskedCpuSteps().num_resolution_reductions.has_value())
      uma_container_->cpu_adapt_timer_.Start(now_ms);
    if (adaptations_.MaskedQualitySteps().num_resolution_reductions.has_value())
...
void SendStatisticsProxy::OnSendEncodedImage(
  ...
  absl::optional<int> downscales =
      adaptations_.MaskedQualitySteps().num_resolution_reductions;
  stats_.bw_limited_resolution |=
      (downscales.has_value() > 0 && downscales.value() > 0);
...
void SendStatisticsProxy::OnAdaptationChanged(
  ...
  MaskedAdaptationSteps receiver = adaptations_.MaskedQualitySteps();
  ...
  switch (reason) {
    ...
    case AdaptationReason::kQuality: {
      TryUpdateInitialQualityResolutionAdaptUp(
          receiver.num_resolution_reductions,
          adaptations_.MaskedQualitySteps().num_resolution_reductions);
    }
