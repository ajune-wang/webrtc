...
void WindowCapturerWin::CaptureFrame() {
  if (!window_) {
    NLOG(LS_ERROR, "Window hasn't been selected: ", GetLastError());
  ...
  if (!GetWindowDrawableRect(window_, &cropped_rect, &original_rect)) {
    NLOG(LS_WARNING, "Failed to get drawable window area: ", GetLastError());
  ...
  if (!window_dc) {
    NLOG(LS_WARNING, "Failed to get window DC: ", GetLastError());
    callback_->OnCaptureResult(Result::ERROR_TEMPORARY, nullptr);
    return;
  }
  ...
  if (result) {
    ...
    NLOG(LS_ERROR, "Both PrintWindow() and BitBlt() failed.");
    callback_->OnCaptureResult(Result::ERROR_TEMPORARY, nullptr);
  }
}
