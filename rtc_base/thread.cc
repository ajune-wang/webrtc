...
bool Thread::Get(Message* pmsg, int cmsWait, bool process_io) {
  ...
  TimeDelta remaining = cmsWait == kForever ? TimeDelta::PlusInfinity()
                                            : TimeDelta::Millis(cmsWait);
  Timestamp current = Now();
  const Timestamp deadline = current + remaining;
  while (true) {
    ...
    if (!ss_->Wait(std::min(delay_next, remaining).ms_or(kForever), process_io))
      return false;
...
void Thread::PostDelayedHighPrecisionTask(absl::AnyInvocable<void() &&> task,
  ...
  DoDelayPost(RTC_FROM_HERE, Now() + delay, GetAnyInvocableMessageHandler(),
              /*id=*/0, new AnyInvocableMessage(std::move(task)));
}
...
#endif
    Message msg;
    if (!Get(&msg, next.ms_or(kForever)))
      return !IsQuitting();
    Dispatch(&msg);

    if (next != TimeDelta::PlusInfinity()) {
      next = deadline - Now();
