...
void ApplyDigitalGain(const int32_t gains[11],
                      size_t num_bands,
                      float* const* out) {
  constexpr float kFactor = 1.f / 1048576.f;
  const float delta = gains[1] - gains[0];
  float gain = gains[0] * 16.f;
  for (size_t n = 0; n < 16; n++) {
    for (size_t i = 0; i < num_bands; ++i) {
      float out_tmp = out[i][n] * (gain + 127.f) * kFactor;
      if (out_tmp > 32767.f) {
        out[i][n] = 32767.f;
      } else if (out_tmp < -32768.f) {
        out[i][n] = -32768.f;
      } else {
        out[i][n] *= gain * kFactor;
      }
    }
  ...
  for (int k = 1; k < 10; k++) {
    const float delta = gains[k + 1] - gains[k];
    gain = gains[k] * 16.f;

    for (size_t n = 0; n < 16; n++) {
      for (size_t i = 0; i < num_bands; ++i) {
        out[i][k * 16 + n] *= gain * kFactor;
        out[i][k * 16 + n] =
            std::min(32767.f, std::max(-32768.f, out[i][k * 16 + n]));
      }
      gain += delta;
    }
  }
