...
class BandwidthScalerUnderTest : public BandwidthScaler {
 public:
  ...
  using BandwidthScaler::GetBitRateLimitedForResolution;
};
...
class BandwidthScalerTest : public ::testing::Test,
 ...
 protected:
  ...
  void TriggerBandwidthScalerTest(
      const std::vector<CreateFrameConfig>& CrateFrameConfigs) {
    if (CrateFrameConfigs.size() == 0) {
      return;
    }

    int tot_frame_nums = 0;
    for (size_t i = 0; i < CrateFrameConfigs.size(); ++i) {
      tot_frame_nums += CrateFrameConfigs[i].frame_num;
    }
    if (tot_frame_nums !=
        kFramerateForTest * kDefaultBitrateStateUpdateIntervalSeconds) {
      RTC_LOG(LS_INFO) << "The total number of frames is not 150";
      return;
    }

    absl::optional<VideoEncoder::ResolutionBitrateLimits> suitable_bitrate =
        bandwidth_scaler_->GetBitRateLimitedForResolution(
            CrateFrameConfigs[0].actual_width,
            CrateFrameConfigs[0].actual_height);
    ...
    for (size_t i = 0; i < CrateFrameConfigs.size(); ++i) {
      const CreateFrameConfig& config = CrateFrameConfigs[i];
      for (int j = 0; j <= config.frame_num; ++j) {
        time_send_to_scaler_ms_ += kDefaultEncodeDeltaTimeMs;
        int frame_contribute_bytes =
            GetFrameContributeBytes(config, suitable_bitrate.value());
        bandwidth_scaler_->ReportEncodeInfo(
            frame_contribute_bytes, time_send_to_scaler_ms_,
...
TEST_P(BandwidthScalerTest, AllNoramlFrame_640X360) {
  task_queue_.SendTask(
      [this] {
        ...
        TriggerBandwidthScalerTest(CrateFrameConfigs);
      },
      RTC_FROM_HERE);
