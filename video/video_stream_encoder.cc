...
void VideoStreamEncoder::OnBitrateUpdated(uint32_t bitrate_bps,
  ...
  const int bitrate_delta = abs_diff(bitrate_bps, encoder_start_bitrate_bps_);
  ...
  if (!has_seen_first_significant_bwe_change_ &&
      IsInitialFramedropOnBWEEnabled() &&
      bitrate_delta >= kFramedropThreshold * encoder_start_bitrate_bps_) {
