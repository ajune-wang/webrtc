...
FrameDependenciesControllerFullSvc::DependencyStructure() const {
  ...
  structure.num_chains = max_spatial_layers_;
  structure.decode_target_protected_by_chain.reserve(
  ...
  for (int sid = 0; sid < max_spatial_layers_; ++sid) {
    for (int tid = 0; tid < max_temporal_layers_; ++tid) {
      ...
      a_template.chain_diffs.resize(max_spatial_layers_);
...
FrameDependenciesControllerFullSvc::NextFrameConfig(bool reset) {
  ...
  int num_decode_targets = max_spatial_layers_ * max_temporal_layers_;
  for (int sid = 0; sid < max_spatial_layers_; ++sid) {
    result[sid].chains.resize(max_spatial_layers_, false);
    result[0].active_decode_targets.resize(num_decode_targets, true);
  }
  switch (temporal_unit_template_idx_) {
    ...
    case 1:  // Delta frame T0
      ...
      result[0].chains = {true, true, true};
      ...
      result[1].chains = {false, true, true};
      ...
      result[2].chains = {false, false, true};
