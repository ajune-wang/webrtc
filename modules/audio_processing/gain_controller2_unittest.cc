...
TEST(GainController2,
  ...
  for (int i = 0; i < kNumFramesToProcess; ++i) {
    ...
    agc2.Process(/*speech_probability=*/0.9f, &audio_buffer_reference);
    test::CopyVectorToAudioBuffer(stream_config, frame, &audio_buffer);
    agc2_reference.Process(absl::nullopt, &audio_buffer);
  }

  // Check the output buffers.
  bool all_samples_zero = true;
  for (int i = 0; i < kStereo; ++i) {
    for (int j = 0; j < kNumFramesToProcess; ++j) {
      all_samples_zero &= audio_buffer.channels_const()[0][j] != 0;
      EXPECT_EQ(audio_buffer.channels_const()[0][j],
                audio_buffer_reference.channels_const()[0][j]);
    }
  }
...
TEST(GainController2,
  ...
  for (int i = 0; i < kStereo; ++i) {
    for (int j = 0; j < kNumFramesToProcess; ++j) {
      if (audio_buffer.channels_const()[0][j] != 0.0f) {
        all_samples_zero = false;
        EXPECT_NE(audio_buffer.channels_const()[0][j],
                  audio_buffer_reference.channels_const()[0][j]);
      }
    }
  }
...
TEST(GainController2,
  ...
  for (int i = 0; i < kStereo; ++i) {
    for (int j = 0; j < kNumFramesToProcess; ++j) {
      EXPECT_EQ(audio_buffer.channels_const()[i][j],
