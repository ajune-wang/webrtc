...
void RoundRobinPacketQueue::Push(int priority,
  ...
  if (size_packets_ == 0) {
    ...
    MaybePromotSinglePacketToNormalQueue();
...
std::unique_ptr<RtpPacketToSend> RoundRobinPacketQueue::Pop() {
  ...
  SubtractPacketFromQueueTimeSum(queued_packet);
...
bool RoundRobinPacketQueue::Empty() const {
  RTC_CHECK(
      ((!stream_priorities_.empty() || single_packet_queue_.has_value()) &&
       size_packets_ > 0) ||
...
void RoundRobinPacketQueue::Push(QueuedPacket packet) {
  ...
  if (packet.EnqueueTimeIterator() == enqueue_times_.end()) {
    // Promotion form single-packet queue. Just add to enqueue times.
...
void RoundRobinPacketQueue::SubtractPacketFromQueueTimeSum(
  ...
  TimeDelta time_in_non_paused_state =
      time_last_updated_ - packet.EnqueueTime() - pause_time_sum_;
  queue_time_sum_ -= time_in_non_paused_state;
}

void RoundRobinPacketQueue::MaybePromotSinglePacketToNormalQueue() {
  if (single_packet_queue_.has_value()) {
