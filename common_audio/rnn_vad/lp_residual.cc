...
void ComputeCrossCorrelation(
  ...
  // TODO(alessiob): Maybe optimize using vectorization (max_lag is small).
  for (size_t lag = 0; lag < max_lag; ++lag)
    x_corr[lag] =
        std::inner_product(x.begin(), x.end() - lag, y.begin() + lag, 0.f);
}

// Denoise the auto-correlation coefficients.
...
// Compute the initial inverse filter coefficients given the auto-correlation
...
void MaybeComputeInverseFilterCoefficients(
    rtc::ArrayView<const float> x,
  ...
  if (auto_corr[0] == 0.f) {  // Empty frame.
    std::memset(lpc_coeffs.data(), 0, lpc_coeffs.size() * sizeof(float));
    return;
  }
  ...
  // Add a zero to account for lip radiation and apply another smoothing step.
  {
    const float c = 0.8f;
