...
class DtlsTestClient : public sigslot::has_slots<> {
 public:
  ...
  void OnTransportReadPacket(rtc::PacketTransportInternal* transport,
    ...
    if ((certificate_ && IsRtpLeadByte(packet.payload()[0]))) {
      ASSERT_EQ(packet.decryption_info(), rtc::ReceivedPacket::kSrtpEncrypted);
    } else {
      ASSERT_EQ(packet.decryption_info(), rtc::ReceivedPacket::kDtlsDecrypted);
    }
  }
