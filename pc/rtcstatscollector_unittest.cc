...
rtc::scoped_refptr<MockRtpSenderInternal> CreateMockSender(
  ...
  EXPECT_CALL(*sender, Stop()).Times(AtLeast(0));
  return sender;
}
...
class RTCStatsCollectorWrapper {
 public:
  ...
  void SetupRemoteTrackAndReceiver(cricket::MediaType media_type,
    ...
    pc_->AddReceiver(new rtc::RefCountedObject<FakeRtpReceiverForStats>(
        track, ssrc, 62,
        std::vector<rtc::scoped_refptr<MediaStreamInterface>>{remote_stream}));
  }
  ...
  void CreateMockRtpSendersReceiversAndChannels(
    ...
    for (auto& pair : remote_audio_track_info_pairs) {
      ...
      pc_->AddReceiver(new rtc::RefCountedObject<FakeRtpReceiverForStats>(
          remote_audio_track, voice_receiver_info.local_stats[0].ssrc,
          attachment_id++, remote_streams));
    }
    ...
    for (auto& pair : remote_video_track_info_pairs) {
      ...
      pc_->AddReceiver(new rtc::RefCountedObject<FakeRtpReceiverForStats>(
          remote_video_track, video_receiver_info.local_stats[0].ssrc,
          attachment_id++, remote_streams));
    }
