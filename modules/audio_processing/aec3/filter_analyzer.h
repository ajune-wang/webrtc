...
class FilterAnalyzer {
 ...
 private:
  void UpdateFilterGain(rtc::ArrayView<const float> filter_time_domain,
                        size_t max_index);
  bool PreProcessFilter(rtc::ArrayView<const float> filter_time_domain);
  ...
  const size_t hp_analysis_step_size_;
  std::vector<float> h_highpass_;
  CascadedBiQuadFilter hp_filter_;
  size_t h_highpass_index_ = 0;
  int delay_blocks_ = 0;
