...
void NullReceiver::OnPacket(EmulatedIpPacket packet) {}

ActionReceiver::ActionReceiver(std::function<void()> action)
    : action_(action) {}

void ActionReceiver::OnPacket(EmulatedIpPacket packet) {
  action_();
}
...
void NetworkNode::OnPacket(EmulatedIpPacket packet) {
  rtc::CritScope crit(&crit_sect_);
  ...
  if (behavior_->EnqueuePacket(
          PacketInFlightInfo(packet.data.size() + packet_overhead_,
                             packet.ingress_time.us(), packet_id))) {
...
void NetworkNode::Process(Timestamp at_time) {
  ...
  for (PacketDeliveryInfo& delivery_info : delivery_infos) {
    ...
    packet->packet.ingress_time = Timestamp::us(delivery_info.receive_time_us);
    receiver->OnPacket(std::move(packet->packet));
    {
      rtc::CritScope crit(&crit_sect_);
...
bool NetworkNodeTransport::SendRtp(const uint8_t* packet,
  ...
  send_net_->OnPacket(EmulatedIpPacket(rtc::SocketAddress() /*from*/,
                                       rtc::SocketAddress() /*to*/,
                                       receiver_id_, buffer, send_time));
  return true;
}
...
void CrossTrafficSource::Process(Timestamp at_time, TimeDelta delta) {
  ...
  if (pending_size_ > config_.min_packet_size) {
    target_->OnPacket(EmulatedIpPacket(
