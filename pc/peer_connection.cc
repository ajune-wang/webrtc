...
RTCError PeerConnection::ApplyLocalDescription(
  ...
  if (IsUnifiedPlan()) {
    for (auto transceiver : transceivers_) {
      ...
      if (content->rejected || !channel || channel->GetLocalContent().empty()) {
...
static RTCError UpdateSimulcastLayerStatusInSender(
  ...
  for (RtpEncodingParameters& encoding : parameters.encodings) {
    ...
    bool active = iter != layers.end() && !iter->is_paused;
...
GetMediaDescriptionOptionsForTransceiver(
  ...
  bool simulcast_requested = send_parameters.encodings.size() > 1;
  ...
  if (simulcast_requested) {
    sender_options.num_sim_layers = 0;  // This is non-compliant simulcast.
    ...
    // Legacy implementation relies on using the |init_send_encodings|
    // which are cleared when the media channel is configured.
