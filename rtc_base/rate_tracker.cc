...
RateTracker::RateTracker(int64_t bucket_milliseconds, uint64_t bucket_count)
    : bucket_milliseconds_(bucket_milliseconds),
      bucket_count_(bucket_count),
      sample_buckets_(new uint64_t[bucket_count + 1]),
...
double RateTracker::ComputeRateForInterval(
  ...
  uint64_t total_samples = ((sample_buckets_[start_bucket] *
...
uint64_t RateTracker::TotalSampleCount() const {
  return total_sample_count_;
}

void RateTracker::AddSamples(uint64_t sample_count) {
  EnsureInitialized();
  int64_t current_time = Time();
