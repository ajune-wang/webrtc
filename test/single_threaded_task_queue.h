...
// DEPRECATED. Please use TaskQueueForTest instead.
...
class DEPRECATED_SingleThreadedTaskQueueForTesting : public TaskQueueBase {
 public:
  ...
  TaskId PostTask(Task task) {
    return AddTask(ToQueuedTask(std::move(task)), 0);
  }
  ...
  TaskId PostDelayedTask(Task task, int64_t delay_ms) {
    return AddTask(ToQueuedTask(std::move(task)), delay_ms);
  }
  ...
  // Implement TaskQueueBase.
  void Delete() override;
  void PostTask(std::unique_ptr<webrtc::QueuedTask> task) override {
    AddTask(std::move(task), 0);
  }
  void PostDelayedTask(std::unique_ptr<webrtc::QueuedTask> task,
                       uint32_t delay_ms) override {
    AddTask(std::move(task), delay_ms);
  }

 private:
  struct QueuedTask {
    QueuedTask(TaskId task_id, std::unique_ptr<webrtc::QueuedTask> task);
    ~QueuedTask();
  ...
  TaskId AddTask(std::unique_ptr<webrtc::QueuedTask> task, int64_t delay_ms);
  ...
  // order by earliest_execution_time
  std::multimap<int64_t, QueuedTask> tasks_ RTC_GUARDED_BY(cs_);
