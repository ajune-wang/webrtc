...
TEST_F(RtpRtcpImplTest, RemoteSenderStatsNotAvailable) {
...
TEST_F(RtpRtcpImplTest, RemoteSenderStatsAvailable) {
...
TEST_F(RtpRtcpImplTest, RemoteSenderStatsNotUpdatedWithUnexpectedSsrc) {
...
TEST_F(RtpRtcpImplTest, RemoteSenderStatsCheckStatsFromLastReport) {
...
TEST_F(RtpRtcpImplTest, RemoteSenderStatsCount) {
  using RemoteSenderStats = RtpRtcpInterface::RemoteSenderStats;
  // Send a frame in order to send an SR.
  SendFrame(&sender_, sender_video_.get(), kBaseLayerTid);
  // Send the first SR.
...
// SR was sent.
TEST_F(RtpRtcpImplTest, RemoteSenderStatsArrivalTimestampSet) {
  // Send a frame in order to send an SR.
  SendFrame(&sender_, sender_video_.get(), kBaseLayerTid);
  // Send an SR.
  ASSERT_THAT(sender_.impl_->SendRTCP(kRtcpReport), Eq(0));
...
// a frame is sent.
TEST_F(RtpRtcpImplTest, RemoteSenderStatsPacketByteCounters) {
  using RemoteSenderStats = RtpRtcpInterface::RemoteSenderStats;
  // Send a frame in order to send an SR.
  SendFrame(&sender_, sender_video_.get(), kBaseLayerTid);
