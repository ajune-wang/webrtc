...
int RunRtpEncode(int argc, char* argv[]) {
  ...
  if (!absl::GetFlag(FLAGS_list_codecs) && args.size() != 2) {
  ...
  // Open files.
  printf("Input file: %s\n", args[0]);
  InputAudioFile input_file(args[0], false);  // Open input in non-looping mode.
  FILE* out_file = fopen(args[1], "wb");
  RTC_CHECK(out_file) << "Could not open file " << args[1] << " for writing";
