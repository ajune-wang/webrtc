...
class FuzzSignalInput : public NetEqInput {
 public:
  ...
  Event PopEvent() override {
    ...
    if (packet_ && packet_->time_ms <= output_event_period_ms_) {
      event_.packet_data = std::move(PopPacket());
    } else {
      event_.audio_output = std::make_unique<GetAudio>(output_event_period_ms_);
      output_event_period_ms_ += output_event_period_ms_;
    }
