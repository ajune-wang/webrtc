...
std::vector<int> GetStreamMapping(const SdpAudioFormat& format) {
  auto result = GetFormatParameter<std::vector<int>>(format, "stream_mapping");
  if (!result.has_value()) {
    // Then we can't construct a codec. How do we signal that? TODO(aleloi) fix.
...
AudioEncoderMultiChannelOpusImpl::SdpToConfig(const SdpAudioFormat& format) {
  ...
  config.num_channels = format.num_channels;  // GetChannelCount(format);
  ...
  static_cast<void>(GetStreamMapping(format));
  return config;
}
