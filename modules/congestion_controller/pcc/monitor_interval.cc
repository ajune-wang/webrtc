...
DataRate MonitorInterval::GetTransmittedPacketsRate() const {
  Timestamp receive_time_of_first_packet =
      received_packets_sent_time_.front() + received_packets_rtt_.front();
  Timestamp receive_time_of_last_packet =
      received_packets_sent_time_.back() + received_packets_rtt_.back();
  return received_packets_size_ /
         (receive_time_of_first_packet - receive_time_of_last_packet);
}
