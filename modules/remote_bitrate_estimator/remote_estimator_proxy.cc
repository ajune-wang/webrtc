...
TimeDelta GetAbsoluteSendTimeDelta(uint32_t previous_send_time,
                                   uint32_t next_send_time) {
  RTC_DCHECK_LT(next_send_time, (1ul << 24));
  RTC_DCHECK_LT(previous_send_time, (1ul << 24));
  int32_t delta =
      static_cast<int32_t>((next_send_time - previous_send_time) << 8) >> 8;
