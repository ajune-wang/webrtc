...
void PeerConnection::TransceiverStableState::set_m_section_if_unset(
...
void PeerConnection::TransceiverStableState::set_remote_stream_ids_if_unset(
    ...
    for (const auto& transceiver : transceivers_) {
      ...
      if (RtpTransceiverDirectionHasRecv(local_direction)) {
        ...
        transceiver_stable_states_by_transceivers_[transceiver]
            .set_remote_stream_ids_if_unset(
                transceiver->receiver()->stream_ids());
...
PeerConnection::AssociateTransceiver(cricket::ContentSource source,
  ...
  if (type == SdpType::kOffer) {
    ...
    if (state_changes) {
      transceiver_stable_states_by_transceivers_[transceiver]
          .set_m_section_if_unset(transceiver->internal()->mid(),
...
RTCError PeerConnection::Rollback(SdpType sdp_type) {
  ...
  for (auto&& transceivers_stable_state_pair :
    ...
    if (state.remote_stream_ids()) {
      // Assumption is that stream vectors are not cleared in
