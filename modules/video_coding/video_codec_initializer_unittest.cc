...
// Test that inconistent scalability mode settings are rejected for H.265 with
// top level scalability mode unset.
TEST_F(VideoCodecInitializerTest,
  ...
  EXPECT_EQ(codec.spatialLayers[0].numberOfTemporalLayers, 3);
  EXPECT_EQ(codec.spatialLayers[1].numberOfTemporalLayers, 1);
}
#endif
