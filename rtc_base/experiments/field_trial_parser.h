...
template <typename F,
          typename S = typename LambdaTypeTraits<F>::src,
          typename T = typename LambdaTypeTraits<F>::ret>
FieldTrialListWrapper* TLW(std::string key, F f) {
  return new TypedFieldTrialListWrapper<T>(
...
class FieldTrialStructList : public FieldTrialStructListBase {
 public:
  FieldTrialStructList(std::initializer_list<FieldTrialListWrapper*> l,
      ...
      : FieldTrialStructListBase(l),
        default_struct_(default_struct),
 ...
 protected:
  void ParseDone() override {
    ...
    std::vector<S> new_values(length, default_struct_);
