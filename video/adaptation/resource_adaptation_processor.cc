...
VideoStreamInputStateForProcessor::VideoStreamInputStateForProcessor()
    : input_pixels_(absl::nullopt),
      input_fps_(absl::nullopt),
      video_codec_type_(VideoCodecType::kVideoCodecGeneric),
      min_pixels_per_frame_(0) {}

void VideoStreamInputStateForProcessor::set_input_pixels(
    absl::optional<int> input_pixels) {
  input_pixels_ = input_pixels;
}

void VideoStreamInputStateForProcessor::set_input_fps(
    absl::optional<int> input_fps) {
  input_fps_ = input_fps;
}

void VideoStreamInputStateForProcessor::set_video_codec_type(
    VideoCodecType video_codec_type) {
  video_codec_type_ = video_codec_type;
}

void VideoStreamInputStateForProcessor::set_min_pixels_per_frame(
    int min_pixels_per_frame) {
  min_pixels_per_frame_ = min_pixels_per_frame;
}

absl::optional<int> VideoStreamInputStateForProcessor::input_pixels() const {
  return input_pixels_;
}

absl::optional<int> VideoStreamInputStateForProcessor::input_fps() const {
  return input_fps_;
}

VideoCodecType VideoStreamInputStateForProcessor::video_codec_type() const {
  return video_codec_type_;
}

int VideoStreamInputStateForProcessor::min_pixels_per_frame() const {
  return min_pixels_per_frame_;
}

bool VideoStreamInputStateForProcessor::HasPixelsAndFps() const {
  return input_pixels_.has_value() && input_fps_.has_value();
}

VideoStreamInputState VideoStreamInputStateForProcessor::SubsetForAdapter()
    const {
  RTC_DCHECK(input_pixels_.has_value() && input_fps_.has_value());
  return VideoStreamInputState(input_pixels_.value(), input_fps_.value(),
                               video_codec_type_);
}
...
VideoStreamInputStateProvider::VideoStreamInputStateProvider(
    ...
    : manager_(manager), frame_rate_provider_(frame_rate_provider) {}

VideoStreamInputStateForProcessor VideoStreamInputStateProvider::InputState() {
  rtc::CritScope lock(&crit_);
  input_state_.set_input_pixels(manager_->last_input_frame_size_.value());
  int input_fps = frame_rate_provider_->GetInputFrameRate();
  input_state_.set_input_fps(input_fps >= 0 ? absl::optional<int>(input_fps)
                                            : absl::nullopt);
  input_state_.set_video_codec_type(
      GetVideoCodecTypeOrGeneric(manager_->encoder_settings_));
  input_state_.set_min_pixels_per_frame(
      MinPixelsPerFrame(manager_->encoder_settings_));
  return input_state_;
}
...
ResourceAdaptationProcessor::ResourceAdaptationProcessor(
  ...
  // TODO(hbos): Do this outside instead. The manager should depend on the
...
bool ResourceAdaptationProcessor::HasInsufficientInput(
    const VideoStreamInputStateForProcessor& input_state) const {
  // TODO(hbos): Bake has_input_video_ into state.
  return effective_degradation_preference_ == DegradationPreference::DISABLED ||
         !has_input_video_ || !input_state.HasPixelsAndFps() ||
         (effective_degradation_preference_ ==
              DegradationPreference::MAINTAIN_RESOLUTION &&
          input_state.input_fps() < kMinFrameRateFps);
}

void ResourceAdaptationProcessor::OnResourceUnderuse(
    const Resource& reason_resource) {
  VideoStreamInputStateForProcessor input_state =
      input_state_provider_->InputState();
  if (HasInsufficientInput(input_state)) {
    return;
  }
  stream_adapter_->SetInput(input_state.SubsetForAdapter(),
  ...
  // rename CanApplyAdaptation to CanApplyAdaptationUp?
  ...
  for (const Resource* resource : resources_) {
    if (!resource->CanApplyAdaptation(stream_adapter_->input_state(),
...
bool VideoStreamEncoderResourceManager::PreventAdaptUpDueToActiveCounts::
    CanApplyAdaptation(const VideoStreamInputState& input_state,
  ...
  int num_downgrades =
      manager_->processor_->stream_adapter_
          ->FilterAdaptationCounters(manager_->active_counts_[reason])
          .Total();
...
bool VideoStreamEncoderResourceManager::
    PreventIncreaseResolutionDueToBitrateResource::CanApplyAdaptation(
        ...
        manager_->encoder_settings_.has_value()
            ? manager_->encoder_settings_->encoder_info()
                  .GetEncoderBitrateLimitsForResolution(
                      ...
                      GetHigherResolutionThan(input_state.frame_size_pixels))
            : absl::nullopt;
    if (bitrate_limits.has_value() && bitrate_bps != 0) {
      RTC_DCHECK_GE(bitrate_limits->frame_size_pixels,
                    input_state.frame_size_pixels);
      return bitrate_bps >=
...
bool VideoStreamEncoderResourceManager::PreventAdaptUpInBalancedResource::
  ...
  if (reason == AdaptationObserverInterface::AdaptReason::kQuality &&
      manager_->processor_->effective_degradation_preference() ==
          DegradationPreference::BALANCED &&
      !manager_->balanced_settings_.CanAdaptUp(
          input_state.video_codec_type, input_state.frame_size_pixels,
...
void VideoStreamEncoderResourceManager::OnFrameDroppedDueToSize() {
  ...
  processor_->OnResourceOveruse(
      ResourceWithReason(AdaptationObserverInterface::AdaptReason::kQuality));
  if (processor_->degradation_preference() == DegradationPreference::BALANCED &&
      adaptation_counters_.fps_adaptations > counters_before.fps_adaptations) {
...
void VideoStreamEncoderResourceManager::ConfigureQualityScaler(
  ...
  const bool quality_scaling_allowed =
      IsResolutionScalingEnabled(processor_->degradation_preference()) &&
...
void VideoStreamEncoderResourceManager::MaybeUpdateTargetFrameRate() {
  ...
  absl::optional<double> target_frame_rate = restrictions_.max_frame_rate();
...
const Resource& VideoStreamEncoderResourceManager::ResourceWithReason(
    AdaptationObserverInterface::AdaptReason reason) const {
  const auto& registered_resource = absl::c_find_if(
      resources_,
      [&reason](const ResourceAndReason& r) { return r.reason == reason; });
  RTC_DCHECK(registered_resource != resources_.end());
...
void VideoStreamEncoderResourceManager::UpdateAdaptationStats(
  ...
  const AdaptationCounters total_counts = adaptation_counters_;
...
VideoStreamEncoderResourceManager::GetActiveCounts(
  ...
  switch (reason) {
    ...
    // VideoStreamAdapter::FilterAdaptationCounters().
    case AdaptationObserverInterface::AdaptReason::kCpu:
      if (!IsFramerateScalingEnabled(processor_->degradation_preference()))
        counts.num_framerate_reductions = absl::nullopt;
      if (!IsResolutionScalingEnabled(processor_->degradation_preference()))
