...
std::string EchoCanceller3Config::ToString() const {
  rtc::StringBuilder ost;
  ost << "{";
  ost << "\"delay\": {";
  ost << "\"default_delay\": " << delay.default_delay << ",";
  ost << "\"down_sampling_factor\": " << delay.down_sampling_factor << ",";
  ost << "\"num_filters\": " << delay.num_filters << ",";
  ost << "\"api_call_jitter_blocks\": " << delay.api_call_jitter_blocks << ",";
  ost << "\"min_echo_path_delay_blocks\": " << delay.min_echo_path_delay_blocks
      << ",";
  ost << "\"delay_headroom_blocks\": " << delay.delay_headroom_blocks << ",";
  ost << "\"hysteresis_limit_1_blocks\": " << delay.hysteresis_limit_1_blocks
      << ",";
  ost << "\"hysteresis_limit_2_blocks\": " << delay.hysteresis_limit_2_blocks
      << ",";
  ost << "\"skew_hysteresis_blocks\": " << delay.skew_hysteresis_blocks << ",";
  ost << "\"fixed_capture_delay_samples\": "
      << delay.fixed_capture_delay_samples << ",";
  ost << "\"delay_estimate_smoothing\": " << delay.delay_estimate_smoothing
      << ",";
  ost << "\"delay_candidate_detection_threshold\": "
      << delay.delay_candidate_detection_threshold << ",";

  ost << "\"delay_selection_thresholds\": {";
  ost << "\"initial\": " << delay.delay_selection_thresholds.initial << ",";
  ost << "\"converged\": " << delay.delay_selection_thresholds.converged;
  ost << "}";
  ...
  ost << "\"filter\": {";
  ost << "\"main\": [";
  ost << filter.main.length_blocks << ",";
  ost << filter.main.leakage_converged << ",";
  ost << filter.main.leakage_diverged << ",";
  ost << filter.main.error_floor << ",";
  ost << filter.main.noise_gate;
  ost << "],";

  ost << "\"shadow\": [";
  ost << filter.shadow.length_blocks << ",";
  ost << filter.shadow.rate << ",";
  ost << filter.shadow.noise_gate;
  ost << "],";

  ost << "\"main_initial\": [";
  ost << filter.main_initial.length_blocks << ",";
  ost << filter.main_initial.leakage_converged << ",";
  ost << filter.main_initial.leakage_diverged << ",";
  ost << filter.main_initial.error_floor << ",";
  ost << filter.main_initial.noise_gate;
  ost << "],";

  ost << "\"shadow_initial\": [";
  ost << filter.shadow_initial.length_blocks << ",";
  ost << filter.shadow_initial.rate << ",";
  ost << filter.shadow_initial.noise_gate;
  ost << "],";

  ost << "\"config_change_duration_blocks\": "
      << filter.config_change_duration_blocks << ",";
  ost << "\"initial_state_seconds\": " << filter.initial_state_seconds << ",";
  ost << "\"conservative_initial_phase\": "
      << (filter.conservative_initial_phase ? "true" : "false") << ",";
  ost << "\"enable_shadow_filter_output_usage\": "
      << (filter.enable_shadow_filter_output_usage ? "true" : "false");

  ost << "},";

  ost << "\"erle\": {";
  ost << "\"min\": " << erle.min << ",";
  ost << "\"max_l\": " << erle.max_l << ",";
  ost << "\"max_h\": " << erle.max_h << ",";
  ost << "\"onset_detection\": " << (erle.onset_detection ? "true" : "false");
  ost << "},";

  ost << "\"ep_strength\": {";
  ost << "\"lf\": " << ep_strength.lf << ",";
  ost << "\"mf\": " << ep_strength.mf << ",";
  ost << "\"hf\": " << ep_strength.hf << ",";
  ost << "\"default_len\": " << ep_strength.default_len << ",";
  ost << "\"reverb_based_on_render\": "
      << (ep_strength.reverb_based_on_render ? "true" : "false") << ",";
  ost << "\"echo_can_saturate\": "
      << (ep_strength.echo_can_saturate ? "true" : "false") << ",";
  ost << "\"bounded_erl\": " << (ep_strength.bounded_erl ? "true" : "false");
  ...
  ost << "\"m8\": " << gain_mask.m8 << ",";
  ost << "\"m9\": " << gain_mask.m9 << ",";
  ost << "\"gain_curve_offset\": " << gain_mask.gain_curve_offset << ",";
  ost << "\"gain_curve_slope\": " << gain_mask.gain_curve_slope << ",";
  ost << "\"temporal_masking_lf\": " << gain_mask.temporal_masking_lf << ",";
  ost << "\"temporal_masking_hf\": " << gain_mask.temporal_masking_hf << ",";
  ost << "\"temporal_masking_lf_bands\": "
      << gain_mask.temporal_masking_lf_bands;
  ost << "},";

  ost << "\"echo_audibility\": {";
  ost << "\"low_render_limit\": " << echo_audibility.low_render_limit << ",";
  ost << "\"normal_render_limit\": " << echo_audibility.normal_render_limit
      << ",";
  ost << "\"floor_power\": " << echo_audibility.floor_power << ",";
  ...
  ost << "\"use_stationary_properties\": "
      << (echo_audibility.use_stationary_properties ? "true" : "false") << ",";
  ost << "\"use_stationarity_properties_at_init\": "
      << (echo_audibility.use_stationarity_properties_at_init ? "true"
                                                              : "false");
  ost << "},";

  ost << "\"render_levels\": {";
  ost << "\"active_render_limit\": " << render_levels.active_render_limit
      << ",";
  ...
  ost << "\"gain_rampup\": {";
  ost << "\"initial_gain\": " << echo_removal_control.gain_rampup.initial_gain
      << ",";
  ost << "\"first_non_zero_gain\": "
      << echo_removal_control.gain_rampup.first_non_zero_gain << ",";
  ost << "\"non_zero_gain_blocks\": "
      << echo_removal_control.gain_rampup.non_zero_gain_blocks << ",";
  ost << "\"full_gain_blocks\": "
      << echo_removal_control.gain_rampup.full_gain_blocks;
  ost << "},";
  ost << "\"has_clock_drift\": "
      << (echo_removal_control.has_clock_drift ? "true" : "false") << ",";
  ost << "\"linear_and_stable_echo_path\": "
      << (echo_removal_control.linear_and_stable_echo_path ? "true" : "false");

  ost << "},";

  ost << "\"echo_model\": {";
  ost << "\"noise_floor_hold\": " << echo_model.noise_floor_hold << ",";
  ost << "\"min_noise_floor_power\": " << echo_model.min_noise_floor_power
      << ",";
  ost << "\"stationary_gate_slope\": " << echo_model.stationary_gate_slope
      << ",";
  ost << "\"noise_gate_power\": " << echo_model.noise_gate_power << ",";
  ost << "\"noise_gate_slope\": " << echo_model.noise_gate_slope << ",";
  ost << "\"render_pre_window_size\": " << echo_model.render_pre_window_size
      << ",";
  ost << "\"render_post_window_size\": " << echo_model.render_post_window_size
      << ",";
  ...
  ost << "\"nonlinear_hold\": " << echo_model.nonlinear_hold << ",";
  ost << "\"nonlinear_release\": " << echo_model.nonlinear_release;
  ost << "},";

  ost << "\"suppressor\": {";
  ost << "\"nearend_average_blocks\": " << suppressor.nearend_average_blocks
      << ",";
  ost << "\"normal_tuning\": {";
  ost << "\"mask_lf\": [";
  ost << suppressor.normal_tuning.mask_lf.enr_transparent << ",";
  ost << suppressor.normal_tuning.mask_lf.enr_suppress << ",";
  ost << suppressor.normal_tuning.mask_lf.emr_transparent;
  ost << "],";
  ost << "\"mask_hf\": [";
  ost << suppressor.normal_tuning.mask_hf.enr_transparent << ",";
  ost << suppressor.normal_tuning.mask_hf.enr_suppress << ",";
  ost << suppressor.normal_tuning.mask_hf.emr_transparent;
  ost << "],";
  ost << "\"max_inc_factor\": " << suppressor.normal_tuning.max_inc_factor
      << ",";
  ...
  ost << "\"nearend_tuning\": {";
  ost << "\"mask_lf\": [";
  ost << suppressor.nearend_tuning.mask_lf.enr_transparent << ",";
  ost << suppressor.nearend_tuning.mask_lf.enr_suppress << ",";
  ost << suppressor.nearend_tuning.mask_lf.emr_transparent;
  ost << "],";
  ost << "\"mask_hf\": [";
  ost << suppressor.nearend_tuning.mask_hf.enr_transparent << ",";
  ost << suppressor.nearend_tuning.mask_hf.enr_suppress << ",";
  ost << suppressor.nearend_tuning.mask_hf.emr_transparent;
  ost << "],";
  ost << "\"max_inc_factor\": " << suppressor.nearend_tuning.max_inc_factor
      << ",";
  ost << "\"max_dec_factor_lf\": "
  ...
  ost << "},";
  ost << "\"floor_first_increase\": " << suppressor.floor_first_increase << ",";
  ost << "\"enforce_transparent\": "
      << (suppressor.enforce_transparent ? "true" : "false") << ",";
  ost << "\"enforce_empty_higher_bands\": "
      << (suppressor.enforce_empty_higher_bands ? "true" : "false");
  ost << "}";
  ost << "}";
...
EchoCanceller3Config EchoCanceller3Config::FromJson(
