...
class DevNullReceiver : public EmulatedNetworkReceiverInterface {
 public:
...
CrossTraffic::CrossTraffic(Clock* clock,
  ...
  dev_null_receiver_ = absl::make_unique<DevNullReceiver>();
  absl::optional<uint16_t> port =
      endpoint_->BindReceiver(0, dev_null_receiver_.get());
  RTC_DCHECK(port);
  dev_null_receiver_port_ = port.value();
}
...
void CrossTraffic::TriggerPacketBurst(size_t num_packets, size_t packet_size) {
  for (size_t i = 0; i < num_packets; ++i) {
    receiver_->OnPacketReceived(EmulatedIpPacket(
        ...
        /*to=*/
        rtc::SocketAddress(endpoint_->GetPeerLocalAddress(),
                           dev_null_receiver_port_),
        endpoint_->GetId(), rtc::CopyOnWriteBuffer(packet_size),
        Timestamp::us(clock_->TimeInMicroseconds())));
  }
}

void CrossTraffic::NetworkDelayedAction(size_t packet_size,
  ...
  receiver_->OnPacketReceived(EmulatedIpPacket(
      /*from=*/rtc::SocketAddress(),
      /*to=*/
      rtc::SocketAddress(endpoint_->GetPeerLocalAddress(), port.value()),
      endpoint_->GetId(), rtc::CopyOnWriteBuffer(packet_size),
      Timestamp::us(clock_->TimeInMicroseconds())));
}
...
rtc::SocketAddress CrossTraffic::GetDefaultAddress() const {
  return rtc::SocketAddress(endpoint_->GetPeerLocalAddress(),
                            dev_null_receiver_port_);
}
...
void RandomWalkCrossTraffic::Process(Timestamp at_time) {
  ...
  if (pending_size_ >= config_.min_packet_size &&
    ...
    cross_traffic_->receiver_->OnPacketReceived(EmulatedIpPacket(
        /*from=*/rtc::SocketAddress(),
        /*to=*/cross_traffic_->GetDefaultAddress(),
        cross_traffic_->GetDestinationId(),
