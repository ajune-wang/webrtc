 ...
 protected:
  ...
  void SetUp() override {
    ...
    controller_.reset(new SendSideCongestionControllerForTest(
        &clock_, &event_log_, pacer_.get(), InitialConstraints()));
  ...
  BitrateConstraints InitialConstraints() {
    BitrateConstraints constraints;
    constraints.min_bitrate_bps = 0;
    constraints.max_bitrate_bps = 5 * kInitialBitrateBps;
    constraints.start_bitrate_bps = kInitialBitrateBps;
    return constraints;
  }
