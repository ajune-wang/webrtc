...
// Enabled by EnableTimeCallbackForTesting should never set in production code.
...
bool OpenSSLStreamAdapter::SetDtlsSrtpCryptoSuites(
  ...
  for (const int cipher : ciphers) {
    bool found = false;
    for (size_t i = 0; i < arraysize(kSrtpCipherMap); ++i) {
      if (cipher == kSrtpCipherMap[i].id) {
        found = true;
        if (!internal_ciphers.empty()) {
          internal_ciphers += ":";
        }
        internal_ciphers += kSrtpCipherMap[i].internal_name;
        break;
      }
    }
