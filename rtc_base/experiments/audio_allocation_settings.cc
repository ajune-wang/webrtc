...
bool AudioAllocationSettings::include_audio_in_feedback() const {
...
bool AudioAllocationSettings::allocate_audio() const {
  return audio_send_side_bwe_;
}
...
bool AudioAllocationSettings::update_audio_target_bitrate(
  ...
  if (allocate_audio_without_feedback_ || has_transport_sequence_number_id)
    return true;
...
bool AudioAllocationSettings::include_audio_in_allocation_on_start(
    ...
    bool has_dscp,
    bool has_transport_sequence_number_id) const {
  if (has_dscp || !min_rate_set || !max_rate_set)
    return false;
  if (has_transport_sequence_number_id && !force_no_audio_feedback_)
    return true;
