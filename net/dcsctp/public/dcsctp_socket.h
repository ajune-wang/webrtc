...
class DcSctpSocketCallbacks {
 public:
  ...
  //
  // The timeout should have no worse precision than specified. Low precision
  // tasks are scheduled more efficiently by using leeway to reduce Idle Wake
  // Ups, but may fire with up to 17 ms additional delay. kHigh does not have
  // this leeway, but is still limited by OS timer precision. See
