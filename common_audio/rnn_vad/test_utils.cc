...
CreateLpResidualReader() {
  ...
  return {std::move(ptr), rtc::CheckedDivExact(ptr->data_length(), 866ul)};
}
...
CreateBandEnergyCoeffsReader() {
  ...
  return {std::move(ptr), rtc::CheckedDivExact(ptr->data_length(), 22ul)};
}
...
CreateFeatureMatrixReader() {
  ...
  return {std::move(ptr), rtc::CheckedDivExact(ptr->data_length(), 43ul)};
}
