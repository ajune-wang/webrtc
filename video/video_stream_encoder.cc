...
VideoLayersAllocation CreateVideoLayersAllocation(
  ...
  if (encoder_config.numberOfSimulcastStreams > 1) {
    ...
    // TODO(bugs.webrtc.org/12000): Implement support for other codecs with
    // scalability.
    ...
    for (int si = 0; si < webrtc::kMaxSpatialLayers; ++si) {
      ...
      // Encoder may drop frames internally if `maxFramerate` is set.
      spatial_layer.frame_rate_fps = std::min(
          static_cast<uint8_t>(encoder_config.spatialLayers[si].maxFramerate),
          static_cast<uint8_t>(
              (current_rate.framerate_fps * frame_rate_fraction) /
              VideoEncoder::EncoderInfo::kMaxFramerateFraction));
      RTC_LOG(INFO) << " set frame rate" << spatial_layer.frame_rate_fps
