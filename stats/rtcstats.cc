...
std::string VectorToString(const std::vector<T>& vector) {
  if (vector.empty())
    return "[]";
  std::string s = absl::StrCat("[", rtc::ToString(vector[0]));
  for (size_t i = 1; i < vector.size(); ++i) {
    absl::StrAppend(&s, ",", rtc::ToString(vector[i]));
  }
  absl::StrAppend(&s, "]");
  return s;
}
...
std::string VectorOfStringsToString(const std::vector<T>& strings) {
  if (strings.empty())
    return "[]";
  std::string s = absl::StrCat("[\"", rtc::ToString(strings[0]), "\"");
  for (size_t i = 1; i < strings.size(); ++i) {
    absl::StrAppend(&s, ",\"", rtc::ToString(strings[i]), "\"");
  }
  absl::StrAppend(&s, "]");
  return s;
}
...
std::string ToStringAsDouble(const T value) {
  ...
  // digits of precision.
  snprintf(buf, kBufferLength, "%.16f", static_cast<double>(value));
