...
void TestGatedRecurrentLayer(
  ...
  for (size_t i = 0; i < input_sequence_length; ++i) {
    ...
    ExpectNear(expected_output, gru_output_view);
  }
}
...
TEST(RnnVad, CheckValidVadProbabilities) {
  ...
  constexpr size_t kInputAudioFrameSize = 2 * kInputFrameSize;  // 48 kHz.
  ...
  const size_t num_frames = samples_reader.data_length() / kInputAudioFrameSize;
  for (size_t i = 0; i < num_frames; ++i) {
    samples_reader.ReadChunk({samples.data(), samples.size()});
    Decimate48k24k({samples.data(), samples.size()},
