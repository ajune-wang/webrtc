...
void RunApmAgcLimiter(AudioFrameView<float> mixing_buffer_view,
  ...
  const int sample_rate = mixing_buffer_view.samples_per_channel() * 1000 /
...
void InterleaveToAudioFrame(AudioFrameView<const float> mixing_buffer_view,
                            AudioFrame* audio_frame_for_mixing) {
  const int number_of_channels = mixing_buffer_view.num_channels();
  const int samples_per_channel = mixing_buffer_view.samples_per_channel();
  // Put data to the result frame.
...
void FrameCombiner::Combine(const std::vector<AudioFrame*>& mix_list,
  ...
  // Frames could have 'num_channels_' different from
  // 'number_of_channels'.
