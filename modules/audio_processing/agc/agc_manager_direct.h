...
class AgcManagerDirect final {
 public:
  ...
  // `set_stream_analog_level()` call. Undefined behavior if called before
  // `AnalyzePreProcess()` or before `Process()`.
 ...
 private:
  ...
  // TODO(bugs.webrtc.org/7494): Create a separate member for the stream input
  // volume.
  ...
  // holds the observed stream input volume. Possibly updated by
