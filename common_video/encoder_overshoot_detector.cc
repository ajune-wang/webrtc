...
EncoderOvershootDetector::EncoderOvershootDetector()
    : time_last_update_ms_(-1),
      ...
      overall_rate_(kWindowSizeMs, 3000),
      overshoot_rate_(kWindowSizeMs, 3000),
...
void EncoderOvershootDetector::OnEncodedFrame(size_t bytes, int64_t time_ms) {
  LeakBits(time_ms);
  int64_t ideal_frame_size = IdealFrameSizeBytes();
  ...
  overall_rate_.Update(bytes * 8, time_ms);
}
...
absl::optional<double> EncoderOvershootDetector::GetUtilizationFactor(
  ...
  LeakBits(time_ms);
  absl::optional<uint32_t> overall_rate_bps = overall_rate_.Rate(time_ms);
  absl::optional<uint32_t> overshoot_rate_bps = overshoot_rate_.Rate(time_ms);
  if (overall_rate_bps && overshoot_rate_bps) {
    return static_cast<double>(*overall_rate_bps) /
           (*overall_rate_bps - *overshoot_rate_bps);
  }
  return absl::nullopt;
}

int64_t EncoderOvershootDetector::IdealFrameSizeBytes() const {
  if (target_framerate_fps_ <= 0) {
    return 0;
  }

  return (target_bitrate_.bps_or(0) * 8) / target_framerate_fps_;
}
