...
constexpr double kMsToSeconds = 1.0 / 1000.0;
...
OSStatus AudioDeviceIOS::OnGetPlayoutData(AudioUnitRenderActionFlags* flags,
  ...
  if (last_hw_output_latency_update_sample_count_ > playout_parameters_.sample_rate() * 5) {
    // We update the hardware output latency every 5 seconds.
  ...
  double output_latency_ = hw_output_latency_.load(std::memory_order_relaxed) +
      1e-3 * playout_parameters_.GetBufferSizeInMilliseconds();
  ...
  total_playout_delay_ms_.fetch_add(output_latency_ * 1e3 * num_frames, std::memory_order_relaxed);
  return noErr;
}
...
std::optional<AudioDeviceModule::Stats> AudioDeviceIOS::GetStats() const {
  ...
  AudioDeviceModule::Stats playout_stats = {
      .synthesized_samples_duration_s =
          kMsToSeconds * total_playout_glitches_duration_ms_.load(std::memory_order_acquire),
      .synthesized_samples_events = num_detected_playout_glitches_.load(std::memory_order_acquire),
      .total_samples_duration_s =
          kMsToSeconds * total_playout_samples_duration_ms_.load(std::memory_order_acquire),
      .total_playout_delay_s =
          kMsToSeconds * total_playout_delay_ms_.load(std::memory_order_acquire),
