...
std::unique_ptr<AudioEncoderOpusStates> CreateCodec(int input_sample_rate_hz,
                                                    size_t num_channels) {
...
class AudioEncoderOpusTest : public ::testing::TestWithParam<int> {
 protected:
  int input_sample_rate_hz_{GetParam()};
};
...
TEST_P(AudioEncoderOpusTest, DefaultApplicationModeMono) {
  auto states = CreateCodec(input_sample_rate_hz_, 1);
...
TEST_P(AudioEncoderOpusTest, DefaultApplicationModeStereo) {
  auto states = CreateCodec(input_sample_rate_hz_, 2);
...
TEST_P(AudioEncoderOpusTest, ConfigBandwidthAdaptation) {
  AudioEncoderOpusConfig config;
  const size_t opus_rate_khz =
      rtc::CheckedDivExact(input_sample_rate_hz_, 1000);
...
TEST_P(AudioEncoderOpusTest, OpusFlagDtxAsNonSpeech) {
  ...
  config.input_sample_rate_hz = input_sample_rate_hz_;
  ...
  const size_t max_loop_length_samples =
      input_sample_rate_hz_ * 10;  // Max 10 second loop.
  const size_t input_block_size_samples =
      10 * input_sample_rate_hz_ / 1000;  // 10 ms.
