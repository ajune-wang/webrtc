...
ResourceAdaptationProcessor::~ResourceAdaptationProcessor() {
  ...
  RTC_DCHECK(processor_listeners_.empty())
      << "There are processor listener(s) depending on a "
  ...
  RTC_DCHECK(resources_.empty())
      ...
      << "being destroyed.";
  // TODO(hbos): BEFORE LANDING - DCHECK that everything is cleaned up.
}
...
void ResourceAdaptationProcessor::AddProcessorListener(
    ResourceAdaptationProcessorListener* processor_listener) {
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  RTC_DCHECK(std::find(processor_listeners_.begin(), processor_listeners_.end(),
                       processor_listener) == processor_listeners_.end());
  processor_listeners_.push_back(processor_listener);
}

void ResourceAdaptationProcessor::RemoveProcessorListener(
    ResourceAdaptationProcessorListener* processor_listener) {
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  auto it = std::find(processor_listeners_.begin(), processor_listeners_.end(),
                      processor_listener);
  RTC_DCHECK(it != processor_listeners_.end());
  processor_listeners_.erase(it);
}
...
void ResourceAdaptationProcessor::MaybeUpdateVideoSourceRestrictions(
  ...
  if (last_reported_source_restrictions_ != new_source_restrictions) {
    ...
    for (auto* processor_listener : processor_listeners_) {
      processor_listener->OnVideoSourceRestrictionsUpdated(
          last_reported_source_restrictions_,
...
ResourceAdaptationProcessor::OnResourceUnderuse(
  ...
  for (const auto* adaptation_constraint : adaptation_constraints_) {
    if (!adaptation_constraint->IsAdaptationUpAllowed(
            input_state, restrictions_before, restrictions_after,
            reason_resource)) {
      processing_in_progress_ = false;
      rtc::StringBuilder message;
      message << "Not adapting up because constraint \""
              << adaptation_constraint->Name() << "\" disallowed it";
      return MitigationResultAndLogMessage(
