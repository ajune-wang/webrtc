...
void PacketRouter::SendPacket(std::unique_ptr<RtpPacketToSend> packet,
  ...
  // If congestion control feedback using RFC 8888 is enabled, a transport
  // sequence number is created for all RTP packets, (but not sent in the RTP
  // packet). Otherwise, the transport sequence number is only created if the
  // TransportSequenceNumber header extension is negotiated for the specific
  // media type.
