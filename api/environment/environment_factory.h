...
class RTC_EXPORT EnvironmentFactory final {
 ...
 private:
  ...
  template <typename T>
  void Store(absl::Nonnull<std::unique_ptr<T>> ptr);
...
template <typename T>
void EnvironmentFactory::Store(absl::Nonnull<std::unique_ptr<T>> ptr) {
  ...
  leaf_ = rtc::make_ref_counted<StorageNode>(leaf_, std::move(ptr));
}

template <typename T>
EnvironmentFactory& EnvironmentFactory::With(
    absl::Nullable<std::unique_ptr<T>> ptr) {
  if (ptr != nullptr) {
    Set(ptr.get());
    Store(std::move(ptr));
  }
...
template <typename T>
EnvironmentFactory& EnvironmentFactory::With(absl::Nullable<T*> ptr) {
  if (ptr != nullptr) {
    Set(ptr);
  }
  return *this;
