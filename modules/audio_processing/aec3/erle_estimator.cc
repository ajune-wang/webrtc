...
void ErleEstimator::Update(rtc::ArrayView<const float> render_spectrum,
  ...
  auto erle_band_update = [](float& erle_band, float new_erle, float alpha_inc,
    ...
    erle_band += alpha * (new_erle - erle_band);
    erle_band = rtc::SafeClamp(erle_band, min_erle, max_erle);
  };
    ...
    for (size_t k = start; k < stop; ++k) {
      if (X2[k] > kX2Min && E2[k] > 0.f) {
        ...
        if (coming_onset_[k - 1]) {
          hold_counters_[k - 1] = 104;
          coming_onset_[k - 1] = false;
        }
        if (hold_counters_[k - 1] > 100) {
          erle_band_update(erle_onsets_[k], new_erle, 0.05f, 0.1f, min_erle_,
                           max_erle);
        } else {
          hold_counters_[k - 1] = 100;
        }
        erle_band_update(erle_[k], new_erle, 0.01f, 0.02f, min_erle_, max_erle);
      }
    }
  };
