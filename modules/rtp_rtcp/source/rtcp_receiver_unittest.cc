...
// Same test as above but enable receive-side RTT using the setter instead of
// the config struct.
TEST(RtcpReceiverTest, SetterEnablesReceiverRtt) {
  ReceiverMocks mocks;
  auto config = DefaultConfiguration(&mocks);
  // Disable non-sender RTT in the config.
  config.non_sender_rtt_measurement = false;
  RTCPReceiver receiver(config, &mocks.rtp_rtcp_impl);
  receiver.SetRemoteSSRC(kSenderSsrc);
  // Enable non-sender RTT using the setter.
  receiver.SetRecvRttEnabled(true);
...
// Same test as above but enable receive-side RTT using the setter instead of
// the config struct.
TEST(RtcpReceiverTest, SetterDisablesReceiverRtt) {
  ReceiverMocks mocks;
  auto config = DefaultConfiguration(&mocks);
  // Enable non-sender RTT in the config.
  config.non_sender_rtt_measurement = true;
  RTCPReceiver receiver(config, &mocks.rtp_rtcp_impl);
  receiver.SetRemoteSSRC(kSenderSsrc);
  // Disable non-sender RTT using the setter.
  receiver.SetRecvRttEnabled(false);
