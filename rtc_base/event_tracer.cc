...
std::atomic<bool> g_event_logging_active(false);
...
class EventLogger final {
 public:
  ...
  void Start(FILE* file, bool owned) {
    ...
    RTC_CHECK(!g_event_logging_active.exchange(true));
  ...
  void Stop() {
    ...
    if (!g_event_logging_active.exchange(false))
      return;
...
void InternalAddTraceEvent(char phase,
  ...
  if (!g_event_logging_active.load(std::memory_order_acquire))
    return;
...
void ShutdownInternalTracer() {
  StopInternalCapture();
  EventLogger* old_logger = g_event_logger.exchange(nullptr);
