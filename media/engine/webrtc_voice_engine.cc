...
bool WebRtcVoiceMediaChannel::SetOutputVolume(uint32_t ssrc, double volume) {
  RTC_DCHECK(worker_thread_checker_.IsCurrent());
  LogRtcMessage(rtc::LS_INFO, "%s({ssrc=%u}, {volume=%.2f})", __func__, ssrc,
                volume);
  const auto it = recv_streams_.find(ssrc);
  if (it == recv_streams_.end()) {
    LogRtcMessage(rtc::LS_WARNING,
                  "%s => (WARNING: no receive stream for SSRC %u)", __func__,
                  ssrc);
    return false;
  }
  it->second->SetOutputVolume(volume);
  LogRtcMessage(rtc::LS_INFO,
                "%s => (stream with SSRC %u now uses volume %.2f)", __func__,
                ssrc, volume);
  return true;
}
