...
// Parses a comma-separated string "1,2,-5,6" into a std::vector<int>.
template <>
absl::optional<std::vector<int>> GetFormatParameter(
  ...
  while (pos < comma_separated_list.size()) {
    const size_t nxt_comma = comma_separated_list.find(',', pos);
    const size_t dist_to_nxt_comma =
        nxt_comma == std::string::npos ? std::string::npos : (nxt_comma - pos);
    auto substring_with_number =
        comma_separated_list.substr(pos, dist_to_nxt_comma);
