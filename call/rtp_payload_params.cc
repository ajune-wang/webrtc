...
RtpPayloadParams::RtpPayloadParams(const uint32_t ssrc,
    ...
    : ssrc_(ssrc) {
  memset(&frame_state_, -1, sizeof(frame_state_));
...
RtpPayloadParams::RtpPayloadParams(const RtpPayloadParams& other)
    : frame_state_(other.frame_state_),
      ssrc_(other.ssrc_),
      state_(other.state_),
      generic_picture_id_experiment_(other.generic_picture_id_experiment_) {}
...
RTPVideoHeader RtpPayloadParams::GetRtpVideoHeader(
  ...
  SetGeneric(&rtp_video_header, shared_frame_id, is_keyframe);
...
void RtpPayloadParams::SetCodecSpecific(RTPVideoHeader* rtp_video_header,
  ...
  // TODO(philipel): Remove this whe the new generic format has been fully
  //                 implemented.
...
void RtpPayloadParams::SetGeneric(RTPVideoHeader* rtp_video_header,
                                  int64_t frame_id,
                                  bool is_keyframe) {
  if (rtp_video_header->codec == kVideoCodecVP8) {
    Vp8ToGeneric(rtp_video_header, frame_id, is_keyframe);
  }
...
void RtpPayloadParams::Vp8ToGeneric(RTPVideoHeader* rtp_video_header,
                                    int64_t shared_frame_id,
                                    bool is_keyframe) {
  ...
  if (is_keyframe) {
    ...
    memset(&frame_state_, -1, sizeof(frame_state_));
    frame_state_[spatial_index][temporal_index] = shared_frame_id;
    return;
  }

  if (vp8_header.layerSync) {
    int64_t tl0_frame_id = frame_state_[spatial_index][0];

    for (int i = 1; i <= RtpGenericFrameDescriptor::kMaxTemporalLayers; ++i) {
      if (frame_state_[spatial_index][i] < tl0_frame_id) {
        frame_state_[spatial_index][i] = -1;
      }
    }

    RTC_DCHECK_NE(tl0_frame_id, -1);
    RTC_DCHECK_LT(tl0_frame_id, shared_frame_id);
    rtp_video_header->dependencies.push_back(tl0_frame_id);
  } else {
    for (int i = 0; i <= temporal_index; ++i) {
      int64_t frame_id = frame_state_[spatial_index][i];

      if (frame_id != -1) {
        RTC_DCHECK_LT(frame_id, shared_frame_id);
        rtp_video_header->dependencies.push_back(frame_id);
      }
    }
  }
