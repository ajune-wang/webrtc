...
TEST_F(PeerConnectionUsageHistogramTest,
       FingerprintWithPrivateIPNonTrickledCaller) {
  auto caller = CreatePeerConnectionWithPrivateLocalAddresses();
  auto callee = CreatePeerConnection();
  caller->AddAudioTrack("audio");
  ASSERT_TRUE(caller->ConnectTo(callee.get()));
...
// Test the usage pattern bits for adding remote private or mDNS candidates are
// set when the remote candidates are retrieved from Offer/Answer SDP instead of
// trickled ICE messages.
TEST_F(PeerConnectionUsageHistogramTest,
       AddRemoteCandidatesFromRemoteDescription) {
  // We construct the following data-channel-only scenario. The caller collects
  // private local candidates and append them in the Offer as in non-trickled
