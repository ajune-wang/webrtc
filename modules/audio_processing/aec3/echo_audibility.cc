...
void EchoAudibility::Update(const RenderBuffer& render_buffer,
  ...
  size_t render_block_number = capture_block_counter - delay_blocks;
  ...
  for (size_t k = 0; k < (num_lookahead + 1); ++k) {
    ...
    render_stationarity_.Update(render_buffer.Spectrum(delay_blocks - k),
                                render_block_number + k, k == 0);
  }
  render_stationarity_.UpdateFlagsStationaryAhead(render_block_number,
                                                  num_lookahead);
}
