...
void RenderDelayBufferImpl::Reset() {
  ...
  const int offset3 = static_cast<int>(offset1 * sub_block_size_);
  ds_render_.read = ds_render_.OffsetIndex(ds_render_.write, offset3);
...
RenderDelayBufferImpl::BufferingEvent RenderDelayBufferImpl::Insert(
  ...
  if (ds_render_.read == ds_render_.write || blocks_.read == blocks_.write) {
...
RenderDelayBufferImpl::PrepareCaptureCall() {
  ...
  if (ds_render_.read == ds_render_.write || blocks_.read == blocks_.write) {
    // Render underrun. Insert zero block.
    event = BufferingEvent::kRenderUnderrun;
    const size_t previous_write = blocks_.write;
    IncreaseInsert();

    for (size_t k = 0; k < blocks_.buffer[blocks_.write].size(); ++k) {
      std::fill(blocks_.buffer[blocks_.write][k].begin(),
                blocks_.buffer[blocks_.write][k].end(), 0.f);
    }
    UpdateBuffersWithLatestBlock(previous_write);
  }
...
void RenderDelayBufferImpl::UpdateBuffersWithLatestBlock(
  ...
  std::copy(render_ds_.rbegin(), render_ds_.rend(),
            ds_render_.buffer.begin() + ds_render_.write);
...
void RenderDelayBufferImpl::IncreaseRead() {
  ds_render_.UpdateReadIndex(-static_cast<int>(sub_block_size_));
...
void RenderDelayBufferImpl::IncreaseInsert() {
  ds_render_.UpdateWriteIndex(-static_cast<int>(sub_block_size_));
