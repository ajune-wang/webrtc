...
std::pair<absl::optional<FrameInformation>, size_t>
VCMDecodedFrameCallback::FindFrameInfo(uint32_t rtp_timestamp) {
  auto it = timestamp_map_.begin();
  absl::optional<FrameInformation> frame_info;
  size_t dropped_frames = 0;
  while (it != timestamp_map_.end() && !frame_info) {
    // The timestamp we are looking for is older than oldest timestamp in the
    // list and so it is not present.
    if (IsNewerTimestamp(it->rtp_timestamp, rtp_timestamp)) {
      break;
    }
    if (it->rtp_timestamp == rtp_timestamp) {
      frame_info = std::move(it->frame_info);
    } else {
      ++dropped_frames;
    }
    ++it;
  }
  timestamp_map_.erase(timestamp_map_.begin(), it);
  return std::make_pair(frame_info, dropped_frames);
}
...
void VCMDecodedFrameCallback::Decoded(VideoFrame& decodedImage,
  ...
  int dropped_frames = 0;
  {
    ...
    timestamp_map_size = timestamp_map_.size();
  }
  if (dropped_frames > 0) {
...
void VCMDecodedFrameCallback::Map(uint32_t timestamp,
  ...
  {
    MutexLock lock(&lock_);
    int initial_size = timestamp_map_.size();
    if (initial_size == kDecoderFrameMemoryLength) {
      timestamp_map_.pop_front();
      dropped_frames = 1;
    }
    timestamp_map_.emplace_back(timestamp, frameInfo);
...
void VCMDecodedFrameCallback::ClearTimestampMap() {
  ...
  {
    MutexLock lock(&lock_);
    dropped_frames = timestamp_map_.size();
