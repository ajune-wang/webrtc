...
void LogStartCaptureResult(StartCaptureResult error) {
...
void LogGetFrameResult(GetFrameResult error) {
  RTC_HISTOGRAM_ENUMERATION(
      "WebRTC.DesktopCapture.WgcCaptureSessionGetFrameResult",
...
HRESULT WgcCaptureSession::StartCapture() {
  ...
  if (item_closed_) {
    ...
    LogStartCaptureResult(StartCaptureResult::kSourceClosed);
    return E_ABORT;
  }
  ...
  if (FAILED(hr)) {
    LogStartCaptureResult(StartCaptureResult::kAddClosedFailed);
    return hr;
  }
  ...
  if (FAILED(hr)) {
    LogStartCaptureResult(StartCaptureResult::kDxgiDeviceCastFailed);
    return hr;
  }

  if (!ResolveCoreWinRTDirect3DDelayload()) {
    LogStartCaptureResult(StartCaptureResult::kD3dDelayLoadFailed);
    return E_FAIL;
  }

  hr = CreateDirect3DDeviceFromDXGIDevice(dxgi_device.Get(), &direct3d_device_);
  if (FAILED(hr)) {
    LogStartCaptureResult(StartCaptureResult::kD3dDeviceCreationFailed);
    return hr;
  }
  ...
  hr = GetActivationFactory<
      ...
      &frame_pool_statics);
  if (FAILED(hr)) {
    LogStartCaptureResult(StartCaptureResult::kFramePoolActivationFailed);
    return hr;
  }
  ...
  if (FAILED(hr)) {
    LogStartCaptureResult(StartCaptureResult::kFramePoolCastFailed);
    return hr;
  }
  ...
  if (FAILED(hr)) {
    LogStartCaptureResult(StartCaptureResult::kGetItemSizeFailed);
    return hr;
  }
  ...
  if (FAILED(hr)) {
    LogStartCaptureResult(StartCaptureResult::kCreateFreeThreadedFailed);
    return hr;
  }

  hr = frame_pool_->CreateCaptureSession(item_.Get(), &session_);
  if (FAILED(hr)) {
    LogStartCaptureResult(StartCaptureResult::kCreateCaptureSessionFailed);
    return hr;
  }
  ...
  if (FAILED(hr)) {
    ...
    LogStartCaptureResult(StartCaptureResult::kStartCaptureFailed);
    return hr;
  }

  LogStartCaptureResult(StartCaptureResult::kSuccess);
...
HRESULT WgcCaptureSession::GetFrame(
  ...
  if (item_closed_) {
    RTC_LOG(LS_ERROR) << "The target source has been closed.";
    LogGetFrameResult(GetFrameResult::kItemClosed);
    return E_ABORT;
  }
  ...
  if (FAILED(hr)) {
    ...
    LogGetFrameResult(GetFrameResult::kTryGetNextFrameFailed);
    return hr;
  }

  if (!capture_frame) {
    LogGetFrameResult(GetFrameResult::kFrameDropped);
    return hr;
  }
  ...
  if (FAILED(hr)) {
    LogGetFrameResult(GetFrameResult::kGetSurfaceFailed);
    return hr;
  }
  ...
  if (FAILED(hr)) {
    LogGetFrameResult(GetFrameResult::kDxgiInterfaceAccessFailed);
    return hr;
  }
  ...
  if (FAILED(hr)) {
    LogGetFrameResult(GetFrameResult::kTexture2dCastFailed);
    return hr;
  }
  ...
  if (!mapped_texture_) {
    ...
    if (FAILED(hr)) {
      LogGetFrameResult(GetFrameResult::kCreateMappedTextureFailed);
      return hr;
    }
  }
  ...
  hr = d3d_context->Map(mapped_texture_.Get(), /*subresource_index=*/0,
                        ...
                        &map_info);
  if (FAILED(hr)) {
    LogGetFrameResult(GetFrameResult::kMapFrameFailed);
    return hr;
  }
  ...
  if (FAILED(hr)) {
    LogGetFrameResult(GetFrameResult::kGetContentSizeFailed);
    return hr;
  }
  ...
  *output_frame = std::make_unique<WgcDesktopFrame>(size, row_data_length,
  ...
  if (previous_size_.Height != new_size.Height ||
    ...
    if (FAILED(hr)) {
      LogGetFrameResult(GetFrameResult::kResizeMappedTextureFailed);
      return hr;
    }
    ...
    if (FAILED(hr)) {
      LogGetFrameResult(GetFrameResult::kRecreateFramePoolFailed);
      return hr;
    }
  }

  LogGetFrameResult(GetFrameResult::kSuccess);
