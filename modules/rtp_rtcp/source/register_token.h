...
// Move-only class that unregisters on destruction or explicetly with reset
// function.
class RegisterToken {
 public:
  RegisterToken() = default;
  RegisterToken(RegisterToken&&) = default;
  // unregister should have operator() without parameters.
  ...
  void reset() { unregister_.reset(); }

 private:
  std::unique_ptr<register_token_internal::UnregisterInterface> unregister_;
};

// Run reg() to the task_queue. return token that can be reset/destroyed on
// any thread and destroyes token returned by reg on the task_queue.
// task_queue must outlive returned RegisterToken.
template <typename RegisterClosure>
RegisterToken RegisterOnTaskQueue(rtc::TaskQueue* task_queue,
                                  RegisterClosure&& reg) {
