...
TEST_P(GoogPingTest, TestGoogPingAnnounceEnable) {
  ...
  if (trials.announce_goog_ping && trials.enable_goog_ping) {
    ...
    ASSERT_EQ(msg2->type(), STUN_BINDING_REQUEST);
    con->SendStunBindingResponse(msg2);
  }
...
// This test that a change in attributes falls back to STUN_BINDING
...
// This test that an error response falls back to STUN_BINDING.
TEST_F(PortTest, TestErrorResponseMakesGoogPingFallsbackToStunBinding) {
  IceFieldTrials trials;
  trials.announce_goog_ping = true;
  trials.enable_goog_ping = true;
