...
class MockVideoEncoder : public VideoEncoder {
 public:
  ...
  EncoderInfo GetEncoderInfo() const override {
    ...
    info.apply_alignment_to_simulcast_layers =
        apply_alignment_to_simulcast_layers_;
  ...
  void set_apply_alignment_to_simulcast_layers(bool apply) {
    apply_alignment_to_simulcast_layers_ = apply;
  }
 ...
 private:
  ...
  bool apply_alignment_to_simulcast_layers_ = false;
...
TEST_F(TestSimulcastEncoderAdapterFake,
  ...
  // No encoder has apply_alignment_to_simulcast_layers, report false.
  EXPECT_EQ(0, adapter_->InitEncode(&codec_, kSettings));
  ASSERT_EQ(3u, helper_->factory()->encoders().size());
  for (MockVideoEncoder* encoder : helper_->factory()->encoders()) {
    encoder->set_apply_alignment_to_simulcast_layers(false);
  }
  EXPECT_FALSE(adapter_->GetEncoderInfo().apply_alignment_to_simulcast_layers);

  // One encoder has apply_alignment_to_simulcast_layers, report true.
  helper_->factory()->encoders()[1]->set_apply_alignment_to_simulcast_layers(
      true);
  EXPECT_EQ(0, adapter_->InitEncode(&codec_, kSettings));
  EXPECT_TRUE(adapter_->GetEncoderInfo().apply_alignment_to_simulcast_layers);
}
