...
float TransientSuppressorImpl::Suppress(float* data,
  ...
  RTC_DCHECK_EQ(detection_length, detection_length_);
  RTC_DCHECK_GE(voice_probability, 0.0f);
  RTC_DCHECK_LE(voice_probability, 1.0f);
  ...
  if (detection_enabled_) {
    ...
    RTC_DCHECK_GE(detector_result, 0.0f);
