...
VCMDecodedFrameCallback::VCMDecodedFrameCallback(VCMTiming* timing,
    ...
    : _clock(clock),
      ...
      _low_latency_renderer_enabled("enabled", true),
      _low_latency_renderer_include_predecode_buffer("include_predecode_buffer",
                                                     true) {
  ntp_offset_ =
  ...
  ParseFieldTrial({&_low_latency_renderer_enabled,
                   &_low_latency_renderer_include_predecode_buffer},
...
void VCMDecodedFrameCallback::Decoded(VideoFrame& decodedImage,
  ...
  if (_low_latency_renderer_enabled && frameInfo->playout_delay.min_ms == 0 &&
      frameInfo->playout_delay.max_ms > 0) {
    absl::optional<int> max_composition_delay_in_frames =
        _timing->MaxCompositionDelayInFrames();
    if (max_composition_delay_in_frames) {
      if (_low_latency_renderer_include_predecode_buffer) {
