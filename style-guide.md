...
The following smart pointer types are recommended:
   ...
   * rtc::weak_ptr for references that can go away

Use of std::shared_ptr is *not permitted*. It is banned in the Chromium
style guide, and offers no compelling advantage over rtc::scoped_refptr
(which is cloned from the corresponding Chromium type).
