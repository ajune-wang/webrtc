...
rtc::scoped_refptr<I420BufferInterface> Y4mFile::GetFrame(
  ...
  if (frame_index < 0 || frame_index >= number_of_frames()) {
    RTC_LOG(LS_ERROR) << "Index out of bounds: " << frame_index;
    return nullptr;
  }
  file_.seekg(frame_positions_[frame_index]);
  ...
  if (!file_.good()) {
    RTC_LOG(LS_ERROR) << "Could not ready YUV data";
    return nullptr;
  }
  return buffer;
}
