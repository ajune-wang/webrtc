...
class VideoQualityMetricsReporter
 ...
 private:
  struct StatsSample {
    StatsSample(Timestamp sample_time) : sample_time(sample_time) {}
    ...
    bool IsEmpty() const {
      return bytes_sent == 0 && retransmitted_bytes_sent == 0 &&
             total_encoded_bytes_target == 0;
    }

    void DebugLog() {
      std::printf(
          "[%lu] bytes_sent=%lu; retransmitted_bytes_sent=%lu; "
          "total_encoded_bytes_target=%lu\n",
          sample_time.us(), bytes_sent, retransmitted_bytes_sent,
