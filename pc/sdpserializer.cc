...
RTCErrorOr<SimulcastLayerList> ParseSimulcastLayerList(absl::string_view str) {
  std::vector<absl::string_view> tokens =
      absl::StrSplit(str, kDelimiterSemicolonChar, absl::AllowEmpty());
  if (tokens.empty()) {
  ...
  for (absl::string_view token : tokens) {
    ...
    std::vector<absl::string_view> rid_tokens =
        absl::StrSplit(token, kDelimiterCommaChar, absl::AllowEmpty());
    if (rid_tokens.empty()) {
...
webrtc::RTCError ParseRidFormatList(absl::string_view payload_list,
  ...
  for (const std::string& payload_type : string_payloads) {
    ...
    if (!value.has_value()) {
      return ParseError("Invalid format specifier: " + payload_type);
    }
...
std::string SdpSerializer::SerializeRidDescription(
  ...
  RTC_DCHECK(rid_description.direction == RidDirection::kRidDirectionSend ||
             rid_description.direction == RidDirection::kRidDirectionReceive);

  rtc::StringBuilder builder;
  builder << rid_description.rid << kDelimiterSpace
          << (rid_description.direction == RidDirection::kRidDirectionSend
                  ? kSendDirection
                  : kReceiveDirection);
...
RTCErrorOr<RidDescription> SdpSerializer::DeserializeRidDescription(
  ...
  std::string rid_id(tokens[0]);
  RidDirection direction = tokens[1] == kSendDirection
                               ? RidDirection::kRidDirectionSend
                               : RidDirection::kRidDirectionReceive;

  RidDescription rid_description(rid_id, direction);

  // If there is a third argument it is a format list and/or restriction list.
  if (tokens.size() == 3) {
    std::vector<absl::string_view> restrictions =
        absl::StrSplit(tokens[2], kDelimiterSemicolonChar, absl::SkipEmpty());

    // Check for malformed restriction list, such as ';' or ';;;' etc.
    if (restrictions.empty()) {
      return ParseError(
          absl::StrCat("Invalid RID restriction list: ", tokens[2]));
    }

    // Parse the restrictions. The format indicator (pt) can only appear first.
    for (absl::string_view restriction : restrictions) {
      std::vector<std::string> parts =
          absl::StrSplit(restriction, kDelimiterEqualChar, absl::SkipEmpty());
      if (parts.empty() || parts.size() > 2) {
        return ParseError(
            absl::StrCat("Invalid format for restriction: ", restriction));
      }
