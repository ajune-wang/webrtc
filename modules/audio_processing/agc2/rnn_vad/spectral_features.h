...
class SpectralFeaturesExtractor {
 public:
  ...
  // is neither computed nor written. The higher bands spectral coefficients are
  // written into |coeffs|, the average over time for the lower bands in |avg|,
  // the first and second derivatives over time for the lower bands in |delta1|
  // and |delta2|, the spectral cross-correlation for the lower bands in |corr|
  // and the spectral variability score in |variability|.
  static_assert(kNumBands > kNumLowerBands, "");
 ...
 private:
  ...
  std::array<std::complex<float>, kFrameSize20ms24kHz> reference_frame_fft_{};
  std::array<std::complex<float>, kFrameSize20ms24kHz> lagged_frame_fft_{};
