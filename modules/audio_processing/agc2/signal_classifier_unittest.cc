...
// Random noise is stationary, but does not trigger the detector every
// frame due to the randomness.
TEST(AutomaticGainController2SignalClassifier, RandomNoise) {
  for (const auto rate : {8000, 16000, 32000, 48000}) {
    const int number_of_noise_frames = RunClassifier(NoiseGenerator, rate);
