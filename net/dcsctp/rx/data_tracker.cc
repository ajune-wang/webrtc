...
void DataTracker::Observe(TSN tsn,
  ...
  if (unwrapped_tsn == last_cumulative_acked_tsn_.next_value()) {
    ...
    while (!gaps_.empty() &&
           *gaps_.begin() == last_cumulative_acked_tsn_.next_value()) {
      last_cumulative_acked_tsn_.Increment();
      gaps_.erase(gaps_.begin());
    }
  } else {
    gaps_.insert(unwrapped_tsn);
  }
...
void DataTracker::HandleForwardTsn(TSN new_cumulative_ack) {
  ...
  // received (and not received at all) data, up until `new_cumulative_ack`.
  ...
  last_cumulative_acked_tsn_ = unwrapped_tsn;
  int erased_gap_tsns =
      std::distance(gaps_.begin(), gaps_.upper_bound(unwrapped_tsn));
  gaps_.erase(gaps_.begin(), gaps_.upper_bound(unwrapped_tsn));
...
std::vector<SackChunk::GapAckBlock> DataTracker::CreateGapAckBlocks() const {
  ...
  absl::optional<std::pair<UnwrappedTSN /*first */, UnwrappedTSN /*last */>>
      current = absl::nullopt;
  auto flush = [&]() {
    if (current.has_value()) {
      int start_diff = current->first.Difference(last_cumulative_acked_tsn_);
      int end_diff = current->second.Difference(last_cumulative_acked_tsn_);
      gap_ack_blocks.emplace_back(static_cast<uint16_t>(start_diff),
                                  static_cast<uint16_t>(end_diff));
      current = absl::nullopt;
    }
  };
  for (UnwrappedTSN tsn : gaps_) {
    if (current.has_value() && current->second.next_value() == tsn) {
      current->second = tsn;
    } else {
      flush();
      current = std::make_pair(tsn, tsn);
    }
  }
  flush();
