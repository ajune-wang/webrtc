...
constexpr size_t kInputAudioFrameSize =
    rnn_vad::kAudioFrameSize * rnn_vad::kSampleRate / kInputSampleRate;
...
int main(int argc, char* argv[]) {
  ...
  std::array<float, rnn_vad::kAudioFrameSize> samples_decimated;
  ...
  while (true) {
    ...
    if (read_samples < kInputAudioFrameSize)
      break;  // EOF.
    // Downsample.
    rnn_vad::Decimate2x({samples_decimated.data(), samples_decimated.size()},
