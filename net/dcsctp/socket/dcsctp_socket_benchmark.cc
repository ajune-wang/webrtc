...
void BM_EchoServer(benchmark::State& state) {
  ...
  for (auto _ : state) {
    while (cb_a.HasPacket() || cb_z.HasPacket()) {
      if (auto packet = cb_a.ConsumeSentPacket(); !packet.empty()) {
        sock_z.ReceivePacket(packet);
      }
      if (auto packet = cb_z.ConsumeSentPacket(); !packet.empty()) {
        sock_a.ReceivePacket(packet);
      }
    }
