...
// Logic to decide if an m= section can be recycled. An m= section is recyclable
// if either the old local or remote m= section is rejected.
// |current_local_content| and |remote_local_content| refer to the m=
// section of the current local and remote descriptions respectively. We need to
// check both the local and remote because either could be the most current from
// the latest negotation.
bool IsMediaSectionRecyclable(const ContentInfo* current_local_content,
...
SessionDescription* MediaSessionDescriptionFactory::CreateOffer(
  ...
  for (const MediaDescriptionOptions& media_description_options :
    ...
    if (current_local_description &&
      ...
      if (!IsMediaSectionRecyclable(current_local_content,
                                    current_remote_content)) {
...
bool MediaSessionDescriptionFactory::AddAudioContentForOffer(
  ...
  if (current_local_content &&
      !IsMediaSectionRecyclable(current_local_content,
                                current_remote_content)) {
