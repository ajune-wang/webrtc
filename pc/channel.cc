...
bool BaseChannel::UpdateRemoteStreams_w(
  ...
  if (!webrtc::RtpTransceiverDirectionHasSend(direction)) {
  ...
  if (ret) {
    set_remote_content_direction(direction);
    UpdateMediaSendRecvState_w();
  }

  RTC_DCHECK(ret || !error_desc.empty());
...
bool VoiceChannel::SetRemoteContent_w(const MediaContentDescription* content,
  ...
  return UpdateRemoteStreams_w(content->streams(), type, content->direction(),
                               error_desc);
}
...
bool VideoChannel::SetRemoteContent_w(const MediaContentDescription* content,
  ...
  return UpdateRemoteStreams_w(video->streams(), type, content->direction(),
                               error_desc);
}
