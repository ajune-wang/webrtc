...
std::pair<std::unique_ptr<ScalableVideoController>,
          absl::optional<ScalabilityMode>>
CreateVp9ScalabilityStructure(const VideoCodec& codec) {
  ...
  return {std::move(scalability_structure_controller),
          std::move(scalability_mode)};
}
...
int LibvpxVp9Encoder::InitEncode(const VideoCodec* inst,
  ...
  if (scalability_mode_.has_value()) {
    ...
    std::tie(svc_controller_, scalability_mode_) =
        CreateVp9ScalabilityStructure(*inst);
  }
