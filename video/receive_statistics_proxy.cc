...
void ReceiveStatisticsProxy::OnDecodedFrame(
  ...
  *stats_.corruption_score += kDummyCorruptionScore;
  // TODO(discuss):
  //   1- Shall corruption_score be included in `content_specific_stats?
  //      Imo this is best, so we can separate corruptions in screensharing and
  //      normal video streams.
  //   2- `rtc::SampleCounter.Add()` does only accept integers. Shall we update
  //      to float/double or multiply by 100 before adding as its now?
  //      Imo multiplying wiht 100 is the simplest way, and we do not lose any
  //      information.
  //   3- `rtc::SampleCounter` can only calculate the min, max and avg. Shall we
  //       use `HistogramPercentileCounter` instead?
  //       Imo yes, but wdyt? Because then we can get a percentile, which is
  //       good i think.
