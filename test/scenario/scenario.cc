...
void Scenario::RunFor(TimeDelta duration) {
  if (start_time_.IsInfinite())
    Start();
  network_manager_.AdvanceTime(duration);
}
...
void Scenario::RunUntil(TimeDelta target_time_since_start,
  ...
  while (check_interval >= TimeUntilTarget(target_time_since_start)) {
    network_manager_.AdvanceTime(check_interval);
    if (exit_function())
      return;
  }
  network_manager_.AdvanceTime(TimeUntilTarget(target_time_since_start));
}
