...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::ParseFile(
  ...
  RTC_PARSE_CHECK_OR_RETURN_GE(filesize, 0);

  // Read file into memory.
  std::string buffer(rtc::checked_cast<size_t>(filesize), '\0');
  size_t bytes_read = file.Read(&buffer[0], buffer.size());
  if (bytes_read != rtc::checked_cast<size_t>(filesize)) {
    RTC_LOG(LS_WARNING) << "Failed to read file " << filename;
    RTC_PARSE_CHECK_OR_RETURN_EQ(bytes_read,
                                 rtc::checked_cast<size_t>(filesize));
  }
