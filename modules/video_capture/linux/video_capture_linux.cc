...
int32_t VideoCaptureModuleV4L2::StartCapture(
  ...
  if ((_deviceFd = open(device, O_RDWR | O_NONBLOCK, 0)) < 0) {
    NLOG(LS_INFO, "error in opening ", device, " errono = ", errno);
    return -1;
  }
  ...
  NLOG(LS_INFO, "Video Capture enumerats supported image formats:");
  while (ioctl(_deviceFd, VIDIOC_ENUM_FMT, &fmt) == 0) {
    NLOG(LS_INFO, "  { pixelformat = ", cricket::GetFourccName(fmt.pixelformat),
  ...
  if (fmtsIdx == nFormats) {
    NLOG(LS_INFO, "no supporting video formats found");
    return -1;
  } else {
    NLOG(LS_INFO, "We prefer format ", cricket::GetFourccName(fmts[fmtsIdx]));
  }
  ...
  if (ioctl(_deviceFd, VIDIOC_S_FMT, &video_fmt) < 0) {
    NLOG(LS_INFO, "error in VIDIOC_S_FMT, errno = ", errno);
    return -1;
  }
  ...
  if (ioctl(_deviceFd, VIDIOC_G_PARM, &streamparms) < 0) {
    NLOG(LS_INFO, "error in VIDIOC_G_PARM errno = ", errno);
    ...
    if (streamparms.parm.capture.capability & V4L2_CAP_TIMEPERFRAME) {
      ...
      if (ioctl(_deviceFd, VIDIOC_S_PARM, &streamparms) < 0) {
        NLOG(LS_INFO, "Failed to set the framerate. errno=", errno);
        driver_framerate_support = false;
      } else {
  ...
  if (!AllocateVideoBuffers()) {
    NLOG(LS_INFO, "failed to allocate video capture buffers");
    return -1;
  }
  ...
  if (ioctl(_deviceFd, VIDIOC_STREAMON, &type) == -1) {
    NLOG(LS_INFO, "Failed to turn on stream");
    return -1;
  }
...
bool VideoCaptureModuleV4L2::AllocateVideoBuffers() {
  ...
  if (ioctl(_deviceFd, VIDIOC_REQBUFS, &rbuffer) < 0) {
    NLOG(LS_INFO, "Could not get buffers from device. errno = ", errno);
    return false;
  }
...
bool VideoCaptureModuleV4L2::DeAllocateVideoBuffers() {
  ...
  if (ioctl(_deviceFd, VIDIOC_STREAMOFF, &type) < 0) {
    NLOG(LS_INFO, "VIDIOC_STREAMOFF error. errno: ", errno);
  }
...
bool VideoCaptureModuleV4L2::CaptureProcess() {
  ...
  if (_captureStarted) {
    ...
    while (ioctl(_deviceFd, VIDIOC_DQBUF, &buf) < 0) {
      if (errno != EINTR) {
        NLOG(LS_INFO, "could not sync on a buffer on device ", strerror(errno));
        return true;
      }
    }
    ...
    if (ioctl(_deviceFd, VIDIOC_QBUF, &buf) == -1) {
      NLOG(LS_INFO, "Failed to enqueue capture buffer");
    }
  }
  usleep(0);
  return true;
