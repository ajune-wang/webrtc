...
int main(int argc, char* argv[]) {
  ...
  if (perf_results_json_path != "") {
    rtc::FileStream json_file;
    int error;
    if (!json_file.Open(perf_results_json_path, "w", &error)) {
      RTC_LOG_F(LS_ERROR) << "Could not open file, err=" << error;
    } else {
      size_t written;
      std::string json_results = webrtc::test::GetPerfResultsJSON();
      if (json_file.Write(json_results.c_str(), json_results.size(), &written,
                          &error) != rtc::SR_SUCCESS) {
        RTC_LOG_F(LS_ERROR) << "Could not write results, err=" << error;
      }
      json_file.Close();
    }
  }
