...
struct EmulatedIpPacket {
 public:
  ...
  ~EmulatedIpPacket();
  EmulatedIpPacket(const EmulatedIpPacket&);
...
class EndpointNode : public EmulatedNetworkReceiverInterface {
 ...
 private:
  ...
  rtc::CriticalSection socket_lock_;
  ...
  uint16_t next_port_ RTC_GUARDED_BY(socket_lock_);
  std::map<uint16_t, FakeNetworkSocket*> port_to_socket_
      RTC_GUARDED_BY(socket_lock_);
