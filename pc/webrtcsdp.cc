...
bool ParseContent(const std::string& message,
      ...
      *bundle_only = true;
      ...
      if (HasAttribute(line, kAttributeRtcpMux)) {
        ...
        *msid_signaling |= cricket::kMsidSignalingMediaSection;
                 ...
                 HasAttribute(line, kAttributeSimulcast)) {
        // Simulcast is only supported for video media sections
        const size_t prefix =
        ...
        RTCErrorOr<SimulcastDescription> errorOrSimulcast =
            deserializer.DeserializeSimulcastDescription(line.substr(prefix));
        if (!errorOrSimulcast.ok()) {
          return ParseFailed(line, errorOrSimulcast.error().message(), error);
        }

        simulcast = errorOrSimulcast.value();
        hasSimulcast = true;
      } else {
  ...
  if (hasSimulcast) {
    // TODO(amithi): verify that the rids in simulcast match rids in sdp
    VideoContentDescription* video_desc =
        static_cast<VideoContentDescription*>(media_desc);
    video_desc->set_simulcast_description(simulcast);
  }
