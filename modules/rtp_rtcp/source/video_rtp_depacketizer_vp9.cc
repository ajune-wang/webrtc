...
//      +-+-+-+-+-+-+-+-+
//
void ParseLayerInfoCommon(BitstreamReader& parser, RTPVideoHeaderVP9* vp9) {
...
//      +-+-+-+-+-+-+-+-+
//
void ParseLayerInfoNonFlexibleMode(BitstreamReader& parser,
                                   RTPVideoHeaderVP9* vp9) {
  vp9->tl0_pic_idx = parser.Read<uint8_t>();
}

void ParseLayerInfo(BitstreamReader& parser, RTPVideoHeaderVP9* vp9) {
  ParseLayerInfoCommon(parser, vp9);
  if (!vp9->flexible_mode) {
    ParseLayerInfoNonFlexibleMode(parser, vp9);
  }
}
...
int VideoRtpDepacketizerVp9::ParseRtpPayload(
  ...
  int num_remaining_bytes = parser.RemainingBitCount() / 8;
  if (num_remaining_bytes <= 0) {
    // Failed to parse or empty vp9 payload data.
    return 0;
  }
  // vp9 descriptor is byte aligned.
  RTC_DCHECK_EQ(parser.RemainingBitCount() % 8, 0);

  return rtp_payload.size() - num_remaining_bytes;
}
}  // namespace webrtc
