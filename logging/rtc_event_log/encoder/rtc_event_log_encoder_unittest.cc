...
TEST_P(RtcEventLogEncoderTest, RtcEventGenericPacketReceived) {
  ...
  for (size_t i = 0; i < event_count_; ++i) {
    events[i] =
        (i == 0 || !force_repeated_fields_)
            ? gen_.NewGenericPacketReceived()
            : absl::make_unique<RtcEventGenericPacketReceived>(*events[0]);
    history_.push_back(
        absl::make_unique<RtcEventGenericPacketReceived>(*events[i]));
  }
...
TEST_P(RtcEventLogEncoderTest, RtcEventGenericPacketSent) {
  ...
  for (size_t i = 0; i < event_count_; ++i) {
    events[i] = (i == 0 || !force_repeated_fields_)
                    ...
                    : absl::make_unique<RtcEventGenericPacketSent>(*events[0]);
    history_.push_back(
        absl::make_unique<RtcEventGenericPacketSent>(*events[i]));
  }
...
TEST_P(RtcEventLogEncoderTest, RtcEventGenericAcksReceived) {
  ...
  for (size_t i = 0; i < event_count_; ++i) {
    events[i] = (i == 0 || !force_repeated_fields_)
                    ...
                    : absl::make_unique<RtcEventGenericAckReceived>(*events[0]);
    history_.push_back(
        absl::make_unique<RtcEventGenericAckReceived>(*events[i]));
  }
