...
DEPRECATED_SingleThreadedTaskQueueForTesting::AddTask(
    std::unique_ptr<webrtc::QueuedTask> task,
    int64_t delay_ms) {
...
void DEPRECATED_SingleThreadedTaskQueueForTesting::RunLoop() {
  ...
  while (true) {
    std::unique_ptr<webrtc::QueuedTask> queued_task;
...

    {
      ...
      if (!tasks_.empty()) {
        auto first = tasks_.begin();
        int64_t remaining_delay_ms =
            rtc::TimeDiff(first->first, rtc::TimeMillis());
        if (remaining_delay_ms <= 0) {
          queued_task = std::move(first->second.task);
          tasks_.erase(first);
        } else {
