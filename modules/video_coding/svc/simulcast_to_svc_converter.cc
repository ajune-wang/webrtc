...
SimulcastToSvcConverter::SimulcastToSvcConverter(const VideoCodec& codec) {
  ...
  layers_.resize(num_spatial_layers);
  for (int i = 0; i < num_spatial_layers; ++i) {
    layers_[i].video_controller = CreateScalabilityStructure(scalability_mode);
    layers_[i].video_controller->OnRatesUpdated(dummy_bitrates);
  }
}
...
bool SimulcastToSvcConverter::ConvertFrame(EncodedImage& encoded_image,
  ...
  if (codec_specific.generic_frame_info) {
    ...
    if (frame_config.TemporalId() != (tid == 255 ? 0 : tid)) {
      return false;
    }
