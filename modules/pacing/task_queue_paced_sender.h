...
class TaskQueuePacedSender : public RtpPacketPacer,
 ...
 private:
  ...
  // The last time |current_stats_| was refreshed.
  // Since we don't want to support synchronous calls that wait for
  // a task execution, we poll the stats at some interval update
  // |current_stats_|, which can in turn be polled at any time.
  Timestamp last_stats_update_ RTC_GUARDED_BY(task_queue_);
