...
class BitstreamReader {
 public:
  ...
  // Reads `bits` from the bitstream and returns result as unsigned integer in
  // range [0, 2^bits - 1].
  ...
  // Reads the exponential golomb encoded value.
...
inline BitstreamReader::~BitstreamReader() {
  RTC_DCHECK(last_read_is_verified_)
      << "Latest calls Read or ConsumeBit were not checked with Ok function.";
}
