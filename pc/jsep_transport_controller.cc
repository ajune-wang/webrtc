          ...
          [this](const std::string& mid,
            ...
            if (config_.transport_observer) {
              return config_.transport_observer->OnTransportChanged(
                  mid, rtp_transport, dtls_transport, data_channel_transport);
            }
            return false;
          },
          [this]() {
...
void JsepTransportController::SetNeedsIceRestartFlag() {
  RTC_DCHECK_RUN_ON(network_thread_);
  for (auto& transport : mid_transport_map_.Transports()) {
...
void JsepTransportController::RollbackTransports() {
  ...
  RTC_DCHECK_RUN_ON(network_thread_);
  mid_transport_map_.RollbackTransports();
}
...
JsepTransportController::GetDtlsTransports() {
  ...
  for (auto jsep_transport : mid_transport_map_.Transports()) {
...
RTCError JsepTransportController::ApplyDescription_n(
  ...
  if (type == SdpType::kAnswer) {
    mid_transport_map_.CommitTransports();
  }
  return RTCError::OK();
}
...
void JsepTransportController::HandleRejectedContent(
  ...
  if (bundle_group && !bundle_group->content_names().empty() &&
    ...
    // group.
    for (const auto& content_name : bundle_group->content_names()) {
      mid_transport_map_.RemoveTransportForMid(content_name);
    ...
    mid_transport_map_.RemoveTransportForMid(content_info.name);
    if (bundle_group) {
...
bool JsepTransportController::HandleBundledContent(
  ...
  // BaseChannel/SctpTransport change the RtpTransport/DtlsTransport first,
  // then destroy the cricket::JsepTransport.
  if (mid_transport_map_.SetTransportForMid(content_info.name,
                                            jsep_transport)) {
...
const cricket::JsepTransport* JsepTransportController::GetJsepTransportForMid(
    const std::string& mid) const {
  return mid_transport_map_.GetTransportForMid(mid);
}
...
const cricket::JsepTransport* JsepTransportController::GetJsepTransportByName(
    const std::string& transport_name) const {
  return mid_transport_map_.GetTransportByName(transport_name);
}
...
RTCError JsepTransportController::MaybeCreateJsepTransport(
  ...
  mid_transport_map_.SetTransportForMid(content_info.name,
                                        jsep_transport.get());

  mid_transport_map_.RegisterTransport(content_info.name,
                                       std::move(jsep_transport));
  UpdateAggregateStates_n();
  return RTCError::OK();
}
...
void JsepTransportController::MaybeDestroyJsepTransport(
  ...
  // to it.
  if (mid_transport_map_.TransportInUse(jsep_transport)) {
    return;
  }
  mid_transport_map_.MaybeDestroyJsepTransport(mid);
  UpdateAggregateStates_n();
}

void JsepTransportController::DestroyAllJsepTransports_n() {
  mid_transport_map_.DestroyAllTransports();
}
