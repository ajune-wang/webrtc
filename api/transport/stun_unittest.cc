...
// and the header is different since the message is shorted.
...
// Validate that we add both MESSAGE-INTEGRITY-32 and MESSAGE-INTEGRITY
// attributes in the same message...although it is stupid !?!
...
TEST_F(StunTest, ClearAttributes) {
  ...
  msg.ClearAttributes();
  EXPECT_EQ(msg.length() + /* 3 + 1 byte padding + header */ 8, len);
