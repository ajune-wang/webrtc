...
// static constexpr fuchsia::sysmem::ColorSpaceType kREC709ColorSpace =
...
ScreenCapturerFuchsia::~ScreenCapturerFuchsia() {
  ...
  for (uint32_t buffer_index = 0;
    ...
    uintptr_t address =
        reinterpret_cast<uintptr_t>(virtual_memory_mapped_addrs[buffer_index]);
...
void ScreenCapturerFuchsia::CaptureFrame() {
  ...
  frame->CopyPixelsFrom(virtual_memory_mapped_addrs[buffer_index], stride,
...
void ScreenCapturerFuchsia::SetupBuffers() {
  ...
  // onto a pointer stored in virtual_memory_mapped_addrs which we can use to
  // access this data.
  uint32_t virt_mem_bytes =
      buffer_collection_info_.settings.buffer_settings.size_bytes;
