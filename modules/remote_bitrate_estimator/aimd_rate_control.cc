...
constexpr char kIncreaseWhileNotAlrExperiment[] =
    "WebRTC-IncreaseDelayBasedBweWhileNotAlr";
...
AimdRateControl::AimdRateControl(const WebRtcKeyValueConfig* key_value_config)
    : min_configured_bitrate_(congestion_controller::GetMinBitrate()),
      ...
      increase_bwe_while_not_in_alr_experiment_(
          IsEnabled(*key_value_config, kIncreaseWhileNotAlrExperiment)),
...
void AimdRateControl::SetInAlr(bool in_alr) {
  in_alr_ = in_alr;
}
...
DataRate AimdRateControl::ChangeBitrate(DataRate new_bitrate,
  ...
  switch (rate_control_state_) {
    ...
    case kRcIncrease:
      ...
      if (!in_alr_ || !increase_bwe_while_not_in_alr_experiment_) {
...
DataRate AimdRateControl::ClampBitrate(DataRate new_bitrate,
                                       DataRate estimated_throughput) const {
  if (!increase_bwe_while_not_in_alr_experiment_) {
