...
TEST(RnnVad, CheckValidVadProbabilities) {
  ...
  BinaryFileReader<int16_t, rnn_vad::kAudioFrameSize, float> samples_reader(
      test::ResourcePath("common_audio/rnn_vad/samples", "pcm"));
  std::array<float, rnn_vad::kAudioFrameSize> samples;
  ...
  for (size_t i = 0; i < num_frames; ++i) {
    ...
    features_extractor.ComputeFeatures({samples.data(), samples.size()});
...
TEST(RnnVad, RnnVadBitExactness) {
  // Init.
  BinaryFileReader<float, rnn_vad::kFeatureVectorSize> features_reader(
  ...
  float is_silence;
  std::array<float, rnn_vad::kFeatureVectorSize> features;
...
TEST(RnnVad, SequenceBufferCopy) {
  SequenceBuffer<uint8_t, 8, 8> seq_buf(0);
  constexpr std::array<uint8_t, 8> chunk = {10, 20, 30, 40, 50, 60, 70, 80};
  seq_buf.Push({chunk.data(), 8});
  std::array<uint8_t, 8> dst;
  seq_buf.CopyBuffer({dst.data(), 8});
  EXPECT_EQ(chunk, dst);
}
