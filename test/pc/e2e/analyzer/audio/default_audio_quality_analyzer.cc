...
void DefaultAudioQualityAnalyzer::OnStatsReports(
  ...
  for (auto& stat : stats) {
    ...
    sample.total_samples_received =
        test::GetStatOrDefault(stat->total_samples_received, 0ul);
    sample.concealed_samples =
        test::GetStatOrDefault(stat->concealed_samples, 0ul);
    sample.removed_samples_for_acceleration =
        test::GetStatOrDefault(stat->removed_samples_for_acceleration, 0ul);
    sample.inserted_samples_for_deceleration =
        test::GetStatOrDefault(stat->inserted_samples_for_deceleration, 0ul);
    sample.silent_concealed_samples =
        test::GetStatOrDefault(stat->silent_concealed_samples, 0ul);
    sample.jitter_buffer_target_delay = TimeDelta::Seconds(
        test::GetStatOrDefault(stat->jitter_buffer_target_delay, 0.));
    sample.jitter_buffer_emitted_count =
        test::GetStatOrDefault(stat->jitter_buffer_emitted_count, 0ul);
