  ...
  bool VerifyRTCMediaStreamTrackStats(
    ...
    if (*media_stream_track.kind == RTCMediaStreamTrackKind::kVideo) {
      ...
      if (*media_stream_track.remote_source) {
        ...
        // Freeze metrics are set only if freeze actually happened during call.
        verifier.MarkMemberTested(media_stream_track.mean_freeze_duration_ms,
                                  true);
        verifier.MarkMemberTested(media_stream_track.num_freezes, true);
      } else {
        verifier.TestMemberIsNonNegative<uint32_t>(
