...
void PeerConnection::ReportUsagePattern() const {
  ...
  RTC_HISTOGRAM_ENUMERATION("WebRTC.PeerConnection.UsagePattern",
...
void PeerConnection::ReportBestConnectionState(
  ...
  for (const cricket::TransportChannelStats& channel_stats :
    ...
    for (const cricket::ConnectionInfo& connection_info :
      ...
      const char* candidate_pair_type_counter_name;
      ...
      if (local.protocol() == cricket::TCP_PROTOCOL_NAME ||
        ...
        candidate_pair_type_counter_name =
            "WebRTC.PeerConnection.CandidatePairType_TCP";
      ...
      RTC_HISTOGRAM_ENUMERATION(candidate_pair_type_counter_name,
...
void PeerConnection::ReportNegotiatedCiphers(
  ...
  for (cricket::MediaType media_type : media_types) {
    const char* srtp_counter_name;
    const char* ssl_counter_name;
    switch (media_type) {
      case cricket::MEDIA_TYPE_AUDIO:
        srtp_counter_name = "WebRTC.PeerConnection.SrtpCryptoSuite.Audio";
        ssl_counter_name = "WebRTC.PeerConnection.SslCipherSuite.Audio";
        break;
      case cricket::MEDIA_TYPE_VIDEO:
        srtp_counter_name = "WebRTC.PeerConnection.SrtpCryptoSuite.Video";
        ssl_counter_name = "WebRTC.PeerConnection.SslCipherSuite.Video";
        break;
      case cricket::MEDIA_TYPE_DATA:
        srtp_counter_name = "WebRTC.PeerConnection.SrtpCryptoSuite.Data";
        ssl_counter_name = "WebRTC.PeerConnection.SslCipherSuite.Data";
        break;
      default:
        RTC_NOTREACHED();
        continue;
    ...
    if (srtp_crypto_suite != rtc::SRTP_INVALID_CRYPTO_SUITE) {
      RTC_HISTOGRAM_ENUMERATION(srtp_counter_name, srtp_crypto_suite,
                                rtc::SRTP_CRYPTO_SUITE_MAX_VALUE);
    }
    if (ssl_cipher_suite != rtc::TLS_NULL_WITH_NULL_NULL) {
      RTC_HISTOGRAM_ENUMERATION(ssl_counter_name, ssl_cipher_suite,
