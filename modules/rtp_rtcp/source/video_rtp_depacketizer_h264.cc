...
bool ParseStapA(rtc::ArrayView<const uint8_t> data,
  ...
  while (reader.Length() > 0) {
    ...
    if (!nalu_size || nalu_size > reader.Length())
      return false;
...
std::optional<VideoRtpDepacketizer::ParsedRtpPayload> ProcessStapAOrSingleNalu(
  ...
  if (nal_type == H264::NaluType::kStapA) {
    if (!ParseStapA(payload_data, &nal_units) || nal_units.empty()) {
        ...
        switch (result) {
          ...
          case SpsVuiRewriter::ParseResult::kVuiRewritten:
            ...
            parsed_payload->video_payload.AppendData(
                &payload_data[end_offset], payload_data.size() - end_offset);
