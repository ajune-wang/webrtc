...
bool CopyVideoFrameToNV12PixelBuffer(id<RTC_OBJC_TYPE(RTCI420Buffer)> frameBuffer, CVPixelBufferRef pixelBuffer) {
...
- (void)frameWasEncoded:(OSStatus)status
  ...
  RTC_OBJC_TYPE(RTCRtpFragmentationHeader) *header;
  {
    ...
    header = [[RTC_OBJC_TYPE(RTCRtpFragmentationHeader) alloc] initWithNativeFragmentationHeader:header_cpp.get()];
    if (!result) {
      return;
    }
  }
...
- (nullable RTC_OBJC_TYPE(RTCVideoEncoderQpThresholds) *)scalingSettings {
  return [[RTC_OBJC_TYPE(RTCVideoEncoderQpThresholds) alloc] initWithThresholdsLow:kLowH264QpThreshold
