...
class RtpSenderReceiverTest
 ...
 public:
  ...
  void VerifyVoiceChannelInput(uint32_t ssrc) {
    ...
    // muted.
    EXPECT_TRUE(voice_media_receive_channel()->HasSource(ssrc));
    EXPECT_FALSE(voice_media_receive_channel()->IsStreamMuted(ssrc));
  }
  ...
  void VerifyVideoChannelInput(uint32_t ssrc) {
    ...
    EXPECT_TRUE(video_media_receive_channel_->HasSource(ssrc));
  }
