...
ScalabilityStructureFullSvc::NextFrameConfig(bool restart) {
  ...
  // Upper temporal layer could have been disabled after previous call to
  // NextFrameConfig,thus need to check it here rather than when setting
  // next_pattern_ below.
  if (next_pattern_ == kDeltaT2A && !TemporalLayerIsActive(/*tid=*/2)) {
    next_pattern_ = kDeltaT1;
  }
  if (next_pattern_ == kDeltaT2B && !TemporalLayerIsActive(/*tid=*/2)) {
