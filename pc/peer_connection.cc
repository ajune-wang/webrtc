...
bool ContainsSharedIceCredentials(const SessionDescriptionInterface* old_desc,
  ...
  for (const cricket::ContentInfo& old_content : old_description->contents()) {
    const cricket::TransportDescription* old_transport_description =
        old_description->GetTransportDescriptionByName(old_content.name);
    if (!old_transport_description)
      continue;
    const cricket::ContentInfo* new_content =
        new_description->GetContentByName(old_content.name);
    if (!new_content || new_content->rejected)
      continue;
    const cricket::TransportDescription* new_transport_description =
        new_description->GetTransportDescriptionByName(new_content->name);
    if (!new_transport_description)
      continue;
    if (!cricket::IceCredentialsChanged(old_transport_description->ice_ufrag,
                                        old_transport_description->ice_pwd,
                                        new_transport_description->ice_ufrag,
                                        new_transport_description->ice_pwd)) {
      return true;
    }
  }
  return false;
