 ...
 public:
  ...
  void Start(dcsctp::DurationMs duration,
    ...
    thread_->PostDelayedTask(
        ToQueuedTask(pending_task_safety_flag_,
                     [timeout_id, this]() { handle_timer_(timeout_id); }),
        duration.value());
  }
  void Stop() override {
    RTC_DCHECK_RUN_ON(thread_);
...
bool DcsctpTransport::Start(int local_sctp_port,
  ...
  // TODO(bugs.webrtc.org/12614): Need to allow reconfiguring the
  // max_message_size
  if (!socket_) {
...
void DcsctpTransport::OnAborted(dcsctp::ErrorKind error,
                                absl::string_view message) {
  RTC_LOG(LS_ERROR) << debug_name_ << "->OnAborted().";
  ready_to_send_data_ = false;
