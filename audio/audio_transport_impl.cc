...
int ProcessCaptureFrame(int analog_level,
                        uint32_t delay_ms,
                        bool key_pressed,
  ...
  RTC_DCHECK_EQ(0, error) <<
      "set_stream_analog_level failed: current_mic_level = " << analog_level;
  ...
  return agc->stream_analog_level();
}
  ...
  // encoding task will be posted internally to each stream.
  {
    ...
    if (!sending_streams_.empty()) {
      ...
      while (++it != sending_streams_.end()) {
        ...
        (*it)->OnAudioData(std::move(audio_frame_copy));
      }
      (*sending_streams_.begin())->OnAudioData(std::move(audio_frame));
    }
  }
