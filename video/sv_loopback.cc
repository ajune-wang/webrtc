...
DEFINE_int(min_bitrate, 100, "Call min bitrate in kbps.");
int MinBitrateKbps() {
  return static_cast<int>(FLAG_min_bitrate);
}

DEFINE_int(max_bitrate, 2000, "Call max bitrate in kbps.");
int MaxBitrateKbps() {
...
void Loopback() {
  ...
  params.video[screenshare_idx] = {
      ...
      flags::MinBitrateKbps() * 1000 / 2,
      flags::ScreenshareTargetBitrateKbps() * 1000,
      flags::MaxBitrateKbps() * 1000 / 2,
      false,
      flags::Codec(),
