...
bool FakeNetworkSocketServer::Wait(int cms, bool process_io) {
  ...
  Timestamp last_wake_up_time = Now();
  while ((Now() - last_wake_up_time).ms() < cms || cms == kForever) {
    wakeup_.Wait(cms);

    Timestamp io_processing_start_time = Now();
    {
      rtc::CritScope crit(&lock_);
    ...
    cms -= (last_wake_up_time - io_processing_start_time).ms();
  }
  return true;
}
