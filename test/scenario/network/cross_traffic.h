...
// Represents the endpoint for cross traffic that is going throw the network.
...
class CrossTraffic {
 ...
 protected:
  friend class RandomWalkCrossTraffic;
  friend class PulsedPeaksCrossTraffic;

  // Returns default address, that can be used to send packet to this endpoint.
  rtc::SocketAddress GetDefaultAddress() const;
 ...
 private:
  ...
  uint16_t dev_null_receiver_port_;
  std::unique_ptr<EmulatedNetworkReceiverInterface> dev_null_receiver_;
...
class RandomWalkCrossTraffic {
 public:
  explicit RandomWalkCrossTraffic(RandomWalkConfig config,
 ...
 private:
  ...
  CrossTraffic* const cross_traffic_;
  webrtc::Random random_;

  // Time when Process(...) was called last time.
  Timestamp last_process_time_ = Timestamp::MinusInfinity();
  // Time since intensity was updated last time.
  TimeDelta time_since_update_ = TimeDelta::Zero();
  // Time when data was sent last time.
  Timestamp last_send_time_ = Timestamp::MinusInfinity();
  double intensity_ = 0;
  // Size of the pending data to send.
...
class PulsedPeaksCrossTraffic {
 public:
  explicit PulsedPeaksCrossTraffic(PulsedPeaksConfig config,
                                   CrossTraffic* cross_traffic);
