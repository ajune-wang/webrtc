...
bool IsWindowResponding(HWND window) {
  // 50ms is chosen in case
  // the system is under heavy load, but it's also not too long to delay window
  // enumeration considerably.
  const UINT uTimeout = 50;  // ms
  return SendMessageTimeout(window, WM_NULL, 0, 0, SMTO_ABORTIFHUNG, uTimeout,
                            nullptr);
}
