...
TEST(RttMultExperimentTest, RttMultTestValue) {
  webrtc::test::ScopedFieldTrials field_trials(
      "WebRTC-RttMult/Enabled-0.60,100.0/");
  EXPECT_EQ(0.60, RttMultExperiment::GetRttMultValue()->rtt_mult_setting);
  EXPECT_EQ(100.0, RttMultExperiment::GetRttMultValue()->rtt_mult_add_cap_ms);
}
...
TEST(RttMultExperimentTest, RttMultTestValueOutOfBoundsPositive) {
  ...
  EXPECT_EQ(1.0, RttMultExperiment::GetRttMultValue()->rtt_mult_setting);
  EXPECT_EQ(2000.0, RttMultExperiment::GetRttMultValue()->rtt_mult_add_cap_ms);
}

TEST(RttMultExperimentTest, RttMultTestValueOutOfBoundsNegative) {
  webrtc::test::ScopedFieldTrials field_trials(
      "WebRTC-RttMult/Enabled--0.5,-100.0/");
  EXPECT_EQ(0.0, RttMultExperiment::GetRttMultValue()->rtt_mult_setting);
  EXPECT_EQ(0.0, RttMultExperiment::GetRttMultValue()->rtt_mult_add_cap_ms);
}

TEST(RttMultExperimentTest, RttMultTestMalformedValue) {
  webrtc::test::ScopedFieldTrials field_trials(
      "WebRTC-RttMult/Enabled-0.2a5,100.0/");
  EXPECT_NE(0.25, RttMultExperiment::GetRttMultValue()->rtt_mult_setting);
}
