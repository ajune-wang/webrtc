...
TEST(SvcConfig, NumSpatialLayers) {
  ...
  std::vector<SpatialLayer> spatial_layers = GetSvcConfig(
      kMinVp9SpatialLayerWidth << (expected_num_spatial_layers - 1),
      kMinVp9SpatialLayerHeight << (expected_num_spatial_layers - 1),
      max_num_spatial_layers, 1);

  EXPECT_EQ(spatial_layers.size(), expected_num_spatial_layers);
}

TEST(SvcConfig, BitrateThresholds) {
  const size_t expected_num_spatial_layers = 3;
  std::vector<SpatialLayer> spatial_layers = GetSvcConfig(
      kMinVp9SpatialLayerWidth << (expected_num_spatial_layers - 1),
      kMinVp9SpatialLayerHeight << (expected_num_spatial_layers - 1),
      expected_num_spatial_layers, 1);
