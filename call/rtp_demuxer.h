...
class RtpDemuxer {
 ...
 private:
  struct SsrcBinding {
    enum class Origin {
      // SSRC binding created implicitly, for example, due
      // resolution by payload type.
      Implicit,
      // SSRC binding added explicitly, for example, originated
      // from explicitly signalled RTP stream,
      Explicit,
    };
    RtpPacketSinkInterface* const sink;
    Origin const origin;

    bool operator==(const SsrcBinding& other) {
      return sink == other.sink && origin == other.origin;
    }

    bool operator!=(const SsrcBinding& other) { return !(*this == other); }
  ...
  // SSRC mapping which receives all MID, payload type, or RSID to SSRC
  // bindings discovered when demuxing packets).
