...
class SctpTransportTest : public testing::Test, public sigslot::has_slots<> {
 protected:
  ...
  void SetupConnectedTransportsWithTwoStreams(int port1, int port2) {
    ...
    NLOG(LS_VERBOSE, "Transport setup ----------------------------- ");
    AddStream(1);
    AddStream(2);

    NLOG(LS_VERBOSE,
         "Connect the transports -----------------------------");  // Both
                                                                   // transports
                                                                   // need to
                                                                   // have
                                                                   // started
...
TEST_F(SctpTransportTest, SendData) {
  ...
  NLOG(LS_VERBOSE, "recv2.received=", receiver2()->received(),
       ", recv2.last_params.sid=", receiver2()->last_params().sid,
       ", recv2.last_params.timestamp=", receiver2()->last_params().timestamp,
       ", recv2.last_params.seq_num=", receiver2()->last_params().seq_num,
  ...
  EXPECT_TRUE_WAIT(ReceivedData(receiver1(), 2, "hi transport1"),
                   kDefaultTimeout);
  NLOG(LS_VERBOSE, "recv1.received=", receiver1()->received(),
       ", recv1.last_params.sid=", receiver1()->last_params().sid,
       ", recv1.last_params.timestamp=", receiver1()->last_params().timestamp,
       ", recv1.last_params.seq_num=", receiver1()->last_params().seq_num,
