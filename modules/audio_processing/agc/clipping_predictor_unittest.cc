...
TEST_P(ClippingPredictorParameterization,
  ...
  if (reference_window_length() + reference_window_delay() > window_length()) {
    ...
    EXPECT_EQ(predictor->EstimateClippedLevelStep(/*channel=*/0, /*level=*/255,
    ...
    EXPECT_THAT(predictor->EstimateClippedLevelStep(
                    /*channel=*/0, /*level=*/255, kDefaultClippedLevelStep,
                    kMinMicLevel, kMaxMicLevel),
...
TEST_P(ClippingPredictorParameterization,
  ...
  if (reference_window_length() + reference_window_delay() > window_length()) {
    const ClippingPredictorConfig config(
        {/*enabled=*/true,
         ClippingPredictorConfig::kAdaptiveStepClippingPeakPrediction,
         window_length(), reference_window_length(), reference_window_delay(),
         /*clipping_threshold=*/-1.0f});
    auto predictor =
...
TEST_P(ClippingEventPredictorParameterization,
  ...
  const ClippingPredictorConfig config(
      {/*enabled=*/true, ClippingPredictorConfig::kClippingEventPrediction,
       kWindowLength, kReferenceWindowLength, kReferenceWindowDelay,
  ...
  ProcessNonZeroCrestFactorAudio(kReferenceWindowLength, kNumChannels,
                                 /*peak_ratio=*/0.99f, *predictor);
  EXPECT_EQ(predictor->EstimateClippedLevelStep(/*channel=*/0,
                                                /*level=*/255,
                                                kDefaultClippedLevelStep,
                                                kMinMicLevel, kMaxMicLevel),
            absl::nullopt);
...
TEST(ClippingPeakPredictorTest, CheckAdaptiveStepEstimate) {
  ...
  ProcessZeroCrestFactorAudio(kWindowLength, kNumChannels,
                              /*peak_ratio=*/0.99f, *predictor);
  EXPECT_THAT(predictor->EstimateClippedLevelStep(/*channel=*/0,
                                                  /*level=*/255,
                                                  kDefaultClippedLevelStep,
                                                  kMinMicLevel, kMaxMicLevel),
              Optional(Eq(17)));
