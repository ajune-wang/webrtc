...
bool BaseChannel::SendPacket(bool rtcp,
  ...
  if (!srtp_active()) {
    if (srtp_required_) {
      ...
      RTC_LOG(LS_ERROR) << "Can't send outgoing RTP packet when SRTP is "
...
void BaseChannel::OnPacketReceived(bool rtcp,
  ...
  if (!srtp_active() && srtp_required_) {
    ...
    //    is used anyway.
    RTC_LOG(LS_WARNING) << "Can't process incoming "
                        << RtpRtcpStringLiteral(rtcp)
                        << " packet when SRTP is inactive and crypto is "
                        << "required";
    return;
  }
...
void BaseChannel::CacheRtpAbsSendTimeHeaderExtension_n(
  ...
  if (srtp_transport_) {
    ...
    RTC_LOG(LS_WARNING) << "Trying to cache the Absolute Send Time extension "
                        << "id but the SRTP is not active.";
  }
}
