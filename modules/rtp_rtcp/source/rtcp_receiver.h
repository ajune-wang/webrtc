 ...
 private:
  ...
  class RegisteredSsrcs {
   ...
   private:
    ...
    // TODO(tommi): This variable should really be guarded by the
    // `packet_sequence_checker_` but because of downstream cod that directly
    // uses the RTCPReceiver class and issues calls to IncomingPacket() on
    // random threads (with external synchronization), `contains()` ends up
    // getting called on random threads.
