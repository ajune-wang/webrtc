...
void SimulatedNetwork::DelayUntil(int64_t until_us) {
  rtc::CritScope crit(&config_lock_);
  delay_until_us_ = until_us;
}
...
bool SimulatedNetwork::EnqueuePacket(PacketInFlightInfo packet) {
  ...
  int64_t network_start_time_us = packet.send_time_us;
  {
    rtc::CritScope crit(&config_lock_);
    if (delay_until_us_) {
      network_start_time_us = std::max(network_start_time_us, *delay_until_us_);
      delay_until_us_.reset();
    }
  }
