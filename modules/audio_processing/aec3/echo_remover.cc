...
#include "modules/audio_processing/aec3/audio_enhancement.h"
...
std::unique_ptr<AudioEnhancement> ConditionallyCreateEnhancement(
    AudioEnhancer* enhancer,
    size_t num_channels,
    size_t num_bands) {
  if (enhancer) {
    return std::make_unique<AudioEnhancement>(num_channels, num_bands,
                                              enhancer);
  }
  return nullptr;
}
...
class EchoRemoverImpl final : public EchoRemover {
 ...
 private:
  ...
  std::unique_ptr<AudioEnhancement> audio_enhancement_;
...
EchoRemoverImpl::EchoRemoverImpl(const EchoCanceller3Config& config,
    ...
    : config_(config),
      ...
      audio_enhancement_(
          ConditionallyCreateEnhancement(audio_enhancer,
                                         num_capture_input_channels_,
                                         NumBandsForRate(sample_rate_hz_))),
      num_capture_output_channels_(audio_enhancement_
                                       ? audio_enhancement_->NumOutputChannels()
                                       : num_capture_input_channels_),
...
void EchoRemoverImpl::ProcessCapture(
  ...
  rtc::ArrayView<FftData> Y_enhancer_output(Y_linear_aec.data(),
                                            num_capture_input_channels_);
  if (audio_enhancement_) {
    audio_enhancement_->Enhance(
    ...
    if (audio_enhancement_->ModifiesOutput()) {
  ...
  const auto& Y_suppressor_input =
      audio_enhancement_ && audio_enhancement_->ModifiesOutput()
          ? Y_enhancer_output
          : Y_linear_aec;
...
EchoRemover* EchoRemover::Create(const EchoCanceller3Config& config,
  ...
  return new EchoRemoverImpl(config, sample_rate_hz, num_render_channels,
                             num_capture_channels, audio_enhancer);
}
