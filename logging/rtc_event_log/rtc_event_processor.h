...
class ProcessableEventListInterface {
 public:
  ...
  virtual size_t GetTieBreaker() const = 0;
};
...
class ProcessableEventList : public ProcessableEventListInterface {
 public:
  ProcessableEventList(int tie_breaker,
                       Iterator begin,
                       Iterator end,
                       std::function<void(const T&)> f)
      : tie_breaker_(tie_breaker), begin_(begin), end_(end), f_(f) {}
  ...
  size_t GetTieBreaker() const override { return tie_breaker_; }
  ...
  void AddEvents(
    ...
    event_lists_.push_back(
        absl::make_unique<event_processor_impl::ProcessableEventList<
            ...
            insertion_order_index_++, iterable.begin(), iterable.end(),
            handler));
