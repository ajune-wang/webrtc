  ...
  virtual Result OnEncodedImage(const EncodedImage& encoded_image,
    ...
    if (encoded_image.SpatialIndex().value_or(0) == 0) {
      if (encoded_image._frameType == kVideoFrameKey) {
        delete[] encoded_key_frame_._buffer;
        encoded_key_frame_.set_buffer(new uint8_t[encoded_image.alloc()],
                                      encoded_image.alloc());
        ...
        delete[] encoded_frame_._buffer;
        encoded_frame_.set_buffer(new uint8_t[encoded_image.alloc()],
                                  encoded_image.alloc());
  ...
  EXPECT_CALL(encoder_callback, OnEncodedImage(_, _, _))
      ...
      .WillRepeatedly(
          testing::Invoke([&](const EncodedImage& encoded_image,
            ...
            encoded_frame[index].set_buffer(new uint8_t[encoded_image.alloc()],
                                            encoded_image.alloc());
