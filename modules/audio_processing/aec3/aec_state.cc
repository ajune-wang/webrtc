...
AecState::AecState(const EchoCanceller3Config& config)
    : data_dumper_(
      ...
      diverged_onset_detector_(data_dumper_.get()) {}
...
AecState::DivergedOnsetDetector::DivergedOnsetDetector(
    ApmDataDumper* data_dumper)
    : data_dumper_(data_dumper) {
  RTC_DCHECK(data_dumper_);
}

void AecState::DivergedOnsetDetector::Detect(bool converged_filter,
                                             const RenderBuffer& render_buffer,
                                             int filter_delay_blocks) {
  int delay_to_use = std::max(0, filter_delay_blocks - 2);

  const auto& x = render_buffer.Block(-delay_to_use)[0];
