...
ScalableVideoControllerL1T2::~ScalableVideoControllerL1T2() = default;

ScalableVideoController::StreamLayersConfig
ScalableVideoControllerL1T2::StreamConfig() const {
...
FrameDependencyStructure ScalableVideoControllerL1T2::DependencyStructure()
    const {
  ...
  structure.templates = {
      Builder().S(0).T(0).Dtis("SS").ChainDiffs({0}).Build(),
      Builder().S(0).T(0).Dtis("SS").ChainDiffs({2}).Fdiffs({2}).Build(),
      Builder().S(0).T(1).Dtis("-D").ChainDiffs({1}).Fdiffs({1}).Build(),
  };
  return structure;
}

std::vector<ScalableVideoController::LayerFrameConfig>
ScalableVideoControllerL1T2::NextFrameConfig(bool restart) {
  if (restart) {
...
absl::optional<GenericFrameInfo> ScalableVideoControllerL1T2::OnEncodeDone(
