...
class FrameReader {
 public:
  ...
  // Reads a next frame. Returns `nullptr` if reading failed.
  virtual rtc::scoped_refptr<I420Buffer> PullFrame() = 0;

  // Reads a next frame. `frame_num` stores unwrapped frame number which can be
  // passed to `ReadFrame` to re-read this frame latter. Returns `nullptr` if
  // reading failed.
  virtual rtc::scoped_refptr<I420Buffer> PullFrame(int* frame_num) = 0;

  // Reads a frame specified by `frame_num`. Returns `nullptr` if reading
  // failed.
  virtual rtc::scoped_refptr<I420Buffer> ReadFrame(int frame_num) = 0;

  // Reads a next frame and scales it. `frame_num` stores unwrapped frame number
  // which can be passed to `ReadFrame` to re-read this frame latter.
  ...
  // Reads a frame specified by `frame_num` and scales it.
  virtual rtc::scoped_refptr<I420Buffer> ReadFrame(int frame_num,
                                                   int desired_width,
                                                   int desired_height) = 0;

  // Total available frame number.
