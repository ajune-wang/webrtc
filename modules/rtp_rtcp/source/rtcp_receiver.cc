...
RTCPReceiver::LastReceivedXrReferenceTimeInfo() const {
  ...
  for (const auto& rrtr : received_rrtrs_) {
    last_xr_rtis.emplace_back(
        /*ssrc=*/rrtr.first, rrtr.second.last_rr_ntp_mid,
        now_ntp - rrtr.second.last_rr_receive_time_ntp_mid);
  }
  return last_xr_rtis;
}
...
void RTCPReceiver::HandleXrReceiveReferenceTime(uint32_t sender_ssrc,
  ...
  rrtr_info.last_rr_ntp_mid = CompactNtp(rrtr.ntp());
  rrtr_info.last_rr_receive_time_ntp_mid = CompactNtp(clock_->CurrentNtpTime());
