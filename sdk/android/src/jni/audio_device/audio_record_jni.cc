...
class ScopedHistogramTimer {
 public:
  ...
  ~ScopedHistogramTimer() {
    ...
    RTC_DLOG(INFO) << histogram_name_ << ": " << life_time_ms;
  }
...
int32_t AudioRecordJni::InitRecording() {
  ...
  if (frames_per_buffer < 0) {
    direct_buffer_address_ = nullptr;
    RTC_DLOG(LS_ERROR) << "InitRecording failed";
    return -1;
  }
...
int32_t AudioRecordJni::StartRecording() {
  ...
  if (!Java_WebRtcAudioRecord_startRecording(env_, j_audio_record_)) {
    RTC_DLOG(LS_ERROR) << "StartRecording failed";
    return -1;
  }
  recording_ = true;
  return 0;
...
int32_t AudioRecordJni::StopRecording() {
  ...
  RTC_HISTOGRAM_BOOLEAN("WebRTC.Audio.SourceMatchesRecordingSession",
                        session_was_ok);
  RTC_DLOG(INFO) << "HISTOGRAM(WebRTC.Audio.SourceMatchesRecordingSession): "
                << session_was_ok;
...
void AudioRecordJni::AttachAudioBuffer(AudioDeviceBuffer* audioBuffer) {
  ...
  RTC_DLOG(INFO) << "SetRecordingSampleRate(" << sample_rate_hz << ")";
...
void AudioRecordJni::DataIsRecorded(JNIEnv* env,
  ...
  if (audio_device_buffer_->DeliverRecordedData() == -1) {
    RTC_DLOG(INFO) << "AudioDeviceBuffer::DeliverRecordedData failed";
  }
}
