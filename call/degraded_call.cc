...
DegradedCall::ScopedTransport::ScopedTransport(Transport* transport)
    : transport_(transport) {}

bool DegradedCall::ScopedTransport::SendRtp(const uint8_t* packet,
                                            size_t length,
                                            const PacketOptions& options) {
...
bool DegradedCall::ScopedTransport::SendRtcp(const uint8_t* packet,
                                             size_t length) {
  rtc::CritScope cs(&lock_);
  if (transport_) {
...
void DegradedCall::ScopedTransport::Stop() {
  rtc::CritScope cs(&lock_);
  transport_ = nullptr;
}

DegradedCall::TransportAdapter::TransportAdapter(
...
bool DegradedCall::TransportAdapter::SendRtp(const uint8_t* packet,
                                             size_t length,
                                             const PacketOptions& options) {
...
bool DegradedCall::TransportAdapter::SendRtcp(const uint8_t* packet,
                                              size_t length) {
  network_pipe_->SendRtcp(packet, length, &real_transport_);
  return true;
}

void DegradedCall::TransportAdapter::Stop() {
  real_transport_.Stop();
}
...
AudioSendStream* DegradedCall::CreateAudioSendStream(
  ...
  if (send_config_) {
    auto transport_adapter = absl::make_unique<TransportAdapter>(
...
void DegradedCall::DestroyAudioSendStream(AudioSendStream* send_stream) {
  ...
  auto it = audio_send_transport_adapters_.find(send_stream);
  if (it != audio_send_transport_adapters_.end()) {
    it->second->Stop();
  }
}
...
VideoSendStream* DegradedCall::CreateVideoSendStream(
  ...
  std::unique_ptr<TransportAdapter> transport_adapter;
  if (send_config_) {
    transport_adapter = absl::make_unique<TransportAdapter>(
        send_pipe_.get(), call_.get(), clock_, config.send_transport);
...
void DegradedCall::DestroyVideoSendStream(VideoSendStream* send_stream) {
  ...
  auto it = video_send_transport_adapters_.find(send_stream);
  if (it != video_send_transport_adapters_.end()) {
    it->second->Stop();
  }
}
