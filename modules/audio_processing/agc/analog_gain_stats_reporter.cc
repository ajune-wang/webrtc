...
constexpr int kMaxAnalogGain = 255;

float ComputeAverageUpdate(int sum_updates, int num_updates) {
  RTC_DCHECK_GE(sum_updates, 0);
  RTC_DCHECK_LE(sum_updates, kMaxAnalogGain * kFramesIn60Seconds);
...
void AnalogGainStatsReporter::UpdateStatistics(int analog_mic_level) {
  ...
  RTC_DCHECK_LE(analog_mic_level, kMaxAnalogGain);
  const bool level_changed =
      (previous_analog_mic_level_.has_value())
