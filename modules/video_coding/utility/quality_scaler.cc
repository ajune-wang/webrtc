...
QualityScaler::QualityScaler(rtc::TaskQueue* task_queue,
    ...
    : observer_(observer),
      ...
      scale_factor_(
          QualityScalerSettings::ParseFromFieldTrials().ScaleFactor().value_or(
              kSamplePeriodScaleFactor)),
      fast_scale_factor_(
...
int64_t QualityScaler::GetSamplingPeriodMs() const {
  ...
  if (fast_scale_factor_ && !last_adapted_) {
    // Last check did not result in a AdaptDown/Up, possibly reduce interval.
    return sampling_period_ms_ * fast_scale_factor_.value();
  }
  return sampling_period_ms_ * scale_factor_;
}
