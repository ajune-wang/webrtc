...
std::vector<webrtc::VideoStream> EncoderStreamFactory::CreateEncoderStreams(
  ...
  if (!encoder_config.encoder_specific_settings) {
    layer.num_temporal_layers = 1;
  } else {
    webrtc::VideoCodec video_codec;
    memset(&video_codec, 0, sizeof(video_codec));
    encoder_config.encoder_specific_settings->FillEncoderSpecificSettings(
        &video_codec);
    if (CodecNamesEq(codec_name_, kVp8CodecName)) {
      layer.num_temporal_layers = video_codec.VP8()->numberOfTemporalLayers;
    } else if (CodecNamesEq(codec_name_, kVp9CodecName)) {
      layer.num_temporal_layers = video_codec.VP9()->numberOfTemporalLayers;
    }
  }
