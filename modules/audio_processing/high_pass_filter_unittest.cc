...
TEST(HighPassFilterAccuracyTest, ResetWithAudioBufferInterface) {
  ...
  HighPassFilter lc(1);
  std::vector<float> y =
      ProcessOneFrameAsAudioBuffer(x_mono, stream_config_mono, &lc);
  lc.Reset(2);
  y = ProcessOneFrameAsAudioBuffer(x_stereo, stream_config_stereo, &lc);
  lc.Reset(1);
  y = ProcessOneFrameAsAudioBuffer(x_mono, stream_config_mono, &lc);
  lc.Reset();
  y = ProcessOneFrameAsAudioBuffer(x_mono, stream_config_mono, &lc);
}
...
TEST(HighPassFilterAccuracyTest, ResetWithVectorInterface) {
  ...
  HighPassFilter lc(1);
  std::vector<float> y =
      ProcessOneFrameAsVector(x_mono, stream_config_mono, &lc);
  lc.Reset(2);
  y = ProcessOneFrameAsVector(x_stereo, stream_config_stereo, &lc);
  lc.Reset(1);
  y = ProcessOneFrameAsVector(x_mono, stream_config_mono, &lc);
  lc.Reset();
  y = ProcessOneFrameAsVector(x_mono, stream_config_mono, &lc);
}
