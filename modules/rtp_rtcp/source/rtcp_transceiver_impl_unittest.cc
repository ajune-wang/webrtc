...
TEST(RtcpTransceiverImplTest, DelaySendingFirstCompondPacket) {
  ...
  int64_t created = rtc::TimeMillis();
  queue.PostTask(
      [&] { rtcp_transceiver = rtc::MakeUnique<RtcpTransceiverImpl>(config); });

  EXPECT_TRUE(transport.WaitPacket(initial_delay_ms + kTaskQueuePrecisionMs));
  EXPECT_GE(rtc::TimeMillis() - created, initial_delay_ms);
...
TEST(RtcpTransceiverImplTest, PeriodicallySendsPackets) {
  const int64_t initial_delay_ms = 0;
  ...
  config.outgoing_transport = &transport;
  config.initial_delay_ms = initial_delay_ms;
  config.min_periodic_report_ms = period_ms;
  config.task_queue = &queue;
  std::unique_ptr<RtcpTransceiverImpl> rtcp_transceiver;
  ...
  EXPECT_TRUE(transport.WaitPacket(period_ms + kTaskQueuePrecisionMs));
  int64_t time_of_2nd_packet_ms = rtc::TimeMillis();

  EXPECT_GE(time_of_2nd_packet_ms - time_of_1st_packet_ms, period_ms);
...
TEST(RtcpTransceiverImplTest, SendCompoundPacketDelaysPeriodicSendPackets) {
  const int64_t initial_delay_ms = 10;
  const int64_t period_ms = 10;
  rtc::TaskQueue queue("rtcp");
  FakeRtcpTransport transport;
  RtcpTransceiverConfig config;
  ...
  EXPECT_FALSE(transport.WaitPacket(period_ms / 2));
  // Send packet now.
  ...
  EXPECT_TRUE(transport.WaitPacket(period_ms + kTaskQueuePrecisionMs));
  EXPECT_GE(rtc::TimeMillis() - time_of_non_periodic_packet_ms, period_ms);
