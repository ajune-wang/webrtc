...
TYPED_TEST(RtpFecTest, WillProtectMediaPacketsWithLargeSequenceNumberGap) {
  ...
  this->media_packets_ =
      this->media_packet_generator_.ConstructMediaPackets(kNumMediaPackets, 1);

  // Overwrite sequence number of last packet, to simulate sequence number gap.
  auto media_packet2 = this->media_packets_.back().get();
  ByteWriter<uint16_t>::WriteBigEndian(&media_packet2->data[2],
                                       kMaxMediaPackets);
