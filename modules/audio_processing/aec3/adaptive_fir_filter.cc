...
void AdaptiveFirFilter::ComputeErl(
    const std::vector<std::array<float, kFftLengthBy2Plus1>>& H2,
    rtc::ArrayView<float> erl) const {
  RTC_DCHECK_EQ(kFftLengthBy2Plus1, erl.size());
  // Update the frequency response and echo return loss for the filter.
  switch (optimization_) {
#if defined(WEBRTC_ARCH_X86_FAMILY)
    case Aec3Optimization::kSse2:
      aec3::UpdateErlEstimator_SSE2(H2, erl);
      break;
#endif
#if defined(WEBRTC_HAS_NEON)
    case Aec3Optimization::kNeon:

      aec3::UpdateErlEstimator_NEON(H2, erl);
      break;
#endif
    default:
      aec3::UpdateErlEstimator(H2, erl);
  }
}
...
void AdaptiveFirFilter::ConstrainAndUpdateImpulseResponse(
  ...
  const size_t current_time_domain_size =
      GetTimeDomainLength(current_size_partitions_);
  if (current_time_domain_size < impulse_response->size()) {
    std::fill(impulse_response->begin() + current_time_domain_size,
              impulse_response->end(), 0.f);
  }
