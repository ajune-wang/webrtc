...
int AudioCodingModuleImpl::PreprocessToAddData(const AudioFrame& in_frame,
  ...
  if (!preprocess_frame_.num_channels_) {
  ...
  *ptr_out = &preprocess_frame_;
  ...
  rtc::ArrayView<const int16_t> resample_src;
  if (down_mix) {
    RTC_DCHECK_GE(audio.size(), preprocess_frame_.samples_per_channel_);
    RTC_DCHECK_GE(audio.size(), in_frame.samples_per_channel_);
    // `down_mix` set to true, implies mono.
    preprocess_frame_.num_channels_ = 1;
    // If resampling is required, the output of a down-mix is written into a
    // local buffer, otherwise, it will be written to the output frame.
    auto downmixed =
        resample
            ? rtc::ArrayView<int16_t>(audio.data(),
                                      in_frame.samples_per_channel_ *
                                          preprocess_frame_.num_channels_)
            : preprocess_frame_.mutable_data(in_frame.samples_per_channel_,
                                             preprocess_frame_.num_channels_);
    DownMixFrame(in_frame, downmixed);

    if (resample) {
      // Set the input of the resampler to the down-mixed signal.
      resample_src = downmixed;
    }
  } else {
    // Set the input of the resampler to the original data.
    resample_src = in_frame.data_view();
    preprocess_frame_.num_channels_ = in_frame.num_channels_;
  }
  ...
  if (resample) {
    RTC_DCHECK(!resample_src.empty());
    // The result of the resampler is written to output frame.
    preprocess_frame_.sample_rate_hz_ = encoder_stack_->SampleRateHz();
    int samples_per_channel = resampler_.Resample10Msec(
        resample_src, in_frame.sample_rate_hz_,
        preprocess_frame_.sample_rate_hz_, preprocess_frame_.num_channels_,
        preprocess_frame_.mutable_data(preprocess_frame_.sample_rate_hz_ / 100,
                                       preprocess_frame_.num_channels_));
    ...
    RTC_DCHECK_EQ(preprocess_frame_.samples_per_channel_,
