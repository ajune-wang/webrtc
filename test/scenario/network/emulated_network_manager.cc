...
void EmulatedNetworkManager::AddEmulatedEndpoint(EmulatedEndpoint* endpoint) {
  {
    rtc::CritScope crit(&endpoints_lock_);
...
void EmulatedNetworkManager::RemoveEmulatedEndpoint(
    EmulatedEndpoint* endpoint) {
  {
    rtc::CritScope crit(&endpoints_lock_);
    bool removed = false;
    for (auto it = endpoints_.begin(); it != endpoints_.end(); ++it) {
      if ((*it)->GetId() == endpoint->GetId()) {
        endpoints_.erase(it);
        removed = true;
        break;
      }
    }
    RTC_CHECK(removed) << "No such interface: "
