...
// Provides a default implementation for OpenSSL that builds the SSL Trusted
// Root Certificate SSLCertChain from the available certificates in the
// sslroots.h header file. In certain builds this header file may simply
// not exist in which case this will return an empty SSLCertChain.
class OpenSSLBuiltinRootCertLoader : public SSLRootCertLoader {
 public:
  OpenSSLBuiltinRootCertLoader() = default;
  ~OpenSSLBuiltinRootCertLoader() override;
  // Return the SSL Certificate Roots in sslroots.h.
  SSLCertChain Load() override;

 private:
  RTC_DISALLOW_COPY_AND_ASSIGN(OpenSSLBuiltinRootCertLoader);
};
