...
TEST(AcmRemixing, DownMixFrame) {
  ...
  int16_t* in_data = in.mutable_data();
  for (size_t k = 0; k < in.samples_per_channel_; ++k) {
    in_data[2 * k] = 2;
    in_data[2 * k + 1] = 0;
  }

  DownMixFrame(in, out.size(), out.data());
...
TEST(AcmRemixing, Remix3ChannelFrameToStereo) {
  ...
  for (size_t k = 0; k < in.samples_per_channel_; ++k) {
    for (size_t j = 0; j < 2; ++j) {
      EXPECT_EQ(out[2 * k + j], static_cast<int>(j));
    }
  }
}
