...
VideoSenderInfo WebRtcVideoChannel::WebRtcVideoSendStream::GetVideoSenderInfo(
  ...
  for (std::map<uint32_t, webrtc::VideoSendStream::StreamStats>::iterator it =
    ...
    if (!stream_stats.is_flexfec) {
      ...
      // will be |stream_stats| with "is_rtx == true". Since we are aggregating
      // all substreams' counters into a single "info" we do not need to know
      // the relationship between RTX streams and RTP streams here.
      ...
      // accounts for RTX retransmissions.
