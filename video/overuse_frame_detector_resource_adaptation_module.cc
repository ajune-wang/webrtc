...
void OveruseFrameDetectorResourceAdaptationModule::OnInputFrame(
    const VideoFrame& frame) {
  last_input_frame_size_ = frame.size();
}

void OveruseFrameDetectorResourceAdaptationModule::
    OnInputFrameDroppedDueToSize() {
...
void OveruseFrameDetectorResourceAdaptationModule::OnInputFrameEncodeStarted(
...
void OveruseFrameDetectorResourceAdaptationModule::OnInputFrameEncodeCompleted(
    uint32_t timestamp,
...
int OveruseFrameDetectorResourceAdaptationModule::LastInputFrameSizeOrDefault()
    const {
  // This 144p resolution was picked because it is the resolution that
  // video_stream_encoder.cc picks for its |last_frame_info_| when it doesn't
  // know the frame size at VideoStreamEncoder::ConfigureEncoder().
  ...
  // OnInputFrame()) and deserves a standalone CL.
  return last_input_frame_size_.value_or(176 * 144);
}
...
OveruseFrameDetectorResourceAdaptationModule::GetQpThresholds() const {
  RTC_DCHECK(last_input_frame_size_.has_value());
