...
int LibvpxVp8Encoder::InitEncode(const VideoCodec* inst,
  ...
  UpdateVpxConfiguration(temporal_layers_[stream_idx].get(),
                         &configurations_[0]);
  if (!temporal_layers_[stream_idx]->SupportsEncoderFrameDropping()) {
    configurations_[stream_idx].rc_dropframe_thresh = 0;
  }
  --stream_idx;
...
int LibvpxVp8Encoder::GetEncodedPartitions(const VideoFrame& input_image) {
  ...
  for (size_t encoder_idx = 0; encoder_idx < encoders_.size();
    ...
    // TODO(sprang): Move temporal layers checker into temporal layers.
    //    if (result != WEBRTC_VIDEO_CODEC_TARGET_BITRATE_OVERSHOOT) {
    //      // Don't run checker on drop before reencode as that will
    //      incorrectly
    //      // increase the pattern index twice.
    //      RTC_DCHECK(
    //        temporal_layers_checkers_[stream_idx]->CheckTemporalConfig(
