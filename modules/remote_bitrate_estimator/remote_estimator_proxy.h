...
class RemoteEstimatorProxy : public RemoteBitrateEstimator {
 ...
 private:
  ...
  // `end_sequence_number_excl`) that can fit in it. If
  // `update_periodic_window_start_seq`, it updates the
  // `periodic_window_start_seq_` variable with the first packet that was not
  // included in the feedback packet. If no incoming packets were added, nullptr
  // is returned.
  //
  ...
  std::unique_ptr<rtcp::TransportFeedback> BuildFeedbackPacket(
      ...
      bool update_periodic_window_start_seq)
      RTC_EXCLUSIVE_LOCKS_REQUIRED(&lock_);
