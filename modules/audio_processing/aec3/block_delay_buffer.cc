...
void BlockDelayBuffer::DelaySignal(AudioBuffer* frame) {
  ...
  const size_t num_bands = buf_.size();
  const size_t num_channels = buf_[0].size();
  ...
  for (size_t ch = 0; ch < num_channels; ++ch) {
    ...
    for (size_t band = 0; band < num_bands; ++band) {
      RTC_DCHECK_EQ(delay_, buf_[band][ch].size());
      i = i_start;

      for (size_t k = 0; k < frame_length_; ++k) {
        const float tmp = buf_[band][ch][i];
        buf_[band][ch][i] = frame_ch[band][k];
        frame_ch[band][k] = tmp;
...
void BlockDelayBuffer::DelaySignal(
  ...
  for (size_t ch = 0; ch < num_channels; ++ch) {
    for (size_t band = 0; band < num_bands; ++band) {
      RTC_DCHECK_EQ(buf_[band].size(), (*frame)[band].size());
      RTC_DCHECK_EQ(delay_, buf_[band][ch].size());
      RTC_DCHECK_EQ(frame_length_, (*frame)[band][ch].size());
      i = i_start;

      for (size_t k = 0; k < frame_length_; ++k) {
        const float tmp = buf_[band][ch][i];
        buf_[band][ch][i] = (*frame)[band][ch][k];
        (*frame)[band][ch][k] = tmp;
