...
class TransportController : public sigslot::has_slots<>,
 ...
 public:
  ...
  // WebRtcSession and BaseChannel to them instead of CreateDtlsTransport.
 ...
 private:
  ...
  struct RtpTransportWrapper {
    ...
    // Non-null if using SDES.
    webrtc::SrtpTransport* srtp_transport = nullptr;
    // Non-null if using DTLS-SRTP.
  ...
  // TODO(zhihuang): Remove all the "_n" version of the methods and create one
  // helper to check if the methods are called on correct thread, if not, invoke
  // the methods on the correct thread.
