...
class NetEqFacade {
 public:
  ...
  // Updates the target buffer filter. Current buffer size is
  // |buffer_size_packets| (Q0). |time_stretched_samples| is subtracted from the
  // filtered value (thus bypassing the filter operation).
  virtual void UpdateTargetBufferFilter(size_t buffer_size_samples,
  ...
  virtual void ReportRelativePacketArrivalDelay(int delay_ms) = 0;

 protected:
  NetEqFacade();
};

//// This interface contains only the needed functionality for NetEqController.
// class PacketBufferInterface {
// public:
//  virtual ~PacketBufferInterface();
//  // Returns true if the packet buffer contains any DTX or CNG packets.
//  virtual bool ContainsDtxOrCngPacket() const = 0;
...
//// This interface contains only the needed functionality for NetEqController.
// class BufferLevelFilterInterface {
// public:
//  virtual ~BufferLevelFilterInterface();
...
class NetEqController {
 public:
  ...
  //  // Give NetEqController information about an arriving packet.
  //  virtual void Update(uint16_t sequence_number,
  //                      uint32_t timestamp,
  //                      uint32_t last_played_out_timestamp,
  //                      bool new_codec,
  //                      bool cng_or_dtmf,
  //                      size_t packet_length_samples) = 0;
