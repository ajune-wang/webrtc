...
class SctpTransport::UsrSctpWrapper {
 public:
  static void InitializeUsrSctp() {
    NLOG(LS_INFO, __FUNCTION__);  // First argument is udp_encapsulation_port,
                                  // which is not releveant for our
    // AF_CONN use of sctp.
    ...
    if (send_size != kSendBufferSize) {
      NLOG(LS_ERROR, "Got different send size than expected: ", send_size);
    }
  ...
  static void UninitializeUsrSctp() {
    NLOG(LS_INFO, __FUNCTION__);  // usrsctp_finish() may fail if it's called
                                  // too soon after the transports
    // are
    ...
    NLOG(LS_ERROR, "Failed to shutdown usrsctp.");
  }
  ...
  static int OnSctpInboundPacket(struct socket* sock,
    ...
    if (!GetDataMediaType(ppid, &type) && !(flags & MSG_NOTIFICATION)) {
      ...
      NLOG(LS_ERROR, "Received an unknown PPID ", ppid,
  ...
  static int SendThresholdCallback(struct socket* sock, uint32_t sb_free) {
    ...
    if (!transport) {
      NLOG(LS_ERROR,
           "SendThresholdCallback: Failed to get transport for socket ", 0);
      return 0;
    }
...
bool SctpTransport::Start(int local_sctp_port, int remote_sctp_port) {
  ...
  if (started_) {
    if (local_sctp_port != local_port_ || remote_sctp_port != remote_port_) {
      NLOG(LS_ERROR, "Can't change SCTP port after SCTP association formed.");
      return false;
    }
    return true;
  }
...
bool SctpTransport::OpenStream(int sid) {
  RTC_DCHECK_RUN_ON(network_thread_);
  if (sid > kMaxSctpSid) {
    NLOG(LS_WARNING, debug_name_,
         "->OpenStream(...): ", "Not adding data stream ", "with sid=", sid,
         " because sid is too high.");
    return false;
...
bool SctpTransport::ResetStream(int sid) {
  ...
  if (found == open_streams_.end()) {
    NLOG(LS_WARNING, debug_name_, "->ResetStream(", sid,
         "): ", "stream not found.");
    return false;
  } else {
    NLOG(LS_VERBOSE, debug_name_, "->ResetStream(", sid,
...
bool SctpTransport::SendData(const SendDataParams& params,
  ...
  if (!sock_) {
    NLOG(LS_WARNING, debug_name_,
         "->SendData(...): ", "Not sending packet with sid=", params.sid,
  ...
  if (params.type != DMT_CONTROL &&
    ...
    NLOG(LS_WARNING, debug_name_, "->SendData(...): ",
         "Not sending data because sid is unknown: ", params.sid);
    return false;
  }
  ...
  if (send_res < 0) {
    if (errno == SCTP_EWOULDBLOCK) {
      *result = SDR_BLOCK;
      ...
      NLOG(LS_INFO, debug_name_, "->SendData(...): EWOULDBLOCK returned");
    } else {
...
bool SctpTransport::OpenSctpSocket() {
  RTC_DCHECK_RUN_ON(network_thread_);
  if (sock_) {
    NLOG(LS_WARNING, debug_name_, "->OpenSctpSocket(): ",
...
bool SctpTransport::SendQueuedStreamResets() {
  ...
  NLOG(LS_VERBOSE, "SendQueuedStreamResets[", debug_name_, "]: Sending [",
...
void SctpTransport::OnPacketRead(rtc::PacketTransportInternal* transport,
  ...
  NLOG(LS_VERBOSE, debug_name_, "->OnPacketRead(...): ", " length=", len,
       ", started: ", started_);  // Only give receiving packets to usrsctp
                                  // after if connected. This enables two
...
void SctpTransport::OnPacketFromSctpToNetwork(
  ...
  if (buffer.size() > (kSctpMtu)) {
    NLOG(LS_ERROR, debug_name_, "->OnPacketFromSctpToNetwork(...): ",
...
void SctpTransport::OnInboundPacketFromSctpToChannel(
    ...
    int flags) {
  RTC_DCHECK_RUN_ON(network_thread_);
  NLOG(LS_VERBOSE, debug_name_, "->OnInboundPacketFromSctpToChannel(...): ",
       "Received SCTP data:", " sid=", params.sid,
       " notification: ", (flags & MSG_NOTIFICATION),
       " length=", buffer.size());  // Sending a packet with data == NULL (no
                                    // data) is SCTPs "close the
  // connection" message. This sets sock_ = NULL;
  if (!buffer.size() || !buffer.data()) {
    NLOG(LS_INFO, debug_name_,
...
void SctpTransport::OnDataFromSctpToChannel(
    ...
    const rtc::CopyOnWriteBuffer& buffer) {
  RTC_DCHECK_RUN_ON(network_thread_);
  NLOG(LS_VERBOSE, debug_name_, "->OnDataFromSctpToChannel(...): ",
       "Posting with length: ", buffer.size(), " on stream ",
       params.sid);  // Reports all received messages to upper layers, no matter
                     // whether the sid
  // is known.
...
void SctpTransport::OnNotificationFromSctp(
  ...
  switch (notification.sn_header.sn_type) {
    case SCTP_ASSOC_CHANGE:
      NLOG(LS_VERBOSE, "SCTP_ASSOC_CHANGE");
      OnNotificationAssocChange(notification.sn_assoc_change);
      break;
    case SCTP_REMOTE_ERROR:
      NLOG(LS_INFO, "SCTP_REMOTE_ERROR");
      break;
    case SCTP_SHUTDOWN_EVENT:
      NLOG(LS_INFO, "SCTP_SHUTDOWN_EVENT");
      break;
    case SCTP_ADAPTATION_INDICATION:
      NLOG(LS_INFO, "SCTP_ADAPTATION_INDICATION");
      break;
    case SCTP_PARTIAL_DELIVERY_EVENT:
      NLOG(LS_INFO, "SCTP_PARTIAL_DELIVERY_EVENT");
      break;
    case SCTP_AUTHENTICATION_EVENT:
      NLOG(LS_INFO, "SCTP_AUTHENTICATION_EVENT");
      break;
    case SCTP_SENDER_DRY_EVENT:
      NLOG(LS_VERBOSE, "SCTP_SENDER_DRY_EVENT");
      SetReadyToSendData();
      break;
    // TODO(ldixon): Unblock after congestion.
    case SCTP_NOTIFICATIONS_STOPPED_EVENT:
      NLOG(LS_INFO, "SCTP_NOTIFICATIONS_STOPPED_EVENT");
      break;
    case SCTP_SEND_FAILED_EVENT:
      NLOG(LS_INFO, "SCTP_SEND_FAILED_EVENT");
      break;
    case SCTP_STREAM_RESET_EVENT:
      OnStreamResetEvent(&notification.sn_strreset_event);
      break;
    case SCTP_ASSOC_RESET_EVENT:
      NLOG(LS_INFO, "SCTP_ASSOC_RESET_EVENT");
      break;
    case SCTP_STREAM_CHANGE_EVENT:
      NLOG(LS_INFO, "SCTP_STREAM_CHANGE_EVENT");  // An acknowledgment we get
                                                  // after our stream resets
                                                  // have gone through,
      ...
      break;
    default:
      NLOG(LS_WARNING, "Unknown SCTP event: ", notification.sn_header.sn_type);
      break;
  }
}
...
void SctpTransport::OnNotificationAssocChange(const sctp_assoc_change& change) {
  ...
  switch (change.sac_state) {
    case SCTP_COMM_UP:
      NLOG(LS_VERBOSE, "Association change SCTP_COMM_UP");
      break;
    case SCTP_COMM_LOST:
      NLOG(LS_INFO, "Association change SCTP_COMM_LOST");
      break;
    case SCTP_RESTART:
      NLOG(LS_INFO, "Association change SCTP_RESTART");
      break;
    case SCTP_SHUTDOWN_COMP:
      NLOG(LS_INFO, "Association change SCTP_SHUTDOWN_COMP");
      break;
    case SCTP_CANT_STR_ASSOC:
      NLOG(LS_INFO, "Association change SCTP_CANT_STR_ASSOC");
      break;
    default:
      NLOG(LS_INFO, "Association change UNKNOWN");
      break;
  }
}
...
void SctpTransport::OnStreamResetEvent(
  ...
  NLOG(LS_VERBOSE, "SCTP_STREAM_RESET_EVENT(", debug_name_, "): Flags = 0x",
       evt->strreset_flags, " (", ListFlags(evt->strreset_flags), ")");
  NLOG(LS_VERBOSE, "Assoc = ", evt->strreset_assoc_id, ", Streams = [",
       ListArray(evt->strreset_stream_list, num_sids), "], Open: [",
  ...
  if (evt->strreset_flags & SCTP_STREAM_RESET_FAILED) {
    ...
    for (int i = 0; i < num_sids; i++) {
      ...
      if (it != sent_reset_streams_.end()) {
        NLOG(LS_VERBOSE, "SCTP_STREAM_RESET_EVENT(", debug_name_,
        ...
        // queued.
        NLOG(LS_VERBOSE, "SCTP_STREAM_RESET_EVENT(", debug_name_,
             "): double-sided close for sid ",
             stream_id);  // Both sides want the stream closed, and the peer got
                          // to send the
