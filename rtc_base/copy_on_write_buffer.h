...
class RTC_EXPORT CopyOnWriteBuffer {
 public:
  ...
  void SetData(const T* data, size_t size) {
    RTC_DCHECK(IsConsistent());
    if (!buffer_) {
      buffer_ = size > 0 ? new Storage(data, size) : nullptr;
    } else if (!buffer_->HasOneRef()) {
      buffer_ = new Storage(data, size, capacity());
    } else {
 ...
 private:
  using Storage = FinalRefCountedObject<Buffer>;
