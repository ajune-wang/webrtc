...
void EchoCanceller3::ProcessCapture(AudioBuffer* capture,
  ...
  if (linear_output && !linear_output_framer_) {
    RTC_LOG(LS_ERROR) << "AEC3: Lazy initialization of the linear_output. "
                         "Please avoid by properly preconfiguring AEC3";
    SetupLinearOutputPath();
  }
...
void EchoCanceller3::SetupLinearOutputPath() {
  RTC_DCHECK_RUNS_SERIALIZED(&capture_race_checker_);
