...
class DelayBasedNetworkController : public NetworkControllerInternalInterface {
 public:
  DelayBasedNetworkController(const Clock* clock, RtcEventLog* event_log);
  ~DelayBasedNetworkController() override;
  units::TimeDelta GetProcessInterval() override;
  void ConnectHandlers(NetworkInformationHandlers::uptr) override;
 ...
 private:
  ...
  std::unique_ptr<NetworkControlJunctions> junctions_;
