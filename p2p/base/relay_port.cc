...
void RelayPort::AddExternalAddress(const ProtocolAddress& addr) {
  ...
  for (std::vector<ProtocolAddress>::iterator it = external_addr_.begin();
    ...
    if ((it->address == addr.address) && (it->proto == addr.proto)) {
      RTC_DLOG(INFO) << "Redundant relay address: " << proto_name << " @ "
...
void RelayConnection::OnSendPacket(const void* data,
  ...
  if (sent <= 0) {
    RTC_DLOG(LS_VERBOSE) << "OnSendPacket: failed sending to "
...
void RelayEntry::Connect() {
  ...
  if (!ra) {
    RTC_DLOG(LS_WARNING) << "No more relay addresses left to try";
    return;
  }
  ...
  RTC_DLOG(LS_INFO) << "Connecting to relay via " << ProtoToString(ra->proto)
  ...
  if (ra->proto == PROTO_UDP) {
    ...
    RTC_DLOG(LS_WARNING) << "Unknown protocol: " << ra->proto;
  }

  // If we failed to get a socket, move on to the next protocol.
  if (!socket) {
    RTC_DLOG(LS_WARNING) << "Socket creation failed";
...
void RelayEntry::OnConnect(const rtc::SocketAddress& mapped_addr,
  ...
  RTC_DLOG(INFO) << "Relay allocate succeeded: " << ProtoToString(proto) << " @ "
...
void RelayEntry::OnMessage(rtc::Message* pmsg) {
  ...
  if (current_connection_) {
    ...
    RTC_DLOG(LS_WARNING) << "Relay " << ra->proto << " connection to "
...
void RelayEntry::OnSocketConnect(rtc::AsyncPacketSocket* socket) {
  RTC_DLOG(INFO) << "relay tcp connected to "
...
void RelayEntry::OnReadPacket(rtc::AsyncPacketSocket* socket,
  ...
  if (current_connection_ == NULL || socket != current_connection_->socket()) {
    ...
    RTC_DLOG(WARNING) << "Dropping packet: unknown address";
    return;
  }
  ...
  if (!port_->HasMagicCookie(data, size)) {
    if (locked_) {
      ...
      RTC_DLOG(WARNING) << "Dropping packet: entry not locked";
    }
    return;
  }
  ...
  RelayMessage msg;
  if (!msg.Read(&buf)) {
    RTC_DLOG(INFO) << "Incoming packet was not STUN";
    return;
  }
  ...
  if (current_connection_->CheckResponse(&msg)) {
    ...
    RTC_DLOG(INFO) << "Received BAD stun type from server: " << msg.type();
    return;
  }
  ...
  if (!addr_attr) {
    RTC_DLOG(INFO) << "Data indication has no source address";
    return;
  } else if (addr_attr->family() != 1) {
    RTC_DLOG(INFO) << "Source address has bad family";
    return;
  }
  ...
  if (!data_attr) {
    RTC_DLOG(INFO) << "Data indication has no data";
    return;
  }
...
void AllocateRequest::OnResponse(StunMessage* response) {
  ...
  if (!addr_attr) {
    RTC_DLOG(INFO) << "Allocate response missing mapped address.";
  } else if (addr_attr->family() != 1) {
    RTC_DLOG(INFO) << "Mapped address has bad family";
  } else {
...
void AllocateRequest::OnErrorResponse(StunMessage* response) {
  ...
  if (!attr) {
    RTC_DLOG(LS_ERROR) << "Missing allocate response error code.";
  } else {
    RTC_DLOG(INFO) << "Allocate error response: code=" << attr->code()
                  << " reason=" << attr->reason();
  }
...
void AllocateRequest::OnTimeout() {
  RTC_DLOG(INFO) << "Allocate request timed out";
