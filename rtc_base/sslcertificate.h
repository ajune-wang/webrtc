...
// SSLRootCertificateLoader is very simple interface that loads the root
// certificates into a SSLCertChain object. This is injected into the
// This allows applications to share their root certificates with WebRTC
// instead of requiring WebRTC to always compile in roots of trust. It is up
// to the loader to load these certificates into any required contexts in its
// concrete implementation.
class SSLRootCertLoader {
 public:
  virtual ~SSLRootCertLoader() = default;
  // Return the SSL Certificate Roots as a SSLCertChain.
  virtual SSLCertChain Load() = 0;
};
