...
// Computes the difference between the target speech level range and the
// estimated speech level. Returns a positive value when the speech level is
// below the target range and a negative value when the speech level is above
// the target range.
// The value of `speech_level_dbfs` is required to be in the [-90.0f, 30.0f]
// range.
...
void MonoInputVolumeController::SetInputVolume(int new_volume) {
  ...
  if (applied_input_volume == 0) {
    RTC_DLOG(LS_INFO) << "The applied input volume is zero, taking no action.";
    return;
  }
  if (applied_input_volume < 0 || applied_input_volume > kMaxMicInputVolume) {
    RTC_LOG(LS_ERROR) << "Invalid value for the applied input volume: "
  ...
  if (applied_input_volume > input_volume_ + kVolumeQuantizationSlack ||
    ...
    RTC_DLOG(LS_INFO) << "The input volume was manually adjusted. Updating "
  ...
  RTC_DLOG(LS_INFO) << "Applied input volume: " << applied_input_volume
...
void MonoInputVolumeController::SetMaxLevel(int input_volume) {
  ...
  RTC_DLOG(LS_INFO) << "Maximum input volume updated: " << max_input_volume_;
}
...
int MonoInputVolumeController::CheckVolumeAndReset() {
  ...
  RTC_DLOG(LS_INFO) << "Initial input volume: " << input_volume;

  if (input_volume < min_input_volume_) {
    input_volume = min_input_volume_;
    RTC_DLOG(LS_INFO) << "The initial input volume is too low, raising to "
                      << input_volume;
...
void MonoInputVolumeController::UpdateInputVolume(int rms_error_dbfs) {
  ...
  // error.
  rms_error_dbfs =
      rtc::SafeClamp(rms_error_dbfs, -KMaxAbsRmsErrorDbfs, KMaxAbsRmsErrorDbfs);
  if (rms_error_dbfs == 0) {
    return;
  }
  SetInputVolume(
      ComputeVolumeUpdate(rms_error_dbfs, input_volume_, min_input_volume_));
}
...
InputVolumeController::InputVolumeController(int num_capture_channels,
  ...
  RTC_LOG(LS_INFO) << "Input volume controller enabled. Minimum input volume: "
                   << min_input_volume_;
...
void InputVolumeController::AnalyzePreProcess(const AudioBuffer& audio_buffer) {
  ...
  if (clipping_detected) {
    RTC_DLOG(LS_INFO) << "Clipping detected (ratio: " << clipped_ratio << ")";
  }

  int step = clipped_level_step_;
  if (clipping_predicted) {
    predicted_step = std::max(predicted_step, clipped_level_step_);
    RTC_DLOG(LS_INFO) << "Clipping predicted (volume down step: "
                      << predicted_step << ")";
