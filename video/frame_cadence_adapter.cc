...
void ZeroHertzAdapterMode::OnFrame(Timestamp post_time,
  ...
  queue_->PostDelayedHighPrecisionTask(
      SafeTask(safety_.flag(),
               [this, post_time, frames_scheduled_for_processing, frame_id,
                frame] {
                 RTC_UNUSED(frame_id);
                 RTC_DCHECK_RUN_ON(&sequence_checker_);
...
void ZeroHertzAdapterMode::ProcessOnDelayedCadence(
  ...
  SendFrameNow(post_time, frames_scheduled_for_processing, front_frame);
}
...
void ZeroHertzAdapterMode::ProcessRepeatedFrameOnDelayedCadence(int frame_id) {
  ...
  // Mark `frames_scheduled_for_processing` with 0 to signal that this is a
  // repeated frame.
  SendFrameNow(scheduled_repeat_->origin,
               /*frames_scheduled_for_processing=*/0, frame);
}
...
void ZeroHertzAdapterMode::SendFrameNow(Timestamp post_time,
  ...
  if (frames_scheduled_for_processing == 0) {
    ...
    RTC_HISTOGRAM_COUNTS_10000("WebRTC.Screenshare.ZeroHz.RepeatDelayMs",
                               delay.ms());
  } else {
    RTC_HISTOGRAM_COUNTS_10000("WebRTC.Screenshare.ZeroHz.NoRepeatDelayMs",
                               delay.ms());
  }
  callback_->OnFrame(/*post_time=*/now, frames_scheduled_for_processing, frame);
}
