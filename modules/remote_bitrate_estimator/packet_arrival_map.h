...
class PacketArrivalTimeMap {
 public:
  ...
  bool IsConsistentForTesting() const;

 private:
  static constexpr int kMinCapacity = 128;

  // Returns index in buffer_ where value for `sequence_number` should be.
  ...
  // Sets values in the given range to values representing not received packet.
  void Clear(int64_t begin_sequence_number_inclusive,
             int64_t end_sequence_number_exclusive);

  // Removes values representing not received packet at the beginning of
  // the valid range.
  void TrimLeadingEmptyEntries();
  ...
  Timestamp* arrival_times_ = nullptr;

  // Allocated size of the `arrival_times_`
  // capacity_ is a power of 2 in range [kMinCapacity, kMaxNumberOfPackets]
  // `capacity - 1` is used much more often than `capacity`, thus prefer to
  // store that value.
  int capacity_minus_1_ = -1;

  // The unwrapped sequence number for valid range of sequence numbers.
  // arrival_times_ entries only valid for sequence number in range
  // `begin_sequence_number_ <= sequence_number < end_sequence_number`
