...
class ScalabilityStructureFullSvc : public ScalableVideoController {
 public:
  ...
  int NumSpatialLayers() const { return num_spatial_layers_; }
  int NumTemporalLayers() const { return num_temporal_layers_; }
  std::bitset<32> ActiveDecodeTargets() const { return active_decode_targets_; }
 ...
 private:
  ...
  // TODO(danilchap): Support up to 3 temporal layers.
  ...
  bool use_temporal_dependency_on_t0_[kMaxNumSpatialLayers] = {false, false,
                                                               false};
