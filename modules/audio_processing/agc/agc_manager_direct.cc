...
AgcManagerDirect::AgcManagerDirect(
  ...
  constexpr int kMinMicLevelMaybeOverridden =
      kMinMicLevelOverride.value_or(kMinMicLevel);
  RTC_LOG(LS_INFO) << "[agc] Min mic level: " << kMinMicLevelMaybeOverridden
                   << " (overridden: " << kMinMicLevelOverride.has_value()
                   << ")";
  for (size_t ch = 0; ch < channel_agcs_.size(); ++ch) {
    ...
    channel_agcs_[ch] = std::make_unique<MonoAgc>(
        ...
        disable_digital_adaptive_, kMinMicLevelMaybeOverridden);
  }
...
void AgcManagerDirect::AggregateChannelLevels() {
  ...
  if (kMinMicLevelOverride.has_value()) {
    stream_analog_level_ =
        std::max(stream_analog_level_, *kMinMicLevelOverride);
  }
}
