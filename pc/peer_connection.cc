  ...
  if (desc->GetType() == SdpType::kRollback) {
    if (IsUnifiedPlan()) {
      if (Rollback()) {
        ...
        PostSetSessionDescriptionFailure(
            observer, RTCError(RTCErrorType::INVALID_STATE,
...
bool PeerConnection::Rollback() {
  ...
  for (auto&& transceivers_stable_state_pair :
    ...
    std::string mid =
        transceivers_stable_state_pair.first->internal()->mid().value_or("");
    ...
    DestroyTransceiverChannel(transceivers_stable_state_pair.first);

    if (transceivers_stable_state_pair.second.created) {
      // Remove added transceivers with no added track.
      if (!transceivers_stable_state_pair.first->internal()
               ->sender()
               ->track()) {
        int remaining_transceiver_count = 0;
        for (auto&& transceiver : transceivers_) {
          if (transceiver != transceivers_stable_state_pair.first) {
            transceivers_[remaining_transceiver_count++] = transceiver;
          }
        }
    ...
    transceivers_stable_state_pair.first->internal()
        ->sender_internal()
        ->set_transport(nullptr);
    transceivers_stable_state_pair.first->internal()
        ->receiver_internal()
        ->set_transport(nullptr);
    transceivers_stable_state_pair.first->internal()->set_direction(
        transceivers_stable_state_pair.second.direction);
    transceivers_stable_state_pair.first->internal()->set_mid(
        transceivers_stable_state_pair.second.mid);
    transceivers_stable_state_pair.first->internal()->set_mline_index(
        transceivers_stable_state_pair.second.mline_index);
  }
  transceiver_stable_states_by_transceivers_.clear();
