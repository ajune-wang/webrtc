...
class SendSideCongestionControllerTest : public ::testing::Test {
 protected:
  ...
  NetworkControllerConfig InitialControllerConfig() {
    NetworkControllerConfig config;
    config.constraints.min_data_rate = DataRate::bps(0);
    config.constraints.max_data_rate = DataRate::bps(5 * kInitialBitrateBps);
    config.starting_rate = DataRate::bps(kInitialBitrateBps);
    return config;
  }
