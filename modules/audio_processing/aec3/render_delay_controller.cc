...
rtc::Optional<DelayEstimate> RenderDelayControllerImpl::GetDelay(
  ...
  if (delay_samples) {
    ...
    if (delay_samples_) {
      delay_samples_->blocks_since_last_change_ =
          delay_samples_->delay == delay_samples->delay
              ? delay_samples_->blocks_since_last_change_ + 1
              : 0;
      delay_samples_->blocks_since_last_updatechange_ = 0;
      delay_samples_->delay = delay_samples->delay;
    ...
    if (delay_samples_) {
      ++delay_samples_.blocks_since_last_change_;
      ++delay_samples_.blocks_since_last_update_;
    }
  }
