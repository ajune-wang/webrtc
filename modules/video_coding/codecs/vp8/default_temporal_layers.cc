...
std::array<bool, DefaultTemporalLayers::kNumReferenceBuffers>
DefaultTemporalLayers::DetermineStaticBuffers(
    const std::vector<DependencyInfo>& temporal_pattern) {
  std::array<bool, DefaultTemporalLayers::kNumReferenceBuffers> buffers;
  buffers.fill(true);
  for (const DependencyInfo& info : temporal_pattern) {
    ...
    for (Vp8BufferReference buffer : kAllBuffers) {
      if (static_cast<uint8_t>(buffer) & updated_buffers) {
        buffers[BufferToIndex(buffer)] = false;
      }
    }
  }
  return buffers;
...
Vp8FrameConfig DefaultTemporalLayers::NextFrameConfig(size_t stream_index,
  ...
  RTC_DCHECK(!checker_ ||
             checker_->CheckTemporalConfig(first_frame, tl_config));
...
void DefaultTemporalLayers::OnEncodeDone(size_t stream_index,
  ...
  if (is_keyframe) {
    ...
    RTC_DCHECK(!checker_ || checker_->CheckTemporalConfig(true, frame_config));
  }
