...
TEST(RtpPacketizerVideoGeneric, HasFrameId_WritesExtendedHeader) {
  const size_t kMaxPayloadLen = 6;
  const size_t kLastPacketReductionLen = 2;
  const size_t kPayloadSize = 13;
  ...
  EXPECT_TRUE(payload[0] & 0x80);  // Extended header bit is set.
  // Frame id is 37.
  EXPECT_EQ((uint8_t)0, payload[1]);
  EXPECT_EQ((uint8_t)37, payload[2]);
}

TEST(RtpPacketizerVideoGeneric, FrameIdOver15bits_WrapsAround) {
  const size_t kMaxPayloadLen = 6;
  const size_t kLastPacketReductionLen = 2;
  const size_t kPayloadSize = 13;
  ...
  EXPECT_TRUE(payload[0] & 0x80);  // Extended header bit is set.
  // Frame id is 137.
  EXPECT_EQ((uint8_t)0x01, payload[1]);
  EXPECT_EQ((uint8_t)0x37, payload[2]);
}

TEST(RtpPacketizerVideoGeneric, NoFrameId_DoesNotWriteExtendedHeader) {
  const size_t kMaxPayloadLen = 6;
  const size_t kLastPacketReductionLen = 2;
  const size_t kPayloadSize = 13;
  ...
  rtc::ArrayView<const uint8_t> payload = packet.payload();
  EXPECT_FALSE(payload[0] & 0x80);
}

TEST(RtpDepacketizerVideoGeneric, NonExtendedHeader_NoFrameId) {
...
TEST(RtpDepacketizerVideoGeneric, ExtendedHeader_ParsesFrameId) {
  const size_t kPayloadLen = 3;
  uint8_t payload[kPayloadLen] = {0x81, 0x13, 0x37};
