...
// Given a `raw_frame` and a `compressed_frame`, calculates the corruption score
// of the a frame pair. The score is calculated by comparing the sample values
// (each pixel has 3 sample values, the Y, U and V samples) of the raw frame and
// the compressed frame at a set of sampled coordinates.
//
...
//
// The dimension of the `raw_frame` and `compressed_frame` does not need to be
// the same, in order to support downscaling caused by simulcast and scalable
// encoding. However, the dimensions of the `raw_frame` must be larger than or
// equal to the dimensions of the `compressed_frame`.
class FramePairCorruptionScore {
 public:
  ...
  // of pixels to sample.
  FramePairCorruptionScore(absl::string_view codec_name,
                           float scale_factor,
  ...
  ~FramePairCorruptionScore() = default;
  ...
  double CalculateScore(int qp,
                        I420BufferInterface& raw_frame,
                        I420BufferInterface& compressed_frame);
