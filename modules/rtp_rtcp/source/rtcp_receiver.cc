...
struct RTCPReceiver::RrtrInformation {
  RrtrInformation(uint32_t ssrc,
                  uint32_t last_rr_ntp_mid,
                  uint32_t last_rr_receive_time_ntp_mid)
      : ssrc(ssrc),
        last_rr_ntp_mid(last_rr_ntp_mid),
        last_rr_receive_time_ntp_mid(last_rr_receive_time_ntp_mid) {}
...
RTCPReceiver::ConsumeReceivedXrReferenceTimeInfo() {
  ...
  for (size_t i = 0; i < last_xr_rtis_size; ++i) {
    ...
    last_xr_rtis.emplace_back(rrtr.ssrc, rrtr.last_rr_ntp_mid,
                              now_ntp - rrtr.last_rr_receive_time_ntp_mid);
...
void RTCPReceiver::HandleXrReceiveReferenceTime(uint32_t sender_ssrc,
  ...
  uint32_t last_rr_ntp_mid = CompactNtp(rrtr.ntp());
  uint32_t last_rr_receive_time_ntp_mid = CompactNtp(clock_->CurrentNtpTime());

  auto it = received_rrtrs_ssrc_it_.find(sender_ssrc);
  if (it != received_rrtrs_ssrc_it_.end()) {
    it->second->last_rr_ntp_mid = last_rr_ntp_mid;
    it->second->last_rr_receive_time_ntp_mid = last_rr_receive_time_ntp_mid;
  } else {
    if (received_rrtrs_.size() < kMaxNumberOfStoredRrtrs) {
      received_rrtrs_.emplace_back(sender_ssrc, last_rr_ntp_mid,
                                   last_rr_receive_time_ntp_mid);
