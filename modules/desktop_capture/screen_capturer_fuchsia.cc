...
#include <algorithm>
#include <cstdint>
// Remove iostream include when tracing is no longer needed
#include <iostream>
#include <memory>
#include <string>
#include <utility>
...
void ScreenCapturerFuchsia::Start(Callback* callback) {
  ...
  // Should we setup buffers here or in initialization of class?
  SetupBuffers();
}
...
void ScreenCapturerFuchsia::CaptureFrame() {
  ...
  MapHostPointer(buffer_collection_info_, buffer_id,
                 // Couldn't capture kBytesPerPixel b/c it does not have
...
void ScreenCapturerFuchsia::SetExcludedWindow(WindowId window) {
  std::cerr << "smpham " << __FILE_NAME__ << ":" << __LINE__ << ": "
            << __FUNCTION__ << "\n";
}
...
bool ScreenCapturerFuchsia::GetSourceList(SourceList* screens) {
  ...
  screens->push_back(
      {static_cast<SourceId>(7), std::string("Fuchsia monitor smpham")});
  return true;
}

bool ScreenCapturerFuchsia::SelectSource(SourceId id) {
  std::cerr << "smpham " << __FILE_NAME__ << ":" << __LINE__ << ": "
            << __FUNCTION__ << " SourceId: " << id << "\n";
  // Magic number for the single monitor Fuchsia display
  if (id == static_cast<SourceId>(7)) {
    return true;
  }
...
ScreenCapturerFuchsia::GetBufferConstraints() {
  ...
  // This stuff was just taken from the screen capture integration tests 🤷
  ...
  // Some sort of size constraints are needed or Vulkan will crash.
  // See http://go/paste/4605448717074432?raw
...
void ScreenCapturerFuchsia::SetupBuffers() {
  ...
  // Should we use this? Or that new display info FIDL protocol?
  // fuchsia.ui.display.singleton
  // We have to use scenic anyway to check for flatland and gfx comes for free
  // in the scenic package (probably)
  ...
  // Do we need a token for Vulkan??
  // fuchsia::sysmem::BufferCollectionTokenSyncPtr vulkan_token;
  // status = sysmem_token->Duplicate(ZX_RIGHT_SAME_RIGHTS,
  // vulkan_token.NewRequest()); if (status != ZX_OK) {
  //   fatal_error_ = true;
  //   std::cerr << "smpham " << __FILE_NAME__ << ":" << __LINE__ << ": " <<
  //   __FUNCTION__<< "\n"; ZX_LOG(ERROR, status) <<
  //   "fuchsia.sysmem.BufferCollectionToken.Duplicate()"; return;
  // }
