...
RTC_OBJC_EXPORT
@protocol RTC_OBJC_TYPE(RTCAudioSessionDelegate) <NSObject>
...
- (void)audioSession:(RTC_OBJC_TYPE(RTCAudioSession) *)audioSession didChangeOutputVolume:(float)outputVolume;
...
/** This is a protocol used to inform RTC_OBJC_TYPE(RTCAudioSession) when the audio session
 *  activation state has changed outside of RTC_OBJC_TYPE(RTCAudioSession). The current known use
 *  case of this is when CallKit activates the audio session for the application
 */
RTC_OBJC_EXPORT
@protocol RTC_OBJC_TYPE(RTCAudioSessionActivationDelegate) <NSObject>
 ...
 *
 *  RTC_OBJC_TYPE(RTCAudioSession) also coordinates activation so that the audio session is
...
/** Whether RTC_OBJC_TYPE(RTCAudioSession) is currently locked for configuration. */
...
@interface RTC_OBJC_TYPE(RTCAudioSession) (Configuration)
