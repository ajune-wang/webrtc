...
int16_t WebRtcOpus_SetComplexity(OpusEncInst* inst, int32_t complexity) {
  if (inst) {
    return opus_multistream_encoder_ctl(inst->encoder, OPUS_SET_COMPLEXITY(complexity));
  } else {
    return -1;
  }
}
...
int32_t WebRtcOpus_GetBandwidth(OpusEncInst* inst) {
  ...
  int32_t bandwidth;
  if (opus_multistream_encoder_ctl(inst->encoder, OPUS_GET_BANDWIDTH(&bandwidth)) == 0) {
    return bandwidth;
  } else {
    return -1;
  }
...
int16_t WebRtcOpus_SetBandwidth(OpusEncInst* inst, int32_t bandwidth) {
  if (inst) {
    return opus_multistream_encoder_ctl(inst->encoder, OPUS_SET_BANDWIDTH(bandwidth));
  } else {
    return -1;
  }
}
...
int16_t WebRtcOpus_GetSurroundParameters(int channels,
                                         int *streams,
                                         int *coupled_streams,
  ...
  OpusMSEncoder* ms_encoder_ptr = opus_multistream_surround_encoder_create(
      ...
      coupled_streams,
      mapping,
      OPUS_APPLICATION_VOIP, /* Application shouldn't affect streams/mapping values. */
      &opus_error);

  /* This shouldn't fail; if it fails, make sure to signal an error and return invalid values. */
...
int16_t WebRtcOpus_DecoderCreate(OpusDecInst** inst, size_t channels) {
  ...
  if (inst != NULL) {
    ...
    int streams = -1;
    int coupled_streams = -1;
    if (WebRtcOpus_GetSurroundParameters(channels, &streams, &coupled_streams, mapping) != 0) {
      return -1;
    }
