...
TEST_F(PeerConnectionIntegrationTestUnifiedPlan, PrAnswerStateTransitions) {
  ...
  // // Apply the pranswer as a definitiv one.
...
// Let caller get a prAnswer followed by and answer and callee is stable.
TEST_F(PeerConnectionIntegrationTestUnifiedPlan,
       PrAnswerStateTransitionsAssymetric) {
  RTCConfiguration config;
  ASSERT_TRUE(CreatePeerConnectionWrappersWithConfig(config, config));
  ConnectFakeSignaling();
  caller()->pc()->AddTransceiver(caller()->CreateLocalAudioTrack());
...
void ReorderPayloadTypes(cricket::MediaContentDescription* media) {
  std::vector<cricket::Codec> codecs = media->codecs();
  ssize_t left = 0;
  ssize_t right = codecs.size() - 1;
  while (left < right) {
...
void SetNewSsrcs(cricket::MediaContentDescription* media,
                 std::function<uint32_t()> ssrc_generator) {
  for (auto& stream : media->mutable_streams()) {
    for (size_t i = 0; i < stream.ssrcs.size(); i++) {
      stream.ssrcs[i] = ssrc_generator();
    }
  }
}
...
// "Scramble" and SDP
// - assign new ssrcs
// - assign new fingerprints
// - reorder payload types
void ScrambleSDP(std::unique_ptr<SessionDescriptionInterface>& sdp) {
  ...
  for (ContentInfo& content : sdp->description()->contents()) {
    ...
    SetNewSsrcs(content.media_description(), [&]() -> uint32_t {
      do {
    ...
    ReorderPayloadTypes(content.media_description());
  }
...
TEST_F(PeerConnectionIntegrationTestUnifiedPlan,
       PrAnswerStateTransitionsAssymetricMunged) {
  RTCConfiguration config;
  ASSERT_TRUE(CreatePeerConnectionWrappersWithConfig(config, config));
  ConnectFakeSignaling();
  caller()->pc()->AddTransceiver(caller()->CreateLocalAudioTrack());
  ...
  // Apply the modified answer as a definitiv one.
  EXPECT_TRUE(caller()->SetRemoteDescription(std::move(answer)));
  EXPECT_EQ(caller()->pc()->signaling_state(),
            PeerConnectionInterface::kStable);
}
