...
SendSideCongestionController::SendSideCongestionController(
    ...
    : clock_(clock),
      ...
      running_token_(new RunningToken()),
      pacer_(pacer),
...
SendSideCongestionController::~SendSideCongestionController() {
  if (running_token_->running) {
    rtc::Event done(false, false);
    task_queue_->PostTask(rtc::NewClosure(
        [this] { running_token_->running = false; }, [&done] { done.Set(); }));
...
void SendSideCongestionController::SetStreamsConfig() {
  rtc::CritScope cs(&streams_config_lock_);
  streams_config_.at_time = Timestamp::ms(clock_->TimeInMilliseconds());

  auto msg = streams_config_;
  WaitOnTask([this, msg]() { controller_->OnStreamsConfig(msg); });
}
...
void SendSideCongestionController::OnSentPacket(
  ...
  if (packet.has_value()) {
    ...
    PostTask([this, msg]() { controller_->OnSentPacket(msg); });
  }
}
...
void SendSideCongestionController::Process() {
  ...
  last_process_update_ms_ = now_ms;
  {
    ...
    auto token = running_token_;
    PostTask([this, msg]() { controller_->OnProcessInterval(msg); });
  }
...
void SendSideCongestionController::OnTransportFeedback(
  ...
  if (feedback_vector.size() > 0) {
    ...
    msg.data_in_flight =
        DataSize::bytes(transport_feedback_adapter_.GetOutstandingBytes());
    PostTask([this, msg]() { controller_->OnTransportPacketsFeedback(msg); });
  }
}
...
void SendSideCongestionController::MaybeUpdateOutstandingData() {
  ...
  msg.in_flight_data =
      DataSize::bytes(transport_feedback_adapter_.GetOutstandingBytes());
  PostTask([this, msg]() { pacer_controller_->OnOutstandingData(msg); });
}
...
void SendSideCongestionController::WaitOnTask(closure_t closure) {
  rtc::Event done(false, false);
  task_queue_->PostTask(rtc::NewClosure(closure, [&done] { done.Set(); }));
  done.Wait(rtc::Event::kForever);
}

void SendSideCongestionController::PostTask(closure_t closure) {
  rtc::scoped_refptr<RunningToken> token = running_token_;
  task_queue_->PostTask([token, closure]() {
    if (!token->running)
      return;
    closure();
  });
...
void SendSideCongestionController::OnReceivedEstimatedBitrate(
  ...
  PostTask([this, msg]() { controller_->OnRemoteBitrateReport(msg); });
}
...
void SendSideCongestionController::OnReceivedRtcpReceiverReport(
  ...
  PostTask([this, report]() { controller_->OnRoundTripTimeReport(report); });
}
...
void SendSideCongestionController::OnReceivedRtcpReceiverReportBlocks(
  ...
  msg.end_time = now;
  PostTask([this, msg]() { controller_->OnTransportLossReport(msg); });
