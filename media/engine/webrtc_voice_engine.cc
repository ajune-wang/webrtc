...
void WebRtcVoiceMediaChannel::OnPacketReceived(
  ...
  worker_thread_->PostTask(SafeTask(task_safety_.flag(), [this, packet] {
    ...
    webrtc::RtpPacketReceived audio_packet(packet);
    audio_packet.IdentifyExtensions(recv_rtp_extension_map_);
    if (!audio_packet.arrival_time().IsFinite()) {
      audio_packet.set_arrival_time(
          webrtc::Timestamp::Micros(rtc::TimeMicros()));
    }

    call_->Receiver()->DeliverRtpPacket(
        webrtc::MediaType::AUDIO, audio_packet,
        absl::bind_front(
            &WebRtcVoiceMediaChannel::MaybeCreateDefaultReceiveStream, this));
  }));
}
