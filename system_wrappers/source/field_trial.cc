...
bool FieldTrialsStringIsValid(const absl::string_view trials_string_view) {
  if (trials_string_view.empty())
    return true;

  size_t next_item = 0;
  std::map<absl::string_view, absl::string_view> field_trials;
  while (next_item < trials_string_view.length()) {
    size_t name_end =
        trials_string_view.find(kPersistentStringSeparator, next_item);
    if (name_end == trials_string_view.npos || next_item == name_end)
      return false;
    size_t group_name_end =
        trials_string_view.find(kPersistentStringSeparator, name_end + 1);
    if (group_name_end == trials_string_view.npos ||
        name_end + 1 == group_name_end)
      return false;
    absl::string_view name =
        trials_string_view.substr(next_item, name_end - next_item);
    absl::string_view group_name =
        trials_string_view.substr(name_end + 1, group_name_end - name_end - 1);
