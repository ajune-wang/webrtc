...
class FrameGeneratorCapturer::InsertFrameTask : public rtc::QueuedTask {
 ...
 private:
  bool Run() override {
    ...
    intended_run_time_ms_ += interval_ms;
    int64_t now_ms = rtc::TimeMillis();
    if (intended_run_time_ms_ < now_ms) {
      int skipped_frames = 1 + (now_ms - intended_run_time_ms_) / interval_ms;
      intended_run_time_ms_ += skipped_frames * interval_ms;
      RTC_LOG(LS_ERROR) << "Frame Generator Capturer can't keep up with "
                           "requested fps, skipped "
                        << skipped_frames << " frame(s).";
    }
    int64_t delay_ms = intended_run_time_ms_ - now_ms;
