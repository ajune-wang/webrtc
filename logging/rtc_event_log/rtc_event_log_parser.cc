...
std::vector<LoggedPacketInfo> ParsedRtcEventLog::GetPacketInfos(
  ...
  auto advance_clock_us = [&](int64_t log_time_us) {
    if (overhead_iter != overheads.end() &&
        log_time_us >= overhead_iter->timestamp_us) {
      current_overhead = overhead_iter->overhead;
      ++overhead_iter;
    }
    RTC_CHECK_GE(log_time_us, last_log_time_us);
    last_log_time_us = log_time_us;
  };

  auto rtp_handler = [&](const LoggedRtpPacket& rtp) {
    advance_clock_us(rtp.log_time_us());
  ...
  auto feedback_handler = [&](const LoggedRtcpPacketTransportFeedback& logged) {
    advance_clock_us(logged.log_time_us());
    auto msg = feedback_adapter.ProcessTransportFeedback(
        logged.transport_feedback, Timestamp::us(logged.log_time_us()));
...
const std::vector<MatchedSendArrivalTimes> GetNetworkTrace(
  ...
  for (auto& packet :
    ...
    if (packet.log_feedback_time.IsFinite()) {
      ...
      printf("%12ld %12li %12li, %12li\n",
             rtp_rtcp_matched.back().arrival_time_ms,
             rtp_rtcp_matched.back().feedback_arrival_time_ms,
             rtp_rtcp_matched.back().payload_size,
             rtp_rtcp_matched.back().send_time_ms);
    }
  }
