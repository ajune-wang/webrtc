...
TEST(DefaultVideoQualityAnalyzerTest, HeavyQualityMetricsFromEqualFrames) {
  ...
  analyzer_options.compute_corruption_metric = true;
  ...
  EXPECT_EQ(it->second.total_corruption_probability.GetAverage(), 0.0);
  EXPECT_EQ(it->second.total_corruption_probability.NumSamples(), 10);
}
