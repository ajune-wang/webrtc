...
// Checks that the sender report stats equal the initial state if no RTCP SR is
// send.
TEST_F(RtpRtcpImpl2Test, NoSenderReportStats) {
  ...
  receiver_.impl_->GetSenderReportStats(last_arrival_timestamp_ms,
                                        last_remote_timestamp_ms, packets_sent,
                                        bytes_sent, reports_count);
  EXPECT_FALSE(last_arrival_timestamp_ms.has_value());
  EXPECT_FALSE(last_remote_timestamp_ms.has_value());
  EXPECT_EQ(packets_sent, 0u);
  EXPECT_EQ(bytes_sent, 0u);
  EXPECT_EQ(reports_count, 0u);
}
...
TEST_F(RtpRtcpImpl2Test, SenderReportStatsCount) {
  ...
  for (uint32_t expected_count = 1; expected_count <= 2; ++expected_count) {
    SCOPED_TRACE(expected_count);
    // Send an SR.
    ASSERT_EQ(0, sender_.impl_->SendRTCP(kRtcpReport));
    receiver_.impl_->GetSenderReportStats(
        last_arrival_timestamp_ms, last_remote_timestamp_ms, packets_sent,
        bytes_sent, reports_count);
    EXPECT_EQ(reports_count, expected_count);
  }
}

// Checks that arrival and remote timestamps are available when an SR is sent.
TEST_F(RtpRtcpImpl2Test, SenderReportStatsTimestampsAvailable) {
  absl::optional<int64_t> last_arrival_timestamp_ms;
  absl::optional<int64_t> last_remote_timestamp_ms;
  uint32_t packets_sent;
  uint64_t bytes_sent;
  ...
  receiver_.impl_->GetSenderReportStats(last_arrival_timestamp_ms,
                                        last_remote_timestamp_ms, packets_sent,
                                        bytes_sent, reports_count);
  EXPECT_TRUE(last_arrival_timestamp_ms.has_value());
  EXPECT_TRUE(last_remote_timestamp_ms.has_value());
}

// Checks that arrival and remote timestamps, that originated from the same
// clock, are in the expected relationship.
TEST_F(RtpRtcpImpl2Test, SenderReportStatsArrivalAfterRemote) {
  // Simulate network delay so that the RTCP SR remote timestamp will be less
  // than the SR arrival timestamp.
  sender_.transport_.SimulateNetworkDelay(1, &time_controller_);
  ...
  receiver_.impl_->GetSenderReportStats(last_arrival_timestamp_ms,
                                        last_remote_timestamp_ms, packets_sent,
                                        bytes_sent, reports_count);
  ASSERT_TRUE(last_arrival_timestamp_ms.has_value());
  ASSERT_TRUE(last_remote_timestamp_ms.has_value());
  EXPECT_LT(last_remote_timestamp_ms.value(),
            last_arrival_timestamp_ms.value());
}

// Checks that the number of sent packets and bytes is positive once something
// has been received.
TEST_F(RtpRtcpImpl2Test, SenderReportStatsSentPacketsAndBytesSet) {
  absl::optional<int64_t> last_arrival_timestamp_ms;
  absl::optional<int64_t> last_remote_timestamp_ms;
  uint32_t packets_sent;
  uint64_t bytes_sent;
  ...
  // Send a frame and make sure that an RTP packet has been sent.
  SendFrame(&sender_, sender_video_.get(), kBaseLayerTid);
  ASSERT_GT(sender_.transport_.rtp_packets_sent_, 0);
