...
const char* ToString(PerfLoggingFeature feature) {
  switch (feature) {
...
class CompositePerfResultsLogger : public PerfResultsLoggerInterface {
 public:
  CompositePerfResultsLogger() { EnableFeature(PerfLoggingFeature::kStdOut); }
  ~CompositePerfResultsLogger() override = default;

  void EnableFeature(PerfLoggingFeature feature) {
    rtc::CritScope crit(&lock_);
    RTC_CHECK(!IsFeatureEnabled(feature))
        << "Perf logging feature [" << ToString(feature) << "] already enabled";
    switch (feature) {
      case PerfLoggingFeature::kStdOut:
        loggers_.insert(
  ...
  void DisableFeature(PerfLoggingFeature feature) {
    rtc::CritScope crit(&lock_);
    RTC_CHECK(IsFeatureEnabled(feature))
        << "Perf logging feature [" << ToString(feature)
        << "] already disabled";
  ...
  void SetOutput(FILE* output) {
    ...
    RTC_CHECK(IsFeatureEnabled(PerfLoggingFeature::kStdOut))
        ...
        << "] have to be enabled. To enable it use "
        << "webrtc::test::EnablePerfLoggingFeature(webrtc::test::"
        << "PerfLoggingFeature::kStdOut) before any call to the perf logging "
        << "infrastructure";
  ...
  std::string ToJSON() const {
    ...
    RTC_CHECK(IsFeatureEnabled(PerfLoggingFeature::kJson))
        ...
        << "] have to be enabled. To enable it use "
        << "webrtc::test::EnablePerfLoggingFeature(webrtc::test::"
        << "PerfLoggingFeature::kJson) before any call to the perf logging "
        << "infrastructure";
...
CompositePerfResultsLogger& GetPerfResultsLogger() {
  static CompositePerfResultsLogger* const logger_ =
      new CompositePerfResultsLogger();
  return *logger_;
}
