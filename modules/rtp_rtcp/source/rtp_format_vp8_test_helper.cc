...
void RtpFormatVp8TestHelper::GetAllPacketsAndCheck(
  ...
  for (size_t i = 0; i < expected_sizes.size(); ++i) {
    ...
    // Verify that the input variable "last" agrees with the position of
    // data_ptr. If data_ptr has advanced to payload_.end() we are at the end
    // and last should be true.
    bool last = i == expected_sizes.size() - 1;
    EXPECT_EQ(last, data_ptr == payload_.end());
  }
}
