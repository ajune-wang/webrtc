...
Operations DecisionLogic::GetDecision(const NetEqStatus& neteq_status,
                                      bool* reset_decoder) {
  ...
  if (neteq_status.last_mode == kModeRfc3389Cng) {
    cng_state_ = kCngRfc3389On;
  } else if (neteq_status.last_mode == kModeCodecInternalCng) {
    cng_state_ = kCngInternalOn;
  }

  size_t cur_size_samples = estimate_dtx_delay_
                                ? neteq_status.packet_buffer_info.span_samples
                                : neteq_status.packet_buffer_info.num_samples;

  prev_time_scale_ = prev_time_scale_ &&
                     (neteq_status.last_mode == kModeAccelerateSuccess ||
                      neteq_status.last_mode == kModeAccelerateLowEnergy ||
                      neteq_status.last_mode == kModePreemptiveExpandSuccess ||
                      neteq_status.last_mode == kModePreemptiveExpandLowEnergy);

  // Do not update buffer history if currently playing CNG since it will bias
  // the filtered buffer level.
  if (neteq_status.last_mode != kModeRfc3389Cng &&
      neteq_status.last_mode != kModeCodecInternalCng &&
      !(neteq_status.packet && neteq_status.packet->is_dtx &&
        !estimate_dtx_delay_)) {
    FilterBufferLevel(cur_size_samples);
  }

  // Guard for errors, to avoid getting stuck in error mode.
  if (neteq_status.last_mode == kModeError) {
    if (!neteq_status.packet) {
      return kExpand;
    } else {
  ...
  if (neteq_status.packet && neteq_status.packet->is_cng) {
    return CngOperation(neteq_status.last_mode, neteq_status.target_timestamp,
                        neteq_status.packet->timestamp,
                        neteq_status.generated_noise_samples);
  }

  // Handle the case with no packet at all available (except maybe DTMF).
  if (!neteq_status.packet) {
    return NoPacket(neteq_status.play_dtmf);
  }
  ...
  const size_t current_span =
      estimate_dtx_delay_ ? neteq_status.packet_buffer_info.span_samples
                          : neteq_status.packet_buffer_info.span_samples_no_dtx;
  if ((neteq_status.last_mode == kModeExpand ||
       neteq_status.last_mode == kModeCodecPlc) &&
      neteq_status.expand_mutefactor < 16384 / 2 &&
  ...
  if (neteq_status.target_timestamp == neteq_status.packet->timestamp) {
    return ExpectedPacketAvailable(neteq_status.last_mode,
                                   neteq_status.play_dtmf);
  } else if (!PacketBuffer::IsObsoleteTimestamp(neteq_status.packet->timestamp,
                                                neteq_status.target_timestamp,
                                                five_seconds_samples)) {
    return FuturePacketAvailable(
        neteq_status.last_packet_samples, neteq_status.last_mode,
        neteq_status.target_timestamp, neteq_status.packet->timestamp,
        neteq_status.play_dtmf, neteq_status.generated_noise_samples,
        neteq_status.packet_buffer_info.span_samples,
