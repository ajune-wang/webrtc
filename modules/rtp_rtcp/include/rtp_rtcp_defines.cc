...
void StreamDataCounters::Add(const StreamDataCounters& other) {
  ...
  fec.Add(other.fec);
  if (other.first_packet_time != Timestamp::MinusInfinity() &&
      (other.first_packet_time < first_packet_time ||
       first_packet_time == Timestamp::MinusInfinity())) {
    // Use oldest time (i.e. smallest, but exclude MinusInfinity)
    first_packet_time = other.first_packet_time;
  }
}

void StreamDataCounters::Subtract(const StreamDataCounters& other) {
  ...
  if (other.first_packet_time > first_packet_time) {
    // Use youngest time (i.e. largest, thus excluding MinusIninifty)
    first_packet_time = other.first_packet_time;
  }
}
