...
void JsepTransportCollection::RegisterTransport(
  ...
  RTC_LOG(LS_ERROR) << "DEBUG: Register transport for mid " << mid
                    << ", transport=" << transport;
...
void JsepTransportCollection::RemoveTransportForMid(const std::string& mid) {
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  RTC_LOG(LS_ERROR) << "DEBUG: Removing transport for mid " << mid;
  RTC_DCHECK(IsConsistent());
  ...
  if (old_transport) {
    ...
    RTC_LOG(LS_ERROR) << "DEBUG: Transport was not mapped";
  }
  RTC_DCHECK(IsConsistent());
}
...
bool JsepTransportCollection::TransportInUse(
  ...
  for (const auto& kv : mid_to_transport_) {
    if (kv.second == jsep_transport) {
      RTC_LOG(LS_ERROR) << "DEBUG: Transport " << jsep_transport
                        << " in use by mid " << kv.first;
      return true;
    }
  }
  return false;
...
void JsepTransportCollection::MaybeDestroyJsepTransport(
  ...
  if (TransportInUse(transport)) {
    RTC_LOG(LS_ERROR) << "DEBUG: Transport is in use, don't destroy";
    return;
  }
  for (const auto& it : jsep_transports_by_name_) {
    if (it.second.get() == transport) {
      RTC_LOG(LS_ERROR) << "DEBUG: Destroying transport for mid " << it.first
                        << ", transport " << transport;
