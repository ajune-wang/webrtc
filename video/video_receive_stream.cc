...
VideoReceiveStream::VideoReceiveStream(
  ...
  frame_buffer_.reset(new video_coding::FrameBuffer(
      clock_, task_queue_factory_, jitter_estimator_.get(), timing_.get(),
      &stats_proxy_));
...
void VideoReceiveStream::StartNextDecode() {
  ...
  struct DecodeTask {
    void operator()() {
      ...
      stream->HandleEncodedFrame(std::move(frame), res);
    }
  ...
  frame_buffer_->NextFrame(
      wait_ms, /*keyframe_required*/ false,
      [this](std::unique_ptr<EncodedFrame> frame, ReturnReason res) {
        decode_queue_.PostTask(DecodeTask{this, std::move(frame), res});
...
void VideoReceiveStream::HandleEncodedFrame(std::unique_ptr<EncodedFrame> frame,
                                            ReturnReason res) {
  if (res == ReturnReason::kStopped || decoder_stopped_) {
    decoder_stopped_ = true;
    return;
  }
