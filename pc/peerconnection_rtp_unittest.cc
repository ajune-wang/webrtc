...
TEST_F(PeerConnectionRtpObserverTest,
       MultipleRemoteSednersNoStreamParamIdAddsReceivers) {
  auto caller = CreatePeerConnection();
  auto callee = CreatePeerConnection();
  ...
  const char kStreamId2[] = "stream2";
  caller->AddTrack(caller->CreateAudioTrack("audio_track1"), {kStreamId1});
  caller->AddTrack(caller->CreateAudioTrack("audio_track2"), {kStreamId2});

  auto offer = caller->CreateOfferAndSetAsLocal();
  auto mutable_streams = offer->description()
                             ->GetContentDescriptionByName("audio")
                             ->mutable_streams();
