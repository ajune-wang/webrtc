...
constexpr int kIatFactor = 32745;
...
int DelayManager::Update(uint16_t sequence_number,
  ...
  if (packet_len_ms > 0) {
    ...
    iat_packets = std::min(iat_packets, iat_histogram_->NumBuckets());
...
int DelayManager::CalculateTargetLevel(int iat_packets, bool reordered) {
  ...
  int target_level = iat_histogram_->ReverseQuantile(limit_probability);
