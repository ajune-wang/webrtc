...
NetworkControlUpdate GoogCcNetworkController::OnSentPacket(
  ...
  if (sent_packet.data_in_flight > initial_data_window_)
    initial_reduced_bandwidth_ = true;
  NetworkControlUpdate update;
...
NetworkControlUpdate GoogCcNetworkController::OnTransportPacketsFeedback(
  ...
  if (initial_data_window_.IsFinite() && acknowledged_bitrate) {
    initial_data_window_ = DataSize::Infinity();
    if (initial_reduced_bandwidth_) {
      initial_reduced_bandwidth_ = false;
  ...
  if (result.updated) {
    if (result.probe) {
      if (initial_data_window_.IsFinite()) {
        initial_data_window_ = DataSize::Infinity();
        if (initial_reduced_bandwidth_) {
          initial_reduced_bandwidth_ = false;
