...
TEST(GoogCcPeerScenarioTest, NoBweChangeFromVideoUnmute) {
  ...
  PeerScenario s(*test_info_);
  PeerScenarioClient::Config peer_scenario_client_config;
  peer_scenario_client_config.video.use_fake_codecs = true;
  auto* caller = s.CreateClient(peer_scenario_client_config);
  auto* callee = s.CreateClient(peer_scenario_client_config);
