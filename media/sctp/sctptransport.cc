...
bool SctpTransport::ResetStream(int sid) {
  ...
  auto it = stream_status_by_sid_.find(sid);
  if (it == stream_status_by_sid_.end() || it->second.reset_initiated) {
  ...
  it->second.reset_initiated = true;
...
bool SctpTransport::SendQueuedStreamResets() {
  ...
  // TODO(crbug.com/559394): Change this to just SCTP_STREAM_RESET_OUTGOING
  // when safe to do so. Older versions of WebRTC don't follow the proper
  // closing procedure, so if we didn't do SCTP_STREAM_RESET_INCOMING when
  // talking to an old client, the incoming stream would never be reset and
  // we'd be stuck at a "closing" state.
  resetp->srs_flags = SCTP_STREAM_RESET_OUTGOING | SCTP_STREAM_RESET_INCOMING;
  ...
  // map, and remove entries for streams that have completed the reset
  // procedure.
  for (auto it = stream_status_by_sid_.begin();
       it != stream_status_by_sid_.end();) {
    if (!it->second.need_outgoing_reset()) {
      ++it;
      continue;
    }
    it->second.outgoing_reset_initiated = true;
    if (it->second.reset_complete()) {
      it = stream_status_by_sid_.erase(it);
    } else {
      ++it;
    }
  }
...
void SctpTransport::OnStreamResetEvent(
  ...
  bool need_to_send_outgoing_reset = false;
  for (int i = 0; i < num_sids; i++) {
    ...
    if (evt->strreset_flags & SCTP_STREAM_RESET_INCOMING_SSN) {
      ...
      // This indicates that the remote side is closing its data channel;
      // fire a signal such that the relevant DataChannel can change its state
      // to "closing" and do its side of the closing procedure.
      SignalIncomingStreamReset(sid);
    }
    ...
    } else if (status.need_outgoing_reset()) {
      need_to_send_outgoing_reset = true;
    }
  }

  // Send outgoing resets if needed.
  if (need_to_send_outgoing_reset) {
    SendQueuedStreamResets();
  }
}
