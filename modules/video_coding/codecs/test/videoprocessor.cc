...
void VideoProcessor::FrameDecoded(const VideoFrame& decoded_frame,
  ...
  bool calc_frame_quality = !config_.measure_cpu;
  if (calc_frame_quality) {
    CalcFrameQuality(*decoded_frame.video_frame_buffer()->ToI420(), frame_stat);
  }

  if (decoded_frame_writers_ != nullptr) {
    WriteDecodedFrame(*decoded_frame.video_frame_buffer()->ToI420(),
                      *decoded_frame_writers_->at(spatial_idx));
  }

  if (calc_frame_quality || decoded_frame_writers_) {
    // Save last decoded frame for filling possible future drops.
    last_decoded_frame_buffer_[spatial_idx] =
        I420Buffer::Copy(*decoded_frame.video_frame_buffer()->ToI420());
  }
...
void VideoProcessor::HandleTailDrops() {
  bool calc_frame_quality = !config_.measure_cpu;
  if (!calc_frame_quality && decoded_frame_writers_ == nullptr) {
    return;
  }
