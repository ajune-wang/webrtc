...
// Use 25% of the frame.
constexpr float kCorruptionDetectionSampleFactor = 0.25;
...
void DefaultVideoQualityAnalyzerFramesComparator::ProcessComparison(
  ...
  if ((options_.compute_psnr || options_.compute_ssim ||
       options_.compute_corruption_metric) &&
    ...
    if (options_.compute_corruption_metric && comparison.decoded.has_value() &&
      ...
      if (corruption_scorer_ == nullptr) {
        corruption_scorer_ = std::make_unique<FramePairCorruptionScorer>(
            ...
            kCorruptionDetectionScaleFactor, kCorruptionDetectionSampleFactor);
      }
      rtc::scoped_refptr<I420BufferInterface> decoded_buffer =
          comparison.decoded->video_frame_buffer()->ToI420();
      corruption_probability = corruption_scorer_->CalculateScore(
          frame_stats.decoded_frame_qp.value(), *reference_buffer,
          *decoded_buffer);
      RTC_CHECK_GE(corruption_probability, 0.0);
      RTC_CHECK_LE(corruption_probability, 1.0);
    }
  }
  ...
  if (corruption_probability >= 0.0) {
    stats->total_corruption_probability.AddSample(StatsSample(
        corruption_probability, frame_stats.received_time, metadata));
  }
