...
int NetEqImpl::GetAudioInternal(AudioFrame* audio_frame,
  ...
  *muted = false;
  ...
  // Copy samples from |algorithm_buffer_| to |sync_buffer_|. We would in the
  // future also like to pass |packet_infos| so that we can do sample-perfect
  // tracking of that information across |sync_buffer_|.
  ...
  // |packet_infos| from all packets decoded by the current |GetAudioInternal()|
