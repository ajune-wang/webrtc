...
void EventLogAnalyzer::CreatePlayoutGraph(Plot* plot) {
  for (const auto& playout_stream : parsed_log_.audio_playout_events()) {
    ...
    for (const auto& playout_time_ms : playout_stream.second) {
      float x = ToCallTime(playout_time_ms * 1000);
...
class NetEqStreamInput : public test::NetEqInput {
 public:
  ...
  NetEqStreamInput(const std::vector<LoggedRtpPacketIncoming>* packet_stream,
                   const std::vector<int64_t>* output_events_ms,
                   rtc::Optional<int64_t> end_time_ms)
      : packet_stream_(*packet_stream),
        packet_stream_it_(packet_stream_.begin()),
        output_events_ms_it_(output_events_ms->begin()),
        output_events_ms_end_(output_events_ms->end()),
        end_time_ms_(end_time_ms) {
    RTC_DCHECK(packet_stream);
    RTC_DCHECK(output_events_ms);
  }
  ...
  rtc::Optional<int64_t> NextOutputEventTime() const override {
    if (output_events_ms_it_ == output_events_ms_end_) {
      return rtc::nullopt;
    }
    if (end_time_ms_ && *output_events_ms_it_ > *end_time_ms_) {
      return rtc::nullopt;
    }
    return *output_events_ms_it_;
  }
 ...
 private:
  ...
  std::vector<int64_t>::const_iterator output_events_ms_it_;
  const std::vector<int64_t>::const_iterator output_events_ms_end_;
...
std::unique_ptr<test::NetEqStatsGetter> CreateNetEqTestAndRun(
  ...
  std::unique_ptr<test::NetEqInput> input(
      new NetEqStreamInput(packet_stream, output_events_ms, end_time_ms));
