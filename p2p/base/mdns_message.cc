...
// |buf| should contain the entire message so that when we encounter a
// compressed name replaced with a pointer to a prior occurrence of the same
// name, we can retrieve it.
bool ReadDomainName(webrtc::MDnsMessageBufferReader* buf,
  ...
  if (is_pointer(label_length)) {
    webrtc::MDnsMessageBufferReader new_buf(buf->start(), buf->len());
    uint8_t next_octet;
    if (!buf->ReadUInt8(&next_octet)) {
      return false;
    }
    size_t offset = ((label_length & 0x3f) << 8) + next_octet;
    new_buf.Consume(offset);
...
MDnsMessageBufferReader::MDnsMessageBufferReader(const char* bytes, size_t len)
    : ByteBufferReader(bytes, len), start_(bytes), len_(len) {}
...
const MDnsResourceRecord* MDnsMessage::GetResourceRecord(
    MDnsMessage::Section section,
    int index) const {
...
const MDnsQuestion* MDnsMessage::GetQuestion(int index) const {
...
const MDnsResourceRecord* MDnsMessage::GetAnswerRecord(int index) const {
  return GetResourceRecord(MDnsMessage::Section::kAnswer, index);
}
const MDnsResourceRecord* MDnsMessage::GetNameServerRecord(int index) const {
  return GetResourceRecord(MDnsMessage::Section::kAuthority, index);
}
const MDnsResourceRecord* MDnsMessage::GetAdditionalRecord(int index) const {
