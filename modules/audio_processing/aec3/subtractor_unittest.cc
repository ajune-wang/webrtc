...
#if defined(NDEBUG)
  const auto kNumRenderChannelsToTest = {1, 2, 8};
  const auto kNumCaptureChannelsToTest = {1, 2, 4};
#else
  const auto kNumRenderChannelsToTest = {1, 2};
  const auto kNumCaptureChannelsToTest = {1, 2};
#endif
