...
rtc::Buffer GenerateNalUnit(size_t size) {
  ...
  // Set some valid header with type IDR_N_LP, layer id 32 and temporal id 2
  buffer[0] = 41;
  buffer[1] = 2;
...
// Creates Buffer that looks like nal unit of given size and header.
rtc::Buffer GenerateNalUnit(size_t size, uint8_t header_h, uint8_t header_l) {
  RTC_CHECK_GT(size, 0);
  rtc::Buffer buffer(size);
  // Set some valid header with type IDR_N_LP and temporal id
  buffer[0] = header_h;
  buffer[1] = header_l;
  for (size_t i = 2; i < size; ++i) {
    buffer[i] = static_cast<uint8_t>(i);
...
TEST(RtpPacketizerH265Test, ApRespectsNoPacketReduction) {
  rtc::Buffer nalus[] = {GenerateNalUnit(/*size=*/3),
                         GenerateNalUnit(/*size=*/3),
...
TEST(RtpPacketizerH265Test, ApRespectsLayerIdAndTemporalId) {
  // Generate 3 Nalus: Nalu 1 with layerid 2 and temporalid 8,
  //                   Nalu 2 with layerid 0 and temporalid 1,
  //                   Nalu 3 with layerid 32 and temporalid 2,
  // So in the AP packet header, layerid should be 0 which is the lowest
  // layerid value of 3 Nalus and temporalid should be 1 which is the lowest
  // temporalid value of 3 Nalus
  rtc::Buffer nalus[] = {GenerateNalUnit(/*size=*/3, 40, 22),
                         GenerateNalUnit(/*size=*/3, 40, 1),
                         GenerateNalUnit(/*size=*/0x123)};
  rtc::Buffer frame = CreateFrame(nalus);
