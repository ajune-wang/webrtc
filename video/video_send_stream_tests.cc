...
// Tests that when the encoder uses an internal source,
// VideoEncoder::SetRateAllocation will be called with a new bitrate allocation
// when turning the stream on with VideoSendStream::UpdateActiveStreams.
// TODO(bugs.webrtc.org/8653): Currently UpdateActiveStreams doesn't update the
// video bitrate allocator. Once this change is made, update this test to
// reflect that the bitrate changes when one simulcast stream is already active,
// and the other stream is updated to be active as well.
TEST_F(VideoSendStreamTest, VideoSendStreamUpdateActiveStreams) {
  test::NullTransport transport;
  StartStopBitrateObserver encoder;
  const size_t num_simulcast_streams = 2;

  task_queue_.SendTask([this, &transport, &encoder]() {
    CreateSenderCall(Call::Config(event_log_.get()));
    // Create two simulcast streams.
    CreateSendConfig(num_simulcast_streams, 0, 0, &transport);
  ...
  task_queue_.SendTask([this]() {
    video_send_stream_->UpdateActiveStreams(video_encoder_config_);
  });
  EXPECT_TRUE(encoder.WaitBitrateChanged(true));
  ...
  video_encoder_config_.simulcast_layers[0].active = false;
  video_encoder_config_.simulcast_layers[1].active = true;
  task_queue_.SendTask([this]() {
    video_send_stream_->UpdateActiveStreams(video_encoder_config_);
  });
  EXPECT_TRUE(encoder.WaitBitrateChanged(true));
