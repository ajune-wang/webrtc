...
class PacedSender : public Pacer {
 ...
 private:
  ...
  std::atomic<uint32_t> pacing_bitrate_kbps_;

  std::atomic<int64_t> time_last_update_us_;
  std::atomic<int64_t> first_sent_packet_ms_;

  const std::unique_ptr<PacketQueue> packets_;

  std::atomic<size_t> queue_size_packets_;
  std::atomic<size_t> queue_size_bytes_;
  std::atomic<int64_t> oldest_queue_time_ms_;

  uint64_t packet_counter_;
  ProcessThread* process_thread_ = nullptr;

  std::atomic<int64_t> queue_time_limit;
  std::atomic<bool> account_for_audio_;
