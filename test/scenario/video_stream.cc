...
void SendVideoStream::Start() {
  sender_->SendTask(ToQueuedTask([this] {
    send_stream_->Start();
...
void SendVideoStream::Stop() {
  sender_->SendTask(ToQueuedTask([this] { send_stream_->Stop(); }));
}
...
ReceiveVideoStream::ReceiveVideoStream(CallClient* receiver,
  ...
  for (size_t i = 0; i < num_streams; ++i) {
    ...
    receiver_->SendTask(ToQueuedTask([this, &recv_config] {
...
ReceiveVideoStream::~ReceiveVideoStream() {
  receiver_->SendTask(ToQueuedTask([this] {
