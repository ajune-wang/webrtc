...
using ::webrtc::RegisterOnTaskQueue;
...
RegisterToken Register(MockResource* mock) {
  mock->OnRegister();
  return RegisterToken([mock] { mock->OnUnregister(); });
}

TEST(RegisterTokenTest, RunsOnDestruction) {
  MockResource mock;
...
TEST(RegisterTokenTest, RunsOnReset) {
  MockResource mock;
  EXPECT_CALL(mock, OnRegister());
  RegisterToken token = Register(&mock);
...
TEST(RegisterTokenTest, MovePassesResponsiblityToRun) {
  MockResource mock;
  EXPECT_CALL(mock, OnRegister());
  RegisterToken token = Register(&mock);
...
TEST(RegisterOnTaskQueueTest, RunsRegisterAndUnregisterOnTaskQueue) {
  ...
  RegisterToken safe_token =
      RegisterOnTaskQueue(&task_queue, [&mock] { return Register(&mock); });
...
TEST(RegisterOnTaskQueueTest, CanRunUnregisterBeforeRegister) {
  MockResource mock;
