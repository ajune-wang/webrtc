...
struct DataReader {
  ...
  void CopyTo(void* destination, size_t num_bytes) {
    uint8_t* dest = reinterpret_cast<uint8_t*>(destination);
    size_t bytes_to_copy = std::min(size_ - offset_, num_bytes);
    memcpy(dest, data_ + offset_, bytes_to_copy);
    offset_ += bytes_to_copy;

    num_bytes -= bytes_to_copy;
    if (num_bytes > 0)
      memset(dest + num_bytes, 0, num_bytes);
  }
