...
void Call::MediaTransportChange(MediaTransportInterface* media_transport) {
  ...
  if (media_transport) {
    ...
    if (media_transport_) {
      ...
      // We can't register again, because the transport_send is already
      // signed up for send_side_cc, and DeRegister didn't remove that observer.
      // transport_send_ documents that after calling DeRegister, you are not
...
void Call::OnTargetTransferRate(TargetTransferRate msg) {
  // Call assumes that on target transfer rate is invoked
