...
TEST(TimeUtilTest, TimeMicrosToNtpMatchRealTimeClockInitially) {
  ...
  NtpTime new_time1 = TimeMicrosToNtp(rtc::TimeMicros());
  NtpTime old_time2 = legacy_clock->CurrentNtpTime();
  NtpTime new_time3 = TimeMicrosToNtp(rtc::TimeMicros());
  ...
  EXPECT_LE(new_time1.ToMs(), old_time2.ToMs() + 1);
  EXPECT_LE(old_time2.ToMs(), new_time3.ToMs() + 1);
}
