...
void FillSubFrameView(std::vector<float>* frame,
                      size_t sub_frame_index,
                      rtc::ArrayView<float>* sub_frame_view) {
  RTC_DCHECK_GE(1, sub_frame_index);
  *sub_frame_view = rtc::ArrayView<float>(
      &(*frame)[sub_frame_index * kSubFrameLength], kSubFrameLength);
}

void ProcessCaptureFrameContent(
    AudioBuffer* capture,
    std::vector<float>* linear_output_frame,
    bool level_change,
    ...
    std::vector<float>* linear_output_block,
    rtc::ArrayView<float>* linear_output_sub_frame_view,
...
EchoCanceller3::EchoCanceller3(const EchoCanceller3Config& config,
  ...
  if (config_.filter.export_linear_aec_output) {
    linear_output_framer_.reset(new BlockFramer(1, 1));
    linear_output_block_.resize(kBlockSize, 0.f);
    linear_output_frame_.resize(kFrameSize, 0.f);
  }
}
...
void EchoCanceller3::ProcessCapture(AudioBuffer* capture, bool level_change) {
  ...
  ProcessCaptureFrameContent(capture, &linear_output_frame_, level_change,
                             ...
                             block_processor_.get(), &linear_output_block_,
  ...
  ProcessRemainingCaptureFrameContent(
      ...
      &linear_output_block_, &capture_block_);
