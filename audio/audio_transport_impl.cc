...
AudioTransportImpl::AudioTransportImpl(AudioDeviceModule* audio_device_module,
...
// TODO: locking
// capture_lock
//   copy params locally in RecordedDataIsAvailable
// processing etc
// capture lock
//   copy stats to members vars
//   fork processed frame out
...
int32_t AudioTransportImpl::RecordedDataIsAvailable(
  ...
  // Audio Processing.
  {
    RTC_DCHECK(!audio_processing_->echo_cancellation()->
               is_drift_compensation_enabled());
    GainControl* agc = audio_processing_->gain_control();
    int err = agc->set_stream_analog_level(voe_mic_level);
    RTC_DCHECK_EQ(0, err) <<
        "set_stream_analog_level failed: current_mic_level = " << voe_mic_level;
    audio_processing_->set_stream_delay_ms(audio_delay_milliseconds);
    ...
    uint32_t new_voe_mic_level = agc->stream_analog_level();
  ...
  if (audio_processing_->voice_detection()->is_enabled()) {
    if (audio_frame->vad_activity_ != AudioFrame::kVadUnknown) {
      ...
      typing_noise_detected_ =
          typing_detection_.Process(key_pressed, vad_active);
    }
  } else {
...
void AudioTransportImpl::SetSendingStream(AudioSendStream* stream,
                                           bool sending,
                                           int sample_rate_hz,
                                           size_t num_channels) {
  RTC_DCHECK(stream);
