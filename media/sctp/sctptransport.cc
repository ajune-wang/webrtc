...
bool SctpTransport::IsStreamAvailable(int sid) const {
  return stream_status_by_sid_.find(sid) == stream_status_by_sid_.end();
}
...
bool SctpTransport::SendQueuedStreamResets() {
  ...
  RTC_LOG(LS_VERBOSE) << "SendQueuedStreamResets[" << debug_name_
                      << "]: Sending " << num_streams
                      << " SCTP_STREAM_RESET_OUTGOING messages";
  ...
  // TODO(deadbeef): Change this to just SCTP_STREAM_RESET_OUTGOING when safe
  // to do so. See crbug.com/559394 for context.
  ...
  // Loop over the list of streams and add the stream ID to the
  // sctp_reset_streams structure for reset if and only if it was not done
  // previously (if need_outgoing_reset returns true).
...
void SctpTransport::OnStreamResetEvent(
  ...
  for (int i = 0; i < num_sids; i++) {
    ...
    // Signal the application if this is an indication of a data channel being
    // closed remotely.
    if (status.is_open()) {
      RTC_LOG(LS_VERBOSE) << "SCTP_STREAM_RESET_EVENT(" << debug_name_
                          << "): closing sid " << sid;
      SignalStreamClosedRemotely(sid);
    }
    ...
    // our map so we can consider it closed (and the SID available for a new
    // DataChannel).
    if (status.reset_complete()) {
      stream_status_by_sid_.erase(it);
      SignalStreamClosed(sid);
