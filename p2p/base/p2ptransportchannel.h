...
class P2PTransportChannel : public IceTransportInternal {
 ...
 private:
  ...
  // Gets the best connection for each pair of local and remote interface ids.
  std::map<std::pair<uint16_t, uint16_t>, Connection*>
  GetBestConnectionByInterfaceIdPair() const;
  std::vector<Connection*> GetBestWritableConnectionPerInterfaceIdPair() const;
  // Prune connections that are inferior than some other connection with the
  // same pair of local and remote interface ids. Different pair of interface
  // ids can represent different pairs of network interfaces, if the interface
  // ids of the remote candidates are signaled; otherwise, the interface id of a
  // remote candidate is always zero, and pairs of ids then represent groups
  // divided by the local interfaces. See the comments in the implementation for
  // the pruning logic.
  void PruneConnectionsByInterfaceIdPair();
