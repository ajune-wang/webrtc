...
size_t GetSimulcastOrSpatialIndex(
    const EncodedImage& encoded_image,
    const CodecSpecificInfo* codec_specific_info) {
  if (codec_specific_info &&
      codec_specific_info->scalability_mode.has_value()) {
    absl::optional<int> index =
        ScalabilityModeToNumSpatialLayers(
            codec_specific_info->scalability_mode.value()) > 1
            ? encoded_image.SpatialIndex_()
            : encoded_image.SimulcastIndex();
    return index.value_or(0);
  } else if (codec_specific_info &&
             codec_specific_info->codecType == VideoCodecType::kVideoCodecVP9) {
    // TODO(https://crbug.com/webrtc/14889): When the legacy SVC path is deleted
    // (and simulcast is default for all codecs when multiple encodings are
    // used) there is no need to treat VP9 differently here.
    return encoded_image.SpatialIndex_().value_or(0);
  }
  return encoded_image.SimulcastIndex().value_or(0);
}
...
EncodedImage VideoStreamEncoder::AugmentEncodedImage(
  ...
  const size_t simulcast_svc_idx =
      GetSimulcastOrSpatialIndex(encoded_image, codec_specific_info);
  frame_encode_metadata_writer_.FillTimingInfo(simulcast_svc_idx, &image_copy);
  ...
  if (image_copy.qp_ < 0 && qp_parsing_allowed_) {
    ...
    image_copy.qp_ = qp_parser_
                         .Parse(codec_type, simulcast_svc_idx,
                                image_copy.data(), image_copy.size())
                         .value_or(-1);
  }
  RTC_LOG(LS_VERBOSE) << __func__ << " simulcast_svc_idx " << simulcast_svc_idx
                      << " qp " << image_copy.qp_;
