...
SessionDescription* MediaSessionDescriptionFactory::CreateOffer(
  ...
  // The following determines how to signaling MSIDs to ensure compatibility
  // with older endpoints (in particular, older Plan B endpoints).
...
SessionDescription* MediaSessionDescriptionFactory::CreateAnswer(
  ...
  if (session_options.is_unified_plan) {
    ...
    if (offer->msid_signaling() == 0) {
      ...
      // 3. Media that's either sendonly or inactive from a Plan B endpoint. A
      //    Unified Plan endpoint would have included MSID so we'll assume the
      //    other side must be Plan B and respond with a=ssrc MSID signaling.
