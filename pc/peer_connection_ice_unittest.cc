...
class PeerConnectionIceBaseTest : public ::testing::Test {
 protected:
  ...
  // Returns a list of ufrags in the order that they appear in |description|, or
  // the empty list if |description| is null.
  std::vector<std::string> GetIceCredentials(
      const SessionDescriptionInterface* description) {
    std::vector<std::string> ice_credentials;
    if (!description)
    ...
    for (const auto& content_info : desc->contents()) {
      ...
      if (transport_info) {
        ice_credentials.push_back(transport_info->description.ice_ufrag);
      }
    }
    return ice_credentials;
  }
