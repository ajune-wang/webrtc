...
class Closure {
 public:
  explicit Closure(MockClosure* mock) : mock_(mock) {}
...
TEST(CancelableTaskTest, CancelDoesNohtingOnEmptyObject) {
...
TEST(CancelableTaskTest, CancelTaskBeforeItRun) {
  ...
  handler.StartPeriodicTask(MoveOnlyClosure(&mock), &task_queue, 20);

  EXPECT_FALSE(done.Wait(10));
  handler.Cancel();
  EXPECT_TRUE(done.Wait(20));
}

TEST(CancelableTaskTest, CancelTaskAfterItRun) {
  rtc::Event done(false, false);
  ...
  rtc::CancelableTaskHandler handler;
  rtc::TaskQueue task_queue("queue");
  handler.StartPeriodicTask(MoveOnlyClosure(&mock), &task_queue, 10);
  SleepMs(20);
  EXPECT_TRUE(run.Wait(0));
  handler.Cancel();
  EXPECT_TRUE(done.Wait(10));
}
...
TEST(CancelableTaskTest, StartPeriodicTaskWithMoveOnlyClosure) {
  rtc::Event done(false, false);
  MockClosure mock;
  ...
  rtc::CancelableTaskHandler handler;
  rtc::TaskQueue task_queue("queue");
  handler.StartPeriodicTask(MoveOnlyClosure(&mock), &task_queue, 0);
  EXPECT_TRUE(done.Wait(100));
}
