...
void RtcpTransceiverImpl::ReceivePacket(rtc::ArrayView<const uint8_t> packet) {
  while (!packet.empty()) {
    ...
    ptrdiff_t rtcp_packet_size = rtcp_block.NextPacket() - packet.data();
    RTC_DCHECK_GT(rtcp_packet_size, 0);
    RTC_DCHECK_LE(rtcp_packet_size, packet.size());
    packet = packet.subview(rtcp_packet_size);
  }
}
