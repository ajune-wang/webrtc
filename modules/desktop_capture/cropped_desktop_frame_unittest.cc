...
TEST(CroppedDesktopFrameTest, IccProfile) {
  const uint8_t fake_icc_profile_data_array[] = {0x1a, 0x00, 0x2b, 0x00,
                                                 0x3c, 0x00, 0x4d};
  const uint8_t* fake_icc_profile_data_ptr =
      reinterpret_cast<const uint8_t*>(fake_icc_profile_data_array);
  const std::vector<uint8_t> icc_profile(
      fake_icc_profile_data_ptr,
      fake_icc_profile_data_ptr + sizeof(fake_icc_profile_data_array));
