...
TEST(DataRateTest, ConvertsToAndFromDouble) {
  ...
  EXPECT_EQ(DataRate::bps(kValue).kbps<float>(), kDoubleKbps);
...
TEST(DataRateTest, MathOperations) {
  ...
  const DataRate size_a = DataRate::bps(kValueA);
  const int32_t kInt32Value = 123;
  const double kFloatValue = 123.0;
  EXPECT_EQ((size_a * kValueB).bps(), kValueA * kValueB);
  EXPECT_EQ((size_a * kInt32Value).bps(), kValueA * kInt32Value);
  EXPECT_EQ((size_a * kFloatValue).bps(), kValueA * kFloatValue);
}
