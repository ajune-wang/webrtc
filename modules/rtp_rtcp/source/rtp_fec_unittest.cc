...
TYPED_TEST(RtpFecTest, WillProtectMediaPacketsWithLargeSequenceNumberGap) {
  ...
  ByteWriter<uint16_t>::WriteBigEndian(
      this->media_packets_.front()->data.Data() + 2, 1);
  ByteWriter<uint16_t>::WriteBigEndian(
      this->media_packets_.back()->data.Data() + 2, kMaxMediaPackets);
...
TYPED_TEST(RtpFecTest,
  ...
  ByteWriter<uint16_t>::WriteBigEndian(
      this->media_packets_.front()->data.Data() + 2, 1);
  ByteWriter<uint16_t>::WriteBigEndian(
      this->media_packets_.back()->data.Data() + 2, kMaxMediaPackets + 1);
...
TEST_F(RtpFecTestFlexfecOnly, FecRecoveryWithSeqNumGapOneFrameNoRecovery) {
  ...
  ByteWriter<uint16_t>::WriteBigEndian(&(*it)->data.Data()[2], 65535);
  ++it;
  ByteWriter<uint16_t>::WriteBigEndian(&(*it)->data.Data()[2], 0);
