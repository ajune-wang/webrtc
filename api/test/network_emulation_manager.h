...
// Classes that represents emulated cross traffic.
class RandomWalkCrossTraffic;
class PulsedPeaksCrossTraffic;
class FakeTcpCrossTraffic;

// Configs for different types of cross traffic.
...
class NetworkEmulationManager {
 public:
  ...
  // Creates a cross traffic route over the given |via_nodes|.
  virtual TrafficRoute* CreateTrafficRoute(
      const std::vector<EmulatedNetworkNode*>& via_nodes) = 0;

  // Starts cross traffic over the given |traffic_route|. The traffic is stable
