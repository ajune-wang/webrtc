...
class RenderBuffer {
 public:
  ...
  const std::vector<std::vector<float>>& MostRecentBlock() const {
    return block_buffer_->buffer[block_buffer_->next_read];
  }

  // Get the spectrum from one of the FFTs in the buffer.
  rtc::ArrayView<const float> Spectrum(size_t buffer_offset_ffts) const {
    size_t position = spectrum_buffer_->OffsetIndex(spectrum_buffer_->next_read,
                                                    buffer_offset_ffts);
  ...
  size_t Position() const { return fft_buffer_->next_read; }
