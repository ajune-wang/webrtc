...
void FuzzOneInput(const uint8_t* data, size_t size) {
  std::unique_ptr<FrameDependencyStructure> structure1;
  std::unique_ptr<FrameDependencyStructure> structure2;

  while (size > 0) {
    // Treat next byte as size of the next extension. That aligns how
    // two-byte rtp header extension sizes are written.
    int next_size = std::min<int>(data[0], size - 1);
    rtc::ArrayView<const uint8_t> raw(data + 1, next_size);
    data += (1 + next_size);
    size -= (1 + next_size);

    // Read the random input.
    DependencyDescriptor descriptor1;
    if (!RtpDependencyDescriptorExtension::Parse(raw, structure1.get(),
                                                 &descriptor1)) {
      // Ignore invalid buffer and move on.
      continue;
    }
    if (descriptor1.attached_structure) {
      structure1 = std::make_unique<FrameDependencyStructure>(
          *descriptor1.attached_structure);
    }
    RTC_CHECK(structure1);
