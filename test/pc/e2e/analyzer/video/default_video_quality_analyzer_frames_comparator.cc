...
void DefaultVideoQualityAnalyzerFramesComparator::RegisterParticipantInCall(
    rtc::ArrayView<std::pair<InternalStatsKey, Timestamp>> stream_stats_to_add,
    Timestamp start_time) {
  MutexLock lock(&mutex_);
...
void DefaultVideoQualityAnalyzerFramesComparator::AddTimeBetweenFreezes(
    InternalStatsKey key,
    SamplesStatsCounter::StatsSample sample) {
  MutexLock lock(&mutex_);
  stream_stats_.at(key).time_between_freezes_ms.AddSample(sample);
}
