...
absl::optional<LossBasedBweV2::Config> LossBasedBweV2::CreateConfig(
  ...
  FieldTrialParameter<bool> trendline_with_max_increase_factor_enabled(
      "TrendlineWithMaxIncreaseFactor", false);
  ...
  if (key_value_config) {
    ParseFieldTrial({&enabled,
                     ...
                     &trendline_with_max_increase_factor_enabled},
  ...
  config->trendline_with_max_increase_factor_enabled =
      trendline_with_max_increase_factor_enabled.Get();
  return config;
}
...
DataRate LossBasedBweV2::GetCandidateBandwidthUpperBound(
  ...
  if (config_->trendline_with_max_increase_factor_enabled) {
...
bool LossBasedBweV2::TrendlineEsimateAllowBitrateIncrease() const {
  if (!config_->trendline_integration_enabled &&
      !config_->trendline_with_max_increase_factor_enabled) {
    return true;
  }
