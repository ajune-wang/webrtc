...
TEST_F(PortTest, TestTcpReconnectNoDestroy) {
  ...
  tcp1->network_thread()->PostDelayedTask(
      webrtc::ToQueuedTask([&]() {
        ...
        // Connection::Destroy().
        // If the socket object is null and/or tcp1 is no longer the `conn()` of
        // ch1, then the object has already been deleted (which is fine).
        if (tcp1 == ch1.conn() && tcp1->socket())
          tcp1->socket()->SignalClose(tcp1->socket(), -1);
      }),
