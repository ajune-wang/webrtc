...
namespace webrtc {
namespace {
bool IsOpenMessage(const cricket::ReceiveDataParams& params,
                   const rtc::CopyOnWriteBuffer& buffer) {
  return params.type == DataMessageType::kControl &&
         webrtc::IsOpenMessage(buffer);
}
}  // namespace
...
bool DataChannelController::HandleOpenMessage_n(
    ...
    const rtc::CopyOnWriteBuffer& buffer) {
  if (!IsOpenMessage(params, buffer))
    return false;
