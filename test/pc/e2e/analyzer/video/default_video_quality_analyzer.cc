...
DefaultVideoQualityAnalyzerFramesComparatorOptions toFramesComparatorOptions(
    DefaultVideoQualityAnalyzerOptions options) {
  DefaultVideoQualityAnalyzerFramesComparatorOptions comparator_options;
  comparator_options.heavy_metrics_computation_enabled =
      options.heavy_metrics_computation_enabled;
  comparator_options.adjust_cropping_before_comparing_frames =
      options.adjust_cropping_before_comparing_frames;
  comparator_options.enable_receive_own_stream =
      options.enable_receive_own_stream;
...
DefaultVideoQualityAnalyzer::DefaultVideoQualityAnalyzer(
    ...
    : options_(options),
      clock_(clock),
      frames_comparator_(clock,
                         cpu_measurer_,
                         toFramesComparatorOptions(options)) {}
DefaultVideoQualityAnalyzer::~DefaultVideoQualityAnalyzer() {
  Stop();
}
...
uint16_t DefaultVideoQualityAnalyzer::OnFrameCaptured(
  ...
  size_t stream_index;
  {
    ...
    // to access it without holding a `lock_` during access to
    // `frames_comparator_`.
...
void DefaultVideoQualityAnalyzer::RegisterParticipantInCall(
  ...
  std::vector<std::pair<InternalStatsKey, Timestamp>> stream_stats_to_add;
  for (auto& key_val : stream_to_sender_) {
    ...
    stream_stats_to_add.push_back(
        {key, stream_states_.at(stream_index).stream_started_time()});
  }
  frames_comparator_.RegisterParticipantInCall(stream_stats_to_add,
                                               start_time_);
...
void DefaultVideoQualityAnalyzer::Stop() {
  ...
  // holds any locks.
  {
    ...
    std::map<InternalStatsKey, Timestamp> stream_last_freeze_end_time =
        frames_comparator_.stream_last_freeze_end_time();
    for (auto& state_entry : stream_states_) {
      ...
      for (size_t i = 0; i < peers_->size(); ++i) {
        ...
        if (stream_state.last_rendered_frame_time(i)) {
          frames_comparator_.AddTimeBetweenFreezes(
              stats_key,
              StatsSample(
                  stream_state.last_rendered_frame_time(i).value().ms() -
                      stream_last_freeze_end_time.at(stats_key).ms(),
                  Now()));
        }
      }
    }
