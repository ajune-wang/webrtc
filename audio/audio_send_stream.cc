...
AudioSendStream::GetMinMaxBitrateConstraints() const {
  ...
  if (allocation_settings_.IncludeOverheadInAudioAllocation()) {
    ...
    const TimeDelta kMaxFrameLength = TimeDelta::ms(120);  // Based on Opus spec
    min_overhead = kOverheadPerPacket / kMaxFrameLength;
    constraints.min += min_overhead;
    // TODO(dklee): This is obviously not right, but was carried over during
    // a refactoring. Carefully reconsider the logic when addressing todo above.
