...
class AudioChannelTest : public ::testing::Test {
 public:
  ...
  rtc::scoped_refptr<AudioChannel> CreateAudioChannel(uint32_t ssrc) {
    ...
    auto audio_channel = new rtc::RefCountedObject<AudioChannel>(
...
TEST_F(AudioChannelTest, TestChannelStatisticsOnRTT) {
  // Create AC2 and AC3
  ...
  // Set up receiver to handle incoming RTP/RTCP.
  rtc::scoped_refptr<AudioChannel> receiver;
  auto route_rtp = [&](const uint8_t* packet, size_t length, Unused) {
    receiver->ReceivedRTPPacket(rtc::ArrayView<const uint8_t>(packet, length));
    return true;
  };
  auto route_rtcp = [&](const uint8_t* packet, size_t length) {
    receiver->ReceivedRTCPPacket(rtc::ArrayView<const uint8_t>(packet, length));
    return true;
  };
  // Same transport is used for all audio channels and will need to route the
  // packets to correct audio channel receiver by using receiver variable.
  EXPECT_CALL(transport_, SendRtp).WillRepeatedly(Invoke(route_rtp));
  EXPECT_CALL(transport_, SendRtcp).WillRepeatedly(Invoke(route_rtcp));
  ...
  absl::optional<ChannelStatistics> channel_stats =
      audio_channel_->GetChannelStatistics();
  EXPECT_TRUE(channel_stats);
  EXPECT_EQ(channel_stats->remote_ssrc.value(), kAc2Ssrc);
  EXPECT_TRUE(channel_stats->remote_rtcp.has_value());
  EXPECT_TRUE(channel_stats->remote_rtcp->round_trip_time.has_value());
  EXPECT_GT(channel_stats->remote_rtcp->round_trip_time.value(), 0.);
  ...
  EXPECT_TRUE(channel_stats);
  EXPECT_EQ(channel_stats->remote_ssrc.value(), kAc3Ssrc);
  EXPECT_TRUE(channel_stats->remote_rtcp.has_value());
  EXPECT_TRUE(channel_stats->remote_rtcp->round_trip_time.has_value());
  EXPECT_GT(channel_stats->remote_rtcp->round_trip_time.value(), 0.);

  // Handle RTCP BYE in the end.
  EXPECT_CALL(transport_, SendRtcp).WillRepeatedly(Return(true));
}
