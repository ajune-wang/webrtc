...
class P2PTransportChannelTestBase : public testing::Test,
 ...
 protected:
  ...
  SocketAddress ReplaceSavedCandidateIpWithHostname(
    ...
    EXPECT_EQ(1u, ed->saved_candidates_.size());
    auto& candidates = ed->saved_candidates_[0];
    EXPECT_EQ(1u, candidates->candidates.size());
...
TEST_F(P2PTransportChannelTest,
  ...
  ConfigureEndpoints(OPEN, OPEN,
                     kDefaultPortAllocatorFlags | PORTALLOCATOR_DISABLE_STUN,
                     kDefaultPortAllocatorFlags | PORTALLOCATOR_DISABLE_STUN);
  // Emulate no remote parameters coming in.
  set_remote_ice_parameter_source(FROM_CANDIDATE);
  ...
  // TODO(zstein): I think we want to start this before the prflx candidate is
  // added, but don't let it finish until after the prflx is added.
