...
TEST_F(PeerConnectionIntegrationTestUnifiedPlan,
  ...
  EXPECT_EQ(video_codecs_vpx_rtx.size(), 4u);  // VP8, VP9 (2 profiles), RTX
  EXPECT_TRUE(
...
TEST_F(PeerConnectionIntegrationTestUnifiedPlan,
  ...
  auto video_codecs = caller()
                          ->pc_factory()
                          ->GetRtpSenderCapabilities(cricket::MEDIA_TYPE_VIDEO)
                          .codecs;

  auto send_transceiver_result =
      caller()->pc()->AddTransceiver(cricket::MEDIA_TYPE_VIDEO);
  ASSERT_EQ(RTCErrorType::NONE, send_transceiver_result.error().type());
  auto send_transceiver = send_transceiver_result.MoveValue();
  ...
  EXPECT_EQ(video_codecs_vpx.size(), 3u);  // VP8, VP9 (2 profiles)
  ...
  auto codecs = offer->description()
                    ...
                    ->as_video()
                    ->codecs();
  EXPECT_EQ(codecs.size(), 3u);  // VP8, VP9 (2 profiles)
  callee()->SetRemoteOfferHandler([&]() {
    auto recv_transceiver = callee()->pc()->GetTransceivers().front();
    auto video_codecs_vp8 = video_codecs;
    auto it = std::remove_if(video_codecs_vp8.begin(), video_codecs_vp8.end(),
                             [](const webrtc::RtpCodecCapability& codec) {
                               bool r = codec.name != cricket::kVp8CodecName &&
                                        codec.name != cricket::kRtxCodecName;
                               return r;
                             });
    video_codecs_vp8.erase(it, video_codecs_vp8.end());
    EXPECT_EQ(video_codecs_vp8.size(), 2u);  // VP8
    recv_transceiver->SetCodecPreferences(video_codecs_vp8);
  });

  caller()->CreateAndSetAndSignalOffer();
  ASSERT_TRUE_WAIT(SignalingStateStable(), kDefaultTimeout);
  auto callee_local_description = callee()->pc()->local_description();
  auto recv_codecs = callee_local_description->description()
                         ->contents()[0]
                         .media_description()
                         ->as_video()
                         ->codecs();
