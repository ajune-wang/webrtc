...
int32_t QualityAnalyzingVideoDecoder::Decode(
  ...
  if (out.discard) {
    ...
    // frame, because in such case receiving part will confused with all frames
    // missing and will request a key frame, which will result into extra load
    // on network and sender side. Because of it discarded image will be always
    // decoded as black frame and will be passed to callback directly without
    // reaching decoder and video quality analyzer.
    //
