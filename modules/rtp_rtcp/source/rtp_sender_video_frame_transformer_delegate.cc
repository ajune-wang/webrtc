...
class TransformableVideoSenderFrame
 ...
 public:
  TransformableVideoSenderFrame(
      ...
      std::unique_ptr<FrameDependencyStructure> frame_dependency_structure)
      : encoded_data_(encoded_image.GetEncodedData()),
        ...
        ssrc_(ssrc),
        csrcs_(csrcs),
        frame_dependency_structure_(std::move(frame_dependency_structure)) {
...
bool RTPSenderVideoFrameTransformerDelegate::TransformFrame(
    ...
    TimeDelta expected_retransmission_time,
    rtc::FunctionView<const FrameDependencyStructure*()>
  ...
  std::unique_ptr<FrameDependencyStructure> frame_dependency_structure;
  {
    ...
    // Only copy and attach the frame_dependency_structure on keyframes to
    // avoid unnecesary structure copies.
    if (encoded_image._frameType == VideoFrameType::kVideoFrameKey) {
      // Invoke the frame dependency structure getter under sender_lock.
      const FrameDependencyStructure* sender_frame_dependency_structure =
          frame_dependency_structure_getter();
      frame_dependency_structure =
          sender_frame_dependency_structure
  ...
  frame_transformer_->Transform(std::make_unique<TransformableVideoSenderFrame>(
      ...
      std::move(frame_dependency_structure)));
  return true;
}
...
void RTPSenderVideoFrameTransformerDelegate::SendVideo(
  ...
  if (transformed_frame->GetDirection() ==
    ...
    if (transformed_video_frame->GetFrameDependencyStructure()) {
      sender_->SetVideoStructureAfterTransformation(
          transformed_video_frame->GetFrameDependencyStructure());
    }
    ...
    auto* transformed_video_frame =
        static_cast<TransformableVideoFrameWithDependencies*>(
            transformed_frame.get());
...
std::unique_ptr<TransformableVideoFrameInterface> CloneSenderVideoFrame(
  ...
  auto* original_frame =
      static_cast<TransformableVideoFrameWithDependencies*>(original);
  ...
  const FrameDependencyStructure* deps =
      original_frame->GetFrameDependencyStructure();
  return std::make_unique<TransformableVideoSenderFrame>(
      ...
      deps ? std::make_unique<FrameDependencyStructure>(*deps) : nullptr);
}
