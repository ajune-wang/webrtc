...
void EchoRemoverImpl::ProcessCapture(
  ...
  subtractor_.Process(*render_buffer, (*y->GetInternalData())[0],
                      render_signal_analyzer_, aec_state_, subtractor_output);

  // Compute spectra.
  for (size_t ch = 0; ch < num_capture_channels_; ++ch) {
    FormLinearFilterOutput(subtractor_output[ch], e[ch]);
    WindowedPaddedFft(fft_, (*y->GetInternalData())[0][ch], y_old_[ch], &Y[ch]);
  ...
  data_dumper_->DumpWav("aec3_output_linear", kBlockSize,
                        y->FloatArray(/*band=*/0, /*channel=*/0), 16000, 1);
  ...
  if (capture_output_used_) {
    ...
    suppression_filter_.ApplyGain(comfort_noise, high_band_comfort_noise, G,
                                  high_bands_gain, Y_fft, y->GetInternalData());
  ...
  data_dumper_->DumpRaw("aec3_output", y->FloatArray(/*band=*/0, /*channel=*/0),
                        kBlockSize);
