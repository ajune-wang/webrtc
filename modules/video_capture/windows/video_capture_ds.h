...
class VideoCaptureDS : public VideoCaptureImpl {
 ...
 protected:
  ...
  DeviceInfoDS _dsInfo RTC_GUARDED_BY(main_checker_);

  IBaseFilter* _captureFilter RTC_GUARDED_BY(main_checker_);
  IGraphBuilder* _graphBuilder RTC_GUARDED_BY(main_checker_);
  IMediaControl* _mediaControl RTC_GUARDED_BY(main_checker_);
  rtc::scoped_refptr<CaptureSinkFilter> sink_filter_
      RTC_GUARDED_BY(main_checker_);
  IPin* _inputSendPin RTC_GUARDED_BY(main_checker_);
  IPin* _outputCapturePin RTC_GUARDED_BY(main_checker_);

  // Microsoft DV interface (external DV cameras)
  IBaseFilter* _dvFilter RTC_GUARDED_BY(main_checker_);
  IPin* _inputDvPin RTC_GUARDED_BY(main_checker_);
  IPin* _outputDvPin RTC_GUARDED_BY(main_checker_);
};
}  // namespace videocapturemodule
}  // namespace webrtc
