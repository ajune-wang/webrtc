...
class ClosureTask : public QueuedTask {
 ...
 private:
  ...
  typename std::remove_reference<Closure>::type closure_;
};
...
class ClosureTaskWithCleanup : public ClosureTask<Closure> {
 ...
 private:
  typename std::remove_reference<Cleanup>::type cleanup_;
};
...
inline void PostTask(TaskQueueBase* task_queue,
...
void PostTask(TaskQueueBase* task_queue, Closure&& closure) {
  task_queue->PostTask(NewClosure(std::forward<Closure>(closure)));
}

template <typename Closure, typename Cleanup>
void PostTask(TaskQueueBase* task_queue, Closure&& closure, Cleanup&& cleanup) {
  task_queue->PostTask(NewClosure(std::forward<Closure>(closure),
...
// TaskQueueBase interface.
inline void PostDelayedTask(TaskQueueBase* task_queue,
                            std::unique_ptr<QueuedTask> task,
                            uint32_t delay_ms) {
...
template <typename Closure,
          typename std::enable_if<!std::is_convertible<
              Closure,
              std::unique_ptr<QueuedTask>>::value>::type* = nullptr>
void PostDelayedTask(TaskQueueBase* task_queue,
                     Closure&& closure,
                     uint32_t delay_ms) {
