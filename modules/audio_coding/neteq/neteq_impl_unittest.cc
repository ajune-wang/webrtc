...
TEST_F(NetEqImplTest, SetBaseMinimumDelay) {
  ...
  EXPECT_EQ(true, neteq_->SetBaseMinimumDelay(delay_ms));
  EXPECT_EQ(false, neteq_->SetBaseMinimumDelay(delay_ms));
}

TEST_F(NetEqImplTest, GetBaseMinimumDelay) {
  UseNoMocks();
  use_mock_delay_manager_ = true;
  CreateInstance();
  ...
  EXPECT_EQ(delay_ms, neteq_->GetBaseMinimumDelay());
}
