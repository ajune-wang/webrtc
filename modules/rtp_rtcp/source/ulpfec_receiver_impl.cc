...
int32_t UlpfecReceiverImpl::AddReceivedRedPacket(
  ...
  if (payload_data_length == 0) {
    NLOG(LS_WARNING, "Corrupt/truncated FEC packet.");
    return -1;
  }
  ...
  if (incoming_rtp_packet[header.headerLength] & 0x80) {
    ...
    if (timestamp_offset != 0) {
      NLOG(LS_WARNING, "Corrupt payload found.");
      return -1;
    }
    ...
    if (incoming_rtp_packet[header.headerLength + 4] & 0x80) {
      NLOG(LS_WARNING, "More than 2 blocks in packet not supported.");
      return -1;
    }
    ...
    if (block_length > payload_data_length - (red_header_length + 1)) {
      NLOG(LS_WARNING, "Block length longer than packet.");
      return -1;
    }
  }
