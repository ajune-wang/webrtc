...
void Call::UpdateAggregateNetworkState() {
  ...
  rtc::CritScope cs(&aggregate_network_up_crit_);
  aggregate_network_up_ = ((have_video && video_network_state_ == kNetworkUp) ||
                           (have_audio && audio_network_state_ == kNetworkUp));

  RTC_LOG(LS_INFO) << "UpdateAggregateNetworkState: aggregate_state="
                   << (aggregate_network_up_ ? "up" : "down");

  transport_send_->OnNetworkAvailability(aggregate_network_up_);
}
