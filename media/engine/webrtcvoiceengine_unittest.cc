...
class WebRtcVoiceEngineTestFake : public testing::Test {
 public:
  ...
  void VerifyVoiceReceiverInfo(const cricket::VoiceReceiverInfo& info) {
    ...
    EXPECT_EQ(static_cast<unsigned int>(info.packets_rcvd), stats.packets_rcvd);
    EXPECT_EQ(static_cast<unsigned int>(info.packets_lost), stats.packets_lost);
    EXPECT_EQ(info.fraction_lost, stats.fraction_lost);
    EXPECT_EQ(info.codec_name, stats.codec_name);
    EXPECT_EQ(info.codec_payload_type, stats.codec_payload_type);
    EXPECT_EQ(static_cast<unsigned int>(info.ext_seqnum), stats.ext_seqnum);
    EXPECT_EQ(static_cast<unsigned int>(info.jitter_ms), stats.jitter_ms);
    EXPECT_EQ(static_cast<unsigned int>(info.jitter_buffer_ms),
              stats.jitter_buffer_ms);
    EXPECT_EQ(static_cast<unsigned int>(info.jitter_buffer_preferred_ms),
              stats.jitter_buffer_preferred_ms);
    EXPECT_EQ(static_cast<unsigned int>(info.delay_estimate_ms),
...
TEST_F(WebRtcVoiceEngineTestFake, SetRecvCodecsAfterAddingStreams) {
  ...
  ASSERT_EQ(static_cast<size_t>(1), dm.count(106));
...
TEST_F(WebRtcVoiceEngineTestFake, CannotSetMaxBitrateForNonexistentStream) {
  ...
  webrtc::RtpParameters nonexistent_parameters =
      channel_->GetRtpSendParameters(kSsrcX);
  EXPECT_EQ(static_cast<size_t>(0), nonexistent_parameters.encodings.size());
...
TEST_F(WebRtcVoiceEngineTestFake, SetSendCodecOpusGoodNBitrateNoStereo) {
  ...
  EXPECT_EQ(static_cast<size_t>(2), spec.format.num_channels);
...
TEST_F(WebRtcVoiceEngineTestFake, SetSendCodecsCNandDTMFAsCaller) {
  ...
  EXPECT_EQ(96, send_codec_spec.payload_type);
  EXPECT_STRCASEEQ("ISAC", send_codec_spec.format.name.c_str());
  EXPECT_EQ(static_cast<size_t>(1), send_codec_spec.format.num_channels);
...
TEST_F(WebRtcVoiceEngineTestFake, GetStatsWithMultipleSendStreams) {
  ...
  // Check stats for the added streams.
  {
    ...
    EXPECT_EQ(info.receivers[0].ssrc(), static_cast<unsigned int>(0));
  }
...
TEST_F(WebRtcVoiceEngineTestFake, RecvUnsignaled) {
  EXPECT_TRUE(SetupChannel());
  EXPECT_EQ(static_cast<size_t>(0), call_.GetAudioReceiveStreams().size());

  DeliverPacket(kPcmuFrame, sizeof(kPcmuFrame));

  EXPECT_EQ(static_cast<size_t>(1), call_.GetAudioReceiveStreams().size());
...
TEST_F(WebRtcVoiceEngineTestFake, RecvUnsignaledAfterSignaled) {
  ...
  EXPECT_TRUE(GetRecvStream(unsignaled_ssrc).VerifyLastPacket(
      packet, sizeof(packet)));
  EXPECT_EQ(static_cast<size_t>(2), call_.GetAudioReceiveStreams().size());
...
TEST_F(WebRtcVoiceEngineTestFake, AddRecvStreamAfterUnsignaled_NoRecreate) {
  ...
  const auto& streams = call_.GetAudioReceiveStreams();
  EXPECT_EQ(static_cast<size_t>(1), streams.size());
...
TEST_F(WebRtcVoiceEngineTestFake, StreamCleanup) {
  ...
  EXPECT_EQ(static_cast<size_t>(1), call_.GetAudioSendStreams().size());
  EXPECT_EQ(static_cast<size_t>(2), call_.GetAudioReceiveStreams().size());
  delete channel_;
  channel_ = NULL;
  EXPECT_EQ(static_cast<size_t>(0), call_.GetAudioSendStreams().size());
  EXPECT_EQ(static_cast<size_t>(0), call_.GetAudioReceiveStreams().size());
}
...
TEST_F(WebRtcVoiceEngineTestFake, SetAudioOptions) {
  ...
  EXPECT_EQ(static_cast<size_t>(50),
            GetRecvStreamConfig(kSsrcY).jitter_buffer_max_packets);
...
TEST_F(WebRtcVoiceEngineTestFake, SetsSyncGroupFromStreamId) {
  ...
  ASSERT_EQ(static_cast<size_t>(2), call_.GetAudioReceiveStreams().size());
  EXPECT_EQ(kStreamId,
...
TEST_F(WebRtcVoiceEngineTestFake, ConfiguresAudioReceiveStreamRtpExtensions) {
  ...
  for (uint32_t ssrc : ssrcs) {
    ...
    EXPECT_NE(nullptr, s);
    EXPECT_EQ(static_cast<size_t>(0), s->GetConfig().rtp.extensions.size());
  }
...
TEST(WebRtcVoiceEngineTest, CollectRecvCodecs) {
  ...
  EXPECT_EQ(static_cast<size_t>(11), codecs.size());
