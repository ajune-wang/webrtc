...
float FecControllerDefault::GetProtectionOverheadRateThreshold() {
  float overheadThreshold = strtof(webrtc::field_trial::FindFullName(
                                       "WebRTC-ProtectionOverheadRateThreshold")
                                       .c_str(),
                                   nullptr);
  if (overheadThreshold > 0 && overheadThreshold <= 1) {
    RTC_LOG(LS_INFO) << "ProtectionOverheadRateThreshold is set to "
                     << overheadThreshold;
    return overheadThreshold;
  }
  RTC_LOG(LS_WARNING)
      << "ProtectionOverheadRateThreshold field trial is not "
         "found, protection overhead threshold will be set to default value: "
