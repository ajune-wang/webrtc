...
TEST(ArrayViewTest, TestArrayView) {
  ...
  EXPECT_EQ(kArrSize, len);
  // len = CallFixed<float, kArrSize>(rtc::ArrayView<float>(arr));           // Expected to Compile!!!
  EXPECT_EQ(kArrSize, len);
  ...
  // len = CallFixed<const float, kArrSize>(rtc::ArrayView<float,
  // kArrSize>(arr.data(), arr.size()));                                     // Expected to Compile!!!
  EXPECT_EQ(kArrSize, len);
