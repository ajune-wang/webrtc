...
class ProcessableEventList : public ProcessableEventListInterface {
 public:
  ProcessableEventList(int index,
                       Iterator begin,
                       Iterator end,
                       std::function<void(const T&)> f)
      : index_(index), begin_(begin), end_(end), f_(f) {}
 ...
 public:
  ...
  void AddEvents(
    ...
    event_lists_.emplace_back(
        new event_processor_impl::ProcessableEventList<
            typename Iterable::const_iterator, typename Iterable::value_type>(
            next_index_++, iterable.cbegin(), iterable.cend(), handler));
