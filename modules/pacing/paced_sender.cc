...
void PacedSender::OnProcessInterval(network::ProcessInterval msg) {
  ...
  if (is_probing) {
    ...
    bool probing_send_failure = bytes_sent == 0;
    if (!probing_send_failure)
      prober_->ProbeSent(now_ms, bytes_sent);
    int time_to_probe_ms = prober_->TimeUntilNextProbe(now_ms);
    if (time_to_probe_ms > 0 ||
        (time_to_probe_ms == 0 && !probing_send_failure)) {
      RTC_LOG(LS_INFO) << time_to_probe_ms;
      TimeDelta delay = TimeDelta::ms(time_to_probe_ms);
      ProcessInterval msg;
      msg.at_time = Timestamp::ms(now_ms) + delay;
      msg.elapsed_time = delay;
      ProcessIntervalJunction.DelayMessage(msg, delay);
    }
  }
  SyncPacketsState();
}

void PacedSender::SyncPacketsState() {
  rtc::CritScope cs(&critsect_);
