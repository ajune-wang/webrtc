...
class TransformableAudioFrame : public TransformableFrameInterface {
 public:
  ...
  uint32_t timestamp() const override { return rtp_timestamp_; }
  uint32_t ssrc() const override { return ssrc_; }
  AudioFrameType frame_type() const { return frame_type_; }
...
int32_t ChannelSendFrameTransformerDelegate::Transform(
...
void ChannelSendFrameTransformerDelegate::SendFrame(
  ...
  send_frame_callback_(
      transformed_frame->frame_type(), transformed_frame->payload_type(),
      transformed_frame->timestamp(), transformed_frame->payload(),
      transformed_frame->absolute_capture_timestamp_ms());
}
