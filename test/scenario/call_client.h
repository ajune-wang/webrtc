...
class CallClient {
 public:
  ...
  // Switches to specified |endpoint|. |endpoint| have to be one from the
  // vector, returned by GetEndpoints();
  void SwitchToEndpoint(EndpointNode* endpoint);
 ...
 private:
  ...
  std::atomic<EndpointNode*> current_endpoint_{nullptr};
