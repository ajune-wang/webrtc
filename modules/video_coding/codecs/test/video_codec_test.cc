...
TEST_P(EncodeDecodeTest, TestEncodeDecode) {
  ...
  for (auto fs = frame_settings.begin(); fs != frame_settings.end(); ++fs) {
    ...
    auto vs = stats->SliceAndCalcLayerVideoStatistic(first_frame, last_frame);

    EXPECT_LE(vs[0].avg_bitrate_mismatch_pct, 10);
    EXPECT_GE(vs[0].avg_bitrate_mismatch_pct, -10);
  }
