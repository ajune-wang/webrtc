...
// SendAudioData() is called when we have a new audio frame input. This MAY end
// getting queued to be processed and eventually encoded and set onto the
// network, but this method is invoked even if we are disconnected.
void AudioSendStream::SendAudioData(std::unique_ptr<AudioFrame> audio_frame) {
  ...
  {
    // Note: Audio level is computed here even if we never end up sending the
    // data. On this stage in the pipeline we are concerned with the input
    // audio, not necessarily with the samples sent.
