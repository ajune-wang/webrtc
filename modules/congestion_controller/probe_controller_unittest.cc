...
#include "modules/pacing/mock/mock_alr_detector.h"
 ...
 protected:
  ProbeControllerTest() : clock_(100000000L) {
    probe_controller_.reset(
        new ProbeController(&clock_, &pacer_, &alr_detector_));
  }
  ~ProbeControllerTest() override {}

  SimulatedClock clock_;
  NiceMock<MockPacedSender> pacer_;
  NiceMock<MockAlrDetector> alr_detector_;
...
TEST_F(ProbeControllerTest, RequestProbeInAlr) {
  ...
  EXPECT_CALL(alr_detector_, GetApplicationLimitedRegionStartTime())
      .WillRepeatedly(
...
TEST_F(ProbeControllerTest, PeriodicProbingAfterReset) {
  ...
  testing::StrictMock<MockAlrDetector> local_alr_detector;
  probe_controller_.reset(
      new ProbeController(&clock_, &local_pacer, &local_alr_detector));
  int64_t alr_start_time = clock_.TimeInMilliseconds();
  EXPECT_CALL(local_alr_detector, GetApplicationLimitedRegionStartTime())
