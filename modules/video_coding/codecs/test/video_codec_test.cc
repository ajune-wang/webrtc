...
ABSL_FLAG(std::string, codec_name, "AV1", "Codec name.");
...
// TODO: move to tester
std::map<LayerId, LayerSettings> CreateLayerSettings(
...
// TODO: move to tester?
FrameSettings CreateFrameSettings(std::string codec_name,
                                  std::string scalability_name,
                                  int width,
                                  int height,
                                  std::vector<std::vector<int>> bitrate_kbps,
                                  double framerate_fps,
                                  int num_frames,
  ...
  if ((num_spatial_layers > 1 || num_temporal_layers > 1) &&
    ...
    // Only total bitrate is provided. Use a standard rate allocator to
    // distribute the total bitrate between layers.
    layer_settings =
        CreateLayerSettings(codec_type, scalability_mode, width, height,
                            bitrate_kbps[0][0], framerate_fps);
  } else {
    RTC_CHECK_EQ(num_spatial_layers, bitrate_kbps.size())
        << "number of bitrates must be equal to number of spatial layers";
    for (int sidx = 0; sidx < num_spatial_layers; ++sidx) {
      int layer_width = width >> (num_spatial_layers - sidx - 1);
      int layer_height = height >> (num_spatial_layers - sidx - 1);
      for (int tidx = 0; tidx < num_temporal_layers; ++tidx) {
        RTC_CHECK_EQ(num_temporal_layers, bitrate_kbps[sidx].size())
            << "number of bitrates must be equal to number of temporal "
               "layers";
        layer_settings.emplace(
            ...
            LayerSettings{
                ...
                .framerate = Frequency::MilliHertz(
                    static_cast<int>(1000 * framerate_fps) >>
                    (num_temporal_layers - tidx - 1)),
                .bitrate = DataRate::KilobitsPerSec(bitrate_kbps[sidx][tidx])});
      }
    }
  }
...
TEST(VideoCodecTest, EncodeDecode) {
  ...
  std::vector<std::vector<int>> bitrate_kbps;
  for (int sidx = 0; sidx < num_spatial_layers; ++sidx) {
    std::vector<int> temporal_layer_bitrates_kbps;
    for (int tidx = 0; tidx < num_temporal_layers; ++tidx) {
      int layer_idx = sidx * num_temporal_layers + tidx;
      std::string bitrate_str = absl::GetFlag(FLAGS_bitrate_kbps)[layer_idx];
      temporal_layer_bitrates_kbps.push_back(std::stoi(bitrate_str));
    }
    bitrate_kbps.push_back(temporal_layer_bitrates_kbps);
  }

  std::unique_ptr<VideoCodecStats> stats = TestEncodeDecode(
      kFourPeople_1280x720_30, absl::GetFlag(FLAGS_codec_name),
      absl::GetFlag(FLAGS_codec_impl), absl::GetFlag(FLAGS_scalability_mode),
      absl::GetFlag(FLAGS_width), absl::GetFlag(FLAGS_height),
      absl::GetFlag(FLAGS_framerate_fps), bitrate_kbps,
      absl::GetFlag(FLAGS_num_frames));

  stats->LogMetrics(/*metadata=*/
                    {{"codec_type", absl::GetFlag(FLAGS_codec_name)},
                     {"codec_impl", codec_impl},
...
TEST_P(SpatialQualityTest, SpatialQuality) {
  ...
  TestEncodeDecode(video_info, codec_name, codec_impl, scalability_mode, width,
...
class BitrateAdaptationTest
    : public ::testing::TestWithParam<
          std::tuple</*codec_name=*/std::string,
                     /*codec_impl=*/std::string,
                     RawVideoInfo,
 ...
 public:
  static std::string TestParamsToString(
    ...
    auto [codec_name, codec_impl, video_info, bitrate_kbps] = info.param;
    return std::string(codec_name + codec_impl + video_info.name +
