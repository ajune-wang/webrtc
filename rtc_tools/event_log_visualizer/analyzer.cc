...
class NetEqStreamInput : public test::NetEqInput {
 public:
  ...
  rtc::Optional<int64_t> NextPacketTime() const override {
    ...
    return TimeStampToPacketTime(packet_stream_it_->rtp.log_time_us());
  }
  ...
  std::unique_ptr<PacketData> PopPacket() override {
    ...
    packet_data->time_ms =
        TimeStampToPacketTime(packet_stream_it_->rtp.log_time_us());
...
EventLogAnalyzer::SimulateNetEq(const std::string& replacement_file_name,
  ...
  std::map<uint32_t, std::unique_ptr<test::NetEqStatsGetter>> neteq_stats;
