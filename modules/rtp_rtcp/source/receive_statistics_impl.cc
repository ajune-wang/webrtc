...
void StreamStatisticianImpl::MaybeAppendReportBlockAndReset(
  ...
  // packets lost is a 24 bit signed field, and thus should be clamped,
  // as described in https://datatracker.ietf.org/doc/html/rfc3550#appendix-A.3
  int packets_lost =
      std::min(0x7f'ffff, cumulative_loss_ + cumulative_loss_rtcp_offset_);
  if (packets_lost < 0) {
