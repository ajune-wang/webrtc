...
class VirtualSocketServer : public SocketServer {
 public:
  ...
  uint32_t bandwidth() const RTC_LOCKS_EXCLUDED(mutex_);
  ...
  uint32_t network_capacity() const RTC_LOCKS_EXCLUDED(mutex_);
  ...
  uint32_t delay_mean() const RTC_LOCKS_EXCLUDED(mutex_);
  uint32_t delay_stddev() const RTC_LOCKS_EXCLUDED(mutex_);
  uint32_t delay_samples() const RTC_LOCKS_EXCLUDED(mutex_);
  ...
  double drop_probability() const RTC_LOCKS_EXCLUDED(mutex_);
  ...
  // be dropped.
  size_t max_udp_payload() const RTC_LOCKS_EXCLUDED(mutex_);
  void set_max_udp_payload(size_t payload_size) RTC_LOCKS_EXCLUDED(mutex_);

  size_t largest_seen_udp_payload() const RTC_LOCKS_EXCLUDED(mutex_);
  ...
  // Requires mutex_ to be held prior to calling this function.
 ...
 private:
  ...
  void AddPacketToNetwork(VirtualSocket* socket,
                          ...
                          size_t data_size,
                          size_t header_size,
                          bool ordered) RTC_EXCLUSIVE_LOCKS_REQUIRED(mutex_);
  ...
  size_t largest_seen_udp_payload_ RTC_GUARDED_BY(mutex_) = 0;
