...
TEST(PeerConnectionFactoryTest, NativeToJavaPeerConnectionFactory) {
  ...
  jclass helper_cls =
      jni->FindClass("org/webrtc/PeerConnectionFactoryInitializationHelper");
  RTC_LOG(INFO) << "Helper class: " << helper_cls;
  jmethodID init_method = jni->GetStaticMethodID(
      helper_cls, "initializePeerConnectionFactoryForTests", "()V");
  RTC_LOG(INFO) << "Init method: " << init_method;
  jni->CallStaticVoidMethod(helper_cls, init_method);
