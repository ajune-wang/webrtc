...
constexpr int kMaxWaitForReceivedKeyFrameMs = 200;
constexpr int kMaxWaitForReceivedFrameMs = 3000;

int MaxWaitForReceivedKeyframeMs() {
  ...
  return kMaxWaitForReceivedKeyFrameMs;
}

int MaxWaitForReceivedFrameMs() {
  const absl::optional<int> override =
      KeyframeIntervalSettings::ParseFromFieldTrials()
  ...
  return kMaxWaitForReceivedFrameMs;
}
...
VideoReceiveStream::VideoReceiveStream(
    ...
    : task_queue_factory_(task_queue_factory),
      ...
      max_wait_for_received_keyframe_ms_(MaxWaitForReceivedKeyframeMs()),
      max_wait_for_received_frame_ms_(MaxWaitForReceivedFrameMs()) {
...
bool VideoReceiveStream::Decode() {
  ...
  const int wait_ms = keyframe_required_ ? max_wait_for_received_keyframe_ms_
  ...
  if (frame) {
    ...
    if (decode_result == WEBRTC_VIDEO_CODEC_OK ||
      ...
      if (decode_result == WEBRTC_VIDEO_CODEC_OK_REQUEST_KEYFRAME)
        RequestKeyFrame();
               ...
               (last_keyframe_request_ms_ + max_wait_for_received_keyframe_ms_ <
                now_ms)) {
    ...
    bool receiving_keyframe =
        ...
        now_ms - *last_keyframe_packet_ms < max_wait_for_received_keyframe_ms_;
