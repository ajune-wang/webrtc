...
std::map<Key, T> JavaToNativeMap(JNIEnv* env,
  ...
  auto j_map_entry_set = GetJavaMapEntrySet(env, j_map);

  for (JavaRef<jobject> const& j_entry : j_map_entry_set) {
    auto j_key = GetJavaMapEntryKey(env, j_entry);
    auto j_value = GetJavaMapEntryValue(env, j_entry);
    container.emplace(convert(env, j_key, j_value));
  }
