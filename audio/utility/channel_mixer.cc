...
void ChannelMixer::Transform(AudioFrame* frame) {
  ...
  if (UpMixing()) {
    /*
    RTC_DLOG(INFO) << "reserve:" << frame->samples_per_channel() *
    output_channels_; out_vec_.reserve(frame->samples_per_channel() *
    output_channels_); out_audio = out_vec_.data(); RTC_DLOG(INFO) << "size:" <<
    out_vec_.max_size();
    */
    out_vec_.reset(
        new int16_t[frame->samples_per_channel() * output_channels_]);
    out_audio = out_vec_.get();
  }
