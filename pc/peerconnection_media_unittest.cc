...
TEST_F(PeerConnectionMediaTest, SetLocalOfferTwiceWorks) {
...
TEST_F(PeerConnectionMediaTest, SetRemoteOfferTwiceWorks) {
  auto caller = CreatePeerConnection();
...
TEST_F(PeerConnectionMediaTest, FailToSetNullLocalDescription) {
  auto caller = CreatePeerConnection();
  std::string error;
...
TEST_F(PeerConnectionMediaTest, FailToSetNullRemoteDescription) {
  auto caller = CreatePeerConnection();
  std::string error;
...
TEST_F(PeerConnectionMediaTest, FailToCreateAnswerWithNoRemoteDescription) {
  auto callee = CreatePeerConnection();
  std::string error;
...
TEST_F(PeerConnectionMediaTest, FailToSetRemoteOfferAfterLocalOfferSet) {
  auto caller = CreatePeerConnection();
...
TEST_F(PeerConnectionMediaTest, FailToSetLocalOfferAfterRemoteOfferSet) {
  auto caller = CreatePeerConnection();
  auto callee = CreatePeerConnection();
...
TEST_F(PeerConnectionMediaTest, FailToSetRemoteAnswerWithoutOffer) {
  auto caller = CreatePeerConnection();
  auto callee = CreatePeerConnection();
...
TEST_F(PeerConnectionMediaTest, FailToSetLocalAnswerWithoutOffer) {
  auto caller = CreatePeerConnection();
  auto callee = CreatePeerConnection();
...
TEST_F(PeerConnectionMediaTest, AudioVideoOfferAnswerCreateSendRecvStreams) {
  const std::string kCallerAudioLabel = "caller_a";
  const std::string kCallerVideoLabel = "caller_v";
  const std::string kCalleeAudioLabel = "callee_a";
  const std::string kCalleeVideoLabel = "callee_v";

  auto caller = CreatePeerConnection();
  caller->AddAudioTrack(kCallerAudioLabel);
  caller->AddVideoTrack(kCallerVideoLabel);

  auto callee = CreatePeerConnection();
  callee->AddAudioTrack(kCalleeAudioLabel);
  callee->AddVideoTrack(kCalleeVideoLabel);
...
TEST_P(PeerConnectionMediaAnswerDirectionTest, VerifyDirection) {
  ...
  auto offer_direction =
      cricket::RtpTransceiverDirection::FromMediaContentDirection(
          offer_direction_);
  cricket::RtpTransceiverDirection answerer_direction(send_media_,
                                                      (offer_to_receive_ != 0));

  // The negotiated components determine the direction set in the answer.
  bool negotiate_send = (answerer_direction.send && offer_direction.recv);
  bool negotiate_recv = (answerer_direction.recv && offer_direction.send);
...
TEST_F(PeerConnectionMediaTest, InitiatorFlagSetOnCallerAndNotOnCallee) {
  auto caller = CreatePeerConnectionWithAudioVideo();
  auto callee = CreatePeerConnectionWithAudioVideo();
...
TEST_F(PeerConnectionMediaTest, CreateOffersAndShutdown) {
  auto caller = CreatePeerConnection();
