...
// Fixed-size queue similar to SwapQueue, but lock-free and no
// QueueItemVerifierFunction. The queue is designed to be accessed by one
// producer thread (calling Insert()), and one consumer thread (calling
// Remove()).
template <typename T>
class MessageQueue {
 public:
