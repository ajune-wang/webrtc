...
std::unique_ptr<DesktopFrameIOSurface> DesktopFrameIOSurface::Wrap(
  ...
  RTC_LOG(LS_VERBOSE) << "DesktopFrameIOSurface wrapping IOSurface with size " << surfaceWidth
                      << "x" << surfaceHeight << ". Cropping to (" << offsetColumns << ","
                      << offsetRows << "; " << width << "x" << height
  ...
  return std::unique_ptr<DesktopFrameIOSurface>(
      new DesktopFrameIOSurface(io_surface, data + offset, width, height, stride));
}
...
DesktopFrameIOSurface::DesktopFrameIOSurface(rtc::ScopedCFTypeRef<IOSurfaceRef> io_surface,
                                             ...
                                             int32_t height,
                                             int32_t stride)
    : DesktopFrame(DesktopSize(width, height), stride, data, nullptr), io_surface_(io_surface) {
