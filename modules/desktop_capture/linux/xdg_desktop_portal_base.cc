...
void XdgDesktopPortalBase::InitPortal(rtc::Callback1<void, bool> callback,
  ...
  std::unique_ptr<ConnectionData> connection_data(new ConnectionData(web_id));
  connection_data->callback_ = callback;
  connection_data->capture_source_type_ = type;

  connection_data_map_.insert({web_id, std::move(connection_data)});
...
ConnectionData* XdgDesktopPortalBase::GetConnectionData(int32_t id) const {
  int32_t valid_id = id ? id : current_connection_id_;
...
void XdgDesktopPortalBase::OnProxyRequested(GObject* /*object*/,
  ...
  auto* portalBase = data->GetXdgDesktopPortalBase();
  RTC_CHECK(portalBase);

  auto connection_data = portalBase->GetConnectionData(data->GetDataId());
  RTC_CHECK(connection_data);
  ...
  portalBase->SessionRequest(data);
}
...
void XdgDesktopPortalBase::OnSessionRequestResponseSignal(
  ...
  portalBase->SourcesRequest(data);
}
...
void XdgDesktopPortalBase::OnStartRequestResponseSignal(
  ...
  if (g_variant_lookup(response_data.get(), "streams", "a(ua{sv})", &iter)) {
    Scoped<GVariant> variant;
    ...
    while (g_variant_iter_next(iter.get(), "@(ua{sv})", &variant)) {
      ...
      g_variant_get(variant.get(), "(u@a{sv})", &stream_id, &options);
...
void XdgDesktopPortalBase::OpenPipeWireRemote(
  ...
  connection_data->pw_callback_ = pwCallback;
...
void XdgDesktopPortalBase::OnOpenPipeWireRemoteRequested(
  ...
  if (!variant) {
    RTC_LOG(LS_ERROR) << "Failed to open the PipeWire remote: "
                      << error->message;
    connection_data->portal_init_failed_ = true;
    connection_data->pw_callback_(false, 0);
    return;
  }
  ...
  if ((connection_data->pw_fd_ =
           g_unix_fd_list_get(outlist.get(), index, &error)) == -1) {
  ...
  connection_data->pw_callback_(true, connection_data->id_);
}
