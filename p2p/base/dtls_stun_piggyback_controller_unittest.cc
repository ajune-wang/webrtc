...
TEST_F(DtlsStunPiggybackControllerTest, BasicHandshakeAckData) {
  ...
  SendClientToServer(dtls_flight1, STUN_BINDING_REQUEST);
  SendServerToClient(dtls_flight2, STUN_BINDING_RESPONSE);
  EXPECT_EQ(server_.GetAckToPiggyback(STUN_BINDING_RESPONSE), 6 << 2);
  EXPECT_EQ(client_.GetAckToPiggyback(STUN_BINDING_REQUEST), 0 << 2);

  // Flight 3+4
  SendClientToServer(dtls_flight3, STUN_BINDING_REQUEST);
  SendServerToClient(dtls_flight4, STUN_BINDING_RESPONSE);
  // EXPECT_EQ(server_.GetAckToPiggyback(STUN_BINDING_RESPONSE), (7 << 2) + 1);
  EXPECT_EQ(client_.GetAckToPiggyback(STUN_BINDING_REQUEST),
            std::numeric_limits<uint64_t>::max());
...
TEST_F(DtlsStunPiggybackControllerTest, HighestAckData) {
  // Flight 1+2
  SendClientToServer(dtls_flight1, STUN_BINDING_REQUEST);
  EXPECT_EQ(server_.GetAckToPiggyback(STUN_BINDING_REQUEST), 6 << 2);
  SendClientToServer(dtls_flight3, STUN_BINDING_REQUEST);
  EXPECT_EQ(server_.GetAckToPiggyback(STUN_BINDING_REQUEST), (7 << 2) + 1);
...
TEST_F(DtlsStunPiggybackControllerTest, PackedDtlsHelloFormat) {
  ...
  EXPECT_EQ(server_.GetAckToPiggyback(STUN_BINDING_RESPONSE), 1 << 2);

  SendClientToServer(dtls_flight2_fragment_packed2, STUN_BINDING_REQUEST);
  EXPECT_EQ(server_.GetAckToPiggyback(STUN_BINDING_RESPONSE), (4 << 2) + 4);
}
