...
class LibaomAv1Encoder final : public VideoEncoder {
 ...
 private:
  void InitScalability(ScalableVideoController::StreamLayersConfig svc_config);
  // Sets scalability parameter to encode next frame with.
  // returns aom flags to pass to encode function.
  aom_enc_frame_flags_t SetLayerPattern(
...
int LibaomAv1Encoder::InitEncode(const VideoCodec* codec_settings,
  ...
  InitScalability(svc_controller_->StreamConfig());

  return WEBRTC_VIDEO_CODEC_OK;
}

void LibaomAv1Encoder::InitScalability(
...
aom_enc_frame_flags_t LibaomAv1Encoder::SetLayerPattern(
    const ScalableVideoController::LayerFrameConfig& layer_frame) {
  ...
  static constexpr aom_enc_frame_flags_t kNoRefFlagName[] = {
      AOM_EFLAG_NO_REF_LAST, AOM_EFLAG_NO_REF_LAST2, AOM_EFLAG_NO_REF_LAST3,
      AOM_EFLAG_NO_REF_GF,   AOM_EFLAG_NO_REF_BWD,   AOM_EFLAG_NO_REF_ARF2,
      AOM_EFLAG_NO_REF_ARF,
  };
  static constexpr aom_enc_frame_flags_t kNoReferences =
      AOM_EFLAG_NO_REF_LAST | AOM_EFLAG_NO_REF_LAST2 | AOM_EFLAG_NO_REF_LAST3 |
      AOM_EFLAG_NO_REF_GF | AOM_EFLAG_NO_REF_BWD | AOM_EFLAG_NO_REF_ARF2 |
      AOM_EFLAG_NO_REF_ARF;
  ...
  ret = aom_codec_control(&ctx_, AV1E_SET_SVC_REF_FRAME_CONFIG,
                          &ref_frame_config);
  if (ret != AOM_CODEC_OK) {
    RTC_LOG(LS_WARNING) << "LibaomAv1Encoder::Encode returned " << ret
...
int32_t LibaomAv1Encoder::Encode(
  ...
  for (ScalableVideoController::LayerFrameConfig& layer_frame : layer_frames) {
    aom_enc_frame_flags_t flags = SetLayerPattern(layer_frame);
