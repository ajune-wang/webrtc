...
AudioDecoderOpusImpl::AudioDecoderOpusImpl(const FieldTrialsView& field_trials,
    ...
    : channels_(num_channels),
      ...
      last_packet_num_channels_(num_channels) {
...
void AudioDecoderOpusImpl::UpdateLastPacketWasMono(const uint8_t* encoded,
                                                   size_t encoded_len) {
  const int num_channels = WebRtcOpus_PacketNumChannels(encoded, encoded_len);
  if (num_channels > 0) {
    last_packet_num_channels_ = num_channels;
  } else {
    // Do not update `last_packet_num_channels_` when no packet is provided.
