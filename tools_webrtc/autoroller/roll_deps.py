...
def _RemoveFromStart(string, substring):
  ...
  for path, webrtc_deps_entry in webrtc_entries.iteritems():
    ...
    if cr_deps_entry:
      ...
      if isinstance(cr_deps_entry, CipdDepsEntry):
        assert ({p['package'] for p in cr_deps_entry.packages} ==
                {p['package'] for p in webrtc_deps_entry.packages})
        for cr_pkg in cr_deps_entry.packages:
          for rtc_pkg in webrtc_deps_entry.packages:
            old_version = _RemoveFromStart(rtc_pkg['version'], 'version:')
            new_version = _RemoveFromStart(cr_pkg['version'], 'version:')
            if (cr_pkg['package'] == rtc_pkg['package'] and
                old_version != new_version):
              logging.debug('Roll dependency %s to %s', path, new_version)
              result.append(ChangedCipdPackage(path, rtc_pkg['package'],
                                               old_version, new_version))
        continue
