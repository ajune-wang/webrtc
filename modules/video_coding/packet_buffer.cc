...
bool PacketBuffer::InsertPacket(VCMPacket* packet) {
  std::vector<std::unique_ptr<RtpFrameObject>> found_frames;
  {
    ...
    new_entry.frame_begin = packet->is_first_packet_in_frame();
    new_entry.frame_end = packet->is_last_packet_in_frame();
    new_entry.continuous = false;
    new_entry.frame_created = false;
    new_entry.used = true;
...
bool PacketBuffer::PotentialNewFrame(uint16_t seq_num) const {
  ...
  if (entry.seq_num() != seq_num)
    return false;
  if (entry.frame_created)
    return false;
  if (entry.frame_begin)
    return true;
  if (!prev_entry.used)
    return false;
  if (prev_entry.frame_created)
    return false;
    ...
    if (buffer_[index].frame_end) {
      ...
      if (is_h264) {
        ...
        buffer_[first_packet_index].data.video_header.frame_type =
            is_h264_keyframe ? VideoFrameType::kVideoFrameKey
