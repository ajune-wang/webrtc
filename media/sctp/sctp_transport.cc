...
class SctpTransport::UsrSctpWrapper {
 public:
  static void InitializeUsrSctp(rtc::Thread* usrsctp_thread) {
    RTC_LOG(LS_INFO) << __FUNCTION__;
    RTC_DCHECK(usrsctp_ == nullptr);
    usrsctp_ = new InitialiezedUsrSctp(usrsctp_thread,
                                       &UsrSctpWrapper::OnSctpOutboundPacket);
  }
  ...
  static void IncrementUsrSctpUsageCount(rtc::Thread* usrsctp_thread) {
    rtc::GlobalLockScope lock(&g_usrsctp_lock_);
    if (!g_usrsctp_usage_count) {
      InitializeUsrSctp(usrsctp_thread);
    }
...
bool SctpTransport::OpenSctpSocket() {
  ...
  UsrSctpWrapper::IncrementUsrSctpUsageCount(network_thread_);
