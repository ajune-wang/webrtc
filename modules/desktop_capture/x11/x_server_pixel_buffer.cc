...
void XServerPixelBuffer::InitShm(const XWindowAttributes& attributes) {
    ...
    if (shm_segment_info_->shmid != -1) {
      ...
      if (shmat_result != reinterpret_cast<void*>(-1)) {
        ...
        if (error_trap.GetLastErrorAndDisable() != 0)
          using_shm = false;
        if (using_shm) {
          NLOG(LS_VERBOSE, "Using X shared memory segment ",
               shm_segment_info_->shmid);
        }
      }
    } else {
  ...
  if (!using_shm) {
    NLOG(LS_WARNING, "Not using shared memory. Performance may be degraded.");
    ReleaseSharedMemorySegment();
    return;
  }
  ...
  NLOG(LS_VERBOSE, "Using X shared memory extension v", major, ".", minor,
