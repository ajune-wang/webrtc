...
H264SpsPpsTracker::PacketAction H264SpsPpsTracker::CopyAndFixBitstream(
      ...
      case H264::NaluType::kIdr: {
        ...
        if (video_header.is_first_packet_in_frame) {
          ...
          if (sps->second.data && pps->second.data) {
            ...
            const bool out_of_band_headers =
                !sps->second.rtp_seq_num.has_value() &&
                !pps->second.rtp_seq_num.has_value();
            // Insert out-of-band SPS/PPS or in-band SPS/PPS if its timestamp
            // is different from IDR's one and there is no gap in sequence
            // number.
            // TODO(ssilkin): There could be other NALU(s), e.g. SEI, inbetween
            // SPS/PPS and IDR. In this case SPS/PPS won't be inserted because
            // there will be gap in sequence number.
            if (out_of_band_headers ||
  ...
  // If packet does not contain VCL NAL unit(s) then reset frame begin/end flags
  // to prevent this packet to be interpreted as a frame by the packet buffer.
...
void H264SpsPpsTracker::StoreSps(const uint8_t* data,
  ...
  memcpy(sps_data, data, sps_info.size);
...
void H264SpsPpsTracker::StorePps(const uint8_t* data,
  ...
  memcpy(pps_data, data, pps_info.size);
