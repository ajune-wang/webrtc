...
bool IsValidOneByteHeaderExtension(int id, size_t length) {
  return (length > 0 &&
...
rtc::ArrayView<uint8_t> RtpPacket::AllocateRawExtension(int id, size_t length) {
  RTC_DCHECK(IsValidHeaderExtension(id, length));
  ...
  const bool two_byte_header_required =
      !IsValidOneByteHeaderExtension(id, length);
...
absl::optional<rtc::ArrayView<const uint8_t>> RtpPacket::FindExtension(
  ...
  return absl::make_optional(rtc::MakeArrayView(data() + extension_info->offset,
...
rtc::ArrayView<uint8_t> RtpPacket::AllocateExtension(ExtensionType type,
  ...
  if (!IsValidHeaderExtension(id, length)) {
    return nullptr;
  }
...
bool RtpPacket::HasExtension(ExtensionType type) const {
  return FindExtension(type) != absl::nullopt;
}
...
bool RtpPacket::IsValidHeaderExtension(int id, size_t length) const {
