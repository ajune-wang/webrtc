...
PeerConnection::PeerConnection(rtc::scoped_refptr<ConnectionContext> context,
    ...
    : context_(context),
      ...
      rtp_manager_([this]() { return IsUnifiedPlan(); },
                   signaling_thread(),
                   worker_thread(),
                   channel_manager(),
                   &usage_pattern_,
...
PeerConnection::~PeerConnection() {
  ...
  // stopping.
  for (const auto& transceiver : rtp_manager_.transceivers()->List()) {
...
RTCErrorOr<rtc::scoped_refptr<RtpSenderInterface>> PeerConnection::AddTrack(
  ...
  if (rtp_manager_.FindSenderForTrack(track)) {
    LOG_AND_RETURN_ERROR(
        RTCErrorType::INVALID_PARAMETER,
        "Sender already exists for track " + track->id() + ".");
  }
  auto sender_or_error =
      (IsUnifiedPlan() ? rtp_manager_.AddTrackUnifiedPlan(track, stream_ids)
                       : rtp_manager_.AddTrackPlanB(track, stream_ids));
...
RTCError PeerConnection::RemoveTrackNew(
  ...
  if (IsUnifiedPlan()) {
    ...
    if (sender->media_type() == cricket::MEDIA_TYPE_AUDIO) {
      removed =
          rtp_manager_.GetAudioTransceiver()->internal()->RemoveSender(sender);
    } else {
      RTC_DCHECK_EQ(cricket::MEDIA_TYPE_VIDEO, sender->media_type());
      removed =
          rtp_manager_.GetVideoTransceiver()->internal()->RemoveSender(sender);
    }
    if (!removed) {
      LOG_AND_RETURN_ERROR(
          RTCErrorType::INVALID_PARAMETER,
...
PeerConnection::FindTransceiverBySender(
    rtc::scoped_refptr<RtpSenderInterface> sender) {
  return rtp_manager_.transceivers()->FindBySender(sender);
}
...
PeerConnection::AddTransceiver(
  ...
  std::string sender_id = (track && !rtp_manager_.FindSenderById(track->id())
                               ? track->id()
                               : rtc::CreateRandomUuid());
  auto sender = rtp_manager_.CreateSender(
      media_type, sender_id, track, init.stream_ids, parameters.encodings);
  auto receiver =
      rtp_manager_.CreateReceiver(media_type, rtc::CreateRandomUuid());
  auto transceiver = rtp_manager_.CreateAndAddTransceiver(sender, receiver);
...
rtc::scoped_refptr<RtpSenderInterface> PeerConnection::CreateSender(
  ...
  if (kind == MediaStreamTrackInterface::kAudioKind) {
    ...
    audio_sender->SetMediaChannel(rtp_manager_.voice_media_channel());
    new_sender = RtpSenderProxyWithInternal<RtpSenderInternal>::Create(
        signaling_thread(), audio_sender);
    rtp_manager_.GetAudioTransceiver()->internal()->AddSender(new_sender);
  } else if (kind == MediaStreamTrackInterface::kVideoKind) {
    auto video_sender = VideoRtpSender::Create(
        worker_thread(), rtc::CreateRandomUuid(), rtp_manager());
    video_sender->SetMediaChannel(rtp_manager_.video_media_channel());
    new_sender = RtpSenderProxyWithInternal<RtpSenderInternal>::Create(
        signaling_thread(), video_sender);
    rtp_manager_.GetVideoTransceiver()->internal()->AddSender(new_sender);
  } else {
...
std::vector<rtc::scoped_refptr<RtpSenderInterface>> PeerConnection::GetSenders()
  ...
  for (const auto& sender : rtp_manager_.GetSendersInternal()) {
...
PeerConnection::GetReceivers() const {
  ...
  for (const auto& receiver : rtp_manager_.GetReceiversInternal()) {
...
void PeerConnection::GetStats(
  ...
  if (selector) {
    for (const auto& proxy_transceiver : rtp_manager_.transceivers()->List()) {
...
PeerConnection::GetRemoteAudioSSLCertChain() {
  RTC_DCHECK_RUN_ON(signaling_thread());
  auto audio_transceiver = rtp_manager_.GetFirstAudioTransceiver();
...
void PeerConnection::OnAudioTrackAdded(AudioTrackInterface* track,
  ...
  rtp_manager_.AddAudioTrack(track, stream);
  sdp_handler_.UpdateNegotiationNeeded();
}
...
void PeerConnection::OnAudioTrackRemoved(AudioTrackInterface* track,
  ...
  rtp_manager_.RemoveAudioTrack(track, stream);
  sdp_handler_.UpdateNegotiationNeeded();
}
...
void PeerConnection::OnVideoTrackAdded(VideoTrackInterface* track,
  ...
  rtp_manager_.AddVideoTrack(track, stream);
  sdp_handler_.UpdateNegotiationNeeded();
}
...
void PeerConnection::OnVideoTrackRemoved(VideoTrackInterface* track,
  ...
  rtp_manager_.RemoveVideoTrack(track, stream);
  sdp_handler_.UpdateNegotiationNeeded();
}
