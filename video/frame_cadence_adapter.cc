...
class ZeroHertzAdapterMode : public AdapterMode {
 public:
  ZeroHertzAdapterMode(TaskQueueBase* queue,
                       ...
                       FrameCadenceAdapterInterface::Callback* callback,
                       double max_fps,
                       const std::atomic<int>& frames_scheduled_for_processing,
 ...
 private:
  ...
  bool zero_hertz_queue_overload_;
...
class FrameCadenceAdapterImpl : public FrameCadenceAdapterInterface {
 ...
 private:
  ...
  const bool frame_cadence_adapter_zero_hertz_queue_overload_;
...
ZeroHertzAdapterMode::ZeroHertzAdapterMode(
    ...
    : queue_(queue),
      ...
      zero_hertz_queue_overload_(zero_hertz_queue_overload) {
  sequence_checker_.Detach();
...
FrameCadenceAdapterImpl::FrameCadenceAdapterImpl(
    ...
    : clock_(clock),
      ...
      frame_cadence_adapter_zero_hertz_queue_overload_(
          !field_trials.IsDisabled("WebRTC-ZeroHertzQueueOverload")) {
  RTC_DLOG(LS_INFO) << "FCA0HzQO: "
                    << frame_cadence_adapter_zero_hertz_queue_overload_;
}
