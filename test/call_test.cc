...
void CallTest::CreateAudioAndFecSendConfigs(size_t num_audio_streams,
  ...
  if (num_audio_streams > 0) {
    CreateAudioConfig(send_transport, {{"stereo", "1"}});
  }
...
void CallTest::CreateAudioConfig(Transport* send_transport,
  ...
  audio_send_config_.send_codec_spec = AudioSendStream::Config::SendCodecSpec(
      kAudioSendPayloadType, {"opus", 48000, 2, params});
...
void CallTest::CreateMatchingAudioAndFecConfigs(
  ...
  if (num_audio_streams_ == 1) {
    CreateMatchingAudioConfigs(rtcp_send_transport, false, nullptr);
  }
...
void CallTest::CreateMatchingAudioConfigs(Transport* transport,
  ...
  audio_config.rtp.transport_cc = send_side_bwe;
