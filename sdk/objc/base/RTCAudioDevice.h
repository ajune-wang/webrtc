...
typedef OSStatus (^RTC_OBJC_TYPE(RTCAudioDeviceRenderRecordedDataBlock))(
    AudioUnitRenderActionFlags *_Nonnull actionFlags,
    const AudioTimeStamp *_Nonull timestamp,
    NSInteger inputBusNumber,
    UInt32 frameCount,
    AudioBufferList *_Nonnull inputData);
 ...
 * samle rate and/or IO buffer duration had possibly changed.
 *
...
/**
 * Notifies native ADM that audio input is interrupted and futher audio playout
...
/**
 * Notifies native ADM that audio output is interrupted and futher audio playout
 * and recording might happen on a different thread.
 *
 * NOTE: Must be called within block executed via `dispatchAsync` or `dispatchSync`.
 */
...
/**
 * Syncronyously execute block of code within the context of
