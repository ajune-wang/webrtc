...
void PacedSenderTaskQueue::MaybeProcessPackets(
  ...
  const Timestamp next_probe_time = pacing_controller_.NextProbeTime();
  Timestamp next_process_time = next_probe_time;
  if (next_process_time == Timestamp::MinusInfinity()) {
    // Minus infinity means "as soon as possible", schedule it for right now.
    next_process_time = now;
  } else if (next_probe_time == Timestamp::PlusInfinity()) {
    // Plus infinity indicates no probe scheduled, set it to whatever the pacing
    // controller wants for normal execution.
    next_process_time = pacing_controller_.NextSendTime();
  }
  RTC_DCHECK(next_process_time.IsFinite());

  const bool probing = next_probe_time != Timestamp::PlusInfinity();
  if (!is_probing_ && probing) {
    // Remove any other pending tasks while probing.
    scheduled_process_times_ = decltype(scheduled_process_times_)();
  }
  is_probing_ = probing;

  if (probing ||  // If probing, schedule task regardless.
    ...
    task_queue_.PostDelayedTask(
        [this, probing, next_process_time]() {
          RTC_DCHECK_RUN_ON(&task_queue_);
          MaybeProcessPackets(probing, next_process_time);
        },
        delay_ms);
  }
