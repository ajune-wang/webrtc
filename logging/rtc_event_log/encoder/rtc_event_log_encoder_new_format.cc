...
std::string RtcEventLogEncoderNewFormat::EncodeBatch(
...

  {
    ...
    std::vector<const RtcEventDtlsWritable*> dtls_writables;
    ...
    for (auto it = begin; it != end; ++it) {
      switch ((*it)->GetType()) {
        ...
        case RtcEvent::Type::DtlsWritable: {
          auto* rtc_event =
              static_cast<const RtcEventDtlsWritable* const>(it->get());
          dtls_writables.push_back(rtc_event);
          break;
        }
    ...
    EncodeDtlsWritable(dtls_writables, &event_stream);
...
void RtcEventLogEncoderNewFormat::EncodeDtlsWritable(
    rtc::ArrayView<const RtcEventDtlsWritable*> batch,
    rtclog2::EventStream* event_stream) {
  for (const RtcEventDtlsWritable* base_event : batch) {
    rtclog2::DtlsWritable* proto_batch = event_stream->add_dtls_writable();
    proto_batch->set_timestamp_ms(base_event->timestamp_ms());
