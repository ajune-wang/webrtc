...
TEST(VideoRtpDepacketizerVp8Test, BasicHeader) {
  ...
  size_t offset =
      VideoRtpDepacketizerVp8::ParseRtpPayload(packet, &video_header);
...
TEST(VideoRtpDepacketizerVp8Test, TooShortHeader) {
  ...
  EXPECT_EQ(VideoRtpDepacketizerVp8::ParseRtpPayload(packet, &unused), 0u);
}
...
TEST(VideoRtpDepacketizerVp8Test, FailsOnEmptyPayload) {
  ...
  RTPVideoHeader video_header;
  EXPECT_EQ(VideoRtpDepacketizerVp8::ParseRtpPayload(empty, &video_header), 0u);
}
