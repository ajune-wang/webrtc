...
void ParseY4mHeader(std::string filepath,
  ...
  RTC_CHECK(sscanf(h, "YUV4MPEG2 W%d H%d", width, height) == 2)
      << filepath << " is not a valid Y4M file";

  RTC_CHECK_GT(*width, 0) << "Width must be positive";
  RTC_CHECK_GT(*height, 0) << "Height must be positive";
...
Y4mFrameReaderImpl::Y4mFrameReaderImpl(std::string filepath,
    ...
    : YuvFrameReaderImpl(filepath, /*width=*/0, /*height=*/0, repeat_mode) {}

void Y4mFrameReaderImpl::Init() {
  file_ = fopen(filepath_.c_str(), "rb");
  RTC_CHECK(file_ != nullptr) << "Cannot open " << filepath_;

  ParseY4mHeader(filepath_, &width_, &height_, &header_size_bytes_);
