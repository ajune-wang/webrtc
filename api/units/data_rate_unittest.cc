...
TEST(DataRateTest, CompilesWithChecksAndLogs) {
  DataRate a = DataRate::KilobitsPerSecond(300);
  DataRate b = DataRate::KilobitsPerSecond(210);
  RTC_CHECK_GT(a, b);
  RTC_LOG(LS_INFO) << a;
}
...
TEST(DataRateTest, ConstExpr) {
  ...
  static_assert(kDataRateInf.BitsPerSecondOr(-1) == -1, "");
  static_assert(kDataRateInf > kDataRateZero, "");

  constexpr DataRate kDataRateBps = DataRate::BitsPerSecond(kValue);
  constexpr DataRate kDataRateKbps = DataRate::KilobitsPerSecond(kValue);
  static_assert(kDataRateBps.BitsPerSecond<double>() == kValue, "");
  static_assert(kDataRateBps.BitsPerSecondOr(0) == kValue, "");
  static_assert(kDataRateKbps.KilobitsPerSecondOr(0) == kValue, "");
}

TEST(DataRateTest, GetBackSameValues) {
  const int64_t kValue = 123 * 8;
  EXPECT_EQ(DataRate::BitsPerSecond(kValue).BitsPerSecond(), kValue);
  EXPECT_EQ(DataRate::KilobitsPerSecond(kValue).KilobitsPerSecond(), kValue);
}

TEST(DataRateTest, GetDifferentPrefix) {
  const int64_t kValue = 123 * 8000;
  EXPECT_EQ(DataRate::BitsPerSecond(kValue).KilobitsPerSecond(), kValue / 1000);
}

TEST(DataRateTest, IdentityChecks) {
  const int64_t kValue = 3000;
  EXPECT_TRUE(DataRate::Zero().IsZero());
  EXPECT_FALSE(DataRate::BitsPerSecond(kValue).IsZero());

  EXPECT_TRUE(DataRate::Infinity().IsInfinite());
  EXPECT_FALSE(DataRate::Zero().IsInfinite());
  EXPECT_FALSE(DataRate::BitsPerSecond(kValue).IsInfinite());

  EXPECT_FALSE(DataRate::Infinity().IsFinite());
  EXPECT_TRUE(DataRate::BitsPerSecond(kValue).IsFinite());
...
TEST(DataRateTest, ComparisonOperators) {
  ...
  const DataRate small = DataRate::BitsPerSecond(kSmall);
  const DataRate large = DataRate::BitsPerSecond(kLarge);

  EXPECT_EQ(DataRate::Zero(), DataRate::BitsPerSecond(0));
...
TEST(DataRateTest, ConvertsToAndFromDouble) {
  ...
  EXPECT_EQ(DataRate::BitsPerSecond(kValue).BitsPerSecond<double>(),
            kDoubleValue);
  EXPECT_EQ(DataRate::BitsPerSecond(kValue).KilobitsPerSecond<double>(),
            kDoubleKbps);
  EXPECT_EQ(DataRate::BitsPerSecond(kValue).KilobitsPerSecond<float>(),
            kFloatKbps);
  EXPECT_EQ(DataRate::BitsPerSecond(kDoubleValue).BitsPerSecond(), kValue);
  EXPECT_EQ(DataRate::KilobitsPerSecond(kDoubleKbps).BitsPerSecond(), kValue);

  const double kInfinity = std::numeric_limits<double>::infinity();
  EXPECT_EQ(DataRate::Infinity().BitsPerSecond<double>(), kInfinity);
  EXPECT_TRUE(DataRate::BitsPerSecond(kInfinity).IsInfinite());
  EXPECT_TRUE(DataRate::KilobitsPerSecond(kInfinity).IsInfinite());
}
TEST(DataRateTest, Clamping) {
  const DataRate upper = DataRate::KilobitsPerSecond(800);
  const DataRate lower = DataRate::KilobitsPerSecond(100);
  const DataRate under = DataRate::KilobitsPerSecond(100);
  const DataRate inside = DataRate::KilobitsPerSecond(500);
  const DataRate over = DataRate::KilobitsPerSecond(1000);
...
TEST(DataRateTest, MathOperations) {
  ...
  const DataRate rate_a = DataRate::BitsPerSecond(kValueA);
  const DataRate rate_b = DataRate::BitsPerSecond(kValueB);
  const int32_t kInt32Value = 123;
  const double kFloatValue = 123.0;

  EXPECT_EQ((rate_a + rate_b).BitsPerSecond(), kValueA + kValueB);
  EXPECT_EQ((rate_a - rate_b).BitsPerSecond(), kValueA - kValueB);

  EXPECT_EQ((rate_a * kValueB).BitsPerSecond(), kValueA * kValueB);
  EXPECT_EQ((rate_a * kInt32Value).BitsPerSecond(), kValueA * kInt32Value);
  EXPECT_EQ((rate_a * kFloatValue).BitsPerSecond(), kValueA * kFloatValue);

  EXPECT_EQ(rate_a / rate_b, static_cast<double>(kValueA) / kValueB);

  EXPECT_EQ((rate_a / 10).BitsPerSecond(), kValueA / 10);
  EXPECT_NEAR((rate_a / 0.5).BitsPerSecond(), kValueA * 2, 1);

  DataRate mutable_rate = DataRate::BitsPerSecond(kValueA);
  mutable_rate += rate_b;
  EXPECT_EQ(mutable_rate.BitsPerSecond(), kValueA + kValueB);
  mutable_rate -= rate_a;
  EXPECT_EQ(mutable_rate.BitsPerSecond(), kValueB);
}
...
TEST(UnitConversionTest, DataRateAndDataSizeAndTimeDelta) {
  ...
  const DataRate rate_b = DataRate::BitsPerSecond(kBitsPerSecond);
  ...
  EXPECT_EQ((size_c / delta_a).BitsPerSecond(), kBytes * 8 / kSeconds);
...
TEST(UnitConversionTest, DataRateAndDataSizeAndFrequency) {
  ...
  const DataRate rate_b = DataRate::BitsPerSecond(kBitsPerSecond);
  const DataSize size_c = DataSize::Bytes(kBytes);
  EXPECT_EQ((freq_a * size_c).BitsPerSecond(), kHertz * kBytes * 8);
  EXPECT_EQ((size_c * freq_a).BitsPerSecond(), kHertz * kBytes * 8);
...
TEST(UnitConversionTest, DivisionFailsOnLargeSize) {
  ...
  const DataRate data_rate = DataRate::KilobitsPerSecond(100);
  const TimeDelta time_delta = TimeDelta::Milliseconds(100);
