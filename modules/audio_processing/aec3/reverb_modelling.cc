...
void ReverbModelling::UpdateReverb(rtc::ArrayView<const float> tail,
                                   float gain_tail,
                                   float reverb_decay) {
  if (reverb_decay > 0) {
...
void ReverbModelling::AddReverb(rtc::ArrayView<const float> tail,
                                float gain_tail,
                                float reverb_decay,
                                std::array<float, kFftLengthBy2Plus1>* out) {
  UpdateReverb(tail, gain_tail, reverb_decay);

  // Add the power of the echo reverb to the residual echo power.
  std::transform(out->begin(), out->end(), reverb_.begin(), out->begin(),
                 std::plus<float>());
}
