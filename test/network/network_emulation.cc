...
EmulatedEndpointImpl::EmulatedEndpointImpl(uint64_t id,
  ...
  stats_.AddLocalAddress(peer_local_addr_);
}
...
void EmulatedEndpointImpl::SendPacket(const rtc::SocketAddress& from,
  ...
  task_queue_->PostTask([this, packet = std::move(packet)]() mutable {
    ...
    stats_.IncrementPacketsSent(1);
    stats_.IncrementBytesSent(DataSize::Bytes(packet.ip_packet_size()));
...
void EmulatedEndpointImpl::OnPacketReceived(EmulatedIpPacket packet) {
  ...
  if (it == port_to_receiver_.end()) {
    ...
    std::map<rtc::IPAddress, EmulatedNetworkIncomingStats>&
        incoming_stats_per_source = stats_.MutableIncomingStatsPerSource();
    incoming_stats_per_source[packet.from.ipaddr()].packets_dropped++;
    incoming_stats_per_source[packet.from.ipaddr()].bytes_dropped +=
...
void EmulatedEndpointImpl::UpdateReceiveStats(const EmulatedIpPacket& packet) {
  ...
  std::map<rtc::IPAddress, EmulatedNetworkIncomingStats>&
      incoming_stats_per_source = stats_.MutableIncomingStatsPerSource();
  if (incoming_stats_per_source[packet.from.ipaddr()]
          .first_packet_received_time.IsInfinite()) {
    incoming_stats_per_source[packet.from.ipaddr()].first_packet_received_time =
        current_time;
    incoming_stats_per_source[packet.from.ipaddr()].first_received_packet_size =
        DataSize::Bytes(packet.ip_packet_size());
  }
  incoming_stats_per_source[packet.from.ipaddr()].last_packet_received_time =
      current_time;
  incoming_stats_per_source[packet.from.ipaddr()].packets_received++;
  incoming_stats_per_source[packet.from.ipaddr()].bytes_received +=
      DataSize::Bytes(packet.ip_packet_size());
}
...
std::unique_ptr<EmulatedNetworkStats> EndpointsContainer::GetStats() const {
  ...
  for (auto* endpoint : endpoints_) {
    ...
    stats->IncrementPacketsSent(endpoint_stats->PacketsSent());
    stats->IncrementBytesSent(endpoint_stats->BytesSent());
    ...
    for (const rtc::IPAddress& addr : endpoint_stats->LocalAddresses()) {
      stats->AddLocalAddress(addr);
    }

    const std::map<rtc::IPAddress, EmulatedNetworkIncomingStats>
        incoming_stats_per_source = endpoint_stats->IncomingStatsPerSource();
    for (const auto& entry : incoming_stats_per_source) {
      const EmulatedNetworkIncomingStats& source = entry.second;
      EmulatedNetworkIncomingStats& in_stats =
          stats->MutableIncomingStatsPerSource()[entry.first];
