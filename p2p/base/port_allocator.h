...
struct RelayCredentials {
  RelayCredentials() {}
  RelayCredentials(const absl::string_view username,
...
struct RTC_EXPORT RelayServerConfig {
  ...
  RelayServerConfig(const rtc::SocketAddress& address,
                    ...
                    const absl::string_view password,
                    ProtocolType proto);
  RelayServerConfig(const absl::string_view address,
                    int port,
                    const absl::string_view username,
                    const absl::string_view password,
                    ProtocolType proto);
...
class RTC_EXPORT PortAllocatorSession : public sigslot::has_slots<> {
 public:
  ...
  PortAllocatorSession(const absl::string_view content_name,
                       int component,
 ...
 private:
  void SetIceParameters(const absl::string_view content_name,
                        int component,
                        const absl::string_view ice_ufrag,
                        const absl::string_view ice_pwd) {
...
class RTC_EXPORT PortAllocator : public sigslot::has_slots<> {
 public:
  ...
  void set_proxy(const absl::string_view agent, const rtc::ProxyInfo& proxy) {
    CheckRunOnValidThreadIfInitialized();
    agent_ = agent;
    proxy_ = proxy;
  }
 ...
 protected:
  virtual PortAllocatorSession* CreateSessionInternal(
      ...
      int component,
      const absl::string_view ice_ufrag,
      const absl::string_view ice_pwd) = 0;
