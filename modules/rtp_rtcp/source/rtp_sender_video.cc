...
bool IsNonBaseLayer(const RTPVideoHeader* video_header) {
  bool non_base_layer = false;
  switch (video_header->codec) {
    case kVideoCodecVP8: {
      const auto& vp8 =
          absl::get<RTPVideoHeaderVP8>(video_header->video_type_header);
      non_base_layer =
          vp8.temporalIdx != 0 && vp8.temporalIdx != kNoTemporalIdx;
    } break;
    case kVideoCodecVP9: {
      const auto& vp9 =
          absl::get<RTPVideoHeaderVP9>(video_header->video_type_header);
      non_base_layer =
          vp9.temporal_idx != 0 && vp9.temporal_idx != kNoTemporalIdx;
    } break;
...
bool RTPSenderVideo::SendVideo(enum VideoCodecType video_type,
  ...
  bool set_color_space = false;
  {
    ...
    // sending color space information until the first base layer frame.
    if (video_header->color_space != last_color_space_) {
      last_color_space_ = video_header->color_space;
      set_color_space = true;
      transmit_color_space_next_frame_ = IsNonBaseLayer(video_header);
    } else {
      set_color_space =
