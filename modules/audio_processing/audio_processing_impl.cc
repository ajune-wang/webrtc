...
bool AudioProcessingImpl::CreateAndAttachAecDump(webrtc::FileWrapper file,
                                                 int64_t max_log_size_bytes,
                                                 rtc::TaskQueue* worker_queue) {
  std::unique_ptr<AecDump> aec_dump =
      AecDumpFactory::Create(std::move(file), max_log_size_bytes, worker_queue);
  if (!aec_dump) {
    return false;
  }
...
bool AudioProcessingImpl::CreateAndAttachAecDump(std::string file_name,
                                                 int64_t max_log_size_bytes,
                                                 rtc::TaskQueue* worker_queue) {
  std::unique_ptr<AecDump> aec_dump =
