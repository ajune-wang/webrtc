...
// Class for finding the delay based on the properties of a linear adaptive
// filter.
class FilterBasedDelayEstimator {
 public:
  FilterBasedDelayEstimator(const EchoCanceller3Config& config,
  ...
  // Returns any identified delay.
  rtc::Optional<int> DelayBlocks() { return delay_blocks_; }

  // Updates the estimates with the latest input data.
  ...
  void DumpFilter() {
    filter_.DumpFilter("aec3_filter_based_delay_estimator_H",
                       "aec3_filter_based_delay_estimator_h");
  }
