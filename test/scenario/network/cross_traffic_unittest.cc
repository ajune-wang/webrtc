...
TEST(CrossTrafficTest, TriggerPacketBurst) {
  ...
  CrossTrafficRoute* traffic =
      network_manager.CreateCrossTraffic({node_a, node_b});

  std::unique_ptr<CountingReceiver> receiver =
      absl::make_unique<CountingReceiver>();
  CountingReceiver* counter = receiver.get();
  Destination destination = traffic->RegisterDestination(std::move(receiver));

  traffic->TriggerPacketBurst(100, 1000, destination);
...
TEST(CrossTrafficTest, PulsedPeaksSendStrategy) {
  ...
  CountingReceiver* counter = receiver.get();
  Destination destination = traffic->RegisterDestination(std::move(receiver));
  network_manager.CreatePulsedPeaksCrossTraffic(traffic, config, destination);
