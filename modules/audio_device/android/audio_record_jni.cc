...
class ScopedHistogramTimer {
 public:
  ...
  ~ScopedHistogramTimer() {
    ...
    NLOG(INFO, histogram_name_, ": ", life_time_ms);
  }
...
int32_t AudioRecordJni::InitRecording() {
  ...
  if (frames_per_buffer < 0) {
    direct_buffer_address_ = nullptr;
    NLOG(LS_ERROR, "InitRecording failed");
    return -1;
  }
...
int32_t AudioRecordJni::StartRecording() {
  ...
  if (!j_audio_record_->StartRecording()) {
    NLOG(LS_ERROR, "StartRecording failed");
    return -1;
  }
  recording_ = true;
  return 0;
...
int32_t AudioRecordJni::StopRecording() {
  NLOG(INFO, "StopRecording");
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
  if (!initialized_ || !recording_) {
    return 0;
  }
  if (!j_audio_record_->StopRecording()) {
    NLOG(LS_ERROR, "StopRecording failed");
    return -1;
  }
...
void AudioRecordJni::AttachAudioBuffer(AudioDeviceBuffer* audioBuffer) {
  ...
  NLOG(INFO, "SetRecordingSampleRate(", sample_rate_hz, ")");
  ...
  NLOG(INFO, "total_delay_in_milliseconds: ", total_delay_in_milliseconds_);
}

int32_t AudioRecordJni::EnableBuiltInAEC(bool enable) {
  NLOG(INFO, "EnableBuiltInAEC(", enable, ")");
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
...
int32_t AudioRecordJni::EnableBuiltInNS(bool enable) {
  NLOG(INFO, "EnableBuiltInNS(", enable, ")");
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
...
void AudioRecordJni::OnDataIsRecorded(int length) {
  ...
  if (audio_device_buffer_->DeliverRecordedData() == -1) {
    NLOG(INFO, "AudioDeviceBuffer::DeliverRecordedData failed");
  }
}
