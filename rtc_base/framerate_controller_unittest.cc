...
TEST_F(FramerateControllerTest, NoFrameDroppedForLargeTimestampOffset) {
  ...
  EXPECT_FALSE(controller_.DropFrame(kLargeOffsetNs + input_interval_ns));
}
...
TEST_F(FramerateControllerTest, NoFrameDroppedIfInputWithJitterRequested) {
  ...
  int64_t input_interval_ns = rtc::kNumNanosecsPerSec / kInputFps;
  EXPECT_FALSE(controller_.DropFrame(input_interval_ns * 0 / 10));
  EXPECT_FALSE(controller_.DropFrame(input_interval_ns * 10 / 10 - 1));
  EXPECT_FALSE(controller_.DropFrame(input_interval_ns * 25 / 10));
  EXPECT_FALSE(controller_.DropFrame(input_interval_ns * 30 / 10));
  EXPECT_FALSE(controller_.DropFrame(input_interval_ns * 35 / 10));
  EXPECT_FALSE(controller_.DropFrame(input_interval_ns * 50 / 10));
}
