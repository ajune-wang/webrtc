...
class RTC_EXPORT XdgDesktopPortalBase : public rtc::RefCountedBase {
 public:
  ...
  // web_id, this web_id is later used to associate it with ConnectionData
  // structure, where we keep all the necessary information about the DBus
  // communication.
  //
  // This starts a series of DBus calls:
  //   1) SessionRequest - requests a session, which will be associated with
  //   this screen sharing request,
  //                       passing a handle for identification so we can watch
  //                       for success/failure
  //   2) SourceRequest - requests what content we want to share on given
  //   session (either monitor, screen or both) 3) StartRequest - requests to
  ...
  void InitPortal(rtc::Callback1<void, bool> callback,
                  int32_t web_id,
                  CaptureSourceType type = CaptureSourceType::kAny);

  // This gets called when we have PipeWire capturer initialized with the stream
  // information obtained from StartRequest
  ...
  ConnectionData* GetConnectionData(int32_t id) const;
