...
// Keeps track of queued delayed tasks and kills any outstanding tasks when
// destroyed
class PendingTaskHandler {
 ...
 public:
  ...
  ~PendingTaskHandler();
  void PostDelayedTask(closure_t&& closure,
                       closure_t&& destroy,
                       units::TimeDelta delay);

 private:
  void OnTaskDone(PendingTask* finished_task);
  rtc::TaskQueue* target_queue_;
  rtc::CriticalSection pending_lock_;
  bool stopped_ = false;
  std::set<PendingTask*> pending_tasks_;
...
class TaskQueueReceiver : public HandlingReceiver<MSG_T> {
 public:
  ...
  explicit TaskQueueReceiver(rtc::TaskQueue* target_queue)
      : target_queue_(target_queue), delay_tasks_(target_queue) {
    task_count_.store(0);
  }
  void SetHandler(handler_t handler) override {
    RTC_DCHECK(!handler_);
 ...
 protected:
  void OnMessage(MSG_T msg) override {
    ...
    if (task_count_++ != kReceiverStopped) {
      target_queue_->PostTask(rtc::NewClosure(std::bind(handler_, msg),
                                              [this]() { task_count_--; }));
    }
  }
  void DelayMessage(MSG_T msg, units::TimeDelta delay) override {
    ...
    RTC_DCHECK(target_queue_);
    if (task_count_++ != kReceiverStopped) {
      delay_tasks_.PostDelayedTask(std::bind(handler_, msg),
                                   [this]() { task_count_--; }, delay);
    }
  }
 ...
 private:
  ...
  handler_t handler_;
  rtc::TaskQueue* target_queue_;
  PendingTaskHandler delay_tasks_;
