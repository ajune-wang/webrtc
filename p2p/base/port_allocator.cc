...
Candidate PortAllocator::SanitizeCandidate(const Candidate& c,
                                           bool is_local) const {
  CheckRunOnValidThreadAndInitialized();
  // For a local host candidate, we need to conceal its IP address candidate if
  // the mDNS obfuscation is enabled.
  bool use_hostname_address =
      is_local && c.type() == LOCAL_PORT_TYPE && MdnsObfuscationEnabled();
  // If the remote endpoint signaled us a hostname host candidate, we assume it
  // is supposed to be sanitized.
  use_hostname_address |= !is_local && c.type() == LOCAL_PORT_TYPE &&
                          !c.address().hostname().empty();
  // Remove the address for remote prflx candidates. See
  // https://w3c.github.io/webrtc-stats/#dom-rtcicecandidatestats.
  use_hostname_address |= !is_local && c.type() == PRFLX_PORT_TYPE;
