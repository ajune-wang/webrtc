...
// delay. Note delay in `TaskQueue::PostDelayedTask` is time from now but not
// from previously posted task.
class Pacer {
 public:
...
class TesterDecoder {
 public:
  TesterDecoder(std::unique_ptr<Decoder> decoder,
    ...
    RTC_CHECK(analyzer_ != nullptr) << "Analyzer must be provided";
  }
