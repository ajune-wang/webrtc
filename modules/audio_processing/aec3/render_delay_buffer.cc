...
RenderDelayBufferImpl::PrepareCaptureProcessing() {
  ...
  if (DetectExcessRenderBlocks()) {
    ...
    RTC_DLOG(LS_INFO) << "Excess render blocks detected at block "
                     << capture_call_counter_;
    Reset();
    ...
    // render underrun.
    RTC_DLOG(LS_INFO) << "Render buffer underrun detected at block "
                     << capture_call_counter_;
...
bool RenderDelayBufferImpl::AlignFromDelay(size_t delay) {
  ...
  if (!external_audio_buffer_delay_verified_after_reset_ &&
    ...
    RTC_DLOG(LS_INFO) << "Mismatch between first estimated delay after reset "
...
void RenderDelayBufferImpl::ApplyTotalDelay(int delay) {
  RTC_DLOG(LS_INFO) << "Applying total delay of " << delay << " blocks.";
