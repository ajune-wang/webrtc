...
// AudioChannel represents single media session and provides APIs over
// AudioIngress and AudioEgress. Note that a single RTP stack is shared with
// these two classes as it has both sending and receiving capabilities. Some
// logic that requires some orchestration between the two functions such as
// recv-only use case as RTCP is still expected to be exchanged.
class AudioChannel : public rtc::RefCountInterface {
 public:
  ...
  void ReceivedRTPPacket(rtc::ArrayView<const uint8_t> rtp) {
    ingress_->ReceivedRTPPacket(rtp);
  }
  void ReceivedRTCPPacket(rtc::ArrayView<const uint8_t> rtcp) {
    ingress_->ReceivedRTCPPacket(rtcp);
  }
 ...
 private:
  ...
  // Synchronization is handled internally by AudioIngress.
  std::unique_ptr<AudioIngress> ingress_;

  // Synchronization is handled internally by AudioEgress.
