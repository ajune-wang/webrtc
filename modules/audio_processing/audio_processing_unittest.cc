...
void ApmTest::ProcessDelayVerificationTest(int delay_ms,
  ...
  EXPECT_EQ(kNoErr, apm_->Initialize());
  ...
  for (int frame_count = 0; frame_count < 450; ++frame_count) {
    ...
    EXPECT_EQ(kNoErr, apm_->ProcessReverseStream(
                          ...
                          StreamConfig(reverse_frame->sample_rate_hz,
                                       reverse_frame->num_channels()),
                          reverse_frame->data.data()));
    EXPECT_EQ(kNoErr, apm_->set_stream_delay_ms(system_delay_ms));
    EXPECT_EQ(kNoErr,
...
void ApmTest::StreamParametersTest(Format format) {
  ...
  EXPECT_EQ(kNoErr, ProcessStreamChooser(format));
  ...
  apm_config.echo_canceller.mobile_mode = false;
  apm_->ApplyConfig(apm_config);
  EXPECT_EQ(kNoErr, apm_->set_stream_delay_ms(100));
  EXPECT_EQ(apm_->kStreamParameterNotSetError, ProcessStreamChooser(format));
  ...
  EXPECT_EQ(kNoErr, AnalyzeReverseStreamChooser(format));
  EXPECT_EQ(kNoErr, ProcessStreamChooser(format));
...
TEST_F(ApmTest, PreAmplifier) {
  ...
  for (int i = 0; i < 20; ++i) {
    ...
    EXPECT_EQ(kNoErr, ProcessStreamChooser(kIntFormat));
  }
...
TEST_F(ApmTest, GainControl) {
  ...
  for (auto mode :
    ...
    EXPECT_EQ(kNoErr, ProcessStreamChooser(kFloatFormat));
  }
...
TEST_F(ApmTest, SplittingFilter) {
  ...
  frame_.FillData(1000);
  frame_copy.CopyFrom(frame_);
  EXPECT_EQ(kNoErr, apm_->set_stream_delay_ms(0));
  EXPECT_EQ(kNoErr,
            apm_->ProcessStream(
                frame_.data.data(),
                StreamConfig(frame_.sample_rate_hz, frame_.num_channels()),
