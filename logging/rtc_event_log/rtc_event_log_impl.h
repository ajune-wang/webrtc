...
class RtcEventLogImpl final : public RtcEventLog {
 ...
 public:
  ...
  // would be a bit more expensive than execution on current thread with
  // locking. It is because the TaskQueue switches the thread context for task
  // execution.
 ...
 private:
  ...
  void LogEventsToOutput(EventHistories events) RTC_RUN_ON(task_queue_);
  ...
  void ScheduleOutput() RTC_EXCLUSIVE_LOCKS_REQUIRED(mutex_);
  ...
  absl::optional<int64_t> output_period_ms_ RTC_GUARDED_BY(mutex_);
  int64_t last_output_ms_ RTC_GUARDED_BY(mutex_);
  bool output_scheduled_ RTC_GUARDED_BY(mutex_) = false;
