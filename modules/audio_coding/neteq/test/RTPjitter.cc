...
int main(int argc, char* argv[]) {
  ...
  if (argc != 4) {
    ...
    printf(
        "The RTP stream is modified to have the same jitter as described in "
        "the timing files.\n");
    ...
    printf(
        "Statistics -> RTP -> Show All Streams -> [select a stream] -> "
        "Save As\n\n");
    printf("Usage:\n\n");
  ...
  time_vec = (arr_time*)malloc(
      sizeof(arr_time) * (filelen(dat_file) / sizeof(float)) + 1000);
  if (time_vec == NULL) {
  ...
  EXPECT_EQ(kRtpDumpHeaderSize,
            fread(firstline, 1, kRtpDumpHeaderSize, in_file));
  EXPECT_EQ(kRtpDumpHeaderSize,
            fwrite(firstline, 1, kRtpDumpHeaderSize, out_file));
  ...
  while (len == 2) {
    ...
    rtp_len += 2;
    if (fread(&rtp_vec[rtp_len], sizeof(unsigned char), len - 2, in_file) !=
        (unsigned)(len - 2)) {
    ...
    if (time_vec[k].time < FLT_MAX && time_vec[k].ix < Npack) {
      ...
      if (time_vec[k].time >= 0) {
        ...
        *offset = htonl((uint32_t)0);
        fprintf(stderr,
                "Warning: negative receive time in dat file transformed"
                " to 0.\n");
      }
      ...
      if (fwrite(temp_packet, sizeof(unsigned char),
                 ...
                 out_file) != ntohs(*((uint16_t*)temp_packet))) {
        return -1;
      }
    }
  }
