...
class API_AVAILABLE(macos(13.0)) SckScreenCapturer final : public DesktopCapturer {
 public:
 ...
 private:
  ...
  // rectangles.
  // TODO: lambroslambrou - Replace this flag with ScreenCapturerHelper to more accurately track the
  // dirty rectangles from the SCStreamFrameInfoDirtyRects attachment.
...
SckScreenCapturer::SckScreenCapturer(const DesktopCaptureOptions& options)
    : desktop_config_monitor_(options.configuration_monitor()) {
...
void SckScreenCapturer::OnShareableContentCreated(SCShareableContent* content) {
  ...
  // The stream will be started or reconfigured, so ensure that the first new frame is marked as
  // dirty.
  {
    MutexLock lock(&latest_frame_lock_);
...
void SckScreenCapturer::OnNewIOSurface(IOSurfaceRef io_surface, CFDictionaryRef attachment) {
  ...
  if (!dirty_rects_ptr) {
    // This is never expected to happen - SCK attaches a non-empty dirty-rects list to every frame,
  ...
  bool found_non_empty_rect = false;
  for (int i = 0; i < size; i++) {
    const void* rect_ptr = CFArrayGetValueAtIndex(dirty_rects_array, i);
    if (CFGetTypeID(rect_ptr) != CFDictionaryGetTypeID()) {
      // This is never expected to happen - the dirty-rects attachment should always be an array of
      // dictionaries.
      return;
    }
    CGRect rect{};
    ...
    if (!CGRectIsEmpty(rect)) {
      found_non_empty_rect = true;
      break;
    }
  }

  if (!found_non_empty_rect) {
    // Nothing has changed according to the dirty-rects attachment, so there's no value in updating
    // `latest_frame_` here.
    return;
  }
...
void SckScreenCapturer::StartOrReconfigureCapturer() {
  SckHelper* helper = helper_;
  auto handler = ^(SCShareableContent* content, NSError* error) {
    [helper onShareableContentCreated:content];
  };
