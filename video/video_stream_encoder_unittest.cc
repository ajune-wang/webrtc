 ...
 public:
  ...
  class TestEncoder : public test::FakeEncoder {
    ...
    VideoEncoder::EncoderInfo GetEncoderInfo() const override {
      ...
      if (initialized_ == EncoderState::kInitialized) {
        ...
        for (int i = 0; i < kMaxSpatialLayers; ++i) {
          int num_layers = temporal_layers_supported_[i] ? 2 : 1;
   ...
   private:
    ...
    bool temporal_layers_supported_[kMaxSpatialLayers] RTC_GUARDED_BY(
        local_crit_sect_) = {false};
...
TEST_F(VideoStreamEncoderTest, TemporalLayersNotDisabledIfSupported) {
  webrtc::test::ScopedFieldTrials field_trials(
      "WebRTC-Target-Bitrate-Rtcp/Enabled/");
...
TEST_F(VideoStreamEncoderTest, VerifyBitrateAllocationForTwoStreams) {
  ...
  const int kS0Tl0Bps =
      kS0Bps * webrtc::SimulcastRateAllocator::GetTemporalRateAllocation(
                   /*num_temporal_layers*/ 2, 0);
  const int kS0Tl1Bps =
      kS0Bps * webrtc::SimulcastRateAllocator::GetTemporalRateAllocation(
                   /*num_temporal_layers*/ 2, 1);
  const int kS1Bps =
      kTargetBitrateBps -
      kS0Bps * webrtc::SimulcastRateAllocator::GetTemporalRateAllocation(
                   /*num_temporal_layers*/ 2, 1);
