...
TEST(BitrateTrackerTest, ReturnsRateAfterOneMeasurementsOverExactlyFullWindow) {
  const DataSize kPacketSize = DataSize::Bytes(1'000);
  const Timestamp start = Timestamp::Seconds(12'345);
  BitrateTracker stats(kWindow);

  stats.Update(kPacketSize, start);

  // Shouldn't work! (Only one sample, not full window size.)
  EXPECT_EQ(stats.Rate(start + kWindow - 2 * kEpsilon), absl::nullopt);

  // Window size should be full, and the single data point is accepted.
  EXPECT_EQ(stats.Rate(start + kWindow - kEpsilon), kPacketSize / kWindow);

  // The single sample should be outside the window now.
  EXPECT_EQ(stats.Rate(start + kWindow), absl::nullopt);
}
