...
// Method for performing the simulations needed to test the refined filter update
// gain functionality.
void RunFilterUpdateTest(int num_blocks_to_process,
  ...
  AdaptiveFirFilter refined_filter(config.filter.refined.length_blocks,
  ...
  std::vector<std::vector<float>> h(
      kNumCaptureChannels,
      std::vector<float>(
          GetTimeDomainLength(refined_filter.max_filter_size_partitions()), 0.f));
  ...
  RefinedFilterUpdateGain refined_gain(config.filter.refined,
                                 config.filter.config_change_duration_blocks);
