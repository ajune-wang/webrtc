...
QualityRampUpExperimentHelper::Create(
    QualityRampUpExperimentListener* experiment_listener,
    Clock* clock) {
  QualityRampupExperiment experiment = QualityRampupExperiment::ParseSettings();
  if (experiment.Enabled()) {
    return std::make_unique<QualityRampUpExperimentHelper>(experiment_listener,
                                                           clock, experiment);
  }
  return nullptr;
}
...
void QualityRampUpExperimentHelper::PerformQualityRampupExperiment(
  ...
  RTC_LOG(INFO) << "try=" << try_quality_rampup;
  RTC_LOG(INFO) << "qp_res=" << qp_resolution_adaptations_;
  RTC_LOG(INFO) << "cpu_adapt=" << cpu_adapted_;
...
void QualityRampUpExperimentHelper::SetMaxBitrate(int input_size,
