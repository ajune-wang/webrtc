  ...
  class MockVideoMediaChannel : public cricket::FakeVideoMediaChannel {
   public:
    ...
    MOCK_METHOD(void,
                ...
                (uint32_t),
                (override));
    MOCK_METHOD(void, GenerateRecvKeyFrame, (uint32_t), (override));
...
TEST_F(VideoRtpReceiverTest, GeneratesKeyFrame) {
  EXPECT_CALL(channel_, GenerateRecvKeyFrame(0));
...
TEST_F(VideoRtpReceiverTest,
  ...
  EXPECT_CALL(channel_, GenerateRecvKeyFrame).Times(0);
  EXPECT_CALL(channel2, GenerateRecvKeyFrame).Times(0);
  ...
  EXPECT_CALL(channel2, GenerateRecvKeyFrame).Times(1);
  Source()->GenerateKeyFrame();
  MockVideoMediaChannel channel3(nullptr, cricket::VideoOptions());
  EXPECT_CALL(channel3, GenerateRecvKeyFrame);
