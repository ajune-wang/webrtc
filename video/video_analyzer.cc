...
void VideoAnalyzer::PrintResults() {
  ...
  if (total_frames_duration_ms_double > 0) {
    test::PrintResult(
        ...
        "unitless", false, webrtc::test::ImproveDirection::kSmallerIsBetter);
  ...
  if (1000 * sum_squared_frame_durations_ > 0) {
    test::PrintResult(
        ...
        "fps", false, webrtc::test::ImproveDirection::kBiggerIsBetter);
  }
  ...
  if (receive_stream_ != nullptr) {
    PrintResultWithExternalMean(
        "decode_time", mean_decode_time_ms_, decode_time_ms_, "ms",
        webrtc::test::ImproveDirection::kSmallerIsBetter);
  }
