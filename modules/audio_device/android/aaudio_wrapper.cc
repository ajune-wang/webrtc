...
bool AAudioWrapper::Init() {
  ...
  // Delete the resources associated with the stream builder since it is no
  // longer needed.
  DeleteStreamBuilder();
  LogStreamState();
  return true;
...
double AAudioWrapper::EstimateLatencyMillis() const {
  ...
  if (direction() == AAUDIO_DIRECTION_INPUT) {
    ...
    if (result == AAUDIO_OK) {
      ...
      // Results are only valid when the stream is in
...
void AAudioWrapper::SetStreamConfiguration() {
  ...
  // Given that WebRTC applications require low latency, our audio streams uses
