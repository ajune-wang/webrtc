...
TEST(PacketRouterTest, Sanity_NoModuleRegistered_TimeToSendPacket) {
  ...
  EXPECT_FALSE(packet_router
                   .TimeToSendPacket(ssrc, sequence_number, timestamp,
...
TEST(PacketRouterTest, TimeToSendPacket) {
  ...
  EXPECT_TRUE(
      packet_router
          .TimeToSendPacket(kSsrc1, sequence_number, timestamp, retransmission,
  ...
  EXPECT_TRUE(
      packet_router
          .TimeToSendPacket(kSsrc2, sequence_number, timestamp, retransmission,
  ...
  EXPECT_TRUE(
      packet_router
          .TimeToSendPacket(kSsrc1 + kSsrc2, sequence_number, timestamp,
                            retransmission,
                            PacedPacketInfo(1, kProbeMinProbes, kProbeMinBytes))
          .value_or(false));
  ...
  EXPECT_TRUE(
      packet_router
          .TimeToSendPacket(kSsrc1, sequence_number, timestamp, retransmission,
                            PacedPacketInfo(PacedPacketInfo::kNotAProbe,
...
TEST(PacketRouterTest, SenderOnlyFunctionsRespectSendingMedia) {
  ...
  EXPECT_TRUE(
      packet_router
          .TimeToSendPacket(kSsrc, 1, 1, false,
                            PacedPacketInfo(PacedPacketInfo::kNotAProbe,
                                            kProbeMinBytes, kProbeMinBytes))
          .value_or(false));
