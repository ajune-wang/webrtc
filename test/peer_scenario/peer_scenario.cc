...
PeerScenario::PeerScenario()
    : signaling_thread_(rtc::Thread::Current()),
      net_(new NetworkEmulationManagerImpl()) {}

PeerScenarioClient* PeerScenario::CreateClient(
    PeerScenarioClient::Config config) {
  peer_clients_.push_back(
      absl::make_unique<PeerScenarioClient>(net(), thread(), config));
  return peer_clients_.back().get();
}
...
void PeerScenario::SimpleConnection(
    ...
    std::vector<EmulatedNetworkNode*> send_link,
    std::vector<EmulatedNetworkNode*> ret_link) {
  net()->CreateRoute(caller->ep(), send_link, callee->ep());
  net()->CreateRoute(callee->ep(), ret_link, caller->ep());
...
void PeerScenario::AttachVideoQualityAnalyzer(Clock* capture_clock,
                                              VideoQualityAnalyzer* analyzer,
                                              VideoTrackInterface* send_track,
                                              PeerScenarioClient* receiver) {
  video_quality_pairs_.push_back(
      absl::make_unique<PeerVideoQualityPair>(capture_clock, analyzer));
  auto pair = video_quality_pairs_.back().get();
...
bool PeerScenario::Wait(rtc::Event* event, TimeDelta max_duration) {
...
void PeerScenario::Sleep(TimeDelta duration) {
