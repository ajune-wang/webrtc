...
TEST_F(TestSimulcastEncoderAdapterFake, ReportsHardwareAccelerated) {
  ...
  // All encoders are HW accelerated, so simulcast adapter reports true.
  for (MockVideoEncoder* encoder : helper_->factory()->encoders())
    encoder->set_is_hardware_accelerated(true);
  EXPECT_EQ(0, adapter_->InitEncode(&codec_, 1, 1200));
  EXPECT_TRUE(adapter_->GetEncoderInfo().is_hardware_accelerated);

  // One encoder is not HW accelerated, so simulcast adapter reports false.
  helper_->factory()->encoders()[2]->set_is_hardware_accelerated(false);
  EXPECT_EQ(0, adapter_->InitEncode(&codec_, 1, 1200));
