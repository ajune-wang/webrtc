...
bool OutstandingData::IsConsistent() const {
  ...
  for (UnwrappedTSN tsn = first_outstanding_tsn(); tsn < next_tsn_;
       tsn = tsn.next_value()) {
    const Item& item = outstanding_data_[Index(tsn)];
  ...
  for (auto& block : gap_ack_blocks) {
    ...
    for (UnwrappedTSN tsn = prev_block_last_acked.next_value();
      ...
      if (tsn <= max_tsn_to_nack) {
        ack_info.has_packet_loss |= NackItem(
            tsn, outstanding_data_[Index(tsn)], /*retransmit_now=*/false,
...
void OutstandingData::AbandonAllFor(IsUnordered unordered,
  ...
  if (discard_from_send_queue_(unordered, stream_id, message_id)) {
    ...
    Item& item = outstanding_data_[Index(tsn)];
    item.Allocate(std::move(message_end), TimeMs(0), MaxRetransmits::NoLimit(),
  ...
  for (UnwrappedTSN tsn = first_outstanding_tsn(); tsn < next_tsn_;
       tsn = tsn.next_value()) {
    Item& other = outstanding_data_[Index(tsn)];
  ...
  if (current_size == capacity) {
    ...
    for (UnwrappedTSN tsn = first_outstanding_tsn(); tsn < next_tsn_;
      ...
      new_data[tsn.Wrap().value() & (new_capacity - 1)] =
          std::move(outstanding_data_[tsn.Wrap().value() & (capacity - 1)]);
    }
...
bool OutstandingData::ShouldSendForwardTsn() const {
  return !empty() &&
         outstanding_data_[Index(first_outstanding_tsn())].is_abandoned();
}
