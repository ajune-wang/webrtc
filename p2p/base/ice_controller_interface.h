...
class IceControllerInterface {
 public:
  ...
  struct PingResult {
    ...
    // again.
    //
    // Since the IceController determine which connection to ping and
    // only return one connection at a time, the recheck_delay_ms does not have
    // any obvious implication on bitrate for pings. E.g the recheck_delay_ms
    // will be shorter if there any more connections available.
