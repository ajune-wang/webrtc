...
void TaskQueuePacedSender::MaybeProcessPackets(
  ...
  // TODO(jianhui): Move to PacingController.
  const TimeDelta kMaxEarlyProbeProcessing = TimeDelta::Millis(1);
  const TimeDelta early_processing = pacing_controller_.IsProbing()
  ...
  while (next_send_time.IsMinusInfinity() ||
         next_send_time <= now + early_processing) {
