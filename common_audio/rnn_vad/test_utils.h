...
class BinaryFileReader {
 public:
  ...
  void SeekForward(size_t items) {
    is_.seekg(items * sizeof(T), is_.cur);
  }
...
// Helper to extract a specific frame from the test data.
template <size_t S>
std::array<float, S> ReadAudioFrame(size_t frame_index) {
  BinaryFileReader<int16_t, S, float> samples_reader(
      test::ResourcePath("common_audio/rnn_vad/samples", "pcm"));
  samples_reader.SeekForward(frame_index * S);
  std::array<float, S> samples;
