...
void VideoQualityMetricsReporter::OnStatsReports(
  ...
  for (auto& s : outbound_rtp_stats) {
    ...
    sample.retransmitted_bytes_sent +=
        DataSize::Bytes(GetStatOrDefault(s->retransmitted_bytes_sent, 0ul));
    sample.bytes_sent += DataSize::Bytes(GetStatOrDefault(s->bytes_sent, 0ul));
    sample.header_bytes_sent +=
        DataSize::Bytes(GetStatOrDefault(s->header_bytes_sent, 0ul));
  }
