...
def CheckApiDepsFileIsUpToDate(input_api, output_api):
  ...
  for f in input_api.AffectedFiles():
    ...
    if len(path_tokens) > 1:
      if os.path.isdir(os.path.join(input_api.PresubmitLocalPath(),
                                    path_tokens[0])):
        dirs_to_check.append(path_tokens[0])
  ...
  if missing_include_rules:
    example_str = ('include_rules = [\n'
                   '  ...\n')
    for r in missing_include_rules:
      example_str += '  "%s",\n' % str(r)
    example_str += '  ...\n'
    example_str += ']\n'
    results.append(output_api.PresubmitError(
