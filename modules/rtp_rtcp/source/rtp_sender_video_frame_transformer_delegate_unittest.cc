...
class TestRTPVideoFrameSender : public RTPVideoFrameSender {
 public:
  MOCK_METHOD(bool,
              SendVideo,
              (int payload_type,
...
class RtpSenderVideoFrameTransformerDelegateTest : public ::testing::Test {
 protected:
  ...
  TestRTPVideoFrameSender test_sender_;
...
TEST_F(RtpSenderVideoFrameTransformerDelegateTest,
       TransformCalledOnTransformFrame) {
  auto delegate(rtc::make_ref_counted<RTPSenderVideoFrameTransformerDelegate>(
      &test_sender_, frame_transformer_,
      /*ssrc=*/1111, /*csrcs=*/std::vector<uint32_t>(),
      time_controller_.CreateTaskQueueFactory().get()));
...
TEST_F(RtpSenderVideoFrameTransformerDelegateTest,
       SenderSendFrameCalledOnTransformedFrame) {
  rtc::Event event;
  std::vector<uint32_t> csrcs = {234, 345, 456};
  auto delegate(rtc::make_ref_counted<RTPSenderVideoFrameTransformerDelegate>(
      &test_sender_, frame_transformer_,
      /*ssrc=*/1111, csrcs, time_controller_.CreateTaskQueueFactory().get()));
  ...
  EXPECT_CALL(test_sender_, SendVideo)
      .WillOnce([&](int payload_type, absl::optional<VideoCodecType> codec_type,
...
TEST_F(RtpSenderVideoFrameTransformerDelegateTest, CloneSenderVideoFrame) {
  ...
  EncodedImage encoded_image;
  encoded_image.SetEncodedData(webrtc::EncodedImageBuffer::Create(1));
  TransformableVideoFrameInterface* video_frame = nullptr;
  EXPECT_CALL(*frame_transformer_, Transform)
      .WillOnce(
          [&video_frame](std::unique_ptr<TransformableFrameInterface> frame) {
            video_frame =
