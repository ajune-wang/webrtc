...
//
// Example:
// If we have ResourceA, ResourceListenerX and ResourceListenerY we can register
// a BroadcastResourceListener that listens to ResourceA, use CreateAdapter() to
...
class BroadcastResourceListener : public ResourceListener {
 public:
  ...
  // Cut all ties to previously created adapters, making it safe to destroy
 ...
 private:
  ...
  void UnregisterAdapter(AdapterResource* adapter);
  ...
  std::vector<AdapterResource*> adapters_ RTC_GUARDED_BY(lock_);
};
