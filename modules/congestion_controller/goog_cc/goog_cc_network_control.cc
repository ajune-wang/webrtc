...
GoogCcNetworkController::GoogCcNetworkController(RtcEventLog* event_log,
    ...
    : event_log_(event_log),
      ...
      link_capacity_estimate_(*config.constraints.starting_rate),
...
NetworkControlUpdate GoogCcNetworkController::OnTransportPacketsFeedback(
  ...
  if (result.probe) {
    link_capacity_estimate_ = DataRate::bps(result.target_bitrate_bps);
  } else if (acknowledged_bitrate) {
    if (result.overusing) {
      link_capacity_estimate_ = std::min(link_capacity_estimate_,
                                         DataRate::bps(*acknowledged_bitrate));
    } else {
      link_capacity_estimate_ = std::max(link_capacity_estimate_,
                                         DataRate::bps(*acknowledged_bitrate));
    }
  }
