...
CallReceiveStatistics ChannelReceive::GetRTCPStatistics() const {
  ...
  if (statistician) {
    if (use_standard_bytes_stats_) {
      stats.bytesReceived = rtp_stats.data.transmitted.payload_bytes;
    } else {
      stats.bytesReceived = rtp_stats.data.transmitted.TotalBytes();
    }
    stats.packetsReceived = rtp_stats.data.transmitted.packets;
    stats.last_packet_received_timestamp_ms =
        rtp_stats.data.last_packet_received_timestamp_ms;
  } else {
