...
class RTC_EXPORT CopyOnWriteBuffer {
 public:
  ...
  template <
      typename V,
      typename E =
          typename std::remove_pointer_t<decltype(std::declval<V>().data())>,
      typename std::enable_if_t<!std::is_same<V, CopyOnWriteBuffer>::value &&
                                HasDataAndSize<V, E>::value &&
                                internal::BufferCompat<uint8_t, E>::value>* =
          nullptr>
  explicit CopyOnWriteBuffer(const V& v)
  ...
  template <
      ...
      typename E =
          typename std::remove_pointer_t<decltype(std::declval<V>().data())>,
      typename std::enable_if_t<HasDataAndSize<V, E>::value &&
                                internal::BufferCompat<uint8_t, E>::value>* =
          nullptr>
