...
const uint8_t kSBit = 0x80;
const uint8_t kStartCode[] = {0, 0, 0, 1};
const int kBufferSize = 2048;
const std::vector<uint8_t> kSomePayload = {9, 9, 9};
...
class Packet {
 public:
  ...
  Packet& Idr(std::vector<uint8_t> payload = kSomePayload);
  Packet& Slice(std::vector<uint8_t> payload = kSomePayload);
  Packet& Sps(std::vector<uint8_t> payload = kSomePayload);
  Packet& SpsWithResolution(int width,
                            int height,
                            std::vector<uint8_t> payload = kSomePayload);
  Packet& Pps(std::vector<uint8_t> payload = kSomePayload);
  Packet& Aud();
  Packet& Marker();
...
rtc::CopyOnWriteBuffer Packet::BuildFuaPayload() const {
  ...
  const uint8_t header =
      h264_header_.nalus[0].type | (first_fragment_ ? kSBit : 0);
...
struct ArrayElem {
  template <size_t N>
  ArrayElem(const uint8_t (&d)[N]) : data(std::begin(d), std::end(d)) {}
  ArrayElem(std::initializer_list<uint8_t> d) : data(d.begin(), d.end()) {}
  ArrayElem(uint8_t d) : data(1, d) {}
...
TEST(H264PacketBufferTest, SeqNumJumpDoesNotCompleteFrame) {
  ...
  // Next packet is 1 + kBufferSize ahead of the last packet.
  EXPECT_THAT(
      packet_buffer
          .InsertPacket(
...
//   EXPECT_THAT(InsertH264(kStartSize - 2, kKeyFrame, kFirst, kLast,
//   0).packets,
//               SizeIs(1));
...
//   EXPECT_THAT(
//       InsertH264(kStartSize + kStartSize, kDeltaFrame, kNotFirst, kLast,
//       2000)
//           .packets,
//       IsEmpty());
