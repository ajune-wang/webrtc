...
constexpr TimeDelta kStatsWaitTimeout = TimeDelta::Millis<1000>();
...
class NetworkEmulationManagerThreeNodesRoutingTest : public ::testing::Test {
 public:
  ...
  void SendPacketsAndValidateDelivery() {
    ...
    emulation_.AdvanceTime(kNetworkPacketWaitTimeout);
  }
 ...
 private:
  ...
  NetworkEmulationManagerImpl emulation_{/*real_time=*/true};
...
TEST(NetworkEmulationManagerTest, GeneratedIpv4AddressDoesNotCollide) {
  NetworkEmulationManagerImpl network_manager(/*real_time=*/true);
...
TEST(NetworkEmulationManagerTest, Run) {
  ...
  for (uint64_t j = 0; j < 2; j++) {
    ...
    network_manager.AdvanceTime(TimeDelta::seconds(1));
  ...
  ASSERT_TRUE(network_manager.WaitAndProcess(
      [&] { return received_stats_count.load() == 2; }, kStatsWaitTimeout));
}
...
TEST(NetworkEmulationManagerTest, ThroughputStats) {
  ...
  for (int i = 0; i < kNumPacketsSent; i++) {
    ...
    network_manager.AdvanceTime(kDelay);
  }
  ...
  ASSERT_TRUE(network_manager.WaitAndProcess(
      [&] { return received_stats_count.load() == 1; }, kStatsWaitTimeout));
