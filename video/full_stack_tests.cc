...
std::string GetExtendedFieldTrials(std::string new_trial_string) {
...
TEST(FullStackTest, ForemanCifPlr5H264SpsPpsIdrIsKeyframe) {
  ...
  test::ScopedFieldTrials override_field_trials(
      GetExtendedFieldTrials("WebRTC-SpsPpsIdrIsH264Keyframe/Enabled/"));
...
TEST(FullStackTest, ConferenceMotionHd3TLModerateLimitsAltTLPattern) {
  ...
  test::ScopedFieldTrials field_trial(
      GetExtendedFieldTrials("WebRTC-UseShortVP8TL3Pattern/Enabled/"));
  ParamsWithLogging conf_motion_hd;
  conf_motion_hd.call.send_side_bwe = true;
  conf_motion_hd.video[0] = {
      true,    1280,    720,   50,    30000,
...
TEST(FullStackTest, MAYBE_SimulcastFullHdOveruse) {
  ...
  simulcast.ss[0] = {
      ...
      true};
  webrtc::test::ScopedFieldTrials override_trials(GetExtendedFieldTrials(
...
TEST_P(DualStreamsTest,
  ...
  test::ScopedFieldTrials field_trial(
      GetExtendedFieldTrials(std::string(kRoundRobinPacingQueueExperiment) +
