...
class CreateSessionDescriptionObserverAdapter
 ...
 public:
  CreateSessionDescriptionObserverAdapter(
      void (^completionHandler)(RTC_OBJC_TYPE(RTCSessionDescription) *sessionDescription,
                                NSError *error)) {
...
void PeerConnectionDelegateAdapter::OnAddStream(
  ...
  RTC_OBJC_TYPE(RTCMediaStream) *mediaStream =
      [[RTC_OBJC_TYPE(RTCMediaStream) alloc] initWithFactory:peer_connection.factory nativeMediaStream:stream];
  [peer_connection.delegate peerConnection:peer_connection
...
void PeerConnectionDelegateAdapter::OnDataChannel(
  ...
  RTC_OBJC_TYPE(RTCPeerConnection) *peer_connection = peer_connection_;
  RTC_OBJC_TYPE(RTCDataChannel) *dataChannel = [[RTC_OBJC_TYPE(RTCDataChannel) alloc] initWithFactory:peer_connection.factory
...
void PeerConnectionDelegateAdapter::OnIceConnectionChange(
  ...
  RTCIceConnectionState state = [RTC_OBJC_TYPE(RTCPeerConnection) iceConnectionStateForNativeState:new_state];
...
void PeerConnectionDelegateAdapter::OnConnectionChange(
  ...
  if ([peer_connection_.delegate
    ...
    RTCPeerConnectionState state = [RTC_OBJC_TYPE(RTCPeerConnection) connectionStateForNativeState:new_state];
...
void PeerConnectionDelegateAdapter::OnIceCandidatesRemoved(
  ...
  for (const auto& candidate : candidates) {
    ...
    RTC_OBJC_TYPE(RTCIceCandidate)* ice_candidate = [[RTC_OBJC_TYPE(RTCIceCandidate) alloc]
...
void PeerConnectionDelegateAdapter::OnIceSelectedCandidatePairChanged(
  ...
  RTC_OBJC_TYPE(RTCIceCandidate) *local_candidate =
      [[RTC_OBJC_TYPE(RTCIceCandidate) alloc] initWithNativeCandidate:local_candidate_wrapper.release()];
  ...
  RTC_OBJC_TYPE(RTCIceCandidate) *remote_candidate =
      [[RTC_OBJC_TYPE(RTCIceCandidate) alloc] initWithNativeCandidate:remote_candidate_wrapper.release()];
  RTC_OBJC_TYPE(RTCPeerConnection) *peer_connection = peer_connection_;
...
void PeerConnectionDelegateAdapter::OnAddTrack(
  ...
  if ([peer_connection.delegate respondsToSelector:@selector(peerConnection:
    ...
    for (const auto &nativeStream : streams) {
      RTC_OBJC_TYPE(RTCMediaStream) *mediaStream = [[RTC_OBJC_TYPE(RTCMediaStream) alloc] initWithFactory:peer_connection.factory
                                                          nativeMediaStream:nativeStream];
      [mediaStreams addObject:mediaStream];
    }
    RTC_OBJC_TYPE(RTCRtpReceiver) *rtpReceiver = [[RTC_OBJC_TYPE(RTCRtpReceiver) alloc] initWithFactory:peer_connection.factory
...
- (RTC_OBJC_TYPE(RTCRtpTransceiver) *)addTransceiverWithTrack:(RTC_OBJC_TYPE(RTCMediaStreamTrack) *)track {
  return [self addTransceiverWithTrack:track init:[[RTC_OBJC_TYPE(RTCRtpTransceiverInit) alloc] init]];
}
...
- (RTC_OBJC_TYPE(RTCRtpTransceiver) *)addTransceiverWithTrack:(RTC_OBJC_TYPE(RTCMediaStreamTrack) *)track
  ...
  return [[RTC_OBJC_TYPE(RTCRtpTransceiver) alloc] initWithFactory:self.factory
...
- (RTC_OBJC_TYPE(RTCRtpTransceiver) *)addTransceiverOfType:(RTCRtpMediaType)mediaType
                                       init:(RTC_OBJC_TYPE(RTCRtpTransceiverInit) *)init {
  webrtc::RTCErrorOr<rtc::scoped_refptr<webrtc::RtpTransceiverInterface>> nativeTransceiverOrError =
      _peerConnection->AddTransceiver([RTC_OBJC_TYPE(RTCRtpReceiver) nativeMediaTypeForMediaType:mediaType],
                                      init.nativeInit);
  if (!nativeTransceiverOrError.ok()) {
...
- (RTC_OBJC_TYPE(RTCRtpSender) *)senderWithKind:(NSString *)kind
  ...
  return nativeSender ?
      [[RTC_OBJC_TYPE(RTCRtpSender) alloc] initWithFactory:self.factory nativeRtpSender:nativeSender] :
      nil;
}
...
- (NSArray<RTC_OBJC_TYPE(RTCRtpReceiver) *> *)receivers {
  ...
  for (const auto &nativeReceiver : nativeReceivers) {
    RTC_OBJC_TYPE(RTCRtpReceiver) *receiver =
        [[RTC_OBJC_TYPE(RTCRtpReceiver) alloc] initWithFactory:self.factory nativeRtpReceiver:nativeReceiver];
...
- (NSArray<RTC_OBJC_TYPE(RTCRtpTransceiver) *> *)transceivers {
  ...
  for (const auto &nativeTransceiver : nativeTransceivers) {
    RTC_OBJC_TYPE(RTCRtpTransceiver) *transceiver = [[RTC_OBJC_TYPE(RTCRtpTransceiver) alloc] initWithFactory:self.factory
                                                           nativeRtpTransceiver:nativeTransceiver];
