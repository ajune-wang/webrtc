...
double ToKb(DataSize datasize) { return datasize.bytes() / 1000.0; }
...
LossBasedBweV2::Derivatives LossBasedBweV2::GetDerivatives(
  ...
  for (const Observation& observation : observations_) {
    ...
    if (config_->use_byte_loss_rate) {
      derivatives.first +=
          temporal_weight * ((ToKb(observation.lost_size) / loss_probability) -
                             (ToKb(observation.size - observation.lost_size) /
                              (1.0 - loss_probability)));
      derivatives.second -=
          temporal_weight *
          ((ToKb(observation.lost_size) / std::pow(loss_probability, 2)) +
           (ToKb(observation.size - observation.lost_size) /
  ...
  for (const Observation& observation : observations_) {
    ...
    if (config_->use_byte_loss_rate) {
      objective += temporal_weight *
                   ((ToKb(observation.lost_size) * std::log(loss_probability)) +
                    (ToKb(observation.size - observation.lost_size) *
                     std::log(1.0 - loss_probability)));
      objective +=
          temporal_weight * high_bandwidth_bias * ToKb(observation.size);
    } else {
      objective +=
          temporal_weight *
