...
std::unique_ptr<ScalableVideoController> CreateVp9ScalabilityStructure(
  ...
  int num_temporal_layers = std::max<int>(1, vp9.numberOfTemporalLayers);
  if (vp9.numberOfSpatialLayers == 1 && num_temporal_layers == 1) {
    return std::make_unique<ScalableVideoControllerNoLayering>();
  }

  if (vp9.interLayerPred != InterLayerPredMode::kOn ||
  ...
  ss << "L" << vp9.numberOfSpatialLayers << "T" << num_temporal_layers;
