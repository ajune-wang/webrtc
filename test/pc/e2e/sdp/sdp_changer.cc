...
LocalAndRemoteSdp SignalingInterceptor::PatchOffer(
  ...
  for (auto& content : offer->description()->contents()) {
    ...
    auto it = params_.stream_label_to_track_params.find(stream_label);
    if (it == params_.stream_label_to_track_params.end()) {
      continue;
    }
    context_.mid_to_track_params.insert({content.mid(), it->second});
    media_desc->set_conference_mode(it->second.use_conference_mode);
  }
...
LocalAndRemoteSdp SignalingInterceptor::PatchVp9Offer(
  ...
  for (auto& content : offer->description()->contents()) {
    ...
    auto it = context_.mid_to_track_params.find(content.mid());
    if (it == context_.mid_to_track_params.end()) {
      continue;
    }
    if (it->second.simulcast_streams_count == 0) {
      // SVC track has non 0 simulcast_streams_count.
      continue;
    }
