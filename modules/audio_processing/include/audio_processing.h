...
class AudioProcessing : public rtc::RefCountInterface {
 public:
  ...
  struct AudioProcessingStats {
    ...
    // Fraction of time that the AEC linear filter is divergent, in a 1-second
    // non-overlapped aggregation window.
    rtc::Optional<float> divergent_filter_fraction;
    ...
    // |GetStatistics()| during a session the first call from any of them will
    // change to one second aggregation window for all.
    rtc::Optional<int> delay_median_ms;
    rtc::Optional<int> delay_standard_deviation_ms;
  ...
  // GetStatistics.
  virtual AudioProcessingStats GetStatistics(bool has_recv_streams) const;
