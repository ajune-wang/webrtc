...
void PacingController::ProcessPackets() {
  ...
  Timestamp last_process_time = last_process_time_;
  ...
  while (!paused_) {
    ...
    if (mode_ == ProcessMode::kDynamic &&
        last_process_time < target_send_time) {
      // Reduce buffer levels with amount corresponding to time between last
      // process as target send time for the next packet.
      ...
      UpdateBudgetWithElapsedTime(target_send_time - last_process_time);
      last_process_time = target_send_time;
    }
...
RoundRobinPacketQueue::QueuedPacket* PacingController::GetPendingPacket(
  ...
  if (!unpaced_audio_packet && !is_probe) {
    ...
    if (mode_ == ProcessMode::kPeriodic) {
      ...
      if (now - target_send_time <= TimeDelta::Zero()) {
        ...
        if (flush_time > target_send_time - now) {
          return nullptr;
        }
      } else {
