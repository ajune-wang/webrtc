...
@interface RTC_OBJC_TYPE(RTCMTLVideoView) (Testing)
...
- (id)frameMockWithCVPixelBuffer:(BOOL)hasCVPixelBuffer {
  ...
  if (hasCVPixelBuffer) {
    ...
    OCMStub([frameMock buffer]).andReturn([[RTC_OBJC_TYPE(RTCI420Buffer) alloc] initWithWidth:200 height:200]);
  }
...
- (void)testInitAssertsIfMetalUnavailabe {
  ...
  BOOL asserts = NO;
  @try {
    RTC_OBJC_TYPE(RTCMTLVideoView) *realView = [[RTC_OBJC_TYPE(RTCMTLVideoView) alloc] initWithFrame:CGRectZero];
    (void)realView;
...
- (void)testRTCVideoRenderNilFrameCallback {
  ...
  RTC_OBJC_TYPE(RTCMTLVideoView) *realView = [[RTC_OBJC_TYPE(RTCMTLVideoView) alloc] initWithFrame:CGRectMake(0, 0, 640, 480)];
