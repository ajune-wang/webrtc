...
namespace webrtc {
// CoDel Active queue management emulation. Based on CoDel pseudocode from
// ACMQueue. CoDel keeps queuing delays low by dropping packets when delay is
// high. For each packet ready for dequeue, call DropDequeuePacket with the
// packet parameters to update the CoDel state. DropDequeuePacket returns true
...
class CoDelController {
 ...
 private:
  ...
  Timestamp last_drop_at_ = Timestamp::PlusInfinity();
  int drop_count_ = 0;
...
class SimulatedNetwork : public NetworkBehaviorInterface {
 ...
 private:
  ...
  CoDelController codel_controller_ RTC_GUARDED_BY(process_checker_);
