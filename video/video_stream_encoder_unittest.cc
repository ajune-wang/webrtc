...
class VideoStreamEncoderTest : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    fake_clock_.SetTime(Timestamp::Microseconds(1234));
  ...
  void VerifyAllocatedBitrate(const VideoBitrateAllocation& expected_bitrate) {
    ...
    video_stream_encoder_->OnBitrateUpdated(
        DataRate::BitsPerSecond(kTargetBitrateBps),
        DataRate::BitsPerSecond(kTargetBitrateBps),
        DataRate::BitsPerSecond(kTargetBitrateBps), 0, 0);
...
TEST_F(VideoStreamEncoderTest, DropsFramesWhenRateSetToZero) {
  ...
  video_stream_encoder_->OnBitrateUpdated(DataRate::BitsPerSecond(0),
                                          DataRate::BitsPerSecond(0),
                                          DataRate::BitsPerSecond(0), 0, 0);
  // The encoder will cache up to one frame for a short duration. Adding two
  // frames means that the first frame will be dropped and the second frame will
...
TEST_F(VideoStreamEncoderTest, AdaptUpIfBwEstimateIsHigherThanMinBitrate) {
  ...
  video_stream_encoder_->OnBitrateUpdated(
      DataRate::BitsPerSecond(kEncoderBitrateLimits720p.min_start_bitrate_bps),
      DataRate::BitsPerSecond(kEncoderBitrateLimits720p.min_start_bitrate_bps),
      DataRate::BitsPerSecond(kEncoderBitrateLimits720p.min_start_bitrate_bps),
      0, 0);
  ...
  video_stream_encoder_->OnBitrateUpdated(
      DataRate::BitsPerSecond(kEncoderBitrateLimits540p.min_start_bitrate_bps),
      DataRate::BitsPerSecond(kEncoderBitrateLimits540p.min_start_bitrate_bps),
      DataRate::BitsPerSecond(kEncoderBitrateLimits540p.min_start_bitrate_bps),
      0, 0);
  video_stream_encoder_->TriggerQualityLow();
...
class BalancedDegradationTest : public VideoStreamEncoderTest {
 protected:
  ...
  void OnBitrateUpdated(int bitrate_bps) {
    video_stream_encoder_->OnBitrateUpdated(
        DataRate::BitsPerSecond(bitrate_bps),
        DataRate::BitsPerSecond(bitrate_bps),
        DataRate::BitsPerSecond(bitrate_bps), 0, 0);
  }
...
TEST_F(VideoStreamEncoderTest, CallsBitrateObserver) {
  ...
  EXPECT_CALL(bitrate_observer, OnBitrateAllocationUpdated(expected_bitrate))
      .Times(1);
  video_stream_encoder_->OnBitrateUpdated(
      DataRate::BitsPerSecond(kLowTargetBitrateBps),
      DataRate::BitsPerSecond(kLowTargetBitrateBps),
      DataRate::BitsPerSecond(kLowTargetBitrateBps), 0, 0);
  ...
  fake_clock_.AdvanceTime(TimeDelta::Milliseconds(1) / kDefaultFps);
...
TEST_F(VideoStreamEncoderTest, DropsFramesAndScalesWhenBitrateIsTooLow) {
  ...
  video_stream_encoder_->OnBitrateUpdated(
      DataRate::BitsPerSecond(kTooLowBitrateForFrameSizeBps),
      DataRate::BitsPerSecond(kTooLowBitrateForFrameSizeBps),
      DataRate::BitsPerSecond(kTooLowBitrateForFrameSizeBps), 0, 0);
  const int kWidth = 640;
  const int kHeight = 360;
...
TEST_F(VideoStreamEncoderTest, InitialFrameDropActivatesWhenBweDrops) {
  ...
  video_stream_encoder_->OnBitrateUpdated(
      DataRate::BitsPerSecond(kNotTooLowBitrateForFrameSizeBps),
      DataRate::BitsPerSecond(kNotTooLowBitrateForFrameSizeBps),
      DataRate::BitsPerSecond(kNotTooLowBitrateForFrameSizeBps), 0, 0);
  video_source_.IncomingCapturedFrame(CreateFrame(2, kWidth, kHeight));
  // Frame should not be dropped.
  WaitForEncodedFrame(2);
...
TEST_F(VideoStreamEncoderTest, RampsUpInQualityWhenBwIsHigh) {
  ...
  video_stream_encoder_->OnBitrateUpdated(
      DataRate::BitsPerSecond(kLowBitrateBps),
      DataRate::BitsPerSecond(kLowBitrateBps),
      DataRate::BitsPerSecond(kLowBitrateBps), 0, 0);
  ...
  video_stream_encoder_->OnBitrateUpdated(
      DataRate::BitsPerSecond(config.max_bitrate_bps),
      DataRate::BitsPerSecond(config.max_bitrate_bps),
      DataRate::BitsPerSecond(config.max_bitrate_bps), 0, 0);
  ...
  fake_clock_.AdvanceTime(TimeDelta::Milliseconds(2000));
...
TEST_F(VideoStreamEncoderTest, DoesntAdaptDownPastMinFramerate) {
  ...
  do {
    ...
    for (int j = 0; j < kFramerateFps; ++j) {
      ...
      timestamp_ms += kFrameIntervalMs;
      fake_clock_.AdvanceTime(TimeDelta::Milliseconds(kFrameIntervalMs));
    }
...
TEST_F(VideoStreamEncoderTest, DropsFramesWhenEncoderOvershoots) {
  ...
  video_stream_encoder_->OnBitrateUpdated(
      DataRate::BitsPerSecond(kTargetBitrateBps + 1000),
      DataRate::BitsPerSecond(kTargetBitrateBps + 1000),
      DataRate::BitsPerSecond(kTargetBitrateBps + 1000), 0, 0);
  num_dropped = 0;
  for (int i = 0; i < kNumFramesInRun; ++i) {
    video_source_.IncomingCapturedFrame(
        CreateFrame(timestamp_ms, kFrameWidth, kFrameHeight));
...
TEST_F(VideoStreamEncoderTest, SetsFrameTypesSimulcast) {
  ...
  video_stream_encoder_->OnBitrateUpdated(
      DataRate::BitsPerSecond(kSimulcastTargetBitrateBps),
      DataRate::BitsPerSecond(kSimulcastTargetBitrateBps),
      DataRate::BitsPerSecond(kSimulcastTargetBitrateBps), 0, 0);
...
TEST_F(VideoStreamEncoderTest, BandwidthAllocationLowerBound) {
  ...
  // Initial rate.
  video_stream_encoder_->OnBitrateUpdated(
      /*target_bitrate=*/DataRate::KilobitsPerSecond(300),
      /*stable_target_bitrate=*/DataRate::KilobitsPerSecond(300),
      /*link_allocation=*/DataRate::KilobitsPerSecond(300),
      /*fraction_lost=*/0,
      /*rtt_ms=*/0);
  ...
  DataRate min_rate = DataRate::KilobitsPerSecond(codec_config.minBitrate);
  DataRate target_rate = min_rate - DataRate::KilobitsPerSecond(1);
  video_stream_encoder_->OnBitrateUpdated(
  ...
  DataRate allocation_sum =
      DataRate::BitsPerSecond(rate_settings->bitrate.get_sum_bps());
...
TEST_F(VideoStreamEncoderTest, BitrateEncoderSwitch) {
  ...
  video_stream_encoder_->OnBitrateUpdated(
      /*target_bitrate=*/DataRate::KilobitsPerSecond(50),
      /*stable_target_bitrate=*/DataRate::KilobitsPerSecond(kDontCare),
      /*link_allocation=*/DataRate::KilobitsPerSecond(kDontCare),
      /*fraction_lost=*/0,
      /*rtt_ms=*/0);
...
TEST_F(VideoStreamEncoderTest, ResolutionEncoderSwitch) {
  ...
  // not fail.
  video_stream_encoder_->OnBitrateUpdated(
      /*target_bitrate=*/DataRate::KilobitsPerSecond(
          kSufficientBitrateToNotDrop),
      /*stable_target_bitrate=*/
      DataRate::KilobitsPerSecond(kSufficientBitrateToNotDrop),
      /*link_allocation=*/
      DataRate::KilobitsPerSecond(kSufficientBitrateToNotDrop),
      /*fraction_lost=*/0,
      /*rtt_ms=*/0);
...
TEST_F(VideoStreamEncoderTest,
  ...
  // Set initial rate.
  auto rate = DataRate::KilobitsPerSecond(100);
  video_stream_encoder_->OnBitrateUpdated(
      /*target_bitrate=*/rate,
  ...
  auto new_stable_rate = rate - DataRate::KilobitsPerSecond(5);
  video_stream_encoder_->OnBitrateUpdated(
