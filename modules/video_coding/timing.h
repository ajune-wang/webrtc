...
class VCMTiming {
 public:
  ...
  void IncomingTimestamp(uint32_t time_stamp, Timestamp last_packet_time);

  // Returns the receiver system time when the frame with timestamp
  // `frame_timestamp` should be rendered, assuming that the system time
  // currently is `now_ms`.
  virtual Timestamp RenderTimeMs(uint32_t frame_timestamp, Timestamp now) const;
  ...
  virtual bool GetTimings(TimeDelta* max_decode_ms,
 ...
 protected:
  ...
  Timestamp RenderTimeMsInternal(uint32_t frame_timestamp, Timestamp now) const
