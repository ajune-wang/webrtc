...
#define RTC_LOG_THREAD_BLOCK_COUNT()                                        \
  rtc::Thread::ScopedCountBlockingCalls blocked_call_count_printer(         \
      [func = __func__](uint32_t actual_block, uint32_t would_block) {      \
        auto total = actual_block + would_block;                            \
        if (total) {                                                        \
...
class RTC_LOCKABLE RTC_EXPORT Thread : public webrtc::TaskQueueBase {
 public:
  ...
  class ScopedCountBlockingCalls {
   public:
    ...
    uint32_t GetActualBlockedCallCount() const;
    uint32_t GetWouldBlockCallCount() const;
    uint32_t GetTotalBlockedCallCount() const;

   private:
    Thread* const thread_;
    const uint32_t start_actual_count_;
    const uint32_t start_would_count_;
 ...
 private:
  ...
  uint32_t would_block_call_count_ RTC_GUARDED_BY(this) = 0;
