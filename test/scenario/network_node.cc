...
std::unique_ptr<SimulationNode> SimulationNode::Create(
  ...
  std::unique_ptr<SimulationNode> node(
      new SimulationNode(config, std::move(network)));
...
bool NetworkNodeTransport::SendRtp(const uint8_t* packet,
  ...
  sender_->call_->OnSentPacket(
      rtc::SentPacket(options.packet_id, clock_->TimeInMilliseconds()));
  Timestamp send_time = Timestamp::ms(clock_->TimeInMilliseconds());
