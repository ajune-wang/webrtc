...
//   - the connection probes should be started
...
class ActiveIceControllerInterface {
 public:
  ...
  // Called when the state of the connections is mutated.
  ...
  // Enumerate all the connections known to the ICE controller.
  virtual rtc::ArrayView<const Connection*> Connections() const = 0;

  // Compute "STUN_ATTR_USE_CANDIDATE" for a STUN ping to the given connection.
  virtual bool GetUseCandidateAttribute(const Connection* connection,
                                        NominationMode mode,
                                        IceMode remote_ice_mode) const = 0;

  // Called when STUN ping actions are requested or reported.
