...
bool RtpEndsWithFragment(
    uint8_t aggregation_header) {  // i.e. to be continued...
...
absl::optional<RtpDepacketizerAv1::Frame> RtpDepacketizerAv1::AssembleFrame(
  ...
  for (rtc::ArrayView<const uint8_t> rtp_payload : rtp_payloads) {
    if (rtp_payload.empty()) {
      RTC_LOG(WARNING) << "Failed to find aggregation header in a packet";
      return absl::nullopt;
    }
    ...
    if (continues_obu != expect_continues_obu) {
      RTC_LOG(WARNING) << "Unexpected Z-bit " << continues_obu;
      return absl::nullopt;
    }
    ...
    for (remaining = rtp_payload.subview(1), obu_index = 1; !remaining.empty();
      ...
      if (has_fragment_size) {
        ...
        if (fragment_size > remaining.size()) {
          ...
          RTC_LOG(WARNING) << "Malformed fragment size " << fragment_size
  ...
  if (expect_continues_obu) {
    RTC_LOG(WARNING) << "Last packet shouldn't have last obu fragmented.";
    return absl::nullopt;
  }
  ...
  for (ObuInfo& obu : obus) {
    ...
    if (ObuHasSize(obu_header)) {
      ...
      if (size.size == 0) {
        RTC_LOG(WARNING) << "Failed to read obu_size.";
        return absl::nullopt;
      }
    ...
    if (obu.signaled_size > 0 && obu.signaled_size != obu.data_size) {
      ...
      RTC_LOG(WARNING) << "Mismatch in obu_size. signaled: "
  ...
  Frame av1_frame;
  av1_frame.video_header.frame_type = VideoFrameType::kVideoFrameDelta;
  av1_frame.video_header.codec = VideoCodecType::kVideoCodecAV1;
  av1_frame.bitstream = EncodedImageBuffer::Create(frame_size);
  uint8_t* write_at = av1_frame.bitstream->data();
  for (ObuInfo& obu : obus) {
    uint8_t obu_header = obu.data[0][0];
    // Store obu_header
    *write_at = obu_header | kObuHasSizeBit;
    ...
    if (ObuType(obu_header) == kObuTypeSequenceHeader) {
      // TODO(danilchap): Check frame_header OBU and/or frame OBU too for other
      // conditions of the start of a new coded video sequence.
      // See https://aomediacodec.github.io/av1-spec/av1-spec.pdf section 7.5
      av1_frame.video_header.frame_type = VideoFrameType::kVideoFrameKey;
    }
  }
  RTC_CHECK_EQ(write_at - av1_frame.bitstream->data(),
               av1_frame.bitstream->size());
  return av1_frame;
}
