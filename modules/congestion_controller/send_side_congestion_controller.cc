...
void SendSideCongestionController::Process() {
  ...
  auto queue_time_ms = pacer_->TryGetExpectedQueueTimeMs();
  if (queue_time_ms) {
    internal::PacerQueueUpdate msg;
    msg.expected_queue_time = TimeDelta::ms(queue_time_ms.value());
...
void SendSideCongestionController::MaybeUpdateOutstandingData() {
  // Only process if a congestion window has been set
