...
int32_t RTCPReceiver::RTT(uint32_t remote_ssrc,
  ...
  if (avg_rtt_ms)
    *avg_rtt_ms = report_block_data->sum_rtt_ms / report_block_data->num_rtts;
...
void RTCPReceiver::HandleReportBlock(const ReportBlock& report_block,
  ...
  report_block_data->report_block_timestamp_us = rtc::TimeUTCMicros();
  report_block_data->report_block.sender_ssrc = remote_ssrc;
  report_block_data->report_block.source_ssrc = report_block.source_ssrc();
  report_block_data->report_block.fraction_lost = report_block.fraction_lost();
  report_block_data->report_block.packets_lost =
  ...
  report_block_data->report_block.jitter = report_block.jitter();
  report_block_data->report_block.delay_since_last_sender_report =
      report_block.delay_since_last_sr();
  report_block_data->report_block.last_sender_report_timestamp =
  ...
  if (send_time_ntp != 0) {
    ...
    if (rtt_ms > report_block_data->max_rtt_ms)
      report_block_data->max_rtt_ms = rtt_ms;

    if (report_block_data->num_rtts == 0 ||
        rtt_ms < report_block_data->min_rtt_ms)
      report_block_data->min_rtt_ms = rtt_ms;

    report_block_data->last_rtt_ms = rtt_ms;
    report_block_data->sum_rtt_ms += rtt_ms;
    ++report_block_data->num_rtts;

    packet_information->rtt_ms = rtt_ms;
  }

  packet_information->report_blocks.push_back(report_block_data->report_block);
...
void RTCPReceiver::TriggerCallbacksFromRtcpPacket(
  ...
  if (!receiver_only_) {
    ...
    if (report_block_data_observer_) {
      for (auto& report_block_data : packet_information.report_block_datas) {
        report_block_data_observer_->OnReportBlockDataUpdated(
            std::move(report_block_data));
      }
    }
  }
}
