...
RTPVideoHeader TransformableEncodedFrame::video_header() {
  return video_header_;
}

const absl::optional<VideoCodecType>& TransformableEncodedFrame::codec_type()
    const {
  return codec_type_;
}

int64_t TransformableEncodedFrame::capture_time_ms() const {
  return capture_time_ms_;
}

RTPFragmentationHeader* TransformableEncodedFrame::fragmentation_header() {
  return fragmentation_header_.get();
}

const absl::optional<int64_t>&
TransformableEncodedFrame::expected_retransmission_time_ms() const {
