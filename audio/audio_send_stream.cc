...
bool AudioSendStream::HasTransportSeqNum(
    const AudioSendStream::Config& config) {
  return TransportSeqNumId(config) != 0;
}
...
void AudioSendStream::ConfigureStream(
  ...
  if (first_time || (transport_seq_num_id_changed &&
                     !stream->allocation_settings_.force_no_audio_feedback())) {
    if (!first_time) {
    ...
    if (stream->allocation_settings_.include_audio_in_feedback(
...
void AudioSendStream::Start() {
  ...
  if (allocation_settings_.include_audio_in_allocation_on_start(
          config_.min_bitrate_bps != -1, config_.max_bitrate_bps != -1,
          config_.has_dscp, HasTransportSeqNum(config_))) {
...
bool AudioSendStream::SetupSendCodec(AudioSendStream* stream,
  ...
  // codec's default.
  if (stream->allocation_settings_.update_audio_target_bitrate(
          HasTransportSeqNum(new_config)) &&
...
void AudioSendStream::ReconfigureBitrateObserver(
  ...
  if (stream->config_.min_bitrate_bps == new_config.min_bitrate_bps &&
      ...
      (TransportSeqNumId(stream->config_) == TransportSeqNumId(new_config) ||
       stream->allocation_settings_.ignore_seq_num_id_change())) {
    return;
  }

  if (stream->allocation_settings_.include_audio_in_allocation_on_reconfigure(
          new_config.min_bitrate_bps != -1, new_config.max_bitrate_bps != -1,
          new_config.has_dscp, HasTransportSeqNum(new_config))) {
