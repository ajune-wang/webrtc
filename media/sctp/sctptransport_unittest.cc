...
class SctpTransportObserver : public sigslot::has_slots<> {
 public:
  SctpTransportObserver(SctpTransport* transport) {
    transport->SignalIncomingStreamReset.connect(
        this, &SctpTransportObserver::OnIncomingStreamReset);
 ...
 private:
  void OnIncomingStreamReset(int stream) { incoming_reset_streams_.push_back(stream); }
  void OnClosingProcedureComplete(int stream) { closed_streams_.push_back(stream); }
  void OnReadyToSend() { ready_to_send_ = true; }

  std::vector<int> incoming_reset_streams_;
...
TEST_F(SctpTransportTest, ReusesAStream) {
  ...
  transport1()->ResetStream(1);
  EXPECT_EQ_WAIT(2, transport2_observer.StreamCloseCount(1),
                   kDefaultTimeout);
}
