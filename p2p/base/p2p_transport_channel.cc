...
void P2PTransportChannel::AddAllocatorSession(
  ...
  if (shared_gatherer_) {
    // ICE restarts after the use of a shared_gatherer_ need to have a generation
    // larger than the shared_gatherer_ had.
    generation +=
...
void P2PTransportChannel::OnPortReady(PortAllocatorSession* session,
  ...
  if (IsSharedSession(session)) {
    ...
    port->SignalRoleConflict.connect(this,
                                     &P2PTransportChannel::OnRoleConflictIgnored);
...
void P2PTransportChannel::OnUnknownAddress(PortInterface* port,
  ...
  if (ice_param != nullptr) {
    ...
    RTC_LOG(LS_WARNING) << "Ignoring peer-refexive ICE candidate because the ufrag is unknown.";
    return;
  }
