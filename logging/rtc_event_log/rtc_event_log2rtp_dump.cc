...
int main(int argc, char* argv[]) {
  ...
  for (const auto& stream : parsed_stream.incoming_rtp_packets_by_ssrc()) {
    ...
    if (ShouldSkipStream(media_type, stream.ssrc, ssrc_filter))
      continue;
    event_processor.AddEvents(stream.incoming_packets.begin(),
                              stream.incoming_packets.end(), handle_rtp);
  }
  ...
  event_processor.AddEvents(parsed_stream.incoming_rtcp_packets().begin(),
                            parsed_stream.incoming_rtcp_packets().end(),
                            handle_rtcp);
