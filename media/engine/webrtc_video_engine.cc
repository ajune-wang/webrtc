...
void WebRtcVideoChannel::WebRtcVideoReceiveStream::RecreateWebRtcVideoStream() {
  ...
  absl::optional<webrtc::VideoReceiveStream::EncodedFrameDispatchState>
      dispatch_state;
  if (stream_) {
    base_minimum_playout_delay_ms = stream_->GetBaseMinimumPlayoutDelayMs();
    dispatch_state = stream_->SetEncodedFrameDispatchState(
        webrtc::VideoReceiveStream::EncodedFrameDispatchState(),
  ...
  if (dispatch_state) {
    stream_->SetEncodedFrameDispatchState(std::move(*dispatch_state),
                                          /*generate_key_frame=*/false);
  }
...
void WebRtcVideoChannel::WebRtcVideoReceiveStream::
  ...
  if (stream_) {
    stream_->SetEncodedFrameDispatchState(
        webrtc::VideoReceiveStream::EncodedFrameDispatchState(
            std::move(callback)),
