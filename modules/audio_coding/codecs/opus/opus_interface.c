...
static int WebRtcOpus_DurationEst_Legacy(OpusDecInst* inst,
...
static int WebRtcOpus_DurationEst_New(OpusDecInst* inst,
                                      const uint8_t* payload,
                                      size_t payload_length_bytes) {
  if (payload_length_bytes == 0) {
    // WebRtcOpus_Decode calls PLC when payload length is zero. So we return
...
// TODO(webrtc:10772): remove the legacy method.
static int WebRtcOpus_PacketHasFec_Legacy(const uint8_t* payload,
                                          size_t payload_length_bytes) {
...
static int WebRtcOpus_PacketHasFec_New(const uint8_t* payload,
                                       size_t payload_length_bytes) {
  if (payload == NULL || payload_length_bytes == 0) {
    return 0;
  }
