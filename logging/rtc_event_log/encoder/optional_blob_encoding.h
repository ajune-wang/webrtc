...
// Encode/decode a sequence of optional strings, whose length is not known to be
...
// to 0, is an error.
// DecodeOptionalBlobs() returns an empty vector if it fails, which can happen
// if `encoded_blobs` is corrupted.
// When successful, DecodeOptionalBlobs() returns a vector of string_view
// objects, which refer to the original input (`encoded_blobs`), and therefore
// may not outlive it.
//
// Note that the returned std::string might have been reserved for significantly
// more memory than it ends up using. If the caller to EncodeOptionalBlobs()
// intends to store the result long-term, they should consider
// shrink_to_fit()-ing it.
