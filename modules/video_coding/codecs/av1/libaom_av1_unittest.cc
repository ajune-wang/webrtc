...
class TestAv1Decoder {
 public:
  ...
  size_t num_decoded_frames() const { return callback_.num_called(); }
...
TEST(LibaomAv1Test, EncodeDecode) {
  ...
  // Assert.
  // Check encoder produced same frames for decoder to decode.
  ASSERT_THAT(decoder.requested_frame_ids(), Not(IsEmpty()));
  ...
  EXPECT_EQ(decoder.num_decoded_frames(), decoder.decoded_frame_ids().size());
}
