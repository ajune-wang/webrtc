...
class RTC_EXPORT SequenceCheckerImpl {
 public:
  ...
  // Where we print the pointer values of each of the three values + an optional
  // "(not current)" following each value if the current context does not match
  // with the state bound to by the SequenceCheckerImpl instance.
...
template <typename ThreadLikeObject,
          typename std::enable_if<!std::is_same<
              typename std::remove_const<
                  typename std::remove_reference<ThreadLikeObject>::type>::type,
              webrtc::SequenceChecker>::value>::type* = nullptr>
std::string ExpectationToString(const ThreadLikeObject*) {
  return std::string("(unknown thread checker type)");
}
