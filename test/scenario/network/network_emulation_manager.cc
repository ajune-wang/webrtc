...
NetworkEmulationManager::NetworkEmulationManager(
    ...
    : time_controller_(time_controller), next_node_id_(1) {}
NetworkEmulationManager::~NetworkEmulationManager() {
  Stop();
}
...
CrossTrafficRoute* NetworkEmulationManager::CreateCrossTraffic(
    std::vector<EmulatedNetworkNode*> via_nodes) {
  RTC_CHECK(!via_nodes.empty());
  ...
  std::unique_ptr<CrossTrafficRoute> cross_traffic =
      absl::make_unique<CrossTrafficRoute>(time_controller_->clock(),
                                           via_nodes[0], endpoint);
  CrossTrafficRoute* out = cross_traffic.get();
...
RandomWalkCrossTraffic* NetworkEmulationManager::CreateRandomWalkCrossTraffic(
  ...
  return CreateRandomWalkCrossTraffic(cross_traffic, std::move(config),
                                      cross_traffic->GetDefaultDestination());
}
...
RandomWalkCrossTraffic* NetworkEmulationManager::CreateRandomWalkCrossTraffic(
  ...
  auto traffic = absl::make_unique<RandomWalkCrossTraffic>(
      std::move(config), cross_traffic, destination);
...
PulsedPeaksCrossTraffic* NetworkEmulationManager::CreatePulsedPeaksCrossTraffic(
  ...
  return CreatePulsedPeaksCrossTraffic(cross_traffic, std::move(config),
                                       cross_traffic->GetDefaultDestination());
}
...
PulsedPeaksCrossTraffic* NetworkEmulationManager::CreatePulsedPeaksCrossTraffic(
    ...
    PulsedPeaksConfig config,
    Destination destination) {
  auto traffic = absl::make_unique<PulsedPeaksCrossTraffic>(
      std::move(config), cross_traffic, destination);
