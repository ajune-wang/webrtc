...
void IOBuffer::Initialize(uint32_t expected) {
  if ((_pBuffer != NULL) || (_size != 0) || (_published != 0) ||
      (_consumed != 0))
...
bool IOBuffer::ReadFromTCPFd(SOCKET_TYPE fd,
                             uint32_t expected,
                             int32_t& recvAmount,
                             int& err) {
  ...
  if ((end - _lastInMs) > 100) {
    std::cout << "    IOBuffer::ReadFromTCPFd read=" << recvAmount
              << "  prev=" << (end - _lastInMs) << " ms" << std::endl;
  }
  _lastInMs = end;

  if ((end - start) > 10) {
    std::cout << "IOBuffer::ReadFromTCPFd interval=" << (end - start) << " ms "
              << std::endl;
  }
...
bool IOBuffer::WriteToTCPFd(SOCKET_TYPE fd,
                            uint32_t size,
                            int32_t& sentAmount,
                            int& err) {
  uint64_t start = GetTimeMillis();
  ...
  if (size == 0)
    return true;
  sentAmount =
      send(fd, (char*)(_pBuffer + _consumed),
  ...
  if (sentAmount < 0) {
    ...
    if ((err != SOCKET_ERROR_EAGAIN) && (err != SOCKET_ERROR_EINPROGRESS)) {
      printf(
          "Unable to send %u bytes of data data. Size advertised by network "
          "layer was %u. Permanent error (%d): %s",
  ...
  if ((end - _lastMs) > 100) {
    std::cout << "    IOBuffer::WriteToTCPFd sent=" << sentAmount
              << "  prev=" << (end - _lastMs) << " ms" << std::endl;
  }
  _lastMs = end;

  if ((end - start) > 10) {
    std::cout << "IOBuffer::WriteToTCPFd " << (end - start) << " ms  "
              << (send_end - start) << " ms" << std::endl;
  }
