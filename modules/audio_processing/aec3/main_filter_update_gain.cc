...
MainFilterUpdateGain::MainFilterUpdateGain(
    const EchoCanceller3Config::Filter& config,
    ...
    : data_dumper_(
      ...
      E2_shadow_smoother_(kFftLengthBy2Plus1,
                          config.num_error_comparison_blocks),
      E2_main_smoother_(kFftLengthBy2Plus1, config.num_error_comparison_blocks),
      convergence_threshold_(config.convergence_threshold) {
  SetConfig(config.main_initial, true);
