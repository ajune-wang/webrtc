...
// Coefficients for high-pass biquad filter created by
// b, a = signal.butter(2, 200/8000.0, 'highpass', analog=False).
...
// Cyclicly updates a pre-processed version of the time-domain adaptive filter.
FilterAnalyzer::FilterAnalyzer(const EchoCanceller3Config& config)
    : data_dumper_(
      ...
      h_cached_(GetTimeDomainLength(config.filter.main.length_blocks), 0.f),
...
void FilterAnalyzer::PreProcessFilter(
  ...
  // Handle any changes in the filter size.
  ...
  // Gradually increase the size of the processed filter until it matches the
  // size of the incoming filter.
  h_highpass_.resize(std::min(h_highpass_.size() + hp_analysis_step_size_,
                              filter_time_domain.size()));

  // Reset the pre-processing highpass filter when the analysis restarts at the
  // beginning of the filter.
  if (h_highpass_index_ == 0) {
    hp_filter_.Reset();
  }

  // Update another portion of the pre-processed adaptive filter.
  ...
  // Circularly update the index of the first sample of the next segment to
  // analyze in the adaptive filter.
  h_highpass_index_ += length;
  RTC_DCHECK_GE(h_highpass_.size(), h_highpass_index_);
  if (h_highpass_index_ == h_highpass_.size()) {
    h_highpass_index_ = 0;
  }
}
...
void FilterAnalyzer::Update(rtc::ArrayView<const float> filter_time_domain,
  ...
  if (use_preprocessed_filter_) {
    if (h_highpass_index_ == 0) {
      std::copy(filter_time_domain.begin(), filter_time_domain.end(),
                h_cached_.begin());
    }
    PreProcessFilter(h_cached_);
  }
  ...
  bool fully_preprocessed_filter =
      filter_to_analyze.size() == filter_time_domain.size();
  ...
  if (use_preprocessed_filter_) {
    // Store the preprocessed filter for analysis.
