...
TEST(RnnVadTest, LpResidualOfEmptyFrame) {
  ...
  MaybeComputeInverseFilterCoefficients({empty_frame},
...
// TODO(https://bugs.chromium.org/9076): Remove when the issue is fixed.
TEST(RnnVadTest, LpResidualPipelineBitExactness) {
  ...
  // Read ground-truth.
  auto lp_residual_reader = CreatePitchPeriodGainAndLpResidualReader();
