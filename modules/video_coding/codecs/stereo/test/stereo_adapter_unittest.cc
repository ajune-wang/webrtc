...
class TestStereoAdapter : public VideoCodecTest {
 public:
  TestStereoAdapter()
      : internal_decoder_factory_(new cricket::InternalDecoderFactory()),
        internal_encoder_factory_(new cricket::InternalEncoderFactory()) {}

 protected:
  VideoDecoder* CreateDecoder() override {
    return new StereoDecoderAdapter(new cricket::InternalDecoderFactory());
  }

  VideoEncoder* CreateEncoder() override {
    return new StereoEncoderAdapter(new cricket::InternalEncoderFactory());
  }
 ...
 private:
  const std::unique_ptr<cricket::WebRtcVideoDecoderFactory>
      internal_decoder_factory_;
  const std::unique_ptr<cricket::WebRtcVideoEncoderFactory>
