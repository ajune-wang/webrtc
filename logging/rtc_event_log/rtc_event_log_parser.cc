...
#ifndef RTC_PARSE_CHECK
#define RTC_PARSE_CHECK(X)                                                  \
  do {                                                                      \
    if (!(X))                                                               \
      return ParsedRtcEventLog::ParseStatus::Error(#X, __FILE__, __LINE__); \
  } while (0)
#define RTC_PARSE_CHECK_OP(OP, X, Y)                                    \
  do {                                                                  \
    if (!((X)OP(Y)))                                                    \
      return ParsedRtcEventLog::ParseStatus::Error(#X #OP #Y, __FILE__, \
                                                   __LINE__);           \
  } while (0)
#define RTC_PARSE_CHECK_EQ(X, Y) RTC_PARSE_CHECK_OP(==, X, Y)
#define RTC_PARSE_CHECK_NE(X, Y) RTC_PARSE_CHECK_OP(!=, X, Y)
#define RTC_PARSE_CHECK_LT(X, Y) RTC_PARSE_CHECK_OP(<, X, Y)
#define RTC_PARSE_CHECK_LE(X, Y) RTC_PARSE_CHECK_OP(<=, X, Y)
#define RTC_PARSE_CHECK_GT(X, Y) RTC_PARSE_CHECK_OP(>, X, Y)
#define RTC_PARSE_CHECK_GE(X, Y) RTC_PARSE_CHECK_OP(>=, X, Y)
#endif
...
ABSL_MUST_USE_RESULT ParsedRtcEventLog::ParseStatusOr<uint64_t> ParseVarInt(
  ...
  RTC_PARSE_CHECK(false);
}

ABSL_MUST_USE_RESULT ParsedRtcEventLog::ParseStatus GetHeaderExtensions(
    std::vector<RtpExtension>* header_extensions,
    const RepeatedPtrField<rtclog::RtpHeaderExtension>&
        proto_header_extensions) {
  header_extensions->clear();
  for (auto& p : proto_header_extensions) {
    RTC_PARSE_CHECK(p.has_name());
    RTC_PARSE_CHECK(p.has_id());
...
ABSL_MUST_USE_RESULT ParsedRtcEventLog::ParseStatus StoreRtpPackets(
    const ProtoType& proto,
    std::map<uint32_t, std::vector<LoggedType>>* rtp_packets_map) {
  RTC_PARSE_CHECK(proto.has_timestamp_ms());
  RTC_PARSE_CHECK(proto.has_marker());
  RTC_PARSE_CHECK(proto.has_payload_type());
  RTC_PARSE_CHECK(proto.has_sequence_number());
  RTC_PARSE_CHECK(proto.has_rtp_timestamp());
  RTC_PARSE_CHECK(proto.has_ssrc());
  RTC_PARSE_CHECK(proto.has_payload_size());
  RTC_PARSE_CHECK(proto.has_header_size());
  RTC_PARSE_CHECK(proto.has_padding_size());
  ...
  // Base event
  {
    ...
    if (proto.has_audio_level()) {
      RTC_PARSE_CHECK(proto.has_voice_activity());
      ...
      RTC_PARSE_CHECK_LE(audio_level, 0x7Fu);
  ...
  RTC_PARSE_CHECK_EQ(timestamp_ms_values.size(), number_of_deltas);

  // marker (RTP base)
  std::vector<absl::optional<uint64_t>> marker_values =
      DecodeDeltas(proto.marker_deltas(), proto.marker(), number_of_deltas);
  RTC_PARSE_CHECK_EQ(marker_values.size(), number_of_deltas);
  ...
  RTC_PARSE_CHECK_EQ(payload_type_values.size(), number_of_deltas);
  ...
  std::vector<absl::optional<uint64_t>> sequence_number_values =
      DecodeDeltas(proto.sequence_number_deltas(), proto.sequence_number(),
                   number_of_deltas);
  RTC_PARSE_CHECK_EQ(sequence_number_values.size(), number_of_deltas);
  ...
  RTC_PARSE_CHECK_EQ(rtp_timestamp_values.size(), number_of_deltas);

  // ssrc (RTP base)
  std::vector<absl::optional<uint64_t>> ssrc_values =
      DecodeDeltas(proto.ssrc_deltas(), proto.ssrc(), number_of_deltas);
  RTC_PARSE_CHECK_EQ(ssrc_values.size(), number_of_deltas);
  ...
  RTC_PARSE_CHECK_EQ(payload_size_values.size(), number_of_deltas);
  ...
  RTC_PARSE_CHECK_EQ(header_size_values.size(), number_of_deltas);
  ...
  RTC_PARSE_CHECK_EQ(padding_size_values.size(), number_of_deltas);
  ...
  std::vector<absl::optional<uint64_t>> transport_sequence_number_values;
  {
    ...
    RTC_PARSE_CHECK_EQ(transport_sequence_number_values.size(),
                       number_of_deltas);
  }
  ...
  std::vector<absl::optional<uint64_t>> transmission_time_offset_values;
  {
    ...
    RTC_PARSE_CHECK_EQ(transmission_time_offset_values.size(),
                       number_of_deltas);
  }
  ...
  std::vector<absl::optional<uint64_t>> absolute_send_time_values;
  {
    ...
    absolute_send_time_values =
        DecodeDeltas(proto.absolute_send_time_deltas(), base_absolute_send_time,
                     number_of_deltas);
    RTC_PARSE_CHECK_EQ(absolute_send_time_values.size(), number_of_deltas);
  }
  ...
  std::vector<absl::optional<uint64_t>> video_rotation_values;
  {
    ...
    RTC_PARSE_CHECK_EQ(video_rotation_values.size(), number_of_deltas);
  }
  ...
  std::vector<absl::optional<uint64_t>> audio_level_values;
  {
    ...
    RTC_PARSE_CHECK_EQ(audio_level_values.size(), number_of_deltas);
  }
  ...
  std::vector<absl::optional<uint64_t>> voice_activity_values;
  {
    ...
    RTC_PARSE_CHECK_EQ(voice_activity_values.size(), number_of_deltas);
  }

  // Delta decoding
  for (size_t i = 0; i < number_of_deltas; ++i) {
    RTC_PARSE_CHECK(timestamp_ms_values[i].has_value());
    RTC_PARSE_CHECK(marker_values[i].has_value());
    RTC_PARSE_CHECK(payload_type_values[i].has_value());
    RTC_PARSE_CHECK(sequence_number_values[i].has_value());
    RTC_PARSE_CHECK(rtp_timestamp_values[i].has_value());
    RTC_PARSE_CHECK(ssrc_values[i].has_value());
    RTC_PARSE_CHECK(payload_size_values[i].has_value());
    RTC_PARSE_CHECK(header_size_values[i].has_value());
    RTC_PARSE_CHECK(padding_size_values[i].has_value());

    int64_t timestamp_ms;
    RTC_PARSE_CHECK(ToSigned(timestamp_ms_values[i].value(), &timestamp_ms));
    ...
    if (transmission_time_offset_values.size() > i &&
      ...
      RTC_PARSE_CHECK(ToSigned(transmission_time_offset_values[i].value(),
    ...
    if (audio_level_values.size() > i && audio_level_values[i].has_value()) {
      RTC_PARSE_CHECK(voice_activity_values.size() > i &&
...
template <typename ProtoType, typename LoggedType>
ABSL_MUST_USE_RESULT ParsedRtcEventLog::ParseStatus StoreRtcpPackets(
    const ProtoType& proto,
    std::vector<LoggedType>* rtcp_packets,
    bool remove_duplicates) {
  RTC_PARSE_CHECK(proto.has_timestamp_ms());
  RTC_PARSE_CHECK(proto.has_raw_packet());
  ...
  // raw_packet
  RTC_PARSE_CHECK(proto.has_raw_packet_blobs());
  std::vector<absl::string_view> raw_packet_values =
      DecodeBlobs(proto.raw_packet_blobs(), number_of_deltas);
  RTC_PARSE_CHECK_EQ(raw_packet_values.size(), number_of_deltas);
...
ABSL_MUST_USE_RESULT ParsedRtcEventLog::ParseStatus StoreRtcpBlocks(
  ...
  for (const uint8_t* block = packet_begin; block < packet_end;
    ...
    RTC_PARSE_CHECK(header.Parse(block, packet_end - block));
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::ParseFile(
  ...
  if (!file.good() || !file.is_open()) {
    ...
    RTC_PARSE_CHECK(file.good() && file.is_open());
  }
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::ParseStreamInternal(
  ...
  while (1) {
    ...
    if (!tag.ok()) {
      ...
      if (allow_incomplete_logs_) {
        RTC_LOG(LS_WARNING) << "Could not parse the entire log. Only the "
    ...
    if (wire_type != 2) {
      ...
      RTC_PARSE_CHECK_EQ(wire_type, 2);
    }
    ...
    if (!message_length.ok()) {
      ...
      RTC_PARSE_CHECK_LE(message_length.value(), kMaxEventSize);
    }
    ...
    if (stream.gcount() != static_cast<int>(message_length.value())) {
      ...
      if (allow_incomplete_logs_) {
        RTC_LOG(LS_ERROR) << "Could not parse the entire log. Only the "
                             "beginning will be used.";
        return ParseStatus::Success();
      }
      RTC_PARSE_CHECK(false);
    ...
    if (tag.value() == kExpectedV1Tag) {
      ...
      RTC_PARSE_CHECK_EQ(event_stream.stream_size(), 1);
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::StoreParsedLegacyEvent(
  ...
  RTC_PARSE_CHECK(event.has_type());
  RTC_PARSE_CHECK(event.has_type());
  switch (event.type()) {
    ...
    case rtclog::Event::RTCP_EVENT: {
      ...
      RTC_PARSE_CHECK(event.has_timestamp_us());
      int64_t timestamp_us = event.timestamp_us();
      RTC_PARSE_CHECK_LE(total_length, IP_PACKET_SIZE);
      if (direction == kIncomingPacket) {
    ...
    case rtclog::Event::BWE_PROBE_RESULT_EVENT: {
      ...
      RTC_PARSE_CHECK(event.has_probe_result());
      RTC_PARSE_CHECK(event.probe_result().has_result());
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::GetRtpHeader(
  ...
  RTC_PARSE_CHECK(event.has_type());
  RTC_PARSE_CHECK_EQ(event.type(), rtclog::Event::RTP_EVENT);
  RTC_PARSE_CHECK(event.has_rtp_packet());
  const rtclog::RtpPacket& rtp_packet = event.rtp_packet();
  // Get direction of packet.
  RTC_PARSE_CHECK(rtp_packet.has_incoming());
  if (incoming != nullptr) {
    *incoming = rtp_packet.incoming() ? kIncomingPacket : kOutgoingPacket;
  }
  // Get packet length.
  RTC_PARSE_CHECK(rtp_packet.has_packet_length());
  if (total_length != nullptr) {
    *total_length = rtp_packet.packet_length();
  }
  // Get header length.
  RTC_PARSE_CHECK(rtp_packet.has_header());
  ...
  if (probe_cluster_id != nullptr) {
    if (rtp_packet.has_probe_cluster_id()) {
      *probe_cluster_id = rtp_packet.probe_cluster_id();
      RTC_PARSE_CHECK_NE(*probe_cluster_id, PacedPacketInfo::kNotAProbe);
    } else {
  ...
  if (header != nullptr) {
    ...
    RTC_PARSE_CHECK_GE(rtp_packet.header().size(), kMinRtpHeaderSize);
    RTC_PARSE_CHECK_LE(rtp_packet.header().size(),
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::GetRtcpPacket(
    ...
    size_t* length) const {
  RTC_PARSE_CHECK(event.has_type());
  RTC_PARSE_CHECK_EQ(event.type(), rtclog::Event::RTCP_EVENT);
  RTC_PARSE_CHECK(event.has_rtcp_packet());
  const rtclog::RtcpPacket& rtcp_packet = event.rtcp_packet();
  // Get direction of packet.
  RTC_PARSE_CHECK(rtcp_packet.has_incoming());
  if (incoming != nullptr) {
    *incoming = rtcp_packet.incoming() ? kIncomingPacket : kOutgoingPacket;
  }
  // Get packet length.
  RTC_PARSE_CHECK(rtcp_packet.has_packet_data());
  if (length != nullptr) {
    *length = rtcp_packet.packet_data().size();
  }
  // Get packet contents.
  if (packet != nullptr) {
    RTC_PARSE_CHECK_LE(rtcp_packet.packet_data().size(),
...
ParsedRtcEventLog::GetVideoReceiveConfig(const rtclog::Event& event) const {
  ...
  RTC_PARSE_CHECK(event.has_type());
  RTC_PARSE_CHECK_EQ(event.type(), rtclog::Event::VIDEO_RECEIVER_CONFIG_EVENT);
  RTC_PARSE_CHECK(event.has_video_receiver_config());
  const rtclog::VideoReceiveConfig& receiver_config =
      event.video_receiver_config();
  // Get SSRCs.
  RTC_PARSE_CHECK(receiver_config.has_remote_ssrc());
  config.remote_ssrc = receiver_config.remote_ssrc();
  RTC_PARSE_CHECK(receiver_config.has_local_ssrc());
  config.local_ssrc = receiver_config.local_ssrc();
  config.rtx_ssrc = 0;
  // Get RTCP settings.
  RTC_PARSE_CHECK(receiver_config.has_rtcp_mode());
  config.rtcp_mode = GetRuntimeRtcpMode(receiver_config.rtcp_mode());
  RTC_PARSE_CHECK(receiver_config.has_remb());
  ...
  for (int i = 0; i < receiver_config.rtx_map_size(); i++) {
    ...
    RTC_PARSE_CHECK(map.has_payload_type());
    RTC_PARSE_CHECK(map.has_config());
    RTC_PARSE_CHECK(map.config().has_rtx_ssrc());
    RTC_PARSE_CHECK(map.config().has_rtx_payload_type());
  ...
  for (int i = 0; i < receiver_config.decoders_size(); i++) {
    RTC_PARSE_CHECK(receiver_config.decoders(i).has_name());
    RTC_PARSE_CHECK(receiver_config.decoders(i).has_payload_type());
...
ParsedRtcEventLog::GetVideoSendConfig(const rtclog::Event& event) const {
  rtclog::StreamConfig config;
  RTC_PARSE_CHECK(event.has_type());
  RTC_PARSE_CHECK_EQ(event.type(), rtclog::Event::VIDEO_SENDER_CONFIG_EVENT);
  RTC_PARSE_CHECK(event.has_video_sender_config());
  ...
  // WebRTC version.
  RTC_PARSE_CHECK_EQ(sender_config.ssrcs_size(), 1);
  config.local_ssrc = sender_config.ssrcs(0);
  RTC_PARSE_CHECK_LE(sender_config.rtx_ssrcs_size(), 1);
  ...
  // Get the codec.
  RTC_PARSE_CHECK(sender_config.has_encoder());
  RTC_PARSE_CHECK(sender_config.encoder().has_name());
  RTC_PARSE_CHECK(sender_config.encoder().has_payload_type());
...
ParsedRtcEventLog::GetAudioReceiveConfig(const rtclog::Event& event) const {
  rtclog::StreamConfig config;
  RTC_PARSE_CHECK(event.has_type());
  RTC_PARSE_CHECK_EQ(event.type(), rtclog::Event::AUDIO_RECEIVER_CONFIG_EVENT);
  RTC_PARSE_CHECK(event.has_audio_receiver_config());
...
ParsedRtcEventLog::GetAudioSendConfig(const rtclog::Event& event) const {
  rtclog::StreamConfig config;
  RTC_PARSE_CHECK(event.has_type());
  RTC_PARSE_CHECK_EQ(event.type(), rtclog::Event::AUDIO_SENDER_CONFIG_EVENT);
  RTC_PARSE_CHECK(event.has_audio_sender_config());
  const rtclog::AudioSendConfig& sender_config = event.audio_sender_config();
  // Get SSRCs.
  RTC_PARSE_CHECK(sender_config.has_ssrc());
...
ParsedRtcEventLog::GetAudioPlayout(const rtclog::Event& event) const {
  RTC_PARSE_CHECK(event.has_type());
  RTC_PARSE_CHECK_EQ(event.type(), rtclog::Event::AUDIO_PLAYOUT_EVENT);
  RTC_PARSE_CHECK(event.has_audio_playout_event());
  const rtclog::AudioPlayoutEvent& playout_event = event.audio_playout_event();
  LoggedAudioPlayoutEvent res;
  RTC_PARSE_CHECK(event.has_timestamp_us());
  res.timestamp_us = event.timestamp_us();
  RTC_PARSE_CHECK(playout_event.has_local_ssrc());
...
ParsedRtcEventLog::GetLossBasedBweUpdate(const rtclog::Event& event) const {
  RTC_PARSE_CHECK(event.has_type());
  RTC_PARSE_CHECK_EQ(event.type(), rtclog::Event::LOSS_BASED_BWE_UPDATE);
  RTC_PARSE_CHECK(event.has_loss_based_bwe_update());
  ...
  RTC_PARSE_CHECK(loss_event.has_bitrate_bps());
  bwe_update.bitrate_bps = loss_event.bitrate_bps();
  RTC_PARSE_CHECK(loss_event.has_fraction_loss());
  bwe_update.fraction_lost = loss_event.fraction_loss();
  RTC_PARSE_CHECK(loss_event.has_total_packets());
...
ParsedRtcEventLog::GetDelayBasedBweUpdate(const rtclog::Event& event) const {
  RTC_PARSE_CHECK(event.has_type());
  RTC_PARSE_CHECK_EQ(event.type(), rtclog::Event::DELAY_BASED_BWE_UPDATE);
  RTC_PARSE_CHECK(event.has_delay_based_bwe_update());
  ...
  RTC_PARSE_CHECK(event.has_timestamp_us());
  res.timestamp_us = event.timestamp_us();
  RTC_PARSE_CHECK(delay_event.has_bitrate_bps());
  res.bitrate_bps = delay_event.bitrate_bps();
  RTC_PARSE_CHECK(delay_event.has_detector_state());
...
ParsedRtcEventLog::GetAudioNetworkAdaptation(const rtclog::Event& event) const {
  RTC_PARSE_CHECK(event.has_type());
  RTC_PARSE_CHECK_EQ(event.type(),
                     rtclog::Event::AUDIO_NETWORK_ADAPTATION_EVENT);
  RTC_PARSE_CHECK(event.has_audio_network_adaptation());
...
ParsedRtcEventLog::GetBweProbeClusterCreated(const rtclog::Event& event) const {
  ...
  RTC_PARSE_CHECK(event.has_probe_cluster());
  const rtclog::BweProbeCluster& pcc_event = event.probe_cluster();
  LoggedBweProbeClusterCreatedEvent res;
  RTC_PARSE_CHECK(event.has_timestamp_us());
  res.timestamp_us = event.timestamp_us();
  RTC_PARSE_CHECK(pcc_event.has_id());
  res.id = pcc_event.id();
  RTC_PARSE_CHECK(pcc_event.has_bitrate_bps());
  res.bitrate_bps = pcc_event.bitrate_bps();
  RTC_PARSE_CHECK(pcc_event.has_min_packets());
  res.min_packets = pcc_event.min_packets();
  RTC_PARSE_CHECK(pcc_event.has_min_bytes());
...
ParsedRtcEventLog::GetBweProbeFailure(const rtclog::Event& event) const {
  RTC_PARSE_CHECK(event.has_type());
  RTC_PARSE_CHECK_EQ(event.type(), rtclog::Event::BWE_PROBE_RESULT_EVENT);
  RTC_PARSE_CHECK(event.has_probe_result());
  const rtclog::BweProbeResult& pr_event = event.probe_result();
  RTC_PARSE_CHECK(pr_event.has_result());
  RTC_PARSE_CHECK_NE(pr_event.result(), rtclog::BweProbeResult::SUCCESS);

  LoggedBweProbeFailureEvent res;
  RTC_PARSE_CHECK(event.has_timestamp_us());
  res.timestamp_us = event.timestamp_us();
  RTC_PARSE_CHECK(pr_event.has_id());
  res.id = pr_event.id();
  RTC_PARSE_CHECK(pr_event.has_result());
  if (pr_event.result() ==
  ...
  RTC_PARSE_CHECK(!pr_event.has_bitrate_bps());
...
ParsedRtcEventLog::GetBweProbeSuccess(const rtclog::Event& event) const {
  ...
  const rtclog::BweProbeResult& pr_event = event.probe_result();
  RTC_PARSE_CHECK(pr_event.has_result());
  RTC_PARSE_CHECK_EQ(pr_event.result(), rtclog::BweProbeResult::SUCCESS);
...
ParsedRtcEventLog::GetAlrState(const rtclog::Event& event) const {
  RTC_PARSE_CHECK(event.has_type());
  RTC_PARSE_CHECK_EQ(event.type(), rtclog::Event::ALR_STATE_EVENT);
  RTC_PARSE_CHECK(event.has_alr_state());
  const rtclog::AlrState& alr_event = event.alr_state();
  LoggedAlrStateEvent res;
  RTC_PARSE_CHECK(event.has_timestamp_us());
  res.timestamp_us = event.timestamp_us();
  RTC_PARSE_CHECK(alr_event.has_in_alr());
...
ParsedRtcEventLog::GetIceCandidatePairConfig(
  ...
  RTC_PARSE_CHECK(rtc_event.has_type());
  RTC_PARSE_CHECK_EQ(rtc_event.type(),
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::StoreAlrStateEvent(
  ...
  RTC_PARSE_CHECK(proto.has_timestamp_ms());
  RTC_PARSE_CHECK(proto.has_in_alr());
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::StoreRouteChangeEvent(
  ...
  RTC_PARSE_CHECK(proto.has_timestamp_ms());
  RTC_PARSE_CHECK(proto.has_connected());
  RTC_PARSE_CHECK(proto.has_overhead());
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::StoreRemoteEstimateEvent(
  ...
  RTC_PARSE_CHECK_EQ(link_capacity_lower_kbps_values.size(), number_of_deltas);
  ...
  RTC_PARSE_CHECK_EQ(link_capacity_upper_kbps_values.size(), number_of_deltas);
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::StoreAudioPlayoutEvent(
  ...
  RTC_PARSE_CHECK(proto.has_timestamp_ms());
  RTC_PARSE_CHECK(proto.has_local_ssrc());
  ...
  RTC_PARSE_CHECK_EQ(local_ssrc_values.size(), number_of_deltas);

  // Delta decoding
  for (size_t i = 0; i < number_of_deltas; ++i) {
    RTC_PARSE_CHECK(timestamp_ms_values[i].has_value());
    RTC_PARSE_CHECK(local_ssrc_values[i].has_value());
    RTC_PARSE_CHECK_LE(local_ssrc_values[i].value(),
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::StoreStartEvent(
  ...
  RTC_PARSE_CHECK(proto.has_timestamp_ms());
  RTC_PARSE_CHECK(proto.has_version());
  RTC_PARSE_CHECK(proto.has_utc_time_ms());
  RTC_PARSE_CHECK_EQ(proto.version(), 2);
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::StoreBweLossBasedUpdate(
  ...
  RTC_PARSE_CHECK(proto.has_timestamp_ms());
  RTC_PARSE_CHECK(proto.has_bitrate_bps());
  RTC_PARSE_CHECK(proto.has_fraction_loss());
  RTC_PARSE_CHECK(proto.has_total_packets());
  ...
  RTC_PARSE_CHECK_EQ(bitrate_bps_values.size(), number_of_deltas);
  ...
  RTC_PARSE_CHECK_EQ(fraction_loss_values.size(), number_of_deltas);
  ...
  RTC_PARSE_CHECK_EQ(total_packets_values.size(), number_of_deltas);
  ...
  for (size_t i = 0; i < number_of_deltas; ++i) {
    ...
    RTC_PARSE_CHECK(bitrate_bps_values[i].has_value());
    RTC_PARSE_CHECK_LE(bitrate_bps_values[i].value(),
                       std::numeric_limits<uint32_t>::max());
    const uint32_t bitrate_bps =
        static_cast<uint32_t>(bitrate_bps_values[i].value());

    RTC_PARSE_CHECK(fraction_loss_values[i].has_value());
    RTC_PARSE_CHECK_LE(fraction_loss_values[i].value(),
                       std::numeric_limits<uint32_t>::max());
    const uint32_t fraction_loss =
        static_cast<uint32_t>(fraction_loss_values[i].value());

    RTC_PARSE_CHECK(total_packets_values[i].has_value());
    RTC_PARSE_CHECK_LE(total_packets_values[i].value(),
                       std::numeric_limits<uint32_t>::max());
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::StoreBweDelayBasedUpdate(
  ...
  RTC_PARSE_CHECK(proto.has_timestamp_ms());
  RTC_PARSE_CHECK(proto.has_bitrate_bps());
  RTC_PARSE_CHECK(proto.has_detector_state());
  ...
  RTC_PARSE_CHECK_EQ(detector_state_values.size(), number_of_deltas);
  ...
  for (size_t i = 0; i < number_of_deltas; ++i) {
    ...
    RTC_PARSE_CHECK(detector_state_values[i].has_value());
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::StoreBweProbeFailureEvent(
  ...
  RTC_PARSE_CHECK(proto.has_id());
  probe_result.id = proto.id();
  RTC_PARSE_CHECK(proto.has_failure());
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::StoreGenericAckReceivedEvent(
  ...
  RTC_PARSE_CHECK(proto.has_timestamp_ms());
  RTC_PARSE_CHECK(proto.has_packet_number());
  RTC_PARSE_CHECK(proto.has_acked_packet_number());
  ...
  RTC_PARSE_CHECK_EQ(packet_number_values.size(), number_of_deltas);
  ...
  RTC_PARSE_CHECK_EQ(acked_packet_number_values.size(), number_of_deltas);
  ...
  std::vector<absl::optional<uint64_t>> receive_acked_packet_time_ms_values =
      DecodeDeltas(proto.receive_acked_packet_time_ms_deltas(),
                   ...
                   number_of_deltas);
  RTC_PARSE_CHECK_EQ(receive_acked_packet_time_ms_values.size(),
                     number_of_deltas);
  ...
  for (size_t i = 0; i < number_of_deltas; i++) {
    ...
    RTC_PARSE_CHECK(ToSigned(packet_number_values[i].value(), &packet_number));
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::StoreGenericPacketSentEvent(
  ...
  // Base event
  RTC_PARSE_CHECK(proto.has_packet_number());
  RTC_PARSE_CHECK(proto.has_overhead_length());
  RTC_PARSE_CHECK(proto.has_payload_length());
  RTC_PARSE_CHECK(proto.has_padding_length());
  ...
  std::vector<absl::optional<uint64_t>> overhead_length_values =
      DecodeDeltas(proto.overhead_length_deltas(), proto.overhead_length(),
                   number_of_deltas);
  RTC_PARSE_CHECK_EQ(overhead_length_values.size(), number_of_deltas);
  ...
  RTC_PARSE_CHECK_EQ(payload_length_values.size(), number_of_deltas);
  ...
  RTC_PARSE_CHECK_EQ(padding_length_values.size(), number_of_deltas);
  ...
  for (size_t i = 0; i < number_of_deltas; i++) {
    ...
    int64_t packet_number;
    RTC_PARSE_CHECK(ToSigned(packet_number_values[i].value(), &packet_number));
    RTC_PARSE_CHECK(overhead_length_values[i].has_value());
    RTC_PARSE_CHECK(payload_length_values[i].has_value());
    RTC_PARSE_CHECK(padding_length_values[i].has_value());
    generic_packets_sent_.push_back(
...
ParsedRtcEventLog::StoreGenericPacketReceivedEvent(
  ...
  // Base event
  RTC_PARSE_CHECK(proto.has_packet_number());
  RTC_PARSE_CHECK(proto.has_packet_length());
  ...
  RTC_PARSE_CHECK_EQ(packet_length_values.size(), number_of_deltas);
  ...
  for (size_t i = 0; i < number_of_deltas; i++) {
    ...
    RTC_PARSE_CHECK(ToSigned(packet_length_values[i].value(), &packet_length));
...
ParsedRtcEventLog::StoreAudioNetworkAdaptationEvent(
  ...
  // Base event
  {
    ...
    if (proto.has_uplink_packet_loss_fraction()) {
      ...
      RTC_PARSE_CHECK(ParsePacketLossFractionFromProtoFormat(
  ...
  RTC_PARSE_CHECK_EQ(frame_length_ms_values.size(), number_of_deltas);
  ...
  RTC_PARSE_CHECK_EQ(uplink_packet_loss_fraction_values.size(),
                     number_of_deltas);
  ...
  RTC_PARSE_CHECK_EQ(enable_fec_values.size(), number_of_deltas);
  ...
  RTC_PARSE_CHECK_EQ(enable_dtx_values.size(), number_of_deltas);
  ...
  RTC_PARSE_CHECK_EQ(num_channels_values.size(), number_of_deltas);
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::StoreDtlsTransportState(
  ...
  RTC_PARSE_CHECK(proto.has_dtls_transport_state());
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::StoreDtlsWritableState(
  ...
  RTC_PARSE_CHECK(proto.has_writable());
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::StoreIceCandidateEvent(
  ...
  RTC_PARSE_CHECK(proto.has_event_type());
...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::StoreVideoRecvConfig(
  ...
  RTC_PARSE_CHECK(proto.has_remote_ssrc());
