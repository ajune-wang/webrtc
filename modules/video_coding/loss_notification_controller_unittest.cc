 ...
 protected:
  ...
  void OnReceivedPacket(const Packet& packet) {
    ...
    if (packet.first_in_frame) {
      ...
      frame_details.emplace();
      frame_details->is_keyframe = packet.is_keyframe;
      frame_details->frame_id = packet.frame_id;
      frame_details->frame_dependencies = packet.frame_dependencies;
    }

    uut_.OnReceivedPacket(packet.seq_num, frame_details);
  }
