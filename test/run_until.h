...
#ifndef TEST_RUN_UNTIL_H_
#define TEST_RUN_UNTIL_H_
...
#include "test/run_until_internal.h"  // IWYU pragma: private

namespace webrtc {

struct RunUntilSettings {
  ...
  absl::Nonnull<Clock*> clock = Clock::GetRealTimeClock();
...
auto RunUntil(const Fn& fn, Matcher matcher, RunUntilSettings settings = {})
    -> RTCErrorOr<decltype(fn())> {
  RTC_CHECK(rtc::Thread::Current())
      << "A current thread is required. An rtc::AutoThread can work for tests.";
  Timestamp start = settings.clock->CurrentTime();
  ...
  } while (settings.clock->CurrentTime() < start + settings.timeout);

  // One more try after the last sleep. This failure will contain the error
  // message.
  auto result = fn();
  testing::StringMatchResultListener listener;
  if (run_until_internal::ExplainMatchResult(matcher, result, &listener,
