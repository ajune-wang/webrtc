...
static absl::optional<VideoEncoder::ResolutionBitrateThresholds>
GetEncoderBitrateLimits(const VideoEncoder::EncoderInfo& encoder_info,
                        int frame_size_pixels) {
  for (size_t i = 0; i < encoder_info.resolution_bitrate_thresholds.size();
       ++i) {
    if (i > 0) {
      RTC_DCHECK_GT(
          encoder_info.resolution_bitrate_thresholds[i].frame_size_pixels,
          encoder_info.resolution_bitrate_thresholds[i - 1].frame_size_pixels);
    }

    if (encoder_info.resolution_bitrate_thresholds[i].frame_size_pixels >=
        frame_size_pixels) {
      return absl::optional<VideoEncoder::ResolutionBitrateThresholds>(
          encoder_info.resolution_bitrate_thresholds[i]);
    }
  }
...
void VideoStreamEncoder::ReconfigureEncoder() {
  ...
  if (encoder_bitrate_limits_ && encoder_config_.max_bitrate_bps <= 0 &&
      streams.size() == 1) {
