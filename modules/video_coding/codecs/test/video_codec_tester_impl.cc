...
std::unique_ptr<VideoCodecTestStats> VideoCodecTesterImpl::RunEncodeDecodeTest(
  ...
  FrameReaderLocked frame_reader_locked(std::move(frame_reader));
  VideoCodecAnalyser perf_analyser(&frame_reader_locked);
  ...
  while (auto frame = frame_reader_locked.PullFrame()) {
