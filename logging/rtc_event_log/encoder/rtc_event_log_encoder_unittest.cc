...
class RtcEventLogEncoderTest : public testing::TestWithParam<int> {
 protected:
  ...
  std::pair<RtpExtension, RtpExtension> RandomRtpExtension() {
    RTC_DCHECK(arraysize(kExtensions) >= 2);
    size_t id_1 = prng_.Rand(0, arraysize(kExtensions) - 1);
    size_t id_2 = prng_.Rand(0, arraysize(kExtensions) - 2);
    if (id_2 == id_1)
      id_2 = arraysize(kExtensions) - 2;
...
TEST_P(RtcEventLogEncoderTest, RtcEventAudioReceiveStreamConfig) {
  ...
  std::pair<RtpExtension, RtpExtension> extensions = RandomRtpExtension();
  stream_config->rtp_extensions.push_back(extensions.first);
  stream_config->rtp_extensions.push_back(extensions.second);
