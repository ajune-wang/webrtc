...
TaskQueue::TaskQueue(const char* queue_name, Priority priority)
    ...
    // fact that both classes are wrappers around a raw pointer.
...
TaskQueue::~TaskQueue() {
  // TODO(danilchap): change impl_ to webrtc::TaskQueueBase* when all
  // implementations derive from that interface and thus do not
  ...
  webrtc::TaskQueueDeleter deleter;
  deleter(static_cast<webrtc::TaskQueueBase*>(impl_.get()));
