...
BbrNetworkController::BbrNetworkController()
    : random_(10),
      ...
      congestion_window_gain_constant_(
          static_cast<double>(kProbeBWCongestionWindowGain)),
      rtt_variance_weight_(
          static_cast<double>(FLAGS_quic_bbr_rtt_variation_weight)),
...
TimeDelta BbrNetworkController::GetProcessInterval() {
...
void BbrNetworkController::OnTransportPacketsFeedback(
  ...
  auto last_sent_packet = msg.PacketsWithFeedback().back().sent_packet;
  ...
  auto acked_packets = msg.ReceivedWithSendInfo();
  auto lost_packets = msg.LostWithSendInfo();
