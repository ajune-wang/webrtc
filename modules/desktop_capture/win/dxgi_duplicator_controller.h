...
// Frames passed in to the Duplicate methods may be held onto by the underlying
// DxgiOutputDuplicator so that they can guarantee delivery of new frames when
// requested; since if there are no updates, they may be unable to capture a
// frame.
class RTC_EXPORT DxgiDuplicatorController {
 public:
  ...
  // this function returns false. May return a reference to `frame`'s underlying
  // |SharedDesktopFrame|.
