...
// at least every |kMinTimeBetweenStatsUpdates| as long as the pacer isn't
// completely drained.
constexpr TimeDelta kMinTimeBetweenStatsUpdates = TimeDelta::Millis<33>();
// Don't call UpdateStats() more than |kMaxTimeBetweenStatsUpdates| apart,
// for performance reasons.
constexpr TimeDelta kMaxTimeBetweenStatsUpdates = TimeDelta::Millis<1>();
}  // namespace
...
void TaskQueuePacedSender::MaybeProcessPackets(
  ...
  // now scheduled task and we need to process immediately.
  ...
  Timestamp next_process_time = std::max(now, next_send_time);
...
void TaskQueuePacedSender::MaybseUpdateStats(bool is_scheduled_call) {
  if (is_shutdown_) {
    return;
  }
  ...
  if (!is_scheduled_call &&
      now - last_stats_time_ < kMaxTimeBetweenStatsUpdates) {
  ...
  if (!pacer_drained && (is_scheduled_call || !stats_update_scheduled_)) {
    task_queue_.PostDelayedTask(
        [this]() {
          RTC_DCHECK_RUN_ON(&task_queue_);
          MaybseUpdateStats(true);
        },
        kMinTimeBetweenStatsUpdates.ms<uint32_t>());
  } else {
    stats_update_scheduled_ = false;
  }
}
