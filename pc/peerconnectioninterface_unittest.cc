...
// Test creating senders without a connected PC.
...
TEST_F(PeerConnectionInterfaceTest, GetRTCStatsAfterCalling) {
  InitiateCall();
  EXPECT_TRUE(DoGetRTCStats());
}

TEST_F(PeerConnectionInterfaceTest, GetRTCStatsBeforeCalling) {
  CreatePeerConnectionWithoutDtls();
  // EXPECT_TRUE(DoGetRTCStats());
  pc_->ClearStatsCache();
  AddAudioVideoStream(kStreamLabel1, "audio_track", "video_track");
