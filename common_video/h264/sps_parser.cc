...
absl::optional<SpsParser::SpsState> SpsParser::ParseSps(const uint8_t* data,
  ...
  BitReader bit_buffer(unpacked_buffer);
...
absl::optional<SpsParser::SpsState> SpsParser::ParseSpsUpToVui(
  ...
  absl::optional<SpsState> sps(absl::in_place);
  ...
  // formats.
  uint8_t profile_idc = bit_reader.Read<uint8_t>();
  // constraint_set0_flag through constraint_set5_flag + reserved_zero_2bits
  // 1 bit each for the flags + 2 bits = 8 bits.
  bit_reader.ConsumeBits(8);
  // level_idc: u(8)
  bit_reader.ConsumeBits(8);
  // seq_parameter_set_id: ue(v)
  sps->id = bit_reader.ReadExponentialGolomb();
  ...
  if (profile_idc == 100 || profile_idc == 110 || profile_idc == 122 ||
    ...
    chroma_format_idc = bit_reader.ReadExponentialGolomb();
    if (chroma_format_idc == 3) {
      // separate_colour_plane_flag: u(1)
      sps->separate_colour_plane_flag = bit_reader.ReadBit();
    }
    ...
    bool seq_scaling_matrix_present_flag = bit_reader.Read<bool>();
    if (seq_scaling_matrix_present_flag) {
      ...
      for (int i = 0; i < scaling_list_count; ++i) {
        ...
        bool seq_scaling_list_present_flags = bit_reader.Read<bool>();
        if (seq_scaling_list_present_flags) {
          int last_scale = 8;
          int next_scale = 8;
          ...
          for (int j = 0; j < size_of_scaling_list; j++) {
            if (next_scale != 0) {
              // delta_scale: se(v)
              int32_t delta_scale = bit_reader.ReadSignedExponentialGolomb();
  ...
  uint32_t log2_max_frame_num_minus4 = bit_reader.ReadExponentialGolomb();
  ...
  sps->pic_order_cnt_type = bit_reader.ReadExponentialGolomb();
  if (sps->pic_order_cnt_type == 0) {
    ...
    sps->delta_pic_order_always_zero_flag = bit_reader.ReadBit();
  ...
  sps->max_num_ref_frames = bit_reader.ReadExponentialGolomb();
  ...
  uint32_t pic_width_in_mbs_minus1 = bit_reader.ReadExponentialGolomb();
  // pic_height_in_map_units_minus1: ue(v)
  uint32_t pic_height_in_map_units_minus1 = bit_reader.ReadExponentialGolomb();
  // frame_mbs_only_flag: u(1)
  sps->frame_mbs_only_flag = bit_reader.ReadBit();
  ...
  if (bool frame_cropping_flag = bit_reader.Read<bool>()) {
    // frame_crop_{left, right, top, bottom}_offset: ue(v)
    frame_crop_left_offset = bit_reader.ReadExponentialGolomb();
    frame_crop_right_offset = bit_reader.ReadExponentialGolomb();
    frame_crop_top_offset = bit_reader.ReadExponentialGolomb();
    frame_crop_bottom_offset = bit_reader.ReadExponentialGolomb();
  }
  // vui_parameters_present_flag: u(1)
  sps->vui_params_present = bit_reader.ReadBit();
