...
int16_t WebRtcOpus_EncoderCreate(OpusEncInst** inst,
  ...
  if (channels <= 2) {
    ...
    state->encoder.multistream_encoder = opus_multistream_surround_encoder_create(
        48000,
        channels,
        /* mapping family */ 1,
        &streams,
  ...
  if (error != OPUS_OK || (!state->encoder.encoder && !state->encoder.multistream_encoder)) {
...
int16_t WebRtcOpus_DecoderCreate(OpusDecInst** inst, size_t channels) {
  ...
  if (inst != NULL) {
    ...
    if (channels <= 2) {
      state->decoder.decoder = opus_decoder_create(48000, (int)channels, &error);
    } else {
      unsigned char mapping[255];
      memset(mapping, 0, 255);
      int streams = -1;
    ...
    if (error == OPUS_OK && (state->decoder.decoder != NULL ||
                             state->decoder.multistream_decoder != NULL)) {
...
void WebRtcOpus_DecoderInit(OpusDecInst* inst) {
  if (inst->channels <= 2) {
    ...
    opus_multistream_decoder_ctl(inst->decoder.multistream_decoder, OPUS_RESET_STATE);
  }
  inst->in_dtx_mode = 0;
}
