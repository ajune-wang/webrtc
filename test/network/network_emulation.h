...
class EmulatedNetworkStatsImpl final : public EmulatedNetworkStats {
 public:
  ...
  const std::vector<rtc::IPAddress>& LocalAddresses() const override {
 ...
 protected:
  ...
  void IncPacketsSent(int64_t count) { packets_sent_ += count; }

  void IncBytesSent(DataSize bytes_sent) { bytes_sent_ += bytes_sent; }
