...
absl::optional<double> RateUtilizationTracker::GetRateUtilizationFactor(
  ...
  for (size_t i = 0; i < data_points_.size(); ++i) {
    ...
    allocated_send_data_size += allocated_since_last_data_point;

    if (update.produced_data.IsZero()) {
      data_allocated_for_last_data += allocated_since_last_data_point;
    } else {
  ...
  if (data_allocated_for_last_data < size_of_last_data &&
      !current_rate_.IsZero()) {
    // Not enough time has passed since the last data was produced for it to
    // be sent at a nominal rate. Extend the window assuming 1.0 utilization
    // for that last data point.
    allocated_send_data_size +=
        size_of_last_data - data_allocated_for_last_data;
  }

  if (allocated_send_data_size.IsZero()) {
    return absl::nullopt;
  }
