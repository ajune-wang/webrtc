...
bool RtpDependencyDescriptorExtensionMandatory::Parse(
  ...
  descriptor->first_packet_in_frame = data[0] & 0b1000'0000;
  descriptor->last_packet_in_frame = data[0] & 0b0100'0000;
  descriptor->template_id = data[0] & 0b0011'1111;
  descriptor->frame_number = (uint16_t{data[1]} << 8) | data[2];
  return true;
}
