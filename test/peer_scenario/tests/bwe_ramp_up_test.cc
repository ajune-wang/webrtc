...
TEST(BweRampupTest, RampUpWithUndemuxableRtpPackets) {
  ...
  EXPECT_GT(final_bwe, initial_bwe + DataRate::KilobitsPerSec(500));

  auto callee_inbound_stats =
      GetStatsAndProcess(s, callee)->GetStatsOfType<RTCInboundRtpStreamStats>();
  ASSERT_THAT(callee_inbound_stats, ::testing::SizeIs(1));
  EXPECT_EQ(*callee_inbound_stats[0]->frames_received, 0);
}
}  // namespace test
}  // namespace webrtc
