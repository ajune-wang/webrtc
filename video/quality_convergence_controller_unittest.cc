...
TEST(QualityConvergenceController, UnsetStaticQpThreshold) {
  test::ScopedKeyValueConfig field_trials;
  QualityConvergenceController controller;
  controller.Initialize(1, absl::nullopt, kVideoCodecVP8, field_trials);

  EXPECT_FALSE(controller.AddSampleAndCheckTargetQuality(
      /*layer_index=*/0, kStaticQpThreshold, /*is_refresh_frame=*/false));
  EXPECT_FALSE(controller.AddSampleAndCheckTargetQuality(
      /*layer_index=*/0, 0, /*is_refresh_frame=*/false));
}

// Unset static QP threshold.
