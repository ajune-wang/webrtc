...
void Scenario::At(TimeDelta offset, std::function<void()> function) {
  ...
  task_queue_.PostDelayedTask(function, (offset - TimeSinceStart()).ms());
}
...
void Scenario::RunUntil(TimeDelta target_time_since_start,
  ...
  if (start_time_.IsInfinite())
    Start();
  while (target_time_since_start >= TimeSinceStart()) {
    TimeDelta time_left = target_time_since_start - TimeSinceStart();
    time_controller_->Sleep(std::min(check_interval, time_left));
    if (exit_function())
      break;
  }
}
