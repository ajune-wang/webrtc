...
void AudioProcessingImpl::ApplyAgc1Config(
  ...
  RTC_DCHECK_EQ(kNoError, agc->Enable(config.enabled));
  RTC_DCHECK_EQ(kNoError,
                agc->set_mode(Agc1ConfigModeToInterfaceMode(config.mode)));
  RTC_DCHECK_EQ(kNoError, agc->set_target_level_dbfs(config.target_level_dbfs));
  RTC_DCHECK_EQ(kNoError,
                agc->set_compression_gain_db(config.compression_gain_db));
  RTC_DCHECK_EQ(kNoError, agc->enable_limiter(config.enable_limiter));
  RTC_DCHECK_EQ(kNoError,
                agc->set_analog_level_limits(config.analog_level_minimum,
                                             config.analog_level_maximum));
}

GainControl* AudioProcessingImpl::agc1() const {
  if (constants_.use_experimental_agc) {
  ...
  while (capture_runtime_settings_.Remove(&setting)) {
    ...
    switch (setting.type()) {
      ...
      case RuntimeSetting::Type::kCaptureCompressionGain: {
        ...
        RTC_DCHECK_EQ(kNoError, agc1()->set_compression_gain_db(int_value));
        break;
      }
      case RuntimeSetting::Type::kCustomRenderProcessingRuntimeSetting:
        RTC_NOTREACHED();
...
void AudioProcessingImpl::set_stream_analog_level(int level) {
  rtc::CritScope cs_capture(&crit_capture_);
  RTC_DCHECK_EQ(kNoError, agc1()->set_stream_analog_level(level));
}
