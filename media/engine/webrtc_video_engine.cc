...
bool WebRtcVideoChannel::AddSendStream(const StreamParams& sp) {
  ...
  config.encoder_settings.capabilities.loss_notification =
      config.rtp.lntf.enabledd;
...
void WebRtcVideoChannel::WebRtcVideoSendStream::SetCodec(
  ...
  // TODO(eladalon): Fix this error-prone information duplication.
  const bool has_lntf = HasLntf(codec_settings.codec);
  parameters_.config.rtp.lntf.enabledd = has_lntf;
