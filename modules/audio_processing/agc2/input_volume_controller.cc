...
// TODO(webrtc:7494): Hardcode and remove after no longer needed in the unit tests.
...
MonoInputVolumeController::MonoInputVolumeController(
  ...
  RTC_DCHECK_LE(max_digital_gain_db, 90);
  RTC_DCHECK_GE(min_digital_gain_db, 0);
  RTC_DCHECK_LE(min_digital_gain_db, 90);
}
...
InputVolumeController::InputVolumeController(const Config& config)
...
void InputVolumeController::HandleCaptureOutputUsedChange(
    bool capture_output_used) {
  for (auto& estimator : channel_estimators_) {
    estimator->HandleCaptureOutputUsedChange(capture_output_used);
  }
  capture_output_used_ = capture_output_used;
}
...
void InputVolumeController::set_stream_analog_level(int level) {
  ...
  for (auto& estimator : channel_estimators_) {
    estimator->set_stream_analog_level(level);
  }
