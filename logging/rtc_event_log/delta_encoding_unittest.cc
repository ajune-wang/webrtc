...
TEST_P(DeltaEncodingTest, MinDeltaNoWrapAround) {
  ...
  const auto values =
      CreateSequenceByFirstValue(base.value() + 1, num_of_values_);
...
TEST_P(DeltaEncodingTest, ZeroDelta) {
  ...
  const std::vector<uint64_t> deltas = {0,      312, 11, 1,  1, 0, 0, 12,
                                        400321, 3,   3,  12, 5, 0, 6};
  const auto values =
      CreateSequenceByDeltas(base.value(), deltas, num_of_values_);
...
class DeltaEncodingFuzzerLikeTest
 ...
 public:
  ...
  // with the same sequence; avoid this by making the seed different.
  uint64_t Seed() const {
    constexpr uint64_t non_zero_base_seed = 1985;
    return non_zero_base_seed + static_cast<uint64_t>(signedness_) +
           delta_max_bit_width_ + delta_max_bit_width_ + num_of_values_ +
