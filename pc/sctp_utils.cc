...
bool ParseDataChannelOpenMessage(const rtc::CopyOnWriteBuffer& payload,
  ...
  if (message_type != DATA_CHANNEL_OPEN_MESSAGE_TYPE) {
    RTC_DLOG(LS_WARNING) << "Data Channel OPEN message of unexpected type: "
                        << message_type;
    return false;
  }
  ...
  if (!buffer.ReadUInt8(&channel_type)) {
    RTC_DLOG(LS_WARNING) << "Could not read OPEN message channel type.";
    return false;
  }
  ...
  if (!buffer.ReadUInt32(&reliability_param)) {
    RTC_DLOG(LS_WARNING) << "Could not read OPEN message reliabilility param.";
    return false;
  }
  uint16_t label_length;
  if (!buffer.ReadUInt16(&label_length)) {
    RTC_DLOG(LS_WARNING) << "Could not read OPEN message label length.";
    return false;
  }
  uint16_t protocol_length;
  if (!buffer.ReadUInt16(&protocol_length)) {
    RTC_DLOG(LS_WARNING) << "Could not read OPEN message protocol length.";
    return false;
  }
  if (!buffer.ReadString(label, (size_t)label_length)) {
    RTC_DLOG(LS_WARNING) << "Could not read OPEN message label";
    return false;
  }
  if (!buffer.ReadString(&config->protocol, protocol_length)) {
    RTC_DLOG(LS_WARNING) << "Could not read OPEN message protocol.";
    return false;
  }
...
bool ParseDataChannelOpenAckMessage(const rtc::CopyOnWriteBuffer& payload) {
  if (payload.size() < 1) {
    RTC_DLOG(LS_WARNING) << "Could not read OPEN_ACK message type.";
    return false;
  }

  uint8_t message_type = payload[0];
  if (message_type != DATA_CHANNEL_OPEN_ACK_MESSAGE_TYPE) {
    RTC_DLOG(LS_WARNING) << "Data Channel OPEN_ACK message of unexpected type: "
                        << message_type;
    return false;
  }
  return true;
