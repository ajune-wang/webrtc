...
- (void)testQueryCodecSupportDelegatesToObjcFactory {
  ...
  Vwebrtc::ideoEncoderFactory::CodecSupport s1 =
      encoder_factory.QueryCodecSupport(codec, absl::nullopt);
  EXPECT_TRUE(s1.is_supported);
  EXPECT_TRUE(s1.is_power_efficient);

  webrtc::VideoEncoderFactory::CodecSupport s2 = encoder_factory.QueryCodecSupport(codec, "L1T2");
  EXPECT_TRUE(s2.is_supported);
  EXPECT_FALSE(s2.is_power_efficient);

  webrtc::VideoEncoderFactory::CodecSupport s3 = encoder_factory.QueryCodecSupport(codec, "L2T1");
