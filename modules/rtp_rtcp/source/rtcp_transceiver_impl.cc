...
void RtcpTransceiverImpl::HandleReportBlocks(
  ...
  static constexpr int64_t kNtpJan1970Millisecs = 2'208'988'800'000;
  Timestamp now_utc = Timestamp::Millis(now_ntp.ToMs() - kNtpJan1970Millisecs);
  ...
  for (const rtcp::ReportBlock& block : rtcp_report_blocks) {
    ...
    if (sender_it != local_senders_by_ssrc_.end()) {
      ...
      // No register sender for this report block, still report it to the
      // network link.
      ReportBlockData report_block;
