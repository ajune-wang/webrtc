...
Desktop::~Desktop() {
  if (own_ && desktop_ != NULL) {
    if (!::CloseDesktop(desktop_)) {
      NLOG(LS_ERROR,
           "Failed to close the owned desktop handle: ", GetLastError());
    }
  }
}
...
bool Desktop::GetName(std::wstring* desktop_name_out) const {
  ...
  if (!GetUserObjectInformationW(desktop_, UOI_NAME, &buffer[0],
    ...
    NLOG(LS_ERROR, "Failed to query the desktop name: ", GetLastError());
    return false;
  }
...
bool Desktop::SetThreadDesktop() const {
  if (!::SetThreadDesktop(desktop_)) {
    NLOG(LS_ERROR, "Failed to assign the desktop to the current thread: ",
         GetLastError());
    return false;
  }
...
Desktop* Desktop::GetDesktop(const WCHAR* desktop_name) {
  ...
  if (desktop == NULL) {
    NLOG(LS_ERROR, "Failed to open the desktop '", desktop_name,
         "': ", GetLastError());
    return NULL;
  }
