...
int Resample(const AudioFrame& frame,
  ...
  RTC_CHECK_GE(destination.size(),
               frame.num_channels_ * target_number_of_samples_per_channel);

  if (frame.muted()) {
    // Is this necessary or can we assume that the destination buffer is already
    // zeroed out?
    memset(destination.data(), 0, destination.size() * sizeof(int16_t));
    // Return the destination buffer size. This assumes caller has pre-allocated
