...
class RTCStatsCollectorTest : public testing::Test {
 public:
  ...
  std::unique_ptr<RTCStatsCollectorWrapper> CreateStatsCollector(
      rtc::scoped_refptr<FakePeerConnectionForStats> pc) {
    return rtc::MakeUnique<RTCStatsCollectorWrapper>(pc);
  }
...
TEST_F(RTCStatsCollectorTest, SingleCallback) {
  auto stats = CreateStatsCollector(CreatePeerConnection());

  rtc::scoped_refptr<const RTCStatsReport> result;
  stats->stats_collector()->GetStatsReport(RTCStatsObtainer::Create(&result));
...
TEST_F(RTCStatsCollectorTest, MultipleCallbacks) {
  ...
  stats->stats_collector()->GetStatsReport(RTCStatsObtainer::Create(&a));
  stats->stats_collector()->GetStatsReport(RTCStatsObtainer::Create(&b));
  stats->stats_collector()->GetStatsReport(RTCStatsObtainer::Create(&c));
  EXPECT_TRUE_WAIT(a && b && c, kGetStatsReportTimeoutMs);
...
TEST_F(RTCStatsCollectorTest, CachedStatsReports) {
  ...
  rtc::scoped_refptr<const RTCStatsReport> a = stats->GetStatsReport();
  rtc::scoped_refptr<const RTCStatsReport> b = stats->GetStatsReport();
  EXPECT_EQ(a.get(), b.get());
  // Invalidate cache by clearing it.
  stats->stats_collector()->ClearCachedStatsReport();
  rtc::scoped_refptr<const RTCStatsReport> c = stats->GetStatsReport();
  EXPECT_NE(b.get(), c.get());
  // Invalidate cache by advancing time.
  fake_clock_.AdvanceTime(rtc::TimeDelta::FromMilliseconds(51));
  rtc::scoped_refptr<const RTCStatsReport> d = stats->GetStatsReport();
  EXPECT_TRUE(d);
...
TEST_F(RTCStatsCollectorTest, CollectRTCCodecStats) {
  ...
  auto* voice_media_channel = pc->AddVoiceChannel("audio", "TransportName");
  voice_media_channel->SetStats(voice_media_info);
  ...
  auto* video_media_channel = pc->AddVideoChannel("video", "TransportName");
  video_media_channel->SetStats(video_media_info);
...
TEST_F(RTCStatsCollectorTest, CollectRTCCertificateStatsMultiple) {
  ...
  pc->AddVoiceChannel("audio", kAudioTransport);
  auto audio_local_certinfo =
      CreateFakeCertificateAndInfoFromDers({"(local) audio"});
  pc->SetLocalCertificate(kAudioTransport, audio_local_certinfo->certificate);
  auto audio_remote_certinfo =
      CreateFakeCertificateAndInfoFromDers({"(remote) audio"});
  pc->SetRemoteCertificate(kAudioTransport,
                           audio_remote_certinfo->certificate->ssl_certificate()
                               .GetUniqueReference());

  pc->AddVideoChannel("video", kVideoTransport);
  auto video_local_certinfo =
      CreateFakeCertificateAndInfoFromDers({"(local) video"});
  pc->SetLocalCertificate(kVideoTransport, video_local_certinfo->certificate);
  auto video_remote_certinfo =
      CreateFakeCertificateAndInfoFromDers({"(remote) video"});
  pc->SetRemoteCertificate(kVideoTransport,
                           video_remote_certinfo->certificate->ssl_certificate()
                               .GetUniqueReference());

  auto report = stats->GetStatsReport();
...
TEST_F(RTCStatsCollectorTest, CollectRTCCertificateStatsChain) {
  ...
  auto local_certinfo = CreateFakeCertificateAndInfoFromDers(
      {"(local) this", "(local) is", "(local) a", "(local) chain"});
  pc->SetLocalCertificate(kTransportName, local_certinfo->certificate);

  auto remote_certinfo = CreateFakeCertificateAndInfoFromDers(
      {"(remote) this", "(remote) is", "(remote) another", "(remote) chain"});
  pc->SetRemoteCertificate(
      kTransportName,
      remote_certinfo->certificate->ssl_certificate().GetUniqueReference());
...
TEST_F(RTCStatsCollectorTest, CollectRTCDataChannelStats) {
  ...
  pc->AddSctpDataChannel(new MockDataChannel(0, "MockDataChannel0",
  ...
  pc->AddSctpDataChannel(new MockDataChannel(
  ...
  pc->AddSctpDataChannel(new MockDataChannel(2, "MockDataChannel2",
  ...
  pc->AddSctpDataChannel(new MockDataChannel(3, "MockDataChannel3",
...
TEST_F(RTCStatsCollectorTest, CollectRTCIceCandidateStats) {
  ...
  pc->AddVoiceChannel("audio", "a");
  pc->SetTransportStats("a", a_transport_channel_stats);
  ...
  pc->AddVideoChannel("video", "b");
  pc->SetTransportStats("b", b_transport_channel_stats);
...
TEST_F(RTCStatsCollectorTest, CollectRTCIceCandidatePairStats) {
  ...
  pc->AddVideoChannel("video", kTransportName);
  pc->SetTransportStats(kTransportName, transport_channel_stats);
  ...
  pc->SetTransportStats(kTransportName, transport_channel_stats);
  report = stats->GetFreshStatsReport();
...
TEST_F(RTCStatsCollectorTest, CollectRTCPeerConnectionStats) {
  ...
  pc->SignalDataChannelCreated()(dummy_channel_a.get());
  ...
  pc->SignalDataChannelCreated()(dummy_channel_b.get());
...
TEST_F(RTCStatsCollectorTest,
  ...
  pc->mutable_local_streams()->AddStream(local_stream);
  ...
  stats->CreateMockRtpSendersReceiversAndChannels(
...
TEST_F(RTCStatsCollectorTest,
  ...
  rtc::scoped_refptr<MediaStream> remote_stream =
      MediaStream::Create("RemoteStreamLabel");
  pc->mutable_remote_streams()->AddStream(remote_stream);
...
TEST_F(RTCStatsCollectorTest, CollectRTCInboundRTPStreamStats_Audio) {
  ...
  auto* voice_media_channel = pc->AddVoiceChannel("voice", kTransportName);
  voice_media_channel->SetStats(voice_media_info);
  stats->SetupRemoteTrackAndReceiver(cricket::MEDIA_TYPE_AUDIO,
  ...
  expected_audio.transport_id =
      "RTCTransport_" + rtc::ToString<>(kTransportName) + "_" +
      rtc::ToString<>(cricket::ICE_CANDIDATE_COMPONENT_RTP);
...
TEST_F(RTCStatsCollectorTest, CollectRTCInboundRTPStreamStats_Video) {
  ...
  auto* video_media_channel = pc->AddVideoChannel("video", "TransportName");
  video_media_channel->SetStats(video_media_info);
  stats->SetupRemoteTrackAndReceiver(cricket::MEDIA_TYPE_VIDEO,
...
TEST_F(RTCStatsCollectorTest, CollectRTCOutboundRTPStreamStats_Audio) {
  ...
  auto* voice_media_channel = pc->AddVoiceChannel("audio", "TransportName");
  voice_media_channel->SetStats(voice_media_info);
  stats->SetupLocalTrackAndSender(cricket::MEDIA_TYPE_AUDIO,
...
TEST_F(RTCStatsCollectorTest, CollectRTCOutboundRTPStreamStats_Video) {
  ...
  auto* video_media_channel = pc->AddVideoChannel("video", "TransportName");
  video_media_channel->SetStats(video_media_info);
  stats->SetupLocalTrackAndSender(cricket::MEDIA_TYPE_VIDEO,
...
TEST_F(RTCStatsCollectorTest, CollectRTCTransportStats) {
  ...
  pc->SetTransportStats(kTransportName, {rtp_transport_channel_stats});
