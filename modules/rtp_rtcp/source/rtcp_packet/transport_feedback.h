...
class TransportFeedback : public Rtpfb {
 public:
  ...
  // For received packets pass receieve time as `delta_from_base` since the
  // `BaseTime()`. For missed packets calls `handler` with `delta_from_base =
  // PlusInfinity()`.
  void ForAllPackets(
      rtc::FunctionView<void(uint16_t sequence_number,
                             TimeDelta delta_from_base)> handler) const;
