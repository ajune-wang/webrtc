...
// Max number of frames for which decode status would be saved.
// As at most 13 bits are used to store picture id difference in
// GenericFrameDescriptor. In plain RTP only 7 bits are used for that.
// Thus, it's impossible to reference frames with more than 2^13 picture id
// difference.
