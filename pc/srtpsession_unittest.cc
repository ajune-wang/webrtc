...
TEST_F(SrtpSessionTest, TestTamperReject) {
  ...
  EXPECT_EQ(1, webrtc::metrics::NumSamples(
                   "WebRTC.PeerConnection.UnprotectSrtpError"));
  EXPECT_EQ(
      1, webrtc::metrics::NumEvents("WebRTC.PeerConnection.UnprotectSrtpError",
                                    srtp_err_status_bad_param));
  EXPECT_FALSE(s2_.UnprotectRtcp(rtcp_packet_, rtcp_len_, &out_len));
  EXPECT_EQ(1, webrtc::metrics::NumSamples(
                   "WebRTC.PeerConnection.UnprotectSrtcpError"));
  EXPECT_EQ(
      1, webrtc::metrics::NumEvents("WebRTC.PeerConnection.UnprotectSrtcpError",
