...
TEST_P(RtpSenderTest, RidIncludedOnRtxSentPackets) {
  ...
  EXPECT_CALL(
      ...
      EnqueuePackets(ElementsAre(AllOf(
          Pointee(Property(&RtpPacketToSend::GetExtension<RtpStreamId>,
                           Optional(StrEq(kRid)))),
          Pointee(Property(&RtpPacketToSend::HasExtension<RepairedRtpStreamId>,
                           false))))))
  ...
  EXPECT_CALL(
      mock_paced_sender_,
      EnqueuePackets(ElementsAre(AllOf(
          Pointee(Property(&RtpPacketToSend::GetExtension<RepairedRtpStreamId>,
                           Optional(StrEq(kRid)))),
          Pointee(
...
TEST_P(RtpSenderTest, MidAndRidNotIncludedOnSentPacketsAfterAck) {
  ...
  EXPECT_CALL(mock_paced_sender_,
              EnqueuePackets(ElementsAre(AllOf(
                  Pointee(Property(&RtpPacketToSend::GetExtension<RtpMid>,
                                   Optional(StrEq(kMid)))),
                  Pointee(Property(&RtpPacketToSend::GetExtension<RtpStreamId>,
                                   Optional(StrEq(kRid))))))));
  ...
  EXPECT_CALL(
      mock_paced_sender_,
      EnqueuePackets(ElementsAre(AllOf(
          Pointee(Property(&RtpPacketToSend::HasExtension<RtpMid>, false)),
          Pointee(
              Property(&RtpPacketToSend::HasExtension<RtpStreamId>, false))))));
  SendGenericPacket();
}
