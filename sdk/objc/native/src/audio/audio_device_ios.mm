...
AudioDeviceGeneric::InitStatus AudioDeviceIOS::Init() {
  ...
  UpdateRecordAudioDeviceBuffer();
  UpdatePlayoutAudioDeviceBuffer();
  initialized_ = true;
  return InitStatus::OK;
}
...
int32_t AudioDeviceIOS::InitPlayout() {
  ...
  return audio_output_unit_ != nullptr ? 0 : -1;
}
...
int32_t AudioDeviceIOS::StartPlayout() {
  ...
  if (audio_output_unit_->GetState() == BaseAudioUnit::State::kUninitialized) {
    bool can_play_or_record = false;
    if (!InitializeAudioSession(can_play_or_record)) {
      RTCLogError(@"StartPlayout failed initialize audio session");
      return -1;
    }
    if (can_play_or_record) {
      SetupPlayoutAudioBuffersForActiveAudioSession();
      if (!audio_output_unit_->Initialize(playout_parameters_.sample_rate())) {
        RTCLogWarning(@"StartPlayout failed initialize audio output unit");
      }
    }
  }
...
int32_t AudioDeviceIOS::StartRecording() {
  ...
  if (audio_input_unit_->GetState() == BaseAudioUnit::State::kUninitialized) {
    bool can_play_or_record = false;
    if (!InitializeAudioSession(can_play_or_record)) {
    ...
    if (can_play_or_record) {
      SetupRecordAudioBuffersForActiveAudioSession();
...
void AudioDeviceIOS::HandleInterruptionBegin() {
  ...
  std::array<BaseAudioUnit*, 2> audio_units = {audio_input_unit_.get(), audio_output_unit_.get()};
  for (auto audio_unit : audio_units) {
    if (audio_unit && audio_unit->GetState() == BaseAudioUnit::State::kStarted) {
...
void AudioDeviceIOS::HandleInterruptionEnd() {
  ...
  RTCLog(@"Interruption ended. IsInterrupted changed from %d to 0. "
         ...
         is_interrupted_);
  is_interrupted_ = false;
  if (audio_input_unit_ == nullptr && audio_output_unit_ == nullptr) return;
  bool setup_record_audio_buffers = false;
  bool setup_playout_audio_buffers = false;
  bool prepare_for_new_start = false;
  const auto restat_audio_units = webrtc::field_trial::IsEnabled("WebRTC-Audio-iOS-Holding");
  if (restat_audio_units) {
    if (auto audio_unit = audio_input_unit_.get()) {
      ...
      setup_playout_audio_buffers = true;
    }
    if (auto audio_unit = audio_output_unit_.get()) {
      ...
      setup_record_audio_buffers = true;
    }
  }
  if (prepare_for_new_start) {
    PrepareForNewStart();
  }
  if (setup_playout_audio_buffers) {
    // Allocate new buffers given the potentially new stream format.
    SetupPlayoutAudioBuffersForActiveAudioSession();
  }
  if (setup_record_audio_buffers) {
    // Allocate new buffers given the potentially new stream format.
    SetupRecordAudioBuffersForActiveAudioSession();
  }
  const auto can_play_or_record = [RTC_OBJC_TYPE(RTCAudioSession) sharedInstance].canPlayOrRecord;
  if (audio_input_unit_) {
    UpdateAudioUnit(*audio_input_unit_, record_parameters_, can_play_or_record, recording_);
  }
  if (audio_output_unit_) {
    UpdateAudioUnit(*audio_output_unit_, playout_parameters_, can_play_or_record, playing_);
  }
}
...
void AudioDeviceIOS::UpdateRecordAudioDeviceBuffer() {
  LOGI() << "UpdateRecordAudioDeviceBuffer";
...
void AudioDeviceIOS::UpdatePlayoutAudioDeviceBuffer() {
  LOGI() << "UpdatePlayoutAudioDeviceBuffer";
  // AttachAudioBuffer() is called at construction by the main class but check
  // just in case.
  RTC_DCHECK(audio_device_buffer_) << "AttachAudioBuffer must be called first";
...
void AudioDeviceIOS::SetupRecordAudioBuffersForActiveAudioSession() {
  LOGI() << "SetupRecordAudioBuffersForActiveAudioSession";
  ...
  if (sample_rate <= DBL_EPSILON && record_parameters_.sample_rate() > 0) {
    RTC_LOG(LS_ERROR) << "Reported rate is invalid: " << sample_rate << ". Using "
                      << record_parameters_.sample_rate() << " as sample rate instead.";
    sample_rate = record_parameters_.sample_rate();
  }
...
void AudioDeviceIOS::SetupPlayoutAudioBuffersForActiveAudioSession() {
  LOGI() << "SetupPlayoutAudioBuffersForActiveAudioSession";
  // Verify the current values once the audio session has been activated.
  RTC_OBJC_TYPE(RTCAudioSession)* session = [RTC_OBJC_TYPE(RTCAudioSession) sharedInstance];
  double sample_rate = session.sampleRate;
  NSTimeInterval io_buffer_duration = session.IOBufferDuration;
...
void AudioDeviceIOS::UpdateAudioUnit(BaseAudioUnit& audio_unit,
                                     const AudioParameters& audio_params,
                                     const bool can_play_or_record,
                                     const bool should_play_or_record) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  RTCLog(@"Updating audio unit state. CanPlayOrRecord=%d IsInterrupted=%d ShouldPlayOrRecord=%d",
         can_play_or_record,
         is_interrupted_,
         should_play_or_record);
  ...
  if (should_initialize_audio_unit) {
    ...
    if (&audio_unit == audio_input_unit_.get()) {
      SetupRecordAudioBuffersForActiveAudioSession();
    } else {
      SetupPlayoutAudioBuffersForActiveAudioSession();
    }
  ...
  if (should_start_audio_unit) {
    RTCLog(@"Starting audio input unit for UpdateAudioUnit");
