...
void PacketBuffer::OnTimestampReceived(uint32_t rtp_timestamp) {
  ...
  if (rtp_timestamps_history_.insert(rtp_timestamp).second) {
    ++unique_frames_seen_;
    if (rtp_timestamps_history_.size() > kMaxTimestampsHistory) {
      auto discarded_timestamp = rtp_timestamps_history_.begin();
      // If inserted timestamp is too old for the history, don't count it
      // because it may be counted arbitrary many times if repeated.
      if (*discarded_timestamp == rtp_timestamp)
        unique_frames_seen_--;
      rtp_timestamps_history_.erase(discarded_timestamp);
    }
  }
}
