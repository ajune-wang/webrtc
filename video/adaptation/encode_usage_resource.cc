...
void EncodeUsageResource::AdaptUp() {
  RTC_DCHECK_RUN_ON(encoder_queue_);
  resource_adaptation_queue_->PostTask([this_weak_ptr =
                                            weak_ptr_factory_.GetWeakPtr()] {
    if (!this_weak_ptr) {
      // The EncodeUsageResource was destroyed before this task had a chance to
      // execute. No action needed.
      return;
    }
    RTC_DCHECK_RUN_ON(this_weak_ptr->resource_adaptation_queue_);
    this_weak_ptr->OnResourceUsageStateMeasured(ResourceUsageState::kUnderuse);
  });
}
...
void EncodeUsageResource::AdaptDown() {
  ...
  resource_adaptation_queue_->PostTask([this_weak_ptr =
    ...
    RTC_DCHECK_RUN_ON(this_weak_ptr->resource_adaptation_queue_);
    this_weak_ptr->OnResourceUsageStateMeasured(ResourceUsageState::kOveruse);
  });
}
