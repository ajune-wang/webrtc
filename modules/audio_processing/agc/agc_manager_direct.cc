...
void AgcManagerDirect::Process(const float* audio,
  ...
  if (check_volume_on_next_process_) {
    ...
    CheckVolumeAndReset(analog_level);
  }

  agc_->Process(audio_fix, safe_length, sample_rate_hz);

  UpdateGain(analog_level);
  if (!disable_digital_adaptive_) {
    UpdateCompressor(gain_control);
  }
...
void AgcManagerDirect::SetLevel(int new_level, int* analog_level) {
  int voe_level = *analog_level;
  if (voe_level == 0) {
    RTC_DLOG(LS_INFO)
...
int AgcManagerDirect::CheckVolumeAndReset(int* analog_level) {
  int level = *analog_level;
...
void AgcManagerDirect::UpdateGain(int* analog_level) {
  int rms_error = 0;
...
void AgcManagerDirect::UpdateCompressor(GainControl* gain_control) {
  ...
  if (new_compression != compression_) {
    ...
    if (gain_control->set_compression_gain_db(compression_) != 0) {
