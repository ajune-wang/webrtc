...
AsyncSocket* AsyncTCPSocketBase::ConnectSocket(
  ...
  if (socket->Connect(remote_address) < 0) {
    RTC_DLOG(LS_ERROR) << "Connect() failed with error " << socket->GetError();
    return nullptr;
  }
...
AsyncTCPSocketBase::AsyncTCPSocketBase(AsyncSocket* socket,
  ...
  if (listen_) {
    if (socket_->Listen(kListenBacklog) < 0) {
      RTC_DLOG(LS_ERROR) << "Listen() failed with error " << socket_->GetError();
    }
  }
}
...
void AsyncTCPSocketBase::OnReadEvent(AsyncSocket* socket) {
  ...
  if (listen_) {
    ...
    if (!new_socket) {
      ...
      // to the user.
      RTC_DLOG(LS_ERROR) << "TCP accept failed with error "
                        << socket_->GetError();
      return;
    }
    ...
    while (true) {
      ...
      if (len < 0) {
        ...
        if (!socket_->IsBlocking()) {
          RTC_DLOG(LS_ERROR) << "Recv() returned error: " << socket_->GetError();
        }
        break;
      }
