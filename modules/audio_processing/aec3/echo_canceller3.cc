...
void FillSubFrameView(std::vector<std::vector<float>>* frame,
  ...
  for (size_t channel = 0; channel < frame->size(); ++channel) {
    (*sub_frame_view)[channel] = rtc::ArrayView<float>(
        &(*frame)[channel][sub_frame_index * kSubFrameLength], kSubFrameLength);
  }
}

void ProcessCaptureFrameContent(
    AudioBuffer* capture,
    std::vector<std::vector<float>>* linear_output_frame,
    bool level_change,
    ...
    std::vector<std::vector<float>>* linear_output_block,
    std::vector<rtc::ArrayView<float>>* linear_output_sub_frame_view,
  ...
  if (linear_output_framer) {
    ...
    RTC_DCHECK(linear_output_frame);
    RTC_DCHECK_EQ(kFrameSize, linear_output_frame->size());
    FillSubFrameView(linear_output_frame, sub_frame_index,
...
EchoCanceller3::EchoCanceller3(const EchoCanceller3Config& config,
  ...
  if (config_.filter.export_linear_filter_output) {
    linear_output_framer_.reset(new BlockFramer(0, num_capture_channels_));
    linear_output_block_.reset(new std::vector<std::vector<float>>(
        num_capture_channels_, std::vector<float>(kBlockSize, 0.f)));
    linear_output_frame_ = std::vector<std::vector<float>>(
        num_capture_channels_, std::vector<float>(kFrameSize, 0.f));
    linear_output_sub_frame_view_.reset(
        new std::vector<rtc::ArrayView<float>>(num_render_channels_));
  }
}
...
void EchoCanceller3::ProcessCapture(AudioBuffer* capture, bool level_change) {
  ...
  ProcessCaptureFrameContent(capture, &linear_output_frame_, level_change,
                             ...
                             linear_output_sub_frame_view_.get(),
