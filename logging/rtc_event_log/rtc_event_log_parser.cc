...
bool ParsedRtcEventLog::ParseStream(std::istream& stream) {
  ...
  while (1) {
    ...
    if (!success) {
      RTC_LOG(LS_WARNING) << "Missing field tag from beginning of protobuf "
                          << "event.";
      return false;
    } else if (tag != kExpectedTag) {
      RTC_LOG(LS_WARNING) << "Unexpected field tag at beginning of protobuf "
                          << "event.";
      return false;
    }
...
std::vector<rtclog::StreamConfig> ParsedRtcEventLog::GetVideoSendConfig(
  ...
  if (sender_config.rtx_ssrcs_size() > 0 &&
    ...
    RTC_LOG(WARNING) << "VideoSendConfig is configured for RTX but the number "
                    "of SSRCs doesn't match the number of RTX SSRCs.";
  }
