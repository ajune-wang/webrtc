...
PeerScenarioClient::PeerScenarioClient(
  ...
  pcf_deps.audio_processing = AudioProcessingBuilder().Create();
  ...
  pcf_deps.media_engine_factory = CreateMediaEngineFactoryForTest(
      {.time_controller = net->time_controller()});
