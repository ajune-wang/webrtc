...
class FuzzSignalInput : public NetEqInput {
 public:
  ...
  std::unique_ptr<Event> PopEvent() override {
    ...
    if (packet_ && packet_->get_timestamp_ms() <= next_output_event_ms_) {
      event_ = PopPacket();
    } else {
  ...
  std::unique_ptr<Event> PopPacket() {
    ...
    if (fuzz_data_.CanReadBytes(1)) {
      PacketData& packet_data = static_cast<PacketData&>(*packet_);
      ...
      packet_data.time_ms =
          std::max(packet_to_return_time_ms,
                   packet_data.time_ms + arrival_time_offset_ms);
    } else {
