...
class NetEqFacadeImpl final : public NetEqFacade {
 public:
  ...
  ~NetEqFacadeImpl() = default;
  NetEqFacadeImpl(PacketBuffer* packet_buffer,
                  BufferLevelFilter* buffer_level_filter,
                  DecoderDatabase* decoder_database,
                  StatisticsCalculator* statistics_calculator);

  bool ContainsDtxOrCngPacket() const;
  size_t GetSpanSamples(size_t last_decoded_length,
                        size_t sample_rate,
                        bool count_dtx_waiting_time) const;
  size_t NumSamplesInBuffer(size_t last_decoded_length) const;
  void UpdateTargetBufferFilter(size_t buffer_size_samples,
                                int time_stretched_samples);
  void SetTargetBufferLevel(int target_buffer_level_packets);
  void ReportRelativePacketArrivalDelay(int delay_ms);
