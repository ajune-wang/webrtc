...
SendVideoStream::SendVideoStream(CallClient* sender,
  ...
  send_stream_ =
      sender_->thread()->Invoke<VideoSendStream*>(RTC_FROM_HERE, [&]() {
        return sender_->call_->CreateVideoSendStream(std::move(send_config),
                                                     std::move(encoder_config));
        if (analyzer->Active()) {
          frame_tap_.reset(new ForwardingCapturedFrameTap(
              sender_->clock_, analyzer, video_capturer_.get()));
