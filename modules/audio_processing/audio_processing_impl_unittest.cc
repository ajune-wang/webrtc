...
// Disable creation of build-optional submodules, and verify APM does not use
// the submodules.
TEST(ApmWithSubmodulesExcludedTest, BitexactWithDisabledModules) {
  rtc::scoped_refptr<webrtc::AudioProcessingImpl> apm =
      new rtc::RefCountedObject<AudioProcessingImpl>(webrtc::Config());
  ASSERT_EQ(apm->Initialize(), AudioProcessing::kNoError);
  apm->SetCreateOptionalSubmodulesForTesting(/*create_submodules=*/false);
  AudioProcessing::Config apm_config = apm->GetConfig();
  apm_config.transient_suppression.enabled = true;
  apm->ApplyConfig(apm_config);

  rtc::scoped_refptr<webrtc::AudioProcessing> apm_reference =
  ...
  webrtc::StreamConfig stream_config(/*sample_rate_hz=*/kSampleRateHz,
...
TEST(ApmWithSubmodulesExcludedTest, ReinitializeTransientSuppressor) {
  ...
  webrtc::StreamConfig initial_stream_config(/*sample_rate_hz=*/16000,
                                             /*num_channels=*/1,
                                             /*has_keyboard=*/false);
  for (int i = 0; i < kFramesToProcessPerConfiguration; ++i) {
    RandomizeSampleVector(&random_generator, buffer);
  ...
  webrtc::StreamConfig stereo_stream_config(/*sample_rate_hz=*/16000,
                                            /*num_channels=*/2,
                                            /*has_keyboard=*/false);
  for (int i = 0; i < kFramesToProcessPerConfiguration; ++i) {
    RandomizeSampleVector(&random_generator, buffer);
  ...
  webrtc::StreamConfig high_sample_rate_stream_config(/*sample_rate_hz=*/48000,
                                                      /*num_channels=*/1,
                                                      /*has_keyboard=*/false);
  for (int i = 0; i < kFramesToProcessPerConfiguration; ++i) {
    RandomizeSampleVector(&random_generator, buffer);
  ...
  webrtc::StreamConfig keyboard_stream_config(/*sample_rate_hz=*/16000,
                                              /*num_channels=*/1,
                                              /*has_keyboard=*/true);
  for (int i = 0; i < kFramesToProcessPerConfiguration; ++i) {
    RandomizeSampleVector(&random_generator, buffer);
...
TEST(ApmWithSubmodulesExcludedTest, ToggleTransientSuppressor) {
  ...
  Random random_generator(2341U);
  constexpr int kFramesToProcessPerConfiguration = 3;
  webrtc::StreamConfig stream_config(/*sample_rate_hz=*/16000,
                                     /*num_channels=*/1,
                                     /*has_keyboard=*/false);
