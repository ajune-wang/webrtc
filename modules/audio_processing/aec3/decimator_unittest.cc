...
void ProduceDecimatedSinusoidalOutputPower(int sample_rate_hz,
  ...
  for (size_t k = 0; k < kNumBlocks; ++k) {
    ...
    decimator.Decimate(input_multichannel, sub_block, true);
...
TEST(Decimator, WrongInputSize) {
  ...
  EXPECT_DEATH(decimator.Decimate(x, x_downsampled, true), "");
}
...
TEST(Decimator, NullOutput) {
  ...
  EXPECT_DEATH(decimator.Decimate(x, nullptr, true), "");
}
