...
TEST(SpsVuiRewriterOutgoingVuiTest, ParseOutgoingBitstreamOptimalVui) {
  ...
  rtc::Buffer buffer;
  constexpr int kNumNalus = 2;
  RTPFragmentationHeader nalu;
  nalu.VerifyAndAllocateFragmentationHeader(kNumNalus);
  buffer.AppendData(kStartSequence);
  nalu.Set(/*index=*/0, /*offset=*/buffer.size(),
           /*length=*/optimal_sps.size());
  buffer.AppendData(optimal_sps);
  buffer.AppendData(kStartSequence);
  nalu.Set(/*index=*/1, /*offset=*/buffer.size(), /*length=*/sizeof(kIdr1));
  buffer.AppendData(kIdr1);

  rtc::Buffer modified_buffer;
  RTPFragmentationHeader modified_nalu;
  modified_nalu.VerifyAndAllocateFragmentationHeader(kNumNalus);

  SpsVuiRewriter::ParseOutgoingBitstreamAndRewriteSps(
      buffer, nalu, nullptr, &modified_buffer, &modified_nalu);
...
TEST(SpsVuiRewriterOutgoingVuiTest, ParseOutgoingBitstreamNoVui) {
  ...
  rtc::Buffer buffer;
  RTPFragmentationHeader nalu(3);
  buffer.AppendData(kStartSequence);
  nalu.Set(0, /*offset=*/buffer.size(), /*length=*/sizeof(kIdr1));
  buffer.AppendData(kIdr1);
  buffer.AppendData(kStartSequence);
  nalu.Set(1, /*offset=*/buffer.size(),
           /*length=*/sizeof(kSpsNaluType) + sps.size());
  buffer.AppendData(kSpsNaluType);
  buffer.AppendData(sps);
  buffer.AppendData(kStartSequence);
  nalu.Set(2, /*offset=*/buffer.size(), /*length=*/sizeof(kIdr2));
  ...
  RTPFragmentationHeader expected_nalu(3);
  expected_buffer.AppendData(kStartSequence);
  expected_nalu.Set(0, expected_buffer.size(), sizeof(kIdr1));
  expected_buffer.AppendData(kIdr1);
  expected_buffer.AppendData(kStartSequence);
  expected_nalu.Set(1, expected_buffer.size(),
                    sizeof(kSpsNaluType) + optimal_sps.size());
  expected_buffer.AppendData(kSpsNaluType);
  expected_buffer.AppendData(optimal_sps);
  expected_buffer.AppendData(kStartSequence);
  expected_nalu.Set(1, expected_buffer.size(), sizeof(kIdr2));
  expected_buffer.AppendData(kIdr2);

  rtc::Buffer modified_buffer;
  RTPFragmentationHeader modified_nalu(3);
  ...
  EXPECT_EQ(modified_nalu, expected_nalu);
}
}  // namespace webrtc
