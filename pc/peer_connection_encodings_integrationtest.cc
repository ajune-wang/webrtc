...
Matcher<const RTCOutboundRtpStreamStats*> RidIs(absl::string_view rid) {
  return Field("rid", &RTCOutboundRtpStreamStats::rid, Optional(StrEq(rid)));
}

template <typename WidthMatcher, typename HeightMatcher>
Matcher<const RTCOutboundRtpStreamStats*> ResolutionIs(
    WidthMatcher width_matcher,
    HeightMatcher height_matcher) {
...
flat_map<std::string, const RTCOutboundRtpStreamStats*>
GetOutboundRtpStreamStatsByRid(
    const std::vector<const RTCOutboundRtpStreamStats*>& outbound_rtps) {
  flat_map<std::string, const RTCOutboundRtpStreamStats*> result;
  for (const auto* outbound_rtp : outbound_rtps) {
    result[outbound_rtp->rid.value_or("")] = outbound_rtp;
  }
  return result;
}
...
TEST_P(PeerConnectionEncodingsIntegrationParameterizedTest,
  ...
  auto outbound_rtp_by_rid = GetOutboundRtpStreamStatsByRid(
      encoding_resolutions_result.value()
          ->GetStatsOfType<RTCOutboundRtpStreamStats>());
...
TEST_P(PeerConnectionEncodingsIntegrationParameterizedTest,
  ...
  auto outbound_rtp_by_rid = GetOutboundRtpStreamStatsByRid(
      error_or_stats.value()->GetStatsOfType<RTCOutboundRtpStreamStats>());
  // XXX: Pairwise
  ASSERT_THAT(outbound_rtp_by_rid,
...
TEST_P(PeerConnectionEncodingsIntegrationParameterizedTest,
  ...
  EXPECT_THAT(GetStatsUntil(local_pc_wrapper,
                            OutboundRtpStatsAre(UnorderedElementsAre(
                                AllOf(RidIs("q"), ResolutionIs(20, 10)),
                                AllOf(RidIs("h"), ResolutionIs(40, 20)),
                                AllOf(RidIs("f"), ResolutionIs(80, 40)))),
                            {.timeout = kLongTimeoutForRampingUp}),
              IsRtcOk());
}
