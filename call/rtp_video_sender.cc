...
RtpVideoSender::RtpVideoSender(
  ...
  transport_->GetTransportFeedbackProvider()->RegisterPacketFeedbackObserver(
...
RtpVideoSender::~RtpVideoSender() {
  ...
  transport_->GetTransportFeedbackProvider()->DeRegisterPacketFeedbackObserver(
      this);
}
...
void RtpVideoSender::OnPacketFeedbackVector(
    std::vector<FeedbackInfo> packet_feedback_vector) {
  if (fec_controller_->UseLossVectorMask()) {
    rtc::CritScope cs(&crit_);
    for (const FeedbackInfo& packet : packet_feedback_vector) {
