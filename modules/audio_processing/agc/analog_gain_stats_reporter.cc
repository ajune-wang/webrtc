...
float ComputeAverageUpdate(int sum_updates, int num_updates) {
  ...
  RTC_DCHECK_LT(num_updates, kFramesIn60Seconds);
  return (num_updates > 0) ? std::round(static_cast<float>(sum_updates) /
...
void AnalogGainStatsReporter::UpdateStatistics(int analog_mic_level) {
  RTC_DCHECK_GE(analog_mic_level, 0);
  RTC_DCHECK_LE(analog_mic_level, kMaxUpdate);
  const bool level_changed = previous_analog_mic_level_ != -1 &&
                             analog_mic_level != previous_analog_mic_level_;
  if (level_changed) {
    const int level_change = analog_mic_level - previous_analog_mic_level_;
    if (level_change < 0) {
