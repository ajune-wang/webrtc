...
class DataSize {
 public:
  ...
  int64_t kilobytes() const {
    return units_impl::DivideAndRound(bytes(), 1000);
  }
  int64_t bits() const { return bytes() * 8; }
  int64_t kilobits() const { return units_impl::DivideAndRound(bits(), 1000); }
  ...
  DataSize operator*(double scalar) const {
    return DataSize::bytes(units_impl::DoubleToIntRounded(bytes() * scalar));
  }
