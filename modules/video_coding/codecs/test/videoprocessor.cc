...
const int kMaxBufferedInputFrames = 30;
...
VideoProcessor::~VideoProcessor() {
  ...
  for (size_t simulcast_svc_idx = 0;
       simulcast_svc_idx < num_simulcast_or_spatial_layers_; ++i) {
...
void VideoProcessor::FrameDecoded(const VideoFrame& decoded_frame) {
  ...
  // layers. In case of frame drops, never buffer more than
  // |kMaxBufferedInputFrames| frames.
  ...
  const size_t min_buffered_frame_num =
      std::max(0, static_cast<int>(frame_number) - kMaxBufferedInputFrames);
  RTC_CHECK(min_last_decoded_frame_num != last_decoded_frame_num_.cend());
  const auto input_frames_erase_to = input_frames_.lower_bound(
