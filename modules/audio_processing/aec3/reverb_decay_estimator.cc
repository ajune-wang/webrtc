...
void ReverbDecayEstimator::EarlyReverbLengthEstimator::Accumulate(
  ...
  // needs to be updated.
  int first_section = std::max(block_ - kBlocksPerSection + 1, 0);
  int last_section = std::min(block_, static_cast<int>(nz_.size() - 1));
  ...
  value_to_add += (block_ - last_section) * value_to_inc;
