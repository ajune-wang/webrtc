...
bool WebRtcSession::SetLocalDescription(
    std::unique_ptr<SessionDescriptionInterface> desc_owned,
    std::string* err_desc) {
  RTC_DCHECK(signaling_thread()->IsCurrent());

  SessionDescriptionInterface* const desc_ptr = desc_owned.get();

  // Validate SDP.
  if (!ValidateSessionDescription(desc_ptr, cricket::CS_LOCAL, err_desc)) {
    return false;
  }

  // Update the initial_offerer flag if this session is the initial_offerer.
  Action action = GetAction(desc_ptr->type());
  ...
  if (action == kAnswer) {
    current_local_description_ = std::move(desc_owned);
    pending_local_description_ = nullptr;
    current_remote_description_ = std::move(pending_remote_description_);
  } else {
    pending_local_description_ = std::move(desc_owned);
  }
  RTC_DCHECK_EQ(desc_ptr, local_description());

  // Transport and Media channels will be created only when offer is set.
  if (action == kOffer && !CreateChannels(local_description()->description())) {
    // TODO(mallinath) - Handle CreateChannel failure, as new local description
    // is applied. Restore back to old description.
    return BadLocalSdp(desc_ptr->type(), kCreateChannelFailed, err_desc);
  }
  ...
  if (error() != ERROR_NONE) {
    return BadLocalSdp(desc_ptr->type(), GetSessionErrorMsg(), err_desc);
  }
  return true;
}
...
bool WebRtcSession::SetRemoteDescription(
  ...
  // Validate SDP.
  if (!ValidateSessionDescription(desc_ptr, cricket::CS_REMOTE, err_desc)) {
    return false;
  }
  ...
  if (action == kAnswer) {
    ...
    current_remote_description_ = std::move(desc_owned);
    ...
    pending_remote_description_ = std::move(desc_owned);
  }
  RTC_DCHECK_EQ(desc_ptr, remote_description());

  // Transport and Media channels will be created only when offer is set.
  if (action == kOffer && !CreateChannels(desc_ptr->description())) {
    // TODO(mallinath) - Handle CreateChannel failure, as new local description
    // is applied. Restore back to old description.
    return BadRemoteSdp(desc_ptr->type(), kCreateChannelFailed, err_desc);
  }

  // Remove unused channels if MediaContentDescription is rejected.
  RemoveUnusedChannels(desc_ptr->description());
  ...
  if (local_description() && !UseCandidatesInSessionDescription(desc_ptr)) {
    return BadRemoteSdp(desc_ptr->type(), kInvalidCandidates, err_desc);
  }
  ...
  if (old_remote_description) {
    for (const cricket::ContentInfo& content :
      ...
      if (CheckForRemoteIceRestart(old_remote_description, desc_ptr,
                                   content.name)) {
        if (action == kOffer) {
  ...
  if (error() != ERROR_NONE) {
    return BadRemoteSdp(desc_ptr->type(), GetSessionErrorMsg(), err_desc);
  }
  ...
  if (desc_ptr->type() != SessionDescriptionInterface::kOffer &&
      ice_connection_state_ == PeerConnectionInterface::kIceConnectionNew) {
