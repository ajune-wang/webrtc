...
TEST(DelayVariationCalculatorTest, PartialTimeSeriesWithOneFrame) {
  ...
  // Inter-frame metrics are unset, since we don't yet have a pair.
  EXPECT_EQ(v0(ts.inter_departure_times_ms), -1);
  EXPECT_EQ(v0(ts.inter_arrival_times_ms), -1);
  EXPECT_EQ(v0(ts.inter_delay_variations_ms), -1);
  EXPECT_EQ(v0(ts.inter_size_variations_bytes), -1);
}
