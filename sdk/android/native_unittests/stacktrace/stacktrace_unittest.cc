...
bool StackTraceContainsRange(const std::vector<StackTraceElement>& stack_trace,
  ...
  for (const StackTraceElement& stack_trace_element : stack_trace) {
    if (pc_low - 2 <= stack_trace_element.program_counter &&
        pc_high + 2 >= stack_trace_element.program_counter) {
      return true;
    }
  }
  return false;
...
class SpinDeadlock : public DeadlockInterface {
 private:
  ...
  volatile bool is_deadlocked_ = true;
};
...
void TestStacktrace(std::unique_ptr<DeadlockInterface> deadlock_impl) {
  ...
  EXPECT_TRUE(false && StackTraceContainsRange(
                           stack_trace, params.deadlock_region_start_address,
