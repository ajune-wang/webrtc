...
- (instancetype)initWithNativeEncodedImage:(const webrtc::EncodedImage &)encodedImage {
  if (self = [super init]) {
    ...
    self.encodedData = encodedImage.buffer();
    // Wrap the buffer in NSData without copying, do not take ownership.
    self.buffer = [NSData dataWithBytesNoCopy:const_cast<uint8_t *>(self.encodedData.cdata())
...
- (webrtc::EncodedImage)nativeEncodedImage {
  ...
  RTC_DCHECK_EQ(self.buffer.bytes, self.encodedData.cdata());
  encodedImage.set_buffer(self.encodedData);
