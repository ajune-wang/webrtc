...
struct EventCounts {
  ...
  size_t total_nonconfig_events() const {
    return alr_states + route_changes + audio_playouts + ana_configs +
           bwe_loss_events + bwe_delay_events + bwe_goog_cc_events +
...
class RtcEventLogSession
 ...
 private:
  ...
  std::vector<std::unique_ptr<RtcEventBweUpdateGoogCc>> bwe_goog_cc_list_;
...
void RtcEventLogSession::WriteLog(EventCounts count,
  ...
  for (; remaining_events > 0; remaining_events--) {
    ...
    if (selection < count.bwe_goog_cc_events) {
      auto event = gen_.NewBweUpdateGoogCc();
      event_log->Log(event->Copy());
      bwe_goog_cc_list_.push_back(std::move(event));
      count.bwe_goog_cc_events--;
      continue;
    }
    selection -= count.bwe_goog_cc_events;
...
void RtcEventLogSession::ReadAndVerifyLog() {
  ...
  auto& parsed_bwe_goog_cc_updates = parsed_log.bwe_goog_cc_updates();
  ASSERT_EQ(parsed_bwe_goog_cc_updates.size(), bwe_goog_cc_list_.size());
  for (size_t i = 0; i < parsed_bwe_goog_cc_updates.size(); i++) {
    verifier_.VerifyLoggedGoogCcBweUpdate(*bwe_goog_cc_list_[i],
                                          parsed_bwe_goog_cc_updates[i]);
  }
