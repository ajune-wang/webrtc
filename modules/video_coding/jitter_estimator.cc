...
int VCMJitterEstimator::GetJitterEstimate(
    ...
    absl::optional<double> jitterEstCapMs) {
  ...
  if (_nackCount >= _nackLimit || 1) {  // CAVEAT forced ***************
    if (jitterEstCapMs.has_value()) {
      jitterMS += std::min(/*_rttFilter.RttMs()*/ 250 * rttMultiplier,
                           jitterEstCapMs.value());  // CAVEAT forced ******
    } else {
