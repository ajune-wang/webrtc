...
std::vector<SdpVideoFormat> SupportedVP9Codecs() {
  return {SdpVideoFormat(cricket::kVp9CodecName,
                         {{VP9::kVP9Profile, std::to_string(VP9::kProfile0)}}),
          SdpVideoFormat(cricket::kVp9CodecName,
                         {{VP9::kVP9Profile, std::to_string(VP9::kProfile2)}})};
}
...
VP9EncoderImpl::VP9EncoderImpl(const cricket::VideoCodec& codec)
    : encoded_image_(),
      ...
      profile_(VP9::ParseSdpProfile(codec.params).value_or(VP9::kProfile0)),
      inited_(false),
      timestamp_(0),
      cpu_speed_(3),
...
int VP9EncoderImpl::Encode(const VideoFrame& input_image,
  ...
  RTC_DCHECK(i420_buffer->bit_depth() ==
             (config_->g_bit_depth == VPX_BITS_8
...
int VP9DecoderImpl::ReturnFrame(const vpx_image_t* img,
  ...
  PlanarYuvBuffer::BitDepth bit_depth = PlanarYuvBuffer::BitDepth::kBitDepth8;
  switch (img->bit_depth) {
    case 8:
      bit_depth = PlanarYuvBuffer::BitDepth::kBitDepth8;
      break;
    case 10:
      bit_depth = PlanarYuvBuffer::BitDepth::kBitDepth10;
      break;
    default:
      RTC_NOTREACHED();
      return WEBRTC_VIDEO_CODEC_NO_OUTPUT;
  ...
  rtc::scoped_refptr<I420BufferInterface> img_wrapped_buffer =
      WrapI420Buffer(
