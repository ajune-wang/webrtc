...
CrossTrafficRoute::CrossTrafficRoute(Clock* clock,
...
CrossTrafficRoute::~CrossTrafficRoute() = default;

void CrossTrafficRoute::TriggerPacketBurst(size_t num_packets,
                                           size_t packet_size) {
  TriggerPacketBurst(num_packets, packet_size, GetDefaultDestination());
}
...
void CrossTrafficRoute::TriggerPacketBurst(size_t num_packets,
  ...
  for (size_t i = 0; i < num_packets; ++i) {
    SendPacket(rtc::SocketAddress(), rtc::CopyOnWriteBuffer(packet_size),
               destination);
  }
}

void CrossTrafficRoute::NetworkDelayedAction(size_t packet_size,
...
Destination CrossTrafficRoute::GetDefaultDestination() const {
  // Uses endpoint port as a destination id.
  return Destination{null_receiver_port_};
}

Destination CrossTrafficRoute::RegisterDestination(
    std::unique_ptr<EmulatedNetworkReceiverInterface> receiver) {
  absl::optional<uint16_t> port = endpoint_->BindReceiver(0, receiver.get());
  RTC_DCHECK(port);
  external_receivers_.push_back(std::move(receiver));
  return Destination{port.value()};
}

void CrossTrafficRoute::RemoveDestination(const Destination& destination) {
  RTC_DCHECK(destination.id != null_receiver_port_)
      << "Can't remove default destination";
  endpoint_->UnbindReceiver(destination.id);
}

void CrossTrafficRoute::SendPacket(rtc::SocketAddress source_addr,
                                   rtc::CopyOnWriteBuffer data,
                                   const Destination& destination) {
  SendPacket(source_addr, data, Timestamp::us(clock_->TimeInMicroseconds()),
             destination);
}
...
void CrossTrafficRoute::SendPacket(rtc::SocketAddress source_addr,
  ...
  receiver_->OnPacketReceived(EmulatedIpPacket(
      source_addr,
      rtc::SocketAddress(endpoint_->GetPeerLocalAddress(), destination.id),
      endpoint_->GetId(), data, at_time));
}
...
void RandomWalkCrossTraffic::Process(Timestamp at_time) {
  ...
  if (pending_size_ >= config_.min_packet_size &&
    ...
    cross_traffic_->SendPacket(rtc::SocketAddress(),
                               rtc::CopyOnWriteBuffer(pending_size_.bytes()),
                               at_time, destination_);
