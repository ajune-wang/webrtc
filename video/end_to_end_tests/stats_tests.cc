    ...
    bool CheckReceiveStats() {
      for (size_t i = 0; i < receive_streams_.size(); ++i) {
        ...
        receive_stats_filled_["StatisticsUpdated"] |=
            stats.rtp_stats.packets_lost != 0 ||
            stats.rtp_stats.extended_highest_sequence_number != 0 ||
            stats.rtp_stats.jitter != 0;

        receive_stats_filled_["DataCountersUpdated"] |=
            stats.rtp_stats.packet_counter.transmitted.payload_bytes != 0 ||
            stats.rtp_stats.packet_counter.fec.packets != 0 ||
            stats.rtp_stats.packet_counter.transmitted.header_bytes != 0 ||
            stats.rtp_stats.packet_counter.transmitted.packets != 0 ||
            stats.rtp_stats.packet_counter.transmitted.padding_bytes != 0 ||
            stats.rtp_stats.packet_counter.retransmitted.packets != 0;
   ...
   private:
    ...
    Action OnSendRtp(const uint8_t* packet, size_t length) override {
      if (sent_rtp_ >= kNumRtpPacketsToSend) {
        ...
        if (kNumRtpPacketsToSend ==
            stats.rtp_stats.packet_counter.transmitted.packets) {
