...
void RTCStatsCollector::ProduceMediaStreamAndTrackStats_s(
  ...
  ProduceMediaStreamStats(timestamp_us, senders, receivers, report);
  RTC_LOG(LS_INFO) << "Size: " << transceiver_stats_infos_.size();
  for (const RtpTransceiverStatsInfo& stats : transceiver_stats_infos_) {
    ProduceSenderMediaTrackStats(
        timestamp_us, *stats.track_media_info_map,
        stats.transceiver->internal()->senders_internal(), report);
    ProduceReceiverMediaTrackStats(
        timestamp_us, *stats.track_media_info_map,
        stats.transceiver->internal()->receivers_internal(), report);
  }
}
...
RTCStatsCollector::PrepareTransceiverStatsInfos_s() const {
  ...
  for (auto& stats : transceiver_stats_infos) {
    ...
    RTC_DCHECK(transceiver->internal()->channel());
    ...
    stats.track_media_info_map = rtc::MakeUnique<TrackMediaInfoMap>(
        ...
        transceiver->internal()->senders_internal(),
        transceiver->internal()->receivers_internal());
  }
