...
absl::optional<std::unique_ptr<BasicDesktopFrame>>
...
void SharedScreenCastStreamPrivate::ProcessBuffer(pw_buffer* buffer) {
  ...
  if (spa_buffer->datas[0].type == SPA_DATA_MemFd) {
    ...
    src_unique_ptr = egl_dmabuf_->ImageFromDmaBuf(
        desktop_size_, spa_video_format_.format, n_planes, fds,
        strides, offsets, modifier_);
...
SharedScreenCastStream::SharedScreenCastStream()
    : d(std::make_unique<SharedScreenCastStreamPrivate>()) {}
...
bool SharedScreenCastStream::StartScreenCastStream(uint32_t stream_node_id,
                                                   int32_t fd) {
  return d->StartScreenCastStream(stream_node_id, fd);
}

absl::optional<std::unique_ptr<BasicDesktopFrame>>
SharedScreenCastStream::CaptureFrame() {
