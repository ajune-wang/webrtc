...
TEST_F(DelayManagerTest, DecelerationTargetLevelOffset) {
  ...
  const int kOffset = dm_->deceleration_target_level_offset_ms() >> 8;
  // Solution of |target * 3/4 = target - kOffset|.
  const int kBoarderTargetLevel = kOffset * 4;
  {
...

  {
    ...
    EXPECT_EQ(target_level_ms - ((kOffset << 8) / kFrameSizeMs), lower);
    EXPECT_EQ(target_level_ms, higher);
  }
}
