...
void AecState::FilteringQualityAnalyzer::Update(
  ...
  // Verify necessary conditions for that the filter filter convergence used as
  // a prerequisite for using the linear filter. has A prerequisite is that
  // sufficient filter adaptation has occurred for a filter estimate to have
  // formed. The required amount of filter adaptation at call startup is more
  // restrictive than after an in-call reset.
