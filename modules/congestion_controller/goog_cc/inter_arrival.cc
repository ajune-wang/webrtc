...
#include "modules/congestion_controller/goog_cc/inter_arrival.h"
...
InterArrival::InterArrival(TimeDelta send_time_group_length)
    : send_time_group_length_(send_time_group_length),
      current_timestamp_group_(),
      prev_timestamp_group_(),
      num_consecutive_reordered_packets_(0) {}

bool InterArrival::ComputeDeltas(Timestamp send_time,
  ...
  if (current_timestamp_group_.IsFirstPacket()) {
      ...
      if (*arrival_time_delta < TimeDelta::Zero()) {
        ...
        if (num_consecutive_reordered_packets_ >= kReorderedResetThreshold) {
          RTC_LOG(LS_WARNING)
              ...
              << " arrival_time_delta" << arrival_time_delta->ms() << " send time delta " << send_time_delta->ms();
          Reset();
        }
        return false;
      } else {
    ...
    current_timestamp_group_.first_send_time = send_time;
    current_timestamp_group_.send_time = send_time;
  //  RTC_LOG(INFO) << "changing current_timestamp_group_.timestamp " << current_timestamp_group_.send_time.ms();
    current_timestamp_group_.first_arrival = arrival_time;
    current_timestamp_group_.size = 0;
  } else {
    current_timestamp_group_.send_time = std::max(current_timestamp_group_.send_time, send_time);
  }
...
// |current_timestamp_group_|.
bool InterArrival::NewTimestampGroup(Timestamp arrival_time,
  ...
  if (current_timestamp_group_.IsFirstPacket()) {
    ...
    return
        send_time - current_timestamp_group_.first_send_time > send_time_group_length_;
  }
}

bool InterArrival::BelongsToBurst(Timestamp arrival_time,
                                  Timestamp send_time) const {
