...
struct RTC_EXPORT RtpEncodingParameters {
  ...
  // Forwards reads and writes to `scale_resolution_down_to`.
  ...
  class RTC_EXPORT ScaleToAccessor {
   public:
    explicit ScaleToAccessor(RtpEncodingParameters& parent)
        : scale_resolution_down_to_(parent.scale_resolution_down_to) {}

    // Has value/value getter.
    operator bool() const { return scale_resolution_down_to_.has_value(); }
    bool has_value() const { return scale_resolution_down_to_.has_value(); }
    const Resolution& value() const {
      return scale_resolution_down_to_.value();
    }
    Resolution value() { return scale_resolution_down_to_.value(); }
    // Setters.
    void operator=(std::optional<Resolution> value) {
      scale_resolution_down_to_ = value;
    }
    void operator=(Resolution value) { scale_resolution_down_to_ = value; }
    void operator=(const ScaleToAccessor& other) {
      scale_resolution_down_to_ = static_cast<std::optional<Resolution>>(other);
    }
    // Getter.
    operator std::optional<Resolution>() const {
      return scale_resolution_down_to_;
    }
    // Comparison.
    bool operator==(const ScaleToAccessor& other) const {
      return static_cast<std::optional<Resolution>>(*this) ==
             static_cast<std::optional<Resolution>>(other);
    }
    bool operator!=(const ScaleToAccessor& other) const {
      return !(*this == other);
    }
    bool operator==(const std::optional<Resolution>& other) const {
      return static_cast<std::optional<Resolution>>(*this) == other;
    }
    bool operator!=(const std::optional<Resolution>& other) const {
      return !(*this == other);
    }
    // Accessors.
    const Resolution* operator->() const {
      return &scale_resolution_down_to_.value();
    }
    Resolution* operator->() { return &scale_resolution_down_to_.value(); }

   private:
    std::optional<Resolution>& scale_resolution_down_to_;
  };
  ScaleToAccessor requested_resolution;
  ...
  // when ScaleToAccessor has been deleted, it's the only reason why this
  // operator is not auto generated by the compiler.
