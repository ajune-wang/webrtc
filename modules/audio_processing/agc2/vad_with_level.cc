...
VadWithLevel::LevelAndProbability VadWithLevel::AnalyzeFrame(
  ...
  SetSampleRate(frame.samples_per_channel() * 100);
  ...
  bool is_silence = features_extractor_.CheckSilenceComputeFeatures(
...
void VadWithLevel::SetSampleRate(int sample_rate_hz) {
  // Assume always mono (for testing only!).
  resampler_.InitializeIfNeeded(sample_rate_hz, rnn_vad::kSampleRate24kHz, 1);
}
