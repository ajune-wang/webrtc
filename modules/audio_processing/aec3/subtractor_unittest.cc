...
// Verifies that the subtractor is able to handle different filter lengths for
// the shadow and main filters.
TEST(Subtractor, DifferentFilterLengths) {
  std::vector<int> blocks_with_echo_path_changes;
  float echo_to_nearend_power =
      RunSubtractorTest(400, 64, 20, 10, false, blocks_with_echo_path_changes);
