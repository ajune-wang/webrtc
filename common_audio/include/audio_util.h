...
inline float DbfsToFloat16(float v) {
  static const float kMaximumAbsFloatS16 = -limits_int16::min();
...
inline float FloatS16ToDbfs(float v) {
  RTC_DCHECK_GE(v, 0);
  static const float kMaximumAbsFloatS16 = -limits_int16::min();
  static const float kMinDbfs = -20.0 * std::log10(kMaximumAbsFloatS16);
  if (v <= 1.0) {
    return kMinDbfs;
  }
  return 20.0 * std::log10(v / kMaximumAbsFloatS16);
}
