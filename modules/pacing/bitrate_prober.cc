...
BitrateProberConfig::BitrateProberConfig(
    ...
    : min_probe_packets_sent_("min_probe_packets_sent", 5),
      min_probe_delta_("min_probe_delta", TimeDelta::ms(1)),
      min_probe_duration_("min_probe_duration", TimeDelta::ms(15)),
      max_probe_delay_("max_probe_delay", TimeDelta::ms(3)) {
  ParseFieldTrial({&min_probe_packets_sent_, &min_probe_delta_,
                   &min_probe_duration_, &max_probe_delay_},
                  key_value_config->Lookup(kConfigurableProbingFieldTrialName));
}

BitrateProber::~BitrateProber() {
  RTC_HISTOGRAM_COUNTS_1000("WebRTC.BWE.TotalProbeClustersRequested",
                            total_probe_count_);
  RTC_HISTOGRAM_COUNTS_1000("WebRTC.BWE.TotalFailedProbeClusters",
...
void BitrateProber::CreateProbeCluster(int bitrate_bps,
  ...
  cluster.pace_info.probe_cluster_min_probes = config_.min_probe_packets_sent_;
  cluster.pace_info.probe_cluster_min_bytes =
      static_cast<int32_t>(static_cast<int64_t>(bitrate_bps) *
                           config_.min_probe_duration_->ms() / 8000);
...
int BitrateProber::TimeUntilNextProbe(int64_t now_ms) {
  ...
  if (next_probe_time_ms_ >= 0) {
    ...
    if (time_until_probe_ms < -config_.max_probe_delay_->ms()) {
...
size_t BitrateProber::RecommendedMinProbeSize() const {
  ...
  return clusters_.front().pace_info.send_bitrate_bps * 2 *
         config_.min_probe_delta_->ms() / (8 * 1000);
}
...
void BitrateProber::ProbeSent(int64_t now_ms, size_t bytes) {
  ...
  if (!clusters_.empty()) {
    ...
    if (cluster->sent_bytes >= cluster->pace_info.probe_cluster_min_bytes &&
      ...
      RTC_HISTOGRAM_COUNTS_100000("WebRTC.BWE.ProbeClusterSizeInBytes",
                                  cluster->sent_bytes);
      RTC_HISTOGRAM_COUNTS_100("WebRTC.BWE.ProbesPerCluster",
                               cluster->sent_probes);
      RTC_HISTOGRAM_COUNTS_10000("WebRTC.BWE.TimePerProbeCluster",
