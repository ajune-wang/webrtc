...
void AudioEncoderOpusImpl::OnReceivedRtt(int rtt_ms) {
  RTC_LOG(LS_INFO) << "AudioEncoderOpusImpl::OnReceivedRtt, rtt=" << rtt_ms;
  if (!audio_network_adaptor_)
    return;
  RTC_LOG(LS_INFO) << "AudioEncoderOpusImpl::OnReceivedRtt, setting rtt";
...
void AudioEncoderOpusImpl::SetTargetBitrate(int bits_per_second) {
  ...
  RTC_LOG(LS_INFO) << "Set target bitrate, bits_ps=" << bits_per_second;
