...
bool LibaomAv1Encoder::SetSvcParams(
  ...
  if (svc_config.num_spatial_layers > 4) {
    RTC_LOG(LS_WARNING) << "Av1 doesn't support more than 4 spatial layers. "
                        << svc_config.num_spatial_layers << " configured.";
    return false;
  }
  if (svc_config.num_temporal_layers > 8) {
    RTC_LOG(LS_WARNING) << "Av1 doesn't support more than 8 temporal layers. "
