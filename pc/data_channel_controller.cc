...
void DataChannelController::AddSctpDataStream(int sid) {
  if (data_channel_transport()) {
    network_thread()->BlockingCall(RTC_FROM_HERE, [this, sid] {
      if (data_channel_transport()) {
...
void DataChannelController::OnDataReceived(
  ...
  signaling_thread()->PostTask(
      RTC_FROM_HERE, [self = weak_factory_.GetWeakPtr(), params, buffer] {
        if (self) {
...
void DataChannelController::OnChannelClosing(int channel_id) {
  RTC_DCHECK_RUN_ON(network_thread());
  signaling_thread()->PostTask(
      RTC_FROM_HERE, [self = weak_factory_.GetWeakPtr(), channel_id] {
        if (self) {
          RTC_DCHECK_RUN_ON(self->signaling_thread());
...
void DataChannelController::OnTransportClosed(RTCError error) {
  RTC_DCHECK_RUN_ON(network_thread());
  signaling_thread()->PostTask(RTC_FROM_HERE,
                               [self = weak_factory_.GetWeakPtr(), error] {
                                 if (self) {
                                   RTC_DCHECK_RUN_ON(self->signaling_thread());
...
bool DataChannelController::DataChannelSendData(
  ...
  RTCError error = network_thread()->BlockingCall(
