...
Packet::Packet(const RtpPacketReceived& rtp_packet,
...
std::vector<std::unique_ptr<Packet>> PacketBuffer::FindFrames(
    uint16_t seq_num) {
  ...
  for (size_t i = 0; i < buffer_.size(); ++i) {
    ...
    if (buffer_[index]->is_last_packet_in_frame()) {
      ...
      bool is_h26x_descriptor =
          (buffer_[start_index]->codec() == kVideoCodecH264 ||
           buffer_[start_index]->codec() == kVideoCodecH265) &&
          !is_generic;

      if (!is_h26x_descriptor) {
        ...
        CheckFrames(seq_num, start, index, sps_pps_idr_is_h264_keyframe_,
                    buffer_, found_frames, missing_packets_, received_padding_);
      }
    }
    ++seq_num;
  }
