...
bool OpenSSLStreamAdapter::SetPeerCertificateDigest(
  ...
  if (!OpenSSLDigest::GetDigestSize(digest_alg, &expected_len)) {
    RTC_DLOG(LS_WARNING) << "Unknown digest algorithm: " << digest_alg;
    if (error) {
...
bool OpenSSLStreamAdapter::SetDtlsSrtpCryptoSuites(
  ...
  for (const int cipher : ciphers) {
    ...
    if (!found) {
      RTC_DLOG(LS_ERROR) << "Could not find cipher: " << cipher;
      return false;
    }
  }
...
void OpenSSLStreamAdapter::Error(const char* context,
                                 ...
                                 uint8_t alert,
                                 bool signal) {
  RTC_DLOG(LS_WARNING) << "OpenSSLStreamAdapter::Error(" << context << ", "
...
// SSL_send_fatal_alert is only available in BoringSSL.
    ...
    if (alert) {
      ...
      if (ret < 0) {
        RTC_DLOG(LS_WARNING) << "SSL_send_fatal_alert failed, error = "
...
bool OpenSSLStreamAdapter::VerifyPeerCertificate() {
  if (!HasPeerCertificateDigest() || !peer_cert_chain_ ||
    ...
    RTC_DLOG(LS_WARNING) << "Missing digest or peer certificate.";
    return false;
  }
  ...
  if (!OpenSSLCertificate::ComputeDigest(
    ...
    RTC_DLOG(LS_WARNING) << "Failed to compute peer cert digest.";
    return false;
  }
