...
struct RandomWalkConfig {
  int random_seed = 1;
  DataRate peak_rate = DataRate::KilobitsPerSecond(100);
  DataSize min_packet_size = DataSize::Bytes(200);
  TimeDelta min_packet_interval = TimeDelta::Milliseconds(1);
  TimeDelta update_interval = TimeDelta::Milliseconds(200);
  double variance = 0.6;
  double bias = -0.1;
};
...
struct PulsedPeaksConfig {
  ...
  DataSize min_packet_size = DataSize::Bytes(200);
  TimeDelta min_packet_interval = TimeDelta::Milliseconds(1);
  TimeDelta send_duration = TimeDelta::Milliseconds(100);
  TimeDelta hold_duration = TimeDelta::Milliseconds(2000);
};
...
struct FakeTcpConfig {
  ...
  TimeDelta process_interval = TimeDelta::Milliseconds(200);
