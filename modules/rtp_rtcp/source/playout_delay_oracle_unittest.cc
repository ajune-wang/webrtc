...
TEST(PlayoutDelayOracleTest, DisabledByDefault) {
  ...
  EXPECT_FALSE(playout_delay_oracle.send_playout_delay());
  EXPECT_EQ(-1, playout_delay_oracle.playout_delay().min_ms);
  EXPECT_EQ(-1, playout_delay_oracle.playout_delay().max_ms);
}
...
TEST(PlayoutDelayOracleTest, SendPlayoutDelayUntilSeqNumberExceeds) {
  ...
  playout_delay_oracle.UpdateRequest(playout_delay, kSequenceNumber);
  EXPECT_TRUE(playout_delay_oracle.send_playout_delay());
  EXPECT_EQ(kMinPlayoutDelay, playout_delay_oracle.playout_delay().min_ms);
  EXPECT_EQ(kMaxPlayoutDelay, playout_delay_oracle.playout_delay().max_ms);
