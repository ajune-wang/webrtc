...
TEST(LoopbackMediaTransport, AudioWithNoSinkSilentlyIgnored) {
  ...
  transport_pair.GetTransports().first->SendAudioFrame(1, CreateAudioFrame(0));
  transport_pair.GetTransports().second->SendAudioFrame(2, CreateAudioFrame(0));
}
...
TEST(LoopbackMediaTransport, AudioDeliveredToSink) {
  ...
  EXPECT_CALL(sink,
              OnData(1, testing::Property(
                            ...
                            testing::Eq(10))));
  transport_pair.GetTransports().second->SetReceiveAudioSink(&sink);
  transport_pair.GetTransports().first->SendAudioFrame(1, CreateAudioFrame(10));

  transport_pair.GetTransports().second->SetReceiveAudioSink(nullptr);
}
