...
int VP9EncoderImpl::InitEncode(const VideoCodec* inst,
  ...
  encoded_image_.set_buffer(new uint8_t[frame_size], frame_size);
...
int VP9EncoderImpl::GetEncodedLayerFrame(const vpx_codec_cx_pkt* pkt) {
  ...
  if (pkt->data.frame.sz > encoded_image_.alloc()) {
    delete[] encoded_image_._buffer;
  ...
  RTC_DCHECK_LE(encoded_image_._length, encoded_image_.alloc());
