...
int SimulcastEncoderAdapter::InitEncode(
  ...
  int num_active_streams = number_of_streams;
  ...
  for (int i = 0; i < number_of_streams; ++i) {
    ...
    const bool send_stream =
        doing_simulcast ? start_bitrate_kbps > 0 : num_active_streams > 0;
    if (!doing_simulcast) {
      stream_codec = codec_;
