...
struct StreamStats {
 public:
  ...
  // a sum of time for which video was frozen over 200ms (freeze - no new frames
  // from decoder)
  uint64_t freeze_200_ms = 0;
  // a sum of time for which video was frozen over 1s (freeze - no new frames
  // from decoder)
...
class DefaultVideoQualityAnalyzer : public VideoQualityAnalyzerInterface {
 public:
  ...
  // TODO(titovartem) maybe have some analyzer internal metrics to verify
  // analyzer behavior
  SamplesStatsCounter GetComparisonsQueueSize() const;
 ...
 private:
  ...
  void ReportResult(const std::string& name,
  ...
  SamplesStatsCounter comparisons_queue_on_add_size_
      RTC_GUARDED_BY(comparison_lock_);
