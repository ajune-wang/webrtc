...
TEST(LibaomAv1Encoder, EncodeKeyframe) {
  ...
  enc->Encode(raw_frame, {}, {Fb().Key().Rate(kCbr).Res(640, 360).Build()},
              res.CallBack());
...
TEST(LibaomAv1Encoder, ResolutionSwitching) {
  ...
  enc->Encode(in0, {}, {Fb().Rate(kCbr).Res(320, 180).Key().Build()},
              res.CallBack());

  rtc::scoped_refptr<I420Buffer> in1 = frame_reader->PullFrame();
  enc->Encode(in1, {}, {Fb().Rate(kCbr).Res(640, 360).Ref({0}).Build()},
              res.CallBack());

  rtc::scoped_refptr<I420Buffer> in2 = frame_reader->PullFrame();
  enc->Encode(in2, {}, {Fb().Rate(kCbr).Res(160, 90).Ref({0}).Build()},
              res.CallBack());
...
TEST(LibaomAv1Encoder, InputResolutionSwitching) {
  ...
  rtc::scoped_refptr<I420Buffer> in0 = frame_reader->PullFrame();
  enc->Encode(in0, {}, {Fb().Rate(kCbr).Res(160, 90).Key().Build()},
              res.CallBack());
  ...
  rtc::scoped_refptr<I420Buffer> in1 = frame_reader->PullFrame(
      ...
      /*resolution=*/{320, 180},
      /*framerate_scale=*/{1, 1});
  enc->Encode(in1, {}, {Fb().Rate(kCbr).Res(160, 90).Ref({0}).Build()},
              res.CallBack());
  ...
  Av1Decoder dec("input_res_switch");
  VideoFrame f0 = dec.Decode(*res.FrameAt(0));
...
TEST(LibaomAv1Encoder, TempoSpatial) {
  ...
  enc->Encode(frame_reader->PullFrame(), {},
              {Fb().Rate(k10Fps).Res(160, 90).S(0).Key().Build(),
...
TEST(LibaomAv1Encoder, L3T1) {
  ...
  enc->Encode(frame_reader->PullFrame(), {.effort_level = 2},
              {Fb().Rate(kCbr).Res(160, 90).S(0).Key().Build(),
               Fb().Rate(kCbr).Res(320, 180).S(1).Ref({0}).Upd({1}).Build(),
               Fb().Rate(kCbr).Res(640, 360).S(2).Ref({1}).Upd({2}).Build()},
              res.CallBack());
...
TEST(LibaomAv1Encoder, HigherEffortLevelYieldsHigherQualityFrames) {
  ...
  for (int i = effort_range.first; i <= effort_range.second; ++i) {
    auto enc = LibaomAv1EncoderFactory().CreateEncoder(kCbrEncoderSettings, {});
    EncodeResults res;
    enc->Encode(frame_in, {.effort_level = i},
                {Fb().Rate(kCbr).Res(640, 360).Key().Build()}, res.CallBack());
...
TEST(LibaomAv1Encoder, KeyframeAndStartrameAreApproximatelyEqual) {
  ...
  for (int sid = 0; sid < max_spatial_layers; ++sid) {
    ...
    enc_key->Encode(frame_in, {},
                    {Fb().Rate(kRate).Res(640, 360).S(sid).Key().Build()},
                    res_key.CallBack());
    enc_start->Encode(
        frame_in, {},
...
// TODO: Wait for https://aomedia-review.googlesource.com/c/aom/+/184724
